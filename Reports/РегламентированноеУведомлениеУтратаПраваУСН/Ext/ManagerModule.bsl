...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр.ПутьФормы = "Отчет.РегламентированноеУведомлениеУтратаПраваУСН.Форма.Форма2014_1";
...
Процедура Проверить_Форма2014_1(Данные, УникальныйИдентификатор)
	...
	Если Не ЗначениеЗаполнено(Титульный.ДАТА_УТРАТЫ_ПРАВА) Тогда
		РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
			"Не указана дата перехода на иной режим налогообложения", УникальныйИдентификатор);
		Ошибок = Ошибок + 1;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Титульный.ПОДПУНКТ1) 
		Или ЗначениеЗаполнено(Титульный.ПУНКТ1)
		Или ЗначениеЗаполнено(Титульный.СТАТЬЯ1)
		Или ЗначениеЗаполнено(Титульный.ГОД1)
		Или ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД1) Тогда
		
		Если Не ЗначениеЗаполнено(Титульный.ПОДПУНКТ1) 
			И Титульный.ПУНКТ1 <> "4" Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указаны соответствующие подпункты пункта 3 статьи 346.12, пункта 3 статьи 346.14 Налогового кодекса Российской Федерации", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Титульный.ПУНКТ1) Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указаны пункт 3 или пункт 4 статьи 346.12, пункта 3 статьи 346.14 Налогового кодекса Российской Федерации", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Титульный.СТАТЬЯ1) Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указана статья 346.12 или статья 346.14 Налогового кодекса Российской Федерации", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД1) Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указан отчетный период", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Титульный.ГОД1) Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указан отчетный год", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Титульный.ГОД2) Или ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД2) Тогда 
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Если заполнены данные по ст. 346.12 или 346.14, то не следует заполнять данные по ст.346.13", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД2)
		...
		Если Не ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД2) Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указан отчетный период, за который был превышен доход", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Титульный.ГОД2) Тогда
			РегламентированнаяОтчетность.СообщитьПользователюОбОшибкеВУведомлении(
				"Не указан отчетный год, за который был превышен доход", УникальныйИдентификатор);
			Ошибок = Ошибок + 1;
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2014_1(Данные, УникальныйИдентификатор)
	...
	Если ЗначениеЗаполнено(Титульный.ПОДПУНКТ1) 
		Или ЗначениеЗаполнено(Титульный.ПУНКТ1)
		Или ЗначениеЗаполнено(Титульный.СТАТЬЯ1)
		Или ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД1)
		Или ЗначениеЗаполнено(Титульный.ГОД1) Тогда
		...
		Если ЗначениеЗаполнено(Титульный.ГОД2) Или ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД2) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Если заполнены данные по ст. 346.12 или 346.14, то не следует заполнять данные по ст.346.13", "Титульный", "ГОД2"));
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД2)
		Или ЗначениеЗаполнено(Титульный.ГОД2) Тогда
		Если Не ЗначениеЗаполнено(Титульный.НАЛОГОВЫЙ_ПЕРИОД2) Тогда
