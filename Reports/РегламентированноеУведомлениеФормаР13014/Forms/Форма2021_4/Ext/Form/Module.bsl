	...
	Если ТипЗнч(МногострочныеЧасти) = Тип("СписокЗначений")
	   И МногострочныеЧасти.Количество() > 0 Тогда
		Для Каждого МнгСтр Из МногострочныеЧасти Цикл
			...
			ИсхСтр = ОбщегоНазначения.СкопироватьРекурсивно(ЭтотОбъект["ДанныеДопСтрокСтраницы"][ПолноеИмяМСЧ][0].Значение);
...
Процедура ДобавитьСтрокуНаСервере(ИмяОбласти)
	...
	НоваяСтрока = ОбщегоНазначения.СкопироватьРекурсивно(ЭтотОбъект["ДанныеДопСтрокСтраницы"][ПолноеИмяМСЧ][0].Значение);
