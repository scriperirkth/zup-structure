...
Процедура ВывестиТабличнуюЧастьТ3(ДокументРезультат, Данные, СписокНадбавок, Макет, ПредставлениеТарифовИНадбавок)
	...
	Для Каждого СтрокаДанных Из Данные Цикл
		...
		Для Каждого ДанныеНадбавки Из СписокНадбавок Цикл
			...
			СтрокиДанныхНадбавки = СтрокаДанных.Строки.НайтиСтроки(Новый Структура("Начисление", ДанныеНадбавки.Значение));
			Если ЗначениеЗаполнено(СтрокиДанныхНадбавки) Тогда
				
				Для Каждого СтрокаДанныхНадбавки Из СтрокиДанныхНадбавки Цикл
					РазмерНадбавки = ПредставлениеРазмераНадбавки(
						СтрокаДанныхНадбавки, ПредставлениеТарифовИНадбавок, ИспользоватьВилкуСтавокВШтатномРасписании);
					Если Не ПустаяСтрока(РазмерНадбавки) Тогда
						СтрокаНадбавки.Параметры.Заполнить(СтрокаДанныхНадбавки);
						СтрокаНадбавки.Параметры.РазмерНадбавки = РазмерНадбавки;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
