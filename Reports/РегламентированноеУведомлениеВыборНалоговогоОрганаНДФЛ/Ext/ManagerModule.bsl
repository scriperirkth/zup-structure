...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2019_1(Объект, Данные, УникальныйИдентификатор)
	...
	Если Не ЗначениеЗаполнено(Титульная.КППОП)
		Или Не РегламентированнаяОтчетностьВызовСервера.КППСоответствуетТребованиямФНС(Титульная.КППОП) Тогда  
	...
	Для Каждого Стр Из Данные.ДанныеДопСтрокБД.МнгСтр Цикл 
		...
		ИначеЕсли Не РегламентированнаяОтчетностьВызовСервера.КППСоответствуетТребованиямФНС(Стр.КПП_ОП)
