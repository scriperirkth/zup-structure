...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ОбработкаПерсональныхДанных;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001.ИмяМакета = "Форма_ОбработкаПерсональныхДанных_Титульная";
	...
	СтрРег.ИндексКартинки = 1;
	СтрРег.ИмяМакета = "Форма_ОбработкаПерсональныхДанных_Цель";
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Ложь;
	СтрРег.УИД = Новый УникальныйИдентификатор;
	СтрРег.ИДНаименования = "Цель";
КонецПроцедуры
...
Процедура ЗаполнитьТитульный()
	...
	Титульная.Вставить("ДАТА_ПОДПИСИ", Объект.ДатаПодписи);
	Титульная.Вставить("ДатаПодписи", Объект.ДатаПодписи);
	
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Объект.Организация) Тогда 
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Объект.Организация, ДП, "ИННЮЛ,НаимЮЛПол,НаимЮЛСокр,АдрПочт,ОГРН,ДолжнРук,ФамилияРук,ИмяРук,ОтчествоРук");
		Титульная.Вставить("ФИО_Должность", СведенияОбОрганизации.ФамилияРук + " " 
			+ СведенияОбОрганизации.ИмяРук + " " + СведенияОбОрганизации.ОтчествоРук + ", " + СведенияОбОрганизации.ДолжнРук);
		Титульная.Вставить("Данные1", СведенияОбОрганизации.НаимЮЛПол + " / "
			+ СведенияОбОрганизации.НаимЮЛСокр + " ИНН:" + СведенияОбОрганизации.ИННЮЛ + " ОГРН:" + СведенияОбОрганизации.ОГРН);
		Титульная.Вставить("Данные2", СведенияОбОрганизации.АдрПочт);
	Иначе 
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Объект.Организация, ДП, "ИННФЛ,ФИО,АдрПрописки,ОГРН");
		Титульная.Вставить("ФИО_Должность", СведенияОбОрганизации.ФИО + ", индивидуальный предприниматель");
		Титульная.Вставить("Данные1", СведенияОбОрганизации.ФИО + " ИНН:" + СведенияОбОрганизации.ИННФЛ + " ОГРН:" + СведенияОбОрганизации.ОГРН);
		Титульная.Вставить("Данные2", СведенияОбОрганизации.АдрПрописки);
	КонецЕсли;
КонецПроцедуры
