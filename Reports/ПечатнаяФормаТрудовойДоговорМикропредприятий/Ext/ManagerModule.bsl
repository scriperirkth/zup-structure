...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода, СписокСотрудников = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(МассивОбъектов) И ТипЗнч(МассивОбъектов[0]) = Тип("СправочникСсылка.Сотрудники") Тогда
		...
		ЭДОБЗК.ПечатьСтороннейПечатнойФормы(Отчеты.ПечатнаяФормаТрудовойДоговорМикропредприятий,
	...
	Для Каждого ДанныеСсылок Из РезультатКомпоновки.ДанныеОтчета.Строки Цикл
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеСсылок.Строки Цикл
			...
			Если ПараметрыПечати <> Неопределено Тогда
				ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, РезультатКомпоновки.ИдентификаторыМакета,
...
Процедура ТрудовойДоговорВывестиРаздел(ИмяОбласти, ДокументРезультат, Макет, ДанныеПараметров)
	...
	Для каждого ВложеннаяОбласть Из ОбластьРаздела.Области Цикл
		...
		Если ВложеннаяОбласть.Имя = "Подписи_05_СогласияНаОбработкуПерсональныхДанных"
			И (Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПараметров.ДанныеСсылок, "РаботаТрудовойДоговорДатаОформления")
				Или Не ЗначениеЗаполнено(ДанныеПараметров.ДанныеСсылок.РаботаТрудовойДоговорДатаОформления)
				Или ДанныеПараметров.ДанныеСсылок.РаботаТрудовойДоговорДатаОформления >= '20250901') Тогда
