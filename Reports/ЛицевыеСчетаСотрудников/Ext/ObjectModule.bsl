...
Функция ФактическиеОтпуска(КлючВарианта, ПараметрыОтчетов)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ОтпускаСотрудников.Дата, РеестрОтпусков.ИсходныйДокументДата) КАК ДатаПриказа,
		|	ВЫБОР
		|		КОГДА ОтпускаСотрудников.Номер ЕСТЬ NULL
		|			ТОГДА РеестрОтпусков.ИсходныйДокументНомер
		|		КОГДА ОтпускаСотрудников.НомерПервичногоДокумента = """"
		|			ТОГДА ОтпускаСотрудников.Номер
		|		ИНАЧЕ ОтпускаСотрудников.НомерПервичногоДокумента
