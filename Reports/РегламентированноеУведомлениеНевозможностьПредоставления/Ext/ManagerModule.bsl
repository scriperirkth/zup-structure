...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр.ОписаниеФормы = "Письмо ФНС России от 08.11.2024 № ШЮ-4-17/12784@ КНД 1150111 в формате 5.01";
	Стр.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ПродлениеСрокаПредставленияДокументов;
	Стр.ДатаНачала = Дата("20250430");
	Стр.ДатаКонца = Дата("20500101");
...
Функция ИдентификаторФайлаЭлектронногоПредставления_2026(СведенияОтправки)
	Префикс = "NO_ZVPSODOKNO";
	Возврат Документы.УведомлениеОСпецрежимахНалогообложения.ИдентификаторФайлаЭлектронногоПредставления(Префикс, СведенияОтправки);
КонецФункции
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2026_1(Данные, УникальныйИдентификатор)
	...
	Если Не СтрПодобнаПоРегулярномуВыражению(Реквизиты._КодНО, "[0-9]{4}", Истина) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			"Не указан / неправильно указан код налогового органа", "", "РегистрацияВИФНС"));
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Реквизиты._КолРабДн) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			"Не заполнено количество дней продления срока", "ОсновныеСведения", "_КолРабДн"));
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Реквизиты._НомУведом) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			"Не указан номер уведомления", "ОсновныеСведения", "_НомУведом"));
	...
	Если Не ЗначениеЗаполнено(Реквизиты._ОбосПрод.Количество()) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
			"Не заполнен список причин", "ОбоснованиеПричин", "_ОбосПрод"));
	КонецЕсли;
...
Функция ОсновныеСведенияЭлектронногоПредставления_Форма2026_1(Объект, УникальныйИдентификатор)
	...
	ОсновныеСведения.Вставить("_ДатаУведомленияДата", Формат(СтруктураРеквизитов._ДатаУведомления, "ДФ=dd.MM.yyyy; ДП=01.01.0001"));
	ОсновныеСведения.Вставить("_ДатаУведомленияВремя", Формат(СтруктураРеквизитов._ДатаУведомления, "ДФ=hh:mm:ss; ДП=00:00:00"));
	ОсновныеСведения.Вставить("_КолРабДн", СтруктураРеквизитов._КолРабДн);
	
	ИдентификаторФайла = ИдентификаторФайлаЭлектронногоПредставления_2026(ОсновныеСведения);
	ОсновныеСведения.Вставить("ИдУведом", СтрЗаменить(Сред(ИдентификаторФайла, СтрНайти(ИдентификаторФайла, "_", НаправлениеПоиска.СКонца) + 1), "-", ""));
	ОсновныеСведения.Вставить("ИдФайл", ИдентификаторФайла);
...
Функция ЗаполнитьДанными_Форма2026_1(Объект, СтруктураВыгрузки, СписокФайлов, ИменаФайлов, ОсновныеСведения)
	ПрефиксТаблицы = УведомлениеОСпецрежимахНалогообложенияПовтИсп.ПрефиксТаблицы();
	ПрефиксРеквизита = УведомлениеОСпецрежимахНалогообложенияПовтИсп.ПрефиксРеквизита();
	ОТД = Новый ОписаниеТипов("Дата");
	Данные = Объект.ДанныеУведомления.Получить();
	...
	Узел_Документ = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(СтруктураВыгрузки, "Документ");
	РегламентированнаяОтчетность.ВывестиПоказательВXML(
		Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_Документ, "КолПрилЗаявл"), КолФайл);
		
	Узел_СвПродДокИн = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_Документ, "СвПродДокИн");
	Прототип_ОбосПрод = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_СвПродДокИн, "ОбосПрод");
	Для Каждого Стр0 Из Данные.СтруктураРеквизитов._ОбосПрод Цикл
		Узел_ОбосПрод = Документы.УведомлениеОСпецрежимахНалогообложения.НовыйУзелИзПрототипа(Прототип_ОбосПрод);
		Документы.УведомлениеОСпецрежимахНалогообложения.ВывестиПоказательВXML(Узел_ОбосПрод, Стр0.ОбосПрод);
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(Прототип_ОбосПрод);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		Прототип_ИмяФайл = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПодчиненныйЭлемент(Узел_СвПродДокИн, "ИмяФайлСканДок");
		Для Каждого Стр0 Из Данные.СтруктураРеквизитов._СвНевозмПред Цикл
			Для Каждого ИФ Из ФайлыПоСтраницам[Данные.СтруктураРеквизитов._СвНевозмПред.Индекс(Стр0)] Цикл
				Узел_ИмяФайлТек = Документы.УведомлениеОСпецрежимахНалогообложения.НовыйУзелИзПрототипа(Прототип_ИмяФайл);
				Документы.УведомлениеОСпецрежимахНалогообложения.ВывестиПоказательВXML(Узел_ИмяФайлТек, ИФ);
			КонецЦикла;
		КонецЦикла;
		РегламентированнаяОтчетность.УдалитьУзел(Прототип_ИмяФайл);
	КонецЕсли;
КонецФункции
...
Функция ЭлектронноеПредставление_Форма2026_1(Объект, УникальныйИдентификатор)
	...
	Документы.УведомлениеОСпецрежимахНалогообложения.ОбработатьУсловныеЭлементы(ОсновныеСведения, СтруктураВыгрузки);
	Документы.УведомлениеОСпецрежимахНалогообложения.ЗаполнитьПараметры(ОсновныеСведения, СтруктураВыгрузки);
	ЗаполнитьДанными_Форма2026_1(Объект, СтруктураВыгрузки, СписокФайлов, ИменаФайлов, ОсновныеСведения);
	Документы.УведомлениеОСпецрежимахНалогообложения.ОтсечьНезаполненныеНеобязательныеУзлы(СтруктураВыгрузки);
	УведомлениеОСпецрежимахНалогообложения.ВыгрузитьДеревоВТаблицу(СтруктураВыгрузки, ОсновныеСведения, СведенияЭлектронногоПредставления);
