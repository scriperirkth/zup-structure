...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ПродлениеСрокаПредставленияДокументов;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормы_ОбосПрод

&НаКлиенте
Процедура _ОбосПродПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
	ОбосПродПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр);
КонецПроцедуры

&НаКлиенте
Процедура _ОбосПродВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ОбосПродОбосПрод" Тогда 
		СтандартнаяОбработка = Ложь;
		АсинхОбосПродВыбор(Элемент, ВыбраннаяСтрока, Поле);
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьФайлы(Команда)
	Элементы._СвНевозмПред.ДобавитьСтроку();
	ДокументыСканированныеДокументыОписаниеНачалоВыбора(
		Элементы._СвНевозмПред, Элементы._СвНевозмПред.ТекущиеДанные, Ложь);
КонецПроцедуры
...
Процедура НачальнаяИнициализация()
	...
	Элементы._КолРабДн.УстановитьДействие("ПриИзменении", "ПриИзмененииРеквизитов");
КонецПроцедуры
...
&НаКлиенте
Асинх Процедура ОбосПродПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	ТекстРедактирования = ?(Копирование И Элемент.ТекущиеДанные <> Неопределено, Элемент.ТекущиеДанные.ОбосПрод, "");
	
	ТекстРедактирования = Ждать ВвестиСтрокуАсинх(ТекстРедактирования, "Причина продления срока", 1500, Истина);
	Если ЗначениеЗаполнено(ТекстРедактирования) Тогда 
		НовСтр = _ОбосПрод.Добавить();
		НовСтр.УИД = Новый УникальныйИдентификатор;
		НовСтр.ОбосПрод = ТекстРедактирования;
		Модифицированность = Истина;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Асинх Процедура АсинхОбосПродВыбор(Элемент, ВыбраннаяСтрока, Поле)
	ТекстРедактирования = Ждать ВвестиСтрокуАсинх(Элемент.ТекущиеДанные.ОбосПрод, "Причина продления срока", 1500, Истина);
	Если ЗначениеЗаполнено(ТекстРедактирования) Тогда 
		Элемент.ТекущиеДанные.ОбосПрод = ТекстРедактирования;
		Модифицированность = Истина;
	КонецЕсли;
КонецПроцедуры
