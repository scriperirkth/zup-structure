...
Процедура ВывестиМакетКарточкаУчета2015(ДокументРезультат, ДанныеОтчета, ДатаОтчета, НастройкиОтчета)
	...
	СведенияОбОрганизациях = СведенияОбОрганизациях(Организации, ДатаОтчета);
	...
	Для Каждого ДанныеОрганизации Из ДанныеОтчета.Строки Цикл
		...
		ТолькоВоинскийУчет = Ложь;
		ВходитВОрганГосударственнойВласти = Ложь;
		
		СведенияОбОрганизации = СведенияОбОрганизациях.Найти(ДанныеОрганизации.Организация, "Организация");
		Если СведенияОбОрганизации <> Неопределено Тогда
			ТолькоВоинскийУчет = СведенияОбОрганизации.ВедетсяТолькоВоинскийУчет;
			ВходитВОрганГосударственнойВласти = СведенияОбОрганизации.ВходитВОрганГосударственнойВласти;
			Если ДатаОтчета >= Дата(2017, 1, 1) Тогда 
				СведенияОбОрганизации.КодОКВЭД = СведенияОбОрганизации.КодОКВЭД2;
				СведенияОбОрганизации.НаименованиеОКВЭД = СведенияОбОрганизации.НаименованиеОКВЭД2;
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ОбластьКарточка.Параметры, СведенияОбОрганизации);
		КонецЕсли;
		...
		ЗаполнитьЧисленностьРаботающихГраждан(ОбластьКарточка, ДатаОтчета, ДанныеОрганизации.Организация, НастройкиОтчета, ТолькоВоинскийУчет);
...
Процедура ЗаполнитьЧисленностьРаботающихГраждан(ОбластьКарточка, ДатаОтчета, Организация, НастройкиФормы, ТолькоВоинскийУчет)
	
	Если ДатаОтчета < '20170101' Тогда 
		Возврат;
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБронированиеГраждан") И Не ТолькоВоинскийУчет Тогда 
		Возврат;
	КонецЕсли;
	...
	ВидЗанятостиВидСравнения = Неопределено;
	...
	Для Каждого ЭлементОтбора Из НастройкиФормы.Отбор.Элементы Цикл
		...
		КонецЕсли;
		Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидЗанятости") И ЭлементОтбора.Использование Тогда
			ВидЗанятости = ЭлементОтбора.ПравоеЗначение;
			ВидЗанятостиВидСравнения = ЭлементОтбора.ВидСравнения;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ЭлементОтбора Из НастройкиОтчета.Отбор.Элементы Цикл
		...
		Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Организация")
			...
			Или ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Подразделение")
			Или ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидЗанятости") Тогда
	...
	Если ВидЗанятости <> Неопределено Тогда 
		ЭлементОтбора = НастройкиОтчета.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидЗанятости");
		ЭлементОтбора.ВидСравнения = ВидЗанятостиВидСравнения;
		ЭлементОтбора.ПравоеЗначение = ВидЗанятости;
		ЭлементОтбора.Использование = Истина;
	КонецЕсли;
...
Функция СведенияОбОрганизациях(Организации, ДатаОтчета)
	...
	Запрос.Текст = 
		...
		|	Организации.КодОКФС КАК КодОКФС,
		|	Организации.НаименованиеОКВЭД КАК НаименованиеОКВЭД,
		|	Организации.НаименованиеОКВЭД2 КАК НаименованиеОКВЭД2,
		|	Организации.НаименованиеОКОПФ КАК НаименованиеОКОПФ,
		|	Организации.НаименованиеОКФС КАК НаименованиеОКФС,
		...
		|	ЕСТЬNULL(ДанныеВоинскогоУчетаОрганизаций.РегистрационныйНомер, """") КАК РегистрационныйНомер,
		|	ЕСТЬNULL(ДанныеВоинскогоУчетаОрганизаций.ВышестоящаяОрганизация, """") КАК ВышестоящаяОрганизация,
		|	ЕСТЬNULL(ДанныеВоинскогоУчетаОрганизаций.НеосновныеКодыОКВЭД, """") КАК НеосновныеКодыОКВЭД,
		|	ЕСТЬNULL(ДанныеВоинскогоУчетаОрганизаций.ВедетсяТолькоВоинскийУчет, ЛОЖЬ) КАК ВедетсяТолькоВоинскийУчет,
		|	ЕСТЬNULL(ДанныеВоинскогоУчетаОрганизаций.ОрганГосударственнойВласти, """") КАК ОрганГосударственнойВласти,
		|	ЕСТЬNULL(ДанныеВоинскогоУчетаОрганизаций.ВходитВОрганГосударственнойВласти, ЛОЖЬ) КАК ВходитВОрганГосударственнойВласти
		|ИЗ
		|	ВТОрганизацииПериоды КАК ДанныеДокументов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
		|		ПО ДанныеДокументов.СтруктурнаяЕдиница = Организации.Ссылка
		...
		|		ПО ДанныеДокументов.СтруктурнаяЕдиница = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.СтруктурнаяЕдиница
		|			И ДанныеДокументов.Период = ИсторияРегистрацийВНалоговомОрганеСрезПоследних.Период
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеВоинскогоУчетаОрганизаций КАК ДанныеВоинскогоУчетаОрганизаций
		|		ПО ДанныеДокументов.СтруктурнаяЕдиница = ДанныеВоинскогоУчетаОрганизаций.Организация";
