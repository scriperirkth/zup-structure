...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ВыбОбласть.ТипОбласти <> ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
		Или ВыбОбласть.Защита
		Или ВыбОбласть.ЦветФона = СтруктураРеквизитовФормы.мЦветБелый Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли; 
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	ИмяДляПроверки = ТабличныйДокумент.ТекущаяОбласть.Имя;
...
Процедура ГенерироватьМакетИСтруктуруДанныхРаздела(УзелСекция, ЭлементДереваРазделыОтчета)
	...
	Для каждого УзелОписанияСтроки Из УзлыОписанияСтрок Цикл
		...
		Для каждого УзелОписанияЯчейки Из УзлыОписанияЯчеек Цикл
			...
			Если СтрНачинаетсяС(ПараметрыЯчейки["format"], "N(") И "" = ПараметрыЯчейки["dic"]
				И СтрЗаканчиваетсяНа(ПараметрыЯчейки["format"], ",0)") Тогда 
				
				Если "M" = ПараметрыСтроки["type"] Тогда
					...
					Для Каждого ЭлементКолонка Из СписокПараметровКолонок Цикл 
						Если ЭлементКолонка.Представление = КодКолонки Тогда 
							...
							Если ОписаниеКолонки["type"] = "S" И Не ЗначениеЗаполнено(ОписаниеКолонки["dic"]) Тогда
								...
								ПараметрыЯчейки.Вставить("mask", СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("9", ОТЧ.ПривестиЗначение(НоваяДлинна)));
							КонецЕсли;
						КонецЕсли;
					КонецЦикла;
				ИначеЕсли ЗначениеЗаполнено(ПараметрыЯчейки["vld"]) И ПараметрыЯчейки["vldType"] = "2" Тогда 
					// ИНН-КПП-ОКПО иногда описываются как vld="000000001-999999999", из-за этого нет возможности указать их начинающиеся с нуля
					Разложение = СтрРазделить(ПараметрыЯчейки["vld"], "-");
					НоваяДлинна = СтрЗаменить(СтрЗаменить(ПараметрыЯчейки["format"], "N(", ""), ",0)", "");
					чНоваяДлинна = ОТЧ.ПривестиЗначение(НоваяДлинна);
					Если ОТЧ.ПривестиЗначение(Разложение[0]) = 1 И чНоваяДлинна = СтрДлина(Разложение[0]) И чНоваяДлинна = СтрДлина(Разложение[1]) Тогда 
						ПараметрыЯчейки["format"] = "C(" + НоваяДлинна + ")";
						ПараметрыЯчейки.Вставить("mask", СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("9", чНоваяДлинна));
						ПараметрыЯчейки["vld"] = "";
						ПараметрыЯчейки["vldType"] = "0";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Если ЕстьМногострочностьВРазделе Тогда 
			Для Каждого Колонка Из СписокПараметровКолонок Цикл 
				...
				ОписаниеКолонки.Вставить("mask", СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("9", ОТЧ.ПривестиЗначение(НоваяДлинна)));
			КонецЦикла;
		КонецЕсли;
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	ЕстьДанныеАвторасчета = (РасчитываемыеЯчейки.Количество() > 0 
		Или Зависимости.Количество() > 0
		Или ЗависимостиМножественныхСтрок.Количество() > 0
		Или ИтогиПоМножественнымСтрокам.Количество() > 0);
...
&НаСервере
Функция ПодсказкаЯчейки(ОбластьИмя)
	Подсказки = ПодсказкиДляЯчеекСРасчетом.НайтиСтроки(Новый Структура("ИмяЯчейки", ОбластьИмя));
	Возврат ?(Подсказки.Количество() = 1, Подсказки[0].Подсказка, "");
КонецФункции

&НаСервере
Процедура РаскраситьЯчейкиАвторасчета()
	ОблЦветФона = ?(СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет, ЦветаСтиля.ЦветМакетаБРОРучноеЗаполнение, ЦветаСтиля.ЦветМакетаБРОАвторасчет);
	Для Каждого Стр Из РасчитываемыеЯчейки.НайтиСтроки(Новый Структура("Раздел", СтруктураРеквизитовФормы.НаимТекущегоРаздела)) Цикл 
		...
		Если ТипЗнч(Обл) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда
			Обл.ЦветФона = ОблЦветФона;
			Обл.Защита = (Не СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет);
			Обл.Примечание.Текст = ?(СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет, "", ПодсказкаЯчейки(Обл.Имя));
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Стр Из ИтогиПоМножественнымСтрокам.НайтиСтроки(Новый Структура("Раздел", СтруктураРеквизитовФормы.НаимТекущегоРаздела)) Цикл 
		...
		Если ТипЗнч(Обл) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда 
			Обл.ЦветФона = ОблЦветФона;
			Обл.Защита = (Не СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет);
			Обл.Примечание.Текст = ?(СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет, "", Стр.Подсказка);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Стр Из РасчитываемыеЯчейкиМножественныхСтрок.НайтиСтроки(Новый Структура("Раздел", СтруктураРеквизитовФормы.НаимТекущегоРаздела)) Цикл 
		...
		Пока Истина Цикл 
			...
			Если ТипЗнч(Обл) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда
				Обл.ЦветФона = ОблЦветФона;
				Обл.Защита = (Не СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет);
				Обл.Примечание.Текст = ?(СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет, "", ПодсказкаЯчейки(Стр.ЗависимоеЗначение));
			Иначе
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Асинх Процедура ВводЗначенияИзСписка(СписокВвода, Область, Элемент)
	...
	ЗначениеВыбора = Ждать ВыбратьИзСпискаАсинх(СписокВвода, ТабличныйДокумент.ТекущаяОбласть, НайденныйЭлемент);
...
Процедура Подключаемый_ОбработкаНажатияКнопкиВВажномСообщении(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОбработатьНажатиеКнопкиВВажномСообщении(ЭтотОбъект, Команда.Имя, "ФСГС");
КонецПроцедуры
...
Процедура Подключаемый_ОбработкаНавигационнойСсылкиВВажномСообщении(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОбработатьНавигационнуюСсылкуВВажномСообщении(
		ЭтотОбъект, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, "ФСГС");
КонецПроцедуры
