...
Функция ТаблицаФормОтчета() Экспорт
	...
	ТаблицаФормОтчета.Колонки.Добавить("НаименованиеИзСервиса",   ОписаниеТиповСтрока);
	ТаблицаФормОтчета.Колонки.Добавить("КолонкаСуммирования",     ОписаниеТиповЧисло3);
	...
	Пока ВыборкаШаблоныЭВФ.Следующий() Цикл
		...
		НоваяФорма.НаименованиеИзСервиса   = ВыборкаШаблоныЭВФ.Наименование;
		...
		НоваяФорма.КолонкаСуммирования     = 1;
		...
		Если Не ЗначениеЗаполнено(НоваяФорма.СтрПериодичность) Тогда 
			Если НоваяФорма.Периодичность = 1 Тогда 
				НоваяФорма.СтрПериодичность = "Годовая";
			ИначеЕсли НоваяФорма.Периодичность = 4 Тогда 
				НоваяФорма.СтрПериодичность = "Квартальная";
			ИначеЕсли НоваяФорма.Периодичность = 12 Тогда 
				НоваяФорма.СтрПериодичность = "Месячная";
			ИначеЕсли НоваяФорма.Периодичность = 2 Тогда 
				НоваяФорма.СтрПериодичность = "Полугодовая";
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	ТаблицаФормОтчета.Сортировать("ОписаниеОтчета");
	Детализация = ТаблицаФормОтчета.Скопировать(, "ИндексФормыОтчета,ОКУД,КолонкаСуммирования");
	Детализация.Свернуть("ИндексФормыОтчета,ОКУД", "КолонкаСуммирования");
	Для Каждого Стр Из Детализация Цикл 
		Если Стр.КолонкаСуммирования < 2 Тогда 
			Продолжить;
		КонецЕсли;
		
		Для Каждого Стр2 Из ТаблицаФормОтчета.НайтиСтроки(Новый Структура("ИндексФормыОтчета,ОКУД", Стр.ИндексФормыОтчета, Стр.ОКУД)) Цикл 
			Стр2.ИндексФормыОтчета = Стр2.НаименованиеИзСервиса;
		КонецЦикла;
	КонецЦикла;
