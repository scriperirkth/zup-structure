...
Функция ДействующаяРедакцияФормы(Дата) Экспорт
	Возврат ?(ТекущаяДатаСеанса() < '20260701', "Форма2023_1", "Форма2026_1");
КонецФункции
...
Функция НовыйСогласиеЕНС(Организация) Экспорт 
	...
	ДанныеУведомления.Титульная.Вставить("ФормРеорг", "");
	ДанныеУведомления.Титульная.Вставить("ИННРеорг", "");
	ДанныеУведомления.Титульная.Вставить("КППРеорг", "");
	...
	СтруктураПараметров.Вставить("ДеревоСтраниц",
		?(УведомлениеОбъект.ИмяФормы = "Форма2023_1", ДеревоСтраниц2023(), ДеревоСтраниц2026()));
...
Функция ДеревоСтраниц2026() Экспорт 
	КорневойУровень = УведомлениеОСпецрежимахНалогообложения.НовоеДеревоСтраницФормыУведомления().Строки;
...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.СогласиеНаРаскрытиеНалоговойТайны;
	Стр.ДатаНачала = Дата('20221213');
	Стр.ДатаКонца = Дата('20260630');
	
	Стр = Результат.Добавить();
	Стр.ИмяФормы = "Форма2026_1";
	Стр.ОписаниеФормы = "В соответствии с приказом ФНС России от 09.06.2026 № АБ-36-19/4921@ в формате 5.04";
	Стр.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.СогласиеНаРаскрытиеНалоговойТайны;
	Стр.ДатаНачала = Дата('20260701');
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2026_1(Объект, Данные, УникальныйИдентификатор)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Данные.Организация) Тогда 
		УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаИННКПП(Истина, Титульная, ТаблицаОшибок);
		Если УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(Титульная, "ФормРеорг,ИННРеорг,КППРеорг") Тогда 
			Если Не ЗначениеЗаполнено(Титульная.ФормРеорг) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указан код формы реорганизации", "Титульная", "ФормРеорг"));
			КонецЕсли;
			УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаИНН(
				Титульная, ТаблицаОшибок, "Титульная", "ИННРеорг", Истина, Истина);
			УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаКПП(
				Титульная, ТаблицаОшибок, "Титульная", "КППРеорг", Истина);
		КонецЕсли;
	ИначеЕсли Титульная.ПризНП = "1" Тогда 
		УведомлениеОСпецрежимахНалогообложенияСлужебный.ПроверкаИННКПП(Ложь, Титульная, ТаблицаОшибок);
	ИначеЕсли Титульная.ПризНП = "2" Тогда
...
Функция СформироватьСписокЛистовФорма2017_1(Объект) Экспорт
	...
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(Объект.ИмяОтчета, СтруктураПараметров.ДанныеУведомления["Титульная"],
		НомСтр, УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2017_1_Титульная"),
	...
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(Объект.ИмяОтчета, СтруктураПараметров.ДанныеУведомления["Лист002"],
		НомСтр, УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2017_1_Лист002"), ПечатнаяФорма, ИННКПП);
	Инд = 0;
...
Функция СформироватьСписокЛистовФорма2022_1(Объект) Экспорт
	...
	УведомлениеОСпецрежимахНалогообложения.НапечататьДанные(Объект.ИмяОтчета,
		Титульная, УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2022_1_Титульная"),
		ПечатнаяФорма, ИННКПП);
	...
	Если Лист001Заполнен Тогда 
		...
		Для Каждого Стр Из СтруктураПараметров.ДанныеДопСтрокБД.МнгСтр1 Цикл
			...
			Если Инд = 96 Тогда 
				УведомлениеОСпецрежимахНалогообложения.НапечататьДанные(Объект.ИмяОтчета,
					СтруктураПечати, УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2022_1_Лист001"),
					ПечатнаяФорма, ИННКПП);
				УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, ИННКПП.НомСтр, Ложь);
				Инд = 0;
				СтруктураПечати = ОбщегоНазначения.СкопироватьРекурсивно(Лист001);
		...
		Если Инд <> 0 Тогда 
			УведомлениеОСпецрежимахНалогообложения.НапечататьДанные(Объект.ИмяОтчета,
				СтруктураПечати, УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2022_1_Лист001"), ПечатнаяФорма, ИННКПП);
			УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, ИННКПП.НомСтр, Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Если Лист002Заполнен Тогда 
		...
		Для Каждого Стр Из СтруктураПараметров.ДанныеДопСтрокБД.МнгСтр2 Цикл
			...
			КонецЕсли;
			Если Инд = 96 Тогда 
				УведомлениеОСпецрежимахНалогообложения.НапечататьДанные(Объект.ИмяОтчета, СтруктураПечати,
					УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2022_1_Лист002"),
					ПечатнаяФорма, ИННКПП);
				УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, ИННКПП.НомСтр, Ложь);
				Инд = 0;
				СтруктураПечати = ОбщегоНазначения.СкопироватьРекурсивно(Лист002);
...
Функция СформироватьСписокЛистовФорма2026_1(Объект) Экспорт
	...
	Титульная = СтруктураПараметров.ДанныеУведомления.Титульная;
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтруктуру(Объект.ИмяОтчета, СтруктураПараметров.ДанныеУведомления["Титульная"], НомСтр,
		УведомлениеОСпецрежимахНалогообложения.ПустойМакетИзСписка(Объект, СписокИспользуемыхМакетов, "Печать_Форма2026_1_Титульная"), ПечатнаяФорма, ИННКПП);
	УведомлениеОСпецрежимахНалогообложения.ПоложитьПФВСписокЛистов(Объект, Листы, ПечатнаяФорма, НомСтр, Ложь);
	
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтраницыСМногострочнойЧастью(Объект, Листы, СтруктураПараметров,
		"Лист001", "МнгСтр1", "Печать_Форма2026_1_Лист001", НомСтр, ИННКПП);
	Лист002 = СтруктураПараметров.ДанныеУведомления.Лист002;
	ИННКПП.Вставить("ИННЛист002", ?(ЗначениеЗаполнено(Лист002.ИННЮЛ), Лист002.ИННЮЛ, Лист002.ИННФЛ));
	ИННКПП.Вставить("НаимОргЛист002", ?(ЗначениеЗаполнено(Лист002.НаимОрг), Лист002.НаимОрг, 
		"" + Лист002.Фамилия + " " + Лист002.Имя + " " + Лист002.Отчество));
	УведомлениеОСпецрежимахНалогообложения.НапечататьСтраницыСМногострочнойЧастью(Объект, Листы, СтруктураПараметров,
		"Лист002", "МнгСтр2", "Печать_Форма2026_1_Лист002", НомСтр, ИННКПП);
	Возврат Листы;
КонецФункции
