...
Процедура ЗагрузитьИзXMLНаСервере(ПараметрыЗагрузкиXML)
	...
	МнгСтр.Колонки.Добавить("Период");
	ЭтотОбъект["ДанныеДопСтрок"].Вставить("МнгСтр", ПоместитьВоВременноеХранилище(МнгСтр));
	ДеревоСтраниц.ПолучитьЭлементы()[0].Многострочность = Истина;
	ДеревоСтраниц.ПолучитьЭлементы()[0].МногострочныеЧасти.Добавить("МнгСтр");
	...
	Для Каждого Стр Из ПолучитьИзВременногоХранилища(ЭтотОбъект["ДанныеДопСтрок"]["МнгСтр"]) Цикл 
		Инд = Инд + 1;
		Титульная.Вставить("Период" + Инд, Стр.Период);
	КонецЦикла;
