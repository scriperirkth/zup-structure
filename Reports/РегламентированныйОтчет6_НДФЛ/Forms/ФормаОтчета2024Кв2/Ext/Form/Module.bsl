...
Процедура СформироватьСпискиВыбора()
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2027 Тогда
		...
		ИмяСекции_КодыВычета_Приложение1_Раздел3 = "КодыВычета_Приложение1_Раздел3_20270101";
		ИмяСекции_КодыДохода = "КодыДохода_20270101";
		ИмяСекции_КодыВычета_Приложение1_Приложение = "КодыВычета_Приложение1_Приложение_20270101";
...
Процедура ЗаполнитьАвто(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ПредложитьОформитьЗаявлениеНаПодключениеПриЗаполнении(ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьАвтоПродолжение()
	...
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("Команда", Новый Структура("Действие,Имя", "ЗаполнитьАвто", "Заполнить"));
...
#КонецОбласти
	...
	КонецЕсли;
	Если РегламентированнаяОтчетностьКлиент.ЭтоСобытиеЗакрытияФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(ЭтаФорма,
		ИмяСобытия, Параметр) Тогда
			ЗаполнитьАвтоПродолжение();
	КонецЕсли;
КонецПроцедуры
