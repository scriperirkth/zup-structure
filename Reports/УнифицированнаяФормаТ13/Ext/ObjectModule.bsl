...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если КлючСхемы <> КлючВарианта Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.УчетБюджетныхУчреждений") Тогда
			ЭтоФорма0504421с2015 = КлючВарианта = "Форма0504421с2015";
			НайденныйПараметр = СхемаКомпоновкиДанных.Параметры.Найти("ОтветственныйИсполнитель");
			Если НайденныйПараметр <> Неопределено Тогда
				НайденныйПараметр.ВключатьВДоступныеПоля = ЭтоФорма0504421с2015;
				НайденныйПараметр.ОграничениеИспользования = Не ЭтоФорма0504421с2015;
			КонецЕсли;
			НайденныйПараметр = СхемаКомпоновкиДанных.Параметры.Найти("Бухгалтер");
			Если НайденныйПараметр <> Неопределено Тогда
				НайденныйПараметр.ВключатьВДоступныеПоля = ЭтоФорма0504421с2015;
				НайденныйПараметр.ОграничениеИспользования = Не ЭтоФорма0504421с2015;
			КонецЕсли;
