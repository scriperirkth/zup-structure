...
Функция РелизованаПоддержкаЗаполненияВОблачнойОтчетности(ИДРедакцииОтчета) Экспорт
	...
	МассивПоддерживаемых.Добавить("ФормаОтчета2026Кв2");
...
Процедура СформироватьСтруктуруРеквизитовФормы(ИмяФормы, СтруктураРеквизитовФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2")
		ИЛИ ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1")
...
Функция ИзвлечьСтруктуруXML(ИмяФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2") Тогда
		
		Макет = ЭтотОбъект.ПолучитьМакет("СхемаВыгрузки2026Кв2");
		
	ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	Если СтроковыеФункцииКлиентСервер.СтрокаВЧисло((ПараметрыВыгрузки.ОтчетГод)) < 2023 Тогда
		СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Календарный год не должен быть ранее 2023 года", "Титульный", "ОтчетГод");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2")
		ИЛИ ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
...
Функция ВозможныеЗначенияИзСпискаВыбораПоказателя(ИмяФормы, ИмяГруппыЗначений)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2") Тогда
		...
		МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СпискиВыбора2026Кв2");
	...
	Если ТипЗнч(СтрокаДерева) = Тип("ДеревоЗначений") Тогда
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2")
			...
			Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2") Тогда
				...
				ПотокXML.ЗаписатьАтрибут("xmlns", "http://пф.рф/ЕФС-1/2026-06-01");
				
			Иначе// "ФормаОтчета2026Кв1"
			...
			Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2") Тогда
				...
				ПотокXML.ЗаписатьАтрибут("xmlns:ЕФС9", "http://пф.рф/ВС/ЕФС/2026-06-01");
...
Процедура ВывестиНаПечатьЛист(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов, СтруктураМногоуровневыхРазделов)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв2") Тогда
		...
		Макет = ЭтотОбъект.ПолучитьМакет("ПечатныйБланк2026Кв2");
