...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	СкрытьНастройкиСостоянияСтруктурыПредприятия(КлючВарианта, НовыеНастройкиКД);
...
Процедура СкрытьНастройкиСостоянияСтруктурыПредприятия(КлючВарианта, НовыеНастройкиКД)
	
	НастройкиКД = ?(НовыеНастройкиКД = Неопределено, КомпоновщикНастроек.Настройки, НовыеНастройкиКД);
	Если ОрганизационнаяСтруктура.ПодсистемаСостоянияСтруктурыПредприятияСуществует()
		Или Не ЭтоВариантУправленческаяСтруктура(КлючВарианта, НастройкиКД) Тогда
		Возврат;
	КонецЕсли;

	СкрытьПользовательскуюНастройкуПараметра(НастройкиКД, "ПоказыватьНовыеПодразделения");
	СкрытьПользовательскуюНастройкуПараметра(НастройкиКД, "ПоказыватьРасформированныеПодразделения");

КонецПроцедуры

Функция ЭтоВариантУправленческаяСтруктура(КлючВарианта, НастройкиКД)

	Если КлючВарианта = "УправленческаяСтруктура" Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если НастройкиКД.ДополнительныеСвойства.Свойство("КлючПредопределенногоВарианта", КлючПредопределенногоВарианта) Тогда
		Возврат КлючПредопределенногоВарианта = "УправленческаяСтруктура";
	КонецЕсли;
...
Процедура СкрытьПользовательскуюНастройкуПараметра(НастройкиКД, ИмяПараметра)
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(ИмяПараметра));
	Если ПараметрДанных = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ПараметрДанных.ИдентификаторПользовательскойНастройки = "";
	ПараметрДанных.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
...
Функция ФотографииСотрудников(ДанныеОтчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеФайла) = Тип("Картинка") Тогда
			Фотография = ДанныеФайла;
		Иначе
			Фотография = Новый Картинка(ДанныеФайла);
		КонецЕсли;
