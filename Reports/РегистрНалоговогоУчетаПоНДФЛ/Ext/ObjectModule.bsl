...
Процедура ПолучитьДанныеДляПостроенияОтчета(СписокФизЛицОрганизация)
	...
	КадровыеДанныеОтветственного = ?(Ответственный = Неопределено, Неопределено, КадровыйУчет.КадровыеДанныеСотрудника(Ложь, Ответственный.Значение, "Должность", ДатаПодписи));
	ДолжностьОтветственного = ?(КадровыеДанныеОтветственного = Неопределено, "", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КадровыеДанныеОтветственного.Должность, "Наименование"));
	
	// Создание запроса и установка всех необходимых параметров.
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ДолжностьОтветственного", ДолжностьОтветственного);
	...
	|	&ДолжностьОтветственного КАК Ответственный_Должность,
