...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьНачальноеЗначениеОтражениеВзаиморасчетовССотрудниками");
КонецПроцедуры
...
Процедура УстановитьНастройки(Настройки) Экспорт
	...
	ОчиститьМакетыКомпоновкиДанных = Ложь;
	
	Запись = РегистрыСведений.ДополнительныеНастройкиЗарплатаКадры.СоздатьМенеджерЗаписи();
	Запись.Прочитать();
	Если Не Запись.Выбран()
		Или Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Настройки, "ОтражениеВзаиморасчетовССотрудниками")
		Или Запись.ОтражениеВзаиморасчетовССотрудниками <> Настройки.ОтражениеВзаиморасчетовССотрудниками Тогда
		
		ОчиститьМакетыКомпоновкиДанных = Истина;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(Запись, Настройки);
	Запись.Записать();
	
	Если ОчиститьМакетыКомпоновкиДанных Тогда
