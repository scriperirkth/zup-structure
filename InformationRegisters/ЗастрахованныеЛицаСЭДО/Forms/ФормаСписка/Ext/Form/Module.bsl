...
Процедура ОбновитьНадписьСообщенияОжидаемыеОтФСС()
	...
	Иначе
		...
		Если КоличествоОжидаемыхСообщений = 1 Тогда
			ЗаголовокКнопки = НСтр("ru = 'Проверить наличие ответа'");
			Текст = НСтр("ru = 'Ожидается 1 сообщение СЭДО о регистрации сведений о застрахованном лице'");
			Элементы.НадписьСообщенияОжидаемыеОтФСС.Заголовок = Текст;
		Иначе
			ЗаголовокКнопки = НСтр("ru = 'Проверить наличие ответов'");
			...
			Элементы.НадписьСообщенияОжидаемыеОтФСС.Заголовок = СтрШаблон(
				НСтр("ru = 'Ожидается %1 о регистрации сведений о застрахованных лицах'"),
	...
	ИначеЕсли Количество = 1 Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ПоследниеСведения)
			...
			ПоказатьВопрос(Оповещение,
				СтрШаблон(НСтр("ru = 'У сотрудника уже есть документ %1, сформировать новые сведения?'"), Сведения),
...
Процедура ПометитьНаУдалениеНаСервере(МассивСсылок)
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Если Документы.СведенияОЗастрахованномЛицеФСС.ОбъектЗафиксирован(ДокументОбъект) Тогда
			Текст = СтрШаблон(НСтр("ru = '%1 не может быть помечен на удаление, поскольку отправлен в СЭДО.'"), Ссылка);
