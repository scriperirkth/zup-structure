...
// Заполняет ученые звания физического лица.
// 
// Параметры:
//  ФизическоеЛицо - СправочникСсылка.ФизическиеЛица
//  УченыеЗвания - Массив из Структура - см. НовоеУченоеЗвание
Процедура ЗаполнитьУченыеЗванияФизическогоЛица(ФизическоеЛицо, УченыеЗвания) Экспорт
	
	Если УченыеЗвания.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	НаборЗаписей = РегистрыСведений.УченыеЗванияФизическихЛиц.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ФизическоеЛицо.Установить(ФизическоеЛицо);
	НаборЗаписей.Прочитать();
	
	Для Каждого ТекущаяСтрока Из УченыеЗвания Цикл
...
// Создает новую структуру для сведений об ученом звании.
// 
...
//   * НаучнаяСпециальность - СправочникСсылка.КлассификаторСпециальностейПоОбразованию -
//   * АттестатСерия 		- Строка -
//   * АттестатНомер 		- Строка -
//   * АттестатВыданОрганизация		- Строка -
//   * ДатаПрисвоенияУченогоЗвания	- Дата -
Функция НовоеУченоеЗвание() Экспорт
	Возврат ОбщегоНазначенияБЗК.СтруктураПоИмениРегистраСведений("УченыеЗванияФизическихЛиц");
КонецФункции
