...
// Загружает ошибку логического контроля (тип 14) полученную в ответ на документ СЭДО 104.
Процедура ЗагрузитьОшибкуСообщения10410(Страхователь, ИдентификаторСообщения, ТекстОшибки, Результат) Экспорт
	ЗагрузитьОшибкуСообщения104(Страхователь, ИдентификаторСообщения, ТекстОшибки, Результат);
КонецПроцедуры
...
Процедура ЗагрузитьСообщение(ТипСообщения, Страхователь, РегистрацияИдентификатор, ТекстXML, Результат, Кэш) Экспорт
	Если ТипСообщения = 106 Тогда
		ЗагрузитьСообщение106(Страхователь, РегистрацияИдентификатор, ТекстXML, Результат);
	ИначеЕсли ТипСообщения = 10610 Тогда
		ЗагрузитьСообщение10610(ТипСообщения, Страхователь, РегистрацияИдентификатор, ТекстXML, Результат, Кэш);
	КонецЕсли;
КонецПроцедуры
	...
	Если ПротоколDOM <> Неопределено Тогда
		...
		Для Каждого ГруппаОшибокDOM Из ПротоколDOM.ДочерниеУзлы Цикл
			...
			Для Каждого ОшибкаDOM Из ГруппаОшибокDOM.ДочерниеУзлы Цикл
				...
				// Пример: <notificationID>N_1234567890_2000_01_01_0001:1</notificationID>
...
// Загружает результат уведомления о прекращении отпуска по уходу за ребенком до полутора лет.
Процедура ЗагрузитьСообщение10610(ТипСообщения, Страхователь, РегистрацияИдентификатор, ТекстXML, Результат, Кэш) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	
	// Пример:
	//<n1:noticeTermination05Response
	//    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	//    xmlns:n1="urn:ru:ecp:integration:vnim:noticeTermination05:response:v3.0"
	//    specVersion="3.0.0"
	//    responseOn="a0930cd4-7626-464f-93c9-64e2b7be0779"
	//    messageDateTime="2026-10-15T09:31:47Z"
	//    xsi:schemaLocation="urn:ru:ecp:integration:vnim:noticeTermination05:response:v3.0 NoticeTermination05Response.xsd">
	//    <status>PROCESSED</status>
	//</n1:noticeTermination05Response>
	
	СтруктураDOM = СериализацияБЗК.СтруктураDOM(ТекстXML);
	КореньDOM = СериализацияБЗК.НайтиУзелDOMПоИмени(СтруктураDOM, "noticeTermination05Response");
	Если КореньDOM = Неопределено Тогда
		СЭДОФСС.ЗаписатьОшибкуПоискаУзла(Результат, "noticeTermination05Response");
		Возврат;
	КонецЕсли;
	
	АтрибутыКорня = СериализацияБЗК.АтрибутыЭлементаDOM(КореньDOM, "specVersion, responseOn, messageDateTime");
	ИдентификаторСообщения = СериализацияБЗК.СтрокаИзXML(АтрибутыКорня.responseOn);
	Если Не ЗначениеЗаполнено(ИдентификаторСообщения) Тогда
	...
	РегистрацияДата = СЭДОФСС.ДатаСообщения(РегистрацияИдентификатор, Кэш, АтрибутыКорня.messageDateTime);
	
	РеквизитыКорня = СериализацияБЗК.УзлыЭлементаDOM(КореньDOM, "status, errorList, warnList");
	...
	Если ТекстыПредупреждений <> "" Тогда
		ТекстыОшибок = СокрЛП(ТекстыОшибок + Символы.ПС + "----" + Символы.ПС + ТекстыПредупреждений);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(РегистрацияСтатус) И Не ЗначениеЗаполнено(СведенияОСтатусе.Расшифровка) Тогда
		...
		ТекстыОшибок = СокрЛП(Текст + Символы.ПС + "----" + Символы.ПС + ТекстыОшибок);
	КонецЕсли;
	...
	КорневаяЗапись.РегистрацияДата = РегистрацияДата;
	КорневаяЗапись.РегистрацияСтатус = РегистрацияСтатус;
	КорневаяЗапись.РегистрацияТекстОшибки = ТекстыОшибок;
	КорневаяЗапись.РегистрацияИдентификатор = РегистрацияИдентификатор;
	КорневаяЗапись.РегистрацияВерсияСпецификации = ВерсияСпецификации;
	КорневаяЗапись.РегистрацияТипСообщения = ТипСообщения;
...
Функция ВозможныеСтатусы() Экспорт
	...
	// Статусы из спецификации типа 10610.
	СтрокаТаблицы = Результат.Добавить();
	СтрокаТаблицы.Статус          = "ERROR";
	СтрокаТаблицы.Зарегистрирован = Ложь;
	СтрокаТаблицы.Расшифровка     = НСтр("ru = 'Не принят Фондом, выявлены ошибки'");
	СтрокаТаблицы.Состояние       = Перечисления.СостоянияДокументаСЭДОФСС.НеПринят;
	
	СтрокаТаблицы = Результат.Добавить();
	СтрокаТаблицы.Статус          = "PROCESSED";
	...
	// Статусы из спецификации типа 106.
	СтрокаТаблицы = Результат.Добавить();
