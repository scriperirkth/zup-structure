...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	ОбновитьПодключаемыеХарактеристикиДолжностей(Замещение);
...
Процедура ОбновитьПодключаемыеХарактеристикиДолжностей(Замещение)
	
	ОтборНаборов = РегистрыБЗК.ОтборыЗаписейНабора(ЭтотОбъект, Замещение);

	ОтборОбъект = Неопределено;
	ОтборКлассификатор = Неопределено;
	
	Для Каждого ОтборНабора Из ОтборНаборов Цикл
		
		Если ОтборНабора["Объект_Использование"] Тогда
			ОтборОбъект = ОтборНабора["Объект"];
		КонецЕсли;
		
		Если Не ОтборНабора["Классификатор_Использование"] Тогда
			ОтборКлассификатор = ОтборНабора["Классификатор"];
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОтборОбъект) Или ЗначениеЗаполнено(ОтборКлассификатор) Тогда
		...
		ПодключаемыеХарактеристики.Колонки.Добавить("ИдентификаторИсточника", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(100)));
		Для Каждого ХарактеристикаДолжности Из ПодключаемыеХарактеристики Цикл
			...
			ХарактеристикаДолжности.ИдентификаторИсточника = "НСИФРМР";
		КонецЦикла;
	...
	ИначеЕсли Количество() = 0 И НЕ РегистрыБЗК.ЭтоУдалениеНабораЗаписей(Замещение) Тогда
