...
&НаКлиенте
Процедура ДобавитьПользовательскиеПечатныеФормы(Команда)
	
	СписокПользовательскихПечатныхФорм = ИдентификаторыПользовательскихПечатныхФорм();
	Если СписокПользовательскихПечатныхФорм.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Не найдено пользовательских печатных форм'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ОбработатьВыборВнешнейПечатнойФормы", ЭтотОбъект);
		СписокПользовательскихПечатныхФорм.ПоказатьВыборЭлемента(Оповещение, НСтр("ru = 'Выберите печатную форму'"), );
	КонецЕсли;
...
&НаСервере
Функция ИдентификаторыПользовательскихПечатныхФорм()
	
	ПользовательскиеПечатныеФормы = Новый СписокЗначений;
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	МакетыПечатныхФормИсточникиДанных.Ссылка.Идентификатор КАК Идентификатор,
		|	МакетыПечатныхФормИсточникиДанных.Ссылка.Наименование КАК Представление,
		...
		|ГДЕ
		|	МакетыПечатныхФормИсточникиДанных.Ссылка.ТипМакета = ""MXL""
		|
		|УПОРЯДОЧИТЬ ПО
		|	Идентификатор,
		|	ИсточникДанных";
	...
	Пока Выборка.СледующийПоЗначениюПоля("Идентификатор") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			МетаданныеОбъекта = ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору(Выборка.ИсточникДанных, Ложь);
			Если МетаданныеОбъекта <> Null
				И ЭДОБЗК.ЭтоТипОбъектаСПечатнымиФормами(
					ТипЗнч(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
						МетаданныеОбъекта.ПолноеИмя()).ПустаяСсылка())) Тогда
				
				Если ПредставленияОбъектовМетаданных.Количество() = 4 Тогда
					ПредставленияОбъектовМетаданных.Добавить("...");
					Прервать;
				ИначеЕсли ПредставленияОбъектовМетаданных.Количество() < 4 Тогда
					ПредставленияОбъектовМетаданных.Добавить(МетаданныеОбъекта.Представление());
				КонецЕсли;
		...
		Если ДобавитьИдентификатор Тогда
			Представление = Выборка.Представление + ": (" + СтрСоединить(ПредставленияОбъектовМетаданных, ", ") + ")";
			ПользовательскиеПечатныеФормы.Добавить("ПФ_" + Выборка.Идентификатор, Представление);
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ПользовательскиеПечатныеФормы;
