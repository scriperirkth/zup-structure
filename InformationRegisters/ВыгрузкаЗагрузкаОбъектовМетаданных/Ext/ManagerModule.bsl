...
// Параметры:
//  ИдентификаторЗадания - УникальныйИдентификатор - идентификатор фонового задания потока для которого получается объект
//  ПоПриоритетуОбработки - Булево - Признак получения объекта с учетом приоритета обработки объектов
// 
// Возвращаемое значение:
//  Структура - Получить объект к обработке:
// * ЕстьОбъектыКОбработке - Булево
// * Объект - см. РегистрыСведений.ВыгрузкаЗагрузкаОбъектовМетаданных.ВыборкаОбъектовКОбработке
Функция ПолучитьОбъектКОбработке(ИдентификаторЗадания, ПоПриоритетуОбработки = Ложь) Экспорт
	...
	Попытка
		...
		Если ВыборкаОбъектов.Следующий() Тогда
			...
			Если РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.НужноЗавершитьПоток(ИдентификаторЗадания) Тогда
				Результат.ЕстьОбъектыКОбработке = Ложь;
				Результат.Объект = Неопределено;
			КонецЕсли;
...
Функция ВыборкаОбъектовКОбработке(Обрабатывается = Ложь, КоличествоОбъектов = 0) Экспорт
	...
	ТекстПервые = ?(ЗначениеЗаполнено(КоличествоОбъектов), "ПЕРВЫЕ " + Формат(КоличествоОбъектов, "ЧГ=0;"), "");// @query-part-1
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПЕРВЫЕ 1000", ТекстПервые);// @query-part-1
...
// Есть не обработанные объекты метаданных.
// 
// Возвращаемое значение:
//  Булево
Функция ЕстьНеОбработанныеОбъектыМетаданных() Экспорт
