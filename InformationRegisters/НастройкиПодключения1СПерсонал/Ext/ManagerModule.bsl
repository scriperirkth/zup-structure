...
Процедура УстановитьАдресРесурсовAPI(НовоеЗначение) Экспорт
	
	НаборЗаписей = РегистрыСведений.НастройкиПодключения1СПерсонал.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	Записывать = Ложь;
	Если НаборЗаписей.Количество() > 0 Тогда
		Если НовоеЗначение <> НаборЗаписей[0].АдресРесурсовAPI Тогда
			НаборЗаписей[0].АдресРесурсовAPI = НовоеЗначение;
			Записывать = Истина;
		КонецЕсли;
	Иначе
		ЗаписьНабора = НаборЗаписей.Добавить();
		ЗаписьНабора.АдресРесурсовAPI = НовоеЗначение;
		Записывать = Ложь;
	КонецЕсли;
...
Процедура УстановитьСостояниеБлокировки(ПриложениеЗаблокировано) Экспорт

	НаборЗаписей = РегистрыСведений.НастройкиПодключения1СПерсонал.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	Записывать = Ложь;
	Если НаборЗаписей.Количество() > 0 Тогда
		Если ПриложениеЗаблокировано <> НаборЗаписей[0].ПриложениеЗаблокировано Тогда
			НаборЗаписей[0].ПриложениеЗаблокировано = ПриложениеЗаблокировано;
			Записывать = Истина;
		КонецЕсли;
	Иначе
		ЗаписьНабора = НаборЗаписей.Добавить();
		ЗаписьНабора.ПриложениеЗаблокировано = ПриложениеЗаблокировано;
		Записывать = Истина;
	КонецЕсли;
