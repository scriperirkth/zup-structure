...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УдалитьЛишниеЗаписиДляГруппыПозицийПодразделения");
КонецПроцедуры
...
Процедура ПодразделенияФизическихЛицПриЗаписиРегистра(Источник, Замещение) Экспорт
	...
	Если НовыйНабор.Количество() > 0 Тогда
		ЗаписьНабора = НовыйНабор[НовыйНабор.Количество() - 1];
	...
	Если ПрежнийНабор.Количество() > 0 Тогда
		ЗаписьНабора = ПрежнийНабор[ПрежнийНабор.Количество() - 1];
