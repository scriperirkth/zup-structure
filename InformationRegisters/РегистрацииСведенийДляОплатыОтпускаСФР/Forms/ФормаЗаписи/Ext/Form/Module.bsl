...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Менеджер.ЗаполнитьСписокВыбораСтатусов(Элементы.РегистрацияСтатус, Запись.РегистрацияСтатус,
		ТипЗнч(Запись.ИсходящийДокумент) = Тип("ДокументСсылка.ДополнительныеСведенияДляОплатыОтпускаСФР"));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_СведенияДляОплатыОтпускаСФР"
		И (Источник = Запись.ИсходящийДокумент Или Не ЗначениеЗаполнено(Источник)) Тогда
		ПодключитьОбработчикОжиданияПрочитать();
	ИначеЕсли ИмяСобытия = "Запись_ДополнительныеСведенияДляОплатыОтпускаСФР"
		И (Источник = Запись.ИсходящийДокумент Или Не ЗначениеЗаполнено(Источник)) Тогда
		ПодключитьОбработчикОжиданияПрочитать();
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("Запись_РегистрацииСведенийДляОплатыОтпускаСФР", Запись.ИсходящийДокумент, Запись.ИсходящийДокумент);
...
Процедура ПоясняющаяНадписьВШапкеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ТипДокумента = ТипЗнч(Запись.ИсходящийДокумент);
	Если ТипДокумента = Тип("ДокументСсылка.СведенияДляОплатыОтпускаСФР") Тогда
		ОткрытьФорму("Документ.СведенияДляОплатыОтпускаСФР.ФормаОбъекта", Новый Структура("Ключ", Запись.ИсходящийДокумент));
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ДополнительныеСведенияДляОплатыОтпускаСФР") Тогда
		ОткрытьФорму("Документ.ДополнительныеСведенияДляОплатыОтпускаСФР.ФормаОбъекта", Новый Структура("Ключ", Запись.ИсходящийДокумент));
	КонецЕсли;
КонецПроцедуры
