...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьИнформациюОПриеме");
	Методы.Вставить("ЗаполнитьНомераРеестровВСлучаеНесколькихОтветов");
КонецПроцедуры
...
Функция ЗаполнитьПоДокументу(ИсходящийДокументОбъект, РезультатОтправки = Неопределено) Экспорт
...
Функция ЗаполнитьПоРезультатуДоставки(ИсходящийДокумент, Сообщение) Экспорт
...
Процедура ПриЗаписиУведомленияОСтатусеВыплатыПособия(Документ, УдалитьСсылки = Неопределено) Экспорт
	...
	Если НомерПроцессаЗаполнен Тогда
		...
		|	И Регистр.ГоловнаяОрганизация = &ГоловнаяОрганизация
		|	И Регистр.ДатаОтправки <= &ДатаСообщения
		|	И Регистр.ВыплатаСтатус <= &СтатусВыплаты";
		ТекстыЗапроса.Добавить(ТекстЗапроса);
		Запрос.УстановитьПараметр("ДатаСообщения", Документ.ДатаСообщения);
...
Процедура ПриЗаписиДокументаУведомлениеОбИзмененииРасчетаПособия(Документ) Экспорт
	Если Документ.ПометкаУдаления
		Или Не ЗначениеЗаполнено(Документ.ВидУведомления)
		Или Не ЗначениеЗаполнено(Документ.ГоловнаяОрганизация)
		Или Не ЗначениеЗаполнено(Документ.НомерПроцесса) Тогда
...
Процедура УдалитьСсылкиНаУведомлениеОбИзмененииРасчетаПособия(ДокументСсылка) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Шапка.ИсходящийДокумент КАК ИсходящийДокумент
...
Процедура ПриЗаписиДокументаУведомлениеОбОтсутствииОтвета(Документ) Экспорт
	Если Документ.ПометкаУдаления
		Или Не ЗначениеЗаполнено(Документ.ДатаСообщения)
		Или Не ЗначениеЗаполнено(Документ.НомерПроцесса) Тогда
		УдалитьСсылкиНаУведомлениеОбОтсутствииОтвета(Документ.Ссылка);
		Возврат;
	КонецЕсли;
	...
	|	И Шапка.ДатаОтправки <= &ДатаСообщения";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Набор = НачатьЗаписьНабора(Выборка.Ссылка);
		Если Набор = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Для Каждого Запись Из Набор Цикл
			Запись.УведомлениеОбОтсутствииОтвета = Документ.Ссылка;
			Запись.УведомлениеОбОтсутствииОтветаДата = Документ.ДатаСообщения;
		КонецЦикла;
		Набор.ДополнительныеСвойства.Вставить("УведомлениеОбОтсутствииОтветаОбновлено", Истина);
		ЗавершитьЗаписьНабора(Набор);
	КонецЦикла;
КонецПроцедуры

Процедура УдалитьСсылкиНаУведомлениеОбОтсутствииОтвета(ДокументСсылка) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Шапка.ИсходящийДокумент КАК ИсходящийДокумент
	...
	|	РегистрСведений.РегистрацииОтветовНаЗапросыФССДляРасчетаПособий КАК Шапка
	|ГДЕ
	|	Шапка.УведомлениеОбОтсутствииОтвета = &ДокументСсылка";
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		Для Каждого Запись Из Набор Цикл
			Запись.УведомлениеОбОтсутствииОтвета = Неопределено;
			Запись.УведомлениеОбОтсутствииОтветаДата = Неопределено;
		КонецЦикла;
		ЗавершитьЗаписьНабора(Набор);
	КонецЦикла;
КонецПроцедуры
