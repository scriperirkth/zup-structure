...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Для Каждого ОтборЗаписейНабора Из ОтборыЗаписейНабора Цикл
		...
		ОтборТолькоПоФизическомуЛицу = ОтборЗаписейНабора["ФизическоеЛицо_Использование"];
		Если ОтборТолькоПоФизическомуЛицу Тогда
			СтруктураПоиска.Вставить("ФизическоеЛицо", ОтборЗаписейНабора["ФизическоеЛицо"]);
		КонецЕсли;
		
		Если ОтборЗаписейНабора["ВидСтажа_Использование"] Тогда
			СтруктураПоиска.Вставить("ВидСтажа", ОтборЗаписейНабора["ВидСтажа"]);
		КонецЕсли;
		
		НаборЗаписейОтбора = НаборЗаписей.НайтиСтроки(СтруктураПоиска);
		
		Если ОтборТолькоПоФизическомуЛицу Тогда
			// Проверяем только записываемый набор.
			
			ВидыСтажа = ОбщегоНазначения.ВыгрузитьКолонку(НаборЗаписейОтбора, "ВидСтажа", Истина);
			Запрос = Новый Запрос;
		...
		Иначе
			...
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("ЗаписываемыйНабор", НаборЗаписей.Скопировать(НаборЗаписейОтбора));
			Запрос.Текст =
				"ВЫБРАТЬ
