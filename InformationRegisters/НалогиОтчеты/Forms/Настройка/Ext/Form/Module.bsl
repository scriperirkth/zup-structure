...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоРедактированияНастроек = Ложь;
	ЗадачиБухгалтераПереопределяемый.ПроверитьПравоРедактированияНалогиОтчеты(ПравоРедактированияНастроек);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ЭтоВидПодакцизныхТоваров = Ложь;
	ЗадачиБухгалтераКлиентПереопределяемый.ПроверитьЭтоВидПодакцизныхТоваров(ЭтоВидПодакцизныхТоваров, ТипЗнч(Строка.Ссылка));
	...
	ИначеЕсли ЭтоВидПодакцизныхТоваров Тогда
		Родитель = Строка.ПолучитьРодителя();
		Если Родитель = Неопределено Тогда
			Возврат;
		КонецЕсли;
	...
	Если Строка.Идентификатор = "БухгалтерскаяОтчетность" Тогда
		...
		ЗадачиБухгалтераКлиентПереопределяемый.ОткрытьФормуПоСтрокеБухгалтерскаяОтчетность(Организация);
		
	ИначеЕсли Строка.Идентификатор = "НалогНаПрибыль" Тогда
		
		ЗадачиБухгалтераКлиентПереопределяемый.ОткрытьФормуПоСтрокеНалогНаПрибыль(Организация);
	...
	Для ИндексПараметра = 0 По ОписаниеПараметров.ВГраница() Цикл
		Если ОписаниеПараметров[ИндексПараметра] = "ПорядокУплатыНалоговНаМестах" Тогда
			...
			Если ТипЗнч(НастройкиУплатыНалога) = Тип("Структура") Тогда
				...
				ЗадачиБухгалтераПереопределяемый.УстановитьОписаниеПорядокУплатыНалоговНаМестах(НастройкиУплатыНалога);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьУсловноеОформление()
	...
	ЗадачиБухгалтераПереопределяемый.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "Включен");
...
Процедура ОповеститьОбИзмененииНастроек()
	...
	ЗадачиБухгалтераКлиентПереопределяемый.ОповеститьОбИзмененииНастроекНалогиОтчеты();
...
Процедура ОбновитьНастройкиУчетаНалогаНаПрибыль()
	...
	Порядок = "";
	УсловияПримененияТребованийЗаконодательстваПереопределяемый.ОпределитьПорядокУплатыАвансовДействующейНастройки(Порядок, Организация);
	Для Каждого ЭлементДерева Из Список.ПолучитьЭлементы() Цикл
		Если ЭлементДерева.Наименование = "Налог на прибыль" Тогда
			
			ЭлементДерева.ДетальнаяНастройкаПредставление = НСтр("ru = 'Уплата авансов: '") + Строка(Порядок);
...
Процедура ОбновитьНастройкиУчетнойПолитики()
	...
	Вариант = "";
	УсловияПримененияТребованийЗаконодательстваПереопределяемый.ОпределитьВариантБухгалтерскойОтчетностиДействующейНастройки(Вариант, Организация);
	Для Каждого ЭлементДерева Из Список.ПолучитьЭлементы() Цикл
		Если ЭлементДерева.Наименование = "Бухгалтерская отчетность" Тогда
			
			ЭлементДерева.ДетальнаяНастройкаПредставление = НСтр("ru = 'Состав форм: '") + Строка(Вариант);
