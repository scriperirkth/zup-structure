...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УсловияПримененияТребованийЗаконодательстваПереопределяемый.ПроверитьЭтоВидПодакцизныхТоваровАлкогольнаяПродукция(ЭтоАлкогольнаяПродукция, ПредметНастройки);
	Если ЭтоАлкогольнаяПродукция Тогда
		Элементы.УплачиваютсяНеУплачиваютсяАвансы.Заголовок = Параметры.ЗаголовокВыбора;
		ЭтаФорма.УплачиваютсяНеУплачиваютсяАвансы = Параметры.АвансыПриЗакупкеСырья;
		Элементы.УплачиваютсяНеУплачиваютсяАвансы.Видимость = Истина;
		Элементы.УплачиваютсяНеУплачиваютсяАвансы.Доступность = Не Параметры.ТолькоПросмотр;
		Акцизы = Истина;
	ИначеЕсли ТипЗнч(Параметры.НалогОтчет) = Тип("СправочникСсылка.ЗадачиБухгалтера")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.НалогОтчет, "Наименование") = "акцизы" Тогда
		Элементы.ПолученоНеПолучено.Заголовок = Параметры.ЗаголовокВыбора;
		ЭтаФорма.ПолученоНеПолучено = Параметры.ЛьготныйСрокУплаты;
		Элементы.ПолученоНеПолучено.Видимость = Истина;
		Элементы.ПолученоНеПолучено.Доступность = Не Параметры.ТолькоПросмотр;
		Акцизы = Истина;
	Иначе
	...
	Если (ПараметрыНастройкиСервер = Неопределено
		...
		Или ЗначенияНастройки.Количество() = 0)
		И НЕ Акцизы Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура КомандаОК(Команда)
	...
	Если Акцизы Тогда
		...
		"ЛьготныйСрокУплаты, АвансыПриЗакупкеСырья", ПолученоНеПолучено, УплачиваютсяНеУплачиваютсяАвансы)
	Иначе
		...
		Для Каждого ОписаниеПараметра Из ПараметрыНастройки Цикл
			...
			Если ОписаниеПараметра.ПорядокУплатыНалоговНаМестах Тогда
				...
				ПорядокУплаты.Вставить("Налог",                   Налог);
