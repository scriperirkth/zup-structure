...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru = 'Новые сообщения СЭДО об ЭЛН'");
		Дело.Количество     = Требования.ОжидающихПолучения;
		Дело.Подсказка      = НСтр("ru = 'Получить из СЭДО сообщения об ЭЛН.'");
		...
		Дело.Подсказка      = НСтр("ru = 'Сообщения СФР об изменении ЭЛН, по которым требуется создать больничные или отправить данные в Фонд.'");
...
Процедура ЗагрузитьУведомлениеОНаличииСообщения5(Страхователь, ИдентификаторСообщения) Экспорт
	...
	Запись.ДатаСообщения = СЭДОФСС.ДатаСообщения(ИдентификаторСообщения, Неопределено);
...
Процедура ЗагрузитьСообщение111(Страхователь, ИдентификаторСообщения, ТекстXML, Результат, Кэш) Экспорт
	...
	Если Запись.Страхователь <> Страхователь Тогда
		...
		Запись.Организация  = Страхователь;
	КонецЕсли;
	Запись.ДатаСообщения = СЭДОФСС.ДатаСообщения(ИдентификаторСообщения, Кэш);
	Запись.ГоловнаяОрганизация    = ЗарплатаКадры.ГоловнаяОрганизация(Запись.Страхователь);
	Запись.ИдентификаторСообщения = ИдентификаторСообщения;
...
Процедура ОбновитьВторичныеДанные(Организация = Неопределено, НомерЛН = Неопределено, ИдентификаторЗагружаемогоСообщения = Неопределено, ДанныеЭЛН = Неопределено) Экспорт
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА СообщенияФСС.ДатаСообщения = ДАТАВРЕМЯ(1, 1, 1)
	|					И НЕ ВходящиеСообщенияСЭДОФСС.Дата ЕСТЬ NULL
	|				ТОГДА ВходящиеСообщенияСЭДОФСС.Дата
	|			ИНАЧЕ СообщенияФСС.ДатаСообщения
	...
	|					ИЛИ СообщенияФСС.СостояниеЭЛН = ЗНАЧЕНИЕ(Перечисление.СостоянияЭЛНВФСС.ВозвращенМО)
	...
	|						И ЕСТЬNULL(СведенияОбЭЛН.СостояниеФСС, ЗНАЧЕНИЕ(Перечисление.СостоянияЭЛНВФСС.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Перечисление.СостоянияЭЛНВФСС.ВозвращенМО)
	|			ИНАЧЕ ЛОЖЬ
	...
	|		ПО СообщенияФСС.ИдентификаторСообщения = ВходящиеСообщенияСЭДОФСС.Идентификатор
	|			И СообщенияФСС.Страхователь = ВходящиеСообщенияСЭДОФСС.Организация
	|ГДЕ
	...
	|				ТОГДА ИСТИНА
	|			КОГДА СообщенияФСС.Приоритет = Сообщения2.Приоритет
	|					И СообщенияФСС.ДатаСообщения > Сообщения2.ДатаСообщения
	|				ТОГДА ИСТИНА
	|			КОГДА СообщенияФСС.Приоритет = Сообщения2.Приоритет
	|					И СообщенияФСС.ДатаСообщения = Сообщения2.ДатаСообщения
...
Процедура ИзменитьОтсутствия(Запись, ЕстьИзменения, Отсутствия, ПрочиеОтклонения, ДанныеЭЛН)
	...
	Если Запись.СостояниеЭЛН = Состояния.Аннулирован Или Запись.СостояниеЭЛН = Состояния.ВозвращенМО Тогда
	...
	Если Запись.СостояниеЭЛН = Состояния.Закрыт Тогда
		...
		Если СоздаватьБольничные
			...
			Если СоздаватьОтсутствия Тогда
				СторнироватьОтсутствия(Запись, ЕстьИзменения, Отсутствия, ПрочиеОтклонения, ДанныеЭЛН);
			КонецЕсли;
			Больничный = СоздатьБольничный(Запись.ГоловнаяОрганизация, Запись.ФизическоеЛицо, ДанныеЭЛН, Настройки);
	...
	Иначе
		Если СоздаватьОтсутствия Тогда
			Отбор = Новый Структура("Сотрудник", КадровыеДанные.Сотрудник);
			ПродлитьОтсутствия(Запись, ЕстьИзменения, Отсутствия.Скопировать(Отбор), ПрочиеОтклонения.Скопировать(Отбор),
				ДанныеЭЛН, КадровыеДанные.Сотрудник, Настройки);
			Если ДанныеЭЛН.КадровыеДанныеСотрудника.Свойство("ВсеСотрудники") Тогда
				Для Каждого СтрокаСотрудника Из ДанныеЭЛН.КадровыеДанныеСотрудника.ВсеСотрудники Цикл
					Если СтрокаСотрудника.Сотрудник = КадровыеДанные.Сотрудник
						Или (СтрокаСотрудника.ДатаУвольненияФакт <> '00010101' И СтрокаСотрудника.ДатаУвольненияФакт < Запись.ДатаНачалаСобытия) Тогда
						Продолжить;
					КонецЕсли;
					Отбор.Сотрудник = СтрокаСотрудника.Сотрудник;
					ПродлитьОтсутствия(Запись, ЕстьИзменения, Отсутствия.Скопировать(Отбор), ПрочиеОтклонения.Скопировать(Отбор),
						ДанныеЭЛН, СтрокаСотрудника.Сотрудник, Настройки);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьБольничный(ГоловнаяОрганизация, ФизическоеЛицо, ДанныеЭЛН, Настройки) Экспорт
	...
	Проводить = ПроводитьБольничныеПоЭЛН(Настройки);
	...
	Попытка
		...
		Если Не Отказ Тогда
			...
			БольничныйОбъект.Заполнить(ПараметрыЗаполнения);
			РезультатЗаполнения = ПараметрыЗаполнения.Результат;
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТекстыСообщений, РезультатЗаполнения.ОшибкиЗаполнения);
			
			// Расчет.
			Если Настройки.РассчитыватьБольничныеПоЭЛН Тогда
				...
				ПараметрыРасчета.РасчетУтвержден = УтверждатьБольничныеПоЭЛН(Настройки);
				...
				Если Не РезультатРасчета.Успех И ЗначениеЗаполнено(РезультатРасчета.ПричинаОтказа) Тогда
					ТекстыСообщений.Добавить(РезультатРасчета.ПричинаОтказа);
					Проводить = Ложь;
				КонецЕсли;
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ТекстыСообщений, РезультатРасчета.ОшибкиЗаполнения);
			КонецЕсли;
		КонецЕсли;
		...
		Если Не Отказ И ЗначениеЗаполнено(БольничныйОбъект.Сотрудник) Тогда
			...
			БольничныйОбъект.Записать(РежимЗаписи);
		КонецЕсли;
	Исключение
		Отказ = Истина;
...
Функция УтверждатьБольничныеПоЭЛН(Настройки)
	Возврат Ложь;
КонецФункции

Функция ПроводитьБольничныеПоЭЛН(Настройки)
	Возврат Ложь;
КонецФункции
