...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	ТаблицаЗаписываемогоНабора = ЭтотОбъект.Выгрузить();
	...
	Для Каждого ОтборЗаписейНабора Из ОтборыЗаписейНабора Цикл
		Если Не ОтборЗаписейНабора["Регистратор_Использование"] Тогда
			Продолжить;
		КонецЕсли;
		
		Регистратор = ОтборЗаписейНабора["Регистратор"];
		ТекущийНабор = ТаблицаЗаписываемогоНабора.Скопировать(Новый Структура("Регистратор", Регистратор));
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		
		Запрос.УстановитьПараметр("НовыйНабор", ТекущийНабор);
		...
		Для Каждого Запись Из ЭтотОбъект Цикл
			...
			Если Запись.Регистратор <> Регистратор
				И ЗначениеЗаполнено(Запись.Регистратор) Тогда
