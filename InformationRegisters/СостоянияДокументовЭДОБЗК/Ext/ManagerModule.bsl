...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьСостояния");
	Методы.Вставить("УстановитьСостояниеПриемаОригиналовСогласийНаПрисоединениеКЭДО");
	Методы.Вставить("ОбновитьСостоянияДокументовСПЭП");
КонецПроцедуры
...
Функция ДанныеДляРасчетаСостоянийДокументовЭДОБЗК(ДокументыЭДОБЗК)
	...
	Запрос.Текст =
		...
		|			КОГДА ЕСТЬNULL(Подписи.РезультатСогласования, ЗНАЧЕНИЕ(Перечисление.РезультатыСогласованияБЗК.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.РезультатыСогласованияБЗК.Отклонено)
		|				ТОГДА 0
		|			КОГДА ЕСТЬNULL(Подписи.ИдентификаторВладельцаПодписи, """") <> """"
		|					И ДанныеДокументовЭДОБЗК.ВариантПодписания = ЗНАЧЕНИЕ(Перечисление.ВариантыПодписанияДокументовЭДОБЗК.Требуется)
		|				ТОГДА 1
		|			ИНАЧЕ 0
		...
		|						И ЕСТЬNULL(Подписи.ИдентификаторВладельцаПодписи, НЕОПРЕДЕЛЕНО) = """"
		...
		|		ГДЕ
		|			Подписи.ИдентификаторВладельцаПодписи <> """"
		|		
		|		СГРУППИРОВАТЬ ПО
