...
Процедура УстановитьДоступностьЭлементовФормы(Форма, ТолькоПросмотрЗаполненных)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"НомерЛицевогоСчета",
		"ТолькоПросмотр",
		ЗначениеЗаполнено(Запись.ДокументОснование) Или (Не ЗначениеЗаполнено(Запись.ЗарплатныйПроект)
...
Процедура СменитьФорматФайла()
	...
	Для Каждого СпособЗачисления Из Элементы.СпособЗачисления.СписокВыбора Цикл
		...
		ИначеЕсли СпособЗачисления.Значение = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР Тогда
			ПоддерживаетсяЗачислениеПоНомеруЛицевогоСчетаЦР	= Истина;
		КонецЕсли;
