...
Процедура ЗаполнитьВторичныеДанные(Подразделения = Неопределено) Экспорт
	...
	|	И ДатыНачала.СтруктурнаяЕдиница В(&Подразделения)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если Подразделения = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДатыНачала.СтруктурнаяЕдиница В(&Подразделения)", "(ИСТИНА)");
		Набор.Записать();
	Иначе
	...
	Если Выборка.Количество() = 0 Тогда
		Если Подразделения <> Неопределено Тогда
			Набор.Отбор.Подразделение.Установить(Подразделения);
			Набор.Записать();
		КонецЕсли;
		Возврат;
	КонецЕсли;
