	...
	Если РеквизитыДляПроведения.ДокументРассчитан Тогда 
		...
		Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
			...
			Если РегистрироватьПлановыеНачисления(РеквизитыДляПроведения) Тогда
				...
				РасчетЗарплатыРасширенный.СформироватьДвиженияЗначенийПоказателейСреднегоЗаработкаСотрудников(Движения, ДанныеДляПроведения.ЗначенияПоказателейСреднегоЗаработкаСотрудников);
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Если РеквизитыДляПроведения.ДокументРассчитан Тогда
			...
			Если РегистрироватьПлановыеНачисления(РеквизитыДляПроведения) Тогда
				...
				ДанныеДляПроведения.Вставить("ЗначенияПоказателейСреднегоЗаработкаСотрудников", ЗначенияПоказателейСреднегоЗаработкаСотрудников(РеквизитыДляПроведения));
...
Функция РеквизитыДляПроведения(ДокументСсылка)
	...
	РеквизитыДляПроведения.ЗначенияПоказателейСреднегоДневногоЗаработка = Новый Соответствие;
	РеквизитыДляПроведения.ЗначенияПоказателейСреднегоЧасовогоЗаработка = Новый Соответствие;
	...
	Иначе
		...
		Если ДополнительныеПараметры.ПоЧасам Тогда 
			РеквизитыДляПроведения.ЗначенияПоказателейСреднегоЧасовогоЗаработка = РеквизитыДляПроведения.ЗначенияПоказателейСреднегоЗаработка;
			ДополнительныеПараметры.ПоЧасам = Ложь;
			РеквизитыДляПроведения.ЗначенияПоказателейСреднегоДневногоЗаработка = УчетСреднегоЗаработкаКлиентСервер.ЗначенияПоказателейСреднегоЗаработкаОбщего(РеквизитыДляПроведения.СреднийЗаработокОбщий, РеквизитыДляПроведения.ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры);
		Иначе 
			РеквизитыДляПроведения.ЗначенияПоказателейСреднегоДневногоЗаработка = РеквизитыДляПроведения.ЗначенияПоказателейСреднегоЗаработка;
			ДополнительныеПараметры.ПоЧасам = Истина;
			РеквизитыДляПроведения.ЗначенияПоказателейСреднегоЧасовогоЗаработка = УчетСреднегоЗаработкаКлиентСервер.ЗначенияПоказателейСреднегоЗаработкаОбщего(РеквизитыДляПроведения.СреднийЗаработокОбщий, РеквизитыДляПроведения.ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры);
		КонецЕсли;
	КонецЕсли;
...
Функция РеквизитыДляПроведенияПустаяСтруктура()
	...
	Результат = Новый Структура(
		...
		|ЗначенияПоказателейСреднегоДневногоЗаработка,
		|ЗначенияПоказателейСреднегоЧасовогоЗаработка");
	Возврат Результат;
...
Функция ЗначенияПоказателейСреднегоЗаработкаСотрудников(РеквизитыДляПроведения)
	
	ЗначенияПоказателей = Новый ТаблицаЗначений;
	ЗначенияПоказателей.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
	...
	ЗначенияПоказателей.Колонки.Добавить("ДневноеЗначение", Новый ОписаниеТипов("Число"));
	ЗначенияПоказателей.Колонки.Добавить("ЧасовоеЗначение", Новый ОписаниеТипов("Число"));
	...
	Для Каждого КлючИЗначение Из РеквизитыДляПроведения.ЗначенияПоказателейСреднегоДневногоЗаработка Цикл
		
		ДанныеПоказателя = ЗначенияПоказателей.Добавить();
		ДанныеПоказателя.Период = ДатаСобытия;
		ДанныеПоказателя.Сотрудник = РеквизитыДляПроведения.Сотрудник;
		ДанныеПоказателя.Организация = РеквизитыДляПроведения.Организация;
		ДанныеПоказателя.ФизическоеЛицо = РеквизитыДляПроведения.ФизическоеЛицо;
		ДанныеПоказателя.Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты." + КлючИЗначение.Ключ);
		ДанныеПоказателя.ДокументОснование = РеквизитыДляПроведения.Ссылка;
		Если Не ЗаполнятьЗначенияПоказателейДолейРКСН И ИдентификаторыПоказателиДолейРКСН.Свойство(КлючИЗначение.Ключ) Тогда
			ДанныеПоказателя.ДневноеЗначение = 0;
			ДанныеПоказателя.ЧасовоеЗначение = 0;
		Иначе
			ДанныеПоказателя.ДневноеЗначение = КлючИЗначение.Значение;
			ДанныеПоказателя.ЧасовоеЗначение = РеквизитыДляПроведения.ЗначенияПоказателейСреднегоЧасовогоЗаработка[КлючИЗначение.Ключ];
		КонецЕсли;
		ДанныеПоказателя.ДействуетДо = КонецДня(РеквизитыДляПроведения.ДатаОкончания) + 1;
