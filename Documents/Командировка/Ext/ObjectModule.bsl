...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
		МодульОбучениеРазвитие = ОбщегоНазначения.ОбщийМодуль("ОбучениеРазвитие");
		МодульОбучениеРазвитие.ЗаписатьОтсутствиеНаОснованииОбучения(ЭтотОбъект);
	КонецЕсли;
...
Процедура Рассчитать(ПараметрыРасчета = Неопределено) Экспорт
	...
	Если ПараметрыРасчета.ПерезаполнитьНачисления Тогда
		...
		СреднийЗаработокВЧасовомЗначении = ?(ДополнительныеПараметры.ПоЧасам, Истина, Ложь); 
		...
		ЗначенияПоказателейСреднегоЗаработкаДополнительные = Новый Структура;
		...
		Иначе
			...
			Для Каждого КлючИЗначение Из ЗначенияПоказателейСреднегоЗаработка Цикл
				Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты." + КлючИЗначение.Ключ);
				ЗначениеПоказателя = КлючИЗначение.Значение;
				МенеджерРасчета.ДобавитьИзвестноеЗначениеПоказателя(НоваяСтрока, Показатель, ЗначениеПоказателя, ДополнительныеПараметрыПоказателейСреднегоЗаработка(СреднийЗаработокВЧасовомЗначении));
			КонецЦикла;
			ДополнительныеПараметры.ПоЧасам = Не ДополнительныеПараметры.ПоЧасам;
			ЗначенияПоказателейСреднегоЗаработкаДополнительные = УчетСреднегоЗаработкаКлиентСервер.ЗначенияПоказателейСреднегоЗаработкаОбщего(ЭтотОбъект.СреднийЗаработокОбщий, ЭтотОбъект.ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры);
			Если ЭтотОбъект.ПланируемаяДатаВыплаты < ДатаНачалаРаздельногоРасчетаРКСН Тогда
				РасчетЗарплатыРасширенный.ОчиститьЗначенияПоказателейДолейРКСН(ЗначенияПоказателейСреднегоЗаработкаДополнительные);
			КонецЕсли;
			Для Каждого КлючИЗначение Из ЗначенияПоказателейСреднегоЗаработкаДополнительные Цикл
				Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты." + КлючИЗначение.Ключ);
				ЗначениеПоказателя = КлючИЗначение.Значение;
				МенеджерРасчета.ДобавитьИзвестноеЗначениеПоказателя(НоваяСтрока, Показатель, ЗначениеПоказателя, ДополнительныеПараметрыПоказателейСреднегоЗаработка(Не СреднийЗаработокВЧасовомЗначении));
			КонецЦикла;
		КонецЕсли;
		...
		Если РасчетЗарплатыРасширенный.ЗаполнятьНачислениеДолиРКСН(ЗначенияПоказателейСреднегоЗаработка, ЭтотОбъект.ВидРасчетаДолиРайонногоКоэффициента, ЭтотОбъект.ПланируемаяДатаВыплаты) Тогда
			...
			РасчетЗарплатыРасширенный.ЗаполнитьПоказателиРКСН(СтрокаДоляРайонногоКоэффициента, МенеджерРасчета, ЗначенияПоказателейСреднегоЗаработка, ДополнительныеПараметрыПоказателейСреднегоЗаработка(СреднийЗаработокВЧасовомЗначении));
			РасчетЗарплатыРасширенный.ЗаполнитьПоказателиРКСН(СтрокаДоляРайонногоКоэффициента, МенеджерРасчета, ЗначенияПоказателейСреднегоЗаработкаДополнительные, ДополнительныеПараметрыПоказателейСреднегоЗаработка(Не СреднийЗаработокВЧасовомЗначении));
		КонецЕсли;
		...
		Если РасчетЗарплатыРасширенный.ЗаполнятьНачислениеДолиРКСН(ЗначенияПоказателейСреднегоЗаработка, ЭтотОбъект.ВидРасчетаДолиСевернойНадбавки, ЭтотОбъект.ПланируемаяДатаВыплаты) Тогда
			...
			РасчетЗарплатыРасширенный.ЗаполнитьПоказателиРКСН(СтрокаДоляСевернойНадбавки, МенеджерРасчета, ЗначенияПоказателейСреднегоЗаработка, ДополнительныеПараметрыПоказателейСреднегоЗаработка(СреднийЗаработокВЧасовомЗначении));
			РасчетЗарплатыРасширенный.ЗаполнитьПоказателиРКСН(СтрокаДоляСевернойНадбавки, МенеджерРасчета, ЗначенияПоказателейСреднегоЗаработкаДополнительные, ДополнительныеПараметрыПоказателейСреднегоЗаработка(Не СреднийЗаработокВЧасовомЗначении));
		КонецЕсли;
...
Функция ДополнительныеПараметрыПоказателейСреднегоЗаработка(ЗначениеВЧасах)
	
	Возврат Новый Структура("ЗначениеВЧасах", ЗначениеВЧасах);
