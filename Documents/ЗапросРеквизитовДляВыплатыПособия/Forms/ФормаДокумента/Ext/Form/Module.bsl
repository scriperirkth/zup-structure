...
Функция НедостающиеСведенияОднойСтрокой()
	...
	СтруктураДокумента = Документы.ЗапросРеквизитовДляВыплатыПособия.СтруктураДокументаИзXML(ТекстXML, Результат);
	Если СтруктураДокумента = Неопределено Тогда
		Возврат ""; // Нет данных.
	КонецЕсли;
	НедостающиеСведения = СтруктураДокумента.НедостающиеСведения;
