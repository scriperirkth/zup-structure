...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Обработчик = "УправлениеПечатьюБЗККлиент.ВыполнитьКомандуПечати";
	КомандаПечати.Идентификатор = "ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании";
	КомандаПечати.Представление = НСтр("ru = 'Сопроводительное письмо к выписке из проекта приказа о бронировании'");
	КомандаПечати.Порядок = 50;
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Обработчик = "УправлениеПечатьюБЗККлиент.ВыполнитьКомандуПечати";
	КомандаПечати.Идентификатор = "ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПриказаОБронировании";
	КомандаПечати.Представление = НСтр("ru = 'Сопроводительное письмо к выписке из приказа о бронировании'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт	
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, 
			"ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании", 
			НСтр("ru = 'Сопроводительное письмо к выписке из проекта приказа о бронировании'"), 
			ПечатнаяФормаСопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании(МассивОбъектов, ОбъектыПечати));
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПриказаОБронировании") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, 
			"ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПриказаОБронировании", 
			НСтр("ru = 'Сопроводительное письмо к выписке из приказа о бронировании'"), 
			ПечатнаяФормаСопроводительноеПисьмоКВыпискеИзПриказаОБронировании(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;
...
Функция ПечатнаяФормаСопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании(МассивОбъектов, ОбъектыПечати)
	
	ДокументРезультат = Новый ТабличныйДокумент;
	ДокументРезультат.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании";
	ДокументРезультат.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриказОБронированииРаботников.ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПроектаПриказаОБронировании");
		
		ДатаПисьма = ЗарплатаКадрыОтчеты.ФорматДатыЧислоВКавычкахМесяцПрописью(ВыборкаПоШапкеДокумента.Дата, Истина);
		ИсходящийНомер = "    ";
		ДатаПриказа = Формат(ВыборкаПоШапкеДокумента.Дата, "ДЛФ=ДД");
		НомерПриказа = ЗарплатаКадрыОтчеты.НомерНаПечать(ВыборкаПоШапкеДокумента.Номер, ВыборкаПоШапкеДокумента.НомерПриказа);
		...
		НаименованиеОрганизации = ВыборкаПоШапкеДокумента.НаименованиеОрганизации;
		Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.НаименованиеСокращенное) Тогда
			НаименованиеОрганизации = ВыборкаПоШапкеДокумента.НаименованиеСокращенное;
		КонецЕсли;
		
		ДолжностьОтветственного = Строка(ВыборкаПоШапкеДокумента.ДолжностьОтветственногоПоБронированию);
		ОтветственныйРасшифровкаПодписи = ФизическиеЛицаЗарплатаКадры.РасшифровкаПодписи(Строка(ВыборкаПоШапкеДокумента.ОтветственныйПоБронированию));
		
		ОбластьПисьмо = Макет.ПолучитьОбласть("Письмо");
		
		ОбластьПисьмо.Параметры.ПолноеНаименованиеОрганизации = ВыборкаПоШапкеДокумента.НаименованиеОрганизации;
		ОбластьПисьмо.Параметры.СведенияОбОрганизации = СведенияОбОрганизации;
		ОбластьПисьмо.Параметры.ДатаПисьма = ДатаПисьма;
		ОбластьПисьмо.Параметры.ИсходящийНомер = ИсходящийНомер;
		ОбластьПисьмо.Параметры.НаименованиеОрганизации = НаименованиеОрганизации;
		ОбластьПисьмо.Параметры.ДатаПриказа = ДатаПриказа;
		ОбластьПисьмо.Параметры.НомерПриказа = НомерПриказа;
		ОбластьПисьмо.Параметры.ДолжностьОтветственного = ДолжностьОтветственного;
		ОбластьПисьмо.Параметры.ОтветственныйРасшифровкаПодписи = ОтветственныйРасшифровкаПодписи;
		
		КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(ВыборкаПоШапкеДокумента.Организация, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации, ВыборкаПоШапкеДокумента.Дата, Ложь);
		Если КонтактнаяИнформация.Количество() > 0 Тогда 
			ОбластьПисьмо.Параметры.АдресОрганизации = КонтактнаяИнформация[0].Представление;
		КонецЕсли;
		...
		Если ВыборкаПоРаботникам.НайтиСледующий(Новый Структура("Ссылка", ВыборкаПоШапкеДокумента.Ссылка)) Тогда
			...
			Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("Военкомат") Цикл 
				...
				ОбластьПисьмо.Параметры.НаименованиеВоенкомата = Строка(ВыборкаПоРаботникам.Военкомат);
				ОбластьПисьмо.Параметры.АдресВоенкомата = ВыборкаПоРаботникам.АдресВоенкомата; 
				ДокументРезультат.Вывести(ОбластьПисьмо);
...
Функция ПечатнаяФормаСопроводительноеПисьмоКВыпискеИзПриказаОБронировании(МассивОбъектов, ОбъектыПечати)
	
	ДокументРезультат = Новый ТабличныйДокумент;
	ДокументРезультат.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СопроводительноеПисьмоКВыпискеИзПриказаОБронировании";
	ДокументРезультат.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриказОБронированииРаботников.ПФ_MXL_СопроводительноеПисьмоКВыпискеИзПриказаОБронировании");
