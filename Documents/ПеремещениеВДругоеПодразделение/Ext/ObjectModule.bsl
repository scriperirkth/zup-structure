...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ДанныеДляПроведения.Свойство("ЗанятостьПозицийШтатногоРасписания") Тогда
		КадровыйУчетРасширенный.СформироватьДвиженияЗанятостьПозицийШтатногоРасписания(
			Движения, ДанныеДляПроведения.ЗанятостьПозицийШтатногоРасписания, ЭтотОбъект);
	КонецЕсли;
...
Функция ДанныеДляПроведенияДокумента()Экспорт
	...
	Если ЗарплатаКадры.ВТСодержитСтроки(Запрос.МенеджерВременныхТаблиц, "ВТСотрудникиСИзменениемДолжности") Тогда
		...
		Запрос.Текст =
			...
			|	ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ПериодВозвратногоСобытия, ДАТАВРЕМЯ(1, 1, 1)) КАК ДействуетДо,
			|	ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ПланируемаяДатаЗавершения, ДАТАВРЕМЯ(1, 1, 1)) КАК ПланируемаяДатаЗавершения,
			|	ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ЗамещаемыйСотрудник, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)) КАК ЗамещаемыйСотрудник,
			|	ВЫБОР
			|		КОГДА ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ПериодВозвратногоСобытия, ДАТАВРЕМЯ(1, 1, 1)) > ДАТАВРЕМЯ(1, 1, 1)
			|				ИЛИ ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ПланируемаяДатаЗавершения, ДАТАВРЕМЯ(1, 1, 1)) > ДАТАВРЕМЯ(1, 1, 1)
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			...
			|			И (ЗанятостьПозицийШтатногоРасписания.ВидЗанятостиПозиции В (ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиПозицийШтатногоРасписания.Свободна), ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиПозицийШтатногоРасписания.Занята)))
