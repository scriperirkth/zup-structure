...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДатаВыплатПоОСС = Параметры.ДатаВыплатПоОСС;
...
Процедура ОтменитьИсправленияПоСтажуНаСервере()
	...
	Если ЗначениеЗаполнено(ДатаВыходаНаПенсию) Тогда 
		...
		ДатыВыходаНаПенсию.Вставить(Сотрудник, ДатаВыходаНаПенсию);
	КонецЕсли;
	
	ДатаВыплат = Неопределено;
	Если ЗначениеЗаполнено(ДатаВыплатПоОСС) Тогда 
		ДатаВыплат = ДатаВыплатПоОСС;
	КонецЕсли;
	
	ТаблицаЗаписиОСтаже = ЗаписиОСтаже.Выгрузить();
	ТаблицаЗаписиОСтаже.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
	
	ПерсонифицированныйУчет.ЗаполнитьДанныеОСтажеСотрудников(
		ТаблицаЗаписиОСтаже, Организация, СписокСотрудников, Год, РедактируемыйДокументСсылка, ДатыВыходаНаПенсию, ДатаВыплат);
		
	Для Каждого СтрокаТаблицы Из ТаблицаЗаписиОСтаже Цикл 
		ЗаполнитьЗначенияСвойств(ЗаписиОСтаже.Добавить(), СтрокаТаблицы);
	КонецЦикла;
