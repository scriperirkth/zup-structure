...
Процедура ЗаполнитьДанныеДляНаправленияНаМедицинскийОсмотр(ДанныеЗаполнения) Экспорт
	...
	Если ТипЗнч(Основание) <> Тип("ДокументСсылка.РешениеОКадровомПереводе") Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеРешения = ДанныеОПереводе(Основание);
	ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ДанныеРешения);
	ДанныеЗаполнения.ТипОсмотра = Перечисления.ТипыМедицинскогоОсмотра.Внеплановый;
...
Функция ДанныеОПереводе(ДокументСсылка)
