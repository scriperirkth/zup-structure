...
Процедура ДолжностьПоШтатномуРасписаниюПриИзменении(Элемент)
	...
	ЗаполнитьДанныеПоПозицииНаКлиенте();
	УстановитьВидимостьОплатыТруда();
	ОтобразитьПозициюДоПереводаНаСервере(Истина);
...
Процедура ИзменитьПодразделениеИДолжностьПриИзменении(Элемент)
	...
	УстановитьСвязиПараметровВыбораДляРабочегоМеста();
...
Процедура ОтветственныйПриИзменении(Элемент)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АдаптацияУвольнение") Тогда
		...
		МодульАдаптацияУвольнениеКлиент.ПоказатьВопросПерезаполнитьНазначенныеМерпориятия(ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПоПозицииНаСервере()
	...
	ИзменитьРабочееМестоПриИзмененииНаСервере();
	Если Объект.ИзменитьРабочееМесто Тогда
		ЗаполнитьРабочееМесто(ЭтотОбъект, СписокРабочихМестПоДаннымПозиции(ДанныеПозиции));
	КонецЕсли;
...
&НаСервере
Процедура ОтобразитьПозициюДоПереводаНаСервере(ИзменятьРабочееМесто = Ложь)
	
	ДанныеПозиции = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Объект.Сотрудник, "ДолжностьПоШтатномуРасписанию, Должность");
	...
	Если ЗначениеЗаполнено(ТекущаяПозицияСотрудника) И Объект.ДолжностьПоШтатномуРасписанию <> ТекущаяПозицияСотрудника Тогда
		...
		Если ЗначениеЗаполнено(Объект.ДолжностьПоШтатномуРасписанию) Тогда
			Должность = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДолжностьПоШтатномуРасписанию, "Должность");
			Если ИзменятьРабочееМесто Тогда
				Объект.ИзменитьРабочееМесто = Истина;
			КонецЕсли;
		Иначе
			Должность = ДанныеПозиции[0].Должность;
			Если ИзменятьРабочееМесто Тогда
	...
	Иначе
		...
		Должность = ?(ЗначениеЗаполнено(ТекущаяПозицияСотрудника), ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяПозицияСотрудника, "Должность"), Неопределено);
		Если ИзменятьРабочееМесто Тогда
			Объект.ИзменитьРабочееМесто = Ложь;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьСвязиПараметровВыбораДляРабочегоМеста()
	
	СвязиПараметровВыбора = Новый Массив();
	СвязиПараметровВыбора.Добавить(Новый СвязьПараметраВыбора("ДатаНачала", "Объект.ДатаНачала"));
	СвязиПараметровВыбора.Добавить(Новый СвязьПараметраВыбора("Отбор.Должность", "Должность"));
	...
	Иначе
		...
		СвязиПараметровВыбора.Добавить(Новый СвязьПараметраВыбора("Отбор.Подразделение", "ТекущееПодразделениеСотрудника"));
	КонецЕсли;
	
	Элементы.РабочееМесто.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметровВыбора);
...
Процедура ИзменитьРабочееМестоПриИзмененииНаСервере()
	...
	УстановитьДоступностьИзмененияРабочегоМеста(ЭтотОбъект);
...
Процедура УстановитьВидимостьРабочегоМетста(Форма);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		...
		РабочиеМестаОхраныТрудаИспользуются =
			МодульРабочиеМестаОхраныТруда.РабочиеМестаОхраныТрудаИспользуются(Форма.Объект.Организация);
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ГруппаИзменитьРабочееМесто", "Видимость", РабочиеМестаОхраныТрудаИспользуются);
...
Процедура ЗаполнитьРабочееМесто(Форма, СписокРабочихМест)
	...
	Если ЗначениеЗаполнено(Форма.Объект.РабочееМесто)
		И СписокРабочихМест.НайтиПоЗначению(Форма.Объект.РабочееМесто) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПерезаполнитьНазначенныеМерпориятияЗавершение(Ответ, ДополнительныеПараметры = Неопределено) Экспорт
