...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьОрганизациюПоПустымЗапросам");
КонецПроцедуры
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("40a9760f-2e8d-11f1-8174-4cedfb43b11a");
	Обработчик.Процедура = "Документы.СтатусыВыплатыПособия.ЗаполнитьОрганизациюПоПустымЗапросам";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение организации в статусах выплаты пособий.'");
...
//
Процедура ЗаполнитьОрганизациюПоПустымЗапросам(ПараметрыОбновления = Неопределено) Экспорт
	...
	|ИЗ
	|	Документ.СтатусыВыплатыПособия КАК Шапка
	|ГДЕ
	|	Шапка.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|	И Шапка.ИсходящийДокумент = ЗНАЧЕНИЕ(Документ.ЗапросСтатусовВыплатыПособия.ПустаяСсылка)
	...
	Пока Выборка.Следующий() Цикл
		Если ОбработчикОтложенный Тогда
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
					ПараметрыОбновления,
					"Документ.СтатусыВыплатыПособия",
					"Ссылка",
...
// Продолжение обработчика "Документы.ЗапросСтатусовВыплатыПособия.ЗаполнитьОрганизацию".
Процедура ЗаполнитьОрганизациюПоМассивуЗапросов(ПараметрыОбновления, МассивЗапросов) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Шапка.Ссылка КАК Ссылка,
	|	ЗапросСтатусов.Организация КАК Организация
	|ИЗ
	|	Документ.СтатусыВыплатыПособия КАК Шапка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросСтатусовВыплатыПособия КАК ЗапросСтатусов
	|		ПО Шапка.ИсходящийДокумент = ЗапросСтатусов.Ссылка
	|			И (ЗапросСтатусов.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	|ГДЕ
	|	Шапка.ИсходящийДокумент В(&МассивЗапросов)
	...
	Пока Выборка.Следующий() Цикл
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
				...
				"Ссылка",
				Выборка.Ссылка) Тогда
			ПараметрыОбновления.ОбработкаЗавершена = Ложь; // Объект заблокирован, требуется повторная обработка.
			Продолжить;
		КонецЕсли;
...
// Возвращает значения реквизитов документа ЗапросСтатусовВыплатыПособия.
Функция РеквизитыИсходящегоДокумента(ИсходящийДокумент)
	ИменаПолей = "Ссылка, ФизическоеЛицо, Организация, ИдентификаторРеестра, ИдентификаторСтрокиРеестра";
...
Функция СоздатьПоСообщению85(Запись, ТаблицаСведенийОВыплатах, ТаблицаНеобработанных) Экспорт
	...
	// Определение ссылки на организацию.
	Организация = РеквизитыИсходящегоДокумента.Организация;
	Если Не ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		КадровыеДанные = КадровыйУчет.КадровыеДанныеОсновногоСотрудникаФизическогоЛица(
			Запись.Страхователь,
			ФизическоеЛицо,
			"Организация",
			Запись.РегистрацияДата,
			Ложь);
	...
	КонецЕсли;
	ДокументОбъект.Организация = ?(ЗначениеЗаполнено(Организация), Организация, Запись.Страхователь);
