...
Функция ДанныеДляПроведения()
	...
	Если РазмерОплатыУтвержден Тогда
		...
		ЗначенияПоказателейСреднегоДневногоЗаработка = Новый Соответствие;
		ЗначенияПоказателейСреднегоЧасовогоЗаработка = Новый Соответствие;
		
		Если ДополнительныеПараметры.ПоЧасам Тогда 
			ЗначенияПоказателейСреднегоЧасовогоЗаработка = ЗначенияПоказателейСреднегоЗаработка;
			ДополнительныеПараметры.ПоЧасам = Ложь;
			ЗначенияПоказателейСреднегоДневногоЗаработка = УчетСреднегоЗаработкаКлиентСервер.ЗначенияПоказателейСреднегоЗаработкаОбщего(СреднийЗаработокОбщий, ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры);
		Иначе 
			ЗначенияПоказателейСреднегоДневногоЗаработка = ЗначенияПоказателейСреднегоЗаработка;
			ДополнительныеПараметры.ПоЧасам = Истина;
			ЗначенияПоказателейСреднегоЧасовогоЗаработка = УчетСреднегоЗаработкаКлиентСервер.ЗначенияПоказателейСреднегоЗаработкаОбщего(СреднийЗаработокОбщий, ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры);
		КонецЕсли;
		...
		ЗначенияПоказателейСреднегоЗаработка = Новый ТаблицаЗначений;
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("Показатель", Новый ОписаниеТипов("СправочникСсылка.ПоказателиРасчетаЗарплаты"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("ДокументОснование", Метаданные.ОпределяемыеТипы.ОснованиеНачисленияУдержания.Тип);
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("ДневноеЗначение", Новый ОписаниеТипов("Число"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("ЧасовоеЗначение", Новый ОписаниеТипов("Число"));
		ЗначенияПоказателейСреднегоЗаработка.Колонки.Добавить("ДействуетДо", Новый ОписаниеТипов("Дата"));
		
		Для Каждого КлючИЗначение Из ЗначенияПоказателейСреднегоДневногоЗаработка Цикл
			
			ДанныеПоказателя = ЗначенияПоказателейСреднегоЗаработка.Добавить();
			ДанныеПоказателя.Период = Выборка.ДатаСобытия;
			ДанныеПоказателя.Сотрудник = Выборка.Сотрудник;
			ДанныеПоказателя.Организация = Выборка.Организация;
			ДанныеПоказателя.ФизическоеЛицо = Выборка.ФизическоеЛицо;
			...
			ДанныеПоказателя.ДокументОснование = Выборка.ДокументОснование;
			ДанныеПоказателя.ДневноеЗначение = КлючИЗначение.Значение;
			ДанныеПоказателя.ЧасовоеЗначение = ЗначенияПоказателейСреднегоЧасовогоЗаработка[КлючИЗначение.Ключ];
			ДанныеПоказателя.ДействуетДо = Выборка.ДействуетДо;
			
		КонецЦикла;
		
		ДанныеДляПроведения.Вставить("ЗначенияПоказателейСреднегоЗаработкаСотрудников", ЗначенияПоказателейСреднегоЗаработка);
