...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если Параметры.Отбор.Свойство("Обработан") Тогда
		ВРаботе = Параметры.Отбор.Обработан;
		Параметры.Отбор.Удалить("Обработан");
		Если ВРаботе Тогда
			КлючНазначенияИспользования = "ВРаботе";
			КлючСохраненияПоложенияОкна = КлючНазначенияИспользования;
		КонецЕсли;
	КонецЕсли;
	...
	Если СЭДОФССПовтИсп.ЕстьПравоОтправки() Тогда
		ПравоИзменения = СЭДОФСС.ЕстьПравоОбмена() И ПравоДоступа("Изменение", Метаданные.Документы.ДанныеСФРДляРасчетаПособия);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_ДанныеСФРДляРасчетаПособия"
...
Процедура СписокПриИзменении(Элемент)
	Оповестить("Запись_ДанныеСФРДляРасчетаПособия", Новый Структура, Неопределено);
...
Процедура ОбновитьПараметрыСписка()
	...
	КонецЕсли;
	ЭлементОтбораКД.Использование = ВРаботе;
