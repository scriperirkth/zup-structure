...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
			...
			МодульОбучениеПоОхранеТрудаФормы.ДокументОтсутствияПриСозданииНаСервере(ЭтотОбъект);
		КонецЕсли;
	...
	Иначе
		Если Параметры.Свойство("ФизическоеЛицо") Тогда
			СтрокиФизическогоЛица = Объект.Сотрудники.НайтиСтроки(Новый Структура("ФизическоеЛицо", Параметры.ФизическоеЛицо));
			Если СтрокиФизическогоЛица.Количество() > 0 Тогда
				Элементы.Сотрудники.ТекущаяСтрока = СтрокиФизическогоЛица[0].ПолучитьИдентификатор();
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
		МодульОбучениеПоОхранеТрудаФормы = ОбщегоНазначения.ОбщийМодуль("ОбучениеПоОхранеТрудаФормы");
		МодульОбучениеПоОхранеТрудаФормы.ДокументОтсутствияПриЧтенииНаСервере(ЭтотОбъект);
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
		МодульОбучениеПоОхранеТрудаФормы = ОбщегоНазначения.ОбщийМодуль("ОбучениеПоОхранеТрудаФормы");
		МодульОбучениеПоОхранеТрудаФормы.ДокументОтсутствияПередЗаписьюНаСервере(ЭтотОбъект, ТекущийОбъект);
	КонецЕсли;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Оповестить("Закрытие_ОплатаПоСреднемуЗаработкуСписком");
