...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
		...
		МодульОбучениеПоОхранеТруда.ПриЗаписиДокументаОтсутствия(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		Если ДанныеЗаполнения.Свойство("Действие") И ДанныеЗаполнения.Действие = "ЗаполнитьИзОбучения" Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
				МодульОбучениеРазвитие = ОбщегоНазначения.ОбщийМодуль("ОбучениеРазвитие");
				МодульОбучениеРазвитие.ЗаполнитьОплатуПоСреднемуСотрудниковИзДокументаОбучения(ЭтотОбъект, ДанныеЗаполнения);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьСвязаннуюОплатуПоСреднему(Строка, Отказ)
	...
	Попытка
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обновить отсутствие с сохранением оплаты %1, связанный документ рассчитан.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка.ОплатаПоСреднемуЗаработку, НомерТекущейСтроки + 1);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Объект.Сотрудники[" + НомерТекущейСтроки + "].ОплатаПоСреднемуЗаработку" , , Отказ);
		КонецЕсли;
