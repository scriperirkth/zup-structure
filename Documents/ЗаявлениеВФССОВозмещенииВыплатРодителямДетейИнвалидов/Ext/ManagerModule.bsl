...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПовторноОбработатьСообщения12210");
	Методы.Вставить("АдаптироватьКФорме2022");
	Методы.Вставить("ПовторноОбработатьСообщения12110");
	Методы.Вставить("ЗаполнитьКраткийСостав");
	Методы.Вставить("ПовторноОбработатьСообщенияПослеПереходаНаЕЦП");
	Методы.Вставить("ЗаполнитьТипСообщения");
КонецПроцедуры
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c4341d02-3295-11f1-8174-4cedfb43b11a");
	Обработчик.Процедура = "Документы.ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов.ЗаполнитьТипСообщения";
	Обработчик.Комментарий = НСтр("ru = 'СЭДО ЕЦП: Заполнение типа сообщений в документах ""Заявление в ФСС о возмещении выплат родителям детей-инвалидов"".'");
...
// Заполнение типа сообщений.
//
// Параметры:
//   ПараметрыОбновления - Структура - Параметры отложенного обновления.
//
Процедура ЗаполнитьТипСообщения(ПараметрыОбновления = Неопределено) Экспорт
	ОбработкаЗавершена = Истина;
	ОбработчикОтложенный = (ПараметрыОбновления <> Неопределено);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументОбъект.ТипСообщения = 0 Тогда
			ДокументОбъект.ТипСообщения = 120;
		КонецЕсли;
