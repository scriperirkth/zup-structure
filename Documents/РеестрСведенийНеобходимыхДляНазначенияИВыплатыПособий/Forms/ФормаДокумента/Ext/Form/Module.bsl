...
Процедура ЗаполнитьДокументыКОтправкеПодтвержденияОПолучении()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИзвещениеФСС КАК Извещение
	|		ПО ВТДляПоискаИзвещений.ВходящийНомер = Извещение.ВходящийНомер
	|			И ВТДляПоискаИзвещений.ВходящаяДата = Извещение.ВходящаяДата
	|ГДЕ
	|	НЕ Извещение.ПодтверждениеПолученоФСС
	...
	|ИЗ
	|	ВТДляПоискаИзвещений КАК ВТДляПоискаИзвещений
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросНедостающихСведенийПособияСФР КАК Извещение
	|		ПО ВТДляПоискаИзвещений.ВходящийНомер = Извещение.ВходящийНомер
	|			И ВТДляПоискаИзвещений.ВходящаяДата = Извещение.ВходящаяДата
	|ГДЕ
	|	НЕ Извещение.ПодтверждениеДоставлено
	|	И Извещение.ДатаОтправкиПодтверждения = &ПустаяДата";
