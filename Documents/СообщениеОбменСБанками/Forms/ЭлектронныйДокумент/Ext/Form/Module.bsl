...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоказатьОстаток = Параметры.ПоказатьОстаток;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПоказатьОстаток Тогда
		ОбластьИсходящийОстаток = ТабличныйДокументФормы.Области.Найти("ИсходящийОстаток");
		Элементы.ТабличныйДокумент.УстановитьВыделенныеОбласти(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
				ОбластьИсходящийОстаток));
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовПоРолям()
	...
	Если Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI Тогда
		Элементы.КомандаПроверитьПодписи.Видимость = Ложь;
		Элементы.ПроверитьПодписи.Видимость = Ложь;
	КонецЕсли;
	...
	ПодготовленКОтправке = НЕ ИзменилсяПротоколОбмена()
					И (Объект.Статус = Перечисления.СтатусыОбменСБанками.ПодготовленКОтправке
						...
						ИЛИ (Объект.Статус = Перечисления.СтатусыОбменСБанками.Подписан
							И Объект.ПрограммаБанка <> Перечисления.ПрограммыБанка.SberAPI
							И НЕ ДокументПодписывается)
						ИЛИ (Объект.Статус = Перечисления.СтатусыОбменСБанками.Подписан
							И Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI
							И ДокументПодписывается));
...
Функция МожноЗапроситьСостояние()
	...
	Возврат (Объект.ВидЭД = Перечисления.ВидыЭДОбменСБанками.ЗапросНаОтзывЭД
			...
			И (Объект.Статус = Перечисления.СтатусыОбменСБанками.Принят
				...
				ИЛИ Объект.Статус = Перечисления.СтатусыОбменСБанками.ОтклоненБанком)
...
Функция ДоступныДляПодписиСертификаты(МассивОтпечатков)
	...
	Если Объект.Статус = Перечисления.СтатусыОбменСБанками.Сформирован Тогда
		...
		|	ВЫРАЗИТЬ(СостояниеПодписания.Подписант КАК Справочник.Пользователи) КАК Подписант,
	...
	ЗапросПоСертификатам.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ОбменСБанкамиСлужебный.СоздатьВТСертификатыДоступныеПользователю(ЗапросПоСертификатам.МенеджерВременныхТаблиц);
...
Процедура ЗапроситьСостояниеБезПодписи()
	...
	ИначеЕсли ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI") Тогда
		...
		Если ИспользуетсяКриптография(Объект.НастройкаОбмена) Тогда
			...
			ОповещениеПослеАутентификации = Новый ОписаниеОповещения(
				"ПолучитьСтатусДокументаПослеАутентификацииSberAPI", ЭтотОбъект, ПараметрыВыполнения);
			...
			ПараметрыАутентификацииНаТокене.Вставить("НастройкаОбмена", Объект.НастройкаОбмена);
			ПараметрыАутентификацииНаТокене.Вставить("Организация", Объект.Организация);
			ПараметрыАутентификацииНаТокене.Вставить("Банк", Объект.Банк);
		...
		Иначе
			...
			ПараметрыВыполнения = Новый Структура("НастройкаОбмена, НомерСчета, Ссылка, Идентификатор");
			ЗаполнитьЗначенияСвойств(ПараметрыВыполнения, Объект);
			РезультатВыполнения = ЗапускЗаданияПолученияСтатусаДокументаSberAPI(
				ПараметрыВыполнения, УникальныйИдентификатор);
			...
			Если РезультатВыполнения.Свойство("ТребуетсяАутентификация") Тогда
				...
				ЗаполнитьЗначенияСвойств(
					...
					Объект,
					"НастройкаОбмена,Организация,Банк,НомерСчета");
			...
			Иначе
				...
				ПараметрыОжидания.ВыводитьСообщения = Истина;
				Оповещение = Новый ОписаниеОповещения(
					"ПослеПолученияСтатусаДокументаSberAPI", ЭтотОбъект, ПараметрыВыполнения);
				ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатВыполнения, Оповещение, ПараметрыОжидания);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияПолученияСтатусаДокументаSberAPI(Знач ПараметрыПолучения, Знач УникальныйИдентификатор)
	
	ПараметрыСессии = ОбменСБанкамиСлужебный.ПараметрыУстановленнойСессииСбербанк(ПараметрыПолучения.НастройкаОбмена);
	Если ПараметрыСессии = Неопределено Или Не ПараметрыСессии.Свойство("ДоступныеСчета")
		Или Не ЗначениеЗаполнено(ПараметрыСессии.ДоступныеСчета)
		Или ПараметрыСессии.ДоступныеСчета.Получить(ПараметрыПолучения.НомерСчета) = Неопределено Тогда
		Возврат Новый Структура("ТребуетсяАутентификация", Истина);
	КонецЕсли;
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор());
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение статуса документа через сервис SberAPI'");
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("НастройкаОбмена",        ПараметрыПолучения.НастройкаОбмена);
	ПараметрыПроцедуры.Вставить("НомерСчета",             ПараметрыПолучения.НомерСчета);
	ПараметрыПроцедуры.Вставить("ИдентификаторСессии",    ПараметрыСессии.ИдентификаторСессии);
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"ОбменСБанкамиSberAPIСлужебный.ПолучитьИУстановитьСтатусДокумента", ПараметрыПроцедуры, ПараметрыВыполнения);
	
КонецФункции

&НаКлиенте
Процедура ПолучитьСтатусДокументаПослеАутентификацииSberAPI(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ОбменСБанкамиSberAPIСлужебныйКлиентСервер.ВыполненоУспешно(Результат) Тогда
		...
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжидания.ВыводитьСообщения = Истина;
		Оповещение = Новый ОписаниеОповещения("ПослеПолученияСтатусаДокументаSberAPI", ЭтотОбъект);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатВыполнения, Оповещение, ПараметрыОжидания);

	ИначеЕсли ТипЗнч(Результат) = Тип("Структура")
		И Не Результат.Успех
		И Результат.Свойство("ТребуетсяТокен", ТребуетсяТокен)
		И ТребуетсяТокен = Истина Тогда
			
		ОповещениеПослеАутентификации = Новый ОписаниеОповещения(
				"ПолучитьСтатусДокументаПослеАутентификацииSberAPI", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ПослеПолученияСтатусаДокументаSberAPI(РезультатЗадания, ДополнительныеПараметры) Экспорт
	
	Если РезультатЗадания = Неопределено Тогда // задание было отменено
		ОбменСБанкамиSberAPIСлужебныйКлиент.ОтозватьТокенДоступа(Объект.НастройкаОбмена);
		Возврат;
	КонецЕсли;
	
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		
		ВидОперации = НСтр("ru = 'Отправка запроса статуса платежа через сервис SberAPI'");
...
Процедура СформироватьЗапросСостоянияЭД(
	...
	Если ТребуетсяПодпись Тогда
		...
		ТаблицаСертификатов = ОбменСБанкамиСлужебный.ТаблицаДоступныхСертификатовИзНастройкиОбменаПоВидуЭД(
			НастройкаОбмена, Перечисления.ВидыЭДОбменСБанками.ЗапросОСостоянииЭД);
