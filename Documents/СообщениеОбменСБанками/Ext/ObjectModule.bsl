	...
	Если ДополнительныеСвойства.Свойство("ОбновитьСостояниеОбъектовИБ") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				...
				Если Статус = Перечисления.СтатусыОбменСБанками.ОтклоненБанком Тогда
					Запись.ТребуетсяУведомлениеПользователю = Истина;
				КонецЕсли;
...
Функция СостояниеСообщенияОбмена()
	...
	ИначеЕсли ТекущийСтатус = Перечисления.СтатусыОбменСБанками.НеПодтвержден Тогда
		Если ПрограммаБанка = Перечисления.ПрограммыБанка.FintechIntegration
			ИЛИ ПрограммаБанка = Перечисления.ПрограммыБанка.SberAPI Тогда
