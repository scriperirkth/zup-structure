...
Функция ПечатнаяФормаВыпискиБанка(СообщениеОбмена, ИдентификаторПечати, ДляПечати, ПрограммаБанка)
	...
	Иначе
		...
		РеквизитыБанкаНастройки = Неопределено;
		...
		Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
			...
			Если ЗначениеЗаполнено(Банк) Тогда
				РеквизитыБанкаНастройки = ОбменСБанкамиСлужебный.РеквизитыБанкаПоСсылке(Банк);
			КонецЕсли;
		КонецЕсли;
		ЗаполнитьТабличныйДокументВыпискиБанка(ДеревоРазбора, ТабличныйДокумент, ДляПечати, РеквизитыБанкаНастройки);
	КонецЕсли;
...
Функция ПолучитьДанныеВыпискиДляПечати(ДеревоДанных, РеквизитыБанкаНастройки = Неопределено)
	...
	Для Каждого Выписка Из СтрокаТаблицыВыписок.Строки Цикл
		...
		Если Банк = Истина Тогда
			...
			БанкВладелецСчета = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.БанкВладелецСчета");
			Если БанкВладелецСчета = Истина Тогда
				НаименованиеБанкаВладельцаМеждународное = ЗначениеРеквизитаВДереве(Выписка,
					"Выписки.НомерСтроки.БанкВладелецСчета.НаименованиеМеждународное");
				НаименованиеБанкаВладельцаЛокальное = ЗначениеРеквизитаВДереве(Выписка,
					"Выписки.НомерСтроки.БанкВладелецСчета.Наименование");
				НаименованиеБанкаВладельца = ?(ЗначениеЗаполнено(НаименованиеБанкаВладельцаЛокальное),
					НаименованиеБанкаВладельцаЛокальное, НаименованиеБанкаВладельцаМеждународное);
				
				Если НаименованиеБанкаВладельца <> "" И НаименованиеБанкаВладельца <> НаименованиеБанка Тогда
					НаименованиеБанка = СтрШаблон(НСтр("ru = '%1 через %2'"), НаименованиеБанкаВладельца, НаименованиеБанка);
				КонецЕсли;
				
				БИКБанкаВладельца = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.БанкВладелецСчета.БИК");
				Если БИКБанкаВладельца <> "" И БИКБанкаВладельца <> БИКБанка Тогда
					БИКБанка = СтрШаблон(НСтр("ru = '%1 через %2'"), БИКБанкаВладельца, БИКБанка);
				КонецЕсли;
			ИначеЕсли ЗначениеЗаполнено(РеквизитыБанкаНастройки) И РеквизитыБанкаНастройки.БИК <> БИКБанка Тогда
				НаименованиеБанка = СтрШаблон(НСтр("ru = '%1 через %2'"), НаименованиеБанка, РеквизитыБанкаНастройки.Наименование);
				БИКБанка = СтрШаблон(НСтр("ru = '%1 через %2'"), БИКБанка, РеквизитыБанкаНастройки.БИК);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		СтруктураШтампа = Новый Структура();
		БИКБанка = ЗначениеРеквизитаВДереве(Выписка, "Выписки.НомерСтроки.Штамп.БИК");
		Если ЗначениеЗаполнено(РеквизитыБанкаНастройки) И РеквизитыБанкаНастройки.БИК <> БИКБанка Тогда
			СтруктураШтампа.Вставить("БИК", РеквизитыБанкаНастройки.БИК);
			СтруктураШтампа.Вставить("НаименованиеБанка", РеквизитыБанкаНастройки.Наименование);
		Иначе
...
Процедура ЗаполнитьТабличныйДокументВыпискиБанка(
	ДеревоРазбора,
	ТабличныйДокумент,
	ДляПечати,
	РеквизитыБанкаНастройки = Неопределено)
	
	ДанныеПечати = ПолучитьДанныеВыпискиДляПечати(ДеревоРазбора, РеквизитыБанкаНастройки);
