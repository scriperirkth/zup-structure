...
&НаКлиенте
Процедура СтатьяФинансированияНДФЛПриИзменении(Элемент)
	ОбновитьБухучетНДФЛ();
КонецПроцедуры

&НаКлиенте
Процедура СтатьяРасходовНДФЛПриИзменении(Элемент)
	ОбновитьБухучетНДФЛ();
КонецПроцедуры
...
&НаКлиенте
Процедура ОбновитьБухучетНДФЛ()

	Если ИспользоватьСтатьиФинансирования И Объект.НДФЛ.Количество() > 0 Тогда 
		ОбновитьБухучетНДФЛНаСервере();
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ОбновитьБухучетНДФЛНаСервере()
	...
	ИсходныеДанные = ОтражениеЗарплатыВБухучетеКлиентСерверРасширенный.ОписаниеИсходныхДанныхДляОбновленияЗависимыхТаблиц();
	ИсходныеДанные.ИменаТаблицДляОбновления = "НДФЛ";
	МенеджерРасчета.ЗаполнитьНастройкиОбновленияБухучета(ИсходныеДанные);
	МенеджерРасчета.ОбновитьБухучетРезультатовРасчета();
	
	КодыСтатей = ОтражениеЗарплатыВБухучетеРасширенный.КодыСтатейФинансирования();
	Для Каждого КлючИЗначение Из МенеджерРасчета.Зарплата Цикл
		ТаблицаДанных = КлючИЗначение.Значение;
		Если ТипЗнч(ТаблицаДанных ) <> Тип("ТаблицаЗначений") Тогда
			Продолжить;
		КонецЕсли;
		Если ТаблицаДанных.Колонки.Найти("РаспределениеПоСтатьям") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
			Для каждого СтрокаРаспределения Из СтрокаТЗ.РаспределениеПоСтатьям Цикл
				СтрокаРаспределения.КодСтатьиФинансирования = КодыСтатей[СтрокаРаспределения.СтатьяФинансирования];
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Процедура УстановитьВидимостьСпособаРасчетов()
	...
	Если Не РаботаВБюджетномУчреждении Тогда
		...
		Если ПравилоУчета <> Неопределено И ПравилоУчета.Свойство("ДоступноИзменениеСпособаРасчетов") тогда
			ПравилоУчета.ДоступноИзменениеСпособаРасчетов = Истина;		
		КонецЕсли;
