...
Процедура СформироватьДвижения() Экспорт
	...
	Запрос.Текст = 
		...
		|	ВариантыОтветовНаТестовыеВопросы.Ссылка КАК СсылкаНаВариант,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВариантыОтветовНаТестовыеВопросы КАК ВариантыОтветовНаТестовыеВопросы
		|		ПО ТестовыеВопросы.Ссылка = ВариантыОтветовНаТестовыеВопросы.Владелец
		|		И ТестовыеВопросы.ТипВопроса = ЗНАЧЕНИЕ(Перечисление.ТипыТестовыхВопросов.ВводСКлавиатуры)
		|		И НЕ ВариантыОтветовНаТестовыеВопросы.ПометкаУдаления
		|ГДЕ
	...
	ВыборкаВопросов = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам); // Выбираем вопросы, которые есть в базе
	
	ДанныеКурса = Новый Структура("Учащийся, Контекст, ЭлектронныйКурс", Учащийся, Контекст, ЭлектронныйКурс);
		...
		Для каждого СтрокаТО Из ОтветыНаВопрос Цикл
			...
			Если ВыборкаВопросов.ТипВопроса = Перечисления.ТипыТестовыхВопросов.ОдинИзМногих
				...
				Для каждого ВариантОтвета Из СтрокаТО.ОтветНаВопрос.variants Цикл
					...
					ВариантОтветаУИД = Новый УникальныйИдентификатор(ВариантОтвета.variant);
					ВариантОтветаСсылка = Справочники.ВариантыОтветовНаТестовыеВопросы.ПолучитьСсылку(ВариантОтветаУИД); // Наличие в базе варианта ответа не проверяем
					ВариантОтвета.Вставить("Ссылка", ВариантОтветаСсылка);
					СформироватьДвижениеВыбранныеВариантыОтветов(Движения, СтрокаТО, ВыборкаВопросов.Ссылка, 
						ВариантОтвета, НомерВарианта, ДанныеКурса);
					
					НомерВарианта = НомерВарианта + 1;
					
				КонецЦикла;
				
			ИначеЕсли ВыборкаВопросов.ТипВопроса = Перечисления.ТипыТестовыхВопросов.ВводСКлавиатуры Тогда
				ВыборкаВариантовОтвета = ВыборкаВопросов.Выбрать();
				ВведенныеДанные = ДанныеПоВопросуВведенныеСКлавиатуры(СтрокаТО.ОтветНаВопрос.inputs);
				
				Пока ВыборкаВариантовОтвета.Следующий() Цикл
					
					ВведеныйОтвет = ВведенныеДанные[ВыборкаВариантовОтвета.НомерПоляВвода];
					ОтветВерный = ВведеныйОтвет <> Неопределено 
						И НРег(ВведеныйОтвет) = НРег(ВыборкаВариантовОтвета.ТекстВарианта.Получить());
					ВариантОтвета = Новый Структура();
					ВариантОтвета.Вставить("selected", ОтветВерный);
					ВариантОтвета.Вставить("trueSelected", ОтветВерный);
					ВариантОтвета.Вставить("trueNotSelected", Не ОтветВерный);
					ВариантОтвета.Вставить("falseSelected", Ложь);
					ВариантОтвета.Вставить("falseNotSelected", Ложь);
					ВариантОтвета.Вставить("Ссылка", ВыборкаВариантовОтвета.СсылкаНаВариант);

					СформироватьДвижениеВыбранныеВариантыОтветов(Движения, СтрокаТО, ВыборкаВопросов.Ссылка, 
						ВариантОтвета, ВыборкаВариантовОтвета.НомерПоляВвода, ДанныеКурса);
				КонецЦикла;
			КонецЕсли;
...
Функция ДанныеПоВопросуВведенныеСКлавиатуры(ВведенныеДанные)
	
	ДанныеПоВопросу = Новый Соответствие;
	
	Для Каждого Ответ Из ВведенныеДанные Цикл
		
		НомерВарианта = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Ответ.input);
		Если НомерВарианта <> Неопределено Тогда
			ДанныеПоВопросу[НомерВарианта] = Ответ.text;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ДанныеПоВопросу;
	
КонецФункции

Процедура СформироватьДвижениеВыбранныеВариантыОтветов(Движения, СтрокаОтвета, Вопрос, ВариантОтвета, Номер, ДанныеКурса)
	
	Движение = Движения.ВыбранныеВариантыОтветовЭлектронныхВопросов.Добавить();
	Движение.Период = СтрокаОтвета.ОтветНаВопрос.start;
	Движение.Вопрос = Вопрос;
	Движение.ВариантОтвета = ВариантОтвета.Ссылка;
	...
	Движение.Учащийся = ДанныеКурса.Учащийся;
	ЭлементСодержаниеУИД = Новый УникальныйИдентификатор(СтрокаОтвета.ПопыткаТестирования.slide);
	Движение.ЭлементСодержания = Справочники.СодержаниеЭлектронныхКурсов.ПолучитьСсылку(ЭлементСодержаниеУИД);
	Движение.Контекст = ДанныеКурса.Контекст;
	Движение.ЭлектронныйКурс = ДанныеКурса.ЭлектронныйКурс;
	ЭлектронныйТестУИД = Новый УникальныйИдентификатор(СтрокаОтвета.ПопыткаТестирования.quiz);
	Движение.ЭлектронныйТест = Справочники.ЭлектронныеТесты.ПолучитьСсылку(ЭлектронныйТестУИД); // Наличие в базе шаблона теста не проверяем
	Движение.ИдентификаторПопыткиТестирования = Новый УникальныйИдентификатор(СтрокаОтвета.ПопыткаТестирования.uuid);
