...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РасчетЗарплатыРасширенныйФормы.ДокументыПриСозданииНаСервере(ЭтаФорма, ОписаниеДокумента(РегистрацияНачисленийДоступна));
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
			Если ЗначениеЗаполнено(Объект.ИсправленныйДокумент) Тогда
				Параметры.Свойство("РасчетДенежногоСодержанияДоступен", РасчетДенежногоСодержанияДоступен);
			Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
			...
			МодульОбучениеРазвитие.ДокументОтсутствияПриСозданииНаСервере(ЭтотОбъект);
		КонецЕсли;
		...
		Если РасчетЗарплатыРасширенныйКлиентСервер.ФормаДокументаГотоваДляРасчетаЗарплаты(ЭтаФорма, ОписаниеДокумента(РегистрацияНачисленийДоступна), Ложь) Тогда
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Оповестить("Закрытие_ОплатаПоСреднемуЗаработку");
...
Процедура Подключаемый_ПланируемаяДатаВыплатыПриИзменении(Элемент)
	...
	Если ЭтоМежрасчетнаяВыплата(Объект.ПорядокВыплаты) Тогда
		Если РасчетЗарплатыРасширенныйКлиентСервер.ФормаДокументаГотоваДляРасчетаЗарплаты(ЭтотОбъект, ОписаниеДокумента(РегистрацияНачисленийДоступна), Ложь) Тогда
			РассчитатьНачисленияНаСервере();
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_Исправить(Команда)
	ИсправлениеДокументовЗарплатаКадрыКлиент.Исправить(ЭтотОбъект,, Новый Структура("РасчетДенежногоСодержанияДоступен", РасчетДенежногоСодержанияДоступен));
КонецПроцедуры
...
Процедура РассчитатьНачисления(Команда)
	...
	Если РасчетЗарплатыРасширенныйКлиентСервер.ФормаДокументаГотоваДляРасчетаЗарплаты(ЭтаФорма, ОписаниеДокумента(РегистрацияНачисленийДоступна)) Тогда
...
Процедура ПодробнееОРасчетеУдержаний(Команда)
	...
	РасчетЗарплатыРасширенныйКлиент.ОткрытьФормуПодробнееОРасчетеУдержаний(Объект, "ОплатаПоСреднемуЗаработку", ЭтаФорма, ОписаниеДокумента(РегистрацияНачисленийДоступна));
...
Функция ПодготовитьСтруктуруОбъектаДляРасчетаДенежногоСодержания() 
	...
	ОписаниеДокумента = ОписаниеДокумента(РегистрацияНачисленийДоступна);
...
Процедура ПерезаполнитьНачисленияСотрудника(Сотрудник = Неопределено, СохранятьИсправления = Истина, ВыводитьСообщения = Ложь) Экспорт
	...
	ПараметрыРасчета.ИспользуетсяСреднечасовойЗаработок = ИспользуетсяСреднечасовойЗаработок;
	...
	ПараметрыРасчета.ИспользоватьПроцентОплатыПоСреднему = ИспользоватьПроцентОплатыПоСреднему;
...
Процедура ДанныеФормыВДанныеМенеджераРасчета(МенеджерРасчета)
	...
	МенеджерРасчета.НастройкиБухучета.КоэффициентыСреднегоЗаработкаДокумента = УчетСреднегоЗаработка.КоэффициентыРаспределенияСреднегоЗаработкаДокумента(Объект, ОписаниеДокумента(РегистрацияНачисленийДоступна));
...
Процедура ЗаполнитьНастройкиМенеджераРасчета(МенеджерРасчета, СохранятьИсправления = Истина)
	...
	Документ.ЗаполнитьНастройкиМенеджераРасчета(МенеджерРасчета, ПараметрыРасчета, СохранятьИсправления);
...
Процедура ДанныеВРеквизиты()
	...
	Если ИспользуетсяРасчетЗарплаты Тогда
		...
		ОтражениеЗарплатыВБухучетеРасширенный.ОбъектПриЧтенииНаСервереПредставлениеРаспределения(ЭтаФорма, ОписаниеДокумента(РегистрацияНачисленийДоступна));
...
Функция ОписаниеТаблицыСохраняемоеДенежноеСодержание()
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписаниеТаблицыСохраняемоеДенежноеСодержание(); 
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТаблицыНачислений(ПравоНаЧтениеДокументаБезОграничений)
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписаниеТаблицыНачислений(ПравоНаЧтениеДокументаБезОграничений); 
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТаблицыПерерасчетов(ПравоНаЧтениеДокументаБезОграничений)
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписаниеТаблицыПерерасчетов(ПравоНаЧтениеДокументаБезОграничений); 
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТаблицыПерерасчетовНулевыеСторно()
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписаниеТаблицыПерерасчетовНулевыеСторно(); 
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТаблицыУдержаний()
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписаниеТаблицыУдержаний();  
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТаблицыНДФЛ()
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписаниеТаблицыНДФЛ(); 
КонецФункции

&НаСервереБезКонтекста
Функция ОписанияТаблицДляРаспределенияРезультата(ПравоНаЧтениеДокументаБезОграничений)
	Возврат Документы.ОплатаПоСреднемуЗаработку.ОписанияТаблицДляРаспределенияРезультата(ПравоНаЧтениеДокументаБезОграничений);
КонецФункции
...
&НаСервереБезКонтекста
Функция ОписаниеДокумента(РегистрацияНачисленийДоступна)
...
Процедура ДополнитьСтрокуНаСервере(ИдентификаторСтроки, ОписаниеТаблицы, ЗаполнятьСведенияСотрудников, ЗаполнятьЗначенияПоказателей)
	РасчетЗарплатыРасширенныйФормы.ДополнитьСтрокуРасчета(ЭтаФорма, ОписаниеДокумента(РегистрацияНачисленийДоступна), ИдентификаторСтроки, ОписаниеТаблицы, ЗаполнятьСведенияСотрудников, ЗаполнятьЗначенияПоказателей);
КонецПроцедуры
...
Процедура ЗаполнитьИнформациюЗаполненностиДанныхРасчетаДенежногоСодержания(Форма)
	...
	Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
		...
		Модуль.ЗаполнитьИнформациюЗаполненностиДанныхСохраняемоеДенежноеСодержаниеДокумента(Форма, ОписаниеДокумента(Форма.РегистрацияНачисленийДоступна), СтруктураИнфонадписи);	
	КонецЕсли;
...
Процедура ОчиститьДанныеСреднегоЗаработка()
	...
	Документ.ОчиститьДанныеСреднегоЗаработка();
	ЗначениеВРеквизитФормы(Документ, "Объект");
	ДанныеВРеквизиты();
...
Функция РасчетДенежногоСодержанияДоступен()
	...
	Документ = РеквизитФормыВЗначение("Объект");
	Возврат Документ.РасчетДенежногоСодержанияДоступен();
...
Процедура ПриНажатииПолеРаспределениеПоТерриториямУсловиямТруда(ОписаниеТаблицы, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыДляВыбора.ОписаниеДокумента = ОписаниеДокумента(РегистрацияНачисленийДоступна);
