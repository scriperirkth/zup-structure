...
Процедура НовыйДокумент(Команда)
	...
	Если Объект.ТипЗаявки = ПредопределенноеЗначение("Перечисление.ТипыЗаявокКабинетСотрудника.ОтпускПоУходуЗаРебенком") Тогда
		ПараметрыЗаполнения = ЗаявкиСотрудниковФормыКлиентРасширенный.ПараметрыЗаполненияОтпускПоУходу(ЭтаФорма);
		ОткрытьФорму("Документ.ОтпускПоУходуЗаРебенком.ФормаОбъекта", ПараметрыЗаполнения, ЭтотОбъект);
	Иначе
		ПараметрыЗаполнения = ПараметрыЗаполненияВозвратИзОтпускПоУходу();
		ОткрытьФорму("Документ.ВозвратИзОтпускаПоУходуЗаРебенком.ФормаОбъекта", ПараметрыЗаполнения, ЭтотОбъект);
	КонецЕсли;
...
Процедура ВыбратьДокумент(Команда)
	...
	ЗначенияДляВыбораДокумента = ЗначенияДляВыбораДокумента(Объект.Организация, Объект.ФизическоеЛицо, Объект.ТипЗаявки);
...
Процедура ИнициализацияФормы()
	...
	ТекстЗаявления = "";
	ПоказыватьДатуВыходаНаРаботу = Ложь;
	ПоказыватьПериодОтпуска = Ложь;
	Если Объект.ТипЗаявки = Перечисления.ТипыЗаявокКабинетСотрудника.ОтпускПоУходуЗаРебенком Тогда
		ТекстЗаявления = НСтр("ru = 'Заявление на отпуск по уходу за ребенком'");
		ПоказыватьПериодОтпуска = Истина;
	Иначе
		ТекстЗаявления = НСтр("ru = 'Заявление на возврат из отпуска по уходу за ребенком'");
		ПоказыватьДатуВыходаНаРаботу = Истина;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДатаВыходаНаРаботу", "Видимость", ПоказыватьДатуВыходаНаРаботу);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаПериодОтпуска", "Видимость", ПоказыватьПериодОтпуска);
...
Функция ЗначенияДляВыбораДокумента(Организация, ФизическоеЛицо, ТипЗаявки)
	
	Отбор = Новый Структура;
	Отбор.Вставить("Организация", 	Организация);
	Отбор.Вставить("Сотрудник", 	ФизическоеЛицо);
	
	ЗначенияДляВыбора = Новый СписокЗначений;
	Если ТипЗаявки = Перечисления.ТипыЗаявокКабинетСотрудника.ОтпускПоУходуЗаРебенком Тогда
	...
	Иначе
		ЗначенияДляВыбора.Добавить(Метаданные.Документы.ВозвратИзОтпускаПоУходуЗаРебенком.ПолноеИмя(), Метаданные.Документы.ВозвратИзОтпускаПоУходуЗаРебенком.Синоним);
	КонецЕсли;
...
&НаКлиенте
Функция ПараметрыЗаполненияВозвратИзОтпускПоУходу()
	...
	ДанныеЗаполнения.Вставить("ОсновнойСотрудник", 	Объект.Сотрудник);
	ДанныеЗаполнения.Вставить("ДатаВозврата", 		Объект.ДатаВыходаНаРаботу);
