...
Процедура ДатаНачалаПериодаПриИзменении(Элемент)
	...
	ОчиститьДанныеЗаПределамиПериода();
	ПериодРегистрацииПриИзменении();
...
&НаКлиенте
Процедура ОчиститьДанныеЗаПределамиПериода()
	
	МассивДней = Новый Массив;
	НомерДняДатаНачала = День(Объект.ДатаНачалаПериода);
	НомерДняДатаОкончания = День(Объект.ДатаОкончанияПериода);
	
	Для НомерДня = 1 По 31 Цикл
		
		Если НомерДня < НомерДняДатаНачала Или НомерДня > НомерДняДатаОкончания Тогда
	...
	Для Каждого Стр Из Объект.ДанныеОВремени Цикл
		...
		Для Каждого НомерДня Из МассивДней Цикл
			...
			Стр["Часов" + НомерДня] = 0;
			Стр["ВидВремени" + НомерДня] = ПредопределенноеЗначение("Справочник.ВидыИспользованияРабочегоВремени.ПустаяСсылка");
			Стр["ПереходящаяЧастьСмены" + НомерДня] = Ложь;
			Стр["Время" + НомерДня + "Представление"] = "";
