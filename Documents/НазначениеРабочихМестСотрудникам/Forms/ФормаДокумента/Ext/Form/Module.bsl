...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		СотрудникиКДобавлению = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "Сотрудники");
		Если ЗначениеЗаполнено(СотрудникиКДобавлению) Тогда
			ДобавитьДанныеПоСотрудникам(СотрудникиКДобавлению);
		КонецЕсли;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
			...
			МодульОбучениеПоОхранеТрудаФормы.ДокументОтстраненияПриСозданииНаСервере(ЭтотОбъект, "НачалоПериода");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьДанныеПоСотрудникам(СотрудникКДобавлению)
	...
	Для Каждого ДанныеСотрудника Из КадровыеДанныеСотрудников Цикл
		...
		Если СтрокиРабочегоМеста.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(ДанныеСотрудника.РабочееМестоОхраныТруда)
				И ЗначениеЗаполнено(СтрокаРабочегоМеста.РабочееМесто)
				И Не СтрокаРабочегоМеста.РабочееМесто = ДанныеСотрудника.РабочееМестоОхраныТруда Тогда
			...
			Иначе
				ДополнитьМассивСотрудников(СтрокаРабочегоМеста, ДанныеСотрудника,
					ЗначениеЗаполнено(ДанныеСотрудника.РабочееМестоОхраныТруда));
			КонецЕсли;
		Иначе
			ДобавитьРабочееМесто(ДанныеСотрудника, Истина);
		КонецЕсли;
...
Процедура ДополнитьМассивСотрудников(СтрокаРабочегоМеста, ДанныеДляДополнения, ЗаполнятьТекущееРабочееМесто = Ложь)
	...
	Иначе
		ЗаполнитьЗначенияСвойств(Описание, ДанныеДляДополнения, "ГрафикРаботы, Сотрудник, ФамилияИО");
	КонецЕсли;
...
&НаСервере
Процедура Подключаемый_ДобавитьСотрудников(СотрудникиКДобавлению) Экспорт
	ДобавитьДанныеПоСотрудникам(СотрудникиКДобавлению);
КонецПроцедуры
