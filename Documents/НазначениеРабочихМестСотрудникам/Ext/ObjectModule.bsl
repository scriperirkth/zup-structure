...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
		...
		МодульОбучениеПоОхранеТруда.ПриЗаписиДокументаОтстранения(ЭтотОбъект, Ложь);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.ШтатноеРасписание") Тогда
		ЗначенияРеквизитовПозиции = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, "Подразделение,Владелец");
		Подразделение = ЗначенияРеквизитовПозиции.Подразделение;
		Организация = ЗначенияРеквизитовПозиции.Владелец;
	КонецЕсли;
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Иначе
			...
			Подразделение = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеЗаполнения, "Подразделение");
			НачалоПериода = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеЗаполнения, "НачалоПериода");
		КонецЕсли;
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
		МодульОбучениеПоОхранеТруда = ОбщегоНазначения.ОбщийМодуль("ОбучениеПоОхранеТруда");
		МодульОбучениеПоОхранеТруда.ПриЗаполненииНазначенияРабочихМестСотрудникам(ЭтотОбъект, ДанныеЗаполнения, СтандартнаяОбработка);
	КонецЕсли;
