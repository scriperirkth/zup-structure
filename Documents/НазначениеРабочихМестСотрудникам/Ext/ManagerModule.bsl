...
Процедура ЗаполнитьДатыЗапрета(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	НазначениеРабочихМестСотрудникам.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.НазначениеРабочихМестСотрудникам КАК НазначениеРабочихМестСотрудникам
		|ГДЕ
		|	НазначениеРабочихМестСотрудникам.ДатаЗапрета = ДАТАВРЕМЯ(1, 1, 1)
		|	И НЕ НазначениеРабочихМестСотрудникам.Ссылка В (&Обработанные)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НазначениеРабочихМестСотрудникам.Дата УБЫВ";
	...
	ОбновляемыеДанные = ОбновлениеИБ.ВыполнитьЗапросПолученияОбновляемыхДанных(Запрос, ПараметрыОбновления,, 100);
	...
	Пока Выборка.Следующий() Цикл
		Если Не ОбновлениеИБ.ПодготовитьОбновлениеДанных(
			ПараметрыОбновления, "Документ.НазначениеРабочихМестСотрудникам", "Ссылка", Выборка.Ссылка) Тогда
			Продолжить;
		КонецЕсли;
