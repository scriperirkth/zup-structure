...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Обработан.Заголовок = Метаданные.Документы.УведомлениеОбОтсутствииОтветаСЭДО.Реквизиты.Обработан.Подсказка;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_БольничныйЛист"
		...
		ОтключитьОбработчикОжидания("ПрочитатьИлиОбновитьВторичныеДанныеКлиент");
		ПодключитьОбработчикОжидания("ПрочитатьИлиОбновитьВторичныеДанныеКлиент", 0.2, Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура ВсеВходящиеЗапросыПоФизлицу(Команда)
	ПараметрыФормы = Новый Структура("Отбор", Новый Структура("ФизическоеЛицо", Объект.ФизическоеЛицо));
	ОткрытьФорму("Документ.ВходящийЗапросФССДляРасчетаПособия.ФормаСписка", ПараметрыФормы);
КонецПроцедуры
...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект)
	...
	Если Модифицированность Или Объект.ДатаСообщения >= НачалоМесяца(ДобавитьМесяц(ТекущаяДатаСеанса(), -3)) Тогда
		ОбновитьВторичныеДанныеНаСервере();
	КонецЕсли;
	ОбновитьЭлементыФормы();
КонецПроцедуры
...
Процедура ОбновитьЭлементыФормы()
	...
	// Поиск ответа на запрос и обновление элементов запроса и ответа.
	ЕстьВходящийЗапрос = ЗначениеЗаполнено(Объект.ВходящийЗапрос);
	Если ЕстьВходящийЗапрос Тогда
		ОтветНаЗапрос = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВходящийЗапрос, "ОтветНаЗапрос");
	Иначе
	...
	Элементы.ВсеВходящиеЗапросыПоФизлицу.Видимость = Не ЕстьВходящийЗапрос;
	Элементы.ОтветНаЗапрос.Видимость = ЕстьОтветНаЗапрос;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СоздатьОтветНаЗапрос", "Видимость", Не ЕстьОтветНаЗапрос);
...
Процедура Подключаемый_ЗафиксироватьИзменениеРеквизитаВФорме(Элемент, СтандартнаяОбработка = Ложь) Экспорт
	ЗафиксироватьИзменениеРеквизита(Элемент.Имя);
КонецПроцедуры

&НаСервере
Процедура ЗафиксироватьИзменениеРеквизита(Знач Имя)
...
&НаКлиенте
Процедура ПрочитатьИлиОбновитьВторичныеДанныеКлиент()
