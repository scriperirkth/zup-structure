...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		УправлениеШтатнымРасписаниемФормы.ПоместитьДанныеОбъектаВДанныеФормы(ЭтотОбъект, Объект.ДатаВступленияВСилу, Объект, Ложь);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	УправлениеШтатнымРасписаниемФормы.ПоместитьДанныеОбъектаВДанныеФормы(ЭтотОбъект, ТекущийОбъект.ДатаВступленияВСилу, ТекущийОбъект);	
	ПриПолученииДанныхНаСервере();
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		...
		МодульОрганизационнаяСтруктура.ДополнитьФормуДокументовИзмененияШтатногоРасписания(ЭтотОбъект, "Объект.ДатаВступленияВСилу");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
		...
		МодульОбучениеРазвитие.ДополнитьФормуДокументовИзмененияШтатногоРасписания(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОбработатьИзменениеПоказателейНаСервере(ЗначенияПоказателей)
	...
	УправлениеШтатнымРасписаниемКлиентСервер.ЗаполнитьИтоговыйФОТПоПозициям(ЭтотОбъект, Объект.Позиции);
	УправлениеШтатнымРасписаниемФормы.ПоместитьДанныеОбъектаВДанныеФормы(ЭтотОбъект, Объект.ДатаВступленияВСилу, Объект);
...
Функция ОбработатьВыбранныеПозиции(ВыбранноеЗначение)
	...
	Для каждого Позиция Из ВыбранныеПозиции Цикл
		...
		Если Объект.Позиции.НайтиСтроки(СтруктураПоиска).Количество() = 0 Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
				МодульХарактеристикиПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ОбучениеРазвитиеФормы");
				МодульХарактеристикиПерсоналаФормы.ЗаполнитьТекущиеДанныеПозиции(ЭтотОбъект, ДобавленнаяСтрока);
			КонецЕсли;
...
Функция ОписаниеДанныхПозицииВХранилище(ИдентификаторСтрокиТекущейПозиции, ИдентификаторПозицииИсточника = Неопределено) 
	...
	Возврат УправлениеШтатнымРасписаниемФормы.ОписаниеДанныхПозицииВХранилище(ЭтотОбъект, Объект, ИдентификаторСтрокиТекущейПозиции, 
	Объект.ДатаВступленияВСилу,Объект.Подразделение,,ИдентификаторПозицииИсточника);
