...
Функция ДанныеЗаполненияВедомости() Экспорт
	...
	ДанныеЗаполнения.Вставить("ПериодВыплаты", "");
...
Функция ДанныеВедомостиНаВыплатуЗарплатыВБанк(МассивДокументов, ДатаПолученияДанных = Неопределено, ПлатежныйДокумент = Неопределено, ТолькоПроведенные = Истина) Экспорт
	...
	|	ВедомостьСостав.НомерЛицевогоСчетаЦР КАК НомерЛицевогоСчетаЦР,
	|	ВедомостьСостав.ИНН КАК ИНН,
	|	Ведомость.ПериодРегистрации КАК ПериодВыплаты
	...
	|		ИНАЧЕ Ведомость.ВидЗачисления
	|	КОНЕЦ,
	|	ВедомостьСостав.НомерЛицевогоСчетаЦР,
	|	ВедомостьСостав.ИНН";
	Запрос.Выполнить();
	...
	|	СписокФизическихЛиц.ИНН КАК ИНН
	...
	|	КОНЕЦ КАК ВидЗачисления,
	|	ВедомостьНаВыплатуЗарплатыВБанк.ПериодРегистрации КАК ПериодВыплаты
	...
	|	ДанныеДокументов.ПериодВыплаты КАК ПериодВыплаты
	|ИЗ
	...
	|	КОНЕЦ КАК СНИЛС,
	|	ВЫБОР
	|		КОГДА ДанныеСтрокДокументов.СпособЗачисления = ЗНАЧЕНИЕ(Перечисление.СпособыЗачисленияВыплат.ПоСчетуЦР)
	|			ТОГДА ДанныеСтрокДокументов.НомерЛицевогоСчетаЦР
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК НомерЛицевогоСчетаЦР,
	|	ВЫБОР
	...
	|	ДанныеСтрокДокументов.ИНН КАК ИНН
	|ИЗ
...
//		ЛицевыеСчетаЦР - Булево - признак того, что передаются лицевые счета ЦР.
//
// Возвращаемое значение:
//		Соответствие, где ключ - это номер лицевого счета, значение - физическое лицо.
//
Функция ФизическиеЛицаЛицевыхСчетов(Ведомости, ЛицевыеСчета, ЛицевыеСчетаЦР = Ложь) Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("Ведомости",    Ведомости);
	Запрос.УстановитьПараметр("ЛицевыеСчета", ЛицевыеСчета);
	Запрос.УстановитьПараметр("ЛицевыеСчетаЦР", ЛицевыеСчетаЦР);
	
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА &ЛицевыеСчетаЦР
	|			ТОГДА ВедомостьСостав.НомерЛицевогоСчетаЦР
	|		ИНАЧЕ ВедомостьСостав.НомерЛицевогоСчета
	|	КОНЕЦ КАК НомерЛицевогоСчета,
	|	ВедомостьСостав.ФизическоеЛицо КАК ФизическоеЛицо
	|ИЗ
	...
	|	И ВЫБОР
	|			КОГДА &ЛицевыеСчетаЦР
	|				ТОГДА ВедомостьСостав.НомерЛицевогоСчетаЦР В (&ЛицевыеСчета)
	|			ИНАЧЕ ВедомостьСостав.НомерЛицевогоСчета В (&ЛицевыеСчета)
	|		КОНЕЦ";
...
Функция ПечатьСпискаПеречисленийПоXML(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	ТабличныйДокумент = ПечатьСписокПеречисленийПоЗарплатномуПроекту(МассивОбъектов, ОбъектыПечати,
...
Функция ПечатьСписокПеречисленийПоЗарплатномуПроекту(МассивОбъектов, ОбъектыПечати, ДанныеДляПечати)
	...
	Для Каждого ДанныеДокументаДляПечати Из ДанныеДляПечати Цикл
		...
		Если ЗначениеЗаполнено(ДанныеДокумента.БИКБанка) Тогда
		...
		Для Каждого ДанныеДляПечатиСтроки Из ДанныеДокумента.Сотрудники Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(ДанныеДляПечатиСтроки.НомерЛицевогоСчетаЦР) Тогда
				ДанныеПечати.Строка.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР;
...
Функция ПечатьСпискаПеречисленийПоJSON(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабличныйДокумент = ПечатьСписокПеречисленийПоЗарплатномуПроекту(МассивОбъектов, ОбъектыПечати,
		ОбменСБанкамиПоЗарплатнымПроектам.ДанныеСпискаПеречисленийПоJSON(МассивОбъектов));
