...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ВариантИсправления = Перечисления.ВариантыИсправленияЗаявокКабинетСотрудника.Отмена Тогда
		Шаблон = НСтр("ru = 'Отмена заявления на выходные дни для ухода за детьми-инвалидами %1 от %2'");
	Иначе
		Шаблон = НСтр("ru = 'Заявление на выходные дни для ухода за детьми-инвалидами %1 от %2'");
	КонецЕсли;
...
Функция ОписаниеДокументаДляЗаявки(СписокОбщихПолей) Экспорт
	...
	СтрокаСвойств = "ИспользоватьВсеНакопленныеДни,НетСправкиВторогоРодителя,ПричинаОтсутствияСправкиВторогоРодителя";
	СписокПолей = СтрШаблон("%1,%2", СписокОбщихПолей, СтрокаСвойств);
	
	Описание = Новый Структура(СписокПолей);
	Описание.Вставить("ВыходныеДни", ОписаниеВыходныеДни());
...
Функция НаименованиеЗадачи(ДокументОбъект) Экспорт
	...
	Если ДокументОбъект.ВариантИсправления = Перечисления.ВариантыИсправленияЗаявокКабинетСотрудника.Отмена Тогда
		ШаблонНаименования = НСтр("ru = 'Отмена заявки на выходные дни для ухода за детьми-инвалидами'");
	Иначе
		ШаблонНаименования = НСтр("ru = 'Заявки на выходные дни для ухода за детьми-инвалидами'");
	КонецЕсли;
...
Функция СодержаниеЗадачи(ДокументОбъект) Экспорт
	...
	Если ДокументОбъект.ВариантИсправления = Перечисления.ВариантыИсправленияЗаявокКабинетСотрудника.Отмена Тогда
		ШаблонНаименования = НСтр("ru = 'Обработать заявление на отмену заявления на выходные дни для ухода за детьми-инвалидами'");
	Иначе
		ШаблонНаименования = НСтр("ru = 'Обработать заявление на выходные дни для ухода за детьми-инвалидами'");
	КонецЕсли;
