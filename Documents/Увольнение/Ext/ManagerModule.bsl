...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РеквизитыДляПроведения.ДокументРассчитан Тогда
		...
		Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
			...
			Для Каждого СтрокаПериодовРегистрации Из РеквизитыДляПроведения.ПериодыРегистрации Цикл
				...
				Иначе
					...
					Запрос = ЗапросВТНачисленияПоДокументу(ДанныеДляПроведения.МенеджерВременныхТаблиц, ПериодРегистрации, СтрокаПериодовРегистрации.ЭтоПервыйПериод);
					Запрос.Выполнить();
			...
			ОчиститьСведенияОДоходахУмершихСотрудников(Движения, РеквизитыДляПроведения.ФизическиеЛицаУвольняемыеВСвязиСоСмертью, СтруктураВидовУчета);
			...
			ОстаткиОтпусков.СформироватьДвиженияОснованийУвольненияСотрудников(Движения, ДанныеДляПроведения.ОснованияУвольнения);
		...
		Если СтруктураВидовУчета.СтраховыеВзносы Тогда
			...
			Если РезультатыИсправления.ИсправлениеВТекущемПериоде Тогда
				Движения.СтраховыеВзносыПоФизическимЛицам.ДополнительныеСвойства.Вставить("ВычитатьСторноЗаписи", Истина);
			КонецЕсли;
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
			...
			МодульНаставничество.СформироватьДвиженияПоНаставничеству(ДокументСсылка, Объект, Движения, ДанныеДляПроведения, Ложь);
		КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьУвольнениеСОсновногоМестаРаботы");
	Методы.Вставить("ЗаполнитьДатуРасчетаДенежногоСодержания");
	Методы.Вставить("ЗаполнитьПредоставлятьМероприятияТДВЭлектроннойФорме");
	Методы.Вставить("ЗаполнитьДатыЗапрета");
	Методы.Вставить("УстановитьФлагНачислитьЗарплату");
	Методы.Вставить("ЗаполнитьПериодРасчетаСохраняемогоДенежногоСодержания");
	Методы.Вставить("ЗаполнитьИсходныеДанныеПерерасчетов");
	Методы.Вставить("ЗаполнитьПериодыРегистрацииНачислений");
КонецПроцедуры
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.УведомлениеОСпецрежимахНалогообложения) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Представление =
			НСтр("ru = 'Уведомление о расторжении договора с иностранным гражданином'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.Обработчик =
			"КадровыйУчетКлиент.ВвестиУведомлениеОРасторженииДоговораСИностранцем";
		
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Представление =
			НСтр("ru = 'Уведомлениие об исполнении обязательств перед иностранным гражданином'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.Обработчик =
			"КадровыйУчетКлиент.ВвестиУведомлениеОбИсполненииОбязательствПередИностраннымСпециалистом";
	КонецЕсли;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
		МодульНаставничество = ОбщегоНазначения.ОбщийМодуль("Наставничество");
		МодульНаставничество.ДобавитьКомандыПечатиПоОтменеНаставничества(КомандыПечати, ИдентификаторыПФ);
	КонецЕсли;
...
Процедура ПечатьПодраздела_1_2_ФормыЕФС_1(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, ПерсонифицированныйУчет.ИдентификаторПечатнойФормы_ФормаЕФС_1())
		...
		Для Каждого ОбъектыТипа Из ОбъектыПоТипам Цикл
			...
			КодВПИГ = ПерсонифицированныйУчет.ВидыЗастрахованныхЛицВПИГ(ТекущаяДатаСеанса());
			КодВКС = ПерсонифицированныйУчет.ВидыЗастрахованныхЛицВКС();
			КодМДИГ = ПерсонифицированныйУчет.ВидыЗастрахованныхЛицМДИГ(ТекущаяДатаСеанса());
			...
			Запрос.Текст =
				...
				|		КОГДА КадровыеДанные.ВидЗастрахованногоЛица В (&КодМДИГ)
...
Функция ДанныеДокументовДляПечатиРасчетаСреднегоЗаработка(МассивСсылок, ПоСтатьямФинансирования = Ложь, ИмяМакета = "") Экспорт
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Если КонецДня(Выборка.ДатаНачалаСобытия) = КонецМесяца(Выборка.ДатаНачалаСобытия) Тогда 
			ДополнительныеПараметры.ДатаНачалаСобытия = НачалоМесяца(ДобавитьМесяц(Выборка.ДатаНачалаСобытия, 1));
		КонецЕсли;
					...
					Если МакетыРСВ <> Неопределено Тогда
						...
						Если Макеты <> Неопределено Тогда
							...
							Если НачалоВыводаБланка < ДокументРезультат.ВысотаТаблицы + 1 Тогда
								...
								ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, ИдентификаторПечатнойФормыРаздел3РСВ(),
...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, МенеджерВременныхТаблиц, ПериодРегистрации, Организация, ПеремещаемыеСовместители = Неопределено, СтруктураВидовУчета = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
			МодульНаставничество = ОбщегоНазначения.ОбщийМодуль("Наставничество");
			МодульНаставничество.ДополнитьДанныеДляПроведенияПоНаставничеству(РеквизитыДляПроведения.Ссылка, ДанныеДляПроведения);
		КонецЕсли;

		ЗаполнитьПериодыОтпусков(РеквизитыДляПроведения, ДанныеДляПроведения);
		ЗаполнитьДанныеОбИспользованииОтгулов(РеквизитыДляПроведения, ДанныеДляПроведения);
		ЗаполнитьОснованияУвольненияСотрудников(РеквизитыДляПроведения, ДанныеДляПроведения);
...
Процедура ЗаполнитьДанныеОбИспользованииОтгулов(РеквизитыДляПроведения, ДанныеДляПроведения)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	УвольнениеОтработанныеДни.Ссылка.ДатаУвольнения КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	УвольнениеОтработанныеДни.Ссылка.Организация КАК Организация,
		|	УвольнениеОтработанныеДни.Ссылка.Сотрудник КАК Сотрудник,
		|	УвольнениеОтработанныеДни.Дата КАК ОтработанныйДень,
		|	ВЫБОР
		|		КОГДА УвольнениеОтработанныеДни.Часы = 0
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Дни,
		|	УвольнениеОтработанныеДни.Часы КАК Часы
		|ИЗ
		|	Документ.Увольнение.ОтработанныеДни КАК УвольнениеОтработанныеДни
		|ГДЕ
		|	УвольнениеОтработанныеДни.Ссылка = &Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	УвольнениеСпискомСотрудники.Ссылка.Организация,
		|	УвольнениеСпискомСотрудники.Сотрудник,
		|	УвольнениеСпискомОтработанныеДни.Дата,
		|	ВЫБОР
		|		КОГДА УвольнениеСпискомОтработанныеДни.Часы = 0
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	УвольнениеСпискомОтработанныеДни.Часы
		|ИЗ
		|	Документ.УвольнениеСписком.Сотрудники КАК УвольнениеСпискомСотрудники
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УвольнениеСписком.ОтработанныеДни КАК УвольнениеСпискомОтработанныеДни
		|		ПО УвольнениеСпискомСотрудники.Ссылка = УвольнениеСпискомОтработанныеДни.Ссылка
		|			И УвольнениеСпискомСотрудники.ИдентификаторСтрокиСотрудника = УвольнениеСпискомОтработанныеДни.ИдентификаторСтрокиСотрудника
...
Процедура ЗаполнитьОснованияУвольненияСотрудников(РеквизитыДляПроведения, ДанныеДляПроведения)
	...
	ОснованияУвольнения = Запрос.Выполнить().Выгрузить();
	ДанныеДляПроведения.Вставить("ОснованияУвольнения", ОснованияУвольнения);	
...
Функция ЗапросВТНачисленияПоДокументу(МенеджерВременныхТаблиц, ПериодРегистрации, ЭтоПервыйПериод)
...
Процедура ОчиститьСведенияОДоходахУмершихСотрудников(Движения, ФизическиеЛицаУвольняемыеВСвязиСоСмертью, СтруктураВидовУчета)
	...
	СведенияОбИсправлении = ИсправлениеДокументовЗарплатаКадры.СведенияОбИсправленииДокумента(СсылкаНаДокумент);
		...
		Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
			...
			ДанныеПервичного = ?(ЗначениеЗаполнено(СведенияОбИсправлении.ИсправленныйДокумент),
				ЭлектронныеТрудовыеКнижкиРасширенный.ДатаНомерИсправленногоДокументаОснования(СведенияОбИсправлении.ИсправленныйДокумент, Выборка.НомерПриказа),
				Неопределено);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ДанныеПервичного <> Неопределено Тогда
					ЗаполнитьЗначенияСвойств(Запись, ДанныеПервичного);
				КонецЕсли;
