...
Функция ДанныеДляПроведенияМероприятияТрудовойДеятельности(СсылкаНаДокумент, ТолькоПроведенные = Ложь) Экспорт
	...
	Если ЗарплатаКадры.ВТСодержитСтроки(Запрос.МенеджерВременныхТаблиц, "ВТДанныеДокументов") Тогда
		...
		Если НеобходимоПолучениеКадровыхДанных Тогда
			...
			ПоляОтбора.Вставить("МероприятияТрудовойДеятельности", Отборы);
			ПоляОтбора.Вставить("ТерриторииСотрудников", Отборы);
...
Функция ДанныеДляПроведения(ДокументСсылка, ТолькоБронированиеПозиции, НачисленияУтверждены)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Запрос.Текст =
			...
			|	ВЫБОР
			|		КОГДА &ТолькоБронированиеПозиции = ЛОЖЬ
			|				И (Сотрудники.ДатаОкончания > ДАТАВРЕМЯ(1, 1, 1)
			|					ИЛИ ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ПериодВозвратногоСобытия, ДАТАВРЕМЯ(1, 1, 1)) > ДАТАВРЕМЯ(1, 1, 1))
			|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
			|		КОГДА ЕСТЬNULL(ЗанятостьПозицийШтатногоРасписания.ВидЗанятостиПозиции, ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиПозицийШтатногоРасписания.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиПозицийШтатногоРасписания.Свободна)
			...
			|				ИЛИ ТИПЗНАЧЕНИЯ(ЗанятостьПозицийШтатногоРасписания.Регистратор) = ТИП(Документ.ПриостановлениеТрудовыхДоговоровКонтрактов))";
	...
	Если Не ТолькоБронированиеПозиции Тогда
		...
		Если НачисленияУтверждены Тогда
			...
			Если РасчетЗарплаты.ПрименяетсяАУСН() Тогда
				ДополнитьСтрокамиПрекращенияНачисленийРВОП = Ложь;
				Для Каждого СтрокаНачислений Из ПлановыеНачисленияДляРеестраПриказов Цикл
					Если ЗарплатаКадрыРасширенный.ЭтоНачислениеРедактируемоеВОтдельныхПолях(СтрокаНачислений.Начисление) Тогда
						ДополнитьСтрокамиПрекращенияНачисленийРВОП = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если ДополнитьСтрокамиПрекращенияНачисленийРВОП Тогда
					
					ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
						"ВТСотрудникиДатыВремя", "Сотрудник");
					ОписаниеФильтра.СоответствиеИзмеренийРегистраИзмерениямФильтра.Вставить("Период", "ДатаСобытия");
					...
					ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(ПараметрыПостроения.Отборы,
						"Начисление", "В", ЗарплатаКадрыРасширенный.НачисленияРедактируемыеВОтдельныхПолях());
					...
					ТекущиеНачисления = Запрос.Выполнить().Выгрузить();
					
					ЗавершитьНачисленияРедактируемыеВОтдельныхПолях(ПлановыеНачисленияДляРеестраПриказов, ТекущиеНачисления);
...
Процедура ЗавершитьНачисленияРедактируемыеВОтдельныхПолях(ДанныеДляПроведенияПлановыеНачисления, ТекущиеНачисления)
	
	Для Каждого СтрокаТаблицы Из ДанныеДляПроведенияПлановыеНачисления Цикл
		Если ЗарплатаКадрыРасширенный.ЭтоНачислениеРедактируемоеВОтдельныхПолях(СтрокаТаблицы.Начисление) Тогда
			ДобавитьНачисленияРВОП(
				ДанныеДляПроведенияПлановыеНачисления,
				ТекущиеНачисления,
				ЗарплатаКадрыРасширенныйПовтИсп.НачислениеРайонныйКоэффициент(Истина),
				ЗарплатаКадрыРасширенныйПовтИсп.НачислениеРайонныйКоэффициент(Ложь));
			ДобавитьНачисленияРВОП(
				ДанныеДляПроведенияПлановыеНачисления,
				ТекущиеНачисления,
				ЗарплатаКадрыРасширенныйПовтИсп.НачислениеСевернаяНадбавка(Истина),
				ЗарплатаКадрыРасширенныйПовтИсп.НачислениеСевернаяНадбавка(Ложь));
			ДобавитьНачисленияРВОП(
				ДанныеДляПроведенияПлановыеНачисления,
				ТекущиеНачисления,
				ЗарплатаКадрыРасширенныйПовтИсп.НачислениеНадбавкаЗаВредность(Истина),
				ЗарплатаКадрыРасширенныйПовтИсп.НачислениеНадбавкаЗаВредность(Ложь));
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьНачисленияРВОП(ДанныеДляПроведенияПлановыеНачисления, ТекущиеНачисления, ВидРасчета, ВидРасчетаАльтернатива)
	ДобавитьНачислениеРВОП(ДанныеДляПроведенияПлановыеНачисления, ТекущиеНачисления, ВидРасчета, ВидРасчетаАльтернатива);
	ДобавитьНачислениеРВОП(ДанныеДляПроведенияПлановыеНачисления, ТекущиеНачисления, ВидРасчетаАльтернатива, ВидРасчета)
КонецПроцедуры

Процедура ДобавитьНачислениеРВОП(ДанныеДляПроведенияПлановыеНачисления, ТекущиеНачисления, ВидРасчета, ВидРасчетаАльтернатива)
	
	СтрокаВР = ДанныеДляПроведенияПлановыеНачисления.НайтиСтроки(
		Новый Структура("Начисление,Действие", ВидРасчета, Перечисления.ДействияСНачислениямиИУдержаниями.Утвердить));
	Если ЗначениеЗаполнено(СтрокаВР) Тогда
		СтрокаВРАльтернатива = ТекущиеНачисления.НайтиСтроки(
			Новый Структура("Начисление,Используется", ВидРасчетаАльтернатива, Истина));
		Если ЗначениеЗаполнено(СтрокаВРАльтернатива) Тогда
			НоваяСтрока = ДанныеДляПроведенияПлановыеНачисления.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаВР[0]);
			НоваяСтрока.Начисление = ВидРасчетаАльтернатива;
			НоваяСтрока.Действие = Перечисления.ДействияСНачислениямиИУдержаниями.Отменить;
			НоваяСтрока.Используется = Ложь;
			НоваяСтрока.ИдентификаторСтрокиВидаРасчета = Неопределено;
		КонецЕсли;
	КонецЕсли;
