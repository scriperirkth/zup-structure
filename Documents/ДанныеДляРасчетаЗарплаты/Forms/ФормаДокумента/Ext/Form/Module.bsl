...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	БлокировкаИзмененияОбъектов.ПриЧтенииНаСервереФормыОбъекта(ЭтотОбъект, ТекущийОбъект,
		ПравоДоступа("Изменение", Метаданные.Документы.ДанныеДляРасчетаЗарплаты));
	// Конец БлокировкаИзмененияОбъектов
...
Процедура ДанныеДетальноПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ЗаполнитьСтрокуДетально(ТекущиеДанные, ПоказателиРасчетаЗарплаты, ВидыВремени, ВидыРабот);
...
Процедура НастроитьЭлементыВвода()
	...
	ДокументИсправлен = ИсправлениеДокументовЗарплатаКадрыКлиентСервер.ДокументИсправлен(ЭтотОбъект);
	...
	Элементы.ДанныеСводноОбъект.ТолькоПросмотр = НастройкиВидаДокумента.ЗаполнятьСписокОбъектов Или ДокументИсправлен;
	...
	Элементы.ДанныеДетально.ПоложениеКоманднойПанели = ?(НастройкиВидаДокумента.ЗаполнятьСписокОбъектов,
														ПоложениеКоманднойПанелиЭлементаФормы.Нет,
														ПоложениеКоманднойПанелиЭлементаФормы.Авто);
	Элементы.ДанныеДетальноОбъект.ТолькоПросмотр = НастройкиВидаДокумента.ЗаполнятьСписокОбъектов Или ДокументИсправлен;
...
Процедура ОбработкаПодбораСотрудниковНаСервере(ВыбранноеЗначение, ПутьКТаблицеСотрудников, ЗаполнятьТаблицуИсполнителиРабот = Ложь)
	...
	Для Каждого Сотрудник Из ВыбранныеСотрудники Цикл
		...
		Если СтрокиПоСотруднику.Количество() = 0 Тогда
			...
			Если ТаблицаСотрудников = ДанныеДетально Тогда
				ЗаполнитьСтрокуДетально(НоваяСтрока, ПоказателиРасчетаЗарплаты, ВидыВремени, ВидыРабот);
			КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСтрокуДетально(ТекущиеДанные, ПоказателиРасчетаЗарплаты, ВидыВремени, ВидыРабот)
...
Процедура УстановитьПоляИсправления(Форма)
	...
	Форма.Элементы.ДанныеСводно.КоманднаяПанель.Доступность = Не ДокументИсправлен;
	Форма.Элементы.ДанныеДетально.КоманднаяПанель.Доступность = Не ДокументИсправлен;
