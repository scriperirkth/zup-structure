...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.Свойство("КадровыеДанные") Тогда
		Кэш.Вставить("КадровыеДанные", ДополнительныеПараметры.КадровыеДанные);
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьКоличествоДнейОплаты");
	Методы.Вставить("ЗаполнитьИсходныеДанныеПерерасчетов");
	Методы.Вставить("ИсправитьДвиженияЗанятостьПозицийШтатногоРасписания");
	Методы.Вставить("ЗаполнитьДвиженияЗанятостьПозицийШтатногоРасписания");
	Методы.Вставить("ЗаполнитьДниНетрудоспособностиПериодыУходаЗаБольнымиРодственниками");
КонецПроцедуры
...
Функция ДатаНачалаОплатыЗаСчетСФР(Больничный) Экспорт
	Если Больничный.СдвигатьПериодОплаты И Больничный.ИсключаемыеПериоды.Количество() > 0 Тогда
		...
		Если ДатаНачалаОплатыЗаСчетСФР <> Неопределено Тогда
			Возврат ДатаНачалаОплатыЗаСчетСФР;
		КонецЕсли;
	ИначеЕсли ИзменитьДатуНачалаОплатыОтпускаПоБИРЗаСчетСФР(Больничный) Тогда
		Возврат Больничный.ДатаНачалаОплаты;
	КонецЕсли;
...
Функция ДанныеСостоянийСотрудников(РеквизитыДляПроведения, ДругиеСотрудники, Кэш) Экспорт
	...
	КадровыеДанные = КадровыеДанныеСотрудника(РеквизитыДляПроведения.Сотрудник, РеквизитыДляПроведения.Организация, РеквизитыДляПроведения.ДатаНачалаСобытия, Кэш);
	Кэш.Вставить("КадровыеДанные", КадровыеДанные);
...
Процедура ЗаполнитьИсключаемыеПериоды(Больничный, Кэш)
	...
	Для Каждого Период Из ПериодыКИсключению Цикл
		ИсключаемыйПериод = Больничный.ИсключаемыеПериоды.Добавить();
		ИсключаемыйПериод.Регистрировать  = Период.Причина = Перечисления.ПричиныНеоплатыБольничных.ПределОплаты5МесяцевДляИнвалидов;
...
Функция ДатыКИсключению(Больничный, Кэш) Экспорт
	...
	ДобавитьДатыКИсключениюПревышенияПределаОплаты(Больничный, ДатыКИсключению, Кэш);
...
Функция ДатыНачислений(Больничный, Кэш) Экспорт
	...
	Если Больничный.СдвигатьПериодОплаты Тогда
		...
		КонецЕсли;
	ИначеЕсли ИзменитьДатуНачалаОплатыОтпускаПоБИРЗаСчетСФР(Больничный) Тогда
		ДатаНачалаОплатыЗаСчетСФР = Больничный.ДатаНачалаОплаты;
	Иначе
...
Функция ИзменитьДатуНачалаОплатыОтпускаПоБИРЗаСчетСФР(Больничный)
	Возврат Больничный.КодПричиныНетрудоспособности = "05" И Больничный.ДатаНачалаОплаты <> '00010101'
		И Больничный.ДатаНачалаСобытия = Больничный.ДатаНачала И Больничный.ДатаНачала <> Больничный.ДатаНачалаОплаты;
КонецФункции

Процедура ДобавитьДатыКИсключениюПревышенияПределаОплаты(Больничный, ДатыКИсключению, Кэш)
	Если Кэш["ПревышенПределОплаты"] <> Неопределено
		Или Не УчетПособийСоциальногоСтрахования.ИспользуетсяУсловиеИсчисления(Больничный, "45")
		Или Не (Больничный.КодПричиныНетрудоспособности = "01" Или Больничный.КодПричиныНетрудоспособности = "02") Тогда
		Возврат;
	КонецЕсли;
	
	Остаток = 150;
	ПревышенПределОплаты = Ложь;
	ДатаНачала = Больничный.ДатаНачала;
	ДатаОкончания = Больничный.ДатаОкончания;
	КодыПричин = Новый Массив;
	КодыПричин.Добавить("01");
	КодыПричин.Добавить("02");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НачалоГода", НачалоГода(ДатаНачала));
	Запрос.УстановитьПараметр("КонецГода", КонецГода(ДатаНачала));
	Запрос.УстановитьПараметр("Сотрудник", Больничный.Сотрудник);
	Запрос.УстановитьПараметр("Ссылка", Больничный.Ссылка);
	Запрос.УстановитьПараметр("КодыПричин", КодыПричин);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ДанныеСостоянийСотрудников.Сторно КАК Сторно,
		|	ВЫБОР
		|		КОГДА НАЧАЛОПЕРИОДА(ДанныеСостоянийСотрудников.Начало, ГОД) = &НачалоГода
		|			ТОГДА ДанныеСостоянийСотрудников.Начало
		|		ИНАЧЕ НАЧАЛОПЕРИОДА(ДанныеСостоянийСотрудников.Окончание, ГОД)
		|	КОНЕЦ КАК НачалоПериода,
		|	ВЫБОР
		|		КОГДА НАЧАЛОПЕРИОДА(ДанныеСостоянийСотрудников.Окончание, ГОД) = &НачалоГода
		|			ТОГДА ДанныеСостоянийСотрудников.Окончание
		|		ИНАЧЕ КОНЕЦПЕРИОДА(ДанныеСостоянийСотрудников.Начало, ГОД)
		|	КОНЕЦ КАК ОкончаниеПериода
		|ПОМЕСТИТЬ ДанныеСостояний
		|ИЗ
		|	РегистрСведений.ДанныеСостоянийСотрудников КАК ДанныеСостоянийСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.БольничныйЛист КАК БольничныйЛист
		|		ПО ДанныеСостоянийСотрудников.ДокументОснование = БольничныйЛист.Ссылка
		|ГДЕ
		|	ДанныеСостоянийСотрудников.Сотрудник = &Сотрудник
		|	И ДанныеСостоянийСотрудников.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияСотрудника.Болезнь)
		|	И ДанныеСостоянийСотрудников.Начало <= &КонецГода
		|	И ДанныеСостоянийСотрудников.Окончание >= &НачалоГода
		|	И БольничныйЛист.Ссылка <> &Ссылка
		|	И БольничныйЛист.КодПричиныНетрудоспособности В(&КодыПричин)
		|	И ВЫБОР
		|			КОГДА БольничныйЛист.УсловияИсчисленияКод1 = ""45""
		|				ТОГДА ИСТИНА
		|			КОГДА БольничныйЛист.УсловияИсчисленияКод2 = ""45""
		|				ТОГДА ИСТИНА
		|			КОГДА БольничныйЛист.УсловияИсчисленияКод3 = ""45""
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ
		|;
		...
		|				КОГДА ДанныеСостояний.Сторно
		|					ТОГДА -(РАЗНОСТЬДАТ(ДанныеСостояний.НачалоПериода, ДанныеСостояний.ОкончаниеПериода, ДЕНЬ) + 1)
		|				ИНАЧЕ РАЗНОСТЬДАТ(ДанныеСостояний.НачалоПериода, ДанныеСостояний.ОкончаниеПериода, ДЕНЬ) + 1
		|			КОНЕЦ), 0) КАК ОплаченоДней
		|ИЗ
		|	ДанныеСостояний КАК ДанныеСостояний";
	
	// В соответствии с письмом СФР № 14-41308 от 19.01.2023 есть 3 варианта расчета оплаты дней:
	// - первый случай в году и нетрудоспособность начинается 1 числа месяца, то считаем 5 полных календарных месяцев
	//   например с 01.02.2026 по 30.06.2026;
	// - первый случай в году и нетрудоспособность начинается после 1 числа месяца, срок истекает в соответствующее число последнего
	//   месяца срока например с 03.02.2026 по 03.07.2026;
	// - не первый случай в году лимит рассчитывается исходя из суммарного количества дней, но не более 150.
	ЭтоПервыйСлучай = Истина;
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	Если Выборка.Следующий() Тогда
		Остаток = Остаток - Выборка.ОплаченоДней;
		ЭтоПервыйСлучай = Выборка.ОплаченоДней = 0;
	КонецЕсли;
	
	Если ЭтоПервыйСлучай Тогда
		Остаток = ОбщегоНазначенияБЗК.КоличествоДнейВПериоде(
			ДатаНачала, ДобавитьМесяц(ДатаНачала, 5)) - ?(День(ДатаНачала) = 1, 1, 0);
	КонецЕсли;
	
	Если Год(ДатаНачала) = Год(ДатаОкончания) Тогда
		ДобавитьДатыПревышенияПределаОплаты(ДатыКИсключению, ДатаНачала, ДатаОкончания, ПревышенПределОплаты, Остаток);
	Иначе
		ДобавитьДатыПревышенияПределаОплаты(ДатыКИсключению, ДатаНачала, КонецГода(ДатаНачала), ПревышенПределОплаты, Остаток);
		НачалоСледующегоГода = НачалоГода(ДатаОкончания);
		КалендарныхДней = ОбщегоНазначенияБЗК.КоличествоДнейВПериоде(
			НачалоСледующегоГода, ДобавитьМесяц(НачалоСледующегоГода, 5)) - 1;
		ДобавитьДатыПревышенияПределаОплаты(
			ДатыКИсключению, НачалоСледующегоГода, ДатаОкончания, ПревышенПределОплаты, КалендарныхДней);
	КонецЕсли;
	
	Кэш.Вставить("ПревышенПределОплаты", ПревышенПределОплаты);
КонецПроцедуры

Процедура ДобавитьДатыПревышенияПределаОплаты(ДатыКИсключению, ДатаНачала, ДатаОкончания, ПревышенПределОплаты, Остаток)
	КалендарныхДней = ОбщегоНазначенияБЗК.КоличествоДнейВПериоде(ДатаНачала, ДатаОкончания);
	Если Остаток >= КалендарныхДней Тогда
		Возврат;
	КонецЕсли;
	
	Отбор = Новый Структура("Дата");
	ПревышенПределОплаты = Истина;
	ДатаНачала = ДатаНачала + Остаток * 86400;
	Пока ДатаНачала <= ДатаОкончания Цикл
		Отбор.Дата = ДатаНачала;
		НайденныеСтроки = ДатыКИсключению.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() = 0 Тогда
			НоваяСтрока = ДатыКИсключению.Добавить();
			НоваяСтрока.Дата = ДатаНачала;
			НоваяСтрока.Причина = Перечисления.ПричиныНеоплатыБольничных.ПределОплаты5МесяцевДляИнвалидов;
		КонецЕсли;
...
Функция ЗаполнитьПериодРасчетаСреднегоЗаработка(Документ) Экспорт
	...
	ДатаНачалаСобытия = Документ.ДатаНачалаСобытия;
	Если ИзменитьДатуНачалаОплатыОтпускаПоБИРЗаСчетСФР(Документ) Тогда
		ДатаНачалаСобытия = Документ.ДатаНачалаОплаты;
	КонецЕсли;
	
	Период = УчетПособийСоциальногоСтрахованияКлиентСервер.ПериодРасчетаСреднегоЗаработкаФСС(
		ДатаНачалаСобытия, ПорядокРасчетаСреднегоЗаработкаФСС(Документ));
...
Функция КадровыеДанныеСотрудника(Сотрудник, Организация, ДатаНачалаСобытия, Кэш = Неопределено) Экспорт
	Если Кэш = Неопределено Тогда
		Кэш = Новый Структура;
	ИначеЕсли ТипЗнч(Кэш) = Тип("Структура") И Кэш.Свойство("КадровыеДанные") И Кэш.КадровыеДанные <> Неопределено Тогда
		Возврат Кэш.КадровыеДанные;
	ИначеЕсли ТипЗнч(Кэш) = Тип("Соответствие") И Кэш["КадровыеДанные"] <> Неопределено Тогда
		Возврат Кэш["КадровыеДанные"];
	КонецЕсли;
	...
	Кэш.Вставить("КадровыеДанные", Результат);
...
Функция ПересекающиесяОтпуска(Больничный) Экспорт
	...
	ДатаОкончания = Больничный.ДатаОкончания;
	Для Каждого ИсключаемыйПериод Из Больничный.ИсключаемыеПериоды Цикл
		Если ИсключаемыйПериод.ДатаНачала <= ДатаНачала И ИсключаемыйПериод.ДатаОкончания >= ДатаНачала
				И ИсключаемыйПериод.ДатаОкончания < ДатаОкончания Тогда
			ДатаНачала = ИсключаемыйПериод.ДатаОкончания + 86400;
		ИначеЕсли ИсключаемыйПериод.ДатаОкончания >= ДатаОкончания И ИсключаемыйПериод.ДатаНачала >= ДатаНачала
			И ИсключаемыйПериод.ДатаНачала < ДатаОкончания Тогда
			ДатаОкончания = НачалоДня(ИсключаемыйПериод.ДатаНачала - 1);
		КонецЕсли;
	КонецЦикла;
	Если ДатаНачала >= Больничный.ДатаОкончания Или ДатаОкончания <= Больничный.ДатаНачала Тогда
		// Период больничного полностью исключен
		Возврат Неопределено;
	КонецЕсли;
	...
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(ОтборыДляФПД, "ПериодДействияНачало", "<=", ДатаОкончания);
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(ОтборыДляФПД, "ПериодДействияКонец", ">=", ДатаНачала);
	...
	Для Каждого Отпуск Из ОтпускаТребующиеИсправления Цикл
		ОтпускИсключенИзБольничного = Ложь;
		Для Каждого ИсключаемыйПериод Из Больничный.ИсключаемыеПериоды Цикл
			ПериодПересечения = ОбщегоНазначенияБЗК.ПериодПересеченияДвухИнтервалов(
				Отпуск.ДатаНачалаПериодаОтсутствия, Отпуск.ДатаОкончанияПериодаОтсутствия,
				ИсключаемыйПериод.ДатаНачала, ИсключаемыйПериод.ДатаОкончания);
			ОтпускИсключенИзБольничного = ПериодПересечения <> Неопределено
				И Отпуск.ДатаНачалаПериодаОтсутствия = ПериодПересечения.ДатаНачала
				И Отпуск.ДатаОкончанияПериодаОтсутствия = ПериодПересечения.ДатаОкончания
		КонецЦикла;
		Если Не ОтпускИсключенИзБольничного Тогда
...
Процедура ЗаполнитьНастройкиМенеджераРасчета(Документ, МенеджерРасчета, ПериодРасчетаЗарплаты, Кэш, СохранятьИсправления = Истина)
	...
	МенеджерРасчета.НастройкиБухучета.НастройкиБухучетаДокумента = ДанныеДляБухучетаЗарплатыПервичныхДокументов(Документ)["ТаблицаБухучетЗарплаты"];
...
Функция ДанныеДляБухучетаЗарплатыПервичныхДокументов(Объект) Экспорт
	...
	ТаблицаБухучетЗарплаты = ОтражениеЗарплатыВБухучетеРасширенный.ДанныеДляБухучетаЗарплатыПервичныхДокументовМножественный(Объект.Ссылка, Объект.НастройкиБухучета);
	
	Если ЗначениеЗаполнено(Объект.СпособРасчетовСФизическимиЛицами) И ДоступноИзменениеСпособаРасчетов(Объект) Тогда
		СтатьиРасходовПоСпособамРасчетов  = ЗарплатаКадры.СтатьиРасходовПоСпособамРасчетовСФизическимиЛицами();
		СтатьяРасходов = СтатьиРасходовПоСпособамРасчетов[Объект.СпособРасчетовСФизическимиЛицами];
		Для каждого СтрокаТЗ Из ТаблицаБухучетЗарплаты Цикл
...
Функция ДоступноИзменениеСпособаРасчетов(Объект) Экспорт

	Возврат Объект.СотрудникДоговорник
		...
		И Объект.ДатаНачалаОплаты < ДатаНачалаОплатыЗаСчетСФР(Объект);
