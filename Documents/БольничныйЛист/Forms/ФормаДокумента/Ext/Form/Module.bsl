...
&НаКлиенте
Процедура ДоляРаботодателяПредставлениеРаспределенияБухучетаНажатие(Элемент)

	ОткрытьФормуРедактированияРаспределенияБухучетаДокумента(ДоляРаботодателяНастройкиБухучета, "ДоляРаботодателяНастройкиБухучета");
	
КонецПроцедуры

&НаКлиенте
Процедура ДоплатаПредставлениеРаспределенияБухучетаНажатие(Элемент)
	
	ОткрытьФормуРедактированияРаспределенияБухучетаДокумента(ДоплатаНастройкиБухучета, "ДоплатаНастройкиБухучета")
	
КонецПроцедуры

&НаКлиенте
Процедура ДоляРаботодателяВариантУказанияБухучетаПриИзменении(Элемент)
	
	УстановитьСтраницуНастроекБухучетаДоляРаботодателя(ЭтотОбъект);
	ОбновитьБухучетРезультатовРасчета();
	
КонецПроцедуры

&НаКлиенте
Процедура ДоплатаВариантУказанияБухучетаПриИзменении(Элемент)
	
	УстановитьСтраницуНастроекБухучетаДоплата(ЭтотОбъект);
	ОбновитьБухучетРезультатовРасчета();
	
КонецПроцедуры

&НаКлиенте
Процедура ДоляРаботодателяСтатьяФинансированияПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоляРаботодателяСтатьяРасходовПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоляРаботодателяСпособОтраженияЗарплатыВБухучетеПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоплатаСтатьяФинансированияПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоплатаСтатьяРасходовПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоплатаСпособОтраженияЗарплатыВБухучетеПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоляРаботодателяПодразделениеУчетаЗатратПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры

&НаКлиенте
Процедура ДоплатаПодразделениеУчетаЗатратПриИзменении(Элемент)
	ОбновитьБухучетРезультатовРасчета();
КонецПроцедуры
...
Функция ПроверитьПересечениеПериодовОтсутствияНаСервере()
	...
	КадровыеДанные = Новый ФиксированнаяСтруктура;
	
	ДанныеСостоянийСотрудников = Документы.БольничныйЛист.ДанныеСостоянийСотрудников(Объект, ДругиеСотрудники, Кэш);
	
	Если Кэш["КадровыеДанные"] <> Неопределено Тогда
		КадровыеДанные = Новый ФиксированнаяСтруктура(Кэш["КадровыеДанные"]);
	КонецЕсли;
	...
	ИначеЕсли Не ОснованиеПрогулНеявка.Пустая() Тогда
		ИсправленныйДокумент = ОснованиеПрогулНеявка;
	КонецЕсли;
	
	Возврат СостоянияСотрудников.ПроверитьПересечениеПериодовОтсутствия(
		ДанныеСостоянийСотрудников, Объект.Ссылка, ИсправленныйДокумент);
КонецФункции
...
Процедура ЗаполнитьПоКадровымДаннымСотрудника()
	...
	Если Не ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения) Или ДанныеСотрудника.ДатаУвольнения < Объект.ДатаНачалаСобытия Тогда
		...
		Если ТекущийДоговор = Неопределено Тогда
			УстановитьПривилегированныйРежим(Истина);
			БылДоговор = СЭДОФСС.ДоговорФизлицаНаДатуСобытия(Объект.ГоловнаяОрганизация, Объект.ФизическоеЛицо, Объект.ДатаНачалаСобытия);
			УстановитьПривилегированныйРежим(Ложь);
			Объект.СотрудникДоговорник = БылДоговор <> Неопределено И БылДоговор.ГПХ;
		Иначе
...
Процедура УстановитьДоступностьПолейВводаНачислений()
	...
	ДоступностьВидаОплатыЗаСчетРаботодателя = ДоступностьВидаОплатыЗаСчетРаботодателя();
...
Процедура ЗаполнитьПараметрыОплаты(Знач АвтозаполнениеПериодовОплаты, Знач Кэш = Неопределено)
	...
	КонецЕсли;
	Документы.БольничныйЛист.ЗаполнитьПараметрыОплаты(Объект, Кэш);
	Если Объект.КодПричиныНетрудоспособности = "05" Тогда
		ЗаполнитьПериодРасчетаСреднегоЗаработка();
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиМенеджераРасчета(МенеджерРасчета, ПериодРасчетаЗарплаты, Кэш, СохранятьИсправления = Истина)
	...
	МенеджерРасчета.НастройкиБухучета.НастройкиБухучетаДокумента = Документы.БольничныйЛист.ДанныеДляБухучетаЗарплатыПервичныхДокументов(ДокументОбъект)["ТаблицаБухучетЗарплаты"];
...
Процедура РеквизитыВДанные(ТекущийОбъект)
	...
	Если ЗначениеЗаполнено(КадровыеДанные) Тогда
		ТекущийОбъект.ДополнительныеСвойства.Вставить("КадровыеДанные", КадровыеДанные);
	КонецЕсли;
...
Процедура ОбновитьЭлементыСпособРасчетовЗаСчетРаботодателя()
	...
	Показывать = Не РаботаВБюджетномУчреждении И Не Объект.ЯвляетсяПродолжениемБолезни И Объект.СотрудникДоговорник
		И Объект.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ОбщееЗаболевание;
		
	Если Показывать Тогда
		Показывать = Объект.ДатаНачалаОплаты < ДатаНачалаОплатыЗаСчетСФР
	КонецЕсли;
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"СпособРасчетовСФизическимиЛицами", "Видимость", Показывать);

КонецПроцедуры

&НаСервере
Функция ДоступностьВидаОплатыЗаСчетРаботодателя()

	Возврат Объект.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ОбщееЗаболевание
...
Процедура НастройкиБухучетаДанныеВРеквизиты()
	...
	ОписаниеНастроекБухучета = Новый Структура("ДоляРаботодателя,Доплата");
	...
	Для каждого СтрокаТЧ Из НастройкиБухучета Цикл
		...
		Если СтрокаТЧ.Начисление = Объект.ВидОплатыЗаСчетРаботодателя Тогда
			ОписаниеНастроекБухучета.ДоляРаботодателя.Добавить(Новый ФиксированнаяСтруктура(СтрокаНастроек));
		ИначеЕсли СтрокаТЧ.Начисление = Объект.ВидРасчетаДоплаты Тогда
			ОписаниеНастроекБухучета.Доплата.Добавить(Новый ФиксированнаяСтруктура(СтрокаНастроек));
		КонецЕсли;
	КонецЦикла;
...
Процедура НастройкиБухучетаРеквизитыВДанные(ТекущийОбъект)
	...
	ОписаниеНастроекБухучета = Новый Структура("ДоляРаботодателя,Доплата");
	ОписаниеНастроекБухучета.ДоляРаботодателя 	= Объект.ВидОплатыЗаСчетРаботодателя;
	ОписаниеНастроекБухучета.Доплата 			= Объект.ВидРасчетаДоплаты;
...
Процедура УстановитьДоступностьВидимостьНастроекБухучета()
	...
	УстановитьВидимостьБухучетДолиРаботодателя();
	УстановитьСтраницуНастроекБухучетаДоляРаботодателя(ЭтотОбъект);
	УстановитьВидимостьБухучетДоплаты();
	УстановитьСтраницуНастроекБухучетаДоплата(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСтраницуНастроекБухучетаДоляРаботодателя(Форма)

	Если Форма.ДоляРаботодателяВариантУказанияБухучета = 0 Тогда
		Форма.Элементы.ДоляРаботодателяСтраницыБухучет.ТекущаяСтраница = Форма.Элементы.ДоляРаботодателяСтраницаБухучет;
	Иначе
		Форма.Элементы.ДоляРаботодателяСтраницыБухучет.ТекущаяСтраница = Форма.Элементы.ДоляРаботодателяСтраницаБухучетРаспределение;
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьБухучетДолиРаботодателя()

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ГруппаДоляРаботодателя",
		"Видимость",
		ДоступностьВидаОплатыЗаСчетРаботодателя());

КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьБухучетДоплаты()

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ГруппаДоплатаБухучет",
		"Видимость",
		Объект.ДоплачиватьДоСреднегоЗаработка);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСтраницуНастроекБухучетаДоплата(Форма)

	Если Форма.ДоплатаВариантУказанияБухучета = 0 Тогда
		Форма.Элементы.ДоплатаСтраницыБухучет.ТекущаяСтраница = Форма.Элементы.ДоплатаСтраницаБухучет;
	Иначе
		Форма.Элементы.ДоплатаСтраницыБухучет.ТекущаяСтраница = Форма.Элементы.ДоплатаСтраницаБухучетРаспределение;
	КонецЕсли;
