...
Процедура ПродлениеОсновногоОтпускаЗавершение(Результат, КоличествоДнейПродления) Экспорт
	...
	Иначе
		...
		"НадписьПродлитьОтпуск",
		"Заголовок",
		СтрШаблон(НСтр("ru='Перенести дату окончания основного отпуска на %1 дн...'"), КоличествоДнейПродления));
		ПерезаполнитьВидыОтпусковПриПриостановке();
...
Процедура ПродлениеДопОтпускаЗавершение(Результат, ПродлениеДопОтпуска) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			"НадписьПродлитьОтпуск", 
			"Заголовок", 
			СтрШаблон(НСтр("ru='Отменить перенос даты окончания дополнительного отпуска на %1 дн...'"), 
	...
	Иначе
		...
		"НадписьПродлитьОтпуск",
		"Заголовок",
		СтрШаблон(НСтр("ru='Перенести дату окончания дополнительного отпуска на %1 дн...'"), ПродлениеДопОтпуска.КоличествоДнейПересечения));
		ПерезаполнитьВидыОтпусковПриПриостановке();
	КонецЕсли;
	ПерезаполнитьПериодыПересечения();
КонецПроцедуры
...
&НаКлиенте
Процедура ДополнительныеОтпускаОснованиеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДополнительныеОтпускаОснованиеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
...
Процедура ИзменениеПериодаПриостановки(Элемент)
	...
	ИзмененоНачалоПериода = Элемент = Элементы.ПериодыПриостановкиОтпускаДатаНачалаПериода;
	ТекущиеДанные = Элементы.ПериодыПриостановкиОтпуска.ТекущиеДанные;
		
	Если ИзмененоНачалоПериода И ТекущиеДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончанияПериода) И ТекущиеДанные.ДатаНачалаПериода > ТекущиеДанные.ДатаОкончанияПериода Тогда
			ТекущиеДанные.ДатаОкончанияПериода = ТекущиеДанные.ДатаНачалаПериода;
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(ТекущиеДанные.ДатаНачалаПериода) И ТекущиеДанные.ДатаНачалаПериода > ТекущиеДанные.ДатаОкончанияПериода Тогда
			ТекущиеДанные.ДатаНачалаПериода = ТекущиеДанные.ДатаОкончанияПериода;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПериодыПриостановкиОтпускаПослеУдаления(Элемент)
	ПериодыПриостановкиОтпускаПриОкончанииРедактированияНаСервере();
	ВыполнитьРасчетИПерезаполнениеНачислений();
КонецПроцедуры
...
Процедура ЗаполнитьОтсутствияПоДругимМестамРаботы()
	...
	КадровыйУчетФормыРасширенный.ОтсутствияПоДругимМестамРаботы(
		ЭтотОбъект, ГоловнаяОрганизация, ПериодОтсутствия.НачалоПериода, ПериодОтсутствия.ОкончаниеПериода);
КонецПроцедуры
...
&НаСервере
Процедура ПериодыПриостановкиОтпускаПриОкончанииРедактированияНаСервере()
	
	Если Элементы.ПериодыПриостановкиОтпуска.ТекущаяСтрока <> Неопределено Тогда 
		ТекущаяСтрока = Объект.ПериодыПриостановкиОтпуска.НайтиПоИдентификатору(Элементы.ПериодыПриостановкиОтпуска.ТекущаяСтрока);
		Документы.Отпуск.ЗаполнитьКоличествоДнейПриостановки(Объект, ТекущаяСтрока);
	КонецЕсли;
...
&НаСервере
Процедура ПерезаполнитьВидыОтпусковПриПриостановке()
	
	Если Не ПолучитьФункциональнуюОпцию("ПересчитыватьПродолжительностьВидовОтпусковПриИсправленииОтпуска") Тогда 
		Возврат;
	КонецЕсли;
	...
	ОкончаниеОтпуска = ?(Объект.ПредоставитьОсновнойОтпуск, Объект.ДатаОкончанияОсновногоОтпуска, Дата(1, 1, 1));
	КоличествоДнейОтпуска = ?(Объект.ПредоставитьОсновнойОтпуск, Объект.КоличествоДнейОсновногоОтпуска, 0);
	Если Объект.ПредоставитьДополнительныйОтпуск Тогда
		Для Каждого ДополнительныйОтпуск Из Объект.ДополнительныеОтпуска Цикл
			Если Не ЗначениеЗаполнено(ДополнительныйОтпуск.ДатаНачала) Или Не ЗначениеЗаполнено(ДополнительныйОтпуск.ДатаОкончания) Тогда 
				Продолжить;
			КонецЕсли;
			ОкончаниеОтпуска = Макс(ОкончаниеОтпуска, ДополнительныйОтпуск.ДатаОкончания);
			КоличествоДнейОтпуска = КоличествоДнейОтпуска + ДополнительныйОтпуск.КоличествоДней;
		КонецЦикла;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОкончаниеОтпуска) Или КоличествоДнейОтпуска = 0 Тогда 
		Возврат;
	КонецЕсли;
	
	ПериодыПриостановки = Объект.ПериодыПриостановкиОтпуска.Выгрузить();
	ПериодыПриостановки.Сортировать("ДатаНачалаПериода");
	ПериодыПриостановки.Колонки.Добавить("ДнейОтпуска");
	
	СтрокиКУдалению = Новый Массив;
	Для Каждого ПериодПриостановки Из ПериодыПриостановки Цикл
		Если Не ЗначениеЗаполнено(ПериодПриостановки.ДатаНачалаПериода) 
			Или Не ЗначениеЗаполнено(ПериодПриостановки.ДатаОкончанияПериода)
			Или ПериодПриостановки.ДатаНачалаПериода > ОкончаниеОтпуска Тогда 
			СтрокиКУдалению.Добавить(ПериодПриостановки);
			Продолжить;
		КонецЕсли;
		ПериодПриостановки.ДатаОкончанияПериода = Мин(ОкончаниеОтпуска, ПериодПриостановки.ДатаОкончанияПериода);
		ПериодПриостановки.ДнейОтпуска = УчетРабочегоВремениРасширенный.ДлительностьИнтервала(
			Объект.Сотрудник,
			ПериодПриостановки.ДатаНачалаПериода,
			ПериодПриостановки.ДатаОкончанияПериода,
			ОписаниеВидаОтпуска.СпособРасчетаПоКалендарнымДням,
	...
	Для Каждого СтрокаКУдалению Из СтрокиКУдалению Цикл
		ПериодыПриостановки.Удалить(СтрокаКУдалению);
	КонецЦикла;
	
	КоличествоДнейОтпуска = КоличествоДнейОтпуска - ПериодыПриостановки.Итог("ДнейОтпуска");
	...
	ВидыОтпусковДокумента = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ВидОтпуска");
	Если ВидыОтпусковДокумента.Количество() <= 1 Тогда 
		Возврат;
	КонецЕсли;
	
	ПараметрыОстатков = ОстаткиОтпусков.ОписаниеПараметровДляОстаткиОтпусков();
	ПараметрыОстатков.Сотрудники = Объект.Сотрудник;
	ПараметрыОстатков.ДатаОстатков = Объект.ДатаНачалаСобытия - 86400;
	ПараметрыОстатков.ИсключаемыеРегистраторы = Объект.Ссылка;
	ПараметрыОстатков.ВидыОтпусков = ВидыОтпусковДокумента;
	...
	Иначе
		Объект.ПредоставитьОсновнойОтпуск = Истина;
		Объект.ДатаНачалаОсновногоОтпуска = Объект.ДатаНачалаСобытия;
		Объект.КоличествоДнейОсновногоОтпуска = ПродолжительностьОтпуска;
		Объект.ДатаОкончанияОсновногоОтпуска = ОстаткиОтпусковКлиентСервер.ДатаОкончанияОтпуска(Объект.Сотрудник, Объект.ДатаНачалаОсновногоОтпуска, Объект.КоличествоДнейОсновногоОтпуска, ОписаниеВидаОтпуска);
		Для Каждого ПериодПриостановки Из ПериодыПриостановки Цикл
			Если ПериодПриостановки.ДатаОкончанияПериода >= Объект.ДатаНачалаОсновногоОтпуска 
				И Объект.ДатаОкончанияОсновногоОтпуска >= ПериодПриостановки.ДатаНачалаПериода Тогда
				Объект.КоличествоДнейОсновногоОтпуска = Объект.КоличествоДнейОсновногоОтпуска + ПериодПриостановки.ДнейОтпуска;
				Объект.ДатаОкончанияОсновногоОтпуска = ОстаткиОтпусковКлиентСервер.ДатаОкончанияОтпуска(Объект.Сотрудник, Объект.ДатаНачалаОсновногоОтпуска, Объект.КоличествоДнейОсновногоОтпуска, ОписаниеВидаОтпуска);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ВидыОтпусков Цикл
		...
		НоваяСтрока.ЭтоЕжегодныйОтпуск = ОписаниеВидаОтпуска.ЕжегодныйОтпуск;
		Для Каждого ПериодПриостановки Из ПериодыПриостановки Цикл
			Если ПериодПриостановки.ДатаОкончанияПериода >= НоваяСтрока.ДатаНачала 
				И НоваяСтрока.ДатаОкончания >= ПериодПриостановки.ДатаНачалаПериода Тогда
				НоваяСтрока.КоличествоДней = НоваяСтрока.КоличествоДней + ПериодПриостановки.ДнейОтпуска;
				НоваяСтрока.ДатаОкончания = ОстаткиОтпусковКлиентСервер.ДатаОкончанияОтпуска(Объект.Сотрудник, НоваяСтрока.ДатаНачала, НоваяСтрока.КоличествоДней, ОписаниеВидаОтпуска);
			КонецЕсли;
		КонецЦикла;
		ДополнительныеОтпускаДатаНачалаПриИзмененииНаСервере(НоваяСтрока.ПолучитьИдентификатор(), Ложь);
	...
	Если Объект.ПредоставитьДополнительныйОтпуск Тогда 
		ПоследняяСтрока = Объект.ДополнительныеОтпуска[Объект.ДополнительныеОтпуска.Количество()-1];
		Если ПоследняяСтрока.ДатаОкончания <> ОкончаниеОтпуска Тогда 
			ПоследняяСтрока.ДатаОкончания = ОкончаниеОтпуска;
			ПоследняяСтрока.КоличествоДней = УчетРабочегоВремениРасширенный.ДлительностьИнтервала(
				Объект.Сотрудник,
				ПоследняяСтрока.ДатаНачала,
				ПоследняяСтрока.ДатаОкончания,
				ОписаниеВидаОтпуска.СпособРасчетаПоКалендарнымДням,
				ОписаниеВидаОтпуска.ЕжегодныйОтпуск);
		КонецЕсли;
	ИначеЕсли Объект.ПредоставитьОсновнойОтпуск Тогда
		Если Объект.ДатаОкончанияОсновногоОтпуска <> ОкончаниеОтпуска Тогда 
			Объект.ДатаОкончанияОсновногоОтпуска = ОкончаниеОтпуска;
			Объект.КоличествоДнейОсновногоОтпуска = УчетРабочегоВремениРасширенный.ДлительностьИнтервала(
				Объект.Сотрудник,
				Объект.ДатаНачалаОсновногоОтпуска,
				Объект.ДатаОкончанияОсновногоОтпуска,
				ОписаниеВидаОтпуска.СпособРасчетаПоКалендарнымДням,
				ОписаниеВидаОтпуска.ЕжегодныйОтпуск);
		КонецЕсли;
