...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект)
	...
	ВывестиНедостающиеСведенияОднимПолемПриНеобходимости();
...
&НаСервере
Процедура ВывестиНедостающиеСведенияОднимПолемПриНеобходимости()
	// Поведение по умолчанию.
	Элементы.НедостающиеСведения.Видимость = Истина;
	ИмяНадписи = "НедостающиеСведенияСтрокой";
	ПолеНадписи = Элементы.Найти(ИмяНадписи);
	Если ПолеНадписи <> Неопределено Тогда
		ПолеНадписи.Видимость = Ложь;
	КонецЕсли;
	// Проверки перед получением XML.
	Если Не Объект.Загружен Или Не ЗначениеЗаполнено(Объект.ИдентификаторСообщения) Тогда
		Возврат; // Не загружено.
	КонецЕсли;
	ВсеСведенияПомещаются = Истина;
	Для Каждого СтрокаТаблицы Из Объект.НедостающиеСведения Цикл
		Если СтрДлина(СтрокаТаблицы.Наименование) > 50
			Или СтрДлина(СтрокаТаблицы.Поле) > 50
			Или СтрДлина(СтрокаТаблицы.Описание) > 100 Тогда
			ВсеСведенияПомещаются = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если ВсеСведенияПомещаются Тогда
		Возврат; // Нет необходимости выводить одним полем.
	КонецЕсли;
	НедостающиеСведенияОднойСтрокой = НедостающиеСведенияОднойСтрокой();
	Если Не ЗначениеЗаполнено(НедостающиеСведенияОднойСтрокой) Тогда
		Возврат; // Вероятно, недоступен исходный XML.
	КонецЕсли;
	Если ПолеНадписи = Неопределено Тогда
		Родитель = Элементы.НедостающиеСведенияГруппа;
		ПолеНадписи = Элементы.Вставить(ИмяНадписи, Тип("ДекорацияФормы"), Родитель, Элементы.НедостающиеСведения);
	...
	КонецЕсли;
	ПолеНадписи.Видимость = Истина;
	ПолеНадписи.Заголовок = НедостающиеСведенияОднойСтрокой;
	Элементы.НедостающиеСведения.Видимость = Ложь;
КонецПроцедуры

&НаСервере
Функция НедостающиеСведенияОднойСтрокой()
	// Получение XML и структуры документа из XML.
	ВходящееСообщение = СЭДОФСС.ВходящееСообщение(Объект.ИдентификаторСообщения);
	Если ТипЗнч(ВходящееСообщение.Содержимое) <> Тип("ХранилищеЗначения")
		Или (Не ЗначениеЗаполнено(ВходящееСообщение.ДатаЗагрузки) И Не ЗначениеЗаполнено(ВходящееСообщение.Дата)) Тогда
		Возврат ""; // Не загружено.
	КонецЕсли;
	ТекстXML = ВходящееСообщение.Содержимое.Получить();
	Результат = Новый Структура("Обработано,ОписаниеОшибки,ОшибкаОбработки", Ложь, "", Ложь);
	СтруктураДокумента = Документы.ЗапросНедостающихСведенийПособияСФР.СтруктураДокументаИзXML(ТекстXML, Результат);
	Если СтруктураДокумента = Неопределено Или СтруктураДокумента.Пособия = Неопределено Тогда
		Возврат ""; // Нет данных.
	КонецЕсли;
	// Таблица пособий может содержать несколько строк в некоторых случаях.
	Количество = СтруктураДокумента.Пособия.Количество();
	Если Количество = 0 Тогда
		Возврат ""; // Нет данных.
	ИначеЕсли Количество = 1 Тогда
		Пособия = СтруктураДокумента.Пособия;
	Иначе
		Фильтр = Новый Структура("НомерСтрокиСообщения", Объект.НомерСтрокиСообщения);
		Пособия = СтруктураДокумента.Пособия.Скопировать(Фильтр);
		Если Пособия.Количество() = 0 Тогда
			Пособия = СтруктураДокумента.Пособия;
		КонецЕсли;
	КонецЕсли;
	СтрокиПоПособиям = Новый Массив;
	Для Каждого ИнформацияОПособии Из Пособия Цикл
		НедостающиеСведения = ИнформацияОПособии.НедостающиеСведения;
		Если НедостающиеСведения = Неопределено Или НедостающиеСведения.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		Строки = Новый Массив;
		Описания = КоллекцииБЗК.УникальныеЗначенияКолонки(НедостающиеСведения, "Описание");
		Для Каждого Описание Из Описания Цикл
			Найденные = НедостающиеСведения.НайтиСтроки(Новый Структура("Описание", Описание));
			Для Каждого НедостающийРеквизит Из Найденные Цикл
				Если ЗначениеЗаполнено(НедостающийРеквизит.Наименование) Тогда
					Строки.Добавить(СтрШаблон(НСтр("ru = 'Наименование: %1'"), НедостающийРеквизит.Наименование));
				КонецЕсли;
				Если ЗначениеЗаполнено(НедостающийРеквизит.Поле) Тогда
					Строки.Добавить(СтрШаблон(НСтр("ru = 'Поле: %1'"), НедостающийРеквизит.Поле));
				КонецЕсли;
			КонецЦикла;
			Если ЗначениеЗаполнено(Описание) Тогда
				Строки.Добавить(СтрШаблон(НСтр("ru = 'Текст запроса: %1'"), Описание));
			КонецЕсли;
		КонецЦикла;
		Если Строки.Количество() > 0 Тогда
			СтрокиПоПособиям.Добавить(СтрСоединить(Строки, Символы.ПС));
		КонецЕсли;
	КонецЦикла;
	Возврат СтрСоединить(СтрокиПоПособиям, Символы.ПС + Символы.ПС);
КонецФункции
