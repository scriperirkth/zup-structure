...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДокументыВРаботе = Параметры.ДокументыВРаботе;
	...
	Если Не ОбщегоНазначенияБЗК.ЕстьСохраненныеНастройкиФормы(ЭтотОбъект) Тогда
		Если ПоказыватьОрганизации И Не ЗначениеЗаполнено(Организация) И Параметры.Отбор.Количество() = 0 Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_ЗапросНедостающихСведенийПособияСФР"
...
Процедура СписокПриИзменении(Элемент)
	Оповестить("Запись_ЗапросНедостающихСведенийПособияСФР", Новый Структура, Неопределено);
...
Процедура ЗадаватьВопросОПодвержденииПолучения(Команда)
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Включено напоминание о необходимости отправки подтверждения получения перед открытием извещений'"));
КонецПроцедуры
...
Процедура ОбновитьФорму()
	Если ПравоДоступа("Изменение", Метаданные.Документы.ЗапросНедостающихСведенийПособияСФР) Тогда
