...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Объект.ГодПостановкиНаУчет = Год(ТекущаяДатаСеанса()) + 1;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ПервоначальнаяПостановкаНаВоинскийУчет", ПараметрыЗаписи, Объект.Ссылка);
...
&НаКлиенте
Процедура ГодПостановкиНаУчетПриИзменении(Элемент)
...
Процедура ЗаполнитьДанныеСтрокиСотрудника(СтрокаСотрудника, КадровыеДанные)
	...
	Документы.ПервоначальнаяПостановкаНаВоинскийУчет.ЗаполнитьДанныеСтрокиСотрудника(СтрокаСотрудника, КадровыеДанные, Объект.Дата);
...
Функция КадровыеДанныеСотрудников(СписокСотрудников)
	...
	КадровыеДанные = Документы.ПервоначальнаяПостановкаНаВоинскийУчет.КадровыеДанныеСотрудников(ОсновныеСотрудники);
...
Процедура ЗаполнитьНаСервере()
	...
	ПараметрыПолученияСотрудников.КадровыеДанные = "Организация, ФИОПолные, ДатаРождения, Пол";
	...
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(
		ПараметрыПолученияСотрудников.Отборы, "Пол", "=", "ЗНАЧЕНИЕ(Перечисление.ПолФизическогоЛица.Мужской)");
	
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(
		ПараметрыПолученияСотрудников.Отборы, "ДатаРождения", ">=", ДобавитьМесяц(Объект.НачалоПериода, -17*12));
	
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(
		ПараметрыПолученияСотрудников.Отборы, "ДатаРождения", "<=", ДобавитьМесяц(Объект.ОкончаниеПериода, -17*12));
	...
	КадровыеДанныеФизическихЛиц = Документы.ПервоначальнаяПостановкаНаВоинскийУчет.КадровыеДанныеСотрудников(ОсновныеСотрудники);
...
Процедура ЗаполнитьОтчетныйПериод(Форма)
	...
	Объект = Форма.Объект;
	Если Не ЗначениеЗаполнено(Объект.ГодПостановкиНаУчет) Тогда 
		Возврат;
	КонецЕсли;
	
	Объект.НачалоПериода = Дата(Объект.ГодПостановкиНаУчет, 1, 1);
