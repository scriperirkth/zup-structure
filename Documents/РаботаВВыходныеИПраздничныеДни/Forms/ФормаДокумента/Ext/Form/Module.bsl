...
Процедура СотрудникиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Копирование Тогда
			ИдентификаторКопируемойСтроки = Элемент.ТекущиеДанные.ИдентификаторСтрокиСотрудника;
		КонецЕсли;
		
		РасчетЗарплатыРасширенныйКлиентСервер.ЗаполнитьИдентификаторСтрокиСотрудника(ЭтотОбъект, Элемент.ТекущиеДанные);
		Если ИдентификаторКопируемойСтроки <> Неопределено Тогда
			ЗаполнитьСвязанныеКоллекцииКопируемойСтроки(ИдентификаторКопируемойСтроки, Элемент.ТекущиеДанные.ИдентификаторСтрокиСотрудника);
		КонецЕсли;
		
		Для Каждого ДеньРаботы Из СписокДат Цикл
			ТекущиеДанные = Элемент.ТекущиеДанные;
			ТекущиеДанные[ИмяКолонкиРаботалСтрока(ДеньРаботы.Значение)] = Истина;
			ТекущиеДанные[ИмяКолонкиЧасыПодробноСтрока(ДеньРаботы.Значение)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтаФорма, ДеньРаботы.Значение, ТекущиеДанные.ИдентификаторСтрокиСотрудника);
		КонецЦикла;
...
Процедура СотрудникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПрефиксКолонкиЧасыПодробно = ИмяКолонкиЧасыПодробноСтрока("");
	Если ПрефиксКолонкиЧасыПодробно <> Поле.Имя И СтрНайти(Поле.Имя, ПрефиксКолонкиЧасыПодробно) <> 0 Тогда
	
		Дата = Дата(Прав(Поле.Имя, 8));
		СтрокаТаблицыСотрудники = Сотрудники.НайтиПоИдентификатору(ВыбраннаяСтрока);
		
		ПараметрыРедактирования = Новый Структура("Дата, Сотрудник, ИдентификаторСтрокиСотрудника, ДанныеОВремени");
		ЗаполнитьЗначенияСвойств(ПараметрыРедактирования, СтрокаТаблицыСотрудники);
		ПараметрыРедактирования.Дата = Дата;
		
		Отбор = Новый Структура("Дата, ИдентификаторСтрокиСотрудника", Дата, СтрокаТаблицыСотрудники.ИдентификаторСтрокиСотрудника);
		ЗаполнитьПараметрыРедактированияПодробногоВремени(Отбор, ПараметрыРедактирования);
		
		Оповещение = Новый ОписаниеОповещения("ОткрытьПодробныйВводВремениЗавершение", ЭтотОбъект);
		ОткрытьФорму("ОбщаяФорма.ВводДополнительногоРабочегоВремени", ПараметрыРедактирования, ЭтаФорма,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте
Процедура ОткрытьПодробныйВводВремениЗавершение(РезультатРедактирования, ДополнительныеПараметры) Экспорт
	
	Если РезультатРедактирования <> Неопределено И РезультатРедактирования.Модифицированность Тогда
		
		Модифицированность = Истина;
		
		ПеренестиРезультатРедактированияВДокумент(РезультатРедактирования);
		//Обновление представления поля выбора
		ТекущиеДанные = Элементы.Сотрудники.ТекущиеДанные;
		ТекущиеДанные[ИмяКолонкиЧасыПодробноСтрока(РезультатРедактирования.Дата)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтаФорма, РезультатРедактирования.Дата, РезультатРедактирования.ИдентификаторСтрокиСотрудника);
...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект = Неопределено)
	...
	ПодробныйВводВремени = НастройкиРасчетаЗарплаты.ИспользоватьПодробныйВводВремениПраздничныхИВыходных;
	...
	РасчетЗарплатыРасширенныйФормы.ЗаполнитьСуточныеВидыВремени(ЭтотОбъект);
...
Процедура ОбработкаПодбораНаСервере(МассивСотрудников)
	...
	Для Каждого Сотрудник Из МассивСотрудников Цикл
		Если Сотрудники.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник)).Количество() = 0 Тогда
			...
			РасчетЗарплатыРасширенныйКлиентСервер.ЗаполнитьИдентификаторСтрокиСотрудника(ЭтотОбъект, СтрокаТаблицы);
			...
			Для Каждого ДеньРаботы Из СписокДат Цикл 
				...
				СтрокаТаблицы[ИмяКолонкиЧасыПодробноСтрока(ДеньРаботы.Значение)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтаФорма, ДеньРаботы.Значение, СтрокаТаблицы.ИдентификаторСтрокиСотрудника)
			КонецЦикла; 
			...
			НоваяСтрокаПодробно = ДанныеОВремениПодробно.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПодробно, СтрокаТаблицы);
...
Процедура ДобавитьКолонкиТаблицыСотрудники(МассивДат)
	...
	Для Каждого ДатаРаботы Из МассивДат Цикл 
		...
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы(
				...
				Формат(ДатаРаботы, "ДЛФ=Д"), 
				Истина));		
		// Подробный ввод времени
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы(
				ИмяКолонкиЧасыПодробноСтрока(ДатаРаботы), 
	...
	Для Каждого РеквизитФормы Из ДобавляемыеРеквизиты Цикл 
		...
		ИначеЕсли РеквизитФормы.ТипЗначения = Новый ОписаниеТипов("Строка") Тогда
			СледующийЭлемент = СледующийЭлементТаблицыСотрудники(РеквизитФормы.Имя, Элементы.СотрудникиДатыЧасыПодробно, "ЧасыПодробно");
			Элемент = Элементы.Вставить(РеквизитФормы.Имя, Тип("ПолеФормы"), Элементы.СотрудникиДатыЧасыПодробно, СледующийЭлемент);
...
Процедура ДниРаботыПриИзмененииНаСервере()
	...
	Для Каждого РеквизитТаблицы Из РеквизитыТаблицыСотрудники Цикл
		Если Лев(РеквизитТаблицы.Имя, 4) = "Дата" Или Лев(РеквизитТаблицы.Имя, 7) = "Работал" Или Лев(РеквизитТаблицы.Имя, 12) = "ЧасыПодробно" Тогда 
	...
	Для Каждого ЭлементСписка Из СписокДат Цикл 
		...
		ИмяКолонки = ИмяКолонкиЧасыПодробноСтрока(ДатаРаботы);
		Если ИменаКолонок.Свойство(ИмяКолонки) Тогда
			ИменаКолонок.Удалить(ИмяКолонки);
		ИначеЕсли ДобавленныеДаты.Найти(ДатаРаботы) = Неопределено Тогда 
			ДобавленныеДаты.Добавить(ДатаРаботы);
...
Процедура ЗаполнитьОбъектПоДаннымФормы(ТекущийОбъект)
	...
	Для Каждого СтрокаТаблицы Из Сотрудники Цикл
		...
		Для Каждого ДеньРаботы Из СписокДат Цикл
				...
				Если ПодробныйВводВремени И ТекущийОбъект.ВремяВЧасах Тогда
					СтрокиПодробно = ДанныеОВремениПодробно.НайтиСтроки(Новый Структура("Дата, ИдентификаторСтрокиСотрудника", ДеньРаботы.Значение, СтрокаТаблицы.ИдентификаторСтрокиСотрудника));
				КонецЕсли;
					
				Если СтрокиПодробно.Количество() > 0 Тогда
					
					Для Каждого Стр Из СтрокиПодробно Цикл
				...
				Иначе
					...
					Если ТекущийОбъект.ВремяВЧасах Тогда
						Если ПодробныйВводВремени Тогда
					...
					КонецЕсли;
					НоваяСтрока.ИдентификаторСтрокиСотрудника = СтрокаТаблицы.ИдентификаторСтрокиСотрудника;
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы, ПоляБухучета);
...
Процедура ЗаполнитьФормуПоДаннымОбъекта(ТекущийОбъект)
	...
	СписокДат.Очистить();
	Сотрудники.Очистить();
	ДанныеОВремениПодробно.Очистить();
	...
	ЗаполнятьИдентификаторСтрокиСотрудника = МаксимальныйИдентификаторСтрокиСотрудника = 0;
	...
	Для Каждого СтрокаТЧ Из ТекущийОбъект.Сотрудники Цикл
		...
		Если СтрокиСотрудника = Неопределено Тогда
			...
			СтрокиСотрудникаПоИдентификатору = Новый Соответствие;
			СтрокиСотрудника = Новый Соответствие;
			СтрокиСотрудникаПоИдентификатору.Вставить(СтрокаТЧ.СпособКомпенсацииПереработки, СтрокиСотрудникаПоСпособуКомпенсации);
			СтрокиСотрудника.Вставить(СтрокаТЧ.ИдентификаторСтрокиСотрудника, СтрокиСотрудникаПоИдентификатору);
			СтрокиСотрудников.Вставить(СтрокаТЧ.Сотрудник, СтрокиСотрудника);
		Иначе
			СтрокиСотрудникаПоИдентификатору = СтрокиСотрудника[СтрокаТЧ.ИдентификаторСтрокиСотрудника];
			Если СтрокиСотрудникаПоИдентификатору = Неопределено Тогда
				СтрокиСотрудникаПоИдентификатору = Новый Соответствие;
				СтрокиСотрудника.Вставить(СтрокаТЧ.ИдентификаторСтрокиСотрудника, СтрокиСотрудникаПоИдентификатору);
			КонецЕсли;
			СтрокиСотрудникаПоСпособуКомпенсации = СтрокиСотрудникаПоИдентификатору[СтрокаТЧ.СпособКомпенсацииПереработки];
	...
	Для Каждого СотрудникДокумента Из СотрудникиДокумента Цикл
		...
		СтрокиСотрудникаПоИдентификатору = СтрокиСотрудников[СотрудникДокумента];	
		Для Каждого СтрокиИдентификатора Из СтрокиСотрудникаПоИдентификатору Цикл
			
			СтрокиСотрудника = СтрокиИдентификатора.Значение;
			Для Каждого СтрокиПоСпособуКомпенсации Из СтрокиСотрудника Цикл
				...
				Если ЗаполнятьИдентификаторСтрокиСотрудника Тогда
					РасчетЗарплатыРасширенныйКлиентСервер.ЗаполнитьИдентификаторСтрокиСотрудника(ЭтотОбъект, НоваяСтрока);
				КонецЕсли;
				...
				Для Каждого СтрокаТЧ Из СтрокиПоСпособуКомпенсации.Значение Цикл
					...
					Если ЗначениеЗаполнено(СтрокаТЧ.Дата) Тогда
						...
						НоваяСтрока[ИмяКолонкиДатаСтрока(СтрокаТЧ.Дата)] = НоваяСтрока[ИмяКолонкиДатаСтрока(СтрокаТЧ.Дата)] + СтрокаТЧ.ОтработаноЧасов;
						НоваяСтрока[ИмяКолонкиРаботалСтрока(СтрокаТЧ.Дата)] = НЕ СтрокаТЧ.НеРаботал;
						
						Если СтрокаТЧ.ОтработаноЧасов > 0 Тогда
							НоваяСтрокаПодробно = ДанныеОВремениПодробно.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрокаПодробно, СтрокаТЧ);
							Если ЗаполнятьИдентификаторСтрокиСотрудника Тогда
								НоваяСтрокаПодробно.ИдентификаторСтрокиСотрудника = НоваяСтрока.ИдентификаторСтрокиСотрудника;
							КонецЕсли;
						КонецЕсли;
				...
				Для Каждого Дата Из СписокДат Цикл
					...
					Если ЗначениеЗаполнено(Дата.Значение) Тогда
						НоваяСтрока[ИмяКолонкиЧасыПодробноСтрока(Дата.Значение)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтаФорма, Дата.Значение, НоваяСтрока.ИдентификаторСтрокиСотрудника);
					КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовФормыПоСпособуВвода()
	...
	Элементы.СотрудникиДатыЧасы.Видимость = Не ВводПоДням И Не ПодробныйВводВремени;
	Элементы.СотрудникиДатыПризнак.Видимость = ВводПоДням;
	Элементы.СотрудникиДатыЧасыПодробно.Видимость = Не ВводПоДням И ПодробныйВводВремени;
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяКолонкиЧасыПодробноСтрока(ДатаДень) Экспорт
	Возврат "ЧасыПодробно" + Формат(ДатаДень, "ДФ=""ггггММдд""");
КонецФункции
...
&НаСервере
Процедура ЗаполнитьПараметрыРедактированияПодробногоВремени(ПараметрыОтбора, ПараметрыРедактирования)

	ПараметрыРедактирования.ДанныеОВремени = ПоместитьВоВременноеХранилище(ЭтаФорма.ДанныеОВремениПодробно.Выгрузить(ПараметрыОтбора));
	
КонецПроцедуры

&НаСервере
Процедура ПеренестиРезультатРедактированияВДокумент(РезультатРедактирования)
	
	ДанныеОВремени = ПолучитьИзВременногоХранилища(РезультатРедактирования.ДанныеОВремени);
	
	Отбор = Новый Структура("Дата, ИдентификаторСтрокиСотрудника", РезультатРедактирования.Дата, РезультатРедактирования.ИдентификаторСтрокиСотрудника);
	УдаляемыеСтроки = ЭтаФорма.ДанныеОВремениПодробно.НайтиСтроки(Отбор);
	Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		ЭтаФорма.ДанныеОВремениПодробно.Удалить(УдаляемаяСтрока);
	КонецЦикла;	
	
	Если ДанныеОВремени <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДанныеОВремени, ЭтаФорма.ДанныеОВремениПодробно);
	КонецЕсли;
			
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСвязанныеКоллекцииКопируемойСтроки(ПрежнийИдентификаторСтроки, НовыйИндентификаторСтроки)

	НайденныеСтроки = ДанныеОВремениПодробно.НайтиСтроки(Новый Структура("ИдентификаторСтрокиСотрудника", ПрежнийИдентификаторСтроки));
	
	Для Каждого Стр Из НайденныеСтроки Цикл
		НоваяСтрока = ДанныеОВремениПодробно.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр);
		Стр.ИдентификаторСтрокиСотрудника = НовыйИндентификаторСтроки;
	КонецЦикла;
