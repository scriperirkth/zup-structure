...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ВремяУчтено И ПолучитьФункциональнуюОпцию("ИспользоватьРасчетЗарплатыРасширенная") Тогда
		...
		УчетОтгулов.ЗарегистрироватьДанныеОбИспользованииОтгулов(Движения, ДанныеДляПроведения.ДанныеОбОтгулах);
...
Функция ДанныеДляПроведения()
	...
	ДанныеОбОтгулах.Колонки.Добавить("ОтработанныйДень", Новый ОписаниеТипов("Дата"));
	...
	ВидВремениПраздникиНочноеВремя = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиНочноеВремя");
	ВидВремениПраздникиБезОплатыНочноеВремя = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиБезПовышеннойОплатыНочноеВремя");
	ВидВремениПраздникиВечернееВремя = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиВечернееВремя");
	ВидВремениПраздникиБезОплатыВечернееВремя = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиБезПовышеннойОплатыВечернееВремя");
	...
	Для Каждого СтрокаТаблицы Из Сотрудники Цикл
		...
		Если СтрокаТаблицы.СпособКомпенсацииПереработки = ПредопределенноеЗначение("Перечисление.СпособыКомпенсацииПереработки.Отгул") Тогда 
			...
			Если СтрокаТаблицы.СуточныйВидВремени = Перечисления.ВидыВремениСуток.ВечернееВремя Тогда
				ПрисваиваемыйВидВремени = ВидВремениПраздникиБезОплатыВечернееВремя;
			ИначеЕсли СтрокаТаблицы.СуточныйВидВремени = Перечисления.ВидыВремениСуток.НочноеВремя Тогда
				ПрисваиваемыйВидВремени = ВидВремениПраздникиБезОплатыНочноеВремя;
			Иначе
			...
			НоваяСтрока.ОтработанныйДень = СтрокаТаблицы.Дата;
		...
		Иначе
			...
			Если СтрокаТаблицы.СуточныйВидВремени = Перечисления.ВидыВремениСуток.ВечернееВремя Тогда
				ПрисваиваемыйВидВремени = ВидВремениПраздникиВечернееВремя;
			ИначеЕсли СтрокаТаблицы.СуточныйВидВремени = Перечисления.ВидыВремениСуток.НочноеВремя Тогда
				ПрисваиваемыйВидВремени = ВидВремениПраздникиНочноеВремя;
			Иначе
	...
	ДанныеОбОтгулах.Свернуть("Организация, Сотрудник, Период, ВидДвижения, ОтработанныйДень", "Дни, Часы");
