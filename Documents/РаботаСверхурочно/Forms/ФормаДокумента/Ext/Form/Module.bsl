...
Процедура СотрудникиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		...
		Для Каждого ДеньРаботы Из ДниСверхурочнойРаботы Цикл
			ТекущиеДанные = Элемент.ТекущиеДанные;
			ТекущиеДанные[ИмяКолонкиЧасыПодробноСтрока(ДеньРаботы.Значение)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтотОбъект, ДеньРаботы.Значение, ТекущиеДанные.ИдентификаторСтрокиСотрудника);
		КонецЦикла;
...
Процедура ОткрытьПодробныйВводВремениЗавершение(РезультатРедактирования, ДополнительныеПараметры) Экспорт
	...
	Если РезультатРедактирования <> Неопределено И РезультатРедактирования.Модифицированность Тогда
		...
		ТекущиеДанные[ИмяКолонкиЧасыПодробноСтрока(РезультатРедактирования.Дата)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтотОбъект, РезультатРедактирования.Дата, РезультатРедактирования.ИдентификаторСтрокиСотрудника);
...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект = Неопределено)
	...
	ПодробныйВводВремени = НастройкиРасчетаЗарплаты.ИспользоватьПодробныйВводВремениСверхурочных;
...
Процедура ОбработкаПодбораНаСервере(МассивСотрудников)
	...
	Для Каждого Сотрудник Из МассивСотрудников Цикл
		Если Сотрудники.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник)).Количество() = 0 Тогда
			...
			Для Каждого ДеньРаботы Из ДниСверхурочнойРаботы Цикл 
				СтрокаТаблицы[ИмяКолонкиЧасыПодробноСтрока(ДеньРаботы.Значение)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтотОбъект, ДеньРаботы.Значение, СтрокаТаблицы.ИдентификаторСтрокиСотрудника)
			КонецЦикла;
...
Процедура ДобавитьКолонкиТаблицыСотрудники(МассивДат)
	...
	Для Каждого РеквизитФормы Из ДобавляемыеРеквизиты Цикл 
		...
		Если РеквизитФормы.ТипЗначения = Новый ОписаниеТипов("Строка") Тогда
		...
		Иначе
			СледующийЭлемент = СледующийЭлементТаблицыСотрудники(РеквизитФормы.Имя, Элементы.СотрудникиДаты, "Дата");
...
Процедура ДниСверхурочнойРаботыПриИзмененииНаСервере()
	...
	Для Каждого РеквизитТаблицы Из РеквизитыТаблицыСотрудники Цикл
		Если Лев(РеквизитТаблицы.Имя, 4) = "Дата" Или Лев(РеквизитТаблицы.Имя, 12) = "ЧасыПодробно" Тогда 
	...
	Для каждого СотрудникДокумента Из СотрудникиДокумента Цикл
		...
		Для Каждого СтрокиИдентификатора Из СтрокиСотрудникаПоИдентификатору Цикл
			...
			Для каждого СтрокиПоСпособуКомпенсации Из СтрокиСотрудника Цикл
				...
				Для Каждого Дата Из ДниСверхурочнойРаботы Цикл
					
					Если ЗначениеЗаполнено(Дата.Значение) Тогда
						НоваяСтрока[ИмяКолонкиЧасыПодробноСтрока(Дата.Значение)] = РасчетЗарплатыРасширенныйКлиентСервер.ПолучитьПредставлениеПоляПодробногоВводаВремени(ЭтотОбъект, Дата.Значение, НоваяСтрока.ИдентификаторСтрокиСотрудника);
					КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовФормыПоСпособуВвода()
	...
	Элементы.СотрудникиДаты.Видимость = Не ПодробныйВводВремени;
	Элементы.СотрудникиДатыЧасыПодробно.Видимость = ПодробныйВводВремени;
