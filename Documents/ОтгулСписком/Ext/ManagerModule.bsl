...
Процедура ЗаполнитьДанныеОбИспользованииОтгулов(РеквизитыДляПроведения, ДанныеДляПроведения)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ОтгулСпискомОтработанныеДни.ДатаНачала КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		|	ОтгулСпискомОтработанныеДни.Ссылка.Организация КАК Организация,
		|	ОтгулСпискомОтработанныеДни.Сотрудник КАК Сотрудник,
		|	ОтгулСпискомОтработанныеДни.Дата КАК ОтработанныйДень,
		|	ВЫБОР
		|		КОГДА ОтгулСпискомОтработанныеДни.Часы = 0
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК Дни,
		|	ОтгулСпискомОтработанныеДни.Часы КАК Часы
		|ИЗ
		|	Документ.ОтгулСписком.ОтработанныеДни КАК ОтгулСпискомОтработанныеДни
		|ГДЕ
		|	ОтгулСпискомОтработанныеДни.Ссылка = &Ссылка";
