...
Функция ДанныеДляБухучетаЗарплатыПервичныхДокументов(Объект) Экспорт
	...
	|	НастройкиБухучета.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	Документ.УвольнениеСписком.Сотрудники КАК Сотрудники
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УвольнениеСписком.НастройкиБухучета КАК НастройкиБухучета
	|		ПО Сотрудники.Ссылка = НастройкиБухучета.Ссылка
	|			И Сотрудники.ИдентификаторСтрокиСотрудника = НастройкиБухучета.ИдентификаторСтрокиСотрудника
	|ГДЕ
	|	Сотрудники.Ссылка = &Ссылка";
	НастройкиБухучета = Запрос.Выполнить().Выгрузить();
	
	ДанныеДляБухучета = Новый Структура;
	ДанныеДляБухучета.Вставить("ДокументОснование", Объект.Ссылка);
	
	ТаблицаБухучетЗарплаты = ОтражениеЗарплатыВБухучетеРасширенный.ДанныеДляБухучетаЗарплатыПервичныхДокументовМножественный(Объект.Ссылка, НастройкиБухучета);
