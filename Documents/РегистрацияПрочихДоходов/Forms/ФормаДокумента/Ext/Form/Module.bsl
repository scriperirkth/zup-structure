...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьОтображениеМестаПолученияДохода();
...
&НаКлиенте
Процедура НачисленияУдержанияВзносыМестоПолученияДоходаОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)

	Если ТипЗнч(ВыбранноеЗначение) = ТИП("СправочникСсылка.ПодразделенияОрганизаций")
		ИЛИ ТипЗнч(ВыбранноеЗначение) = ТИП("СправочникСсылка.ТерриторииВыполненияРабот") Тогда
		
		ДанныеСтроки = Элементы.НачисленияУдержанияВзносы.ТекущиеДанные;
		Если ДанныеСтроки <> Неопределено Тогда 
			ДанныеСтроки.ФиксСуммаВычета = Ложь; 
			ДанныеСтроки.МестоПолученияДохода = ВыбранноеЗначение; 
		КонецЕсли;
...
&НаКлиенте
Процедура НачисленияУдержанияВзносыМестоПолученияДоходаПриИзменении(Элемент)
	РассчитатьНДФЛиВзносыПоТекущейСтроке();   
КонецПроцедуры
...
Процедура НачисленияОбработкаВыбораНаСервере(МассивФизическихЛиц)
	...
	РассчитатьНДФЛИВзносыНаСервере(МассивРассчитываемыхПолучателей, , Истина);
...
Процедура РассчитатьНДФЛиВзносыВСтроке(ИдентификаторСтроки)
	...
	Если ДанныеСтроки.ФизическоеЛицо.Пустая() Или ДанныеСтроки.Начислено = 0 Или Объект.ВидПрочегоДохода.Пустая() Или Не ЗначениеЗаполнено(Объект.ПланируемаяДатаВыплаты) Тогда
		...
		Если Не ДанныеСтроки.ФизическоеЛицо.Пустая() Тогда
			РассчитатьНДФЛИВзносыНаСервере(ДанныеСтроки.ФизическоеЛицо, , Истина);
		КонецЕсли;
...
Функция СведенияОбНДФЛ(ФизическоеЛицо = Неопределено) Экспорт
	...
	НастройкиРасчета = Документ.ПараметрыРасчета();
	НастройкиРасчета.ФизическоеЛицо = ФизическоеЛицо;
	НастройкиРасчета.РасчетНДФЛНарастающимИтогомСНачалаГода = РасчетНДФЛНарастающимИтогомСНачалаГода;

	НастройкиРасчета.ЗаполнятьТерритории = Ложь; 
...
Процедура РассчитатьНДФЛИВзносыНаСервере(ФизическоеЛицо = Неопределено, 
										ЗаполнятьУдержания = Истина, 
										ЗаполнятьТерритории = Ложь)
	...
	ПараметрыРасчета.ЗаполнятьТерритории = ЗаполнятьТерритории;
...
Процедура ПересчитатьСотрудникаНаСервере()
	...
	Если ВыбранныеФизическиеЛица.Количество() Тогда
		РассчитатьНДФЛИВзносыНаСервере(ВыбранныеФизическиеЛица, , Истина);  
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьОтображениеМестаПолученияДохода()
	
	ИспользуютсяТерриторииИлиУсловияТруда = ЗарплатаКадрыРасширенный.ИспользоватьРаспределениеПоТерриториямУсловиямТруда(Объект.Организация);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "НачисленияУдержанияВзносыМестоПолученияДохода", "Видимость", ИспользуютсяТерриторииИлиУсловияТруда);
