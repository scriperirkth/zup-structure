...
Функция ПараметрыРасчета() Экспорт
	...
	ПараметрыРасчета.Вставить("ИспользуютсяТерриторииИлиУсловияТруда", 
		ЗарплатаКадрыРасширенный.ИспользоватьРаспределениеПоТерриториямУсловиямТруда(ЭтотОбъект.Организация));
...
Функция Рассчитать(ПараметрыРасчета = Неопределено) Экспорт
	...
	Если НЕ ПараметрыРасчета.ЗаполнятьТерритории Тогда
		...
		ТаблицаНачисленияУдержанияВзносы = ЭтотОбъект.НачисленияУдержанияВзносы.Выгрузить();
		...
		Если ЗначениеЗаполнено(ЭтотОбъект.Подразделение) Тогда
			Для Каждого СтрокаТаблицаНачисленияУдержанияВзносы Из ТаблицаНачисленияУдержанияВзносы Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицаНачисленияУдержанияВзносы.МестоПолученияДохода) Тогда
					СтрокаТаблицаНачисленияУдержанияВзносы.МестоПолученияДохода = ЭтотОбъект.Подразделение;		
				КонецЕсли;
			КонецЦикла;
		КонецЕсли; 
