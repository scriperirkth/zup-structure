...
Функция СведенияОбНДФЛ(ФизическоеЛицо = Неопределено) Экспорт
	...
	НастройкиРасчета.Вставить("ЗаполнятьТерритории", Ложь); 
	НастройкиРасчета.Вставить("ТаблицаСотрудникТерритория", Неопределено);
	НастройкиРасчета.Вставить("ИспользуютсяТерриторииИлиУсловияТруда", 
		ЗарплатаКадрыРасширенный.ИспользоватьРаспределениеПоТерриториямУсловиямТруда(Объект.Организация));
...
Процедура РассчитатьНДФЛИВзносыНаСервере(ФизическоеЛицо = Неопределено, 
	...
	ПараметрыРасчета.ИспользуютсяТерриторииИлиУсловияТруда = ИспользуютсяТерриторииИлиУсловияТруда; 
	...
	Если НЕ ПараметрыРасчета.ЗаполнятьТерритории Тогда
		...
		ПараметрыРасчета.ТаблицаСотрудникТерритория = ТаблицаНачисленияУдержанияВзносы; 
