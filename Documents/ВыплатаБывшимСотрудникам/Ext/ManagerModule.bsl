...
Функция ДанныеПечатиДокументов(МассивОбъектов)
	...
	Запрос.Текст =
		...
		|	ВыплатаБывшимСотрудникам.Исполнитель КАК Исполнитель,
	...
	БывшиеСотрудники = Запрос.Выполнить().Выгрузить(); 
	ПолучателиБывшиеСотрудники = ОбщегоНазначения.ВыгрузитьКолонку(БывшиеСотрудники, "ФизическоеЛицо", Истина);
	
	УчетПрочихДоходов.СоздатьВТОсновныеБанковскиеСчетаКонтрагентов(Запрос.МенеджерВременныхТаблиц, ПолучателиБывшиеСотрудники); 
	...
	Запрос.Текст = 
		...
		|	ОсновныеБанковскиеСчетаКонтрагентов.Банк КАК Банк,
		|	ОсновныеБанковскиеСчетаКонтрагентов.НомерСчета КАК НомерСчета,
		...
		|		ПО ВыплатаБывшимСотрудникам.ФизическоеЛицо = ОсновныеБанковскиеСчетаКонтрагентов.Контрагент
		|ГДЕ
		|	ВыплатаБывшимСотрудникам.Ссылка В(&МассивОбъектов)
		|ИТОГИ
		|	СУММА(Начислено),
