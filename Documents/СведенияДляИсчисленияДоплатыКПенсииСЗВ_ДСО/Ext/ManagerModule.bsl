...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьКорректируемыйПериодСЗВ_ДСО");
КонецПроцедуры
...
Функция ИмяФайла(Организация, Дата, УИД = Неопределено) Экспорт
	...
	Если Дата < ПерсонифицированныйУчет.ДатаПереходаНаФорматСЗВДСО2025() Тогда
		НачалоИмениФайла = "ПФР";
	КонецЕсли;
...
Функция СформироватьВыходнойФайл(Ссылка)
	...
	Если ВыборкаПоШапкеДокумента.Дата >= ПерсонифицированныйУчет.ДатаПереходаНаФорматСЗВДСО2025() Тогда
		Возврат СформироватьВыходнойФайл2025(ВыборкаПоШапкеДокумента);
	КонецЕсли;
...
Функция СформироватьВыходнойФайл2025(ВыборкаПоШапкеДокумента)
	
	СписокДокументов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыборкаПоШапкеДокумента.Ссылка);
	...
	// Загружаем формат файла сведений
	ДеревоФорматаXML = ПолучитьОбщийМакет("ФорматСЗВ_ДСО_2025XML");
	ТекстФорматаXML = ДеревоФорматаXML.ПолучитьТекст();
	...
	Атрибуты = Новый Соответствие;
	Атрибуты.Вставить("xmlns", "http://пф.рф/СЗВ-ДСО/2024-01-01");
	Атрибуты.Вставить("xmlns:УТ7", "http://пф.рф/УТ/2023-04-03");
	Атрибуты.Вставить("xmlns:АФ7", "http://пф.рф/АФ/2023-04-03");
	...
	Если ЗарплатаКадры.ИспользованиеРегистрационногоНомераСФР(ВыборкаПоШапкеДокумента.Дата, ВыборкаПоШапкеДокумента.Организация) 
		...
		НаборЗаписейРаботодатель.РегНомер = ВыборкаПоШапкеДокумента.РегистрационныйНомерСФР;
	КонецЕсли;
	НаборЗаписейРаботодатель.УТ7__ИНН = ВыборкаПоШапкеДокумента.ИНН;
	Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.КПП) Тогда
		НаборЗаписейРаботодатель.УТ7__КПП = ВыборкаПоШапкеДокумента.КПП;
	Иначе
		НаборЗаписейРаботодатель.Удалить("УТ7__КПП");
		СтрокаПоляКПП = ФорматСЗВ_ДСО.Работодатель.Поля.Найти("УТ7__КПП", "ИмяПоля");
		Если СтрокаПоляКПП <> Неопределено Тогда
			ФорматСЗВ_ДСО.Работодатель.Поля.Удалить(СтрокаПоляКПП);
		КонецЕсли;		
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПрежнийСтраховательНаименованиеПолное) Тогда
		...
		НаборЗаписейРаботодатель.НаименованиеОрганизации = ПрежнийСтраховательНаименованиеПолное;
		НаборЗаписейРаботодатель.РегНомер = СокрЛП(ВыборкаПоШапкеДокумента.ПрежнийСтраховательРегистрационныйНомерПФР);
		НаборЗаписейРаботодатель.УТ7__ИНН = СокрЛП(ВыборкаПоШапкеДокумента.ПрежнийСтраховательИНН);
		Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПрежнийСтраховательКПП) Тогда
			НаборЗаписейРаботодатель.УТ7__КПП = СокрЛП(ВыборкаПоШапкеДокумента.ПрежнийСтраховательКПП);
		Иначе
			НаборЗаписейРаботодатель.Удалить("УТ7__КПП");
			СтрокаПоляКПП = ФорматСЗВ_ДСО.РаботодательЗаКоторого.Поля.Найти("УТ7__КПП", "ИмяПоля");
			Если СтрокаПоляКПП <> Неопределено Тогда
				ФорматСЗВ_ДСО.РаботодательЗаКоторого.Поля.Удалить(СтрокаПоляКПП);
			КонецЕсли;
		КонецЕсли;
	...
	УзелФормы = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелСФР, "СЗВ-ДСО", "");
	ЗарплатаКадры.ДобавитьИнформациюВДерево(УзелФормы, ФорматСЗВ_ДСО);
	...
	Пока ВыборкаПоРаботникам.Следующий() Цикл  
		...
		НаборЗаписейФИО = НаборЗаписейЗЛ.УТ7__ФИО.Значение;
		НаборЗаписейФИО.УТ7__Фамилия = ВРег(Фамилия);
		НаборЗаписейФИО.УТ7__Имя = ВРег(Имя);
		НаборЗаписейФИО.УТ7__Отчество = ВРег(Отчество);
		
		ПерсонифицированныйУчет.УстановитьВыводВФайлСвойстваЗначенияЭлемента(НаборЗаписейЗЛ.УТ7__ФИО, "УТ7__Фамилия");
		ПерсонифицированныйУчет.УстановитьВыводВФайлСвойстваЗначенияЭлемента(НаборЗаписейЗЛ.УТ7__ФИО, "УТ7__Имя");
		ПерсонифицированныйУчет.УстановитьВыводВФайлСвойстваЗначенияЭлемента(НаборЗаписейЗЛ.УТ7__ФИО, "УТ7__Отчество");
		
		НаборЗаписейЗЛ.УТ7__СНИЛС.Значение = ВыборкаПоРаботникам.СтраховойНомерПФР;
		Если ВыборкаПоШапкеДокумента.ТипСведений <> Перечисления.ТипыСведенийСЗВ_ДСО.Отменяющая Тогда 
			НаборЗаписейЗЛ.КодДСО.Значение = ?(ВыборкаПоШапкеДокумента.ИспользуетсяТрудШахтеров, "ДСОУ", "ДСОЛ");
		КонецЕсли;
		...
		Если ВыборкаПоШапкеДокумента.ИспользуетсяТрудЧленовЛетныхЭкипажей И ВыборкаПоСтажуЧленовЛетныхЭкипажей.Количество() > 0 Тогда 
			...
			Пока ВыборкаПоСтажуЧленовЛетныхЭкипажей.НайтиСледующий(Отбор) Цикл 
				НаборЗаписейДСОЛ = ОбщегоНазначения.СкопироватьРекурсивно(ФорматПериодыДСО.ДСОЛ.Значение.ПодпунктА.Значение);
				НаборЗаписейДСОЛ.Период.Значение.УТ7__С = Формат(ВыборкаПоСтажуЧленовЛетныхЭкипажей.ДатаНачалаПериода, "ДФ=yyyy-MM-dd");
				НаборЗаписейДСОЛ.Период.Значение.УТ7__По = Формат(ВыборкаПоСтажуЧленовЛетныхЭкипажей.ДатаОкончанияПериода, "ДФ=yyyy-MM-dd");
				НаборЗаписейДСОЛ.Налет.Значение.Часы = ВыборкаПоСтажуЧленовЛетныхЭкипажей.Часы;
				НаборЗаписейДСОЛ.Налет.Значение.Минуты = ВыборкаПоСтажуЧленовЛетныхЭкипажей.Минуты;
				УзелДСОЛ = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелПериодыДСО, "ДСОЛ", "");
				Если ВыборкаПоСтажуЧленовЛетныхЭкипажей.ВидСтажа = Перечисления.ВидыСтажаЛетныхЭкипажей.ПодпунктА Тогда 
		...
		Если ВыборкаПоШапкеДокумента.ИспользуетсяТрудШахтеров И ВыборкаПоСтажуШахтеров.Количество() > 0 Тогда 
			...
			Пока ВыборкаПоСтажуШахтеров.НайтиСледующий(Отбор) Цикл 
				НаборЗаписейДСОУ = ОбщегоНазначения.СкопироватьРекурсивно(ФорматПериодыДСО.ДСОУ.Значение.Ведущая.Значение);
				НаборЗаписейДСОУ.УТ7__С.Значение = Формат(ВыборкаПоСтажуШахтеров.ДатаНачалаПериода, "ДФ=yyyy-MM-dd");
				НаборЗаписейДСОУ.УТ7__По.Значение = Формат(ВыборкаПоСтажуШахтеров.ДатаОкончанияПериода, "ДФ=yyyy-MM-dd");
				УзелДСОУ = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелПериодыДСО, "ДСОУ", "");
				Если ВыборкаПоСтажуШахтеров.ВидСтажа = Перечисления.ВидыСтажаШахтеров.ВедущиеПрофессии Тогда 
					УзелВидСтажа = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелДСОУ, "Ведущая", "");
				ИначеЕсли ВыборкаПоСтажуШахтеров.ВидСтажа = Перечисления.ВидыСтажаШахтеров.НеведущиеПрофессии Тогда 
	...
	НаборЗаписейРуководитель.Руководитель.Значение.УТ7__Должность.Значение = СокрЛП(ВРег(ВыборкаПоШапкеДокумента.ДолжностьРуководителя));
	НаборЗаписейФИО = НаборЗаписейРуководитель.Руководитель.Значение.УТ7__ФИО.Значение;
	НаборЗаписейФИО.УТ7__Фамилия = СокрЛП(ВРег(ВыборкаПоШапкеДокумента.ФамилияРуководителя));
	НаборЗаписейФИО.УТ7__Имя = СокрЛП(ВРег(ВыборкаПоШапкеДокумента.ИмяРуководителя));
	НаборЗаписейФИО.УТ7__Отчество = СокрЛП(ВРег(ВыборкаПоШапкеДокумента.ОтчествоРуководителя));
	ПерсонифицированныйУчет.УстановитьВыводВФайлСвойстваЗначенияЭлемента(НаборЗаписейРуководитель.Руководитель.Значение.УТ7__ФИО, "УТ7__Отчество");
	ЗарплатаКадры.ДобавитьИнформациюВДерево(УзелФормы, НаборЗаписейРуководитель);
	...
	Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ИмяФайлаДляСФР) Тогда
		ЧастиИмениФайла = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СокрЛП(ВыборкаПоШапкеДокумента.ИмяФайлаДляСФР),"_");
		НаборЗаписейСлужебнаяИнформация.АФ7__GUID.Значение = ЧастиИмениФайла[ЧастиИмениФайла.ВГраница()];
	КонецЕсли;
	НаборЗаписейСлужебнаяИнформация.АФ7__ДатаВремя.Значение = ПерсонифицированныйУчет.ПредставлениеДатыВФорматеdateTimeСЧасовымПоясом(ВыборкаПоШапкеДокумента.Дата);
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		Если ВыборкаПоРаботникам.НайтиСледующий(Новый Структура("Ссылка", ВыборкаПоШапкеДокумента.Ссылка)) Тогда
			...
			Пока ВыборкаПоРаботникам.Следующий() Цикл
				...
				ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, ИдентификаторКомандыПечати_ФормаСЗВ_ДСО(),
