...
Процедура НовыйДокумент(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ВидУчебногоОтпуска) Тогда
		ВыбратьВидОтпуска();
	Иначе
		СоздатьНовыйДокументПоВидуУчебногоОтпуска();
	КонецЕсли;
...
&НаКлиенте
Процедура СоздатьНовыйДокументПоВидуУчебногоОтпуска()
	
	ВидОтпуска = ПодобратьВидОтпуска(Объект.ВидУчебногоОтпуска);
	ОткрытьФорму("Документ.Отпуск.Форма.ФормаДокумента", ПараметрыЗаполненияДокумента(ВидОтпуска), ЭтотОбъект);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПодобратьВидОтпуска(ВидУчебногоОтпуска)
	
	Если ВидУчебногоОтпуска = Перечисления.ВидыУчебныхОтпусковКабинетСотрудника.НеОплачивается Тогда
		Возврат ПредопределенноеЗначение("Справочник.ВидыОтпусков.ОтпускБезОплатыУчебный");
	Иначе
		Возврат ПредопределенноеЗначение("Справочник.ВидыОтпусков.ОтпускУчебный");
	КонецЕсли;
...
Процедура ИнициализацияФормы()
	...
	Если Объект.ВидУчебногоОтпуска = Перечисления.ВидыУчебныхОтпусковКабинетСотрудника.Оплачивается Тогда
		ЗаголовокВидОтпуска =  НСтр("ru = 'Учебный отпуск оплачиваемый с'");
	ИначеЕсли Объект.ВидУчебногоОтпуска = Перечисления.ВидыУчебныхОтпусковКабинетСотрудника.НеОплачивается Тогда
		ЗаголовокВидОтпуска =  НСтр("ru = 'Учебный отпуск без оплаты с'");
	Иначе
		ЗаголовокВидОтпуска =  НСтр("ru = 'Учебный отпуск с'");
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ДатаНачала", "Заголовок", ЗаголовокВидОтпуска);
