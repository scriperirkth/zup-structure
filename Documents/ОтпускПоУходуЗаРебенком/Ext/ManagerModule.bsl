...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		ЗарегистрироватьСостояниеФизическогоЛица(Движения, ДанныеДляПроведения, РеквизитыДляПроведения);
	...
	Если РеквизитыДляПроведения.НачисленияУтверждены Тогда 
		Если РеквизитыДляПроведения.РассчитатьЗарплату Тогда 
			...
			Если СтруктураВидовУчета.СтраховыеВзносы Тогда
				...
				Если РезультатИсправления.ИсправлениеВТекущемПериоде Тогда
...
Процедура ПриПолученииОсновныхСведенийОДокументеДляСЭДО(БольничныйСсылка, ДокументСЭДО, ОсновныеСведения) Экспорт
	ИменаРеквизитов = "Представление, Проведен, НачисленияУтверждены, Организация, Сотрудник";
	Значения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(БольничныйСсылка, ИменаРеквизитов);
	ЗаполнитьЗначенияСвойств(ОсновныеСведения, Значения, , "Сотрудник");
	ОсновныеСведения.ГоловнаяОрганизация = ЗарплатаКадры.ГоловнаяОрганизация(Значения.Организация);
	ОсновныеСведения.ФизическоеЛицо = Значения.Сотрудник;
	ОсновныеСведения.Рассчитан = Значения.НачисленияУтверждены;
КонецПроцедуры
...
Процедура ЗаполнитьОтветНаЗапросФССДляРасчетаПособия(ОтпускПоУходу, ДокументСЭДО, Реквизиты, ДанныеДляРасчетаСреднегоЗаработка) Экспорт
	...
	Если ДанныеОтпуска.ДанныеШапки <> Неопределено Тогда
		...
		Реквизиты.Вставить("СреднийДневнойЗаработок",    Шапка.СреднийДневнойЗаработок);
		Реквизиты.Вставить("УходЗаНесколькимиДетьми",    Шапка.УходЗаНесколькимиДетьми);
	КонецЕсли;
...
Функция ДанныеОтпускаДляСЭДО(ОтпускПоУходу, ДокументСЭДО)
	...
	|	ШапкаДокумента.КоличествоДетей > 1 КАК УходЗаНесколькимиДетьми,
	|	ШапкаДокумента.СреднийДневнойЗаработок КАК СреднийДневнойЗаработок
	|ИЗ
	|	Документ.ОтпускПоУходуЗаРебенком КАК ШапкаДокумента
	|ГДЕ
...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, СтруктураВидовУчета) 
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		ЗаполнитьТаблицуСотрудниковФизическогоЛица(РеквизитыДляПроведения, ДанныеДляПроведения);
		Если РеквизитыДляПроведения.НачисленияУтверждены Тогда
...
Процедура ЗаполнитьТаблицуСотрудниковФизическогоЛица(РеквизитыДляПроведения, ДанныеДляПроведения)
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ПараметрыПолученияСотрудников = КадровыйУчет.ПараметрыПолученияСотрудниковОрганизацийПоСпискуФизическихЛиц();
	ПараметрыПолученияСотрудников.НачалоПериода = РеквизитыДляПроведения.ДатаНачала;
	ПараметрыПолученияСотрудников.ОкончаниеПериода = РеквизитыДляПроведения.ДатаОкончания;
	ПараметрыПолученияСотрудников.Организация = РеквизитыДляПроведения.Организация;
	...
	ПараметрыПолученияСотрудников.СписокФизическихЛиц = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РеквизитыДляПроведения.Сотрудник);
	...
	Запрос.Текст = 
		...
		|	ВТСотрудникиОрганизации.ДатаПриема > &ДатаНачала КАК ЕстьНовыеПриемы
		|ИЗ
	...
	Если ТаблицаСотрудников.Количество() > 1 И ТаблицаСотрудников.НайтиСтроки(Новый Структура("ЕстьНовыеПриемы", Истина)).Количество() > 0 Тогда
		// Есть еще сотрудники в периоде исключим сотрудников принятых в будующем, но после возврата из отпуска в случае перепроведения документов
		...
		Запрос.Текст = ТекстЗапросаСотрудниковФизическогоЛицаСУчетомВозврата();
		ТаблицаСотрудников = Запрос.Выполнить().Выгрузить();
	КонецЕсли;
...
Функция ТекстЗапросаСотрудниковФизическогоЛицаСУчетомВозврата()
	Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВозвратИзОтпускаПоУходуЗаРебенком.ДатаВозврата КАК ДатаВозврата
		|ПОМЕСТИТЬ ВТДатаВозврата
		|ИЗ
		...
		|		ПО ДанныеСостоянийСотрудников.Регистратор = ВозвратИзОтпускаПоУходуЗаРебенком.Ссылка
		|ГДЕ
		|	ВозвратИзОтпускаПоУходуЗаРебенком.Проведен
		|	И ВозвратИзОтпускаПоУходуЗаРебенком.Сотрудник = &Сотрудник
		|	И ВозвратИзОтпускаПоУходуЗаРебенком.ДатаВозврата МЕЖДУ &ДатаНачала И &ДатаОкончания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ВТСотрудникиОрганизации КАК ВТСотрудникиОрганизации
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДатаВозврата КАК ВТДатаВозврата
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ВТДатаВозврата.ДатаВозврата ЕСТЬ NULL
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ВТСотрудникиОрганизации.ДатаПриема < ВТДатаВозврата.ДатаВозврата
		|		КОНЕЦ";
	Возврат Текст;
КонецФункции

Процедура ЗарегистрироватьСостояниеФизическогоЛица(Движения, ДанныеДляПроведения, РеквизитыДляПроведения)
	// Подбираем состояние.
	...
	Если Не РеквизитыДляПроведения.НеНачислятьЗарплатуИНеВыплачиватьАвансВоВремяОтпуска Тогда
		Состояние = Перечисления.СостоянияСотрудника.РаботаВОтпускеПоУходуЗаРебенком;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ДанныеДляПроведения.ТаблицаСотрудников Цикл
		...
		НоваяСтрока.Начало = ?(РеквизитыДляПроведения.ДатаНачала < СтрокаТаблицы.ДатаПриема, СтрокаТаблицы.ДатаПриема, РеквизитыДляПроведения.ДатаНачала);
		НоваяСтрока.ОкончаниеПредположительно = РеквизитыДляПроведения.ДатаОкончания;
...
Процедура ЗаполнитьПрименениеПлановыхНачислений(РеквизитыДляПроведения, ДанныеДляПроведения)
	...
	Если РеквизитыДляПроведения.НеНачислятьЗарплатуИНеВыплачиватьАвансВоВремяОтпуска Тогда
		Для каждого СтрокаТаблицы Из ДанныеДляПроведения.ТаблицаСотрудников Цикл
			...
			ПустоеПрименениеНачислений.Сотрудник = СтрокаТаблицы.Сотрудник;
			ПустоеПрименениеНачислений.ДатаСобытия = РеквизитыДляПроведения.ДатаНачала;
		КонецЦикла;
	КонецЕсли;
