...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьДокументаКомандировка" И Источник = ЭтаФорма Тогда
		...
		Если Строки <> Неопределено Тогда
			...
			Строки[0].ОтсутствиеУчтено		 = Параметр.ОтсутствиеУчтено;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Оповестить("Закрытие_КомандировкиСотрудников");
	...
	Для каждого Строка Из Объект.Сотрудники Цикл
		Если ЗначениеЗаполнено(Строка.Командировка) Тогда
			...
			Если РезультатПоКомандировке <> Неопределено Тогда
				...
				Строка.ОтсутствиеУчтено = Истина;
			Иначе
				Строка.ОтсутствиеУчтено = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПредупреждение(Форма)
	...
	КоличествоНепроведенныхОтсутствий = Форма.Объект.Сотрудники.НайтиСтроки(Новый Структура("ОтсутствиеУчтено", Ложь)).Количество();
