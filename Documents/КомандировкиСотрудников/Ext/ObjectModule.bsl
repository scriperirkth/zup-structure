...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ДополнительныеСвойства.Вставить("ПометкаУдаленияИзменена", Не Ссылка.ПометкаУдаления = ПометкаУдаления);
...
Процедура ОбновитьСвязанныеКомандировки(Отказ)
	...
	ИзменениеПометкиУдаления = ДополнительныеСвойства.ПометкаУдаленияИзменена;
	Если ИзменениеПометкиУдаления Тогда
		УсловиеОтбораИзмененийТекст = "ИСТИНА";
	Иначе 
		УсловиеОтбораИзмененийТекст = "(КомандировкиСотрудниковСотрудники.Сотрудник <> КомандировкаДокументы.Сотрудник
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ОбновитьСвязаннуюКомандировку(Сотрудники[Выборка.НомерСтроки -1], Отказ, ИзменениеПометкиУдаления);
		КонецЦикла;	
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбновитьСвязаннуюКомандировку(Строка, Отказ, ИзменениеПометкиУдаления)
	...
	Попытка
		...
		Если ИзменениеПометкиУдаления Тогда
			//изменение пометки на удаление связанных командировок
			...
			КонецЕсли;
		Иначе 
			//обновляем связанные командировки
	...
	Исключение
		НомерТекущейСтроки = Строка.НомерСтроки - 1;
		ТекстСообщения = ?(ПометкаУдаления, НСтр("ru = 'Не удалось установить пометку удаления командировки %1.'"),НСтр("ru = 'Не удалось обновить командировку %1.'"));
