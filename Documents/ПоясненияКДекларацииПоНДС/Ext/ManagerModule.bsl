...
Функция ПредставлениеКонтрагента(Строка, Представление) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Строка, "КПП") И ЗначениеЗаполнено(Строка.КПП) Тогда
		ИННКПП = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1/%2'"), Строка.ИНН, Строка.КПП);
	Иначе
		ИННКПП = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1'"), Строка.ИНН);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Представление) Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)'"), ИННКПП, Представление);
	Иначе
		Возврат ИННКПП;
	КонецЕсли;
...
#Область СпискиСчетовФактур

Функция РеквизитыСчетовФактур() Экспорт
	
	Результат = "Номер, Дата"; 
	Возврат Результат;
	
КонецФункции

Функция ПредставлениеСчетовФактур(Строка) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ПериодОтраженияЗаписи) Тогда
		...
		Для каждого ПериодИзМассива Из МассивПериодов Цикл
			...
			Иначе
				...
				ИначеЕсли ВидПериода = 71 Тогда
					Представление = "за январь " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 72 Тогда
					Представление = "за февраль " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 73 Тогда
					Представление = "за март " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 74 Тогда
					Представление = "за апрель " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 75 Тогда
					Представление = "за май " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 76 Тогда
					Представление = "за июнь " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 77 Тогда
					Представление = "за июль " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 78 Тогда
					Представление = "за август " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 79 Тогда
					Представление = "за сентябрь " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 80 Тогда
					Представление = "за октябрь " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 81 Тогда
					Представление = "за ноябрь " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 82 Тогда
					Представление = "за декабрь " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 83 Тогда
					Представление = "I квартал " + Формат(Год, "ЧГ=") + " при снятии с учета, в качестве ИП";
				ИначеЕсли ВидПериода = 87 Тогда
					Представление = "II квартал " + Формат(Год, "ЧГ=") + " при снятии с учета, в качестве ИП";
				ИначеЕсли ВидПериода = 88 Тогда
					Представление = "III квартал " + Формат(Год, "ЧГ=") + " при снятии с учета, в качестве ИП";
				ИначеЕсли ВидПериода = 89 Тогда
					Представление = "IV квартал " + Формат(Год, "ЧГ=") + " при снятии с учета, в качестве ИП";
				КонецЕсли;
			КонецЕсли;
...
Функция ДанныеПоясненияКДекларацииПоНДС(Пояснение) Экспорт
	...
	|	ПоясненияКДекларацииПоНДСКнигаПокупок.СчетФактураВСчетРанееПолученнойОплаты КАК СчетФактураВСчетРанееПолученнойОплаты,
	|	ПоясненияКДекларацииПоНДСКнигаПокупок.СчетФактураВСчетРанееПолученнойОплатыРасхождение КАК СчетФактураВСчетРанееПолученнойОплатыРасхождение
	|ИЗ
	...
	|	ПоясненияКДекларацииПоНДСКнигаПокупокДл.СчетФактураВСчетРанееПолученнойОплаты КАК СчетФактураВСчетРанееПолученнойОплаты,
	|	ПоясненияКДекларацииПоНДСКнигаПокупокДл.СчетФактураВСчетРанееПолученнойОплатыРасхождение КАК СчетФактураВСчетРанееПолученнойОплатыРасхождение
	|ИЗ
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость22 КАК Стоимость22,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость20 КАК Стоимость20,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость1820 КАК Стоимость1820,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС22 КАК СуммаНДС22,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС20 КАК СуммаНДС20,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС1820 КАК СуммаНДС1820,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость22Расхождение КАК Стоимость22Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость20Расхождение КАК Стоимость20Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость1820Расхождение КАК Стоимость1820Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС22Расхождение КАК СуммаНДС22Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС20Расхождение КАК СуммаНДС20Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС1820Расхождение КАК СуммаНДС1820Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СчетФактураПродавца КАК СчетФактураПродавца,
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СчетФактураПродавцаРасхождение КАК СчетФактураПродавцаРасхождение
	|ИЗ
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость22 КАК Стоимость22,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость20 КАК Стоимость20,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость1820 КАК Стоимость1820,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС22 КАК СуммаНДС22,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС20 КАК СуммаНДС20,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС1820 КАК СуммаНДС1820,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость22Расхождение КАК Стоимость22Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость20Расхождение КАК Стоимость20Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость1820Расхождение КАК Стоимость1820Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС22Расхождение КАК СуммаНДС22Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС20Расхождение КАК СуммаНДС20Расхождение,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС1820Расхождение КАК СуммаНДС1820Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СчетФактураПродавца КАК СчетФактураПродавца,
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СчетФактураПродавцаРасхождение КАК СчетФактураПродавцаРасхождение
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоясненияКДекларацииПоНДССведенияСФВСчетРанееПолученнойОплаты.ИдентификаторЗначения КАК ИдентификаторЗначения,
	|	ПоясненияКДекларацииПоНДССведенияСФВСчетРанееПолученнойОплаты.Номер КАК Номер,
	|	ПоясненияКДекларацииПоНДССведенияСФВСчетРанееПолученнойОплаты.Дата КАК Дата
	|ИЗ
	|	Документ.ПоясненияКДекларацииПоНДС.СведенияСФВСчетРанееПолученнойОплаты КАК ПоясненияКДекларацииПоНДССведенияСФВСчетРанееПолученнойОплаты
	|ГДЕ
	|	ПоясненияКДекларацииПоНДССведенияСФВСчетРанееПолученнойОплаты.Ссылка = &ОтветНаТребование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоясненияКДекларацииПоНДССведенияСФПродавца.ИдентификаторЗначения КАК ИдентификаторЗначения,
	|	ПоясненияКДекларацииПоНДССведенияСФПродавца.Номер КАК Номер,
	|	ПоясненияКДекларацииПоНДССведенияСФПродавца.Дата КАК Дата
	|ИЗ
	|	Документ.ПоясненияКДекларацииПоНДС.СведенияСФПродавца КАК ПоясненияКДекларацииПоНДССведенияСФПродавца
	|ГДЕ
	|	ПоясненияКДекларацииПоНДССведенияСФПродавца.Ссылка = &ОтветНаТребование";
	...
	СведенияСФВСчетРанееПолученнойОплаты = Результат.Получить(14).Выгрузить();
	СведенияСФВСчетРанееПолученнойОплаты.Индексы.Добавить("ИдентификаторЗначения");
	ДанныеПоясненияКДекларацииПоНДС.Вставить("СведенияСФВСчетРанееПолученнойОплаты", СведенияСФВСчетРанееПолученнойОплаты);

	СведенияСФПродавца = Результат.Получить(15).Выгрузить();
	СведенияСФПродавца.Индексы.Добавить("ИдентификаторЗначения");
	ДанныеПоясненияКДекларацииПоНДС.Вставить("СведенияСФПродавца", СведенияСФПродавца);
...
Функция ТаблицаРаздела8() Экспорт
	...
	Раздел8.Колонки.Добавить("СвЧастСчФ", Новый ОписаниеТипов("Массив")); // массив СтруктураСвЧастСчФ()
...
Функция ТаблицаРаздела9() Экспорт
	...
	Раздел9.Колонки.Добавить("СтоимПродСФ1820", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
	...
	Раздел9.Колонки.Добавить("СумНДССФ1820", ОбщегоНазначения.ОписаниеТипаЧисло(19, 2));
	...
	Раздел9.Колонки.Добавить("СвЧастСчФ", Новый ОписаниеТипов("Массив")); // массив СтруктураСвЧастСчФ()
...
Функция СтруктураСвЧастСчФ() Экспорт
	
	СтруктураПараметров = Новый Структура;
	
	СтруктураПараметров.Вставить("НомЧастСчФ", "");
	СтруктураПараметров.Вставить("ДатаЧастСчФ", '00010101');
...
Процедура ЗаполнитьРасхожденияСтрокиРаздела9(ДанныеСтроки) Экспорт
	...
	ДанныеСтроки.Стоимость22Расхождение  = ДанныеСтроки.Стоимость22;
	ДанныеСтроки.Стоимость20Расхождение  = ДанныеСтроки.Стоимость20;
	ДанныеСтроки.Стоимость1820Расхождение = ДанныеСтроки.Стоимость1820;
	...
	ДанныеСтроки.СуммаНДС22Расхождение   = ДанныеСтроки.СуммаНДС22;
	ДанныеСтроки.СуммаНДС20Расхождение   = ДанныеСтроки.СуммаНДС20;
	ДанныеСтроки.СуммаНДС1820Расхождение = ДанныеСтроки.СуммаНДС1820;
...
Процедура ЗаполнитьРаздел8(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		ЗаписьКнигиПокупок.СчетФактураВСчетРанееПолученнойОплаты = Строка(Новый УникальныйИдентификатор);
		МассивСчетФактур = Новый Массив;
		Для Каждого СвЧастСчФ ИЗ СтрокаТаблицыРаздела.СвЧастСчФ Цикл
			ЗначенияРеквизитов = Новый Структура(РеквизитыСчетовФактур());
			ЗначенияРеквизитов.Номер = СвЧастСчФ.НомЧастСчФ;
			ЗначенияРеквизитов.Дата  = СвЧастСчФ.ДатаЧастСчФ;
			МассивСчетФактур.Добавить(ЗначенияРеквизитов);
		КонецЦикла;
		ЗаписатьМассивРеквизитовВТаблицуХранения(МассивСчетФактур,
		Объект.СведенияСФВСчетРанееПолученнойОплаты, ЗаписьКнигиПокупок.СчетФактураВСчетРанееПолученнойОплаты);
		...
		Если ДанныеКопированияСтроки = Неопределено Тогда
			...
			СкопироватьСписокРеквизитаСтроки(ЗаписьКнигиПокупок, "СчетФактураВСчетРанееПолученнойОплатыРасхождение", ЗаписьКнигиПокупок,
			"СчетФактураВСчетРанееПолученнойОплаты", РеквизитыСчетовФактур(), Объект.СведенияСФВСчетРанееПолученнойОплаты, Объект.СведенияСФВСчетРанееПолученнойОплаты);
		Иначе
			...
			СкопироватьСписокРеквизитаСтроки(ЗаписьКнигиПокупок, "СчетФактураВСчетРанееПолученнойОплатыРасхождение", ДанныеКопированияСтроки,
			"СчетФактураВСчетРанееПолученнойОплатыРасхождение", РеквизитыСчетовФактур(), Объект.СведенияСФВСчетРанееПолученнойОплаты, ДанныеКопирования.СведенияСФВСчетРанееПолученнойОплаты);
		КонецЕсли;
...
Процедура ЗаполнитьРаздел9(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		ЗаписьКнигиПродаж.Стоимость22               = СтрокаТаблицыРаздела.СтоимПродСФ22;
		ЗаписьКнигиПродаж.Стоимость20               = СтрокаТаблицыРаздела.СтоимПродСФ20;
		ЗаписьКнигиПродаж.Стоимость1820             = СтрокаТаблицыРаздела.СтоимПродСФ1820;
		...
		ЗаписьКнигиПродаж.СуммаНДС22                = СтрокаТаблицыРаздела.СумНДССФ22;
		ЗаписьКнигиПродаж.СуммаНДС20                = СтрокаТаблицыРаздела.СумНДССФ20;
		ЗаписьКнигиПродаж.СуммаНДС1820              = СтрокаТаблицыРаздела.СумНДССФ1820;
		...
		ЗаписьКнигиПродаж.СчетФактураПродавца = Строка(Новый УникальныйИдентификатор);
		МассивСчетФактур = Новый Массив;
		Для Каждого СвЧастСчФ ИЗ СтрокаТаблицыРаздела.СвЧастСчФ Цикл
			ЗначенияРеквизитов = Новый Структура(РеквизитыСчетовФактур());
			ЗначенияРеквизитов.Номер = СвЧастСчФ.НомЧастСчФ;
			...
			МассивСчетФактур.Добавить(ЗначенияРеквизитов);
		КонецЦикла;
		ЗаписатьМассивРеквизитовВТаблицуХранения(МассивСчетФактур, Объект.СведенияСФПродавца, ЗаписьКнигиПродаж.СчетФактураПродавца);
		...
		Если ДанныеКопированияСтроки = Неопределено Тогда
			...
			СкопироватьСписокРеквизитаСтроки(ЗаписьКнигиПродаж, "СчетФактураПродавцаРасхождение", ЗаписьКнигиПродаж,
			"СчетФактураПродавца", РеквизитыСчетовФактур(), Объект.СведенияСФПродавца, Объект.СведенияСФПродавца);
		Иначе
			ЗаполнитьЗначенияСвойств(ЗаписьКнигиПродаж, ДанныеКопированияСтроки, "РезультатПроверки, НомерСчетаФактурыРасхождение, ДатаСчетаФактурыРасхождение, НомерИсправленияСчетаФактурыРасхождение, ДатаИсправленияСчетаФактурыРасхождение,
					...
					|КодВалютыРасхождение, СтоимостьВсегоРасхождение, СтоимостьРубВсегоРасхождение, Стоимость22Расхождение,Стоимость20Расхождение,Стоимость1820Расхождение,
					|Стоимость18Расхождение, Стоимость10Расхождение, Стоимость7Расхождение, Стоимость5Расхождение, Стоимость0Расхождение,
					|СуммаНДС22Расхождение, СуммаНДС20Расхождение, СуммаНДС1820Расхождение, СуммаНДС18Расхождение, СуммаНДС10Расхождение,
					|СуммаНДС7Расхождение, СуммаНДС5Расхождение, СтоимостьОсвобождаемаяРасхождение, КодВидаОперацииРасхождение, ПосредникИННРасхождение");
			...
			СкопироватьСписокРеквизитаСтроки(ЗаписьКнигиПродаж, "СчетФактураПродавцаРасхождение", ДанныеКопированияСтроки,
			"СчетФактураПродавцаРасхождение", РеквизитыСчетовФактур(), Объект.СведенияСФПродавца, ДанныеКопирования.СведенияСФПродавца);
		КонецЕсли;
...
Функция ПолучитьДаннныеДляВыгрузки(ДокументСсылка)
	...
	Для Каждого СтрокаРаздела8 Из ДанныеПояснения.Раздел8 Цикл
		...
		СписокСчетовФактурЧастичнойОплаты = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФВСчетРанееПолученнойОплаты,
		СтрокаРаздела8.СчетФактураВСчетРанееПолученнойОплаты, РеквизитыСчетовФактур());

		СписокСчетовФактурЧастичнойОплатыРасхождение = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФВСчетРанееПолученнойОплаты,
		СтрокаРаздела8.СчетФактураВСчетРанееПолученнойОплатыРасхождение, РеквизитыСчетовФактур());
		
		СпискиРеквизитов = Новый Структура("Продавец, ДокументОплаты, ДатаПринятияНаУчет, СчетФактураЧастичнойОплаты");
		...
		СпискиРеквизитов.СчетФактураЧастичнойОплаты = Новый Структура("Реквизиты, РеквизитыРасхождение",
		СписокСчетовФактурЧастичнойОплаты, СписокСчетовФактурЧастичнойОплатыРасхождение);
	...
	Для Каждого СтрокаРаздела8_1 Из ДанныеПояснения.Раздел8_1 Цикл
		...
		СписокСчетовФактурЧастичнойОплаты = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФВСчетРанееПолученнойОплаты,
		СтрокаРаздела8_1.СчетФактураВСчетРанееПолученнойОплаты, РеквизитыСчетовФактур());
		
		СписокСчетовФактурЧастичнойОплатыРасхождение = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФВСчетРанееПолученнойОплаты,
		СтрокаРаздела8_1.СчетФактураВСчетРанееПолученнойОплатыРасхождение, РеквизитыСчетовФактур());
	...
	Для Каждого СтрокаРаздела9 Из ДанныеПояснения.Раздел9 Цикл
		...
		Если СтрокаРаздела9.СуммаНДС22 <> СтрокаРаздела9.СуммаНДС22Расхождение
			Или СтрокаРаздела9.СуммаНДС20 <> СтрокаРаздела9.СуммаНДС20Расхождение
			Или СтрокаРаздела9.СуммаНДС1820 <> СтрокаРаздела9.СуммаНДС1820Расхождение
		...
		СписокСчетовФактурЧастичнойОплаты = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФПродавца,
		СтрокаРаздела9.СчетФактураПродавца, РеквизитыСчетовФактур());
		
		СписокСчетовФактурЧастичнойОплатыРасхождение = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФПродавца,
		СтрокаРаздела9.СчетФактураПродавцаРасхождение, РеквизитыСчетовФактур());
		
		СпискиРеквизитов = Новый Структура("Покупатель, ДокументОплаты, СчетФактураЧастичнойОплаты");
	...
	Для Каждого СтрокаРаздела9_1 Из ДанныеПояснения.Раздел9_1 Цикл
		...
		Если СтрокаРаздела9_1.СуммаНДС22 <> СтрокаРаздела9_1.СуммаНДС22Расхождение 
			Или СтрокаРаздела9_1.СуммаНДС20 <> СтрокаРаздела9_1.СуммаНДС20Расхождение
			Или СтрокаРаздела9_1.СуммаНДС1820 <> СтрокаРаздела9_1.СуммаНДС1820Расхождение 
		...
		СписокСчетовФактурЧастичнойОплаты = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФПродавца,
		СтрокаРаздела9_1.СчетФактураПродавца, РеквизитыСчетовФактур());
		
		СписокСчетовФактурЧастичнойОплатыРасхождение = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.СведенияСФПродавца,
		СтрокаРаздела9_1.СчетФактураПродавцаРасхождение, РеквизитыСчетовФактур());
...
Процедура ЗаполнитьСтрокуРаздела8(СтрокаВыгрузкиРаздела8, ДанныеСтроки, СпискиРеквизитов, ПостфиксРеквизита)
	...
	Реквизиты = Неопределено;
	СпискиРеквизитов.СчетФактураЧастичнойОплаты.Свойство("Реквизиты" + ПостфиксРеквизита, Реквизиты);
	Для Каждого Строка Из Реквизиты Цикл
		МассивСчетовФактур.Добавить(Новый Структура("НомЧастСчФ, ДатаЧастСчФ", Строка.Номер, Строка.Дата));
	КонецЦикла;
	СтрокаВыгрузкиРаздела8.СвЧастСчФ = МассивСчетовФактур;
...
Процедура ЗаполнитьСтрокуРаздела9(СтрокаВыгрузкиРаздела9, ДанныеСтроки, СпискиРеквизитов, ПостфиксРеквизита)
	...
	Для Каждого Строка Из Реквизиты Цикл
		МассивСчетовФактур.Добавить(Новый Структура("НомЧастСчФ, ДатаЧастСчФ", Строка.Номер, Строка.Дата));
	КонецЦикла;
	СтрокаВыгрузкиРаздела9.СвЧастСчФ = МассивСчетовФактур;
...
Функция ТаблицаКнигаПокупок()
	...
	КнигаПокупок.Колонки.Добавить("СвЧастСчФ", Новый ОписаниеТипов("Массив"));
...
Функция ТаблицаКнигаПродаж()
	...
	КнигаПродаж.Колонки.Добавить("СвЧастСчФ", Новый ОписаниеТипов("Массив"));
