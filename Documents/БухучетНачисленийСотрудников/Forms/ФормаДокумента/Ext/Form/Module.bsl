...
&НаКлиенте
Процедура БухучетУказываетсяРаспределениемПриИзменении(Элемент)
	
	УстановитьВидимостьНастроекБухучета(ЭтотОбъект, Объект.БухучетУказываетсяРаспределением);
	БухучетУказываетсяРаспределениемПриИзмененииНаСервере()
...
Процедура СотрудникиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не Копирование И НоваяСтрока Тогда
		Если Объект.БухучетУказываетсяРаспределением Тогда
			Элемент.ТекущиеДанные.ПредставлениеРаспределения = ПредставлениеРаспределенияСтроки(Неопределено, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении);
		КонецЕсли;
	КонецЕсли;
...
Процедура СотрудникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СтрНайти(Поле.Имя, "СотрудникиПредставлениеРаспределения") <> 0 Тогда
...
Процедура ЗаполнитьСведения(Команда)
	...
	ПараметрыОткрытия.Вставить("БухучетУказываетсяРаспределением", Объект.БухучетУказываетсяРаспределением);
...
Процедура ОбработкаПодбораНаСервере(Знач Сотрудники)
	...
	Для Каждого Сотрудник Из Сотрудники Цикл
		...
		Если СтрокиСотрудников.Количество() = 0 Тогда
			...
			Если Объект.БухучетУказываетсяРаспределением Тогда
				НоваяСтрока.ПредставлениеРаспределения = ПредставлениеРаспределенияСтроки(Неопределено, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении);
			КонецЕсли;
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьНастроекБухучета(Форма, БухучетУказываетсяРаспределением)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"СотрудникиБухучет",
		"Видимость",
		Не БухучетУказываетсяРаспределением);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		ЭлементыФормы,
		"СотрудникиПредставлениеРаспределения",
		"Видимость",
		БухучетУказываетсяРаспределением);
...
&НаСервере
Процедура БухучетУказываетсяРаспределениемПриИзмененииНаСервере()
	
	Если Объект.БухучетУказываетсяРаспределением Тогда
		БухучетНачислений = Объект.БухучетНачислений.Выгрузить().СкопироватьКолонки();
		БухучетНачислений.Колонки.Добавить("КодСтатьиФинансирования", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(7)));
		...
		Для каждого СтрокаТЧ Из Объект.Сотрудники Цикл
			СтрокаТЧ.ИдентификаторСтрокиСотрудника = ИдентификаторСтроки;
			ТаблицаРаспределения = БухучетНачислений.СкопироватьКолонки(); 
			Если ЗначениеЗаполнено(СтрокаТЧ.СтатьяФинансирования) Или ЗначениеЗаполнено(СтрокаТЧ.СтатьяРасходов)
				Или ЗначениеЗаполнено(СтрокаТЧ.СпособОтраженияЗарплатыВБухучете) Тогда
				НоваяСтрока = ТаблицаРаспределения.Добавить();
				НоваяСтрока.СтатьяФинансирования 				= СтрокаТЧ.СтатьяФинансирования;
				НоваяСтрока.СтатьяРасходов 						= СтрокаТЧ.СтатьяРасходов;
				НоваяСтрока.СпособОтраженияЗарплатыВБухучете 	= СтрокаТЧ.СпособОтраженияЗарплатыВБухучете;
				НоваяСтрока.ДоляРаспределения 					= 100;
				НоваяСтрока.ИдентификаторСтрокиСотрудника 		= ИдентификаторСтроки;
				НоваяСтрока.КодСтатьиФинансирования 			= КодыСтатейФинансирования[СтрокаТЧ.СтатьяФинансирования];
			КонецЕсли;
			...
			СтрокаТЧ.ПредставлениеРаспределения = ПредставлениеРаспределенияСтроки(СтруктураРаспределения, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении);
			ИдентификаторСтроки = ИдентификаторСтроки + 1;
		КонецЦикла;
	Иначе
		Объект.СуммарнаяДоляСтоПроцентов = Ложь;
		Для каждого СтрокаТЧ Из Объект.Сотрудники Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.РезультатРаспределения) Тогда
				ПерваяСтрока = СтрокаТЧ.РезультатРаспределения[0];
				СтрокаТЧ.СтатьяФинансирования 				= ПерваяСтрока.СтатьяФинансирования;
				СтрокаТЧ.СтатьяРасходов 					= ПерваяСтрока.СтатьяРасходов;
				СтрокаТЧ.СпособОтраженияЗарплатыВБухучете 	= ПерваяСтрока.СпособОтраженияЗарплатыВБухучете;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДанныеВРеквизиты()
	...
	Если Не Объект.БухучетУказываетсяРаспределением Тогда
		Возврат;
	КонецЕсли;
	
	БухучетНачислений = Объект.БухучетНачислений.Выгрузить();
	Если ИспользоватьСтатьиФинансирования Тогда
		ОтражениеЗарплатыВБухучетеРасширенный.ДополнитьТаблицуРаспределенияКодомСтатьиФинансирования(БухучетНачислений);
	КонецЕсли;
	...
	Для каждого СтрокаТЧ Из Объект.Сотрудники Цикл
		...
		ТаблицаРаспределения = БухучетНачислений.Скопировать(ОтборСтрок);
		СтруктураРаспределения = ЗарплатаКадрыРасширенный.ТаблицаЗначенийВСтруктуру(ТаблицаРаспределения);
		СтрокаТЧ.РезультатРаспределения = СтруктураРаспределения;
		СтрокаТЧ.ПредставлениеРаспределения = ПредставлениеРаспределенияСтроки(СтруктураРаспределения, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении);
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеРаспределенияСтроки(РезультатРаспределения, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении)
	
	Если РезультатРаспределения = Неопределено Или РезультатРаспределения.Количество() = 0 Тогда
		Возврат НСтр("ru='Не задано'");
	КонецЕсли;
	
	Возврат ОтражениеЗарплатыВБухучетеКлиентСерверРасширенный.ПредставлениеБухучетаНачисления(РезультатРаспределения, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении);
...
Процедура РеквизитыВДанные(ТекущийОбъект)
	...
	ТекущийОбъект.БухучетНачислений.Очистить();
	...
	Для каждого СтрокаТЧ Из Объект.Сотрудники Цикл
		...
		Если РезультатРаспределения <> Неопределено Тогда
			Для Каждого СтрокаРаспределения Из РезультатРаспределения Цикл
				НоваяСтрока = ТекущийОбъект.БухучетНачислений.Добавить();
...
Процедура РедактироватьСтроку(ДанныеСтроки, ВыбраннаяСтрока)
	...
	ПараметрыОткрытия.Вставить("БухучетНачисления", 		ДанныеСтроки.РезультатРаспределения);
	ПараметрыОткрытия.Вставить("ТолькоПросмотр", 			ТолькоПросмотр);
	ПараметрыОткрытия.Вставить("Сотрудник", 				ДанныеСтроки.Сотрудник);
	ПараметрыОткрытия.Вставить("Начисление", 				ДанныеСтроки.Начисление);
	...
	ОткрытьФорму("Документ.БухучетНачисленийСотрудников.Форма.ФормаБухучетНачисления",
				ПараметрыОткрытия, ЭтаФорма, Истина, , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура РедактироватьСтрокуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекущиеДанные.РезультатРаспределения = Результат.РезультатРаспределения;
	ТекущиеДанные.ПредставлениеРаспределения = ПредставлениеРаспределенияСтроки(ТекущиеДанные.РезультатРаспределения, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении);
