...
Процедура ДанныеФормыВДанныеМенеджераРасчета(МенеджерРасчета, Отбор = Неопределено, ПозицииВставки = Неопределено, ПерезаполнитьНачисления = Ложь)
	...
	Если ПерезаполнитьНачисления Тогда
		...
		ВыделятьРКСН = ДополнительныеВидыРасчета.Получить(Объект.Начисление) <> Неопределено И Объект.ПланируемаяДатаВыплаты >= УчетНДФЛРасширенный.ДатаЗаконаПроРКиЕАЭС();
...
Процедура ДополнитьСтрокиНаСервере(ИдентификаторыСтрок, ОписаниеТаблицы, ЗаполнятьСведенияСотрудников, ЗаполнятьЗначенияПоказателей)
	...
	Если ЗаполнятьЗначенияПоказателей И Объект.ПланируемаяДатаВыплаты < УчетНДФЛРасширенный.ДатаЗаконаПроРКиЕАЭС() Тогда
		ОбнулитьЗначенияПоказателейДолейРКСН();	
	КонецЕсли;
...
&НаСервере
Процедура ОбнулитьЗначенияПоказателейДолейРКСН()
	
	ВидРасчетаИнфо = ЗарплатаКадрыРасширенныйПовтИсп.ПолучитьИнформациюОВидеРасчета(Объект.Начисление);
	КоличествоПоказателейНачисления = ВидРасчетаИнфо.Показатели.Количество();
	...
	Для Каждого СтрокаНачисления Из Объект.Начисления Цикл
		Для Каждого ОписаниеПоказателя Из ВидРасчетаИнфо.Показатели Цикл				
			Если НулевыеЗначенияПоказателейРКСН.Свойство(ОписаниеПоказателя.Идентификатор) Тогда
				Для НомерПоказателя = 1 По КоличествоПоказателейНачисления Цикл
					Если СтрокаНачисления["Показатель" + НомерПоказателя].Идентификатор = ОписаниеПоказателя.Идентификатор Тогда
						СтрокаНачисления["Значение" + НомерПоказателя] = 0;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
Функция ПроверитьПересечениеПериодовОтсутствияНаСервере()
	...
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	РезультатПроверки = ДокументОбъект.РезультатПроверкиПересеченийПериодовОтсутствия();
	
	Больничные = Новый Массив;
	Для Каждого РезультатПоСотруднику Из РезультатПроверки.ДанныеСотрудников Цикл
		Если ТипЗнч(РезультатПоСотруднику.Значение.Регистратор) = Тип("ДокументСсылка.БольничныйЛист") Тогда
			Больничные.Добавить(РезультатПоСотруднику.Значение.Регистратор);
		КонецЕсли;
	КонецЦикла;
	
	Если Больничные.Количество() > 0 Тогда
		ДанныеПростоя = Объект.Начисления.Выгрузить(, "Сотрудник,ДатаНачала,ДатаОкончания,ВремяВЧасах,ОплаченоДней,ОплаченоЧасов,Результат");
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ДанныеПростоя.Сотрудник КАК Сотрудник,
			|	ДанныеПростоя.ДатаНачала КАК ДатаНачала,
			|	ДанныеПростоя.ДатаОкончания КАК ДатаОкончания,
			|	ВЫБОР
			|		КОГДА ДанныеПростоя.ВремяВЧасах = ЛОЖЬ
			|				И ДанныеПростоя.ОплаченоДней > 0
			|			ТОГДА ОКР(ДанныеПростоя.Результат / ДанныеПростоя.ОплаченоДней, 2)
			|		КОГДА ДанныеПростоя.ВремяВЧасах = ИСТИНА
			|				И ДанныеПростоя.ОплаченоЧасов > 0
			|			ТОГДА ОКР(ДанныеПростоя.Результат / ДанныеПростоя.ОплаченоЧасов, 2)
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК СреднийЗаработок
			|ПОМЕСТИТЬ ДанныеПростоя
			|ИЗ
			|	&ДанныеПростоя КАК ДанныеПростоя
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	БольничныйЛист.ДатаОкончания КАК ДатаОкончанияБольничного
			|ПОМЕСТИТЬ ПростойБольничных
			|ИЗ
			|	Документ.БольничныйЛист КАК БольничныйЛист
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.БольничныйЛист.ПериодыПростоя КАК БольничныйЛистПериодыПростоя
			|		ПО БольничныйЛист.Ссылка = БольничныйЛистПериодыПростоя.Ссылка
			|ГДЕ
			|	БольничныйЛист.Ссылка В(&Больничные)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ПростойБольничных.Сотрудник КАК Сотрудник,
			|	ПростойБольничных.СреднийЗаработок КАК СреднийЗаработок,
			|	ДанныеПростоя.СреднийЗаработок КАК СреднийЗаработокПростой
			|ИЗ
			|	ПростойБольничных КАК ПростойБольничных
			|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПростоя КАК ДанныеПростоя
			|		ПО ПростойБольничных.Сотрудник = ДанныеПростоя.Сотрудник
			|			И ПростойБольничных.ДатаНачала = ДанныеПростоя.ДатаНачала
			|			И (ЦЕЛ(ПростойБольничных.СреднийЗаработок) = ЦЕЛ(ДанныеПростоя.СреднийЗаработок))
			|			И (ВЫБОР
			|				КОГДА ДанныеПростоя.ДатаОкончания > ПростойБольничных.ДатаОкончанияБольничного
			|					ТОГДА ПростойБольничных.ДатаОкончанияБольничного = ПростойБольничных.ДатаОкончания
			|				ИНАЧЕ ПростойБольничных.ДатаОкончания = ДанныеПростоя.ДатаОкончания
			|			КОНЕЦ)
			|ГДЕ
			|	ДанныеПростоя.Сотрудник ЕСТЬ НЕ NULL ";
		Запрос.УстановитьПараметр("Больничные", Больничные);
		Запрос.УстановитьПараметр("ДанныеПростоя", ДанныеПростоя);
		УстановитьПривилегированныйРежим(Истина);
		Результат = Запрос.Выполнить();
		УстановитьПривилегированныйРежим(Ложь);
		Выборка = Результат.Выбрать();
		Пока Выборка.Следующий() Цикл
			// Проверим погрешность округления в 1 копейку, если больше - нужен пересчет
			Погрешность = Выборка.СреднийЗаработок - Выборка.СреднийЗаработокПростой;
			Погрешность = ?(Погрешность < 0, - Погрешность, Погрешность);
			Если Погрешность <= 0.01 Тогда
				РезультатПроверки.ДанныеСотрудников.Удалить(Выборка.Сотрудник);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура РассчитатьСреднийЗаработок(ДанныеСотрудника)
	...
	Если Объект.ПланируемаяДатаВыплаты < УчетНДФЛРасширенный.ДатаЗаконаПроРКиЕАЭС() Тогда
		НулевыеЗначенияПоказателейРКСН = Справочники.ПоказателиРасчетаЗарплаты.НулевыеЗначенияПоказателейРКСН();
		ЗаполнитьЗначенияСвойств(ДанныеСотрудника, НулевыеЗначенияПоказателейРКСН);
	Иначе
