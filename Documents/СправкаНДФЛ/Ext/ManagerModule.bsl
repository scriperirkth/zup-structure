...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПроставитьСтавкуВТаблицеВычетов");
КонецПроцедуры
...
Функция ТаблицыДанныхДокументаДляПечати(Ссылка)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(2000, 1, 1), ГОД, СправкаНДФЛСведенияОДоходах.Ссылка.НалоговыйПериод - 2000), МЕСЯЦ, СправкаНДФЛСведенияОДоходах.МесяцНалоговогоПериода - 1) КАК Период,
	|	СправкаНДФЛСведенияОДоходах.Ссылка.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация
	...
	|	ВЫБОР
	|		КОГДА ПрименяемыеТарифыСтраховыхВзносовСрезПоследних.ВидТарифа = ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.АУСН)
	...
	|		ПО СправкаНДФЛСведенияОДоходах.Ссылка.Организация.ГоловнаяОрганизация = ПрименяемыеТарифыСтраховыхВзносовСрезПоследних.ГоловнаяОрганизация
	|			И (ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(2000, 1, 1), ГОД, СправкаНДФЛСведенияОДоходах.Ссылка.НалоговыйПериод - 2000), МЕСЯЦ, СправкаНДФЛСведенияОДоходах.МесяцНалоговогоПериода - 1) = НАЧАЛОПЕРИОДА(ПрименяемыеТарифыСтраховыхВзносовСрезПоследних.Период, МЕСЯЦ))
	|ГДЕ
	|	СправкаНДФЛСведенияОДоходах.Ссылка В(&Ссылка)
	|;
	|
