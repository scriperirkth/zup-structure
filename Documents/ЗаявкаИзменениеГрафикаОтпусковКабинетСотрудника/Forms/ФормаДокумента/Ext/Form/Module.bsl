...
Функция ЗначенияДляВыбораВидаДокумента()
	...
	МетаданныеОбъекта = Метаданные.Документы.ПереносОтпуска;
...
Процедура СоздатьДокументы()
	...
	ДанныеЗаявки = ОстаткиОтпусков.ДанныеЗаявкиНаПереносОтпускаПоГрафику();
	ДанныеЗаявки.Организация 		= Объект.Организация;
	ДанныеЗаявки.Сотрудник 			= Объект.Сотрудник;
	ДанныеЗаявки.ФизическоеЛицо 	= Объект.ФизическоеЛицо;
	ДанныеЗаявки.ПериодыОтпуска 	= Объект.ПериодыОтпуска.Выгрузить();
	ДанныеЗаявки.ИсходныеПериоды 	= Объект.ИсходныеПериодыОтпуска.Выгрузить();
	
	ПереносыОтпусков = ОстаткиОтпусков.СоздатьДокументыПереносОтпускаПоДаннымЗаявки(ДанныеЗаявки);
	Для каждого ПереносОтпуска Из ПереносыОтпусков Цикл
		НоваяСтрока = Объект.ДокументыПоЗаявке.Добавить();
		НоваяСтрока.ДокументСсылка = ПереносОтпуска;
	КонецЦикла;
