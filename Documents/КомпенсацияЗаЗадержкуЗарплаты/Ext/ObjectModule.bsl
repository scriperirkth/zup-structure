...
Процедура ЗарегистрироватьДоходыСтраховыхВзносов(ЗаполняемыеДвижения, НачисленияКомпенсации, Отказ)
	...
	НачисленияКомпенсации.Колонки.Добавить("СуммаПоБюджетномуТарифуНСиПЗ", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(13, 2)));
	СтатьиБюджетныйТарифНСиПЗ = ОтражениеЗарплатыВБухучетеРасширенный.СтатьиБюджетныйТарифНСиПЗ();
	...
	Для Каждого Строка Из НачисленияКомпенсации Цикл
		...
		Если СтатьиБюджетныйТарифНСиПЗ[Строка.СтатьяФинансирования] = Истина Тогда
			Строка.СуммаПоБюджетномуТарифуНСиПЗ = Строка.Сумма;
		КонецЕсли;
	КонецЦикла;
	...
	|	НачисленияКомпенсации.СуммаПоБюджетномуТарифуНСиПЗ КАК СуммаПоБюджетномуТарифуНСиПЗ,
	...
	НачисленияКомпенсации.Колонки.Удалить("СуммаПоБюджетномуТарифуНСиПЗ");
