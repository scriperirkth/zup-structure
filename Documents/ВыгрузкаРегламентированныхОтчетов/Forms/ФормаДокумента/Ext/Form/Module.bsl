...
&НаСервере
Процедура ПолучитьДополнительныеФайлыБухгалтерскойОтчетности(ТекстОсновногоФайла, ПутьВыгрузки,
	МассивДополнительныхФайлов, СписокДополнительныхФайлов, ИменаЗаписанныхФайловВыгрузки)
	...
	Если ЗначениеЗаполнено(ОтчетОснование) Тогда
		...
		// С форматов по приказу ФНС от 20.05.2026 № ЕД-1-1/326@ аудиторское заключение заменено на аудиторский отчет.
		// Способ хранения присоединенного файла в информационной оставлен без изменений.
		ЭлементыАудиторскийОтчет = КорневойУзел.ПолучитьЭлементыПоИмени("АудитОтчет");
		Если ЭлементыАудиторскийОтчет.Количество() > 0 Тогда
			УзелДополнительногоФайла = ЭлементыАудиторскийОтчет[0];
			АтрибутУзла = УзелДополнительногоФайла.Атрибуты.ПолучитьИменованныйЭлемент("НаимФайлАО");
			Если АтрибутУзла.Указан Тогда
				ЗаписьРегистраСведений = РегистрыСведений.ДополнительныеФайлыРегламентированныхОтчетов.СоздатьМенеджерЗаписи();
				ЗаписьРегистраСведений.РегламентированныйОтчет = ЗначенияРеквизитов.Ссылка;
				ЗаписьРегистраСведений.ВидДополнительногоФайла = "Аудиторское заключение";
				...
				Если ЗаписьРегистраСведений.Выбран() Тогда
					...
					МассивДополнительныхФайлов.Добавить(СтруктураДанных);
				Иначе
					СписокДополнительныхФайлов.Добавить(АтрибутУзла.Значение, "- файл аудиторского отчета, присвоив ему имя");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
