...
Процедура СторнированоВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОписаниеТаблицы = ОписаниеТаблицыСторнировано();
	ПриНажатииПолеРаспределениеПоТерриториямУсловиямТруда(ОписаниеТаблицы, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
	
	Если Не СтандартнаяОбработка Тогда
		Возврат;	
	КонецЕсли;
	
	ВыполнитьКомандуРедактированияРезультатовРаспределенияНачисленийИУдержаний(ОписаниеТаблицы, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура СторнированоПоДоговорамВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ОписаниеТаблицы = ОписаниеТаблицыСторнированоПоДоговорам();
	ПриНажатииПолеРаспределениеПоТерриториямУсловиямТруда(ОписаниеТаблицы, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка);
...
&НаСервере
Процедура НастроитьФормуПоДанным()
	
	Сотрудники = Объект.Сторнировано.Выгрузить(,"Сотрудник");
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Объект.СторнированоПоДоговорам.Выгрузить(,"Сотрудник"), Сотрудники);
	...
	ЭтоСторноДоговора = ЗначениеЗаполнено(Объект.СторнируемыйДокумент)
		И Документы.СторнированиеНачислений.РегистраторыДоговоров().СодержитТип(ТипЗнч(Объект.СторнируемыйДокумент));
	
	Элементы.СторнированоПоДоговорам.Видимость = ЭтоСторноДоговора;
	Элементы.Сторнировано.Видимость = Не ЭтоСторноДоговора;
...
Функция ОписаниеДокумента()
	...
	ЭтоСторноДоговора = Документы.СторнированиеНачислений.РегистраторыДоговоров().СодержитТип(ТипЗнч(Объект.СторнируемыйДокумент));
	...
	Описание.СторноИмя = ?(ЭтоСторноДоговора, "СторнированоПоДоговорам", "Сторнировано");
...
&НаКлиентеНаСервереБезКонтекста
Функция ОписаниеТаблицыСторнированоПоДоговорам()
	
	ОписаниеТаблицы = РасчетЗарплатыРасширенныйКлиентСервер.ОписаниеТаблицыРасчета();
	ОписаниеТаблицы.ИмяТаблицы = "СторнированоПоДоговорам";
	ОписаниеТаблицы.ПутьКДанным = "Объект.СторнированоПоДоговорам";
	ОписаниеТаблицы.НомерТаблицы = 2;
	ОписаниеТаблицы.СодержитПолеСотрудник = Истина;
	ОписаниеТаблицы.ИмяРеквизитаСотрудник = "Сотрудник";
	ОписаниеТаблицы.ИмяПоляДляВставкиПоказателей = "ДатыСторнирование";
	...
	ОписаниеТаблицы.ИмяПоляДляВставкиРаспределенияРезультатов 	= "СторнированоПоДоговорамРезультат";
	ОписаниеТаблицы.ВставлятьПослеПоля 							= Истина;
	ОписаниеТаблицы.СодержитПолеМестоПолученияДохода 			= Истина;
	ОписаниеТаблицы.СодержитПолеИсходныйДокумент				= Истина;
...
Функция ОписанияТаблицДляРаспределенияРезультата()
	...
	ОписанияТаблиц.Добавить(ОписаниеТаблицыСторнированоПоДоговорам());
...
Функция СтруктураОписанияТаблицДляРаспределенияРезультата()
	...
	ОписанияТаблиц.Вставить("СторнированоПоДоговорам", ОписаниеТаблицыСторнированоПоДоговорам());
...
Функция ОписанияТаблицСРаспределениемПоТерриториямУсловиямТруда()
	...
	Описания.Добавить(ОписаниеТаблицыСторнированоПоДоговорам());
...
Процедура ДанныеВРеквизиты()
	...
	Если Объект.РаспределениеРезультатовНачислений.Количество() > 0 Тогда
...
Процедура РассчитатьПослеЗаполнения(ДокументОбъект)
	...
	Если ИсправлениеВТекущемПериоде Тогда
		...
		Если ПараметрыСторнируемогоДокумента.Ссылка.Метаданные().Движения.Содержит(Метаданные.РегистрыРасчета.Начисления) Тогда
...
&НаКлиенте
Процедура СторнируемыйДокументОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Типы = Новый Массив();
	Типы.Добавить(Тип("ДокументСсылка.ДоговорРаботыУслуги"));
	Типы.Добавить(Тип("ДокументСсылка.ДоговорАвторскогоЗаказа"));
	Типы = Новый ОписаниеТипов(Типы);
	
	Если Типы.СодержитТип(ТипЗнч(ВыбранноеЗначение)) Тогда
		Сообщение = "";
		Если Не ПроверитьВозможностьСторнирования(ВыбранноеЗначение, Сообщение) Тогда
...
Функция ПроверитьВозможностьСторнирования(Знач Договор, Сообщение)
	...
	МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Договор);
	Возврат МенеджерДокумента.ПроверитьВозможностьСторнирования(Договор, Сообщение);
