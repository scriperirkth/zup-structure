...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПроведениеСервер.ПодготовитьНаборыЗаписейКРегистрацииДвиженийПоВидамУчета(
	...
	РеквизитыДляПроведения.Перепроведение = ?(ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.Свойство("ПрежнийИсправленныйДокумент"),
		ДополнительныеПараметры.ПрежнийИсправленныйДокумент = СторнируемыйДокумент, Ложь);
		
	ДанныеДляПроведения = ДанныеДляПроведения(РеквизитыДляПроведения, СтруктураВидовУчета, ПараметрыСторнируемого, СписокФизическихЛиц);
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		УчетНДФЛ.СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(Движения, Отказ, РеквизитыДляПроведения.Организация, ДатаОперации,
			ДанныеДляПроведения.НачисленияПоДоговорамДляУчетаНДФЛ, , , ДокументСсылка);
	...
	Если СтруктураВидовУчета.СтраховыеВзносы Тогда
		...
		УчетСтраховыхВзносов.СформироватьСведенияОДоходахПоДоговорамСтраховыеВзносы(
			Движения, Отказ, РеквизитыДляПроведения.Организация, РеквизитыДляПроведения.ПериодРегистрации, ДанныеДляПроведения.МенеджерВременныхТаблиц, Истина, РеквизитыДляПроведения.Ссылка);
	КонецЕсли;
...
Функция ТипыДокументовСторнируемыхИнтерактивно() Экспорт
	...
	ВычитаемыеТипы.Добавить(Тип("ДокументСсылка.НачислениеПоДоговорам"));
...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, СтруктураВидовУчета, ПараметрыСторнируемого, СписокФизическихЛиц)
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Или СтруктураВидовУчета.СтраховыеВзносы Тогда
		...
		МесяцНачисления = РеквизитыДляПроведения.МесяцНачисления;
		РеквизитыДляПроведения.МесяцНачисления = РеквизитыДляПроведения.ПериодРегистрацииСторнируемого;
		РасчетЗарплатыРасширенный.ЗаполнитьДанныеПоДоговорамПодряда(ДанныеДляПроведения, РеквизитыДляПроведения, СписокФизическихЛиц, "СторнированоПоДоговорам");
		РеквизитыДляПроведения.МесяцНачисления = МесяцНачисления;
...
Функция РеквизитыДляПроведения(ДокументСсылка, ПараметрыСторнируемого)
	...
	ТекстЗапроса = 
		...
		|	СторнированиеНачислений.ПериодРегистрации КАК МесяцНачисления,
...
Функция РегистраторыДоговоров() Экспорт
