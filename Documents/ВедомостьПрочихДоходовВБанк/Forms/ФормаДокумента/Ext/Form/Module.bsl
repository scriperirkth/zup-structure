...
&НаКлиенте
Процедура РедактироватьВыплатуСтроки(ДанныеСтроки, ИмяПоля = "") Экспорт
	ВедомостьПрочихДоходовКлиент.РедактироватьВыплатуСтроки(ЭтотОбъект, ДанныеСтроки, ИмяПоля);
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СоставНомерЛицевогоСчетаЦР.Имя);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Состав.СпособЗачисления");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР;
	Элемент.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь); 
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СоставИНН.Имя);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ЗарплатныйПроектПоддерживаетВыгрузкуИНН");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
...
Процедура СменитьФорматФайла()
	...
	ЗарплатныйПроектПоддерживаетВыгрузкуИНН = 
		ОбменСБанкамиПоЗарплатнымПроектам.ЗарплатныйПроектПоддерживаетВыгрузкуИНН(
			Объект.ЗарплатныйПроект);
	...
	Если Не ЗарплатныйПроектПоддерживаетСпособыЗачисленияВыплат И Не ЗарплатныйПроектДляПеречисленияНаКартыМир Тогда
		Для Каждого СтрокаСостава Из Объект.Состав Цикл
			...
			СтрокаСостава.НомерЛицевогоСчетаЦР = "";
		КонецЦикла;
	ИначеЕсли Не ЗарплатныйПроектПоддерживаетСпособыЗачисленияВыплат И ЗарплатныйПроектДляПеречисленияНаКартыМир Тогда
		Для Каждого СтрокаСостава Из Объект.Состав Цикл
			...
			СтрокаСостава.НомерЛицевогоСчетаЦР = "";
		КонецЦикла;
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЗарплатныйПроект, "ДляПеречисленийВЦифровыхРублях") Тогда
		Для Каждого СтрокаСостава Из Объект.Состав Цикл
			СтрокаСостава.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР;
			СтрокаСостава.СНИЛС = "";
			СтрокаСостава.НомерТелефонаПредставление = "";
			СтрокаСостава.НомерТелефона = "";
			СтрокаСостава.БанковскийСчет = Неопределено;
	...
	Иначе
		Для Каждого СтрокаСостава Из Объект.Состав Цикл
			Если Элементы.СоставСпособЗачисленияВыплаты.СписокВыбора.НайтиПоЗначению(СтрокаСостава.СпособЗачисления) = Неопределено Тогда
				СтрокаСостава.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету;
				СтрокаСостава.СНИЛС = "";
				СтрокаСостава.НомерТелефонаПредставление = "";
				СтрокаСостава.НомерТелефона = "";
...
Процедура УстановитьВидВыплатыНаКартыМир()
	Для Каждого СтрокаТЧЗарплата Из Объект.Выплаты Цикл
