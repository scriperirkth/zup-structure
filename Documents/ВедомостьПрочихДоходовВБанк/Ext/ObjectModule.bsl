...
Функция ИзменилисьКлючевыеРеквизитыЭлектронногоДокумента() Экспорт
	...
	ИзменилисьКлючевыеРеквизиты = 
		...
		Или ЭлектронноеВзаимодействиеБЗК.ИзмениласьТабличнаяЧастьОбъекта(ЭтотОбъект, "Состав", "СпособЗачисления, НомерБанковскойКарты, БанковскаяКарта, СНИЛС, Телефон, НомерТелефона, БанковскийСчет, БИКБанкаСчета, ВзысканнаяСумма, НомерЛицевогоСчета, НомерЛицевогоСчетаЦР, ИНН");
...
Процедура ДополнитьПоТаблицеВыплат(ТаблицаВыплат) Экспорт
	...
	ДополнитьТаблицуЗарплатСпособамиВыплаты(ТаблицаВыплат, Истина);
	КолонкиДляЗачисления = Новый Массив;
	Для Каждого Реквизит Из РеквизитыПоСпособуЗачисления() Цикл
		КолонкиДляЗачисления.Добавить(Реквизит.Имя);
	КонецЦикла;
...
Процедура ДополнитьТаблицуЗарплатСпособамиВыплаты(ТаблицаВыплат, ДобавлятьСтрокуПриОтсутствииСпособаВыплат = Ложь)
	...
	ЗначенияПоУмолчанию = ЗначенияРеквизитовСоставаВедомостиПоУмолчанию(СпособыЗачисленияПоЗарплатномуПроекту);	
	...
	Для Каждого СтрокаВыплаты Из ТаблицаВыплат Цикл
		...
		Если СтрокаЛС = Неопределено Тогда
			Если ДобавлятьСтрокуПриОтсутствииСпособаВыплат Тогда
		...
		Если СпособыЗачисленияПоЗарплатномуПроекту[СтрокаЛС.СпособЗачисления] <> Истина Тогда
			СтрокаЛС.СпособЗачисления = ЗначенияПоУмолчанию.СпособЗачисления;
		КонецЕсли;
		...
		СтрокиВыплат.Добавить(СтрокаВыплаты);
		
	КонецЦикла;
	
	ТаблицаВыплат = ТаблицаВыплат.Скопировать(СтрокиВыплат);
...
Функция РеквизитыПоСпособуЗачисления()
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МетаданныеРеквизитов, РеквизитыВедомостиВБанкДляЗачисленияПоЛицевомуСчетуЦР()); 
	
	МетаданныеРеквизитов.Добавить(Метаданные().ТабличныеЧасти.Состав.Реквизиты.СпособЗачисления);
	
	МетаданныеРеквизитов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МетаданныеРеквизитов);
...
Функция ЗначенияРеквизитовСоставаВедомостиПоУмолчанию(СпособыЗачисленияПоЗарплатномуПроекту)
	
	ЗначенияПоУмолчанию = Новый Структура;
	ЗначенияПоУмолчанию.Вставить("НомерЛицевогоСчета", "");
	ЗначенияПоУмолчанию.Вставить("НомерЛицевогоСчетаЦР", "");
	...
	ЗначенияПоУмолчанию.Вставить("ИНН", "");
	
	Если СпособыЗачисленияПоЗарплатномуПроекту[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету] Тогда 
		ЗначенияПоУмолчанию.Вставить("СпособЗачисления", Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету);
	Иначе
		Для Каждого СпособЗачисления Из СпособыЗачисленияПоЗарплатномуПроекту Цикл
			Если СпособЗачисления.Значение = Истина Тогда
				ЗначенияПоУмолчанию.Вставить("СпособЗачисления", СпособЗачисления.Ключ);
				Прервать;
			КонецЕсли;
			ЗначенияПоУмолчанию.Вставить("СпособЗачисления", Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету);
		КонецЦикла;
...
Функция СоответствиеСпособаЗачисленияКолонкеВедомостиВБанк()
	...
	СоответствиеСпособаЗачисления.Вставить(
		Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР, 
		РеквизитыВедомостиВБанкДляЗачисленияПоЛицевомуСчетуЦР());
...
Функция РеквизитыВедомостиВБанкДляЗачисленияПоЛицевомуСчету()
	...
	Возврат ОбщегоНазначенияКлиентСервер.МассивЗначений(Метаданные().ТабличныеЧасти.Состав.Реквизиты.НомерЛицевогоСчета,
														Метаданные().ТабличныеЧасти.Состав.Реквизиты.ИНН);
	
КонецФункции

Функция РеквизитыВедомостиВБанкДляЗачисленияПоЛицевомуСчетуЦР()
	
	Возврат ОбщегоНазначенияКлиентСервер.МассивЗначений(Метаданные().ТабличныеЧасти.Состав.Реквизиты.НомерЛицевогоСчетаЦР,
														Метаданные().ТабличныеЧасти.Состав.Реквизиты.ИНН);
...
Функция РеквизитыВедомостиВБанкДляЗачисленияПоСНИЛС()
	...
	Возврат ОбщегоНазначенияКлиентСервер.МассивЗначений(Метаданные().ТабличныеЧасти.Состав.Реквизиты.СНИЛС,
														Метаданные().ТабличныеЧасти.Состав.Реквизиты.ИНН);
