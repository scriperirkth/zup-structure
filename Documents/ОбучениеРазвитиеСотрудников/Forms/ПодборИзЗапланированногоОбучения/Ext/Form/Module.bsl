...
Процедура ВыборСотрудниковНаКлиенте()
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		СотрудникСумма = Новый Соответствие;
		...
		Если ТекущиеДанные.Подобрано 
			...
			Продолжить;
		КонецЕсли;
		СотрудникСумма.Вставить(ТекущиеДанные.Сотрудник,ТекущиеДанные.СуммаРасходов);
		МассивСотрудников.Добавить(СотрудникСумма);
		ТекущиеДанные.Подобрано = Истина;
	КонецЦикла;

	Оповестить("ПодобранНовыйСотрудникИзПланаОбучения", МассивСотрудников, ЭтотОбъект.ВладелецФормы);
...
Процедура ЗаполнитьТаблицуЗапланированногоОбучения()
	...
	Запрос.Текст = 
		...
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК Подобрано,
		|	ПланОбученияСотрудников.СуммаРасходов КАК СуммаРасходов
		|ИЗ
