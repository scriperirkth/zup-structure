...
&НаКлиенте
Процедура ЛицевыеСчетаИННПриИзменении(Элемент)
	ТекущиеДанные = Элементы.ЛицевыеСчета.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ФиксацияЗафиксироватьИзменениеРеквизита("ИНН");
	СотрудникиКлиентСервер.ОбработатьОтображениеПоляИНН(ТекущиеДанные.ИНН, Элемент, ЭтотОбъект);
КонецПроцедуры
...
Функция ФиксацияЭлементыОбработчикаЗафиксироватьИзменение()
	...
	ОписаниеЭлементов.Вставить("ЛицевыеСчетаИНН",						"ИНН");
