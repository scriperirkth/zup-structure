...
Процедура ПриЗаписи(Отказ)
	Если ЗарплатаКадры.ОтключитьБизнесЛогикуПриЗаписи(ЭтотОбъект) Тогда
		Возврат;
	КонецЕсли;
	НаборРегистраций = РегистрыСведений.РегистрацииОтветовНаЗапросыФССДляРасчетаПособий.ЗаполнитьПоДокументу(ЭтотОбъект);
	Документы.ВходящийЗапросФССДляРасчетаПособия.ПриЗаписиОтветаНаЗапрос(ЭтотОбъект);
	Документы.УведомлениеОбОтсутствииОтветаСЭДО.ПриЗаписиОтветаНаЗапрос(ЭтотОбъект, НаборРегистраций);
...
Функция ЗначенияРеквизитовДоЗаписи() Экспорт
	ИменаРеквизитов = "Дата, НомерЛН, ГоловнаяОрганизация, ФизическоеЛицо, ПометкаУдаления, ВходящийЗапрос,
	|НомерПроцесса, Проведен, ДатаОтправки";
	ЭтоНовый = ЭтоНовый();
	Если ЭтоНовый Тогда
...
Функция ЗаполнитьПервичныеДанныеИзВходящегоЗапроса(ПараметрыФиксации) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыЗапроса.Сотрудник) Тогда
		Реквизиты.Вставить("Сотрудник", РеквизитыЗапроса.Сотрудник);
	ИначеЕсли ЗначениеЗаполнено(РеквизитыЗапроса.ФизическоеЛицо) И Не ЗначениеЗаполнено(Сотрудник) Тогда
		Реквизиты.Вставить("ФизическоеЛицо", РеквизитыЗапроса.ФизическоеЛицо);
	КонецЕсли;
	Если ЗначениеЗаполнено(РеквизитыЗапроса.Ребенок) Тогда
...
Функция ДанныеЗаполняемыеИзПервичногоДокумента()
	...
	Если ДанныеВходящегоЗапроса <> Неопределено Тогда
		Если ЗначениеЗаполнено(ДанныеВходящегоЗапроса.Сотрудник) Тогда
			Реквизиты.Удалить("Сотрудник");
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеВходящегоЗапроса.ФизическоеЛицо) Тогда
			Реквизиты.Удалить("ФизическоеЛицо");
		КонецЕсли;
