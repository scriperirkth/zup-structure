...
Процедура СотрудникиСпособКомпенсацииПереработкиПриИзменении(Элемент)
	...
	Если ТекущиеДанные.СпособКомпенсацииПереработки = ПредопределенноеЗначение("Перечисление.СпособыКомпенсацииПереработки.ПовышеннаяОплата") Тогда
		СотрудникКРасчету = Новый Массив;
		СотрудникКРасчету.Добавить(ТекущиеДанные.Сотрудник);
		РассчитатьСотрудников(СотрудникКРасчету);
	Иначе
...
Процедура РассчитатьСотрудников(МассивСотрудников, Очищать = Ложь)
	...
	Если Очищать Тогда
		...
		Для каждого Сотрудник Из ТаблицаСотрудников Цикл
			...
			НоваяСтрока.Всего = Сотрудник.Переработки;
			НоваяСтрока.Сверхурочно1_5 = Сотрудник.Переработки1_5;
			НоваяСтрока.Сверхурочно2 = Сотрудник.Переработки - Сотрудник.Переработки1_5;
		КонецЦикла; 
	Иначе
		Для каждого Сотрудник Из МассивСотрудников Цикл
			...
			Если СтрокаСотрудник.Количество() > 0 Тогда
				...
				ТекущиеДанные.Всего = РассчитанныйСотрудник.Переработки;
				ТекущиеДанные.Сверхурочно1_5 = РассчитанныйСотрудник.Переработки1_5;
				ТекущиеДанные.Сверхурочно2 = РассчитанныйСотрудник.Переработки - РассчитанныйСотрудник.Переработки1_5;
			КонецЕсли;
		КонецЦикла; 
	КонецЕсли;
