...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7434b301-2df1-11f1-8174-4cedfb43b11a");
	Обработчик.Процедура = "Документы.ЗапросСтатусовВыплатыПособия.ЗаполнитьОрганизацию";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение организации в запросах статусов выплаты пособий.'");
...
Процедура ЗаполнитьОрганизацию(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(Ответ.Организация, Шапка.Страхователь) КАК Организация,
	|	ЕСТЬNULL(ОтчитывающиесяПоВзносамОрганизации.ОтчитывающаясяОрганизация, ЕСТЬNULL(Ответ.Организация, Шапка.Страхователь)) КАК СтраховательОрганизации
	|ИЗ
	|	Документ.ЗапросСтатусовВыплатыПособия КАК Шапка
	...
	|		ПО (ЕСТЬNULL(Ответ.Организация, Шапка.Страхователь) = ОтчитывающиесяПоВзносамОрганизации.Организация)
	...
	Пока Выборка.Следующий() Цикл
		Если ОбработчикОтложенный Тогда
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
					ПараметрыОбновления,
					"Документ.ЗапросСтатусовВыплатыПособия",
					"Ссылка",
		...
		Если ДокументОбъект.ТипСообщения = 0 Тогда
			ДокументОбъект.ТипСообщения = 84;
		КонецЕсли;
	...
	Документы.СтатусыВыплатыПособия.ЗаполнитьОрганизациюПоМассивуЗапросов(ПараметрыОбновления, ИзмененныеЗапросы);
