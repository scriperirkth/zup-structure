...
Функция ЗначенияРеквизитовДоЗаписи()
	ИменаРеквизитов = "Дата, Организация, Страхователь, ГоловнаяОрганизация, ФизическоеЛицо, ДатаОтправки,
...
Функция ОбновитьВторичныеДанные(ПараметрыФиксации = Неопределено) Экспорт
	...
	Если ЗаполнитьПоОтветуНаЗапрос(ПараметрыФиксации) Тогда
		Модифицирован = Истина;
	КонецЕсли;
	Если ЗаполнитьСтрахователя() Тогда
		Модифицирован = Истина;
	КонецЕсли;
...
Функция ЗаполнитьПоОтветуНаЗапрос(ПараметрыФиксации = Неопределено)
	...
	Реквизиты = Новый Структура("ФизическоеЛицо, Организация");
...
Функция ЗаполнитьСтрахователя()
	Если ФиксацияВторичныхДанныхВДокументах.РеквизитШапкиЗафиксирован(ЭтотОбъект, "Страхователь") Тогда
		Возврат Ложь;
	КонецЕсли;
	НовоеЗначение = СЭДОФСС.СтраховательОрганизации(Организация);
	Возврат ФиксацияВторичныхДанныхВДокументах.ИзменитьЗначениеРеквизита(Страхователь, НовоеЗначение);
КонецФункции
...
Функция ЗаполнитьГоловнуюОрганизацию()
	...
	ИначеЕсли ЗначениеЗаполнено(Страхователь) Тогда
