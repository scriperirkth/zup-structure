...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ИсправитДвиженияЗанятостьПозицийШтатногоРасписанияПоСовместителям");
...
Процедура ИсправитДвиженияЗанятостьПозицийШтатногоРасписанияПоСовместителям(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ЗанятостьПозицийШтатногоРасписанияИнтервальный.РегистраторЗаписи = ОтпускПоУходуЗаРебенком.Ссылка
		|			И (ЗанятостьПозицийШтатногоРасписанияИнтервальный.ДокументОснование ССЫЛКА Документ.Совмещение)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтпускПоУходуЗаРебенком КАК ОтпускПоУходуЗаРебенкомПродолжение
		|		ПО (ОтпускПоУходуЗаРебенком.ДатаНачала = ДОБАВИТЬКДАТЕ(ОтпускПоУходуЗаРебенкомПродолжение.ДатаОкончания, ДЕНЬ, 1))
		|			И (ОтпускПоУходуЗаРебенком.Проведен)
		|			И (ОтпускПоУходуЗаРебенкомПродолжение.Проведен)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 1000
		|	ВозвратИзОтпускаПоУходуЗаРебенком.Ссылка КАК Регистратор,
		...
		|ИЗ
		|	Документ.ВозвратИзОтпускаПоУходуЗаРебенком КАК ВозвратИзОтпускаПоУходуЗаРебенком
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтпускаПоУходу КАК ОтпускаПоУходу
		|		ПО ВозвратИзОтпускаПоУходуЗаРебенком.ДокументОснование = ОтпускаПоУходу.РегистраторЗаписи
		|			И (ВозвратИзОтпускаПоУходуЗаРебенком.Проведен)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратИзОтпускаПоУходуЗаРебенком КАК ВозвратИзОтпускаПоУходуЗаРебенкомИсправление
		|		ПО ВозвратИзОтпускаПоУходуЗаРебенком.Ссылка = ВозвратИзОтпускаПоУходуЗаРебенкомИсправление.ИсправленныйДокумент
		|			И (ВозвратИзОтпускаПоУходуЗаРебенкомИсправление.Проведен)
		|ГДЕ
		|	ВозвратИзОтпускаПоУходуЗаРебенкомИсправление.Ссылка ЕСТЬ NULL
		|	И НЕ ВозвратИзОтпускаПоУходуЗаРебенком.Ссылка В (&МассивОбновленных)";
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивРегистраторов.Добавить(Выборка.Регистратор);
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
			ПараметрыОбновления, "РегистрСведений.ЗанятостьПозицийШтатногоРасписания.НаборЗаписей",
			"Регистратор", Выборка.Регистратор) Тогда
			Продолжить;
		КонецЕсли;
		Сотрудники = КадровыйУчетРасширенный.МассивСотрудников(Выборка.ФизическоеЛицо, Выборка.Организация, Выборка.ДатаВозврата);
		ДокументОбъект = Выборка.Регистратор.ПолучитьОбъект();
		...
		ЗарплатаКадрыРасширенный.УстановитьВремяРегистрацииДокумента(
			ДокументОбъект.Движения,
			КадровыйУчетРасширенный.ТаблицаСотрудникиДатыСобытия(Сотрудники, Выборка.ДатаВозврата),
			Выборка.Регистратор);
		
		СформироватьДвиженияВозвратаВременноОсвобожденныхПозиции(
			ДокументОбъект.Движения, Сотрудники, Выборка.ДатаВозврата);
	...
	Если ВсегоДокументов = 1000 Тогда
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПродолжитьОбработчик(ПараметрыОбновления);
	Иначе
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ЗавершитьОбработчик(ПараметрыОбновления);
	КонецЕсли;
...
Процедура СформироватьДвиженияВозвратаВременноОсвобожденныхПозиции(Движения, Сотрудники, ДатаВозврата) Экспорт
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ЗанятостьПозиций.ДокументОснование ССЫЛКА Документ.Совмещение
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиПозицийШтатногоРасписания.Совмещена)
		...
		|	И ВЫБОР
		|			КОГДА ЗанятостьПозиций.ДокументОснование ССЫЛКА Документ.Совмещение
		|				ТОГДА Не ЗанятостьПозиций.РегистраторЗаписи ССЫЛКА Документ.ОтменаСовмещения
		|			ИНАЧЕ ЗанятостьПозиций.ЗамещаемыйСотрудник = ЗанятостьПозиций.Сотрудник
		|		КОНЕЦ";
