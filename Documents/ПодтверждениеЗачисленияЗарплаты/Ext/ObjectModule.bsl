...
Процедура ЗаполнитьДокументИзОбъектаXDTO(ОбъектXDTO, ХешСумма, СсылкаНаПервичныйДокумент, Отказ) Экспорт
	...
	СтруктураДанныхДляЗаполненияДокумента = ОбменСБанкамиПоЗарплатнымПроектам.СтруктураДляЗаполненияДокументаПоПодтверждениюБанкаПоXML(
	...
	СопоставлениеПоНомеруСчетаЦР = СтруктураДанныхДляЗаполненияДокумента.Сотрудники.Колонки.Найти("НомерЛицевогоСчетаЦР") <> Неопределено;
	...
	Если СопоставлениеПоЛицевомуСчету Тогда
		НомераЛицевыхСчетов = ?(СопоставлениеПоНомеруСчетаЦР,
			СтруктураДанныхДляЗаполненияДокумента.Сотрудники.ВыгрузитьКолонку("НомерЛицевогоСчетаЦР"),
	...
	Если СопоставлениеПоЛицевомуСчету Тогда
		...
		Для Каждого СтрокаТЧ Из СтруктураДанныхДляЗаполненияДокумента.Сотрудники Цикл
			СтрокаТЧ.ФизическоеЛицо = ФизическиеЛицаЛицевыхСчетов.Получить(?(СопоставлениеПоНомеруСчетаЦР, СтрокаТЧ.НомерЛицевогоСчетаЦР, СтрокаТЧ.НомерЛицевогоСчета));
		КонецЦикла;
...
Процедура ЗаполнитьДокументИзОбъектаJSON(ДанныеJSON, ХешСумма, СсылкаНаПервичныйДокумент, Отказ) Экспорт
	
	ПервичныйДокумент = СсылкаНаПервичныйДокумент;
	
	СтруктураДанныхДляЗаполненияДокумента = ОбменСБанкамиПоЗарплатнымПроектам.СтруктураДляЗаполненияДокументаПоПодтверждениюБанкаПоJSON(
			"ПодтверждениеЗачисленияЗарплаты", ДанныеJSON, ХешСумма, ПервичныйДокумент, Отказ);
