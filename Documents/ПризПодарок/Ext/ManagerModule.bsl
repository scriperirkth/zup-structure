...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПроставитьКатегориюДохода");
КонецПроцедуры
...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, СтруктураВидовУчета) 
	...
	СтатьяРасходов = ОтражениеЗарплатыВБухучетеРасширенный.СтатьяОплатаТруда();
	Если РеквизитыДляПроведения.Свойство("Ссылка") Тогда
		СтатьяРасходовДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыДляПроведения.Ссылка, "СпособРасчетовСФизическимиЛицами");
		Если ЗначениеЗаполнено(СтатьяРасходовДокумента) Тогда
			ОписаниеСтатейРасходов = ЗарплатаКадры.СтатьиРасходовПоСпособамРасчетовСФизическимиЛицами();
			СтатьяРасходов = ОписаниеСтатейРасходов[СтатьяРасходовДокумента];	
		КонецЕсли;
	КонецЕсли;  
