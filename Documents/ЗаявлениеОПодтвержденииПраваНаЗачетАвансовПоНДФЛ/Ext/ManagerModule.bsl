...
Функция ПечатнаяФормаЗаявления(МассивОбъектов, ОбъектыПечати)
	...
	СоответствиеРегистрацияПредставитель = Новый Соответствие;
	...
	Пока ВыборкаПоШапкеДокумента.Следующий() Цикл
		...
		Сведения = СоответствиеРегистрацияПредставитель.Получить(ВыборкаПоШапкеДокумента.РегистрацияВНалоговомОргане);
		Если Сведения = Неопределено Тогда
			Сведения = РегистрацияВНОСервер.ДанныеРегистрации(ВыборкаПоШапкеДокумента.РегистрацияВНалоговомОргане);
			СоответствиеРегистрацияПредставитель.Вставить(ВыборкаПоШапкеДокумента.РегистрацияВНалоговомОргане,Сведения);
		КонецЕсли;
			
		Если ЗначениеЗаполнено(Сведения.Представитель) Тогда // Представитель
			МакетСтраницы.Параметры.ВидПодписанта = "3";
			ДанныеФЛ = РегламентированнаяОтчетность.ПолучитьФИОФизЛица(Сведения.Представитель, ВыборкаПоШапкеДокумента.Дата);
			
			ИННРуководителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сведения.Представитель,"ИНН");
			
			Если РегламентированнаяОтчетность.ПредставительЯвляетсяФизЛицом(Сведения.Представитель) Тогда
				ФИОРуководителя = СокрЛП(ДанныеФЛ.Фамилия) + ?(ЗначениеЗаполнено(ДанныеФЛ.Фамилия)," ","") + СокрЛП(ДанныеФЛ.Имя)
					+ ?(ЗначениеЗаполнено(ДанныеФЛ.Имя)," ","") + СокрЛП(ДанныеФЛ.Отчество);
			Иначе
				ФИОРуководителя = Сведения.УполномоченноеЛицоПредставителя;
			КонецЕсли;
			ДокументПредставителя = Сведения.ДокументПредставителя;
		Иначе
			МакетСтраницы.Параметры.ВидПодписанта = Формат(ОрганизацияЭтоЮрЛицо,"БИ=1; БЛ=2"); // налоговый агент
			ФИОРуководителя = ВыборкаПоШапкеДокумента.ФамилияРуководителя + ?(ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ФамилияРуководителя)," ","")
					+ ВыборкаПоШапкеДокумента.ИмяРуководителя + ?(ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ИмяРуководителя)," ","")
					+ ВыборкаПоШапкеДокумента.ОтчествоРуководителя;
			ИННРуководителя = ВыборкаПоШапкеДокумента.ИННРуководителя;
			ДокументПредставителя = НезаполненнаяСтрока;
		КонецЕсли;
		...
		ЗарплатаКадры.ВывестиДанныеПоБуквенно(?(ЗначениеЗаполнено(ИННРуководителя), ИННРуководителя, НезаполненнаяСтрока), МакетСтраницы, "ИННРуководителя", 12);
		...
		ЗарплатаКадры.ВывестиДанныеПоБуквенно(ДокументПредставителя, МакетСтраницы, "НаимДок", 40);
