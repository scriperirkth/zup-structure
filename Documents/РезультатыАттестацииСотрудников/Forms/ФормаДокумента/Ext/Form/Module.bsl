...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйРезерв") Тогда
		...
		МодульКадровыйРезервФормы.РезультатыАттестацииПриСозданииНаСервере(ЭтотОбъект);
	КонецЕсли;
...
Процедура Подключаемый_ИсключениеИзКадровогоРезерва(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйРезерв") Тогда
		МодульКадровыйРезервКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("КадровыйРезервКлиент");
		МодульКадровыйРезервКлиент.ИсключитьИзКадровогоРезерваПоРезультатамАттестации(ЭтотОбъект);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_РезультатыАттестацииПозицияРезерваПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ПозицияРезерва.Пустая() Тогда
		ВидРезерва = Неопределено;
		ТекущиеДанные.СтатусГотовности = Неопределено;
	Иначе
		ВидРезерва = ВидРезерваПозиции(ТекущиеДанные.ПозицияРезерва);
	КонецЕсли;
	
	ТекущиеДанные.ВидРезерва = ВидРезерва;
...
Процедура ЗаполнитьПлановыеНачисленияСотрудников(ТаблицаПодбораПлановыхНачислений)
	...
	Для каждого ДанныеНачисленийСотрудника Из ДанныеНачислений Цикл
		...
		Для каждого ОписаниеНачисления Из ДанныеНачисленийСотрудника.Значение.Начисления Цикл
			...
			ВидРасчетаИнфо = ЗарплатаКадрыРасширенныйПовтИсп.ПолучитьИнформациюОВидеРасчета(ПлановоеНачислениеСотрудника.Начисление);
			Если Не ВидРасчетаИнфо.Рассчитывается Тогда 
				ПлановоеНачислениеСотрудника.ФиксированнаяСумма = Истина;
				Если ПлановоеНачислениеСотрудника.Свойство("ТочностьПоказателя1") Тогда 
					ПлановоеНачислениеСотрудника["ТочностьПоказателя1"] = "ЧДЦ=2";
					Если ПлановоеНачислениеСотрудника.Свойство("Значение1") Тогда
						ПлановоеНачислениеСотрудника["Значение1"] = ПлановоеНачислениеСотрудника.Размер;
						Если Не ВидРасчетаИнфо.ВключатьВФОТ Тогда
							ПлановоеНачислениеСотрудника.Размер = 0;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
&НаСервере
Функция ВидРезерваПозиции(ПозицияРезерва)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйРезерв.КадровыйРезервГосслужащих") Тогда
		...
		ВидРезерва = МодульКадровыйРезервГосслужащих.СвойстваПозицииКадровогоРезерва(ПозицияРезерва).ВидРезерва;
	КонецЕсли;
