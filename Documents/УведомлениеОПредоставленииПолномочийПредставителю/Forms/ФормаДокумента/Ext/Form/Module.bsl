...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = РегламентированнаяОтчетностьКлиент.ИмяСобытияЗавершениеОтправки() Тогда
...
Процедура ИнициализироватьДанные(Параметры)
	...
	УведомлениеПрименяется = ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.УПУП");
...
Процедура УправлениеФормой(Форма)
	...
	Форма.Элементы.ФормаКомандаОтправить.Видимость = НЕ Форма.УведомлениеОтправлено И Форма.УведомлениеПрименяется;
...
Процедура Подключаемый_ДетальноеОписаниеСтатусаПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(Результат, "e1cib/command") Тогда
		
		ПерейтиПоНавигационнойСсылке(Результат);
