...
Функция ДанныеИзФайлаОбмена(
	...
	ИначеЕсли ТипЗнч(ВходящиеДанные) = Тип("Строка") Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ВходящиеДанные, "ВходящиеДанные", "ДанныеИзФайлаОбмена");		
		ЭтоФайл = Истина;
	Иначе
...
Функция ПроверитьВозможностьВыгрузки(ОбъектИлиСсылка) Экспорт
	...
	ИначеЕсли Страхователь_ЮридическоеЛицо Тогда
		Если НЕ ЗначениеЗаполнено(РеквизитыСтрахователя.РегистрационныйНомерПФР) Тогда
			ТекстСообщенияПользователю = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
				НСтр("ru = 'Не задан регистрационный номер СФР страхователя'"), Истина);
		...
		ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьРегистрационныйНомерПФР(
			...
			ТекстСообщенияПользователю = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
				НСтр("ru = 'Регистрационный номер СФР должен состоять из 10 цифр (ХХХХХХХХХХ) или 12 цифр (ХХХ-ХХХ-ХХХХХХ)'"), Истина);
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле", ТекстСообщенияПользователю));
		КонецЕсли;
	...
	ИначеЕсли Представитель_ЮридическоеЛицо Тогда
		Если НЕ ЗначениеЗаполнено(РеквизитыПредставителя.РегистрационныйНомерПФР) Тогда
			ТекстСообщенияПользователю = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
				НСтр("ru = 'Не задан регистрационный номер СФР представителя'"), Истина);
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле", ТекстСообщенияПользователю));
		ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьРегистрационныйНомерПФР(
...
Функция ПодготовитьДокументDOM(ВходящиеДанные)
	...
	ИначеЕсли ТипЗнч(ВходящиеДанные) = Тип("Строка") Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ВходящиеДанные, "ВходящиеДанные", "ПодготовитьДокументDOM");		
		ЭтоФайл = Истина;
	Иначе
		ТекстОшибки = НСтр("ru = 'Передан недопустимый параметр ВходящиеДанные %1.'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция НайтиДокументИБ(ЗначениеПоУмолчанию, ФильтрыПоиска)
	...
	Для каждого СтрокаКлюча Из ФильтрыПоиска Цикл
		...
		Если СтрокаКлюча.Ключ = "НаименованиеДокумента"
			ИЛИ СтрокаКлюча.Ключ = "КемВыдан"
			ИЛИ СтрокаКлюча.Ключ = "Комментарий" Тогда
...
Функция ЗаписатьДокумент(ДанныеУведомления, ОбновлятьСуществующийИлиСсылкаИлиРежим)
	...
	Иначе
		Если ОбновлятьСуществующийИлиСсылкаИлиРежим = Истина
			...
			СтрокаПоиска.Вставить("НаименованиеДокумента", ДанныеУведомления.НаименованиеДокумента);
			СтрокаПоиска.Вставить("КемВыдан", ДанныеУведомления.КемВыдан);
			СтрокаПоиска.Вставить("ДатаНачалаДействия", ДанныеУведомления.ДатаНачалаДействия);
			СтрокаПоиска.Вставить("ДатаОкончанияДействия", ДанныеУведомления.ДатаОкончанияДействия);
			СтрокаПоиска.Вставить("Страхователь", ДанныеУведомления.Страхователь);
			СтрокаПоиска.Вставить("СтраховательСотрудник", ДанныеУведомления.СтраховательСотрудник);
			СтрокаПоиска.Вставить("Представитель", ДанныеУведомления.Представитель);
			СтрокаПоиска.Вставить("ПредставительСотрудник", ДанныеУведомления.ПредставительСотрудник);
			СтрокаПоиска.Вставить("Получатель", ДанныеУведомления.Получатель);
			СтрокаПоиска.Вставить("РегНомерПФР", ДанныеУведомления.РегНомерПФР);
			СтрокаПоиска.Вставить("Комментарий", ДанныеУведомления.Комментарий);
...
Функция НовыеДанныеУведомления()
	...
	ДанныеУведомления.Вставить("НомерДокумента", "");
	ДанныеУведомления.Вставить("НаименованиеДокумента", "");
	ДанныеУведомления.Вставить("КемВыдан", "");
	ДанныеУведомления.Вставить("ДатаНачалаДействия", Дата('00010101'));
	ДанныеУведомления.Вставить("ДатаОкончанияДействия", Дата('00010101'));
	ДанныеУведомления.Вставить("Страхователь", "");
	ДанныеУведомления.Вставить("СтраховательСотрудник", "");
	ДанныеУведомления.Вставить("Представитель", "");
	ДанныеУведомления.Вставить("Представитель", "");
	ДанныеУведомления.Вставить("ПредставительСотрудник", "");
	ДанныеУведомления.Вставить("Получатель", "");
	ДанныеУведомления.Вставить("РегНомерПФР", "");
	ДанныеУведомления.Вставить("Комментарий", "");
