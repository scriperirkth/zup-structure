...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КарточкаУчетаИсполнительныхДокументовСОплатами") Тогда
		ДанныеПечатиОбъектов = ДанныеПечатиКарточкиУчетаИсполнительногоДокументаСОплатами(МассивОбъектов);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СправкаКЗаявлениюОБанкротстве") Тогда
		ДанныеПечатиОбъектов = ДанныеПечатиСправкиКЗаявлениюОБанкротстве(МассивОбъектов);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаявлениеОВыдачеСправки") Тогда
		ДанныеПечатиОбъектов = ДанныеПечатиЗаявленияОВыдачеСправки(МассивОбъектов);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
...
Функция ДанныеПечатиКарточкиУчетаИсполнительногоДокументаСОплатами(МассивОбъектов) Экспорт
	...
	ЧастиЗапроса.Добавить(
		...
		|ПОМЕСТИТЬ ВТВыплатыПредварительно
		|ИЗ
		...
		|	ВЫБОР
		|		КОГДА ВТНачисленияУдержания.ГруппаНачисленияУдержанияВыплаты = ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Начислено)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЕстьНачислениеЗаМесяц,
		|	ВЫБОР
		|		КОГДА Начисления.НачисляетсяТолькоПриРасчетеПервойПоловиныМесяца
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК НачисляетсяТолькоВАванс
		|ПОМЕСТИТЬ ВТНачисленияУдержанияАвансом
		|ИЗ
		|	РегистрНакопления.НачисленияУдержанияПоСотрудникамАвансом КАК НачисленияУдержанияПоСотрудникамАвансом
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПлановыеУдержанияПериоды КАК ПлановыеУдержанияПериоды
		|		ПО НачисленияУдержанияПоСотрудникамАвансом.Организация.ГоловнаяОрганизация = ПлановыеУдержанияПериоды.Организация
		|			И НачисленияУдержанияПоСотрудникамАвансом.ФизическоеЛицо = ПлановыеУдержанияПериоды.ФизическоеЛицо
		|			И (НачисленияУдержанияПоСотрудникамАвансом.Период >= НАЧАЛОПЕРИОДА(ПлановыеУдержанияПериоды.НачалоПериода, МЕСЯЦ))
		|			И НачисленияУдержанияПоСотрудникамАвансом.Период <= ПлановыеУдержанияПериоды.КонецПериода
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудники КАК ВТСотрудники
		|		ПО (ВТСотрудники.ИсполнительныйЛист = ПлановыеУдержанияПериоды.ДокументОснование)
		|			И (ВТСотрудники.Удержание = ПлановыеУдержанияПериоды.Удержание)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияУдержания КАК ВТНачисленияУдержания
		|		ПО НачисленияУдержанияПоСотрудникамАвансом.Организация = ВТНачисленияУдержания.Организация
		|			И НачисленияУдержанияПоСотрудникамАвансом.ФизическоеЛицо = ВТНачисленияУдержания.ФизическоеЛицо
		|			И НачисленияУдержанияПоСотрудникамАвансом.Период = ВТНачисленияУдержания.Период
		|			И НачисленияУдержанияПоСотрудникамАвансом.Сотрудник = ВТНачисленияУдержания.Сотрудник
		|			И НачисленияУдержанияПоСотрудникамАвансом.НачислениеУдержание = ВТНачисленияУдержания.НачислениеУдержание
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовРасчета.Начисления КАК Начисления
		|		ПО НачисленияУдержанияПоСотрудникамАвансом.НачислениеУдержание = Начисления.Ссылка
		|ГДЕ
		|	НачисленияУдержанияПоСотрудникамАвансом.ГруппаНачисленияУдержанияВыплаты В (ЗНАЧЕНИЕ(перечисление.ГруппыНачисленияУдержанияВыплаты.Начислено), ЗНАЧЕНИЕ(перечисление.ГруппыНачисленияУдержанияВыплаты.Удержано))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ВТНачисленияУдержанияАвансом.ЕстьНачислениеЗаМесяц) КАК ЕстьНачислениеЗаМесяц,
		|	ВТНачисленияУдержанияАвансом.Организация КАК Организация,
		|	ВТНачисленияУдержанияАвансом.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТНачисленияУдержанияАвансом.Период КАК Период,
		|	МАКСИМУМ(ВТНачисленияУдержанияАвансом.НачисляетсяТолькоВАванс) КАК НачисляетсяТолькоВАванс
		|ПОМЕСТИТЬ ВТНачисленияЗаМесяц
		|ИЗ
		|	ВТНачисленияУдержанияАвансом КАК ВТНачисленияУдержанияАвансом
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТНачисленияУдержанияАвансом.Организация,
		|	ВТНачисленияУдержанияАвансом.ФизическоеЛицо,
		|	ВТНачисленияУдержанияАвансом.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТНачисленияУдержанияАвансом.ИсполнительныйЛист КАК ИсполнительныйЛист,
		|	ВТНачисленияУдержанияАвансом.Период КАК Период,
		|	ВТНачисленияУдержанияАвансом.Организация КАК Организация,
		|	ВТНачисленияУдержанияАвансом.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТНачисленияУдержанияАвансом.НачислениеУдержание КАК НачислениеУдержание,
		|	ВТНачисленияУдержанияАвансом.Сумма КАК Сумма
		|ПОМЕСТИТЬ ВТБазаИсполнительныхЛистовАвансомПредварительно
		|ИЗ
		|	ВТНачисленияУдержанияАвансом КАК ВТНачисленияУдержанияАвансом
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТБазовыеНачисления КАК БазовыеНачисления
		|		ПО ВТНачисленияУдержанияАвансом.Удержание = БазовыеНачисления.Удержание
		|			И ВТНачисленияУдержанияАвансом.НачислениеУдержание = БазовыеНачисления.БазовоеНачисление
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВТНачисленияУдержанияАвансом.ИсполнительныйЛист,
		|	ВТНачисленияУдержанияАвансом.Период,
		|	ВТНачисленияУдержанияАвансом.Организация,
		|	ВТНачисленияУдержанияАвансом.ФизическоеЛицо,
		|	ВТНачисленияУдержанияАвансом.НачислениеУдержание,
		|	ВТНачисленияУдержанияАвансом.Сумма
		|ИЗ
		|	ВТНачисленияУдержанияАвансом КАК ВТНачисленияУдержанияАвансом
		|ГДЕ
		|	ВТНачисленияУдержанияАвансом.НачислениеУдержание В(&ВидыНачисленийДополнения)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТБазаИсполнительныхЛистовАвансомПредварительно.ИсполнительныйЛист КАК ИсполнительныйЛист,
		|	ВТБазаИсполнительныхЛистовАвансомПредварительно.Период КАК Период,
		|	СУММА(ВТБазаИсполнительныхЛистовАвансомПредварительно.Сумма) КАК Сумма
		|ПОМЕСТИТЬ ВТБазаИсполнительныхЛистовАвансом
		|ИЗ
		|	ВТБазаИсполнительныхЛистовАвансомПредварительно КАК ВТБазаИсполнительныхЛистовАвансомПредварительно
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТБазаИсполнительныхЛистовАвансомПредварительно.ИсполнительныйЛист,
		|	ВТБазаИсполнительныхЛистовАвансомПредварительно.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТНачисленияУдержанияАвансом.Период КАК Период,
		|	СУММА(ВЫБОР
		|			КОГДА ВТНачисленияУдержанияАвансом.НачислениеУдержание ССЫЛКА ПланВидовРасчета.Начисления
		|					ИЛИ ВТНачисленияУдержанияАвансом.НачислениеУдержание В (&ВидыНачисленийДополнения)
		|				ТОГДА ВТНачисленияУдержанияАвансом.Сумма
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК СуммаНачислений,
		|	СУММА(ВЫБОР
		|			КОГДА ВТНачисленияУдержанияАвансом.НачислениеУдержание В (&ВидыНДФЛ)
		|				ТОГДА ВТНачисленияУдержанияАвансом.Сумма
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК НДФЛ
		|ПОМЕСТИТЬ ВТНачисленияНалогиАвансом
		|ИЗ
		|	ВТНачисленияУдержанияАвансом КАК ВТНачисленияУдержанияАвансом
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	УдержанияПоСотрудникамАвансом.Период КАК Период,
		|	УдержанияПоСотрудникамАвансом.Организация КАК Организация,
		|	УдержанияПоСотрудникамАвансом.ФизическоеЛицо КАК ФизическоеЛицо,
		|	УдержанияПоСотрудникамАвансом.ДокументОснование КАК ИсполнительныйДокумент,
		|	УдержанияПоСотрудникамАвансом.Сумма КАК СуммаУдержания
		|ПОМЕСТИТЬ ВТУдержанияПоИсполнительнымДокументамАвансом
		|ИЗ
		|	РегистрНакопления.УдержанияПоСотрудникамАвансом КАК УдержанияПоСотрудникамАвансом
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудники КАК Сотрудники
		|		ПО УдержанияПоСотрудникамАвансом.Организация = Сотрудники.Организация
		|			И УдержанияПоСотрудникамАвансом.ФизическоеЛицо = Сотрудники.ФизическоеЛицо
		|			И УдержанияПоСотрудникамАвансом.ДокументОснование = Сотрудники.ИсполнительныйЛист
		|
		|СГРУППИРОВАТЬ ПО
		|	УдержанияПоСотрудникамАвансом.Период,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ВТУдержанияПоИсполнительнымДокументамАвансом.Организация, ВТНачисленияНалогиАвансом.Организация) КАК Организация,
		|	ЕСТЬNULL(ВТУдержанияПоИсполнительнымДокументамАвансом.ФизическоеЛицо, ВТНачисленияНалогиАвансом.ФизическоеЛицо) КАК ФизическоеЛицо,
		|	ЕСТЬNULL(ВТУдержанияПоИсполнительнымДокументамАвансом.ИсполнительныйДокумент, ВТНачисленияНалогиАвансом.ИсполнительныйЛист) КАК ИсполнительныйЛист,
		|	ЕСТЬNULL(ВТУдержанияПоИсполнительнымДокументамАвансом.Период, ВТНачисленияНалогиАвансом.Период) КАК Период,
		|	ВТНачисленияНалогиАвансом.СуммаНачислений КАК СуммаНачислений,
		|	ВТНачисленияНалогиАвансом.СуммаНачислений - ЕСТЬNULL(ВТБазаИсполнительныхЛистовАвансом.Сумма, 0) КАК НеудерживаемаяСумма,
		|	ВТНачисленияНалогиАвансом.НДФЛ КАК НДФЛ,
		|	ЕСТЬNULL(ВТУдержанияПоИсполнительнымДокументамАвансом.СуммаУдержания, 0) КАК СуммаУдержания
		|ПОМЕСТИТЬ ВТВыплатыАвансомПредварительно
		|ИЗ
		|	ВТНачисленияНалогиАвансом КАК ВТНачисленияНалогиАвансом
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБазаИсполнительныхЛистовАвансом КАК ВТБазаИсполнительныхЛистовАвансом
		|		ПО ВТНачисленияНалогиАвансом.ИсполнительныйЛист = ВТБазаИсполнительныхЛистовАвансом.ИсполнительныйЛист
		|			И ВТНачисленияНалогиАвансом.Период = ВТБазаИсполнительныхЛистовАвансом.Период
		|		ПОЛНОЕ СОЕДИНЕНИЕ ВТУдержанияПоИсполнительнымДокументамАвансом КАК ВТУдержанияПоИсполнительнымДокументамАвансом
		|		ПО ВТНачисленияНалогиАвансом.ИсполнительныйЛист = ВТУдержанияПоИсполнительнымДокументамАвансом.ИсполнительныйДокумент
		|			И ВТНачисленияНалогиАвансом.Период = ВТУдержанияПоИсполнительнымДокументамАвансом.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТВыплатыАвансомПредварительно.Организация КАК Организация,
		|	ВТВыплатыАвансомПредварительно.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТВыплатыАвансомПредварительно.ИсполнительныйЛист КАК ИсполнительныйЛист,
		|	ВТВыплатыАвансомПредварительно.Период КАК Период,
		|	ВТВыплатыАвансомПредварительно.СуммаНачислений КАК СуммаНачислений,
		|	ВТВыплатыАвансомПредварительно.НеудерживаемаяСумма КАК НеудерживаемаяСумма,
		|	ВТВыплатыАвансомПредварительно.НДФЛ КАК НДФЛ,
		|	ВТВыплатыАвансомПредварительно.СуммаУдержания КАК СуммаУдержания
		|ПОМЕСТИТЬ ВТНеСгруппированныеВыплаты
		|ИЗ
		|	ВТВыплатыАвансомПредварительно КАК ВТВыплатыАвансомПредварительно
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНачисленияЗаМесяц КАК ВТНачисленияЗаМесяц
		|		ПО ВТВыплатыАвансомПредварительно.Организация = ВТНачисленияЗаМесяц.Организация
		|			И ВТВыплатыАвансомПредварительно.ФизическоеЛицо = ВТНачисленияЗаМесяц.ФизическоеЛицо
		|			И ВТВыплатыАвансомПредварительно.Период = ВТНачисленияЗаМесяц.Период
		|ГДЕ
		|	НЕ ВТНачисленияЗаМесяц.ЕстьНачислениеЗаМесяц
		|	И НЕ ВТНачисленияЗаМесяц.НачисляетсяТолькоВАванс
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВТВыплатыПредварительно.Организация,
		|	ВТВыплатыПредварительно.ФизическоеЛицо,
		|	ВТВыплатыПредварительно.ИсполнительныйЛист,
		|	ВТВыплатыПредварительно.Период,
		|	ВТВыплатыПредварительно.СуммаНачислений,
		|	ВТВыплатыПредварительно.НеудерживаемаяСумма,
		|	ВТВыплатыПредварительно.НДФЛ,
		|	ВТВыплатыПредварительно.СуммаУдержания
		|ИЗ
		|	ВТВыплатыПредварительно КАК ВТВыплатыПредварительно
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТНеСгруппированныеВыплаты.Организация КАК Организация,
		|	ВТНеСгруппированныеВыплаты.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТНеСгруппированныеВыплаты.ИсполнительныйЛист КАК ИсполнительныйЛист,
		|	ВТНеСгруппированныеВыплаты.Период КАК Период,
		|	СУММА(ВТНеСгруппированныеВыплаты.СуммаНачислений) КАК СуммаНачислений,
		|	СУММА(ВТНеСгруппированныеВыплаты.НеудерживаемаяСумма) КАК НеудерживаемаяСумма,
		|	СУММА(ВТНеСгруппированныеВыплаты.НДФЛ) КАК НДФЛ,
		|	СУММА(ВТНеСгруппированныеВыплаты.СуммаУдержания) КАК СуммаУдержания
		|ПОМЕСТИТЬ ВТВыплатыПоИсполнительнымДокументамПредварительно
		|ИЗ
		|	ВТНеСгруппированныеВыплаты КАК ВТНеСгруппированныеВыплаты
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТНеСгруппированныеВыплаты.Организация,
		|	ВТНеСгруппированныеВыплаты.ФизическоеЛицо,
		|	ВТНеСгруппированныеВыплаты.ИсполнительныйЛист,
		|	ВТНеСгруппированныеВыплаты.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТНачисленияУдержанияАвансом
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТБазаИсполнительныхЛистовАвансомПредварительно
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТБазаИсполнительныхЛистовАвансом
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТНачисленияНалогиАвансом
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТУдержанияПоИсполнительнымДокументамАвансом
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТВыплатыАвансомПредварительно
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТНеСгруппированныеВыплаты");
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОтработанноеВремяПоСотрудникамАвансом.Период КАК Период,
		|	ОтработанноеВремяПоСотрудникамАвансом.Организация КАК Организация,
		|	ОтработанноеВремяПоСотрудникамАвансом.ФизическоеЛицо КАК ФизическоеЛицо,
		|	МАКСИМУМ(ОтработанноеВремяПоСотрудникамАвансом.ОтработаноДней) КАК ОтработаноДней
		|ПОМЕСТИТЬ ВТОтработанноеВремяАванс
		|ИЗ
		|	РегистрНакопления.ОтработанноеВремяПоСотрудникамАвансом КАК ОтработанноеВремяПоСотрудникамАвансом
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНачисленияЗаМесяц КАК ВТНачисленияЗаМесяц
		|		ПО ОтработанноеВремяПоСотрудникамАвансом.Организация = ВТНачисленияЗаМесяц.Организация
		|			И ОтработанноеВремяПоСотрудникамАвансом.ФизическоеЛицо = ВТНачисленияЗаМесяц.ФизическоеЛицо
		|			И ОтработанноеВремяПоСотрудникамАвансом.Период = ВТНачисленияЗаМесяц.Период
		|			И (ВТНачисленияЗаМесяц.ЕстьНачислениеЗаМесяц = ЛОЖЬ)
		|
		|СГРУППИРОВАТЬ ПО
		|	ОтработанноеВремяПоСотрудникамАвансом.Период,
		|	ОтработанноеВремяПоСотрудникамАвансом.Организация,
		|	ОтработанноеВремяПоСотрудникамАвансом.ФизическоеЛицо
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(ОтработанноеВремя.ОтработаноДней, ЕСТЬNULL(ВТОтработанноеВремяАванс.ОтработаноДней, 0)) КАК ОтработаноДней
		|ИЗ
		|	ВТСотрудники КАК Сотрудники
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТОтработанноеВремяАванс КАК ВТОтработанноеВремяАванс
		|			ПО ВыплатыПоИсполнительнымДокументам.ФизическоеЛицо = ВТОтработанноеВремяАванс.ФизическоеЛицо
		|				И ВыплатыПоИсполнительнымДокументам.Период = ВТОтработанноеВремяАванс.Период
