...
Процедура СоздатьФайлСправкиНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ВидСправки) Тогда
		...
		Иначе
			ИмяФайла = ИнтеграцияУправлениеПерсоналом.ЗаменитьЗапрещенныеСимволыВИмениФайла(ИмяФайла);
		КонецЕсли;
	Иначе
	...
	Попытка
		...
		Иначе
			...
			ЗаявкиСотрудников.СоздатьДокументЭДОБЗКСправкаСотруднику(Объект.Организация, Объект.ФизическоеЛицо, Объект.Ссылка, ПрисоединенныйФайл, Истина, ВидДокумента);
...
Процедура ДобавитьФайлСправки(ПомещенныйФайл)
	...
	Попытка
		...
		ВидДокумента = Справочники.ВидыДокументовЭДОБЗК.ВидПоИдентификаторуПечатнойФормы(Документы.ЗаявкаСправкаСМестаРаботыКабинетСотрудника.ИдентификаторПечатнойФормы());
		ЗаявкиСотрудников.СоздатьДокументЭДОБЗКСправкаСотруднику(Объект.Организация, Объект.ФизическоеЛицо, Объект.Ссылка, ПрисоединенныйФайл, Не ТребуетсяПодписьФайла, ВидДокумента);
