...
Процедура СоздатьВТИзмененияНачисленийПозиций(Запрос, ДанныеПозиций, ПредыдущиеДанныеПозиций, ПараметрыПолучения)
	...
	Для Каждого КлючИЗначение Из ДанныеПозиций Цикл
		...
		Для Каждого Начисление Из ДанныеПозиции.Начисления Цикл
			...
			Иначе
				Если ЗарплатаКадрыРасширенный.ЭтоНачислениеРедактируемоеВОтдельныхПолях(Начисление.Начисление) Тогда
					НоваяСтрокаНачислений.Действие = Перечисления.ДействияСНачислениямиИУдержаниями.Утвердить;
				КонецЕсли;
...
Процедура СоздатьВТНачисленияПозицииИСотрудников(Запрос)
	...
	|	НачисленияПоПозиции.Размер КАК Размер,
	...
	|	ЕСТЬNULL(НачисленияСотрудниковПозиции.Размер, ЕСТЬNULL(ПлановыеНачисления.Размер, 0)) КАК Размер,
	|	ВЫБОР
