...
Функция ПолучитьДанныеДляПроведения()
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА НачисленияСотрудников.Действие = ЗНАЧЕНИЕ(Перечисление.ДействияСНачислениямиИУдержаниями.Отменить) 
		|			И НачисленияПоказатели.ОтменяемыйПоказатель
		|			ТОГДА 0
		|		ИНАЧЕ МАКСИМУМ(ПоказателиСотрудников.Значение)
		|	КОНЕЦ КАК Значение
		|		
		|ИЗ
		|	ВТСотрудникиДокумента КАК ИзменениеПлановыхНачисленийСотрудники
		...
		|			ПО НачисленияПоказатели.Ссылка = НачисленияСотрудников.Начисление
		...
		|			ИЛИ НачисленияПоказатели.ОтменяемыйПоказатель
		...
		|	ПоказателиСотрудников.ДокументОснование,
		|	НачисленияПоказатели.ОтменяемыйПоказатель";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержания");
		КоэффициентыИндексацииДенежногоСодержания = Модуль.ТаблицаКоэффициентовИндексацииДенежногоСодержанияСотрудников(
			ДанныеДляПроведения.КоэффициентыИндексации);
		ДанныеДляПроведения.Вставить("КоэффициентыИндексацииДенежногоСодержания", КоэффициентыИндексацииДенежногоСодержания);
		
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоДовольствия");
		КоэффициентыИндексации = Модуль.СформироватьТаблицуКоэффициентовИндексации(
			Ссылка, ДанныеДляПроведения.ЗначенияПоказателей, ДанныеДляПроведения.КоэффициентыИндексации);
		ДанныеДляПроведения.Вставить("КоэффициентыИндексацииДенежногоДовольствия", КоэффициентыИндексации);
	КонецЕсли;
