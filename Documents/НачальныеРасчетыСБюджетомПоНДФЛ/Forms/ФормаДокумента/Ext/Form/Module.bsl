...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ДатаОстатков) Тогда
			Объект.ДатаОстатков = Мин(НачалоМесяца(Объект.Дата), НачалоМесяца(ДатаЗавершенияУчетаПеречисленногоНалога - 1));
		КонецЕсли;
...
Процедура ДатаОстатковПриИзменении(Элемент)
	...
	Если Объект.ДатаОстатков >= ДатаЗавершенияУчетаПеречисленногоНалога Тогда
		Объект.ДатаОстатков = '00010101';
		ТекстПредупреждения = НСтр("ru = 'С 2024 года расчеты с бюджетом по НДФЛ не ведутся.'");
...
Функция ФормаДокументаГотоваДляЗаполнения()
	...
	Если Не ЗначениеЗаполнено(Объект.ДатаОстатков) Тогда
		ФормаДокументаГотова = Ложь;		
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Необходимо указать дату остатков.'"), , "ДатаОстатков");
	КонецЕсли;
