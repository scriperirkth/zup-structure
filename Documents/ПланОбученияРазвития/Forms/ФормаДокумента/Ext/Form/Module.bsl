...
Процедура СотрудникиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НоваяСтрока Тогда
		...
		ТекущиеДанные = Элементы.Мероприятия.ТекущиеДанные;
		СотрудникиПередОкончаниемРедактированияЗадатьВопрос(ТекущиеДанные);
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьКоличествоСотрудниковЗавершение(Ответ, Параметры) Экспорт

	Если Ответ = КодВозвратаДиалога.Да Тогда
		УстановитьКоличествоСотрудниковВТабличнойЧастиМероприятия(ЭтотОбъект, Параметры.НомерСтрокиМероприятия, Истина);
	Иначе
		УстановитьКоличествоСотрудниковВТабличнойЧастиМероприятия(ЭтотОбъект, Параметры.НомерСтрокиМероприятия);
	КонецЕсли;
...
Процедура СотрудникиПослеУдаления(Элемент)
	...
	УстановитьКоличествоСотрудниковВТабличнойЧастиМероприятия(ЭтотОбъект, Элемент.ОтборСтрок.НомерСтрокиМероприятия, Истина);
...
&НаКлиенте
Процедура СотрудникиПередОкончаниемРедактированияЗадатьВопрос(ТекущиеДанные)
	
	Отбор = Новый Структура();
	Отбор.Вставить("НомерСтрокиМероприятия",ТекущиеДанные.НомерСтрокиМероприятия);
	
	КоличествоСотрудников = Объект.Сотрудники.НайтиСтроки(Отбор).Количество();
	СуммаКратнаКоличеству = ПредопределенноеЗначение("Перечисление.СпособыРасчетаСуммыРасходовНаПерсонал.СуммаКратнаКоличествуСотрудников");
	
	Если ЗначениеЗаполнено(ТекущиеДанные.СуммаРасходов)
		И КоличествоСотрудников > ТекущиеДанные.КоличествоСотрудников
		И ТекущиеДанные.СпособОпределенияСтоимости = СуммаКратнаКоличеству Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("НомерСтрокиМероприятия", ТекущиеДанные.НомерСтрокиМероприятия);
		ТекстВопроса = НСтр("ru = 'Сумма расходов изменится. Пересчитать после добавления сотрудников?'");
		Оповещение = Новый ОписаниеОповещения("УстановитьКоличествоСотрудниковЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
	Иначе
		УстановитьКоличествоСотрудниковВТабличнойЧастиМероприятия(ЭтотОбъект, ТекущиеДанные.НомерСтрокиМероприятия, Истина);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьКоличествоСотрудниковВТабличнойЧастиМероприятия(Форма, НомерСтрокиМероприятия, ВыполнитьПересчетСуммы = Ложь)
	...
	Если СтрокаМероприятия.КоличествоВычисляется Тогда
		...
		Если Не ЗначениеЗаполнено(СтрокаМероприятия.СуммаРасходов) Или ВыполнитьПересчетСуммы Тогда
