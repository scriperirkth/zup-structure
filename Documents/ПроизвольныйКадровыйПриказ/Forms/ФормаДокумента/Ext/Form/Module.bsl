...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда 
		...
		Если Параметры.Свойство("Основание") И ТипЗнч(Параметры.Основание) = Тип("СправочникСсылка.Сотрудники") Тогда
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("НесколькоСотрудников", Объект.Шаблон.Метаданные()) Тогда
	...
	Для	Каждого КлючИЗначение Из ПроверяемыеПоля Цикл
		...
		ИначеЕсли ЧастиПутиКРеквизиту[0] = "Сотрудники" Тогда
			...
			Для Каждого СтрокаКоллекции Из Объект.Сотрудники Цикл
				...
				Если Не ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не заполнена колонка ""%1"" в строке %2 списка ""Сотрудники""'"),
						Элементы[ИмяПоляФормы].Заголовок, СтрокаКоллекции.НомерСтроки);
...
Процедура Подключаемый_ВыполнитьЗадачуПоОбработкеДокумента(Команда)
	...
	// ПроцессыОбработкиДокументов
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ПроцессыОбработкиДокументовЗарплата") Тогда
		МодульПроцессыОбработкиДокументовЗарплатаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
...
Процедура Подключаемый_КомментарийНаправившегоОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ПроцессыОбработкиДокументовЗарплата") Тогда
		МодульПроцессыОбработкиДокументовЗарплатаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"ПроцессыОбработкиДокументовЗарплатаКлиент");
		МодульПроцессыОбработкиДокументовЗарплатаКлиент.КомментарийНаправившегоОткрытие(ЭтотОбъект, Элемент,
...
Процедура Подключаемый_КомментарийСледующемуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ПроцессыОбработкиДокументовЗарплата") Тогда
		МодульПроцессыОбработкиДокументовЗарплатаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
			"ПроцессыОбработкиДокументовЗарплатаКлиент");
		МодульПроцессыОбработкиДокументовЗарплатаКлиент.КомментарийСледующемуНачалоВыбора(ЭтотОбъект, Элемент,
...
Процедура УстановитьВидимостьПодписей()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ОрганизацияПодписиБухгалтер",
			"Видимость",
			НастройкиДокумента.ПодписываетБухгалтер);
			
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ОрганизацияПодписиКассир",
			"Видимость",
			НастройкиДокумента.ПодписываетКассир);
			
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ОрганизацияПодписиРуководительКадровойСлужбы",
			"Видимость",
...
Процедура ОтобразитьДополнительныеРеквизитыШапки(СоответствиеПроверяемыхРеквизитов)
	...
	Для каждого ОписаниеРеквизита Из ТаблицаРеквизитовШапки Цикл
		...
		Реквизит = Новый РеквизитФормы(ИмяРеквизита, ОписаниеРеквизита.ТипЗначения, , ОписаниеРеквизита.Наименование,
			Истина);
...
Процедура ОтобразитьДополнительныеКолонкиТабличнойЧасти(СоответствиеПроверяемыхРеквизитов)
	...
	Для Каждого ОписаниеРеквизита Из ТаблицаРеквизитовСпискаСотрудников Цикл
		ИмяРеквизита = ИмяРеквизита(ОписаниеРеквизита.Ссылка);
		Реквизит = Новый РеквизитФормы(ИмяРеквизита, ОписаниеРеквизита.ТипЗначения, "Объект.Сотрудники",
			ОписаниеРеквизита.Наименование, Истина);
	...
	Для Каждого ОписаниеРеквизита Из ТаблицаРеквизитовСпискаСотрудников Цикл
		ИмяРеквизита = ИмяРеквизита(ОписаниеРеквизита.Ссылка);
		Элемент = Элементы.Найти("Сотрудники"+ИмяРеквизита);
...
Процедура СохранитьЗначенияРеквизитовШапки(ТекущийОбъект)
	...
	ТаблицаРеквизитовШапки = ТаблицаРеквизитов(Объект.Шаблон);	
	Для Каждого ОписаниеРеквизита Из ТаблицаРеквизитовШапки Цикл
		ЗначениеРеквизита = ЭтотОбъект[ИмяРеквизита(ОписаниеРеквизита.Ссылка)];
...
Процедура ПрочитатьДополнительныеРеквизитыШапки()
	...
	Для каждого ОписаниеРеквизита Из ТаблицаРеквизитовШапки Цикл
		...
		Если СтрокиСоЗначениемРеквизита.Количество() > 0 Тогда
			ЭтотОбъект[ИмяРеквизита(ОписаниеРеквизита.Ссылка)] = СтрокиСоЗначениемРеквизита[0].Значение;
		КонецЕсли;
	КонецЦикла;	
...
Процедура ЗаполнитьСписокСотрудников()
	...
	ПараметрыСотрудники.НачалоПериода = Объект.Дата;
	ПараметрыСотрудники.ОкончаниеПериода = Объект.Дата;
