...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПовторноОбработатьСообщения107");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("40a97611-2e8d-11f1-8174-4cedfb43b11a");
	Обработчик.Процедура = "Документы.УведомлениеОбИзмененииРасчетаПособия.ЗаполнитьОрганизацию";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение организации в уведомлениях об изменении расчета пособия.'");
...
Процедура ЗаполнитьОрганизацию(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(Ответ.Организация, ЕСТЬNULL(Запрос.Организация, Шапка.Страхователь)) КАК Организация,
	|	ЕСТЬNULL(ОтчитывающиесяПоВзносамОрганизации.ОтчитывающаясяОрганизация, ЕСТЬNULL(Ответ.Организация, ЕСТЬNULL(Запрос.Организация, Шапка.Страхователь))) КАК СтраховательОрганизации
	|ИЗ
	|	Документ.УведомлениеОбИзмененииРасчетаПособия КАК Шапка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтветНаЗапросФССДляРасчетаПособия КАК Ответ
	|		ПО Шапка.ОтветНаЗапрос = Ответ.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВходящийЗапросФССДляРасчетаПособия КАК Запрос
	|		ПО Шапка.ВходящийЗапрос = Запрос.Ссылка
	...
	|		ПО (ЕСТЬNULL(Ответ.Организация, ЕСТЬNULL(Запрос.Организация, Шапка.Страхователь)) = ОтчитывающиесяПоВзносамОрганизации.Организация)
	...
	КонецЕсли;
	ИзмененныеЗапросы = Новый Массив;
	
	Пока Выборка.Следующий() Цикл
		Если ОбработчикОтложенный Тогда
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
					ПараметрыОбновления,
					"Документ.УведомлениеОбИзмененииРасчетаПособия",
					"Ссылка",
		...
		// Организация заполняется таким образом, чтобы она соответствовала страхователю в документе.
		Если Выборка.СтраховательОрганизации = ДокументОбъект.Страхователь Тогда
			ДокументОбъект.Организация = Выборка.Организация;
		Иначе
			ДокументОбъект.Организация = ДокументОбъект.Страхователь;
		КонецЕсли;
		
		// Если пользователь изменил страхователя, то фиксация перебрасывается на организация.
		ФиксацияОрганизация = ДокументОбъект.ФиксацияИзменений.Найти("Организация", "ИмяРеквизита");
		ФиксацияСтрахователь = ДокументОбъект.ФиксацияИзменений.Найти("Страхователь", "ИмяРеквизита");
		Если ФиксацияОрганизация = Неопределено И ФиксацияСтрахователь <> Неопределено Тогда
			ФиксацияОрганизация = ФиксацияСтрахователь;
			ФиксацияОрганизация.ИмяРеквизита = "Организация";
		КонецЕсли;
		
		СЭДОФСС.ЗаписатьДокумент(ДокументОбъект, Истина, Ложь, РежимЗаписиДокумента.Запись);
		ИзмененныеЗапросы.Добавить(ДокументОбъект.Ссылка);
...
Функция ДанныеСвязанныхДокументов(Страхователь, НомерПроцесса, ДатаСообщения) Экспорт
	...
	Реквизиты = Новый Структура("ОтветНаЗапрос,ВходящийЗапрос,Организация,ФизическоеЛицо,СНИЛС,Сотрудник");
...
Функция ФиксируемыеРеквизиты()
	...
	ФиксацияВторичныхДанныхВДокументах.ДобавитьФиксируемыйРеквизит(ФиксируемыеРеквизиты, Шаблон, "Организация", Истина);
