...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Оповестить("Закрытие_ОтпускаСотрудников");
...
Процедура ЗаполнитьПоГрафикуОтпусков(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьПоГрафикуОтпусковЗавершение",ЭтотОбъект);      							  
	ОткрытьФорму("Документ.ОтпускаСотрудников.Форма.ЗаполнитьПоГрафикуОтпусков", ПараметрыФормы, ЭтаФорма,,,,Оповещение);    
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоГрафикуОтпусковЗавершение(РезультатЗакрытия, ДополнительныеПараметры)  Экспорт        					 
	Если ЗначениеЗаполнено(РезультатЗакрытия) тогда  
		ЗаполнитьПоГрафикуОтпусковНаСервере(РезультатЗакрытия);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПоГрафикуОтпусковНаСервере(Результат)
	...
	УчестьФактическиеОтпускаВГрафике(ГрафикОтпусков);  
	...
	ДатаОкончания = Неопределено; 
	ОтпускБезОплаты = Истина; 
	ВидыОтпускаПоСотруднику = Новый Массив;           				    
	ФактическийОтпуск = Неопределено;                 				   
	ОтпускПроведен = Ложь;                                          
	ОтпускРассчитан = Ложь;                                         
	ФорматДаты = "ДФ='dd.MM.yyyy'";                                 
	
	ОписанияВидовОтпусков = Новый Соответствие; 
	Для Каждого ДанныеГрафика Из ГрафикОтпусков Цикл                                                                                      
		Если ДанныеГрафика.ИсходныйДокумент <> Неопределено тогда                                                                        
			ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 - %2'"),Формат(ДанныеГрафика.ДатаНачала, ФорматДаты), Формат(ДанныеГрафика.ДатаОкончания, ФорматДаты));
			ТекстСообщения = НСтр("ru = 'По сотруднику %1 исключен период %2, учтенный в документе %3'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ДанныеГрафика.Сотрудник, ОписаниеПериода, ДанныеГрафика.ИсходныйДокумент);
		...
		Если ПредыдущаяСтрока = Неопределено Или ПредыдущаяСтрока.Сотрудник <> ДанныеГрафика.Сотрудник 
			Или ПредыдущаяСтрока.ДатаОкончания + 86400 <> ДанныеГрафика.ДатаНачала 
			Или ВидыОтпускаПоСотруднику.Найти(ДанныеГрафика.ВидОтпуска) <> Неопределено тогда       										
			Если ПредыдущаяСтрока <> Неопределено Тогда
				СтрокаСотрудника.ДатаНачала = ДатаНачала;
				СтрокаСотрудника.ДатаОкончания = ДатаОкончания;
				СтрокаСотрудника.ОтпускБезОплаты = ОтпускБезОплаты; 
				СтрокаСотрудника.Отпуск = ФактическийОтпуск;                             													 
				СтрокаСотрудника.Проведен = ОтпускПроведен;                                                                                 
				СтрокаСотрудника.Рассчитан = ОтпускРассчитан;                                                                                
				УстановитьПредставленияСтрокиСотрудника(СтрокаСотрудника);
				ДатаНачала = Неопределено;
				ДатаОкончания = Неопределено; 
				ВидыОтпускаПоСотруднику.Очистить();                                                 										    
				ОтпускБезОплаты = Истина;
			КонецЕсли;
			СтрокаСотрудника = Объект.Сотрудники.Добавить();
			СтрокаСотрудника.Сотрудник = ДанныеГрафика.Сотрудник;  
			ЗаполнитьИдентификаторСтрокиСотрудника(ЭтаФорма, СтрокаСотрудника); 
			ФактическийОтпуск = ДанныеГрафика.ФактическийОтпуск;
		КонецЕсли;
		...
		Если ФактическийОтпуск <> ДанныеГрафика.ФактическийОтпуск тогда
			ФактическийОтпуск = Неопределено;   																							
		КонецЕсли;
		...
		ВидыОтпускаПоСотруднику.Добавить(ДанныеГрафика.ВидОтпуска);                                 										  
		ОтпускПроведен = ДанныеГрафика.Проведен;                                                                                             
		ОтпускРассчитан = ДанныеГрафика.Рассчитан;                                                                                           
		Если Не ОписаниеВидаОтпуска.ОтпускБезОплаты Тогда 
			ОтпускБезОплаты = Ложь;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура УчестьФактическиеОтпускаВГрафике(ГрафикОтпусков)      															  
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ГрафикОтпусков", ГрафикОтпусков);
	...
	Запрос.Текст =
		...
		|	ГрафикОтпусков.ДатаОкончания КАК ДатаОкончания,
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА УчтенныеОтпуска.Регистратор ССЫЛКА Документ.Отпуск
		|				ТОГДА ВЫРАЗИТЬ(УчтенныеОтпуска.Регистратор КАК Документ.Отпуск)
		|			КОГДА УчтенныеОтпуска.Регистратор ССЫЛКА Документ.ОтпускБезСохраненияОплаты
		|				ТОГДА ВЫРАЗИТЬ(УчтенныеОтпуска.Регистратор КАК Документ.ОтпускБезСохраненияОплаты)
		|			ИНАЧЕ НЕОПРЕДЕЛЕНО
		|		КОНЕЦ, НЕОПРЕДЕЛЕНО) КАК ФактическийОтпуск,
		|	ЕСТЬNULL(УчтенныеОтпуска.Регистратор.Проведен, ЛОЖЬ) КАК Проведен,
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА УчтенныеОтпуска.Регистратор ССЫЛКА Документ.Отпуск
		|				ТОГДА ВЫРАЗИТЬ(УчтенныеОтпуска.Регистратор КАК Документ.Отпуск).ДокументРассчитан
		|			КОГДА УчтенныеОтпуска.Регистратор ССЫЛКА Документ.ОтпускБезСохраненияОплаты
		|				ТОГДА ВЫРАЗИТЬ(УчтенныеОтпуска.Регистратор КАК Документ.ОтпускБезСохраненияОплаты).ПерерасчетВыполнен
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ, ЛОЖЬ) КАК Рассчитан,
		|	ЕСТЬNULL(РанееУчтенныеОтпуска.Ссылка, НЕОПРЕДЕЛЕНО) КАК ИсходныйДокумент
		|ИЗ
		|	ВТГрафикОтпусков КАК ГрафикОтпусков
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПериодыОтпусков КАК УчтенныеОтпуска
		|		ПО ГрафикОтпусков.Сотрудник = УчтенныеОтпуска.Сотрудник
		|			И ГрафикОтпусков.ДатаНачала = УчтенныеОтпуска.ДатаНачала
		|			И ГрафикОтпусков.ДатаОкончания = УчтенныеОтпуска.ДатаОкончания
		|			И ГрафикОтпусков.ВидОтпуска = УчтенныеОтпуска.ВидОтпуска
		|			И (НЕ УчтенныеОтпуска.Компенсация)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтпускаСотрудников.Сотрудники КАК РанееУчтенныеОтпуска
		|		ПО (РанееУчтенныеОтпуска.Сотрудник = УчтенныеОтпуска.Сотрудник)
		|			И (РанееУчтенныеОтпуска.Отпуск = УчтенныеОтпуска.Регистратор)
		|			И (РанееУчтенныеОтпуска.Ссылка <> &ТекущийДокумент)
		|			И (НЕ РанееУчтенныеОтпуска.Ссылка.ПометкаУдаления)";
	
	ГрафикОтпусков = Запрос.Выполнить().Выгрузить();   
	ГрафикОтпусков.Сортировать("Сотрудник, ДатаНачала");
