...
Процедура ВыборОтработанныхДней(Команда)
	...
	ПараметрыФормы.Вставить("НачалоОтгула", Объект.ДатаНачала);
...
Процедура СотрудникПриИзмененииНаСервере()
	...
	ЗаполнитьДниЧасыНакопленныхОтгулов();	
...
&НаСервере
Процедура ЗаполнитьДниЧасыНакопленныхОтгулов()
	...
	Если Не ЗначениеЗаполнено(Объект.ДатаНачала)
		...
		Или Объект.КоличествоДнейОтгула = 0
		Или Объект.ОтсутствиеВТечениеЧастиСмены 
		Или Объект.ДатаНачала < УчетОтгулов.НачалоУчетаОтработанныхДней() Тогда 
		Возврат;
	КонецЕсли;
	
	Объект.РасходДнейОтгула = Мин(Объект.РасходДнейОтгула, Объект.КоличествоДнейОтгула);
	Если Объект.РасходЧасовОтгула = 0 Или Объект.РасходДнейОтгула = Объект.КоличествоДнейОтгула Тогда 
		Объект.РасходЧасовОтгула = 0;
		Возврат;
	КонецЕсли;
	
	ДанныеСотрудников = УчетОтгулов.СведенияОбОтработанныхЧасах(Объект.Сотрудник, Объект.ДатаНачала, Объект.Ссылка, Объект.ИсправленныйДокумент);
	...
	Для Каждого ОтработанныйДень Из ДанныеСотрудника.НеиспользованныеОтработанныеДни Цикл 
		Если Объект.РасходДнейОтгула + ОтработанныеДниВсего = Объект.КоличествоДнейОтгула Тогда 
			Объект.РасходЧасовОтгула = ОтработанныеЧасыВсего;
			Прервать;
		КонецЕсли;
		...
		Если ОтработанныеЧасы = Неопределено Тогда 
			Продолжить;
		КонецЕсли;
		Если ОтработанныйДень >= Объект.ДатаНачала
			Или ОтработанныеЧасыВсего + ОтработанныеЧасы > Объект.РасходЧасовОтгула Тогда 
			Прервать;
		КонецЕсли;
		Если ДобавитьМесяц(ОтработанныйДень, 12) < Объект.ДатаНачала Тогда
    		Объект.РасходЧасовОтгула = Макс(Объект.РасходЧасовОтгула - ОтработанныеЧасы, 0); 
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьОтработанныеДни()
	...
	ДанныеСотрудников = УчетОтгулов.СведенияОбОтработанныхДнях(Объект.Сотрудник, Объект.ДатаНачала, Объект.Ссылка, Объект.ИсправленныйДокумент);
...
Процедура ЗаполнитьОтработанныеЧасы()
	...
	Если Объект.РасходЧасовОтгула = 0
		Или Объект.ДатаНачала < УчетОтгулов.НачалоУчетаОтработанныхДней() Тогда 
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ВыборОтработанныхДней", "Видимость", Не Объект.ОтсутствиеВТечениеЧастиСмены И Объект.ДатаНачала >= УчетОтгулов.НачалоУчетаОтработанныхДней());
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ВыборОтработанныхЧасов", "Видимость", Объект.ДатаНачала >= УчетОтгулов.НачалоУчетаОтработанныхДней());
