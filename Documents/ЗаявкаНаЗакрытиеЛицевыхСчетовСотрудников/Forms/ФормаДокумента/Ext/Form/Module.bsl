...
Процедура СотрудникиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		ЗаполнитьСтрокиНаСервере(ВыбранноеЗначение);
	КонецЕсли;
...
Процедура СотрудникПриИзменении(Элемент)
	...
	ЗаполнитьСтрокиНаСервере(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.Сотрудник), ТекущиеДанные.НомерСтроки);
КонецПроцедуры
...
Процедура Подбор(Команда)
	...
	ПараметрыОткрытия = Новый Структура("ДоступныВсеСотрудники", Истина);
	
	КадровыйУчетКлиент.ВыбратьСотрудниковРаботающихНаДатуПоПараметрамОткрытияФормыСписка(
		Элементы.Сотрудники, Объект.Организация, , , Истина, АдресСпискаПодобранных(), ПараметрыОткрытия);
...
Процедура ЗаполнитьНаСервере(УчитыватьФиксацию = Ложь)
	...
	ПараметрыФункции = Новый Структура("Ссылка, Организация, ЗарплатныйПроект, Подразделение, Дата");
	ЗаполнитьЗначенияСвойств(ПараметрыФункции, Объект);
	
	ДанныеЗаполненияСотрудников = Документы.ЗаявкаНаЗакрытиеЛицевыхСчетовСотрудников.ДанныеЗаполненияСотрудников(ПараметрыФункции);
	Выборка = ДанныеЗаполненияСотрудников.Выбрать();
...
&НаСервере
Процедура ЗаполнитьСтрокиНаСервере(МассивСотрудников, НомерСтроки = 0)
	
	ЛицевыеСчетаСотрудников = ОбменСБанкамиПоЗарплатнымПроектам.ЛицевыеСчетаСотрудников(МассивСотрудников, Истина, Объект.Организация, Объект.Дата, Объект.ЗарплатныйПроект);
	ЛицевыеСчетаСотрудников.Колонки["НомерЛицевогоСчета"].Имя = "ЛицевойСчет";
	...
	Если НомерСтроки > 0 Тогда
		// Была изменена строка ТЧ
		СтрокаСотрудника = Объект.Сотрудники[НомерСтроки - 1];
		ЛицевойСчетСотрудника = ЛицевыеСчетаСотрудников[0];
		СтрокаСотрудника.ДатаЗакрытия = Объект.Дата;
		ЗаполнитьЗначенияСвойств(СтрокаСотрудника, ЛицевойСчетСотрудника);
	Иначе
		// Осуществлен подбор сотрудников
		Для Каждого Сотрудник Из МассивСотрудников Цикл
			СтруктураПоиска = Новый Структура("Сотрудник", Сотрудник);
			ЛицевыеСчетаСотрудника = ЛицевыеСчетаСотрудников.НайтиСтроки(СтруктураПоиска);
			Для Каждого ЛицевойСчетСотрудника Из ЛицевыеСчетаСотрудника Цикл
				НоваяСтрока = Объект.Сотрудники.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ЛицевойСчетСотрудника);
				НоваяСтрока.ДатаЗакрытия = Объект.Дата;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли
