...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПовторноОбработатьСообщения11010");
	Методы.Вставить("ПовторноОбработатьСообщения110");
	Методы.Вставить("ПовторноОбработатьСообщения11000");
	Методы.Вставить("ЗаполнитьОрганизацию");
КонецПроцедуры
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7434b305-2df1-11f1-8174-4cedfb43b11a");
	Обработчик.Процедура = "Документы.УведомлениеОСтатусеВыплатыПособия.ЗаполнитьОрганизацию";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение организации в уведомлениях о статусе выплаты пособий.'");
...
// Заполнение организации в запросах статусов выплаты пособий.
//
// Параметры:
//   ПараметрыОбновления - Структура - Параметры отложенного обновления.
//
Процедура ЗаполнитьОрганизацию(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	Шапка.ДатаСообщения КАК ДатаСообщения
	|ПОМЕСТИТЬ ВтДокументы
	|ИЗ
	|	Документ.УведомлениеОСтатусеВыплатыПособия КАК Шапка
	|ГДЕ
	|	Шапка.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	...
	|	ВтДокументы.ДатаСообщения КАК ДатаСообщения
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Если ОбработчикОтложенный Тогда
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
					ПараметрыОбновления,
					"Документ.УведомлениеОСтатусеВыплатыПособия",
					"Ссылка",
		...
		Если ДокументОбъект.ТипСообщения = 0 Тогда
			ДокументОбъект.ТипСообщения = 110;
		КонецЕсли;
		ЗаполнитьОрганизациюВДокументе(ДокументОбъект);
...
Процедура ЗагрузитьСообщение(ТипСообщения, Страхователь, ИдентификаторСообщения, ТекстXML, ТребуетсяПодтверждение, Результат, Кэш) Экспорт
	...
	Документ.Вставить("ГоловнаяОрганизация", ЗарплатаКадры.ГоловнаяОрганизация(Страхователь));
	...
	Попытка
		...
		Для Каждого КлючИЗначение Из Документ Цикл
			Если ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") Тогда
				Если ФиксацияВторичныхДанныхВДокументах.ЗагрузитьТаблицу(
						ДокументОбъект[КлючИЗначение.Ключ], КлючИЗначение.Значение) Тогда
					ЕстьИзменения = Истина;
				КонецЕсли;
				ФиксацияВторичныхДанныхВДокументах.ОтменитьФиксациюТабличнойЧасти(ДокументОбъект, КлючИЗначение.Ключ);
			Иначе
				Если ФиксацияВторичныхДанныхВДокументах.ИзменитьЗначениеРеквизита(
						ДокументОбъект[КлючИЗначение.Ключ], КлючИЗначение.Значение) Тогда
					ЕстьИзменения = Истина;
				КонецЕсли;
				ФиксацияВторичныхДанныхВДокументах.ОтменитьФиксациюРеквизитаШапки(ДокументОбъект, КлючИЗначение.Ключ);
			КонецЕсли;
		КонецЦикла;
		...
		Если ДокументОбъект.ЗаполнитьФизическоеЛицо() Тогда
			ЕстьИзменения = Истина;
		КонецЕсли;
		
		СвязанныеДокументы = СвязанныеДокументы(ДокументОбъект);
		Если ТипСообщения = 11010
			И ЗаполнитьДанныеФизлицаПоВозмещениямРодителямДетейИнвалидов(ДокументОбъект, СвязанныеДокументы) Тогда
			ЕстьИзменения = Истина;
		КонецЕсли;
		
		Если ЗаполнитьОрганизациюВДокументе(ДокументОбъект, СвязанныеДокументы) Тогда
			ЕстьИзменения = Истина;
		КонецЕсли;
...
Функция ЗаполнитьДанныеФизлицаПоВозмещениямРодителямДетейИнвалидов(ДокументОбъект, СвязанныеДокументы)
	...
	Если Таблица.Количество() = 1 Тогда
		...
		Реквизиты.Вставить("СотрудникСНИЛС", СтрокаТаблицы.СотрудникСНИЛС);
		Реквизиты.Вставить("СотрудникФамилия", СтрокаТаблицы.СотрудникФамилия);
		Реквизиты.Вставить("СотрудникИмя", СтрокаТаблицы.СотрудникИмя);
		Реквизиты.Вставить("СотрудникОтчество", СтрокаТаблицы.СотрудникОтчество);
	Иначе
		СНИЛС = КоллекцииБЗК.УникальныеЗначенияКолонки(Таблица, "СотрудникСНИЛС");
		Если СНИЛС.Количество() = 1 Тогда
			Реквизиты.Вставить("СотрудникСНИЛС", СНИЛС[0]);
		КонецЕсли;
	КонецЕсли;
	Возврат ФиксацияВторичныхДанныхВДокументах.ОбновитьДанныеШапкиБезУчетаФиксации(ДокументОбъект, Реквизиты);
КонецФункции

Функция ЗаполнитьОрганизациюВДокументе(ДокументОбъект, СвязанныеДокументы = Неопределено)
	Организация = Неопределено;
	Если СвязанныеДокументы = Неопределено Тогда
		СвязанныеДокументы = СвязанныеДокументы(ДокументОбъект);
	КонецЕсли;
	Если ОрганизацияПодходит(ДокументОбъект, СвязанныеДокументы.Организация) Тогда
		Организация = СвязанныеДокументы.Организация;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(ДокументОбъект.ФизическоеЛицо) Тогда
		КадровыеДанные = КадровыйУчет.КадровыеДанныеОсновногоСотрудникаФизическогоЛица(
			ДокументОбъект.Страхователь,
			ДокументОбъект.ФизическоеЛицо,
			"Организация",
			ДокументОбъект.ДатаСообщения,
			Ложь);
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Организация = ДокументОбъект.Страхователь;
	КонецЕсли;
	ФиксацияВторичныхДанныхВДокументах.ОтменитьФиксациюРеквизитаШапки(ДокументОбъект, "Организация");
	Возврат ФиксацияВторичныхДанныхВДокументах.ИзменитьЗначениеРеквизита(ДокументОбъект.Организация, Организация);
КонецФункции

Функция ОрганизацияПодходит(Документ, Организация)
	Возврат ЗначениеЗаполнено(Организация)
		И (Организация = Документ.Страхователь
			Или СЭДОФСС.СтраховательОрганизации(Организация, Документ.ДатаСообщения) = Документ.Страхователь);
КонецФункции
...
Функция СвязанныеДокументы(Документ) Экспорт
	...
	|ИсходящийДокументПоИдентификаторуРеестра, ВходящийДокументПоНомеруПроцесса, Организация");
	...
	Если ЗначениеЗаполнено(Документ.НомерПроцесса) Тогда
		...
		|	Уведомление.Организация КАК Организация,
	...
	Если ЗначениеЗаполнено(Документ.ИдентификаторРеестра) Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтветНаЗапросФССДляРасчетаПособия КАК Ответ
		|		ПО Регистрации.ИсходящийДокумент = Ответ.Ссылка
		|ГДЕ
	...
	Таблица1.Свернуть("Ссылка, Организация, СтатусВыплаты, Дата, ДатаСообщения");
	...
	Для Каждого СтрокаТаблицы Из Таблица1 Цикл
		Если СтрокаТаблицы.ДатаСообщения < Документ.ДатаСообщения Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Организация) Тогда
				Результат.Организация = СтрокаТаблицы.Организация;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(Документ.ИдентификаторРеестра) Тогда
		...
		Если Таблица2.Количество() > 0 Тогда
			СтрокаТаблицы = Таблица2[0];
			Результат.ИсходящийДокументПоИдентификаторуРеестра = СтрокаТаблицы.ИсходящийДокумент;
			Если ЗначениеЗаполнено(СтрокаТаблицы.Организация) Тогда
				Результат.Организация = СтрокаТаблицы.Организация;
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Документ.НомерПроцесса) Тогда
		...
		Если Таблица3.Количество() > 0 Тогда
			СтрокаТаблицы = Таблица3[0];
			Результат.ВходящийДокументПоНомеруПроцесса = СтрокаТаблицы.Ссылка;
			Если ЗначениеЗаполнено(СтрокаТаблицы.Организация) Тогда
				Результат.Организация = СтрокаТаблицы.Организация;
			КонецЕсли;
		КонецЕсли;
