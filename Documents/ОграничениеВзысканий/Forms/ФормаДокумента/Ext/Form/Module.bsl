...
Процедура ЗаполнитьДокумент(ФизическиеЛица = Неопределено)
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам
		|ИЗ
		...
		|	НачисленияУдержанияПоКонтрагентамАкционерам.Период КАК Период
		|ПОМЕСТИТЬ ВТРегистрНакопления_НачисленияУдержанияПоКонтрагентамАкционерам
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам.НачислениеУдержание КАК НачислениеУдержание,
		|	ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам.Сумма КАК Сумма
		|ПОМЕСТИТЬ ВТРегистрНакопления_ВТНачисленияУдержания
		|ИЗ
		|	ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам КАК ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВТ_НачисленияУдержанияПоКонтрагентамАкционерам.ФизическоеЛицо,
		|	ВТ_НачисленияУдержанияПоКонтрагентамАкционерам.НачислениеУдержание,
		|	ВТ_НачисленияУдержанияПоКонтрагентамАкционерам.Сумма
		|ИЗ
		|	ВТРегистрНакопления_НачисленияУдержанияПоКонтрагентамАкционерам КАК ВТ_НачисленияУдержанияПоКонтрагентамАкционерам
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 0
		...
		|	СведенияОДоходахНДФЛ.Сторно КАК Сторно,
		...
		|			И (СведенияОДоходахНДФЛ.Период МЕЖДУ &МесяцНачисления И КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ))
		|			И (СведенияОДоходахНДФЛ.Организация = &Организация)
		|			И (СведенияОДоходахНДФЛ.Регистратор <> &Ссылка)
		|			И (НЕ СведенияОДоходахНДФЛ.Сотрудник В (&ИсключаемыеСотрудники))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	Удержания.ОграничениеВзыскания КАК ОграничениеВзыскания
		...
		|			И (Удержания.ПериодРегистрации = &МесяцНачисления)
		|			И (Удержания.Регистратор <> &Ссылка)
		|			И (Удержания.ОграничениеВзыскания = ЛОЖЬ)
		|			И (НЕ Удержания.Сотрудник В (&ИсключаемыеСотрудники))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ВТРегистрРасчета_Удержания.ФизическоеЛицо КАК ФизическоеЛицо
		|ИЗ
		|	ВТРегистрРасчета_Удержания КАК ВТРегистрРасчета_Удержания";
	...
	Если Результат.Пустой() Тогда
		ЧастиЗапроса.Добавить(
			...
			|	ВТНачисленияУдержания.Сумма КАК Сумма
			|ПОМЕСТИТЬ ВТНачисленияУдержанияПоСотрудникам
			|ИЗ
			|	ВТРегистрНакопления_ВТНачисленияУдержания КАК ВТНачисленияУдержания
			|ГДЕ
			|	ВТНачисленияУдержания.НачислениеУдержание В(&ВидыНачисленийУдержаний)");
	Иначе
		ОписаниеВТ = Новый Структура("ИмяВТ, ИмяПоляНачалоПериода, ИмяПоляОкончаниеПериода",
			"ВТРегистрРасчета_Удержания", "БазовыйПериодНачало", "БазовыйПериодКонец");
		НалогПоДоходам = ИсполнительныеЛисты.ИсчисленныйПоДоходамНалог(Запрос.МенеджерВременныхТаблиц, ОписаниеВТ, 
			Объект.МесяцНачисления, ЗаймыСотрудникам.ТаблицаПогашениеЗаймов(), Объект.Ссылка,
			Новый Структура("РассчитыватьТолькоПоТекущемуДокументу", Истина));
		Запрос.УстановитьПараметр("ВидыНДФЛ", Обработки.МенеджерРасчетаЗарплаты.ВидыНДФЛДополненияРасчетнойБазыУдержаний());
		Запрос.УстановитьПараметр("ВТНалогПоДоходам", НалогПоДоходам);
		ЧастиЗапроса.Добавить(
			"ВЫБРАТЬ
			|	НалогПоДоходам.ФизическоеЛицо КАК ФизическоеЛицо,
			|	НалогПоДоходам.Налог КАК Налог,
			|	НалогПоДоходам.Начисление КАК Начисление
			|ПОМЕСТИТЬ ВТНалогПОДоходам
			|ИЗ
			|	&ВТНалогПоДоходам КАК НалогПоДоходам
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|ГДЕ
			|	ВТНачисленияУдержания.НачислениеУдержание В(&ВидыНачисленийУдержаний)
			|	И НЕ ВТНачисленияУдержания.НачислениеУдержание В (&ВидыНДФЛ)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ВТНалогПОДоходам.ФизическоеЛицо,
			|	ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.НДФЛ),
			|	СУММА(ВТНалогПОДоходам.Налог)
			|ИЗ
			|	ВТНалогПОДоходам КАК ВТНалогПОДоходам
			|ГДЕ
			|	ВТНалогПОДоходам.Начисление В(&ВидыНачисленийУдержаний)
			|
			|СГРУППИРОВАТЬ ПО
			|	ВТНалогПОДоходам.ФизическоеЛицо
			|
			...
			|
			|ВЫБРАТЬ
			|	ВТРегистрНакопления_ВТНачисленияУдержания.ФизическоеЛицо,
			|	ВТРегистрНакопления_ВТНачисленияУдержания.НачислениеУдержание,
			|	ВТРегистрНакопления_ВТНачисленияУдержания.Сумма
			|ИЗ
			|	ВТРегистрНакопления_ВТНачисленияУдержания КАК ВТРегистрНакопления_ВТНачисленияУдержания
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНалогПОДоходам КАК ВТНалогПОДоходам
			|		ПО ВТРегистрНакопления_ВТНачисленияУдержания.ФизическоеЛицо = ВТНалогПОДоходам.ФизическоеЛицо
			|ГДЕ
			|	ВТНалогПОДоходам.ФизическоеЛицо ЕСТЬ NULL
			|	И ВТРегистрНакопления_ВТНачисленияУдержания.НачислениеУдержание В (&ВидыНДФЛ)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВТНалогПОДоходам");
	КонецЕсли;
	
	ЧастиЗапроса.Добавить(
		"УНИЧТОЖИТЬ ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТРегистрНакопления_НачисленияУдержанияПоКонтрагентамАкционерам
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТРегистрНакопления_ВТНачисленияУдержания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТРегистрНакопления_СведенияОДоходахНДФЛ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТРегистрРасчета_Удержания");
