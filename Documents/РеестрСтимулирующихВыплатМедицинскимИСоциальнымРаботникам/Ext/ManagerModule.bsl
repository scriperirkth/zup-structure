...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьДатыНачалаИОкончания");
	Методы.Вставить("ЗаполнитьБанкИЛицевойСчет");
КонецПроцедуры
...
Функция ПриИзмененииСтатусаОтправкиДокумента(РеестрСсылка, СтатусОтправки) Экспорт
	...
	Если ТипЗнч(СостояниеРеестра) = Тип("ПеречислениеСсылка.СостоянияРеестровФСС") Тогда
		...
		СвойстваПоследнейОтправки = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.СвойстваПоследнейОтправкиОтчетаВФСС(РеестрСсылка);
		Если СвойстваПоследнейОтправки <> Неопределено
			И ЗначениеЗаполнено(СвойстваПоследнейОтправки.ИдентификаторОтправкиНаСервере) Тогда
			ДокументОбъект.ВходящийНомерРеестра = СвойстваПоследнейОтправки.ИдентификаторОтправкиНаСервере;
		КонецЕсли;
...
Функция ЗаписатьРеестрВСтрокуXML(CovidWorkerRequest,СоциальныеВыплатыОтдельнымКатегориямМедРаботников, ВидПособия) Экспорт
	
	ИмяКорневогоЭлемента = ИмяКорневогоЭлемента(СоциальныеВыплатыОтдельнымКатегориямМедРаботников, ВидПособия);
...
Функция ИдентификаторФСС(Реестр) Экспорт
	...
	Если Реестр.ВидПособия = Перечисления.ПолучательСтимулирующихВыплатФСС.ТерриториальныйОрганФедеральныйГосОргана
		И ЗначениеЗаполнено(Реестр.РегистрационныйНомерСФР)Тогда
		РегистрационныйНомерФСС = Реестр.РегистрационныйНомерСФР;
	Иначе
	...
	КонецЕсли;
	НомерДокумента = Прав("000"+ НомерДокумента, 4);
	Префикс = ?(Реестр.ВидПособия = Перечисления.ПолучательСтимулирующихВыплатФСС.ТерриториальныйОрганФедеральныйГосОргана, "G_","C_");
	Возврат Префикс + РегистрационныйНомерФСС + "_" + ДатаЗаявления + "_" + НомерДокумента;
КонецФункции
...
Функция ДобавитьКомандыСозданияДокументов(КомандыСозданияДокументов, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		МодульГосударственнаяСлужба.ДобавитьКомандуВыплатТерриториальнымОрганам(КомандыСозданияДокументов);
	КонецЕсли;
	
КонецФункции

Функция ИмяКорневогоЭлемента(СоциальныеВыплатыОтдельнымКатегориямМедРаботников, ВидПособия)
	ИмяКорневогоЭлемента = "submitCovidWorkerRegistrRequest";
	Если ВидПособия = Перечисления.ПолучательСтимулирующихВыплатФСС.ТерриториальныйОрганФедеральныйГосОргана Тогда
		ИмяКорневогоЭлемента = "PaymentsFederalCivilServantsRequest";
	ИначеЕсли СоциальныеВыплатыОтдельнымКатегориямМедРаботников Тогда
		ИмяКорневогоЭлемента = "submit2023WorkerRegistrRequest";
	КонецЕсли;
	Возврат ИмяКорневогоЭлемента;
КонецФункции

Функция КорневойЭлементРеестра(СоциальныеВыплатыОтдельнымКатегориямМедРаботников, ВидПособия)
...
Функция СведенияДляРеестраПоШапкеДокументов(Ссылки)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокументов.ВидПособия = ЗНАЧЕНИЕ(Перечисление.ПолучательСтимулирующихВыплатФСС.ТерриториальныйОрганФедеральныйГосОргана)
	|			ТОГДА ""87""
...
Функция СведенияДляРеестраПоСотрудникам(Ссылка)
	...
	|						ТОГДА 30000
	|					ИНАЧЕ ВЫРАЗИТЬ(30000 * Сотрудники.ФактЧасов / Сотрудники.НормаЧасов КАК ЧИСЛО(15, 0))
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВыплатыТерОргану,
	|	ВЫБОР
	|		КОГДА Сотрудники.НормаЧасов <> 0
	|			ТОГДА ВЫБОР
	|					КОГДА Сотрудники.ФактЧасов / Сотрудники.НормаЧасов >= 1
...
Функция СформироватьФайлРеестра(ВыборкаПоШапкеДокумента, ВыборкаСведенийСотрудников)
	...
	РеестрВыплатТерриториальнымОрганам = (ВыборкаПоШапкеДокумента.ВидПособия = Перечисления.ПолучательСтимулирующихВыплатФСС.ТерриториальныйОрганФедеральныйГосОргана);
	CovidWorkerRequest = КорневойЭлементРеестра(ВыборкаПоШапкеДокумента.СоциальныеВыплатыОтдельнымКатегориямМедРаботников, ВыборкаПоШапкеДокумента.ВидПособия);
	...
	Если РеестрВыплатТерриториальнымОрганам
		И ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.РегистрационныйНомерСФР)Тогда
		CovidWorkerRequest.emplRegnum = ВыборкаПоШапкеДокумента.РегистрационныйНомерСФР;
	Иначе
	...
	Если РеестрВыплатТерриториальнымОрганам Тогда
		CovidWorkerRequest.dt1 = Формат(ВыборкаПоШапкеДокумента.НачалоПериода,"ДФ=yyyy-MM-dd");
	ИначеЕсли ВыборкаПоШапкеДокумента.СоциальныеВыплатыОтдельнымКатегориямМедРаботников Тогда
		CovidWorkerRequest.dt1 = Формат(ВыборкаПоШапкеДокумента.НачалоПериода,"ДФ=yyyy-MM-dd");
	...
	Пока ВыборкаСведенийСотрудников.Следующий() Цикл
		...
		ТегДанных = "InsuredData";
		Если РеестрВыплатТерриториальнымОрганам Тогда
			ТегДанных = "CivilServantData";
		КонецЕсли;
		ТипInsuredData = ROW.Свойства().Получить(ТегДанных).Тип;
		...
		Если РеестрВыплатТерриториальнымОрганам
			И ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.РегистрационныйНомерСФР)Тогда
			EmployerData.emplRegnumSFR = ВыборкаПоШапкеДокумента.РегистрационныйНомерСФР;
		Иначе
		...
		Если ВыборкаПоШапкеДокумента.СоциальныеВыплатыОтдельнымКатегориямМедРаботников
			ИЛИ РеестрВыплатТерриториальнымОрганам Тогда
		...
		Если РеестрВыплатТерриториальнымОрганам Тогда
			CalculationData.paymentSum = ОКР(ВыборкаСведенийСотрудников.СуммаВыплатыТерОргану);
	...
	СтрокаXML = ЗаписатьРеестрВСтрокуXML(CovidWorkerRequest, ВыборкаПоШапкеДокумента.СоциальныеВыплатыОтдельнымКатегориямМедРаботников, ВыборкаПоШапкеДокумента.ВидПособия);
...
Функция СведенияПоСотрудникамНаСервере(Организация, НачалоПериода, ОкончаниеПериода, Сотрудники, СоциальныеВыплатыОтдельнымКатегориямМедРаботников, Автозаполнение = Ложь, ВыплатыТерриториальномуОргану = Ложь) Экспорт
	...
	Если НЕ ВыплатыТерриториальномуОргану И 
		Автозаполнение Тогда
...
Функция ДанныеОтправленныхРеестров(Организация, ПолучательСтимулирующихВыплатФСС, Сотрудники, НачалоПериода, ОкончаниеПериода, СоциальныеВыплатыОтдельнымКатегориямМедРаботников, Ссылка = Неопределено) Экспорт
	...
	|	И ((Реестр.ВидПособия = ЗНАЧЕНИЕ(Перечисление.ПолучательСтимулирующихВыплатФСС.МедицинскаяОрганизация) ИЛИ Реестр.ВидПособия = ЗНАЧЕНИЕ(Перечисление.ПолучательСтимулирующихВыплатФСС.ТерриториальныйОрганФедеральныйГосОргана))
...
Функция СотрудникиПолучателиСтимулирующихВыплатФСС(Организация, НачалоПериода, ОкончаниеПериода, СоциальныеВыплатыОтдельнымКатегориямМедРаботников = Ложь, ВыплатыТерриториальномуОргану = Ложь) Экспорт
	
	Если ВыплатыТерриториальномуОргану
		И ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		МодульГосударственнаяСлужба = ОбщегоНазначения.ОбщийМодуль("ГосударственнаяСлужба");
		Возврат МодульГосударственнаяСлужба.СотрудникиПолучателиВыплатТерриториальнымОрганам(Организация, НачалоПериода, ОкончаниеПериода);
	КонецЕсли;
