...
Процедура ЗаполнитьПерерасчеты(МенеджерРасчета, СотрудникиОтбора, ПараметрыРасчета) Экспорт
	...
	УникальныеСотрудники = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СотрудникиОтбора);
...
Процедура РасчетЗарплатыВДанныeОбъекта(ДанныеМенеджераРасчета, ПараметрыРасчета) Экспорт
	...
	УстановитьМаксимальныйИдентификаторСтрокиНДФЛ(ОписаниеТаблицыНДФЛ);
...
Процедура УстановитьМаксимальныйИдентификаторСтрокиНДФЛ(ОписаниеТаблицыНДФЛ) 
	
	Если ЭтотОбъект.НДФЛ.Количество() = 0 Тогда
		МаксимальныйИдентификаторСтрокиНДФЛ = ОписаниеТаблицыНДФЛ.НомерТаблицы * 1000000;
	Иначе
		МаксимальныйИдентификаторСтрокиНДФЛ = 0;
		Для Каждого СтрокаНДФЛ Из ЭтотОбъект.НДФЛ Цикл
			МаксимальныйИдентификаторСтрокиНДФЛ = Макс(МаксимальныйИдентификаторСтрокиНДФЛ, СтрокаНДФЛ.ИдентификаторСтрокиНДФЛ);	
		КонецЦикла;
	КонецЕсли; 
	ОписаниеТаблицыНДФЛ.Вставить("МаксимальныйИдентификаторСтроки", МаксимальныйИдентификаторСтрокиНДФЛ);
