...
Процедура РассчитатьВсеНаСервере()
	...
	РассчитатьСотрудниковНаСервере(Сотрудники, Документы.МатериальнаяПомощь.ОписаниеТаблицыНачислений(РегистрацияНачисленийДоступна));
...
Процедура ЗаполнитьНастройкиМенеджераРасчета(ФизическиеЛица, МенеджерРасчета, СохранятьИсправления = Ложь)
	...
	ПараметрыРасчета.Вставить("ОкончательныйРасчетНДФЛ", ОкончательныйРасчетНДФЛ); 
	ПараметрыРасчета.Вставить("СохранятьИсправления", СохранятьИсправления); 
...
Процедура РассчитатьСотрудниковНаСервере(Знач Сотрудники, ОписаниеТаблицы, СохранятьИсправления = Ложь, ВыводитьСообщения = Ложь)
	...
	ПараметрыРасчета.ИзмененныеДанные = ЭтаФорма.ИзмененныеДанные;	
	ПараметрыРасчета.Сотрудники = Сотрудники;	
	ПараметрыРасчета.ФизическиеЛицаСотрудников = ФизическиеЛицаСотрудников;	
	...
	ОбновитьДанныеФормыПослеРасчета(Сотрудники, ФизическиеЛицаСотрудников); 
...
Процедура ПересчитатьСотрудникаНаСервере(ИмяТаблицы, ВыбранныеСтроки, ВедущееПоле, ТипВедущегоПоля)
	...
	Для Каждого ИдентификаторСтроки Из ВыбранныеСтроки Цикл
		СтрокаТабличнойЧасти = Объект[ИмяТаблицы].НайтиПоИдентификатору(ИдентификаторСтроки);
		СтрокаТабличнойЧасти.СкидкаПоВзносам = 0;
	КонецЦикла;	
...
Функция СотрудникиФизическихЛиц(Знач ФизическиеЛица)
	...
	Документ = РеквизитФормыВЗначение("Объект");
    Возврат Документ.СотрудникиФизическихЛиц(ФизическиеЛица);
...
Процедура ОбработатьИзменениеПоказателейНаСервере(ЗначенияПоказателей)
	...
	Для Каждого СтрокаСотрудника Из Объект.Начисления Цикл
		...
		СтрокаСотрудника.СкидкаПоВзносам = 0;
...
Функция ПолучитьКонтролируемыеПоля() Экспорт	
	Возврат Документы.МатериальнаяПомощь.ПолучитьКонтролируемыеПоля();	
КонецФункции
...
&НаСервере
Процедура ОбновитьДанныеФормыПослеРасчета(Сотрудники, ФизическиеЛицаСотрудников)
	...
	Для Каждого СтрокаСотрудника Из Объект.Начисления Цикл
		...
		Если ФизическоеЛицо <> Неопределено Тогда
			...
			ЗаполнитьНалогиУдержанияСотрудника(СтрокаСотрудника, ФизическоеЛицо);
		КонецЕсли;
	КонецЦикла; 
