...
Перем КоличествоПроизвольныхФайлов;
Перем РазмерПроизвольныхФайлов;
...
Перем ВыгружатьДанныеРасширений1СПлюс;
...
Перем НомерПотока; // Число
Перем НомерКлиентскогоСеанса; // Число
...
Перем КоличествоОбрабатываемыхОбъектов; // ТаблицаЗначений
...
Процедура ИнициализироватьВыгрузку(Знач ПараметрыВыгрузки) Экспорт
	...
	Если Не ПараметрыВыгрузки.Свойство("ВыгружатьДанныеРасширений1СПлюс", ВыгружатьДанныеРасширений1СПлюс) Тогда
		ВыгружатьДанныеРасширений1СПлюс = Ложь;
	КонецЕсли;
	...
	Если Не ПараметрыВыгрузки.Свойство("Extensions1CPlus", Расширения1СПлюсАгент) Тогда
		Расширения1СПлюсАгент = Новый Соответствие();
	КонецЕсли;
	...
	НомерКлиентскогоСеанса = ВыгрузкаЗагрузкаДанныхСлужебный.НомерКлиентскогоСеансаИзПараметров(ПараметрыВыгрузки);
...
Процедура ФайлЗаписан(Знач ПолныйПутьКФайлу) Экспорт
	...
	РазмерПроизвольногоФайла = Неопределено;
	
	СтрокаСостава = НайтиСтрокуСостава(ПолныйПутьКФайлу, "ПолноеИмя");
	Если СтрокаСостава <> Неопределено Тогда
		СтрокаСостава.Размер = Файл.Размер();
	Иначе
		РазмерПроизвольногоФайла = Файл.Размер();
	КонецЕсли;
	...
	Если ЭтоДочернийПоток() Тогда
		...
		Если РазмерПроизвольногоФайла <> Неопределено Тогда
			
			ДанныеСообщения.Вставить("РазмерПроизвольныхФайлов", РазмерПроизвольногоФайла);
			ДанныеСообщения.Вставить("КоличествоПроизвольныхФайлов", 1);
	...
	Иначе
		...
		Если РазмерПроизвольногоФайла <> Неопределено Тогда
			...
			РазмерПроизвольныхФайлов = РазмерПроизвольныхФайлов + РазмерПроизвольногоФайла;
			КоличествоПроизвольныхФайлов = КоличествоПроизвольныхФайлов + 1;
...
// Завершает выгрузку. Записывает информацию о выгрузке в файл.
//
// Возвращаемое значение:
...
Функция ФинализироватьВыгрузку() Экспорт
	...
	ИмяФайлаДайджеста = СоздатьФайл(ВыгрузкаЗагрузкаДанныхСлужебный.Digest(), ВыгрузкаЗагрузкаДанныхСлужебный.CustomData());
	...
	ИмяФайлаСодержимого = СоздатьФайл(ВыгрузкаЗагрузкаДанныхСлужебный.PackageContents(), ВыгрузкаЗагрузкаДанныхСлужебный.CustomData());
...
// 
// Возвращаемое значение:
//  Булево - Это выгрузка данных
Функция ЭтоВыгрузкаДанных() Экспорт
...
Процедура ИнициализироватьЗагрузку(Знач ДанныеФайла, Знач ПараметрыЗагрузки) Экспорт
	...
	НомерКлиентскогоСеанса = ВыгрузкаЗагрузкаДанныхСлужебный.НомерКлиентскогоСеансаИзПараметров(ПараметрыЗагрузки);
...
Функция ПолучитьПолноеИмяФайла(Знач ОтносительноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		...
		ЧастиПути = СтрРазделить(ОтносительноеИмяФайла, "/\");
		
		СтрокаСостава = Новый Структура;
		СтрокаСостава.Вставить("Имя", ЧастиПути[ЧастиПути.ВГраница()]);
		СтрокаСостава.Вставить("ПолноеИмя", ВременныйКаталог + СтрСоединить(ЧастиПути, ПолучитьРазделительПути()));
		ЧастиПути.Удалить(ЧастиПути.ВГраница());
		СтрокаСостава.Вставить("Каталог", СтрСоединить(ЧастиПути, ПолучитьРазделительПути()));
...
Функция ПолучитьОтносительноеИмяФайла(Знач ПолноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		Возврат СтрЗаменить(ПолноеИмяФайла, ВременныйКаталог, "");
	Иначе
...
Функция ПолучитьПараметрыИнициализацииВПотоке() Экспорт
	...
	ПараметрыИнициализации.Вставить("НомерКлиентскогоСеанса", НомерКлиентскогоСеанса);
...
Процедура ОбработатьСообщениеПотока(ИмяМетода, ДанныеСообщения) Экспорт
	...
	ИначеЕсли ИмяМетода = "ФайлЗаписан" Тогда
		...
		Если ДанныеСообщения.Свойство("РазмерПроизвольныхФайлов") Тогда
			
			РазмерПроизвольныхФайлов = РазмерПроизвольныхФайлов + ДанныеСообщения.РазмерПроизвольныхФайлов;
			
		КонецЕсли;
		
		Если ДанныеСообщения.Свойство("КоличествоПроизвольныхФайлов") Тогда
			
			КоличествоПроизвольныхФайлов = КоличествоПроизвольныхФайлов + ДанныеСообщения.КоличествоПроизвольныхФайлов;
...
Функция ВидЗадания() Экспорт
	
	Если Параметры.Свойство("ВидЗадания") И ЗначениеЗаполнено(Параметры.ВидЗадания) Тогда
		Возврат Параметры.ВидЗадания;
	ИначеЕсли ДляЗагрузки Тогда
		Возврат Перечисления.ВидыЗаданийВыгрузкиЗагрузкиДанных.ПрочаяЗагрузкаДанных;
	Иначе
		Возврат Перечисления.ВидыЗаданийВыгрузкиЗагрузкиДанных.ПрочаяВыгрузкаДанных;
	КонецЕсли;
	
КонецФункции

// Следующий номер потока.
// 
// Возвращаемое значение:
//  Число - Следующий номер потока
Функция СледующийНомерПотока() Экспорт
...
// Возвращаемое значение:
//  Булево
Функция КлиентскийСеансСуществует() Экспорт
	
	Если Не ЗначениеЗаполнено(НомерКлиентскогоСеанса) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Для Каждого СеансИБ Из ПолучитьСеансыИнформационнойБазы() Цикл
		
		Если СеансИБ.НомерСеанса = НомерКлиентскогоСеанса Тогда
			Возврат Истина;
		КонецЕсли;
...
Процедура ПрерватьПроцессПриОтсутствииКлиентскогоСеанса() Экспорт
	
	ТекстИсключения = СтрШаблон(
		НСтр("ru = '%1 данных прервана, отсутствует клиентский сеанс ожидающий окончания процесса.'"),
		?(ЭтоВыгрузкаДанных(), НСтр("ru = 'Выгрузка'"), НСтр("ru = 'Загрузка'")));
...
Функция ДобавитьФайл(Знач ВидФайла, Знач Расширение = "xml", Знач ТипДанных = Неопределено)
	...
	ИмяФайла = ПолучитьИмяФайла(ВидФайла, Расширение);
...
Процедура ОжидатьСкачиванияЧастей()
	...
	Пока Истина Цикл
		...
		Если Не КлиентскийСеансСуществует() Тогда
			ПрерватьПроцессПриОтсутствииКлиентскогоСеанса();
		КонецЕсли;
...
Функция ПолучитьИмяФайла(Знач ВидФайла, Знач Расширение = "xml")
	
	Возврат ВыгрузкаЗагрузкаДанныхСлужебный.ПолучитьИмяФайла(ВидФайла, Расширение);
...
Процедура ЗаписатьДайджест(ИмяФайла)
	...
	ЧислоОбъектов = Состав.Итог("ЧислоОбъектов") + КоличествоПроизвольныхФайлов;
	РазмерДанных  = Состав.Итог("Размер") + РазмерПроизвольныхФайлов;
...
Процедура ЗаписатьИнформациюОРасширениях()
	...
	ИмяФайлаПоставляемыхРасширений = СоздатьФайл(ВыгрузкаЗагрузкаДанныхСлужебный.Extensions(), ВыгрузкаЗагрузкаДанныхСлужебный.CustomData());
	...
	Расширения1СПлюс = УстановленныеРасширения1СПлюс();
	...
	Для Каждого РасширениеКонфигурации Из АктивныеРасширения Цикл
		...
		Иначе
			...
			ДанныеРасширения1СПлюс = Расширения1СПлюс.Получить(РасширениеКонфигурации.УникальныйИдентификатор);
			
			Если ВыгружатьДанныеРасширений И ДанныеРасширения1СПлюс = Неопределено Тогда
				ИмяФайлаДанныхПользовательскогоРасширения = СоздатьПроизвольныйФайл("cfe", "ExtensionData");
				
				ЗаписатьРасширениеВФайл(РасширениеКонфигурации, ИмяФайлаДанныхПользовательскогоРасширения);			
			...
			ИнформацияОПользовательскомРасширении.Вставить("PlusExtVersionId", "");
			ИнформацияОПользовательскомРасширении.Вставить("PlusExtId", "");
			ИнформацияОПользовательскомРасширении.Вставить("Version", РасширениеКонфигурации.Версия);
			
			Если ДанныеРасширения1СПлюс <> Неопределено Тогда
				Если ВыгружатьДанныеРасширений1СПлюс Тогда
					ИмяФайлаДанныхПользовательскогоРасширения = СоздатьПроизвольныйФайл("cfe", "ExtensionData");
					ЗаписатьРасширениеВФайл(
						РасширениеКонфигурации, 
						ИмяФайлаДанныхПользовательскогоРасширения);
					ИмяФайлаДанныхПользовательскогоРасширения = 
						ПолучитьОтносительноеИмяФайла(ИмяФайлаДанныхПользовательскогоРасширения)
				Иначе
					ИмяФайлаДанныхПользовательскогоРасширения = ДанныеРасширения1СПлюс.ИдВерсииРасширения + ".cfe";
				КонецЕсли;
				
				ИнформацияОПользовательскомРасширении.PlusExtVersionId = ДанныеРасширения1СПлюс.ИдВерсииРасширения;
				ИнформацияОПользовательскомРасширении.PlusExtId = ДанныеРасширения1СПлюс.ИдРасширения;
				ИнформацияОПользовательскомРасширении.Вставить("FileName", ИмяФайлаДанныхПользовательскогоРасширения);
				ИнформацияОПользовательскомРасширении.Вставить("FileSaved", ВыгружатьДанныеРасширений1СПлюс);
				
				Если ДанныеРасширения1СПлюс.Свойство("ЗаданиеНаУстановку") Тогда
					ИнформацияОПользовательскомРасширении.Вставить(
						"InstallTask", ДанныеРасширения1СПлюс.ЗаданиеНаУстановку.Получить());					
				КонецЕсли;
...
Процедура ЗаписатьРасширениеВФайл(РасширениеКонфигурации, ИмяФайлаДанныхПользовательскогоРасширения)
...
Функция УстановленныеРасширения1СПлюс()
	УстановитьПривилегированныйРежим(Истина);
	Результат = Новый Соответствие();
	
	Если Расширения1СПлюсАгент.Количество() <> 0 Тогда
		Для Каждого КлючЗначение Из Расширения1СПлюсАгент Цикл
			Результат.Вставить(КлючЗначение.Ключ, 
				Новый Структура("ИдРасширения, ИдВерсииРасширения", 
					КлючЗначение.Значение.ExtId, КлючЗначение.Значение.ExtVersionId));
		КонецЦикла;
		
		Возврат Результат;
	КонецЕсли;
	
	ВыгрузкаЗагрузкаДанныхСлужебный.ЗаполнитьУстановленныеРасширения1СПлюс(Результат);
...
Процедура ОтправитьЧастьФайлаS3(Результат, ИмяФайла)
	...
	КонецЕсли;
	СоставнаяЗагрузка.Части.Добавить(ОбщегоНазначенияБТСКлиентСервер.ЗаголовокHTTP(Ответ, "ETag"));
...
Процедура ОтправитьЧастьФайлаDT(Результат, ИмяФайла, ПоследняяЧасть)
	...
	ПараметрыОтправки.Вставить("SetCookie", ОбщегоНазначенияБТСКлиентСервер.ЗаголовокHTTP(Результат, "SetCookie"));
...
Процедура ИнициализироватьПеременныеВыгрузки()
	...
	КоличествоПроизвольныхФайлов = 0;
	РазмерПроизвольныхФайлов = 0;
...
Процедура ИнициализироватьПеременныеВПотоке(ПараметрыКонтейнера)
	...
	НомерКлиентскогоСеанса = ВыгрузкаЗагрузкаДанныхСлужебный.НомерКлиентскогоСеансаИзПараметров(ПараметрыКонтейнера);
