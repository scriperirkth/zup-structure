...
Процедура ПолучениеИнформацииОПриложенииЗавершение(АдресРезультата)
	...
	Элементы.ИзменитьОтветственногоПриложения.Доступность = Истина;
...
Процедура ПрочитатьНастройки()
	...
	Объект.АдресРесурсовAPI 				= Настройки.АдресРесурсовAPI;
	...
	ИспользоватьАдресРесурсовAPI = ЗначениеЗаполнено(Объект.АдресРесурсовAPI);
...
&НаСервере
Процедура ОбновитьРеквизитыОплаты(АбонентСервиса, ПриложениеЗаблокировано)

	ИспользуетсяПробныйПериод 			= АбонентСервиса.ИспользуетсяПробныйПериод;
	ОплаченоПользователей 				= АбонентСервиса.ОплаченоКабинетов;
	...
	КонецЕсли;
	Строка2 = "";
	Если ОплаченоПользователей > 0 Тогда
		ШаблонСтроки = НСтр("ru = 'Оплачено пользователей: %1 по %2'");
		Строка2 = СтрШаблон(ШаблонСтроки, ОплаченоПользователей, Формат(ДатаОкончанияОплаченногоПериода, "ДЛФ=D"));
	Иначе
		Строка2 = НСтр("ru = 'Нет оплаченных пользователей'");
	КонецЕсли;
	ПереводСтроки = Символы.ПС;
	СтрокаСостояниеОплаты = СтрШаблон("%1%2%3%4", Строка1, ?(ПустаяСтрока(Строка1),"",ПереводСтроки), Строка2, ?(ПустаяСтрока(Строка2),"",ПереводСтроки));
	Элементы.ДекорацияСостояниеОплаты.Заголовок = СтрокаСостояниеОплаты;
	
	ЗаголовокПриложениеЗаблокировано = "";
	Если ПриложениеЗаблокировано <> Неопределено Тогда
		РегистрыСведений.НастройкиПодключения1СПерсонал.УстановитьСостояниеБлокировки(ПриложениеЗаблокировано);
		Если ПриложениеЗаблокировано Тогда
			Если ОплаченоПользователей = 0 Тогда
				ЗаголовокПриложениеЗаблокировано = НСтр("ru = 'Доступ к приложению ограничен: закончился оплаченный (тестовый) период'");
			Иначе
				ЗаголовокПриложениеЗаблокировано = НСтр("ru = 'Доступ к приложению ограничен: превышен оплаченный лимит на пользователей'");
			КонецЕсли;
		КонецЕсли;
		Элементы.ДекорацияБлокировка.Заголовок = ЗаголовокПриложениеЗаблокировано;
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
				"ДекорацияБлокировка", "Видимость", Не ПустаяСтрока(ЗаголовокПриложениеЗаблокировано));
