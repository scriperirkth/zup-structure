...
Процедура ЗаполнитьТабличныйДокументРасчетаСреднегоЗаработка(ПараметрыПечати, ТабличныйДокумент, ОбъектыПечати, ИмяМакета, ДанныеДокументов, ВыводитьЗаголовок)
	...
	Для Каждого Ссылка Из Ссылки Цикл
		...
		Для каждого ДанныеДокумента Из ДанныеДокументов Цикл
            ...
            ПрименениеРКСН = ПрименениеРКСН(ДанныеДокумента.ДанныеРасчетаСреднего.Заработок);
			...
			ОбластиМакета = ОбластиМакетаСреднегоЗаработка(ИспользуетсяИндексация, ДанныеДокумента.ПараметрыРасчета.СпособРасчета, ПрименениеРКСН);
...
Процедура ВывестиЗаработокЗаРасчетныйПериод(ТабличныйДокумент, НомерРаздела, ОбластьШапкаЗаработка, ОбластьСтрокаЗаработка, ОбластьПодвалЗаработка, Заработок)
	...
	ЗначенияПараметров.Вставить("УчтеноРК", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(Заработок, "УчтеноРК"));
	ЗначенияПараметров.Вставить("УчтеноСН", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(Заработок, "УчтеноСН"));
...
Процедура ВывестиПремииУчитываемыеПолностью(ТабличныйДокумент, НомерРаздела, ОбластьШапкаПремииПолностью, ОбластьСтрокаПремииПолностью, ОбластьПодвалПремииПолностью, ПремииПолностью)
	...
	ЗначенияПараметров.Вставить("УчтеноРК", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ПремииПолностью, "УчтеноРК"));
	ЗначенияПараметров.Вставить("УчтеноСН", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ПремииПолностью, "УчтеноСН"));
...
Процедура ВывестиПремииУчитываемыеПропорционально(ТабличныйДокумент, НомерРаздела, ОбластьШапкаПремииПропорционально, ОбластьСтрокаПремииПропорционально, ОбластьПодвалПремииПропорционально, ПремииПропорционально)
	...
	ЗначенияПараметров.Вставить("УчтеноРК", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ПремииПропорционально, "УчтеноРК"));
	ЗначенияПараметров.Вставить("УчтеноСН", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ПремииПропорционально, "УчтеноСН"));
...
Процедура ВывестиРасчетСреднегоЗаработка(ТабличныйДокумент, НомерРаздела, ОбластьРасчетСреднегоЗаработка, ДанныеРасчетаСреднего, ИспользоватьСреднеЧасовойЗаработок)
	...
	ЗначенияПараметров.Вставить("УчтеноРК", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ДанныеРасчетаСреднего.Заработок, "УчтеноРК"));
	ЗначенияПараметров.Вставить("УчтеноСН", УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ДанныеРасчетаСреднего.Заработок, "УчтеноСН"));
	ЗначенияПараметров.Вставить("СреднедневнойЗаработокРК", УчетСреднегоЗаработкаКлиентСервер.СреднийЗаработок(ЗначенияПараметров.УчтеноРК, ЗначенияПараметров.УчтеноВремени)); 
	ЗначенияПараметров.Вставить("СреднедневнойЗаработокСН", УчетСреднегоЗаработкаКлиентСервер.СреднийЗаработок(ЗначенияПараметров.УчтеноСН, ЗначенияПараметров.УчтеноВремени)); 
...
Функция ОбластиМакетаСреднегоЗаработка(ИспользуетсяИндексация = Истина, СпособРасчетаОтпуска = Неопределено, ПрименениеРКСН = Неопределено)
	
	ЗаработокРКСН = Ложь;
	ПремииРКСН = Ложь;
	
	Если ПрименениеРКСН <> Неопределено Тогда 
		ЗаработокРКСН = ПрименениеРКСН.Заработок;
		ПремииРКСН = ПрименениеРКСН.Премии;
	КонецЕсли;
	...
	Если ИспользуетсяИндексация Тогда
		Если ЗаработокРКСН Тогда 
			ОбластиМакета.Вставить("ОбластьШапкаЗаработка", Макет.ПолучитьОбласть("ЗаработокШапкаРКСН"));
			ОбластиМакета.Вставить("ОбластьСтрокаЗаработка", Макет.ПолучитьОбласть("ЗаработокСтрокаРКСН"));
			ОбластиМакета.Вставить("ОбластьПодвалЗаработка", Макет.ПолучитьОбласть("ЗаработокПодвалРКСН"));
		Иначе 
		...
		КонецЕсли;
		Если ПремииРКСН Тогда 
			ОбластиМакета.Вставить("ОбластьШапкаПремииПолностью", Макет.ПолучитьОбласть("ПремииПолностьюШапкаРКСН"));
			ОбластиМакета.Вставить("ОбластьСтрокаПремииПолностью", Макет.ПолучитьОбласть("ПремииПолностьюСтрокаРКСН"));
			ОбластиМакета.Вставить("ОбластьПодвалПремииПолностью", Макет.ПолучитьОбласть("ПремииПолностьюПодвалРКСН"));
			ОбластиМакета.Вставить("ОбластьШапкаПремииПропорционально", Макет.ПолучитьОбласть("ПремииПропорциональноШапкаРКСН"));
			ОбластиМакета.Вставить("ОбластьСтрокаПремииПропорционально", Макет.ПолучитьОбласть("ПремииПропорциональноСтрокаРКСН"));
			ОбластиМакета.Вставить("ОбластьПодвалПремииПропорционально", Макет.ПолучитьОбласть("ПремииПропорциональноПодвалРКСН"));
		Иначе 
	...
	Иначе
		Если ЗаработокРКСН Тогда 
			ОбластиМакета.Вставить("ОбластьШапкаЗаработка", Макет.ПолучитьОбласть("ЗаработокШапкаБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьСтрокаЗаработка", Макет.ПолучитьОбласть("ЗаработокСтрокаБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьПодвалЗаработка", Макет.ПолучитьОбласть("ЗаработокПодвалБезИндексацииРКСН")); 
		Иначе 
		...
		КонецЕсли;
		Если ПремииРКСН Тогда 
			ОбластиМакета.Вставить("ОбластьШапкаПремииПолностью", Макет.ПолучитьОбласть("ПремииПолностьюШапкаБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьСтрокаПремииПолностью", Макет.ПолучитьОбласть("ПремииПолностьюСтрокаБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьПодвалПремииПолностью", Макет.ПолучитьОбласть("ПремииПолностьюПодвалБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьШапкаПремииПропорционально", Макет.ПолучитьОбласть("ПремииПропорциональноШапкаБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьСтрокаПремииПропорционально", Макет.ПолучитьОбласть("ПремииПропорциональноСтрокаБезИндексацииРКСН"));
			ОбластиМакета.Вставить("ОбластьПодвалПремииПропорционально", Макет.ПолучитьОбласть("ПремииПропорциональноПодвалБезИндексацииРКСН"));
		Иначе 
	...
	Если ЗаработокРКСН Тогда 
		ОбластиМакета.Вставить("ОбластьРасчетСреднегоЗаработка", Макет.ПолучитьОбласть("РасчетСреднегоЗаработкаРКСН"));
	Иначе 
...
Функция ПрименениеРКСН(Заработок)
	
	ЗаработокРКСН = Ложь;
	ПремииРКСН = Ложь;
	Для Каждого СтрокаЗаработка Из Заработок Цикл
		Если СтрокаЗаработка.УчтеноРК > 0 Или СтрокаЗаработка.УчтеноСН > 0 Тогда
			ЗаработокРКСН = Истина;
			Если Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.Премии().Найти(СтрокаЗаработка.СоставнаяЧасть) <> Неопределено Тогда
				ПремииРКСН= Истина;
			КонецЕсли;
			Если ЗаработокРКСН И ПремииРКСН Тогда 
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;		
	
	Возврат Новый Структура("Заработок, Премии", ЗаработокРКСН, ПремииРКСН);	
