...
Процедура ЗарплатаДекорацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ЗарегистрироватьПерерасчетыГПХ" Тогда
		ЗарегистрироватьНеобходимостьПерерасчетаГПХ();
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьПерерасчетГПХ(Команда)
...
&НаКлиенте
Процедура ЗарегистрироватьНеобходимостьПерерасчетаГПХ()
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		ОткрытьФорму("Обработка.УправлениеПерерасчетами.Форма.ФормаНовойЗаписиПерерасчетовГПХ", ПараметрыОткрытия, ЭтаФорма, , , , Оповещение);
	...
	Если УдаляемыеСтроки.Количество() > 0 Тогда
		...
		Для каждого ИдентификаторУдаляемойСтроки Из УдаляемыеСтроки Цикл
			...
			Если ТекущиеДанные <> Неопределено Тогда
				...
				ДоговорыГПХРасширенный.УдалитьЗаписьПерерасчетов(ТекущиеДанные.Период, ТекущиеДанные.ФизическоеЛицо, Организация, ТекущиеДанные.ДокументНачисления);
...
Процедура ЗаполнитьПерерасчетыЗарплаты(Сотрудник = Неопределено, Период = Неопределено)
	...
	ДоговорыГПХРасширенный.ДобавитьОбъединениеСПерерасчетамиДоговоров(Запрос.Текст);
	...
	Если ТаблицаЗарплата.Количество() = 0 Тогда
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			Если ПолучитьФункциональнуюОпциюФормы("ИспользоватьНачисленияПоДоговорамРасширенная") Тогда
				ТекстДекорации = Новый ФорматированнаяСтрока(
					ТекстДекорации,
					", ",
					Новый ФорматированнаяСтрока(НСтр("ru='договоров ГПХ'"), , , , "ЗарегистрироватьПерерасчетыГПХ"));
			КонецЕсли;
