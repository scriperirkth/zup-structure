...
Процедура ОбновитьСписокФайлов()
	...
	Для Каждого СведенияОФайле Из Файлы.files Цикл
		...
		КонецЕсли;
		НоваяСтрока.НавигационнаяСсылка = СведенияОФайле.objectID.navigationRef;
...
Процедура ВыбратьВыполнить()
	...
	Результат.Вставить("РеквизитТип", "DMFile");
	Результат.Вставить("РеквизитПредставление", ТекущиеДанные.Файл);
	Результат.Вставить("РеквизитНавигационнаяСсылка", ТекущиеДанные.НавигационнаяСсылка);
	Закрыть(Результат);
