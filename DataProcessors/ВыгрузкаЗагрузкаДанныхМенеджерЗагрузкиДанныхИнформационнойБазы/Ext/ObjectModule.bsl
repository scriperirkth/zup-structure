...
Процедура ЗагрузитьДанные() Экспорт
	...
	Если ИспользоватьМногопоточность() Тогда
		...
		ЗаданиеЗагрузки = ВыгрузкаЗагрузкаДанныхСлужебный.ПолучитьЗаданиеПараллельнойВыгрузкиЗагрузкиДанных();
		
		РегистрыСведений.ПотокиВыгрузкиЗагрузкиДанных.ДобавитьЗапись(
			ЗаданиеЗагрузки,
			ТекущийКонтейнер,
			Перечисления.ВидыПотоковВыгрузкиЗагрузкиДанных.Управляющий);
		
		МенеджерОбработкиЗагрузки = Обработки.ВыгрузкаЗагрузкаДанныхМенеджерЗагрузкиДанныхИнформационнойБазы;
		ПараметрыПроцессаЗагрузки = МенеджерОбработкиЗагрузки.НовыеПараметрыПроцессаЗагрузкиДанныхИнформационнойБазы(
			ТекущийКонтейнер,
			ТекущийПотокЗаменыСсылок);
		Задания = МенеджерОбработкиЗагрузки.ЗапуститьПотокиЗагрузкиДанных(
			ПараметрыПроцессаЗагрузки,
			ТекущийКонтейнер.КоличествоПотоков(),
			ВыгрузкаЗагрузкаДанныхСлужебный.ВидПотокаПараллельнойВыгрузкиЗагрузкиДанных(ТекущийКонтейнер));
		
		ВыгрузкаЗагрузкаДанныхСлужебный.ОжидатьВыгрузкуЗагрузкуДанныхВПотоках(Задания, ПараметрыПроцессаЗагрузки);
...
Процедура ВыполнитьЗагрузкуДанных()
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		Если Не ТекущийКонтейнер.КлиентскийСеансСуществует() Тогда
			ТекущийКонтейнер.ПрерватьПроцессПриОтсутствииКлиентскогоСеанса();
		КонецЕсли;
