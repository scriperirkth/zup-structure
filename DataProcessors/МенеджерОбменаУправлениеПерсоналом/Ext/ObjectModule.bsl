...
Перем ИспользоватьОтдельныйСервисАутентификации;
...
Процедура Инициализировать(ПриложениеДляОбмена, ПодготовитьДанныеДляТехПоддержки) Экспорт
	...
	АдресРесурсовAPI 		= НастройкиПодключения.АдресРесурсовAPI;
	
	Если ЗначениеЗаполнено(АдресРесурсовAPI) Тогда
		АдресAPIПриложения = АдресРесурсовAPI;
	Иначе
		АдресAPIПриложения = АдресПриложения;
	КонецЕсли;
	СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресAPIПриложения);
	
	ИспользоватьОтдельныйСервисАутентификации = ЗначениеЗаполнено(АдресСервисаАвторизации);
	Если Не ИспользоватьОтдельныйСервисАутентификации Тогда
		АдресСервисаАвторизации = АдресПриложения;
	КонецЕсли;
...
Процедура ЗаполнитьТипОбъектаАдресРесурса()
	...
	Описание.Вставить(МенеджерТипОбъекта.ГрафикОтпусковПодразделения, 			РесурсГрафикОтпусковПодразделения());
...
Функция РесурсГрафикОтпусковПодразделения()
	
	Если ИспользуетсяВерсия("2.4") Тогда
		Возврат "/vacations-schedule-draft/divisions";
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ПриложениеДоступно() Экспорт
	...
	ПолныйАдресРесурса = ПолныйАдресРесурса(СтруктураURI.ПутьНаСервере, "/api", РесурсПинг());
...
// 			* АдресПриложения - Строка - адрес приложения для пользователей
// 			* АдресСервисаАвторизации - Строка - адрес сервиса аутентификации (получение токена)
// 			* АдресРесурсовAPI - Строка - адрес API приложения (для отправки прочих запросов)
// 			* Ключи - Структура
...
Функция ПроверитьНастройкиПодключения(НастройкиПодключения) Экспорт
	...
	АдресРесурсовAPI = Неопределено;
	НастройкиПодключения.Свойство("АдресРесурсовAPI", АдресРесурсовAPI);
	Если Не ЗначениеЗаполнено(АдресРесурсовAPI) Тогда
		АдресРесурсовAPI = АдресПриложения;
	КонецЕсли;
	СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресРесурсовAPI);
	СоздатьНовоеHTTPСоединение();
	...
	ПолныйАдресРесурса = ПолныйАдресРесурса(СтруктураURI.ПутьНаСервере, АдресРесурсаAPI, РесурсПинг());
	Запрос = Новый HTTPЗапрос(ПолныйАдресРесурса, Заголовки);
	ИмяМетода = "GET";
	Попытка
		Ответ = HTTPСоединение.ВызватьHTTPМетод(ИмяМетода, Запрос);
...
Функция ПроверитьАдресаСервисаАвторизации(АдресСервиса) Экспорт
	...
	СтруктураURIАвторизации = ОбщегоНазначенияКлиентСервер.СтруктураURI(АдресСервисаАвторизации);
	СоздатьНовоеHTTPСоединениеАвторизации();
	ИспользоватьОтдельныйСервисАутентификации = Истина;
...
Функция РезультатВыгрузкиУдаления(ОбъектыКУдалению, ТипОбъекта) Экспорт
	...
	Для каждого Ссылка Из ОбъектыКУдалению Цикл
		...
		Если Не ЗначениеЗаполнено(Идентификатор) Тогда
			Результат.ОтменитьРегистрацию.Добавить(Ссылка);
...
Функция ОписаниеТекстаЗапросаДляЖР(ИмяМетода, HTTPЗапрос, АдресСервера = "")
...
Функция ТокенАутентификации()
	...
	Если Токен = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Ошибка аутентификации.'");
...
Функция НовыйТокен(КлючиПриложения)
	...
	СообщениеОбОшибке = НСтр("ru = 'Не удалось получить ответ от сервера аутентификации'");
	...
	Если ИспользоватьОтдельныйСервисАутентификации Тогда
		...
		ИначеЕсли СтрЗаканчиваетсяНа(Врег(АдресСервисаАвторизации), ВРег("/auth/oidc/token")) Тогда
			АдресРесурсаАвторизации = СтруктураURIАвторизации.ПутьНаСервере;
		Иначе
			АдресРесурсаАвторизации = ПолныйАдресРесурса(СтруктураURIАвторизации.ПутьНаСервере, "/auth/oidc/token");
		КонецЕсли;
		
		Ответ = ОтправитьДляОбработкиПолученияТокена(АдресРесурсаАвторизации, Заголовки, СтрокаТела);
		Если Ответ = Неопределено Тогда
	...
	Иначе
		...
		Если КлючиПриложения.Свойство("АутентификацияSysToken") И КлючиПриложения.АутентификацияSysToken = Истина Тогда
			АдресРесурса = ПолныйАдресРесурса(СтруктураURIАвторизации.ПутьНаСервере, "/sys/token");
...
Функция ОтправитьДляОбработкиПолученияТокена(АдресРесурса, Заголовки, СтрокаТела)
	...
	Если ВестиПротокол Тогда
		АдресСервера = СтрШаблон("%1://%2", СтруктураURIАвторизации.Схема, СтруктураURIАвторизации.ИмяСервера);
		ЗапросТекст = ОписаниеТекстаЗапросаДляЖР("POST", Запрос, АдресСервера);
		ШаблонОписания = НСтр("ru = '%1
		|%2'");
		Комментарий = СтрШаблон(ШаблонОписания, ЗапросТекст, СтрокаТела);
		ЗаписьЖурналаРегистрации(ИменаСобытийЖР.Запрос, УровеньЖурналаРегистрации.Информация,,, Комментарий);
	КонецЕсли;
...
Процедура ЗаписатьИспользоватьНовуюАвторизацию()
	...
	АдресХраненияКлючей = ИнтеграцияУправлениеПерсоналом.АдресХраненияКлючейПриложения(Приложение);
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(АдресХраненияКлючей, Истина, "АутентификацияSysToken");
...
Функция HTTPОтветСервераВерсииФормата()
	...
	АдресРесурса = ПолныйАдресРесурса(СтруктураURI.ПутьНаСервере, РесурсПриложения);
	Запрос = Новый HTTPЗапрос(АдресРесурса, Заголовки);
...
Функция HTTPОтветСервера(ПараметрыЗапроса)
	...
	ПолныйАдресРесурса = ПолныйАдресРесурса(СтруктураURI.ПутьНаСервере, АдресРесурсаAPI, АдресРесурса);
...
Функция ПолныйАдресРесурса(ЧастьАдреса1, ЧастьАдреса2, ЧастьАдреса3 = "")
	
	Если Не ПустаяСтрока(ЧастьАдреса1) И Лев(ЧастьАдреса1, 1) <> "/" Тогда
		ЧастьАдреса1 = "/" + ЧастьАдреса1;
	КонецЕсли;
	
	Если Не ПустаяСтрока(ЧастьАдреса2) И Лев(ЧастьАдреса2, 1) <> "/" Тогда
		ЧастьАдреса2 = "/" + ЧастьАдреса2;
	КонецЕсли;
	
	Если Не ПустаяСтрока(ЧастьАдреса3) И Лев(ЧастьАдреса3, 1) <> "/" Тогда
		ЧастьАдреса3 = "/" + ЧастьАдреса3;
	КонецЕсли;
	
	Возврат СтрШаблон("%1%2%3", ЧастьАдреса1, ЧастьАдреса2, ЧастьАдреса3);
