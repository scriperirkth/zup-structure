...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЕстьКлючиПриложения Тогда
		КлючиПриложенияАктуальные = ИнтеграцияУправлениеПерсоналомОбмен.КлючиПриложенияАктуальные(Приложение);
	КонецЕсли;
...
Процедура ОпубликованныеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.ДокументЭДОБЗК) Тогда
		ПараметрыФормы = Новый Структура("Ключ", Элемент.ТекущиеДанные.ДокументЭДОБЗК);
		ОткрытьФорму("Документ.ДокументЭДОБЗК.Форма.ФормаДокумента", ПараметрыФормы);
	КонецЕсли;
...
Процедура ОбновитьСписокКПубликации(ОбновитьСписокПубликации = Ложь);
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Иначе
			...
			|			И (НЕ ФизическиеЛица.УсловноВыгружается)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОшибкиЗаполненияОбъектовУправлениеПерсоналом КАК ОшибкиЗаполнения
			|		ПО Сотрудники.ФизическоеЛицо = ОшибкиЗаполнения.Ссылка
			|			И (ОшибкиЗаполнения.БлокирующаяОшибка)
			|			И (ОшибкиЗаполнения.Приложение = ЗНАЧЕНИЕ(Перечисление.ПриложенияДляИнтеграции.КабинетСотрудника))
...
&НаСервере
Функция ДлительнаяОперацияОпубликоватьНаСервере(ПовторнаяПубликация, АдресВременногоХранилища)
	...
	ПараметрыПубликации = ПараметрыПубликации(ПовторнаяПубликация, АдресВременногоХранилища);
...
Функция ФизическиеЛицаСОпубликованнымиЛистками()
	...
	АдресВременногоХранилищаФизическихЛиц = "";
	...
	Если ЗначениеЗаполнено(Объект.ПодразделениеОпубликованных) Тогда
		...
		Если ФизическиеЛица.Количество() = 0 Тогда
			Возврат АдресВременногоХранилищаФизическихЛиц;
		КонецЕсли;
	...
	Если СписокФизическихЛиц.Количество() > 0 Тогда
		АдресВременногоХранилищаФизическихЛиц = ПоместитьВоВременноеХранилище(СписокФизическихЛиц, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ОпубликоватьПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		ОчиститьСообщения();
		ВыполнитьПубликациюСохранитьЖурнал(ДополнительныеПараметры.ПовторнаяПубликация, ДополнительныеПараметры.ОбновитьВсе);
	КонецЕсли;
...
Процедура ОпубликоватьРасчетныеЛистки(ПовторнаяПубликация, ОбновитьВсе)
	...
	АдресВременногоХранилища = ПолучитьСписокФизическихЛиц(ПовторнаяПубликация, ОбновитьВсе);
	...
	ДлительнаяОперация = ДлительнаяОперацияОпубликоватьНаСервере(ПовторнаяПубликация, АдресВременногоХранилища);
...
&НаСервере
Функция ПараметрыПубликации(ПовторнаяПубликация, АдресВременногоХранилища = Неопределено)
	
	ПараметрыПубликации = ИнтеграцияКабинетСотрудникаОбмен.НовоеОписаниеПараметрыПубликацииРасчетныхЛистков();
	ПараметрыПубликации.ПовторнаяПубликация 						= ПовторнаяПубликация;
	ПараметрыПубликации.Организация 								= Объект.Организация;
	ПараметрыПубликации.ПодготовитьДанныеДляТехПоддержки 			= ПодготовитьДанныеДляТехПоддержки;
	ПараметрыПубликации.МесяцРасчетныхЛистов 						= ?(ПовторнаяПубликация,
																		Объект.МесяцОпубликованных,
																		Объект.Месяц);
	ПараметрыПубликации.ВариантОтчета 								= ВариантОтчета;
	ПараметрыПубликации.ПерваяПоловинаМесяца 						= Не РасчетныеЛистыЗаМесяц;
	ПараметрыПубликации.ФормироватьРасчетныеЛистыОтдельнымЗапросом 	= ФормироватьРасчетныеЛистыОтдельнымЗапросом;
	Если АдресВременногоХранилища <> Неопределено Тогда
		ПараметрыПубликации.СписокФизическихЛиц = ПолучитьИзВременногоХранилища(АдресВременногоХранилища);
...
&НаКлиенте
Функция ПолучитьСписокФизическихЛиц(ПовторнаяПубликация, ОбновитьВсе)
	...
	Если ПовторнаяПубликация Тогда
		Если ОбновитьВсе Тогда
			АдресВременногоХранилищаФизическихЛиц = ФизическиеЛицаСОпубликованнымиЛистками();
		Иначе
	...
	Если Не ЗначениеЗаполнено(АдресВременногоХранилищаФизическихЛиц) И СписокФизическихЛиц.Количество() > 0 Тогда
		АдресВременногоХранилищаФизическихЛиц = ПоместитьВоВременноеХранилище(СписокФизическихЛиц, УникальныйИдентификатор);
	КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьПубликациюСохранитьЖурнал(ПовторнаяПубликация, ОбновитьВсе)
	...
	ПараметрыОткрытия = ПараметрыОбменаССохранениемРезультата(ПовторнаяПубликация, АдресВременногоХранилища);
...
&НаСервере
Функция ПараметрыОбменаССохранениемРезультата(ПовторнаяПубликация, АдресВременногоХранилища)
	...
	ПараметрыОткрытия.Вставить("АдресСпискаФизическихЛиц", 	АдресВременногоХранилища);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыОткрытия, ПараметрыПубликации(ПовторнаяПубликация), Ложь);
