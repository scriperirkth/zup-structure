	...
	Если Не ПустаяСтрока(Правило.ПередУдалением) Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Правило,
						"ПередУдалением"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПередУдалением, Неопределено);
				КонецЕсли;
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
						"ПередКонвертациейОбъекта"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПередКонвертациейОбъекта, Неопределено);
				КонецЕсли;
	...
	Если ПКО.ЕстьОбработчикПередВыгрузкой Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКО, "ПередВыгрузкой"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПКО.ПередВыгрузкой, Неопределено);
				КонецЕсли;
	...
	Если ПКО.ЕстьОбработчикПриВыгрузке Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКО, "ПриВыгрузке"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПКО.ПриВыгрузке, Неопределено);
				КонецЕсли;
	...
	Если ПКО.ЕстьОбработчикПослеВыгрузки Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКО, "ПослеВыгрузки"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПКО.ПослеВыгрузки, Неопределено);
				КонецЕсли;
	...
	Если ПКО.ЕстьОбработчикПослеВыгрузкиВФайл Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКО, "ПослеВыгрузкиВФайл"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПКО.ПослеВыгрузкиВФайл, Неопределено);
				КонецЕсли;
	...
	Если Не ПустаяСтрока(Правило.ПередОбработкой) Тогда
		...
		Попытка
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПередОбработкой, Неопределено);
				КонецЕсли;
	...
	Если Не ПустаяСтрока(Правило.ПослеОбработки) Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Правило, "ПослеОбработки"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПослеОбработки, Неопределено);
				КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКО,
						"ПоследовательностьПолейПоиска"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(АлгоритмПоиска, Неопределено);
				КонецЕсли;
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
						"ПередЗагрузкойОбъекта"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПередЗагрузкойОбъекта, Неопределено);
				КонецЕсли;
	...
	Если ЕстьОбработчикПередЗагрузкой Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Правило, "ПередЗагрузкой"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПередЗагрузкой, Неопределено);
				КонецЕсли;
	...
	Пока ФайлОбмена.Прочитать() Цикл
				...
				Если ЕстьОбработчикПриЗагрузке Тогда
					...
					Попытка
						...
						Иначе
							...
							Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
								МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
								МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПриЗагрузке, Неопределено);
							КонецЕсли;
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
				...
				Попытка
					...
					Если ФлагРежимОтладкиОбработчиков Тогда
						...
						Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
							МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
							МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
								"ПослеЗагрузкиОбъекта"), Неопределено);
						КонецЕсли;
					...
					Иначе
						...
						Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
							МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
							МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПослеЗагрузкиОбъекта, Неопределено);
						КонецЕсли;
			...
			Если ЕстьОбработчикПослеЗагрузки Тогда
				...
				Попытка
					...
					Если ФлагРежимОтладкиОбработчиков Тогда
						...
						Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
							МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
							МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Правило, "ПослеЗагрузки"), Неопределено);
						КонецЕсли;
					...
					Иначе
						...
						Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
							МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
							МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПослеЗагрузки, Неопределено);
						КонецЕсли;
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗаписи = "Проведение" Или Объект.Проведен Тогда
...
Функция ВычислитьВыражение(Знач Выражение)
	...
	// ++ Требование безопасности. Поддержка стандарта №770
	...
	Если ПКГС.ЕстьОбработчикПередОбработкойВыгрузки Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКГС,
						"ПередОбработкойВыгрузки"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПКГС.ПередОбработкойВыгрузки, Неопределено);
				КонецЕсли;
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКГС,
							"ПослеОбработкиВыгрузки"), Неопределено);
					КонецЕсли;
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПКГС.ПослеОбработкиВыгрузки, Неопределено);
					КонецЕсли;
		...
		Если ПКГС.ЕстьОбработчикПередВыгрузкой Тогда
			...
			Попытка
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПКГС.ПередВыгрузкой, Неопределено);
					КонецЕсли;
		...
		Если ПКГС.ЕстьОбработчикПриВыгрузке Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКГС, "ПриВыгрузке"),
							Неопределено);
					КонецЕсли;
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПКГС.ПриВыгрузке, Неопределено);
					КонецЕсли;
		...
		Если ПКГС.ЕстьОбработчикПослеВыгрузки Тогда
			...
			Попытка
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПКГС.ПослеВыгрузки, Неопределено);
					КонецЕсли;
	...
	Если ПКС.ЕстьОбработчикПередВыгрузкой Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКС, "ПередВыгрузкой"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПКС.ПередВыгрузкой, Неопределено);
				КонецЕсли;
		...
		Если ПКС.ЕстьОбработчикПриВыгрузке Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКС, "ПриВыгрузке"), Неопределено);
					КонецЕсли;
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПКС.ПриВыгрузке, Неопределено);
					КонецЕсли;
		...
		Если ПКС.ЕстьОбработчикПослеВыгрузки Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКС, "ПослеВыгрузки"),
							Неопределено);
					КонецЕсли;
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПКС.ПослеВыгрузки, Неопределено);
					КонецЕсли;
		...
		Если ПКС.ЕстьОбработчикПередВыгрузкой Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКС,
							"ПередВыгрузкой"), Неопределено);
					КонецЕсли;
		...
		Если ПКС.ЕстьОбработчикПриВыгрузке Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКС, "ПриВыгрузке"),
							Неопределено);
					КонецЕсли;
		...
		Если ПКС.ЕстьОбработчикПослеВыгрузки Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(ПКС, "ПослеВыгрузки"), Неопределено);
					КонецЕсли;
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
						"ПередВыгрузкойОбъекта"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПередВыгрузкойОбъекта, Неопределено);
				КонецЕсли;
	...
	Если Не ПустаяСтрока(Правило.ПередВыгрузкой) Тогда
		...
		Попытка
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПередВыгрузкой, Неопределено);
				КонецЕсли;
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
						"ПослеВыгрузкиОбъекта"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПослеВыгрузкиОбъекта, Неопределено);
				КонецЕсли;
	...
	Если Не ПустаяСтрока(Правило.ПослеВыгрузки) Тогда
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Правило, "ПослеВыгрузки"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Правило.ПослеВыгрузки, Неопределено);
				КонецЕсли;
					...
					Если Не ПустаяСтрока(ПоследняяСтрокаПравилаВыгрузки.ПослеОбработки) Тогда
						...
						Попытка
							...
							Если ФлагРежимОтладкиОбработчиков Тогда
								...
								Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
									МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
									МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(
										ПоследняяСтрокаПравилаВыгрузки, "ПослеОбработки"), Неопределено);
								КонецЕсли;
							...
							Иначе
								...
								Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
									МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
									МодульОбщегоНазначения.ПроверитьАлгоритм(
										ПоследняяСтрокаПравилаВыгрузки.ПослеОбработки, Неопределено);
								КонецЕсли;
						...
						Попытка
							...
							Если ФлагРежимОтладкиОбработчиков Тогда
								...
								Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
									МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
									МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(
										ТекущаяСтрокаПравилаВыгрузки, "ПередОбработкой"), Неопределено);
								КонецЕсли;
		...
		Если Не ПустаяСтрока(ПоследняяСтрокаПравилаВыгрузки.ПослеОбработки) Тогда
			...
			Попытка
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(ПоследняяСтрокаПравилаВыгрузки.ПослеОбработки,
							Неопределено);
					КонецЕсли;
...
Процедура ЗагрузитьЗначенияПараметровОбменаДанными()
	...
	Если СобытияПослеЗагрузкиПараметров.Свойство(Имя, АлгоритмПослеЗагрузкиПараметра)
		...
		Иначе
			...
			Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
				МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
				МодульОбщегоНазначения.ПроверитьАлгоритм(АлгоритмПослеЗагрузкиПараметра, Неопределено);
			КонецЕсли;
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ТекстСобытияПослеЗагрузкиПравилОбмена, Неопределено);
				КонецЕсли;
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Иначе
							...
							Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
								МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
								МодульОбщегоНазначения.ПроверитьАлгоритм(ТекстАлгоритма, Неопределено);
							КонецЕсли;
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
						"ПередЗагрузкойДанных"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПередЗагрузкойДанных, Неопределено);
				КонецЕсли;
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
						"ПослеЗагрузкиДанных"), Неопределено);
				КонецЕсли;
			...
			Иначе
				...
				Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
					МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
					МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПослеЗагрузкиДанных, Неопределено);
				КонецЕсли;
		...
		Если Не Отказ Тогда
			...
			Попытка
				...
				Если ФлагРежимОтладкиОбработчиков Тогда
					...
					Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
						...
						Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
							МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
							МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
								"ПередВыгрузкойДанных"), Неопределено);
						КонецЕсли;
				...
				Иначе
					...
					Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
						МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
						МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПередВыгрузкойДанных, Неопределено);
					КонецЕсли;
			...
			Если Не Отказ Тогда
				...
				Попытка
					...
					Если ФлагРежимОтладкиОбработчиков Тогда
						...
						Если Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
							...
							Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
								МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
								МодульОбщегоНазначения.ПроверитьАлгоритм(ПолучитьСтрокуВызоваОбработчика(Конвертация,
									"ПослеВыгрузкиДанных"), Неопределено);
							КонецЕсли;
					...
					Иначе
						...
						Если ВстроеннаяБСПСодержитПроверкуАлгоритма() Тогда
							МодульОбщегоНазначения = ОбщийМодуль("ОбщегоНазначения");
							МодульОбщегоНазначения.ПроверитьАлгоритм(Конвертация.ПослеВыгрузкиДанных, Неопределено);
						КонецЕсли;
...
Функция ВстроеннаяБСПСодержитПроверкуАлгоритма() Экспорт
	
	Если ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
		МодульСтандартныеПодсистемыСервер = ОбщийМодуль("СтандартныеПодсистемыСервер");
		ТекущаяВерсияБСП = МодульСтандартныеПодсистемыСервер.ВерсияБиблиотеки();
		
		МодульОбщегоНазначенияКлиентСервер = ОбщийМодуль("ОбщегоНазначенияКлиентСервер");
		Возврат МодульОбщегоНазначенияКлиентСервер.СравнитьВерсии(ТекущаяВерсияБСП, "3.1.12.212") > 0;
	Иначе
		Возврат Ложь;
	КонецЕсли;
