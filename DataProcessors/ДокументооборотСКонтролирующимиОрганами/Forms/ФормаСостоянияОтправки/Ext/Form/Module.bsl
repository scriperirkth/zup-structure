...
&НаКлиенте
Процедура НадписьПанелиПриемаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ПодтвердитьЧтоЭДООбязателен(Команда)
	...
	ТребованияФНСКлиент.ОткрытьФормуПодтвержденияВыбораОбязательностиЭДО(Истина, Ссылка, Организация, ОписаниеОповещения);
...
Процедура ПодтвердитьЧтоЭДОНеОбязателен(Команда)
	...
	ТребованияФНСКлиент.ОткрытьФормуПодтвержденияВыбораОбязательностиЭДО(Ложь, Ссылка, Организация, ОписаниеОповещения);
...
Процедура ВыгрузитьПакетыПоДокументооборотамСдачиОтчетностиВФНС(Ответ, Параметры) Экспорт
	...
	КонтекстЭДОКлиент.ПроверитьПрикрепленныеПодписиИВыгрузитьПакетыПоФНС(Отправка)
...
Процедура ПрорисоватьТаблицу(ТекущееСостояниеОтправки)
	...
	ЭтоКвитанцияОПрисвоенииСтатусаДОПП = Ложь;
	...
	Если КоличествоДокументовРеализацииПолномочийНО > 0 Тогда
		Для каждого ДокументРеализацииПолномочийНО Из ТекущееСостояниеОтправки.ДокументыРеализацииПолномочийНО Цикл
			Если ЗначениеЗаполнено(ДокументРеализацииПолномочийНО.СтатусДОПП) Тогда
				ЭтоКвитанцияОПрисвоенииСтатусаДОПП = Истина;
			КонецЕсли;
	...
	Элементы.ДокументыРеализацииПолномочийНО.Заголовок = ?(КоличествоДокументовРеализацииПолномочийНО = 1,
		?(ЭтоКвитанцияОПрисвоенииСтатусаДОПП,
			НСтр("ru = 'Квитанция о присвоении статуса ДОПП'"), НСтр("ru = 'Квитанция о присвоении РНПТ'")),
		СтрШаблон(
			?(ЭтоКвитанцияОПрисвоенииСтатусаДОПП,
				НСтр("ru = 'Квитанции о присвоении статуса ДОПП (%1)'"), НСтр("ru = 'Квитанции о присвоении РНПТ (%1)'")),
...
Процедура ПрорисоватьБлокЭтаповДокументовРеализацииПолномочий(ТекущееСостояниеОтправки)
	...
	ТребованияФНС.ИзменитьОформлениеПанелиПриема(ЭтотОбъект, Ссылка);
	
	Элементы.СведенияПоЭтапуТребования.Видимость = 
		НЕ Элементы.НадписьПанелиПриема.Видимость
		И НЕ Элементы.КомандаПодтвердитьПрием.Видимость
		И НЕ Элементы.ПодтвердитьЧтоЭДООбязателен.Видимость
		И НЕ Элементы.ПодтвердитьЧтоЭДОНеОбязателен.Видимость;
...
Процедура КакУзнать_Завершение(Результат, ВходящийКонтекст) Экспорт
	...
	ТребованияФНСКлиент.СохранитьРезультатВыбораЭДОДляСсылки(Ссылка, Результат);
	УправлениеЭУ();
