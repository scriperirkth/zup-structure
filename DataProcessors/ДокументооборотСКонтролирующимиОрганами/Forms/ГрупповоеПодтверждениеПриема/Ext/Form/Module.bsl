...
Процедура ОтборПриИзменении(Элемент = Неопределено)
	...
	Для каждого Организация Из Организации Цикл
		...
		Для каждого Строка Из Организация.ПолучитьЭлементы() Цикл
			...
			Строка.Видимость = 
				ПоказыватьИстекающие И Строка.Приоритет <= 2 И НЕ Строка.ЭтоГруппа
...
&НаСервере
Функция ОбязательностьЭДОпоОрганизации(Организация) Экспорт
	
	Отбор = Новый Структура();
	Отбор.Вставить("Организация", Организация);
	
	НайденныеСтроки = ЭДОпоОрганизациям.НайтиСтроки(Отбор);

	Если НайденныеСтроки.Количество() = 0 Тогда
		
		Возврат ТребованияФНСВызовСервера.ЭДООбязательноПоОрганизации(Организация);
		
	Иначе
		
		Возврат НайденныеСтроки[0].ЭДООбязателен;
...
Процедура ПодтвердитьПриемПоследовательно(ВходящийКонтекст) Экспорт
	...
	Результат  = ОбязательностьЭДОпоОрганизации(ТекущаяОрганизация);
	
	Если НЕ ТекущаяСтрока.ПолученоПозже5февраля2025 Тогда

		// Если требование/уведмление получено раньше 5.02.2025,
		// то сразу отправляем подтверждение без всяких вопросов
		ПодтвердитьПриемПоследовательно_ОтправитьСледующееПодтверждение(Неопределено, ВходящийКонтекст);
		
	ИначеЕсли ТекущаяСтрока.ОтветилиНаВопросПроЭДО Тогда
		
		// Если по требованию/уведомлению уже ответили про обязательность ЭДО,
		// то еще раз не спрашиваем
		ПодтвердитьПриемПоследовательно_ОтправитьСледующееПодтверждение(ТекущаяСтрока.ЭДООбязателен, ВходящийКонтекст);
		
	ИначеЕсли Результат = Неопределено Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ПодтвердитьПриемПоследовательно_ПослеВопросаПроЭДО", 
			ЭтотОбъект, 
			ВходящийКонтекст);
		
		ТребованияФНСКлиент.СпроситьПроОбязательностьЭДО(ТекущаяОрганизация, ОписаниеОповещения, Истина);
		
	Иначе
		
		ПодтвердитьПриемПоследовательно_ОтправитьСледующееПодтверждение(Результат, ВходящийКонтекст);
...
&НаКлиенте
Процедура ПодтвердитьПриемПоследовательно_ПослеВопросаПроЭДО(ЭДООбязателен, ВходящийКонтекст) Экспорт
	
	Если ЭДООбязателен = Неопределено Тогда
		ПриЗавершенииПодтверждения(ВходящийКонтекст);
		Возврат;
	КонецЕсли;
	...
	НоваяСтрока = ЭДОпоОрганизациям.Добавить();
	НоваяСтрока.Организация = Организация;
	НоваяСтрока.ЭДООбязателен = ЭДООбязателен;
	
	ПодтвердитьПриемПоследовательно_ОтправитьСледующееПодтверждение(ЭДООбязателен, ВходящийКонтекст)
	
КонецПроцедуры

&НаКлиенте
Процедура ПодтвердитьПриемПоследовательно_ОтправитьСледующееПодтверждение(ЭДООбязателен, ВходящийКонтекст) Экспорт

	ТекущийНомер  = ВходящийКонтекст.ТекущийНомер;
	ТекущаяСтрока = ВходящийКонтекст.ТекущаяСтрока;
	ТекущееТребование = ТекущаяСтрока.Требование;
	
	// Если ЭДО обязательно, то требование не подтверждаем
	Если ПропуститьПодтверждениеЕслиНеТребуется(ЭДООбязателен, ВходящийКонтекст) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция ПропуститьПодтверждениеЕслиНеТребуется(ЭДООбязателен, ВходящийКонтекст) Экспорт

	ТекущаяСтрока = ВходящийКонтекст.ТекущаяСтрока;
	ТекущееТребование = ТекущаяСтрока.Требование;
	
	Если ТекущаяСтрока.ПолученоПозже5февраля2025 Тогда
		
		Если НЕ ТекущаяСтрока.ОтветилиНаВопросПроЭДО И ЭДООбязателен <> Неопределено Тогда
			ТребованияФНСВызовСервера.СохранитьРезультатВыбораЭДОДляСсылки(ТекущееТребование, ЭДООбязателен);
		КонецЕсли;
		
		Если ЭДООбязателен Тогда
			
			ТекущаяСтрока.ПредставлениеСостояния = НСтр("ru = 'Подтверждение не требуется'");
			ТекущаяСтрока.Пометка = Ложь;
			
			Оповестить("УспешноеПодтверждение",,ТекущаяСтрока.Требование);
			Оповестить("Запись_ДокументыРеализацииПолномочийНалоговыхОрганов",,ТекущаяСтрока.Требование);
			
			ПослеПодтвержденияПриемаОдного(ВходящийКонтекст);
...
&НаКлиенте
Процедура ПослеПодтвержденияПриемаОдного(ВходящийКонтекст) Экспорт
...
&НаКлиенте
Процедура ПриЗавершенииПодтверждения(ВходящийКонтекст) Экспорт
	
	Для каждого Строка Из ВходящийКонтекст.Успешные Цикл
		Строка.Пометка = Ложь;
		ПроставитьПометкиВверх(Строка);
	КонецЦикла;
...
Процедура ЗаполнитьОбщиеСведения(НоваяСтрока, КонтекстЭДОСервер, Требование, РасчетСрока)
	...
	Иначе
		...
		НоваяСтрока.ПолученоПозже5февраля2025 = ТребованияФНСВызовСервера.ПолученоПозже5февраля2025(Требование);
		НоваяСтрока.ЭДООбязателен = ТребованияФНСВызовСервера.ЭДООбязателенПоСсылке(Требование);
		НоваяСтрока.ОтветилиНаВопросПроЭДО = ТребованияФНСВызовСервера.ОтветилиНаВопросПроЭДО(Требование);
	КонецЕсли;
