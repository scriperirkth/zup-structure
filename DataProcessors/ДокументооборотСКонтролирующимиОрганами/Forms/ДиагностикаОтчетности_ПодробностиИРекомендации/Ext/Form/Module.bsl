...
Функция ЗаполнитьРегистрациюНаСервере(
	...
	НесколькоПодписантов = Регистрация.Подписанты.Количество() > 0;
	
	Если НесколькоПодписантов Тогда
		НоваяСтрока = Регистрация.Подписанты.Добавить();
		Приемник = НоваяСтрока;
	Иначе
		Приемник = НоваяРегистрация;
	КонецЕсли;
	
	Приемник.Доверенность = Доверенность;
	Приемник.ДокументПредставителя = Строка(НоваяРегистрация.Доверенность);
	Приемник.Представитель = Представитель;
...
Процедура ДобавитьНовуюДоверенностьЗавершение(Результат, ВхПараметры) Экспорт
	...
	Представитель = МультирежимВызовСервера.ФизЛицоПоФИОиСНИЛС(
...
Процедура ПодробностиУчетнойЗаписи(Знач Проверка, МакетПодсказки, КонтекстМодуля)
	...
	ИначеЕсли Проверка = 1 Тогда 
		...
		НоваяЗапись.ИндексЗаписи = ТабличныеДанные.Количество() - 1;
...
Процедура ТабличныеДанныеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТабличныеДанныеРекомендуемоеДействие" Тогда
		Индекс = Параметры.Сводка.Фильтр[Элементы.ТабличныеДанные.ТекущиеДанные.ИндексЗаписи];
