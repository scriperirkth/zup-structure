...
Функция ИнформацияДляРасшифровкиСообщения(ТранспортноеСообщение, ОбщиеПараметры)
	...
	Попытка
		...
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ДанныеКонтейнера, "ДанныеКонтейнера", "ИнформацияДляРасшифровкиСообщения");		
		...
		Если ФайлАрхива <> Неопределено Тогда
			...
			РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ИмяФайлаОписания);
			//@skip-check 1c-fresh-audit
			...
			Иначе
				ЧтениеZip.Извлечь(ФайлАрхива, ВременныйКаталог, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
				РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ПервыйЗашифрованный);
				//@skip-check 1c-fresh-audit
...
Процедура ЗаполнитьРекомендации(ОписаниеОшибки, СводнаяИнформация)
	...
	Если СводнаяИнформация.СертификатовНезаблокированных = 0 И СводнаяИнформация.ОшибокОбработкиКонтейнера = 0 Тогда
		...
		Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'закройте'"),,,, "#action?close_window"));
	...
	ИначеЕсли СводнаяИнформация.СертификатовНезаблокированных = 0 И СводнаяИнформация.ОшибокОбработкиКонтейнера > 0 Тогда
		...
		Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Перейти к настройке'"),,,, "#action?configure_documents"));
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "Ошибка расшифровки файла документа при распаковке пакета") > 0 Тогда 
		...
		Иначе
			...
			Если ПараметрыПоказа <> Неопределено И ПараметрыПоказа.ПоказДляДругихПользователей Тогда
				...
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'других пользователей'"),,,,"#action?user_list"));
			...
			Если СводнаяИнформация.СертификатовНеизвестных > 0 Тогда // нераспознанный сертификат
				...
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'КриптоПро CSP'"),,,, "https://its.1c.ru/bmk/elreps/key_trans_cryptopro"));
			ИначеЕсли СводнаяИнформация.СертификатовНеизвестных = 0 
					И СводнаяИнформация.СертификатовКриптоПро > 0
					И СводнаяИнформация.СертификатовВипнет = 0 Тогда // однозначно КриптоПро
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Инструкция по переносу'"),,,, "https://its.1c.ru/bmk/elreps/key_trans_cryptopro"));
			ИначеЕсли СводнаяИнформация.СертификатовНеизвестных = 0 
					И СводнаяИнформация.СертификатовКриптоПро = 0
					И СводнаяИнформация.СертификатовВипнет > 0 Тогда // однозначно Випнет
				Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Инструкция по переносу'"),,,, "https://its.1c.ru/bmk/elreps/key_trans_vipnet"));
			КонецЕсли;
			Рекомендации.Добавить(".");
			Рекомендации.Добавить(Символы.ПС);
		...
		Если СводнаяИнформация.ЗашифрованоНаТекущем > 0 Тогда
			...
			Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'отправьте заявление'"),,,, "#action?show_master"));
		...
		Если СводнаяИнформация.СертификатовНезаблокированных > 0 Тогда
			...
			Рекомендации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Перейти к настройке'"),,,, "#action?configure_certificates"));
			Рекомендации.Добавить(".");
			Рекомендации.Добавить(Символы.ПС);
		КонецЕсли;
