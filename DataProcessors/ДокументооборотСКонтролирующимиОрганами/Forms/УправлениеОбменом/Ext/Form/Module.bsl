...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаФСС.Заголовок = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(Элементы.ГруппаФСС.Заголовок,
		Истина);
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ЗПЭД") Тогда
		Элементы.ТаблицаИсходящиеДокументыПФРКомандаИсходящиеДокументыПФРСоздатьЗаявлениеНаПодключениеКЭДОсПФР.Заголовок =
			НСтр("ru = 'Заявление на подключение к ЭДО СФР (не применяется с 04.04.2026)'");
		Элементы.ТаблицаИсходящиеДокументыПФРКомандаИсходящиеДокументыПФРСоздатьЗаявлениеНаОтключениеОтЭДОсПФР.Заголовок =
			НСтр("ru = 'Заявление на отключение от ЭДО СФР (не применяется с 04.04.2026)'");
	КонецЕсли;
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.УПУП") Тогда
		Элементы.ТаблицаИсходящиеДокументыПФРКомандаИсходящиеДокументыПФРСоздатьУведомлениеОПредоставленииПолномочийПредставителю.Заголовок =
			НСтр("ru = 'Уведомление о предоставлении полномочий представителю (не применяется с 04.04.2026)'");
		Элементы.ТаблицаИсходящиеДокументыПФРКомандаИсходящиеДокументыПФРСоздатьУведомлениеОПрекращенииПолномочийПредставителя.Заголовок =
			НСтр("ru = 'Уведомление о прекращении полномочий представителя (не применяется с 04.04.2026)'");
	КонецЕсли;
	...
	ИмяДокументаДокументацияПоКонтролируемымСделкам =
		ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСерверПереопределяемый.ИмяОбъектаМетаданных(
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИмяСобытияЗаписиДокументаДокументацияПоКонтролируемымСделкам =
		?(НЕ ЗначениеЗаполнено(ИмяДокументаДокументацияПоКонтролируемымСделкам),
		"", "Запись_" + ИмяДокументаДокументацияПоКонтролируемымСделкам);
	...
	ИначеЕсли ИмяСобытия = "Запись_ЭлектронныеПредставленияРегламентированныхОтчетов" 
		...
		ИЛИ ИмяСобытия = ИмяСобытияЗаписиДокументаДокументацияПоКонтролируемымСделкам
		...
		ИЛИ ИмяСобытия = РегламентированнаяОтчетностьКлиент.ИмяСобытияЗаписьУведомления() Тогда
	...
	ИначеЕсли ИмяСобытия = "Показать циклы обмена уведомления" 
		...
		Если ЗначениеЗаполнено(ИмяДокументаДокументацияПоКонтролируемымСделкам) Тогда
			Если Тип("ДокументСсылка." + ИмяДокументаДокументацияПоКонтролируемымСделкам) = ТипЗначенияПараметра Тогда
				СменитьЗакладкуУведомленийПриНеобходимости("ДокументацияПоКонтролируемымСделкам", Ложь);
			КонецЕсли;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СуществуетРегламентированныйОтчетДеятельностьИнОргВРФ()
	Если Метаданные.Отчеты.Найти("РегламентированныйОтчетДеятельностьИнОргВРФ") <> Неопределено Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если ЗначениеЗаполнено(ИмяДокументаДокументацияПоКонтролируемымСделкам) Тогда
		СписокВариантовВыбора.Добавить("ДокументацияПоКонтролируемымСделкам",
			НСтр("ru = 'Документация по контролируемым сделкам'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетДеятельностьИнОргВРФ() Тогда
		СписокВариантовВыбора.Добавить("ДеятельностьИнОргВРФ",
			НСтр("ru = 'Годовой отчет о деятельности иностранной организации в РФ'"));
	КонецЕсли;
...
Процедура КомандаУведомленияСоздатьЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		ИначеЕсли РезультатВыбора.Значение = "ДокументацияПоКонтролируемымСделкам" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("ДокументацияПоКонтролируемымСделкам");
			ОткрытьФорму("Документ." + ИмяДокументаДокументацияПоКонтролируемымСделкам + ".ФормаОбъекта",
		...
		ИначеЕсли РезультатВыбора.Значение = "ДеятельностьИнОргВРФ" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("ДеятельностьИнОргВРФ");
			
			ИсточникОтчета = "РегламентированныйОтчетДеятельностьИнОргВРФ";
...
&НаКлиенте
Процедура КомандаОткрытьДокументацияПоКонтролируемымСделкам(Команда)
	УправлениеЗакладкойУведомлений("ДокументацияПоКонтролируемымСделкам");
КонецПроцедуры
...
&НаКлиенте
Процедура КомандаОткрытьДеятельностьИнОргВРФ(Команда)
	УправлениеЗакладкойУведомлений("ДеятельностьИнОргВРФ");
КонецПроцедуры
...
Процедура УправлениеЗакладкойУведомлений(Вид = Неопределено)
	...
	Элементы.ТаблицаУведомленияКомандаОткрытьДокументацияПоКонтролируемымСделкам.Видимость = Истина; // документация по контролируемым сделкам
	...
	Элементы.ТаблицаУведомленияКомандаОткрытьДеятельностьИнОргВРФ.Видимость = Истина; // годовой отчет о деятельности иностранной организации в РФ
	...
	// документ Документация по контролируемым сделкам
	Если НЕ ЗначениеЗаполнено(ИмяДокументаДокументацияПоКонтролируемымСделкам) Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьДокументацияПоКонтролируемымСделкам.Видимость = Ложь;
	КонецЕсли;
	...
	Если НЕ СуществуетРегламентированныйОтчетДеятельностьИнОргВРФ() Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьДеятельностьИнОргВРФ.Видимость = Ложь;
	КонецЕсли;
	...
	ИначеЕсли Вид = "ДокументацияПоКонтролируемымСделкам" Тогда
		// документ Документация по контролируемым сделкам
		Элементы.ТаблицаУведомленияКомандаОткрытьДокументацияПоКонтролируемымСделкам.Пометка = Истина; // нажатость подменю
		Элементы.ТаблицаУведомленияНалоговыйОрган.Видимость = Ложь;
	...
	ИначеЕсли Вид = "ДеятельностьИнОргВРФ" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетДеятельностьИнОргВРФ,
		// нажатость подменю
		Элементы.ТаблицаУведомленияКомандаОткрытьДеятельностьИнОргВРФ.Пометка = Истина;
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках)
		И НЕ ЗначениеЗаполнено(ИмяДокументаДокументацияПоКонтролируемымСделкам)
		И НЕ СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак()
		...
		И НЕ СуществуетРегламентированныйОтчетДеятельностьИнОргВРФ()
	...
	// запрос для электронных представлений с типом УведомлениеОКонтролируемыхСделках,
	// ДокументацияПоКонтролируемымСделкам, ИсходящееУведомлениеФНС, УведомлениеОРозничныхЦенахНаТабак, СтрановойОтчет,
	// УведомлениеОбУчастииВМеждународнойГруппеКомпаний, ДоходыОтДеятельностиМузеяТеатраБиблиотеки,
	// ДвижениеСредствПоСчетуВБанкеЗаПределамиРФ, ДвижениеСредствНерезидентаЗаПределамиРФ, ФинОтчетностьМГК,
	// ДеятельностьИнОргВРФ, ПрослеживаемыеТоварыОтчетОбОперациях, ПрослеживаемыеТоварыВвоз,
	// ПрослеживаемыеТоварыПеремещение и ПрослеживаемыеТоварыОстатки будет в любом случае
	...
	// Если в системе существует документ ДокументацияПоКонтролируемымСделкам
	// тогда добавляем в запрос для динамического списка запрос по документу Документация по контролируемым сделкам.
	Если ЗначениеЗаполнено(ИмяДокументаДокументацияПоКонтролируемымСделкам)
		И Вид = "ДокументацияПоКонтролируемымСделкам" Тогда
			
			ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ
										  |	ВЫБОР
										  |		КОГДА ДокументацияПоКонтролируемымСделкам.ПометкаУдаления
										  |			ТОГДА ВЫБОР
										  |					КОГДА СтатусОтправки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправки.Отправлен)
										  |						ТОГДА 10
										  |					ИНАЧЕ ВЫБОР
										  ...
										  |	ВЫРАЗИТЬ(ДокументацияПоКонтролируемымСделкам.Организация КАК Справочник.Организации) КАК Организация,
										  |	ГОД(ДокументацияПоКонтролируемымСделкам.ОтчетныйГод) КАК ОтчетныйГод,
										  |	ДокументацияПоКонтролируемымСделкам.Ссылка,
										  |	"""" КАК НалоговыйОрган,
										  |	ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.ДокументацияПоКонтролируемымСделкам) КАК ВидУведомления
										  |ИЗ
										  ...
										  |		ИЗ
										  |			РегистрСведений.СтатусыОтправки КАК СтатусыОтправки) КАК СтатусОтправки
										  |		ПО (СтатусОтправки.Объект = ДокументацияПоКонтролируемымСделкам.Ссылка)
										  |ГДЕ
										  |	ДокументацияПоКонтролируемымСделкам.Организация В(&Организация)";
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДокументацияПоКонтролируемымСделкам", ИмяДокументаДокументацияПоКонтролируемымСделкам);
			
			Уведомления.ОсновнаяТаблица = "Документ.ДокументацияПоКонтролируемымСделкам";
	...
	// Добавляем в запрос для динамического списка запрос по документу РегламентированныйОтчет
	// с источником РегламентированныйОтчетДеятельностьИнОргВРФ.
	Если СуществуетРегламентированныйОтчетДеятельностьИнОргВРФ()
		И Вид = "ДеятельностьИнОргВРФ" Тогда
	...
	Если Вид = "Файлы" Тогда
		...
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.ДеятельностьИнОргВРФ);
		...
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.ДокументацияПоКонтролируемымСделкам);
...
Функция ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления(ИсточникОтчета)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	                ...
	                |			ИЛИ РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетДеятельностьИнОргВРФ""
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетДеятельностьИнОргВРФ""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.ДеятельностьИнОргВРФ)
...
Процедура СменитьЗакладкуУведомленийПриНеобходимости(ИмяСобытия, ВыводитьПредупреждение = Истина)
	...
	Если ИмяСобытия = "Документы" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьДокументы.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "ДокументацияПоКонтролируемымСделкам"
		И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьДокументацияПоКонтролируемымСделкам.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	...
	ИначеЕсли ИмяСобытия = "ФинОтчетностьМГК"
		...
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "ДеятельностьИнОргВРФ"
		И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьДеятельностьИнОргВРФ.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
