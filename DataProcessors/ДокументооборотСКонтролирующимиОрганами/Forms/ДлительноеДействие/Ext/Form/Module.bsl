...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Иначе
		...
		Отказ = Истина;
		ПрерватьОбменПоРешениюПользователя();
...
Процедура ПриОткрытииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДОКлиент.ОбменПрерванПользователем = Ложь;
...
Функция ЕстьТолькоНезавершенныеОтправки() Экспорт
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		Если СтрокаДерева.ИсходнаяГруппа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения Тогда
			
			Если СтрокаДерева.НеПрочитано И ЗначениеЗаполнено(СтрокаДерева.Ссылка) Тогда
				ЕстьПолученныеСообщения = Истина;
			КонецЕсли;
				
		ИначеЕсли СтрокаДерева.ИсходнаяГруппа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ОбработанныеЗапросы Тогда 
			
			Если СтрокаДерева.НеПрочитано И ЗначениеЗаполнено(СтрокаДерева.Ссылка) Тогда
				 ЕстьОбработанныеЗапросы = Истина;
			КонецЕсли;
			
		ИначеЕсли СтрокаДерева.ИсходнаяГруппа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ЗавершенныеОтправки Тогда 
			
			Если СтрокаДерева.НеПрочитано И ЗначениеЗаполнено(СтрокаДерева.Ссылка) Тогда
				ЕстьЗавершенныеОтправки = Истина;
			КонецЕсли;
		
		ИначеЕсли СтрокаДерева.ИсходнаяГруппа = Перечисления.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.НезавершенныеОтправки Тогда 
...
&НаКлиенте
Асинх Процедура ПрерватьОбменПоРешениюПользователя()
	
	Если ВопросПоказан Тогда
		Возврат;
	КонецЕсли;
	
	ВопросПоказан = Истина;
	РезультатВопроса = Ждать ВопросАсинх("Прервать получение сообщений от контролирующих органов?", РежимДиалогаВопрос.ДаНет);
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		КонтекстЭДОКлиент.ОбменПрерванПользователем = Истина;
