...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДеревоВсехИспользуемыхОтчетовПоКатегориям.Колонки.Добавить("Найден", Новый ОписаниеТипов("Булево"));
...
Функция ДеревоВсехИспользуемыхОтчетов()
	...
	ДеревоВсехИспользуемыхОтчетов.Колонки.Добавить("Неактивный", Новый ОписаниеТипов("Булево"));
	ОтборДерева = Новый Структура("ЭтоГруппа", Ложь);
	МассивВсехИспользуемыхОтчетов = ДеревоВсехИспользуемыхОтчетов.Строки.НайтиСтроки(ОтборДерева, Истина);
	Для каждого СтрокаВсехИспользуемыхОтчетов Из МассивВсехИспользуемыхОтчетов Цикл
		СтрокаВсехИспользуемыхОтчетов.Неактивный = НеактивныеВидыДокументов.ВидыОтправляемыхДокументов.Найти(
				СтрокаВсехИспользуемыхОтчетов.ВидОО) <> Неопределено
			ИЛИ НеактивныеВидыДокументов.ВидыДокументовДляПоиска.Найти(СтрокаВсехИспользуемыхОтчетов.ПолноеИмя) <> Неопределено
			ИЛИ НеактивныеВидыДокументов.ВидыДокументовДляПоиска.Найти(СтрокаВсехИспользуемыхОтчетов.ИсточникОтчета) <>
				Неопределено;
	КонецЦикла;
...
Функция ДеревоИзСправочникаРегламентированныеОтчеты()
	...
	|	ВидыОтправляемыхДокументов.Ссылка КАК ВидОО
	|ИЗ
...
Функция ЗаполнитьДеревоОтчетовБезГруппировки(ДеревоВсехИспользуемыхОтчетов)
	...
	ДеревоЗначенийДляСортировки.Строки.Сортировать("Неактивный, Наименование, Получатель", Истина);
...
&НаКлиенте
Процедура КопироватьДанныеФормыУдовлетворяющиеСтрокеПоиска(
		ЭлементДереваИсточник,
	...
	Если ДанныеПолнотекстовогоПоиска = Неопределено Тогда
		...
		ТаблицаОтчетовДляПоиска.Очистить();
		Найден = Ложь;
		КопироватьДеревоВТаблицуОтчетовДляПоиска(ЭлементДереваИсточник, ТаблицаОтчетовДляПоиска, Найден);
		...
		Для каждого ВидОтправляемыхДокументов Из ВидыДокументов.ВидыОтправляемыхДокументов Цикл
			...
			СтрокиТаблицы = ТаблицаОтчетовДляПоиска.НайтиСтроки(ОтборТаблицы);
		...
		Для каждого ВидДокументовДляПоиска Из ВидыДокументов.ВидыДокументовДляПоиска Цикл
			ОтборТаблицы = Новый Структура("ПолноеИмя", ВидДокументовДляПоиска);
			СтрокиТаблицы = ТаблицаОтчетовДляПоиска.НайтиСтроки(ОтборТаблицы);
			МассивыСтрокТаблицы.Добавить(СтрокиТаблицы);
		КонецЦикла;
		
		Для каждого ВидДокументовДляПоиска Из ВидыДокументов.ВидыДокументовДляПоиска Цикл
			ОтборТаблицы = Новый Структура("ИсточникОтчета", ВидДокументовДляПоиска);
			СтрокиТаблицы = ТаблицаОтчетовДляПоиска.НайтиСтроки(ОтборТаблицы);
			МассивыСтрокТаблицы.Добавить(СтрокиТаблицы);
		КонецЦикла;
		...
		Для каждого СтрокиТаблицы Из МассивыСтрокТаблицы Цикл
			Для каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
				СтрокаИсточника = ЭлементДереваИсточник.НайтиПоИдентификатору(СтрокаТаблицы.ИдентификаторЭлементаДерева);
				Если СтрокаИсточника <> Неопределено Тогда
					СтрокаИсточника.Найден = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	...
	Для Каждого СтрокаИсточника Из СтрокиИсточника Цикл
		...
		Если НЕ ЭтоГруппа Тогда
			...
			СтрокаИсточникаНаименованиеВРег = ВРег(СтрокаИсточника.Наименование);
			Если СтрНайти(СтрокаИсточникаНаименованиеВРег, СтрокаПоискаВРег) = 0
				И (ДанныеПолнотекстовогоПоиска.СтрокаПоискаВРегИсправленнаяРаскладка = Неопределено
					ИЛИ СтрНайти(СтрокаИсточникаНаименованиеВРег,
						ДанныеПолнотекстовогоПоиска.СтрокаПоискаВРегИсправленнаяРаскладка) = 0)
				И НЕ СтрокаИсточника.Найден Тогда
		...
		КопироватьДанныеФормыУдовлетворяющиеСтрокеПоиска(СтрокаИсточника, НоваяСтрокаПриемника, ДанныеПолнотекстовогоПоиска);
...
&НаКлиенте
Процедура КопироватьДеревоВТаблицуОтчетовДляПоиска(ЭлементДереваИсточник, ТаблицаПриемник, Найден)
	...
	Для каждого СтрокаИсточника Из СтрокиИсточника Цикл
		...
		Если НЕ ЭтоГруппа Тогда
			...
			СтрокаИсточника.Найден = Найден;
		КонецЕсли;
		
		КопироватьДеревоВТаблицуОтчетовДляПоиска(СтрокаИсточника, ТаблицаПриемник, Найден);
	КонецЦикла;
