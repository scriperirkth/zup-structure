...
Функция ДанныеДляПечатиГрафикаРаботыСотрудникаЗаМесяц()
	...
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВременныхТаблиц, Ложь, "ФИОПолные, ДатаУвольнения, ВидЗанятости");
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Перечисление.ВидыЗанятости КАК ВидыЗанятости
	|		ПО КадровыеДанныеСотрудников.ВидЗанятости = ВидыЗанятости.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ПлановоеВремя.Сотрудник,
	|	КадровыеДанныеСотрудников.ФИОПолные,
	|	ВидыЗанятости.Порядок,
	|	ПлановоеВремя.ДатаАктуальности,
	|	ПлановоеВремя.Дата
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура СформироватьСправкуПоОтпускамСотрудника()
	...
	Запрос.УстановитьПараметр("ФизическоеЛицо", ФизическоеЛицо);
	Запрос.УстановитьПараметр("ДатаОтчета", ДатаОстаткаОтпуска);
					   
	Запрос.Выполнить();		
					   			   
	ОписательВременныхТаблиц = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеСотрудников(Запрос.МенеджерВременныхТаблиц, "ВТСотрудники");
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВременныхТаблиц, Ложь, "ДатаУвольнения, ВидЗанятости");
