...
Процедура УстановитьЗаголовокДокумента(Форма) Экспорт
	...
	ПредставлениеТипа = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(Форма.Тип);
...
Процедура ЗаполнитьПараметрыХронометража(Форма, ОбъектXDTO) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("1.3.2.3.CORP") Тогда
		...
		УстановитьСвойстваЭлементовХронометража(
			Форма.ВключенХронометраж,
			Форма.Команды.ПереключитьХронометраж,
			Форма.Элементы.ФормаПереключитьХронометраж);
	КонецЕсли;
...
Процедура ЗаполнитьСтандартнуюШапкуБизнесПроцесса(Форма, ОбъектXDTO) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ОбъектXDTO, "state")
			И ОбъектXDTO.state.objectID.ID = "Прерван" Тогда //@NON-NLS-2
		Форма.Элементы.ФормаЗаписать.Доступность = Ложь;
		Если Форма.Элементы.Найти("Предметы") <> Неопределено Тогда
			Форма.Элементы.Предметы.Доступность = Ложь;
		КонецЕсли;
...
//   ОбъектXDTO - ОбъектXDTO - Объект XDTO типа, наследующего типу DMBusinessProcess, с данными процесса.
//
Процедура ПередатьСрокОбработкиРезультата(Прокси, Форма, ОбъектXDTO) Экспорт
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектXDTOИзОбъектногоРеквизита(
		...
		ОбъектXDTO.resultProcessingDueDateSpecificationOption,
		"DMDueDateSpecificationOption");
	ОбъектXDTO.resultProcessingDueDate = Форма.СрокОбработкиРезультатов;
	ОбъектXDTO.resultProcessingDueDateDays = Форма.СрокОбработкиРезультатовДни;
	ОбъектXDTO.resultProcessingDueDateHours = Форма.СрокОбработкиРезультатовЧасы;
	ОбъектXDTO.resultProcessingDueDateMinutes = Форма.СрокОбработкиРезультатовМинуты;
...
// Устанавливает срок обработки результатов в форме бизнес-процесса.
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма обработки, соответствующая бизнес-процессу.
//   ОбъектXDTO - ОбъектXDTO - Объект XDTO типа, наследующего типу DMBusinessProcess, с данными процесса.
//
Процедура УстановитьСрокОбработкиРезультатов(Форма, ОбъектXDTO) Экспорт
	
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(
			ОбъектXDTO, "resultProcessingDueDateSpecificationOption") Тогда
		
		ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
			Форма,
			ОбъектXDTO.resultProcessingDueDateSpecificationOption,
			"ВариантУстановкиСрокаОбработкиРезультатов");
		Форма.СрокОбработкиРезультатов = ОбъектXDTO.resultProcessingDueDate;
		Форма.СрокОбработкиРезультатовДни = ОбъектXDTO.resultProcessingDueDateDays;
		Форма.СрокОбработкиРезультатовЧасы = ОбъектXDTO.resultProcessingDueDateHours;
		Форма.СрокОбработкиРезультатовМинуты = ОбъектXDTO.resultProcessingDueDateMinutes;
