...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(ТипСоздаваемогоОбъекта) Тогда
		...
		ПредставлениеТипаОбъектаДО = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(
			ТипСоздаваемогоОбъекта);
...
Функция ОбработатьФормуСогласноВерсииСервиса()
	...
	Попытка
		...
		Иначе
			...
			ДоступенФункционалПочта = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.2.8.1.CORP");
			ДоступенФункционалКонтрагенты = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.3.2.3");
			ДоступенФункционалВложенныеПапки = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.2.7.3");
			ДоступенФункционалЗадачи = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.2.6.2");
			ДоступенФункционалВизы = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("2.0.5.4");
			ДоступенФункционалРезолюции = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("2.0.5.4");
			ДоступенФункционалХронометраж = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.3.2.3.CORP");
...
Процедура ПолучитьДанныеДокумента()
	...
	Иначе
		...
		Если ОбъектXDTO.Свойства().Получить("resolutions") <> Неопределено
			...
			ИнтеграцияС1СДокументооборот.ОбновитьПредставлениеРезолюций(
				ОбъектXDTO.resolutions,
				РезолюцииHTMLПредставление,
				Элементы.ГруппаРезолюции);
	...
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(ТипСоздаваемогоОбъекта) Тогда
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = СтрШаблон("%1:",
				ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(ТипСоздаваемогоОбъекта));
...
Процедура ОбновитьСписокФайлов()
	...
	Файлы = ИнтеграцияС1СДокументооборот.ФайлыПоОбъектуПотребителю(
	...
	Для Каждого СведенияОФайле Из Файлы.files Цикл
		...
		Для Каждого ПодписьXDTO Из СведенияОФайле.signatures Цикл
			...
			Подпись.УстановившийПодписьID = ПодписьXDTO.signer.objectID.ID;
		КонецЦикла;
...
Процедура ВыбратьСуществующийФайлВ1СДокументооборотЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		СкопироватьСуществующийФайлВПапку(Результат.РеквизитID);
		ОбновитьСписокФайловКлиент(Результат.РеквизитID);
	КонецЕсли;
...
//
&НаКлиенте
Асинх Процедура СоздатьФайлСДискаПослеПомещенияФайла(Знач ПомещенныйФайл, Знач ПараметрыОповещения) Экспорт
	...
	ПараметрыОповещения.Вставить("АдресВременногоХранилищаФайла", ПомещенныйФайл.Хранение);
	...
	Если НастройкиДокументооборот.НужноИзвлечьТекст Тогда
		ИзвлеченныйТекст = Ждать ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ИзвлечьТекстАсинх(
			ПараметрыОповещения.ПолноеИмяФайла);
...
//
&НаКлиенте
Процедура СоздатьФайлСДискаПослеПомещенияФайлаБезРасширения(ПомещенныйФайл, ПараметрыОповещения) Экспорт
	...
	ПараметрыОповещения.Вставить("АдресВременногоХранилищаФайла", ПомещенныйФайл.Хранение);
	ПараметрыОповещения.Вставить("ПолноеИмяФайла", ПомещенныйФайл.Имя);
