...
Процедура УстановитьЗаголовокПредметаПоТипуОбъекта()
	...
	Элементы.Предмет.Заголовок = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(ПредметТип);
	Если Элементы.Предмет.Заголовок = ПредметТип Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Предмет'");
	КонецЕсли;
...
Процедура ЗаполнитьДеревоШаблонов(ПолучатьВидДокумента = Ложь)
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(РезультатДеревоШаблонов, "formHeader") Тогда
		Заголовок = РезультатДеревоШаблонов.formHeader;
	КонецЕсли;
