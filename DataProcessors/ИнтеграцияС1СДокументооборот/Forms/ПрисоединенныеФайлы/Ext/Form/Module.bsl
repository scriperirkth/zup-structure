...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ДокументооборотФайл"
			...
			И Параметр.ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов Тогда
...
Функция ОбработатьФормуСогласноВерсииСервиса()
	...
	Попытка
		...
		Иначе
			...
			ДоступенЗахватФайлов = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса(
				"1.4.9.1");
			
			ДоступныПомеченныеНаУдаление = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса(
				"2.0.6.3");
...
Процедура УстановитьДоступностьКомандСпискаФайлов()
	...
	Иначе
		...
		Элементы.СохранитьВместеСЭП.Доступность = ТекущиеДанные.ПодписанЭП;
		Элементы.СохранитьВместеСЭПКонтекст.Доступность = ТекущиеДанные.ПодписанЭП;
