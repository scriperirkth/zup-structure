...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Документооборот_ВыбратьЗначениеИзСпискаВТаблицеЗавершение"
			И Источник = ЭтотОбъект
			И Параметр.Реквизит = "Проект" Тогда
...
Процедура ОкончаниеДняПриИзменении(Элемент)
	...
	Если (СпособУказанияВремениID = ЗначениеПеречисленияВремяНачала)
			И (Работы.Количество() > 0)
			И ЗначениеЗаполнено(ОкончаниеДня)
			И (ОкончаниеДня < Работы[Работы.Количество() - 1].Начало) Тогда
...
Процедура РаботыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не НоваяСтрока Или ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура РаботыВидРаботОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Элементы.Работы.ТекущиеДанные.ПроектЗадача = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеПроектаЗадачи(
			Элементы.Работы.ТекущиеДанные.Проект,
			Элементы.Работы.ТекущиеДанные.ПроектнаяЗадача);
...
&НаКлиенте
Процедура РаботыПроектЗадачаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
		СтандартнаяОбработка)
...
Процедура РаботыПроектЗадачаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Строка = Элементы.Работы.ТекущиеДанные;
	Строка.ПроектЗадача = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеПроектаЗадачи(
		Строка.Проект,
		Строка.ПроектнаяЗадача);
...
Процедура ЗаполнитьСписокТиповИсточника(СписокТипов)
	...
	ТипыИсточника.Вставить("DMInternalDocument");
	ТипыИсточника.Вставить("DMIncomingEMail");
	ТипыИсточника.Вставить("DMIncomingDocument");
	ТипыИсточника.Вставить("DMDailyReport");
	ТипыИсточника.Вставить("DMBusinessProcessTask");
	ТипыИсточника.Вставить("DMOutgoingEMail");
	ТипыИсточника.Вставить("DMOutgoingDocument");
	ТипыИсточника.Вставить("DMActivity");
	ТипыИсточника.Вставить("DMProject");
	ТипыИсточника.Вставить("DMProjectTask");
	ТипыИсточника.Вставить("DMFile");
	
	Для Каждого Элемент Из ТипыИсточника Цикл
		Элемент.Значение = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(Элемент.Ключ);
	КонецЦикла;
	
	Для Каждого ТипИсточника Из ТипыИсточника Цикл
		СписокТипов.Добавить(
			Новый Структура("xdtoClassName, presentation", ТипИсточника.Ключ, ТипИсточника.Значение),
			ТипИсточника.Значение);
	КонецЦикла;
...
Процедура ЗаполнитьФормуОбъекта(ОбъектXDTO)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
		ЭтотОбъект,
		ОбъектXDTO.mainProject,
		"ОсновнойПроект",
		Ложь);
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьОбъектныйРеквизит(
		ЭтотОбъект,
		ОбъектXDTO.mainWorkType,
		"ОсновнойВидРабот",
		Ложь);
...
Процедура СформироватьЗаголовокФормы()
	...
	Иначе
		Заголовок = СтрШаблон(НСтр("ru = 'Ежедневный отчет за %1'"), Формат(Дата, "ДЛФ=D"));
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ)
	...
	Для Каждого Строка Из Работы Цикл
		...
		Строка.Начало = НачалоМинуты(Строка.Начало);
		Строка.Окончание = НачалоМинуты(Строка.Окончание);
		...
		Если Не ЗначениеЗаполнено(Строка.Работа) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				ТекстСообщения,,
				"Работы[" + Формат(ИндексСтроки, "ЧН=0; ЧГ=0") + "].Работа",,
				Отказ);
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Строка.ВидРабот) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				ТекстСообщения,,
				"Работы[" + Формат(ИндексСтроки, "ЧН=0; ЧГ=0") + "].ВидРабот",,
				Отказ);
		КонецЕсли;
		...
		Если СпособУказанияВремениID = ЗначениеПеречисленияДлительность Тогда
			...
			Если Не ЗначениеЗаполнено(Длительность) Тогда
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					ТекстСообщения,,
					"Работы[" + Формат(ИндексСтроки, "ЧН=0; ЧГ=0") + "].ДлительностьСтр",,
					Отказ);
			КонецЕсли;
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Строка.Начало) Тогда
				...
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					ТекстСообщения,,
					"Работы[" + Формат(ИндексСтроки, "ЧН=0; ЧГ=0") + "].Начало",,
					Отказ);
			КонецЕсли;
			...
			Если ДоступенФункционалХронометраж Тогда
				Если Не ЗначениеЗаполнено(Строка.Окончание) Тогда
					...
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						ТекстСообщения,,
						"Работы[" + Формат(ИндексСтроки, "ЧН=0; ЧГ=0") + "].Окончание",,
						Отказ);
				КонецЕсли;
	...
	Если ДоступенФункционалХронометраж И СпособУказанияВремениID = ЗначениеПеречисленияВремяНачала Тогда
		...
		Иначе
			...
			Для Каждого Строка Из ЭтотОбъект[СтрокаСоответствия.Ключ] Цикл
				СтрокаXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
					Прокси,
					СтрокаСоответствия.Значение.Тип);
...
Процедура ПересчитатьНачалоДня()
	...
	Если Не ДоступенФункционалХронометраж И (СпособУказанияВремениID = ЗначениеПеречисленияВремяНачала) Тогда
