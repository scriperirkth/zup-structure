...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВариантСогласованияПредставление.СписокВыбора.Добавить(
		"Параллельно",
		НСтр("ru='Всем сразу'")); //@NON-NLS-1
	Элементы.ВариантСогласованияПредставление.СписокВыбора.Добавить(
		"Последовательно",
		НСтр("ru='По очереди'")); //@NON-NLS-1
	Элементы.ВариантСогласованияПредставление.СписокВыбора.Добавить(
		"Смешанно",
		НСтр("ru='Смешанно'")); //@NON-NLS-1
...
Процедура ЗаполнитьКарточкуПоШаблону(ДанныеШаблона)
	...
	РезультатЗаполнения = ИнтеграцияС1СДокументооборот.ЗаполнитьБизнесПроцессПоШаблону(ЭтотОбъект, ДанныеШаблона);
...
Процедура ЗаполнитьФормуИзОбъектаXDTO(ОбъектXDTO)
	...
	Обработки.ИнтеграцияС1СДокументооборот.УстановитьСрокОбработкиРезультатов(ЭтотОбъект, ОбъектXDTO);
...
Функция ПодготовитьБизнесПроцесс(Прокси)
	...
	ОбъектXDTO = Обработки.ИнтеграцияС1СДокументооборот.ПодготовитьШапкуБизнесПроцесса(
		...
		ЭтотОбъект,
		"Срок");
	Обработки.ИнтеграцияС1СДокументооборот.ПередатьСрокОбработкиРезультата(Прокси, ЭтотОбъект, ОбъектXDTO);
