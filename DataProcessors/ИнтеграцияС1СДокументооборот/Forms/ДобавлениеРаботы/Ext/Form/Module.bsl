...
&НаКлиенте
Процедура ВидРаботОчистка(Элемент, СтандартнаяОбработка)
...
Процедура Добавить(Команда)
	...
	Если Не ЗначениеЗаполнено(ВидРабот) Или Не ЗначениеЗаполнено(ВидРаботID) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru = 'Вид работ'")),,
			"ВидРабот",,
			Отказ);
	КонецЕсли;
