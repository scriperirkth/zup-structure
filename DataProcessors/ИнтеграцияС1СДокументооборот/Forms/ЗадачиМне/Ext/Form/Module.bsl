...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	Если ПроцессТип = "DMBusinessProcessRegistration" И ТочкаМаршрута = "Зарегистрировать" Тогда //@NON-NLS-2
		Для Каждого СтрокаПредмета Из СтрокаЗадачи.Предметы Цикл
			Если (Не ДоступнаМультипредметность Или СтрокаПредмета.РольПредмета = "Основной")
				И ИнтеграцияС1СДокументооборотКлиентСервер.ЭтоДокумент(СтрокаПредмета.Тип) Тогда //@NON-NLS-1
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" И ТочкаМаршрута = "РассмотрениеИнициатором" //@NON-NLS-2
			И ВидВопросаID = "Иное" Тогда //@NON-NLS-2 @NON-NLS-3
		ТребоватьРезультатВыполнения = Истина;
	КонецЕсли;
	...
	Если ПроцессТип = "DMBusinessProcessInvitation" И ТочкаМаршрута = "Ознакомиться"
			И (РезультатID = "НеПринятоОбязательнымиУчастниками"
				Или РезультатID = "НеПринятоВсемиУчастниками") Тогда //@NON-NLS-2 @NON-NLS-3 @NON-NLS-4
...
Процедура ВыполнитьЗадачуВторая(Команда)
	...
	Если (ПроцессТип = "DMBusinessProcessApproval" И ТочкаМаршрута = "Согласовать")
			Или (ПроцессТип = "DMBusinessProcessInvitation" И ТочкаМаршрута = "Пригласить") Тогда //@NON-NLS-2 @NON-NLS-4
		ТребоватьРезультатВыполнения = Истина;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" И ТочкаМаршрута = "РассмотрениеИнициатором"
			И ВидВопросаID = "ПереносСрока" Тогда //@NON-NLS-2 @NON-NLS-3
		ТребоватьРезультатВыполнения = Истина;
	КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessInvitation" И ТочкаМаршрута = "Ознакомиться"
			И РезультатID = "ПринятоОбязательнымиУчастниками" Тогда //@NON-NLS-2 @NON-NLS-3
...
Процедура ЗаполнитьСтрокуЗадачиXDTO(СтрокаЗадачи, ЗадачаXDTO)
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(ЗадачаXDTO.parentBusinessProcess, "state") Тогда
		СтрокаЗадачи.Состояние = ?(ЗадачаXDTO.parentBusinessProcess.state = Неопределено,
			"Активен",
			ЗадачаXDTO.parentBusinessProcess.state.name); //@NON-NLS-1
	ИначеЕсли СтрокаЗадачи.ТочкаМаршрутаКратко = "Ознакомиться"
			И (СтрокаЗадачи.ПроцессТип = "DMBusinessProcessApproval"
				...
				Или СтрокаЗадачи.ПроцессТип = "DMBusinessProcessInvitation") Тогда //@NON-NLS-1
		Прокси = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ПолучитьПрокси();
...
Функция ПолучитьДетальныеСведенияНаСервере(РеквизитыЗадачи)
	...
	СтруктураКнопок = ИнтеграцияС1СДокументооборот.СтруктураИсполненияЗадачи(РеквизитыЗадачи);
