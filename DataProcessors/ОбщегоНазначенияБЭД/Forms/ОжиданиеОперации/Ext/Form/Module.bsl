...
// Параметры:
//  ИмяСобытия - Строка
//  ПараметрыОповещения - См. ОжиданиеОперацийБЭДСлужебныйКлиентСервер.НовыеПараметрыОповещенияСобытияЗакрытияФормы
//                      - См. ОжиданиеОперацийБЭДКлиент.НовыйКонтекст
//  Источник - Произвольный
&НаКлиенте
Процедура ОбработкаОповещения(ИмяСобытия, ПараметрыОповещения, Источник)
	...
	Если ИмяСобытия = ОжиданиеОперацийБЭДСлужебныйКлиентСервер.ИмяСобытияЗакрытияФормы()
		...
		Если ПараметрыОповещения.ИдентификаторОперации = Параметры.ИдентификаторОперации Тогда
			ЗакрытьФорму(Ложь);
		КонецЕсли;
	ИначеЕсли ИмяСобытия = ОжиданиеОперацийБЭДСлужебныйКлиентСервер.ИмяСобытияОбновленияПрогресса() Тогда
		Если ПараметрыОповещения.ИдентификаторОперации = Параметры.ИдентификаторОперации Тогда
			Если ПараметрыОповещения.ПараметрыОжиданияОперации.ОперацияЗавершена Тогда 
				ОбновитьФормуПоПараметрам(ПараметрыОповещения.ПараметрыОжиданияОперации);
				ПерейтиВСостояниеЗавершеннойОперации();
			Иначе 
				ОбновитьСостояниеПрогрессаДлительнойОперации(ПараметрыОповещения.ПараметрыОжиданияОперации);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
