...
&НаКлиенте
Процедура Подключаемый_СкачатьФайл(Элемент, ИмяКнопкиУдаления, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
		
	ОтборФайлов = Новый Структура;
	ОтборФайлов.Вставить("ИмяКнопкиУдаления", ИмяКнопкиУдаления);
	СтрокиФайлов = ВыбираемыеФайлы.НайтиСтроки(ОтборФайлов);
	
	Если СтрокиФайлов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаФайла = СтрокиФайлов[0];
...
Процедура СоздатьЭлементыФормыДляВложенногоФайла()
	...
	Для Каждого ВыбираемыйФайл Из ВыбираемыеФайлы Цикл
		...
		ИмяКнопкиУдаления = СтрШаблон("КнопкаУдаленияФайла%1", Строка(Индекс));
		...
		ТекстИмениФайла.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			"<a href=""%1"">%2</a>", ИмяКнопкиУдаления, ПредставлениеФайла);
		...
		КнопкаУдаленияФайла = Элементы.Добавить(ИмяКнопкиУдаления, Тип("ДекорацияФормы"), ГруппаФайла);
		...
		ВыбираемыйФайл.ИмяКнопкиУдаления = ИмяКнопкиУдаления;
