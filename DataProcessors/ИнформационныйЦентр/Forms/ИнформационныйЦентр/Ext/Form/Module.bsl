...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаПоиска.Видимость = Не ПустаяСтрока(СсылкаПоискаИнформации);
...
Процедура ОбработкаОжиданияПоискаОтветаНаВопрос()
	...
	СтрокаПоискаВКодировкеURL = СтрокаПоискаВКодировкеURL(СтрокаПоиска);
	// Локализация	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.БазоваяФункциональностьБИП") Тогда
		...
		МодульИнтернетПоддержкаПользователейКлиент.ОткрытьСтраницуИнтегрированногоСайта(СсылкаПоискаИнформации + СтрокаПоискаВКодировкеURL);
	Иначе
	// Конец Локализация
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(СсылкаПоискаИнформации + СтрокаПоискаВКодировкеURL);
	// Локализация
	КонецЕсли;
	// Конец Локализация
...
Функция СтрокаПоискаВКодировкеURL(СтрокаПоиска)
	Возврат КодироватьСтроку(СтрокаПоиска, СпособКодированияСтроки.КодировкаURL); 
КонецФункции 
