...
Процедура ВычеркнутьНеСоответствующиеКадровомуОтбору(СотрудникиПериоды, Отбор, НастройкаРезервов = Неопределено)
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкаРезервов, "Списание", Ложь)
		Или ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкаРезервов, "Корректировка ", Ложь) Тогда
		СкорректироваПериодыУволенных(СотрудникиПериоды);
	КонецЕсли;
...
Процедура СкорректироваПериодыУволенных(СотрудникиПериоды)
	
	ИнтервалыКадровойИстории = КадроваяИстория();
	Для Каждого СтрокаТаблицы Из СотрудникиПериоды Цикл
		Если СтрокаТаблицы.БезусловноСоответствуетКадровымДанным
			Или СтрокаТаблицы.НеСоответствуетПодразделение
			Или Не СтрокаТаблицы.Вычеркивать Тогда
			Продолжить;
		КонецЕсли;
		Отбор = Новый Структура("Сотрудник, ВидСобытия", СтрокаТаблицы.Сотрудник, Перечисления.ВидыКадровыхСобытий.Увольнение);
		ИсторияСотрудника = ИнтервалыКадровойИстории.НайтиСтроки(Отбор);
		Если ИсторияСотрудника.Количество() > 0 Тогда
			СтрокаТаблицы.Вычеркивать = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьСотрудникамиКРасчету(ТаблицаСотрудников, Начисления, Отборы = Неопределено, НастройкаРезервов)
	...
	ВычеркнутьНеСоответствующиеКадровомуОтбору(ТаблицаСотрудников, Отборы, НастройкаРезервов);
