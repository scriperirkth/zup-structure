...
Процедура ОтобразитьСостояниеКомпонента(Имя, Состояние = "", ТекстОшибки = "")
	...
	Если ЭлементКартинка <> Неопределено Тогда
		...
		Если НадоУстановить И Имя = "УстановитьКриптопровайдер" Тогда
...
Процедура ПодключитьИнтернетПоддержку()
	...
	РезультатПодключения = ПодключитсяКИнтернетПоддержкеНаСервере();
	Если НЕ РезультатПодключения.Выполнено Тогда
		УстановитьДоступностьЭлементовОжиданияПодключения(Ложь);
		Элементы.ГруппаОшибкаПодключения.Видимость = Истина;
		Элементы.ТекстОшибкиПодключения.Заголовок = РезультатПодключения.ОписаниеОшибки;
		Возврат;
	КонецЕсли;
...
Функция ПодключитсяКИнтернетПоддержкеНаСервере()
	...
	Если ЕстьБИП Тогда
		...
		ВерсияБИПНовыйИнтерфейс = "2.9.1.51";
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБИП, ВерсияБИПНовыйИнтерфейс) >= 0 Тогда
			ДоступноПодключениеИнтернетПоддержки = МодульИнтернетПоддержкаПользователей.ДоступноПодключениеИнтернетПоддержки();
			Если ДоступноПодключениеИнтернетПоддержки Тогда
				РезультатДанныеВалидны = МодульИнтернетПоддержкаПользователейКлиентСервер.ПроверитьДанныеАутентификации(
					ДанныеАутентификации);
				Если НЕ РезультатДанныеВалидны.Отказ Тогда
					РезультатПодключения = МодульИнтернетПоддержкаПользователей.Подключить(ДанныеАутентификации);
					Если НЕ РезультатПодключения.Подключена Тогда
						Результат.Выполнено = Ложь;
						Результат.ОписаниеОшибки = РезультатПодключения.СообщениеОбОшибке;
					КонецЕсли;
				Иначе
					Результат.Выполнено = Ложь;
					Результат.ОписаниеОшибки = РезультатДанныеВалидны.СообщениеОбОшибке;
				КонецЕсли;
			Иначе
				Результат.Выполнено = Ложь;
				Результат.ОписаниеОшибки = НСтр("ru='Подключение интернет-поддержки недоступно.'");
			КонецЕсли;
		Иначе
...
Процедура НастроитьОтображениеVipNet()
	...
	Элементы.КарандашУстановитьКриптопровайдер.Видимость = ГОСТ34122018ViPNetCSPДоступен;
