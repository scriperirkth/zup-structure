...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЭтоПолноправныйПользователь = Пользователи.ЭтоПолноправныйПользователь();

	ЭтотОбъект.ОбластьДанных = -1;
	ЭтотОбъект.РазделениеВключено                     = ОбщегоНазначения.РазделениеВключено();
	ЭтотОбъект.ДоступноИспользованиеРазделенныхДанных = ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных();
	ЭтотОбъект.РазрешенДоступКИнтернетСервисам        = ОбщегоНазначения.РазрешенДоступКИнтернетСервисам();
...
Процедура НадписьБлокировкаРаботыИнтернетСервисовОбработкаНавигационнойСсылки(
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "action:settings" Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура СписокОбновляемыхКатегорийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ВРег(ТекущиеДанные.Список) = ВРег("Список лент новостей") Тогда // АПК:1297 Это идентификатор.
			// Перейти на закладку лент новостей.
			Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаНастройкиЛентНовостей;
		ИначеЕсли ВРег(ТекущиеДанные.Список) = ВРег("Список категорий новостей") Тогда // АПК:1297 Это идентификатор.
			...
			ОткрытьФорму(
				...
				ЭтотОбъект,
				"");
		ИначеЕсли ВРег(ТекущиеДанные.ТипСписка) = ВРег("Список значений классификатора") Тогда // АПК:1297 Это идентификатор.
...
Процедура КомандаДиагностикаПолученияНовостей(Команда)
	...
	СписокШагов = Новый СписокЗначений;
		СписокШагов.Добавить( 1, "Настройки программы: значения констант"                           , Истина);
		СписокШагов.Добавить( 2, "Настройки программы: значения предопределенных категорий"         , Истина);
		СписокШагов.Добавить( 3, "Настройки программы: заполненность объектов метаданных"           , Истина);
		СписокШагов.Добавить( 4, "Настройки программы: фильтры"                                     , Истина);
		СписокШагов.Добавить( 5, "Настройки программы: правильность внедрения подсистемы"           , Истина);
		СписокШагов.Добавить( 6, "Настройки программы: параметры сеанса"                            , Истина);
		СписокШагов.Добавить( 7, "Настройки программы: данные о конфигурации из метаданных"         , Истина);
		СписокШагов.Добавить( 8, "Настройки программы: список важных и очень важных новостей"       , Истина);
		СписокШагов.Добавить( 9, "Проверка подключения: доступ к HTTP соединению"                   , Истина);
		СписокШагов.Добавить(10, "Проверка окружения: размер свободного места во временном каталоге", Истина);
		СписокШагов.Добавить(11, "Проверка открытых форм: контекстные новости и настройки"          , Ложь);
		СписокШагов.Добавить(12, "Настройки программы: Регламентное задание (неразделенное)"        , Истина);
		СписокШагов.Добавить(13, "Настройки программы: Регламентное задание (для области данных)"   , Истина);
		СписокШагов.Добавить(14, "Настройки программы: Заголовки системы"                           , Ложь);
		СписокШагов.Добавить(15, "Настройки программы: Категории новостей"                          , Истина);
		СписокШагов.Добавить(16, "Данные программы: Ленты новостей"                                 , Истина);
		СписокШагов.Добавить(17, "Данные программы: Последние новости с рассчитанными отборами"     , Истина);
		СписокШагов.Добавить(18, "Журнал регистрации: состояние системы"                            , Истина);
		СписокШагов.Добавить(19, "Логи: фоновые задания"                                            , Истина);
	// АПК:1297 -вкл
...
Процедура УправлениеФормой(Форма)
	...
	ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет = Форма.РазделениеВключено ИЛИ Форма.РазрешенДоступКИнтернетСервисам;
	ЛокальноРазрешенаРаботаСНовостямиЧерезИнтернет        = Форма.РазрешенаРаботаСНовостямиЧерезИнтернет;
	...
	Элементы.ГруппаНастройкиПрокси.Доступность                                    = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ГруппаОбновлениеОбщихСписков.Доступность                             = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ГруппаПолучениеНовостей.Доступность                                  = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.КомандаЗагрузитьСтандартныеСписки.Доступность                        = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.КонтекстноеМенюКомандаСброситьДатуЗагрузкиКлассификатора.Доступность = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ЛентыНовостейГруппаПолучитьНовости.Видимость                         = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ЛентыНовостейКонтекстноеМенюПолучитьНовости.Видимость                = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ЛентыНовостейКонтекстноеМенюГруппаСбросДаты.Видимость                = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;

	Элементы.ЛентыНовостейКонтекстноеМенюКомандаЗагрузитьНовостиПоВсемЛентамНовостей.Видимость       = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ЛентыНовостейКомандаЗагрузитьНовостиПоВсемЛентамНовостей.Видимость                      = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ЛентыНовостейКонтекстноеМенюКомандаЗагрузитьНовостиПоВыделеннымЛентамНовостей.Видимость = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ЛентыНовостейКомандаЗагрузитьНовостиПоВыделеннымЛентамНовостей.Видимость                = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.КомандаЗагрузитьСтандартныеСписки.Видимость                                             = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.КонтекстноеМенюКомандаСброситьДатуЗагрузкиКлассификатора.Видимость                      = ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;

	Элементы.ДекорацияРежимРаботыСНовостямиЧерезИнтернет_ОбновлениеКлассификаторов.Видимость = НЕ ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ДекорацияРежимРаботыСНовостямиЧерезИнтернет_ЛентыНовостей.Видимость             = НЕ ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет;
	...
	Если Не Форма.ЭтоПолноправныйПользователь Тогда
		Элементы.НадписьБлокировкаРаботыИнтернетСервисов.Заголовок =
			НСтр("ru = 'В программе централизованно заблокирована работа с Интернет-сервисами. Для разблокировки обратитесь к администратору.'")
	КонецЕсли;

	Элементы.ГруппаБлокировкаИнтернетСервисов.Видимость             = НЕ ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ГруппаРазрешенаРаботаСНовостямиЧерезИнтернет.Видимость = ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ГруппаНастройкиПрокси.Видимость                        = ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ГруппаОбновлениеОбщихСписков.Видимость                 = ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет;
	Элементы.ГруппаПолучениеНовостей.Видимость                      = ЦентрализованноРазрешенаРаботаСНовостямиЧерезИнтернет;
...
Процедура ПослеОтветаУдалениеНовостей(РезультатВопроса, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		НаименованиеМетода = "Обработка.УправлениеНовостями.ФормаНастроекНовостей.ПослеОтветаУдалениеНовостей"; // АПК:1297 Это идентификатор.
...
Функция ВыполнитьШагПроверки_НастройкиОткрытыхФорм()
	...
	Оповестить(ВсеОповещения.СобратьИнформациюОбОкнах, , ЭтотОбъект); // АПК:1297 Это идентификатор.
...
Функция ВыполнитьШагПроверки_КатегорииНовостей()
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения.Добавить("Удал;Код;Наименование;УИН;ЗагруженоССервера;"); // АПК:1297 Это идентификатор.
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"%1;%2;%3;%4;%5;", // АПК:1297 Это идентификатор.
...
Функция ВыполнитьШагПроверки_ЛентыНовостей()
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения.Добавить("Удал;Код;Наименование;ЗагруженоССервера;ЧастотаОбновления;ЛокальнаяЛента;URL;ВидимостьПоУмолчанию;"); // АПК:1297 Это идентификатор.
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.Следующий() Цикл
			...
			ЛогШаговВыполнения.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"%1;%2;%3;%4;%5;%6;%7;%8;%9", // АПК:1297 Это идентификатор.
					?(Выборка.ПометкаУдаления, "[DEL]", "[ ]"),
					Выборка.Код,
					Выборка.Наименование,
...
Функция ВыполнитьШагПроверки_ПоследниеНовостиСОтборами()
	...
	Запрос.УстановитьПараметр("Недоступно"         , "[НЕДОСТУПНО]"); // АПК:1297 Это идентификатор.
	Запрос.УстановитьПараметр("ПоУмолчаниюВидна"   , "[ПО-УМОЛЧАНИЮ_ВИДНА]"); // АПК:1297 Это идентификатор.
	Запрос.УстановитьПараметр("Видна"              , "[ВИДНА]"); // АПК:1297 Это идентификатор.
	Запрос.УстановитьПараметр("Скрыта"             , "[СКРЫТА]"); // АПК:1297 Это идентификатор.
	Запрос.УстановитьПараметр("Неопределено"       , "[НЕОПРЕДЕЛЕНО]"); // АПК:1297 Это идентификатор.

	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЛогШаговВыполнения.Добавить("Удал;Наименование;УИН;ДатаПубликации;"); // АПК:1297 Это идентификатор.
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
		Пока Выборка.СледующийПоЗначениюПоля("ЛентаНовостей") Цикл
			ЛогШаговВыполнения.Добавить("-" + НСтр("ru = 'Лента новостей'") + ": " + Выборка.ЛентаНовостей); // АПК:1297 Это идентификатор.
...
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(Знач МассивВыделенныхЛентНовостей)
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ФормаНастроекНовостей.ПолучитьИОбработатьНовостиПоЛентамНовостей/ЗагрузитьНовости.Вручную"; // АПК:1297 Это идентификатор.
