...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗагрузитьФайлыНовостейССервера"; // АПК:1297 Это идентификатор.
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ПолучениеНастроекНовостей"); // АПК:1297 Это идентификатор.
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ФормированиеСпискаЛентНовостей"); // АПК:1297 Это идентификатор.
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ПравильностьВнедренияПодсистемы/ТолькоПереопределения"); // АПК:1297 Это идентификатор.
	...
	Если ЕстьОшибкиВнедрения = Истина Тогда
		НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ЛентыНовостейПриНеправильномВнедренииПодсистемы"); // АПК:1297 Это идентификатор.
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ПредопределенныеКатегории"); // АПК:1297 Это идентификатор.
...
#Область ЗагрузкаФайлов_Файл_ЛентаНовостей
				НачатьШагВыполнения(КонтекстВыполненияЛентыНовостей, "Цикл:Файл:" + ТекущаяЛентаНовостей.ЛентаНовостей); // АПК:1297 Это идентификатор.
				...
				ЛогШаговВыполненияЛентыНовостей.Добавить(ТекстСообщения);
				ЗавершитьШагВыполнения(КонтекстВыполненияЛентыНовостей, "", ЛогШаговВыполненияЛентыНовостей);
#КонецОбласти
			КонецЦикла;
...
#Область Получение_HTTP_ЛентаНовостей
				НачатьШагВыполнения(КонтекстВыполненияЛентыНовостей, "Цикл:HTTP:" + ТекущаяЛентаНовостей.ЛентаНовостей); // АПК:1297 Это идентификатор.
				ЛогШаговВыполненияЛентыНовостей = Новый Массив;
				лкИмяФайлаНаСервере = "";
				ЕстьОшибки = Ложь;
							...
							Если (HTTPОтвет.КодСостояния >= 500) И (HTTPОтвет.КодСостояния <= 599) Тогда
								...
								Исключение
									...
									ОбработкаНовостейСлужебный.ЗаписатьСообщениеВЖурналРегистрации(
										...
										ОбработкаНовостейСлужебный.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
								КонецПопытки;
							КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.РазборXMLФайловНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область ОсновныеОбработки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "СохранениеДатыПоследнихЗагрузок"); // АПК:1297 Это идентификатор.
	...
	КонтекстВыполненияЛентыНовостей = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		"Основные/ЦиклПоЛентамНовостей"); // АПК:1297 Это идентификатор.
		...
		Если В.ЕстьОшибки = Ложь Тогда
			...
			Если В.ЕстьОшибки = Ложь Тогда
				...
				КонтекстВыполненияРазбораXMLФайла = НачатьШагВыполненияВложенный(
					...
					"Основные/ЦиклПоЛентамНовостей/РазборXMLФайлаНовостей"); // АПК:1297 Это идентификатор.
...
#Область ВспомогательныеОбработкиПереопределяемые
	...
	НачатьШагВыполнения(КонтекстВыполнения, "ПереопределяемыеДействия:ДополнительноОбработатьНовостиПослеПолучения"); // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.РазборXMLФайлаНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:РазрешенаРаботаСНовостямиЧерезИнтернет"); // АПК:1297 Это идентификатор.
	...
	ЛокальноРазрешенаРаботаСНовостямиЧерезИнтернет        = ОбработкаНовостейВызовСервера.РазрешенаРаботаСНовостямиЧерезИнтернет();
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основное:ДатыЗагрузкиПоследнихНовостей"); // АПК:1297 Это идентификатор.
	Запись = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьМенеджерЗаписи();
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗагрузитьНовость"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
		КонтекстВыполнения,
		НаименованиеМетода + "/Основное:ЗагрузкаНовости"); // АПК:1297 Это идентификатор.
		...
		Если ЗаписыватьНовость = Истина Тогда
			...
			Попытка
				...
				Иначе
					...
					ОбработкаНовостейСлужебный.ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовость, НовостьХДТО);
					...
					ОбработкаНовостейСлужебный.ДополнительноОбработатьНовостьПослеПолученияПослеЗаписи(ОбъектНовость.Ссылка);
...
Процедура ЗагрузитьБлокДанныхКатегорий1С_Категория(ДанныеХДТО, ОбъектНовость)
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ВРег(КатегорияНовостейСтрока) = ВРег("Важность") // АПК:1297 Это идентификатор.
				И ВРег(ЗначениеКатегорииНовостейСтрока) = ВРег("Очень важная") // АПК:1297 Это идентификатор.
				И ВРег(УсловиеОтбора) = ВРег("eq") Тогда
			ОбъектНовость.Важность = 1; // Перезапишет ОбъектНовость.Важность, если было установлено ранее из тега priority.
		ИначеЕсли ВРег(КатегорияНовостейСтрока) = ВРег("Важность")
				И ВРег(ЗначениеКатегорииНовостейСтрока) = ВРег("Важная") // АПК:1297 Это идентификатор.
				И ВРег(УсловиеОтбора) = ВРег("eq") Тогда
			ОбъектНовость.Важность = 2; // Перезапишет ОбъектНовость.Важность, если было установлено ранее из тега priority.
		Иначе
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:ТипОбъектаОбновления"); // АПК:1297 Это идентификатор.
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Данные:ПолучениеСвойствОбъекта"); // АПК:1297 Это идентификатор.
	Попытка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Данные:ЗаписьДанных"); // АПК:1297 Это идентификатор.
	Если ЕстьОшибки = Ложь Тогда

		Если listName = "Список лент новостей" Тогда // АПК:1297 Это идентификатор.
			...
			Запись.Записать(Истина);
		ИначеЕсли listName = "Список категорий новостей" Тогда // АПК:1297 Это идентификатор.
			Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
				Запись.Список = listName;
			Запись.Прочитать(); // Запись будет ниже.
				Запись.ТекущаяВерсияНаСервере = updated;
			Запись.Записать(Истина);
		ИначеЕсли listName = "Значения категории новостей" Тогда // АПК:1297 Это идентификатор.
			...
			Если Значение.Пустая() Тогда
				Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
					Запись.Список = "Значения категории новостей:" + categoryGuid; // АПК:1297 Это идентификатор.
				Запись.Прочитать(); // Запись будет ниже.
					Запись.ТекущаяВерсияНаСервере = updated;
				Запись.Записать(Истина);
			Иначе
...
//
Функция ПолучитьСтандартныеСпискиТребующиеОбновления()
	...
	Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
	ПроверяемыйСписок = "Список лент новостей"; // АПК:1297 Это идентификатор.
	...
	Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
	ПроверяемыйСписок = "Список категорий новостей"; // АПК:1297 Это идентификатор.
	Запись.Список = ПроверяемыйСписок;
	Запись.Прочитать(); // Запись будет ниже.
	Если НЕ Запись.Выбран() Тогда
		Запись.Список = ПроверяемыйСписок;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для каждого ТекущаяСтрока Из ТаблицаОбновляемыхДанных Цикл
			...
			Иначе
				Если ТекущаяСтрока.Список = "Список лент новостей" Тогда // АПК:1297 Это идентификатор.
					ТекстСообщения = НСтр("ru='Загрузка списка лент новостей'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли ТекущаяСтрока.Список = "Список категорий новостей" Тогда // АПК:1297 Это идентификатор.
					ТекстСообщения = НСтр("ru='Загрузка списка категорий новостей'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли СтрНайти(ВРег(ТекущаяСтрока.Список), ВРег("Значения категории новостей:")) = 1 Тогда // АПК:1297 Это идентификатор.
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ОбновитьСтандартныйСписокССервера"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Проверки
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Проверка:УдалосьПодключитьсяКВебСервисам"); // АПК:1297 Это идентификатор.
	Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
...
#Область Обработка
	...
	ЛогШаговВыполнения = Новый Массив;
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ЗагрузкаДанных"); // АПК:1297 Это идентификатор.
	...
	ИначеЕсли Список = "Список лент новостей" Тогда // АПК:1297 Это идентификатор.
		...
		Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
			...
			Если (СписокОбновленныхДанных.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsTypes")
				...
				Если ТипЗнч(СписокОбновленныхДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					//  все ли данные корректно записаны (возможно, было исключение на каком-то элементе).
					Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
					Запись.Список = "Список лент новостей"; // АПК:1297 Это идентификатор.
					Запись.Прочитать(); // Запись будет ниже.
					Если Запись.Выбран() Тогда
						Если Запись.ТекущаяВерсияНаКлиенте < ДатаСамогоНовогоЭлемента Тогда
							Запись.Список = "Список лент новостей"; // АПК:1297 Это идентификатор.
	...
	ИначеЕсли Список = "Список категорий новостей" Тогда // АПК:1297 Это идентификатор.
		СписокОбновленныхДанных = Неопределено;
		Попытка
		...
		Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
			...
			Если (СписокОбновленныхДанных.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsTypes")
				...
				Если ТипЗнч(СписокОбновленныхДанных.category) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					//  все ли данные корректно записаны (возможно, было исключение на каком-то элементе).
					Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
					Запись.Список = "Список категорий новостей"; // АПК:1297 Это идентификатор.
					Запись.Прочитать(); // Запись будет ниже.
					Если Запись.Выбран() Тогда
						Если Запись.ТекущаяВерсияНаКлиенте < ДатаСамогоНовогоЭлемента Тогда
							Запись.Список = "Список категорий новостей"; // АПК:1297 Это идентификатор.
	...
	ИначеЕсли СтрНайти(ВРег(Список), ВРег("Значения категории новостей:")) = 1 Тогда // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Подключение:ВебСервисыОбновленияСтандартныхСписков"); // АПК:1297 Это идентификатор.// АПК:1297 Это идентификатор.
	Прокси = Неопределено;
	Если ИтогоРазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
		КонтекстВыполненияВложенный = НачатьШагВыполненияВложенный(
			КонтекстВыполнения,
		...
		Прокси = ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполненияВложенный);
		ЗавершитьШагВыполнения(КонтекстВыполнения, "", "", КонтекстВыполненияВложенный);
		// Запрет на получение новостей из интернета - не ошибка (т.к. новости могут получать и через файл и через макет), продолжаем выполнение.
	КонецЕсли;
	...
	Иначе
		...
		НачатьШагВыполнения(КонтекстВыполнения, "Основное:ПолучениеСписковТребующихОбновление"); // АПК:1297 Это идентификатор.
		ТаблицаОбновляемыхДанных = ПолучитьСтандартныеСпискиТребующиеОбновления();

		Для каждого ТекущийОбновляемыйСписок Из ТаблицаОбновляемыхДанных Цикл
			Если ТекущийОбновляемыйСписок.Список = "Список лент новостей" Тогда // АПК:1297 Это идентификатор.
				БылоОбновлениеЛентНовостей = Истина;
			КонецЕсли;
			Если ТекущийОбновляемыйСписок.Список = "Список категорий новостей" Тогда // АПК:1297 Это идентификатор.
				БылоОбновлениеКатегорийНовостей = Истина;
			КонецЕсли;
			НачатьШагВыполнения(КонтекстВыполнения, "Основное:ОбновитьСтандартныйСписокССервера/" + ТекущийОбновляемыйСписок.Список); // АПК:1297 Это идентификатор.
		...
		Если БылоОбновлениеЛентНовостей <> Истина Тогда
			ИдентификаторСписка = "Список лент новостей"; // АПК:1297 Это идентификатор.
			НачатьШагВыполнения(КонтекстВыполнения, "Основное:ОбновитьСтандартныйСписокССервера/" + ИдентификаторСписка); // АПК:1297 Это идентификатор.
			БылаОшибка = НЕ ОбновитьСтандартныйСписокССервера(
				ИдентификаторСписка,
				'00010101',
				Прокси,
		...
		Если БылоОбновлениеКатегорийНовостей <> Истина Тогда
			ИдентификаторСписка = "Список категорий новостей"; // АПК:1297 Это идентификатор.
			НачатьШагВыполнения(КонтекстВыполнения, "Основное:ОбновитьСтандартныйСписокССервера/" + ИдентификаторСписка); // АПК:1297 Это идентификатор.
			БылаОшибка = НЕ ОбновитьСтандартныйСписокССервера(
				ИдентификаторСписка,
				'00010101',
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ОбновитьСписокСправочниковИВерсииССервера"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка_ПолучениеПараметровВыполнения
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ПолучениеПараметровВыполнения"); // АПК:1297 Это идентификатор.
	configName    = СокрЛП(Метаданные.Имя);
...
#Область Обработка_ПолучениеДатОбновленияСписков
	НачатьШагВыполнения(КонтекстВыполнения, "Инициализация:ДатыОбновленияСписков"); // АПК:1297 Это идентификатор.
	Попытка
...
#Область Обработка_ЗагрузкаДанных
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основное:ПолучениеДанных"); // АПК:1297 Это идентификатор.
		...
		Если ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO Тогда
			Если ТипЗнч(ДатыОбновленияСписков.updatesItem) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
				Для каждого ТекущаяЗапись Из ДатыОбновленияСписков.updatesItem Цикл
					Если (ТекущаяЗапись.listName = "Список категорий новостей") // АПК:1297 Это идентификатор.
							ИЛИ (ТекущаяЗапись.listName = "Список лент новостей") Тогда // АПК:1297 Это идентификатор.
						Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
							...
							КонецЕсли;
						Запись.Записать(Истина);
					ИначеЕсли ТекущаяЗапись.listName = "Значения категории новостей" Тогда // АПК:1297 Это идентификатор.
						...
						Если ТекущаяЗапись.Свойства().Получить("categoryGuid") <> Неопределено Тогда
							Если НЕ ПустаяСтрока(ТекущаяЗапись.categoryGuid) Тогда
								...
								Если КатегорияНайдена = Истина Тогда
									...
									Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
									Запись.Список = "Значения категории новостей:" + ТекущаяЗапись.categoryGuid; // АПК:1297 Это идентификатор.
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ЗаполнитьОбъектЛентаНовостей"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Основные:ЗаполнениеОбъектаЛентаНовостей"); // АПК:1297 Это идентификатор.
	...
	Если ПустаяСтрока(ЛентаНовостейОписатель.ВерсияДанных) Тогда // Новый объект
		...
		Режим = "Создание"; // АПК:1297 Это идентификатор.
	Иначе // Объект есть в базе данных
		лкОбъект = ЛентаНовостейОписатель.Ссылка.ПолучитьОбъект();
		Режим = "Обновление"; // АПК:1297 Это идентификатор.
	КонецЕсли;
...
#Область ИнициализацияМетода
	...
	НаименованиеМетода = "Обработка.УправлениеНовостями.ПодключитьсяКВебСервисуОбновленияСтандартныхСписков"; // АПК:1297 Это идентификатор.
	Если КонтекстВыполнения = Неопределено Тогда
		КонтекстВыполнения = НовыйКонтекстВыполнения(НаименованиеМетода, Истина); // Этот контекст.
	КонецЕсли;
...
#Область Обработка
	...
	НачатьШагВыполнения(КонтекстВыполнения, "Подключение"); // АПК:1297 Это идентификатор.
