...
Процедура ОбновитьИндикаторЗагрузки(СтатусВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если СтатусВыполнения.Статус <> "Выполняется" Тогда
		Возврат;
	КонецЕсли;
	
	Если СтатусВыполнения.Прогресс <> Неопределено Тогда
		ИндикаторОбновления = СтатусВыполнения.Прогресс.Процент;
	КонецЕсли;
...
Процедура НачатьОбновлениеКлассификаторовЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если ЗначениеЗаполнено(РезультатОперации.КодОшибки) Тогда
			ИнформацияОбОшибке = РезультатОперации.СообщениеОбОшибке;
			УстановитьОтображениеИнформацииОбОшибке(
				ЭтотОбъект,
				ИнформацияОбОшибке,
				Ложь,
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ДанныеОшибки = ОбщегоНазначенияКлиентСервер.УточнениеИсключения(
			...
			НСтр("ru = 'Ошибка загрузки классификаторов.'"));
