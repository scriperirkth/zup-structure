...
Процедура ТаблицаСоответствийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтоСопоставлениеЛичныхДанныхКандидатов И Поле.Имя <> "ТаблицаСоответствияСоздается" Тогда
...
Процедура ПодготовитьФорму(Отказ)
	...
	ВывестиОшибкиИзОписанияОбъектов(Описание);
	
	ЭтоСопоставлениеЛичныхДанныхКандидатов =
		(КлючОписания = ПереносДанныхEStaffКлиентСервер.КлючЛичныхДанныхКандидата());
	Если ЭтоСопоставлениеЛичныхДанныхКандидатов Тогда
		ОписаниеИОбъектыКандидатов = ОписанияИОбъекты[ПереносДанныхEStaffКлиентСервер.КлючКандидата()];
		ОписаниеКандидатов = ОписаниеИОбъектыКандидатов.Описание;
		ВывестиОшибкиИзОписанияОбъектов(ОписаниеКандидатов);
	КонецЕсли;
...
&НаСервере
Процедура ВывестиОшибкиИзОписанияОбъектов(Описание)
	
	Для Каждого ТекстОшибки Из Описание.Ошибки Цикл
...
Функция РезультатПоискаПохожихЛюдей(ДанныеПоиска)
	
	СтруктураФИО = ФизическиеЛицаКлиентСервер.ЧастиИмени(ДанныеПоиска.Наименование);

	ПараметрыПоиска = ПоискФизическихЛиц.ПараметрыПоискаПохожихЛюдей();
	ПараметрыПоиска.Ссылка = ДанныеПоиска.ОбъектСсылка;
	ЗаполнитьЗначенияСвойств(ПараметрыПоиска, СтруктураФИО);
	
	Возврат ПоискФизическихЛиц.ПохожиеЛюди(ПараметрыПоиска);
...
Процедура ПроверитьОднофамильцев(СтандартнаяОбработка)
	...
	ДанныеПоиска.Вставить("Наименование", ТекущиеДанные.Наименование);
	ДанныеПоиска.Вставить("ОбъектСсылка", ТекущиеДанные.ОбъектСсылка);
	РезультатПоиска = РезультатПоискаПохожихЛюдей(ДанныеПоиска);
...
Процедура ПроверкаОднофамильцевЗавершение(РезультатВыбора, ДопПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(РезультатВыбора.ФизическоеЛицо) Тогда
		ОбъектСсылка = РезультатВыбора.ФизическоеЛицо;
	ИначеЕсли ЗначениеЗаполнено(РезультатВыбора.ЛичныеДанныеКандидата) Тогда
		ОбъектСсылка = РезультатВыбора.ЛичныеДанныеКандидата;
	Иначе
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = ТаблицаСоответствия.НайтиПоИдентификатору(ДопПараметры.ТекущаяСтрока);
	ТекущиеДанные.ОбъектСсылка = ОбъектСсылка;
...
Процедура УстановитьТекстПояснений(ПредставлениеОбъекта, ПредставлениеСписка)
	...
	Если КлючОписания = ПереносДанныхEStaffКлиентСервер.КлючПодразделения()
		Или КлючОписания = ПереносДанныхEStaffКлиентСервер.КлючЛичныхДанныхКандидата() Тогда
