...
Процедура КаталогСФайламиВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЭтотОбъект[ИмяКаталога] = Результат[0];
...
Процедура ПоместитьФайлыПоОписаниюОбъектовЗавершение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы <> Неопределено Тогда
		Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
			ТекущееОписание.ХранениеФайлов.Добавить(ПомещенныйФайл.Хранение);
		КонецЦикла;
	КонецЕсли;
...
// Параметры:
//  ОписанияОбъектов - Массив из Структура
&НаСервере
...
Процедура СопоставитьОбъектыНаСервере(КлючОписания, ОткрыватьФормуСопоставления, АдресТаблицыСоответствия)
	...
	Если Не ОткрыватьФормуСопоставления Тогда
		Обработки.ПереносДанныхEStaff.СохранитьТаблицуСоответствий(ТаблицаСоответствия, АдресОписанийИОбъектов,
			КлючОписания);
	Иначе
...
Процедура ПоказатьФормуСопоставления(КлючОписания, АдресОписанийИОбъектов, ДополнительныеПараметры)
	...
	Если Не ОбновлятьЛичныеДанныеКандидатов 
		И КлючОписания = ПереносДанныхEStaffКлиентСервер.КлючЛичныхДанныхКандидата() Тогда
		СоздаватьНовые = Ложь;
	КонецЕсли;
	...
	ОткрытьФорму("Обработка.ПереносДанныхEStaff.Форма.СопоставлениеОбъектов", ПараметрыФормы, ЭтотОбъект, , , ,
...
Процедура НайтиФайлыФизическихЛиц()
	...
	Ключ = ПереносДанныхEStaffКлиентСервер.КлючЛичныхДанныхКандидата();
