...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("мСохраненныйДок", мСохраненныйДок);
	ТекстЗаменыЗаголовок = "";
	ТекстЗаменыСписокСсылок = "";
	ТекстЗаменыПредложениеКонвертации = "";
	РеквизитыДокумента = Новый Структура;
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПредлагатьКонвертацию", Ложь) Тогда 
		ТекстЗаменыПредложениеКонвертации = "<p align=""left"" ><font size=""2"" face=""Microsoft Sans Serif"">"
			+ "Можно <a href=""СконвертироватьОтчет"">сконвертировать отчет</a> до актуальной редакции<br></p>";
	КонецЕсли;
	
	Если ТипЗнч(мСохраненныйДок) = Тип("ДокументСсылка.РегламентированныйОтчет") И ЗначениеЗаполнено(мСохраненныйДок) Тогда
		...
		РеквизитыДокумента.Вставить("ИмяФормыОтчета", ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ИмяФормыОтчета", ""));
		
		СтруктураРеквизитовФормы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "СтруктураРеквизитовФормы");
		Если ЭтоАдресВременногоХранилища(СтруктураРеквизитовФормы) Тогда 
			СтруктураРеквизитовФормы = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы);
			Если ТипЗнч(СтруктураРеквизитовФормы) = Тип("Структура") 
				И ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураРеквизитовФормы, "ЕстьАктуальныеФормы", Ложь) Тогда 
				...
				ТекстЗаменыСписокСсылок = "<p align=""left""><font size=""2"" face=""Microsoft Sans Serif"">";
				Пока Выборка.Следующий() Цикл
					ТекстЗаменыЗаголовок = "Сохраненные отчеты актуальной редакции";
					ТекстЗаменыСписокСсылок = ТекстЗаменыСписокСсылок + "<a href=""" 
						+ ПолучитьНавигационнуюСсылку(Выборка.Ссылка) + """>" + Строка(Выборка.Ссылка) + "</a><br>";
				КонецЦикла;
				ТекстЗаменыСписокСсылок = ТекстЗаменыСписокСсылок + "</p>";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "СохраненныеОтчетыАктуальнойРедакции", ТекстЗаменыЗаголовок);
	ТекстHTML = СтрЗаменить(ТекстHTML, "СписокСсылок", ТекстЗаменыСписокСсылок);
	ТекстHTML = СтрЗаменить(ТекстHTML, "ПредложениеКонвертации", ТекстЗаменыПредложениеКонвертации);
КонецПроцедуры
...
Асинх Процедура Конвертировать()
	...
	ПараметрыФормы.Вставить("ЗначениеКопирования", мСохраненныйДок);
