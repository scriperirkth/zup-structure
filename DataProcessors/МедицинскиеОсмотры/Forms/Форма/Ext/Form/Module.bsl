...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьОтборТолькоБлижайшиеОсмотры(ЭтотОбъект, ПоказыватьТолькоБлижайшиеОсмотры);
...
Процедура ЗаполнитьДанныеОсмотров(ДанныеОсмотров)
	...
	Для Каждого ДанныеСтроки Из Элементы.ПериодическиеОсмотры.ВыделенныеСтроки Цикл
		...
		ДанныеРегистра = РегистрыСведений.ПериодическиеМедицинскиеОсмотры.СоздатьМенеджерЗаписи();
		ЗаполнитьЗначенияСвойств(ДанныеРегистра, ДанныеСтроки);
		ДанныеРегистра.Прочитать();
		ЗаполнитьЗначенияСвойств(Описание, ДанныеРегистра);
		Если ЗначениеЗаполнено(ДанныеРегистра.ДатаУвольнения) Тогда
			Описание.Информация = СтрШаблон(НСтр("ru = 'Увольняется с %1'"), Формат(ДанныеРегистра.ДатаУвольнения, "ДФ='dd.MM.yyyy''г.'''"));	
		КонецЕсли;
...
Процедура НачатьВводНаправления()
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ДатаУвольнения) Тогда
		ПоказатьВопросЗаполненаДатаУвольнения(ТекущиеДанные);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьВопросЗаполненаДатаУвольнения(ТекущиеДанные)
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'У сотрудника %1 заполнена Дата увольнения.'"),
		ТекущиеДанные.ФизическоеЛицо);
		
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ru = 'Сформировать направление'"));
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		
	ОбработчикОповещения = Новый ОписаниеОповещения("ВвестиНовоеНаправлениеПослеВопросаНаправлениеСуществует", ЭтотОбъект, ТекущиеДанные);
	СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(ОбработчикОповещения, ТекстВопроса, Кнопки);
...
Процедура УстановитьПараметрТекущаяДата(Форма)
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.ПериодическиеОсмотры, "ТекущаяДата", Форма.ТекущаяДата);
...
Процедура УстановитьУсловноеОформление()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ЭлементОформления.Отбор, "ДатаСледующегоОсмотра", ТекущаяДата, ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
	...
	// Увольнение раньше следующего медосмотра.
	ЭлементОформления = ПериодическиеОсмотры.УсловноеОформление.Элементы.Добавить();
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ЭлементОформления.Отбор, "ДатаУвольнения", , ВидСравненияКомпоновкиДанных.Заполнено);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ЭлементОформления.Отбор, "ДатаСледующегоОсмотра", , ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаНеАктивнойСтрокиОхраныТруда);
	
	// Увольнение позже следующего медосмотра.
	ЭлементОформления = ПериодическиеОсмотры.УсловноеОформление.Элементы.Добавить();
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ЭлементОформления.Отбор, "ДатаУвольнения", "ДатаСледующегоОсмотра", ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '< Не требуется >'"));
	ПолеЭлемента = ЭлементОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("ДатаСледующегоОсмотра");
...
Процедура ОформитьСтрокиПериодическихОсмотров(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого СтрокаСписка Из Строки Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСтроки.Направление) И Не ЗначениеЗаполнено(ДанныеСтроки.РезультатОсмотра) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		// Уволен раньше следующего медосмотра
		Если ЗначениеЗаполнено(ДанныеСтроки.ДатаУвольнения) И НЕ ЗначениеЗаполнено(ДанныеСтроки.ДатаСледующегоОсмотра) Тогда
			ОтображатьКартинкуПредупреждение = Ложь;
			ТекстСостояния = СтрШаблон(НСтр("ru = 'Увольняется с %1'"), Формат(ДанныеСтроки.ДатаУвольнения, "ДФ='dd.MM.yyyy''г.'''"));	
		КонецЕсли;
		// Будет уволен позже следующего медосмотра
		Если ЗначениеЗаполнено(ДанныеСтроки.ДатаУвольнения) И ЗначениеЗаполнено(ДанныеСтроки.ДатаСледующегоОсмотра) Тогда
			ОтображатьКартинкуПредупреждение = Истина;
			ВыделятьКраснымЦветом = Истина;
			ТекстСостояния = СтрШаблон(НСтр("ru = 'Увольняется с %1'"), Формат(ДанныеСтроки.ДатаУвольнения, "ДФ='dd.MM.yyyy''г.'''"));	
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтборТолькоБлижайшиеОсмотры(Форма, Использование)
...
Функция ЕстьНеактуальныеЗаписиПериодическихОсмотров(Организация)
	...
	Запрос.Текст = 
		...
		|	ИСТИНА
		|ИЗ
		|	РегистрСведений.БудущиеСобытияМедицинскиеОсмотры КАК БудущиеСобытия
		|ГДЕ
