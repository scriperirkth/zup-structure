...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеШтатнымРасписаниемФормы.УстановитьУсловноеОформлениеСпискаПодразделений(
		ЭтотОбъект, "ПодразделенияОрганизаций", "СформированоНаСегодня", "РасформированоНаСегодня");
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		МодульОрганизационнаяСтруктура = ОбщегоНазначения.ОбщийМодуль("ОрганизационнаяСтруктура");
		Если МодульОрганизационнаяСтруктура.ТребуетсяУсловноеОформлениеСтруктурыПредприятия() Тогда
			УправлениеШтатнымРасписаниемФормы.УстановитьУсловноеОформлениеСпискаПодразделений(
				ЭтотОбъект, "СтруктураПредприятия", "СформированоНаСегодня", "РасформированоНаСегодня");
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		...
		МодульРабочиеМестаОхраныТрудаФормы.ПриСозданииФормСпискаИлиВыбораСотрудников(ЭтотОбъект, "СписокКоличествоСтавок");
	КонецЕсли;
	
	ИнициализироватьСписок();
	
	Для Каждого ЭлементОтбора Из СтруктураПредприятия.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
	...
	Если Не Отборы.ПоказыватьТолькоДоговорниковГПХ
		И (Не Параметры.Свойство("Отбор") Или Не Параметры.Отбор.Свойство("ПоказыватьДоговорниковГПХ")) Тогда
		
		ЭлементСписокДоговорникиГПХ = Элементы.Найти("СписокДоговорникиГПХ");
		Если ЭлементСписокДоговорникиГПХ <> Неопределено
			И ЭлементСписокДоговорникиГПХ.Видимость Тогда
			
			ОтборПоказыватьДоговорниковГПХ = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ФормаСпискаСотрудников", "ПоказыватьДоговорниковГПХ", Неопределено);
			Если ОтборПоказыватьДоговорниковГПХ <> Неопределено
				И Отборы.ПоказыватьДоговорниковГПХ <> ОтборПоказыватьДоговорниковГПХ Тогда
				
				ОтборыФормы = Новый Структура(Отборы);
				ОтборыФормы.ПоказыватьДоговорниковГПХ = ОтборПоказыватьДоговорниковГПХ;
				Отборы = Новый ФиксированнаяСтруктура(ОтборыФормы);
				УстановитьОтборПоДоговорникамГПХ(ЭтотОбъект, Отборы.ПоказыватьДоговорниковГПХ, Отборы.ПоказыватьТолькоДоговорниковГПХ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ИнтеграцияС1САрхивомЗарплатаКадрыРасширенная") Тогда
		МодульИнтеграцияС1САрхивом = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияС1САрхивомРасширенный");
		МодульИнтеграцияС1САрхивом.ПриСозданииНаСервереФормыСписка(ЭтотОбъект, "Список", "Сотрудник");
	КонецЕсли;
...
Процедура ДоговорникиГПХ(Команда)
	...
	УстановитьОтборПоДоговорникамГПХ(ЭтотОбъект, Не Элементы.СписокДоговорникиГПХ.Пометка, Отборы.ПоказыватьТолькоДоговорниковГПХ, Истина);
...
Функция ДоступныеДляОтбораПоля()
	...
	ДоступныеДляОтбораПоля.Вставить("ПодключенКЭДО");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		...
		МодульРабочиеМестаОхраныТруда.ДополнитьПоляРабочимМестом(ПараметрОрганизация, ДоступныеДляОтбораПоля);
	КонецЕсли;
...
Функция ПоляИнтерактивныхОтборовСписка()
	...
	ПоляОтборов.Вставить("ПодключенКЭДО");
...
Процедура ЗаполнитьКадровыеДанные(НастройкиСписка, СтрокиСписка)
	...
	ЗаполнитьПолучаемыеПоля(НастройкиСписка, ДетальныеЗаписи, СтрокиСписка[СтрокиСписка.ПолучитьКлючи()[0]].Данные);
...
Процедура ЗаполнитьОтборПолучаемыхДанных(ОтборПолученияДанных, ОтборДинамическогоСписка)
	...
	Для Каждого ЭлементОтбораСписка Из ОтборДинамическогоСписка.Элементы Цикл
		...
		Если ТипЗнч(ЭлементОтбораСписка) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			Если ЭлементОтбораСписка.ЛевоеЗначение <> Новый ПолеКомпоновкиДанных("ПодключенКЭДО") Тогда
...
&НаСервереБезКонтекста
Процедура ЗаполнитьПолучаемыеПоля(Настройки, ДетальныеЗаписи, ПоляДинамическогоСписка)
	
	ДоступныеПоля = ОписаниеПолейСписка(Настройки);
...
&НаСервереБезКонтекста
Функция ОписаниеПолейСписка(Настройки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		МодульРабочиеМестаОхраныТруда = ОбщегоНазначения.ОбщийМодуль("РабочиеМестаОхраныТруда");
		МодульРабочиеМестаОхраныТруда.ДополнитьОписаниеПолейРабочимМестом(ЗначениеПараметраОрганизация(Настройки), ОписаниеПолей, ПоляОписания);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЗначениеПараметраОрганизация(Настройки)

	Организация = Неопределено;
	
	ПараметрФилиал = Настройки.ПараметрыДанных.Элементы.Найти("Филиал");
	ПараметрОрганизация = Настройки.ПараметрыДанных.Элементы.Найти("Организация");
	Если ПараметрФилиал.Использование Тогда
		Организация = ПараметрФилиал.Значение;
	ИначеЕсли ПараметрОрганизация.Использование Тогда
...
Процедура УстановитьОтборПоПериоду(Форма)
	...
	Если Форма.ОтбиратьРаботающихВПериоде Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.СтруктураПредприятия, "ДатаОкончания", Форма.ДатаОкончания, Истина);
	...
	Иначе
		...
		Иначе
			...
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.СтруктураПредприятия, "ДатаОкончания", Форма.ДатаНачала, Истина);
			
		КонецЕсли;
		
	КонецЕсли;
	
	СкрыватьУволенных = Форма.Элементы.Список.РежимВыбора
		И (Форма.ОтбиратьРаботающихВПериоде
			Или Не (Форма.Отборы <> Неопределено И Форма.Отборы.ПоказыватьТолькоДоговорниковГПХ));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		Форма.Список,
		"ВидСобытияУвольнение",
		ПредопределенноеЗначение("Перечисление.ВидыКадровыхСобытий.Увольнение"),
		СкрыватьУволенных);
...
Процедура ПриИзмененииВспомогательногоОтбора(Форма, ИмяКомандыВспомогательногоОтбора, СохранитьВНастройках = Истина)
	...
	ОтборПоПодразделениям = Форма.ПодразделениеИспользование Или (ИмяКомандыВспомогательногоОтбора = "ПоПодразделениям");
	ОтборПоСтруктуреПредприятия = (ИмяКомандыВспомогательногоОтбора = "ПоСтруктуреПредприятия");

	Если ОтборПоПодразделениям Тогда
		УстановитьОтборПоСтруктуреПредприятия(Форма, Новый Массив);
		УстановитьОтборПоПодразделению(Форма);
	ИначеЕсли ОтборПоСтруктуреПредприятия Тогда
		УстановитьОтборПоПодразделению(Форма, Новый Массив);
...
Процедура УстановитьОтборСотрудниковПоГруппам(Форма, АдресХранилища = Неопределено)
	...
	Если АдресХранилища = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
			Список, "СписокСотрудников", Неопределено, Ложь);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтборПоДоговорникамГПХ(Форма, Значение, ПоказыватьТолькоДоговорниковГПХ, СохранитьНастройку = Ложь)
	...
	Если СохранитьНастройку И Не ПоказыватьТолькоДоговорниковГПХ Тогда
		
		ЭлементСписокДоговорникиГПХ = Элементы.Найти("СписокДоговорникиГПХ");
		Если ЭлементСписокДоговорникиГПХ <> Неопределено
			И ЭлементСписокДоговорникиГПХ.Видимость Тогда
		
			ЗапомнитьОтборПоказыватьДоговорниковГПХ(Значение);
		КонецЕсли;
...
Процедура УстановитьОтборыПараметровФормы()
	...
	Если Параметры.Свойство("Отбор") Тогда
		...
		Если Параметры.Отбор.Свойство("ПодключенКЭДО") Тогда
			
			Если ПолучитьФункциональнуюОпцию("ИспользуетсяЭДОБЗККабинетСотрудника") Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
					Список, "ПодключенКЭДО", Параметры.Отбор.ПодключенКЭДО, ВидСравненияКомпоновкиДанных.ВСписке,
					НСтр("ru = 'Подключен к ЭДО'"), Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ);
				СписокНастройкиОтбора.Добавить(НСтр("ru = 'Поключенные к ЭДО'"));
			КонецЕсли;
			Параметры.Отбор.Удалить("ПодключенКЭДО");
		...
		СписокНастройкиОтбораРабочихМест = Новый Массив;
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
			ПараметрОрганизация = Неопределено;
			Если ФилиалИспользование Тогда
				ПараметрОрганизация = Филиал;
			ИначеЕсли ОрганизацияИспользование Тогда
				ПараметрОрганизация = Организация;
			КонецЕсли;
			МодульРабочиеМестаОхраныТрудаФормы = ОбщегоНазначения.ОбщийМодуль("РабочиеМестаОхраныТрудаФормы");
			МодульРабочиеМестаОхраныТрудаФормы.УточнитьПараметрыОтбораФормыВыбораСотрудников(ЭтотОбъект, ПараметрОрганизация, СписокНастройкиОтбораРабочихМест);
		КонецЕсли;
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокНастройкиОтбора, СписокНастройкиОтбораРабочихМест);
...
&НаСервереБезКонтекста
Процедура ЗапомнитьОтборПоказыватьДоговорниковГПХ(Значение)
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ФормаСпискаСотрудников", "ПоказыватьДоговорниковГПХ", Значение);
