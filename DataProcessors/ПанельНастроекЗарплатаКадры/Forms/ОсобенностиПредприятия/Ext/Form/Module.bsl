...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолучитьНастройкиОбразованиеГосслужащих();
...
&НаКлиенте
Процедура ИспользоватьОбразовательныеСертификатыПриИзменении(Элемент)

	УстановитьВидимостьЭлементовФормыОбразованиеГосслужащих(ЭтаФорма);
...
Процедура ВыключитьНастройкиГосударственнойСлужбы()
	...
	ВыключитьНастройкуПротиводействиеКоррупции();
	ИспользоватьОбразовательныеСертификаты = Ложь;
	КонтролироватьОстаткиПоказателейДПО = Ложь;
...
Процедура ПолучитьНастройкиПрограммы()
	...
	Если ИспользоватьГосударственнуюСлужбу Тогда
		...
		ИспользоватьОбразовательныеСертификаты = НастройкиПриложения.ИспользоватьОбразовательныеСертификаты; 
		КонтролироватьОстаткиПоказателейДПО    = НастройкиПриложения.КонтролироватьОстаткиПоказателейДПО;
...
Процедура ОбновитьФормуПоНастройкам(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
		"ГруппаОбразованиеГосслужащих", "Доступность", Форма.ИспользоватьГосударственнуюСлужбу);
	...
	УстановитьВидимостьЭлементовФормыОбразованиеГосслужащих(Форма);
...
Функция ЗаписатьНастройкиНаСервере()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие.ОбразованиеГосслужащих") Тогда
		...
		МодульОбразованиеГосслужащих.ПанельЗарплатаКадрыЗаполнитьНастройки(ЭтаФорма, ПараметрыНастроек);
	КонецЕсли;
...
&НаСервере
Процедура ПолучитьНастройкиОбразованиеГосслужащих()
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗарплатаКадрыКорпоративнаяПодсистемы")
		И ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие.ОбразованиеГосслужащих") Тогда 
		
		МодульОбразованиеГосслужащих = ОбщегоНазначения.ОбщийМодуль("ОбразованиеГосслужащих");
		
		НастройкаДоступна = МодульОбразованиеГосслужащих.ДоступноИзменениеИспользованияФункционалаОбразовательныхСертификатов();
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ГруппаОбразованиеГосслужащих",
			"Видимость",
			НастройкаДоступна);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьЭлементовФормыОбразованиеГосслужащих(Форма)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		"КонтролироватьОстаткиПоказателейДПО",
		"Видимость",
		Форма.ИспользоватьОбразовательныеСертификаты);
