...
&НаКлиенте
Перем НомерПопыткиПолученияВыписки; // Номер попытки получения выписки из банка
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ВидОперации = "ПолучениеВыписки" Тогда
		...
		Если ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы.ПолучениеВыписокАсинхронныйОбмен;
			Элементы.КартинкаПолученияВыписокАсинхронныйОбмен.Картинка = БиблиотекаКартинок.ДлительнаяОперация16;
			Элементы.ПрогрессПолученияВыписокАсинхронныйОбмен.Заголовок =
				СтрШаблон(НСтр("ru = '0/%1'"), СписокСообщенийОбмена.Количество());
			Элементы.ТекстПредупрежденияПриПолученииВыпискиАсинхронныйОбмен.Видимость = Ложь;
		КонецЕсли;
	...
	МаксимальноеКоличествоПопыток = ОбменСБанкамиСлужебный.КоличествоПопытокЗапросаВыписок(
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения,
	...
	Если ПроизошлаОшибка Тогда
		...
		Если ЗначениеЗаполнено(ТекстСообщенийПользователю) Тогда
			Элементы.ТекстОшибкиЗапросаВыписки.Видимость = Истина;
			Элементы.ТекстОшибкиЗапросаВыписки.Заголовок = ТекстСообщенийПользователю;
		КонецЕсли;
	ИначеЕсли Не ЗавершениеРаботы И ВидОперации = "ПолучениеВыписки" И Не ПолученыВсеВыписки Тогда
		Если ОписаниеОповещенияОЗакрытии <> Неопределено Тогда
			ВыполнитьОбработкуОповещения(ОписаниеОповещенияОЗакрытии, РезультатПолученияВыписки);
...
&НаКлиенте
Процедура ГиперссылкаОшибкаВЖурналеРегистрацииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Отбор = Новый Структура;
	Отбор.Вставить("Уровень", "Ошибка");
	Отбор.Вставить("Данные", НастройкаОбмена);
...
Процедура ОбработатьРезультатЗапросаВыписки(РезультатЗадания,
	...
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗадания.КраткоеПредставлениеОшибки) Тогда
			Элементы.ТекстОшибкиЗапросаВыписки.Видимость = Истина;
			Элементы.ТекстОшибкиЗапросаВыписки.Заголовок = РезультатЗадания.КраткоеПредставлениеОшибки;
		Иначе
			Элементы.ТекстОшибкиЗапросаВыписки.Видимость = Ложь;
			Элементы.ТекстОшибкиЗапросаВыписки.Заголовок = "";
		КонецЕсли;
		...
		ЭтоПолноправныйПользователь = ПользователиКлиент.ЭтоПолноправныйПользователь();
		Если ЭтоПолноправныйПользователь Тогда
			Элементы.ГиперссылкаОшибкаВЖурналеРегистрации.Видимость = Истина;
		Иначе
			Элементы.ГиперссылкаОшибкаВЖурналеРегистрации.Видимость = Ложь;
		КонецЕсли;
...
Процедура ПослеСохраненияПолученныхПодписейВыписки(ЕстьОшибка, ДополнительныеПараметры) Экспорт
	...
	КоличествоПолученныхВыписок = ВыпискиБанка.Количество();
	КоличествоЗапрашиваемыхВыписок = СписокСообщенийОбмена.Количество();
	Элементы.ПрогрессПолученияВыписокАсинхронныйОбмен.Заголовок = 
		СтрШаблон(НСтр("ru = '%1/%2'"), КоличествоПолученныхВыписок, КоличествоЗапрашиваемыхВыписок);
	ПолученыВсеВыписки = (КоличествоПолученныхВыписок = КоличествоЗапрашиваемыхВыписок);
	
	Если ПолученыВсеВыписки Или НомерПопыткиПолученияВыписки > МаксимальноеКоличествоПопыток Тогда
		
		Если Не ПолученыВсеВыписки Тогда
			СообщениеПроблемПолученияВыписок = СообщениеПроблемПолученияВыписок(СписокСообщенийОбмена);
			Элементы.КартинкаПолученияВыписокАсинхронныйОбмен.Картинка = БиблиотекаКартинок.Предупреждение;
			
			Элементы.ТекстПредупрежденияПриПолученииВыпискиАсинхронныйОбмен.Видимость = Истина;
			ТекстПредупрежденияПриПолученииВыпискиАсинхронныйОбмен = СообщениеПроблемПолученияВыписок;
			
			ТекущаяОперация = НСтр("ru = 'Получение выписок'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
			Подсистема = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСБанками;
			ОбработкаНеисправностейБЭДВызовСервера.ОбработатьОшибку(ТекущаяОперация, Подсистема,
				СообщениеПроблемПолученияВыписок);
		Иначе
			Элементы.КартинкаПолученияВыписокАсинхронныйОбмен.Картинка = БиблиотекаКартинок.Успешно;
		КонецЕсли;
		
		Оповестить("ОбновитьСостояниеОбменСБанками");
		
		ЗаголовокОповещения = НСтр("ru = '1С:ДиректБанк'");
		ТекстОповещения = НСтр("ru = 'Получено выписок: (%1) из (%2).'");
		ТекстОповещения = СтрШаблон(ТекстОповещения, КоличествоПолученныхВыписок, КоличествоЗапрашиваемыхВыписок);
		...
		Если ВидОперации = "ПробноеПолучениеВыписки" Тогда
			...
			Иначе
				Закрыть(Истина);
			КонецЕсли;
		ИначеЕсли Не ПолученыВсеВыписки Тогда
			РезультатПолученияВыписки = РезультатПолученияВыписки(ВыпискиБанка);
		...
		КонецЕсли;
	Иначе
		НомерПопыткиПолученияВыписки = НомерПопыткиПолученияВыписки + 1;
...
Функция ВыпискиПоЗапросам(Знач НастройкаОбмена, Знач СписокЗапросов)
	...
	Возврат ОбменСБанкамиСлужебный.ВыпискиПоЗапросам(НастройкаОбмена, СписокЗапросов);
...
Процедура ПослеПолученияНовыхДокументовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ВыпискиБанка.Количество() = 0 Тогда // Произошла ошибка
			ЗакрытьФормуНаКлиенте(Истина);
		ИначеЕсли ВыпискиБанка.Количество() Тогда
			ТекстСообщения = СообщениеПроблемПолученияВыписок(СписокСообщенийОбмена);
...
&НаСервереБезКонтекста
Функция СообщениеПроблемПолученияВыписок(СписокСообщенийОбмена)
	
	Возврат ОбменСБанкамиСлужебный.СообщениеПроблемПолученияВыписок(СписокСообщенийОбмена);
...
НомерПопыткиПолученияВыписки = 1;
