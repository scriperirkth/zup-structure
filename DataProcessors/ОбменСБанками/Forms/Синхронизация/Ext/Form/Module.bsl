	...
	Для Каждого ЭлементКоллекции Из НастройкиОбмена Цикл
		Если ЭлементКоллекции.Выбран И Не ЭлементКоллекции.Выполнено Тогда
			...
			#Если МобильныйКлиент Тогда
				...
				ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(ТекущаяНастройкаОбмена, Реквизиты);
				Если Не ОбменСБанкамиСлужебныйКлиент.ДоступнаРаботаНаМобильномКлиенте(Реквизиты.ПрограммаБанка) Тогда
					...
					ТекстСообщения = НСтр("ru = 'Для %1 синхронизация не выполнена и будет пропущена.'");
					ТекстСообщения = СтрШаблон(ТекстСообщения, Строка(ТекущаяНастройкаОбмена));
					ТекстСообщения = ТекстСообщения + Символы.ПС
						+ ОбменСБанкамиСлужебныйКлиентСервер.ТекстПредупрежденияМобильныйКлиентНедоступнаРабота(
			...
			ОбменСБанкамиСлужебныйКлиент.СинхронизироватьСБанком(
				Оповещение, ТекущаяНастройкаОбмена, Ложь, ПослеЗапускаДлительнойОперации);
			Прервать;
