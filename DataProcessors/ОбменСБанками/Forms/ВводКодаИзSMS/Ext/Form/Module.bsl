...
Процедура Подключаемый_ПерейтиПоСсылке()
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(URL, Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеОткрытияНавигационнойСсылки(ПриложениеЗапущено, ДополнительныеПараметры) Экспорт
	
	Если Не ПриложениеЗапущено Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось открыть веб-страницу для подтверждения платежа.'");
		ТекстОшибки = НСтр("ru = 'Не найден браузер по умолчанию или произошел сбой при его запуске.
			|Проверьте настройки в операционной системе приложений по умолчанию для открытия по HTTP/HTTPS.'");
		ПоказатьОшибку(ЗаголовокОшибки, ТекстОшибки);
	КонецЕсли;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	// Явно отключаем обработчик ожидания, чтобы не запускать новое фоновое задание.
	ОтключитьОбработчикОжидания("Подключаемый_НачалоПолученияДокументов");
