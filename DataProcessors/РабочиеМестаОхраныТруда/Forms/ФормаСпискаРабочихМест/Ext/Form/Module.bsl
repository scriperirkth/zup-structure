...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыРазмещения.Источники = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Метаданные.Справочники.РабочиеМестаОхраныТруда);
	...
	ИмяЗаменяемойФормы = "Справочник.РабочиеМестаОхраныТруда.Форма.ФормаСписка";
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КлассыУсловийТруда.Должность КАК Должность,
		|	КлассыУсловийТруда.КлассУсловийТруда КАК КлассУсловийТруда
		|ИЗ
		|	РегистрСведений.КлассыУсловийТрудаПоДолжностям.СрезПоследних(, Должность В (&Должности)) КАК КлассыУсловийТруда";
	Запрос.УстановитьПараметр("Должности", Ключи);

	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтрокаСписка = Строки[Выборка.Должность];
		Если СтрокаСписка <> Неопределено Тогда
			СтрокаСписка.Данные["КлассУсловийТруда"] = Выборка.КлассУсловийТруда;
		КонецЕсли;
	КонецЦикла;
...
Процедура НастроитьДинамическийСписок()
	...
	ДоступныПсихиатрические = ДоступныМедосмотры
		И ПолучитьФункциональнуюОпцию("ИспользоватьПсихиатрическиеОсвидетельствования", Новый Структура("Организация", ОтборОрганизация));
	ДоступныСИЗ = ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ПериодичностьВыдачиСИЗРабочихМест);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаНормыВыдачиСИЗ", "Видимость", ДоступныСИЗ);
	...
	Если ДоступныСИЗ Тогда
		МассивПолейЗапроса.Добавить("
			|	ПериодичностьВыдачиПереопределяемый.СИЗПолноеПредставление КАК СИЗПредставление,
			|	ПериодичностьВыдачиПереопределяемый.ПредставлениеПериодичностиВыдачи КАК ПредставлениеПериодичностиВыдачи");
		МассивСоединений.Добавить("
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПериодичностьВыдачиСИЗРабочихМест КАК ПериодичностьВыдачиПереопределяемый
			|		ПО РабочиеМестаПереопределяемый.Ссылка = ПериодичностьВыдачиПереопределяемый.РабочееМесто");
	Иначе
		МассивПолейЗапроса.Добавить("
			|	НЕОПРЕДЕЛЕНО КАК СИЗПредставление,
			|	НЕОПРЕДЕЛЕНО КАК ПредставлениеПериодичностиВыдач");
		МассивСоединений.Добавить("");
	КонецЕсли;
	...
	Если ДоступныПсихиатрические Тогда
		МассивПолейЗапроса.Добавить("
	...
	ТекстЗапроса = 
		...
		|	НЕОПРЕДЕЛЕНО КАК КлассУсловийТруда
		|ИЗ
		|	Справочник.РабочиеМестаОхраныТруда КАК РабочиеМестаПереопределяемый
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиОхраныТруда КАК НастройкиОхраныТрудаПереопределяемый
		|		ПО РабочиеМестаПереопределяемый.Владелец = НастройкиОхраныТрудаПереопределяемый.Организация
		|		И (НЕ НастройкиОхраныТрудаПереопределяемый.ИспользоватьРабочиеМестаОхраныТруда)
		|		&ТекстСоединений
		|ГДЕ
		|	ВЫБОР
		...
		|		КОНЕЦ
		|	И НастройкиОхраныТрудаПереопределяемый.ИспользоватьРабочиеМестаОхраныТруда ЕСТЬ NULL";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстПолейЗапроса", СтрСоединить(МассивПолейЗапроса, ","));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстСоединений", СтрСоединить(МассивСоединений, Символы.ПС));
	...
	ИзменяемыеПоля.Добавить("КлассУсловийТруда");
