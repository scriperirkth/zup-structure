...
Функция ЗапросРабочееВремяПоИсточникамДанных(ИмяВТФильтр, ОписаниеТаблицыНаборовВидовВремени, ТребованияПолученияДанных, ПолучатьДетальныеЗаписиПоТерриторияУсловияТруда = Ложь)		
	...
	Если ТребованияПолученияДанных.ИспользоватьОграничениеСверхурочно1_5 Тогда  
		
		ОписаниеПакета.Параметры.Вставить("Дата144ФЗ", ЗарплатаКадрыПовтИсп.ДатаВступленияВСилуНА("ФедеральныйЗаконОт25мая2026Номер144ФЗ"));
		
		//Отбор записей по сверхурочному времени в ТаблицаИсходныхИсточниковДанных 
		ШаблонИсточникиДанныхСверхурочногоВремени = 
		"ВЫБРАТЬ
		|	ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени КАК ИсточникДанныхФактическогоВремени,
		|	ТаблицаИсходныхИсточниковДанных.ПериодРегистрацииФактическогоВремени КАК ПериодРегистрацииФактическогоВремени,
		|	ТаблицаИсходныхИсточниковДанных.ОбщийГрафик КАК ОбщийГрафик,
		...
		|	ТаблицаИсходныхИсточниковДанных.ДатаОкончания КАК ДатаОкончания,
		|	ТаблицаИсходныхИсточниковДанных.КлючНаборовВидовВремени КАК КлючНаборовВидовВремени
		|ПОМЕСТИТЬ ВТИсточникиДанныхСверхурочногоВремени
		|ИЗ
		|	ВТТаблицаИсходныхИсточниковДанных КАК ТаблицаИсходныхИсточниковДанных
		|ГДЕ
		|	ТаблицаИсходныхИсточниковДанных.ПолучатьСверхурочныеЧасы"; 
		
		ОписаниеЗапросаВТИсточникиДанныхСверхурочногоВремени = ЗарплатаКадрыПериодическиеРегистры.ОписаниеЗапросаПоТексту(ШаблонИсточникиДанныхСверхурочногоВремени);	
		ОператорЗапросаВТИсточникиДанныхСверхурочногоВремени = ОписаниеЗапросаВТИсточникиДанныхСверхурочногоВремени.Операторы[0];  
		
		ЗарплатаКадрыПериодическиеРегистры.ЗаменитьТаблицуВОператореЗапроса(
		ОператорЗапросаВТИсточникиДанныхСверхурочногоВремени, 
		"ТаблицаИсходныхИсточниковДанных", 
		ИмяВТФильтр);
		
		ОписаниеПакета.ЗапросыПолученияДанных.Вставить(0, ОписаниеЗапросаВТИсточникиДанныхСверхурочногоВремени);
		ДобавитьВременныеТаблицыКУничтожению("ВТИсточникиДанныхСверхурочногоВремени");
		
		//ВТ Фильтр возможных ключей отбора сверхурочного времени 
		ШаблонКлючиНаборовВидовВремениСверхурочно = 
		"ВЫБРАТЬ
		|	НачисленияВидыВремени.Ссылка КАК КлючНабораВидаВремениСверхурочно
		|ПОМЕСТИТЬ ВТКлючиНаборовВидовВремениСверхурочно
		|ИЗ
		|	ПланВидовРасчета.Начисления.ВидыВремени КАК НачисленияВидыВремени
		|ГДЕ
		|	НачисленияВидыВремени.ВидВремени В
		|			(ВЫБРАТЬ
		|				ВидыСверхурочногоВремени.Ссылка
		|			ИЗ
		|				Справочник.ВидыИспользованияРабочегоВремени КАК ВидыСверхурочногоВремени
		|			ГДЕ
		|				ВидыСверхурочногоВремени.ОсновноеВремя = &Сверхурочные)";
		
		ОписаниеЗапросаВТКлючиНаборовВидовВремениСверхурочно = ЗарплатаКадрыПериодическиеРегистры.ОписаниеЗапросаПоТексту(ШаблонКлючиНаборовВидовВремениСверхурочно);
		ОписаниеПакета.ЗапросыПолученияДанных.Вставить(1, ОписаниеЗапросаВТКлючиНаборовВидовВремениСверхурочно);
		ДобавитьВременныеТаблицыКУничтожению("ВТКлючиНаборовВидовВремениСверхурочно");
		
		//Осталось сверхурочных часов, допустимых к оплате с коэффициентом 1,5, на начало периода расчета
		ШаблонОстатокСверхурочныхЧасовДоПредела = 
		"ВЫБРАТЬ
		|	ОтработаноСверхурочноСНачалаГода.ИсточникДанныхФактическогоВремени КАК ИсточникДанныхФактическогоВремени,
		|	ОтработаноСверхурочноСНачалаГода.Месяц КАК Месяц,
		|	ОтработаноСверхурочноСНачалаГода.ДатаНачала КАК ДатаНачала,
		|	ОтработаноСверхурочноСНачалаГода.ДатаОкончания КАК ДатаОкончания,
		|	ВЫБОР
		|		КОГДА 120 - ОтработаноСверхурочноСНачалаГода.ОтработаноОплаченоЧасовСверхурочно > 0
		|			ТОГДА 120 - ОтработаноСверхурочноСНачалаГода.ОтработаноОплаченоЧасовСверхурочно
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК СверхурочныхЧасовДоПределаНаНачалоПериода
		|ПОМЕСТИТЬ ВТОстатокСверхурочныхЧасовДоПредела
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени КАК ИсточникДанныхФактическогоВремени,
		|		ТаблицаИсходныхИсточниковДанных.ПериодРегистрацииФактическогоВремени КАК ПериодРегистрацииФактическогоВремени,
		...
		|		ТаблицаИсходныхИсточниковДанных.ДатаНачала КАК ДатаНачала,
		|		ТаблицаИсходныхИсточниковДанных.ДатаОкончания КАК ДатаОкончания,
		|		СУММА(ЕСТЬNULL(НачисленияФПД.ОтработаноЧасов, 0)) КАК ОтработаноОплаченоЧасовСверхурочно
		|	ИЗ
		|		ВТИсточникиДанныхСверхурочногоВремени КАК ТаблицаИсходныхИсточниковДанных
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.Начисления.ФактическийПериодДействия КАК НачисленияФПД
		|			ПО ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени = НачисленияФПД.Сотрудник
		|				И (НачисленияФПД.ПериодДействияНачало >= НАЧАЛОПЕРИОДА(ТаблицаИсходныхИсточниковДанных.Месяц, ГОД))
		|				И (НачисленияФПД.ПериодДействияКонец < ТаблицаИсходныхИсточниковДанных.Месяц)
		|	ГДЕ
		|		НачисленияФПД.ВидРасчета В
		|				(ВЫБРАТЬ
		|					КлючиНаборовВидовВремениСверхурочно.КлючНабораВидаВремениСверхурочно
		|				ИЗ
		|					ВТКлючиНаборовВидовВремениСверхурочно КАК КлючиНаборовВидовВремениСверхурочно)
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ТаблицаИсходныхИсточниковДанных.ДатаНачала,
		|		ТаблицаИсходныхИсточниковДанных.ДатаОкончания,
		|		ТаблицаИсходныхИсточниковДанных.Месяц,
		|		ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени,
		|		ТаблицаИсходныхИсточниковДанных.ПериодРегистрацииФактическогоВремени) КАК ОтработаноСверхурочноСНачалаГода
		|
		|СГРУППИРОВАТЬ ПО
		|	ОтработаноСверхурочноСНачалаГода.ИсточникДанныхФактическогоВремени,
		|	ОтработаноСверхурочноСНачалаГода.Месяц,
		|	ОтработаноСверхурочноСНачалаГода.ДатаНачала,
		|	ОтработаноСверхурочноСНачалаГода.ДатаОкончания,
		|	ВЫБОР
		|		КОГДА 120 - ОтработаноСверхурочноСНачалаГода.ОтработаноОплаченоЧасовСверхурочно > 0
		|			ТОГДА 120 - ОтработаноСверхурочноСНачалаГода.ОтработаноОплаченоЧасовСверхурочно
		|		ИНАЧЕ 0
		...
		ОписаниеЗапросаВТОстатокСверхурочныхЧасовДоПредела = ЗарплатаКадрыПериодическиеРегистры.ОписаниеЗапросаПоТексту(ШаблонОстатокСверхурочныхЧасовДоПредела);	
		
		ОписаниеПакета.ЗапросыПолученияДанных.Вставить(2, ОписаниеЗапросаВТОстатокСверхурочныхЧасовДоПредела);
		ДобавитьВременныеТаблицыКУничтожению("ВТОстатокСверхурочныхЧасовДоПредела"); 
		
		//Сверхурочное время за месяц нарастающим итогом
		ШаблонДанныеСверхурочногоВремениЗаМесяц = 
		"ВЫБРАТЬ
		|	ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени КАК ИсточникДанныхФактическогоВремени,
		|	ТаблицаИсходныхИсточниковДанных.ПериодРегистрацииФактическогоВремени КАК ПериодРегистрацииФактическогоВремени,
		|	ТаблицаИсходныхИсточниковДанных.ОбщийГрафик КАК ОбщийГрафик,
		...
		|			ИНАЧЕ 0
		|		КОНЕЦ КАК ОтработаноСверхурочно
		|ПОМЕСТИТЬ ВТСверхурочноЗаМесяцПредварительно
		|ИЗ
		|	ВТИсточникиДанныхСверхурочногоВремени КАК ТаблицаИсходныхИсточниковДанных
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ПодневныеДанныеУчетаВремени
		|		ПО (ПодневныеДанныеУчетаВремени.ГрафикРаботы = ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени)
		|			И (ПодневныеДанныеУчетаВремени.Месяц = ТаблицаИсходныхИсточниковДанных.Месяц)
		|			И (ПодневныеДанныеУчетаВремени.ПериодРегистрации = ТаблицаИсходныхИсточниковДанных.ПериодРегистрацииФактическогоВремени) 
		|			И (ПодневныеДанныеУчетаВремени.Дата <= ТаблицаИсходныхИсточниковДанных.ДатаОкончания) 
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовРасчета.Начисления.ВидыВремени КАК ТаблицаНаборовВидовВремени
		|		ПО 
		|			(ТаблицаНаборовВидовВремени.Ссылка = ТаблицаИсходныхИсточниковДанных.КлючНаборовВидовВремени)
		|			И (ТаблицаНаборовВидовВремени.ВидВремени = ПодневныеДанныеУчетаВремени.ВидУчетаВремени
		|				ИЛИ &ВыделятьНерабочиеДниОтдельнымВидомВремени
		|					И ТаблицаНаборовВидовВремени.ВидВремени = &Явка
		|					И ПодневныеДанныеУчетаВремени.ВидУчетаВремени = &ОплачиваемыеНерабочиеДни)
		...
		ОписаниеЗапросаВТОтработаноСверхурочноМесяц = ЗарплатаКадрыПериодическиеРегистры.ОписаниеЗапросаПоТексту(ШаблонДанныеСверхурочногоВремениЗаМесяц);	
		ОператорЗапросаВТОтработаноСверхурочноМесяц = ОписаниеЗапросаВТОтработаноСверхурочноМесяц.Операторы[0];   
		
		Если ПолучатьДетальныеЗаписиПоТерриторияУсловияТруда Тогда
			ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаВТОтработаноСверхурочноМесяц, 0, "ЕСТЬNULL(ПодневныеДанныеУчетаВремени.Территория, ЗНАЧЕНИЕ(Справочник.ТерриторииВыполненияРабот.ПустаяСсылка))", "Территория");	
			ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаВТОтработаноСверхурочноМесяц, 0, "ЕСТЬNULL(ПодневныеДанныеУчетаВремени.УсловияТруда, ЗНАЧЕНИЕ(Справочник.УсловияТруда.ПустаяСсылка))", "УсловияТруда");	
		Иначе
			ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаВТОтработаноСверхурочноМесяц, 0, "ЕСТЬNULL(ПодневныеДанныеУчетаВремени.Территория, ЗНАЧЕНИЕ(Справочник.ТерриторииВыполненияРабот.ПустаяСсылка))", "Территория");	
			ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаВТОтработаноСверхурочноМесяц, 0, "ЕСТЬNULL(ПодневныеДанныеУчетаВремени.УсловияТруда, ЗНАЧЕНИЕ(Справочник.УсловияТруда.ПустаяСсылка))", "УсловияТруда");
		КонецЕсли;	
		
		ОписаниеПакета.ЗапросыПолученияДанных.Вставить(3, ОписаниеЗапросаВТОтработаноСверхурочноМесяц);  
		ДобавитьВременныеТаблицыКУничтожению("ВТСверхурочноЗаМесяцПредварительно"); 
		
		//Сводные данные о сверхурочном времени
		ШаблонСводныеДанныеОСверхурочномВремени = 
		"ВЫБРАТЬ
		|	СверхурочноПоДатам.ИсточникДанныхФактическогоВремени КАК ИсточникДанныхФактическогоВремени,
		|	СверхурочноПоДатам.ПериодРегистрацииФактическогоВремени КАК ПериодРегистрацииФактическогоВремени,
		|	СверхурочноПоДатам.ОбщийГрафик КАК ОбщийГрафик,
		|	СверхурочноПоДатам.КлючНаборовВидовВремени КАК КлючНаборовВидовВремени,
		|	СверхурочноПоДатам.Территория КАК Территория,
		|	СверхурочноПоДатам.УсловияТруда КАК УсловияТруда,
		|	СверхурочноПоДатам.Месяц КАК Месяц,
		|	СверхурочноПоДатам.ДатаНачала КАК ДатаНачала,
		|	СверхурочноПоДатам.ДатаОкончания КАК ДатаОкончания,
		|	СверхурочноПоДатам.Дата КАК Дата,
		|	СверхурочноПоДатам.ОтработаноСверхурочно КАК ОтработаноСверхурочно,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ОстатокСверхурочныхЧасовДоПредела.СверхурочныхЧасовДоПределаНаНачалоПериода, 120) - СУММА(ЕСТЬNULL(СверхурочноПоДатамИтоги.ОтработаноСверхурочно, 0)) > 0
		|			ТОГДА ЕСТЬNULL(ОстатокСверхурочныхЧасовДоПредела.СверхурочныхЧасовДоПределаНаНачалоПериода, 120) - СУММА(ЕСТЬNULL(СверхурочноПоДатамИтоги.ОтработаноСверхурочно, 0))
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК СверхурочныхЧасовДоПределаНаНачалоДня
		|ПОМЕСТИТЬ ВТСводныеДанныеОСверхурочномВремени
		|ИЗ
		|	ВТСверхурочноЗаМесяцПредварительно КАК СверхурочноПоДатам
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСверхурочноЗаМесяцПредварительно КАК СверхурочноПоДатамИтоги
		|		ПО СверхурочноПоДатам.ИсточникДанныхФактическогоВремени = СверхурочноПоДатамИтоги.ИсточникДанныхФактическогоВремени
		|			И СверхурочноПоДатам.ПериодРегистрацииФактическогоВремени = СверхурочноПоДатамИтоги.ПериодРегистрацииФактическогоВремени
		|			И СверхурочноПоДатам.ОбщийГрафик = СверхурочноПоДатамИтоги.ОбщийГрафик
		|			И СверхурочноПоДатам.КлючНаборовВидовВремени = СверхурочноПоДатамИтоги.КлючНаборовВидовВремени
		|			И СверхурочноПоДатам.Территория = СверхурочноПоДатамИтоги.Территория
		|			И СверхурочноПоДатам.УсловияТруда = СверхурочноПоДатамИтоги.УсловияТруда
		|			И СверхурочноПоДатам.Месяц = СверхурочноПоДатамИтоги.Месяц
		|			И СверхурочноПоДатам.ДатаНачала = СверхурочноПоДатамИтоги.ДатаНачала
		|			И СверхурочноПоДатам.Дата > СверхурочноПоДатамИтоги.Дата
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОстатокСверхурочныхЧасовДоПредела КАК ОстатокСверхурочныхЧасовДоПредела
		|		ПО СверхурочноПоДатам.ИсточникДанныхФактическогоВремени = ОстатокСверхурочныхЧасовДоПредела.ИсточникДанныхФактическогоВремени
		|			И СверхурочноПоДатам.Месяц = ОстатокСверхурочныхЧасовДоПредела.Месяц
		|			И СверхурочноПоДатам.ДатаНачала = ОстатокСверхурочныхЧасовДоПредела.ДатаНачала
		|
		|СГРУППИРОВАТЬ ПО
		|	СверхурочноПоДатам.Дата,
		|	СверхурочноПоДатам.ДатаНачала,
		|	СверхурочноПоДатам.КлючНаборовВидовВремени,
		|	СверхурочноПоДатам.ОбщийГрафик,
		|	СверхурочноПоДатам.Территория,
		|	СверхурочноПоДатам.УсловияТруда,
		|	СверхурочноПоДатам.ОтработаноСверхурочно,
		|	СверхурочноПоДатам.Месяц,
		|	СверхурочноПоДатам.ПериодРегистрацииФактическогоВремени,
		|	СверхурочноПоДатам.ИсточникДанныхФактическогоВремени,
		|	СверхурочноПоДатам.ДатаОкончания,
		|	ОстатокСверхурочныхЧасовДоПредела.СверхурочныхЧасовДоПределаНаНачалоПериода";	
		
		ОписаниеЗапросаВТСводныеДанныеОСверхурочномВремени = ЗарплатаКадрыПериодическиеРегистры.ОписаниеЗапросаПоТексту(ШаблонСводныеДанныеОСверхурочномВремени);	
		ОператорЗапросаВТСводныеДанныеОСверхурочномВремени = ОписаниеЗапросаВТСводныеДанныеОСверхурочномВремени.Операторы[0]; 
		
		ОписаниеПакета.ЗапросыПолученияДанных.Вставить(4, ОписаниеЗапросаВТСводныеДанныеОСверхурочномВремени);  
		ДобавитьВременныеТаблицыКУничтожению("ВТСводныеДанныеОСверхурочномВремени"); 
		
		//Добавим результирующую ВТСводныеДанныеОСверхурочномВремени в запрос детальных записей
		СтруктураЗапроса = Новый Структура;
		СтруктураЗапроса.Вставить("ОписаниеЗапроса", ОписаниеЗапросаВТСводныеДанныеОСверхурочномВремени);
		СтруктураЗапроса.Вставить("Псевдоним", "СводныеДанныеОСверхурочномВремени");
		СтруктураЗапроса.Вставить("Имя", "ВТСводныеДанныеОСверхурочномВремени"); 
		СтруктураЗапроса.Вставить("Тип", "ОписаниеТаблицыЗапроса"); 
		
		ОператорЗапросаДетальныхДанных.Таблицы.Добавить(СтруктураЗапроса);
		ОператорЗапросаДетальныхДанных.ИсточникСоответствие.Вставить("СводныеДанныеОСверхурочномВремени", СтруктураЗапроса);
		
		Соединение = ОператорЗапросаДетальныхДанных.Соединения.Добавить();
		Соединение.ВедущаяТаблица = "ТаблицаИсходныхИсточниковДанных";
		Соединение.ПрисоединяемаяТаблица = "СводныеДанныеОСверхурочномВремени"; 
		Соединение.ПорядокСоединения = 1;
		Соединение.ТипСоединения = "Левое внешнее";
		
		Соединение.Условия = Новый Массив;
		
		УсловиеСоединения = "ТаблицаИсходныхИсточниковДанных.ИсточникДанныхФактическогоВремени = СводныеДанныеОСверхурочномВремени.ИсточникДанныхФактическогоВремени
		|И ТаблицаИсходныхИсточниковДанных.ДатаНачала = СводныеДанныеОСверхурочномВремени.ДатаНачала 
		|И ТаблицаИсходныхИсточниковДанных.ПериодРегистрацииФактическогоВремени = СводныеДанныеОСверхурочномВремени.ПериодРегистрацииФактическогоВремени 
		|И ТаблицаИсходныхИсточниковДанных.КлючНаборовВидовВремени = СводныеДанныеОСверхурочномВремени.КлючНаборовВидовВремени
		|И ПодневныеДанныеУчетаВремени.Территория = СводныеДанныеОСверхурочномВремени.Территория
		|И ПодневныеДанныеУчетаВремени.УсловияТруда = СводныеДанныеОСверхурочномВремени.УсловияТруда
		|И  ПодневныеДанныеУчетаВремени.Дата = СводныеДанныеОСверхурочномВремени.Дата";
		
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьУсловиеСоединения(ОператорЗапросаДетальныхДанных, "СводныеДанныеОСверхурочномВремени", УсловиеСоединения);   
		
		//Добавляем поля в запрос детальных записей
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаДетальныхДанных, 0, "ЕСТЬNULL(СводныеДанныеОСверхурочномВремени.СверхурочныхЧасовДоПределаНаНачалоДня, 0)", "СверхурочныхЧасовДоПределаНаНачалоДня");			
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьГруппировку(ОператорЗапросаДетальныхДанных, "ЕСТЬNULL(СводныеДанныеОСверхурочномВремени.СверхурочныхЧасовДоПределаНаНачалоДня,0)");
		
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаДетальныхДанных, 0, "ЕСТЬNULL(СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно, 0)", "ОтработаноСверхурочно");			
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьГруппировку(ОператорЗапросаДетальныхДанных, "ЕСТЬNULL(СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно,0)");
		
		//Добавляем расчет сверхурочных в пределах 2 часов
		ОператорЗапросаДетальныхДанных.ПсевдонимыПолей.Добавить("ОтработаноСверхурочноВПределах2Часов");
		ПолеСверхурочноВПределах2Часов = 
		"	ВЫБОР
		|	КОГДА ТаблицаИсходныхИсточниковДанных.ДатаНачала < &Дата144ФЗ
		|	ТОГДА ВЫБОР
		|	КОГДА СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно >= 2
		|	ТОГДА 2
		|ИНАЧЕ СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно
		|	КОНЕЦ
		|ИНАЧЕ ВЫБОР
		|	КОГДА СводныеДанныеОСверхурочномВремени.СверхурочныхЧасовДоПределаНаНачалоДня > 0
		|	ТОГДА ВЫБОР
		|	КОГДА СводныеДанныеОСверхурочномВремени.СверхурочныхЧасовДоПределаНаНачалоДня >= СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно
		|	ТОГДА ВЫБОР
		|	КОГДА СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно >= 2
		|	ТОГДА 2
		|ИНАЧЕ СводныеДанныеОСверхурочномВремени.ОтработаноСверхурочно
		|	КОНЕЦ
		|ИНАЧЕ ВЫБОР
		|	КОГДА СводныеДанныеОСверхурочномВремени.СверхурочныхЧасовДоПределаНаНачалоДня >= 2
		|	ТОГДА 2
		|ИНАЧЕ СводныеДанныеОСверхурочномВремени.СверхурочныхЧасовДоПределаНаНачалоДня
		|	КОНЕЦ
		|	КОНЕЦ
		|ИНАЧЕ 0
		|	КОНЕЦ
		...
		ОператорЗапросаДетальныхДанных.ВыбираемыеПоля.Добавить(ПолеСверхурочноВПределах2Часов);
		ОписаниеЗапросаДетальныхДанных.Колонки.Добавить("ОтработаноСверхурочноВПределах2Часов");
		
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьПолеВОписаниеЗапроса(ОписаниеЗапросаГруппировкиПоИнтервалам, 0, "СУММА(ДанныеОВремениПоДатам.ОтработаноСверхурочноВПределах2Часов)", "ОтработаноСверхурочноВПределах2Часов");			
		
		ЗарплатаКадрыПериодическиеРегистры.ДобавитьГруппировку(ОператорЗапросаДетальныхДанных, ПолеСверхурочноВПределах2Часов);
...
Функция НовыйТребованияПолученияЗарегистрированногоВремени()
	...
	Требования.Вставить("ИспользоватьОграничениеСверхурочно1_5", Ложь); 
...
Функция СводныеТребованияПолученияЗарегистрированногоВремени(ТаблицаФильтр)
	...
	Дата144ФЗ = ЗарплатаКадрыПовтИсп.ДатаВступленияВСилуНА("ФедеральныйЗаконОт25мая2026Номер144ФЗ");  
	Для Каждого СтрокаТаблицы Из ТаблицаФильтр Цикл
		...
		ИспользоватьОграничениеСверхурочно1_5 = ?(СтрокаТаблицы.ДатаНачала >= Дата144ФЗ И СтрокаТаблицы.ПолучатьСверхурочныеЧасы, Истина, Ложь);    
		...
		Требования.ИспользоватьОграничениеСверхурочно1_5 = Макс(Требования.ИспользоватьОграничениеСверхурочно1_5, ИспользоватьОграничениеСверхурочно1_5);   
