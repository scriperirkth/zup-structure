...
Процедура Техподдержка(Команда)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		МодульИнтерфейсДокументовЭДОКлиент.ОткрытьОбращениеВСлужбуТехническойПоддержки();
	Иначе
...
Процедура ВывестиОшибки()
	...
	Для каждого Ошибка Из ОбработкаНеисправностейБЭДКлиентСервер.ПолучитьОшибки(КонтекстДиагностики) Цикл
		...
		Если ВидОшибки.ВыводитьСсылкуНаСписокОшибок Тогда
			...
			ШаблонЗаголовка = НСтр("ru = 'См. <a href = ""%1"">список ошибок</a>'");
			СтрокаЗаголовка = СтрШаблон(ШаблонЗаголовка, "ОбработкаНеисправностейБЭДОткрытьСписокОшибок");
			ДекорацияСписокОшибок.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(СтрокаЗаголовка);
