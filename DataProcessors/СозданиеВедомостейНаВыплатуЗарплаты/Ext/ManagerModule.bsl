...
Функция СоздатьВедомостиПоРасчетномуДокументу(РасчетныйДокумент, ПараметрыСоздания) Экспорт
	...
	Для Каждого КлючВедомости Из КлючиВедомостей Цикл
		...
		Финансирование = Новый Структура("СтатьяРасходов, СтатьяФинансирования");
		ЗаполнитьЗначенияСвойств(Финансирование, КлючВедомости);
		Ведомость.ЗагрузитьВыплаты(ЗарплатаВедомости, НДФЛВедомости, Финансирование);
...
Функция ОписаниеОперацииВыплаты(РасчетныйДокумент, СпособВыплаты, ДатаВыплаты)
	...
	//Если расчетный документ Начисление за первую половину месяца, используем выборку из регистра накопления ЗарплатаКВыплатеАвансом
	ТипыДокументовАванса = РасчетЗарплаты.ТипыДокументовНачисленияАванса();
	Если ТипыДокументовАванса[ТипЗнч(РасчетныйДокумент)] <> Неопределено Тогда
...
Процедура СоздатьВТСотрудникиДляВедомости(МенеджерВременныхТаблиц, ОписаниеОперации, СпособВыплаты)
	...
	Если ЗначениеЗаполнено(ИмяВТСотрудники) Тогда
		...
		Если Не Результат.Пустой() Тогда
			ОписаниеОперации.Вставить("ФизическиеЛица", Результат.Выгрузить().ВыгрузитьКолонку("ФизическоеЛицо"));
		КонецЕсли;		
