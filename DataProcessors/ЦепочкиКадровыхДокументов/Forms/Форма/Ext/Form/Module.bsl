...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ИдентификаторСтрокиОснования", ИдентификаторСтрокиОснования);
...
Процедура ОтменитьСвязьСОснованием(Команда)
	...
	Если ОтменитьСвязьНаСервере() Тогда
		СформироватьНаСервере();
		ЦепочкиДокументовКлиент.ОповеститьЗаписанПодчиненныйДокумент(ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуСвязей()
	...
	Пока АнализируемыеДокументы.Количество() > 0 И СчетчикИтераций < 100 Цикл
		...
		Для каждого СтрокаОснование Из Основания Цикл
			НайденныеСтроки = ТаблицаСвязей.НайтиСтроки(Новый Структура("Основание, Подчиненный",
				СтрокаОснование.Основание, СтрокаОснование.Подчиненный));
			Если НайденныеСтроки.Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
			НоваяСвязь = ТаблицаСвязей.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСвязь, СтрокаОснование);
			НоваяСвязь.ВидСвязи = 0;
			НовыеДокументы.Добавить(СтрокаОснование.Основание);
		КонецЦикла;
		
		Подчиненные = ЦепочкиДокументов.ПодчиненныеДокументы(АнализируемыеДокументы, ИдентификаторСтрокиОснования);
		Для каждого СтрокаПодчиненных Из Подчиненные Цикл
			Отбор = Новый Структура("Основание, Подчиненный, ИдентификаторСтрокиОснования", СтрокаПодчиненных.Основание,
				СтрокаПодчиненных.Подчиненный, ИдентификаторСтрокиОснования);
			НайденныеСтроки = ТаблицаСвязей.НайтиСтроки(Отбор);
			Если НайденныеСтроки.Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
			НоваяСвязь = ТаблицаСвязей.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСвязь, СтрокаПодчиненных);
			НоваяСвязь.ВидСвязи = 0;
...
Процедура ДополнитьОснованиями()
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивДляАнализа,
		ТаблицаСвязей.Выгрузить().ВыгрузитьКолонку("Основание"), Истина);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивДляАнализа,
		ТаблицаСвязей.Выгрузить().ВыгрузитьКолонку("Подчиненный"), Истина);
	
	МаксимумИтераций = 10;
	НомерИтерации = 1;
	
	ТаблицаРезультата = НоваяТаблицаСвязей();
...
Процедура СформироватьОтчет()
	...
	Если ТаблицаСвязей.Количество() > 0 Тогда
		Отбор = Новый Структура("Основание, ИдентификаторСтрокиОснования", ТаблицаСвязей[0].Основание,
			ТаблицаСвязей[0].ИдентификаторСтрокиОснования);
		ПодчиненныеДокументы = ТаблицаСвязей.НайтиСтроки(Отбор);
...
Функция СтрокаПериодаНаПечать(ДокументНаПечать)
	...
	Если ЦепочкиДокументов.ЭтоМногосотрудниковыйДокумент(ДокументНаПечать) Тогда
		Возврат "";
	КонецЕсли;
	...
	СтрокаРеквизитов = "";
	Если ЗначениеЗаполнено(ОписаниеДокумента.ИмяРеквизитаДатаНачалаСобытия) Тогда
		СтрокаРеквизитов = СтрокаРеквизитов + ОписаниеДокумента.ИмяРеквизитаДатаНачалаСобытия;
	КонецЕсли;
	Если ЗначениеЗаполнено(ОписаниеДокумента.ИмяРеквизитаДатаОкончанияСобытия) Тогда
		СтрокаРеквизитов = СтрокаРеквизитов + ?(ПустаяСтрока(СтрокаРеквизитов), "", ",")
			+ ОписаниеДокумента.ИмяРеквизитаДатаОкончанияСобытия;
	КонецЕсли;
	
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументНаПечать, СтрокаРеквизитов);
	
	ДатаНачала = ?(ЗначениеЗаполнено(ОписаниеДокумента.ИмяРеквизитаДатаНачалаСобытия),
		ЗначенияРеквизитов[ОписаниеДокумента.ИмяРеквизитаДатаНачалаСобытия], '00010101');
	ДатаОкончания = ?(ЗначениеЗаполнено(ОписаниеДокумента.ИмяРеквизитаДатаОкончанияСобытия),
		ЗначенияРеквизитов[ОписаниеДокумента.ИмяРеквизитаДатаОкончанияСобытия], '00010101');
...
Процедура ДополнитьТекстЗапросаВТДокументы(Запрос, АнализируемыйТипДокумента, ПервыйЗапрос)
	...
	МетаданныеДокумента = Метаданные.НайтиПоТипу(АнализируемыйТипДокумента);
	
	Если Не ОбщегоНазначения.ЕстьРеквизитОбъекта("ИсправленныйДокумент", МетаданныеДокумента) Тогда
		Возврат;
	КонецЕсли;
...
// Параметры:
//  ТаблицаРезультата - см. НоваяТаблицаСвязей
//  НомерСвязи - Число
&НаСервере
Процедура ДополнитьТаблицуСвязей(ТаблицаРезультата, НомерСвязи)
	...
	Запрос.Текст = 
		...
		|	ВТДокументы.ПодчиненныйСотрудник КАК СотрудникПодчиненного,
		|	ВТДокументы.ОснованиеСотрудник КАК СотрудникОснования,
...
//  ТаблицаЗначений - Пустая таблица связей:
//   * Подчиненный - ДокументСсылка
//   * Основание - ДокументСсылка 
//   * ПодчиненныйСотрудник - СправочникСсылка.Сотрудники -
//   * ОснованиеСотрудник - СправочникСсылка.Сотрудники -
&НаСервере
Функция НоваяТаблицаСвязей()
