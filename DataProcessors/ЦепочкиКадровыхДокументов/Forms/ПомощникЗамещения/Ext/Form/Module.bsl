...
Процедура ИнфоНадписьПодвалОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура("ТекущийДокумент, СформироватьПриОткрытии, ИдентификаторСтрокиОснования",
		ДокументСсылка, Истина, ИдентификаторСтрокиОснования);
...
Процедура СоздатьПодчиненныйДокумент(ВидДокумента)
	...
	ПараметрыФормы.Вставить("ЗамещаемыйСотрудник", Сотрудник);
	ПараметрыФормы.Вставить("ИдентификаторСтрокиОснования", ИдентификаторСтрокиОснования);
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Иначе
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры, "Сотрудник,ДатаНачала,ДатаОкончания,ИдентификаторСтрокиОснования");
	КонецЕсли;
...
Процедура ПрочитатьИнформациюОДокументахЗамещения()
	...
	Если ЦепочкиДокументов.ЭтоМногосотрудниковыйДокумент(ДокументОснованиеСсылка) Тогда
		КлючСтроки = Новый Структура("Основание,ИдентификаторСтрокиОснования", ДокументОснованиеСсылка,
			ИдентификаторСтрокиОснования);
		МассивПодчиненныхДокументов = ЦепочкиДокументов.ДокументыПодчиненныеСтроке(ДокументОснованиеСсылка, КлючСтроки);
	Иначе
		МассивПодчиненныхДокументов = ОбщегоНазначения.ТаблицаЗначенийВМассив(ЦепочкиДокументов.ПодчиненныеДокументы(
...
Функция МассивСтрокИнфоНадписиПодчиненных()
	...
	Для каждого ПодчиненныйДокумент Из ПодчиненныеДокументы Цикл
		Если ПодчиненныеДокументы.Найти(ПодчиненныйДокумент) <> 0 Тогда
			ТекстНадписи.Добавить(", ");
		КонецЕсли;
		СсылкаСотрудника = ЦепочкиДокументовФормы.ИнфоНавигационнаяСсылка("ОткрытьДокумент",
			ПодчиненныйДокумент.Подчиненный);
		ТекстСотрудника = СтрШаблон(НСтр("ru = '%1 (%2)'"), ПодчиненныйДокумент.СотрудникПодчиненногоФамилияИО,
			ЦепочкиДокументов.ОписаниеДокументовЗамещения(ПодчиненныйДокумент.Подчиненный).КраткоеНазваниеИменительныйПадеж);
...
Функция МассивСтрокИнфоНадписиОснования()
	...
	СклонениеФИО = СклонениеПредставленийОбъектов.ПросклонятьПредставление(ИнфоДокументОснование.СотрудникОснования, 4);
	
	ТекстСотрудника = ИнфоДокументОснование.СотрудникОснованияФамилияИО;
	ТекстСотрудника = СтрШаблон(НСтр("ru = '%1 (%2)'"),
		ФизическиеЛицаКлиентСервер.ФамилияИнициалы(СклонениеФИО),
		ЦепочкиДокументов.ОписаниеДокументовЗамещения(ИнфоДокументОснование.Основание).КраткоеНазваниеИменительныйПадеж);
	ТекстНадписи.Добавить(Новый ФорматированнаяСтрока(ТекстСотрудника,,,, СсылкаСотрудника));
...
Процедура ПерейтиПоСсылке(НавигационнаяСсылка)
	...
	Закрыть();
	ЦепочкиДокументовКлиент.ОткрытьФормуДокументаЗамещения(ЭтотОбъект,
		ЦепочкиДокументовКлиентСервер.СтруктураНавигационнойСсылки(НавигационнаяСсылка));
...
Процедура УстановитьРеквизитыФормыПоДокументу()
	...
	ТекстРеквизитов = ОписаниеДокумента.ИмяРеквизитаСотрудник + "," + ОписаниеДокумента.ИмяРеквизитаДатаНачалаСобытия
