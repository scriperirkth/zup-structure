...
Процедура Опубликовать(Команда)
	...
	Если НужноЗадатьВопросМножественнаяПубликация() Тогда
		ТекстВопроса = НСтр("ru = 'Указано несколько регионов для публикации.
			|
			|Средства со счета могут быть списаны за публикацию в каждом из регионов. Продолжить?'");
		
		ПоказатьВопрос(Новый ОписаниеОповещения("МножественнаяПубликацияВопрос", ЭтотОбъект),
			ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ОпубликоватьПродолжение();
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если Классификаторы[Сайт] = Неопределено Тогда
...
Процедура МножественныйВыборИзСписка(ТекущиеДанные, СписокВыбора)
	Если ЗначениеЗаполнено(ТекущиеДанные.ПубликацияВакансии) Тогда
		Для Каждого Элемент Из ТекущиеДанные.ПубликацияВакансии Цикл
...
Процедура ЗаполнитьДерево(МассивПолейВакансии)
	...
	ДанныеПубликации = ДанныеПубликации(Вакансия, Сайт, ИдентификаторВакансии);
...
&НаСервере
Функция НужноЗадатьВопросМножественнаяПубликация()
	
	Дерево = РеквизитФормыВЗначение("ДеревоНастроек", Тип("ДеревоЗначений"));
	
	Возврат РекрутинговыеСайты.РегионыДереваПубликацииВакансии(Дерево, ДанныеСайтов[Сайт]).Количество() > 1;
	
КонецФункции

&НаКлиенте
Процедура МножественнаяПубликацияВопрос(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОпубликоватьПродолжение()
...
Функция ОбновитьПубликациюВакансии()
	...
	Если ПроверитьЗаполнениеПолей(Дерево) Тогда
		Возврат РекрутинговыеСайты.ОбновитьПубликациюВакансии(Вакансия, Сайт, Дерево);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СинхронизироватьСвойстваПубликацииВакансии()
	...
	Если РекрутинговыеСайты.СинхронизироватьСвойстваПубликацииВакансии(Вакансия, Сайт, ИдентификаторВакансии) Тогда
		...
		ОбновленныеСвойства = РекрутинговыеСайты.СвойстваПубликацииВакансии(Вакансия, Сайт, ИдентификаторВакансии);
...
&НаСервере
Функция ДанныеПубликации(Вакансия, Сайт, ИдентификаторВакансии)
	...
	|	И ДанныеПубликацииВакансий.ПолеДанных <> &ИмяПоляРегион
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&ИмяПоляРегион,
	|	""[{""""Значение"""":"""""" + ВЫРАЗИТЬ(ПубликацияВакансий.ИдентификаторРегиона КАК СТРОКА(100)) + """""",""""Представление"""":"""""" +
	|		ВЫРАЗИТЬ(ПубликацияВакансий.Регион КАК СТРОКА(100)) + """"""}]""
	|ИЗ
	...
	Запрос.УстановитьПараметр("ИмяПоляРегион", ДанныеСайтов[Сайт].ИмяПоляИдентификатораРегионаВакансии);
