...
Процедура Подбор(Команда)
	...
	КадровыйУчетКлиент.ВыбратьСотрудниковРаботающихНаДатуПоПараметрамОткрытияФормыСписка(
		Элементы.Сотрудники, , , ДатаСеанса,,АдресСпискаПодобранныхСотрудников());
...
Процедура СотрудникиОбработкаВыбораНаСервере(ФизическиеЛица)
	...
	ВыбранныеФизическиеЛица = ВыбранныеФизлицаСотрудников(ФизическиеЛица);
	
	Для Каждого ВыбранноеФизическоеЛицо Из ВыбранныеФизическиеЛица Цикл
		Сотрудники.Добавить().ФизическоеЛицо = ВыбранноеФизическоеЛицо;
	КонецЦикла;
	...
	Для Каждого СтрокаФизическогоЛица Из Сотрудники Цикл 
		...
		Если ВидЗаявки = "ЗаявкаНаКомандировку" Или ВидЗаявки = "ЗаявкаНаОтпуск" Тогда 
			...
			Если Не ЗначениеЗаполнено(СтрокаФизическогоЛица.ДатаНачала) Тогда
				ИндексСтроки = Сотрудники.Индекс(СтрокаФизическогоЛица);
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Работник %1: не указана дата начала.'"), СтрокаФизическогоЛица.ФизическоеЛицо);
			...
			Если Не ЗначениеЗаполнено(СтрокаФизическогоЛица.ДатаОкончания) Тогда
				ИндексСтроки = Сотрудники.Индекс(СтрокаФизическогоЛица);
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Работник %1: не указана дата окончания.'"), СтрокаФизическогоЛица.ФизическоеЛицо);
			...
			Если ЗначениеЗаполнено(СтрокаФизическогоЛица.ДатаНачала) И ЗначениеЗаполнено(СтрокаФизическогоЛица.ДатаОкончания) 
				...
				ИндексСтроки = Сотрудники.Индекс(СтрокаФизическогоЛица);
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Работник %1: дата начала больше даты окончания.'"), СтрокаФизическогоЛица.ФизическоеЛицо);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Сотрудники[" + Формат(ИндексСтроки, "ЧН=0; ЧГ=0") + "].ДатаНачала", , Отказ);
			КонецЕсли;
...
Функция АдресСпискаПодобранныхСотрудников()
	...
	СписокСотрудниковПоФизическимЛицам = ТаблицаСотрудниковИФизлиц(Сотрудники.Выгрузить(,"ФизическоеЛицо").ВыгрузитьКолонку("ФизическоеЛицо"));
