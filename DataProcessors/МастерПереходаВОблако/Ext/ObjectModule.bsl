	...
	Если Результат.Данные.type = "s3" Тогда
		...
		Пока Истина Цикл
			...
			Если Ответ.КодСостояния <> 200 Тогда
				...
				Возврат;
			КонецЕсли;
			Части.Добавить(СтрЗаменить(ОбщегоНазначенияБТСКлиентСервер.ЗаголовокHTTP(Ответ, "ETag"),"""",""));
...
Процедура ПроверитьРасширения1СПлюс(Параметры, АдресРезультата) Экспорт
	Результат = Новый Структура;
	Результат.Вставить("Ошибка", Ложь);
	Результат.Вставить("СообщениеОбОшибке", "");
	
	Попытка
		Результат.Вставить("Данные", ВыгрузкаЗагрузкаДанныхСлужебный.ПроверитьРасширения1СПлюс());
	Исключение
		Результат.Ошибка = Истина;
