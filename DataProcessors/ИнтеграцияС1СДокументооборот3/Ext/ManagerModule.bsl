...
Процедура ВыполнитьПоискПоРеквизитам(ТипОбъекта, РеквизитыПоиска, АдресВоВременномХранилище,
	...
	СписокОбъектовXDTO = ИнтеграцияС1СДокументооборотБазоваяФункциональность.НайтиСписокОбъектов(
		Прокси,
		ТипОбъекта,
		СписокУсловий);
	
	НайденныеОбъекты = Новый Массив;
	Для Каждого ОбъектXDTO Из СписокОбъектовXDTO.items Цикл
		...
		НайденныйОбъект.Вставить("navigationRef", ОбъектXDTO.object.objectID.navigationRef);
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.СвойствоУстановлено(СписокОбъектовXDTO, "tooManyObjects")
			И СписокОбъектовXDTO.tooManyObjects = Истина Тогда
		Представление = Представление
