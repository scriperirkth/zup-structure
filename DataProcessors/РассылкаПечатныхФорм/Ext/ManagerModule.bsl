...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОтправитьДокументыПолучателям", Истина);
КонецПроцедуры
...
Функция ОтправитьДокументыПолучателям(Получатели, ДляПечати, ДляОтправки) Экспорт
	...
	ВыводимыеФайлы = ЭДОБЗКВызовСервера.ВыводимыеФайлыПечатныхФорм(ДляПечати.ВыводимыеФайлы);
	...
	Для Каждого СтрокаПолучателя Из Получатели Цикл
		...
		Если ПечатныеФормы.Количество() > 0 Тогда
			...
			ДляСохраненияВФайл.ФорматыСохранения.Добавить(ЭДОБЗКВызовСервера.ТипФайлаЭлектронногоДокумента());
