...
Процедура НачатьОтправкуДокументов()
	...
	Если ДанныеФормСЭЦП.ФайлыНаПодпись.Количество() > 0 Тогда
		...
		ЭДОБЗККлиент.ПодписатьФайлы(
			ДанныеФормСЭЦП.ФайлыНаПодпись, Оповещение, ЭтотОбъект, ПредопределенноеЗначение("Перечисление.ДействияСФайламиДокументовЭДОБЗК.Подписать"));
...
Функция ДлительнаяОперацияОтправкиДокументов(ВыводимыеФайлы)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"Обработки.РассылкаПечатныхФорм.ОтправитьДокументыПолучателям",
...
Функция ДанныеПечатныхФормСЭЦП()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Если МодульЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи() Тогда
			...
			ОписанияФайловПечатныхФормПоОбъектам = ЭДОБЗК.ОписанияФайловПечатныхФормПоОбъектам(
...
Функция ФайлыНаПодпись(ОбъектыИдентификаторов)
	...
	Для Каждого ОписаниеТипаОбъектов Из ТипыОбъектов Цикл
		...
		Если Не ЭДОБЗК.ЭтоТипОбъектаСПечатнымиФормами(ОписаниеТипаОбъектов.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ОписаниеПечатныеФормы Из ПечатныеФормы Цикл
			...
			Если Не ЭДОБЗККлиентСервер.ЭтоИдентификаторыЭлектронногоДокумента(ОписаниеПечатныеФормы.Идентификатор) Тогда
				Продолжить;
			КонецЕсли;
			...
			ФормыНаПодписьОбъектов = ЭДОБЗКВызовСервера.ПечатныеФормыОбъектов(ПечатныеФормыСсылок,
...
Процедура УстановитьСостояниеПечатныхФормПриУспешномЗавершенииОтправки()
	...
	Для Каждого СтрокаПолучателя Из Получатели Цикл
		Если СтрокаПолучателя.Отправлено <> Истина Тогда
			Продолжить;
		КонецЕсли;
		Для Каждого ПечатнаяФорма Из ВыбранныеПечатныеФормы Цикл
