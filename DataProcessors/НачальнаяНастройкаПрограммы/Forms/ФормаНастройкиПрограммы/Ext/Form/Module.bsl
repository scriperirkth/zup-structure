...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("РаботаВХозрасчетнойОрганизации") И ПустаяСтрока(СистемнаяИнформация.ИнформацияПрограммыПросмотра) И Не РазделениеВключено Тогда
...
Процедура ВыполнитьДействияПередСменойСтраницы(Отмена,Направление)
	...
	ИначеЕсли Направление = 1 И ТекущаяСтраницаИмя = "ПерваяСтраницаПервоначальногоВводаДанных" Тогда
		
		ПорядокСтраницПоНастройкамФормы(ЭтаФорма);
...
Функция НовоеФизическоеЛицо(ДанныеФизическогоЛица, СоответствиеДокументовУдостоверяющихЛичность = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ФизическоеЛицоОбъект.Пол) Тогда
		ФизическоеЛицоОбъект.Пол = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ПолПоЧастямФИО(ДанныеФизическогоЛица);
	КонецЕсли;
...
Функция ТаблицаОрганизацийДляЗагрузкиИзФайлов()
	...
	ТаблицаОрганизаций.Колонки.Добавить("РегистрационныйНомерСФР", Новый ОписаниеТипов("Строка"));
...
Функция ПараметрыОрганизацииСозданиеИзФайловЗагрузки()
	...
	ПараметрыОрганизации.Вставить("РегистрационныйНомерСФР", "");
...
Функция НоваяОрганизация(ПараметрыОрганизации)
	...
	Если ПараметрыОрганизации.Свойство("РегистрационныйНомерПФР")
		Или ПараметрыОрганизации.Свойство("РегистрационныйНомерСФР") Тогда
		РегистрыСведений.ИсторияРегистрацийВОрганеПФР.ЗаписатьДанные(
...
Процедура ДополнитьСведенияОрганизации(ДанныеОрганизации, ПараметрыОрганизации)
	...
	Если ЗначениеЗаполнено(ПараметрыОрганизации.ОГРН) И Не ЗначениеЗаполнено(ОрганизацияОбъектИзФайла.ОГРН) Тогда
		...
		ЗаписыватьОбъект = Истина;
	КонецЕсли;
	Если (ЗначениеЗаполнено(ПараметрыОрганизации.РегистрационныйНомерПФР)
			И Не ЗначениеЗаполнено(ДанныеОрганизации.РегистрационныйНомерПФР))
		Или (ЗначениеЗаполнено(ПараметрыОрганизации.РегистрационныйНомерСФР)
			И Не ЗначениеЗаполнено(ДанныеОрганизации.РегистрационныйНомерСФР)) Тогда
		РегистрыСведений.ИсторияРегистрацийВОрганеПФР.ЗаписатьДанные(
...
Функция ПорядокСтраницПоНастройкамФормы(Форма)
	...
	Иначе
		...
		Иначе
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(Форма.ОрганизацияСсылка) Тогда
...
Процедура ИндивидуальныйПредпринимательФИОПриИзменении(Элемент)
	...
	Если СтруктураФИО.Фамилия <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(ИндивидуальныйПредприниматель.Пол) Тогда
			ИндивидуальныйПредприниматель.Пол = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ПолПоЧастямФИО(СтруктураФИО);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьФормуПервоначальногоВводаПоНастройкам()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,"ГруппаОрганизации","Видимость", ИспользоватьНесколькоОрганизаций);
