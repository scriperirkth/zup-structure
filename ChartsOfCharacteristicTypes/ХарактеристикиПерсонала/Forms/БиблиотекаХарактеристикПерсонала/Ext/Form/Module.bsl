...
Процедура ЗаполнитьТаблицуХарактеристик()
	...
	БиблиотекаТаблица.Свернуть("Наименование, ВидХарактеристики, ШкалаЗначений");
	Для Каждого ТекущаяСтрока Из БиблиотекаТаблица Цикл
...
Процедура СохранитьВыбранныеСтроки(Знач ВыбранныеСтроки, ТекущаяСсылка)
	...
	Для каждого НомерСтроки Из ВыбранныеСтроки Цикл
		...
		СоздаватьЗначенияШкалы = Ложь;
		ШкалаВБазе = Справочники.ШкалыЗначенийХарактеристикПерсонала.НайтиПоНаименованию(ТекущиеДанные.ШкалаЗначений, Истина);
		Если Не ЗначениеЗаполнено(ШкалаВБазе) Тогда
			ШкалаОбъект = Справочники.ШкалыЗначенийХарактеристикПерсонала.СоздатьЭлемент();
			ШкалаОбъект.Наименование = ТекущиеДанные.ШкалаЗначений;
			ШкалаОбъект.Записать();
			ШкалаВБазе = ШкалаОбъект.Ссылка;
			СоздаватьЗначенияШкалы = Истина;
		КонецЕсли;
		...
		НоваяСтрока.ШкалаЗначений = ШкалаВБазе;
		НоваяСтрока.Записать();
		
		Если СоздаватьЗначенияШкалы Тогда
