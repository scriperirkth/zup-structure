...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РольДоступнаАдминистратор = ОбработкаНовостейСлужебный.ЭтоАдминистратор();
	...
	Если ЗначенияКатегорийНовостейСправочник.КомпоновщикНастроек.Настройки.Отбор.Элементы.Количество() = 1 Тогда
		// Может вызвать ошибку в неразделенном режиме в модели сервиса.
		НовыйОтбор = ЗначенияКатегорийНовостейСправочник.КомпоновщикНастроек.Настройки.Отбор.Элементы[0];
		НовыйОтбор.ПравоеЗначение = Объект.Ссылка;
		НовыйОтбор.Использование  = Истина;
	Иначе
		ЗначенияКатегорийНовостейСправочник.Отбор.Элементы.Очистить();
		НовыйОтбор = ЗначенияКатегорийНовостейСправочник.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	...
	ОткрытаИзОбработки_УправлениеНовостями = Параметры.ОткрытаИзОбработки_УправлениеНовостями;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗначенияКатегорийНовостейСправочник.КомпоновщикНастроек.Настройки.Отбор.Элементы[0].ПравоеЗначение = Объект.Ссылка;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ВсеОповещения.КлассификаторыОбновлены Тогда
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Иначе
		Если Объект.ОбновляетсяССервера Тогда
			...
			ЛокальноРазрешенаРаботаСНовостямиЧерезИнтернет        = ПолучитьФункциональнуюОпцию("РазрешенаРаботаСНовостямиЧерезИнтернет");
