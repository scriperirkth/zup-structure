...
Процедура ЗаданиеСправкиНДФЛПриОкончанииРедактированияНаСервере(СправкаКУдалению = Неопределено)
	...
	Если ЗначениеЗаполнено(СправкаКУдалению) Тогда
		ПометитьНаУдалениеДокументЭДОБЗКСправкиНДФЛ(СправкаКУдалению);
	...
	Попытка
		Для Каждого СтрокаТЧ Из Задание.СправкиНДФЛ Цикл	
			...
			Иначе
				...
				БизнесПроцессыЗаявокСотрудников.СоздатьИзменитьДокументЭДОБЗКСправкаСотруднику(ПрисоединенныйФайл, СтрокаТЧ.СправкаНДФЛ);
...
Процедура ЗаполнитьЭлементыФайлаОтвета(ФайлОтветаЭлемент, ФайлОтветаСсылка, ИмяРеквизита, ЗначенияРеквизитов)
	...
	Элементы[ИмяРеквизита + "Подписан"].Видимость = Не ФайлПустой И (ИспользуетсяЭДОБЗК Или ФайлСЭП); 
...
Процедура ЗавершениеВыбратьФайлОтветаНаСервере(ПомещенныйФайл, НомерФайла)
	...
	Попытка
		...
		ПрисоединенныйФайл = РаботаСФайлами.ДобавитьФайл(ПараметрыФайла, АдресХранилища);		
		БизнесПроцессыЗаявокСотрудников.СоздатьИзменитьДокументЭДОБЗКСправкаСотруднику(ПрисоединенныйФайл, СправкаНДФЛ);
		Если ИспользуетсяЭДОБЗК Тогда
			РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.ЗарегистрироватьОбработкуФайлов(
...
Процедура ЗавершитьПодписатьЭПНаСервере(МассивФайловСправок)
	...
	Попытка
		...
		Для Каждого ФайлСправки Из МассивФайловСправок Цикл
			БизнесПроцессыЗаявокСотрудников.СоздатьИзменитьДокументЭДОБЗКСправкаСотруднику(ФайлСправки, РеквизитыСправокНДФЛ[ФайлСправки]);
			РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.ЗарегистрироватьОбработкуФайлов(
...
Функция РезультатЗапросаФайловОтвета(СправкиНДФЛ, ТолькоНеподписанные = Ложь)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			ПО ДокументЭДОБЗК.ЭлектронныйДокумент = СправкаНДФЛПрисоединенныеФайлы.Ссылка
...
&НаСервереБезКонтекста
Процедура ПометитьНаУдалениеДокументЭДОБЗКСправкиНДФЛ(СправкаНДФЛ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ДокументЭДОБЗК.ОснованиеДокумента В(&СправкаНДФЛ)
	...
	Если Выборка.Следующий() Тогда
		...
		ДокументЭДОБЗКОбъект.ДополнительныеСвойства.Вставить("РазрешенаПометкаУдаления");
