	...
	Попытка
		...
		Если ИспользуетсяЭДОБЗК Тогда
			...
			Если ЗначениеЗаполнено(ФайлОтветаПоЗаявке) Тогда
				ЭДОБЗКВызовСервера.УдалитьФайлыИзОбработкиПользователя(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ФайлОтветаПоЗаявке));
			КонецЕсли;
		...
		Иначе
			...
			БизнесПроцессыЗаявокСотрудников.СоздатьИзменитьДокументЭДОБЗКСправкаСотруднику(ПрисоединенныйФайл, Задание.Ссылка);
...
Процедура ЗавершитьПодписатьЭПНаСервере()
	...
	Если Не ЭтоФайлПечатнойФормы(ФайлОтветаПоЗаявке) Тогда
		НачатьТранзакцию();
		Попытка
			БизнесПроцессыЗаявокСотрудников.СоздатьИзменитьДокументЭДОБЗКСправкаСотруднику(ФайлОтветаПоЗаявке, Задание.Ссылка);
			РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.ЗарегистрироватьОбработкуФайлов(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ФайлОтветаПоЗаявке),
				Перечисления.ДействияСФайламиДокументовЭДОБЗК.ПередатьВКабинетСотрудников);
			ЗафиксироватьТранзакцию();
...
Процедура УдалитьПрисоединенныйФайлЗавершениеНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО ЗаявкаСотрудникаСправкаСМестаРаботыПрисоединенныеФайлы.Ссылка = ДокументЭДОБЗК.ЭлектронныйДокумент
	               |ГДЕ
	               |	ЗаявкаСотрудникаСправкаСМестаРаботыПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла
	               |	И ЗаявкаСотрудникаСправкаСМестаРаботыПрисоединенныеФайлы.ФайлОтвета = ИСТИНА
	               |	И ЗаявкаСотрудникаСправкаСМестаРаботыПрисоединенныеФайлы.ПометкаУдаления = ЛОЖЬ";
