...
// Открывает актуальную форму регламентированного отчета с учетом существования
// внешней формы отчета (внешний отчет будет зарегистрирован), или открывает
// любую другую форму по имени. Вызывается из обработки "Помощник уплаты налога".
//
// Параметры:
// Соответствуют параметрам метода глобального контекста "ОткрытьФорму()".
// В параметрах процедуры передаются имя формы, параметры формы, владелец,
// ключ уникальности, окно, в котором должна быть открыта форма, навигационная ссылка
// и описание оповещения, содержащее описание процедуры, которая будет вызвана при закрытии формы.
//
Процедура ОткрытьФормуРегламентированногоОтчета(Знач ИмяФормы,
	ПараметрыФормы, Владелец = Неопределено, ИдентификаторФормы = Ложь,
	Окно = Неопределено, НавигационнаяСсылка = Неопределено, ОповещениеОЗакрытии = Неопределено) Экспорт
	
	ЧастиИмениФормы = СтрРазделить(ИмяФормы, ".");
	
	Если (ЧастиИмениФормы[0] = "ВнешнийОтчет"
	  ИЛИ ЧастиИмениФормы[0] = "Отчет")
	    И ЧастиИмениФормы[2] = "Форма" Тогда
		
		ИсточникОтчета = ЧастиИмениФормы[1];
		ВыбраннаяФорма = ЧастиИмениФормы[3];
		
		// Внешний отчет будет зарегистрирован при вызове, если ранее не регистрировался.
		ИмяФормы = РегламентированнаяОтчетностьВызовСервера.ПолныйПутьКФорме(ИсточникОтчета, ВыбраннаяФорма);
		
	КонецЕсли;
	
	ОткрытьФорму(ИмяФормы, ПараметрыФормы, Владелец, ИдентификаторФормы, Окно, НавигационнаяСсылка, ОповещениеОЗакрытии);
