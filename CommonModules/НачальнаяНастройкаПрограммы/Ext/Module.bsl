...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("НачальнаяНастройкаПрограммыПервыйЗапуск"); 
	Методы.Вставить("ОбновитьДоступностьПереносаИзПрежнихПрограмм");
КонецПроцедуры
...
Функция ДоступенПереносДанныхИзПрежнихПрограмм(НеВыполнятьНачальнуюНастройкуПрограммы) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("РаботаВБюджетномУчреждении") Тогда
		Возврат Ложь;
	ИначеЕсли НеВыполнятьНачальнуюНастройкуПрограммы = Неопределено Тогда
...
Процедура ПолучитьНастройкиПрограммы(НастройкиПриложения) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.УчетБюджетныхУчреждений") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УчетБюджетныхУчреждений");
		Модуль.ПолучитьНастройкиПрограммы(НастройкиПриложения);
	КонецЕсли;
...
Процедура ВывестиНастройкиПрограммы(НастройкиПриложения, ТабДокумент) Экспорт

	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.УчетБюджетныхУчреждений") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УчетБюджетныхУчреждений");
		Модуль.ВывестиНастройкиПрограммы(НастройкиПриложения, ТабДокумент);
	КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.30.32";
	Обработчик.Процедура = "НачальнаяНастройкаПрограммы.ОбновитьДоступностьПереносаИзПрежнихПрограмм";
...
Процедура ОбновитьДоступностьПереносаИзПрежнихПрограмм() Экспорт
	
	Если ПолучитьФункциональнуюОпцию("РаботаВБюджетномУчреждении") И 
		ПолучитьФункциональнуюОпцию("ДоступенПереносДанныхИзПрежнихПрограмм") Тогда
		Константы.ДоступенПереносДанныхИзПрежнихПрограмм.Установить(Ложь);
	КонецЕсли;
