	...
	Для каждого Организация Из Организации Цикл
		...
		Если НЕ СвойстваОбменаПоСЭДОЧерезОператора.ОбменПоСЭДОНапрямую
			...
			Результат = МетаданныеВходящихСообщенийСЭДОФСС(
				...
				ТекущаяДатаСервера, ,
				СвойстваОбменаПоСЭДОЧерезОператора);
			Если ЗначениеЗаполнено(Результат.ДанныеСообщений) Тогда
				ПолученыСообщения = Истина;
			КонецЕсли;
	...
	Если ПолученыСообщения Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии("8.3.26.0", СистемнаяИнформация.ВерсияПриложения) <= 0 Тогда
			ОбщегоНазначения.ВыполнитьВБезопасномРежиме(
				"УведомленияКлиента.ОтправитьУведомление(""ЭлектронныйДокументооборотСФСС_ПолученыСообщенияСЭДО"", Неопределено)");
		КонецЕсли;
	КонецЕсли;
...
Функция ПриПолученииСодержимогоСообщенияСЭДО(Организация, ДанныеСообщения) Экспорт
	...
	ИначеЕсли ДанныеСообщения.Тип = ТипыСообщенийСЭДО.ТипСообщенияУведомлениеОтветНаВходящийЗапросНеПоступилВовремя Тогда
		...
		// Уведомление о том, что ответ на входящих запрос СФР не поступил вовремя (тип 124).
		// Вызываем переопределяемый метод.
		ЭлектронныйДокументооборотСФССПереопределяемый.
			ПослеРасшифровкиСообщенияСЭДО(Организация, ДанныеСообщения, РезультатОбработки);
		Если НЕ РезультатОбработки.Обработано Тогда
	...
	// Добавление в форму Новое регламентированной отчетности.
	Если ДокументооборотСКОПовтИсп.ЭтоРежимОблачной1СО()
		И РезультатОбработки.Обработано Тогда
		МодульКалендарьОтчетности = ОбщегоНазначения.ОбщийМодуль("КалендарьОтчетности");
		МодульКалендарьОтчетности.ПриПолученииСодержимогоСообщенияСЭДО(
			Организация, ДанныеСообщения, РезультатОбработки);
	КонецЕсли;
...
Функция РасширенноеСостояниеПодтвержденияИОтветаНаТребованиеСФР(Требование, Сейчас) Экспорт
	...
	Результат.Вставить("ЕстьОтправленныеОтветы", ЕстьОтправленныеОтветыНаТребованиеСФР(Требование));
...
Функция ЕстьОтправленныеОтветыНаТребованиеСФР(Требование) Экспорт
	
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	КоличествоОтветовНаТребования =
		КонтекстЭДОСервер.ПолучитьКоличествоОтветовНаТребованияСФР(
			Требование,
			Истина);
			
	ДанныеТребования = КоличествоОтветовНаТребования[Требование];
	Если ДанныеТребования = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ДанныеТребования.КоличествоОтправлено > 0;
...
Функция ПолучитьТипыСообщенийСДОФССДоходнаяЧасть()
	...
	// Типы 6хх
	Результат.Добавить(ТипыСообщений.РешениеОбУстановленииСкидкиКСтраховомуТарифу);
	Результат.Добавить(ТипыСообщений.РешениеОбОтменеУстановленнойСкидкиКСтраховомуТарифу);
	Результат.Добавить(ТипыСообщений.РешениеОбУстановленииНадбавкиКСтраховомуТарифу);
	Результат.Добавить(ТипыСообщений.РешениеОНесоответствииСведенийОбОВЭДПоРезультатамКамеральнойПроверки);
	Результат.Добавить(ТипыСообщений.РешениеОНесоответствииСведенийОбОВЭДПоРезультатамВыезднойПроверки);
...
Функция ВремяЗапускаЗаданияПопадаетВПиковыеЧасы()
	...
	Интервал = ОбщегоНазначения.СкопироватьРекурсивно(ИнтервалОбразец);
	Интервал.С = 8;
	Интервал.По = 10;
