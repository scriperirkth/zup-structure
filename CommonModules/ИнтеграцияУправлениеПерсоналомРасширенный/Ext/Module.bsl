...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УстановитьНастройкиПечатнойФормыУведомлениеОНачалеОтпуска");
	Методы.Вставить("ОтключитьВыгрузкуДанныхУправленческойОрганизации");
КонецПроцедуры
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.36";
	Обработчик.Процедура = "ИнтеграцияУправлениеПерсоналомРасширенный.УстановитьНастройкиПечатнойФормыУведомлениеОНачалеОтпуска";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("95dd021a-0874-47af-8155-bc596236d6be");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистра Настройки печатных форм (зарплата кадры).'");
...
Функция ДанныеСотрудников(Сотрудники, ИспользоватьРабочиеМестаОхраныТруда) Экспорт
	...
	ВыбираемыеПоля = "Сотрудник,ФизическоеЛицо,Организация,Должность,Подразделение,ДатаПриема,
	...
	Если ВедетсяШтатноеРасписание Тогда
		Если ПубликоватьСтруктуруЮридическихЛиц Тогда
			ВыбираемыеПоля = СтрШаблон("%1,%2", ВыбираемыеПоля, "ДолжностьПоШтатномуРасписанию");
		Иначе
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		...
		Если ВедетсяШтатноеРасписание И ПубликоватьСтруктуруЮридическихЛиц Тогда
			СтрокаТЗ.Подразделение = СтрокаТЗ.ПодразделениеОрганизации;
...
Процедура УстановитьНастройкиПечатнойФормыУведомлениеОНачалеОтпуска(ПараметрыОбновления = Неопределено) Экспорт 
	
	ИдентификаторПечатнойФормы = "ПФ_MXL_УведомлениеОНачалеОтпуска";
	ВидДокументаПрочие = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыДокументовЭДОБЗК.Прочие");
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ИдентификаторПечатнойФормы", ИдентификаторПечатнойФормы);
	Запрос.УстановитьПараметр("ВидДокумента", ВидДокументаПрочие);
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НастройкиПечатныхФормБЗК.ИдентификаторПечатнойФормы = &ИдентификаторПечатнойФормы
		|	И НастройкиПечатныхФормБЗК.ВидДокумента = &ВидДокумента";
	...
	ВидДокументаУведомление = Справочники.ВидыДокументовЭДОБЗК.ВидПоИдентификаторуПечатнойФормы(ИдентификаторПечатнойФормы);
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			НаборЗаписей[0].ВидДокумента = ВидДокументаУведомление;
		КонецЕсли;
