...
Процедура ОтборПериодНачалоВыбора(
		Форма,
		ОписаниеОповещения,
	...
	НастройкиВызова.Вставить("ИмяРеквизитаНачалоПериода", "");
	НастройкиВызова.Вставить("ИмяРеквизитаКонецПериода", "");
	НастройкиВызова.Вставить("ИмяРеквизитаТолькоУказанныйПериод", "");
	...
	НачалоПериодаВыбор = ?(ЗначениеЗаполнено(НастройкиВызова.ИмяРеквизитаНачалоПериода),
		Форма[НастройкиВызова.ИмяРеквизитаНачалоПериода], Форма.НачалоПериода);
	КонецПериодаВыбор = ?(ЗначениеЗаполнено(НастройкиВызова.ИмяРеквизитаКонецПериода),
		Форма[НастройкиВызова.ИмяРеквизитаКонецПериода], Форма.КонецПериода);
	ТолькоУказанныйПериод = ?(ЗначениеЗаполнено(НастройкиВызова.ИмяРеквизитаТолькоУказанныйПериод),
		Форма[НастройкиВызова.ИмяРеквизитаТолькоУказанныйПериод], Форма.ТолькоУказанныйПериод);
	...
	ПараметрыВыбора.Вставить("ВидПериода", Неопределено);
	ПараметрыВыбора.Вставить("ТолькоУказанныйПериод", ТолькоУказанныйПериод);
...
Процедура ОтборВыбратьПериодЗавершение(РезультатВыбора, ВходящийКонтекст) Экспорт
	...
	ТолькоУказанныйПериод = ?(ЗначениеЗаполнено(ВходящийКонтекст.Настройки.ИмяРеквизитаТолькоУказанныйПериод),
		ВходящийКонтекст.Форма[ВходящийКонтекст.Настройки.ИмяРеквизитаТолькоУказанныйПериод],
		ВходящийКонтекст.Форма.ТолькоУказанныйПериод);
...
Процедура ВывестиМашиночитаемуюФормуВТонкомКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиент.ЭтоWindowsКлиент() Тогда
		ПоказатьПредупреждение(,НСтр("ru='Внимание! Печать бланка формы с двухмерным штрихкодом PDF417 поддерживается только в ОС ""Microsoft Windows"".'"));
		Возврат;
	КонецЕсли;
...
Функция ЭтоСобытиеЗакрытияФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(ОповещаемаяФорма, ИмяСобытия, Параметр) Экспорт
	Если ТипЗнч(Параметр) = Тип("ФормаКлиентскогоПриложения") Тогда
		Возврат (ОповещаемаяФорма.УникальныйИдентификатор = Параметр.УникальныйИдентификатор)
			И (ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ИмяСобытияЗакрытияФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении()
				= ИмяСобытия);
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПредложитьОформитьЗаявлениеНаПодключениеПриЗаполнении(Форма) Экспорт
	ПараметрыФормы = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.НовыеПараметрыФормыОформитьЗаявлениеНаПодключениеПриЗаполнении();
	ПараметрыФормы.Организация = Форма.СтруктураРеквизитовФормы.Организация;
	Параметрыформы.ИмяФормы = Форма.ИмяФормы;
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(ПараметрыФормы);
КонецПроцедуры
