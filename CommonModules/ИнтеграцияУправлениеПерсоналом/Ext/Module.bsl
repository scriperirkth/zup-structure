...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтменитьИзмененияОбъектовПоТипуКОбменуФоновоеЗадание", Истина);
	Методы.Вставить("ПроверитьЗаполнениеОбъектовПриложенияФоновоеЗадание", Истина);
	Методы.Вставить("ЗарегистрироватьИзмененияОбъектовПоТипуКОбменуФоновоеЗадание", Истина);
	Методы.Вставить("ОтменитьРегистрациюОбъектаКОбменуФоновоеЗадание", Истина);
	Методы.Вставить("ЗаполнитьПриложениеВИсключенияхИзПравил");
	Методы.Вставить("ЗаполнитьПриложениеВСостоянииПодключения");
	Методы.Вставить("ОбновитьДанныеПоСотрудникамКУдалению");
	Методы.Вставить("ОбновитьСостояниеБлокировкиФоновоеЗадание", Истина);
	Методы.Вставить("ПодключитьПриложениеЛокальноФоновоеЗадание", Истина);
	Методы.Вставить("ОбновлениеОтветственногоФоновоеЗадание", Истина);
	Методы.Вставить("ВыгрузкаЖурналаРегистрацииФоновоеЗадание", Истина);
	Методы.Вставить("ИзменитьСостояниеБлокировкиПользователейФоновоеЗадание", Истина);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.38.36";
	Обработчик.РежимВыполнения 	= ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор 	= Новый УникальныйИдентификатор("e66bc65a-e9ca-4572-a0e3-b7f51708d7bc");
	Обработчик.Процедура       = "ИнтеграцияУправлениеПерсоналом.ОбновитьДанныеПоСотрудникамКУдалению";
	Обработчик.Комментарий 		= НСтр("ru = 'Обновление данных публикуемых сотрудников.'");
...
Процедура ПодключитьПриложениеЛокальноФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Если Не Отказ Тогда
		...
		Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
			ИнтеграцияКабинетСотрудника.ЗаписатьАдресПриложения(НастройкиПодключения.АдресПриложения, НастройкиПодключения.АдресСервисаАвторизации, НастройкиПодключения.АдресРесурсовAPI);
		ИначеЕсли Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
			Интеграция1СПерсонал.ЗаписатьАдресПриложения(НастройкиПодключения.АдресПриложения, НастройкиПодключения.АдресСервисаАвторизации, НастройкиПодключения.АдресРесурсовAPI);
		КонецЕсли;
...
Функция ОбъектыДляРегистрацииПриЗаписиНовыхПравил(Приложение, ДанныеДляРегистрации)
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника И ДоступенРасширенныйФункционал Тогда
		...
		НовыеГрафики = ОбщегоНазначения.ВыгрузитьКолонку(ИзмененияДанныхГрафика, "ГрафикРаботы", Истина);
		Для каждого НовыйГрафик Из НовыеГрафики Цикл
			НоваяСтрока = ИзмененияДляОбмена.Добавить();
			НоваяСтрока.Ссылка 		= НовыйГрафик;
...
Функция ОбъектыДляРегистрацииОбмена(Приложение, ДанныеДляРегистрации)
	...
	|			И Таблица.Удалить = Изменения.ВыгружатьУдаление
	|ГДЕ
	...
	|	Сотрудники.Удалить КАК Удалить
	|ИЗ
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатЗапроса[1].Выгрузить(), ВыгружаемыеОбъекты);
	ДополнитьТаблицуИзмененияДляОбмена(РезультатЗапроса[2].Выгрузить(), ИзмененияДляОбмена);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатЗапроса[3].Выгрузить(), ВыгружаемыеОбъекты);
...
Процедура СохранитьНовыйАдресРесурсовAPI(Приложение, АдресРесурсовAPI) Экспорт
	
	УдалитьТокенПриложения(Приложение);
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ИнтеграцияКабинетСотрудника.СохранитьНовыйАдресРесурсовAPI(АдресРесурсовAPI);
	Иначе
		Интеграция1СПерсонал.СохранитьНовыйАдресРесурсовAPI(АдресРесурсовAPI);
	КонецЕсли;
...
Процедура ОбновитьДанныеПоСотрудникамКУдалению(ПараметрыОбновления = Неопределено) Экспорт

	Если Не ИспользуетсяИнтеграция() Тогда
	...
	|	РегистрСведений.ОшибкиЗаполненияОбъектовУправлениеПерсоналом КАК ОшибкиЗаполнения
	|ГДЕ
	|	ОшибкиЗаполнения.ТипОбъекта = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовИнтеграцияУправлениеПерсоналом.Сотрудник)
	|	И ОшибкиЗаполнения.БлокирующаяОшибка";
	РезультатЗапроса = Запрос.Выполнить();
	...
	Запрос.УстановитьПараметр("Сотрудники", РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка"));
	
	СотрудникиУП = Новый Массив;
	Если Используется1СПерсонал() Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВыгружаемыеОбъекты.Ссылка КАК Ссылка
		...
		|ГДЕ
		|	ВыгружаемыеОбъекты.Ссылка В(&Сотрудники)";
		СотрудникиУП = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	КонецЕсли;
	
	СотрудникиКС = Новый Массив;
	Если ИспользуетсяКабинетСотрудника() Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВыгружаемыеОбъекты.Ссылка КАК Ссылка
		...
		|ГДЕ
		|	ВыгружаемыеОбъекты.Ссылка В(&Сотрудники)";
		СотрудникиКС = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	КонецЕсли;
	
	ВсеСотрудники = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВсеСотрудники, СотрудникиКС);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВсеСотрудники, СотрудникиУП, Истина);
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Ложь, ВсеСотрудники, "Организация,ВидСобытия");
	ИспользоватьКадровыйУчет = ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет");
	Для каждого Сотрудник Из ВсеСотрудники Цикл
		ДанныеСотрудника = КадровыеДанные.Найти(Сотрудник, "Сотрудник");
		Если ДанныеСотрудника = Неопределено Или ИспользоватьКадровыйУчет И Не ЗначениеЗаполнено(ДанныеСотрудника.ВидСобытия) Тогда
			СотрудникиКУдалению.Добавить(Сотрудник);
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого Сотрудник Из СотрудникиУП Цикл
		
		Если СотрудникиКУдалению.Найти(Сотрудник) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрСведений.ВыгружаемыеОбъекты1СПерсонал", "Ссылка", Сотрудник) Тогда
			ОбработкаЗавершена = Ложь;
			Продолжить;
		КонецЕсли;
		
		НаборЗаписей = РегистрыСведений.ВыгружаемыеОбъекты1СПерсонал.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Ссылка.Установить(Сотрудник);
	...
	Для каждого Сотрудник Из СотрудникиКС Цикл
		
		Если СотрудникиКУдалению.Найти(Сотрудник) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника", "Ссылка", Сотрудник) Тогда
			ОбработкаЗавершена = Ложь;
			Продолжить;
		КонецЕсли;
...
Функция НастройкиПодключения(Приложение) Экспорт
	...
	|Идентификатор,ИдентификаторПриложения,АдресРесурсовAPI";
