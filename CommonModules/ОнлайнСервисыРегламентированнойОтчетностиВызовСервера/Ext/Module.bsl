...
Функция ПрочитатьТекстовыйФайл(ИмяФайла, ОписаниеОшибки, УдалятьПослеПрочтения = Ложь, Кодировка = "windows-1251")
	...
	Попытка
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ИмяФайла, "ИмяФайла", "ПрочитатьТекстовыйФайл");
...
Функция РаспаковатьФайл(ФайлАрхива, ОписаниеОшибки = Неопределено)
	...
	Попытка
		...
		ЧтениеЗИП.Закрыть();
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ФайлАрхива);
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ИмяПервогоФайлаАрхива);	
		//@skip-check 1c-fresh-audit
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу) Экспорт
	...
	Попытка
		...
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ФайлОтчета.ПолноеИмя,
			"ФайлОтчета.ПолноеИмя",
			"ОбработатьИЗаписатьВнешнийОтчетВСправочникРО");
				
		//@skip-check 1c-fresh-audit
		ХранилищеВнешнегоОтчета = Новый ХранилищеЗначения(
			Новый ДвоичныеДанные(ФайлОтчета.ПолноеИмя), Новый СжатиеДанных(9));
...
Функция ПрочитатьМанифест(КаталогРаспаковки)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		КаталогРаспаковки, "КаталогРаспаковки", "ПрочитатьМанифест");	
