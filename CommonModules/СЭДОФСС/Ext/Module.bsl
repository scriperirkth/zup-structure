...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Документы.ЗапросСтатусовВыплатыПособия.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	Документы.ОтказВВозмещенииВыплатРодителямДетейИнвалидов.ПриДобавленииОбработчиковОбновления(Обработчики);
	Документы.СтатусыВыплатыПособия.ПриДобавленииОбработчиковОбновления(Обработчики);
	Документы.УведомлениеОбОтсутствииОтветаСЭДО.ПриДобавленииОбработчиковОбновления(Обработчики);
	...
	Обработчик.Комментарий     = НСтр("ru = 'Добавление ролей извещений СФР в непоставляемые (пользовательские) профили групп доступа.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение вида извещений СФР.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение таблицы доступа к извещениям СФР.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Повторная обработка входящих сообщений СЭДО СФР.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Исправление вида пособия в документах СЭДО СФР.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Обновление статусов регистраций ответов на запросы СФР для расчета пособий.'");
...
// Отменяет все открытые транзакцию в обработке исключения - чтобы исключить ошибки при последующих обращениях к СУБД.
Функция ОтменитьВсеТранзакции() Экспорт
	Количество = 0;
	Пока ТранзакцияАктивна() Цикл
		Количество = Количество + 1;
		ОтменитьТранзакцию(); // АПК:325 Отмена незакрытых транзакций.
	КонецЦикла;
	БылиТранзакции = Количество > 0;
	Если БылиТранзакции Тогда
		Текст = НСтр("ru='При обработке исключения отменены незакрытые транзакции (%1 шт).'");
		Текст = СтрШаблон(Текст, Количество);
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала(), УровеньЖурналаРегистрации.Предупреждение, , , Текст);
	КонецЕсли;
	Возврат БылиТранзакции;
КонецФункции

// Добавляет роли извещений СФР в непоставляемые (пользовательские) профили групп доступа.
//
// Параметры:
...
// Повторно загружает входящие сообщения СЭДО СФР.
//
// Параметры:
...
//   МинимальнаяДата - Дата, Неопределено
//
Процедура ПовторноОбработатьВходящиеСообщенияСЭДО(ПараметрыОбновления = Неопределено, ТипыЧислом = Неопределено, Идентификаторы = Неопределено, Страхователи = Неопределено, МинимальнаяДата = Неопределено) Экспорт
...
// Исправляет вид пособия в документах СЭДО СФР.
//
// Параметры:
//   ПараметрыОбновления - Структура - Параметры отложенного обновления.
//
...
// Повторно рассчитывает статус в регистрациях ответов на запросы СФР для расчета пособий.
//
// Параметры:
//   ПараметрыОбновления - Структура - Параметры отложенного обновления.
//
...
Функция ЗапросПоВходящимСообщениямСЭДО(ТипыЧислом = Неопределено, Идентификаторы = Неопределено, Страхователи = Неопределено, МинимальнаяДата = Неопределено, Количество = Неопределено) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура ПриОпределенииОбъектовСКомандамиПечати(СписокОбъектов) Экспорт
	...
	СписокОбъектов.Добавить(Документы.УведомлениеОбОтсутствииОтветаСЭДО);
...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	УправлениеСвойствамиБЗК.ЗарегистрироватьНаборСвойств(Наборы, "c901fbd7-0cc4-11f1-8173-4cedfb43b11a", Метаданные.Документы.УведомлениеОбОтсутствииОтветаСЭДО);
...
Процедура ПриОпределенииОбработчиковТекущихДел(Обработчики) Экспорт
	...
	Обработчики.Добавить(Документы.УведомлениеОбОтсутствииОтветаСЭДО);
...
Процедура ПослеРасшифровкиСообщенияСЭДО(Страхователь, Сообщение, Результат) Экспорт
	...
	ИначеЕсли ТипСообщения = 105 Тогда
		// Результат ответа на запрос сведений для расчета и выплаты пособия СФР.
	...
	ИначеЕсли ТипСообщения = 122 Или ТипСообщения = 12210 Тогда
		...
		Документы.ОтказВВозмещенииВыплатРодителямДетейИнвалидов.ЗагрузитьСообщение(
			...
			Результат,
			Кэш);
	ИначеЕсли ТипСообщения = 124 Тогда
		// С появлением документа не требуется информировать пользователя о 124 сообщении через список ошибок.
		Результат.ОшибкаОбработки = Ложь;
		Результат.ОписаниеОшибки = "";
		// Уведомление об отсутствии ответа на входящий запрос.
		Документы.УведомлениеОбОтсутствииОтветаСЭДО.ЗагрузитьСообщение(
			ТипСообщения,
			Страхователь,
			ИдентификаторСообщения,
			Сообщение.ТекстСообщения,
	...
	Если ЗначениеЗаполнено(СНИЛСВФорматеФСС) Тогда
		...
		// Результат обработки сообщения о информировании СФР о жизненных событиях.
...
Процедура ПослеПолученияОшибокЛогическогоКонтроляСЭДО(Страхователь, ИсходноеСообщение, ТекстОшибки, Результат) Экспорт
	...
	ИначеЕсли ТипСообщения = 101 Тогда
		// Ответ на запрос СФР для расчета пособий.
	...
	ИначеЕсли ТипСообщения = 109 Тогда
		// Исходящее сообщение о страховом случае СФР.
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Документы.УведомлениеОбОтсутствииОтветаСЭДО, Истина);
	...
	Списки.Вставить(Метаданные.Справочники.УведомлениеОбОтсутствииОтветаСЭДОПрисоединенныеФайлы, Истина);
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	ДобавитьОписаниеНаличияОграченияПрав(Описание, Метаданные.Документы.УведомлениеОбОтсутствииОтветаСЭДО);
...
Процедура ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(ИсточникиДанных) Экспорт
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(
		ИсточникиДанных,
		Метаданные.Документы.УведомлениеОбОтсутствииОтветаСЭДО.ПолноеИмя(),
		"Дата",
		"Зарплата",
		"Страхователь");
...
Процедура ПолучитьПрефиксообразующиеРеквизиты(Объекты) Экспорт
	...
	СтрокаТаблицы = Объекты.Добавить();
	СтрокаТаблицы.Объект = Метаданные.Документы.УведомлениеОбОтсутствииОтветаСЭДО;
	СтрокаТаблицы.Реквизит = "Страхователь";
...
// Возвращает признак наличия прав на обмен с СФР.
Функция ЕстьПравоОбмена() Экспорт
	Возврат ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ЖурналЗапросовКСерверамФСС);
КонецФункции

// Возвращает признак наличия прав на отправку электронных документов в СФР.
...
Процедура ПроверитьНаличиеПраваОбмена() Экспорт
	Если Не ЕстьПравоОбмена() Тогда
		ТекстОшибки = НСтр("ru = 'Недостаточно прав на обмен с СЭДО СФР.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
КонецПроцедуры

// Проверяет наличие права обмена электронными документами с СФР.
...
//
// Возвращаемое значение:
//   СправочникСсылка.Организации - Страхователь СФР, отчитывающийся за филиал.
//
...
// Возвращает Истина если у страхователя есть филиалы без самостоятельной регистрации в СФР.
//
// Параметры:
//   Страхователь - СправочникСсылка.Организации
//   ДатаПолучения - Дата - Дата получения сведений.
...
Функция СтрахователиГоловнойОрганизации(ГоловнаяОрганизация) Экспорт
	Возврат КоллекцииБЗК.УникальныеЗначенияКолонки(ВсеСтрахователи(ГоловнаяОрганизация), "Страхователь");
КонецФункции
...
Процедура ОбновитьИнформационнуюПанельБРО(Форма) Экспорт
	ПараметрыОтображения = ЭлектронныйДокументооборотСФСС.ПараметрыИнформационнойПанелиБРО();
	ПараметрыОтображения.ЭтоСЭДО = Истина;
	ПараметрыОтображения.Форма = Форма;
	ПараметрыОтображения.Группа = Форма.Элементы.ИнформационнаяПанельБРО;
	ПараметрыОтображения.Организация = ОтборПоСтрахователямФормыСписка(Форма);
	ЭлектронныйДокументооборотСФСС.ОбновитьИнформационнуюПанельБРО(ПараметрыОтображения);
КонецПроцедуры

Функция ОтборПоСтрахователямФормыСписка(Форма) Экспорт
	ИменаРеквизитов = "ГоловнаяОрганизация, Организации, Организация, Страхователь, Список";
	РеквизитыФормы = ОбщегоНазначенияБЗККлиентСервер.ЗначенияСвойств(Форма, ИменаРеквизитов);
	
	// В формах списков без быстрых отборов используются отборы из СКД.
	Если РеквизитыФормы.Организации = Неопределено
		И РеквизитыФормы.Организация = Неопределено
		И РеквизитыФормы.Страхователь = Неопределено
		И РеквизитыФормы.ГоловнаяОрганизация = Неопределено Тогда
		ОтборыСпискаНРег = СЭДОФССКлиентСервер.ОтборыСпискаНРег(РеквизитыФормы.Список);
		ЗначениеОтбора = ОтборыСпискаНРег[НРег("Организация")];
		Если ЗначениеОтбора <> Неопределено Тогда
			РеквизитыФормы.Организации = СЭДОФССКлиентСервер.СписокЗаполненныхОрганизаций(ЗначениеОтбора);
		КонецЕсли;
		РеквизитыФормы.ГоловнаяОрганизация = ОтборыСпискаНРег[НРег("ГоловнаяОрганизация")];
	КонецЕсли;
	
	// Отбор по организациям и страхователям взаимоисключающий.
	// Но любой из них сильнее отбора по головной организации.
	Если ТипЗнч(РеквизитыФормы.Организации) = Тип("СписокЗначений") И РеквизитыФормы.Организации.Количество() > 0 Тогда
		Возврат СтрахователиОрганизаций(РеквизитыФормы.Организации.ВыгрузитьЗначения());
	ИначеЕсли СЭДОФССКлиентСервер.СсылкаОрганизацииЗаполнена(РеквизитыФормы.Организация) Тогда
		Возврат СтраховательОрганизации(РеквизитыФормы.Организация);
	ИначеЕсли СЭДОФССКлиентСервер.СсылкаОрганизацииЗаполнена(РеквизитыФормы.Страхователь) Тогда
		Возврат РеквизитыФормы.Страхователь;
	ИначеЕсли СЭДОФССКлиентСервер.СсылкаОрганизацииЗаполнена(РеквизитыФормы.ГоловнаяОрганизация) Тогда
		Возврат СтрахователиГоловнойОрганизации(РеквизитыФормы.ГоловнаяОрганизация);
	КонецЕсли;
...
Процедура ПроверитьНомерИзвещенияФСС(Отказ, Объект, ИмяНомера) Экспорт
	...
	ИначеЕсли СтрДлина(Номер) > 12 Тогда
		Текст = НСтр("ru = 'В номере извещения %1 символов, а должно быть не более 12.'");
	...
	Иначе
		...
		Если Не ПустаяСтрока(ЛишниеСимволы) Тогда
			Если СтрДлина(ЛишниеСимволы) = 1 Тогда
				Текст = НСтр("ru = 'В номере извещения должны быть только цифры (%1 - не цифра).'");
			Иначе
				Текст = НСтр("ru = 'В номере извещения должны быть только цифры (%1 - не цифры).'");
			КонецЕсли;
...
Функция СведенияОВидеДокумента(Представление, ИмяПредопределенныхДанных, КодМВД) Экспорт
	...
	// Особенность СФР (и СЭДО и прямых выплат):
	// Виду документов "Свидетельство о предоставлении временного убежища на территории РФ"
	// в спецификации присвоен код 19, а в классификации ФНС - код 18.
...
Функция ЗначенияОтборовСКД(Компоновщик) Экспорт
	...
	Для Каждого ЭлементОтбора Из Компоновщик.Настройки.Отбор.Элементы Цикл
		...
		Если ЗначениеЗаполнено(ЭлементОтбора.ИдентификаторПользовательскойНастройки) Тогда
			Настройка = Компоновщик.ПользовательскиеНастройки.Элементы.Найти(
...
Функция СведенияОПроцессе(НомерПроцесса) Экспорт
	СведенияОПроцессе = Новый Структура("ОтветНаЗапрос, ВходящийЗапрос, ФизическоеЛицо, Организация, ДатаОтправки,
	|РегистрацияДата, Состояние");
	Если Не ЗначениеЗаполнено(НомерПроцесса) Тогда
		Возврат СведенияОПроцессе;
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Шапка.Ссылка КАК ОтветНаЗапрос,
	|	Шапка.ВходящийЗапрос КАК ВходящийЗапрос,
	|	Шапка.ФизическоеЛицо КАК ФизическоеЛицо,
	|	Шапка.Организация КАК Организация,
	|	Шапка.ДатаОтправки КАК ДатаОтправки,
	|	ЕСТЬNULL(Регистрация.РегистрацияДата, НЕОПРЕДЕЛЕНО) КАК РегистрацияДата,
	|	ЕСТЬNULL(Регистрация.Состояние, НЕОПРЕДЕЛЕНО) КАК Состояние
	|ИЗ
	...
	|ГДЕ
	|	Шапка.НомерПроцесса = &НомерПроцесса
	|
	|УПОРЯДОЧИТЬ ПО
	|	Шапка.ПометкаУдаления,
	|	Шапка.ДатаОтправки УБЫВ,
	|	Шапка.Дата УБЫВ";
	Запрос.УстановитьПараметр("НомерПроцесса", НомерПроцесса);
	Таблица = Запрос.Выполнить().Выгрузить();
	Если Таблица.Количество() > 0 Тогда
		ЗаполнитьЗначенияСвойств(СведенияОПроцессе, Таблица[0]);
	Иначе
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	НЕОПРЕДЕЛЕНО КАК ОтветНаЗапрос,
		|	Шапка.Ссылка КАК ВходящийЗапрос,
		|	Шапка.ФизическоеЛицо КАК ФизическоеЛицо,
		|	Шапка.Организация КАК Организация,
		|	НЕОПРЕДЕЛЕНО КАК ДатаОтправки,
		|	НЕОПРЕДЕЛЕНО КАК РегистрацияДата,
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ДобавитьРолиИзвещенийВПользовательскиеПрофили");
	Методы.Вставить("ПовторноОбработатьРегистрацииОтветовНаЗапросыФССДляРасчетаПособий");
	Методы.Вставить("ИсправитьВидПособияВДокументах");
	Методы.Вставить("ПовторноОбработатьНесоответствияСведенийОЗастрахованномЛице");
	Методы.Вставить("ПовторноОбработатьВходящиеСообщенияСЭДО");
КонецПроцедуры
