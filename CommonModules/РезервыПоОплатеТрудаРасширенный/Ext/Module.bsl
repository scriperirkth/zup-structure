...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьПараметрыОграниченийРасчетаРезервовПоОплатеТруда");
	Методы.Вставить("ЗаполнитьРеквизитыНУДокументаРезервыПоОплатеТруда");
	Методы.Вставить("УстановитьПризнакРасчетаПоМРОТ");
КонецПроцедуры
...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	УправлениеСвойствамиБЗК.ЗарегистрироватьНаборСвойств(Наборы, "fe0422ef-a5fc-4b77-9ee3-5918d13b0577", Метаданные.Документы.ПереносРезервовМеждуСтатьямиФинансирования);
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Документы.ПереносРезервовМеждуСтатьямиФинансирования, Истина);
	...
	Списки.Вставить(Метаданные.Справочники.ПереносРезервовМеждуСтатьямиФинансированияПрисоединенныеФайлы, Истина);
	...
	Списки.Вставить(Метаданные.РегистрыСведений.БухучетРезервовПоОплатеТруда, Истина);
...
Процедура ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(ИсточникиДанных) Экспорт
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "РегистрСведений.БухучетРезервовПоОплатеТруда", "ДатаЗапрета", "БухгалтерскийУчетЗарплаты", "Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ПереносРезервовМеждуСтатьямиФинансирования", "ПериодРегистрации", "БухгалтерскийУчетЗарплаты", "Организация");
...
Процедура ПриОпределенииОбъектовСКомандамиПечати(СписокОбъектов) Экспорт
	...
	СписокОбъектов.Добавить(Документы.ПереносРезервовМеждуСтатьямиФинансирования);
...
Функция ДанныеДляЗаполненияТаблицДокумента(ОписаниеДокумента, Организация, Резерв, Знач МесяцНачисления, ДополнительныеПараметры = Неопределено) Экспорт	
	...
	Если Сотрудники = Неопределено Тогда
		...
		ПараметрыПолучения.КадровыеДанные = "Организация,Подразделение,ВидСобытия,ДатаУвольнения";
		...
		Если ЗначениеЗаполнено(Подразделение) Тогда
			МассивПодразделений = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Подразделение);
			ВыборкаПодразделений = Справочники.ПодразделенияОрганизаций.ВыбратьИерархически(Подразделение);
			Пока ВыборкаПодразделений.Следующий() Цикл
				МассивПодразделений.Добавить(ВыборкаПодразделений.Ссылка);
			КонецЦикла;
			КадровыеДанныеВременная = КадровыеДанные.СкопироватьКолонки();
			Для Каждого СтрокаТаблицы Из КадровыеДанные Цикл
				Если МассивПодразделений.Найти(СтрокаТаблицы.Подразделение) <> Неопределено 
					Или СтрокаТаблицы.Организация <> Организация Тогда
		...
		Если ДопПараметрыОтбора <> Неопределено Тогда
			...
			УточнитьСписокСотрудников(Организация, Сотрудники, ДопПараметрыОтбора.Период, ДопПараметрыОтбора.ИмяПоказателяДопОтбора);
		КонецЕсли;
...
Процедура ЗапросДанныеПечатиДокументов(Запрос) Экспорт
	...
	|	ИСТИНА КАК ЕстьРасшифровкаНачислений
...
Процедура ВидыРасчетовРезервовПоОплатеТрудаОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.Организация)
		Или Не ЗначениеЗаполнено(Объект.Резерв)
		Или Не ЗначениеЗаполнено(Объект.ВидРасчета)
		Или Не ЗначениеЗаполнено(Объект.НазначениеПоказателя) Тогда
	...
	Если Объект.СпособВыполненияНачисления = Перечисления.СпособыВыполненияНачислений.ПоЗначениюПоказателяПриОкончательномРасчете Тогда 
	...
	Если Объект.ПериодРасчетаБазовыхНачислений = Перечисления.ПериодыРасчетаБазовыхНачислений.НесколькоПредыдущихМесяцев
		И Не ЗначениеЗаполнено(Объект.СдвигБазовогоПериода) Тогда
		ТекстСообщения = НСтр("ru = 'Поле ""Сдвиг базового периода"" не заполнено.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "СдвигБазовогоПериода", , Отказ);
	КонецЕсли;
...
Процедура ДополнительныеПоказатели(ДополнительныеПоказатели) Экспорт
	
	ПоказательУчитыватьМРОТ =
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.УчитыватьМРОТ");
	Если ПоказательУчитыватьМРОТ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДополнительныеПоказатели.Добавить(ПоказательУчитыватьМРОТ);
	
КонецПроцедуры

Процедура ЗаполнитьЗначенияДополнительныхПоказателейПоТаблицеЗначений(ДополнительныеПоказатели, ТаблицаПоказателей, МенеджерРасчета) Экспорт
	
	НастройкаРезервов = МенеджерРасчета.НастройкиРасчета.НастройкаРезервов;
	Если НастройкаРезервов = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	УчитыватьМРОТ = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкаРезервов, "УчитыватьМРОТПриОплатеПоСреднемуЗаработку", Ложь);
	
	ПоказательУчитыватьМРОТ =
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.УчитыватьМРОТ");
	
	Для Каждого СтрокаПоказателя Из ДополнительныеПоказатели Цикл
		Если СтрокаПоказателя.Показатель <> ПоказательУчитыватьМРОТ Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрокаПоказателя.Показатель = ПоказательУчитыватьМРОТ Тогда
			НоваяСтрока.Значение = УчитыватьМРОТ;
		КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьЗначенияПоказателейСохраняемыйЗаработок(СтрокиНачисленийКРасчету, МенеджерРасчетаЗарплаты)
	...
	Если УчитыватьОсобенностиПредприятия Тогда
		...
		Для каждого Сотрудник Из СотрудникиПоСохраняемомуЗаработку Цикл
			...
			ПериодРасчета = УчетСреднегоЗаработка.ПериодРасчетаОбщегоСреднегоЗаработкаСотрудника(
				ДатаСобытия, 
				Сотрудник);
			...
			Для Каждого ИдентификаторСтроки Из СтрокиТаблицыНачисления Цикл
				СтрокаТаблицыНачисления = СтрокиПоИдентификатору[ИдентификаторСтроки];
				СтрокаТаблицыНачисления.ПериодРасчетаСреднегоЗаработкаНачало    = ПериодРасчета.ДатаНачала;
				СтрокаТаблицыНачисления.ПериодРасчетаСреднегоЗаработкаОкончание = ПериодРасчета.ДатаОкончания;
				МенеджерРасчетаЗарплаты.ЗаполнитьЗначениеПоказателяСтрокиРасчета(СтрокаТаблицыНачисления, ПоказательСохраняемыйЗаработок, СреднийЗаработок, Истина);
			КонецЦикла;
			МенеджерРасчетаЗарплаты.ЗаполнитьЗначениеПоказателяСтрокиРасчета(СтрокаТаблицыНачисления, ПоказательСохраняемыйЗаработок, СреднийЗаработок, Истина);
		КонецЦикла;
	...
	Если СтрокиНачисленийКРасчету.Количество() > 0 Тогда
		...
		КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, МассивСотрудников, "ГрафикРаботы,КоличествоСтавок", ДатаСобытия);
		
		МассивГрафиков = ОбщегоНазначения.ВыгрузитьКолонку(КадровыеДанные, "ГрафикРаботы", Истина);
		Для Индекс = 0 По МассивГрафиков.ВГраница() Цикл
			Если Не ЗначениеЗаполнено(МассивГрафиков[Индекс]) Тогда
				МассивГрафиков.Удалить(Индекс);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		ДанныеГрафиков = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивГрафиков, "НеполноеРабочееВремя");
		
		Для Каждого СтрокаМассива Из СтрокиНачисленийКРасчету Цикл
			
			КадровыеДанныеСотрудника = КадровыеДанные.Найти(СтрокаМассива.Сотрудник, "Сотрудник");
			Если КадровыеДанныеСотрудника = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(КадровыеДанныеСотрудника.КоличествоСтавок) Или КадровыеДанныеСотрудника.КоличествоСтавок < 1 Тогда
				Продолжить;
			КонецЕсли;
			Если ЗначениеЗаполнено(КадровыеДанныеСотрудника.ГрафикРаботы) И ДанныеГрафиков[КадровыеДанныеСотрудника.ГрафикРаботы] Тогда
				Продолжить;
			КонецЕсли;
			
			// Наличее показателя сохраняемого заработка
			...
			Если СтрокаПоказателяСреднийЗаработок = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			// Предусмотрен расчет по МРОТ
			СтрокаПоказателяПоказательУчитыватьМРОТ = СтрокаМассива.Показатели.Найти(ПоказательУчитыватьМРОТ);
			Если СтрокаПоказателяПоказательУчитыватьМРОТ = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если СтрокаПоказателяПоказательУчитыватьМРОТ.Значение = 0 Тогда
				Продолжить;
			КонецЕсли;
			// Есть сведения о календарных днях
		...
		//Убираем технические показатели
		Для Каждого СтрокаТаблицыНачислений Из СтрокиНачисленийКРасчету Цикл
			НовыеПоказатели = СтрокаТаблицыНачислений.Показатели.СкопироватьКолонки();
			Для Каждого СрокаПоказателя Из СтрокаТаблицыНачислений.Показатели Цикл
				Если СрокаПоказателя.Показатель <> ПоказательМРОТ
					И СрокаПоказателя.Показатель <> ПоказательУчитыватьМРОТ
					И СрокаПоказателя.Показатель <> ПоказательКалендарныеДниМесяцаБезПраздников Тогда
					ЗаполнитьЗначенияСвойств(НовыеПоказатели.Добавить(), СрокаПоказателя);
				КонецЕсли;
			КонецЦикла;
			СтрокаТаблицыНачислений.Показатели = НовыеПоказатели.Скопировать();
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстЗапросаНастройкаРезервов() Экспорт
	...
	|	НастройкиРасчетаРезервовПоОплатеТруда.УчитыватьМРОТПриОплатеПоСреднемуЗаработку КАК УчитыватьМРОТПриОплатеПоСреднемуЗаработку,
	|	РаспределятьВыявленныеИзлишки КАК РаспределятьВыявленныеИзлишки,
	|	Ложь КАК ИспользоватьВсеДоступныеОстаткиПоСтатьямФинансирования
	|ИЗ
	|	Справочник.НастройкиРасчетаРезервовПоОплатеТруда КАК НастройкиРасчетаРезервовПоОплатеТруда
	|ГДЕ
	|	НастройкиРасчетаРезервовПоОплатеТруда.Организация = &Организация
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.72";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура = "РезервыПоОплатеТрудаРасширенный.УстановитьПризнакРасчетаПоМРОТ";
	Обработчик.НачальноеЗаполнение = Ложь;
...
Процедура УстановитьПризнакРасчетаПоМРОТ() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	НастройкиРасчетаРезервовПоОплатеТруда.Ссылка КАК Обработано
	|ИЗ
	|	Справочник.НастройкиРасчетаРезервовПоОплатеТруда КАК НастройкиРасчетаРезервовПоОплатеТруда
	|ГДЕ
	|	НастройкиРасчетаРезервовПоОплатеТруда.УчитыватьМРОТПриОплатеПоСреднемуЗаработку
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Справочник.НастройкиРасчетаРезервовПоОплатеТруда КАК НастройкиРасчетаРезервовПоОплатеТруда
	|ГДЕ
	|	НЕ НастройкиРасчетаРезервовПоОплатеТруда.УчитыватьМРОТПриОплатеПоСреднемуЗаработку";
	...
	Пока Выборка.Следующий() Цикл
		СправочникОбъект = Выборка.Ссылка.ПолучитьОбъект();
		СправочникОбъект.УчитыватьМРОТПриОплатеПоСреднемуЗаработку = Истина;
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(СправочникОбъект);
	КонецЦикла;
...
Процедура ПереопределитьСпособОтраженияСтатьюРасходов(ТаблицаРасчетРезервов, Организация, Резерв, Период, ВидОперации) Экспорт
	...
	ТаблицаКомбинаций = РегистрыСведений.БухучетРезервовПоОплатеТруда.ТаблицаКомбинацийВесаПриоритетаПолейОтбора();
	...
	Запрос.УстановитьПараметр("ПустойРезерв", Справочники.Резервы.ПустаяСсылка());
	Запрос.УстановитьПараметр("ПустойВидОперации", Перечисления.ВидыОперацийРезервовПоОплатеТруда.ПустаяСсылка());
	Запрос.УстановитьПараметр("ПустоеНаправление", Перечисления.НаправленияРасчетовРезервовПоОплатеТруда.ПустаяСсылка());
	Запрос.УстановитьПараметр("ПустойСпособОтражения", Справочники.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка());
	Запрос.УстановитьПараметр("ТаблицаКомбинаций", ТаблицаКомбинаций);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаКомбинаций.Вес КАК Вес,
	|	ТаблицаКомбинаций.Приоритет КАК Приоритет,
	|	ТаблицаКомбинаций.ПоляОтбора КАК ПоляОтбора
	|ПОМЕСТИТЬ ВТ_ТаблицаКомбинаций
	|ИЗ
	|	&ТаблицаКомбинаций КАК ТаблицаКомбинаций
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.Подразделение КАК Подразделение,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.СпособОтраженияЗарплатыВБухучете КАК СпособОтраженияЗарплатыВБухучете,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.СтатьяФинансированияЗарплаты КАК СтатьяФинансирования,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.СпособОтраженияРезервовПоОплатеТрудаВБухучете КАК СпособОтраженияРезервовПоОплатеТрудаВБухучете,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.СтатьяФинансированияРезервовПоОплатеТруда КАК СтатьяФинансированияРезервовПоОплатеТруда,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.Приоритет КАК Приоритет,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.Вес КАК Вес,
	|	ВТ_ТаблицаКомбинаций.ПоляОтбора КАК ПоляОтбора,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.ВидОперации КАК ВидОперации,
	|	БухучетРезервовПоОплатеТрудаСрезПоследних.Направление КАК Направление,
	|	ВЫБОР
	|		КОГДА БухучетРезервовПоОплатеТрудаСрезПоследних.Направление = &ПустоеНаправление
	|			ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Порядок1,
	|	ВЫБОР
	|		КОГДА БухучетРезервовПоОплатеТрудаСрезПоследних.СпособОтраженияРезервовПоОплатеТрудаВБухучете = &ПустойСпособОтражения
	|			ТОГДА 2
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Порядок2
	|ИЗ
	|	РегистрСведений.БухучетРезервовПоОплатеТруда.СрезПоследних(
	|			&Период,
	|			(Организация = &Организация
	|				ИЛИ Организация = &ПустаяОрганизация)
	|				И (Резерв = &Резерв
	|					ИЛИ Резерв = &ПустойРезерв)
	|				И (КОНЕЦПЕРИОДА(ДатаОкончания, МЕСЯЦ) >= НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|					ИЛИ ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1))
	|				И (ВидОперации = &ВидОперации
	|					ИЛИ ВидОперации = &ПустойВидОперации)) КАК БухучетРезервовПоОплатеТрудаСрезПоследних
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаКомбинаций КАК ВТ_ТаблицаКомбинаций
	|		ПО БухучетРезервовПоОплатеТрудаСрезПоследних.Вес = ВТ_ТаблицаКомбинаций.Вес
	|
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет,
	|	Порядок1,
	...
	ОбщаяТаблицаСоответствий = Результат.Выгрузить();
	
	ТаблицаРасчетРезервовОбщая = ТаблицаРасчетРезервов.Скопировать();
	ТаблицаРасчетРезервовОбщая.Колонки.Добавить("Обработано", Новый ОписаниеТипов("Булево"));
	
	ТаблицаРасчетРезервовНовая = ТаблицаРасчетРезервовОбщая.СкопироватьКолонки();
	ТаблицаРасчетРезервовПеренесено = ТаблицаРасчетРезервовНовая.СкопироватьКолонки();
	ТаблицаРасчетРезервовСписано = ТаблицаРасчетРезервовНовая.СкопироватьКолонки();
	ТаблицаРасчетРезервовНачислено = ТаблицаРасчетРезервовНовая.СкопироватьКолонки();
	
	КолонкиНакоплено = "СуммаРезерваНакоплено,СуммаРезерваНакопленоНУ,СуммаРезерваСтраховыхВзносовНакоплено,СуммаРезерваСтраховыхВзносовНакопленоНУ,
		|СуммаРезерваФССНесчастныеСлучаиНакоплено,СуммаРезерваФССНесчастныеСлучаиНакопленоНУ";
	КолонкиЗачтено = "СуммаРезерва,СуммаРезерваНУ,СуммаРезерваСтраховыхВзносов,СуммаРезерваСтраховыхВзносовНУ,
		|СуммаРезерваФССНесчастныеСлучаи,СуммаРезерваФССНесчастныеСлучаиНУ";
	ОбщийСписокКолонок = КолонкиНакоплено + "," + КолонкиЗачтено;
	КолонкиУчета = "СпособОтраженияЗарплатыВБухучете,СтатьяФинансирования";
	
	МассивИменКолонок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КолонкиЗачтено,,Истина, Истина);
	Для Каждого СтрокаТаблицы Из ТаблицаРасчетРезервовОбщая Цикл
		Если СтрокаТаблицы.ФиксЗаполнение Или СтрокаТаблицы.ФиксСтрока Тогда
			ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовНовая.Добавить(), СтрокаТаблицы);
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Начисление
			Или ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация Тогда
			Если СтрокаТаблицы.Перемещен Тогда
				ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовПеренесено.Добавить(), СтрокаТаблицы, , КолонкиУчета);
			Иначе
				Если СтрокаТаблицы.Зачислен Тогда
					ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовПеренесено.Добавить(), СтрокаТаблицы, , КолонкиЗачтено + "," + КолонкиУчета);
				КонецЕсли;
				НоваяСтрокаНачислено = Неопределено;
				НоваяСтрокаСписано = Неопределено;
				Для Каждого ИмяКолонки Из МассивИменКолонок Цикл
					Если СтрокаТаблицы[ИмяКолонки] < 0 Тогда
						Если НоваяСтрокаСписано = Неопределено Тогда
							НоваяСтрокаСписано = ТаблицаРасчетРезервовСписано.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрокаСписано, СтрокаТаблицы, , ОбщийСписокКолонок);
						КонецЕсли;
						НоваяСтрокаСписано[ИмяКолонки] = СтрокаТаблицы[ИмяКолонки];
					ИначеЕсли СтрокаТаблицы[ИмяКолонки] > 0 Тогда
						Если НоваяСтрокаНачислено = Неопределено Тогда
							НоваяСтрокаНачислено = ТаблицаРасчетРезервовНачислено.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрокаНачислено, СтрокаТаблицы, , ОбщийСписокКолонок);
						КонецЕсли;
						НоваяСтрокаНачислено[ИмяКолонки] = СтрокаТаблицы[ИмяКолонки];
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Списание Тогда
			Если СтрокаТаблицы.Перемещен Тогда
				ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовПеренесено.Добавить(), СтрокаТаблицы);
			Иначе
				ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовСписано.Добавить(), СтрокаТаблицы);
			КонецЕсли;
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Корректировка Тогда
			Если СтрокаТаблицы.Зачислен Тогда
				ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовПеренесено.Добавить(), СтрокаТаблицы, , КолонкиЗачтено + "," + КолонкиУчета);
				ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовНачислено.Добавить(), СтрокаТаблицы, , КолонкиНакоплено);
			Иначе
				ЗаполнитьЗначенияСвойств(ТаблицаРасчетРезервовНачислено.Добавить(), СтрокаТаблицы);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	МассивТаблицРасчета = Новый Массив;
	МассивТаблицРасчета.Добавить(Новый Структура("Направление, Таблицы",
		Перечисления.НаправленияРасчетовРезервовПоОплатеТруда.Начислено, 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТаблицаРасчетРезервовНачислено)));
	МассивТаблицРасчета.Добавить(Новый Структура("Направление, Таблицы",
		Перечисления.НаправленияРасчетовРезервовПоОплатеТруда.Списано,
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТаблицаРасчетРезервовСписано)));
	МассивТаблицРасчета.Добавить(Новый Структура("Направление, Таблицы",
		Перечисления.НаправленияРасчетовРезервовПоОплатеТруда.Перенесено,
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТаблицаРасчетРезервовПеренесено)));
	
	ВидыОпераций = Новый Массив;
	ВидыОпераций.Добавить(ВидОперации);
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийРезервовПоОплатеТруда.ПустаяСсылка());
	
	Для Каждого СтрокаМассива Из МассивТаблицРасчета Цикл
		Для Каждого ТаблицаРасчетРезервовДляОбработки Из СтрокаМассива.Таблицы Цикл
			Если Не ЗначениеЗаполнено(ТаблицаРасчетРезервовДляОбработки) Тогда
				Продолжить;
			КонецЕсли;
			Для Каждого ТекущийВидОперации Из ВидыОпераций Цикл
				ТаблицаСоответствий = ОбщаяТаблицаСоответствий.Скопировать(Новый Структура("ВидОперации", ТекущийВидОперации));
				Если ТаблицаСоответствий.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;
				ТаблицаСоответствий.Сортировать("Порядок1, Порядок2, Приоритет");
				
				Отбор = Новый Структура("Подразделение, СпособОтраженияЗарплатыВБухучете, СтатьяФинансирования, Обработано");
				Для Каждого СтрокаСоответствий Из ТаблицаСоответствий Цикл
					Если СтрокаСоответствий.Направление = СтрокаМассива.Направление
						Или СтрокаСоответствий.Направление = Перечисления.НаправленияРасчетовРезервовПоОплатеТруда.ПустаяСсылка() Тогда
						ОписаниеСтруктуры = "Обработано," + СтрокаСоответствий.ПоляОтбора;
						Отбор = Новый Структура(ОписаниеСтруктуры);
						ЗаполнитьЗначенияСвойств(Отбор, СтрокаСоответствий);
						Отбор.Обработано = Ложь;
						
						МассивСтрокДляЗамены = ТаблицаРасчетРезервовДляОбработки.НайтиСтроки(Отбор);
						Для Каждого СтрокаДляЗамены Из МассивСтрокДляЗамены Цикл
							Если СтрокаДляЗамены.Обработано Тогда
								Продолжить;
							КонецЕсли;
							Если ЗначениеЗаполнено(СтрокаСоответствий.СпособОтраженияРезервовПоОплатеТрудаВБухучете) Тогда
								СтрокаДляЗамены.СпособОтраженияЗарплатыВБухучете =
									СтрокаСоответствий.СпособОтраженияРезервовПоОплатеТрудаВБухучете;
							КонецЕсли;
							СтрокаДляЗамены.СтатьяФинансирования =
								СтрокаСоответствий.СтатьяФинансированияРезервовПоОплатеТруда;
							СтрокаДляЗамены.Обработано = Истина;
						КонецЦикла;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаРасчетРезервовДляОбработки, ТаблицаРасчетРезервовНовая);
		КонецЦикла;
	КонецЦикла;
	
	ТаблицаРасчетРезервов = ТаблицаРасчетРезервовНовая;
	
	РезервыПоОплатеТрудаПереопределяемый.ПереопределитьСпособОтраженияСтатьюРасходов(ТаблицаРасчетРезервов, Организация, Резерв, Период, ВидОперации);
...
Процедура ДополнитьЗапросТаблицыВходящихОстатков(Запрос) Экспорт
	...
	// Обработка условий ВТ_ОстаткиДляГруппировки
	ПакетЗапросов = СхемаЗапроса.ПакетЗапросов[2];
	Условия = Новый Массив;
	Для Каждого Отбор Из ПакетЗапросов.Операторы[1].Отбор Цикл
		Условия.Добавить(Отбор);
	КонецЦикла;
	ИсходныеУсловие = Новый ВыражениеСхемыЗапроса(СтрСоединить(Условия, " И "));
	Условия = Новый Массив;
	Условия.Добавить(ИсходныеУсловие);
	Условия.Добавить("ОценочныеОбязательстваПоСотрудникамОбороты.Регистратор ССЫЛКА Документ.ПереносРезервовМеждуСтатьямиФинансирования");
	УсловиеПоТипамДокументов = Новый ВыражениеСхемыЗапроса(СтрСоединить(Условия, " ИЛИ "));
	ПакетЗапросов.Операторы[1].Отбор.Очистить();
	ПакетЗапросов.Операторы[1].Отбор.Добавить(УсловиеПоТипамДокументов);
...
Процедура ДополнитьЗапросТаблицыКорректировок(Запрос) Экспорт
	...
	// Обработка условий ВТ_ТекущиеОборотыСотрудников
	ПакетЗапросов = СхемаЗапроса.ПакетЗапросов[1];
	Условия = Новый Массив;
	Для Каждого Отбор Из ПакетЗапросов.Операторы[0].Отбор Цикл
		Условия.Добавить(Отбор);
	КонецЦикла;
	ИсходныеУсловие = Новый ВыражениеСхемыЗапроса(СтрСоединить(Условия, " И "));
	Условия = Новый Массив;
	...
	Условия.Добавить("ОценочныеОбязательстваПоСотрудникамОбороты.Регистратор ССЫЛКА Документ.ПереносРезервовМеждуСтатьямиФинансирования");
	УсловиеПоТипамДокументов = Новый ВыражениеСхемыЗапроса(СтрСоединить(Условия, " ИЛИ "));
	ПакетЗапросов.Операторы[0].Отбор.Очистить();
	ПакетЗапросов.Операторы[0].Отбор.Добавить(УсловиеПоТипамДокументов);
	
	// Обработка условий ВТ_ТекущиеОборотыСводные
	ПакетЗапросов = СхемаЗапроса.ПакетЗапросов[4];
	Условия = Новый Массив;
	Для Каждого Отбор Из ПакетЗапросов.Операторы[0].Отбор Цикл
		Условия.Добавить(Отбор);
	КонецЦикла;
	...
	Условия = Новый Массив;
	Условия.Добавить(ИсходныеУсловие);
	Условия.Добавить("ОценочныеОбязательстваОбороты.Регистратор ССЫЛКА Документ.ПереносРезервовМеждуСтатьямиФинансирования");
	УсловиеПоТипамДокументов = Новый ВыражениеСхемыЗапроса(СтрСоединить(Условия, " ИЛИ "));
	ПакетЗапросов.Операторы[0].Отбор.Очистить();
	ПакетЗапросов.Операторы[0].Отбор.Добавить(УсловиеПоТипамДокументов);
