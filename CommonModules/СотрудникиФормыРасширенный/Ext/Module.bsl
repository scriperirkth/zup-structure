...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОбновитьРежимыРаботыФормы");
КонецПроцедуры
...
Процедура СотрудникиПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников") Тогда
		МодульФормы = ОбщегоНазначения.ОбщийМодуль("АттестацииСотрудниковФормы");
		МодульФормы.ПриСозданииНаСервереФормыСотрудника(Форма);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.МатериальноОтветственныеЛица") Тогда
		МодульМатериальноОтветственныеЛица = ОбщегоНазначения.ОбщийМодуль("МатериальноОтветственныеЛица");
		МодульМатериальноОтветственныеЛица.СоздатьКомандуМатериальнаяОтветственностьВФормеСотрудника(Форма, "ГруппаДополнительныеДанные2");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
		МодульФормы = ОбщегоНазначения.ОбщийМодуль("Наставничество");
		МодульФормы.СоздатьКомандуНаставничествоВФорме(Форма, Форма.Элементы.ГруппаНаставничество, ВидКнопкиФормы.Гиперссылка);
	КонецЕсли;
	
	//Дисциплинарные взыскания
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ДисциплинарныеВзыскания") Тогда
		МодульФормы = ОбщегоНазначения.ОбщийМодуль("ДисциплинарныеВзыскания");
		МодульФормы.ПриСозданииНаСервереФормыСотрудника(Форма);
	КонецЕсли;
...
Функция СообщениеОКонфликтахВидовЗанятостиСотрудников(ТаблицаПроверяемыхСотрудников, Организация, ИсключаемыеРегистраторы, ИсключаемыеИзПроверкиСотрудники = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ТаблицаСотрудников.ВидЗанятости КАК ВидЗанятости,
		|	НАЧАЛОПЕРИОДА(ТаблицаСотрудников.ДатаСобытия, ДЕНЬ) КАК Период,
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		КадровыеДанные = "ГоловнаяОрганизация,Организация,ТабельныйНомер,Должность,ВидЗанятости,ОформленПоТрудовомуДоговору,ДатаПриема,ДатаЗавершенияРаботы";
		...
		Запрос.Текст =
			...
			|	ВЫРАЗИТЬ(ТаблицаПроверяемыхСотрудников.Сотрудник КАК Справочник.Сотрудники).ФизическоеЛицо КАК ФизическоеЛицо,
			|	ВЫБОР
			...
			|		КОГДА ТаблицаСотрудников.Сотрудник = КадровыеДанныеСотрудников.Сотрудник
			|			ТОГДА &Организация
			|		КОГДА КадровыеДанныеСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)
			|			ТОГДА NULL
...
Функция ТаблицаСовпаденийФизическихЛицПоИННСНИЛСДокументаУдостоверяющегоЛичность(ФизическоеЛицоСсылка, ИНН,
	СтраховойНомерПФР, ДокументВид, ДокументСерия, ДокументНомер)
...
// * СообщенияПроверки - Массив из Структура:
//    см. НовоеСообщениеПроверкиУникальности
// * ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц - Булево
Функция НовыйРезультатПроверкиУникальностиФизическогоЛица() Экспорт
	
	РезультатПроверки = Новый Структура;
	
	РезультатПроверки.Вставить("Фамилия", "");
	РезультатПроверки.Вставить("Имя", "");
	РезультатПроверки.Вставить("Отчество", "");
	РезультатПроверки.Вставить("ФизическоеЛицоУникально", Истина);
	РезультатПроверки.Вставить("ДанныеФизическихЛицДоступны", Истина);
	РезультатПроверки.Вставить("ДанныеФизическихЛиц", Новый Массив);
	РезультатПроверки.Вставить("СообщенияПроверки", Новый Массив);
	РезультатПроверки.Вставить("ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц", Ложь);
...
// 
// Возвращаемое значение:
//  см. НовыйРезультатПроверкиУникальностиФизическогоЛица
Функция РезультатПроверкиУникальностиФизическогоЛица(ПараметрыПроверки) Экспорт
	
	ФизическоеЛицо = ПараметрыПроверки.ФизическоеЛицо;
	ИНН = ПараметрыПроверки.ИНН;
	СтраховойНомерПФР = ПараметрыПроверки.СтраховойНомерПФР;
	ДокументВид = ПараметрыПроверки.ДокументВид;
	ДокументСерия = СокрЛП(ПараметрыПроверки.ДокументСерия);
	ДокументНомер = СокрЛП(ПараметрыПроверки.ДокументНомер);
	
	РезультатПроверки = НовыйРезультатПроверкиУникальностиФизическогоЛица();
	
	РеквизитыФизическогоЛица = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ФизическоеЛицо, "Фамилия, Имя, Отчество");
	ЗаполнитьЗначенияСвойств(РезультатПроверки, РеквизитыФизическогоЛица);
	
	ТаблицаСовпадений = ТаблицаСовпаденийФизическихЛицПоИННСНИЛСДокументаУдостоверяющегоЛичность(ФизическоеЛицо, ИНН,
		СтраховойНомерПФР, ДокументВид, ДокументСерия, ДокументНомер);
	...
	РезультатПроверки.ФизическоеЛицоУникально = Ложь;
	ПохожиеФизическиеЛица = ТаблицаСовпадений.ВыгрузитьКолонку("ФизическоеЛицо");
	ЗаполнитьДанныеПохожихФизическихЛиц(РезультатПроверки, ПохожиеФизическиеЛица);
	ЗаполнитьСообщенияПроверкиУникальности(РезультатПроверки, ПараметрыПроверки, ТаблицаСовпадений);
	РезультатПроверки.ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц =
...
// Параметры:
//  РезультатПроверки - см. НовыйРезультатПроверкиУникальностиФизическогоЛица
//  ФизическиеЛица - Массив из СправочникСсылка.ФизическиеЛица
Процедура ЗаполнитьДанныеПохожихФизическихЛиц(РезультатПроверки, ФизическиеЛица)
	...
	ДоступныеФизическиеЛица = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ФизическоеЛицо");
	
	Если ДоступныеФизическиеЛица.Количество() = ФизическиеЛица.Количество() Тогда
		РезультатПроверки.ДанныеФизическихЛицДоступны = Истина;
		ДанныеФизическихЛиц = КадровыйУчет.КадровыеДанныеФизическихЛиц(Истина, ФизическиеЛица,
			СотрудникиФормы.КадровыеДанныеДляФормыПохожихФизическихЛиц(), ТекущаяДатаСеанса());
		РезультатПроверки.ДанныеФизическихЛиц = ОбщегоНазначения.ТаблицаЗначенийВМассив(ДанныеФизическихЛиц);
	Иначе
		РезультатПроверки.ДанныеФизическихЛицДоступны = Ложь;
	КонецЕсли;
...
//  РезультатПроверки - см. НовыйРезультатПроверкиУникальностиФизическогоЛица
//  ПараметрыПроверки - см. СотрудникиКлиентСервер.НовыеПараметрыПроверкиУникальностиФизическогоЛица
//  ТаблицаСовпадений - ТаблицаЗначений
Процедура ЗаполнитьСообщенияПроверкиУникальности(РезультатПроверки, ПараметрыПроверки, ТаблицаСовпадений)
	...
	Если ЗначениеЗаполнено(ИНН) Тогда
		...
		Если СтрокиСИНН.Количество() > 0 Тогда
			Сообщение = НовоеСообщениеПроверкиУникальности("ИНН", "ФизическоеЛицо");
			Сообщение.ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдена запись о человеке, имеющем такой же ИНН (%1)'"), ИНН);
			РезультатПроверки.СообщенияПроверки.Добавить(Сообщение);
		КонецЕсли;
	КонецЕсли; 
	
	Если КадровыйУчетКлиентСервер.СНИЛСЗаполнен(СтраховойНомерПФР) Тогда
		СтрокиСоСНИЛС = ТаблицаСовпадений.НайтиСтроки(Новый Структура("СтраховойНомерПФР", СтраховойНомерПФР));
		Если СтрокиСоСНИЛС.Количество() > 0 Тогда
			Сообщение = НовоеСообщениеПроверкиУникальности("СтраховойНомерПФР", "ФизическоеЛицо");
			Сообщение.ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдена запись о человеке, имеющем такой же СНИЛС (%1)'"), СтраховойНомерПФР);
			РезультатПроверки.СообщенияПроверки.Добавить(Сообщение);
		КонецЕсли;
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(ДокументВид) Тогда
		СтрокиСДокументом = ТаблицаСовпадений.НайтиСтроки(
			Новый Структура("ДокументВид, ДокументСерия, ДокументНомер", ДокументВид, ДокументСерия, ДокументНомер));
		Если СтрокиСДокументом.Количество() > 0 Тогда
			Сообщение = НовоеСообщениеПроверкиУникальности("Документ", "ДокументыФизическихЛиц");
			Сообщение.ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Найдена запись о человеке, имеющем такой же документ, удостоверяющий личность (%1 №%2 %3)'"),
				ДокументВид, ?(ПустаяСтрока(ДокументСерия), "", " " + ДокументСерия), ДокументНомер);
			РезультатПроверки.СообщенияПроверки.Добавить(Сообщение);
		КонецЕсли;
	КонецЕсли;
...
Функция НовоеСообщениеПроверкиУникальности(ИмяПоля, ИмяОбъекта)
	
	СообщениеПроверки = Новый Структура;
	
	СообщениеПроверки.Вставить("ИмяПоля", ИмяПоля);
	СообщениеПроверки.Вставить("ИмяОбъекта", ИмяОбъекта);
	СообщениеПроверки.Вставить("ТекстСообщенияОбОшибке", "");
...
Процедура ПроверитьУникальностьФизическогоЛицаВФорме(Форма, Отказ)
	...
	Если Форма.ДокументыФизическихЛиц = Неопределено Тогда
		МенеджерЗаписиДокументыФизическихЛиц = СотрудникиФормыБазовый.МенеджерПоследнейЗаписиДокументовФизическихЛиц(
			Форма.ФизическоеЛицоСсылка);
		Форма.ДокументыФизическихЛиц = Новый ФиксированнаяСтруктура(
			ОбщегоНазначения.СтруктураПоМенеджеруЗаписи(МенеджерЗаписиДокументыФизическихЛиц,
				Метаданные.РегистрыСведений.ДокументыФизическихЛиц));
	КонецЕсли;
	
	ПараметрыПроверки = СотрудникиКлиентСервер.НовыеПараметрыПроверкиУникальностиФизическогоЛица();
	ПараметрыПроверки.ФизическоеЛицо = Форма.ФизическоеЛицоСсылка;
	ПараметрыПроверки.ДокументВид = Форма.ДокументыФизическихЛиц.ВидДокумента;
	ПараметрыПроверки.ДокументСерия = Форма.ДокументыФизическихЛиц.Серия;
	ПараметрыПроверки.ДокументНомер = Форма.ДокументыФизическихЛиц.Номер;
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, Форма.ФизическоеЛицо, "ИНН, СтраховойНомерПФР");
	
	РезультатПроверки = РезультатПроверкиУникальностиФизическогоЛица(ПараметрыПроверки);
	
	Если РезультатПроверки.ФизическоеЛицоУникально Тогда
		Возврат;
	КонецЕсли;
	
	ЭтоНовоеФизическоеЛицо = Форма.Параметры.Ключ.Пустая();
	Если Не ЭтоНовоеФизическоеЛицо И РезультатПроверки.ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц Тогда
		Возврат;
	КонецЕсли;
		
	Для каждого СообщениеПроверки Из РезультатПроверки.СообщенияПроверки Цикл
		ОбщегоНазначения.СообщитьПользователю(СообщениеПроверки.ТекстСообщенияОбОшибке, , СообщениеПроверки.ИмяПоля,
			СообщениеПроверки.ИмяОбъекта, Отказ);
	КонецЦикла;
	
	Если ЭтоНовоеФизическоеЛицо Тогда
		ТекстСообщения = НСтр("ru = 'Запись данных нового человека невозможна.'");
	Иначе
		ТекстСообщения =
			НСтр("ru = 'Запись невозможна. Для разрешения конфликтов, обратитесь к администратору информационной системы.'");
	КонецЕсли;
...
Функция СостояниеИРолиСотрудника(СотрудникСсылка)
	...
	// ВТФизическиеЛица повышает производительность запроса в условиях отсутствия индекса по полю Сотрудник
	// в документах ДоговорАвторскогоЗаказа и ДоговорРаботыУслуги.
	...
	Запрос.Текст =
		...
		|		ПО ВТФизическиеЛица.ФизическоеЛицо = ДоговорАвторскогоЗаказа.ФизическоеЛицо
		|ГДЕ
		...
		|		ПО ВТФизическиеЛица.ФизическоеЛицо = ДоговорРаботыУслуги.ФизическоеЛицо
		|ГДЕ
...
Функция ДоговорГПХНеЗакрытыйАктом(Сотрудник) Экспорт
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ДоговорАвторскогоЗаказа.Ссылка КАК Ссылка,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДоговорыПоАктам.Ссылка КАК Ссылка,
		|	ДоговорыПоАктам.Сумма КАК Сумма,
		|	ДоговорыПоАктам.ДатаОкончания КАК ДатаОкончания,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДоговорыСАктами.Ссылка КАК Ссылка
		|ИЗ
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	Если ПараметрыПостроения = Неопределено ИЛИ ПараметрыПостроения.ОформленПоТрудовомуДоговору Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.КадровыеРешения") Тогда
			...
			МодульКадровыеРешенияРасширенный.ДобавитьПодменюРешенияПоКадровымИзменениям(ОписаниеМеню);
		КонецЕсли;
	КонецЕсли;
	...
	//Раздел "Аттестация"
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников") Тогда
		МодульАттестацииСотрудниковФормы = ОбщегоНазначения.ОбщийМодуль("АттестацииСотрудниковФормы");
		МодульАттестацииСотрудниковФормы.ДобавитьПодменюАттестации(ОписаниеМеню);
	КонецЕсли;
	
	//Раздел "Дисциплинарные взыскания"
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ДисциплинарныеВзыскания") Тогда
		МодульДисциплинарныеВзыскания = ОбщегоНазначения.ОбщийМодуль("ДисциплинарныеВзыскания");
		МодульДисциплинарныеВзыскания.ДобавитьПодменюДисциплинарныеВзыскания(ОписаниеМеню);
	КонецЕсли;
...
Функция ТекстыЗапросовФормыВыбораСотрудников()
	...
	Если ПараметрыСеанса.АльтернативныйПодборСпискаСотрудников Тогда
		...
		ТекстыЗапросов.Текст =
			...
			|		ПО ДанныеДляПодбора.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
			...
			|		ПО ДанныеДляПодбора.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
