...
Процедура ПодписатьЭПФайлыОтвета(Форма, ОписаниеОповещения, МассивФайлов, ЭтоФайлПечатнойФормы = Ложь) Экспорт
	Если ЭтоФайлПечатнойФормы Тогда	
		ДополнительныеПараметрыФормы = ЭДОБЗККлиент.ДополнительныеПараметрыФормыПодписанияДокументов(
			ОписаниеОповещения, Истина, Истина);
		ЭДОБЗККлиент.ОткрытьФормуПодписанияПечатныхФорм(МассивФайлов,
															 Форма,
	...
	Иначе
		Если Форма.ИспользуетсяЭДОБЗК Тогда
			ЭДОБЗККлиент.ПодписатьФайлы(
				ЭДОБЗКВызовСервера.ДанныеФайловНаПодпись(МассивФайлов, Форма.УникальныйИдентификатор),
				ОписаниеОповещения,
				Форма,
				ПредопределенноеЗначение("Перечисление.ДействияСФайламиДокументовЭДОБЗК.ПередатьВКабинетСотрудников"));
		Иначе
...
Процедура ПоказатьОснованиеДокументаЭДОБЗК(ОснованиеДокумента, СтандартнаяОбработка) Экспорт
