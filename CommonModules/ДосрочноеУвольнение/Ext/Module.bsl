...
Процедура УвольнениеЗаполнитьКомпенсациюЗаНеотработанноеВремя(Форма, ТаблицаНачислений, МенеджерРасчета, ДополнительныеВидыРасчета) Экспорт
		...
		Если Объект.КоличествоДнейЧасовКомпенсацииПриУвольнении > 0 Тогда
			...
			Если ЭтоГосслужащий Тогда
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
					СтруктураОбъекта = Форма.ПодготовитьСтруктуруОбъектаДляРасчетаДенежногоСодержания();
			...
			Иначе
				...
				ДополнительныеПараметры.ПоЧасам = Форма.ИспользуетсяСреднечасовойЗаработок;
			...
			ВидыРасчетаДолейРКСН = ДополнительныеВидыРасчета.Получить(Объект.КомпенсацияЗаНеотработанныеДниЧасыПриУвольнении);
			Если ВидыРасчетаДолейРКСН <> Неопределено И Объект.ПланируемаяДатаВыплаты >= УчетНДФЛРасширенный.ДатаЗаконаПроРКиЕАЭС() Тогда
