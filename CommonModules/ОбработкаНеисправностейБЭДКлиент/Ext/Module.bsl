...
Процедура ОбработатьОшибки(КонтекстДиагностики, ПараметрыОбработкиОшибок = Неопределено) Экспорт
	...
	Если ПараметрыОбработкиОшибок.КонтекстныйРежимОбработки Тогда
		...
		ПереключитьОтображенияПодсказкиДляОбходаОсобенностиВебКлиента(ПараметрыОбработкиОшибок.НадписьПредупреждение);
...
Процедура ДополнитьКонтекстДиагностики(Приемник, Источник) Экспорт
	ОбработкаНеисправностейБЭДКлиентСервер.ДополнитьКонтекстДиагностики(Приемник, Источник);
КонецПроцедуры
...
Процедура ЗаполнитьДанныеСлужбыПоддержки(ТелефонСлужбыПоддержки, АдресЭлектроннойПочтыСлужбыПоддержки, КонтекстДиагностики = Неопределено) Экспорт

	// Контактные данные ЗАО "Калуга Астрал"
	ТелефонСлужбыПоддержки = "8-800-333-9313";
	АдресЭлектроннойПочтыСлужбыПоддержки = "edo@1c.ru";
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС")
		И НЕ КонтекстДиагностики = Неопределено Тогда
		МодульЭлектронноеАктированиеЕИСКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЭлектронноеАктированиеЕИСКлиент");
		МодульЭлектронноеАктированиеЕИСКлиент.ЗаполнитьДанныеСлужбыПоддержкиЕИС(ТелефонСлужбыПоддержки,
...
#КонецОбласти // ОбработчикиФормыОшибок
...
// Параметры:
//  ЭлементНадписи - ДекорацияФормы
Процедура ПереключитьОтображенияПодсказкиДляОбходаОсобенностиВебКлиента(ЭлементНадписи)
	Если ЭтоРасширеннаяПодсказка(ЭлементНадписи) Тогда
		ТекущееЗначениеОтображенияПодсказки = ЭлементНадписи.Родитель.ОтображениеПодсказки;
		ЭлементНадписи.Родитель.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		ЭлементНадписи.Родитель.ОтображениеПодсказки = ТекущееЗначениеОтображенияПодсказки;
	КонецЕсли;
КонецПроцедуры
...
// Возвращаемое значение:
//  Булево
Функция ЭтоРасширеннаяПодсказка(ЭлементНадписи)
	Возврат ТипЗнч(ЭлементНадписи.Родитель) <> Тип("ФормаКлиентскогоПриложения") И ЭлементНадписи.Родитель.РасширеннаяПодсказка = ЭлементНадписи;
КонецФункции
