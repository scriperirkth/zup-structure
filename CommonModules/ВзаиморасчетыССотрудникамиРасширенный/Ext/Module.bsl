...
Процедура ЗарегистрироватьНачисленныйАванс(Движения, Отказ, Организация, ПериодРегистрации, Начисления = Неопределено, Удержания = Неопределено) Экспорт
	...
	ПорядокВыплаты = ОбщегоНазначения.ПредопределенныйЭлемент("Перечисление.ХарактерВыплатыЗарплаты.Аванс");
	...
	Если ЕстьДвижениеПорядокВыплатыДокументов Тогда
		Если Движения.ПорядокВыплатыДокументов.Количество() = 0 Тогда 
			...
			ПорядокВыплатыДокумента.ДокументОснование = Движения.ЗарплатаКВыплатеАвансом.Отбор.Регистратор.Значение;
...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	УправлениеСвойствамиБЗК.ЗарегистрироватьНаборСвойств(Наборы, "d42dffff-9802-11e9-80cd-4cedfb43b11a", Метаданные.Документы.ПлановоеПогашениеЗадолженностиСотрудников);
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Документы.ПлановоеПогашениеЗадолженностиСотрудников, Истина);
	Списки.Вставить(Метаданные.Справочники.ПлановоеПогашениеЗадолженностиСотрудниковПрисоединенныеФайлы, Истина);
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	|Документ.ПлановоеПогашениеЗадолженностиСотрудников.Чтение.ГруппыФизическихЛиц
	|Документ.ПлановоеПогашениеЗадолженностиСотрудников.Чтение.Организации
	|Документ.ПлановоеПогашениеЗадолженностиСотрудников.Изменение.ГруппыФизическихЛиц
	|Документ.ПлановоеПогашениеЗадолженностиСотрудников.Изменение.Организации
	|Справочник.ПлановоеПогашениеЗадолженностиСотрудниковПрисоединенныеФайлы.Чтение.ГруппыФизическихЛиц
	|Справочник.ПлановоеПогашениеЗадолженностиСотрудниковПрисоединенныеФайлы.Чтение.Организации
	|Справочник.ПлановоеПогашениеЗадолженностиСотрудниковПрисоединенныеФайлы.Изменение.ГруппыФизическихЛиц
	|Справочник.ПлановоеПогашениеЗадолженностиСотрудниковПрисоединенныеФайлы.Изменение.Организации
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.8";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f878e2a7-98c0-4403-8895-00fbd4b7482c");
	Обработчик.Процедура = "СпособыВыплатыЗарплатыРасширенный.УточнитьСпособВыплатыАванса";
	Обработчик.Комментарий = НСтр("ru = 'Уточнение способа выплаты аванса ""Начисление за первую половину месяца"".'");
...
Функция ВыплатаПроизводилась(Организация, ДокументОснование, ПериодВзаиморасчетов) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если ЗарегистрированаНачисленнаяЗарплатаДокументом(ДокументОснование) Тогда
		...
		Сотрудник = ЕдинственныйСотрудникДокумента(ДокументОснование);
		
	КонецЕсли;
	
	// Движений по документу не было, значит определим была ли выплата по периоду
	Возврат ЗарегистрированаВыплаченнаяЗарплатаЗаПериод(Организация, ПериодВзаиморасчетов, Сотрудник);
...
Процедура СоздатьВТЗарплатаКВыплатеАванс(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники) 
	...
	ВычестьНеавансовыеВыплатыАвансовыхДокументовОснований(МенеджерВременныхТаблиц, Параметры);
	
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСотрудникиБезОкончательногоРасчета");
	
КонецПроцедуры	

Процедура ВычестьНеавансовыеВыплатыАвансовыхДокументовОснований(МенеджерВременныхТаблиц, Параметры)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц; 
	...
	|ПОМЕСТИТЬ ВТПоложительнаяЗарплатаКВыплате
	|ИЗ
	...
	|ПОМЕСТИТЬ ВТЗарплатаКВыплатеРасход
	|ИЗ
	|	ВТПоложительнаяЗарплатаКВыплате КАК ВТЗарплатаКВыплате
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗарплатаКВыплате КАК ЗарплатаКВыплате
	|		ПО (ЗарплатаКВыплате.Сотрудник = ВТЗарплатаКВыплате.Сотрудник)
	|			И (ЗарплатаКВыплате.ФизическоеЛицо = ВТЗарплатаКВыплате.ФизическоеЛицо)
	|			И (ЗарплатаКВыплате.ДокументОснование = ВТЗарплатаКВыплате.ДокументОснование)
	|			И (ЗарплатаКВыплате.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход))
	|			И (ЗарплатаКВыплате.Организация = ВТЗарплатаКВыплате.Организация)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗарплатаКВыплате.Сотрудник,
	|	ЗарплатаКВыплате.ФизическоеЛицо,
	...
	|	ЗарплатаКВыплате.ВидДоходаИсполнительногоПроизводства КАК ВидДоходаИсполнительногоПроизводства,
	|	ЗарплатаКВыплате.ДокументОснование КАК ДокументОснование,
	|	СУММА(ЕСТЬNULL(ЗарплатаКВыплатеАвансом.СуммаКВыплате, 0)) КАК СуммаКВыплате
	|ПОМЕСТИТЬ ВТЗарплатаКВыплатеАвансомРасход
	|ИЗ
	|	ВТЗарплатаКВыплатеРасход КАК ЗарплатаКВыплате
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗарплатаКВыплатеАвансом КАК ЗарплатаКВыплатеАвансом
	|		ПО (ЗарплатаКВыплатеАвансом.Сотрудник = ЗарплатаКВыплате.Сотрудник)
	|			И (ЗарплатаКВыплатеАвансом.ФизическоеЛицо = ЗарплатаКВыплате.ФизическоеЛицо)
	|			И (ЗарплатаКВыплатеАвансом.ПериодВзаиморасчетов = ЗарплатаКВыплате.ПериодВзаиморасчетов)
	|			И (ЗарплатаКВыплатеАвансом.СтатьяФинансирования = ЗарплатаКВыплате.СтатьяФинансирования)
	|			И (ЗарплатаКВыплатеАвансом.СтатьяРасходов = ЗарплатаКВыплате.СтатьяРасходов)
	|			И (ЗарплатаКВыплатеАвансом.ВидДоходаИсполнительногоПроизводства = ЗарплатаКВыплате.ВидДоходаИсполнительногоПроизводства)
	|			И (ЗарплатаКВыплатеАвансом.ДокументОснование = ЗарплатаКВыплате.ДокументОснование)
	|			И (ЗарплатаКВыплатеАвансом.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход))
	|			И (ЗарплатаКВыплатеАвансом.Организация = ЗарплатаКВыплате.Организация)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗарплатаКВыплате.Сотрудник,
	|	ЗарплатаКВыплате.ФизическоеЛицо,
	...
	|	ЗарплатаКВыплатеОстатки.ДокументОснование КАК ДокументОснование,
	|	СУММА(ЗарплатаКВыплатеОстатки.КВыплате) КАК КВыплате
	|ПОМЕСТИТЬ ВТЗарплатаКВыплатеБезНеавансовыхВыплат
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЗарплатаКВыплате.Сотрудник КАК Сотрудник,
	|		ЗарплатаКВыплате.ФизическоеЛицо КАК ФизическоеЛицо,
	...
	|		ЗарплатаКВыплате.ВидДоходаИсполнительногоПроизводства КАК ВидДоходаИсполнительногоПроизводства,
	|		ЗарплатаКВыплате.ДокументОснование КАК ДокументОснование,
	|		-(ЗарплатаКВыплате.СуммаКВыплате - ЕСТЬNULL(ЗарплатаКВыплатеАвансом.СуммаКВыплате, 0)) КАК КВыплате
	|	ИЗ
	|		ВТЗарплатаКВыплатеРасход КАК ЗарплатаКВыплате
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТЗарплатаКВыплатеАвансомРасход КАК ЗарплатаКВыплатеАвансом
	|			ПО ЗарплатаКВыплате.Сотрудник = ЗарплатаКВыплатеАвансом.Сотрудник
	|				И ЗарплатаКВыплате.ФизическоеЛицо = ЗарплатаКВыплатеАвансом.ФизическоеЛицо
	|				И ЗарплатаКВыплате.ПериодВзаиморасчетов = ЗарплатаКВыплатеАвансом.ПериодВзаиморасчетов
	|				И ЗарплатаКВыплате.СтатьяФинансирования = ЗарплатаКВыплатеАвансом.СтатьяФинансирования
	|				И ЗарплатаКВыплате.СтатьяРасходов = ЗарплатаКВыплатеАвансом.СтатьяРасходов
	|				И ЗарплатаКВыплате.ВидДоходаИсполнительногоПроизводства = ЗарплатаКВыплатеАвансом.ВидДоходаИсполнительногоПроизводства
	|				И ЗарплатаКВыплате.ДокументОснование = ЗарплатаКВыплатеАвансом.ДокументОснование
	|				И ЗарплатаКВыплате.Организация = ЗарплатаКВыплатеАвансом.Организация
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ЗарплатаКВыплатеОстатки.КВыплате) <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТЗарплатаКВыплатеРасход
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТЗарплатаКВыплатеАвансомРасход
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.Сотрудник КАК Сотрудник,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.Подразделение КАК Подразделение,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.Организация КАК Организация,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.СтатьяФинансирования КАК СтатьяФинансирования,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.СтатьяРасходов КАК СтатьяРасходов,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.ВидДоходаИсполнительногоПроизводства КАК ВидДоходаИсполнительногоПроизводства,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.ДокументОснование КАК ДокументОснование,
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат.КВыплате КАК КВыплате
	|ПОМЕСТИТЬ ВТЗарплатаКВыплате
	|ИЗ
	|	ВТЗарплатаКВыплатеБезНеавансовыхВыплат КАК ВТЗарплатаКВыплатеБезНеавансовыхВыплат";
...
Процедура СоздатьВТЗарплатаКВыплатеМежрасчет(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники)
	...
	|	ЗарплатаКВыплатеМежрасчет.Организация КАК Организация,
	...
	|	ЗарплатаКВыплатеМежрасчет.Организация,
...
Процедура СоздатьВТПлановыйАванс(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники, КадровыеДанные) Экспорт
	...
	|	АвансБазовый.Организация КАК Организация,
...
//		Организация          - СправочникСсылка.Организация - Организация, в которой проверяется наличие выплат.
//		ПериодВзаиморасчетов - Дата                         - Период, за который требуется проверить выплату зарплаты.
//		Сотрудник            - СправочникСсылка.Сотрудники  - Сотрудник, по которому проверяется наличие выплат.
//
// Возвращаемое значение:
//		Булево - Истина, если за период взаиморасчетов выплачивалась зарплата, иначе Ложь.
//
Функция ЗарегистрированаВыплаченнаяЗарплатаЗаПериод(Организация, ПериодВзаиморасчетов, Сотрудник = Неопределено)
	...
	Если Сотрудник <> Неопределено Тогда
		...
		Запрос.Текст = Запрос.Текст + "И ВзаиморасчетыССотрудниками.Сотрудник = &Сотрудник";
	КонецЕсли;
...
Процедура ПодставитьНулевыеДокументыОснования(МенеджерВременныхТаблиц, Параметры) Экспорт
	...
	Если Параметры.ПорядокВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Зарплата
		...
		Если ЗарплатаКадры.ВТСодержитСтроки(Запрос.МенеджерВременныхТаблиц,"ВТСотрудникиБезНачисенияЗарплаты") Тогда
			...
			|	ВТЗарплатаКВыплатеОграниченнаяСальдоФизлиц.Организация КАК Организация,
...
Функция ЕдинственныйСотрудникДокумента(ДокументСсылка)
	
	МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ДокументСсылка);
	Попытка
		ИмяРеквизитаСотрудник = МенеджерДокумента.ОписаниеДокумента().ИмяРеквизитаСотрудник;
	Исключение
		ИмяРеквизитаСотрудник = Неопределено;
	КонецПопытки;
	
	Если ИмяРеквизитаСотрудник <> Неопределено Тогда
		УстановитьПривилегированныйРежим(Истина);
		Сотрудник = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, ИмяРеквизитаСотрудник);
	Иначе
		Сотрудник = Неопределено;
	КонецЕсли;
