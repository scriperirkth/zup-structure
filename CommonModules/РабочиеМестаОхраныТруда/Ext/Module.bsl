...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьРеквизитИзменитьРабочееМестоВДокументеПеремещениеВДругоеПодразделение");
	Методы.Вставить("СоздатьРабочиеМестаНаОснованииПозицийШтатногоРасписания");
	Методы.Вставить("ОбновитьРеквизитыИДвиженияОтпусковЗаВредность");
	Методы.Вставить("УстановитьИспользованиеОсобыхУсловийТруда");
	Методы.Вставить("ОбновитьДанныеДляПодбораСотрудников");
	Методы.Вставить("УстановитьИспользованиеРабочихМестОхраныТруда");
	Методы.Вставить("ЗаполнитьРабочиеМестаСотрудниковВИсправительномРегистре");
	Методы.Вставить("ВосстановитьИсправленныеРабочиеМестаСотрудников");
	Методы.Вставить("ЗаполнитьРабочиеМестаСотрудников");
	Методы.Вставить("УдалитьРазрядыИзНомеровРабочихМест");
КонецПроцедуры
...
Функция ДобавитьПолеСведенийОРабочихМестахСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Функция ДобавитьКритерийПоискаПоСведениямОРабочихМестахСотрудников(КритерииПоиска, УсловиеПоиска) Экспорт
	...
	Если НеобходимыСведенияОРабочихМестахСотрудников(УсловиеПоиска.ЛевоеЗначение)
...
Процедура УдалитьРазрядыИзНомеровРабочихМест(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ШаблонНПП", "%" + Символы.НПП + "%");
	Запрос.УстановитьПараметр("Обработанные", Обработанные);
	Запрос.Текст =
		"ВЫБРАТЬ
		...
		|	Справочник.РабочиеМестаОхраныТруда КАК РабочиеМестаОхраныТруда
		|ГДЕ
		|	НЕ РабочиеМестаОхраныТруда.НомерВручную
		|	И НЕ РабочиеМестаОхраныТруда.Ссылка В (&Обработанные)
		|	И (РабочиеМестаОхраныТруда.Номер ПОДОБНО &ШаблонНПП
		|		ИЛИ РабочиеМестаОхраныТруда.Наименование ПОДОБНО &ШаблонНПП)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ОбновлениеИБ.ПодготовитьОбновлениеДанных(
			ПараметрыОбновления, "Справочник.РабочиеМестаОхраныТруда", "Ссылка", Выборка.Ссылка) Тогда
			Продолжить;
		КонецЕсли;

		Попытка
			ОбъектРабочееМесто = Выборка.Ссылка.ПолучитьОбъект();
			Если ОбъектРабочееМесто = Неопределено Тогда
				ОбновлениеИБ.ЗавершитьОбновлениеДанных(ПараметрыОбновления);
				Продолжить;
			КонецЕсли;

			ОбъектРабочееМесто.Номер		= СтрЗаменить(ОбъектРабочееМесто.Номер,			Символы.НПП, "");
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось очистить непечатные символы в номере/наименовании рабочего места %1: %2'"),
				Выборка.Ссылка, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации( ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.РабочиеМестаОхраныТруда, Выборка.Ссылка, ТекстСообщения);
		КонецПопытки;
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера(Настройки, Метаданные.Отчеты.ЗанятостьРабочихМестОхраныТруда);
	
	Вариант = ВариантыОтчетов.ОписаниеВарианта(Настройки, Метаданные.Отчеты.ЗанятостьРабочихМестОхраныТруда,
		"ЗанятостьРабочихМестОхраныТруда");
	Вариант.ФункциональныеОпции.Добавить(ИмяФункциональнойОпцииИспользоватьРабочиеМестаОхраныТруда());
...
Процедура ДополнитьКадровуюИсториюРабочимиМестами(Запрос) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрСведений.Период КАК Период",
		"ЕСТЬNULL(КадроваяИсторияРабочиеМеста.Период, РегистрСведений.Период) КАК Период");
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТКадроваяИсторияСотрудниковДляДанныхДляПодбораСотрудников КАК РегистрСведений",
		...
		|			И НЕ РегистрСведений.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)");
	...
	Для Сч = 1 По КоличествоСтрок Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			ИначеЕсли НайденнаяСтрока.РабочееМесто <> СтрокаКадроваяИстория.РабочееМесто
				И НачалоДня(НайденнаяСтрока.ПериодЗаписи) > НачалоДня(СтрокаКадроваяИстория.ПериодЗаписи)
...
Процедура ПриОбработкеПолученияДанныхВыбораСотрудников(Отбор, ТекстыУсловий) Экспорт
	
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Отбор, "РабочееМестоЗаполнено", Ложь) Тогда
		ТекстыУсловий.Добавить("Сотрудники.РабочееМесто <> ЗНАЧЕНИЕ(Справочник.РабочиеМестаОхраныТруда.ПустаяСсылка)");
	КонецЕсли;
...
Функция РасчетПоказателей_1Туст_2020Кв1(ДатаОтчета, Организация, Сотрудники)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНадбавкуЗаВредность") Тогда
		НачислениеНадбавкаЗаВредность = ЗарплатаКадрыРасширенный.НачислениеНадбавкаЗаВредность(Организация, ДатаОтчета);
