...
Процедура РасчетПоказателей_6НДФЛ_2024Кв2(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Если ОписаниеТаблиц.Свойство("Раздел2Доходы") Или ОписаниеТаблиц.Свойство("Раздел2Налоги2024") Или ОписаниеТаблиц.Свойство("КоличествоПолучателейДохода") Или ОписаниеТаблиц.Свойство("Раздел2Вычеты") Или ОписаниеТаблиц.Свойство("Приложение1Налогоплательщики") Тогда
		...
		|		И НЕ СведенияОДоходахНДФЛ.КатегорияДохода В (&АУСН) КАК ДанныеДляОтчета,
		...
		|	НАЧАЛОПЕРИОДА(Доходы.Период, МЕСЯЦ) КАК МесяцНалоговогоПериода,
		|	Доходы.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
		|ПОМЕСТИТЬ ВТДоходыНаАУСН
		|ИЗ
		|	ВТДоходыПоСтавкам КАК Доходы
		|ГДЕ
		|	Доходы.КатегорияДохода В(&АУСН)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	Доходы.КатегорияДохода В(&АУСН)
		|	И Доходы.ЗарегистрированоДляПередачиВБанкНаАУСН
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.РегистрацияВНалоговомОргане В (&РегистрацияВНалоговомОргане)
		|		И ДоходыНаАУСН.ФизическоеЛицо ЕСТЬ NULL КАК ДанныеДляОтчета,
		|	ВЫБОР
		...
		|		ПО ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыНаАУСН КАК ДоходыНаАУСН
		|		ПО ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.ФизическоеЛицо = ДоходыНаАУСН.ФизическоеЛицо
		|			И (НАЧАЛОПЕРИОДА(ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.МесяцНалоговогоПериода, МЕСЯЦ) = ДоходыНаАУСН.МесяцНалоговогоПериода)
		|			И ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.РегистрацияВНалоговомОргане = ДоходыНаАУСН.РегистрацияВНалоговомОргане
		|ГДЕ
		...
		|		И ДоходыНаАУСН.ФизическоеЛицо ЕСТЬ NULL,
		|	ВЫБОР
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыПереданныеВБанкНаАУСН КАК ДоходыПереданныеВБанкНаАУСН
		|		ПО ИмущественныеВычетыНДФЛ.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыНаАУСН КАК ДоходыНаАУСН
		|		ПО ИмущественныеВычетыНДФЛ.ФизическоеЛицо = ДоходыНаАУСН.ФизическоеЛицо
		|			И (НАЧАЛОПЕРИОДА(ИмущественныеВычетыНДФЛ.Период, МЕСЯЦ) = ДоходыНаАУСН.МесяцНалоговогоПериода)
		|			И ИмущественныеВычетыНДФЛ.РегистрацияВНалоговомОргане = ДоходыНаАУСН.РегистрацияВНалоговомОргане
		|ГДЕ
		...
		|	ГДЕ
		|		СтандартныеИмущественныеВычеты.НалоговаяБаза <> ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ПустаяСсылка)
		|		И НЕ СтандартныеИмущественныеВычеты.ЗарегистрированоДляПередачиВБанкНаАУСН) КАК База
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
			...
			РассчитатьПараметрыПревышения(ТаблицаРасчетаНарастающимИтогом, ТаблицаВычетов);
			...
			// Отдельно рассматриваем ситуацию регистрации дохода в дни превышения очередного предела несколькими документами. 
			// Если документ регистрирует доход до/после превышения, эти доходы целиком относим к соответствующему уровню шкалы, 
			// оставляя для распределения по ставкам доходы того документа, в котором произошло превышение.
			// - выделяем в ВТОснованияВДниПревышения документы-основания в дни превышения, отбрасывая случаи переноса дат получения 
			//	дохода при выплате и начисления налога отдельными доками Расчет удержаний - по таким документам нельзя судить о 
			//	соответствии доходов и налогов
			// - сортируем оставшиеся документы-основания этого дня на три группы - не имеющие налога с превышения, не имеющие налога до превышения, имеющие оба налога
			// - первым двум группам вычисляем ставки и КБК по правилам соответствующих уровней шкалы
	...
	Если ОписаниеТаблиц.Свойство("КоличествоПолучателейДохода") Тогда
		...
		|		СведенияОДоходахНДФЛ.КБК КАК КБК
		|	ИЗ
		|		ВТНенулевыеДоходы КАК СведенияОДоходахНДФЛ
		|	
		|	СГРУППИРОВАТЬ ПО
		|		СведенияОДоходахНДФЛ.Ставка,
		|		СведенияОДоходахНДФЛ.КБК
	...
	Если ОписаниеТаблиц.Свойство("Раздел2Вычеты") Тогда
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(СведенияОДоходах.СуммаВычета) <> 0";
	...
	Если ОписаниеТаблиц.Свойство("Приложение1Налогоплательщики") Тогда
		...
		|ИЗ
		|	ВТДоходыДляОтчета КАК СведенияОДоходах";
		СотрудникиДляЗаполнения = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ФизическоеЛицо");
		КадровыйУчет.СоздатьНаДатуВТКадровыеДанныеФизическихЛиц(Запрос.МенеджерВременныхТаблиц, Истина, СотрудникиДляЗаполнения, НеобходимыеКадровыеДанные, ДатаАктуальности);
		
		ЗарплатаКадры.УничтожитьВТ(Запрос.МенеджерВременныхТаблиц,"ВТСтатусыЗастрахованныхФизическихЛицСрезПоследних", Истина);
		КадровыйУчет.СоздатьНаДатуВТКадровыеДанныеФизическихЛиц(Запрос.МенеджерВременныхТаблиц, Истина, СотрудникиДляЗаполнения, "СтатусНалогоплательщика,ВидЗастрахованногоЛица", ОкончаниеПериода, , "ВТСтатусыЗастрахованныхФизическихЛицСрезПоследних");
		...
		|	ВЫБОР
		|		КОГДА СтатусыЗастрахованныхФизическихЛиц.ВидЗастрахованногоЛица В (&ВКС)
		|				И СтатусыЗастрахованныхФизическихЛиц.СтатусНалогоплательщика = ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент)
		|			ТОГДА ""7""
		...
		|			ПО СтатусыЗастрахованныхФизическихЛиц.СтатусНалогоплательщика = СтатусыНалогоплательщиковПоНДФЛ.Ссылка
		|		ПО КадровыеДанныеФизЛиц.ФизическоеЛицо = СтатусыЗастрахованныхФизическихЛиц.ФизическоеЛицо
		...
		|				ИЛИ Налоги.УдержаноЗаНалоговыйПериод <> 0
		|				ИЛИ Налоги.Передано <> 0
		|				ИЛИ Налоги.СуммаДоходаСКоторогоНеУдержанНалог <> 0)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
...
Функция ДанныеОДоходахНалогахВычетах(Сотрудники, НалоговыйПериод, Организация, РегистрацияВНалоговомОргане, ДатаАктуальности, КППОтправки, ВРазрезеРегистраторов, ДополнительныйОтбор) Экспорт
	...
	|ГДЕ
	|	НЕ СведенияОДоходахНДФЛ.КодДохода.НеОблагаетсяУНалоговогоАгента
	|	И СведенияОДоходахНДФЛ.ГоловнаяОрганизация = &ГоловнаяОрганизация
	...
	Иначе
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
			...
			//	соответствии доходов и налогов
			// - сортируем оставшиеся документы-основания этого дня на три группы - не имеющие налога с превышения, не имеющие налога до превышения, имеющие оба налога
			// - первым двум группам вычисляем ставки по правилам соответствующих уровней шкалы
...
// Определяет дни превышения пределов, а также распределяет доходы по ставкам в день превышения очередного предела. 
//
// Параметры:
//		ТаблицаДоходов - ТаблицаЗначений - содержит строки доходов.
//		ТаблицаВычетов - ТаблицаЗначений - суммы вычетов по месяцам.
//
// Ограничения: данные о доходах и вычетах должны относиться к одной головной орг. и одному налоговому периоду.
//
Процедура РассчитатьПараметрыПревышения(ТаблицаДоходов, ТаблицаВычетов)

	ИскатьВычеты = ТаблицаВычетов.Количество() > 0;
	Если ИскатьВычеты Тогда
		// При регистрации крупных имущественных вычетов в некоторых месяцах общая сумма вычета может превысить облагаемые 
		// доходы того же месяца, что мешает алгоритму распределения доходов по ставкам налогообложения с привязкой к месяцу получения дохода. 
		// Переносим такие излишки в предшествующие месяцы с доходами, обеспечивая тем самым помесячное соответствие вычетов доходам.
		ПоискВычетов = Новый Структура("ФизическоеЛицо,НалоговаяБаза,НачалоМесяца");
		ИтогиДоходов = ТаблицаДоходов.Скопировать(, "ФизическоеЛицо,НалоговаяБаза,НачалоМесяца,ОблагаемыйДоход");
		ИтогиДоходов.Свернуть("ФизическоеЛицо,НалоговаяБаза,НачалоМесяца", "ОблагаемыйДоход");
		ИтогиДоходов.Сортировать("ФизическоеЛицо,НалоговаяБаза,НачалоМесяца Убыв");
		ТекущееФизлицо = Неопределено;
		ТекущаяНалоговаяБаза = Неопределено;
		Для каждого СтрокаТаблицы Из ИтогиДоходов Цикл
			...
			Если СтрокиВычетов.Количество() > 0 Тогда
				СтрокаВычетов = СтрокиВычетов[0];
				СтрокаВычетов.Сумма = СтрокаВычетов.Сумма + БудущиеВычеты;
			ИначеЕсли БудущиеВычеты > 0 Тогда
				СтрокаВычетов = ТаблицаВычетов.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаВычетов, СтрокаТаблицы);
				СтрокаВычетов.Сумма = БудущиеВычеты;
			Иначе 
				Продолжить;
			КонецЕсли;
			БудущиеВычеты = 0;
			Если СтрокаВычетов.Сумма > СтрокаТаблицы.ОблагаемыйДоход Тогда
				БудущиеВычеты = СтрокаВычетов.Сумма - СтрокаТаблицы.ОблагаемыйДоход;
				СтрокаВычетов.Сумма = СтрокаВычетов.Сумма - БудущиеВычеты;
			КонецЕсли;
		КонецЦикла;
		Если ТаблицаВычетов.Количество() > 100 Или ТаблицаДоходов.Количество() > 100 Тогда
