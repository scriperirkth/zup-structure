...
Функция ВызватьHTTPМетод(ОписаниеСоединения, HTTPЗапрос, Метод, ВидОперации, КонтекстДиагностики = Неопределено,
	...
	Попытка
		Ответ = ВызватьHTTPМетодРасширенный(Метод, HTTPЗапрос, ОписаниеСоединения.HTTPСоединение, ВидОперации);
...
Функция РасшифровкаКодаСостоянияHTTP(КодСостояния) Экспорт
	...
	ИначеЕсли КодСостояния = 420 Тогда // Not Found
		Расшифровка = НСтр("ru = 'Ресурс на сервере не найден.'");
...
// Возвращаемое значение:
//  Строка
Функция ПредставлениеЗаголовковHTTPСписком(Заголовки) Экспорт
	...
	Для Каждого Заголовок  Из Заголовки Цикл
		...
		КонецЕсли;
		ЧастиПредставления.Добавить(СтрШаблон("%1: %2", КлючЗаголовка, ЗначениеЗаголовка));
	КонецЦикла;
...
// Возвращаемое значение:
//  Строка
Функция СтрокаURLЗапроса(Соединение, Запрос, Метод) Экспорт
...
// Возвращает URL редиректа. Если заголовок Location отсутствует, возвращается Неопределено.
// 
// Параметры:
//  HTTPОтвет - HTTPОтвет
// 
...
//  Строка,Неопределено
//
Функция АдресПеренаправления(Знач HTTPОтвет) Экспорт
	
	Возврат HTTPОтвет.Заголовки.Получить(ИмяЗаголовкаПеренаправления());
	
КонецФункции

// Возвращает интервал повторного обращения. Если заголовок Retry-After отсутствует, возвращается ЗначениеПоУмолчанию,
//  если параметр ЗначениеПоУмолчанию не задан, возвращается Неопределено.
// 
// Параметры:
//  HTTPОтвет - HTTPОтвет
//  ЗначениеПоУмолчанию - Число,Неопределено
// 
// Возвращаемое значение:
//  Число,Неопределено
//
Функция ИнтервалПовторногоОбращения(Знач HTTPОтвет, ЗначениеПоУмолчанию = Неопределено) Экспорт
	
	Интервал = HTTPОтвет.Заголовки.Получить(ИмяЗаголовкаПериодОжидания());
	
	Если Интервал = Неопределено Тогда
		Интервал = ЗначениеПоУмолчанию;
	Иначе
		ТипЧисло = Новый ОписаниеТипов("Число");
		Интервал = ТипЧисло.ПривестиЗначение(Интервал);
	КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ЭтоОшибкаИнтернетСоединение(Ошибка) Экспорт
	
	Возврат Ошибка.ВидОшибки.Идентификатор = ИнтернетСоединениеБЭДКлиентСервер.ИдентификаторВидаОшибкиИнтернетСоединение();
...
Процедура ДописатьЗаголовкиHTTP(Объект, ТекстОшибки)
	...
	Если ТипЗнч(Объект) = Тип("HTTPЗапрос") Тогда 
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ТекстОшибки,
			Объект.АдресРесурса,
			ПредставлениеЗаголовковHTTPСписком(Объект.Заголовки));
...
Функция ИмяЗаголовкаПеренаправления()
