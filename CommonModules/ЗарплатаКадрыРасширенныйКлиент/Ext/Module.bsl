...
Функция ПараметрыОткрытияФормыРедактированияРезультатовРаспределенияПоИсточникамФинансирования(Форма, ОписаниеТаблицы, ТекущиеДанные, Период, Организация)
	...
	ИначеЕсли ОписаниеТаблицы.ИмяТаблицы = "НачисленияПоДоговорам" Или ОписаниеТаблицы.ИмяТаблицы = "ДоговорыПерерасчет" Тогда
...
Процедура ФормаРедактированияРезультатовРаспределенияПоИсточникамФинансированияПослеЗакрытия(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ИзменилсяБюджетныйТарифНСиПЗ 	 = РезультатЗакрытия.ИзменилсяБюджетныйТарифНСиПЗ;
	...
	Если ПроверитьНовоеРаспределение Тогда
		...
		Если РаспределятьЗависимыеТаблицы Или ИзмениласьДоляЕНВД Или ТребуетсяПересчетНДФЛ Тогда
			...
			Если ОписаниеТаблицы.ИмяТаблицы = "НачисленияПоДоговорам" Или ОписаниеТаблицы.ИмяТаблицы = "ДоговорыПерерасчет" Тогда
			...
			ПараметрыДляРаспределения.Вставить("ИзменилсяБюджетныйТарифНСиПЗ", 	ИзменилсяБюджетныйТарифНСиПЗ);
...
Процедура ПослеФормирования(ФормаОтчета, ОтчетСформирован) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПротиводействиеКоррупции") Тогда
		МодульПротиводействиеКоррупцииКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПротиводействиеКоррупцииКлиент");
		МодульПротиводействиеКоррупцииКлиент.ПослеФормирования(ФормаОтчета, ОтчетСформирован);
	КонецЕсли;
...
Процедура ОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПротиводействиеКоррупции") Тогда
		МодульПротиводействиеКоррупцииКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПротиводействиеКоррупцииКлиент");
		МодульПротиводействиеКоррупцииКлиент.ОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПроцессыНаграждения") Тогда
		...
		МодульПроцессыНагражденияКлиент.ОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.КлассныеЧиныРанги") Тогда
		...
		МодульКлассныеЧиныРангиКлиент.ОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ОбработчикКоманды(ФормаОтчета, Команда, Результат) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПроцессыНаграждения") Тогда
		МодульПроцессыНагражденияКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПроцессыНагражденияКлиент");
		МодульПроцессыНагражденияКлиент.ОбработчикКоманды(ФормаОтчета, Команда, Результат);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.КлассныеЧиныРанги") Тогда
		МодульКлассныеЧиныРангиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("КлассныеЧиныРангиКлиент");
		МодульКлассныеЧиныРангиКлиент.ОбработчикКоманды(ФормаОтчета, Команда, Результат);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.УправлениеШтатнымРасписанием") Тогда
		МодульУправлениеШтатнымРасписаниемКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УправлениеШтатнымРасписаниемКлиент");
		МодульУправлениеШтатнымРасписаниемКлиент.ОбработчикКоманды(ФормаОтчета, Команда, Результат);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников") Тогда
		МодульАттестацииСотрудниковКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("АттестацииСотрудниковКлиент");
		МодульАттестацииСотрудниковКлиент.ОбработчикКоманды(ФормаОтчета, Команда, Результат);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ФормаОтчета, ИмяСобытия, Параметр, Источник, ОповещениеОбработано) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПроцессыНаграждения") Тогда
		МодульПроцессыНагражденияКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПроцессыНагражденияКлиент");
		МодульПроцессыНагражденияКлиент.ОбработкаОповещения(ФормаОтчета, ИмяСобытия, Параметр, Источник, ОповещениеОбработано);
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
		...
		МодульОбучениеРазвитиеКлиент.ОбработкаОповещения(ФормаОтчета, ИмяСобытия, Параметр, Источник, ОповещениеОбработано);
	КонецЕсли;
...
Процедура ОбработкаДополнительнойРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	ЗарплатаКадрыКлиентБазовый.ОбработкаДополнительнойРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.КлассныеЧиныРанги") Тогда
		МодульКлассныеЧиныРангиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("КлассныеЧиныРангиКлиент");
		МодульКлассныеЧиныРангиКлиент.ОбработкаДополнительнойРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.УправлениеШтатнымРасписанием") Тогда
		МодульУправлениеШтатнымРасписаниемКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УправлениеШтатнымРасписаниемКлиент");
		МодульУправлениеШтатнымРасписаниемКлиент.ОбработкаДополнительнойРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников") Тогда
		МодульАттестацииСотрудниковКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("АттестацииСотрудниковКлиент");
		МодульАттестацииСотрудниковКлиент.ОбработкаДополнительнойРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПроцессыНаграждения") Тогда
		МодульПроцессыНагражденияКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ПроцессыНагражденияКлиент");
		МодульПроцессыНагражденияКлиент.ОбработкаДополнительнойРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ИзменитьОтборПоОсновнойОрганизации(Список, ИмяРеквизита, ЗначениеОтбора) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗадачиБухгалтера") Тогда
		МодульЗадачиБухгалтераБЗККлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЗадачиБухгалтераБЗККлиент");
		МодульЗадачиБухгалтераБЗККлиент.ИзменитьОтборПоОсновнойОрганизации(Список, ИмяРеквизита, ЗначениеОтбора);
	КонецЕсли;
...
Процедура ОткрытьНастройкуНалоговОтчетов(Организация, ИсточникПерехода, ТекущаяЗакладка = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗадачиБухгалтера") Тогда
		МодульЗадачиБухгалтераБЗККлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЗадачиБухгалтераБЗККлиент");
		МодульЗадачиБухгалтераБЗККлиент.ОткрытьНастройкуНалоговОтчетов(Организация, ИсточникПерехода, ТекущаяЗакладка);
	КонецЕсли;
...
Процедура ОткрытьСписокНалогиИОтчеты(ПараметрыОткрытия = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗадачиБухгалтера") Тогда
		МодульЗадачиБухгалтераБЗККлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЗадачиБухгалтераБЗККлиент");
		МодульЗадачиБухгалтераБЗККлиент.ОткрытьСписокНалогиИОтчеты(ПараметрыОткрытия);
	КонецЕсли;
...
Процедура ОткрытьСписокЗадач() Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗадачиБухгалтера") Тогда
		МодульЗадачиБухгалтераБЗККлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЗадачиБухгалтераБЗККлиент");
		МодульЗадачиБухгалтераБЗККлиент.ОткрытьСписокЗадач();
	КонецЕсли;
...
Процедура ЗаполнитьПояснениеДляДокументаСЭДОНаБоковойПанели(Форма, ПараметрыОтчета) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗадачиБухгалтера") Тогда
		МодульЗадачиБухгалтераБЗККлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ЗадачиБухгалтераБЗККлиент");
		МодульЗадачиБухгалтераБЗККлиент.ЗаполнитьПояснениеДляДокументаСЭДОНаБоковойПанели(Форма, ПараметрыОтчета);
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
// Процедуры и функции для тематических отчетов на рабочих местах пользователей.

// Формирует входной параметр "ПараметрыРасшифровки" для функции ЗарплатаКадрыРасширенный.ДобавитьДействияДополнительнойРасшифровкиОтчета.
//
// Возвращаемое значение:
//  Структура - с ключами:
//   * ИменаПолейРасшифровки      - Массив, строк имен полей расшифровки
//   * ИменаДокументов            - Массив, строк имен объектов метаданных Документ
//   * ВариантОтчетаДляЗаполнения - Булево, признак - вариант отчета позволяет заполнить документ
//   * РегистраторРасшифровки     - ДокументСсылка, документ-регистратор, открываемый из расшифровки
//
Функция НовыеПараметрыДополнительнойРасшифровкиОтчета() Экспорт
	
	ПараметрыРасшифровки = Новый Структура;
	
	ПараметрыРасшифровки.Вставить("ИменаПолейРасшифровки");
	ПараметрыРасшифровки.Вставить("ИменаДокументов");
	ПараметрыРасшифровки.Вставить("ВариантОтчетаДляЗаполнения");
	ПараметрыРасшифровки.Вставить("РегистраторРасшифровки");
	
	Возврат ПараметрыРасшифровки;
	
КонецФункции

Процедура ОбработкаДополнительнойРасшифровкиТематическогоОтчета(ФормаОтчета, Расшифровка, ПараметрыРасшифровки, МодульЗаполненияДокумента) Экспорт
	
	ДоступныеДействия = Новый Массив;
	
	Данные = ЗарплатаКадрыРасширенныйВызовСервера.ДанныеЭлементаРасшифровки(ФормаОтчета.ОтчетДанныеРасшифровки, Расшифровка);
	КомпоновщикНастроек = ФормаОтчета.Отчет.КомпоновщикНастроек;
	
	ДополнительныеПунктыМеню = ЗарплатаКадрыРасширенныйВызовСервера.ДействияДополнительнойРасшифровкиОтчета(ФормаОтчета.ОтчетДанныеРасшифровки, Расшифровка, ПараметрыРасшифровки);
	
	ОбработкаРасшифровки = Новый ОбработкаРасшифровкиКомпоновкиДанных(
		ФормаОтчета.ОтчетДанныеРасшифровки,
		Новый ИсточникДоступныхНастроекКомпоновкиДанных(ФормаОтчета.НастройкиОтчета.АдресСхемы));
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Форма", ФормаОтчета);
	ДополнительныеПараметры.Вставить("Данные", Данные);
	ДополнительныеПараметры.Вставить("РегистраторРасшифровки", ПараметрыРасшифровки.РегистраторРасшифровки);
	ДополнительныеПараметры.Вставить("Расшифровка", Расшифровка);
	ДополнительныеПараметры.Вставить("МодульЗаполненияДокумента", МодульЗаполненияДокумента);

	ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьВыборДействияЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	ОбработкаРасшифровки.ПоказатьВыборДействия(ОписаниеОповещения, Расшифровка, ДоступныеДействия, ДополнительныеПунктыМеню, Истина, ФормаОтчета.Элементы.ОтчетТабличныйДокумент);
...
Процедура ПоказатьВыборДействияЗавершение(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	МодульЗаполненияДокумента = ДополнительныеПараметры.МодульЗаполненияДокумента;
	
	Если ВыполненноеДействие = ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение Тогда
		
		ПоказатьЗначение(, ДополнительныеПараметры.Данные);
		
	ИначеЕсли ВыполненноеДействие = "ОткрытьРегистратор" Тогда
		
		ПоказатьЗначение(, ДополнительныеПараметры.РегистраторРасшифровки);
		
	ИначеЕсли СтрНайти(Строка(ВыполненноеДействие), "ЗаполнитьДокумент") > 0 Тогда
		
		МодульЗаполненияДокумента.ЗаполнитьДокументИзОтчета(ДополнительныеПараметры.Форма, Строка(ВыполненноеДействие));
		
	КонецЕсли;
	
КонецПроцедуры

Функция ДанныеРасшифровкиИзОтчета(Знач ОтчетДанныеРасшифровки, Знач ХранилищеРезультатаОтчета, Знач ОтчетТабличныйДокумент, МассивПолейОтбора, МассивГруппировок, МассивКолонок) Экспорт
	
	МассивРасшифровок = ЗарплатаКадрыРасширенныйКлиентСервер.РасшифровкиВыделенныхЯчеек(ОтчетТабличныйДокумент, ОтчетТабличныйДокумент.ВыделенныеОбласти);
	Возврат ЗарплатаКадрыРасширенныйВызовСервера.ДанныеРасшифровкиИзОтчета(ОтчетДанныеРасшифровки, ХранилищеРезультатаОтчета, МассивРасшифровок, МассивПолейОтбора, МассивГруппировок, МассивКолонок);
