...
Функция ОписаниеРасчетногоДокумента() Экспорт
	...
	|ДоговорыПерерасчетКоманднаяПанельИмя,
...
Функция ОписаниеТаблицыРасчета() Экспорт
	...
	ОписаниеТаблицы.ПутьКРеквизитуОрганизация = "Объект.Организация";
...
#Область МетодыОбслуживанияСверхурочныхДокументов

Функция ПолучитьПредставлениеПоляПодробногоВводаВремени(Форма, Дата, ИдентификаторСтрокиСотрудника) Экспорт
	
	ЗначенияСуточныхВидовВремени = Новый Соответствие; 
	Для Каждого ВидВремениСуток Из Форма.СуточныеВидыВремени Цикл
		ЗначенияСуточныхВидовВремени.Вставить(ВидВремениСуток.Значение, 0);
	КонецЦикла;
	
	Отбор = Новый Структура("Дата, ИдентификаторСтрокиСотрудника", Дата, ИдентификаторСтрокиСотрудника);
	НайденныеСтроки = Форма.ДанныеОВремениПодробно.НайтиСтроки(Отбор);
	
	Для Каждого Стр Из НайденныеСтроки Цикл
		
		ВидВремени = ?(ЗначениеЗаполнено(Стр.СуточныйВидВремени), Стр.СуточныйВидВремени, Форма.СуточныеВидыВремени.ДневноеВремя);
		ЗначенияСуточныхВидовВремени[ВидВремени] = ЗначенияСуточныхВидовВремени[ВидВремени] + Стр.ОтработаноЧасов;
		
	КонецЦикла;
	
	МассивПодстрок = Новый Массив;
	Для Каждого КлючИЗначение Из ЗначенияСуточныхВидовВремени Цикл
		
		Если КлючИЗначение.Значение <> 0 Тогда
			МассивПодстрок.Добавить("" + КлючИЗначение.Значение + "-" + БуквенноеОбозначениеВидаВремениСуток(Форма, КлючИЗначение.Ключ));
		КонецЕсли;
		
	КонецЦикла;
		
	ПредставлениеПоля = СтрСоединить(МассивПодстрок, "; ");
	ПредставлениеПоля = ?(ПредставлениеПоля = "", "0", ПредставлениеПоля);
	
	Возврат ПредставлениеПоля;

КонецФункции

Функция БуквенноеОбозначениеВидаВремениСуток(Форма, ВидВремениСуток)
	
	Если ВидВремениСуток = Форма.СуточныеВидыВремени.ВечернееВремя Тогда
		Возврат "В";
	ИначеЕсли ВидВремениСуток = Форма.СуточныеВидыВремени.НочноеВремя Тогда
		Возврат "Н";
	Иначе
		Возврат "Д";
	КонецЕсли;
...
Процедура ЗаполнитьИдентификаторСтрокиСотрудника(Форма, СтрокаСотрудника) Экспорт
