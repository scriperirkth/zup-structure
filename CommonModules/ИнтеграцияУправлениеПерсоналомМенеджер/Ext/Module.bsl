...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверкаДоступностиАдресаПоИмениФоновоеЗадание", Истина);
	Методы.Вставить("ПолучитьАбонентаИнтернетПоддержкиФоновоеЗадание", Истина);
	Методы.Вставить("ЗарезервироватьИмяДоменаФоновоеЗадание", Истина);
	Методы.Вставить("РазвернутьПриложениеФоновоеЗадание", Истина);
	Методы.Вставить("ПроверитьСостояниеПодключенияФоновоеЗадание", Истина);
	Методы.Вставить("ИнформацияОПриложенииФоновоеЗадание", Истина);
	Методы.Вставить("ОбновлениеСведенийОбОплатеФоновоеЗадание", Истина);
	Методы.Вставить("ОбновитьАдминистратораПриложенияФоновоеЗадание", Истина);
	Методы.Вставить("ИзменитьАбонентаИППФоновоеЗадание", Истина);
	Методы.Вставить("ОтключитьПриложенияФоновоеЗадание", Истина);
	Методы.Вставить("ОбновитьСвойстваПриложенияФоновоеЗадание", Истина);
	Методы.Вставить("ПолучениеКодаАктивацииФоновоеЗадание", Истина);
	Методы.Вставить("ПроверкаДоступностиИнтернетПоддержкиФоновоеЗадание", Истина);
	Методы.Вставить("ПодключениеПоКодуАктивацииФоновоеЗадание", Истина);
...
Процедура ОбновлениеСведенийОбОплатеФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	// Получение состояния ИС.
	Если Не Отказ Тогда
		ИСДоступна = ИнтеграцияУправлениеПерсоналомОбмен.ИнформационнаяСистемаДоступна(Параметры.Приложение);
		Если ИСДоступна <> Неопределено Тогда
			Результат.ПриложениеЗаблокировано = Не ИСДоступна;
		КонецЕсли;
	КонецЕсли;
...
Функция АдресМенеджера(Приложение) Экспорт
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
		АдресМенеджера = "https://hr-app-manager.1c-hrm.ru/applications/hr-app-manager";
...
Функция ДоступноПодключениеПриложения1СПерсонал() Экспорт
	...
	АдресРесурса = АдресМенеджера(Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом);
	...
	ПолныйАдресРесурса = СтрШаблон("/%1%2", СтруктураURI.ПутьНаСервере,"/api/subscribers/registration-mode");
	...
	Попытка
		...
		Если Ответ <> Неопределено И Ответ.КодСостояния = 200 Тогда
			...
			РежимПодключения = ПрочитатьJSON(ЧтениеJSON, Истина);
			Если РежимПодключения = "self_service" Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
	Исключение
