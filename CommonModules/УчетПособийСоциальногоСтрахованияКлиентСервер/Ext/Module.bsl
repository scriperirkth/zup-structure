...
Процедура ДобавитьДанныеОЗаработке(ДанныеРасчета, ПараметрыРасчета)
	...
	НачисленияРК = НачисленияРКСН.НачисленияРК;
	НачисленияСН = НачисленияРКСН.НачисленияСН;
	НачисленияРК2025 = НачисленияРКСН.НачисленияРК2025;
	НачисленияСН2025 = НачисленияРКСН.НачисленияСН2025;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НачисленияРК2025, НачисленияРКСН.ДополнительныеНачисленияРК);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НачисленияСН2025, НачисленияРКСН.ДополнительныеНачисленияСН);
	...
	Для каждого Начисление Из ПараметрыРасчета.ДанныеНачислений Цикл
		...
		Если СтрокаПоГоду <> Неопределено Тогда
			...
			Если Начисление.Период >= Дата(2026, 1, 1) И НачисленияРК.Найти(Начисление.Начисление) <> Неопределено
				Или НачисленияРК2025.Найти(Начисление.Начисление) <> Неопределено Тогда
			...
			Если Начисление.Период >= Дата(2026, 1, 1) И НачисленияСН.Найти(Начисление.Начисление) <> Неопределено 
				Или НачисленияСН2025.Найти(Начисление.Начисление) <> Неопределено Тогда
