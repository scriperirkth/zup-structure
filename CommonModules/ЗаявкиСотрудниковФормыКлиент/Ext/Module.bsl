...
Процедура НачатьВыполнениеКомандыЗаявкаВыполнена(Форма, РезультатВыполненияЗаявки, ЭтоЗаявкаСотрудника = Истина) Экспорт
	...
	Если РезультатВыполненияЗаявки = ПредопределенноеЗначение("Перечисление.РезультатыВыполненияЗаявокКабинетСотрудника.Выполнена") Тогда
		...
		ЗаголовокВопроса = НСтр("ru = 'Подтверждение'");
		Если ЭтоЗаявкаСотрудника Тогда
			ТекстВопроса = НСтр("ru = 'Регистрация выполнение заявки'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Регистрация принятия отчета'");
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ТекстВопроса = НСтр("ru = 'Отклонение отчета сотрудника.
			|Сообщение о причине отклонения будет отправлено сотруднику в личный кабинет.'");
		КонецЕсли;
...
Функция ЗаявкаГотоваКВыполнению(Форма, РезультатВыполненияЗаявки, ОписаниеДокумента) Экспорт
	...
	Иначе
		...
		Если ПустаяСтрока(Форма.Объект.ОтветПоЗаявке) Тогда
			Если ОписаниеДокумента.ЭтоОтчетСотрудника Тогда
				ТекстСообщения = НСтр("ru = 'Необходимо указать причину, по которой отчет отклоняется.'");
			Иначе
...
Процедура ПодписатьФайл(Форма, ПрисоединенныйФайл, ЭтоФайлПечатнойФормы) Экспорт
	...
	Если ЭтоФайлПечатнойФормы Тогда	
		ДополнительныеПараметрыФормы = ЭДОБЗККлиент.ДополнительныеПараметрыФормыПодписанияДокументов(
			ОповещениеОЗавершении, Истина, Истина);
...
Процедура ОткрытьФормуБанковскогоСчета(ПараметрыФормы, Владелец, Уникальность, ОписаниеОповещения, Режим) Экспорт
	...
	ЗаявкиСотрудниковФормыКлиентВнутренний.ОткрытьФормуБанковскогоСчета(ПараметрыФормы, Владелец, Уникальность, ОписаниеОповещения, Режим);
	
КонецПроцедуры

Процедура ОткрытьФормуВыбораБанковскогоСчета(ПараметрыФормы, Владелец, ОписаниеОповещения, Режим) Экспорт
	
	ЗаявкиСотрудниковФормыКлиентВнутренний.ОткрытьФормуВыбораБанковскогоСчета(ПараметрыФормы, Владелец, ОписаниеОповещения, Режим);
