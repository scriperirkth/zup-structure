...
Процедура ОпределитьРазделыСВариантамиОтчетов(Разделы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЭкспертнаяОценкаПерсонала") Тогда
		...
		МодульЭкспертнаяОценкаПерсонала.ОпределитьРазделыСВариантамиОтчетов(Разделы);
	КонецЕсли;
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТруда.НастроитьВариантыОтчетов(Настройки);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		...
		МодульРабочиеМестаОхраныТруда.НастроитьВариантыОтчетов(Настройки);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЭкспертнаяОценкаПерсонала") Тогда
		МодульЭкспертнаяОценкаПерсонала = ОбщегоНазначения.ОбщийМодуль("ЭкспертнаяОценкаПерсонала");
		МодульЭкспертнаяОценкаПерсонала.НастроитьВариантыОтчетов(Настройки);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ТрудоустройствоПоКвотам") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ТрудоустройствоПоКвотам");
		Модуль.НастроитьВариантыОтчетов(Настройки);
	КонецЕсли;
...
Процедура ПриПолученииДоступныхВерсийФормата(ВерсииФормата) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОбменДаннымиУниверсальныйФорматБО") Тогда
		МодульОбменДаннымиУниверсальныйФорматБО = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиУниверсальныйФорматБО");
		МодульОбменДаннымиУниверсальныйФорматБО.ПриПолученииДоступныхВерсийФормата(ВерсииФормата);
	КонецЕсли;
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОбменДаннымиУниверсальныйФормат") Тогда
		...
		МодульОбменДаннымиУниверсальныйФормат.РегистрацияИзмененияДляНачальнойВыгрузки(Получатель, СтандартнаяОбработка, Отбор);
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОбменДаннымиУниверсальныйФорматБО") Тогда
		МодульОбменДаннымиУниверсальныйФорматБО = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиУниверсальныйФорматБО");
		МодульОбменДаннымиУниверсальныйФорматБО.РегистрацияИзмененияДляНачальнойВыгрузки(Получатель, СтандартнаяОбработка, Отбор);
	КонецЕсли;

КонецПроцедуры

// См. ОбменДаннымиПереопределяемый.ПриПоискеИмениПланаОбменаЧерезУниверсальныйФормат.
Процедура ПриПоискеИмениПланаОбменаЧерезУниверсальныйФормат(ИмяПланаОбмена, ИдентификаторНастройки, НайденноеИмяПланаОбмена) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОбменДаннымиУниверсальныйФорматБО") Тогда
		МодульОбменДаннымиУниверсальныйФорматБО = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиУниверсальныйФорматБО");
		МодульОбменДаннымиУниверсальныйФорматБО.ПриПоискеИмениПланаОбменаЧерезУниверсальныйФормат(
			ИмяПланаОбмена,
			ИдентификаторНастройки,
			НайденноеИмяПланаОбмена);
	КонецЕсли;
...
Процедура ПриОпределенииИдентификацииПриложенияДляРаботаРу(ИдентификаторПриложения) Экспорт
	...
	Если Конфигурация.Корпоративная Тогда
		Если Конфигурация.Хозрасчетная Тогда
			ИдентификаторПриложения.client_id		= "636";
			ИдентификаторПриложения.client_secret	= "svDrc9NXw870OF9BmR07e2EhusZ3gYPP";
			ИдентификаторПриложения.redirect_uri	= "https://oauth-integration.1c.ru/hrmcorp-rabotaru/";
		ИначеЕсли Конфигурация.Бюджетная Тогда
			ИдентификаторПриложения.client_id		= "637";
			ИдентификаторПриложения.client_secret	= "6ExCt95bg3ykGJjCsqF5xB3Dti4zMpy6";
			ИдентификаторПриложения.redirect_uri	= "https://oauth-integration.1c.ru/statehrmcorp-rabotaru/";
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииИдентификацииПриложенияДляZoom(ИдентификаторПриложения) Экспорт
	...
	Если Конфигурация.Корпоративная Тогда
		Если Конфигурация.Хозрасчетная Тогда
			...
			ИдентификаторПриложения.Authorization 	=
...
Процедура ПриДобавленииОбработчиковПереходаКорпоративныхПодсистем(Обработчики)
	...
	Фильтр = Новый Структура("НачальноеЗаполнение, НеВыполнятьПриПереходеСДругойПрограммы", Истина, Ложь);
	ОбработчикиНачальногоЗаполнения = ВсеОбработчики.НайтиСтроки(Фильтр);
...
Процедура СобратьОбработчикиКОРППодсистем(Обработчики)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.КадровыеРешения") Тогда
		...
		МодульКадровыеРешенияРасширенный.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда.ОбучениеПоОхранеТруда") Тогда
		...
		МодульОбучениеПоОхранеТруда.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.СостояниеСтруктурыПредприятия") Тогда
		МодульСостояниеСтруктурыПредприятия = ОбщегоНазначения.ОбщийМодуль("СостояниеСтруктурыПредприятия");
		МодульСостояниеСтруктурыПредприятия.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
