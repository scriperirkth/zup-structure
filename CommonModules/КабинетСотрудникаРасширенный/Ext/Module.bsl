...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗарегистрироватьПубликациюГрафиковРаботы");
	Методы.Вставить("ЗарегистрироватьОбновлениеФункциональностиСервиса");
	Методы.Вставить("ОтменитьРегистрациюГруппГрафиковРаботы");
	Методы.Вставить("ВыполнитьНачальныйПереносДанныхГрафикаОтпусков");
	Методы.Вставить("ОбновитьПубликуемыеПодразделенияПредприятия");
	Методы.Вставить("ЗарегистрироватьПубликациюВидовСправок");
	Методы.Вставить("ЗарегистрироватьПубликациюДанныхГрафиковРаботы");
КонецПроцедуры
...
Функция РезультатВыгрузкиСборГрафиковОтпусков(ПараметрыОбмена, МассивОтбора)
	...
	Для каждого Объект Из Данные Цикл
		ИнициаторСбора = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(Объект, "initiatorID");
...
Функция ДанныеШтатногоРасписания(ОтборПозиций) Экспорт
	...
	Иначе
		...
		СтруктураПредприятия = МестаПозицийВСтруктуреПредприятия(ДанныеШтатногоРасписания.ВыгрузитьКолонку(
			"ПозицияШтатногоРасписания"), ТекущаяДатаСеанса());
...
Функция МестаПозицийВСтруктуреПредприятия(Позиции, ДатаПолученияДанных = '00010101')
...
Функция ДанныеСотрудниковДляПубликации(ПараметрыОбмена, МассивОтбора) Экспорт
	...
	Если Не ВедетсяШтатноеРасписание Тогда
		...
		Если Не ПубликоватьСтруктуруЮридическихЛиц Тогда
			...
			СтруктураПредприятия = МестаПозицийВСтруктуреПредприятия(Позиции, ДатаАктуальности);
			Для каждого СтрокаКоллекции Из СтруктураПредприятия Цикл
				ПодразделенияПозиций.Вставить(СтрокаКоллекции.Позиция, СтрокаКоллекции.Подразделение);
			КонецЦикла;
		КонецЕсли;
...
Функция ЗагрузитьГрафикиОтпусков(ПараметрыОбмена, ПолученныеИзменения, РезультатОбработки, ВерсияИзменений) Экспорт
	...
	Для каждого СтрокаТЗ Из ТаблицаГрафиковОтпусков Цикл
		...
		Попытка
			...
			ОписаниеДокумента.ОтветственныйЗаСбор = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(Объект, "Ответственный");
