...
Процедура УстановитьДоступностьСпособовЗачисленияПоЗарплатномуПроекту(СпособыЗачисления, ЗарплатныйПроект) Экспорт
	...
	ИспользоватьЭлектронныйДокументооборотСБанком = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗарплатныйПроект, "ИспользоватьЭлектронныйДокументооборотСБанком");
	...
	Если Не ИспользоватьЭлектронныйДокументооборотСБанком И Не ЗарплатныйПроектДляПеречисленийНаКартыМир Тогда
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР] = Ложь;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету] = Истина;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруКарты] = Ложь;
	...
	Иначе
		...
		ИначеЕсли ФорматФайла > 3.7 Тогда
			ДляПеречисленияВЦифровыхРублях = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗарплатныйПроект, "ДляПеречисленийВЦифровыхРублях");
			Если Не ДляПеречисленияВЦифровыхРублях Тогда
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруКарты] = Истина;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруТелефона] = Истина;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСНИЛС] = Истина;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР] = Ложь;
			Иначе
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР] = Истина;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету] = Ложь;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруКарты] = Ложь;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруТелефона] = Ложь;
				СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСНИЛС] = Ложь;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("РазделитьРолиОбменаСБанкамиПоЗарплатнымПроектам");
КонецПроцедуры
...
// Возвращает БИК оператора платформы цифрового рубля.
//
// Возвращаемое значение:
//  Строка - БИК платформы цифрового рубля
//
...
Функция СпособыВыплатыФизическихЛиц(СписокФизическихЛиц, ТолькоРазрешенные, Организация, ДатаПолученияДанных, ЗарплатныйПроект) Экспорт
	...
	|	&Период КАК Период
	|ПОМЕСТИТЬ ВТСписокФизическихЛицДляПолученияСпособовВыплатСотрудников
	|ИЗ
	...
	ОписательВТ = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеФизическихЛиц(
		Запрос.МенеджерВременныхТаблиц, "ВТСписокФизическихЛицДляПолученияСпособовВыплатСотрудников");
	КадровыйУчет.СоздатьВТКадровыеДанныеФизическихЛиц(ОписательВТ, Истина, "ИНН, СтраховойНомерПФР");
	...
	Запрос.Текст = 
		...
		|	ЛицевыеСчета.НомерЛицевогоСчетаЦР КАК НомерЛицевогоСчетаЦР,
		|	ВТКадровыеДанныеФизическихЛиц.ИНН КАК ИНН,
		|	ВТКадровыеДанныеФизическихЛиц.СтраховойНомерПФР КАК СНИЛС
		|ИЗ
		|	ВТСписокФизическихЛицДляПолученияСпособовВыплатСотрудников КАК ВТСписокФизическихЛицДляПолученияСпособовВыплатСотрудников
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам КАК ЛицевыеСчета
		|		ПО ВТСписокФизическихЛицДляПолученияСпособовВыплатСотрудников.ФизическоеЛицо = ЛицевыеСчета.ФизическоеЛицо
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанныеФизическихЛиц КАК ВТКадровыеДанныеФизическихЛиц
		|		ПО ВТСписокФизическихЛицДляПолученияСпособовВыплатСотрудников.ФизическоеЛицо = ВТКадровыеДанныеФизическихЛиц.ФизическоеЛицо
		|ГДЕ
		|	ЛицевыеСчета.ДатаОткрытияЛицевогоСчета <= &Период
...
Процедура ВыгрузитьФайлыОбменаСБанкомПоВедомости(СтруктураПараметровДляФормированияФайла) Экспорт
	...
	Если (СтруктураПараметровДляФормированияФайла.Свойство("ФормироватьJSON") И СтруктураПараметровДляФормированияФайла.ФормироватьJSON = Ложь)
		Или Не ИспользоватьОбменСБанкомВФорматеJSON(ДокументСсылка) Тогда
	...
	Иначе
		СтруктураПараметровДляФормированияФайла.Вставить("ФормироватьJSON", Истина);
	КонецЕсли;
...
Функция ИмяФайлаОбменаСБанкамиПоЗарплатнымПроектам(Ссылка, ОтделениеБанка, НомерДокумента, ПризнакТипаСписка) Экспорт
	Расширение = ?(ИспользоватьОбменСБанкомВФорматеJSON(Ссылка), "json", "xml");
	ИмяФайла = СтрШаблон(
		"%1%2%3.%4",
...
Процедура ЗаполнитьТабличныйДокументПоПрямомуОбменуСБанками(ИмяФайла, ТабличныйДокумент) Экспорт
	...
	Если Расширение = "xml" Тогда
		ЗаполнитьТабличныйДокументПоПрямомуОбменуСБанкамиXML(ИмяФайла, ТабличныйДокумент);
	ИначеЕсли Расширение = "json" Тогда
		ЗаполнитьТабличныйДокументПоПрямомуОбменуСБанкамиJSON(ИмяФайла, ТабличныйДокумент);
	КонецЕсли;
...
Функция ДанныеСпискаПеречисленийПоXML(МассивОбъектов) Экспорт
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		ДанныеДокумента.НомерРеестра = ОбъектПечати.НомерРеестра;
		ДанныеДокумента.ВидЗачисления = ОбъектПечати.ВидЗачисления;
		ДанныеДокумента.ДатаДоговора = ОбъектПечати.ДатаДоговора;
		ДанныеДокумента.ВидДоходаИсполнительногоПроизводства = ОбъектПечати.КодВидаДохода;
		ДанныеДокумента.БИКБанка = ОбъектПечати.БИК;
		...
		Для Каждого Сотрудник Из ОбъектПечати.ЗачислениеЗарплаты.Сотрудник Цикл
			...
			ДанныеСтрокиДокумента.НомерСтроки = НомерСтроки;
			ДанныеСтрокиДокумента.НомерЛицевогоСчета = Сотрудник.ЛицевойСчет;
			ДанныеСтрокиДокумента.БИКБанкаСчета = Сотрудник.БИК;
			...
			ДанныеСтрокиДокумента.ВзысканнаяСумма = ?(Сотрудник.ОбщаяСуммаУдержаний = Неопределено, 0, Сотрудник.ОбщаяСуммаУдержаний);
			ДанныеСтрокиДокумента.СуммаКВыплате = Сотрудник.Сумма;
			ДанныеСтрокиДокумента.СНИЛС = Сотрудник.СНИЛС;
			ДанныеСтрокиДокумента.НомерТелефона = Сотрудник.НомерТелефона;
...
Функция ДанныеСпискаЗачисленийПоXML(МассивОбъектов) Экспорт
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		СуммаПоДокументу = 0;
		УникальнаяСсылка = Документы.ПодтверждениеЗачисленияЗарплаты.ПолучитьСсылку(Новый УникальныйИдентификатор);
		ДанныеДокумента = Документы.ПодтверждениеЗачисленияЗарплаты.ДанныеЗаполненияПодтверждения();
		ДанныеДокумента.Документ = УникальнаяСсылка;
		ДанныеДокумента.ДатаДокумента = ОбъектПечати.ДатаФормирования;
		ДанныеДокумента.НомерДоговора = ОбъектПечати.НомерДоговора;
		ДанныеДокумента.НомерРеестра = ОбъектПечати.НомерРеестра;
		...
		ДанныеДокумента.НомерРасчетногоСчетаОрганизации = ОбъектПечати.РасчетныйСчетОрганизации;
		НомерСтроки = 0;
		Для Каждого Сотрудник Из ОбъектПечати.РезультатЗачисленияЗарплаты.Сотрудник Цикл
			НомерСтроки = НомерСтроки + 1;
			СуммаПоДокументу = СуммаПоДокументу + Сотрудник.Сумма;
			
			ДанныеСтрокиДокумента = Документы.ПодтверждениеЗачисленияЗарплаты.ДанныеЗаполненияСтрокиПодтверждения();
			ДанныеСтрокиДокумента.НомерСтроки = НомерСтроки;
			ДанныеСтрокиДокумента.НомерЛицевогоСчета = Сотрудник.ЛицевойСчет;
			ДанныеСтрокиДокумента.БИКБанкаСчета = Сотрудник.БИК;
			ДанныеСтрокиДокумента.Фамилия = Сотрудник.Фамилия;
			...
			ДанныеСтрокиДокумента.Отчество = Сотрудник.Отчество;
			ДанныеСтрокиДокумента.СуммаКВыплате = Сотрудник.Сумма;
			ДанныеСтрокиДокумента.Результат = Сотрудник.Результат;
			ДанныеСтрокиДокумента.Комментарий = Сотрудник.РасшифровкаРезультата;
...
#Область ДанныеДляПечатиПоJSON

Функция ДанныеСпискаПеречисленийПоJSON(МассивОбъектов) Экспорт
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		ДанныеДокумента = Документы.ВедомостьНаВыплатуЗарплатыВБанк.ДанныеЗаполненияВедомости();
		ДанныеДокумента.Документ = УникальнаяСсылка;
		ДанныеДокумента.ДатаДокумента = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "date", Дата("00010101"));
		ДанныеДокумента.НомерРеестра = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "number");
		ДанныеДокумента.НомерДоговора = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "contractNumber", "");
		ДанныеДокумента.ПолноеНаименованиеОрганизации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "orgName", "");
		ДанныеДокумента.НомерРасчетногоСчетаОрганизации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "account");
		ДанныеДокумента.ВидЗачисления = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "admissionValue", "");
		ДанныеДокумента.ДатаДоговора = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "contractDate", Дата("00010101"));
		ДанныеДокумента.ВидДоходаИсполнительногоПроизводства = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "incomeTypeCode");
		ДанныеДокумента.БИКБанка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "bic", "");
		НомерСтроки = 0;
		Для Каждого Сотрудник Из ОбъектПечати.employeeSalaries Цикл
			НомерСтроки = НомерСтроки + 1;
			Сумма = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "amount", Новый Структура("amount",0));
			СуммаПоДокументу = СуммаПоДокументу + ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сумма, "amount", 0);
			
			ДанныеСтрокиДокумента = Документы.ВедомостьНаВыплатуЗарплатыВБанк.ДанныеЗаполненияСтрокиВедомости();
			ДанныеСтрокиДокумента.НомерСтроки = НомерСтроки;
			ДанныеСтрокиДокумента.НомерЛицевогоСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "account", "");
			ДанныеСтрокиДокумента.БИКБанкаСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "bic");
			ДанныеСтрокиДокумента.Фамилия = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "lastName", "");
			ДанныеСтрокиДокумента.Имя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "firstName", "");
			ДанныеСтрокиДокумента.Отчество = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "middleName", "");
			ДанныеСтрокиДокумента.ВзысканнаяСумма = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "withheldAmount", 0);
			ДанныеСтрокиДокумента.СуммаКВыплате = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сумма, "amount", 0);
...
Функция ДанныеСпискаЗачисленийПоJSON(МассивОбъектов) Экспорт
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Для Каждого Сотрудник Из ОбъектПечати.employeeSalaries Цикл
			НомерСтроки = НомерСтроки + 1;
			Сумма = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "amount");
			...
			ДанныеСтрокиДокумента.НомерСтроки = НомерСтроки;
			ДанныеСтрокиДокумента.НомерЛицевогоСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "account", "");
			ДанныеСтрокиДокумента.БИКБанкаСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектПечати, "bic");
			ДанныеСтрокиДокумента.Фамилия = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "lastName", "");
			ДанныеСтрокиДокумента.Имя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "firstName", "");
			ДанныеСтрокиДокумента.Отчество = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "middleName", "");
			ДанныеСтрокиДокумента.СуммаКВыплате = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сумма, "amount", 0);
			ДанныеСтрокиДокумента.Результат = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "result");
			ДанныеСтрокиДокумента.Комментарий = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "bankMessage");
...
Функция СоответствиеФорматовФайла() Экспорт
	...
	СоответствиеФорматаФайла.Вставить(Перечисления.ФорматыФайловОбменаПоЗарплатномуПроекту.Версия38, 3.8);
...
// Возвращает поддерживается ли выгрузка ИНН сотрудников в зарплатном проекте
//
// Параметры:
//  ЗарплатныйПроект - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект.
//
Функция ЗарплатныйПроектПоддерживаетВыгрузкуИНН(ЗарплатныйПроект) Экспорт
	...
	Возврат ФорматФайла > 3.7;
	
КонецФункции

// Возвращает поддерживаются ли перечисления в цифровых рублях в зарплатном проекте
//
// Параметры:
//  ФорматФайлаЗарплатногоПроекта - ПеречислениеСсылка.ФорматыФайловОбменаПоЗарплатномуПроекту.
//
Функция ФорматФайлаПоддерживаетПеречисленияВЦифровыхРублях(ФорматФайлаЗарплатногоПроекта) Экспорт
...
Процедура СоздатьВТСпособыВыплатСотрудников(МенеджерВременныхТаблиц, ТолькоРазрешенные, СписокСотрудников, Организация, ЗарплатныйПроект = Неопределено)
	...
	ОписательВТ = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеФизическихЛиц(
		Запрос.МенеджерВременныхТаблиц, "ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников");
	КадровыйУчет.СоздатьВТКадровыеДанныеФизическихЛиц(ОписательВТ, Истина, "ИНН, СтраховойНомерПФР");
	...
	|	ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.НомерЛицевогоСчетаЦР КАК НомерЛицевогоСчетаЦР,
	|	ВТКадровыеДанные.ИНН КАК ИНН,
	|	ВТКадровыеДанные.СтраховойНомерПФР КАК СНИЛС
	...
	|		ПО ВТСписокСотрудниковДляПолученияСпособовВыплатСотрудников.ФизическоеЛицо = ВТКадровыеДанные.ФизическоеЛицо
	|ГДЕ
...
Функция ДоступныеЭлементыЗарплатногоПроекта(ФорматФайла, НаименованиеКоллекции)
	...
	Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.8 Тогда
		
		МассивЭлементов.Добавить(НаименованиеКоллекции + "ИНН");
		МассивЭлементов.Добавить(НаименованиеКоллекции + "Банк");
...
Процедура ПрисоединитьФайлОбменаСБанкамиКОбъекту(ВладелецФайла, ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла)
	...
	Ошибки = Неопределено;
	Если СтруктураПараметровДляФормированияФайла.Свойство("ФормироватьJSON") 
		И СтруктураПараметровДляФормированияФайла.ФормироватьJSON Тогда
		ДанныеШапкиДокумента.ИмяФайла = СтрЗаменить(ДанныеШапкиДокумента.ИмяФайла, "xml", "json");
		ФайлОбменаСБанками = ФайлОбменаСБанкамиПоЗарплатнымПроектамJSON(ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла, Ошибки);
	Иначе
		ДанныеШапкиДокумента.ИмяФайла = СтрЗаменить(ДанныеШапкиДокумента.ИмяФайла, "json", "xml");
		ФайлОбменаСБанками = ФайлОбменаСБанкамиПоЗарплатнымПроектамXML(ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла, Ошибки);
	КонецЕсли;
	...
	ПоложениеТочки = СтрНайти(ДанныеШапкиДокумента.ИмяФайла, ".");
	ПараметрыФайла.ИмяБезРасширения = Лев(ДанныеШапкиДокумента.ИмяФайла, ПоложениеТочки - 1);
	ПараметрыФайла.РасширениеБезТочки = Прав(ДанныеШапкиДокумента.ИмяФайла, СтрДлина(ДанныеШапкиДокумента.ИмяФайла) - ПоложениеТочки);
...
// Получает адрес временного хранилища записываемого файла в формате XML.
//
// Параметры:
//		ДанныеШапкиДокумента - коллекция полей шапки документа.
//		ДанныеСтрокДокумента - коллекция строк документа.
...
//
// Возвращаемое значение:
//		АдресФайла - адрес временного хранилища записываемого файла в формате XML.
//
Функция ФайлОбменаСБанкамиПоЗарплатнымПроектамXML(ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла, Ошибки)
...
// Получает адрес временного хранилища записываемого файла в формате JSON.
//
// Параметры:
//		ДанныеШапкиДокумента - коллекция полей шапки документа.
//		ДанныеСтрокДокумента - коллекция строк документа.
...
//
// Возвращаемое значение:
//		АдресФайла - адрес временного хранилища записываемого файла в формате JSON.
//
Функция ФайлОбменаСБанкамиПоЗарплатнымПроектамJSON(ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла, Ошибки)
	
	АдресФайла = "";
	КодировкаФайла = Строка(ДанныеШапкиДокумента.КодировкаФайла);

	Корень = Новый Структура;
	ДобавитьЭлементВСтруктуру(Корень, "date", Формат(ДанныеШапкиДокумента.ДатаДокумента, "ДФ=yyyy-MM-dd"));
	ДобавитьЭлементВСтруктуру(Корень, "number", СокрЛП(ДанныеШапкиДокумента.НомерРеестра));
	ДобавитьЭлементВСтруктуру(Корень, "orgName",ДанныеШапкиДокумента.ПолноеНаименованиеОрганизации);
	ДобавитьЭлементВСтруктуру(Корень, "account", ДанныеШапкиДокумента.НомерРасчетногоСчетаОрганизации);
	ДобавитьЭлементВСтруктуру(Корень, "orgTaxNumber", ДанныеШапкиДокумента.ИННОрганизации);
	ДобавитьЭлементВСтруктуру(Корень, "externalId", Строка(ДанныеШапкиДокумента.ИдПервичногоДокумента));
	ДобавитьЭлементВСтруктуру(Корень, "contractNumber", ДанныеШапкиДокумента.НомерДоговора);
	ДобавитьЭлементВСтруктуру(Корень, "contractDate", Формат(ДанныеШапкиДокумента.ДатаДоговора, "ДФ=yyyy-MM-dd"));
	ДобавитьЭлементВСтруктуру(Корень, "admissionValue", ДанныеШапкиДокумента.ВидЗачисления);
	ДобавитьЭлементВСтруктуру(Корень, "incomeTypeCode", 
		СтруктураПараметровДляФормированияФайла.СоответствиеПреобразованияЗначений[ДанныеШапкиДокумента.ВидДоходаИсполнительногоПроизводства]);
	ДобавитьЭлементВСтруктуру(Корень, "month", Формат(ДанныеШапкиДокумента.ПериодРегистрации, "ДФ=MMMM"));
	ДобавитьЭлементВСтруктуру(Корень, "year", Формат(Год(ДанныеШапкиДокумента.ПериодРегистрации), "ЧГ=0"));
	ДобавитьЭлементВСтруктуру(Корень, "bic", ДанныеШапкиДокумента.БИКБанка);
	ДобавитьЭлементВСтруктуру(Корень, "employeesNumber", Строка(ДанныеШапкиДокумента.КоличествоЗаписей));
	
	СуммаИтого = Новый Структура;
	ДобавитьЭлементВСтруктуру(СуммаИтого, "amount", Формат(ДанныеШапкиДокумента.СуммаИтого, "ЧДЦ=2; ЧРД=.; ЧГ=0"), "0.00");
	ДобавитьЭлементВСтруктуру(СуммаИтого, "currencyCode", "643");
	ДобавитьЭлементВСтруктуру(СуммаИтого, "currencyName", "RUB");
	ДобавитьЭлементВСтруктуру(Корень, "amount", СуммаИтого);

	МассивСотрудников = Новый Массив;

	Для Каждого Сотрудник Из ДанныеСтрокДокумента Цикл
		СотрудникОбъект = Новый Структура;
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "lastName", Сотрудник.Фамилия);
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "firstName", Сотрудник.Имя);
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "middleName", Сотрудник.Отчество);
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "account", Сотрудник.НомерЛицевогоСчета);
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "withheldAmount", Сотрудник.ВзысканнаяСумма);
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "bic", Сотрудник.БИКБанкаСчета);
		
		СуммаСотрудника = Новый Соответствие;
		ДобавитьЭлементВСтруктуру(СуммаСотрудника, "amount", Формат(Сотрудник.СуммаКВыплате, "ЧДЦ=2; ЧРД=.; ЧГ=0"), "0.00");
		ДобавитьЭлементВСтруктуру(СуммаСотрудника, "currencyCode", "643");
		ДобавитьЭлементВСтруктуру(СуммаСотрудника, "currencyName", "RUB");
		
		ДобавитьЭлементВСтруктуру(СотрудникОбъект, "amount", СуммаСотрудника);
		
		МассивСотрудников.Добавить(СотрудникОбъект);
	КонецЦикла;
	
	ДобавитьЭлементВСтруктуру(Корень, "employeeSalaries", МассивСотрудников);
	...
	ТекстJSON = СтрЗаменить(ТекстJSON, """null""", "null");
	...
	// Если прямой обмен, то временные файлы будут удалены в БЭД
	Если Не СтруктураПараметровДляФормированияФайла.Свойство("СтруктураПараметровЭД") 
		Или СтруктураПараметровДляФормированияФайла.СтруктураПараметровЭД = Неопределено Тогда
		УдалитьФайлы(ИмяВременногоФайла);
	КонецЕсли;
...
//		Исходящий   - Признак для файлов, которые могут быть как входящими, так и исходящими.
//
// Возвращаемое значение:
...
//
Функция СоответствиеПолейФайла(ВидОперации, Знач ФорматФайла = Неопределено, Исходящий = Неопределено)
	...
	Если ВидОперации = "ВедомостьНаВыплатуЗарплатыВБанк" Тогда
		...
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.8 Тогда
			СоответствиеПолей.Вставить("ПериодВыплаты", "ПериодВыплаты");
		КонецЕсли;
		...
		КонецЕсли;
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.8 Тогда
			СоответствиеЗачислениеЗарплаты.Вставить("ЛицевойСчетЦР", "НомерЛицевогоСчетаЦР");
			СоответствиеЗачислениеЗарплаты.Вставить("ИНН", "ИНН");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников" Тогда
		...
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.8 Тогда
			СоответствиеОткрытиеСчетов.Вставить("ИНН", "ИНН");
			СоответствиеОткрытиеСчетов.Вставить("БИК", "БИК");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ПодтверждениеЗачисленияЗарплаты" Тогда
		...
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.8 Тогда
			СоответствиеПолей.Вставить("ЛицевойСчетЦР", "НомерЛицевогоСчетаЦР");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ПодтверждениеОткрытияЛицевыхСчетовСотрудников" Тогда
		...
		Если СоответствиеФорматаФайла.Получить(ФорматФайла) >= 3.8 Тогда
			СоответствиеПолей.Вставить("БИК", "БИК");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "СписокОткрытыхСчетов" Тогда
		Если ЗначениеЗаполнено (Исходящий) И Исходящий = Истина Тогда 
			СоответствиеПолей.Вставить("Нпп", "НомерСтроки");
			СоответствиеПолей.Вставить("ДатаФормирования", "ДатаФормирования");
			СоответствиеПолей.Вставить("Фамилия", "Фамилия");
			СоответствиеПолей.Вставить("Имя", "Имя");
		...
		Иначе
			...
			СоответствиеПолейСпискаСчетов = Новый Соответствие;
			СоответствиеПолейСпискаСчетов.Вставить("Нпп", "НомерСтроки");
			СоответствиеПолейСпискаСчетов.Вставить("ДатаФормирования", "ДатаФормирования");
			СоответствиеПолейСпискаСчетов.Вставить("Фамилия", "Фамилия");
			СоответствиеПолейСпискаСчетов.Вставить("Имя", "Имя");
			СоответствиеПолейСпискаСчетов.Вставить("Отчество", "Отчество");
			СоответствиеПолейСпискаСчетов.Вставить("ЛицевойСчет", "НомерЛицевогоСчета");
			СоответствиеПолейСпискаСчетов.Вставить("БИК", "БИКБанкаСчета");
			СоответствиеПолей.Вставить("СписокСчетов", СоответствиеПолейСпискаСчетов);
			...
			СоответствиеУдостоверениеЛичности.Вставить("КемВыдан", "ДокументКемВыдан");
			СоответствиеУдостоверениеЛичности.Вставить("КодПодразделения", "ДокументКодПодразделения");
			СоответствиеПолейСпискаСчетов.Вставить("УдостоверениеЛичности", СоответствиеУдостоверениеЛичности);
...
Функция СоответствиеТипов(ВидОперации) Экспорт
	...
	Если ВидОперации = "ПодтверждениеЗачисленияЗарплаты" Тогда
		...
		СоответствиеТипов.Вставить("ЛицевойСчетЦР", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(20)));
...
Процедура ДобавитьЭлементВСтруктуру(СтруктураДляЗаполнения, ИмяЭлемента, ЗначениеЭлемента, ЗначениеПоУмолчанию = Неопределено)
	Если ЗначениеЗаполнено(ЗначениеЭлемента) Тогда
		СтруктураДляЗаполнения.Вставить(ИмяЭлемента, ЗначениеЭлемента);
	ИначеЕсли ЗначениеПоУмолчанию <> Неопределено Тогда
		СтруктураДляЗаполнения.Вставить(ИмяЭлемента, ЗначениеПоУмолчанию);
	КонецЕсли;
КонецПроцедуры

Функция ИспользоватьОбменСБанкомВФорматеJSON(СсылкаНаДокумент)
	
	ВедомостьПрочихДоходовВБанк = "ДокументСсылка.ВедомостьПрочихДоходовВБанк";
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк")
		Или ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПодтверждениеЗачисленияЗарплаты") Тогда
		ВидЭДОбменСБанками = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников;
	ИначеЕсли ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников") 
		Или ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПодтверждениеОткрытияЛицевыхСчетовСотрудников") Тогда
		ВидЭДОбменСБанками = Перечисления.ВидыЭДОбменСБанками.СписокНаОткрытиеСчетовПоЗарплатномуПроекту;
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.Выплаты.ВзаиморасчетыПоПрочимДоходам") 
		И ТипЗнч(СсылкаНаДокумент) = Тип(ВедомостьПрочихДоходовВБанк) Тогда
			ВидЭДОбменСБанками = Перечисления.ВидыЭДОбменСБанками.СписокНаЗачислениеДенежныхСредствНаСчетаСотрудников;
	Иначе
		Возврат Ложь;
	КонецЕсли;
	НастройкаОбмена = ОбменСБанками.НастройкаОбмена(СсылкаНаДокумент.ЗарплатныйПроект.Организация,
		СсылкаНаДокумент.ЗарплатныйПроект.Банк, Истина);
	Если НастройкаОбмена = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	SberAPI = Метаданные.Перечисления.ПрограммыБанка.ЗначенияПеречисления.Найти("SberAPI");
	Если SberAPI = Неопределено Или НастройкаОбмена.ПрограммаБанка <> Перечисления.ПрограммыБанка["SberAPI"] Тогда
		Возврат Ложь;
	КонецЕсли;
	Если НастройкаОбмена.ИсходящиеДокументы.Найти(ВидЭДОбменСБанками, "ИсходящийДокумент") = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
//		ОбъектXDTO - Заполненный ОбъектXDTO, по которому будет заполнена структура для заполнения документа.
...
//
Функция СтруктураДляЗаполненияДокументаПоПодтверждениюБанкаПоXML(ВидПодтверждения, ОбъектXDTO, ХешСумма, ПервичныйДокумент, Отказ) Экспорт
...
// Параметры:
//		ВидПодтверждения - Строка - Вид подтверждения, по которому будет производиться загрузка файла.
//		ДанныеJSON - Заполненная структура, по которой будет заполнена структура для заполнения документа.
//		ПервичныйДокумент - Документ, по которому получено подтверждение из банка.
//		Отказ - Булево - Если Истина, то загрузка файла не будет выполнена.
//
// Возвращаемое значение:
...
//		Структура, по которой будет заполнен документ подтверждения.
//
Функция СтруктураДляЗаполненияДокументаПоПодтверждениюБанкаПоJSON(ВидПодтверждения, ДанныеJSON, ХешСумма, ПервичныйДокумент, Отказ) Экспорт
	
	СтруктураДанныхДляЗаполненияДокумента = Новый Структура("ПервичныйДокумент");
	СтруктураДанныхДляЗаполненияДокумента.Вставить("Дата", ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеJSON, "date",""));
	СтруктураДанныхДляЗаполненияДокумента.Вставить("МесяцОткрытия", НачалоМесяца(ТекущаяДатаСеанса()));
	СтруктураДанныхДляЗаполненияДокумента.Вставить("Организация", Справочники.Организации.ПустаяСсылка());
	СтруктураДанныхДляЗаполненияДокумента.Вставить("ЗарплатныйПроект", Справочники.ЗарплатныеПроекты.ПустаяСсылка());
	СтруктураДанныхДляЗаполненияДокумента.Вставить("Подразделение", Справочники.ПодразделенияОрганизаций.ПустаяСсылка());
	...
	Если ВидПодтверждения = "ПодтверждениеЗачисленияЗарплаты" Тогда
		Если ЗначениеЗаполнено(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеJSON, "externalId", "")) И ЗначениеЗаполнено(ПервичныйДокумент) Тогда
			ПолучаемыеРеквизиты = "Организация";
		...
		Сотрудники.Колонки.Добавить("Фамилия");
		Сотрудники.Колонки.Добавить("Имя");
		Сотрудники.Колонки.Добавить("Отчество");
		Сотрудники.Колонки.Добавить("БИКБанкаСчета");
		Сотрудники.Колонки.Добавить("НомерЛицевогоСчета");
		Сотрудники.Колонки.Добавить("Сумма");
		Сотрудники.Колонки.Добавить("РезультатЗачисленияЗарплаты");
		Сотрудники.Колонки.Добавить("КомментарийРезультата");
		Для Каждого Сотрудник Из ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеJSON, "employeeSalaries", Новый Массив) Цикл
			Строка = Сотрудники.Добавить();
			Строка.Фамилия = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "lastName", "");
			Строка.Имя = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "firstName", "");
			Строка.Отчество = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "middleName");
			Строка.БИКБанкаСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "bic");
			Строка.НомерЛицевогоСчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "account", "");
			Сумма =  ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "amount");
			Строка.Сумма = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сумма, "amount", 0);
			Строка.РезультатЗачисленияЗарплаты = СоответствиеПреобразованияЗначений[Врег(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "result"))];
			Строка.КомментарийРезультата = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Сотрудник, "bankMessage");
		КонецЦикла;
	КонецЕсли;
	СтруктураДанныхДляЗаполненияДокумента.Вставить("Сотрудники", Сотрудники);
	...
	Для Каждого СвойствоXDTO Из СвойстваXDTO Цикл
		...
		ИначеЕсли ТипЗнч(НаименованиеКолонки) = Тип("Соответствие") Тогда
			...
			Если ЗначениеОбязательноДляЗаполнения И ОбъектXDTO[СвойствоXDTO.Имя] = Неопределено Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'В файле не указано обязательное значение поля %1.'"),
					СвойствоXDTO.Имя);
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
				Продолжить;
			Иначе
		...
		Если ЗначениеОбязательноДляЗаполнения И ОбъектXDTO[СвойствоXDTO.Имя] = Неопределено Тогда
			ПоляВыбора = ПоляВыбораДляОбязательныхПолей()[СвойствоXDTO.Имя];
			Результат = Ложь;
			Если ПоляВыбора <> Неопределено Тогда
				Для Каждого Поле Из ПоляВыбора Цикл
					Если ОбъектXDTO[Поле] <> Неопределено Тогда
						Результат = Истина;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Функция ПоляВыбораДляОбязательныхПолей()
	
	СоответствиеПолейВыбора = Новый Соответствие;
	
	СоответствиеПолейВыбора.Вставить("ЛицевойСчет", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ЛицевойСчетЦР"));
	
	Возврат СоответствиеПолейВыбора
...
Функция ДанныеСотрудниковДляОткрытияЛицевыхСчетов(МенеджерВременныхТаблиц, ДатаПолученияДанных, Организация, ЗарплатныйПроект, СписокФизическихЛиц = Неопределено, ЗаявкаНаОткрытиеЛицевыхСчетов = Неопределено, ПолучитьТолькоЛицевойСчетНеОткрыт = Ложь) Экспорт
	...
	|	ЗаявкаНаОткрытиеЛицевыхСчетовСотрудниковСотрудники.ИНН КАК ИНН,
	|	ЗаявкаНаОткрытиеЛицевыхСчетовСотрудниковСотрудники.Банк.Код КАК БИК
	|ИЗ
	...
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.Организация КАК Организация,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.Должность КАК Должность,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ЭмбоссированныйТекст1 КАК ЭмбоссированныйТекст1,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ЭмбоссированныйТекст2 КАК ЭмбоссированныйТекст2,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ЭмбоссированныйТекст3 КАК ЭмбоссированныйТекст3,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.СистемаРасчетовПоБанковскимКартам КАК СистемаРасчетовПоБанковскимКартам,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.СуммаПервоначальногоПополнения КАК СуммаПервоначальногоПополнения,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ЯвляетсяЗарплатнойКартой КАК ЯвляетсяЗарплатнойКартой,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ОплатаЗарплатнойКарты КАК ОплатаЗарплатнойКарты,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ЯвляетсяСотрудникомБанка КАК ЯвляетсяСотрудникомБанка,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ИспользуетсяОвердрафт КАК ИспользуетсяОвердрафт,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.КатегорияСотрудника КАК КатегорияСотрудника,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ЯвляетсяУчастникомБонуснойПрограммы КАК ЯвляетсяУчастникомБонуснойПрограммы,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.БонуснаяПрограмма КАК БонуснаяПрограмма,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.НомерУчастникаБонуснойПрограммы КАК НомерУчастникаБонуснойПрограммы,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ТарифСледующийГод КАК ТарифСледующийГод,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ТарифТекущийГод КАК ТарифТекущийГод,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.СпособРассылкиОтчета КАК СпособРассылкиОтчета,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.АдресЭлектроннойПочты КАК АдресЭлектроннойПочты,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.СчетДебета КАК СчетДебета,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.НомерМобильногоТелефона КАК НомерМобильногоТелефона,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.НомерМобильногоТелефонаПредставление КАК НомерМобильногоТелефонаПредставление,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ОператорСвязи КАК ОператорСвязи,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ИспользованиеМобильногоБанка КАК ИспользованиеМобильногоБанка,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ТарифМобильногоБанка КАК ТарифМобильногоБанка,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.РазрешитьПередачуИнформацииВБКИ КАК РазрешитьПередачуИнформацииВБКИ,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.КонтрольнаяИнформация КАК КонтрольнаяИнформация,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.НомерМиграционнойКарты КАК НомерМиграционнойКарты,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ДатаНачалаПребыванияМиграционнойКарты КАК ДатаНачалаПребыванияМиграционнойКарты,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ДатаОкончанияПребыванияМиграционнойКарты КАК ДатаОкончанияПребыванияМиграционнойКарты,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ВидМиграционногоДокумента КАК ВидМиграционногоДокумента,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.НомерМиграционногоДокумента КАК НомерМиграционногоДокумента,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ДатаНачалаПребыванияМиграционногоДокумента КАК ДатаНачалаПребыванияМиграционногоДокумента,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ДатаОкончанияПребыванияМиграционногоДокумента КАК ДатаОкончанияПребыванияМиграционногоДокумента,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ПрогнозируемыйМесячныйДоход КАК ПрогнозируемыйМесячныйДоход,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ДеньВыплатыЗарплаты КАК ДеньВыплатыЗарплаты,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.КодВидаВклада КАК КодВидаВклада,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.КодПодвидаВклада КАК КодПодвидаВклада,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ВалютаВклада КАК ВалютаВклада,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.АдресДляИнформирования КАК АдресДляИнформирования,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.АдресДляИнформированияПредставление КАК АдресДляИнформированияПредставление,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.АдресМестаРаботы КАК АдресМестаРаботы,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.АдресМестаРаботыПредставление КАК АдресМестаРаботыПредставление,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ИдентификаторДизайна КАК ИдентификаторДизайна,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ПВК КАК ПВК,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ПриложениеКартаКод КАК ПриложениеКартаКод,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ПриложениеКартаПараметры КАК ПриложениеКартаПараметры,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.СтраховойНомерПФР КАК СтраховойНомерПФР,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.ИНН КАК ИНН,
	|	ОтложенноеОткрытиеЛицевыхСчетовСотрудников.Банк.Код КАК БИК
	|ИЗ
	...
	КадровыеДанные = "Фамилия, Имя, Отчество, АдресДляИнформирования, АдресДляИнформированияПредставление,"
		...
		+ "ДокументКодПодразделения, МестоРождения, ДатаРождения, ТабельныйНомер, Должность, ДатаПриема, Подразделение, СтраховойНомерПФР, ИНН";
...
Процедура ЗаполнитьТабличныйДокументПоПрямомуОбменуСБанкамиXML(ИмяФайла, ТабличныйДокумент)
	...
	ИначеЕсли ОбъектXDTO.Получить("РезультатЗачисленияЗарплаты") <> Неопределено Тогда
		...
		ТабличныйДокумент = Документы.ПодтверждениеЗачисленияЗарплаты.ПечатьСпискаЗачисленийПоXML(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОбъектXDTO), Новый СписокЗначений);
...
Процедура ЗаполнитьТабличныйДокументПоПрямомуОбменуСБанкамиJSON(ИмяФайла, ТабличныйДокумент)
	
	ТабличныйДокумент = Неопределено;
	
	Данные = ДанныеJSON(ИмяФайла);
	...
	Если Данные.Свойство("employeeSalaries") 
		И Не Данные.Свойство("bankStatus") Тогда
		// Печать списка перечислений
		ТабличныйДокумент = Документы.ВедомостьНаВыплатуЗарплатыВБанк.ПечатьСпискаПеречисленийПоJSON(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Данные), Новый СписокЗначений);
	ИначеЕсли Данные.Свойство("employeeSalaries") 
		И Данные.Свойство("bankStatus") Тогда
		// Печать подтверждения зачисления зарплаты
		ТабличныйДокумент = Документы.ПодтверждениеЗачисленияЗарплаты.ПечатьСпискаЗачисленийПоJSON(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Данные), Новый СписокЗначений);
	КонецЕсли;
	
КонецПроцедуры

Функция ДанныеJSON(ИмяФайла) Экспорт
	Чтение = Новый ЧтениеJSON;
	Попытка
		Чтение.ОткрытьФайл(ИмяФайла, "UTF-8");
		ИменаСвойствСервисаСоЗначениямиДата = ИменаСвойствСервисаСоЗначениямиДата();
		Данные = ПрочитатьJSON(Чтение, Ложь, ИменаСвойствСервисаСоЗначениямиДата, ФорматДатыJSON.ISO);
	Исключение
		ТекстСообщенияОбОшибке = НСтр("ru = 'Неверный формат файла.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщенияОбОшибке);
...
Функция ИменаСвойствСервисаСоЗначениямиДата()
	...
	Результат.Добавить("contractDate");
	Возврат Результат;
