...
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключение(Организация = Неопределено, ПоказыватьКнопки = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ОткрытьФорму(ПараметрыФункциональностиБРО.ИмяФормыПредложенияОформитьЗаявлениеНаПодключение,
				ПарметрыОткрытия, , , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
...
// Формирует структуру параметров для открытия формы предложения о подключении.
//
// Возвращаемое значение:
//  Структура:
//   * Организация - СправочникСсылка.Организации - Организация, для которой предлагается подключение. 
...
//
Функция НовыеПараметрыФормыОформитьЗаявлениеНаПодключениеПриЗаполнении() Экспорт
...
// Открывает форму, предлагающую пользователю оформить заявление на подключение к ЭДО.
// Обычно вызывается при заполнении регламентированного отчета, если проверка показала, что сервис не подключен.
//
// Параметры:
//  ПараметрыФормы - Структура - Структура, подготовленная функцией 
//                   НовыеПараметрыФормыОформитьЗаявлениеНаПодключениеПриЗаполнении.
//  Отказ - Булево - признак отказа заполнения отчета.
//
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(ПараметрыПоказа) Экспорт
	
	ОкноПриложения = АктивноеОкно(); // ОкноКлиентскогоПриложения
	...
	Если ПараметрыПриложения["БРО.БольшеНеПоказыватьПредложениеОформитьЗаявлениеНаПодключение"] = Истина Тогда
		ОповеститьОЗакрытииФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(Форма);
		Возврат;
	КонецЕсли;
	
	ПараметрыПроверки = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.НовыеПараметрыПоказатьПредложениеОформитьЗаявлениеНаПодключениеКнопкаЗаполнить();
	ПараметрыПроверки.ИмяФормы = ПараметрыПоказа.ИмяФормы;
	ПараметрыПроверки.Организация = ПараметрыПоказа.Организация;
	ПоказатьПредложение = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ПоказатьПредложениеОформитьЗаявлениеНаПодключениеКнопкаЗаполнить(
							ПараметрыПроверки);
	
	Если ПоказатьПредложение
		И ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения")
		И Форма.ИмяФормы = ПараметрыПоказа.ИмяФормы Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Организация", ПараметрыПоказа.Организация);
		ПараметрыФормы.Вставить("ВызовПоКнопкеЗаполнить", Истина);
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Форма", Форма);
		ДополнительныеПараметры.Вставить("ПараметрыПоказа", ПараметрыПоказа);
		
		ОповещениеОЗавершении = Новый ОписаниеОповещения(
			"ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключениеПриЗаполненииЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		
		ПараметрыФункциональностиБРО = РегламентированнаяОтчетностьКлиентСервер.ПараметрыФункциональности();
		ОткрытьФорму(ПараметрыФункциональностиБРО.ИмяФормыПредложенияОформитьЗаявлениеНаПодключениеПоКнопкеЗаполнить,
			ПараметрыФормы,
			Форма,
...
&НаКлиенте
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключениеПриЗаполненииЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
// Возвращает имя события закрытия формы предложения оформить заявление на подключение при заполнении.
//
// Возвращаемое значение:
//  Строка
Функция ИмяСобытияЗакрытияФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении() Экспорт
	
	Возврат "ЗакрытиеФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении";
	
КонецФункции

// Выполняет оповещение, активизирует форму и запускает автозаполнение.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Содержит форму, которую необходимо оповестить.
Процедура ОповеститьОЗакрытииФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении(Форма)
	ИмяСобытия = ИмяСобытияЗакрытияФормыПредложениеОформитьЗаявлениеНаПодключениеПриЗаполнении();
	Оповестить(ИмяСобытия, Форма);
КонецПроцедуры
...
Процедура ОжидатьОкончанияФоновогоЗаданияРезультат(РезультатВызова, ПараметрыОперации) Экспорт
	
	Оповестить(ПараметрыОперации.ИмяОповещения, РезультатВызова);
...
Процедура ОткрытьРазделОтчетности(ПараметрыФормы) Экспорт
	
	ИщемФорму = "ОбщаяФорма.РегламентированнаяОтчетность";
	РежимОткрытияОкна = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыФормы, "РежимОткрытияОкна", Неопределено);
	
	Если НЕ ЗначениеЗаполнено(РежимОткрытияОкна) Тогда
		Оповестить("СменаРазделаОблачнойОтчетности", ПараметрыФормы);
	КонецЕсли;
	
	Если ПараметрыФормы.Форма = Неопределено Тогда
		Если ЗначениеЗаполнено(РежимОткрытияОкна) Тогда
			ПараметрыОткрываемойФормы = Новый Структура("ПереданныйРежимОткрытияОкна", РежимОткрытияОкна);
			Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыФормы, "Раздел", Неопределено) = "new-button" Тогда
				ПараметрыОткрываемойФормы.Вставить("Раздел",
					ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Новое"));
				Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.Облачная1СОИспользоватьФормуНовыхСобытий() Тогда
					ДополнительныеПараметры = Новый Структура;
					ДополнительныеПараметры.Вставить("РежимОткрытияОкна", РежимОткрытияОкна);
					ДополнительныеПараметры.Вставить("ПараметрыОткрываемойФормы", ПараметрыОткрываемойФормы);
					ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьРазделОтчетностиПослеПолученияКонтекстаЭДО",
			...
			ОткрытьФорму(
				...
				?(РежимОткрытияОкна = РежимОткрытияОкнаФормы.Независимый, "ОтдельнаяЗакладка", Истина),,,,
				РежимОткрытияОкна);
			
		Иначе
			ОткрытьФорму(ИщемФорму);
...
Процедура ОткрытьРазделОтчетностиПослеПолученияКонтекстаЭДО(Результат, ДополнительныеПараметры) Экспорт
	
	КонтекстЭДОКлиент = Результат.КонтекстЭДО;
	Если КонтекстЭДОКлиент <> Неопределено Тогда
		ОткрытьФорму(
			КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ФормаНовыхСобытий",
			ДополнительныеПараметры.ПараметрыОткрываемойФормы,,
			?(ДополнительныеПараметры.РежимОткрытияОкна = РежимОткрытияОкнаФормы.Независимый, "ОтдельнаяЗакладка", Истина),,,,
			ДополнительныеПараметры.РежимОткрытияОкна);
	КонецЕсли;
	
КонецПроцедуры

Процедура СвернутьНезначимоеНовое(Форма, ВсеГруппы = Ложь, РазвернутьГруппы = Ложь) Экспорт
	
	Если Форма.НовыеСобытияПоСтраницамЖурнала Тогда
		ТребуетсяРазвернуть = Новый Массив;
		ЕстьРазвернутые = Неопределено;
		
		СтрокиДерева = Форма.Новое.ПолучитьЭлементы();
		Если ВсеГруппы = "ПропуститьВсеГруппы" Тогда
			Для каждого СтрокаДерева Из СтрокиДерева Цикл
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					ИдентификаторСтрокиДерева = СтрокаДерева.ПолучитьИдентификатор();
					Если Форма.Элементы.Новое.Развернут(ИдентификаторСтрокиДерева) Тогда
		...
		Иначе
			Для каждого СтрокаДерева Из СтрокиДерева Цикл
				Если СтрокаДерева.ЭтоЗаголовокБлока
					И (ВсеГруппы = Истина ИЛИ СтрокаДерева.Группа = "Прочтенные"
					ИЛИ СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.ВидыДокументовВЖурнале.Рассылка")
					ИЛИ СтрокаДерева.Группа =
						ПредопределенноеЗначение("Перечисление.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ОбработанныеЗапросы")
					ИЛИ СтрокаДерева.Группа =
						ПредопределенноеЗначение("Перечисление.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ЗавершенныеОтправки")) Тогда
					
					ИдентификаторСтрокиДерева = СтрокаДерева.ПолучитьИдентификатор();
					Если Форма.Элементы.Новое.Развернут(ИдентификаторСтрокиДерева) Тогда
						Если НЕ РазвернутьГруппы Тогда
							Форма.Элементы.Новое.Свернуть(ИдентификаторСтрокиДерева);
						КонецЕсли;
					Иначе
						Если РазвернутьГруппы Тогда
							Форма.Элементы.Новое.Развернуть(ИдентификаторСтрокиДерева);
						КонецЕсли;
					КонецЕсли;
					
				ИначеЕсли ЗначениеЗаполнено(СтрокаДерева.Группа) Тогда
					ИдентификаторСтрокиДерева = СтрокаДерева.ПолучитьИдентификатор();
					Если НЕ Форма.Элементы.Новое.Развернут(ИдентификаторСтрокиДерева) Тогда
						ТребуетсяРазвернуть.Добавить(ИдентификаторСтрокиДерева);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			
			Для каждого ИдентификаторСтрокиДерева Из ТребуетсяРазвернуть Цикл
				Если НЕ Форма.Элементы.Новое.Развернут(ИдентификаторСтрокиДерева) Тогда
					Форма.Элементы.Новое.Развернуть(ИдентификаторСтрокиДерева);
				КонецЕсли;
			КонецЦикла;
		...
		Если ВсеГруппы = "ПропуститьВсеГруппы" И ЕстьРазвернутые = Истина
			ИЛИ ВсеГруппы = Ложь ИЛИ РазвернутьГруппы Тогда
			// обход проблемы платформы расширения дерева
			Если Форма.Элементы.Новое.Ширина = 0 Тогда
				Форма.Элементы.Новое.Ширина = 85;
			Иначе
				Форма.Элементы.Новое.Ширина = 0;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
