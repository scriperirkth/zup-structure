...
//  НастройкиПоиска - см. НастройкиПоискаГотовогоОтвета
//
...
//  Строка - код ошибки.
//
Функция КодОшибкиОтсутствуетОбязательныйПараметрЗапроса()
...
//  Строка - код ошибки.
//
Функция КодОшибкиПустойПараметрЗапроса()
...
Функция РезультатПроверкиПараметровОтправки(
	...
	Проверка.Вставить("ИспользоватьСтандартныйШаблон", Неопределено);
	Проверка.Вставить("НастройкиПоискаГотовогоОтвета", Неопределено);
	Проверка.Вставить("ДанныеТехническойИнформации",   Неопределено);
	ЗаполнитьЗначенияСвойств(Проверка, ДанныеСообщения);
	
	Если ТипЗнч(Проверка.ИспользоватьСтандартныйШаблон) <> Тип("Булево") Тогда
		Результат.КодОшибки         = КодОшибкиОтсутствуетОбязательныйПараметрЗапроса();
		Результат.СообщениеОбОшибке = НСтр("ru = 'Параметр ""Использование стандартного шаблона"" заполнен неправильно.'");
		Возврат Результат;
	КонецЕсли;
	
	Если ТипЗнч(Проверка.НастройкиПоискаГотовогоОтвета) <> Тип("Структура") Тогда
		Результат.КодОшибки         = КодОшибкиОтсутствуетОбязательныйПараметрЗапроса();
		Результат.СообщениеОбОшибке = НСтр("ru = 'Параметр ""Настройки поиска готового ответа"" не заполнен или заполнен неправильно.'");
		Возврат Результат;
	КонецЕсли;
	
	Если ТипЗнч(Проверка.ДанныеТехническойИнформации) <> Тип("Структура") Тогда
		Результат.КодОшибки         = КодОшибкиОтсутствуетОбязательныйПараметрЗапроса();
		Результат.СообщениеОбОшибке = НСтр("ru = 'Параметр ""Данные технической информации"" не заполнен или заполнен неправильно.'");
		Возврат Результат;
	КонецЕсли;
...
//    * СистемнаяИнформацияКлиента - См. ОписаниеСистемнойИнформации;
//    * СистемнаяИнформацияСервера - См. ОписаниеСистемнойИнформации.
//
