...
Функция ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018(
		ТипКриптопровайдера = Неопределено,
		ГОСТ34122018ViPNetCSPДоступен = Неопределено,
		ИспользоватьФорматированнуюСтроку = Ложь) Экспорт
	
	ТребуетсяГОСТ34122018 =
		НСтр("ru = 'С 01.04.2026 для обмена с ФНС необходимо использовать криптопровайдер, поддерживающий алгоритмы шифрования ГОСТ 34.12-2018 и ГОСТ 34.13-2018.'");
	ТекстОбновите = НСтр("ru = 'Обновите'");
	
	Если ТипКриптопровайдера = ПредопределенноеЗначение("Перечисление.ТипыКриптоПровайдеров.VipNet") Тогда
		Если ГОСТ34122018ViPNetCSPДоступен = Неопределено Тогда
		...
		Если ГОСТ34122018ViPNetCSPДоступен Тогда
			ТекстViPNetCSP5 = НСтр("ru = 'ViPNet CSP до версии 5.'");
			
			Если ИспользоватьФорматированнуюСтроку Тогда
				Возврат Новый ФорматированнаяСтрока(
					ТребуетсяГОСТ34122018 + "
					|",
					Новый ФорматированнаяСтрока(ТекстОбновите,,,, "ПредупредитьОГОСТ34122018ДляФНС"),
					" " + ТекстViPNetCSP5);
			Иначе
				Возврат ТребуетсяГОСТ34122018 + " " + ТекстОбновите + " " + ТекстViPNetCSP5;
			КонецЕсли;
			
		Иначе
			ТекстУстановленViPNetCSP = НСтр("ru = 'У вас установлен криптопровайдер ViPNet CSP.'");
			ТекстУстановите = НСтр("ru = 'Установите'");
			ТекстСертифицированныйКриптоПроCSP =
				НСтр("ru = 'сертифицированный криптопровайдер с поддержкой новых алгоритмов шифрования КриптоПро CSP версии 5.0.12000 (R2) или выше.'");
			
			Если ИспользоватьФорматированнуюСтроку Тогда
				Возврат Новый ФорматированнаяСтрока(
					ТребуетсяГОСТ34122018 + "
					|",
					ТекстУстановленViPNetCSP + "
					|",
					Новый ФорматированнаяСтрока(ТекстУстановите,,,, "ПредупредитьОГОСТ34122018ДляФНС"),
					" " + ТекстСертифицированныйКриптоПроCSP);
			Иначе
				Возврат ТребуетсяГОСТ34122018 + " " + ТекстУстановленViPNetCSP + " " + ТекстУстановите + " " + ТекстСертифицированныйКриптоПроCSP;
			КонецЕсли;
		КонецЕсли;
		
	Иначе
		ТекстКриптоПроCSPR2 = НСтр("ru = 'КриптоПро CSP до версии 5.0.12000 (R2) или выше.'");
		...
		Если ИспользоватьФорматированнуюСтроку Тогда
			Возврат Новый ФорматированнаяСтрока(
				...
				" " + ТекстКриптоПроCSPR2);
		Иначе
			Возврат ТребуетсяГОСТ34122018 + " " + ТекстОбновите + " " + ТекстКриптоПроCSPR2;
		КонецЕсли;
	КонецЕсли;
