...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СформироватьСогласияНаПрисоединениеКЭДОБЗК", Истина);
	Методы.Вставить("ОбновитьРасписаниеЗаданияОбновленияСостоянийДокументовЭДОБЗК");
	Методы.Вставить("УстановитьЗначениеВедетсяУчетСогласийНаПрисоединениеКЭДОБЗКОбновление");
	Методы.Вставить("ЗарегистрироватьМашиночитаемыеДоверенностиДляОбмена");
	Методы.Вставить("ПроверитьЧтоВсеСотрудникиОзнакомлены");
	Методы.Вставить("ОбновитьРасписаниеЗаданияОбработкиДокументовПечатныхФорм");
	Методы.Вставить("ЗаполнитьТребуетПодписьОрганизацииПечатныхФорм");
	Методы.Вставить("ПроверитьЧтоВсеПодписиПрошлиПроверку");
	Методы.Вставить("ПроверитьЧтоВсеПодписиПроходилиПроверку");
	Методы.Вставить("ОбновитьВидПодписиСогласияНаПрисоединениеКЭДОБЗК");
	Методы.Вставить("УстановитьИспользованиеЭДОБЗК");
	Методы.Вставить("ПроверитьЗавершениеИнтерактивныхЗапланированныхЗаданий");
	Методы.Вставить("ОбновитьСостоянияДокументовЭДОБЗК", Истина);
КонецПроцедуры
...
Процедура ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ПараметрКоманды = ПараметрКомандыФормыПечатьДокументов(Форма);
	Если Не ЗначениеЗаполнено(ПараметрКоманды) Или ЭтоОбъектСПечатнымиФормами(ПараметрКоманды[0]) Тогда
		Если ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи() Тогда
			...
			Если Не Форма.Параметры.ПараметрыПечати.ДополнительныеПараметры.Свойство("СторонняяПечатнаяФорма")
				И Не ЭтоОбъектСПечатнымиФормами(ПараметрКоманды[0]) Тогда
				Возврат;
			КонецЕсли;
			
			ТребуетсяПечатнаяФормаБезПодписи = Не ЗаявкиСотрудников.ТребуетсяПечатнаяФормаСПодписью(ПараметрКоманды);
			...
			ИначеЕсли ТолькоПодписание Тогда
				...
				Если Не ЗаявкиСотрудников.ВладелецЗаявкаСотрудника(ПараметрКоманды) Тогда
...
Функция ПечатнаяФормаСоШтампамиПоОригиналу(ОригиналПечатнойФормы, УстановленныеПодписи, Направления = Неопределено, Комментарии = Неопределено, РазбиватьНаСтраницы = Истина) Экспорт
	...
	Для Каждого ДанныеСертификата Из УстановленныеПодписи Цикл
		Если ДанныеСертификата.Свойство("ИдентификаторВладельцаПодписи") Тогда
			...
			ЗначенияПараметров.Вставить("ИмяВладельцаПодписи", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСертификата.ФизическоеЛицо, "ФИО"));
			ЗначенияПараметров.Вставить("ИдВладельцаПодписи", ДанныеСертификата.ИдентификаторВладельцаПодписи);
			ЗначенияПараметров.Вставить("ДатаВремяПодписи", Формат(ДанныеСертификата.ДатаПодписи, "ДФ='дд.ММ.гггг ЧЧ:мм:сс'") + " " + UTCСеанса());
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ЭДОБЗК.ЗаполнитьТребуетПодписьОрганизацииПечатныхФорм";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("6154d6ba-c080-489c-94d7-247e6d8db338");
	Обработчик.Комментарий = НСтр("ru = 'Инициализация новых шаблонов документов.'");
	Обработчик.НачальноеЗаполнение = Истина;
...
Функция РазместитьКомандуНаФормеПечатьДокументов(УправляемаяФорма, КомандаФормы) Экспорт
	...
	Если ГруппаСохранитьОтправить <> Неопределено Тогда
		...
		КнопкаФормы.Видимость =	ЗаявкиСотрудников.ТребуетсяОтправлятьФайл(ПараметрКомандыФормыПечатьДокументов(УправляемаяФорма));
...
Функция ПараметрКомандыФормыПечатьДокументов(УправляемаяФорма)
	ПараметрКоманды = УправляемаяФорма.Параметры.ПараметрКоманды;
	Если ТипЗнч(ПараметрКоманды) <> Тип("Массив")
		И ТипЗнч(ПараметрКоманды) <> Тип("ФиксированныйМассив") Тогда
		
		ПараметрКоманды = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрКоманды);
	КонецЕсли;
...
Процедура ПроверитьПометкуУдаленияВариантаШаблонаДокументовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ОбменДанными.Загрузка
		...
		Если Источник.Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Отчеты.ШаблоныДокументовКабинетСотрудника)
			И СтрНачинаетсяС(Источник.КлючВарианта, "ШаблоныДокументов") Тогда
		
			Источник.ПометкаУдаления = Ложь;
		ИначеЕсли Источник.Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Отчеты.ОтчетыПоСотрудникам)
			И СтрНачинаетсяС(Источник.КлючВарианта, "ВидыДокументов") Тогда
...
Функция ДанныеРуководителейПодразделений(Подразделения, ДатаСведений) Экспорт
...
Процедура ЗаполнитьТребуетПодписьОрганизацииПечатныхФорм(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ НастройкиПечатныхФормБЗК.ВидДокумента.ТребуетсяПодписьОрганизации
		|	И НастройкиПечатныхФормБЗК.ВидДокумента <> ЗНАЧЕНИЕ(Справочник.ВидыДокументовЭДОБЗК.ПустаяСсылка)
		|	И НЕ НастройкиПечатныхФормБЗК.ВидДокумента В (&МассивОбновленных)";
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивОбновленных.Добавить(Выборка.ВидДокумента);
		
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
			ПараметрыОбновления, "Справочник.ВидыДокументовЭДОБЗК", "Ссылка", Выборка.ВидДокумента) Тогда
			
			Продолжить;
		КонецЕсли;
		
		ВидДокументаОбъект = Выборка.ВидДокумента.ПолучитьОбъект();
		ВидДокументаОбъект.ТребуетсяПодписьОрганизации = Истина;
