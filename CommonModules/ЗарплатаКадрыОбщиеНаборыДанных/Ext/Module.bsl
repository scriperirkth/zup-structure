...
Процедура ВывестиДополнительныеПоляПредставленийВОтчет(Отчет, ДополнительныеПоляПредставлений) Экспорт
	...
	Если ДополнительныеПоляПредставлений <> Неопределено
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Отчет, "КомпоновщикНастроек") Тогда
...
Функция СвойстваПереопределяемойТаблицы(ТекстЗапроса) Экспорт
	...
	СвойстваТаблицы = Новый Структура(
		...
		|ИмяПараметра,
		|КоллекцияПолей,
		|ИменаВТИсточниковПараметров,
		|ПараметрыЗапроса,
		|ТолькоРазрешенные");
	
	ПозицияСловаПоместить = СтрНайти(ВРег(ТекстЗапроса), ВРег("ПОМЕСТИТЬ "));
	СвойстваТаблицы.ИмяИсточникаДанных = ЗарплатаКадрыОбщиеНаборыДанных.ПервоеСловоНачинаяСПозицииВТексте(ТекстЗапроса, ПозицияСловаПоместить + СтрДлина("ПОМЕСТИТЬ "));
	
	МассивПодстрокИмениИсточникаДанных = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СвойстваТаблицы.ИмяИсточникаДанных, "_");
	СвойстваТаблицы.ИмяПредставления = МассивПодстрокИмениИсточникаДанных[1];
	Если МассивПодстрокИмениИсточникаДанных.Количество() > 2 Тогда
		СвойстваТаблицы.ИмяПараметра = МассивПодстрокИмениИсточникаДанных[2];
	КонецЕсли;
	
	СвойстваТаблицы.КоллекцияПолей = МассивПолейЗапроса(Лев(ТекстЗапроса, ПозицияСловаПоместить - 1));
	СвойстваТаблицы.ИменаВТИсточниковПараметров = ЗарплатаКадрыОбщиеНаборыДанных.МассивИменВТИсточниковПараметров(ТекстЗапроса, СвойстваТаблицы.ИмяИсточникаДанных);
	СвойстваТаблицы.ПараметрыЗапроса = ПараметрыЗапроса(ТекстЗапроса, СвойстваТаблицы.ИмяИсточникаДанных);
	ПредставлениеТолькоРазрешенные = ПараметрЗапроса("ТолькоРазрешенные", СвойстваТаблицы.ПараметрыЗапроса);
	СвойстваТаблицы.ТолькоРазрешенные = ВРег(ПредставлениеТолькоРазрешенные) = ВРег("Истина");
	
	Возврат СвойстваТаблицы;
