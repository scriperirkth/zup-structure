...
Процедура УстановитьОтборыВЗапросеПоПараметрам(Запрос, Параметры, Знач ПоляПоиска, ПоискСНачалаСтроки = Истина) Экспорт
	...
	Если НЕ ПустаяСтрока(Параметры.СтрокаПоиска) Тогда
		...
		КонецЦикла;	
		Запрос.УстановитьПараметр("СтрокаПоиска", ?(ПоискСНачалаСтроки, "", "%") + Параметры.СтрокаПоиска + "%");
	КонецЕсли;
