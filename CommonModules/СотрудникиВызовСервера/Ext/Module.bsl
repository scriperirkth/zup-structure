...
Функция СформироватьСведенияОБлокировке(ФизическоеЛицоСсылка, ФизическоеЛицоВерсияДанных, ФормаУникальныйИдентификатор) Экспорт
	Попытка
		ЗаблокироватьДанныеДляРедактирования(ФизическоеЛицоСсылка, ФизическоеЛицоВерсияДанных, ФормаУникальныйИдентификатор);
	Исключение
		ТесктОшибки = ОписаниеОшибки();		
		Если СтрДлина(ТесктОшибки) > 0 Тогда 
			ИмяПользователяНачалоСтроки = СтрНайти(ТесктОшибки,"пользователь:") + 14;
			ИмяПользователяКонецСтроки = СтрНайти(ТесктОшибки,"сеанс:") - 2;
			СведенияОБлокировке = "(" + Сред(ТесктОшибки, ИмяПользователяНачалоСтроки, ИмяПользователяКонецСтроки-ИмяПользователяНачалоСтроки) + ").";
			Возврат СведенияОБлокировке;
		Иначе
			Возврат ".";
		КонецЕсли;
	КонецПопытки;
