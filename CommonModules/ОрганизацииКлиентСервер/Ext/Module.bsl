...
Процедура ОбработатьОтображениеИдентификационногоНомера(Элемент, ОКПО, ЮридическоеФизическоеЛицо, Форма) Экспорт
	...
	Если НЕ ПустаяСтрока(ОКПО) Тогда
		...
		СообщенияПроверки = ?(ПустаяСтрока(СообщенияПроверки), НСтр("ru = 'Идентификационный номер указан правильно'"), СообщенияПроверки);
	...
	Иначе
		...
		СообщенияПроверки = НСтр("ru = 'Не указан Идентификационный номер'");
		ТекстНадписи  = "";
		ЭлементЦветТекста = Форма.ЦветСтиляЦветТекстаПоля;
		Картинка = Новый Картинка;
