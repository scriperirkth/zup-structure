...
Функция ИсключитьНедопустимыеФайлы(ПомещенныеФайлы) Экспорт
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		Если НЕ ЗначениеЗаполнено(ИмяФайла) Тогда
			ИмяФайла = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПомещенныйФайл, "ИмяФайла", "");
		КонецЕсли;
		ПроверяемыеФайлы.Добавить(ИмяФайла);
	...
	КоличествоФайлов = ПроверяемыеФайлы.Количество();
	
	Для Счетчик = 1 По КоличествоФайлов Цикл
		
		Индекс = КоличествоФайлов - Счетчик;
		Если ФайлыНедопустимыхТипов.Найти(ПроверяемыеФайлы[Индекс]) <> Неопределено Тогда
