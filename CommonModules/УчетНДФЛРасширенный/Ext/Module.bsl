...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УточнитьКодДоходаПоКомпенсацииНеИспользованныхДнейЧасовОтдыха");
	Методы.Вставить("ИсправитьФлажокМежрасчетностиВСведенияхОДоходахСторнирования");
	Методы.Вставить("ЗарегистрироватьПереносыДанныхКакУчтенныеПриРасчетеНДФЛ");
	Методы.Вставить("ВосстановитьКод2400");
	Методы.Вставить("ЗарегистрироватьНачисленияЗарплатыКакУчтенныеПриРасчетеНДФЛ");
	Методы.Вставить("ВычеркнутьЛишниеНачисленияЗарплатыИзУчтенныхПриРасчетеНДФЛ");
КонецПроцедуры
...
Процедура СформироватьДоходыНДФЛПоНачислениям(Движения, Отказ, Организация, ДатаОперации, ДатаВыплаты, МенеджерВременныхТаблиц, МесяцНачисления = Неопределено, Записывать = Ложь, ОкончательныйРасчет = Истина, ИмяВТНачисления = "ВТНачисления", ИсключаемыйРегистратор = Неопределено, МенеджерКадровогоУчета = Неопределено, ДоходПолученНаТерриторииРФ = Ложь) Экспорт
	...
	Если НастройкиПрограммыБюджетногоУчреждения.ИспользоватьВоеннуюСлужбу Тогда
		...
		ПоляИсключения.Добавить("СуммаДохода");
		ПоляИсключения.Добавить("СуммаВычетаНДФЛ");
		ПоляИсключения.Добавить("СуммаДоходаНеОблагаемаяНДФЛ");
		ЗарплатаКадры.ЗаполнитьИменаПолейВТ(ТекстЗапроса, МенеджерВременныхТаблиц, ИмяВТНачисления, "НачисленияДокумента", ПоляИсключения);
	...
	Если ДатаВыплаты >= ДатаЗаконаПроРКиЕАЭС() Тогда
		
		// Ведем обработку строк, если был пересчет документов, которые были расчитаны без долей РК и СН, а в пересчитываемом
		// документе есть доли и общая сумма пересчета не равна нулю.
		...
		Запрос.УстановитьПараметр("КатегорииДолейРКСН", КатегорииДолейРКСН);
		
		КатегорииДолейСторнируемогоНачисления = Новый Массив;
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ДенежноеДовольствиеКомпенсацияОтпуска"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ДенежноеСодержаниеКомпенсацияОтпуска"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ДенежноеСодержаниеНаПериодКомандировки"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ДенежноеСодержаниеНаПериодОтпуска"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаОтпуска"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.КомпенсацияОтпуска"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПростояПоВинеРаботодателя"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.СохраняемоеДенежноеСодержание"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПоСреднемуЗаработку"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаКомандировки"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.СохраняемоеДенежноеДовольствие"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛиста"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛистаЗаСчетРаботодателя"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоПрофзаболевание"));
		КатегорииДолейСторнируемогоНачисления.Добавить(ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоНесчастныйСлучайНаПроизводстве"));
		Запрос.УстановитьПараметр("КатегорииДолейСторнируемогоНачисления", КатегорииДолейСторнируемогоНачисления);
		
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	Начисления.Ссылка КАК НачислениеРКСН,
		|	Начисления.ОсновнойВидРасчета КАК ОсновнойВидРасчета,
		|	Начисления.ОсновнойВидРасчета.КодДоходаНДФЛ КАК КодДоходаНДФЛ,
		|	Начисления.ОсновнойВидРасчета.КатегорияНачисленияИлиНеоплаченногоВремени КАК КатегорияНачисления
		|ПОМЕСТИТЬ ВТНачисленияДолейРКСН
		|ИЗ
		|	ПланВидовРасчета.Начисления КАК Начисления
		|ГДЕ
		|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииДолейРКСН)
		|	И Начисления.ПометкаУдаления = ЛОЖЬ
		|	И Начисления.ОсновнойВидРасчета <> ЗНАЧЕНИЕ(ПланВидовРасчета.Начисления.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Начисления.Ссылка КАК НачислениеСторнируемое,
		|	Начисления.КодДоходаНДФЛ КАК КодДоходаНДФЛ,
		|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени КАК КатегорияНачисления
		|ПОМЕСТИТЬ ВТНачисленияСторнируемые
		|ИЗ
		|	ПланВидовРасчета.Начисления КАК Начисления
		|ГДЕ
		|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииДолейСторнируемогоНачисления)
		|	И Начисления.ПометкаУдаления = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ВременнаяТаблица.Сторно
		|			И НачисленияСторнируемые.НачислениеСторнируемое ЕСТЬ НЕ NULL ) КАК ЕстьСторно,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА НачисленияДолейРКСН.ОсновнойВидРасчета ЕСТЬ NULL
		|				ТОГДА ЛОЖЬ
		|			КОГДА ВременнаяТаблица.Начисление = НачисленияДолейРКСН.ОсновнойВидРасчета
		|				ТОГДА ИСТИНА
		|		КОНЕЦ) КАК ЕстьОсновноеНачисление,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА НачисленияДолейРКСН.НачислениеРКСН ЕСТЬ NULL
		|				ТОГДА ЛОЖЬ
		|			КОГДА ВременнаяТаблица.Начисление = НачисленияДолейРКСН.НачислениеРКСН
		|				ТОГДА ИСТИНА
		|		КОНЕЦ) КАК ЕстьДолевоеНачисление
		|ИЗ
		|	ВТВременнаяТаблица КАК ВременнаяТаблица
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияДолейРКСН КАК НачисленияДолейРКСН
		|		ПО (ВременнаяТаблица.Начисление = НачисленияДолейРКСН.НачислениеРКСН
		|				ИЛИ ВременнаяТаблица.Начисление = НачисленияДолейРКСН.ОсновнойВидРасчета)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияСторнируемые КАК НачисленияСторнируемые
		|		ПО ВременнаяТаблица.Начисление = НачисленияСторнируемые.НачислениеСторнируемое
		|
		|ИМЕЮЩИЕ
		|	МАКСИМУМ((ВЫБОР
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.НачислениеЗарплаты
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.НачислениеЗарплаты)
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.Отпуск
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.Отпуск)
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.БольничныйЛист
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.БольничныйЛист)
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.Командировка
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.Командировка)
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.ОплатаПоСреднемуЗаработку
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.ОплатаПоСреднемуЗаработку)
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.Увольнение
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.Увольнение)
		|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.УвольнениеСписком
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.УвольнениеСписком)
		|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
		|		КОНЕЦ).ПланируемаяДатаВыплаты) < ДАТАВРЕМЯ(2026, 1, 1, 0, 0, 0) И
		|	МАКСИМУМ(ВременнаяТаблица.Сторно
		|			И НачисленияСторнируемые.НачислениеСторнируемое ЕСТЬ НЕ NULL ) = ИСТИНА И
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА НачисленияДолейРКСН.НачислениеРКСН ЕСТЬ NULL
		|				ТОГДА ЛОЖЬ
		|			КОГДА ВременнаяТаблица.Начисление = НачисленияДолейРКСН.НачислениеРКСН
		...
		Если НастройкиПрограммыБюджетногоУчреждения.ИспользоватьВоеннуюСлужбу Тогда
			ТекстЗамены = 
			"		КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.ИсключениеИзСписковЧасти
			|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.ИсключениеИзСписковЧасти)
			|			КОГДА ВременнаяТаблица.СторнируемыйДокумент ССЫЛКА Документ.ИсключениеИзСписковЧастиСписком
			|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблица.СторнируемыйДокумент КАК Документ.ИсключениеИзСписковЧастиСписком)
			|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)";
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)",ТекстЗамены);
		КонецЕсли;
		
		Если Не Запрос.Выполнить().Пустой() Тогда
			Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	НачисленияДолейРКСН.НачислениеРКСН КАК ДолевоеНачисление,
			|	НачисленияДолейРКСН.КодДоходаНДФЛ КАК КодДоходаНДФЛ,
			|	НачисленияДолейРКСН.КатегорияНачисления КАК КатегорияНачисления
			|ПОМЕСТИТЬ ВТДолевыеНачисления
			|ИЗ
			|	ВТВременнаяТаблица КАК ВременнаяТаблица
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНачисленияДолейРКСН КАК НачисленияДолейРКСН
			|		ПО ВременнаяТаблица.Начисление = НачисленияДолейРКСН.НачислениеРКСН
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	НачисленияДолейРКСН.ОсновнойВидРасчета КАК ОсновнойВидРасчета,
			|	НачисленияДолейРКСН.КодДоходаНДФЛ КАК КодДоходаНДФЛ,
			|	НачисленияДолейРКСН.КатегорияНачисления КАК КатегорияНачисления
			|ПОМЕСТИТЬ ВТОсновныеВидыРасчета
			|ИЗ
			|	ВТНачисленияДолейРКСН КАК НачисленияДолейРКСН
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ВременнаяТаблица.Сторно
			|		И ДолевыеНачисления.ДолевоеНачисление ЕСТЬ НЕ NULL  КАК СторноДолевое,
			|	ВременнаяТаблица.Сторно
			|		И НачисленияСторнируемые.НачислениеСторнируемое ЕСТЬ НЕ NULL КАК СторноОсновное,
			|	ДолевыеНачисления.ДолевоеНачисление ЕСТЬ НЕ NULL КАК ДолевоеНачисление,
			|	ОсновныеВидыРасчета.ОсновнойВидРасчета ЕСТЬ НЕ NULL И НЕ ВременнаяТаблица.Сторно КАК ОсновноеНачисление,
			|	ЕСТЬNULL(ОсновныеВидыРасчета.КодДоходаНДФЛ, ЕСТЬNULL(ДолевыеНачисления.КодДоходаНДФЛ, НачисленияСторнируемые.КодДоходаНДФЛ)) КАК КодДоходаНДФЛ,
			|	ЕСТЬNULL(ОсновныеВидыРасчета.КатегорияНачисления, ЕСТЬNULL(ДолевыеНачисления.КатегорияНачисления, НачисленияСторнируемые.КатегорияНачисления)) КАК КатегорияНачисления,
			|	ВременнаяТаблица.*
			|ПОМЕСТИТЬ ВТВременнаяТаблицаСДопКолонками
			|ИЗ
			|	ВТВременнаяТаблица КАК ВременнаяТаблица
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОсновныеВидыРасчета КАК ОсновныеВидыРасчета
			|		ПО ВременнаяТаблица.Начисление = ОсновныеВидыРасчета.ОсновнойВидРасчета
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДолевыеНачисления КАК ДолевыеНачисления
			|		ПО ВременнаяТаблица.Начисление = ДолевыеНачисления.ДолевоеНачисление
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияСторнируемые КАК НачисленияСторнируемые
			|		ПО ВременнаяТаблица.Начисление = НачисленияСторнируемые.НачислениеСторнируемое
			|;
			|
			...
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ВременнаяТаблица.Сотрудник КАК Сотрудник,
			|	ВременнаяТаблица.КодДоходаНДФЛ КАК КодДоходаНДФЛ,
			|	ВременнаяТаблица.КатегорияНачисления КАК КатегорияНачисления,
			|	ВременнаяТаблица.Подразделение КАК Подразделение,
			|	СУММА(ВЫБОР
			|			КОГДА ВременнаяТаблица.СторноОсновное
			|				ТОГДА ВременнаяТаблица.СуммаДохода
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК СуммаСторно,
			|	СУММА(ВЫБОР
			|			КОГДА ВременнаяТаблица.ДолевоеНачисление
			|				ТОГДА ВременнаяТаблица.СуммаДохода
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК СуммаДолевоеНачисление,
			|	СУММА(ВЫБОР
			|			КОГДА ВременнаяТаблица.ОсновноеНачисление
			|				ТОГДА ВременнаяТаблица.СуммаДохода
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК СуммаОсновноеНачисление
			|ИЗ
			|	ВТВременнаяТаблицаСДопКолонками КАК ВременнаяТаблица
			|
			|СГРУППИРОВАТЬ ПО
			|	ВременнаяТаблица.Сотрудник,
			|	ВременнаяТаблица.КодДоходаНДФЛ,
			|	ВременнаяТаблица.Подразделение,
			|	ВременнаяТаблица.КатегорияНачисления
			|
			|ИМЕЮЩИЕ
			|	СУММА(ВЫБОР
			|			КОГДА ВременнаяТаблица.СторноДолевое
			|				ТОГДА ВременнаяТаблица.СуммаДохода
			|			ИНАЧЕ 0
			|		КОНЕЦ) = 0 И
			|	СУММА(ВЫБОР
			...
			|
			|////////////////////////////////////////////////////////////////////////////////
			|	ВЫБРАТЬ * ИЗ ВТВременнаяТаблицаСДопКолонками";
			
			РезультатЗапроса = Запрос.ВыполнитьПакет();
			ТаблицаДолей = РезультатЗапроса[3].Выгрузить();
			ВременнаяТаблица = РезультатЗапроса[4].Выгрузить();
			
			СтруктураПоиска = Новый Структура("Сотрудник,КодДоходаНДФЛ,Подразделение,КатегорияНачисления");
			СтруктураПоискаОсновное = Новый Структура("Сотрудник,КодДоходаНДФЛ,Подразделение,КатегорияНачисления,ОсновноеНачисление",,,,,Истина);
			СтруктураПоискаДолевое = Новый Структура("Сотрудник,КодДоходаНДФЛ,Подразделение,КатегорияНачисления,ДолевоеНачисление",,,,,Истина);
			СтруктураПоискаСторно = Новый Структура("Сотрудник,КодДоходаНДФЛ,Подразделение,КатегорияНачисления,СторноОсновное",,,,,Истина);
			
			МассивСтрокКУдалению = Новый Массив;
			Если ТаблицаДолей.Количество() > 100 Тогда
				ВременнаяТаблица.Индексы.Добавить("Сотрудник");
			КонецЕсли;
		
			Для Каждого Доля Из ТаблицаДолей Цикл
				ДоляОсновное = Доля.СуммаОсновноеНачисление;
				ДоляРКСН = Доля.СуммаДолевоеНачисление;
				Сторно = Доля.СуммаСторно;
				Сумма = ДоляОсновное + ДоляРКСН + Сторно;
				//Если Сумма > 0, 
				//	то общую сумму перерасчета записываем по кодам дохода НДФЛ "основного" и "долевого" начисления
				//	пропорционально суммам дохода этих начислений.
				//Если Сумма < 0, 
				//	то общую сумму перерасчета записываем по коду дохода НДФЛ "основного" начисления 
				//	на исходную (фактическую) дату получения дохода.
				//Если Сумма = 0, 
				//	то в учет НДФЛ не пишем ничего.
				
				ЗаполнитьЗначенияСвойств(СтруктураПоиска,Доля); // Сотрудник, КодДоходаНДФЛ, Подразделение, КатегорияНачисления
				ЗаполнитьЗначенияСвойств(СтруктураПоискаОсновное,СтруктураПоиска);
				ЗаполнитьЗначенияСвойств(СтруктураПоискаДолевое,СтруктураПоиска);
				ЗаполнитьЗначенияСвойств(СтруктураПоискаСторно,СтруктураПоиска);
				
				Если Сумма > 0 Тогда
					// Удаляем сторно основного начисления
					СтрокиСторноОсновное = ВременнаяТаблица.НайтиСтроки(СтруктураПоискаСторно);
					Для Каждого СтрокаКУдалению Из СтрокиСторноОсновное Цикл
						МассивСтрокКУдалению.Добавить(СтрокаКУдалению);
					КонецЦикла;
					// Распределяем сумму исходя из доли на основное начисление и РКСН
					СтрокиКОбработке = ВременнаяТаблица.НайтиСтроки(СтруктураПоискаОсновное);
					СтрокиДолевое = ВременнаяТаблица.НайтиСтроки(СтруктураПоискаДолевое);
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтрокиКОбработке, СтрокиДолевое);
					МассивКоэффициентов = Новый Массив;
					Для Каждого СтрокаКОбработке Из СтрокиКОбработке Цикл
						МассивКоэффициентов.Добавить(СтрокаКОбработке.СуммаДохода);
					КонецЦикла;
					МассивДоходов = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Сумма, МассивКоэффициентов);
					Для Индекс = 0 По МассивДоходов.ВГраница() Цикл
						СтрокиКОбработке[Индекс].СуммаДохода = МассивДоходов[Индекс];
					КонецЦикла;
				ИначеЕсли Сумма < 0 Тогда
					СтрокиКОбработке = ВременнаяТаблица.НайтиСтроки(СтруктураПоиска);
					Для Каждого СтрокаКОбработке ИЗ СтрокиКОбработке Цикл
						Если СтрокаКОбработке.ДолевоеНачисление Или СтрокаКОбработке.ОсновноеНачисление Тогда
							МассивСтрокКУдалению.Добавить(СтрокаКОбработке);
						КонецЕсли;
					КонецЦикла;
					Если ДоляРКСН+ДоляОсновное <> 0 Тогда
						СтрокиКОбработке = ВременнаяТаблица.НайтиСтроки(СтруктураПоискаСторно);
						МассивКоэффициентов = Новый Массив;
						Для Каждого СтрокаКОбработке Из СтрокиКОбработке Цикл
							МассивКоэффициентов.Добавить(СтрокаКОбработке.СуммаДохода);
						КонецЦикла;
						МассивДоходов = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(ДоляРКСН+ДоляОсновное, МассивКоэффициентов);
						Для Индекс = 0 По МассивДоходов.ВГраница() Цикл
							СтрокиКОбработке[Индекс].СуммаДохода = СтрокиКОбработке[Индекс].СуммаДохода + МассивДоходов[Индекс];
						КонецЦикла;
					КонецЕсли;
				Иначе // Сумма = 0
					СтрокиКОбработке = ВременнаяТаблица.НайтиСтроки(СтруктураПоиска);
					Для Каждого СтрокаКОбработке ИЗ СтрокиКОбработке Цикл
						Если СтрокаКОбработке.СторноОсновное Или СтрокаКОбработке.ДолевоеНачисление Или СтрокаКОбработке.ОсновноеНачисление Тогда
							МассивСтрокКУдалению.Добавить(СтрокаКОбработке);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			...
			Для Каждого СтрокаКУдалению Из МассивСтрокКУдалению Цикл
				ВременнаяТаблица.Удалить(СтрокаКУдалению);
			КонецЦикла;
			...
			ВременнаяТаблица.Колонки.Удалить("СторноДолевое");
			ВременнаяТаблица.Колонки.Удалить("СторноОсновное");
			ВременнаяТаблица.Колонки.Удалить("ДолевоеНачисление");
			ВременнаяТаблица.Колонки.Удалить("ОсновноеНачисление");
			ВременнаяТаблица.Колонки.Удалить("КодДоходаНДФЛ");
			ВременнаяТаблица.Колонки.Удалить("КатегорияНачисления");
	...
	Запрос.УстановитьПараметр("РегистрацияОрганизации",РегистрацияОрганизации);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Начисления.Подразделение КАК СтруктурнаяЕдиница
	...
	|ПОМЕСТИТЬ ВТРегистрацииВНалоговомОрганеПоСтруктурнымЕдиницам
	|ИЗ
	...
	|				ИЗ
	|					ВТСтруктурныеЕдиницы КАК СтруктурныеЕдиницы)) КАК ИсторияРегистрацийВНалоговомОрганеСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СтруктурныеЕдиницы.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница,
	|	ЕСТЬNULL(РегистрацииВНалоговомОрганеПоСтруктурнымЕдиницам.РегистрацияВНалоговомОргане, &РегистрацияОрганизации) КАК РегистрацияВНалоговомОргане
	|ПОМЕСТИТЬ ВТРегистрацииВНалоговомОргане
	|ИЗ
	|	ВТСтруктурныеЕдиницы КАК СтруктурныеЕдиницы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРегистрацииВНалоговомОрганеПоСтруктурнымЕдиницам КАК РегистрацииВНалоговомОрганеПоСтруктурнымЕдиницам
	|		ПО СтруктурныеЕдиницы.СтруктурнаяЕдиница = РегистрацииВНалоговомОрганеПоСтруктурнымЕдиницам.СтруктурнаяЕдиница
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		Если Набор <> Неопределено И ИсправлениеДокументовЗарплатаКадры.ИзолироватьУчетом(Набор, ИмяУчета) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					// Формирование таблицы дат получения дохода для восстановления их после формирования новых движений.
					Если Выборка.ДокументОснование <> Выборка.Регистратор 
						И (Не Выборка.ДатаПолученияДоходаФиксирована 
							Или Не УчетНДФЛ.ДатаПолученияДоходаДолжнаБытьЗафиксирована(Выборка.КатегорияДохода, Выборка.МесяцНалоговогоПериода))
						И Не ЗначениеЗаполнено(Выборка.ЗарегистрированоПриНачисленииАванса) 
						И ЗначениеЗаполнено(Выборка.УстаревшаяДатаПолученияДохода) Тогда
						ЗаполнитьЗначенияСвойств(СохраняемыеДатыДохода.Добавить(), Выборка);
					КонецЕсли;
...
Функция ИсчисленныйНалогСДоходами(ИсчисленныйНалог, НачисленныеДоходы, ДатаПолученияДохода) Экспорт
	...
	ТаблицаРезультатов.Колонки.Добавить("СуммаДоходаСПревышенияСКоторогоНеУдержанНалог", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(15, 2)));
	ТаблицаРезультатов.Колонки.Добавить("СуммаДоходаСПревышенияПоСтавке18СКоторогоНеУдержанНалог", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(15, 2)));
	ТаблицаРезультатов.Колонки.Добавить("СуммаДоходаСПревышенияПоСтавке20СКоторогоНеУдержанНалог", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(15, 2)));
	ТаблицаРезультатов.Колонки.Добавить("СуммаДоходаСПревышенияПоСтавке22СКоторогоНеУдержанНалог", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(15, 2)));
	
	Если ДатаПолученияДохода >= УчетНДФЛ.ДатаЗакона176ФЗ() Тогда
		...
		ПараметрыПревышения = Новый Соответствие;
		ТаблицаОписаний = УчетНДФЛПовтИсп.СоставНалоговыхБаз();
		СтрокиОсновнойНалоговойБазы = ТаблицаОписаний.НайтиСтроки(Новый Структура("СтавкаНалогообложенияРезидента,СтатусФизическогоЛица", Перечисления.НДФЛСтавкиНалогообложенияРезидента.Ставка13, Справочники.СтатусыНалогоплательщиковПоНДФЛ.Резидент));
		Для каждого СтрокаТаблицы Из СтрокиОсновнойНалоговойБазы Цикл
			ПараметрыПревышения.Вставить(СтрокаТаблицы.КатегорияДохода,СоответствиеПараметровНалоговыхБаз[СтрокаТаблицы.НалоговаяБаза]) 
		КонецЦикла; 
	КонецЕсли;
	
	Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		Если СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = 0 Тогда
			Продолжить;
		КонецЕсли;
		Если СтрокаТаблицы.СуммаСПревышения = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке18 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке20 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке22 = 0 Тогда
			// нет превышения
			Продолжить;
		КонецЕсли;
		Если СтрокаТаблицы.Сумма = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке18 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке20 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке22 = 0 Тогда
			СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог = СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
			СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = 0;
			Продолжить;
		КонецЕсли;
		Если СтрокаТаблицы.Сумма = 0 И СтрокаТаблицы.СуммаСПревышения = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке20 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке22 = 0 Тогда
			СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке18СКоторогоНеУдержанНалог = СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
			СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = 0;
			Продолжить;
		КонецЕсли;
		Если СтрокаТаблицы.Сумма = 0 И СтрокаТаблицы.СуммаСПревышения = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавк18 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке22 = 0 Тогда
			СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке20СКоторогоНеУдержанНалог = СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
			СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = 0;
			Продолжить;
		КонецЕсли;
		Если СтрокаТаблицы.Сумма = 0 И СтрокаТаблицы.СуммаСПревышения = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавк18 = 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке20 = 0 Тогда
			СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке22СКоторогоНеУдержанНалог = СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
			СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = 0;
			Продолжить;
		КонецЕсли;
		// превышен один из пределов или больше
		СуммаДоходаСКоторогоНеУдержанНалог = СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
		СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = 0;
		Если ДатаПолученияДохода >= УчетНДФЛ.ДатаЗакона176ФЗ() Тогда
			ТекущиеПараметрыПревышения = ПараметрыПревышения[СтрокаТаблицы.КатегорияДохода];
			Если ТекущиеПараметрыПревышения = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если ТекущиеПараметрыПревышения.КоличествоПределов = 1 Тогда
				Если СтрокаТаблицы.Сумма = ТекущиеПараметрыПревышения.НалогСПредела1 Тогда
					СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = Мин(ТекущиеПараметрыПревышения.Предел1, СуммаДоходаСКоторогоНеУдержанНалог);
					СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог = СуммаДоходаСКоторогоНеУдержанНалог - СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
				Иначе 	
					Коэффициенты = Новый Массив;
					Коэффициенты.Добавить(СтрокаТаблицы.Сумма / 13);
					Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышения / 15);
					РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(СуммаДоходаСКоторогоНеУдержанНалог, Коэффициенты);
					СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = РезультатРаспределения[0];
					СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог = РезультатРаспределения[1];
				КонецЕсли;
				Продолжить;
			КонецЕсли;
			Коэффициенты = Новый Массив;
			Если СтрокаТаблицы.Сумма = ТекущиеПараметрыПревышения.НалогСПредела1 Тогда
				СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = Мин(ТекущиеПараметрыПревышения.Предел1, СуммаДоходаСКоторогоНеУдержанНалог);
				СуммаДоходаСКоторогоНеУдержанНалог = СуммаДоходаСКоторогоНеУдержанНалог - СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог;
				Если СуммаДоходаСКоторогоНеУдержанНалог = 0 Тогда
					Продолжить;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.Сумма <> 0 Тогда	
				Коэффициенты.Добавить(СтрокаТаблицы.Сумма / 13);
			КонецЕсли;
			Если СтрокаТаблицы.СуммаСПревышения = ТекущиеПараметрыПревышения.НалогСПредела2 Тогда
				СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог = Мин(ТекущиеПараметрыПревышения.Предел2 - ТекущиеПараметрыПревышения.Предел1, СуммаДоходаСКоторогоНеУдержанНалог);
				СуммаДоходаСКоторогоНеУдержанНалог = СуммаДоходаСКоторогоНеУдержанНалог - СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог;
				Если СуммаДоходаСКоторогоНеУдержанНалог = 0 Тогда
					Продолжить;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СуммаСПревышения <> 0 Тогда 	
				Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышения / 15);
			КонецЕсли;
			Если СтрокаТаблицы.СуммаСПревышенияПоСтавке18 = ТекущиеПараметрыПревышения.НалогСПредела3 Тогда
				СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке18СКоторогоНеУдержанНалог = Мин(ТекущиеПараметрыПревышения.Предел3 - ТекущиеПараметрыПревышения.Предел2, СуммаДоходаСКоторогоНеУдержанНалог);
				СуммаДоходаСКоторогоНеУдержанНалог = СуммаДоходаСКоторогоНеУдержанНалог - СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке18СКоторогоНеУдержанНалог;
				Если СуммаДоходаСКоторогоНеУдержанНалог = 0 Тогда
					Продолжить;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СуммаСПревышенияПоСтавке18 <> 0 Тогда 	
				Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышенияПоСтавке18 / 18);
			КонецЕсли;
			Если СтрокаТаблицы.СуммаСПревышенияПоСтавке20 = ТекущиеПараметрыПревышения.НалогСПредела4 Тогда
				СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке20СКоторогоНеУдержанНалог = Мин(ТекущиеПараметрыПревышения.Предел4 - ТекущиеПараметрыПревышения.Предел3, СуммаДоходаСКоторогоНеУдержанНалог);
				СуммаДоходаСКоторогоНеУдержанНалог = СуммаДоходаСКоторогоНеУдержанНалог - СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке20СКоторогоНеУдержанНалог;
				Если СуммаДоходаСКоторогоНеУдержанНалог = 0 Тогда
					Продолжить;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СуммаСПревышенияПоСтавке20 <> 0 Тогда 	
				Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышенияПоСтавке20 / 20);
			КонецЕсли;
			Если Коэффициенты.Количество() = 0 Тогда
				СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке22СКоторогоНеУдержанНалог = СуммаДоходаСКоторогоНеУдержанНалог;
			Иначе
				Если СтрокаТаблицы.СуммаСПревышенияПоСтавке22 <> 0 Тогда
					Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышенияПоСтавке22 / 22);
				КонецЕсли;
				РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(СуммаДоходаСКоторогоНеУдержанНалог, Коэффициенты);
				СледующийИндекс = 0;
				Если СтрокаТаблицы.Сумма <> 0 И СтрокаТаблицы.Сумма <> ТекущиеПараметрыПревышения.НалогСПредела1 Тогда
					СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = РезультатРаспределения[СледующийИндекс];
					СледующийИндекс = СледующийИндекс + 1;
				КонецЕсли;
				Если СтрокаТаблицы.СуммаСПревышения <> 0 И СтрокаТаблицы.СуммаСПревышения <> ТекущиеПараметрыПревышения.НалогСПредела2 Тогда
					СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог = РезультатРаспределения[СледующийИндекс];
					СледующийИндекс = СледующийИндекс + 1;
				КонецЕсли;
				Если СтрокаТаблицы.СуммаСПревышенияПоСтавке18 <> 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке18 <> ТекущиеПараметрыПревышения.НалогСПредела3 Тогда
					СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке18СКоторогоНеУдержанНалог = РезультатРаспределения[СледующийИндекс];
					СледующийИндекс = СледующийИндекс + 1;
				КонецЕсли;
				Если СтрокаТаблицы.СуммаСПревышенияПоСтавке20 <> 0 И СтрокаТаблицы.СуммаСПревышенияПоСтавке20 <> ТекущиеПараметрыПревышения.НалогСПредела4 Тогда
					СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке20СКоторогоНеУдержанНалог = РезультатРаспределения[СледующийИндекс];
					СледующийИндекс = СледующийИндекс + 1;
				КонецЕсли;
				Если СтрокаТаблицы.СуммаСПревышенияПоСтавке22 <> 0 Тогда
					СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке22СКоторогоНеУдержанНалог = РезультатРаспределения[СледующийИндекс];
				КонецЕсли;
			КонецЕсли;
		Иначе 	
			Коэффициенты = Новый Массив;
			...
			Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышенияПоСтавке20 / 20);
			Коэффициенты.Добавить(СтрокаТаблицы.СуммаСПревышенияПоСтавке22 / 22);
			РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(СуммаДоходаСКоторогоНеУдержанНалог, Коэффициенты, 0);
			СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог = РезультатРаспределения[0];
			СтрокаТаблицы.СуммаДоходаСПревышенияСКоторогоНеУдержанНалог = РезультатРаспределения[1];
			СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке18СКоторогоНеУдержанНалог = РезультатРаспределения[2];
			СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке20СКоторогоНеУдержанНалог = РезультатРаспределения[3];
			СтрокаТаблицы.СуммаДоходаСПревышенияПоСтавке22СКоторогоНеУдержанНалог = РезультатРаспределения[4];
		КонецЕсли;
...
// Возвращает дату начала действия положений о гражданах ЕАЭС и РК из Федерального закона от 28.11.2025 № 425-ФЗ
//
// Параметры
//  нет
//
...
//   дата
//
Функция ДатаЗаконаПроРКиЕАЭС() Экспорт 

	Возврат УчетНДФЛ.ДатаЗаконаПроРКиЕАЭС()
...
Процедура ВосстановитьДатыПолученияДохода(Набор, ПараметрыКорректировки)
	...
	ДоступныеСтрокиНабора.Колонки.Добавить("СтруктураСтроки"); // Структура, содержащая строку исходного набора, изменяться будут эти строки.
	Для Каждого Строка Из Набор Цикл
		Если Не Строка.Сторно Тогда
			...
			Если Не ДатаПолученияДоходаФиксирована Тогда
				...
				СтруктураСтроки = Новый Структура("СтрокаНабора, РаспределеноПолностью", Строка, Ложь);
				НоваяСтрока.СтруктураСтроки = СтруктураСтроки;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого КолонкиГруппировки Из ПараметрыКорректировки.ВариантыГруппировки Цикл
		...
		Для Каждого Сохраняемое Из СохраняемыеДатыДохода Цикл
			...
			Для Каждого Строка Из СтрокиДляКорректировки Цикл
				...
				Если Строка.СтруктураСтроки.РаспределеноПолностью Тогда
					Продолжить;
				КонецЕсли;
				...
				Для Каждого Ресурс Из Ресурсы Цикл
					Если Сохраняемое[Ресурс] = 0 И Строка.СтруктураСтроки.СтрокаНабора[Ресурс] = 0 Тогда
						Продолжить;
					КонецЕсли;
						
					Остаток = Сохраняемое[Ресурс] - Строка.СтруктураСтроки.СтрокаНабора[Ресурс];
					Если Остаток = 0 Тогда
						Сохраняемое[Ресурс] = 0;
						Строка.СтруктураСтроки.РаспределеноПолностью = Истина;
					...
					Иначе
						...
						Строка.СтруктураСтроки.СтрокаНабора[Ресурс] = Сохраняемое[Ресурс];
						Сохраняемое[Ресурс] = 0;
					КонецЕсли;
				КонецЦикла;
				
				ЗаполнитьЗначенияСвойств(Строка.СтруктураСтроки.СтрокаНабора, ЗначенияКорректировки);
				Строка.СтруктураСтроки.СтрокаНабора.ДокументОснование = ПараметрыКорректировки.ИсправленныйДокумент;
				Набор.Записывать = Истина;
				
				Если СтрокаНабораНовая <> Неопределено Тогда
					Строка.СтруктураСтроки.СтрокаНабора = СтрокаНабораНовая;
				КонецЕсли;
...
//
// Параметры:
//   Организация - СправочникСсылка.Организации - Организация, для которой определяется признак.
//   ЗарегистрированоДляПередачиВБанкНаАУСН - Булево - Для изменения. Изменяется на Истина
//       в случае, если организация применяет АУСН и автоматически передает данные об НДФЛ в банк/ЛК ФНС.
//   МесяцАктуальности - Дата - Дата актуальности на которую необходимо получить признак.
Процедура ОрганизацияНаАУСНПередаетНДФЛвБанк(Организация, ЗарегистрированоДляПередачиВБанкНаАУСН, МесяцАктуальности) Экспорт
	Если ЗначениеЗаполнено(МесяцАктуальности) И ЗарплатаКадрыРасширенныйПовтИсп.ОрганизацияПрименяетАУСН(Организация, МесяцАктуальности) Тогда
		ЗарегистрированоДляПередачиВБанкНаАУСН = РегистрыСведений["ВзаимодействиеСБанкамиНаАУСН"].ПолучитьПоследнее(
			МесяцАктуальности, Новый Структура("ГоловнаяОрганизация", Организация)).ЗарплатаПередаетсяВБанк = Истина;
	КонецЕсли;
КонецПроцедуры
...
Процедура УточнитьСторнируемыеПоНачислениям(ТаблицаСторнируемыхДокументов, ТаблицаПромежуточныхИсправлений, МенеджерВременныхТаблиц, ИмяВТ)
	...
	Если Зацикливания.Количество() Тогда
		...
		Для Каждого Строка Из Зацикливания Цикл
			ИмяСобытия = НСтр("ru = 'Зацикливание'", ОбщегоНазначения.КодОсновногоЯзыка());
