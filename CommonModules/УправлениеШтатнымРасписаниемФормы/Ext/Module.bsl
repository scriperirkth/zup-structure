...
Процедура УстановитьУсловноеОформлениеСпискаПодразделений(Форма, ИмяСписка, ИмяРеквизитаСформировано = "Сформировано",
	ИмяРеквизитаРасформировано = "Расформировано", ИмяРеквизитаНаименование = "Наименование") Экспорт
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяРеквизитаНаименование);
...
Процедура ДокументыПриЧтенииНаСервере(Форма, ТекущийОбъект, ДатаВступленияВСилу) Экспорт
	...
	ПоместитьДанныеОбъектаВДанныеФормы(Форма, ДатаВступленияВСилу, ТекущийОбъект);
...
Функция ОписаниеДанныхПозицииВХранилище(Форма, Объект, ИдентификаторСтрокиТекущейПозиции, ДатаВступленияВСилу, ПодразделениеПозиций = Неопределено,
										ВыборПодчиненныхПодразделений = Ложь, ИдентификаторПозицииИсточника = Неопределено) Экспорт
	...
	Если ИдентификаторПозицииИсточника <> Неопределено Тогда
		СтрокаИсточник = Объект.Позиции.НайтиПоИдентификатору(ИдентификаторПозицииИсточника);
		ОписаниеПозиции.Вставить("ИдентификаторСтрокиИсточникаПозиции", СтрокаИсточник.ИдентификаторСтрокиПозиции);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
		...
		МодульОбучениеРазвитие.ПоместитьОбучениеВОписаниеПозиции(Форма, ОписаниеПозиции, ДанныеПозиции);
	КонецЕсли;
...
Процедура ПрочитатьДанныеПозицииВФорму(Форма, Объект, АдресДанныхПозиции) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
		МодульОбучениеРазвитие = ОбщегоНазначения.ОбщийМодуль("ОбучениеРазвитиеФормы");
		МодульОбучениеРазвитие.ПрочитатьДанныеПозицииВФорму(Форма, Объект, НовыеДанныеПозиции);
	КонецЕсли;
...
Процедура ЗаполнитьНовыйДокумент(Форма, ДатаВступленияВСилу, ПрочитатьТекущиеДанные = Истина) Экспорт
	...
	Если ДокументОбъект.ДополнительныеСвойства.Свойство("КоллекцияНаименований") Тогда
		Для Каждого ДанныеНаименований Из ДокументОбъект.ДополнительныеСвойства.КоллекцияНаименований Цикл
			
			СтрокиПозиции = Форма.Объект.Позиции.НайтиСтроки(Новый Структура("ИдентификаторСтрокиПозиции", ДанныеНаименований.Ключ));
			Если ЗначениеЗаполнено(СтрокиПозиции) Тогда
				ЗаполнитьЗначенияСвойств(СтрокиПозиции[0], ДанныеНаименований.Значение);
			КонецЕсли;
	...
	ПоместитьДанныеОбъектаВДанныеФормы(Форма, ДатаВступленияВСилу, ДокументОбъект, ПрочитатьТекущиеДанные);
...
Процедура ОбработатьЗаполнениеДокументаВФорме(Форма) Экспорт
	...
	Форма.ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
	ПоместитьДанныеОбъектаВДанныеФормы(Форма, ДанныеЗаполнения.ДатаВступленияВСилу, Форма.Объект);
...
Процедура ПоместитьДанныеОбъектаВДанныеФормы(Форма, ДатаВступленияВСилу, ТекущийОбъект = Неопределено, ПрочитатьТекущиеДанные = Истина) Экспорт
	...
	КонецЕсли;
	Если ПрочитатьТекущиеДанные Тогда
	...
	Иначе
		...
		Для Каждого СтрокаПозиции Из ПозицииДанныеФормы Цикл
			СтрокаПозиции.ДействующаяПозиция = Истина;
			СтрокаПозиции.ТекущееКоличествоСтавок = СтрокаПозиции.КоличествоСтавок;
			СтрокаПозиции.ТекущийФОТ = СтрокаПозиции.ФОТ;
		КонецЦикла;
	КонецЕсли;
...
Процедура СкопироватьДанныеПозиции(Форма, ИдентификаторТекущейПозиции, ИдентификаторПозицииИсточника) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ХарактеристикиПерсонала") Тогда
		МодульХарактеристикиПерсоналаФормы = ОбщегоНазначения.ОбщийМодуль("ХарактеристикиПерсоналаФормы");
		МодульХарактеристикиПерсоналаФормы.ПриКопированииДанныхПозиции(
			Форма, СтрокаИсточник.ИдентификаторСтрокиПозиции, ТекущиеДанные.ИдентификаторСтрокиПозиции);
	КонецЕсли;
			
КонецПроцедуры

Процедура УстановитьВидимостьКолонокСпецоценки(Форма)
	
	РазрешитьВидимостьСпецоценки = Истина;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		"ПозицииКлассУсловийТруда",
		"Видимость",
		РазрешитьВидимостьСпецоценки);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		"ПозицииВыявленыВредныеОпасныеФакторы",
		"Видимость",
		РазрешитьВидимостьСпецоценки);
...
Процедура УстановитьВидимостьЭлементовУправленияЗависящихОтФункциональныхОпций(Форма)
	...
	УстановитьВидимостьКолонокСпецоценки(Форма);
...
Процедура ЗаполнитьТекущиеДанныеПозиции(Объект, ТекущиеДанные, ДатаВступленияВСилу)
	...
	НачислениеРайонныйКоэффициент = ЗарплатаКадрыРасширенный.НачислениеРайонныйКоэффициент(ДанныеТекущейПозиции.Организация, ДатаВступленияВСилу);
	НачислениеСевернаяНадбавка = ЗарплатаКадрыРасширенный.НачислениеСевернаяНадбавка(ДанныеТекущейПозиции.Организация, ДатаВступленияВСилу);
