...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ИсправитьДанныеПроизводственныхКалендарейПомесячно");
	Методы.Вставить("ЗаполнитьНомераОплачиваемыхНерабочихПериодов");
	Методы.Вставить("ОбновитьГрафикиРаботыСотрудников2021");
	Методы.Вставить("УстановитьНастройкуКонтроляПлановогоИФактическогоВремени");
	Методы.Вставить("ОбновитьГрафикиРаботыСотрудников2020");
КонецПроцедуры
...
Процедура СоздатьВТРабочееСотрудниковВремяПоИнтервалам(МенеджерВременныхТаблиц, ДатаАктуальности) Экспорт
	...
	ПараметрыПолученияДанных.ДатаАктуальности = ДатаАктуальности;
	ПараметрыПолученияДанных.ДатаНачала = НачалоМесяца(ДатаАктуальности);
	ПараметрыПолученияДанных.ДатаОкончания = КонецМесяца(ДатаАктуальности);
	ПараметрыПолученияДанных.МесяцДатаНачала = НачалоМесяца(ДатаАктуальности);
	ПараметрыПолученияДанных.МесяцДатаОкончания = КонецМесяца(ДатаАктуальности);
	
	СоздатьВТДанныеУчетаВремениИСостоянийСотрудников(МенеджерВременныхТаблиц, Истина, ПараметрыПолученияДанных);
	...
	|	СУММА(ВЫБОР
	|			КОГДА ЕСТЬNULL(ДанныеУчетаРабочегоВремениСотрудников.Часы, 0) > 0
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК НормаДней,
	|	СУММА(ЕСТЬNULL(ДанныеУчетаРабочегоВремениСотрудников.Часы, 0)) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТРабочееВремяПоИнтервалам
	|ИЗ
	|	ВТСотрудникиИнтервалы КАК СотрудникиИнтервалы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеУчетаВремениИСостоянийСотрудников КАК ДанныеУчетаРабочегоВремениСотрудников
	...
	|ГДЕ
	|	ДанныеУчетаРабочегоВремениСотрудников.ВидУчетаВремени.ОсновноеВремя В (&Явка, &Ночные, &Вечерние, &Сверхурочные)
	|
	|СГРУППИРОВАТЬ ПО
	|	СотрудникиИнтервалы.ДатаНачала,
	|	СотрудникиИнтервалы.ДатаОкончания,
	...
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПериодыРаботыСотрудников");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТМесяцы");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСотрудникиМесяцы");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТПериодыДействияГрафиковДляКалендарей");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСоответствиеСостоянийВидамУчетаВремени");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДатыСостояний");
	...
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТСостоянияСотрудников");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТНормаИтоги");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДанныеУчетаВремениПоСостояниям");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТДанныеУчетаВремениИСостоянийСотрудников");
...
//			РасходДнейОтгула
//			РасходЧасовОтгула
Функция ДниЧасыНакопленныхОтгулов(Организация, Сотрудник, НаДату, ИсключаемыйДокумент = Неопределено, ИсправленныйДокумент = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(ВЫБОР
		|			КОГДА ДниЧасыОтгулов.Сторно
		|				ТОГДА -ДниЧасыОтгулов.Часы
		|			ИНАЧЕ ДниЧасыОтгулов.Часы
		|		КОНЕЦ)
		|ИЗ
		|	РегистрНакопления.ДниЧасыОтгулов КАК ДниЧасыОтгулов
		|ГДЕ
		|	ДниЧасыОтгулов.Регистратор В(&ИсключаемыеДокументы)
		|	И ДниЧасыОтгулов.Сотрудник = &Сотрудник
		|	И ДниЧасыОтгулов.Период < &НаДату
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ДниЧасыОтгулов.РасходДнейОтгула) КАК РасходДнейОтгула,
		|	СУММА(ДниЧасыОтгулов.РасходЧасовОтгула) КАК РасходЧасовОтгула
		|ИЗ
		|	ВТДниЧасыОтгулов КАК ДниЧасыОтгулов";
...
Процедура ПеренестиДанныеУчетаВремениВНовыеРегистры() Экспорт 
	...
	Пока РегистраторыДляПереноса.Следующий() Цикл
		...
		Иначе
			...
			Набор.Отбор.Регистратор.Установить(РегистраторыДляПереноса.Регистратор);
			Набор.Прочитать();
			ЗаписатьПараметрыРегистрируемыхДанных(Набор, Истина, ОписаниеРегистра, Истина);
		КонецЕсли; 
...
Функция ТаблицаГрафикиРаботыСотрудников() Экспорт
...
Функция ПараметрыДляПроверкиЗаполненияСверхурочныхДокументов() Экспорт
	...
	ПараметрыПроверки.Вставить("ПодробныйВводВремени", Ложь);
...
Процедура ПриПроверкеЗаполненияСверхурочныхДокументов(ПараметрыПроверки, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	ПодробныйВводВремени = ПараметрыПроверки.ПодробныйВводВремени;
	
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	СоздатьВТДанныеОСверхурочнойРаботе(МенеджерВТ, ДанныеОРаботе, ПодробныйВводВремени);
	СоздатьВТНомераСтрокСверхурочныхПоИдентификаторам(МенеджерВТ);
...
Функция ВидыВремениСПризнакомВахтовоеВремя(ДатаАктуальности = Неопределено) Экспорт
	...
	Если ДатаАктуальности = Неопределено Или ДатаАктуальности < Дата(2026, 1, 1) Тогда 
...
// Возвращает среднемесячное количество рабочих дней, часов по производственному календарю графика работы.
// Рабочие часы зависят от длительности рабочей недели, установленной в графике работы.
//
// Параметры:
// 	ГрафикРаботы - СправочникСсылка.ГрафикиРаботыСотрудников или Массив элементов типа СправочникСсылка.ГрафикиРаботыСотрудников
//  Год - Число. Год, за который необходимо получить данные. 
//
Функция СреднемесячноеКоличествоДнейЧасовПоПроизводственномуКалендарю(ГрафикРаботы, Год) Экспорт
	
	РабочееВремяПоПроизводственномуКалендарю = Новый Структура;
	РабочееВремяПоПроизводственномуКалендарю.Вставить("РабочиеДни", 0);
	РабочееВремяПоПроизводственномуКалендарю.Вставить("РабочиеЧасы", 0);
	...
	|			ИНАЧЕ 0
	|		КОНЕЦ) / 12 КАК РабочиеЧасы,
	|	ГрафикиРаботыСотрудников.Ссылка КАК ГрафикРаботы
	|ИЗ
	|	Справочник.ГрафикиРаботыСотрудников КАК ГрафикиРаботыСотрудников
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПроизводственногоКалендаря
	|		ПО ГрафикиРаботыСотрудников.ПроизводственныйКалендарь = ДанныеПроизводственногоКалендаря.ПроизводственныйКалендарь
	|			И (ГрафикиРаботыСотрудников.Ссылка = &ГрафикРаботы)
	...
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(РабочееВремяПоПроизводственномуКалендарю, Выборка);
	КонецЦикла;
	
	Возврат РабочееВремяПоПроизводственномуКалендарю;
...
Процедура ПроверитьДублиСтрокСверхурочныхДокументов(МенеджерВТ, Ошибки)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДанныеОРаботе.ИдентификаторСтроки) КАК Строк,
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДанныеОРаботе.ИдентификаторСтроки) > 1
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеОРаботе.ИдентификаторСтроки,
	...
	|ПОМЕСТИТЬ ВТПервыеСтрокиПредварительно
	|ИЗ
	|	ВТДубли КАК ВТДубли
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПервыеСтрокиПредварительно.Сотрудник,
	|	ПервыеСтрокиПредварительно.СпособКомпенсацииПереработки,
	|	ПервыеСтрокиПредварительно.НомерСтроки,
	|	ДанныеОСверхурочнойРаботе.ИдентификаторСтроки
	|ПОМЕСТИТЬ ВТПервыеСтроки
	|ИЗ
	|	ВТПервыеСтрокиПредварительно КАК ПервыеСтрокиПредварительно
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеОСверхурочнойРаботе КАК ДанныеОСверхурочнойРаботе
	|		ПО ПервыеСтрокиПредварительно.НомерСтроки = ДанныеОСверхурочнойРаботе.НомерСтроки
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	МИНИМУМ(НомераСтрокПоИдентификаторам.НомерСтроки - 1) КАК ИндексСтроки,
	|	ВТДубли.Сотрудник
	|ИЗ
	|	ВТДубли КАК ВТДубли
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНомераСтрокСверхурочныхПоИдентификаторам КАК НомераСтрокПоИдентификаторам
	|		ПО ВТДубли.ИдентификаторСтроки = НомераСтрокПоИдентификаторам.ИдентификаторСтроки
	|			И ВТДубли.Дата = НомераСтрокПоИдентификаторам.Дата
	|		
	|ГДЕ
	|	ВТПервыеСтроки.НомерСтроки <> ВТДубли.НомерСтроки
	|	И ВТПервыеСтроки.ИдентификаторСтроки <> ВТДубли.ИдентификаторСтроки
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТДубли.Сотрудник,
	|	ВТДубли.ИдентификаторСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТПервыеСтрокиПредварительно
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПроверитьДублиДнейСверхурочныхДокументов(МенеджерВТ, Ошибки)
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТОтработанныеДни.ИдентификаторСтроки) КАК Строк
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТОтработанныеДни.ИдентификаторСтроки) > 1
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ВТДублиСвернутыеПредварительно
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДублиСвернутыеПредварительно.Сотрудник,
	|	ДублиСвернутыеПредварительно.Дата,
	|	ДублиСвернутыеПредварительно.ПерваяСтрока,
	|	ОтработанныеДни.ИдентификаторСтроки
	|ПОМЕСТИТЬ ВТДублиСвернутые
	|ИЗ
	|	ВТДублиСвернутыеПредварительно КАК ДублиСвернутыеПредварительно
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтработанныеДни КАК ОтработанныеДни
	|		ПО ДублиСвернутыеПредварительно.ПерваяСтрока = ОтработанныеДни.НомерСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНомераСтрокСверхурочныхПоИдентификаторам КАК НомераСтрокПоИдентификаторам
	|		ПО ВТОтработанныеДни.ИдентификаторСтроки = НомераСтрокПоИдентификаторам.ИдентификаторСтроки
	|			И ВТОтработанныеДни.Дата = НомераСтрокПоИдентификаторам.Дата
	|ГДЕ
	|	ВТОтработанныеДни.НомерСтроки <> ВТДублиСвернутые.ПерваяСтрока
	|	И ВТОтработанныеДни.ИдентификаторСтроки <> ВТДублиСвернутые.ИдентификаторСтроки
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТОтработанныеДни.Сотрудник,
	|	ВТОтработанныеДни.Дата,
	|	ВТОтработанныеДни.ИдентификаторСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДублиСвернутыеПредварительно
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДублиСвернутые";
...
Процедура СоздатьВТДанныеОСверхурочнойРаботе(МенеджерВТ, ДанныеОРаботе, ПодробныйВводВремени)
	...
	Запрос.УстановитьПараметр("ПодробныйВводВремени", ПодробныйВводВремени);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА &ПодробныйВводВремени
	|			ТОГДА ДанныеОРаботе.ИдентификаторСтрокиСотрудника
	|		ИНАЧЕ ДанныеОРаботе.НомерСтроки
...
Процедура СоздатьВТНомераСтрокСверхурочныхПоИдентификаторам(МенеджерВТ)

	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеОРаботе.Дата КАК Дата,
	|	ДанныеОРаботе.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ПОМЕСТИТЬ ВТИдентификаторыСтроки
	|ИЗ
	|	ВТДанныеОСверхурочнойРаботе КАК ДанныеОРаботе
	|;
	|
	...
	|	ВТИдентификаторыСтроки.Дата КАК Дата,
	|	ВТИдентификаторыСтроки.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ПОМЕСТИТЬ ВТНомераСтрокСверхурочныхПоИдентификаторам
	|ИЗ
	|	ВТИдентификаторыСтроки КАК ВТИдентификаторыСтроки";
...
Процедура ЗаписатьПараметрыРегистрируемыхДанных(НаборЗаписейУчетаВремени, Замещение, ОписаниеРегистраДанныхУчетаВремени, Загрузка = Ложь) Экспорт
	
	ТипИсточникаДанных = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ОписаниеРегистраДанныхУчетаВремени.МетаданныеРегистра);
	
	ОтборНаборов = РегистрыБЗК.ОтборыЗаписейНабора(НаборЗаписейУчетаВремени, Замещение);
	Регистратор = Неопределено;	
	Для Каждого ОтборНабора Из ОтборНаборов Цикл
		Регистратор = ОтборНабора["Регистратор"];
	КонецЦикла;
...
Процедура КонтрольИзмененияДанныхРегистровПередЗаписью(НаборЗаписей, Замещение) Экспорт
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	НаборЗаписей.ДополнительныеСвойства.Вставить("МенеджерВременныхТаблицИзмененияРегистра", МенеджерВременныхТаблиц);

	Запрос = РегистрыБЗК.ЗапросНабораИзБазыДанных(НаборЗаписей, Замещение, , , "ВТСтарыйНабор");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Выполнить();
	
КонецПроцедуры

Процедура КонтрольИзмененияДанныхРегистровПриЗаписи(НаборЗаписей, ДанныеИзменены, Замещение) Экспорт
	
	Запрос = ЗапросКонтроляИзмененияДанныхРегистра(НаборЗаписей.Метаданные());	

	ОтборНаборов = РегистрыБЗК.ОтборыЗаписейНабора(НаборЗаписей, Замещение);
	Регистратор = Неопределено;	
	Для Каждого ОтборНабора Из ОтборНаборов Цикл
		Регистратор = ОтборНабора["Регистратор"];
	КонецЦикла;
...
Процедура СоздатьВидыИспользованияРабочегоВремениПоНастройкам(НастройкиРасчетаЗарплаты) Экспорт
	...
	Если НастройкиУчетаВремени.УчитыватьНочныеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "ПраздникиНочноеВремя";
		ОписаниеВидаВремени.Наименование = "Праздники (ночное время)";
		ОписаниеВидаВремени.БуквенныйКод = "РВН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "РВН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "РВН";
		ОписаниеВидаВремени.ЦифровойКод = "48";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Праздники (ночное время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("ПраздникиНочноеВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьНочныеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОтгулы 
		И НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "ПраздникиБезПовышеннойОплатыНочноеВремя";
		ОписаниеВидаВремени.Наименование = "Праздники без повышенной оплаты (ночное время)";
		ОписаниеВидаВремени.БуквенныйКод = "ПНН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "ПНН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "ПНН";
		ОписаниеВидаВремени.ЦифровойКод = "49";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Праздники без повышенной оплаты (ночное время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("ПраздникиБезПовышеннойОплатыНочноеВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "ПраздникиВечернееВремя";
		ОписаниеВидаВремени.Наименование = "Праздники (вечернее время)";
		ОписаниеВидаВремени.БуквенныйКод = "РВВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "РВВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "РВВ";
		ОписаниеВидаВремени.ЦифровойКод = "50";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Праздники (вечернее время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("ПраздникиВечернееВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОтгулы 
		И НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "ПраздникиБезПовышеннойОплатыВечернееВремя";
		ОписаниеВидаВремени.Наименование = "Праздники без повышенной оплаты (вечернее время)";
		ОписаниеВидаВремени.БуквенныйКод = "ПНВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "ПНВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "ПНВ";
		ОписаниеВидаВремени.ЦифровойКод = "51";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Праздники без повышенной оплаты (вечернее время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("ПраздникиБезПовышеннойОплатыВечернееВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьНочныеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "СверхурочныеНочноеВремя";
		ОписаниеВидаВремени.Наименование = "Сверхурочно (ночное время)";
		ОписаниеВидаВремени.БуквенныйКод = "СВН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "СВН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "СВН";
		ОписаниеВидаВремени.ЦифровойКод = "52";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Продолжительность сверхурочной работы (ночное время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("СверхурочныеНочноеВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьНочныеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОтгулы 
		И НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "СверхурочныеБезПовышеннойОплатыНочноеВремя";
		ОписаниеВидаВремени.Наименование = "Сверхурочные без повышенной оплаты (ночное время)";
		ОписаниеВидаВремени.БуквенныйКод = "СНН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "СНН";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "СНН";
		ОписаниеВидаВремени.ЦифровойКод = "53";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Сверхурочные без повышенной оплаты (ночное время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("СверхурочныеБезПовышеннойОплатыНочноеВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "СверхурочныеВечернееВремя";
		ОписаниеВидаВремени.Наименование = "Сверхурочно (вечернее время)";
		ОписаниеВидаВремени.БуквенныйКод = "СВВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "СВВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "СВВ";
		ОписаниеВидаВремени.ЦифровойКод = "54";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Продолжительность сверхурочной работы (вечернее время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("СверхурочныеВечернееВремя");
	КонецЕсли;
	
	Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы И НастройкиРасчетаЗарплаты.ИспользоватьОтгулы И 
		НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных Тогда
		ОписаниеВидаВремени = УчетРабочегоВремени.СоздатьОписаниеВидаВремени();
		ОписаниеВидаВремени.ИмяПредопределенныхДанных = "СверхурочныеБезПовышеннойОплатыВечернееВремя";
		ОписаниеВидаВремени.Наименование = "Сверхурочные без повышенной оплаты (вечер. время)";
		ОписаниеВидаВремени.БуквенныйКод = "СНВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный = "СНВ";
		ОписаниеВидаВремени.БуквенныйКодБюджетный2009 = "СНВ";
		ОписаниеВидаВремени.ЦифровойКод = "55";
		ОписаниеВидаВремени.ПолноеНаименование = НСтр("ru = 'Сверхурочные без повышенной оплаты (вечернее время)'");
		ОписаниеВидаВремени.РабочееВремя = Истина;
		ОписаниеВидаВремени.Целосменное = Ложь;
		...
		УчетРабочегоВремени.НовыйВидИспользованияРабочегоВремени(ОписаниеВидаВремени);
	Иначе
		УчетРабочегоВремени.ОтключитьИспользованиеПредопределенногоЭлемента("СверхурочныеБезПовышеннойОплатыВечернееВремя");
	КонецЕсли;
...
Процедура ЗаписатьДанныеВНовыеРегистры(Регистратор, ДанныеДляЗаписи)
	...
	Для Каждого ДанныеРегистра Из ДанныеДляЗаписи Цикл
		...
		ЗаписатьПараметрыРегистрируемыхДанных(Набор, Замещение, ОписаниеРегистра, Истина);
	КонецЦикла;
	ЗафиксироватьТранзакцию();
КонецПроцедуры	
...
Процедура ПриОпределенииДоступныхДляПереводаМакетов(Макеты) Экспорт
	...
	УчетРабочегоВремениБазовый.ПриОпределенииДоступныхДляПереводаМакетов(Макеты);
	
	Макеты.Добавить(Метаданные.Справочники.ГрафикиРаботыСотрудников.Макеты.ПФ_MXL_ФормаГрафикаРаботы);
	Макеты.Добавить(Метаданные.Справочники.РежимыРаботыСотрудников.Макеты.ПФ_MXL_ФормаГрафикаРаботы);
...
Процедура ЗаполнитьПоСотрудникам(Документ, Сотрудники = Неопределено, ВысотаСтроки = 2, ЭтоПерезаполнениеДанныхПоСотруднику = Ложь) Экспорт
	
	ПараметрыЗаполнения = Документ.ПараметрыЗаполнения();		
    ПараметрыЗаполнения.ВысотаСтроки = ВысотаСтроки;
	
	СтрокиПоСотрудникам = Неопределено;
	Если ЭтоПерезаполнениеДанныхПоСотруднику Тогда 
		СтрокиПоСотрудникам = Новый Соответствие;
		Для Каждого Сотрудник Из Сотрудники Цикл
			
			ОбщееКоличествоСтрок = Документ.ДанныеОВремени.Количество();
			
			Если ОбщееКоличествоСтрок = 0 Тогда 
				НомерТекущейСтроки = 1;
			Иначе	
				НомерТекущейСтроки = (ОбщееКоличествоСтрок / ПараметрыЗаполнения.ВысотаСтроки) + 1; 
			КонецЕсли;	
	
			МассивСтрокПоСотруднику = ТабельНоваяСтрокаДанныхОВремени(Документ.ДанныеОВремени, Сотрудник, НомерТекущейСтроки, ПараметрыЗаполнения.ВысотаСтроки);
	...
	Если Сотрудники = Неопределено Тогда 
		ЗаполнитьДокументПоВсемСотрудникамОрганизации(Документ, ПараметрыЗаполнения);
	Иначе	
		ПараметрыЗаполнения.Сотрудники = Сотрудники;
		ПараметрыЗаполнения.СтрокиПоСотрудникам = СтрокиПоСотрудникам;
		ЗаполнитьДокументПоСотрудникам(Документ, ПараметрыЗаполнения);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьДокументПоВсемСотрудникамОрганизации(Документ, ПараметрыЗаполнения = Неопределено) Экспорт
	
	Если ПараметрыЗаполнения = Неопределено Тогда	
		ПараметрыЗаполнения = Документ.ПараметрыЗаполнения();
	КонецЕсли;
	
	Если Не УчетРабочегоВремениРасширенныйФормы.ТабельМожноЗаполнитьАвтоматически(Документ) Тогда
		Возврат;		
	КонецЕсли;	
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если Не Документ.ИсправленныйДокумент.Пустая() Тогда
		
		СтрокиПоСотрудникам = Новый Соответствие;
		СотрудникиДокумента = Новый Массив;
		
		Для Каждого СтрокаДанныхОВремени Из Документ.ДанныеОВремени Цикл
			Если СтрокаДанныхОВремени.ЭтоПерваяСтрокаПоСотруднику Тогда 
				МассивСтрокПоСотруднику = УчетРабочегоВремениРасширенныйКлиентСервер.ТабельСтрокиПоСотруднику(Документ.ДанныеОВремени, СтрокаДанныхОВремени.НомерСтрокиСотрудник);
		...
		РассчитатьДанныеОВремени(Документ, СотрудникиДокумента, СтрокиПоСотрудникам, ПараметрыЗаполнения); 
	Иначе
		Документ.ДанныеОВремени.Очистить();
		РассчитатьДанныеОВремени(Документ, , , ПараметрыЗаполнения);
	КонецЕсли;
...
Процедура ЗаполнитьДокументПоСотрудникам(Документ, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если ПараметрыЗаполнения.Сотрудники = Неопределено Тогда	
		Возврат;		
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
		
	ТаблицаСотрудников = УчетРабочегоВремениРасширенныйФормы.ТабельПустаяТаблицаСотрудников();
	ОбщееКоличествоСтрок = Документ.ДанныеОВремени.Количество();
	...
	Если НЕ ПараметрыЗаполнения.СтрокиПоСотрудникам = Неопределено Тогда
		ДобавляемыеСотрудники = ПараметрыЗаполнения.Сотрудники;
		СтрокиПоСотрудникам = ПараметрыЗаполнения.СтрокиПоСотрудникам;
	Иначе	
		ДобавляемыеСотрудники = Новый Массив;
		СтрокиПоСотрудникам = Новый Соответствие;

		Для Каждого Сотрудник Из ПараметрыЗаполнения.Сотрудники Цикл
			Если Документ.ДанныеОВремени.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник)).Количество() = 0 Тогда
				ДобавляемыеСотрудники.Добавить(Сотрудник);
				
				СтрокиПоСотруднику = ТабельНоваяСтрокаДанныхОВремени(Документ.ДанныеОВремени, Сотрудник, НомерТекущейСтроки, ПараметрыЗаполнения.ВысотаСтроки);
	...
	КонецЕсли;	
  	...
  	РассчитатьДанныеОВремени(Документ, ДобавляемыеСотрудники, СтрокиПоСотрудникам, ПараметрыЗаполнения);

	УстановитьПривилегированныйРежим(Ложь);
		
КонецПроцедуры

Процедура ЗаполнитьДокументАльтернативнаяФорма(Документ, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если ПараметрыЗаполнения.Сотрудники = Неопределено Тогда	
		...
		Сотрудники = Документ.ДанныеОВремени.ВыгрузитьКолонку("Сотрудник");
	...
	Документ.ДанныеОВремени.Очистить();
	Документ.Смены.Очистить();
	
	ДанныеДляЗаполнения = Документы.ТабельУчетаРабочегоВремени.ТаблицаДанныхДляЗаполнения(Документ, ПараметрыЗаполнения.Сотрудники);
	
	Документ.ДанныеОВремени.Загрузить(ДанныеДляЗаполнения.ДанныеОВремени);
	Документ.Смены.Загрузить(ДанныеДляЗаполнения.Смены);

	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры

Процедура РассчитатьДанныеОВремени(Документ, СотрудникиДокумента, СтрокиПоСотрудникам, ПараметрыЗаполнения = Неопределено) Экспорт
	
	Если ПараметрыЗаполнения = Неопределено Тогда
		ПараметрыЗаполнения = Документ.ПараметрыЗаполнения();
	КонецЕсли;
	
	КодыВидовВремени = УчетРабочегоВремениРасширенный.КодыВидовВремени(Документ.ПериодРегистрации);

	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Документ.Ссылка);
	ВыборкаДанныхОВремени = МенеджерОбъекта.ВыборкаДанныхОВремени(Документ, СотрудникиДокумента);
	...
	Пока ВыборкаДанныхОВремени.СледующийПоЗначениюПоля("Сотрудник") Цикл
		...
		Если СтрокиПоСотрудникам = Неопределено Тогда
			МассивСтрокПоСотруднику = ТабельНоваяСтрокаДанныхОВремени(Документ.ДанныеОВремени, ВыборкаДанныхОВремени.Сотрудник, НомерСтрокиСотрудник, ПараметрыЗаполнения.ВысотаСтроки);
		Иначе					
		...
		ТабельЗаполнитьСтрокиПоСотруднику(
			...
			ОписаниеВидовВремени,
			МассивСтрокПоСотруднику, 
			КодыВидовВремени, 
			ПараметрыЗаполнения.ВысотаСтроки,
			ПараметрыЗаполнения.ЗаполнятьТерриторииИУсловия);
...
Функция ТабельНоваяСтрокаДанныхОВремени(ДанныеОВремени, Сотрудник, НомерСтрокиСотрудник, ВысотаСтроки) Экспорт
...
Процедура ТабельЗаполнитьСтрокиПоСотруднику(Документ, ДанныеОВремениПоСотруднику, ОписаниеВидовВремени, МассивСтрокПоСотруднику, КодыВидовВремени, ВысотаСтроки, ЗаполнятьТерриторииИУсловия = Ложь) Экспорт
	...
	Для Каждого СтрокаДанныхОВремени Из ДанныеОВремениПоСотруднику Цикл
		...
		Если НомерВидаВремениНаДату > МассивСтрокПоСотруднику.Количество() Тогда
			СтрокаТабеля = УчетРабочегоВремениРасширенныйКлиентСервер.ТабельНоваяСтрокаПоСотруднику(Документ.ДанныеОВремени, МассивСтрокПоСотруднику[0]);	
	...
	ОбозначенияВидовВремени = УчетРабочегоВремениРасширенныйКлиентСервер.ТабельОбозначенияВидовВремени(ОписаниеВидовВремени.ОписаниеВидовВремени);
