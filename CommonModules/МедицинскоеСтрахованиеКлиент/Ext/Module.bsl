	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Иначе
			ТекущиеДанные.ДокументПредставление = ""
				...
				+ ?(ЗначениеЗаполнено(ТекущиеДанные.ДокументСерия), СтрШаблон(ТекстСерия, ТекущиеДанные.ДокументСерия), "")
				+ ?(ЗначениеЗаполнено(ТекущиеДанные.ДокументНомер), СтрШаблон(ТекстНомер, ТекущиеДанные.ДокументНомер), "")
				+ ?(ЗначениеЗаполнено(ТекущиеДанные.ДокументДатаВыдачи),
					СтрШаблон(ТекстДатаВыдачи, Формат(Дата(ТекущиеДанные.ДокументДатаВыдачи),"ДЛФ=ДД")), "")
				+ ?(ЗначениеЗаполнено(ТекущиеДанные.ДокументСрокДействия),
					СтрШаблон(ТекстСрокДействия, Формат(Дата(ТекущиеДанные.ДокументСрокДействия),"ДЛФ=ДД")), "")
				+ ?(ЗначениеЗаполнено(ТекущиеДанные.ДокументКемВыдан), ", " + ТекущиеДанные.ДокументКемВыдан, "")
				+ ?(ЗначениеЗаполнено(ТекущиеДанные.ДокументКодПодразделения)
					И ТекущиеДанные.ДокументВид = ПредопределенноеЗначение("Справочник.ВидыДокументовФизическихЛиц.ПаспортРФ"),
						СтрШаблон(ТекстКодПодразделения, ТекущиеДанные.ДокументКодПодразделения), "");
...
Процедура ШаблонРассылкаПолисовДМСНажатие() Экспорт

	ШаблонРассылкиПолисовДМС = МедицинскоеСтрахование.ШаблонРассылкаПолисовДМССуществует();

	Если ЗначениеЗаполнено(ШаблонРассылкиПолисовДМС) Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Ключ", ШаблонРассылкиПолисовДМС);
		ОткрытьФорму("Справочник.ШаблоныСообщений.ФормаОбъекта", ПараметрыФормы);
		
	Иначе	
		
		ТекстВопроса = НСтр("ru = 'Не удалось найти шаблон электронного письма для отправки полисов ДМС. 
						|Создать и открыть шаблон сейчас?'");
		
		ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
		ПараметрыВопроса.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		ПараметрыВопроса.Заголовок = НСтр("ru = 'Создание шаблона отправки полисов ДМС'");
		ПараметрыВопроса.Картинка = БиблиотекаКартинок.Предупреждение32;
		
		КнопкиОтвета = Новый СписокЗначений;
		КнопкиОтвета.Добавить(Истина, НСтр("ru = 'Создать'"));
		КнопкиОтвета.Добавить(Ложь, НСтр("ru = 'Отмена'"));
		
		ОбработчикОтвета = Новый ОписаниеОповещения("ЗавершитьНастройкуШаблонаРассылкиДМС", ЭтотОбъект);
...
Процедура ЗавершитьНастройкуШаблонаРассылкиДМС(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено И Результат.Значение = Истина Тогда
		
		ШаблонРассылкиПолисовДМС = МедицинскоеСтрахование.СформироватьШаблонОтправкиПолисовДМС();
