...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ДобавитьСторноДанныеДляРасчетаСреднегоЗаработкаФССПоНачислениям");
	Методы.Вставить("ЗаполнитьДанныеДляРасчетаСреднегоЗаработкаФССПоНачислениям");
	Методы.Вставить("УдалитьТехноЗаписиДанныеДляРасчетаСреднегоЗаработкаФССПоНачислениям");
КонецПроцедуры
	...
	Для Каждого СтрокаТаблицы Из ДатыПолученияДоходов Цикл
		...
		Если ДобавитьСтроку Тогда
			...
			НоваяСтрока.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 получено пособие'"), Формат(СтрокаТаблицы.Дата, "ДЛФ=Д"));
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.35.38";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор   = Новый УникальныйИдентификатор("b433cc97-c5ae-4f50-afc6-7ccea93a1124");
	Обработчик.Процедура       = "Документы.БольничныйЛист.ЗаполнитьДниНетрудоспособностиПериодыУходаЗаБольнымиРодственниками";
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение дней нетрудоспособности регистра ""Периоды ухода за больными родственниками"".'");
...
Функция ДанныеДляРасчетаСреднегоЗаработкаФСССотрудниковСлужебный(ИсходныеДанные, ДополнительныеПараметры)
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДниУходаЗаДетьми.ДнейБолезниУходаЗаДетьми, 0) > ЕСТЬNULL(ДанныеУчетаВремени.ДнейБолезниУходаЗаДетьми, 0)
		|			ТОГДА ДниУходаЗаДетьми.ДнейБолезниУходаЗаДетьми
		|		ИНАЧЕ ЕСТЬNULL(ДанныеУчетаВремени.ДнейБолезниУходаЗаДетьми, 0)
		|	КОНЕЦ КАК ДнейБолезниУходаЗаДетьми,
...
Процедура ДобавитьСреднийПоДаннымУчетаФонда(Результат, ФизическоеЛицо, ГоловнаяОрганизация, НомерЛН, РасчетныеГоды)
	...
	|	ЕСТЬNULL(СреднийЗаработокСЭДО.ИдентификаторСообщения, ДанныеСФРДляРасчетаПособия.ИдентификаторСообщения) КАК ИдентификаторСообщения
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СреднийЗаработокСЭДО КАК СреднийЗаработокСЭДО
	|		ПО СреднийЗаработокОбщийСЭДО.Регистратор = СреднийЗаработокСЭДО.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ДанныеСФРДляРасчетаПособия КАК ДанныеСФРДляРасчетаПособия
	|		ПО СреднийЗаработокОбщийСЭДО.Регистратор = ДанныеСФРДляРасчетаПособия.Ссылка
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ДанныеСФРДляРасчетаПособия КАК ДанныеСФРДляРасчетаПособия
	|		ПО СреднийЗаработокСтрахователейСЭДО.Регистратор = ДанныеСФРДляРасчетаПособия.Ссылка
	|ГДЕ
