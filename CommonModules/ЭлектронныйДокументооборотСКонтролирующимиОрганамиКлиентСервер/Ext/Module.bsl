...
Функция ОбновитьКоличествоНовых(
		ФормаИлиДерево,
		МинимальноеЗначениеДляВыводаКоличества = 1,
		НовыеСобытияПоСтраницамЖурнала = Ложь,
		УчитыватьНепоказанныеИОтборы = Ложь) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("КоличествоНовых", 0);
	Результат.Вставить("КоличествоНепрочитанных", 0);
	Результат.Вставить("КоличествоНепрочитанныхВсе", 0);
	Результат.Вставить("КоличествоВсе", 0);
	Результат.Вставить("КоличествоПолученныеСообщения", 0);
	Результат.Вставить("КоличествоОбработанныеЗапросы", 0);
	Результат.Вставить("КоличествоЗавершенныеОтправки", 0);
	Результат.Вставить("КоличествоНепрочитанныхТребования", 0);
	Результат.Вставить("КоличествоТребования", 0);
	Результат.Вставить("КоличествоНепрочитанныхОтчеты", 0);
	Результат.Вставить("КоличествоОтчеты", 0);
	Результат.Вставить("КоличествоНепрочитанныхУведомления", 0);
	Результат.Вставить("КоличествоУведомления", 0);
	Результат.Вставить("КоличествоНепрочитанныхПисьма", 0);
	Результат.Вставить("КоличествоПисьма", 0);
	Результат.Вставить("КоличествоНепрочитанныхСверки", 0);
	Результат.Вставить("КоличествоСверки", 0);
	Результат.Вставить("КоличествоНепрочитанныхВходящие", 0);
	Результат.Вставить("КоличествоВходящие", 0);
	Результат.Вставить("КоличествоНепрочитанныхВыплатыИПособия", 0);
	Результат.Вставить("КоличествоВыплатыИПособия", 0);
	Результат.Вставить("КоличествоНепрочитанныхРассылки", 0);
	Результат.Вставить("КоличествоРассылки", 0);
	Результат.Вставить("КоличествоПрочтенные", 0);
	...
	СтрокаБлокТребования = Неопределено;
	СтрокаБлокОтчеты = Неопределено;
	СтрокаБлокУведомления = Неопределено;
	СтрокаБлокПисьма = Неопределено;
	СтрокаБлокСверки = Неопределено;
	СтрокаБлокВходящие = Неопределено;
	СтрокаБлокВыплатыИПособия = Неопределено;
	СтрокаБлокРассылки = Неопределено;
	СтрокаБлокПрочтенные = Неопределено;
	
	СтрокиДереваНепоказанные = Неопределено;
	ВключатьПросмотренные = Ложь;
	Если ТипЗнч(ФормаИлиДерево) = Тип("ФормаКлиентскогоПриложения") Тогда
		СтрокиДерева = ФормаИлиДерево.Новое.ПолучитьЭлементы();
		Если УчитыватьНепоказанныеИОтборы Тогда
			СтрокиДереваНепоказанные = ФормаИлиДерево.НовоеНепоказанные.ПолучитьЭлементы();
			ВключатьПросмотренные = (ФормаИлиДерево.ОтборВидСообщения = Истина);
		КонецЕсли;
	ИначеЕсли ТипЗнч(ФормаИлиДерево) = Тип("ДанныеФормыДерево") Тогда
		СтрокиДерева = ФормаИлиДерево.ПолучитьЭлементы(); // только подсчет общего количества новых
	Иначе
		СтрокиДерева = ФормаИлиДерево.Строки; // только подсчет общего количества новых
	КонецЕсли;
	...
	МассивДеревьев = Новый Массив;
	МассивДеревьев.Добавить(СтрокиДерева);
	Если СтрокиДереваНепоказанные <> Неопределено Тогда
		МассивДеревьев.Добавить(СтрокиДереваНепоказанные);
	КонецЕсли;
	
	Для каждого СтрокиДереваМассива Из МассивДеревьев Цикл
		Для каждого СтрокаДерева Из СтрокиДереваМассива Цикл
			Если СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ПолученныеСообщения") Тогда
				...
				ИначеЕсли СтрокаДерева.НеПрочитано Тогда
					Результат.КоличествоПолученныеСообщения = Результат.КоличествоПолученныеСообщения + 1;
					Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
				КонецЕсли;
			...
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ОбработанныеЗапросы") Тогда
				...
				ИначеЕсли СтрокаДерева.НеПрочитано Тогда
					Результат.КоличествоОбработанныеЗапросы = Результат.КоличествоОбработанныеЗапросы + 1;
				КонецЕсли;
			...
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.ГруппыНовыхСобытийДокументооборотаСКонтролирующимиОрганами.ЗавершенныеОтправки") Тогда
				...
				ИначеЕсли СтрокаДерева.НеПрочитано Тогда
					Результат.КоличествоЗавершенныеОтправки = Результат.КоличествоЗавершенныеОтправки + 1;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.ВидыДокументовВЖурнале.ТребованиеИлиУведомление") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокТребования = СтрокаДерева;
				Иначе
					Результат.КоличествоТребования = Результат.КоличествоТребования + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхТребования = Результат.КоличествоНепрочитанныхТребования + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Отчеты") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокОтчеты = СтрокаДерева;
				Иначе
					Результат.КоличествоОтчеты = Результат.КоличествоОтчеты + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхОтчеты = Результат.КоличествоНепрочитанныхОтчеты + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Уведомления") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокУведомления = СтрокаДерева;
				Иначе
					Результат.КоличествоУведомления = Результат.КоличествоУведомления + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхУведомления = Результат.КоличествоНепрочитанныхУведомления + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Письма") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокПисьма = СтрокаДерева;
				Иначе
					Результат.КоличествоПисьма = Результат.КоличествоПисьма + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхПисьма = Результат.КоличествоНепрочитанныхПисьма + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Сверки") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокСверки = СтрокаДерева;
				Иначе
					Результат.КоличествоСверки = Результат.КоличествоСверки + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхСверки = Результат.КоличествоНепрочитанныхСверки + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Входящие") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокВходящие = СтрокаДерева;
				Иначе
					Результат.КоличествоВходящие = Результат.КоличествоВходящие + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхВходящие = Результат.КоличествоНепрочитанныхВходящие + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.ВыплатыИПособия") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокВыплатыИПособия = СтрокаДерева;
				Иначе
					Результат.КоличествоВыплатыИПособия = Результат.КоличествоВыплатыИПособия + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхВыплатыИПособия = Результат.КоличествоНепрочитанныхВыплатыИПособия + 1;
						Результат.КоличествоНепрочитанных = Результат.КоличествоНепрочитанных + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = ПредопределенноеЗначение("Перечисление.ВидыДокументовВЖурнале.Рассылка") Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокРассылки = СтрокаДерева;
				Иначе
					Результат.КоличествоРассылки = Результат.КоличествоРассылки + 1;
					Если СтрокаДерева.НеПрочитано Тогда
						Результат.КоличествоНепрочитанныхРассылки = Результат.КоличествоНепрочитанныхРассылки + 1;
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли СтрокаДерева.Группа = "Прочтенные" Тогда
				Если СтрокаДерева.ЭтоЗаголовокБлока Тогда
					СтрокаБлокПрочтенные = СтрокаДерева;
				Иначе
					Результат.КоличествоПрочтенные = Результат.КоличествоПрочтенные + 1;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Результат.КоличествоНовых = Результат.КоличествоПолученныеСообщения + Результат.КоличествоОбработанныеЗапросы
		+ Результат.КоличествоЗавершенныеОтправки + Результат.КоличествоНепрочитанныхТребования
		+ Результат.КоличествоНепрочитанныхВходящие + Результат.КоличествоНепрочитанныхРассылки;
	Результат.КоличествоНепрочитанныхВсе = Результат.КоличествоНепрочитанныхТребования
		+ Результат.КоличествоНепрочитанныхОтчеты + Результат.КоличествоНепрочитанныхУведомления
		+ Результат.КоличествоНепрочитанныхПисьма + Результат.КоличествоНепрочитанныхСверки
		+ Результат.КоличествоНепрочитанныхВходящие + Результат.КоличествоНепрочитанныхВыплатыИПособия;
	Результат.КоличествоВсе = Результат.КоличествоТребования + Результат.КоличествоОтчеты
		+ Результат.КоличествоУведомления + Результат.КоличествоПисьма + Результат.КоличествоСверки
		+ Результат.КоличествоВходящие + Результат.КоличествоВыплатыИПособия;
	
	Если ТипЗнч(ФормаИлиДерево) = Тип("ФормаКлиентскогоПриложения") Тогда
		Если СтрокаБлокПолученныеСообщения <> Неопределено Тогда
			Если Результат.КоличествоПолученныеСообщения < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокПолученныеСообщения.ЗаголовокБлока = НСтр("ru = 'Полученные сообщения'");
			Иначе
				СтрокаБлокПолученныеСообщения.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Полученные сообщения (%1)'"), Результат.КоличествоПолученныеСообщения);
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокОбработанныеЗапросы <> Неопределено Тогда
			Если Результат.КоличествоОбработанныеЗапросы < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокОбработанныеЗапросы.ЗаголовокБлока = ?(НовыеСобытияПоСтраницамЖурнала, НСтр("ru = 'Обработанные'"),
					НСтр("ru = 'Обработанные запросы'"));
			Иначе
				СтрокаБлокОбработанныеЗапросы.ЗаголовокБлока = СтрШаблон(
					?(НовыеСобытияПоСтраницамЖурнала, НСтр("ru = 'Обработанные (%1)'"), НСтр("ru = 'Обработанные запросы (%1)'")),
					Результат.КоличествоОбработанныеЗапросы);
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокЗавершенныеОтправки <> Неопределено Тогда
			Если Результат.КоличествоЗавершенныеОтправки < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокЗавершенныеОтправки.ЗаголовокБлока = ?(НовыеСобытияПоСтраницамЖурнала, НСтр("ru = 'Завершенные'"),
					НСтр("ru = 'Завершенные отправки'"));
			Иначе
				СтрокаБлокЗавершенныеОтправки.ЗаголовокБлока = СтрШаблон(
					?(НовыеСобытияПоСтраницамЖурнала, НСтр("ru = 'Завершенные (%1)'"), НСтр("ru = 'Завершенные отправки (%1)'")),
					Результат.КоличествоЗавершенныеОтправки);
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокТребования <> Неопределено Тогда
			Если Результат.КоличествоТребования < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокТребования.ЗаголовокБлока = НСтр("ru = 'Требования'");
			Иначе
				СтрокаБлокТребования.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Требования (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоТребования, Результат.КоличествоНепрочитанныхТребования));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокОтчеты <> Неопределено Тогда
			Если Результат.КоличествоОтчеты < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокОтчеты.ЗаголовокБлока = НСтр("ru = 'Отчеты'");
			Иначе
				СтрокаБлокОтчеты.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Отчеты (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоОтчеты, Результат.КоличествоНепрочитанныхОтчеты));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокУведомления <> Неопределено Тогда
			Если Результат.КоличествоУведомления < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокУведомления.ЗаголовокБлока = НСтр("ru = 'Уведомления'");
			Иначе
				СтрокаБлокУведомления.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Уведомления (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоУведомления, Результат.КоличествоНепрочитанныхУведомления));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокПисьма <> Неопределено Тогда
			Если Результат.КоличествоПисьма < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокПисьма.ЗаголовокБлока = НСтр("ru = 'Письма'");
			Иначе
				СтрокаБлокПисьма.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Письма (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоПисьма, Результат.КоличествоНепрочитанныхПисьма));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокСверки <> Неопределено Тогда
			Если Результат.КоличествоСверки < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокСверки.ЗаголовокБлока = НСтр("ru = 'Сверки'");
			Иначе
				СтрокаБлокСверки.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Сверки (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоСверки, Результат.КоличествоНепрочитанныхСверки));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокВходящие <> Неопределено Тогда
			Если Результат.КоличествоВходящие < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокВходящие.ЗаголовокБлока = НСтр("ru = 'Входящие'");
			Иначе
				СтрокаБлокВходящие.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Входящие (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоВходящие, Результат.КоличествоНепрочитанныхВходящие));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокВыплатыИПособия <> Неопределено Тогда
			Если Результат.КоличествоВыплатыИПособия < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокВыплатыИПособия.ЗаголовокБлока = НСтр("ru = 'Выплаты и пособия'");
			Иначе
				СтрокаБлокВыплатыИПособия.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Выплаты и пособия (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоВыплатыИПособия, Результат.КоличествоНепрочитанныхВыплатыИПособия));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокРассылки <> Неопределено Тогда
			Если Результат.КоличествоРассылки < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокРассылки.ЗаголовокБлока = НСтр("ru = 'Рассылки'");
			Иначе
				СтрокаБлокРассылки.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Рассылки (%1)'"),
					?(ВключатьПросмотренные, Результат.КоличествоРассылки, Результат.КоличествоНепрочитанныхРассылки));
			КонецЕсли;
		КонецЕсли;
		
		Если СтрокаБлокПрочтенные <> Неопределено Тогда
			Если Результат.КоличествоПрочтенные < МинимальноеЗначениеДляВыводаКоличества Тогда
				СтрокаБлокПрочтенные.ЗаголовокБлока = НСтр("ru = 'Прочтенные'");
			Иначе
				СтрокаБлокПрочтенные.ЗаголовокБлока = СтрШаблон(
					НСтр("ru = 'Прочтенные (%1)'"), Результат.КоличествоПрочтенные);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция Облачная1СОИспользоватьФормуНовыхСобытий() Экспорт
...
Функция ПараметрыПоказаСообщенийТребующихРасшифровки() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПоказДляТекущегоПользователя", Истина);
	Результат.Вставить("ПоказДляДругихПользователей", Истина);
	Результат.Вставить("НикогдаНеПоказыватьДляДругихПользователей", Ложь);
	Результат.Вставить("Пользователи", Новый Массив);
	Результат.Вставить("СвойстваПользователей", Новый Массив);
	Результат.Вставить("ВсеСообщения", Ложь);
	Результат.Вставить("ДниОжидания", 0);
	Результат.Вставить("СообщенийПоКатегориям", Новый Соответствие);
	Результат.Вставить("СообщенийСТребованиямиУведомлениями", 0);
