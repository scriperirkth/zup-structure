...
//  ПроверяемыеИдентификатор - Строка
Процедура ПроверитьУникальностьФизическогоЛица(Форма, ПроверяемыеИдентификатор) Экспорт
	...
	ПараметрыПроверки.ИНН = ?(ПроверяемыеИдентификатор = "ИНН", Форма.ФизическоеЛицо.ИНН, "");
	ПараметрыПроверки.СтраховойНомерПФР = ?(ПроверяемыеИдентификатор = "СтраховойНомерПФР",
		Форма.ФизическоеЛицо.СтраховойНомерПФР, "");
	ПараметрыПроверки.ДокументВид = ?(ПроверяемыеИдентификатор = "Документ", Форма.ДокументыФизическихЛиц.ВидДокумента,
			ПредопределенноеЗначение("Справочник.ВидыДокументовФизическихЛиц.ПустаяСсылка"));
	ПараметрыПроверки.ДокументСерия = ?(ПроверяемыеИдентификатор = "Документ", Форма.ДокументыФизическихЛиц.Серия, "");
	ПараметрыПроверки.ДокументНомер = ?(ПроверяемыеИдентификатор = "Документ", Форма.ДокументыФизическихЛиц.Номер, "");
	
	РезультатыПроверки = СотрудникиВызовСервераРасширенный.РезультатПроверкиУникальностиФизическогоЛица(ПараметрыПроверки);
	
	Если РезультатыПроверки.ФизическоеЛицоУникально Тогда
		Возврат;
	КонецЕсли;
	...
	Если РезультатыПроверки.ДанныеФизическихЛиц.Количество() > 0 И ВедущийОбъект.Пустая() Тогда
		ОписаниеПредметовПроверки(РезультатыПроверки, Форма);
		ПоказатьПохожихФизическихЛиц(РезультатыПроверки, Форма, ТипЗнч(ВедущийОбъект) = Тип("СправочникСсылка.Сотрудники"));
	Иначе
...
// Параметры:
//  РезультатыПроверки - см. СотрудникиВызовСервераРасширенный.РезультатПроверкиУникальностиФизическогоЛица
//  Форма - ФормаКлиентскогоПриложения
//  ВызовИзФормыСотрудника - Булево
Процедура ПоказатьПохожихФизическихЛиц(РезультатыПроверки, Форма, ВызовИзФормыСотрудника)
	
	ПараметрыОткрытия = СотрудникиКлиент.ПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(РезультатыПроверки);
	ПараметрыОткрытия.ТекстИнформационнойНадписи = ОписаниеПредметовПроверки(РезультатыПроверки, Форма) + "."
	...
	Оповещение = Новый ОписаниеОповещения("ПроверитьУникальностьФизическогоЛицаЗавершение", ЭтотОбъект,
...
//  Строка
Функция ОписаниеПредметовПроверки(РезультатыПроверки, Форма)
	...
	Иначе
		Для каждого СообщениеПроверки Из РезультатыПроверки.СообщенияПроверки Цикл
			Если СообщениеПроверки.ИмяПоля = "ИНН" Тогда
				ОписаниеПредметовПроверки =
					?(ПустаяСтрока(ОписаниеПредметовПроверки), "", ОписаниеПредметовПроверки + ", ") 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='ИНН (%1)'"),
						Форма.ФизическоеЛицо.ИНН);
			ИначеЕсли СообщениеПроверки.ИмяПоля = "СтраховойНомерПФР" Тогда
				ОписаниеПредметовПроверки =
					?(ПустаяСтрока(ОписаниеПредметовПроверки), "", ОписаниеПредметовПроверки + ", ")
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='СНИЛС (%1)'"),
						Форма.ФизическоеЛицо.СтраховойНомерПФР);
			Иначе
				ОписаниеПредметовПроверки =
					?(ПустаяСтрока(ОписаниеПредметовПроверки), "", ОписаниеПредметовПроверки + ", ")
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='документом, удостоверяющим личность'"), Форма.ДокументыФизическихЛиц.ВидДокумента,
	...
	Возврат ОписаниеПредметовПроверки;
