...
Функция ПолучитьПолноеИмяВременногоФайла(КаталогВремФайлов, ИмяФайла, УдалятьЕслиУжеЕсть = Истина) Экспорт
	...
	Иначе
		ПроверитьПолноеИмяВременногоФайла(КаталогВремФайлов,
			"КаталогВремФайлов", "ПолучитьПолноеИмяВременногоФайла");
	КонецЕсли;
	...
	Попытка
		// Делается проверка на то, что был передан каталог временных файлов.
...
Функция СформироватьСтруктуруПараметровФайлаВыгрузки(ФайлВыгрузки) Экспорт
	...
	ПроверитьПолноеИмяВременногоФайла(ФайлВыгрузки,
		"ФайлВыгрузки", "СформироватьСтруктуруПараметровФайлаВыгрузки");
...
Функция ОперацииСРегламентированнымОтчетом(ИмяОтчета, ИмяФормы) Экспорт
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3_НДФЛ.ФормаОтчета2025Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет3_НДФЛ.ФормаОтчета2026Кв3"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
...
Функция СтатусыОбъектовРеглОтчетностиПриРучномВводе(ОбъектРеглОтчетности = Неопределено, ВидКонтролирующегоОргана = Неопределено) Экспорт
	...
	Если ВидКонтролирующегоОргана = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") ИЛИ
		...
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма2" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма2д" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма3д" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма4" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма5" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма6" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма7" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма9" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма11" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма13" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма14" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма16" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма19д" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма20д" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма21" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма23" Тогда
...
// Возвращает параметры функциональности, переопределяемые в конфигурациях-потребителях.
// Используется в общих механизмах библиотеки. Для управления функциональностью внутри подсистем
// рекомендуется использовать аналогичные функции. Допускается группировка значений во вложенной структуре.
// Содержит в себе:
//  - двоичные признаки, определяющие доступность функциональности;
//  - строки, например, имена форм, адреса служб;
//  - числовые значения;
//  - даты.
//
// Возвращаемое значение:
//   Структура:
//     * ИмяФормыПредложенияОформитьЗаявлениеНаПодключение - Строка - путь к форме предложения оформить заявление на подключение
//     * ИмяФормыПредложенияОформитьЗаявлениеНаПодключениеПоКнопкеЗаполнить - Строка - путь к форме предложения оформить заявление по кнопке "Заполнить" формы рег.отчета
//     * ОтображатьЛичныеКабинетыОператоровСвязиНаФормеОтчетности - Булево - признак отображения гиперссылок на личные кабинеты операторов связи на форме 1С-Отчетности
//
Функция ПараметрыФункциональности() Экспорт
	
	ПараметрыФункциональности = Новый Структура;
	ПараметрыФункциональности.Вставить("ИмяФормыПредложенияОформитьЗаявлениеНаПодключение",
		"Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ПредложениеОформитьЗаявлениеНаПодключение");
	ПараметрыФункциональности.Вставить("ИмяФормыПредложенияОформитьЗаявлениеНаПодключениеПоКнопкеЗаполнить",
		"Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ПредложениеОформитьЗаявлениеНаПодключение");
	ПараметрыФункциональности.Вставить("ОтображатьЛичныеКабинетыОператоровСвязиНаФормеОтчетности", Истина);
	ПараметрыФункциональности.Вставить("ИмяРегистраЖурналаНовыхСобытийСЭДО", "ЖурналНовыхСобытий");
	
	РегламентированнаяОтчетностьКлиентСерверПереопределяемый.ПриОпределенииПараметровФункциональности(
		ПараметрыФункциональности);
	
	Возврат ПараметрыФункциональности;
...
//
// Возвращаемое значение:
//   Строка - исходное имя файла, если проверка прошла успешно
//
Функция ПроверитьИмяФайла(Знач ИмяФайла) Экспорт
	
	Если ТипЗнч(ИмяФайла) <> Тип("Строка") Тогда
		ТекстОшибки = НСтр("ru = 'Имя файла не указано или имеет неверный тип.'");
	...
	Если НедопустимыеСимволы.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Обнаружены недопустимые символы в имени файла %1.'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки, ИмяФайла);
		ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
...
// Проверяет, является ли переданный путь именем временного файла.
//
// Параметры:
//   Путь - Строка, Неопределено - путь к файлу, полученный методом ПолучитьИмяВременногоФайла()
//
// Возвращаемое значение:
//   Булево - Истина, если путь указывает на временный файл
Функция ЭтоИмяВременногоФайла(Знач Путь) Экспорт
...
// Проверяет, что указанный путь является путем к временному файлу. Вызывает исключение, если путь не является временным.
//
// Параметры:
//   ИмяФайла - Строка - проверяемый путь к файлу
//   ИмяПараметра - Строка - имя проверяемого параметра (для сообщения об ошибке)
//   ИмяМетода - Строка - имя проверяемого метода (для сообщения об ошибке)
//
Процедура ПроверитьПолноеИмяВременногоФайла(Знач ИмяФайла, ИмяПараметра, ИмяМетода) Экспорт
	...
	ЕстьПуть = СтрНайти(ИмяФайла, Разделитель) > 0
		Или СтрНайти(ИмяФайла, "/") > 0
		Или СтрНайти(ИмяФайла, "\") > 0;
	Если НЕ ЕстьПуть Тогда
		// Была передана строка без разделителей, проверяем как имя файла.
		ПроверитьИмяФайла(ИмяФайла);
		Возврат;
	КонецЕсли;
		
	Если НЕ ЭтоИмяВременногоФайла(ИмяФайла) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
