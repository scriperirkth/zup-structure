...
//      ** ПриоритетОтображения - Число - приоритет значения (чем выше, тем чаще встречается);
//      Свойство может не передаваться.
...
Функция НайтиФИО(ЧастиФИО, ПолноеСовпадение = Истина) Экспорт
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПриоритетОтображения УБЫВ";
	
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ШаблонСоединения",
		?(ПолноеСовпадение,
			"КлассификаторФИО.Значение = ВТ_ЧастиФИО.Значение",
			"КлассификаторФИО.Значение ПОДОБНО ВТ_ЧастиФИО.Значение"));
