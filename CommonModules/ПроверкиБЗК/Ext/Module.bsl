...
//   Обязательный                    - Булево - Признак обязательности заполнения реквизита.
//
Функция ПроверитьПериод(Отказ, Объект, ИмяДатыНачала, ИмяДатыОкончания, ПредставлениеВРодительномПадеже, Обязательный = Истина) Экспорт
...
//   ИмяРеквизита - Строка                   - Имя проверяемого реквизита объекта.
//
Процедура ПроверитьСНИЛС(Отказ, Объект, ИмяРеквизита, Обязательный = Истина) Экспорт
	СНИЛС = Объект[ИмяРеквизита];
	Если Не ЗначениеЗаполнено(СНИЛС) Тогда
		Если СНИЛС <> "" Или Обязательный Тогда
...
Функция ПредставлениеРеквизитаТаблицыОбъекта(Объект, ОбъектМетаданных, ИмяТаблицы, ИмяРеквизита) Экспорт
	Если ОбъектМетаданных = Неопределено Тогда
		ОбъектМетаданных = ОбъектМетаданных(Объект);
	КонецЕсли;
	Если ОбъектМетаданных = Неопределено Тогда
...
Процедура СообщитьОбОшибкеЗаполненияРеквизитаОбъекта(Отказ, Объект, ИмяРеквизита, Знач Текст = "", ОбъектМетаданных = Неопределено) Экспорт
	Если Текст = "" Тогда
		Текст = СтрШаблон(
