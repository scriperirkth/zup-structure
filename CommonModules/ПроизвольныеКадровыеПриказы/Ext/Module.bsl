...
Процедура ПриДобавленииКомандПечати(ИмяФормы, КомандыПечати) Экспорт
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		Если Не КомандаПечати.ДополнительныеПараметры.Свойство("Ссылка") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СоздатьДокументыБИА(СсылкаНаОбъектИсточник, ДокументыБИА) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ИнтеграцияС1САрхивомЗарплатаКадры") Тогда
		...
		Если ТипЗнч(СсылкаНаОбъектИсточник) = Тип("ДокументСсылка.ПроизвольныйКадровыйПриказ") Тогда
			
			МодульИнтеграцияС1САрхивом = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияС1САрхивом");
			
			ПараметрыДокумента = МодульИнтеграцияС1САрхивом.ПараметрыДокумента();
			ПараметрыДокумента.Списочный = Ложь;
			ПараметрыДокумента.ДополнительныеСвойства.Вставить("Краткий состав документа", "КраткийСоставДокумента");
			ПараметрыДокумента.ДополнительныеСвойства.Вставить("Подразделение", "Подразделение");
			
			ДокументыБИА = МодульИнтеграцияС1САрхивом.СоздатьДокументыБИАПоВладельцуФайлов(СсылкаНаОбъектИсточник, ПараметрыДокумента);
