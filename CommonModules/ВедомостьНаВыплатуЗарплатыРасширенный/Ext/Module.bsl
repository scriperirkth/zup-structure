...
Процедура СоздатьВТСотрудникиДляВедомостиПоОснованиям(МенеджерВременныхТаблиц, ОписаниеОперации, ИмяВТСотрудники = "") Экспорт
	...
	Если ОписаниеОперации.ДокументыОснования.Количество() > 0 Тогда
		...
		Если ОписаниеОперации.СпособПолучения = Перечисления.СпособыПолученияЗарплатыКВыплате.Аванс Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗарплатаКВыплате", "ЗарплатаКВыплатеАвансом");
		КонецЕсли;
...
Процедура СоздатьВТУсловияВозвратаЗадолженности(МенеджерВременныхТаблиц) Экспорт
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра("ВТСотрудникиДляВедомости", "Период, Сотрудник");
	ЗарплатаКадрыОбщиеНаборыДанных.СоздатьВТИмяРегистраСрезПоследних("УсловияВозвратаЗадолженности", Запрос.МенеджерВременныхТаблиц, Истина, ОписаниеФильтра);
	
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УсловияВозвратаЗадолженностиСрезПоследних.Сотрудник КАК Сотрудник,
		|	УсловияВозвратаЗадолженностиСрезПоследних.ДокументОснование КАК ДокументОснование
		|ПОМЕСТИТЬ ВТУсловияВозвратаЗадолженности
		|ИЗ
		|	ВТУсловияВозвратаЗадолженностиСрезПоследних КАК УсловияВозвратаЗадолженностиСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТУсловияВозвратаЗадолженностиСрезПоследних";
...
Процедура ПрименитьОграниченияПогашенийЗадолженностей(Зарплата, ИгнорируемыеРегистраторы, ПериодРегистрации) Экспорт
	...
	ЗадолженностьПоОснованиям = Зарплата.Скопировать(,"ПериодВзаиморасчетов, Сотрудник, ДокументОснование,КВыплате");
	ЗадолженностьПоОснованиям.Свернуть("ПериодВзаиморасчетов, Сотрудник, ДокументОснование", "КВыплате");
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ЗадолженностьПоОснованиям", ЗадолженностьПоОснованиям);
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТЗадолженностьПоОснованиям
		|ИЗ
		|	&ЗадолженностьПоОснованиям КАК ЗарплатаКВыплате
		|ГДЕ
		|	ЗарплатаКВыплате.КВыплате < 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЗарплатаКВыплате.ДокументОснование КАК ДокументОснование,
		|	ЗарплатаКВыплате.КВыплате КАК КВыплате
		|ПОМЕСТИТЬ ВТКВыплатеПоОснованиям
		|ИЗ
		|	&ЗадолженностьПоОснованиям КАК ЗарплатаКВыплате
		|ГДЕ
		|	ЗарплатаКВыплате.КВыплате > 0";
	...
	Запрос.Выполнить();
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра("ВТЗадолженностьПоОснованиям", "Период, ДокументОснование, Сотрудник");
	ЗарплатаКадрыОбщиеНаборыДанных.СоздатьВТИмяРегистраСрезПоследних("УсловияВозвратаЗадолженности", Запрос.МенеджерВременныхТаблиц, Истина, ОписаниеФильтра);
	
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗадолженностьПоОснованиям.Сотрудник КАК Сотрудник,
		|	ЗадолженностьПоОснованиям.Период КАК Период
		|ПОМЕСТИТЬ ВТСотрудникиСЗадолженностью
		|ИЗ
		|	ВТЗадолженностьПоОснованиям КАК ЗадолженностьПоОснованиям
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Выплачено.Сотрудник КАК Сотрудник,
		|	Выплачено.ДокументОснование КАК ДокументОснование,
		|	Выплачено.Период КАК Период,
		|	Выплачено.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
		|	СУММА(ВЫБОР
		|			КОГДА Выплачено.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|					И НЕ КВыплатеПоОснованиям.ДокументОснование ЕСТЬ NULL
		|				ТОГДА Выплачено.СуммаКВыплате
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК Выплачено,
		|	СУММА(ВЫБОР
		|			КОГДА Выплачено.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|					И НЕ КВыплатеПоОснованиям.ДокументОснование ЕСТЬ NULL
		|				ТОГДА Выплачено.СуммаКВыплате
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК Начислено
		|ПОМЕСТИТЬ ВТНачисленияВыплатыПоСотрудникамОснованиям
		|ИЗ
		|	ВТСотрудникиСЗадолженностью КАК СотрудникиСЗадолженностью
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗарплатаКВыплате КАК Выплачено
		|		ПО СотрудникиСЗадолженностью.Сотрудник = Выплачено.Сотрудник
		|			И СотрудникиСЗадолженностью.Период = Выплачено.ПериодВзаиморасчетов
		|			И (НЕ Выплачено.Регистратор В (&ИгнорируемыеРегистраторы))
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКВыплатеПоОснованиям КАК КВыплатеПоОснованиям
		|		ПО (Выплачено.Сотрудник = КВыплатеПоОснованиям.Сотрудник)
		|			И (Выплачено.ПериодВзаиморасчетов = КВыплатеПоОснованиям.ПериодВзаиморасчетов)
		|			И (Выплачено.ДокументОснование = КВыплатеПоОснованиям.ДокументОснование)
		|
		|СГРУППИРОВАТЬ ПО
		|	Выплачено.Сотрудник,
		|	Выплачено.ДокументОснование,
		|	Выплачено.Период,
		|	Выплачено.ПериодВзаиморасчетов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВыплатыПоСотрудникамОснованиям.Сотрудник КАК Сотрудник,
		|	ВыплатыПоСотрудникамОснованиям.Период КАК Период,
		|	СУММА(ВыплатыПоСотрудникамОснованиям.Начислено) КАК Начислено,
		|	СУММА(ВыплатыПоСотрудникамОснованиям.Выплачено) КАК Выплачено
		|ПОМЕСТИТЬ ВТНачисленияВыплатыПоСотрудникам
		|ИЗ
		|	ВТНачисленияВыплатыПоСотрудникамОснованиям КАК ВыплатыПоСотрудникамОснованиям
		|
		|СГРУППИРОВАТЬ ПО
		|	ВыплатыПоСотрудникамОснованиям.Сотрудник,
		|	ВыплатыПоСотрудникамОснованиям.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОграниченияЗадолженностей.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
		|	ОграниченияЗадолженностей.Сотрудник КАК Сотрудник,
		|	ОграниченияЗадолженностей.ДокументОснование КАК ДокументОснование,
		|	ОграниченияЗадолженностей.КВыплатеПоОснованию КАК КВыплатеПоОснованию,
		|	ОграниченияЗадолженностей.ВыплаченоПоОснованию КАК ВыплаченоПоОснованию,
		|	ОграниченияЗадолженностей.РазмерОграниченияОснования КАК РазмерОграниченияОснования,
		|	ВЫБОР
		|		КОГДА ОграниченияЗадолженностей.РазмерОграниченияОснования > ОграниченияЗадолженностей.ВыплаченоПоОснованию
		|			ТОГДА ВЫБОР
		|					КОГДА ОграниченияЗадолженностей.РазмерОграниченияОснования - ОграниченияЗадолженностей.ВыплаченоПоОснованию > -ОграниченияЗадолженностей.ВыплаченоПоОснованию
		|						ТОГДА -(ОграниченияЗадолженностей.РазмерОграниченияОснования - ОграниченияЗадолженностей.ВыплаченоПоОснованию)
		|					ИНАЧЕ ОграниченияЗадолженностей.ВыплаченоПоОснованию
		|				КОНЕЦ
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК КВыплатеПоОснованиюОграничено
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЗадолженностьПоОснованиям.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
		|		ЗадолженностьПоОснованиям.Сотрудник КАК Сотрудник,
		|		ЗадолженностьПоОснованиям.ДокументОснование КАК ДокументОснование,
		|		ЗадолженностьПоОснованиям.КВыплате КАК КВыплатеПоОснованию,
		|		ЕСТЬNULL(НачисленияВыплатыПоОснованиям.Выплачено, 0) КАК ВыплаченоПоОснованию,
		|		ЕСТЬNULL(НачисленияВыплатыПоСотрудникам.Начислено, 0) КАК НачисленоПоСотруднику,
		|		УсловияВозвратаЗадолженности.СпособОграничения КАК СпособОграничения,
		|		ВЫБОР
		|			КОГДА УсловияВозвратаЗадолженности.СпособОграничения = ЗНАЧЕНИЕ(Перечисление.СпособОграниченияВыплаты.Процентом)
		|				ТОГДА ЕСТЬNULL(НачисленияВыплатыПоСотрудникам.Начислено, 0) * УсловияВозвратаЗадолженности.Размер / 100
		|			ИНАЧЕ УсловияВозвратаЗадолженности.Размер
		|		КОНЕЦ КАК РазмерОграниченияОснования
		|	ИЗ
		|		ВТЗадолженностьПоОснованиям КАК ЗадолженностьПоОснованиям
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТУсловияВозвратаЗадолженностиСрезПоследних КАК УсловияВозвратаЗадолженности
		|			ПО ЗадолженностьПоОснованиям.Сотрудник = УсловияВозвратаЗадолженности.Сотрудник
		|				И ЗадолженностьПоОснованиям.ДокументОснование = УсловияВозвратаЗадолженности.ДокументОснование
		|				И ЗадолженностьПоОснованиям.Период = УсловияВозвратаЗадолженности.Период
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияВыплатыПоСотрудникамОснованиям КАК НачисленияВыплатыПоОснованиям
		|			ПО ЗадолженностьПоОснованиям.Сотрудник = НачисленияВыплатыПоОснованиям.Сотрудник
		|				И ЗадолженностьПоОснованиям.ДокументОснование = НачисленияВыплатыПоОснованиям.ДокументОснование
		|				И (&ПериодРегистрации = НачисленияВыплатыПоОснованиям.Период)
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияВыплатыПоСотрудникам КАК НачисленияВыплатыПоСотрудникам
		|			ПО ЗадолженностьПоОснованиям.Сотрудник = НачисленияВыплатыПоСотрудникам.Сотрудник
		|				И ЗадолженностьПоОснованиям.Период = НачисленияВыплатыПоСотрудникам.Период) КАК ОграниченияЗадолженностей"; 
	
	УсловияВозвратаЗадолженностей = Запрос.Выполнить().Выгрузить();
	СтруктураОтбора = Новый Структура("ПериодВзаиморасчетов,Сотрудник,ДокументОснование");
	СтрокиКУдалению = Новый Массив;
	Для Каждого ОграничениеПоОснованию Из УсловияВозвратаЗадолженностей Цикл
		ЗаполнитьЗначенияСвойств(СтруктураОтбора,ОграничениеПоОснованию);
		СтрокиЗарплаты = Зарплата.НайтиСтроки(СтруктураОтбора);
		Для Каждого СтрокаЗарплаты Из СтрокиЗарплаты Цикл
			СтрокаЗарплаты.КВыплате = МАКС(ОграничениеПоОснованию.КВыплатеПоОснованиюОграничено, СтрокаЗарплаты.КВыплате);
			ОграничениеПоОснованию.КВыплатеПоОснованиюОграничено = ОграничениеПоОснованию.КВыплатеПоОснованиюОграничено - СтрокаЗарплаты.КВыплате;
			Если СтрокаЗарплаты.КВыплате = 0 Тогда
				СтрокиКУдалению.Добавить(СтрокаЗарплаты);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПараметрыПолученияСотрудниковПоШапкеВедомости(ОписаниеОперации, ОтборСотрудников)
	...
	Если ОписаниеОперации.Свойство("ФизическиеЛица") Тогда
		ПараметрыПолученияСотрудников.СписокФизическихЛиц = ОписаниеОперации.ФизическиеЛица;
	КонецЕсли;
