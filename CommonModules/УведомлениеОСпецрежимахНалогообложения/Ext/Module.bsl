...
Процедура ЗагрузитьМногостраничныеСтраницыРек(Форма, ДополнительныеПараметры, СтрокиКарты, ДеревоСтраницСтроки, УИДРодителя, ТекущаяСтрокаВыгрузки, ПутьВерхнегоУровня)
	Для Каждого СтрокаКарты Из СтрокиКарты Цикл
		...
		Для Инд = 0 По МногостраничныеУзлы.ВГраница() Цикл 
			...
			Иначе
				Если ЗначениеЗаполнено(СтрокаВДереве.УИД) И СтрокаКарты.Строки.Количество() > 1 Тогда
					Для Каждого Стр Из СтрокаКарты.Строки Цикл 
						Для Каждого Элт Из Форма.ДанныеМногостраничныхРазделов[Стр.ИДНаименования] Цикл 
							Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Элт.Значение, "УИДРодителя", Новый УникальныйИдентификатор) = СтрокаВДереве.УИД Тогда 
								Элт.Значение.УИДРодителя = УИДТекСтр;
							КонецЕсли;
						КонецЦикла;
					КонецЦикла;
				КонецЕсли;
