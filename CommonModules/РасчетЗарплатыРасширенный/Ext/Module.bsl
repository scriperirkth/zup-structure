...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьРегистрРаспределенияРезультатовНачислений");
	Методы.Вставить("ДобавитьРольГрупповоеЗаполнениеПоказателей");
	Методы.Вставить("УстановитьНастройкуПереноситьДанныеВДругойУчетСреднегоЗаработка");
	Методы.Вставить("ДобавитьРольПросмотрПодбораВидовРасчета");
	Методы.Вставить("ОписатьПрочиеДоходыИВыплатыБывшимСотрудникам");
	Методы.Вставить("УстановитьОтображениеВДокументахНачисленияПоказателяУчитыватьМРОТ");
	Методы.Вставить("ПереносНастройкиИспользоватьНачисленияПоДоговорам");
КонецПроцедуры
...
Процедура СформироватьДвиженияНачислений(Движения, Отказ, Организация, ПериодРегистрации, Начисления, Показатели = Неопределено, ЗаписыватьДвижения = Ложь) Экспорт	
	...
	Для Каждого Строка Из Начисления Цикл
		...
		Если ЕстьПолеВСтрокеКоллекции(Строка, "НачалоБазовогоПериода") И ЗначениеЗаполнено(Строка.НачалоБазовогоПериода) 
			...
			НоваяСтрока.БазовыйПериодКонец = КонецДня(Строка.ОкончаниеБазовогоПериода);
		КонецЕсли;
...
Функция ДанныеДляЗаполненияТаблицДокумента(ОписаниеДокумента, Организация, МесяцНачисления, ДополнительныеПараметры = Неопределено) Экспорт	
	...
	МенеджерРасчета.НастройкиУдержаний.РассчитыватьТолькоПоТекущемуДокументу = РасчетПервойПоловиныМесяца
		Или (Не ЗаполнятьНачисленияПоТрудовымДоговорам И ЗаполнятьНачисленияПоДоговорамПодряда);
...
Процедура ЗаполнитьНачисления(ДанныеДляПроведения, Документ, ТаблицаНачислений = "Начисления", ПолеДатыДействия = "Ссылка.МесяцНачисления", ПолеВидаНачисления = "Начисление", СписокФизическихЛиц = Неопределено, ПолеПериодРегистрации = "Ссылка.МесяцНачисления", СторнируемыйДокументКакСторно = Ложь) Экспорт
	...
	НеобязательныеПоля.Вставить("ФиксБазовыйПериод", "ЛОЖЬ");
	...
	Если ПолучитьФункциональнуюОпцию("РаботаВБюджетномУчреждении") Или ТипЗнч(Документ) = Тип("ДокументСсылка.НачислениеЗарплаты") Тогда
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.НачислениеЗарплаты") Тогда
		РасчетЗарплаты.ДополнитьВТНачисленияЕдиноличныйРуководительКоммерческойОрганизации(Запрос, ДанныеДляПроведения, Организация, ПериодРегистрации);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПоДоговорамПодряда(ДанныеДляПроведения, РеквизитыДляПроведения, СписокФизическихЛиц = Неопределено, ИмяТаблицыДокумента = "НачисленияПоДоговорам") Экспорт
	...
	|	НачисленияПоДоговорам.Сумма КАК Сумма,
	|	ВЫБОР
	|		КОГДА НачисленияПоДоговорам.КодДохода = ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.РайонныеСеверныеНадбавки)
	|			ТОГДА НачисленияПоДоговорам.Сумма
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаРКиСН,
	|	НачисленияПоДоговорам.Договор КАК Договор,
	|	НачисленияПоДоговорам.НеОблагаетсяНДФЛ КАК НеОблагаетсяНДФЛ,
	|	НачисленияПоДоговорам.ДатаНачала КАК ДатаНачала,
	|	НачисленияПоДоговорам.ДатаОкончания КАК ДатаОкончания,
...
Функция ПустаяТаблицаНачисления(ИспользоватьДляПерерасчета = Ложь) Экспорт
	...
	Начисления.Колонки.Добавить("ФиксБазовыйПериод", Новый ОписаниеТипов("Булево"));
...
Процедура ПриЗаполненииТаблицыЗависимыхДанныхДляОбмена(ЗависимыеДанные) Экспорт
	...
	// Метаданные.РегистрыСведений.ЗначенияПоказателейСреднегоЗаработкаСотрудников
	НоваяСтрока = ЗависимыеДанные.Добавить();
	НоваяСтрока.ВедущиеМетаданные = Метаданные.РегистрыСведений.ЗначенияПоказателейСреднегоЗаработкаСотрудников;
	НоваяСтрока.ЗависимыеМетаданные = Метаданные.РегистрыСведений.ЗначенияПоказателейСреднегоЗаработкаСотрудниковИнтервальный;
...
Процедура СформироватьДвиженияЗначенийПоказателейСреднегоЗаработкаСотрудников(Движения, ЗначенияПоказателей) Экспорт
	
	Движения.ЗначенияПоказателейСреднегоЗаработкаСотрудников.Записывать = Истина;
	
	Для Каждого СтрокаТаблицы Из ЗначенияПоказателей Цикл
		НоваяСтрока = Движения.ЗначенияПоказателейСреднегоЗаработкаСотрудников.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
	КонецЦикла;
...
Процедура СоздатьВТУдержанияФизическихЛицПоПоказателямПерерасчетов(МенеджерВременныхТаблиц, УдалитьВТ)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ПлановыеУдержания.ДокументОснование, НЕОПРЕДЕЛЕНО) КАК ДокументОснование
...
Функция РезультатНачисленияПоДоговорам(Организация, МесяцНачисления, МенеджерВременныхТаблиц, ДокументСсылка, Отборы = Неопределено, ДатаВыплаты = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Не ЕСТЬNULL(УсловияДоговораОпекиСрезПоследних.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки, 0) В (0, 2, 4)
	|			ТОГДА НачисленияПоДоговорамГПХ.Сумма - НачисленияПоДоговорамГПХ.СуммаРКиСН
	|		ИНАЧЕ НачисленияПоДоговорамГПХ.Сумма
	|	КОНЕЦ КАК Сумма,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловияДоговораОпекиСрезПоследних КАК УсловияДоговораОпекиСрезПоследних
	|		ПО (НачисленияПоДоговорамГПХ.Договор ССЫЛКА Документ.ВыплатыПоДоговорамОпеки)
	|			И НачисленияПоДоговорамГПХ.Договор = УсловияДоговораОпекиСрезПоследних.Договор
	|ГДЕ
	...
	ВыплатыПоДоговорамОпеки.ВыделитьДолиРКиСН(Запрос);
...
Функция ПоказателиСуммированногоУчетаСотрудниковЗаПериод(МассивСотрудников, ПериодСуммированногоУчетаНачало, ПериодСуммированногоУчетаОкончание) Экспорт
	...
	Запрос.УстановитьПараметр("Дата144ФЗ", ЗарплатаКадрыПовтИсп.ДатаВступленияВСилуНА("ФедеральныйЗаконОт25мая2026Номер144ФЗ")); 
	Запрос.УстановитьПараметр("ПоказательПереработаноПоСуммированномуУчету", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ПереработаноПоСуммированномуУчету")); 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ 
	|	НормоВремяПоРегиструСУчетомПредела.Сотрудник КАК Сотрудник,
	|	НормоВремяПоРегиструСУчетомПредела.НормаЧасов КАК НормаЧасов
	|ПОМЕСТИТЬ ВТНормоВремяСотрудниковНаСуммированномУчетеСУчетомПредела
	|ИЗ
	|	ВТНормоВремяПоРегиструСУчетомПредела КАК НормоВремяПоРегиструСУчетомПредела
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НормоВремяПоУчетуВремениСУчетомПредела.Сотрудник,
	|	НормоВремяПоУчетуВремениСУчетомПредела.НормаЧасов
	|ИЗ
	|	ВТНормоВремяПоУчетуВремениСУчетомПредела КАК НормоВремяПоУчетуВремениСУчетомПредела
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ 
	|	ИтоговоеНормоВремяСУчетомПредела.Сотрудник КАК Сотрудник,
	|	СУММА(ИтоговоеНормоВремяСУчетомПредела.НормаЧасов) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТНормоВремяСводноСУчетомПредела
	|ИЗ
	|	ВТНормоВремяСотрудниковНаСуммированномУчетеСУчетомПредела КАК ИтоговоеНормоВремяСУчетомПредела
	|
	|СГРУППИРОВАТЬ ПО
	|	ИтоговоеНормоВремяСУчетомПредела.Сотрудник
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТОтработаноЧасовРегистр.ОтработаноДнейСмен КАК ОтработаноДнейСмен
	...
	|	ВТОтработаноЧасовУчетВремени.ОтработаноДнейСмен
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТОтработаноЧасовРегистрСУчетомПредела.Сотрудник КАК Сотрудник,
	|	ВТОтработаноЧасовРегистрСУчетомПредела.ОтработаноЧасов КАК ОтработаноЧасов,
	|	ВТОтработаноЧасовРегистрСУчетомПредела.ОтработаноДнейСмен КАК ОтработаноДнейСмен
	|ПОМЕСТИТЬ ВТОтработанныеЧасыСУчетомПредела
	|ИЗ
	|	ВТОтработаноЧасовРегистрСУчетомПредела КАК ВТОтработаноЧасовРегистрСУчетомПредела
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТОтработаноЧасовУчетВремениСУчетомПредела.Сотрудник,
	|	ВТОтработаноЧасовУчетВремениСУчетомПредела.ОтработаноЧасов,
	|	ВТОтработаноЧасовУчетВремениСУчетомПредела.ОтработаноДнейСмен
	|ИЗ
	|	ВТОтработаноЧасовУчетВремениСУчетомПредела КАК ВТОтработаноЧасовУчетВремениСУчетомПредела
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(ОтработанныеЧасы.ОтработаноДнейСмен) КАК ОтработаноДнейСмен
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтработанныеЧасыСУчетомПредела.Сотрудник КАК Сотрудник,
	|	СУММА(ОтработанныеЧасыСУчетомПредела.ОтработаноЧасов) КАК ОтработаноЧасов,
	|	СУММА(ОтработанныеЧасыСУчетомПредела.ОтработаноДнейСмен) КАК ОтработаноДнейСмен
	|ПОМЕСТИТЬ ВТОтработанныеЧасыСводноСУчетомПредела
	|ИЗ
	|	ВТОтработанныеЧасыСУчетомПредела КАК ОтработанныеЧасыСУчетомПредела
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтработанныеЧасыСУчетомПредела.Сотрудник";
	
	//Получаем отработанное(оплаченное) сверхурочное время и переработки
	//с начала года до начала расчетного периода
    ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Сотрудники.Сотрудник КАК Сотрудник,
	|	ЕСТЬNULL(ПоказательПереработаноПоСуммированномуУчету.ЗначениеПоказателя, 0) + ВЫБОР
	|		КОГДА ЕСТЬNULL(ВТОтработанныеЧасыСводно.ОтработаноЧасов, 0) - ЕСТЬNULL(ВТНормоВремяСводно.НормаЧасов, 0) < 0
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(ВТОтработанныеЧасыСводно.ОтработаноЧасов, 0) - ЕСТЬNULL(ВТНормоВремяСводно.НормаЧасов, 0)
	|	КОНЕЦ КАК ЧасыПереработокЗарегистрированныеРанее
	|ПОМЕСТИТЬ ВТПереработкиЗарегистрированныеРанее
	|ИЗ
	|	ВТРассчитываемыеСотрудники КАК Сотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНормоВремяСводно КАК ВТНормоВремяСводно
	|		ПО Сотрудники.Сотрудник = ВТНормоВремяСводно.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтработанныеЧасыСводно КАК ВТОтработанныеЧасыСводно
	|		ПО Сотрудники.Сотрудник = ВТОтработанныеЧасыСводно.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			СУММА(ЕСТЬNULL(ВЫРАЗИТЬ(ЗначенияРазовыхПоказателейРасчетаЗарплатыСотрудников.Значение КАК ЧИСЛО(15, 5)), 0)) КАК ЗначениеПоказателя,
	...
	|		ГДЕ
	|			ЗначенияРазовыхПоказателейРасчетаЗарплатыСотрудников.Показатель = &ПоказательПереработаноПоСуммированномуУчету
	|			И ЗначенияРазовыхПоказателейРасчетаЗарплатыСотрудников.ПериодДействия МЕЖДУ НАЧАЛОПЕРИОДА(&ПоследнийМесяцНачало, ГОД) И &ПоследнийМесяцНачало
	|			И ЗначенияРазовыхПоказателейРасчетаЗарплатыСотрудников.Сотрудник В
	|					(ВЫБРАТЬ
	|						Сотрудники.Сотрудник
	|					ИЗ
	|						ВТРассчитываемыеСотрудники КАК Сотрудники)
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ЗначенияРазовыхПоказателейРасчетаЗарплатыСотрудников.Сотрудник) КАК ПоказательПереработаноПоСуммированномуУчету
	|		ПО Сотрудники.Сотрудник = ПоказательПереработанопоСуммированномуУчету.Сотрудник";
	
	
	ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьТекстыЗапросов(Запрос.Текст, ТекстЗапроса);
	
	//Объединение данных и расчет часов переработки х1,5
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Сотрудники.Сотрудник КАК Сотрудник,
	...
	|	ЕСТЬNULL(ОтработанныеЧасы.ОтработаноДнейСмен, 0) КАК ОтработаноДнейСмен,
	|	ЕСТЬNULL(ОтработаноВПраздники.ЧасовВПраздники, 0) КАК ОтработаноЧасовВПраздники,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ОтработанныеЧасы.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудников.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздники.ЧасовВПраздники, 0) < 0
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(ОтработанныеЧасы.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудников.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздники.ЧасовВПраздники, 0)
	|	КОНЕЦ КАК Переработки,
	|	0 КАК ЧасыПереработокЗарегистрированныеРанее,
	|	ВЫБОР
	|		КОГДА ВЫБОР
	|				КОГДА ЕСТЬNULL(ОтработанныеЧасы.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудников.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздники.ЧасовВПраздники, 0) > ЕСТЬNULL(ОтработанныеЧасы.ОтработаноДнейСмен, 0) * 2
	|					ТОГДА ЕСТЬNULL(ОтработанныеЧасы.ОтработаноДнейСмен, 0) * 2
	|				ИНАЧЕ ЕСТЬNULL(ОтработанныеЧасы.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудников.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздники.ЧасовВПраздники, 0)
	|			КОНЕЦ < 0
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	...
	|ВЫБРАТЬ
	|	Сотрудники.Сотрудник,
	|	ЕСТЬNULL(НормоВремяСотрудниковСУчетомПредела.НормаЧасов, 0),
	|	ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноЧасов, 0),
	|	ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноДнейСмен, 0),
	|	ЕСТЬNULL(ОтработаноВПраздникиСУчетомПредела.ЧасовВПраздники, 0),
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудниковСУчетомПредела.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздникиСУчетомПредела.ЧасовВПраздники, 0) < 0
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудниковСУчетомПредела.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздникиСУчетомПредела.ЧасовВПраздники, 0)
	|	КОНЕЦ,
	|	ЕСТЬNULL(ПереработкиЗарегистрированныеРанее.ЧасыПереработокЗарегистрированныеРанее, 0),
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПереработкиЗарегистрированныеРанее.ЧасыПереработокЗарегистрированныеРанее, 0) > 120
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	|				КОГДА 120 - ЕСТЬNULL(ПереработкиЗарегистрированныеРанее.ЧасыПереработокЗарегистрированныеРанее, 0) > ВЫБОР
	|						КОГДА ВЫБОР
	|								КОГДА ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудниковСУчетомПредела.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздникиСУчетомПредела.ЧасовВПраздники, 0) > ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноДнейСмен, 0) * 2
	|									ТОГДА ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноДнейСмен, 0) * 2
	|								ИНАЧЕ ЕСТЬNULL(ОтработанныеЧасыСУчетомПредела.ОтработаноЧасов, 0) - ЕСТЬNULL(НормоВремяСотрудниковСУчетомПредела.НормаЧасов, 0) - ЕСТЬNULL(ОтработаноВПраздникиСУчетомПредела.ЧасовВПраздники, 0)
	|							КОНЕЦ < 0
	|							ТОГДА 0
	|						ИНАЧЕ ВЫБОР
	...
	|								КОНЕЦ
	|						КОНЕЦ
	|				ИНАЧЕ 120 - ЕСТЬNULL(ПереработкиЗарегистрированныеРанее.ЧасыПереработокЗарегистрированныеРанее, 0)
	|			КОНЕЦ
	|	КОНЕЦ
	|ИЗ
	|	ВТРассчитываемыеСотрудники КАК Сотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНормоВремяСводноСУчетомПредела КАК НормоВремяСотрудниковСУчетомПредела
	|		ПО Сотрудники.Сотрудник = НормоВремяСотрудниковСУчетомПредела.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтработанныеЧасыСводноСУчетомПредела КАК ОтработанныеЧасыСУчетомПредела
	|		ПО Сотрудники.Сотрудник = ОтработанныеЧасыСУчетомПредела.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПраздничныеСУчетомПредела КАК ОтработаноВПраздникиСУчетомПредела
	|		ПО Сотрудники.Сотрудник = ОтработаноВПраздникиСУчетомПредела.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПереработкиЗарегистрированныеРанее КАК ПереработкиЗарегистрированныеРанее
	|		ПО Сотрудники.Сотрудник = ПереработкиЗарегистрированныеРанее.Сотрудник
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТСводныеДанные.Сотрудник КАК Сотрудник,
	|	СУММА(ВТСводныеДанные.НормаЧасов) КАК НормаЧасов,
	|	СУММА(ВТСводныеДанные.ОтработаноЧасов) КАК ОтработаноЧасов,
	|	СУММА(ВТСводныеДанные.ОтработаноДнейСмен) КАК ОтработаноДнейСмен,
	|	СУММА(ВТСводныеДанные.ОтработаноЧасовВПраздники) КАК ОтработаноЧасовВПраздники,
	|	СУММА(ВТСводныеДанные.Переработки) КАК Переработки,
	|	СУММА(ВТСводныеДанные.Переработки1_5) КАК Переработки1_5
	|ИЗ
	|	ВТСводныеДанные КАК ВТСводныеДанные
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТСводныеДанные.Сотрудник";
...
Процедура ДополнитьЗапросВТПоказателейПоРегиструРасчета(Запрос)
	...
	|	И Начисления.Период < НАЧАЛОПЕРИОДА(&Дата144ФЗ, МЕСЯЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ  
	...
	|	Начисления.ОтработаноЧасов КАК ОтработаноЧасов,
	|	Начисления.ОтработаноДней КАК ОтработаноДней
	|ПОМЕСТИТЬ ВТНачисленияСуммУчетаСУчетомПредела
	|ИЗ
	|	ВТНачисления КАК Начисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК ВТКадровыеДанныеСотрудников
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ГрафикиРаботыСотрудников КАК ГрафикиРаботыСотрудников
	...
	|	СУММА(НачисленияСотрудников.ОтработаноДней) КАК ОтработаноДнейСмен
	...
	|	СУММА(НачисленияСотрудников.ОтработаноЧасов) КАК ОтработаноЧасов,
	|	СУММА(НачисленияСотрудников.ОтработаноДней) КАК ОтработаноДнейСмен
	|ПОМЕСТИТЬ ВТОтработаноЧасовРегистрСУчетомПредела
	|ИЗ
	|	ВТНачисленияСуммУчетаСУчетомПредела КАК НачисленияСотрудников
	|
	|СГРУППИРОВАТЬ ПО
	|	НачисленияСотрудников.Сотрудник
	|;
	...
	|	СУММА(ВЫБОР
	|			КОГДА ВТДанныеПроизводственногоКалендаряПредварительно.НормаЧасов < ЕСТЬNULL(ВТВнутрисменныеОтклоненияЧасы.Часы, 0)
	|				ТОГДА 0
	...
	|ГДЕ
	|	ВТДанныеПроизводственногоКалендаряПредварительно.Дата < &Дата144ФЗ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ИНАЧЕ ВТДанныеПроизводственногоКалендаряПредварительно.НормаЧасов - ЕСТЬNULL(ВТВнутрисменныеОтклоненияЧасы.Часы, 0)
	|		КОНЕЦ) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТДанныеПроизводственногоКалендаряСУчетомПредела
	|ИЗ
	|	ВТДанныеПроизводственногоКалендаряПредварительно КАК ВТДанныеПроизводственногоКалендаряПредварительно
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВнутрисменныеОтклоненияЧасы КАК ВТВнутрисменныеОтклоненияЧасы
	|		ПО ВТДанныеПроизводственногоКалендаряПредварительно.Сотрудник = ВТВнутрисменныеОтклоненияЧасы.Сотрудник
	...
	|	СУММА(ВЫБОР
	|			КОГДА ВТДанныеГрафикаНесгруппированные.НормаЧасов < ЕСТЬNULL(ВТВнутрисменныеОтклоненияЧасы.Часы, 0)
	|				ТОГДА 0
	...
	|ГДЕ
	|	ВТДанныеГрафикаНесгруппированные.Дата < &Дата144ФЗ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ИНАЧЕ ВТДанныеГрафикаНесгруппированные.НормаЧасов - ЕСТЬNULL(ВТВнутрисменныеОтклоненияЧасы.Часы, 0)
	|		КОНЕЦ) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТДанныеГрафикаСУчетомПредела
	|ИЗ
	|	ВТДанныеГрафикаНесгруппированные КАК ВТДанныеГрафикаНесгруппированные
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВнутрисменныеОтклоненияЧасы КАК ВТВнутрисменныеОтклоненияЧасы
	|		ПО ВТДанныеГрафикаНесгруппированные.Сотрудник = ВТВнутрисменныеОтклоненияЧасы.Сотрудник
	...
	|//////////////////////////////////////////////////////////////////////////////// 
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВТДанныеГрафикаСУчетомПредела.Сотрудник, ВТДанныеПроизводственногоКалендаряСУчетомПредела.Сотрудник) КАК Сотрудник,
	|	ЕСТЬNULL(ВТДанныеГрафикаСУчетомПредела.НормаЧасов, 0) + ЕСТЬNULL(ВТДанныеПроизводственногоКалендаряСУчетомПредела.НормаЧасов, 0) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТНормоВремяПоРегиструСУчетомПредела
	|ИЗ
	|	ВТДанныеПроизводственногоКалендаряСУчетомПредела КАК ВТДанныеПроизводственногоКалендаряСУчетомПредела
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВТДанныеГрафикаСУчетомПредела КАК ВТДанныеГрафикаСУчетомПредела
	|		ПО ВТДанныеПроизводственногоКалендаряСУчетомПредела.Сотрудник = ВТДанныеГрафикаСУчетомПредела.Сотрудник
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|//////////////////////////////////////////////////////////////////////////////// 
	|УНИЧТОЖИТЬ ВТДанныеПроизводственногоКалендаряСУчетомПредела
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДанныеГрафикаСУчетомПредела";
...
Процедура ДополнитьЗапросВТПоказателейПоУчетуВремени(Запрос)
	...
	|	РабочееВремя.ПереходящаяЧастьПредыдущейСмены КАК ПереходящаяЧастьПредыдущейСмены,
	...
	|	ВТРабочееВремя.ПереходящаяЧастьПредыдущейСмены КАК ПереходящаяЧастьПредыдущейСмены,
	...
	|	ВТРабочееВремя.ПереходящаяЧастьПредыдущейСмены, 
	...
	|	ДниРаботы.ПереходящаяЧастьПредыдущейСмены КАК ПереходящаяЧастьПредыдущейСмены,
	...
	|	ДниРаботы.Часы КАК Часы,
	|	ВЫБОР
	|		КОГДА ДниРаботы.Часы > 0 И НЕ ДниРаботы.ПереходящаяЧастьТекущейСмены И НЕ ДниРаботы.ПереходящаяЧастьПредыдущейСмены
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДниСмены
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДниСмены
	|ПОМЕСТИТЬ ВТРабочиеЧасыУчетВремениСУчетомПредела
	|ИЗ
	|	ВТДниРаботыРасширенный КАК ДниРаботы
	|ГДЕ
	|	ДниРаботы.Дата МЕЖДУ &ПоследнийМесяцНачало И &ПоследнийМесяцОкончание
	...
	|	СУММА(РабочиеЧасы.ДниСмены) КАК ОтработаноДнейСмен
	...
	|	СУММА(РабочиеЧасы.Часы) КАК ОтработаноЧасов,
	|	СУММА(РабочиеЧасы.ДниСмены) КАК ОтработаноДнейСмен
	|ПОМЕСТИТЬ ВТОтработаноЧасовУчетВремениСУчетомПредела
	|ИЗ
	|	ВТРабочиеЧасыУчетВремениСУчетомПредела КАК РабочиеЧасы
	|
	|СГРУППИРОВАТЬ ПО
	|	РабочиеЧасы.Сотрудник
	|;
	...
	|ГДЕ
	|	ДниРаботыСПраздниками.Дата < &Дата144ФЗ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ДниРаботыСПраздниками.Сотрудник КАК Сотрудник,
	|	СУММА(ДниРаботыСПраздниками.ЧасовВПраздники) КАК ЧасовВПраздники
	|ПОМЕСТИТЬ ВТПраздничныеСУчетомПредела
	|ИЗ
	|	ВТДниРаботыРасширенныйСПраздниками КАК ДниРаботыСПраздниками         
	|ГДЕ
	|	ДниРаботыСПраздниками.Дата >= &Дата144ФЗ
	...
	|	ВТДанныеПроизводственногоКалендаряПредварительно.Сотрудник КАК Сотрудник,
	|	СУММА(ВЫБОР
	|			КОГДА ВТДанныеПроизводственногоКалендаряПредварительно.НормаЧасов < ЕСТЬNULL(ВТВнутрисменныеОтсутствияНормаВремени.Часы, 0)
	|				ТОГДА 0
	...
	|ГДЕ
	|	СотрудникиГрафики.СпособОпределенияНормыСуммированногоУчета = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияНормыСуммированногоУчета.ПоДаннымДругогоГрафика)
	|	И СотрудникиГрафики.Дата < &Дата144ФЗ 
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ИНАЧЕ ЕСТЬNULL(ГрафикиРаботыПоВидамВремени.ДополнительноеЗначение, 0) - ЕСТЬNULL(ВТВнутрисменныеОтсутствияНормаВремени.Часы, 0)
	|	КОНЕЦ КАК НормаЧасов
	|ПОМЕСТИТЬ ВТДанныеГрафикаНесгруппированныеСУчетомПредела
	|ИЗ
	|	ВТСчитатьПоГрафику КАК СотрудникиГрафики
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	|		ПО СотрудникиГрафики.ГрафикНормы = ГрафикиРаботыПоВидамВремени.ГрафикРаботы
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТДанныеГрафикаНесгруппированныеСУчетомПредела.Сотрудник КАК Сотрудник,
	|	СУММА(ЕСТЬNULL(ВТДанныеГрафикаНесгруппированныеСУчетомПредела.НормаЧасов, 0)) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТДанныеГрафикаСУчетомПредела
	|ИЗ
	|	ВТДанныеГрафикаНесгруппированныеСУчетомПредела КАК ВТДанныеГрафикаНесгруппированныеСУчетомПредела
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТДанныеГрафикаНесгруппированныеСУчетомПредела.Сотрудник
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник
	|;
	...
	|	ЕСТЬNULL(ВТДанныеГрафикаСУчетомПредела.Сотрудник, ВТДанныеПроизводственногоКалендаряСУчетомПредела.Сотрудник) КАК Сотрудник,
	|	ЕСТЬNULL(ВТДанныеГрафикаСУчетомПредела.НормаЧасов, 0) + ЕСТЬNULL(ВТДанныеПроизводственногоКалендаряСУчетомПредела.НормаЧасов, 0) КАК НормаЧасов
	|ПОМЕСТИТЬ ВТНормоВремяПоУчетуВремениСУчетомПредела
	|ИЗ
	|	ВТДанныеПроизводственногоКалендаряСУчетомПредела КАК ВТДанныеПроизводственногоКалендаряСУчетомПредела
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВТДанныеГрафикаСУчетомПредела КАК ВТДанныеГрафикаСУчетомПредела
	|		ПО ВТДанныеПроизводственногоКалендаряСУчетомПредела.Сотрудник = ВТДанныеГрафикаСУчетомПредела.Сотрудник
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДанныеГрафикаНесгруппированныеСУчетомПредела
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДанныеГрафика
...
Процедура ЗаполнитьИнформациюОбУчетеВремени(ВидРасчетаОбъект) Экспорт
	...
	Если Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СтоимостьДняЧасаСУчетомРазовыхНачислений") 
		И Не УчетВремениВЧасах Тогда 
		УчетВремениВДняхЧасах = Истина;
	КонецЕсли;
...
Функция СсылкиНачисленийУдержанийНачальнойНастройкиПрограммы() Экспорт
	...
	СсылкиНачисленийУдержаний.Вставить("ДоплатаЗаНаставничество", Неопределено);
...
Процедура ЗаполнитьПоказателиРКСН(СтрокаНачислений, МенеджерРасчета, ЗначенияПоказателей, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для Каждого ОписаниеПоказателя Из ВидРасчетаИнфо.Показатели Цикл
		Если ЗначенияПоказателей.Свойство(ОписаниеПоказателя.Идентификатор,ЗначениеПоказателя) Тогда
			МенеджерРасчета.ДобавитьИзвестноеЗначениеПоказателя(СтрокаНачислений, ОписаниеПоказателя.Показатель, ЗначениеПоказателя, ДополнительныеПараметры);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры 
...
Процедура СоздатьВТПериодыИзмененияЗначенийПоказателяСевернаяНадбавка(ОписательТаблиц) Экспорт
	...
	Запрос.Текст =
		...
		|	ПроцентСевернойНадбавкиСотрудников.Сотрудник.ГоловнаяОрганизация КАК ГоловнаяОрганизация
		|ПОМЕСТИТЬ ВТОтбораОрганизаций
		|ИЗ
		|	ВТПроцентСевернойНадбавкиСотрудников КАК ПроцентСевернойНадбавкиСотрудников";
	Запрос.Выполнить();
	
	ПараметрыПостроения = ЗарплатаКадрыРасширенный.ПараметрыПостроенияВТОрганизацииПрименяющиеАУСН("ВТОтбораОрганизаций");
	ЗапросВТОрганизацииПрименяющиеАУСН = ЗарплатаКадрыРасширенный.ЗапросВТОрганизацииПрименяющиеАУСН(ПараметрыПостроения);
	...
	ЗапросВТНачисленияСевернаяНадбавка = ЗарплатаКадрыРасширенный.ЗапросВТНачисленияСевернаяНадбавка();
	...
	Запрос.Текст =
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ПроцентСевернойНадбавкиСотрудников.Сотрудник.ГоловнаяОрганизация,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПроцентСевернойНадбавкиСотрудников.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		|	ПроцентСевернойНадбавкиСотрудников.Сотрудник КАК Сотрудник,
		|	ПроцентСевернойНадбавкиСотрудников.Период КАК Период,
		...
		|ВЫБРАТЬ
		|	ПроцентСевернойНадбавкиСотрудников.Сотрудник КАК Сотрудник,
		|	НачисленияСевернаяНадбавка.Начисление КАК Начисление,
		|	ПроцентСевернойНадбавкиСотрудников.Период КАК Период,
		...
		|		ПО ПроцентСевернойНадбавкиСотрудников.Период = НачисленияСевернаяНадбавка.Период
		|			И ПроцентСевернойНадбавкиСотрудников.ГоловнаяОрганизация = НачисленияСевернаяНадбавка.ГоловнаяОрганизация
		|ГДЕ
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.35.64";
	Обработчик.Процедура = "РегистрыСведений.ЗначенияПоказателейСреднегоЗаработкаСотрудников.ЗаполнитьЗначенияПоказателейСреднегоЗаработкаСотрудников";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("76d3e5ef-ab5c-4052-a871-aff7ab7fdd5f");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистра Значения показателей среднего заработка (для сотрудников).'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.35.65";
	Обработчик.Процедура = "РегистрыСведений.ЗначенияПоказателейСреднегоЗаработкаСотрудников.ЗаполнитьИнтервальныйРегистр";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f1640beb-865a-4901-93eb-d4dc9cd37062");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистра Значения показателей среднего заработка интервальный.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.35";
	Обработчик.Процедура = "Справочники.ВидыОтпусков.УточнитьНаименованияВидовУчебныхОтпусков";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("d000000b-5cb4-482e-af06-b1ae9e3c740e");
	Обработчик.Комментарий = НСтр("ru = 'Уточнение наименований видов учебных отпусков в соответствии с ТК РФ.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.35";
	Обработчик.Процедура = "ПланыВидовРасчета.Начисления.УточнитьНаименованияНачисленийУчебныхОтпусков";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("09f8240f-939f-4cce-9f27-4b8c14c5d836");
	Обработчик.Комментарий = НСтр("ru = 'Уточнение наименований начислений учебных отпусков в соответствии с ТК РФ.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.42";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("33ad7b9f-b028-4099-8dd3-12f9043fac2b");
	Обработчик.Процедура = "Документы.ДанныеДляРасчетаЗарплаты.ЗаполнитьДатыЗапрета";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Данные для расчета зарплаты"".'");
