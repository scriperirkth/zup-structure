...
Процедура ЗапомнитьИзмененияСтажейФизическихЛицПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущаяТаблица.Период КАК Период,
		|	ТекущаяТаблица.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ТекущаяТаблица.ВидСтажа КАК ВидСтажа,
		|	ТекущаяТаблица.ДатаОтсчета КАК ДатаОтсчета,
		|	ТекущаяТаблица.РазмерМесяцев КАК РазмерМесяцев,
		|	ТекущаяТаблица.РазмерДней КАК РазмерДней,
		|	ТекущаяТаблица.Прерван КАК Прерван
		|ПОМЕСТИТЬ ВТТекущийНабор
		|ИЗ
		|	РегистрСведений.СтажиФизическихЛиц КАК ТекущаяТаблица
		|ГДЕ
		|	&УсловиеОтбора
		|;
		|
	...
	РегистрыБЗК.ОтборЗаписейНабораИзБазыДанных(Источник, Замещение, Запрос);
...
Процедура ПерезаполнитьВторичныеЗаписиНачисленийЗависящихОтСтажаПриЗаписи(Источник, Отказ, Замещение) Экспорт
	...
	Запрос.УстановитьПараметр("НачислениеСевернаяНадбавка", ЗарплатаКадрыРасширенныйПовтИсп.НачислениеСевернаяНадбавка(Ложь));
	Запрос.УстановитьПараметр("НачислениеСевернаяНадбавкаАУСН", ЗарплатаКадрыРасширенныйПовтИсп.НачислениеСевернаяНадбавка(Истина));
	Запрос.УстановитьПараметр("НачисленияВходящиеВСоставФОТ", РасчетЗарплатыРасширенный.НачисленияВходящиеВСоставФОТ());
	...
	Запрос.Текст =
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ОбновляемыеВидыСтажей.ФизическоеЛицо,
		|	&НачислениеСевернаяНадбавкаАУСН
		|ИЗ
		|	ВТОбновляемыеВидыСтажей КАК ОбновляемыеВидыСтажей
		|ГДЕ
		|	ВЫРАЗИТЬ(ОбновляемыеВидыСтажей.ВидСтажа КАК Справочник.ВидыСтажа).КатегорияСтажа = ЗНАЧЕНИЕ(Перечисление.КатегорииСтажа.Северный)
		|	И &НачислениеСевернаяНадбавкаАУСН В (&НачисленияВходящиеВСоставФОТ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	СотрудникиПериодыКРасчету.Сотрудник КАК Сотрудник
		|ИЗ
...
Процедура ПроверитьИзменениеГрафиковРаботыСотрудников(Источник, Отказ, Замещение) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ГрафикРаботыСотрудников.Регистратор В (&Регистраторы)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ПроверитьИзменениеКадровыхДанныхСотрудников(Источник, Отказ, Замещение) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	КадроваяИсторияСотрудников.Регистратор В (&Регистраторы)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ПроверитьИзменениеСреднемесячныхНормВремениГрафиковРаботыСотрудников(Источник, Отказ, Замещение) Экспорт
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущаяТаблица.ГрафикРаботыСотрудников,
		|	ТекущаяТаблица.Год,
		|	ТекущаяТаблица.СреднемесячноеЧислоЧасов,
		|	ТекущаяТаблица.СреднемесячноеЧислоДней
		|ПОМЕСТИТЬ ВТСреднемесячныеНормыВремениТекущийНабор
		|ИЗ
		|	РегистрСведений.СреднемесячныеНормыВремениГрафиковРаботыСотрудников КАК ТекущаяТаблица
		|ГДЕ
		|	&УсловиеОтбора";
