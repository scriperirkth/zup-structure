...
//   * ИспользоватьМастерСозданияОбращения - Булево - Если Истина, и если доступно в Управлении службой поддержки, 
//   												  будет использоваться Мастер создания обращения
//   * ДанныеДляМастера - Структура - данные для передачи в Мастер создания обращений (значения структуры - примитивные типы - Строка, Число, Булево, Дата)
//
...
Функция НовыеПараметрыОбращенияВСлужбуПоддержки() Экспорт
	...
	Результат.Вставить("ИспользоватьМастерСозданияОбращения", Ложь);
	Результат.Вставить("КлючевыеПоляДляМастера", Новый Структура);
...
//
// Возвращаемое значение:
//   Массив из Строка - массив из двух строк, содержащих шестнадцатеричное представление кода пользователя.
//
Функция РазделитьКодПользователя(КодПользователя) Экспорт
	...
	Для каждого Число из МассивЧисел Цикл
		...
		Результат[0] = Результат[0] + ШестнадцатеричноеПредставлениеЧисла(РазделенноеЧисло[0][1], 8);
		Результат[1] = Результат[1] + ШестнадцатеричноеПредставлениеЧисла(РазделенноеЧисло[1][1], 8);
	КонецЦикла;
...
//
// Возвращаемое значение:
//   Строка - шестнадцатеричное представление числа, возможно,
//   дополненное нулями до указанного количества разрядов.
//
Функция ШестнадцатеричноеПредставлениеЧисла(Знач Число, Разрядов = Неопределено)
