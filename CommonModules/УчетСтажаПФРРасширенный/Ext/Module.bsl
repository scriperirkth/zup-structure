...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьДатуНачалаДвиженийПФР");
КонецПроцедуры
...
Процедура СоздатьВТОплаченныеДоговоры(МенеджерВременныхТаблиц, НачалоПериода, ОкончаниеПериода) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДоговораГПХ.ДокументОснование КАК ДокументОснование,
		|	ПараметрыПериодовСтажаПФРИспр.ДокументОснование КАК ИсправленныйДокумент
		|ПОМЕСТИТЬ ВТДоговораГПХИсправленные
		|ИЗ
		|	ВТДоговораГПХ КАК ДоговораГПХ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыПериодовСтажаПФРИспр КАК ПараметрыПериодовСтажаПФРИспр
		|		ПО ДоговораГПХ.ДокументОснование = ПараметрыПериодовСтажаПФРИспр.РегистраторИзмерение";
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ДоговораГПХ.ДокументОснование КАК ДокументОснование,
		|	ДоговораГПХ.ДокументОснование КАК ДоговорАкт
		...
		|
		|ВЫБРАТЬ
		|	ДоговораГПХИсправленные.ДокументОснование,
		|	ДоговораГПХИсправленные.ИсправленныйДокумент
		|ИЗ
		|	ВТДоговораГПХИсправленные КАК ДоговораГПХИсправленные
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	АктПриемки.Ссылка
		|ИЗ
		|	Документ.АктПриемкиВыполненныхРаботОказанныхУслуг КАК АктПриемки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДоговораГПХ КАК ДоговораГПХ
		|		ПО АктПриемки.Договор = ДоговораГПХ.ДокументОснование
		...
		|ИЗ
		|	Документ.АктПриемкиВыполненныхРаботОказанныхУслуг КАК АктПриемки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДоговораГПХИсправленные КАК ДоговораГПХИсправленные
		|		ПО АктПриемки.Договор = ДоговораГПХИсправленные.ИсправленныйДокумент
		|			И (АктПриемки.Проведен)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	ДокументыОплаты.ДокументОснование КАК ДокументОснование
		|ПОМЕСТИТЬ ВТОплаченныеДоговоры
		|ИЗ
		|	ВТДокументыОплаты КАК ДокументыОплаты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОплаченныеДоговоры КАК ОплаченныеДоговоры
		|		ПО ДокументыОплаты.ДоговорАкт = ОплаченныеДоговоры.ДоговорАкт
		|			И (ОплаченныеДоговоры.МесяцНачисления МЕЖДУ &НачалоПериода И &ОкончаниеПериода)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДокументыОплаты.ДокументОснование
		|ИЗ
		|	ВТДокументыОплаты КАК ДокументыОплаты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.НачисленияПоДоговорамГПХ КАК НачисленияПоДоговорамГПХ
		|		ПО ДокументыОплаты.ДоговорАкт = НачисленияПоДоговорамГПХ.Договор
		|			И (НачисленияПоДоговорамГПХ.Период МЕЖДУ &НачалоПериода И &ОкончаниеПериода)";
	
	Запрос.Выполнить();
	
	ИменаВТ = Новый Массив;
	ИменаВТ.Добавить("ВТДоговораГПХИсправленные");
	ИменаВТ.Добавить("ВТДокументыОплаты");
