...
Процедура ВыполнитьОбмен(УчетнаяЗапись) Экспорт
	...
	ПолученыСообщения = ОбработкаЭДО.ВыполнитьОбмен(УчетнаяЗапись);
	...
	Если ПолученыСообщения Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии("8.3.26.0", СистемнаяИнформация.ВерсияПриложения) <= 0 Тогда
			ОбщегоНазначения.ВыполнитьВБезопасномРежиме(
				"УведомленияКлиента.ОтправитьУведомление(""ДокументооборотСКО_ПолученыСообщения"", Неопределено)");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
    Методы.Вставить("УстановитьПараметрСеансаТекущийСеансДокументооборотаСКО");
