...
Процедура НачисленияОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Параметры.Отбор.Свойство("РедактируетсяВОтдельныхПолях") Тогда
		ТекстЗапросаУсловий = ?(ПустаяСтрока(ТекстЗапросаУсловий), "", ТекстЗапросаУсловий + Символы.ПС + " И ")
			+ "ПланВидовРасчетаНачисления.РедактируетсяВОтдельныхПолях = &ОтборРедактируетсяВОтдельныхПолях";
		Запрос.УстановитьПараметр("ОтборРедактируетсяВОтдельныхПолях", Параметры.Отбор.РедактируетсяВОтдельныхПолях);
	КонецЕсли;
	...
	Если Параметры.Отбор.Свойство("ОграничиватьВыборОсновныхВидовРасчетаОплатыПоСреднему") Тогда
		...
		Если Не ПустаяСтрока(ТекстЗапроса) Тогда
			ИсключаемыеКатегории = ПланыВидовРасчета.Начисления.КатегорииНачисленийОплатыДолейРКиСН();
...
Процедура ЗаполнитьОтборДокумента(Данные, ОписаниеДокумента) Экспорт
	Если ЗначениеЗаполнено(Данные.Сотрудник) И (Не ЗначениеЗаполнено(Данные.ФизическоеЛицо) Или Не ЗначениеЗаполнено(Данные.Подразделение)) Тогда
		КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудника(Истина, Данные.Сотрудник, "ФизическоеЛицо, Подразделение", Данные.МесяцНачисления);
		Если КадровыеДанные <> Неопределено Тогда
			СписокСвойств = "ФизическоеЛицо" + ?(ЗначениеЗаполнено(КадровыеДанные.Подразделение), ", Подразделение", "");
			ЗаполнитьЗначенияСвойств(Данные, КадровыеДанные, СписокСвойств);
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Данные.ФизическоеЛицо) И Не ЗначениеЗаполнено(Данные.Сотрудник) Тогда
		Параметры = КадровыйУчет.ПараметрыПолученияСотрудниковОрганизацийПоСпискуФизическихЛиц();
		Параметры.Организация = Данные.Организация;
		Параметры.НачалоПериода = Данные.МесяцНачисления;
		Параметры.ОкончаниеПериода = Данные.МесяцНачисления;
		Параметры.РаботникиПоДоговорамГПХ = Истина;
		Параметры.СписокФизическихЛиц = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Данные.ФизическоеЛицо);
		Параметры.КадровыеДанные = "ВидЗанятости,Подразделение";
		Параметры.Подразделение = Данные.Подразделение;
		СотрудникиФизЛица = КадровыйУчет.СотрудникиОрганизации(Истина, Параметры);
		Если СотрудникиФизЛица.Количество() = 1 Тогда
			ЗаполнитьЗначенияСвойств(Данные, СотрудникиФизЛица[0]);
			Возврат;
		КонецЕсли;
		Для Каждого СотрудникФизлица Из СотрудникиФизЛица Цикл
			Если Данные.Подразделение = СотрудникФизлица.Подразделение Тогда
				Данные.Сотрудник = СотрудникФизлица.Сотрудник;
				Прервать;
			КонецЕсли;
			Если СотрудникФизлица.ВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы Тогда
				ОсновнойСторудник = СотрудникФизлица.Сотрудник;
				Подразделение = СотрудникФизлица.Подразделение;
			КонецЕсли;
		КонецЦикла;
		Если Не ЗначениеЗаполнено(Данные.Сотрудник) Тогда
			Если ЗначениеЗаполнено(ОсновнойСторудник) Тогда
				Данные.Сотрудник = ОсновнойСторудник;
				Данные.Подразделение = Подразделение;
			ИначеЕсли СотрудникиФизЛица.Количество() > 0 Тогда
				ЗаполнитьЗначенияСвойств(Данные, СотрудникиФизЛица[0]);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
