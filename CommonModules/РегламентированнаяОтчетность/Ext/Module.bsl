...
Процедура СохранитьСтатусОтправкиРеглОтчета(Форма) Экспорт
	...
	ЭтоОтчетМинобороны = Реквизиты.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ"
		...
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма2"
		...
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма4"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма5"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма6"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма7"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма9"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма11"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма13"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма14"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма16"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма19д"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма20д"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма21"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма23";
...
// Возвращает признак утраты права на УСН организацией.
//
// Параметры:
//   СтруктураПараметров - Структура:
//        * Организация   - СправочникСсылка.Организации - ссылка на элемент справочника "Организации".
//        * ДатаНачала    - Дата - дата начала периода, для которого нужно получить признак утраты права на УСН.
//        * ДатаОкончания - Дата - дата окончания периода, для которого нужно получить признак утраты права на УСН.
//
// Возвращаемое значение:
//   Истина - организация утратила право на УСН за период;
//   Ложь   - организация не утратила право УСН за период.
//
Функция ОрганизацияУтратилаПравоНаУСН(СтруктураПараметров) Экспорт
	
	ДатаПереходаНаОСНО = '00010101';
	РегламентированнаяОтчетностьПереопределяемый.ОпределитьДатуПереходаНаОСНО(
		ДатаПереходаНаОСНО, СтруктураПараметров);
	
	Возврат ЗначениеЗаполнено(ДатаПереходаНаОСНО);
...
Процедура ЗагрузитьСправочникиШаблонаВРегистр(ИмяФормы, ИсточникОтчета, МакетШаблона) Экспорт
	...
	ИначеЕсли ТипЗнч(МакетШаблона) = Тип("ДвоичныеДанные") Тогда 
		...
		ДокументШаблон = ЗагрузитьФайлXML(ИтоговыйФайл);
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ИтоговыйФайл);
	Иначе
		Возврат;
	КонецЕсли;
...
Функция ВидКонтролирующегоОргана(Знач ИмяОтчета) Экспорт
	...
	Иначе
		...
		ИначеЕсли СтрНайти(ИмяОтчета, "ИсполнениеКонтрактовГОЗ") > 0 
			...
			Или СтрНайти(ИмяОтчета, "ГОЗФорма2") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма2д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма3д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма4") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма5") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма6") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма7") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма9") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма11") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма13") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма14") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма16") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма19д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма20д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма21") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма23") > 0 Тогда
...
Процедура ПолучитьСведенияОПоказателяхУведомления(Форма, ПараметрыОтчета = Неопределено) Экспорт
	...
	// ИнтеграцияСОблачнойОтчетностью
	//
	Если ДокументооборотСКОПовтИсп.ЭтоРежимОблачной1СО() И Не ДокументооборотСКОПовтИсп.ЭтоБизКуб() Тогда
		Попытка
			ЕстьПоддержкаЗаполнения = Форма.РелизованаПоддержкаЗаполненияВОблачнойОтчетности();
			ЕстьПоддержкаРасшифровки = Форма.РелизованаПоддержкаРасшифровкиВОблачнойОтчетности();
			Если ЕстьПоддержкаЗаполнения Или ЕстьПоддержкаРасшифровки Тогда
				ВставитьПоказательВСтруктуру(АвтозаполняемыеПоказателиОтчета, 
					"ПоддержкаЗаполненияВОблачнойОтчетности", ЕстьПоддержкаЗаполнения, ЕстьПоддержкаРасшифровки, Истина);
			КонецЕсли;
		Исключение
	...
	Если ТаблицаРасшифровки = Неопределено Тогда
		...
		Если РегламентированнаяОтчетностьВызовСервера.ВозможноПрисоединениеФайловКОбъекту(СохраненныйДокумент) Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				КаталогРазархивирования = ФайловаяСистема.СоздатьВременныйКаталог();
...
Процедура ЗапретРедактированияОткрытогоОтчета(Форма, МассивНедоступных)
	...
	ИменаПолейТабличногоДокумента = СтрРазделить("ТабличныйДокумент;ПолеТабличногоДокументаФормаОтчета;ТабличныйДокументУчетаНДС", ";");
...
Процедура РазрешитьРедактированиеРеквизитовОтчета(Форма) Экспорт
	...
	ИменаПолейТабличногоДокумента = СтрРазделить("ТабличныйДокумент;ПолеТабличногоДокументаФормаОтчета;ПредставлениеУведомления;ТабличныйДокументУчетаНДС", ";");
...
Функция МакетыВАрхиве(ИмяФормы, ИмяМакетаАрхива) Экспорт
	...
	Для Каждого Элт Из ЧтениеZipФайла.Элементы Цикл
		...
		ИначеЕсли НРег(Элт.Расширение) = "tif" Тогда 
			//@skip-check 1c-fresh-audit
			СоответствиеИспользуемыхМакетов.Вставить(
				Элт.ИмяБезРасширения, Новый ДвоичныеДанные(ИмяФайлаДляРаспаковки));
		КонецЕсли;
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ИмяФайлаДляРаспаковки);
	КонецЦикла;
