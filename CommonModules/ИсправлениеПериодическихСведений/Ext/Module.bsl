...
// Перемещает все данные из Испр-регистра в основной регистр.
// Используется для отказа от использования Испр регистра при обновлении ИБ.
Функция ПереместитьДанныеРегистраИспр(ИмяРегистраИспр, ИмяРегистра) Экспорт
	...
	Оператор.Источники.Добавить("РегистрСведений." + ИмяРегистраИспр, "РегистрИспр");
	Оператор.Источники.Добавить("Документ.СторнированиеНачислений", "СторнированиеНачислений");
	
	Выражение = Оператор.ВыбираемыеПоля.Добавить("РегистрИспр.РегистраторИзмерение");
	Колонка = ПакетЗапросов.Колонки.Найти(Выражение);
	Колонка.Псевдоним = "ДокументИсправление";
	
	Выражение = Оператор.ВыбираемыеПоля.Добавить("ЕСТЬNULL(РегистрИспр.РегистраторИзмерение.ИсправленныйДокумент, СторнированиеНачислений.СторнируемыйДокумент)");
	Колонка = ПакетЗапросов.Колонки.Найти(Выражение);
	Колонка.Псевдоним = "ИсправляемыйДокумент";
	...
	ОбработкаЗавершена = Выборка.Количество() < Оператор.КоличествоПолучаемыхЗаписей;
	...
	Пока Выборка.Следующий() Цикл
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений." + ИмяРегистраИспр);
		ЭлементБлокировки.УстановитьЗначение("РегистраторИзмерение", Выборка.ДокументИсправление);
		Блокировка.Заблокировать();
		
		НаборЗаписейИсходный = РегистрыСведений[ИмяРегистраИспр].СоздатьНаборЗаписей();
		НаборЗаписейИсходный.Отбор.РегистраторИзмерение.Установить(Выборка.ДокументИсправление);
		НаборЗаписейИсходный.Прочитать();
		Если НаборЗаписейИсходный.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		НаборЗаписейЦелевой = РегистрыСведений[ИмяРегистра].СоздатьНаборЗаписей();
		НаборЗаписейЦелевой.Отбор.Регистратор.Установить(Выборка.ИсправляемыйДокумент);
		Для Каждого Строка Из НаборЗаписейИсходный Цикл
			НоваяСтрока = НаборЗаписейЦелевой.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
			НоваяСтрока.Регистратор = Выборка.ИсправляемыйДокумент;
			Если ЭтоПериодическийРегистр Тогда
				НоваяСтрока.Период = Строка.ПериодИзмерение;
			КонецЕсли;
		КонецЦикла;
		НаборЗаписейИсходный.Очистить();
		НаборЗаписейИсходный.ДополнительныеСвойства.Вставить("ОтключитьБизнесЛогикуПриЗаписи", Истина);
		НаборЗаписейИсходный.Записать();
		
		НаборЗаписейЦелевой.ДополнительныеСвойства.Вставить("ОтключитьБизнесЛогикуПриЗаписи", Истина);
		НаборЗаписейЦелевой.Записать();
