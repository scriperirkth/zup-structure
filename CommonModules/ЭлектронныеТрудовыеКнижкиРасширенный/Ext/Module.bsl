		...
		Если СтрокаТабличнойЧасти[ИмяРеквизитаДатаМероприятия] >= '20210701' Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ТрудоваяФункция) Тогда
				...
				КодПоОКЗ = ЭлектронныеТрудовыеКнижкиПовтИсп.КодПоОКЗТрудовойФункции(
					СтрокаТабличнойЧасти.ТрудоваяФункция, СтрокаТабличнойЧасти[ИмяРеквизитаДатаМероприятия]);
...
Функция ДатаНомерИсправленногоДокументаОснования(ИсправленныйДокумент, НомерПриказа) Экспорт
	...
	ПервыйДокумент = ИсправлениеДокументовЗарплатаКадры.ПервыйДокументЦепочкиИсправлений(ИсправленныйДокумент);
	ЗначенияПервого = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПервыйДокумент, "Номер,Дата");
	
	ДатаНомерДокумента = Новый Структура("НомерДокументаОснования, ДатаДокументаОснования");
	ДатаНомерДокумента.ДатаДокументаОснования = ЗначенияПервого.Дата;
	ДатаНомерДокумента.НомерДокументаОснования = ЗарплатаКадрыОтчеты.НомерНаПечать(ЗначенияПервого.Номер, НомерПриказа);
...
Процедура ЗаполнитьКодыКлассификатора2026годаВТрудовыхФункциях(ПараметрыОбновления) Экспорт
	...
	Запрос.Текст =
		...
		|	ТрудовыеФункции.КодПоОКЗ КАК КодПоОКЗ,
		|	ТрудовыеФункции.ОКПДТРКод КАК ОКПДТРКод,
		|	ТрудовыеФункции.ОКПДТРКЧ КАК ОКПДТРКЧ
		|ИЗ
		|	Справочник.ТрудовыеФункции КАК ТрудовыеФункции
		|ГДЕ
		|	НЕ ТрудовыеФункции.Ссылка В (&МассивОбновленных)
		|	И ТрудовыеФункции.ОКПДТРКод <> """"
		|	И ТрудовыеФункции.ОКПДТР2026Код = """"";
	...
	КлассификаторXML = ПолучитьОбщийМакет("КлассификаторОКПДТРДолжности").ПолучитьТекст();
	КлассификаторОКПДТРДолжности = ОбщегоНазначения.ПрочитатьXMLВТаблицу(КлассификаторXML).Данные;
	
	КлассификаторXML = ПолучитьОбщийМакет("КлассификаторОКПДТР2026Должности").ПолучитьТекст();
	КлассификаторОКПДТРДолжности2026 = ОбщегоНазначения.ПрочитатьXMLВТаблицу(КлассификаторXML).Данные;
	
	КлассификаторXML = ПолучитьОбщийМакет("КлассификаторОКПДТРПрофессии").ПолучитьТекст();
	КлассификаторОКПДТРПрофессии = ОбщегоНазначения.ПрочитатьXMLВТаблицу(КлассификаторXML).Данные;
	
	КлассификаторXML = ПолучитьОбщийМакет("КлассификаторОКПДТР2026Профессии").ПолучитьТекст();
	КлассификаторОКПДТРПрофессии2026 = ОбщегоНазначения.ПрочитатьXMLВТаблицу(КлассификаторXML).Данные;
	...
	Пока Выборка.Следующий() Цикл
		...
		Код94 = Выборка.ОКПДТРКод + Выборка.ОКПДТРКЧ;
		Если Лев(Выборка.ОКПДТРКод, 1) = "1" Тогда
			СтрокаКлассификатора = КлассификаторОКПДТРПрофессии.Найти(Код94, "Code");
		Иначе
			СтрокаКлассификатора = КлассификаторОКПДТРДолжности.Найти(Код94, "Code");
		КонецЕсли;
		...
		Если Лев(Код94, 1) = "1" Тогда
			СтрокиНовогоКлассификатора = КлассификаторОКПДТРПрофессии2026.НайтиСтроки(
				Новый Структура("Name,OKZ", СтрокаКлассификатора.Name, СтрокаКлассификатора.OKZ));
			Если Не ЗначениеЗаполнено(СтрокиНовогоКлассификатора) И СтрокаКлассификатора.OKZ = Выборка.КодПоОКЗ Тогда
				СтрокиНовогоКлассификатора = КлассификаторОКПДТРПрофессии2026.НайтиСтроки(
					Новый Структура("Code94", Код94));
			КонецЕсли;
		Иначе
			СтрокиНовогоКлассификатора = КлассификаторОКПДТРДолжности2026.НайтиСтроки(
				Новый Структура("Name,OKZ", СтрокаКлассификатора.Name, СтрокаКлассификатора.OKZ));
			Если Не ЗначениеЗаполнено(СтрокиНовогоКлассификатора) И СтрокаКлассификатора.OKZ = Выборка.КодПоОКЗ Тогда
				СтрокиНовогоКлассификатора = КлассификаторОКПДТРДолжности2026.НайтиСтроки(
					Новый Структура("Code94", Код94));
		...
		Если Не ЗначениеЗаполнено(СтрокиНовогоКлассификатора) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "Справочник.ТрудовыеФункции", "Ссылка", Выборка.Ссылка) Тогда
			Продолжить;
		КонецЕсли;
		
		ОбъектСправочника = Выборка.Ссылка.ПолучитьОбъект();
		
		ОбъектСправочника.ОКПДТР2026Код = СтрокиНовогоКлассификатора[0].Code;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокиНовогоКлассификатора[0], "Category") Тогда
			ОбъектСправочника.ОКПДТР2026Категория = СтрокиНовогоКлассификатора[0].Category;
		Иначе
			ОбъектСправочника.ОКПДТР2026Категория = "4";
		КонецЕсли;
		ОбъектСправочника.КодПоОКЗ2026 = Справочники.КлассификаторЗанятий.НайтиПоКоду(СтрокиНовогоКлассификатора[0].OKZ);
...
Функция ТрудовыеФункцииПоКодуКлассификатора(КодКлассификатора, ПутьКМакету) Экспорт
	
	КлассификаторXML = ПолучитьОбщийМакет(ПутьКМакету).ПолучитьТекст();
	КлассификаторОКПДТР = ОбщегоНазначения.ПрочитатьXMLВТаблицу(КлассификаторXML).Данные;
	
	НайденнаяСтрока = КлассификаторОКПДТР.Найти(КодКлассификатора, "Code");
	Если НайденнаяСтрока <> Неопределено Тогда
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ОКПДТР2026Код", КодКлассификатора);
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НайденнаяСтрока, "Category") Тогда
			Запрос.УстановитьПараметр("ОКПДТР2026Категория", НайденнаяСтрока.Category);
		Иначе
			Запрос.УстановитьПараметр("ОКПДТР2026Категория", Неопределено);
		КонецЕсли;
		КодПоОКЗ2026 = Справочники.КлассификаторЗанятий.НайтиПоКоду(НайденнаяСтрока.OKZ);
		Запрос.УстановитьПараметр("КодПоОКЗ2026", КодПоОКЗ2026);
		
		Запрос.Текст =
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	ТрудовыеФункции.Ссылка КАК Ссылка,
			|	ТрудовыеФункции.ПометкаУдаления КАК ПометкаУдаления
			|ИЗ
			|	Справочник.ТрудовыеФункции КАК ТрудовыеФункции
			|ГДЕ
			|	ТрудовыеФункции.ОКПДТР2026Код = &ОКПДТР2026Код
			|	И (&ОКПДТР2026Категория = НЕОПРЕДЕЛЕНО
			|			ИЛИ ТрудовыеФункции.ОКПДТР2026Категория = &ОКПДТР2026Категория)
			|	И ТрудовыеФункции.КодПоОКЗ2026 = &КодПоОКЗ2026
			|
			|УПОРЯДОЧИТЬ ПО
			|	ПометкаУдаления,
			|	Ссылка";
		...
		ОбъектСправочника.Наименование = НайденнаяСтрока.Name;
		ОбъектСправочника.ОКПДТР2026Код = КодКлассификатора;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НайденнаяСтрока, "Category") Тогда
			ОбъектСправочника.ОКПДТР2026Категория = НайденнаяСтрока.Category;
		Иначе
			ОбъектСправочника.ОКПДТР2026Категория = 4;
		КонецЕсли;
		ОбъектСправочника.КодПоОКЗ2026 = КодПоОКЗ2026;
...
Процедура ЗаполнитьКодыКатегорияТрудовыхФункций(ПараметрыОбновления) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ ТрудовыеФункции.Ссылка В (&МассивОбновленных)
		|	И ТрудовыеФункции.ОКПДТР2026Код ПОДОБНО ""1%""
		|	И ТрудовыеФункции.ОКПДТР2026Категория = ""3""";
