...
Функция КаталогДляРаботыСОбновлениямиКонфигурации() Экспорт
...
//  Строка
//
Функция КаталогДляРаботыСИсправлениями(ЭтоWindowsКлиент) Экспорт
...
Функция КаталогИндексаФайлов()
	
	Возврат КаталогДляРаботыСОбновлениямиКонфигурации()
		+ "FileIndex"
...
Функция НовыйКонтекстПолученияИУстановкиОбновлений(Параметры) Экспорт
	...
	Если Результат.ОбновленияКонфигурации.Количество() > 0 Тогда
		...
		Для Каждого ТекОбновление Из Результат.ОбновленияКонфигурации Цикл
			...
			ТекОбновление.Вставить("КаталогДистрибутива", КаталогШаблонов + ТекОбновление.ПодкаталогШаблонов);
...
Функция ОбновлениеКонфигурацииПолучено(Обновление, Контекст) Экспорт
	...
	Иначе
		...
		Попытка
			...
			ПолноеИмяИндексногоФайла = КаталогИндексаФайлов() + Обновление.ИмяИндексногоФайла;
...
Процедура СоздатьКаталогиДляПолученияОбновления(Обновление, Контекст) Экспорт
	...
	ВременныйКаталогОбновленийКонфигурации = КаталогДляРаботыСОбновлениямиКонфигурации();
	Попытка
	...
	КаталогИндексаФайлов = КаталогИндексаФайлов();
	Попытка
	...
	Если Обновление.ФорматФайлаОбновления <> "zip" Тогда
		...
		// Только cfu-файл.
		Попытка
			Если ПутьККаталогуИзменен(Обновление.КаталогCFUФайлаВКаталогеДистрибутивов, КаталогШаблонов) Тогда
				ВызватьИсключение НСтр("ru = 'Ошибка в адресе каталога дистрибутива.'");
			КонецЕсли;
	...
	Иначе
		...
		Попытка
			Если ПутьККаталогуИзменен(Обновление.КаталогДистрибутива, КаталогШаблонов) Тогда
				ВызватьИсключение НСтр("ru = 'Ошибка в адресе каталога дистрибутива.'");
			КонецЕсли;
...
Процедура ЗавершитьПолучениеОбновления(Обновление, Контекст) Экспорт
	...
	Если Обновление.ФорматФайлаОбновления = "zip" Тогда
		Попытка
			КаталогОбновлений = КаталогДляРаботыСОбновлениямиКонфигурации();
			Если ПутьККаталогуИзменен(Обновление.ИмяПолученногоФайла, КаталогОбновлений) Тогда
				ВызватьИсключение НСтр("ru = 'Ошибка в адресе каталога дистрибутива.'");
			КонецЕсли;
...
Функция ПутьККаталогуИзменен(Путь, ПроверяемыйПуть) Экспорт
	
	Файл = Новый Файл(Путь);
	ПроверяемыйФайл = Новый Файл(ПроверяемыйПуть);
	
	Возврат Не СтрНачинаетсяС(Файл.ПолноеИмя, ПроверяемыйФайл.ПолноеИмя);
