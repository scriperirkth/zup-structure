...
Функция ВсеФизическиеЛицаДокументовВыгружаются(РеквизитыОбъектов)
	...
	Для Каждого РеквизитыОбъекта Из РеквизитыОбъектов Цикл
		...
		Если ТипЗнч(РеквизитыОбъекта.Значение.ФизическоеЛицо) = Тип("Массив") Тогда
			Если РеквизитыОбъекта.Значение.ФизическоеЛицо.Количество() <> РеквизитыОбъекта.Значение.ВыгружаемостьСотрудников.Количество() Тогда
				Для Каждого ФизическоеЛицо Из РеквизитыОбъекта.Значение.ФизическоеЛицо Цикл
					Если РеквизитыОбъекта.Значение.ВыгружаемостьСотрудников.Получить(ФизическоеЛицо) <> Истина Тогда
						ФизическиеЛицаДокументаБезКабинетов.Добавить(ФизическоеЛицо);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		Иначе
			Если РеквизитыОбъекта.Значение.ВыгружаемостьСотрудников.Получить(РеквизитыОбъекта.Значение.ФизическоеЛицо) <> Истина Тогда
				ФизическиеЛицаДокументаБезКабинетов.Добавить(РеквизитыОбъекта.Значение.ФизическоеЛицо);
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ФизическиеЛицаДокументаБезКабинетов) Тогда
			ФизическиеЛицаБезКабинетов.Вставить(РеквизитыОбъекта.Ключ, ФизическиеЛицаДокументаБезКабинетов);
		КонецЕсли;
	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.Значение = "ПередатьТолькоСКабинетами"
			...
			Если ДополнительныеПараметры.Свойство("ФизическиеЛицаБезКабинетов")
				...
				ТекстовыйДокумент.ДобавитьСтроку(НСтр("ru = 'Документы, сотрудники которых не пользуются личными кабинетами:'"));
				Для Каждого ДанныеДокументов Из ДополнительныеПараметры.ФизическиеЛицаБезКабинетов Цикл
					ТекстовыйДокумент.ДобавитьСтроку(ДанныеДокументов.Ключ);
					Для Каждого ФизическоеЛицо Из ДанныеДокументов.Значение Цикл
						ТекстовыйДокумент.ДобавитьСтроку(СтрШаблон(" - %1", ФизическоеЛицо));
					КонецЦикла;
				КонецЦикла;
				ТекстовыйДокумент.Показать(НСтр("ru = 'Протокол проверки подключения к личному кабинету сотрудников'"));
...
Процедура ПодписатьФайлы(ПодписываемыеФайлы, ОповещениеЗавершения, ФормаВладелец, ЦельПодписания, НастройкиПодписания = Неопределено) Экспорт
	...
	ОтправляемыеБезПодписания = Новый Массив;
			...
			Если НастройкиОрганизации <> Неопределено Тогда
				...
				Если НастройкиИдентификатора <> Неопределено Тогда
					...
					Если НастройкиТипаОбъекта <> Неопределено Тогда
						Если Не НастройкиТипаОбъекта.Инициатор И НастройкиТипаОбъекта.ОтветственныеЛица.Количество() = 0 Тогда
							ОтправляемыеБезПодписания.Добавить(ДанныеФайла);
						Иначе
	...
	Если ЗначениеЗаполнено(ОтправляемыеБезПодписания) Тогда
		ОтправляемыеПрисоединенныеФайлы = Новый Массив;
		Для Каждого ИсключениеИзПодписания Из ОтправляемыеБезПодписания Цикл
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(
				ПодписываемыеФайлы, ИсключениеИзПодписания);
			ОтправляемыеПрисоединенныеФайлы.Добавить(ИсключениеИзПодписания.ФайлОбъекта);
		КонецЦикла;
		ЭДОБЗКВызовСервера.ЗапланироватьДействияСПечатнымиФормами(ОтправляемыеПрисоединенныеФайлы, ЦельПодписания);
	КонецЕсли;
...
Процедура ПодписатьПоНастройкамДокумент(УправляемаяФорма, ДокументЭДО, Идентификатор, ЦельПодписания, ОповещениеЗавершения, ТребуетсяПодписьОрганизации = Ложь) Экспорт
	...
	Если ТребуетсяПодписьОрганизации Тогда
		ПараметрыОткрытия.Вставить("ТребуетсяПодписьОрганизации", Истина);
	КонецЕсли;
...
Функция ПараметрыОткрытияФормыНастройкиПодписания(ОбъектыПечати, Идентификаторы, ЦельПодписания, ВыполнитьНастройку = Ложь)
	...
	ПараметрыОткрытия.Вставить("ТребуетсяПодписьОрганизации", Ложь);
