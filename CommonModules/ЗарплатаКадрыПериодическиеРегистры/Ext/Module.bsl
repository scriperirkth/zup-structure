...
//
Процедура СформироватьДвиженияИнтервальногоРегистраПоИзменениям(ИмяРегистра, НаборЗаписей, Замещение) Экспорт
	...
	Если ТаблицаИзменившихсяДанныхНабора.Количество() = 0
		И Не РегистрыБЗК.ЭтоУдалениеНабораЗаписей(Замещение) Тогда
		Возврат;
	КонецЕсли;
	...
	Если РегистрыБЗК.ЭтоУдалениеНабораЗаписей(Замещение) Тогда
		ПараметрыПостроения.ТаблицаНабора.Очистить();
	КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УстановитьСпособФормированияИнтервальныхРегистров");
КонецПроцедуры
...
Процедура ПеренестиВозвратныйРегистрВИнтервальныйРегистрСведений(ИмяРегистра, ПараметрыОбновления = Неопределено) Экспорт
	...
	Если ОписаниеРегистра.ЕстьВозвратныеСобытия Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеВозвратныеСобытия",
			"ВЫБОР
			|	КОГДА РегистрСведений.ДействуетДо = ДАТАВРЕМЯ(1,1,1)
			|		ТОГДА ИСТИНА
			|	ИНАЧЕ
			|		РегистрСведений.Период < РегистрСведений.ДействуетДо
			|КОНЕЦ");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеВозвратныеСобытия", "ИСТИНА");
	КонецЕсли;
	ОписаниеИнтервальногоРегистра = РегистрыСведений[ИмяРегистра].ОписаниеИнтервальногоРегистра();
...
Процедура КонтрольИзмененийПередЗаписью(НаборЗаписейРегистра, Замещение) Экспорт
	...
	СоздатьВТСтарыйНаборЗаписей(НаборЗаписейРегистра, Замещение, МенеджерВременныхТаблиц, ИмяВТСтарыйНаборРегистра(НаборЗаписейРегистра.Метаданные().Имя));
...
// Параметры:
//	НаборЗаписейРегистра - Набор записей регистра
Процедура КонтрольИзмененийПриЗаписи(НаборЗаписейРегистра, Замещение) Экспорт
	
	Если Не НаборЗаписейРегистра.ДополнительныеСвойства.Свойство("ПрежнийНабор") Тогда
		Возврат;
	КонецЕсли;
	
	ЗначенияОтбора = РегистрыБЗК.ЗначенияОтбораЗаписейНабора(НаборЗаписейРегистра, Замещение, "Регистратор");
	
	Запрос = ЗапросВТИзмененияВНаборе(НаборЗаписейРегистра.Метаданные().Имя, ЗначенияОтбора[0]);
...
Процедура СоздатьВТРегистраторыДляОбновления(ИмяРегистра, МенеджерВременныхТаблиц, ИзмеренияОтбора, НаборЗаписей, Замещение) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Не РегистрСведений.Регистратор В (&Регистраторы)
...
Процедура СоздатьВТСтарыйНаборЗаписей(НаборЗаписей, Замещение, МенеджерВременныхТаблиц, ИмяВТРезультат = "ВТСтарыйНаборЗаписей")
	
	ПрежнийНаборЗапрос = РегистрыБЗК.ЗапросНабораИзБазыДанных(НаборЗаписей, Замещение, , , ИмяВТРезультат);
	ПрежнийНаборЗапрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	ПрежнийНабор = ПрежнийНаборЗапрос.Выполнить().Выгрузить();
	НаборЗаписей.ДополнительныеСвойства.Вставить("ЭтоНовыйНабор", НЕ ПрежнийНабор.Количество()
