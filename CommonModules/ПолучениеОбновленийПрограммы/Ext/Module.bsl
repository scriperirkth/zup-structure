...
Функция ЗагрузитьИУстановитьОбновленияВФоновомРежиме(Параметры) Экспорт
	...
	КаталогИсправлений = ПолучениеОбновленийПрограммыКлиентСервер.КаталогДляРаботыСИсправлениями(
		Параметры.ЭтоWindowsКлиент);
	Для Каждого ТекИсправление Из Контекст.Исправления Цикл
		...
		Если Не ТекИсправление.Получено Тогда
			...
			Иначе
				...
				Попытка
					// Проверка что сохраняемый файл находится в ожидаемом каталоге
					Если ПолучениеОбновленийПрограммыКлиентСервер.ПутьККаталогуИзменен(ТекИсправление.ИмяПолученногоФайла, КаталогИсправлений) Тогда
						ВызватьИсключение НСтр("ru = 'Ошибка в адресе каталога хранения исправлений.'");
					КонецЕсли;
	...
	Если Контекст.ОбновленияКонфигурации.Количество() = 0
		...
		РезультатУстановки = УстановкаИУдалениеИсправлений(
			Контекст.Исправления,
			Контекст.ОтозванныеИсправления,
			Истина);
...
Процедура ЗагрузитьОбновлениеПлатформы(Контекст, Параметры, ДанныеАутентификации)
	...
	Если КаталогХраненияДистрибутивов = Неопределено
		Или Не ЭтоФайловаяИБ() Тогда
	...
	ДопПараметры = Новый Структура("ИмяФайла, Таймаут", ИмяФайла, 43200);
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		...
		Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(РезультатПолучения.Содержимое, Ложь) Тогда
			Попытка
				УдалитьФайлы(РезультатПолучения.Содержимое);
			Исключение
				ЗаписатьОшибкуВЖурналРегистрации(
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
	...
	Попытка
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				КаталогДистрибутива));
		ЧтениеZIP = Новый ЧтениеZipФайла(РезультатПолучения.Содержимое);
...
Процедура ЗагрузитьОбновлениеКонфигурации(Обновление, Контекст, Параметры, ДанныеАутентификации)
	...
	// Загрузка файла.
	ДопПараметры = Новый Структура("Таймаут", 43200);
	...
	Если ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		КаталогОбновлений = ПолучениеОбновленийПрограммыКлиентСервер.КаталогДляРаботыСОбновлениямиКонфигурации();
		Если ПолучениеОбновленийПрограммыКлиентСервер.ПутьККаталогуИзменен(Обновление.ИмяПолученногоФайла, КаталогОбновлений) Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка в адресе каталога обовлений.'");
		КонецЕсли;
		ПереместитьФайл(РезультатПолучения.Содержимое, Обновление.ИмяПолученногоФайла);
	Иначе
...
Процедура РегламентноеЗаданиеПолучениеИУстановкаИсправленийКонфигурации() Экспорт
	...
	ПричинаОстановкиЗадания = "";
	ЗагружатьАвтоматически = Константы.ЗагружатьИУстанавливатьИсправленияАвтоматически.Получить();
	Если Не ЗагружатьАвтоматически Тогда
		ПричинаОстановкиЗадания = НСтр("ru = 'Автоматическая загрузка и установка исправлений отключена в настройках.
			|Регламентное задание ПолучениеИУстановкаИсправленийКонфигурации отключено.'");
	ИначеЕсли Не ДоступнаАвтоматическаяЗагрузкаИсправлений() Тогда
		ПричинаОстановкиЗадания = НСтр("ru = 'Использование получения исправлений недоступно в текущем режиме работы.
			|Регламентное задание ПолучениеИУстановкаИсправленийКонфигурации отключено.'");
	КонецЕсли;
	
	Если Не ПустаяСтрока(ПричинаОстановкиЗадания) Тогда
		ЗаписатьИнформациюВЖурналРегистрации(ПричинаОстановкиЗадания);
		РегламентныеЗаданияСервер.УстановитьИспользованиеРегламентногоЗадания(
			Метаданные.РегламентныеЗадания.ПолучениеИУстановкаИсправленийКонфигурации,
			Ложь);
		Возврат;
