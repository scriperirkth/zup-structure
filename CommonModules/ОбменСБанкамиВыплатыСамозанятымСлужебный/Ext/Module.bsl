...
//			** СуммаДокумента - Число - Сумма по документу Выплаты самозанятым
//			** Основной - Булево
...
Функция СформироватьЭДПоВыплатамСамозанятым(СсылкаНаОбъект, НастройкиОбменаЭД, ВидЭД)    Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СуммаДокумента", СсылкаНаОбъект.Метаданные()) Тогда
		РеквизитыДокумента = РеквизитыДокумента + ", СуммаДокумента";
	КонецЕсли;

	ЗначенияРеквизитовДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаОбъект, РеквизитыДокумента);

	СтруктураЭД.Вставить("НомерДокументаОтправителя", ПечатныйНомерДокумента);
	СтруктураЭД.Вставить("ДатаДокументаОтправителя", ЗначенияРеквизитовДокумента.Дата);
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СуммаДокумента", СсылкаНаОбъект.Метаданные()) Тогда
		СтруктураЭД.Вставить(
			"СуммаДокумента", ЗначенияРеквизитовДокумента.СуммаДокумента);
	КонецЕсли;
...
Функция ЗагрузитьРеестрЧековИзВременногоХранилища(АдресДанныхФайла) 
	...
	СозданныйДокумент = ЗагрузитьРеестрЧековИзФайла(ДвоичныеДанныеФайла);
...
//
// Параметры:
//		ДвоичныеДанныеФайла - ДвоичныеДанные - двоичные данные, в которых содержится XML документ.
//
// Возвращаемое значение:
//		Ссылка на документ или Неопределено, если документ не создан.
//
Функция ЗагрузитьРеестрЧековИзФайла(ДвоичныеДанныеФайла)
	
	ДанныеПоТипу = ОбменСБанкамиСлужебный.ПрочитатьXDTOИзДвоичныхДанных(ДвоичныеДанныеФайла);
	
	Если ДанныеПоТипу.ЕстьОшибка Тогда
		ТекстСообщения = НСтр("ru = 'Неверный формат файла'");
		Операция = НСтр("ru = 'Загрузка реестра чеков из банка'");
		ЭлектронноеВзаимодействие.ОбработатьОшибку(Операция, ТекстСообщения, ТекстСообщения, "ОбменСБанками");
		Возврат Неопределено;
	Иначе
		ОбъектXDTOВременный = ДанныеПоТипу.Значение;
		Пакет = ФабрикаXDTO.Пакеты.Получить(
			Метаданные.ПакетыXDTO.ФорматОбменаСБанкамиПоВыплатамСамозанятым.ПространствоИмен);
