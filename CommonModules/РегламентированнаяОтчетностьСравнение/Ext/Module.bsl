...
Функция СравнениеОтчетов(ФормаОтчета, ПараметрыОтчета, ДанныеДляСравнения, ДеревоДляСравнения) Экспорт
	...
	РезультатДлительнойОперации = ДлительныеОперации.ВыполнитьВФоне(
		"РегламентированнаяОтчетностьСравнение.СравнениеОтчетовДлительнаяОперация",
		ПараметрыВыполнения,
		ПараметрыЗапуска);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СравнениеОтчетовДлительнаяОперация", Истина);
КонецПроцедуры
...
Функция ТаблицаДанныхМногоуровневыхРазделов(ДеревоРазделаЛевый, ДеревоРазделаПравый)
	...
	Для Каждого СтрокаДереваРазделаЛевый Из СтрокиДереваРазделаЛевый Цикл
		...
		ДополнитьДанныеСтраницыМногострочнымиПоказателями(СтрокаДереваРазделаЛевый);
	...
	Для Каждого СтрокаДереваРазделаПравый Из СтрокиДереваРазделаПравый Цикл
		...
		ДополнитьДанныеСтраницыМногострочнымиПоказателями(СтрокаДереваРазделаПравый);
	...
	Если ТипЗнч(НомерСтраницы) = Тип("Массив") Тогда // раздел многоуровневый (многостраничный)
		...
		Если ЗначениеЗаполнено(НомерСтроки) Тогда // массив не пустой - определена многоуровневая многострочность в разделе
			Если ТипЗнч(НомерСтроки) = Тип("Число") Тогда
				...
				НомерСтроки = Новый Массив(1);
				НомерСтроки[0] = НомСтроки;
			КонецЕсли;
...
Функция ДанныеСтраницыМУР(ДеревоРаздела, НомерСтраницы)
	...
	ДополнитьДанныеСтраницыМногострочнымиПоказателями(ВетвьДерева);
...
Процедура ДополнитьДанныеСтраницыМногострочнымиПоказателями(СтрокаДерева)
	
	Если ТипЗнч(СтрокаДерева.ДанныеМногострочныхЧастей) = Тип("Структура") Тогда
		Для Каждого ДанныеМногострочныхЧастейКлючИЗначение Из СтрокаДерева.ДанныеМногострочныхЧастей Цикл
			ДеревоМнЧ = ДанныеМногострочныхЧастейКлючИЗначение.Значение;
			
			Если ТипЗнч(ДеревоМнЧ) = Тип("ДеревоЗначений") Тогда 
				Для Каждого ДеревоМнЧСтрока Из ДеревоМнЧ.Строки Цикл
					НомерСтроки = ДеревоМнЧ.Строки.Индекс(ДеревоМнЧСтрока) + 1;
					
					ДополнитьДанныеСтраницыМногострочнымиПоказателями(ДеревоМнЧСтрока);
					
					Для Каждого ДанныеМнЧКлючИЗначение Из ДеревоМнЧСтрока.Данные Цикл
						ИмяПоказателя = ДанныеМнЧКлючИЗначение.Ключ;
						ЗнчПоказателя = ДанныеМнЧКлючИЗначение.Значение;
						
						ЧастиИмениПоказателя = СтрРазделить(ИмяПоказателя, "_", Ложь);
						ЧастиИмениПоказателя.Вставить(1, Формат(НомерСтроки, "ЧГ="));
						
						СтрокаДерева.Данные.Вставить(СтрСоединить(ЧастиИмениПоказателя, "_"), ЗнчПоказателя);
					КонецЦикла;
