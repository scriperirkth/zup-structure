...
//   которая будет вызвана после создания настройки обмена с банком, не вызывается если настройка обмена уже существует.
//   Параметры процедуры:
//    * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - созданная настройка обмена с банком.
//    * ДополнительныеПараметры - Произвольный - значение, которое было указано при создании объекта ОписаниеОповещения.
//  ДополнительныеПараметры - Неопределено, Структура - дополнительные параметры для управления поведением процедуры.
//   Возможные ключи:
//   * ТолькоОткрытьПомощник - Булево - Необязательный. Если Истина, то открывает помощник создания настройки даже
//                             если настройка обмена уже существует.
//   * ТребуетсяПолныйОбмен - Булево - Необязательный. Признак того, что требуется полный обмен с банком.
//
Процедура ОткрытьСоздатьНастройкуОбмена(
	Организация,
	Банк,
	...
	ТолькоОткрытьПомощник = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры,
		"ТолькоОткрытьПомощник", Ложь);
	
	Если ЗначениеЗаполнено(ТекущаяНастройка) И Не ТолькоОткрытьПомощник Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ТребуетсяПолныйОбмен", Ложь) Тогда
		ПараметрыФормы.Вставить("ТребуетсяПолныйОбмен", ДополнительныеПараметры.ТребуетсяПолныйОбмен);
	КонецЕсли;
...
Процедура ПолучитьВыписку(Оповещение, БанковскийСчет, ДатаНачала, Знач ДатаОкончания) Экспорт
	...
	ОбработатьПолучениеВыпискиСПроверкойДат(Оповещение, ПараметрыОбмена, ДатаНачала, ДатаОкончания);
...
//  ДатаНачала - Дата - начало периода запроса.
//  ДатаОкончания - Дата - окончание периода запроса.
//  Банк - Неопределено, ОпределяемыйТип.БанкОбменСБанками - если банк указан, то выписка запрашивается по этому банку.
//
Процедура ПолучитьВыпискуПоНастройкеОбмена(
	Оповещение,
	НастройкаОбмена,
	ДатаНачала,
	Знач ДатаОкончания,
	Банк = Неопределено) Экспорт
	
	ПараметрыОбмена = ОбменСБанкамиСлужебныйВызовСервера.ПараметрыОбменаПоНастройкеОбмена(НастройкаОбмена);
	
	Если Банк <> Неопределено И ПараметрыОбмена <> Неопределено Тогда
		ПараметрыОбмена.РеквизитыСчета.Банк = Банк;
	КонецЕсли;
...
//
Процедура ПолучитьВыпискуПоПараметрам(Оповещение, ПараметрыВыписки) Экспорт
	
	ПараметрыОбмена = ОбменСБанкамиСлужебныйВызовСервера.ПараметрыОбменаПоПараметрамВыписки(ПараметрыВыписки);
	ОбработатьПолучениеВыпискиСПроверкойДат(Оповещение, ПараметрыОбмена, ПараметрыВыписки.ДатаНачала,
		ПараметрыВыписки.ДатаОкончания);
...
Процедура ОбработатьПолучениеВыписки(Оповещение, ПараметрыОбмена, ДатаНачала, ДатаОкончания)
	...
	ПараметрыЗапроса.БанкВладелец = ПараметрыОбмена.РеквизитыСчета.Банк;
	ПараметрыЗапроса.НастройкаОбмена = НастройкаОбмена;
	ПараметрыЗапроса.Организация = ПараметрыОбмена.РеквизитыСчета.Организация;
	// Для асинхронного обмена используется ключ "БанкВладелец".
	// Ключ "Банк" в структуре "ПараметрыЗапроса" оставлен для совместимости с остальными вариантами обмена.
...
Процедура ОбработатьПолучениеВыпискиСПроверкойДат(Оповещение, ПараметрыОбмена, ДатаНачала, ДатаОкончания)
