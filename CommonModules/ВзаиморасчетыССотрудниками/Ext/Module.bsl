...
// Возвращает суммы начисленной зарплаты в разрезе документов, физических лиц и периодов взаиморасчетов.
//
// Параметры:
...
//
Функция НачисленнаяЗарплатаФизическихЛицПоДокументам(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование) Экспорт
	...
	Запрос.Текст =
		...
		|	ФизлицаДокументы.Организация КАК Организация,
		...
		|	ФизлицаДокументы.ДокументОснование КАК ДокументОснование,
		|	ФизлицаДокументы.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
		|	ЕСТЬNULL(ЗарплатаКВыплате.СуммаКВыплате, 0) КАК Сумма
		...
		|		ПО ФизлицаДокументы.ДокументОснование = ЗарплатаКВыплате.ДокументОснование
		|			И ФизлицаДокументы.Организация = ЗарплатаКВыплате.Организация
		|			И ФизлицаДокументы.ФизическоеЛицо = ЗарплатаКВыплате.ФизическоеЛицо
		|			И ФизлицаДокументы.ПериодВзаиморасчетов = ЗарплатаКВыплате.ПериодВзаиморасчетов
		|			И (ЗарплатаКВыплате.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
		...
		|	ЕСТЬNULL(-ЗарплатаКВыплате.СуммаКВыплате, 0)
		|ИЗ
		|	ФизлицаДокументы КАК ФизлицаДокументы
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТНачисленнаяЗарплата.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТНачисленнаяЗарплата.ДокументОснование КАК ДокументОснование,
		|	ВТНачисленнаяЗарплата.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
		...
		|	ВТНачисленнаяЗарплата.ДокументОснование,
		|	ВТНачисленнаяЗарплата.ПериодВзаиморасчетов";
	...
	Если ДетализироватьПоСтатьям Тогда
		Если РассчитыватьВзысканныеСуммыПропорциональноВыплате Тогда
			Если ЗначениеЗаполнено(Финансирование.СтатьяРасходов) Тогда
				ОтборПоСтатьеРасходов = "(ЗарплатаКВыплате.СтатьяРасходов = &СтатьяРасходов)";
			...
			Если ЗначениеЗаполнено(Финансирование.СтатьяФинансирования) Тогда
				ОтборПоСтатьеФинансирования = "(ЗарплатаКВыплате.СтатьяФинансирования = &СтатьяФинансирования)";
...
// Возвращает суммы начисленного аванса в разрезе документов, физических лиц и периодов взаиморасчетов.
//
// Параметры:
//   ФизлицаДокументы 									- ТаблицаЗначений - таблица с колонками:
//     * ФизическоеЛицо - СправочникСсылка.ФизическиеЛица.
...
//
Функция НачисленныйАвансФизическихЛицПоДокументам(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование) Экспорт
	...
	Запрос.Текст = 
		...
		|	ФизлицаДокументы КАК ФизлицаДокументы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗарплатаКВыплатеАвансом КАК ЗарплатаКВыплате
		|		ПО ФизлицаДокументы.ДокументОснование = ЗарплатаКВыплате.Регистратор
		|			И ФизлицаДокументы.Организация = ЗарплатаКВыплате.Организация
		|			И ФизлицаДокументы.ФизическоеЛицо = ЗарплатаКВыплате.ФизическоеЛицо
		|			И ФизлицаДокументы.ПериодВзаиморасчетов = ЗарплатаКВыплате.Период
		|			И (ЗарплатаКВыплате.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
		|			И (&ОтборПоСтатьеРасходов)
		|			И (&ОтборПоСтатьеФинансирования)
		|
...
Функция КорректировкиВыплаты(Организация, Знач Период, ФизическиеЛица, ИгнорируемыеРегистраторы = Неопределено, ИменаКолонок = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ ВТРазницаДолгаПередСотрудникомИВзаиморасчетами
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РазницаДолгаПередСотрудникомИВзаиморасчетами.ФизическоеЛицо КАК ФизическоеЛицо
	|ИЗ
	|	ВТРазницаДолгаПередСотрудникомИВзаиморасчетами КАК РазницаДолгаПередСотрудникомИВзаиморасчетами";
	...
	Если РезультатЗапроса.Количество() = 0 Тогда
		...
		|ИЗ 
		|	ВТРазницаДолгаПередСотрудникомИВзаиморасчетами";
	...
	Иначе
		...
		|	ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки) КАК НалоговаяБаза,
		|	СУММА(ВЫБОР
		|			КОГДА НачисленияУдержанияПоСотрудникам.НачислениеУдержание = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.НДФЛКЗачетуРКСН)
		...
		|ГДЕ
		|	НачисленияУдержанияПоСотрудникам.НачислениеУдержание В (ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.НДФЛКЗачетуРКСН), ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.НДФЛЗачтеноРКСН))
		|	И НачисленияУдержанияПоСотрудникам.ФизическоеЛицо В(&ФизическиеЛица)
		|	И НЕ НачисленияУдержанияПоСотрудникам.Регистратор В (&ИгнорируемыеРегистраторы)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|ВЫБРАТЬ
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.ФизическоеЛицо КАК ФизическоеЛицо,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.Сотрудник КАК Сотрудник,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.Подразделение КАК Подразделение,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.СтатьяФинансирования КАК СтатьяФинансирования,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.СтатьяРасходов КАК СтатьяРасходов,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.ВидДоходаИсполнительногоПроизводства КАК ВидДоходаИсполнительногоПроизводства,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.СуммаКВыплате КАК СуммаКВыплате,
		|	РазницаДолгаПередСотрудникомИВзаиморасчетами.СуммаКорректировки КАК СуммаКорректировки,
		|	ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ОплатаТруда) КАК НалоговаяБаза
		|ИЗ
		|	ВТРазницаДолгаПередСотрудникомИВзаиморасчетами КАК РазницаДолгаПередСотрудникомИВзаиморасчетами";
		Запрос.УстановитьПараметр("ФизическиеЛица",РезультатЗапроса.ВыгрузитьКолонку("ФизическоеЛицо"));
		
		РезультатЗапроса = Запрос.ВыполнитьПакет();
		
		КорректировкиВыплатыРКСН = РезультатЗапроса[0].Выгрузить();
		КорректировкиВыплаты = РезультатЗапроса[1].Выгрузить();
		
		Если КорректировкиВыплатыРКСН.Количество() > 0 Тогда
			СтруктураОтбораФизическоеЛицо = Новый Структура("ФизическоеЛицо,НалоговаяБаза",,
															Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда);
			
			Для Каждого СтрокаКорректировкаРКСН Из КорректировкиВыплатыРКСН Цикл
				
				СтруктураОтбораФизическоеЛицо.ФизическоеЛицо = СтрокаКорректировкаРКСН.ФизическоеЛицо;
				СтрокиКорректировки = КорректировкиВыплаты.НайтиСтроки(СтруктураОтбораФизическоеЛицо);
				СуммаКорректировкиРКСН = СтрокаКорректировкаРКСН.Сумма;
				
				Для Каждого СтрокаКорректировки Из СтрокиКорректировки Цикл
					СуммаКорректировки = СтрокаКорректировки.СуммаКорректировки;
					Если СуммаКорректировкиРКСН = СуммаКорректировки Тогда // 3 = 3
						СтрокаКорректировки.НалоговаяБаза = СтрокаКорректировкаРКСН.НалоговаяБаза;
						Прервать;
					ИначеЕсли СуммаКорректировкиРКСН < 0 И СуммаКорректировки < 0 Тогда
						Если СуммаКорректировкиРКСН > СуммаКорректировки Тогда // -3 > -5
							СтрокаКорректировки.СуммаКорректировки = СтрокаКорректировки.СуммаКорректировки - СуммаКорректировкиРКСН;
							НоваяСтрока = КорректировкиВыплаты.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока,СтрокаКорректировки,,"НалоговаяБаза");
							НоваяСтрока.СуммаКорректировки = 0;
							НоваяСтрока.СуммаВзаиморасчетов = 0;
							НоваяСтрока.СуммаКВыплате = 0;
							НоваяСтрока.НалоговаяБаза = СтрокаКорректировкаРКСН.НалоговаяБаза;
							НоваяСтрока.СуммаКорректировки = СуммаКорректировкиРКСН;
							Прервать;
						Иначе // -5 < -3
							СтрокаКорректировки.НалоговаяБаза = СтрокаКорректировкаРКСН.НалоговаяБаза;
							СуммаКорректировкиРКСН = СуммаКорректировкиРКСН - СуммаКорректировки;
						КонецЕсли;
					Иначе
						Если СуммаКорректировкиРКСН < СуммаКорректировки Тогда // 3 < 5
							СтрокаКорректировки.СуммаКорректировки = СтрокаКорректировки.СуммаКорректировки - СуммаКорректировкиРКСН;
...
Функция ПараметрыПолученияЗарплатыКВыплате() Экспорт
	...
	ПараметрыПолученияЗарплатыКВыплате.Вставить("ВидДокументаОснования",    Перечисления.ВидыДокументовОснованийВедомостейНаВыплатуЗарплаты.ПустаяСсылка());
...
Процедура СоздатьВТЗарплатаКВыплатеОграниченнаяСальдоФизлиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры) Экспорт
	...
	ВедомостьНаВыплатуЗарплаты.СоздатьВТУсловияВозвратаЗадолженности(Запрос.МенеджерВременныхТаблиц);
	
	// Остатки зарплаты к выплате по физлицам.
	// Для расчета будут исключены документы-основания, по которым указаны особые ограничения выплаты в РС УсловияВозвратаЗадолженности.
	...
	|				ИЗ
	|					ВТЗарплатаФизлиц КАК ЗарплатаФизлиц)) КАК ЗарплатаКВыплатеОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловияВозвратаЗадолженности КАК УсловияВозвратаЗадолженности
	|		ПО ЗарплатаКВыплатеОстатки.Сотрудник = УсловияВозвратаЗадолженности.Сотрудник
	|			И ЗарплатаКВыплатеОстатки.ДокументОснование = УсловияВозвратаЗадолженности.ДокументОснование
	|ГДЕ
	|	УсловияВозвратаЗадолженности.ДокументОснование ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	...
	ВедомостьНаВыплатуЗарплаты.ПрименитьОграниченияПогашенийЗадолженностей(ВременнаяТаблицаКВыплате, Параметры.ИгнорируемыеРегистраторы, Параметры.ПериодРегистрации);
	...
	ИмяКлюча = "Сотрудник,ФизическоеЛицо,Подразделение,Организация,ПериодВзаиморасчетов,СтатьяФинансирования,СтатьяРасходов,ВидДоходаИсполнительногоПроизводства,ДокументОснование";
	...
	УничтожаемыеВТ.Добавить("ВТУсловияВозвратаЗадолженности");
	ЗарплатаКадры.УничтожитьВТ(Запрос.МенеджерВременныхТаблиц, УничтожаемыеВТ);
