...
Процедура ЗаполнитьДаннымиДерево(ДеревоДляЗагрузки, ИмяФайла, РазмерТекста = 0);
	...
	Иначе
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ИмяФайла, "ИмяФайла", "ЗаполнитьДаннымиДерево");
...
Функция ПрочитатьТекстИзФайла(ИмяФайла, ЗаданнаяКодировкаТекста = Неопределено, АвтоопределениеКодировкиXML = Ложь, КоличествоСимволов = 0) Экспорт
	...
	Если ЭтоВременныйФайл Тогда
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ИмяФайлаНаДиске);
	КонецЕсли;
...
Функция СвойстваФайлаЗагрузки(ФайлЗагрузки, МаксКоличествоПрочитанныхСвойств, ЭтоXML = Истина, ЗаменятьДефисы = Ложь, ЭтоСтрока = Ложь) Экспорт
	...
	Если ЭтоXML Тогда
		...
		Попытка
			...
			Иначе
				РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
					ФайлЗагрузки, "ФайлЗагрузки", "СвойстваФайлаЗагрузки");				
				//@skip-check 1c-fresh-audit
...
Функция XDTOОбъектИзФайлаИлиСтроки(СтрокаИлиИмяФайлаXML) Экспорт
	...
	Иначе
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			СтрокаИлиИмяФайлаXML, "СтрокаИлиИмяФайлаXML", "XDTOОбъектИзФайлаИлиСтроки");		
		//@skip-check 1c-fresh-audit
...
//
Процедура ЗаписатьДанныеВXMLилиJSON(Данные, Знач ИмяФайла, ТипФайла = "xml") Экспорт
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ИмяФайла, "ИмяФайла", "ЗаписатьДанныеВXMLилиJSON");	
...
Процедура ВыполнитьМетодОбъекта(Объект, ИмяМетода, Параметры)
	...
	Если ЗначениеЗаполнено(Параметры) Тогда
		...
		Если ВРег(ИмяМетода) = ВРег("ПостОбработкаПриЗагрузкеИзФайлаВыгрузкиАЛКО") Тогда
			МодульРегламентированнаяОтчетностьАЛКО = ОбщегоНазначения.ОбщийМодуль("РегламентированнаяОтчетностьАЛКО");
			Если МодульРегламентированнаяОтчетностьАЛКО <> Неопределено Тогда
				МодульРегламентированнаяОтчетностьАЛКО.ПостОбработкаПриЗагрузкеИзФайлаВыгрузкиАЛКО(Параметры[0], Параметры[1]);
			КонецЕсли;
		КонецЕсли;
