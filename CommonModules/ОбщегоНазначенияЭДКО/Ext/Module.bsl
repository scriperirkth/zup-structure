...
//                                 возвращающая массив ссылок на присоединенные файлы из всех справочников
//                                 присоединенных файлов объекта.
//  ИсходноеИмяФайла             - Строка - исходное имя присоединенного файла, хранимое в ресурсе ИсходноеИмяФайла,
//                                 не ограничено 150 символами, как имя файла без расширения в стандартном реквизите
//                                 Наименование, при пустом значении возвращаются все файлы,
//                               - Массив, ФиксированныйМассив - несколько вариантов имени файла, для получения крупных
//                                 файлов, разбитых на несколько записей присоединенных файлов
...
//  ВозвращатьПодобные           - Булево - возвращать все файлы с именем, начинающимся на ИсходноеИмяФайла.
//
...
Функция ПрикрепленныеФайлыКОбъектуИзСправочника(
	...
	Знач ВключатьПомеченныеНаУдаление = Ложь,
	Знач ВозвращатьПодобные = Ложь) Экспорт
	...
	Для каждого ЭлементМассива Из МассивИменСправочников Цикл
		...
		ТекстЗапроса =
			...
			|	И ПрисоединенныеФайлы.ИсходноеИмяФайла"
					+ ?(ВозвращатьПодобные, " ПОДОБНО &МаскаИсходногоИмениФайла СПЕЦСИМВОЛ ""\""",
					?(ТипЗнч(ИсходноеИмяФайла) = Тип("Массив") ИЛИ ТипЗнч(ИсходноеИмяФайла) = Тип("ФиксированныйМассив"),
						" В (&ИсходноеИмяФайла)", " = &ИсходноеИмяФайла")), "")
					+ ?(НЕ ВключатьПомеченныеНаУдаление, "
	...
	Если ЗначениеЗаполнено(ИсходноеИмяФайла) Тогда
		Если ВозвращатьПодобные Тогда
			МаскаИсходногоИмениФайла = ?(ТипЗнч(ИсходноеИмяФайла) = Тип("Массив")
				ИЛИ ТипЗнч(ИсходноеИмяФайла) = Тип("ФиксированныйМассив"), ИсходноеИмяФайла[0], ИсходноеИмяФайла);
			МаскаИсходногоИмениФайла = СтрЗаменить(МаскаИсходногоИмениФайла, "\", "\\");
			МаскаИсходногоИмениФайла = СтрЗаменить(МаскаИсходногоИмениФайла, "_", "\_");
			МаскаИсходногоИмениФайла = СтрЗаменить(МаскаИсходногоИмениФайла, "%", "\%") + "%";
			Запрос.УстановитьПараметр("МаскаИсходногоИмениФайла", МаскаИсходногоИмениФайла);
...
Функция МассивОписанийИДанныхZipФайла(ИмяФайлаПотокИлиДвоичныеДанные, ВозвращатьМассивПриОшибке = Ложь) Экспорт
	...
	Если ТипЗнч(ИмяФайлаПотокИлиДвоичныеДанные) = Тип("Строка") Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ИмяФайлаПотокИлиДвоичныеДанные,
			"ИмяФайлаПотокИлиДвоичныеДанные",
...
Процедура СформироватьGzip(МассивОписанийИДанных, ИмяФайлаИлиПоток) Экспорт
	...
	Если ТипЗнч(ИмяФайлаИлиПоток) = Тип("Строка") Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ИмяФайлаИлиПоток,
			"ИмяФайлаИлиПоток",
			"СформироватьGzip");
...
//  ФильтрыПоискаИлиМассив - Соответствие или Массив из Соответствие - для массива ищется результат, отличный
//    от ЗначениеПоУмолчанию.
//
Функция НайтиЭлементИБ(ЗначениеПоУмолчанию, ФильтрыПоискаИлиМассив) Экспорт
	
	Результат = ЗначениеПоУмолчанию;
	
	Если ТипЗнч(ФильтрыПоискаИлиМассив) = Тип("Массив")
		ИЛИ ТипЗнч(ФильтрыПоискаИлиМассив) = Тип("ФиксированныйМассив") Тогда
		
		Для каждого ФильтрыПоиска Из ФильтрыПоискаИлиМассив Цикл
			Результат = НайтиЭлементИБ(ЗначениеПоУмолчанию, ФильтрыПоиска);
			Если Результат <> ЗначениеПоУмолчанию Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		ФильтрыПоиска = ФильтрыПоискаИлиМассив;
	КонецЕсли;
