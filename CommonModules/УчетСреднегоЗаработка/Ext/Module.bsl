...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьДанныеДляРасчетаОбщегоСреднегоЗаработкаПоНачислениям");
	Методы.Вставить("ЗаполнитьПериодыДействияВДанныхДляРасчетаСреднегоЗаработкаПоНачислениям");
	Методы.Вставить("ЗаполнитьДанныеСреднегоЗаработкаПоНачислениямГПХ");
	Методы.Вставить("УдалитьКоэффициентыПовышенияИзКоэффициентовИндексацииЗаработка");
	Методы.Вставить("ЗаполнитьКоэффициентыПовышенияЗаработкаУчитываемыеПриИндексации");
	Методы.Вставить("УдалитьТехноЗаписиДанныеДляРасчетаОбщегоСреднегоЗаработкаПоНачислениям");
	Методы.Вставить("ДобавитьСторноДанныеДляРасчетаОбщегоСреднегоЗаработкаПоНачислениям");
КонецПроцедуры
...
Процедура РаспределитьВТНачисленияПоБазе(МенеджерВременныхТаблиц, Регистратор, ИсключатьВПериодКомандировок)
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТРасчетнаяБазаСторно
		|ИЗ
		|	ВТРасчетнаяБаза КАК РасчетнаяБаза
		|ГДЕ
		|	РасчетнаяБаза.Регистратор = &Регистратор
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРасчетнаяБазаСторно КАК РасчетнаяБазаСторно
		|		ПО РасчетнаяБаза.Регистратор = РасчетнаяБазаСторно.Регистратор
		|			И РасчетнаяБаза.НомерСтроки = РасчетнаяБазаСторно.НомерСтроки
		|			И РасчетнаяБаза.НомерСтрокиРазрез = РасчетнаяБазаСторно.НомерСтрокиРазрез
		|			И РасчетнаяБаза.ПериодРегистрацииРазрез = РасчетнаяБазаСторно.ПериодРегистрацииРазрез
		|			И РасчетнаяБаза.СотрудникРазрез = РасчетнаяБазаСторно.СотрудникРазрез
		|			И РасчетнаяБаза.ВидРасчетаРазрез = РасчетнаяБазаСторно.ВидРасчетаРазрез
		|ГДЕ
		|	ИсключаемыеСтроки.НомерСтроки ЕСТЬ NULL
		|	И РасчетнаяБазаСторно.НомерСтроки ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	ИменаВТ.Добавить("ВТРасчетнаяБазаСторно");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, ИменаВТ);
