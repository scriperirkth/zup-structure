...
Процедура ВыбратьЗначениеПоТипуОбъекта(ОписаниеОповещения, ТипОбъектаВыбора, Отбор = Неопределено,
	...
	ПараметрыФормы.Вставить("Заголовок",
		ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(ТипОбъектаВыбора,, Истина));
...
Процедура СоздатьИнтегрированныйОбъектБезПравила(Форма, ТипОбъектаИС, ТипОбъектаДО, ИдентификаторОбъектаДО) Экспорт
	...
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Объект1СДокументооборота",
	...
	ПараметрыОповещения.Вставить("НаличиеПрисоединенныхФайлов", Неопределено);
	ПараметрыОповещения.Вставить("ТипСоздаваемогоОбъекта", ТипОбъектаИС);
...
Процедура НачатьВыборТипаБизнесПроцесса(ОписаниеОповещенияОЗакрытии) Экспорт
	...
	ТипыXDTO.Добавить("DMBusinessProcessPerformance");
	ТипыXDTO.Добавить("DMBusinessProcessAcquaintance");
	ТипыXDTO.Добавить("DMBusinessProcessOrder");
	ТипыXDTO.Добавить("DMBusinessProcessConsideration");
	ТипыXDTO.Добавить("DMBusinessProcessRegistration");
	ТипыXDTO.Добавить("DMBusinessProcessApproval");
	ТипыXDTO.Добавить("DMBusinessProcessConfirmation");
	ТипыXDTO.Добавить("DMBusinessProcessInternalDocumentProcessing");
	ТипыXDTO.Добавить("DMBusinessProcessIncomingDocumentProcessing");
	ТипыXDTO.Добавить("DMBusinessProcessOutgoingDocumentProcessing");
	
	СписокДоступныхТиповБизнесПроцессов = Новый СписокЗначений;
	Для Каждого ТипXDTO Из ТипыXDTO Цикл
		Представление = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(ТипXDTO);
		СписокДоступныхТиповБизнесПроцессов.Добавить(
			Новый Структура("XDTOClassName, Presentation", ТипXDTO, Представление));
	КонецЦикла;
...
Функция ЕстьНеЗаполненныеПредметы(Предметы, ИмяКолонкиID, ИмяКолонкиПредмет) Экспорт
	...
	Для Каждого СтрокаПредмета Из Предметы Цикл
		Если СтрокаПредмета.РольПредмета = "Заполняемый"
				И СтрокаПредмета.ОбязательноеЗаполнение = Истина
				И Не ЗначениеЗаполнено(СтрокаПредмета[ИмяКолонкиID]) Тогда //@NON-NLS-1
			ОчиститьСообщения();
...
Процедура СогласоватьАсинхронно(Форма, ПредметСогласования, ОповещениеОЗавершении) Экспорт
	...
	Если ПредметСогласования.Пустая() Тогда
		ДополнительныеПараметры = Новый Структура("Форма, ПредметСогласования, ОповещениеОЗавершении",
...
Процедура СогласоватьАсинхронноЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		СсылкаНаОбъект = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.СсылкаНаИнтегрируемыйОбъектИзФормы(
			ДополнительныеПараметры.Форма,
			ТипЗнч(ДополнительныеПараметры.ПредметСогласования));
		Если Не СсылкаНаОбъект.Пустая() И Не ДополнительныеПараметры.Форма.Модифицированность Тогда
			НачатьСогласованиеАсинхронно(
				ДополнительныеПараметры.Форма,
				СсылкаНаОбъект,
				ДополнительныеПараметры.ОповещениеОЗавершении);
			Возврат;
...
Процедура ПрерватьСогласованиеАсинхронноЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		СсылкаНаОбъект = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.СсылкаНаИнтегрируемыйОбъектИзФормы(
			...
			ТипЗнч(ДополнительныеПараметры.ПредметСогласования));
		Если Не СсылкаНаОбъект.Пустая() И Не ДополнительныеПараметры.Форма.Модифицированность Тогда
			НачатьПрерываниеСогласованияАсинхронно(
				ДополнительныеПараметры.Форма,
				ДополнительныеПараметры.ПредметСогласования,
				ДополнительныеПараметры.ОповещениеОЗавершении);
			Возврат;
		КонецЕсли;
	КонецЕсли;
