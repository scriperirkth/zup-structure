...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьНастройкиСервисаФоновоеЗадание", Истина);
	Методы.Вставить("ОпубликоватьРасчетныеЛистыЗаМесяцВФоне", Истина);
	Методы.Вставить("ВыполнитьОбменФоновоеЗадание", Истина);
	Методы.Вставить("ОтключитьКабинетыНепубликуемыхСотрудниковФоновоеЗадание", Истина);
	Методы.Вставить("ОпубликоватьРасчетныеЛистыВФоне", Истина);
	Методы.Вставить("ЗавершитьСценарийОтложенногоПодключенияФоновоеЗадание", Истина);
	Методы.Вставить("ОбновитьВыгруженныеДанные");
	Методы.Вставить("ЗарегистрироватьОграничениеДоступаКРабочимКонтактам");
	Методы.Вставить("ОбновитьРегистрациюОбработчиковОбмена");
	Методы.Вставить("ЗарегистрироватьОбъектыВыгружаемыеПоСсылкам");
	Методы.Вставить("ОбновитьНастройкиФункциональности");
	Методы.Вставить("ОтменитьРегистрациюГруппВидыДокументовЭДО");
	Методы.Вставить("ПеренестиДанныеВНовыеРегистры");
	Методы.Вставить("ОбновитьПрименяемыеВычетыНДФЛ");
	Методы.Вставить("ОбновитьВидыДокументовЭДО");
	Методы.Вставить("ОтменитьРегистрациюПодразделенийКакПодразделенийОрганизации");
	Методы.Вставить("ЗарегистрироватьБудущиеСобытияВычетыНДФЛ");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия 			= "3.1.38.3";
	Обработчик.Процедура 		= "ИнтеграцияКабинетСотрудника.ОтменитьРегистрациюПодразделенийКакПодразделенийОрганизации";
	Обработчик.РежимВыполнения 	= "Отложенно";
	Обработчик.Идентификатор 	= Новый УникальныйИдентификатор("0936b562-2d88-41cc-8465-2e035df1b6eb");
	Обработчик.Комментарий 		= НСтр("ru = 'Обновление выгрузки подразделений организации.'");
...
Процедура СохранитьНовыйАдресРесурсовAPI(АдресРесурсовAPI) Экспорт
	...
	РегистрыСведений.НастройкиСервисаКабинетСотрудника.УстановитьАдресРесурсовAPI(АдресРесурсовAPI);
...
Процедура ОтменитьРегистрациюПодразделенийКакПодразделенийОрганизации(ПараметрыОбновления = Неопределено) Экспорт

	Если Не ПолучитьФункциональнуюОпцию("ИспользуетсяСервисКабинетСотрудника")
		Или Не ИспользуетсяВерсияDTO("3.4") Тогда
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.УстановитьПараметрОбновления(ПараметрыОбновления, "ОбработкаЗавершена", Истина);
		Возврат;
	КонецЕсли;
	...
	|ГДЕ
	|	Изменения.Ссылка ССЫЛКА Справочник.СтруктураПредприятия
	|	И Изменения.ТипОбъекта = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ПодразделениеОрганизации)";
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура ЗаполнитьНастройкиПодключения(НастройкиПодключения) Экспорт
	...
	НастройкиПодключения.АдресРесурсовAPI 			= СокрЛП(Настройки.АдресРесурсовAPI);
