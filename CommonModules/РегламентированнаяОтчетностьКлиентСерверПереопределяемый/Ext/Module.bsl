...
Процедура ИспользуютсяРегистрацииВОрганеСтатистики(ПризнакИспользования) Экспорт
	...
	КонфигурацииЗарплатаКадрыКлиентСервер.ИспользуютсяРегистрацииВОрганеСтатистики(ПризнакИспользования);
...
// Вызывается при получении параметров функциональности. Предназначена для изменения значения параметра функциональности
// в конфигурации-потребителе.
// 
// Параметры:
//  Параметры - см. РегламентированнаяОтчетностьКлиентСервер.ПараметрыФункциональности
// 
// Пример:
//  Параметры.ОтображатьЛичныеКабинетыОператоровСвязиНаФормеОтчетности = Ложь;
// 
Процедура ПриОпределенииПараметровФункциональности(Параметры) Экспорт
