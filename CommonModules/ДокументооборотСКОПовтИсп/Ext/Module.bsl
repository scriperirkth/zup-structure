...
Функция ЭтоБизКуб() Экспорт
	...
	МодульОблачная1СО = ОбщегоНазначения.ОбщийМодуль("Облачная1СО");
	
	Если МодульОблачная1СО <> Неопределено Тогда
		Возврат МодульОблачная1СО.ЭтоБизКуб();
	КонецЕсли;
...
Функция ПредопределенныеВидыОтправляемыхДокументов(ПриведенныеВРег = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПриведенныеВРег Тогда
			ИмяПредопределенныхДанных = ВРег(ИмяПредопределенныхДанных);
			ИмяПредопределенныхДанных = СокрЛП(ИмяПредопределенныхДанных);
		КонецЕсли;
		Результат[ИмяПредопределенныхДанных] = Выборка.Ссылка;
	КонецЦикла;
...
Функция ВидОтправляемогоДокументаПоВидуОбъекта(ВидОбъекта, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ВидОбъекта) = Тип("Строка") Тогда
		ВидОбъектаПриведенныйВРег = ВРег(ВидОбъекта);
		ВидОбъектаПриведенныйВРег = СокрЛП(ВидОбъектаПриведенныйВРег);
		
		Если ВидОбъектаПриведенныйВРег = ВРег("ПисьмоРПН") Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.УведомлениеПриродопользователю;
			
		ИначеЕсли ВидОбъектаПриведенныйВРег = ВРег("МЧДФНС") Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.МашиночитаемыеДоверенностиФНС;
			
		ИначеЕсли ВидОбъектаПриведенныйВРег = ВРег("МЧДФНСРеестр") Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.МашиночитаемыеДоверенностиРаспределенныйРеестр;
			
		ИначеЕсли ВидОбъектаПриведенныйВРег = ВРег("МЧДФСС") Тогда
			Возврат Справочники.ВидыОтправляемыхДокументов.МашиночитаемыеДоверенностиФСС;
			
		Иначе
			ВидОтправляемогоДокумента =
				ДокументооборотСКОПовтИсп.ПредопределенныеВидыОтправляемыхДокументов(Истина)[ВидОбъектаПриведенныйВРег];
