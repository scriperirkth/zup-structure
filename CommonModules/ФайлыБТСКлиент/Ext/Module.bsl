...
//    * СпособПомещенияФайла - Строка - способ помещения файла. Возможные значения: ВременноеХранилище, ФайлНаСервере
//        или ЗагрузкаВСервис. Если не указано, то определяется по параметру ИмяФайлаИлиАдрес.
//    * ИмяФайлаИлиАдрес - Строка - Адрес во временном хранилище, либо имя файла на сервере (без пути)
...
//      на сервер. Если указано Неопределено, размер клиентского файла контролироваться не будет.
Функция ПараметрыПомещенияФайла() Экспорт
	
	Результат = Новый Структура();
	Результат.Вставить("СпособПомещенияФайла", "");
	Результат.Вставить("ИмяФайлаИлиАдрес", "");
	Результат.Вставить("ПутьФайлаWindows", "");
	Результат.Вставить("ПутьФайлаLinux", "");
	Результат.Вставить("ОписаниеОповещенияОЗавершении", Неопределено);
...
Процедура ПолучитьФайлИнтерактивно(ПараметрыПолученияФайла) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВсеПараметры.ИмяФайлаИлиАдрес) Тогда
		ТекстОшибки = НСтр("ru = 'Не указано исходное хранение файла на сервере'");
...
Процедура ПоместитьФайлИнтерактивно(ПараметрыПомещенияФайла) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыПомещенияФайла.СпособПомещенияФайла) Тогда
		Если ЭтоАдресВременногоХранилища(ПараметрыПомещенияФайла.ИмяФайлаИлиАдрес) Тогда
			ПараметрыПомещенияФайла.СпособПомещенияФайла = "ВременноеХранилище";
		Иначе
			ПараметрыПомещенияФайла.СпособПомещенияФайла = "ФайлНаСервере";
		КонецЕсли;
	КонецЕсли;
	...
	Если ВсеПараметры.СпособПомещенияФайла = "ЗагрузкаВСервис"
		И Не ФайлыБТСВызовСервера.ДоступнаЗагрузкаВСервис() Тогда
		ТекстОшибки = НСтр("ru = 'Для загрузки файлов в сервис необходимо включить аутентификацию токенами доступа'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	Если ВсеПараметры.СпособПомещенияФайла <> "ЗагрузкаВСервис"
		И Не ЗначениеЗаполнено(ВсеПараметры.ИмяФайлаИлиАдрес) Тогда
		ТекстОшибки = НСтр("ru = 'Не указано целевое хранение файла на сервере'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;

	Если ВсеПараметры.СпособПомещенияФайла = "ВременноеХранилище" Тогда
		ИспользоватьПорционнуюПередачу = Ложь;
		...
		Иначе
			ВсеПараметры.МаксимальныйРазмер = Мин(ВсеПараметры.МаксимальныйРазмер, МаксимальныйРазмер);
		КонецЕсли;
	Иначе
		ИспользоватьПорционнуюПередачу = Истина;
	КонецЕсли;
...
Процедура ПослеПолученияРазмера(Размер, ДополнительныеПараметры) Экспорт
	...
	ПараметрыПолучения = Новый Структура;
	ПараметрыПолучения.Вставить("СпособПомещенияФайла", ДополнительныеПараметры.СпособПомещенияФайла);
	ПараметрыПолучения.Вставить("ИмяФайлаИлиАдрес", ДополнительныеПараметры.ИмяФайлаИлиАдрес);
	ПараметрыПолучения.Вставить("ПутьФайлаWindows", ДополнительныеПараметры.ПутьФайлаWindows);
	ПараметрыПолучения.Вставить("ПутьФайлаLinux", ДополнительныеПараметры.ПутьФайлаLinux);
