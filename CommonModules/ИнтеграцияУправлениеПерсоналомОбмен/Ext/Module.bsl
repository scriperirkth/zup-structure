...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверкаАдресаСервисаАвторизацииФоновоеЗадание", Истина);
	Методы.Вставить("ПроверкаНастроекПодключенияФоновоеЗадание", Истина);
	Методы.Вставить("ПроверкаАдресаРесурсовAPI", Истина);
	Методы.Вставить("ОбновитьДанныеСервисаПодписанияФоновоеЗадание", Истина);
...
Функция ПроверкаАдресаРесурсовAPI(Параметры) Экспорт
	
	Результат = Новый Структура("СообщениеОбОшибке");
	
	Приложение = Параметры.Приложение;
	АдресРесурсовAPI = Параметры.АдресРесурсовAPI;
	...
	Ключи = ИнтеграцияУправлениеПерсоналом.КлючиПриложения(Приложение);
	Если Ключи = Неопределено Тогда
		УстановитьРегламентныеЗаданияЗаблокированы(Приложение, Истина);
		СообщениеОбОшибке = НСтр("ru = 'Не заданы ключи приложения.'");
		ВызватьИсключение СообщениеОбОшибке;
	КонецЕсли;
	
	НастройкиПодключения.АдресПриложения			= Настройки.АдресПриложения;
	НастройкиПодключения.АдресСервисаАвторизации	= Настройки.АдресСервисаАвторизации;
	НастройкиПодключения.АдресРесурсовAPI			= АдресРесурсовAPI;
	...
	Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
	Если Ответ.КодСостояния = Неопределено Или Ответ.КодСостояния <> 200 И Не ЗначениеЗаполнено(Результат.СообщениеОбОшибке) Тогда
		...
		Результат.СообщениеОбОшибке = СтрШаблон(ШаблонСообщения, НастройкиПодключения.АдресПриложения);
	КонецЕсли;
...
Функция ИнформационнаяСистемаДоступна(Приложение) Экспорт
...
Функция ВыгрузитьИзмененияОбъектов(МенеджерОбмена)
	...
	Если ИнтеграцияУправлениеПерсоналом.ДоступенРасширенныйФункционал() Тогда
		Если МенеджерОбмена.Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника И ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.0") Тогда
...
Процедура ОбработатьГрафикиОтпусковПодразделений(МенеджерОбмена, ОбъектВсеИзменения, РезультатОбработки, ПовторнаяЗагрузка, ПовторнаяОбработка)
	...
	Для каждого Объект Из Объекты Цикл 
		...
		Если Объект.Свойство("Периоды") И ЗначениеЗаполнено(Объект.Периоды) Тогда
			...
			КонецЦикла;
		КонецЕсли;
		Если Объект.Свойство("ПериодыКомпенсации") И ЗначениеЗаполнено(Объект.ПериодыКомпенсации) Тогда
			Для каждого ЗаписьПериода Из Объект.ПериодыКомпенсации Цикл
				ОшибкиКонвертации = ОшибкиКонвертацииОбъекта(ЗаписьПериода);
				Если ЗначениеЗаполнено(ОшибкиКонвертации) Тогда
					ОшибкиКонвертацииОбъекта.Добавить(ОшибкиКонвертации);
				КонецЕсли;
