...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПометитьРассылкиСШаблонамиSMS");
КонецПроцедуры
...
Процедура ПриНачальномЗаполненииДанныхПодсистемыУведомлений(ПараметрыОбновления = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидУведомления) И Не УведомленияСервер.ЕстьРассылкиПоВидуУведомления(ВидУведомления) Тогда 
		...
		ПараметрыШаблона.Текст = НСтр("ru = 'Поздравляем с днем рождения.
	...
	Если ЗначениеЗаполнено(ВидУведомления) И Не УведомленияСервер.ЕстьРассылкиПоВидуУведомления(ВидУведомления) Тогда 
		...
		ПараметрыШаблона.Текст = НСтр("ru = 'Спасибо, что Вы с нами уже [ДанныеУведомления.ОтработаноЛетПредставление]
			|С уважением, [ДанныеУведомления.Организация]'");
		// АПК:1223-вкл
		ПараметрыШаблона.Назначение = "ФизическиеЛица";
	...
	Если ЗначениеЗаполнено(ВидУведомления) И Не УведомленияСервер.ЕстьРассылкиПоВидуУведомления(ВидУведомления) Тогда 
		...
		ПараметрыШаблона.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.ОбычныйТекст;
		ПараметрыШаблона.ВладелецШаблона = ВидУведомления;
		ПараметрыШаблона.Текст = НСтр("ru = 'Поздравляем с первым рабочим днем
			|С уважением, [ДанныеУведомления.Организация]'");
		ПараметрыШаблона.Назначение = "ФизическиеЛица";
	...
	Если ЗначениеЗаполнено(ВидУведомления) И Не УведомленияСервер.ЕстьРассылкиПоВидуУведомления(ВидУведомления) Тогда 
		...
		ПараметрыШаблона.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.ОбычныйТекст;
		ПараметрыШаблона.ВладелецШаблона = ВидУведомления;
		ПараметрыШаблона.Текст = НСтр("ru = 'Поздравляем с праздником
			|С уважением, [ДанныеУведомления.Организация]'");
		ПараметрыШаблона.Назначение = "ФизическиеЛица";
...
//
// Параметры:
//    РассылкаУведомлений - СправочникСсылка.РассылкиУведомлений - обрабатываемая рассылка уведомлений.
...
Процедура ПриСозданииУведомленийПоИзменениямОбъектов(РассылкаУведомлений, ВидУведомления, Предмет, ДатаИзменения, ДополнительныеСвойства, ТаблицаРезультата) Экспорт
	...
	Если ИмяВидаУведомления = "ПервыйРабочийДень" Тогда
		...
		Отборы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РассылкаУведомлений, "Отбор").Получить();
		ОтборПройден = ОтборПройден(Предмет, Отборы);
		Если ЗначениеЗаполнено(ТекущееЗначениеПроведен) И ОтборПройден Тогда
...
Процедура ПолучениеДанныхВидаУведомленияОтработаноЛет(СтруктураПериода, Знач Отборы, ТаблицаРезультата) Экспорт
	...
	Для Каждого ДанныеСотрудника Из КадровыеДанныеСотрудников Цикл
		...
		Если ОтработаноЛетВКомпанииПоФизЛицу[0].ДатаНачалаРаботы <> ДанныеСотрудника.ДатаПриема Тогда
			Продолжить;
		КонецЕсли;
		ДатаСобытия = Дата(Год(НачалоПериода), Запрос.Параметры.МесяцНачала, Запрос.Параметры.ДеньНачала);
		Если ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения) И ДанныеСотрудника.ДатаУвольнения < ДатаСобытия Тогда
			Продолжить;
		КонецЕсли;
		СтрокаТаблицы = ТаблицаРезультата.Добавить();
		СтрокаТаблицы.Предмет = ДанныеСотрудника.ФизическоеЛицо;
...
Функция ОтборПройден(Предмет, Отборы)
	
	ОтборПройден = Истина;
	РеквизитыОтбораПредмета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Предмет, "Должность, Организация, Подразделение, ФизическоеЛицо");
	Для Каждого Отбор Из Отборы Цикл
		Если Не Отбор.Использование Тогда
			Продолжить;
		КонецЕсли;
		ОтборПройден = Ложь;
		Если Отбор.ЛевоеЗначение = "Должность" Тогда
			Если Отбор.ВидСравнения = ВидСравнения.Равно Тогда
				Если Отбор.ПравоеЗначение = РеквизитыОтбораПредмета.Должность Тогда
					ОтборПройден = Истина;
				КонецЕсли;
			...
			ИначеЕсли Отбор.ВидСравнения = ВидСравнения.ВСписке Тогда
				Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.Должность) <> Неопределено Тогда
					ОтборПройден = Истина;
				КонецЕсли;
		...
		ИначеЕсли Отбор.ЛевоеЗначение = "Организация" Тогда
			Если Отбор.ВидСравнения = ВидСравнения.Равно Тогда
				Если Отбор.ПравоеЗначение = РеквизитыОтбораПредмета.Организация Тогда
					ОтборПройден = Истина;
				КонецЕсли;
			ИначеЕсли Отбор.ВидСравнения = ВидСравнения.НеРавно Тогда
				Если Отбор.ПравоеЗначение <> РеквизитыОтбораПредмета.Организация Тогда
				...
				КонецЕсли;
			ИначеЕсли Отбор.ВидСравнения = ВидСравнения.ВСписке Тогда
				Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.Организация) <> Неопределено Тогда
					ОтборПройден = Истина;
				КонецЕсли;
			ИначеЕсли Отбор.ВидСравнения = ВидСравнения.НеВСписке Тогда
				Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.Организация) = Неопределено Тогда
		...
		ИначеЕсли Отбор.ЛевоеЗначение = "Подразделение" Тогда
				Если Отбор.ВидСравнения = ВидСравнения.Равно Тогда
					Если Отбор.ПравоеЗначение = РеквизитыОтбораПредмета.Подразделение Тогда
						ОтборПройден = Истина;
					КонецЕсли;
				ИначеЕсли Отбор.ВидСравнения = ВидСравнения.НеРавно Тогда
					Если Отбор.ПравоеЗначение <> РеквизитыОтбораПредмета.Подразделение Тогда
					...
					КонецЕсли;
				ИначеЕсли Отбор.ВидСравнения = ВидСравнения.ВСписке Тогда
					Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.Подразделение) <> Неопределено Тогда
						ОтборПройден = Истина;
					КонецЕсли;
				ИначеЕсли Отбор.ВидСравнения = ВидСравнения.НеВСписке Тогда
					Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.Подразделение) = Неопределено Тогда
			...
			ИначеЕсли Отбор.ЛевоеЗначение = "Пол" Тогда
				Если Отбор.ВидСравнения = ВидСравнения.Равно Тогда
					Если Отбор.ПравоеЗначение = РеквизитыОтбораПредмета.ФизическоеЛицо.Пол Тогда
						ОтборПройден = Истина;
					КонецЕсли;
				ИначеЕсли Отбор.ВидСравнения = ВидСравнения.НеРавно Тогда
					Если Отбор.ПравоеЗначение <> РеквизитыОтбораПредмета.ФизическоеЛицо.Пол Тогда
					...
					КонецЕсли;
				ИначеЕсли Отбор.ВидСравнения = ВидСравнения.ВСписке Тогда
					Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.ФизическоеЛицо.Пол) <> Неопределено Тогда
						ОтборПройден = Истина;
					КонецЕсли;
				ИначеЕсли Отбор.ВидСравнения = ВидСравнения.НеВСписке Тогда
					Если Отбор.ПравоеЗначение.НайтиПоЗначению(РеквизитыОтбораПредмета.ФизическоеЛицо.Пол) = Неопределено Тогда
				...
				КонецЕсли;
		КонецЕсли;
		Если ОтборПройден = Ложь Тогда
			// Если хоть один отбор не соответствует - возвращаем Ложь
			Прервать;
		КонецЕсли;
	КонецЦикла;
