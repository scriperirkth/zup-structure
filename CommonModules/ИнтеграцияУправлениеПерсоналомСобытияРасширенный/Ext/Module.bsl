...
Процедура ПриЗаписиНастроекРасчетаЗарплаты(СтрокаНабора, СтароеСостояние) Экспорт
	...
	ПроверяемыеСвойства = "ИспользоватьОплатуПоСреднемуЗаработкуВнутрисменную";
	Если СтрокаНабора[Свойство] <> СтароеСостояние[Свойство] Тогда
		ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.ИсполнениеГособязанностей;
		РегистрыСведений.НастройкиОтсутствийКабинетСотрудника.УстановитьТребуетсяОбновитьНастройки(ПричинаОтсутствия);
	КонецЕсли;
...
Процедура ВредныеФакторыИВидыРаботРабочегоМестаПриЗаписи(НаборЗаписей, Замещение) Экспорт
	...
	ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.МедицинскиеОсмотры;
	РабочееМестоОхраныТрудаПриЗаписи(НаборЗаписей, Замещение, ТипОбъекта);
...
// Обработка записи набора записей РС АктуальныеЗаказыНаВыдачуСИЗ.
//
Процедура АктуальныеЗаказыНаВыдачуСИЗПриЗаписи(НаборЗаписей, Замещение) Экспорт

	Если Не (ИнтеграцияУправлениеПерсоналом.ИспользуетсяКабинетСотрудника()
		И ИнтеграцияКабинетСотрудника.ИспользоватьСредстваИндивидуальнойЗащиты()) Тогда
		Возврат;
	КонецЕсли;
	
	ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ИнформацияОСредствахИЗ;
	ОхранаТрудаПриЗаписи(НаборЗаписей, Замещение, ТипОбъекта);
	
КонецПроцедуры

// Обработка записи набора записей РС АнтропометрияФизическихЛиц.
//
Процедура АнтропометрияФизическихЛицПриЗаписи(НаборЗаписей, Замещение) Экспорт
...
// Обработка записи набора записей РС ВыдачиСИЗ.
//
Процедура ВыдачиСИЗПриЗаписи(НаборЗаписей, Замещение) Экспорт
...
// Обработка записи набора записей РС СИЗРабочегоМеста.
//
Процедура СИЗРабочегоМестаПриЗаписи(НаборЗаписей, Замещение) Экспорт
...
// Обработка записи набора записей РС НастройкиСбораГрафиковОтпусков.
//
Процедура ПриЗаписиНастроекСбораГрафиковОтпусков(Ссылка) Экспорт
	...
	Если Не Документы.СборГрафиковОтпусков.ДокументВыгружается(Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ГрафикОтпусковПредприятия;
	ИнтеграцияУправлениеПерсоналомСобытия.ЗарегистрироватьИзменениеДляОбмена(Ссылка, ТипОбъекта, Ложь, Истина, Неопределено);
...
Процедура ОбработчикПриЗаписиОбъекта(Источник) Экспорт
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаКомпенсацияОтпускаКабинетСотрудника")
		...
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаОтпускПоУходуЗаРебенкомКабинетСотрудника")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаИзменениеГрафикаОтпусковКабинетСотрудника")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаИсполнениеГособязанностейКабинетСотрудника")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаВыходныеДляУходаЗаДетьмиИнвалидамиКабинетСотрудника")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаКомандировкаКабинетСотрудника")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.ОтчетПоКомандировкеКабинетСотрудника")
		Или ТипЗнч(Источник) = Тип("ДокументОбъект.РасходыВКомандировкеКабинетСотрудника") Тогда
	...
	Иначе
		...
		Для каждого Приложение Из Приложения Цикл
			...
			Если ПодразделениеЕстьВПравилах Тогда
				...
				Иначе
					Подразделение = МестоПозицийВСтруктуреПредприятия(Объект.Ссылка, ТекущаяДатаСеанса());
				КонецЕсли;
...
Процедура ПриЗаписиСборГрафиковОтпусков(Объект)
	...
	Если Не ИнтеграцияУправлениеПерсоналом.ИспользуетсяКабинетСотрудника()
		Или Объект.ДополнительныеСвойства.Свойство("НеОбновлятьВыгрузку")
		Или Не ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("1.1") Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Документы.СборГрафиковОтпусков.ДокументВыгружается(Объект) Тогда
		Возврат;
	КонецЕсли;
	
	Если Документы.СборГрафиковОтпусков.ЭтоКорневойДокумент(Объект) Тогда
		ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ГрафикОтпусковПредприятия;
	Иначе
...
Процедура ПриЗаписиВидыОтпусков(Объект)
	...
	РегистрироватьУправлениеПерсоналом 	= ИнтеграцияУправлениеПерсоналом.Используется1СПерсонал() И Интеграция1СПерсонал.ИспользуетсяВерсияDTO("3.2");
	РегистрироватьКабинетСотрудника 	= ИнтеграцияУправлениеПерсоналом.ИспользуетсяКабинетСотрудника() И ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.0");
	...
	ИнтеграцияУправлениеПерсоналомСобытия.ЗарегистрироватьИзменениеДляОбмена(Объект.Ссылка,
		Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ВидОтпуска,
...
Процедура ПриЗаписиИспользоватьРабочиеМестаОхраныТруда(Объект)
	...
	Если Не Объект.Значение Тогда
		...
		Если НастройкиФункциональности.ПоказыватьОхрануТруда Тогда
			...
			ИнтеграцияКабинетСотрудника.СохранитьНастройкиФункциональностиСервиса(НастройкиФункциональности);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Процедура РабочееМестоОхраныТрудаПриЗаписи(НаборЗаписей, Замещение, ТипОбъекта)

	Если НаборЗаписей.ДополнительныеСвойства.Свойство("ПрежнийНабор") Тогда
		Если ОбщегоНазначения.КоллекцииИдентичны(НаборЗаписей.ДополнительныеСвойства["ПрежнийНабор"], НаборЗаписей.Выгрузить()) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ТипСсылкаРабочееМесто = ИнтеграцияУправлениеПерсоналомРасширенный.ТипРабочиеМестаОхраныТрудаСсылка();
	Если ТипСсылкаРабочееМесто = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ЗначенияОтбора = РегистрыБЗК.ЗначенияОтбораЗаписейНабора(НаборЗаписей, Замещение, "РабочееМесто");
	
	РабочиеМеста = Новый Массив;
	Для Каждого ЗначениеОтбора Из ЗначенияОтбора Цикл
		Если ТипЗнч(ЗначениеОтбора) <> ТипСсылкаРабочееМесто Тогда
			Продолжить;
		КонецЕсли;
		РабочиеМеста.Добавить(ЗначениеОтбора);
	КонецЦикла;	
...
Процедура ОхранаТрудаПриЗаписи(НаборЗаписей, Замещение, ТипОбъекта)

	ФизическиеЛица = Новый Массив;
	Если НаборЗаписей.ДополнительныеСвойства.Свойство("ПрежнийНабор") Тогда
		ПрежнийНабор = НаборЗаписей.ДополнительныеСвойства["ПрежнийНабор"];
		Если ОбщегоНазначения.КоллекцииИдентичны(ПрежнийНабор, НаборЗаписей.Выгрузить()) Тогда
			Возврат;
		КонецЕсли;
		ФизическиеЛица = ПрежнийНабор.ВыгрузитьКолонку("ФизическоеЛицо");
	КонецЕсли;
	
	НовыеФизическиеЛица = РегистрыБЗК.ЗначенияОтбораЗаписейНабора(НаборЗаписей, Замещение, "ФизическоеЛицо");
	Если НовыеФизическиеЛица.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФизическиеЛица, НовыеФизическиеЛица);
	КонецЕсли;
...
Функция ИменаКонтролируемыхРеквизитов(Объект) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.СборГрафиковОтпусков") Тогда
		ИменаРеквизитов = "Год,СборЗапущен,СборЗавершен,Состояние,ПометкаУдаления,ИнициаторСбора,СборПоПодразделениям,ДатаОкончанияСбора,ПожеланияСотрудниковВидныТолькоОтветственному";
...
Функция МестоПозицийВСтруктуреПредприятия(ПозицияШР, ДатаПолученияДанных = '00010101')

	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ОрганизационнаяСтруктура");
		Возврат Модуль.МестоПозицииВСтруктуреПредприятия(ПозицияШР, ДатаПолученияДанных);
	КонецЕсли;
