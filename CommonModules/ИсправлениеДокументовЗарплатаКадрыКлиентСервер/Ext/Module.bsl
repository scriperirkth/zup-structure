...
Процедура УстановитьПоляИсправления(Форма, РежимИсправления = "РасчетЗарплаты", ПолеПериодРегистрации = "ПериодРегистрации") Экспорт
	...
	ИначеЕсли РежимИсправления = "ПериодическиеСведения" Тогда
		Если ИспользоватьИсправлениеДокумента Тогда
		...
		ИначеЕсли ИспользоватьСторнированиеДокумента Тогда
			ТекстНадписи = НСтр("ru = 'Если необходимо отменить действие документа, но сохранить данный экземпляр, воспользуйтесь командой Сторнировать'");
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоИсправление(Форма, ПараметрыИсправленного = Неопределено) Экспорт
	...
	Иначе
		...
		ЭтоИсправление = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПараметрыИсправленногоДокумента")
			И Форма.ПараметрыИсправленногоДокумента <> Неопределено И ЗначениеЗаполнено(Форма.ПараметрыИсправленногоДокумента.Ссылка);
		
		Если ЭтоИсправление Тогда
			ПараметрыИсправленного = Форма.ПараметрыИсправленногоДокумента;
		Иначе
			ЭтоИсправление = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "ИсправленныйДокумент")
				И ЗначениеЗаполнено(Форма.Объект.ИсправленныйДокумент);
		КонецЕсли;
...
Функция ДокументИсправлен(Форма) Экспорт
	
	Исправлен = Форма.ВозможностиИсправления.Исправление И Форма.ДокументИсправлен;
	Сторнирован = Форма.ВозможностиИсправления.Сторнирование И Форма.ДокументСторнирован;
	
	Возврат Исправлен Или Сторнирован;
