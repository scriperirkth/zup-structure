...
// Создает временный каталог в системной папке временных файлов с уникальным именем
//
...
// Удаляет временный файл с предварительной проверкой и журналированием результатов выполнения
//
// Параметры:
//   ИмяФайла - Строка
Процедура УдалитьВременныйФайл(ИмяФайла) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьКлиентСервер.ЭтоИмяВременногоФайла(ИмяФайла) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверное значение параметра %1 в %2:
				|Файл не является временным ""%3"".'"), 
			"Путь", "ОперацииСФайламиЭДКО.УдалитьВременныйФайл", ИмяФайла);
	КонецЕсли;
	
	Попытка
		// Предварительно делается проверка на то, что файл является временным
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
			...
			,
			,
			СтрШаблон(НСтр("ru = 'Ошибка удаления временного файла %1, текст ошибки: %2'"), ИмяФайла, ТекстОшибки));
	КонецПопытки;
...
// Обновляет двоичные данные и размер в структуре описания файла
//
// Параметры:
//   НашаКартинка - Картинка - изображение для преобразования в двоичные данные
//   ОписаниеФайла - Структура - структура, в которую будут записаны двоичные данные и размер
Процедура ОбновитьДвДанныеВОписанииФайла(НашаКартинка, ОписаниеФайла)
	
	ДвДанные = НашаКартинка.ПолучитьДвоичныеДанные();
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Электронный документооброт с контролирующими органами. Операции с файлами'", Метаданные.ОсновнойЯзык.КодЯзыка);
