...
// Получает данные внутри структуры по пути
// 
// Параметры:
//  Данные - Структура - Данные, в контексте которых вычисляются данные по пути 
//  Путь   - Строка    - Путь к данным через "."
// 
// Возвращаемое значение:
//  Неопределено, Произвольный - Полученные данные
Функция ЗначениеПоПути(Знач Данные, Знач Путь) Экспорт
	
	ЧастиПути = СтрРазделить(Путь, ".");
	Контекст  = Данные;
	
	Для Каждого ЧастьПути Из ЧастиПути Цикл
		Контекст = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Контекст, ЧастьПути);
	КонецЦикла;
...
Функция НачатьЗамер(ИмяКлючевойОперации) Экспорт
	...
	Возврат МодульОценкаПроизводительности.НачатьЗамерДлительнойОперации(ИмяКлючевойОперации);
КонецФункции
...
Процедура ЗафиксироватьЭтап(ШагДействия, ОписаниеЗамера, ОбработаноДанных = 0, Комментарий = "") Экспорт
	...
	КонецЕсли;
	МодульОценкаПроизводительности = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительности");
	МодульОценкаПроизводительности.ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, ОбработаноДанных, ШагДействия, 
		Комментарий);
КонецПроцедуры
...
Процедура ЗакончитьЗамер(КлючДействия, ОписаниеЗамера, ОбработаноДанных = 0, Комментарий = "") Экспорт
	Если КлючДействия = Неопределено
		...
		Возврат;
	КонецЕсли;
	Если ОписаниеЗамера.Получить("ВремяНачала") = ТекущаяУниверсальнаяДатаВМиллисекундах() Тогда
		Возврат;
	КонецЕсли;
	...
	КонецЕсли;
	МодульОценкаПроизводительности = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительности");
	МодульОценкаПроизводительности.ЗакончитьЗамерДлительнойОперации(ОписаниеЗамера, ОбработаноДанных, КлючДействия, 
		Комментарий);
КонецПроцедуры
