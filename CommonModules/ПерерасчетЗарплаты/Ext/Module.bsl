...
Функция СотрудникиКПерерасчету(Организация, Месяц, Сотрудники = Неопределено) Экспорт
	...
	ДоговорыГПХРасширенный.ДополнитьЗапросСотрудниковПересчетаПериода(Запрос);
...
Процедура РегистрацияПерерасчетовПоПредварительнымДанным(Регистратор = Неопределено, Организация = Неопределено) Экспорт
	...
	Попытка
		...
		ДоговорыГПХРасширенный.СоздатьВТПредварительныеПерерасчетыДоговоров(МенеджерВременныхТаблиц);
		...
		ДоговорыГПХРасширенный.ЗарегистрироватьПерерасчетыДоговоров(МенеджерВременныхТаблиц);
...
//		Организация
//		ПериодРегистрации
//		Начисления - таблица значений с колонками табличных частей начислений по договорам ГПХ.
//
Процедура СформироватьДвиженияИсходныеДанныхПерерасчетовГПХ(Движения, Организация, ПериодРегистрации, Начисления) Экспорт
	...
	Для Каждого ВидРасчета Из ВидыРасчета Цикл
		...
		Значения.Вставить("ТребуетсяРасчетБазы", Ложь);
		Значения.Вставить("ЗачетНормыВремени", Истина);
		Значения.Вставить("СпособВыполненияНачисления", Перечисления.СпособыВыполненияНачислений.ВЗаданныхМесяцахПриОкончательномРасчете);
		Значения.Вставить("ВидДокументаНачисления", Неопределено);
		ЗначенияРеквизитов.Вставить(ВидРасчета, Значения);
	КонецЦикла;
...
Процедура УдалитьПерерасчеты(Регистратор, УдалитьПерерасчетыТекущегоПериода) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПерерасчетДоговоров Тогда
...
Процедура УдалитьПерерасчетыПоРегистратору(Регистратор, Сотрудники = Неопределено, ФизическиеЛица = Неопределено) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПерерасчетДоговоров.ФизическоеЛицо,
		|	ПерерасчетДоговоров.ДокументНачисления КАК ДокументНачисления
		|ИЗ
		|	РегистрСведений.ПерерасчетДоговоров КАК ПерерасчетДоговоров
		|ГДЕ
		|	ПерерасчетДоговоров.ДокументНачисления = &ДокументНачисления
		|	И ПерерасчетДоговоров.ФизическоеЛицо В(&ФизическиеЛица)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДокументНачисления";
	...
	Если ЗначениеЗаполнено(ФизическиеЛица) Тогда
		Запрос.УстановитьПараметр("ФизическиеЛица", ФизическиеЛица);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПерерасчетДоговоров.ФизическоеЛицо,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ПерерасчетДоговоров.ФизическоеЛицо В(&ФизическиеЛица)", "");
	КонецЕсли;
...
Процедура ВосстановитьПерерасчеты(Регистратор, Организация) Экспорт
	...
	ДоговорыГПХРасширенный.ВосстановитьПерерасчеты(Запрос.МенеджерВременныхТаблиц, Регистратор, Организация);
...
Функция РегистрыИсточникиПредварительныхПерерасчетов() Экспорт
	...
	Регистры.Вставить(Метаданные.РегистрыСведений.ПлановыеНачисленияПоДоговорам.Имя, Истина);
	...
	Регистры.Вставить(Метаданные.РегистрыСведений.УсловияДоговораГПХ.Имя, Истина);
	...
	Регистры.Вставить(Метаданные.РегистрыСведений.ЗначенияПоказателейСреднегоЗаработкаСотрудников.Имя, Истина);
...
//
Процедура ЗарегистрироватьПредварительныеПерерасчетыПриЗаписиНабора(НаборЗаписей, Замещение) Экспорт
...
Процедура СвернутьНачисленияПерерасчеты(НачисленияПерерасчет) Экспорт
	...
	СворачиваемаяКоллекция.Сортировать("ПериодДействия,ДатаНачала,Сотрудник,Начисление,РезультатАбсолютный,СторнируемыйИсходный,Сторно,Результат", Новый СравнениеЗначений);
...
Функция ЗапросВТПериодыРасчетаСотрудников(Регистратор, УдалитьПерерасчетыТекущегоПериода)
	...
	ТекстыЗапросов.Добавить(
		...
		|	НачислениеЗарплатыСотрудникиПерерасчет.ПерерасчетДоговоров,
	...
	ДоговорыГПХРасширенный.ДополнитьДоговорамиЗапросВТПериодыРасчетаСотрудников(ТекстыЗапросов, МетаданныеРегистратора);
	...
	Если МетаданныеРегистратора.Имя <> "НачислениеЗарплаты" Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "НачислениеЗарплатыСотрудникиПерерасчет.ПерерасчетДоговоров", "ЛОЖЬ");
	КонецЕсли;
...
Функция ПустаяТаблицаСотрудникиПериодДействияПерерасчет() Экспорт
	...
	СотрудникиПериодДействияПерерасчет.Колонки.Добавить("ПерерасчетДоговоров", Новый ОписаниеТипов("Булево"));
	...
	СотрудникиПериодДействияПерерасчет.Колонки.Добавить("ДокументОснование", Метаданные.ОпределяемыеТипы.ОснованиеНачисленияУдержания.Тип);
...
Процедура СоздатьВТПерерассчитываемыеСотрудники(МенеджерВременныхТаблиц, Организация, Подразделение, МесяцНачисления, СотрудникиПериодДействияПерерасчет, РежимДоначисления, Сотрудники = Неопределено, ИмяВТПерерассчитываемыеСотрудники = "ВТПерерассчитываемыеСотрудники")
	...
	ДоговорыГПХРасширенный.ДополнитьЗапросВТПерерасчетЗарплатыСОбластямиПерерасчетов(Запрос.Текст, РежимДоначисления, Сотрудники);
	...
	ПараметрыПостроения.РаботникиПоДоговорамГПХ = Истина;
	...
	Если СотрудникиПериодДействияПерерасчет = Неопределено
		...
		Запрос.Текст =
			...
			|	ПерерасчетЗарплатыСОбластями.ДокументОснование КАК ДокументОснование,
			|	ПерерасчетЗарплатыСОбластями.ПерерасчетНачислений КАК ПерерасчетНачислений,
			|	ПерерасчетЗарплатыСОбластями.ПерерасчетДоговоров КАК ПерерасчетДоговоров,
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	ПерерассчитываемыеСотрудники.ДокументОснование КАК ДокументОснование,
			...
			|	ПерерассчитываемыеСотрудники.ПерерасчетДоговоров КАК ПерерасчетДоговоров,
			...
			|	СотрудникиПериодовПерерасчетов.ДокументОснование,
			|	МАКСИМУМ(СотрудникиПериодовПерерасчетов.ПерерасчетНачислений),
			|	МАКСИМУМ(СотрудникиПериодовПерерасчетов.ПерерасчетДоговоров),
			...
			|			ИЛИ СотрудникиПериодовПерерасчетов.ПерерасчетДоговоров
...
Процедура ЗаполнитьДанныеПерерасчетаНачислений(ДанныеПерерасчета, Организация, Подразделение = Неопределено, МесяцНачисления, Сотрудники, СотрудникиПериодДействияПерерасчет, ИсключаемыеРегистраторы, РежимДоначисления, МенеджерРасчета)
	...
	Если ЗарплатаКадры.ВТСодержитСтроки(МенеджерВременныхТаблиц, "ВТСторнируемыеРегистраторы") Тогда
		...
		ЗаполнитьСторнированиеПоДоговорам(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, РаспределенияТерриторииПоказатели, МесяцНачисления);
...
Процедура УстановитьОтборыПерерасчетовНачислений(МенеджерРасчета, МенеджерВременныхТаблиц)
	...
	ДоговорыГПХРасширенный.ДополнитьОтборПерерасчетовНачислений(ТаблицаОтборов, Запрос.МенеджерВременныхТаблиц);
...
Процедура ЗаполнитьРезультатРаспределенияПоТерриториям(СтрокаСторноПерерасчет, ТаблицаРаспределенияНачисленийПоТерриториямИУсловиямТруда, ТаблицаПоказателей, Ссылка, ИдентификаторСтрокиВидаРасчета)
	...
	Если СтрокиРаспределенияНачисленийПоТерриториямИУсловиямТруда.Количество() > 0 Тогда
		...
		Для каждого СтрокаРаспределенияНачисленийПоТерриториямИУсловиямТруда Из СтрокиРаспределенияНачисленийПоТерриториямИУсловиямТруда Цикл
			...
			Если СтрокаСторноПерерасчет.СторнируемыйДокумент = Ссылка
				Или (СтрокаСторноПерерасчет.Результат >= 0) <> (НоваяСтрока.Результат >= 0) Тогда
				НоваяСтрока.Результат = -НоваяСтрока.Результат;
...
Процедура ЗаписатьПредварительныеПерерасчетыПоИзменениямВНаборах(ИзмененияВНаборахИсточников, Регистратор)
	...
	ДобавитьПредварительныеПерерасчетыПоИзменениямДоговоровГПХ(ПредварительныеПерерасчеты, ИзмененияВНаборахИсточников, Регистратор);
...
Процедура ДобавитьПредварительныеПерерасчетыПоИзменениямДоговоровГПХ(ПредварительныеПерерасчеты, ИзмененияВНаборахИсточников, РегистраторПерерасчета)
	
	ИзменившиесяДанныеНабора = ИзмененияВНаборахИсточников[Метаданные.РегистрыСведений.ПлановыеНачисленияПоДоговорам.Имя];
	Если ИзменившиесяДанныеНабора <> Неопределено Тогда
		Для каждого СтрокаИзменения Из ИзменившиесяДанныеНабора Цикл
			
			НоваяСтрока = ПредварительныеПерерасчеты.Добавить();

			СведенияОбИсправлении = ИсправлениеДокументовЗарплатаКадры.СведенияОбИсправленииДокумента(СтрокаИзменения.ДоговорАкт);
			Если СведенияОбИсправлении.ИсправленныйДокумент = Неопределено Тогда
				НоваяСтрока.ИзмененныеДанные = СтрокаИзменения.ДоговорАкт;
				НоваяСтрока.ПериодПерерасчета = СтрокаИзменения.МесяцНачисления;
			Иначе
				НоваяСтрока.ИзмененныеДанные = СведенияОбИсправлении.ИсправленныйДокумент;
				Если СтрокаИзменения.Добавление Тогда
					НоваяСтрока.ПериодПерерасчета = НачалоМесяца(СтрокаИзменения.НовоеЗначениеДатаНачала);
				Иначе
					НоваяСтрока.ПериодПерерасчета = НачалоМесяца(СтрокаИзменения.СтароеЗначениеДатаНачала);
				КонецЕсли;
			КонецЕсли;
			
			НоваяСтрока.ВладелецПерерасчета = СтрокаИзменения.Сотрудник;
			НоваяСтрока.ДокументОснование = СтрокаИзменения.ДоговорАкт;
			НоваяСтрока.РегистраторПерерасчета = РегистраторПерерасчета;
			НоваяСтрока.ТипИзменения = Перечисления.ТипИзмененийПерерасчетов.ДоговорыГПХ;
			
			Если СтрокаИзменения.Добавление Или СтрокаИзменения.Удаление Тогда
				
				Если ТипЗнч(СтрокаИзменения.ДоговорАкт) = Тип("ДокументСсылка.АктПриемкиВыполненныхРаботОказанныхУслуг") Тогда
					ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаИзменения.ДоговорАкт, "МесяцНачисления");
					МесяцНачисления = НачалоМесяца(ЗначенияРеквизитов.МесяцНачисления);
				Иначе
					ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаИзменения.ДоговорАкт, "СпособОплаты,ДатаОкончания");
					Если ЗначенияРеквизитов.СпособОплаты = Перечисления.СпособыОплатыПоДоговоруГПХ.ОднократноВКонцеСрока Тогда
						МесяцНачисления = НачалоМесяца(ЗначенияРеквизитов.ДатаОкончания);
					Иначе
						МесяцНачисления = НоваяСтрока.ПериодПерерасчета;
					КонецЕсли;
				КонецЕсли;
				
				Если МесяцНачисления <> НоваяСтрока.ПериодПерерасчета Тогда
					ДопСтрока = ПредварительныеПерерасчеты.Добавить();
					ЗаполнитьЗначенияСвойств(ДопСтрока, НоваяСтрока);
					ДопСтрока.ПериодПерерасчета = МесяцНачисления;
					ДопСтрока.ТипИзменения = Перечисления.ТипИзмененийПерерасчетов.ПереносДоговораГПХ;
				КонецЕсли;
	...
	ИзменившиесяДанныеНабора = ИзмененияВНаборахИсточников[Метаданные.РегистрыСведений.УсловияДоговораГПХ.Имя];
	Если ИзменившиесяДанныеНабора <> Неопределено Тогда
		Для каждого СтрокаИзменения Из ИзменившиесяДанныеНабора Цикл
			
			НоваяСтрока = ПредварительныеПерерасчеты.Добавить();
			
			СведенияОбИсправлении = ИсправлениеДокументовЗарплатаКадры.СведенияОбИсправленииДокумента(СтрокаИзменения.Договор);
			Если СведенияОбИсправлении.ИсправленныйДокумент = Неопределено Тогда
				НоваяСтрока.ИзмененныеДанные = СтрокаИзменения.Договор;
				НоваяСтрока.ПериодПерерасчета = СтрокаИзменения.Период;
			Иначе
				НоваяСтрока.ИзмененныеДанные = СведенияОбИсправлении.ИсправленныйДокумент;
				Если СтрокаИзменения.Добавление Тогда
			...
			НоваяСтрока.ВладелецПерерасчета = СтрокаИзменения.Сотрудник;
			НоваяСтрока.ДокументОснование = СтрокаИзменения.Договор;
			НоваяСтрока.РегистраторПерерасчета = РегистраторПерерасчета;
			НоваяСтрока.ТипИзменения = Перечисления.ТипИзмененийПерерасчетов.ДоговорыГПХ;
		КонецЦикла;
	КонецЕсли;
...
Процедура СоздатьВТСотрудникиОрганизацииКПерерасчету(МенеджерВременныхТаблиц)
	...
	Запрос.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсходныеДанныеПерерасчетов КАК ИсходныеДанныеПерерасчетов
		|		ПО (ИсходныеДанныеПерерасчетов.ПериодДействия >= ФизическиеЛицаПериоды.ПериодДействия)
		|			И ФизическиеЛицаПериоды.ФизическоеЛицо = ИсходныеДанныеПерерасчетов.ФизическоеЛицо
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Процедура ЗаполнитьСторнированиеНачислений(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели, МесяцНачисления)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СторноНачислений.СторнируемыйДокумент, НЕОПРЕДЕЛЕНО) КАК СторноДокумента,
		|	ЕСТЬNULL(Начисления.СторнируемыйДокумент, ЕСТЬNULL(СторноНачислений.СторнируемыйДокумент, НЕОПРЕДЕЛЕНО)) КАК СторнируемыйДокумент
		|ИЗ
		|	РегистрСведений.ИсходныеДанныеПерерасчетов КАК ИсходныеДанныеПерерасчетов
		...
		|		ПО ИсходныеДанныеПерерасчетов.Регистратор = Начисления.Регистратор
		|			И ИсходныеДанныеПерерасчетов.ИдентификаторСтроки = Начисления.ИдентификаторСтроки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.Начисления КАК СторноНачислений
		|		ПО ИсходныеДанныеПерерасчетов.Регистратор = СторноНачислений.Регистратор
		|			И (ИсходныеДанныеПерерасчетов.ИдентификаторСтроки = -СторноНачислений.ИдентификаторСтроки)
		|			И (ИсходныеДанныеПерерасчетов.Сторно)
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрокаСторноНачисленийПерерасчет.СторнируемыйИсходный =
			?(Выборка.СторнируемыйДокумент = Неопределено, Выборка.Ссылка, Выборка.СторнируемыйДокумент);
		...
		ЗаполнитьРезультатРаспределенияПоТерриториям(НоваяСтрокаСторноНачисленийПерерасчет,
			ТаблицаРаспределенияНачисленийПоТерриториямИУсловиямТруда, ТаблицаПоказателей,
			?(Выборка.СторноДокумента = Неопределено, Выборка.Ссылка, Выборка.СторноДокумента),
			Выборка.ИдентификаторСтрокиВидаРасчета);
...
Процедура ЗаполнитьСторнированиеПоДоговорам(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, РаспределенияТерриторииПоказатели, МесяцНачисления)
	
	ДоговорыПерерасчет = ДанныеПерерасчета.ДоговорыПерерасчет;
	Если ДоговорыПерерасчет = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НачисленияПоДоговорам", ДоговорыГПХРасширенный.НачисленияПоДоговорам());
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИсходныеДанныеПерерасчетов.Регистратор КАК Ссылка,
		|	ИсходныеДанныеПерерасчетов.ПериодДействия КАК ПериодДействия,
		|	ИсходныеДанныеПерерасчетов.Сотрудник КАК Сотрудник,
		|	ИсходныеДанныеПерерасчетов.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ИсходныеДанныеПерерасчетов.ДатаНачала КАК ДатаНачала,
		|	ИсходныеДанныеПерерасчетов.ДатаОкончания КАК ДатаОкончания,
		|	ИсходныеДанныеПерерасчетов.Начисление КАК Начисление,
		|	ИсходныеДанныеПерерасчетов.Результат КАК Результат,
		...
		|	ИсходныеДанныеПерерасчетов.ДокументОснование КАК ДокументОснование,
		|	ИсходныеДанныеПерерасчетов.КодВычета КАК КодВычета,
		|	ИсходныеДанныеПерерасчетов.КодВычетаДополнительный КАК КодВычетаДополнительный,
		|	ИсходныеДанныеПерерасчетов.КодДохода КАК КодДохода,
		|	ЕСТЬNULL(НачисленияПоДоговорам.КодДоходаСтраховыеВзносы, ЗарплатаНачисленияПоДоговорам.КодДоходаСтраховыеВзносы) КАК КодДоходаСтраховыеВзносы,
		|	ИсходныеДанныеПерерасчетов.СуммаВычета КАК СуммаВычета,
		|	ИсходныеДанныеПерерасчетов.СуммаВычетаДополнительная КАК СуммаВычетаДополнительная,
		|	ИсходныеДанныеПерерасчетов.СкидкаПоВзносам КАК СкидкаПоВзносам,
		|	ИсходныеДанныеПерерасчетов.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	ЕСТЬNULL(НачисленияПоДоговорам.ПланируемаяДатаВыплаты, ЗарплатаНачисленияПоДоговорам.ПланируемаяДатаВыплаты) КАК ПланируемаяДатаВыплаты
		|ИЗ
		|	РегистрСведений.ИсходныеДанныеПерерасчетов КАК ИсходныеДанныеПерерасчетов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСторнируемыеРегистраторы КАК СторнируемыеРегистраторы
		|		ПО ИсходныеДанныеПерерасчетов.Регистратор = СторнируемыеРегистраторы.Регистратор
		...
		|			И ИсходныеДанныеПерерасчетов.Сотрудник = СторнируемыеРегистраторы.Сотрудник
		|			И ИсходныеДанныеПерерасчетов.Организация = СторнируемыеРегистраторы.Организация
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.НачислениеЗарплаты.НачисленияПоДоговорам КАК ЗарплатаНачисленияПоДоговорам
		|		ПО ИсходныеДанныеПерерасчетов.Регистратор = ЗарплатаНачисленияПоДоговорам.Ссылка
		|			И ИсходныеДанныеПерерасчетов.ИдентификаторСтроки = ЗарплатаНачисленияПоДоговорам.ИдентификаторСтроки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.НачислениеПоДоговорам.НачисленияПоДоговорам КАК НачисленияПоДоговорам
		|		ПО ИсходныеДанныеПерерасчетов.Регистратор = НачисленияПоДоговорам.Ссылка
		|			И ИсходныеДанныеПерерасчетов.ИдентификаторСтроки = НачисленияПоДоговорам.ИдентификаторСтроки
		|ГДЕ
		|	ИсходныеДанныеПерерасчетов.Начисление В(&НачисленияПоДоговорам)";
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрокаСторно = Обработки.МенеджерРасчетаЗарплаты.СтрокаТаблицыНачисленийПоДоговорам(ДоговорыПерерасчет);
		ЗаполнитьЗначенияСвойств(НоваяСтрокаСторно, Выборка);
		
		НоваяСтрокаСторно.Результат = - Выборка.Результат;
		НоваяСтрокаСторно.СкидкаПоВзносам = - Выборка.СкидкаПоВзносам;
		НоваяСтрокаСторно.СуммаВычета = - Выборка.СуммаВычета;
		НоваяСтрокаСторно.СуммаВычетаДополнительная = - Выборка.СуммаВычетаДополнительная;
		
		НоваяСтрокаСторно.ФиксРасчет = Ложь;
		НоваяСтрокаСторно.ФиксСтрока = Ложь;
		НоваяСтрокаСторно.ФиксЗаполнение = Ложь;
		
		НоваяСтрокаСторно.Сторно = Истина;
		НоваяСтрокаСторно.ФиксСторно = Истина;
		НоваяСтрокаСторно.СторнируемыйДокумент = Выборка.Ссылка;
		
		ЗаполнитьРезультатРаспределения(НоваяСтрокаСторно, ТаблицаРаспределенияНачислений, Выборка.Ссылка, Выборка.ИдентификаторСтроки);
		ЗаполнитьРезультатРаспределенияПоТерриториям(НоваяСтрокаСторно, ТаблицаРаспределенияНачисленийПоТерриториямИУсловиямТруда, ТаблицаПоказателей, Выборка.Ссылка, Выборка.ИдентификаторСтроки);
...
Процедура ЗаполнитьПериодыПерерасчетаСотрудников(МенеджерВременныхТаблиц, ДанныеПерерасчета, МенеджерРасчета);
	...
	Запрос.Текст =
		...
		|	СторнируемыеРегистраторы.ПерерасчетДоговоров КАК ПерерасчетДоговоров,
		|	СторнируемыеРегистраторы.ПерерасчетЛьгот КАК ПерерасчетЛьгот,
		|	ВЫБОР
		|		КОГДА СторнируемыеРегистраторы.ПерерасчетЛьгот
		|				ИЛИ СторнируемыеРегистраторы.ПерерасчетДоговоров
...
Процедура СоздатьВТСторнируемыеРегистраторы(МенеджерВременныхТаблиц, ИсключаемыеРегистраторы)
	...
	Запрос.Текст =
		...
		|			И (ВЫБОР
		|				КОГДА ИсходныеДанныеПерерасчетов.Начисление В (&НачисленияПоДоговорам)
		|					ТОГДА ПерерассчитываемыеСотрудники.ПерерасчетДоговоров
		|							И ПерерассчитываемыеСотрудники.ДокументОснование = ИсходныеДанныеПерерасчетов.ДокументОснование
		...
		|					ТОГДА ПерерассчитываемыеСотрудники.ПерерасчетНачислений
		|				КОГДА ИсходныеДанныеПерерасчетов.Начисление.СпособВыполненияНачисления = ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ПоОтдельномуДокументуДоОкончательногоРасчета)
		...
		|				ИЛИ ПерерассчитываемыеСотрудники.ПерерасчетДоговоров
		|					И ПерерассчитываемыеСотрудники.ДокументОснование = ИсходныеДанныеПерерасчетов.ДокументОснование)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТТипыРегистраторовПоОтдельномуДокументу КАК ТипыРегистраторовПоОтдельномуДокументу
		|		ПО (ТИПЗНАЧЕНИЯ(ИсходныеДанныеПерерасчетов.Регистратор) = ТипыРегистраторовПоОтдельномуДокументу.ТипРегистратора)
		|			И (ИсходныеДанныеПерерасчетов.Начисление.ВидДокументаНачисления = ТипыРегистраторовПоОтдельномуДокументу.ВидДокументаНачисления)
