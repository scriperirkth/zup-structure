...
Процедура ПриОпределенииЭтоДокументСЭДО(Признак, Правило) Экспорт
	...
	Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗадачиБухгалтера") Тогда
		МодульЗадачиБухгалтераБЗККлиентСервер = ОбщегоНазначенияБЗККлиентСервер.ОбщийМодуль("ЗадачиБухгалтераБЗККлиентСервер");
		МодульЗадачиБухгалтераБЗККлиентСервер.ПриОпределенииЭтоДокументСЭДО(Признак, Правило);
	КонецЕсли;
...
Функция РазрешеноРедактироватьСторно(Форма, ИмяТаблицы) Экспорт
	
	ИмяРеквизита = ИмяРеквизитаРазрешеноРедактироватьСторно(ИмяТаблицы);
...
Функция ТерриториальныеУсловияПримененияСевренойНадбавки(ТерриториальныеУсловия) Экспорт
	Возврат ПерсонифицированныйУчетКлиентСервер.ТерриториальныеУсловияПФРВКоторыхПрименяетсяСевренаяНадбавка().Найти(ТерриториальныеУсловия) <> Неопределено;
КонецФункции
...
// Возвращает расшифровки выделенных ячеек в табличном документе.
//
// Параметры:
//   ТабличныйДокумент - ТабличныйДокумент - табличный документ, расшифровки которого получаются.
...
//
Функция РасшифровкиВыделенныхЯчеек(Знач ТабличныйДокумент, ВыделенныеОбласти) Экспорт 
	...
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбласти Цикл
		...
		Для НомерКолонки = ВыделеннаяОбластьЛево По ВыделеннаяОбластьПраво Цикл
			...
			Для НомерСтроки = ВыделеннаяОбластьВерх По ВыделеннаяОбластьНиз Цикл
				...
				Если ТипЗнч(Ячейка) = Тип("ОбластьЯчеекТабличногоДокумента") 
					И Ячейка.Видимость = Истина 
					И Ячейка.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
					И Ячейка.Расшифровка <> Неопределено Тогда
...
Функция ПродолжительностьСтажа(Знач ОписаниеСтажа, Знач ДатаАктуальности) Экспорт 
	...
	Если НачалоДня(ДатаАктуальности) = НачалоДня(ОписаниеСтажа.ДатаОтсчета) Или ОписаниеСтажа.Прерван Тогда
		Возврат ПродолжительностьСтажаПоДаннымОСтаже(ОписаниеСтажа.Лет, ОписаниеСтажа.Месяцев, ОписаниеСтажа.Дней);
	КонецЕсли;
		
	Возврат КадровыйУчетРасширенныйВызовСервера.ПродолжительностьСтажа(ОписаниеСтажа, НачалоДня(ДатаАктуальности));
