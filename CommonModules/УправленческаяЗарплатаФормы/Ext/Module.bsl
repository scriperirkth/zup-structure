	...
	Если НачисленияПозиции <> Неопределено Тогда
		...
		Для Каждого ОписаниеНачисления Из НачисленияПозиции Цикл
			...
			Если ЗарплатаКадрыРасширенныйКлиентСервер.РедактироватьНачисленияВОтдельныхПолях(1, ОписаниеТаблицыНачислений) Тогда
				...
				Если (ЗарплатаКадрыРасширенный.ЭтоНачислениеРайонныйКоэффициент(ОписаниеНачисления.Начисление)
					Или ЗарплатаКадрыРасширенный.ЭтоНачислениеСевернаяНадбавка(ОписаниеНачисления.Начисление)
					Или ЗарплатаКадрыРасширенный.ЭтоНачислениеНадбавкаЗаВредность(ОписаниеНачисления.Начисление)) Тогда
					
					Если ЗарплатаКадрыРасширенный.ЭтоНачислениеНадбавкаЗаВредность(ОписаниеНачисления.Начисление) Тогда
					...
					ИначеЕсли ЗарплатаКадрыРасширенный.ЭтоНачислениеРайонныйКоэффициент(ОписаниеНачисления.Начисление) Тогда
					...
					ИначеЕсли ЗарплатаКадрыРасширенный.ЭтоНачислениеСевернаяНадбавка(ОписаниеНачисления.Начисление) Тогда
		...
		Для каждого СтрокаНачислений Из ТЧНачисления Цикл
			...
			Если НачисленияТекущейПозиции.Получить(Начисление) = Истина
				Или Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеТекущейПозиции, "ИспользоватьУправленческиеНачисления", Ложь) Тогда
				
				Если СодержитПолеДействие Тогда
					Если НачисленияИзменяемыхСтрок.Получить(Начисление) <> Истина
						Или Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеПозиции, "ИспользоватьУправленческиеНачисления", Ложь) Тогда
...
Процедура ДополнительныеПараметрыФормаСпискаПриСозданииНаСервере(Форма) Экспорт
	
	Если Не УправленческаяЗарплата.ИспользоватьУправленческуюЗарплату() Тогда 
		Возврат;
	КонецЕсли;
	
	Форма.Параметры.ДополнительныеПараметры.Вставить("ИспользоватьНесколькоОрганизаций", Истина);
...
Процедура ДанныеДляРасчетаЗарплатыФормаСпискаПриСозданииНаСервере(Форма) Экспорт
	
	Если УправленческаяЗарплата.ИспользоватьУправленческуюЗарплату() 
		...
		МодульОрганизационнаяСтруктура.УстановитьПараметрыВыбораУправленческойОрганизации(Форма, Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ДанныеДляРасчетаЗарплатыФормаСпискаПриОбновленииСоставаПользовательскихНастроекНаСервере(Форма, СтандартнаяОбработка) Экспорт
