...
// Возвращает суммы начисленных выплат в разрезе документов, физических лиц и периодов взаиморасчетов.
//
// Параметры:
//   ФизлицаДокументы 									- ТаблицаЗначений - таблица с колонками:
//     * ФизическоеЛицо - СправочникСсылка.ФизическиеЛица.
//     * ДокументОснование - ДокументСсылка - документ, зарегистрировавший выплаты.
//     * ПериодВзаиморасчетов - Дата - период, за который зарегистрированы выплаты. 
...
//
Функция НачисленныеВыплатыФизическихЛицПоДокументам(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование) Экспорт
	...
	Запрос.Текст =
		...
		|	СУММА(ЕСТЬNULL(ВзаиморасчетыСКонтрагентамиАкционерами.СуммаВзаиморасчетовПриход, 0)) КАК Сумма
		|ИЗ
		|	ВТФизлицаДокументы КАК ФизлицаДокументы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВзаиморасчетыСКонтрагентамиАкционерами.Обороты(, , Запись, ) КАК ВзаиморасчетыСКонтрагентамиАкционерами
		|		ПО ФизлицаДокументы.ФизическоеЛицо = ВзаиморасчетыСКонтрагентамиАкционерами.ФизическоеЛицо
		|			И ФизлицаДокументы.ДокументОснование = ВзаиморасчетыСКонтрагентамиАкционерами.ДокументОснование
		|			И ФизлицаДокументы.ПериодВзаиморасчетов = ВзаиморасчетыСКонтрагентамиАкционерами.Период
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный") Тогда
		Если РассчитыватьВзысканныеСуммыПропорциональноВыплате Тогда
			Если ЗначениеЗаполнено(Финансирование.СтатьяРасходов) Тогда
				ОтборПоСтатьеРасходов = "(ВзаиморасчетыСКонтрагентамиАкционерами.СтатьяРасходов = &СтатьяРасходов)";
			...
			Если ЗначениеЗаполнено(Финансирование.СтатьяФинансирования) Тогда
				ОтборПоСтатьеФинансирования = "(ВзаиморасчетыСКонтрагентамиАкционерами.СтатьяФинансирования = &СтатьяФинансирования)";
...
Функция ЗапросОстаткиВзаиморасчетовСКонтрагентамиАкционерами(Регистратор, МоментВремени, Организация, СпособРасчетов, Отбор, ТолькоРазрешенные = Истина) Экспорт
	...
	|	Остатки.ДокументОснование КАК ДокументОснование,
	|	НАЧАЛОПЕРИОДА(&МоментВремени, МЕСЯЦ) КАК ПериодВзаиморасчетов,
	|	Остатки.СтатьяФинансирования КАК СтатьяФинансирования,
	|	Остатки.СтатьяРасходов КАК СтатьяРасходов,
	...
	|		И Взаиморасчеты.Период <= &МоментВремени
	|		И Взаиморасчеты.Организация = &Организация
