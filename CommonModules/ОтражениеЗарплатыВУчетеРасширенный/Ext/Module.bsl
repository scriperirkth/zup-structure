...
Функция ОграниченияВзысканийПоРабочимМестамИСтатьям(Организация, ПериодРегистрации, ОграниченияВзысканий, ИсключаемыйРегистратор) Экспорт
	...
	Если РаботаВБюджетномУчреждении Тогда
		...
		ПустыеЗначенияПервичныйРегистратор = Новый Массив;
		ПустыеЗначенияПервичныйРегистратор.Добавить(Неопределено);
		ПустыеЗначенияПервичныйРегистратор.Добавить(Null);
		Для Каждого ТипДокументОснование Из Метаданные.ОпределяемыеТипы.РегистраторыБухучетНачисленияУдержанияПоСотрудникам.Тип.Типы() Цикл
			ПустыеЗначенияПервичныйРегистратор.Добавить(ОбщегоНазначенияБЗК.МенеджерОбъектаПоТипу(ТипДокументОснование).ПустаяСсылка());
		КонецЦикла;
		Запрос.УстановитьПараметр("ПустыеЗначенияПервичныйРегистратор", ПустыеЗначенияПервичныйРегистратор);
		
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА Удержания.ПервичныйРегистратор В (&ПустыеЗначенияПервичныйРегистратор)
		|			ТОГДА Удержания.Регистратор
		|		ИНАЧЕ Удержания.ПервичныйРегистратор
		...
		|	Удержания.ДанныеМежрасчетногоПериода КАК ДанныеМежрасчетногоПериода
		|ИЗ
		...
		|	И Удержания.ФизическоеЛицо В(&ФизическиеЛица)
		|	И Удержания.НачислениеУдержание В(&Удержания)
		...
		|	Удержания.ДанныеМежрасчетногоПериода,
		|	ВЫБОР
		|		КОГДА Удержания.ПервичныйРегистратор В (&ПустыеЗначенияПервичныйРегистратор)
		|			ТОГДА Удержания.Регистратор
		|		ИНАЧЕ Удержания.ПервичныйРегистратор
	...
	Если РаботаВБюджетномУчреждении Тогда
		РаспределениеУдержаний.Колонки.Добавить(
	...
	Для каждого СтрокаУдержаний Из ОграниченияВзысканий Цикл
		...
		СтрокиВсеУдержания = Новый Массив;
		СтрокиОкончательныйРасчет = Новый Массив;
		ИтогОкончательныйРасчет = 0;
		...
		Если СтрокаУдержаний.КатегорияУдержания <> Перечисления.КатегорииУдержаний.ВознаграждениеПлатежногоАгента
			...
			Для каждого СтрокаБазы Из НайденныеСтроки Цикл
				Если СтрокаБазы.ДокументОснование = СтрокаУдержаний.ДокументОснование Тогда
					Если Не СтрокаБазы.ДанныеМежрасчетногоПериода Тогда
						СтрокиОкончательныйРасчет.Добавить(СтрокаБазы);
						ИтогОкончательныйРасчет = ИтогОкончательныйРасчет + СтрокаБазы.Сумма;
					КонецЕсли;
					СтрокиВсеУдержания.Добавить(СтрокаБазы);
				КонецЕсли;
			КонецЦикла;
		...
		Иначе
			Для каждого СтрокаБазы Из НайденныеСтроки Цикл
				...
				КонецЕсли;
			КонецЦикла;
			СтрокиВсеУдержания = НайденныеСтроки;
		КонецЕсли;
		
		Если ИтогОкончательныйРасчет < -СтрокаУдержаний.Сумма Тогда
			СтрокиБазы = СтрокиВсеУдержания;
		Иначе
			СтрокиБазы = СтрокиОкончательныйРасчет;
		КонецЕсли;
		...
		Иначе
			...
			Для Индекс = 0 По СтрокиБазы.Количество() - 1 Цикл
				...
				Если РаботаВБюджетномУчреждении Тогда
					НоваяСтрока.ПервичныйРегистратор = СтрокаРаспределения.ПервичныйРегистратор;
				КонецЕсли;
...
Процедура СоздатьВТРаспределениеНачисленийДляНДФЛ(ПараметрыРаспределения, СуществуетТаблицаУсловий, ИсключаемыеРегистраторы)
	...
	Запрос.УстановитьПараметр("ИсключаемыеНачисления", ВидыОсобыхНачисленийЕдиновременныеПособия());
	...
	Если Не СуществуетТаблицаУсловий Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНатуральныеДоходыДляБазыНДФЛ КАК НатуральныеДоходы
		|		ПО Начисления.НачислениеУдержание = НатуральныеДоходы.Ссылка
		|ГДЕ
		...
		|	И НЕ Начисления.НачислениеУдержание В (&ИсключаемыеНачисления)
	...
	ПолеНатуральныйДоход = "
	|	ВЫБОР
	|		КОГДА НатуральныеДоходы.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК НатуральныйДоход";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЛОЖЬ КАК НатуральныйДоход", ПолеНатуральныйДоход);
...
Процедура ДополнитьТаблицуНачислений(ПериодРегистрации, Организации, МенеджерВременныхТаблиц, Сотрудники) Экспорт
	...
	|	ВременнаяТаблица.ИсходныйДокумент КАК ИсходныйДокумент
...
Функция ДанныеДляРаспределенияНДФЛ(ПараметрыРаспределения) Экспорт
	...
	СоздатьВТНатуральныеДоходыДляБазыНДФЛ(МенеджерВременныхТаблиц);
	УдалитьВТ.Добавить("ВТНатуральныеДоходыДляБазыНДФЛ");
	...
	Если ОкончательныйРасчет Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНатуральныеДоходыДляБазыНДФЛ КАК НатуральныеДоходы
		|		ПО РаспределениеНачислений.Начисление = НатуральныеДоходы.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	РаспределениеНачислений.Сотрудник,
		|	РаспределениеНачислений.Подразделение,
		...
		|	РаспределениеНачисленийДляБазыНДФЛ.НатуральныйДоход КАК НатуральныйДоход,
...
Функция НДФЛПоРабочимМестамИСтатьям(ТаблицаНДФЛ, База, БазаВсеНачисления,
	...
	Если База.Колонки.Найти("НатуральныйДоход") = Неопределено Тогда
		База.Колонки.Добавить("НатуральныйДоход", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	ОбщегоНазначенияБЗК.ДобавитьИндексКоллекции(БазаВсеНачисления, "ФизическоеЛицо,НатуральныйДоход");
	ОтборВсеНачисления = Новый Структура("ФизическоеЛицо,НатуральныйДоход");
	ОтборВсеНачисления.НатуральныйДоход = Ложь;
	ОтборНатуральныйДоход = Новый Структура("ФизическоеЛицо,НатуральныйДоход");
	ОтборНатуральныйДоход.НатуральныйДоход = Истина;
	...
	// // Строки с положительной суммой НДФЛ, для которых нет базы, и исчислен с натуральных доходов.
	...
	НатуральныеДоходы = Перечисления.КатегорииДоходовНДФЛ.Натуральные();
	...
	Для Каждого СтрокаНДФЛ Из ТаблицаНДФЛ Цикл
		...
		Если СтрокиРаспределения.Количество() = 0 И НатуральныеДоходы.Найти(СтрокаНДФЛ.КатегорияДохода) <> Неопределено Тогда
			ОтборНатуральныйДоход.ФизическоеЛицо = СтрокаНДФЛ.ФизическоеЛицо;
			СтрокиРаспределения = БазаВсеНачисления.НайтиСтроки(ОтборНатуральныйДоход);
		КонецЕсли;
...
Процедура СоздатьВТНатуральныеДоходыДляБазыНДФЛ(МенеджерВТ)

	КатегриииНачислений = Новый Массив;
	КатегриииНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаТрудаВНатуральнойФорме);
	КатегриииНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ДоходВНатуральнойФорме);
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	Запрос.УстановитьПараметр("КатегорииНачислений", КатегриииНачислений);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Начисления.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТНатуральныеДоходыДляБазыНДФЛ
	|ИЗ
	|	ПланВидовРасчета.Начисления КАК Начисления
	|ГДЕ
