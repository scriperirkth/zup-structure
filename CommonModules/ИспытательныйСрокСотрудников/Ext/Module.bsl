...
Процедура ПриЗаполненииСпискаТекущихДел_ДобавитьДело(ТекущиеДела, Знач МенеджерВременныхТаблиц, НачалоПериода, КонецПериода) Экспорт
	...
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ИспытательныйСрокСотрудников) Тогда
		Возврат;
	КонецЕсли;
	
	Раздел                 = "ИспытательныйСрокСотрудников";
	ПредставлениеЗаголовка = НСтр("ru = 'Испытательный срок'");
	Представление          = НСтр("ru = 'Завершение испытательного срока'");
	Подсказка              = НСтр("ru = 'Завершается испытательный срок принятых на работу сотрудников.'");
	
	ДелоЗаголовок = ТекущиеДела.Добавить();
	ДелоЗаголовок.Идентификатор  = Раздел;
	ДелоЗаголовок.Важное         = Ложь;
	ДелоЗаголовок.Владелец       = Метаданные.Подсистемы.Кадры;
	...
	ПараметрыЗаголовка.Вставить("Отбор", Новый Структура("Дата", НачалоПериода));
	ПараметрыЗаголовка.Вставить("СформироватьПриОткрытии", Истина);
	ПараметрыЗаголовка.Вставить("КлючВарианта", "СотрудникиНаИспытательномСроке");
	ДелоЗаголовок.Форма          = "Отчет.ИспытательныйСрокСотрудников.Форма";
	...
	|		ПО (ИспытательныйСрокСотрудников.Сотрудник = ВТСотрудники.Сотрудник)
	|ГДЕ
	|	ИспытательныйСрокСотрудников.ДатаЗавершения МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|УПОРЯДОЧИТЬ ПО
	|	ИспытательныйСрокСотрудников.Организация";
	...
	Для Каждого КлючИЗначение Из ДанныеОрганизаций Цикл
		...
		Дело.Идентификатор  = "ИспытательныйСрокСотрудников" + СтрЗаменить(Организация.УникальныйИдентификатор(), "-", "");
		...
		ПараметрыФормы.Вставить("Отбор", Новый Структура("Дата", НачалоПериода));
		ПараметрыФормы.Вставить("СформироватьПриОткрытии", Истина);
		ПараметрыФормы.Вставить("КлючВарианта", "СотрудникиНаИспытательномСроке");
		...
		ПользовательскиеНастройкиОтчета = КадровыйУчетРасширенный.ПользовательскиеНастройкиОтчетаПоСотрудникам(
			"ИспытательныйСрокСотрудников",
			"СотрудникиНаИспытательномСроке",
			СоответствиеОтборов);
		ПараметрыФормы.Вставить("ПользовательскиеНастройки", ПользовательскиеНастройкиОтчета);
		
		Дело.ПараметрыФормы = ПараметрыФормы;
		Дело.Форма          = "Отчет.ИспытательныйСрокСотрудников.Форма";
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОбновитьДанныеИспытательногоСрокаСотрудников");
	Методы.Вставить("ОбновитьДатуЗавершенияИспытательногоСрокаСотрудниковПриемНаРаботуСписком");
	Методы.Вставить("ОбновитьДанныеПриемНаРаботу");
	Методы.Вставить("ОбновитьДатуЗавершенияИспытательногоСрокаСотрудниковПриемНаРаботу");
	Методы.Вставить("ОбновитьДанныеПриемНаРаботуСписком");
КонецПроцедуры
...
Процедура СформироватьДвиженияИспытательногоСрока(Движения, ДанныеДляПроведения) Экспорт
	...
	ДатыЗавершения = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДанныеДляПроведения.ИспытательныйСрокСотрудников.ВыгрузитьКолонку("ДатаЗавершения"));
	РабочиеДни = КалендарныеГрафики.БлижайшиеРабочиеДаты(ПроизводственныйКалендарь, ДатыЗавершения, ПараметрыПолучения);
	...
	Для Каждого СтрокаТаблицы Из ДанныеДляПроведения.ИспытательныйСрокСотрудников Цикл
		...
		БлижайшийРабочийДень = РабочиеДни[НоваяСтрока.ДатаЗавершения];
		Если БлижайшийРабочийДень <> Неопределено И БлижайшийРабочийДень <> НоваяСтрока.ДатаЗавершения Тогда
			НоваяСтрока.ДатаЗавершения = БлижайшийРабочийДень;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьСостояниеСотрудника(Форма, ПараметрыСостояния) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыСостояния.Сотрудник) Тогда
		...
		ДатаУвольненияЗаполнена = ЗначениеЗаполнено(Форма.ДатаУвольнения);
		Если ДатаУвольненияЗаполнена И ТекущаяДата > Форма.ДатаУвольнения Тогда
			Возврат;
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			Если ДатаУвольненияЗаполнена И Выборка.ДатаЗавершения > Форма.ДатаУвольнения Тогда
				Продолжить;;
			КонецЕсли;
...
Процедура ДополнитьТекстыЗапросовФормыВыбораСотрудников(ТекстЗапроса) Экспорт
	...
	Если Не ДоступноЧтениеСостоянийИспытательногоСрокаСотрудников() Тогда
		Возврат;
	КонецЕсли;
...
#Область ДлительностьИспытательногоСрока

Функция ДобавитьПолеОставшаясяДлительностьИспытательногоСрока(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
	
	ДобавленоПолеСведений = Ложь;
	Если НеобходимыСведенияОставшаясяДлительностьИспытательногоСрока(ИмяПоля) Тогда
		ДобавленоПолеСведений = Истина;
		ИсточникиДанных.Вставить("ДанныеИспытательногоСрокаСотрудников", Истина);
		
		ПутьКДанным = ПутьКДаннымОставшаясяДлительностьИспытательногоСрока(ИмяПоля);
...
Процедура ДобавитьТекстЗапросаВТДанныеИспытательногоСрокаСотрудников(Запрос, ТолькоРазрешенные,
			ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных) Экспорт

	Если ИсточникиДанных.Получить("ДанныеИспытательногоСрокаСотрудников") = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ЗапросВТ = ЗапросВТДанныеИспытательногоСрокаСотрудников(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов,
	...
	ДобавитьСоединениеВТДанныеИспытательногоСрокаСотрудников(ЧастиЗапроса, ОписательВременнойТаблицыОтборов);
	
	Запрос.Текст = СтрСоединить(ЧастиЗапроса, Символы.ПС);
	
	КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ВТКУничтожению", "ВТДанныеИспытательногоСрокаСотрудников");
	
КонецПроцедуры

#КонецОбласти

Функция ДобавитьПолеСведенийОбИспытательномСроке(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных, ОписательВременнойТаблицыОтборов) Экспорт
...
Функция ДоступноЧтениеСостоянийИспытательногоСрокаСотрудников()

	Возврат ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ДанныеИспытательногоСрокаСотрудников);
...
Функция ЗапросВТДанныеИспытательногоСрокаСотрудников(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов,
			ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень,
			ИмяВТДанныеИспытательногоСрокаСотрудников = "ВТДанныеИспытательногоСрокаСотрудников")
	...
	ЗапросВТИмяРегистраСрез = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросВТИмяРегистраСрез("ДанныеИспытательногоСрокаСотрудников",
		ТолькоРазрешенные, ОписаниеФильтра, ПараметрыПостроения, Истина, ИмяВТДанныеИспытательногоСрокаСотрудников);
		
	Возврат ЗапросВТИмяРегистраСрез;

КонецФункции

Функция НеобходимыСведенияОставшаясяДлительностьИспытательногоСрока(Знач ИмяПоля)
	
	ИмяПоля = ВРег(ИмяПоля);
	
	Возврат ИмяПоля = ВРег("ОставшаясяДлительностьИспытательногоСрока");
		
КонецФункции

Функция ПутьКДаннымОставшаясяДлительностьИспытательногоСрока(Знач ИмяПоля)
	
	ИмяПоляВВерхнемРегистре = ВРег(ИмяПоля);
	
	ПутьКДанным = "";
	
	Если ИмяПоляВВерхнемРегистре = ВРег("ОставшаясяДлительностьИспытательногоСрока") Тогда
		ПутьКДанным = "
		|ВЫБОР
		|	КОГДА ЕСТЬNULL(ДанныеИспытательногоСрокаСотрудников.ДатаЗавершения, ДАТАВРЕМЯ(1, 1, 1)) = ДАТАВРЕМЯ(1, 1, 1)
		|			ИЛИ ДанныеИспытательногоСрокаСотрудников.ДатаЗавершения < ДанныеИспытательногоСрокаСотрудников.Период
		|		ТОГДА 0
		|	ИНАЧЕ РАЗНОСТЬДАТ(ЕСТЬNULL(ДанныеИспытательногоСрокаСотрудников.Период, ДАТАВРЕМЯ(1, 1, 1)), ДОБАВИТЬКДАТЕ(ЕСТЬNULL(ДанныеИспытательногоСрокаСотрудников.ДатаЗавершения, ДАТАВРЕМЯ(1, 1, 1)), ДЕНЬ, 1), МЕСЯЦ)
		|КОНЕЦ";
	КонецЕсли;
...
Процедура ДобавитьСоединениеВТДанныеИспытательногоСрокаСотрудников(ЧастиЗапроса, ОписательВременнойТаблицыОтборов)
	
	ТекстЗапроса = ЧастиЗапроса.Получить(ЧастиЗапроса.ВГраница());
	
	Если СтрНайти(ТекстЗапроса, "ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеИспытательногоСрокаСотрудников КАК") > 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЧастиЗапроса.Добавить(
		"		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеИспытательногоСрокаСотрудников КАК ДанныеИспытательногоСрокаСотрудников
		|		ПО ТаблицаОтборов." + ОписательВременнойТаблицыОтборов.ИмяПоляСотрудник + " = ДанныеИспытательногоСрокаСотрудников.Сотрудник
		|			И ТаблицаОтборов." + ОписательВременнойТаблицыОтборов.ИмяПоляПериод + " = ДанныеИспытательногоСрокаСотрудников.Период");
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ИсключаемыеПоля) Экспорт
	...
	ОписаниеПоля.ИмяПоля = "НаходитсяНаИспытательномСроке";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЛОЖЬ";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru='Находится на испытательном сроке'");
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".НаходитсяНаИспытательномСроке";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "ИспытательныйСрокДатаЗавершения";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЛОЖЬ";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru='Дата завершения испытательного срока'");
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".ИспытательныйСрокДатаЗавершения";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "ОставшаясяДлительностьИспытательногоСрока";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "0";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru='Оставшаяся длительность испытательного срока'");
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".ОставшаясяДлительностьИспытательногоСрока";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
