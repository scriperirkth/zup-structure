...
//
// Параметры:
//   КраткоеИмяПодсистемы - Строка - краткое имя объекта метаданных подсистема, без иерархии.
//
// Возвращаемое значение:
//   Булево - Истина, если подсистема существует.
//
Функция ПодсистемаСуществует(Знач КраткоеИмяПодсистемы) Экспорт
	
	КраткоеИмяПодсистемы = НРег(СокрЛП(КраткоеИмяПодсистемы));
	
	Для Каждого ИмяПодсистемы Из ИменаПодсистем() Цикл
		ЧастиИмени = СтрРазделить(ИмяПодсистемы.Ключ, ".", Ложь);
		Если КраткоеИмяПодсистемы = НРег(СокрЛП(ЧастиИмени[ЧастиИмени.ВГраница()])) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
