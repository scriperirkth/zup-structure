...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИнтерактивноеОбновлениеРегламентированныхОтчетовИзСервиса", Истина);
	Методы.Вставить("ИнтерактивноеОбновлениеРегламентированныхОтчетовИзФайла", Истина);
	Методы.Вставить("ОбновлениеРегламентированныхОтчетов", Истина);
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Для Каждого Характеристика Из Дескриптор.Properties.Property Цикл
		...
		ИначеЕсли Характеристика.Code = "ВерсииПрограмм" Тогда
			ВерсииПрограмм = Характеристика.Value;
		Иначе
			Продолжить;
		КонецЕсли;
	КонецЦикла;
	
	Если Не ПоставляемыеДанныеСоответствуютВерсииПрограммы(Идентификатор, Версия, ВерсииПрограмм) Тогда
		Возврат;
	КонецЕсли;
...
// Проверяет соответствуют ли поставляемые данные версии программы.
//
// Параметры:
//  Идентификатор	 - Строка - значение свойства "Идентификатор";
//  Версия			 - Строка - значение свойства "Версия";
//  ВерсииПрограмм	 - Строка - значение свойства "ВерсииПрограмм".
// 
// Возвращаемое значение:
//  Булево - наличие в характеристике текущей версии программы.
//
Функция ПоставляемыеДанныеСоответствуютВерсииПрограммы(
		Идентификатор,
		Версия,
		ВерсииПрограмм)
	
	Результат = Ложь;
	ХарактеристикаВерсийПрограмм = Новый Массив;
	
	Если Не ПустаяСтрока(ВерсииПрограмм) Тогда
		ХарактеристикаВерсийПрограмм = ОбщегоНазначения.ЗначениеИзСтрокиXML(ВерсииПрограмм);
	КонецЕсли;
	
	Если ХарактеристикаВерсийПрограмм.Количество() = 0 Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В характеристиках отсутствует информация о версиях программ.
				|Не удалось обработать поставляемые данные регламентированного отчета:
				|Идентификатор: %1
				|Версия: %2'"),
			Идентификатор,
			Версия);
	...
	ИмяПрограммы    = ИнтернетПоддержкаПользователей.ИмяКонфигурации();
	ВерсияПрограммы = ИнтернетПоддержкаПользователей.ВерсияКонфигурации();
	
	Для Каждого ХарактеристикаВерсииПрограммы Из ХарактеристикаВерсийПрограмм Цикл
		Если ИмяПрограммы = ХарактеристикаВерсииПрограммы.Программа
			И ВерсияПрограммы = ХарактеристикаВерсииПрограммы.Версия Тогда
	...
	Если Не Результат Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В переданной характеристике отсутствует информация о текущей версии программы.
				|Не удалось обработать поставляемые данные регламентированного отчета:
				|Идентификатор: %1
				|Версия: %2'"),
			Идентификатор,
