...
Процедура РассчитатьУдержанияПоИсполнительнымЛистам(СпособРасчета, СтрокиРасчета, МенеджерВременныхТаблиц, ИсключаемыйРегистратор, МесяцНачисления, ПогашениеЗадолженности = Неопределено) Экспорт
	...
	ОграничиватьСуммуУдержаний = ПолучитьФункциональнуюОпцию("ОграничиватьСуммуУдержанийПроцентомОтЗаработнойПлаты");
	...
	Пока Выборка.СледующийПоЗначениюПоля("ИсполнительныйДокумент") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			РасчетФиксированнойСуммой = Выборка.СпособРасчета = Перечисления.СпособыРасчетаУдержанияПоИсполнительномуДокументу.ФиксированнойСуммой;
			...
			Иначе 
				...
				КонецЕсли;
				// Расчетная база при расчете фиксированной суммой показатели не используются
				СтрокаПоказателя = РасчетЗарплатыРасширенный.СтрокаЗначенияПоказателяПоИдентификатору(Строка.Показатели, "РасчетнаяБазаИсполнительногоЛиста");
				...
				КонецЕсли;
				СтрокаПоказателя.Значение = ?(РасчетФиксированнойСуммой, 0, РасчетнаяБазаИсполнительногоЛиста);
				// НДФЛ
				СтрокаПоказателя = РасчетЗарплатыРасширенный.СтрокаЗначенияПоказателяПоИдентификатору(Строка.Показатели, "ИсчисленныйПоИсполнительномуЛистуНДФЛ");
				Если СтрокаПоказателя = Неопределено Тогда 
					СтрокаПоказателя = Строка.Показатели.Добавить();
				...
				КонецЕсли;
				СтрокаПоказателя.Значение = ?(РасчетФиксированнойСуммой, 0, ИсчисленныйПоИсполнительномуЛистуНДФЛ);
			КонецЕсли;
			...
			Если РасчетФиксированнойСуммой Тогда
				Строка.УдерживатьВЦеломЗаМесяц = Истина;
				Если ОграничиватьСуммуУдержаний Или (ЭтоБольничный И РасчетнаяБаза = 0) Тогда
					Строка.Результат = Выборка.Сумма;
				Иначе
					Строка.Результат = ?(РасчетнаяБаза < Выборка.Сумма, РасчетнаяБаза, Выборка.Сумма);
				КонецЕсли;
...
Функция УдержанныеСуммыФизическихЛицПоДокументам(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование, ОкончательныйРасчет = Ложь, УчитыватьСторно = Истина) Экспорт
	...
	УчитыватьОграничения = ОкончательныйРасчет И ПолучитьФункциональнуюОпцию("ОграничиватьСуммуУдержанийПроцентомОтЗаработнойПлаты");
	Возврат УдержанныеСуммыФизическихЛиц(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование, УчитыватьСторно, УчитыватьОграничения);
...
Функция УдержанныеСуммыФизическихЛицПоДокументамАвансом(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(НачисленияУдержанияПоСотрудникамАвансом.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)) КАК СтатьяРасходов,
		|	ЕСТЬNULL(НачисленияУдержанияПоСотрудникамАвансом.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)) КАК СтатьяФинансирования,
		...
		|	ЕСТЬNULL(НачисленияУдержанияПоСотрудникамАвансом.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)),
		|	ЕСТЬNULL(НачисленияУдержанияПоСотрудникамАвансом.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ФизлицаДокументы.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ФизлицаДокументы.ДокументОснование КАК ДокументОснование,
		|	ЕСТЬNULL(ВТВзысканныеСуммыАвансом.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)) КАК СтатьяРасходов,
		|	ЕСТЬNULL(ВТВзысканныеСуммыАвансом.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)) КАК СтатьяФинансирования,
		...
		|	ФизлицаДокументы.ФизическоеЛицо,
		|	ФизлицаДокументы.ДокументОснование,
		|	ЕСТЬNULL(ВТВзысканныеСуммыАвансом.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)),
		|	ЕСТЬNULL(ВТВзысканныеСуммыАвансом.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТВзысканныеСуммыАвансом";
	
	ОтборПоСтатьеРасходов 		= "ИСТИНА";
	ОтборПоСтатьеФинансирования = "ИСТИНА";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы") Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный") Тогда
			Если РассчитыватьВзысканныеСуммыПропорциональноВыплате Тогда
				Если ЗначениеЗаполнено(Финансирование.СтатьяРасходов) Тогда
					ОтборПоСтатьеРасходов = "НачисленияУдержанияПоСотрудникамАвансом.СтатьяРасходов = &СтатьяРасходов";
					Запрос.УстановитьПараметр("СтатьяРасходов", Финансирование.СтатьяРасходов);
				КонецЕсли;
				Если ЗначениеЗаполнено(Финансирование.СтатьяФинансирования) Тогда
					ОтборПоСтатьеФинансирования = "НачисленияУдержанияПоСотрудникамАвансом.СтатьяФинансирования = &СтатьяФинансирования";
					Запрос.УстановитьПараметр("СтатьяФинансирования", Финансирование.СтатьяФинансирования);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоСтатьеРасходов", ОтборПоСтатьеРасходов);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоСтатьеФинансирования", ОтборПоСтатьеФинансирования);
...
Функция УдержанияПоИсполнительнымДокументамДляПлатежейПоРезультатамРасчета(Организации, МесяцНачисления) Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТУдержания.Получатель КАК Получатель
		|ИЗ
		|	ВТУдержания КАК ВТУдержания";
	
	Получатели = Запрос.Выполнить().Выгрузить(); 
	ПолучателиУдержаний = ОбщегоНазначения.ВыгрузитьКолонку(Получатели, "Получатель", Истина);
	
	УчетПрочихДоходов.СоздатьВТОсновныеБанковскиеСчетаКонтрагентов(Запрос.МенеджерВременныхТаблиц, ПолучателиУдержаний); 
	...
	Запрос.Текст = 
		...
		|	Удержания.ИсполнительныйДокумент КАК ИсполнительныйДокумент,
		|	ОсновныеБанковскиеСчетаКонтрагентов.Банк КАК БанкПолучателя,
		|	ОсновныеБанковскиеСчетаКонтрагентов.НомерСчета КАК СчетПолучателя
		|ИЗ
		|	ВТУдержания КАК Удержания
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОсновныеБанковскиеСчетаКонтрагентов КАК ОсновныеБанковскиеСчетаКонтрагентов
		|		ПО Удержания.Получатель = ОсновныеБанковскиеСчетаКонтрагентов.Контрагент";
...
Функция ИсчисленныйПоДоходамНалог(МенеджерВременныхТаблиц, ОписаниеВТ, МесяцНачисления, ПогашениеЗаймов, ИсключаемыйРегистратор, НастройкиУдержаний, ОчередностьРасчета = Неопределено, ЭтоИсправлениеТекущимПериодом = Ложь) Экспорт 
	...
	НалогПоДоходам = РегистрыНакопления.СведенияОДоходахНДФЛ.СоздатьНаборЗаписей().Выгрузить(, "Период,ФизическоеЛицо,Сотрудник,Начисление,СуммаДохода");
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ИЛИ Удержания.ТребуетсяРасчетИсчисленногоНДФЛ)
	...
	Иначе 
		ОблагаемыеНалогомДоходы = РегистрыНакопления.СведенияОДоходахНДФЛ.СоздатьНаборЗаписей().Выгрузить(, "Период,ФизическоеЛицо,Сотрудник,Начисление,СуммаДохода,ИсключаетсяИзРасчетнойБазыИсполнительныхЛистов");
	...
	ОблагаемыеНалогомДоходы.Свернуть("Месяц,ФизическоеЛицо,Сотрудник,Начисление,Ставка,ИсключаетсяИзРасчетнойБазыИсполнительныхЛистов", "Результат");
	...
	НалогПоДоходам.Свернуть("Месяц,ФизическоеЛицо,Сотрудник,Начисление", "Налог");
...
Функция УдержанныеСуммыФизическихЛиц(ФизлицаДокументы, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование, УчитыватьСторно = Истина, УчитыватьОграничения = Ложь)
	...
	Запрос.Текст =
		...
		|	ФизлицаДокументы.ДокументОснование КАК ДокументОснование,
		|	ФизлицаДокументы.ПериодВзаиморасчетов КАК ПериодВзаиморасчетов,
		|	ЕСТЬNULL(УдержанияПоИсполнительнымДокументам.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)) КАК СтатьяРасходов,
		|	ЕСТЬNULL(УдержанияПоИсполнительнымДокументам.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)) КАК СтатьяФинансирования,
		|	СУММА(ЕСТЬNULL(УдержанияПоИсполнительнымДокументам.СуммаУдержания, 0)) КАК СуммаВзыскано
		|ПОМЕСТИТЬ ВТВзысканныеСуммы
		|ИЗ
		|	ВТФизлицаДокументы КАК ФизлицаДокументы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.УдержанияПоИсполнительнымДокументам КАК УдержанияПоИсполнительнымДокументам
		|		ПО ФизлицаДокументы.ДокументОснование = УдержанияПоИсполнительнымДокументам.Регистратор
		|			И ФизлицаДокументы.ФизическоеЛицо = УдержанияПоИсполнительнымДокументам.ФизическоеЛицо
		|			И ФизлицаДокументы.ПериодВзаиморасчетов = УдержанияПоИсполнительнымДокументам.Период
		|			И (&УчитыватьОграничения)
		|			И (&УчитыватьСторно)
		|			И (&ОтборПоСтатьеРасходов)
		|			И (&ОтборПоСтатьеФинансирования)
		|
		...
		|	ФизлицаДокументы.ДокументОснование,
		|	ФизлицаДокументы.ПериодВзаиморасчетов,
		|	ЕСТЬNULL(УдержанияПоИсполнительнымДокументам.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)),
		|	ЕСТЬNULL(УдержанияПоИсполнительнымДокументам.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ФизлицаДокументы.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ФизлицаДокументы.ДокументОснование КАК ДокументОснование,
		|	ЕСТЬNULL(ВТВзысканныеСуммы.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)) КАК СтатьяРасходов,
		|	ЕСТЬNULL(ВТВзысканныеСуммы.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)) КАК СтатьяФинансирования,
		|	СУММА(ФизлицаДокументы.СуммаНачисленнойЗарплаты) КАК СуммаНачисленнойЗарплаты,
		|	СУММА(ФизлицаДокументы.Сумма) КАК СуммаКВыплате,
		|	СУММА(ЕСТЬNULL(ВТВзысканныеСуммы.СуммаВзыскано, 0)) КАК СуммаВзыскано
		|ИЗ
		|	ВТФизлицаДокументы КАК ФизлицаДокументы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВзысканныеСуммы КАК ВТВзысканныеСуммы
		|		ПО ФизлицаДокументы.ДокументОснование = ВТВзысканныеСуммы.ДокументОснование
		|			И ФизлицаДокументы.ФизическоеЛицо = ВТВзысканныеСуммы.ФизическоеЛицо
		|			И ФизлицаДокументы.ПериодВзаиморасчетов = ВТВзысканныеСуммы.ПериодВзаиморасчетов
		|
		|СГРУППИРОВАТЬ ПО
		|	ФизлицаДокументы.ФизическоеЛицо,
		|	ФизлицаДокументы.ДокументОснование,
		|	ЕСТЬNULL(ВТВзысканныеСуммы.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)),
		|	ЕСТЬNULL(ВТВзысканныеСуммы.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТВзысканныеСуммы";
	
	Если УчитыватьСторно Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УчитыватьСторно", "ИСТИНА");
	Иначе
		УчитыватьСторно = "НЕ УдержанияПоИсполнительнымДокументам.Сторно";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УчитыватьСторно", УчитыватьСторно);
	КонецЕсли;
	
	Если УчитыватьОграничения Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УчитыватьОграничения", "ИСТИНА");
	Иначе
		УчитыватьОграничения = "НЕ УдержанияПоИсполнительнымДокументам.ОграничениеВзыскания";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УчитыватьОграничения", УчитыватьОграничения);
	КонецЕсли;
	...
	Если ДетализироватьПоСтатьям Тогда
		Если РассчитыватьВзысканныеСуммыПропорциональноВыплате Тогда
			Если ЗначениеЗаполнено(Финансирование.СтатьяРасходов) Тогда
				ОтборПоСтатьеРасходов = "УдержанияПоИсполнительнымДокументам.СтатьяРасходов = &СтатьяРасходов";
				Запрос.УстановитьПараметр("СтатьяРасходов", Финансирование.СтатьяРасходов);
			КонецЕсли;
			Если ЗначениеЗаполнено(Финансирование.СтатьяФинансирования) Тогда
				ОтборПоСтатьеФинансирования = "УдержанияПоИсполнительнымДокументам.СтатьяФинансирования = &СтатьяФинансирования";
				Запрос.УстановитьПараметр("СтатьяФинансирования", Финансирование.СтатьяФинансирования);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
