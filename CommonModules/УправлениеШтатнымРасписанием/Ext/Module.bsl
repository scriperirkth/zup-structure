...
Процедура СформироватьДвиженияКлассовУсловийТрудаПоШтатномуРасписанию(Организация, ДатаВступленияВСилу, Регистратор, Позиции) Экспорт
	...
	Для каждого СтрокаПозицииОбъекта Из Позиции Цикл
		...
		Запрос.УстановитьПараметр("ВыявленыВредныеОпасныеФакторы", СтрокаПозицииОбъекта.ВыявленыВредныеОпасныеФакторы);
		...
		|	И КлассыУсловийТрудаПоДолжностям.ВыявленыВредныеОпасныеФакторы = &ВыявленыВредныеОпасныеФакторы
		...
		Если ЗаписатьКлассыУсловийТруда Тогда
			...
			НоваяЗаписьНабораЗаписей.ВыявленыВредныеОпасныеФакторы = СтрокаПозицииОбъекта.ВыявленыВредныеОпасныеФакторы;
...
// Возвращает таблицу свободных позиций штатного расписания.
//
// Параметры:
//  Позиции		 - Массив - Список позиций, занятость которых необходимо узнать.
//  ДатаСведений - Дата - Дата, на которую необходимо получить свободные позиции.
// 
// Возвращаемое значение:
//  Соответстие - Свободные позиции.
//
Функция СвободныеПозицииШтатногоРасписания(Позиции = Неопределено, ДатаСведений = Неопределено) Экспорт
	
	Если ДатаСведений = Неопределено Тогда
		ДатаСведений = ТекущаяДатаСеанса();
	КонецЕсли;
	
	ПараметрыПостроения = ПараметрыПостроенияВТШтатноеРасписание(ДатаСведений,,, Позиции);
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	СоздатьВТШтатноеРасписание(
		МенеджерВТ,
		Истина,
		ПараметрыПостроения,
		ИменаСведенийПоСтавкам());
	...
	Запрос.Текст =
		...
		|	ШтатноеРасписание.ЗанятаПостоянно КАК ЗанятаПостоянно,
		|	ШтатноеРасписание.ЗанятаВременно КАК ЗанятаВременно,
		|	ШтатноеРасписание.ПодработкаПостоянно КАК ПодработкаПостоянно,
		|	ШтатноеРасписание.ПодработкаВременно КАК ПодработкаВременно,
		|	ШтатноеРасписание.СовмещенаПостоянно КАК СовмещенаПостоянно,
		|	ШтатноеРасписание.СовмещенаВременно КАК СовмещенаВременно,
		|	ШтатноеРасписание.Забронирована КАК Забронирована,
		|	ЗанятостьПозицийШтатногоРасписания.Регистратор КАК Регистратор
		|ИЗ
		|	ВТШтатноеРасписание КАК ШтатноеРасписание
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗанятостьПозицийШтатногоРасписания.СрезПоследних(&ДатаСведений, ) КАК ЗанятостьПозицийШтатногоРасписания
		|		ПО ШтатноеРасписание.ПозицияШтатногоРасписания = ЗанятостьПозицийШтатногоРасписания.ПозицияШтатногоРасписания
	...
	СвободныеПозицииШтатногоРасписания = Новый Соответствие;
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.СледующийПоЗначениюПоля("ПозицияШтатногоРасписания") Цикл
		СвободноСтавок = Выборка.КоличествоСтавок - (
			Выборка.ЗанятаПостоянно +
			Выборка.ЗанятаВременно +
			Выборка.ПодработкаПостоянно +
			Выборка.ПодработкаВременно +
			Выборка.СовмещенаПостоянно +
			Выборка.СовмещенаВременно +
			Выборка.Забронирована);
		Если СвободноСтавок < 1 Тогда
			Продолжить;
		КонецЕсли;
		СвободноСтавокПоПричине = Новый Структура("СвободноСтавок, Причина");
		СвободноСтавокПоПричине.Вставить("СвободноСтавок", СвободноСтавок);
		СвободноСтавокПоПричине.Вставить("Причина", Выборка.Регистратор);
		СвободныеПозицииШтатногоРасписания.Вставить(Выборка.ПозицияШтатногоРасписания, СвободноСтавокПоПричине);
	КонецЦикла;
	
	Возврат СвободныеПозицииШтатногоРасписания;
	
КонецФункции

#Область ДляВызоваИзДругихПодсистем

Процедура УстановитьРабочийСтолСпециалистПоШтатномуРасписанию(СоставФорм, РабочееМестоУстановлено) Экспорт
	
	Если ДоступенИнтерфейсРабочегоСтолаСпециалистПоШтатномуРасписанию() И ПравоДоступа("Просмотр", Метаданные.Отчеты["СостояниеШтатногоРасписания"]) Тогда
		СоставФорм.ЛеваяКолонка.Добавить("Отчет.СостояниеШтатногоРасписания.Форма.ФормаДляРабочегоСтола");
...
Функция ПолучитьРегистраторРасшифровки(ОтчетДанныеРасшифровки, КомпоновщикНастроек, Расшифровка) Экспорт
	...
	ПолеПозицияШР = ПоляРасшифровки.Найти("ПозицияШтатногоРасписания");
	
	РегистраторРасшифровки = Неопределено;
	
	Если ПолеПозицияШР <> Неопределено Тогда
		
		СведенияОРегистраторе = УправлениеШтатнымРасписаниемФормы.ИнформацияОбУтвержденииИЗакрытииПозиции(ПолеПозицияШР.Значение);
		РегистраторРасшифровки = СведенияОРегистраторе.УтвердившийРегистратор;
		
	КонецЕсли;
	
	Возврат РегистраторРасшифровки;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("РезультатЗаполненияПоТекущейКадровойРасстановке", Истина);
	Методы.Вставить("ЗаполнитьДвиженияБронированийЗанятостиПозицийШтатногоРасписания");
	Методы.Вставить("ЗаполнитьПозицииВДвиженияхРегистраСведенийПараметрыПериодовСтажаПФР");
	Методы.Вставить("ЗаполнитьОснованиеКлассовУсловийТрудаПоДолжностям");
	Методы.Вставить("ЗаполнитьВШтатномРасписанииВидСтажаСЗВ_ДСО");
	Методы.Вставить("СинхронизироватьИспользованиеШтатногоРасписания");
	Методы.Вставить("ЗаполнитьВИсторииШтатногоРасписанияВидСтажаСЗВ_ДСО");
КонецПроцедуры
...
Функция РезультатЗаполненияПоТекущейКадровойРасстановке(ПараметрыВыполнения) Экспорт
	...
	Если МассивСтрокДляЗаполненияНачислений.Количество() > 0 Тогда
		...
		ДанныеИзмененныхПозиций = ДанныеПозицийШтатногоРасписания(Истина, ИзмененныеПозиции, ПараметрыВыполнения.ДатаВступленияВСилу, , Ложь);
...
Функция ПараметрыОбновленияСведенийПодразделений() Экспорт
	
	ПараметрыОбновления = Новый Структура;
	ПараметрыОбновления.Вставить("ПроверятьИспользованиеИсторииИзмененияШтатногоРасписания", Истина);
	ПараметрыОбновления.Вставить("ОтключитьОбновлениеСтруктурыШтатногоРасписания", Истина);
	
	ПодстрокиЗамены = Новый Структура;
	ПодстрокиЗамены.Вставить("ТаблицаПодразделения", "");
	ПодстрокиЗамены.Вставить("ТаблицаПодчиненностьПодразделений", "");
	ПодстрокиЗамены.Вставить("ТаблицаШтатноеРасписание", "");
	ПодстрокиЗамены.Вставить("ТаблицаШтатноеРасписаниеРасширенная", "");
	ПодстрокиЗамены.Вставить("УсловиеСоединения", "");
	ПодстрокиЗамены.Вставить("УсловиеИспользуется", "");
	ПодстрокиЗамены.Вставить("ПолеПозицияШтатногоРасписания", "");
	ПодстрокиЗамены.Вставить("ПолеДата", "");
	ПодстрокиЗамены.Вставить("ПолеМаксимальнаяДата", "");
	ПодстрокиЗамены.Вставить("ПолеМаксимальнаяДатаИзменений", "");
	ПодстрокиЗамены.Вставить("ПолеИспользуется", "");
	ПодстрокиЗамены.Вставить("ПолеПодчиненноеПодразделение", "");
	ПодстрокиЗамены.Вставить("ПолеВышестоящееПодразделение", "");
	
	ПараметрыОбновления.Вставить("ПодстрокиЗамены", ПодстрокиЗамены);
	
	ПараметрыОбновления.Вставить("ТекстЗапросаВспомогательныеТаблицы", "");
	
	Возврат ПараметрыОбновления;
	
КонецФункции

// Обновляет сведения о подразделениях на основании позиций штатного расписания.
//
// Параметры:
//   ОбновляемыеПодразделения - Массив Из СправочникСсылка.ПодразделенияОрганизаций, СправочникСсылка.СтруктураПредприятия - 
//   ПараметрыОбновления - см. ПараметрыОбновленияСведенийПодразделений
Процедура ОбновитьСведенияПодразделений(ОбновляемыеПодразделения, ПараметрыОбновления = Неопределено) Экспорт
	...
	Если ПараметрыОбновления.ПроверятьИспользованиеИсторииИзмененияШтатногоРасписания
		И Не ПолучитьФункциональнуюОпцию("ИспользоватьИсториюИзмененияШтатногоРасписания") Тогда
		Возврат;
	КонецЕсли;
	...
	Пока ОбновляемыеПодразделения.Количество() > 0 Цикл
		...
		ТекстЗапроса =
			...
			|ИЗ
			|	#ТаблицаПодразделения КАК Подразделения
			|ГДЕ
			|	Подразделения.Ссылка В(&ОбновляемыеПодразделения)";

		ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьТекстыЗапросов(ТекстЗапроса,
			ПараметрыОбновления.ТекстЗапросаВспомогательныеТаблицы);

		ТекстЗапросаПодразделенияКОбновлению =
			"ВЫБРАТЬ
			|	ОбновляемыеПодразделения.Подразделение КАК Подразделение,
			|	МИНИМУМ(Подразделения.ДатаСоздания) КАК ДатаСоздания,
			|	МИНИМУМ(Подразделения.Расформировано) КАК Расформировано,
			|	МАКСИМУМ(Подразделения.ДатаРасформирования) КАК ДатаРасформирования
			|ПОМЕСТИТЬ ВТСведенияПодчиненныхПодразделений
			|ИЗ
			|	ВТОбновляемыеПодразделения КАК ОбновляемыеПодразделения
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ #ТаблицаПодразделения КАК Подразделения
			|		ПО ОбновляемыеПодразделения.Подразделение = Подразделения.Родитель
			...
			|ВЫБРАТЬ
			|	ОбновляемыеПодразделения.Подразделение КАК Подразделение,
			|	МИНИМУМ(&ПолеДата) КАК Дата
			|ПОМЕСТИТЬ ВТМинимальныеДатыИспользованияПодразделений
			|ИЗ
			|	ВТОбновляемыеПодразделения КАК ОбновляемыеПодразделения
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ #ТаблицаПодчиненностьПодразделений КАК ПодчиненностьПодразделений
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ #ТаблицаШтатноеРасписаниеРасширенная КАК ШтатноеРасписание
			|			ПО &УсловиеСоединения
			|				И &УсловиеИспользуется
			|		ПО ОбновляемыеПодразделения.Подразделение = &ПолеВышестоящееПодразделение
			|
			|СГРУППИРОВАТЬ ПО
			|	ОбновляемыеПодразделения.Подразделение
			|;
			...
			|ВЫБРАТЬ
			|	ОбновляемыеПодразделения.Подразделение КАК Подразделение,
			|	&ПолеПозицияШтатногоРасписания КАК ПозицияШтатногоРасписания,
			|	МАКСИМУМ(&ПолеМаксимальнаяДата) КАК Дата,
			|	МАКСИМУМ(&ПолеМаксимальнаяДатаИзменений) КАК ДатаПоследнегоИзменения
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ #ТаблицаШтатноеРасписание КАК ШтатноеРасписание
			|		ПО МаксимальныеДатыИзменений.ПозицияШтатногоРасписания = &ПолеПозицияШтатногоРасписания
			|		И МаксимальныеДатыИзменений.ДатаПоследнегоИзменения = &ПолеМаксимальнаяДатаИзменений
			|
			|СГРУППИРОВАТЬ ПО
			...
			|	МАКСИМУМ(ВЫБОР
			|				КОГДА НЕ ПоследниеИспользованияПодразделений.Используется ТОГДА
			|					ВЫБОР 
			|						КОГДА МаксимальныеДатыИзменений.ДатаПоследнегоИзменения > МаксимальныеДатыИзменений.Дата ТОГДА
			|							МаксимальныеДатыИзменений.Дата
			|						ИНАЧЕ МаксимальныеДатыИзменений.ДатаПоследнегоИзменения 
			|					КОНЕЦ
			|				КОГДА МаксимальныеДатыИзменений.Дата <> ДАТАВРЕМЯ(3999, 12, 31, 23, 59, 59) ТОГДА
			|					МаксимальныеДатыИзменений.Дата
			|				ИНАЧЕ NULL
			|			КОНЕЦ) КАК Дата
			...
			|ВЫБРАТЬ
			|	ОбновляемыеПодразделения.Подразделение КАК Подразделение,
			|	ОбновляемыеПодразделения.Родитель КАК Родитель,
			|	ВЫБОР
			...
			|	СформированностьПодразделений.Родитель КАК Родитель,
			|	ВЫБОР
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ #ТаблицаПодразделения КАК Подразделения
			|		ПО СформированностьПодразделений.Подразделение = Подразделения.Ссылка
			|ГДЕ
			|	(СформированностьПодразделений.Сформировано <> Подразделения.Сформировано
			|			ИЛИ СформированностьПодразделений.ДатаСоздания <> Подразделения.ДатаСоздания
			|			ИЛИ СформированностьПодразделений.Расформировано <> Подразделения.Расформировано
			|			ИЛИ СформированностьПодразделений.ДатаРасформирования <> Подразделения.ДатаРасформирования)";
		
		ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьТекстыЗапросов(ТекстЗапроса, ТекстЗапросаПодразделенияКОбновлению);

		Запрос.Текст = ТекстЗапроса;
		
		ПодстрокиЗамены = ПараметрыОбновления.ПодстрокиЗамены;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ТаблицаПодразделения", ПодстрокиЗамены.ТаблицаПодразделения);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ТаблицаПодчиненностьПодразделений", ПодстрокиЗамены.ТаблицаПодчиненностьПодразделений);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ТаблицаШтатноеРасписаниеРасширенная", ПодстрокиЗамены.ТаблицаШтатноеРасписаниеРасширенная);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ТаблицаШтатноеРасписание", ПодстрокиЗамены.ТаблицаШтатноеРасписание);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеСоединения", ПодстрокиЗамены.УсловиеСоединения);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеИспользуется", ПодстрокиЗамены.УсловиеИспользуется);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеПозицияШтатногоРасписания", ПодстрокиЗамены.ПолеПозицияШтатногоРасписания);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеДата", ПодстрокиЗамены.ПолеДата);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеМаксимальнаяДатаИзменений", ПодстрокиЗамены.ПолеМаксимальнаяДатаИзменений);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеМаксимальнаяДата", ПодстрокиЗамены.ПолеМаксимальнаяДата);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеИспользуется", ПодстрокиЗамены.ПолеИспользуется);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеПодчиненноеПодразделение", ПодстрокиЗамены.ПолеПодчиненноеПодразделение);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеВышестоящееПодразделение", ПодстрокиЗамены.ПолеВышестоящееПодразделение);
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ПараметрыОбновления.ОтключитьОбновлениеСтруктурыШтатногоРасписания Тогда
...
Процедура ПриОпределенииДоступныхДляПереводаМакетов(Макеты) Экспорт
	...
	Макеты.Добавить(Метаданные.Документы.ИзменениеШтатногоРасписания.Макеты.ПФ_MXL_ПриказОВнесенииИзмененийШтатноеРасписание);
	Макеты.Добавить(Метаданные.Документы.УтверждениеШтатногоРасписания.Макеты.ПФ_MXL_ПриказОбУтвержденииШтатногоРасписания);
	Макеты.Добавить(Метаданные.Отчеты.ШтатноеРасписание.Макеты.ПФ_MXL_ШтатноеРасписаниеНаПодпись);
	Макеты.Добавить(Метаданные.Отчеты.ШтатноеРасписаниеНачисления.Макеты.ПФ_MXL_Т3);
...
Процедура ЗаполнитьПоставляемыеПрофилиГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля = ОписаниеПрофиляУправлениеШтатнымРасписанием();
	ОписанияПрофилей.Добавить(ОписаниеПрофиля);

КонецПроцедуры

Функция ДоступенИнтерфейсРабочегоСтолаСпециалистПоШтатномуРасписанию() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьШтатноеРасписание")
		И РольДоступна(Метаданные.Роли.ИнтерфейсРабочегоСтолаСпециалистПоШтатномуРасписанию);
...
Процедура ДокументыОбработкаПроверкиЗаполнения(Объект, Отказ, ДатаВступленияВСилу, ИмяЭлемента, ПроверятьВсеПозиции) Экспорт
	...
	Для Каждого СтрокаПозиции Из Объект.Позиции Цикл
		Если СтрокаПозиции.КоличествоСтавок = 0
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У позиции в строке %1 не задано количество ставок'"), СтрокаПозиции.НомерСтроки);
...
Процедура АвтозаполнениеТекущимиДаннымиШтатногоРасписания(ДокументШтатногоРасписанияОбъект, ДатаВступленияВСилу, ПроверятьЗаполненностьПодразделения = Ложь) Экспорт
	...
	КоллекцияНаименований = Новый Соответствие;
	...
	Пока ДанныеДляЗаполнения.СледующийПоЗначениюПоля("Позиция") Цикл
		...
		Если СтрокиПозиции.Количество() = 0 Тогда
			...
			КоллекцияНаименований.Вставить(ПозицияШР.ИдентификаторСтрокиПозиции,
				Новый Структура("Наименование,НаименованиеПолное",
					ДанныеДляЗаполнения.Наименование,
					ДанныеДляЗаполнения.НаименованиеПолное));
		...
		Пока ДанныеДляЗаполнения.СледующийПоЗначениюПоля("Начисление") Цикл
			...
			УправленческиеНачисленияПозиции = Ложь;
			...
			Если Не НезависимыйПоказатель Тогда
				...
				Иначе
					...
					УправленческиеНачисленияПозиции = Истина;
				КонецЕсли;
		...
		Если УправленческиеНачисленияПозиции Тогда
			ПозицияШР.ИспользоватьУправленческиеНачисления = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЗначениеЗаполнено(КоллекцияНаименований) Тогда
		ДокументШтатногоРасписанияОбъект.ДополнительныеСвойства.Вставить("КоллекцияНаименований", КоллекцияНаименований);
	КонецЕсли;
...
Функция СформироватьЗапросПоТекущемуСостояниюШтатногоРасписания(Регистратор, Организация, Дата, Подразделение = Неопределено) 
	...
	Запрос.Текст = 
		...
		|	АктуальныеПозицииШтатногоРасписания.Наименование КАК Наименование,
		|	АктуальныеПозицииШтатногоРасписания.НаименованиеПолное КАК НаименованиеПолное,
	...
	Если Результат.Пустой() Тогда
		...
		СоздатьВТДанныеНачислений(Запрос.МенеджерВременныхТаблиц, ПараметрыПостроения.УправленческиеНачисления);
		
		Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ДанныеНачислений.Начисление КАК Начисление,
			|	ДанныеНачислений.Ссылка.Наименование КАК Наименование,
			|	ДанныеНачислений.Ссылка.НаименованиеПолное КАК НаименованиеПолное,
			|	ДанныеНачислений.ПлановыеНачисления КАК ПлановыеНачисления,
			|	ДанныеНачислений.Показатель КАК Показатель,
			|	ДанныеНачислений.Значение КАК Значение,
			|	ДанныеНачислений.ЗначениеМин КАК ЗначениеМин,
			|	ДанныеНачислений.ЗначениеМакс КАК ЗначениеМакс,
			|	ДанныеНачислений.Позиция КАК Позиция,
			...
			|		ПО ТекущиеПозицииШР.Позиция = ДанныеНачислений.Позиция
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КлассыУсловийТрудаПоДолжностям.СрезПоследних(&ДатаАктуальности, ) КАК КлассыУсловийТрудаПоДолжностям
		...
		Если ПараметрыПостроения.УправленческиеНачисления Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТДанныеНачислений.ПлановыеНачисления КАК ПлановыеНачисления,",
				"ВТДанныеНачислений.ПлановыеНачисления КАК ПлановыеНачисления, ВТДанныеНачислений.УправленческиеНачисления КАК УправленческиеНачисления,");
		КонецЕсли;
...
Функция ОписаниеПозицииШтатногоРасписания() Экспорт
	...
	ДанныеПозиции.Вставить("ВыявленыВредныеОпасныеФакторы");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОбучениеРазвитие") Тогда
		...
		МодульОбучениеРазвитие.ДополнитьОписаниеПозицииШтатногоРасписания(ДанныеПозиции);
	КонецЕсли;
...
Функция ДанныеПозицииШтатногоРасписания(Позиция, ДатаПозиции = '00010101', ЗаполнятьОписаниеНачислений = Истина, ПолноеОписаниеНачислений = Истина, ИсключаемыйРегистратор = Неопределено) Экспорт
	...
	Если ДанныеПозиции = Неопределено Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
			...
			ДанныеПозиции.МестоВСтруктуреПредприятия = МодульОрганизационнаяСтруктура.МестоПозицииВСтруктуреПредприятия(
				Позиция, ДатаПозиции);
		КонецЕсли;
...
Процедура РассчитатьФОТНесколькихПозиций(ОписаниеПозиций) Экспорт
	...
	ВРНадбавкаЗаВредность = ЗарплатаКадрыРасширенный.НачислениеНадбавкаЗаВредность(Организация, ДатаРасчета);
	...
	ВРРайонныйКоэффициент = ЗарплатаКадрыРасширенный.НачислениеРайонныйКоэффициент(Организация, ДатаРасчета);
	...
	ВРСевернаяНабавка = ЗарплатаКадрыРасширенный.НачислениеСевернаяНадбавка(Организация, ДатаРасчета);
...
Процедура ЗаполнитьОбъектИзменениямиПозицийШтатногоРасписания(Объект, ТаблицаПозицийКИзменению, ТарифнаяСетка) Экспорт
	...
	Для каждого СтрокаПозиции Из ТаблицаПозицийКИзменению Цикл
		...
		Если СтрокиПозицииДокумента.Количество() = 0 Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
				МодульУправленческаяЗарплатаФормы = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплата");
				МодульУправленческаяЗарплатаФормы.ЗаполнитьОбъектУправленческимиНачислениямиПозицийШтатногоРасписания(Объект, ДанныеПозиции, СтрокаПозицииДокумента.ИдентификаторСтрокиПозиции);
			КонецЕсли;
...
Процедура ОбновитьСтруктуруШтатногоРасписанияПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ПрежниеДанныеОбъекта <> Неопределено Тогда
		...
		Если ОбновитьСтруктуруШтатногоРасписания Тогда
			...
			Если ОбновитьСтруктуруПоДаннымПодразделений Тогда
				ОбновитьСведенияПодразделений(ПодразделенияПозиции);
			КонецЕсли;
...
Функция ЗапросВТМестоПозицииВСтруктуреПредприятия(ИмяВТ = "ВТВсеПозицииНаДатуАктуальности",
	ИменаПолейВременнойТаблицыОтборов = "ПозицияШтатногоРасписания, Период")
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		...
		СоответствиеПсевдонимовПолей.Вставить("Подразделение", "МестоВСтруктуреПредприятия");
		ПараметрыПостроения = МодульОрганизационнаяСтруктура.ПараметрыПостроенияВТМестоПозицииВСтруктуреПредприятия(
			ИмяВТ, ИменаПолейВременнойТаблицыОтборов);
		Запрос = МодульОрганизационнаяСтруктура.ЗапросВТМестоПозицииВСтруктуреПредприятия(ПараметрыПостроения);
...
Процедура СоздатьВТМестоПозицииВСтруктуреПредприятия(МенеджерВременныхТаблиц, ИмяВТ,
	ИменаПолейВременнойТаблицыОтборов = "ПозицияШтатногоРасписания, Период")
	
	Запрос = ЗапросВТМестоПозицииВСтруктуреПредприятия(ИмяВТ, ИменаПолейВременнойТаблицыОтборов);
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
...
Функция ЗаполнитьПараметрыОбновленияСведенийПодразделений(ПараметрыОбновления)
	...
	ПодстрокиЗамены = ПараметрыОбновления.ПодстрокиЗамены;
	ПодстрокиЗамены.ТаблицаПодразделения = "Справочник.ПодразделенияОрганизаций";
	ПодстрокиЗамены.ТаблицаПодчиненностьПодразделений = "РегистрСведений.ПодчиненностьПодразделенийОрганизаций";
	ПодстрокиЗамены.ТаблицаШтатноеРасписание = "РегистрСведений.ИсторияИспользованияШтатногоРасписания";
	ПодстрокиЗамены.ТаблицаШтатноеРасписаниеРасширенная = ПодстрокиЗамены.ТаблицаШтатноеРасписание;
	ПодстрокиЗамены.УсловиеСоединения =
		"ПодчиненностьПодразделений.Подразделение = ШтатноеРасписание.ПозицияШтатногоРасписания.Подразделение";
	...
	ПодстрокиЗамены.ПолеДата = "ШтатноеРасписание.Дата";
	ПодстрокиЗамены.ПолеМаксимальнаяДата = ПодстрокиЗамены.ПолеДата;
	ПодстрокиЗамены.ПолеМаксимальнаяДатаИзменений = ПодстрокиЗамены.ПолеДата;
	ПодстрокиЗамены.ПолеИспользуется = ПодстрокиЗамены.УсловиеИспользуется;
	ПодстрокиЗамены.ПолеПодчиненноеПодразделение = "ПодчиненностьПодразделений.Подразделение";
	ПодстрокиЗамены.ПолеВышестоящееПодразделение = "ПодчиненностьПодразделений.ВышестоящееПодразделение";
...
Процедура ДополнитьЗапросВТШтатноеРасписаниеНачислениями(Запрос, ИмяВТШтатноеРасписание, ПараметрыПостроения, ОписаниеИсточников = Неопределено)
	...
	ТекстыЗапросовВспомогательныхТаблиц = Новый Массив;
	Схема = Новый СхемаЗапроса;
	Схема.УстановитьТекстЗапроса(Запрос.Текст);
	ПоследнийЗапрос = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросСхемыФормирующийВТ(Схема, ИмяВТШтатноеРасписание);
	ИмяИсточникаОтбора = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросСхемыФормирующийВТ(Схема, ИмяВТШтатноеРасписание).Операторы[0].Источники[0].Источник.ИмяТаблицы;
	...
	ИндексЗапросаОтобора = -1;
	ИндексЗапроса = 0;
	ТекстыЗапросов = СтрРазделить(Запрос.Текст, ";");
	Для Каждого ТекстЗапроса Из ТекстыЗапросов Цикл
		ТекстыЗапросовВспомогательныхТаблиц.Добавить(ТекстЗапроса);
		Если СтрНайти(ТекстЗапроса, "ПОМЕСТИТЬ " + ИмяИсточникаОтбора) Тогда
			ИндексЗапросаОтобора = ИндексЗапроса;
		КонецЕсли;
	...
	Для Каждого ОписаниеИсточникаНачисленийКлючИЗначение Из ОписаниеИсточниковНачислений Цикл
		Для Каждого ОписаниеИсточникаКлючИЗначение Из ОписаниеИсточникаНачисленийКлючИЗначение.ИсточникиДанных Цикл
			...
			Если ОписаниеИсточника.ЗапросВспомогательныхТаблиц <> Неопределено Тогда
				Если Не ПустаяСтрока(ОписаниеИсточника.ЗапросВспомогательныхТаблиц.Текст) Тогда
					ИндексЗапросаОтобора = ИндексЗапросаОтобора + 1;
					ТекстыЗапросовВспомогательныхТаблиц.Вставить(ИндексЗапросаОтобора, ОписаниеИсточника.ЗапросВспомогательныхТаблиц.Текст);
				КонецЕсли;
				ЗарплатаКадрыОбщиеНаборыДанных.СкопироватьПараметрыЗапроса(Запрос, ОписаниеИсточника.ЗапросВспомогательныхТаблиц);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Если ЗначениеЗаполнено(ТекстыЗапросовВспомогательныхТаблиц) Тогда
		Запрос.Текст = СтрСоединить(ТекстыЗапросовВспомогательныхТаблиц,
...
Процедура ДобавитьОписаниеСведенийКлассовУсловийТрудаПоДолжностям(ОписаниеИсточников)
	...
	ИмяДанныхШР = "ВыявленыВредныеОпасныеФакторы";
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточника);
...
Процедура ДобавитьОписаниеСведенийОЗанятости(ОписаниеИсточников)
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТСотрудникиЗанятыхПозиций
		|ИЗ
		|	ВТОтборПозицийШтатногоРасписания КАК ОтборПозицийШтатногоРасписания
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗанятостьПозицийШтатногоРасписанияИнтервальный КАК ЗанятостьПозицийШтатногоРасписанияИнтервальный
		|		ПО ОтборПозицийШтатногоРасписания.ПозицияШтатногоРасписания = ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПозицияШтатногоРасписания
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Сотрудник
		|;
		...
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		...
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПериодЗаписи КАК ПериодЗаписи,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.Год КАК Год,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПериодПредыдущейЗаписи КАК ПериодПредыдущейЗаписи,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПозицияШтатногоРасписания КАК ПозицияШтатногоРасписания,
		...
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ДействуетДо КАК ДействуетДо,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ЗамещаемыйСотрудник КАК ЗамещаемыйСотрудник,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПланируемаяДатаЗавершения КАК ПланируемаяДатаЗавершения,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.РегистраторЗаписи КАК РегистраторЗаписи,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.РегистраторСобытия КАК РегистраторСобытия,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПустойИнтервал КАК ПустойИнтервал
		|ПОМЕСТИТЬ ВТРегистрСведенийЗанятостьПозицийШтатногоРасписанияИнтервальный
		|ИЗ
		|	ВТСотрудникиЗанятыхПозиций КАК СотрудникиЗанятыхПозиций
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗанятостьПозицийШтатногоРасписанияИнтервальный КАК ЗанятостьПозицийШтатногоРасписанияИнтервальный
		|		ПО СотрудникиЗанятыхПозиций.Сотрудник = ЗанятостьПозицийШтатногоРасписанияИнтервальный.Сотрудник
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ПозицияШтатногоРасписания,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ДатаНачала,
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ДатаОкончания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|ИЗ
		|	ВТОтборПозицийШтатногоРасписания КАК ОтборПозицийШтатногоРасписания
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРегистрСведенийЗанятостьПозицийШтатногоРасписанияИнтервальный КАК РегистрСведенийЗанятостьПозицийШтатногоРасписания
		...
		|	ВТОтборПозицийШтатногоРасписания КАК ОтборПозицийШтатногоРасписания
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРегистрСведенийЗанятостьПозицийШтатногоРасписанияИнтервальный КАК РегистрСведенийЗанятостьПозицийШтатногоРасписания
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТРегистрСведенийЗанятостьПозицийШтатногоРасписанияИнтервальный КАК РегистрСведенийЗанятостьПозицийШтатногоРасписанияПредыдущие
	...
	ОписаниеИсточника.ИменаВТКУничтожению.Добавить("ВТСотрудникиЗанятыхПозиций");
	ОписаниеИсточника.ИменаВТКУничтожению.Добавить("ВТРегистрСведенийЗанятостьПозицийШтатногоРасписанияИнтервальный");
...
Функция ОписаниеИсточниковСведенийОНачисленияхШтатногоРасписания(ПараметрыПостроения, ОписаниеИсточниковШтатногоРасписания, Схема, ИмяВТШтатноеРасписание)
	...
	Если ПараметрыПостроения.ПлановыеНачисления Тогда
		...
		ДобавитьОписаниеСведенийОНачислениях(ОписанияИсточниковНачислений, ПсевдонимыИсточников, ПараметрыПостроения);
		...
		Если ПараметрыПостроения.ПолноеОписаниеНачислений Тогда
			ДобавитьОписаниеСведенийОНачисленииРайонногоКоэффициента(
				ОписанияИсточниковНачислений, ОписаниеИсточниковШтатногоРасписания, ПсевдонимыИсточников);
			ДобавитьОписаниеСведенийОНачисленииСевернойНадбавки(
				ОписанияИсточниковНачислений, ОписаниеИсточниковШтатногоРасписания, ПсевдонимыИсточников);
			ДобавитьОписаниеСведенийОНачисленииНадбавкиЗаВредность(
				ОписанияИсточниковНачислений, ОписаниеИсточниковШтатногоРасписания, ПсевдонимыИсточников);
		КонецЕсли;
...
Процедура ДобавитьОписаниеСведенийОНачислениях(ОписанияИсточниковНачислений, ПсевдонимыИсточников, ПараметрыПостроения)
	...
	Запрос.Текст =
		...
		|ИЗ
		|	ВТОтборПозицийШтатногоРасписания КАК ПозицииШтатногоРасписания";
	
	Если ПараметрыПостроения.Свойство("ИмяВТФильтраПозиций") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПозицииШтатногоРасписания.Период", "ПозицииШтатногоРасписания." + ПараметрыПостроения.ИмяПоляПериод);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПозицииШтатногоРасписания.ПозицияШтатногоРасписания", "ПозицииШтатногоРасписания." + ПараметрыПостроения.ИмяПоляПозицияШтатногоРасписания);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТОтборПозицийШтатногоРасписания", ПараметрыПостроения.ИмяВТФильтраПозиций);
	КонецЕсли;
	
	ПараметрыПостроенияВТОрганизацииПрименяющиеАУСН = ЗарплатаКадрыРасширенный.ПараметрыПостроенияВТОрганизацииПрименяющиеАУСН("ВТОтборОрганизаций");
	ЗапросВТОрганизацииПрименяющиеАУСН = ЗарплатаКадрыРасширенный.ЗапросВТОрганизацииПрименяющиеАУСН(ПараметрыПостроенияВТОрганизацииПрименяющиеАУСН);
	
	ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьЗапросы(Запрос, ЗапросВТОрганизацииПрименяющиеАУСН);
...
Процедура ДобавитьОписаниеСведенийОНачисленииРайонногоКоэффициента(ОписанияИсточниковНачислений, ОписаниеИсточниковШтатногоРасписания, ПсевдонимыИсточников)
	...
	ИмяДанныхШР = "Начисление";
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточникаНачислений,
		"НачисленияРайонныйКоэффициент.Начисление");
	...
	ОписаниеИсточника.ЗапросВспомогательныхТаблиц = ЗарплатаКадрыРасширенный.ЗапросВТНачисленияРайонныйКоэффициент();
	ОписаниеИсточника.ЗапросВспомогательныхТаблиц.УстановитьПараметр("ПоказательРайонныйКоэффициент",
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.РайонныйКоэффициент"));
	
	Соединение = ЗарплатаКадрыОбщиеНаборыДанных.ПустоеОписаниеСоединения();
	Соединение.Источник = "ВТНачисленияРайонныйКоэффициент";
	Соединение.Псевдоним = "НачисленияРайонныйКоэффициент";
	Соединение.ТипСоединения = ТипСоединенияСхемыЗапроса.Внутреннее;
	Если ПсевдонимыИсточников.Свойство("ОтборПозицийШтатногоРасписания") Тогда
		Соединение.Условие =
			"ОтборПозицийШтатногоРасписания.Период = НачисленияРайонныйКоэффициент.Период
			|И ОтборПозицийШтатногоРасписания.ПозицияШтатногоРасписания.Владелец.ГоловнаяОрганизация = НачисленияРайонныйКоэффициент.ГоловнаяОрганизация";
	Иначе
		Соединение.Условие =
			"ПозицииШтатногоРасписания.Период = НачисленияРайонныйКоэффициент.Период
			|И ПозицииШтатногоРасписания.ПозицияШтатногоРасписания.Владелец.ГоловнаяОрганизация = НачисленияРайонныйКоэффициент.ГоловнаяОрганизация";
	КонецЕсли;
	ОписаниеИсточника.Соединения.Добавить(Соединение);
...
Процедура ДобавитьОписаниеСведенийОНачисленииСевернойНадбавки(ОписанияИсточниковНачислений, ОписаниеИсточниковШтатногоРасписания, ПсевдонимыИсточников)
	...
	ИмяДанныхШР = "Начисление";
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточникаНачислений,
		"НачисленияСевернаяНадбавка.Начисление");
	...
	ОписаниеИсточника.ЗапросВспомогательныхТаблиц = ЗарплатаКадрыРасширенный.ЗапросВТНачисленияСевернаяНадбавка();
	
	Соединение = ЗарплатаКадрыОбщиеНаборыДанных.ПустоеОписаниеСоединения();
	Соединение.Источник = "ВТНачисленияСевернаяНадбавка";
	Соединение.Псевдоним = "НачисленияСевернаяНадбавка";
	Соединение.ТипСоединения = ТипСоединенияСхемыЗапроса.Внутреннее;
	Если ПсевдонимыИсточников.Свойство("ОтборПозицийШтатногоРасписания") Тогда
		Соединение.Условие =
			"ОтборПозицийШтатногоРасписания.Период = НачисленияСевернаяНадбавка.Период
			|И ОтборПозицийШтатногоРасписания.ПозицияШтатногоРасписания.Владелец.ГоловнаяОрганизация = НачисленияСевернаяНадбавка.ГоловнаяОрганизация";
	Иначе
		Соединение.Условие =
			"ПозицииШтатногоРасписания.Период = НачисленияСевернаяНадбавка.Период
			|И ПозицииШтатногоРасписания.ПозицияШтатногоРасписания.Владелец.ГоловнаяОрганизация = НачисленияСевернаяНадбавка.ГоловнаяОрганизация";
	КонецЕсли;
...
Процедура ДобавитьОписаниеСведенийОНачисленииНадбавкиЗаВредность(ОписанияИсточниковНачислений, ОписаниеИсточниковШтатногоРасписания, ПсевдонимыИсточников)
	...
	ИмяДанныхШР = "Начисление";
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточникаНачислений,
		"НачисленияНадбавкаЗаВредность.Начисление");
	...
	ОписаниеИсточника.ЗапросВспомогательныхТаблиц = ЗарплатаКадрыРасширенный.ЗапросВТНачисленияНадбавкаЗаВредность();
	...
	ОписаниеИсточника.ЗапросВспомогательныхТаблиц.УстановитьПараметр("ПоказательПроцентНадбавкиЗаВредность", ПоказательПроцентНадбавкиЗаВредность);
	
	Соединение = ЗарплатаКадрыОбщиеНаборыДанных.ПустоеОписаниеСоединения();
	Соединение.Источник = "ВТНачисленияНадбавкаЗаВредность";
	Соединение.Псевдоним = "НачисленияНадбавкаЗаВредность";
	Соединение.ТипСоединения = ТипСоединенияСхемыЗапроса.Внутреннее;
	Если ПсевдонимыИсточников.Свойство("ОтборПозицийШтатногоРасписания") Тогда
		Соединение.Условие =
			"ОтборПозицийШтатногоРасписания.Период = НачисленияНадбавкаЗаВредность.Период
			|И ОтборПозицийШтатногоРасписания.ПозицияШтатногоРасписания.Владелец.ГоловнаяОрганизация = НачисленияНадбавкаЗаВредность.ГоловнаяОрганизация";
	Иначе
		Соединение.Условие =
			"ПозицииШтатногоРасписания.Период = НачисленияНадбавкаЗаВредность.Период
			|И ПозицииШтатногоРасписания.ПозицияШтатногоРасписания.Владелец.ГоловнаяОрганизация = НачисленияНадбавкаЗаВредность.ГоловнаяОрганизация";
	КонецЕсли;
...
Функция ИменаСведенийПоСтавкам()
	...
	Возврат ИменаСведенийПоСтавкам;
	
КонецФункции

Функция ИдентификаторПрофиляУправлениеШтатнымРасписанием()
	Возврат "25c8aa5d-7beb-43ed-bf42-7ed30346a122";
КонецФункции

Функция ОписаниеПрофиляУправлениеШтатнымРасписанием(ПраваНаИнтерфейс = Ложь)
	
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Идентификатор = ИдентификаторПрофиляУправлениеШтатнымРасписанием();
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Управление штатным расписанием'");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ГрупповоеЗаполнениеПоказателей");
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ДобавлениеИзменениеНачисленийШтатногоРасписания");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ИзменениеМестаСотрудникаВСтруктуреПредприятия", "ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура");
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "НастройкаСвязейПозицийШтатногоРасписанияПоКадровомуПлану", "ЗарплатаКадрыКорпоративнаяПодсистемы.КадровоеПланирование");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ИнтерфейсРабочегоСтолаСпециалистПоШтатномуРасписанию");
...
Процедура СоздатьВТДанныеНачислений(МенеджерВременныхТаблиц, ИспользоватьУправленческиеНачисления)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ШтатноеРасписаниеНачисления.Начисление КАК Начисление,
		|	ШтатноеРасписаниеНачисления.Ссылка КАК Ссылка,
		...
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(ПланВидовРасчета.Начисления.ПустаяСсылка),
		|	ШтатноеРасписаниеПоказатели.Ссылка КАК Ссылка,
		|	ИСТИНА КАК ПлановыеНачисления,
	...
	Если ИспользоватьУправленческиеНачисления Тогда
		...
		ТекстУправленческогоЗапроса = 
			...
			|ИЗ
			|	ВТТекущиеПозицииШР КАК ТекущиеПозицииШР
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтатноеРасписание.УправленческиеНачисления КАК ШтатноеРасписаниеНачисления
			|		ПО ТекущиеПозицииШР.Позиция = ШтатноеРасписаниеНачисления.Ссылка
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтатноеРасписание.УправленческиеПоказатели КАК ШтатноеРасписаниеПоказатели
			|		ПО (ШтатноеРасписаниеНачисления.Ссылка = ШтатноеРасписаниеПоказатели.Ссылка)
			|			И (ШтатноеРасписаниеНачисления.ИдентификаторСтрокиВидаРасчета = ШтатноеРасписаниеПоказатели.ИдентификаторСтрокиВидаРасчета)
			|			И (ШтатноеРасписаниеПоказатели.Показатель <> ЗНАЧЕНИЕ(Справочник.ПоказателиРасчетаЗарплаты.ПустаяСсылка))
			|			И НЕ (ШтатноеРасписаниеПоказатели.Показатель.ЗначениеРассчитываетсяАвтоматически)
			...
			|	ЛОЖЬ КАК ПлановыеНачисления,
			|	ИСТИНА КАК УправленческиеНачисления,
			|	ШтатноеРасписаниеПоказатели.Показатель,
			|	ШтатноеРасписаниеПоказатели.Значение,
			|	ШтатноеРасписаниеПоказатели.ЗначениеМин,
			|	ШтатноеРасписаниеПоказатели.ЗначениеМакс,
			...
			|ИЗ
			|	ВТТекущиеПозицииШР КАК ТекущиеПозицииШР
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШтатноеРасписание.УправленческиеПоказатели КАК ШтатноеРасписаниеПоказатели
			|		ПО ТекущиеПозицииШР.Позиция = ШтатноеРасписаниеПоказатели.Ссылка
			|			И (ШтатноеРасписаниеПоказатели.ИдентификаторСтрокиВидаРасчета = 0)
			|			И (ШтатноеРасписаниеПоказатели.Показатель <> ЗНАЧЕНИЕ(Справочник.ПоказателиРасчетаЗарплаты.ПустаяСсылка))
			|			И НЕ (ШтатноеРасписаниеПоказатели.Показатель.ЗначениеРассчитываетсяАвтоматически)";
			
		Запрос.Текст = СтрШаблон("%1 ОБЪЕДИНИТЬ ВСЕ
		|	%2", Запрос.Текст, ТекстУправленческогоЗапроса);
	КонецЕсли;
