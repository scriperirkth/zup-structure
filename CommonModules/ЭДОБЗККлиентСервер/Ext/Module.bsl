...
Процедура ОбновитьКоманды(УправляемаяФорма, ОбъектФормы, ВыполнитьСтандартныйОбработчик, УправлятьВидимостью = Истина) Экспорт
	...
	Для Каждого Подменю Из ПараметрыПодключаемыхКоманд.ПодменюСУсловиямиВидимости Цикл
		...
		Для Каждого ОписаниеКоманды Из Подменю.КомандыСУсловиямиВидимости Цикл
			...
			Если СтрНачинаетсяС(ОписаниеКоманды.ИмяВФорме, "ПодменюПечать") Тогда
				УправлятьВидимостьюКоманды = Истина;
			Иначе
				УправлятьВидимостьюКоманды = УправлятьВидимостью;
			КонецЕсли;
			
			Если ЭлементФормы <> Неопределено И
				(Не УправлятьВидимостьюКоманды Или ЭлементФормы.Видимость) Тогда
				...
				Для Каждого Условие Из ОписаниеКоманды.УсловияВидимости Цикл
					...
					Если Не УсловиеВыполняется Или ЗначениеКлюча <> Условие.Значение Тогда
						...
						Если УправлятьВидимостьюКоманды Тогда
			...
			КонецЕсли;
			Если УсловияБЗК И Не УправлятьВидимостьюКоманды И ЭлементФормы <> Неопределено Тогда
...
Процедура УстановитьПодсказкуВидуДокумента(УправляемаяФорма, ИмяРеквизита, ВидДокумента, КодКадровогоДокумента = Неопределено) Экспорт
