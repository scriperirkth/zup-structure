...
Функция ЕстьСохраненныеНастройкиФормы(Форма) Экспорт
	...
	// См. "Настройки, автоматически сохраняемые в системном хранилище" в синтакс-помощнике.
...
// Возвращает Истина если при открытии формы сработает событие СписокПриЗагрузкеПользовательскихНастроекНаСервере.
Функция ЕстьСохраненныеПользовательскиеНастройкиСписка(Форма) Экспорт
	Если Не Форма.Список.АвтоматическоеСохранениеПользовательскихНастроек Тогда
		Возврат Ложь;
	КонецЕсли;
	УстановитьПривилегированныйРежим(Истина);
	// См. "Настройки, автоматически сохраняемые в системном хранилище" в синтакс-помощнике.
	Суффикс = НСтр("ru = 'ТекущиеПользовательскиеНастройки'", Метаданные.ОсновнойЯзык.КодЯзыка);
	Если ЗначениеЗаполнено(Форма.КлючНазначенияИспользования) Тогда
		Суффикс = Форма.КлючНазначенияИспользования + "/" + Суффикс;
	КонецЕсли;
	СписокЗначений = ХранилищеСистемныхНастроек.ПолучитьСписок(Форма.ИмяФормы + ".Список/" + Суффикс);
	Возврат СписокЗначений.Количество() > 0;
КонецФункции
