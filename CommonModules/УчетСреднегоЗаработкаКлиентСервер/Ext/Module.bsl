...
Функция ЗначенияПоказателейСреднегоЗаработкаОбщего(Начисления, Время, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НачисленияРК, НачисленияРКСН.ДополнительныеНачисленияРК);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НачисленияСН, НачисленияРКСН.ДополнительныеНачисленияСН);
...
Процедура РассчитатьУчитываемыйЗаработок(Начисления, УчитываемыйЗаработок, ДоляВремени, ДополнительныеПараметры)
	...
	Для Каждого СтрокаНачислений Из Начисления Цикл
		...
		Если Не ЭтоПремияПроцентом(СтрокаНачислений.СоставнаяЧасть) 
			...
			Если НачисленияРК.Найти(СтрокаНачислений.Начисление) <> Неопределено Тогда 
				СтрокаЗаработка.СуммаРК = СтрокаЗаработка.СуммаРК + СтрокаНачислений.Сумма;
				СтрокаЗаработка.УчтеноРК = Окр(СтрокаЗаработка.СуммаРК * СтрокаЗаработка.КоэффициентИндексации, 2);
			КонецЕсли;
			Если НачисленияСН.Найти(СтрокаНачислений.Начисление) <> Неопределено Тогда 
				СтрокаЗаработка.СуммаСН = СтрокаЗаработка.СуммаСН + СтрокаНачислений.Сумма;
				СтрокаЗаработка.УчтеноСН = Окр(СтрокаЗаработка.СуммаСН * СтрокаЗаработка.КоэффициентИндексации, 2);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		...
		Если НачисленияРК.Найти(СтрокаНачислений.Начисление) <> Неопределено Тогда 
			СтрокаПремии.СуммаРК = СтрокаПремии.СуммаРК + СтрокаНачислений.Сумма;
			СтрокаПремии.УчтеноРК = СтрокаПремии.УчтеноРК + Окр(УчитываемаяСумма * КоэффициентПремии * ДоляВремениПремии * КоэффициентИндексации, 2);
		КонецЕсли;
		
		Если НачисленияСН.Найти(СтрокаНачислений.Начисление) <> Неопределено Тогда 
			СтрокаПремии.СуммаСН = СтрокаПремии.СуммаСН + СтрокаНачислений.Сумма;
			СтрокаПремии.УчтеноСН = СтрокаПремии.УчтеноСН + Окр(УчитываемаяСумма * КоэффициентПремии * ДоляВремениПремии * КоэффициентИндексации, 2);
		КонецЕсли;
...
Функция СтруктураЗаработкаДляРасчетаСреднегоЗаработка()
	...
	Заработок.Вставить("СуммаРК", 					ПустоеЧисло);
	Заработок.Вставить("УчтеноРК", 					ПустоеЧисло);
	Заработок.Вставить("СуммаСН", 					ПустоеЧисло);
	Заработок.Вставить("УчтеноСН", 					ПустоеЧисло);
