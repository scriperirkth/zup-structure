...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ВключитьУчетРасчетовСБюджетомПоНДФЛ");
	Методы.Вставить("ОбновитьНДФЛПеречисленныйВозвратНДФЛ");
КонецПроцедуры
...
Процедура РасчетыСБюджетомПоНДФЛЗарегистрироватьНДФЛКПеречислению(Движения, Отказ) Экспорт
	...
	Если Движения.Найти("НДФЛКПеречислению") <> Неопределено И Движения.НДФЛКПеречислению.Количество() > 0  Тогда
		...
		Если ДатаОперацииМеньшеДатыВводаОстатков(Организация, ДатаОперации) Или ДатаОперации >= УчетНДФЛ.ДатаЗавершенияУчетаПеречисленногоНалога() Тогда
			Возврат;
		КонецЕсли;
