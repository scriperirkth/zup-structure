...
Функция СФРИспользованиеКлючаСоответствуетТребованиям(
		ИспользованиеКлючаИлиДанныеСертификата = "НеПроверять",
		ДействителенПо = Неопределено) Экспорт
	
	Результат = Неопределено;
	Если НЕ ЗначениеЗаполнено(ДействителенПо) ИЛИ ДействителенПо >= '20260901' Тогда
		Если ТипЗнч(ИспользованиеКлючаИлиДанныеСертификата) = Тип("ДвоичныеДанные") Тогда
			СвойстваСертификата = ДокументооборотСКОВызовСервера.СвойстваСертификатаСФР(ИспользованиеКлючаИлиДанныеСертификата);
			ИспользованиеКлюча = ?(СвойстваСертификата <> Неопределено И (ЗначениеЗаполнено(СвойстваСертификата.Размер)
				ИЛИ ЗначениеЗаполнено(СвойстваСертификата.Версия) ИЛИ ЗначениеЗаполнено(СвойстваСертификата.СерийныйНомер)
				ИЛИ ЗначениеЗаполнено(СвойстваСертификата.АлгоритмПубличногоКлюча)),
				СвойстваСертификата.ИспользованиеКлюча, "НеПроверять");
			
		Иначе
			ИспользованиеКлюча = ИспользованиеКлючаИлиДанныеСертификата;
		КонецЕсли;
		
		// сумма флагов keyEncipherment (32), dataEncipherment (16), keyAgreement (8)
		Результат = ИспользованиеКлюча = "НеПроверять"
			ИЛИ ЗначениеЗаполнено(ИспользованиеКлюча) И ПобитовоеИ(ИспользованиеКлюча, 56) = 56;
	КонецЕсли;
