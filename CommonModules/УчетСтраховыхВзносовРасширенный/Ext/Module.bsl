...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьСкидкуПоВзносамВДокументах");
	Методы.Вставить("СкопироватьСкидкиНадбавкиНСиПЗ");
КонецПроцедуры
...
Процедура СформироватьВТНачисленияСДаннымиУчета(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, ИсключаемыйРегистратор, ИмяТаблицыНачислений = "ВТНачисления") Экспорт
	...
	|		СтраховыеВзносыСведенияОДоходахОбороты.Сумма - ВЫБОР
	|			КОГДА СтраховыеВзносыСведенияОДоходахОбороты.ВидДохода.ОблагаетсяВзносамиЧастично
	|				ТОГДА СтраховыеВзносыСведенияОДоходахОбороты.Скидка
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК Результат
	|	ИЗ
	...
	|		ДанныеУчета.Сумма - ДанныеУчета.Скидка
	|	ИЗ
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(СведенияОДоходах.Сумма - СведенияОДоходах.Скидка) >= 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, "ВТНачисленияСДаннымиУчета");
		...
		Если КолонкиТаблицыНачислений.Найти("СуммаПоБюджетномуТарифуНСиПЗ") = Неопределено Тогда
			...
			|	ВременнаяТаблица.СторнируемыйДокумент КАК СторнируемыйДокумент,
			|	ВременнаяТаблица.УсловияТруда КАК УсловияТруда,
		...
		|	Начисления.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ,
		...
		|	СУММА(ВложенныйЗапрос.Сумма - ВложенныйЗапрос.Скидка) < 0 КАК Сторно
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|	СтрокиДокумента.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ,
		...
		|	СтрокиДокументаПоНомерСтроки.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ,
		|	ДанныеДокумента.Начисление КАК Начисление,
		...
		|	ДанныеДокумента.ИспользоватьБюджетныйТарифНСиПЗ,
		...
		|	НачисленияСоСведениямиРегистра.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ,
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Начисления.ДатаПолученияДохода КАК ДатаПолученияДохода,
		|	Начисления.Сотрудник КАК Сотрудник,
		|	Начисления.Начисление КАК Начисление,
		|	Начисления.Подразделение КАК Подразделение,
		|	Начисления.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ,
	...
	Если Не Запрос.Выполнить().Пустой() Тогда // Попробуем скомпенсировать уже имеющиеся сторно-записи прошлых периодов.
		...
		|	СтраховыеВзносыСведенияОДоходах.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ
		...
		|	СтраховыеВзносыСведенияОДоходах.ИспользоватьБюджетныйТарифНСиПЗ
		|
		|ИМЕЮЩИЕ
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда  // Компенсируем сторно-записи.
			...
			|	ЗаписиЗаПрошлыйПериод.ИспользоватьБюджетныйТарифНСиПЗ,
			...
			|	НачисленияСДаннымиУчета.ИспользоватьБюджетныйТарифНСиПЗ,
			...
			|	Временная.ИспользоватьБюджетныйТарифНСиПЗ,
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.38.32";
	Обработчик.Процедура = "УчетСтраховыхВзносовРасширенный.СкопироватьСкидкиНадбавкиНСиПЗ";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7779f0bd-783c-4168-aa4d-d96c3a41eddc");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение бюджетных скидок/надбавок.'");
...
Процедура СкопироватьСкидкиНадбавкиНСиПЗ(ПараметрыОбновления = Неопределено) Экспорт
	
	Если Не ОтражениеЗарплатыВБухучетеРасширенный.ИспользоватьБюджетныйТарифНСиПЗ('20250101') Тогда
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.УстановитьПараметрОбновления(ПараметрыОбновления, "ОбработкаЗавершена", Истина);
		Возврат	
	КонецЕсли;
	...
	|ГДЕ
	|	(СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.НадбавкаБюджетная <> 0
	|			ИЛИ СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СкидкаБюджетная <> 0)";
	...
	|	СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Ставка КАК Ставка,
	|	СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Надбавка КАК Надбавка,
	|	СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Скидка КАК Скидка,
	|	СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Надбавка КАК НадбавкаБюджетная,
	|	СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Скидка КАК СкидкаБюджетная
	|ИЗ
	|	РегистрСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев КАК СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев
	|ГДЕ
	|	(СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Надбавка <> 0
	|			ИЛИ СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.Скидка <> 0)
	|	И СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.НадбавкаБюджетная = 0
	|	И СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СкидкаБюджетная = 0";
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.НаборЗаписей", "Организация", Выборка.Организация) Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ДатыПолученияДоходов Цикл
		...
		Если ДобавитьСтроку Тогда
			...
			НоваяСтрока.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 получен доход, облагаемый страховыми взносами'"), Формат(СтрокаТаблицы.Дата, "ДЛФ=Д"));
		КонецЕсли;
