...
//  ИмяГруппыКадровыйРезерв - Строка - Имя группы кадровый резерв
Процедура ДополнитьФормуПозиции(Форма, ИмяГруппыКадровыйРезерв = "КадровыйРезервСтраница") Экспорт 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйРезерв.КадровыйРезервГосслужащих") Тогда
		...
		Если МодульКадровыйРезервГосслужащих.ИспользоватьКадровыйРезервВГосударственнойСлужбе() Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ИмяГруппыКадровыйРезерв, "Видимость",
...
Процедура СформироватьИнфоСтрокуГруппРезерва(Форма) Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТПозицияРезерва.ПозицияРезерва КАК ПозицияРезерва,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КадровыйРезерв.УчастникКадровогоРезерва) КАК Количество,
		|	ВТПозицияРезерва.ПозицияРезерваПредставление КАК ПозицияРезерваПредставление
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ВыборкаДетальныеЗаписи.ПозицияРезерваПредставление,,,,
			СсылкаПозиции));
		ТекстКоличества = " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru ='(%1 чел.)'"),
	...
	Если ЗначениеЗаполнено(ТекстНадписи) Тогда
		Форма.КадровыйРезервИнфоНадписьГруппРезерва = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Данная позиция включена в следующие группы резерва:'") + " ", ТекстНадписи);
	КонецЕсли;
...
#Область ФормаРезультатыАттестацииСотрудников

// Результаты аттестации при создании на сервере.
// 
// Параметры:
//  Форма - см. Документ.РезультатыАттестацииСотрудников.Форма.ФормаДокумента
Процедура РезультатыАттестацииПриСозданииНаСервере(Форма) Экспорт
	
	КадровыйРезервДоступен = КадровыйРезерв.КадровыйРезервДоступенПользователю();
	
	Если Не КадровыйРезервДоступен Тогда 
		Возврат;
	КонецЕсли;

	ЭлементГруппаКадровыйРезерв = Форма.Элементы.Найти("РезультатыАттестацииГруппаКадровыйРезерв");
	Если ЭлементГруппаКадровыйРезерв = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элемент = Форма.Элементы.Найти("РезультатыАттестацииПозицияРезерва");
	Если Элемент = Неопределено Тогда
		Элемент = Форма.Элементы.Вставить("РезультатыАттестацииПозицияРезерва", Тип("ПолеФормы"), ЭлементГруппаКадровыйРезерв);
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.Заголовок = "Позиция резерва";
		Элемент.ПутьКДанным = "Объект.РезультатыАттестации.ПозицияРезерва";
		Элемент.УстановитьДействие("ПриИзменении", "Подключаемый_РезультатыАттестацииПозицияРезерваПриИзменении");
	КонецЕсли;
	
	Элемент = Форма.Элементы.Найти("РезультатыАттестацииВидРезерва");
	Если Элемент = Неопределено Тогда
		Элемент = Форма.Элементы.Вставить("РезультатыАттестацииВидРезерва", Тип("ПолеФормы"), ЭлементГруппаКадровыйРезерв);
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.Заголовок = "Вид резерва";
		Элемент.ПутьКДанным = "Объект.РезультатыАттестации.ВидРезерва";
	КонецЕсли;
	
	Элемент = Форма.Элементы.Найти("РезультатыАттестацииСтатусГотовности");
	Если Элемент = Неопределено Тогда
		Элемент = Форма.Элементы.Вставить("РезультатыАттестацииСтатусГотовности", Тип("ПолеФормы"), ЭлементГруппаКадровыйРезерв);
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.Заголовок = "Статус готовности";
		Элемент.ПутьКДанным = "Объект.РезультатыАттестации.СтатусГотовности";
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "РезультатыАттестацииПозицияРезерва",
		"Видимость", КадровыйРезервДоступен);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "РезультатыАттестацииВидРезерва",
		"Видимость", КадровыйРезервДоступен);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "РезультатыАттестацииСтатусГотовности",
		"Видимость", КадровыйРезервДоступен);
	
	СоздатьКомандуИсключениеИзКадровогоРезерва(Форма);
	СоздатьЭлементыФормыКомандыКадровогоРезерва(Форма, Форма.КоманднаяПанель);
	
КонецПроцедуры

#КонецОбласти

#Область ФормаДисциплинарноеВзыскание

// Результаты аттестации при создании на сервере.
// 
// Параметры:
//  Форма - см. Документ.ДисциплинарноеВзыскание.Форма.ФормаДокумента
Процедура ДисциплинарноеВзысканиеПриСозданииНаСервере(Форма) Экспорт
...
Процедура СоздатьКомандыФормыКадровогоРезерва(Форма) Экспорт
	...
	СоздатьКомандуВключениеВКадровыйРезерв(Форма);
	СоздатьКомандуЗаявкаНаВключениеВКадровыйРезерв(Форма);
	СоздатьКомандуИсключениеИзКадровогоРезерва(Форма);
...
Процедура СоздатьЭлементыФормыКомандыКадровогоРезерва(Форма, ГруппаПосадкаФормы) Экспорт
	...
	СозданиеВКоманднойПанели = ГруппаПосадкаФормы.Вид = ВидГруппыФормы.КоманднаяПанель
		Или ГруппаПосадкаФормы.Вид = ВидГруппыФормы.ГруппаКнопок;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаВключениеВКадровыйРезерв")
		И Форма.Элементы.Найти("ФормаЗаявкаНаВключениеВКадровыйРезерв") = Неопределено
	...
	Если Форма.Элементы.Найти("ФормаВключениеВКадровыйРезерв") = Неопределено
		И Форма.Команды.Найти("ВключениеВКадровыйРезерв") <> Неопределено Тогда
	...
	Если Форма.Элементы.Найти("ФормаИсключениеИзКадровогоРезерва") = Неопределено
		И Форма.Команды.Найти("ИсключениеИзКадровогоРезерва") <> Неопределено Тогда
...
//  Структура - Новые параметры заполнения позиции резерва по сотруднику:
// * ТабличнаяЧасть - ДанныеФормыКоллекция
// * ДатаНачала - Дата
// * УчастникКадровогоРезерва - ОпределяемыйТип.УчастникКадровогоРезерва
// * Действие - ПеречислениеСсылка.ДействияСКадровымиРезервистами 
Функция ПараметрыЗаполненияПозицииРезерваПоУчастникуКадровогоРезерва() Экспорт
	
	НовыеПараметры = Новый Структура;
	
	НовыеПараметры.Вставить("ТабличнаяЧасть");
	НовыеПараметры.Вставить("ДатаНачала", ТекущаяДатаСеанса());
	НовыеПараметры.Вставить("УчастникКадровогоРезерва", Справочники.ФизическиеЛица.ПустаяСсылка());
	НовыеПараметры.Вставить("Действие", Перечисления.ДействияСКадровымиРезервистами.ПустаяСсылка());
	
	Возврат НовыеПараметры;
	
КонецФункции

// Заполняет табличную часть позициями резерва по участнику кадрового резерва.
// 
// Параметры:
// 	Параметры - см. ПараметрыЗаполненияПозицииРезерваПоУчастникуКадровогоРезерва
Процедура ЗаполнитьПозицииРезерваПоУчастникуКадровогоРезерва(Параметры) Экспорт
	
	Если Не КадровыйРезерв.КадровыйРезервДоступенПользователю() Или Не ЗначениеЗаполнено(Параметры)
		Или Не Параметры.Свойство("ТабличнаяЧасть")
		Или Не ЗначениеЗаполнено(Параметры.УчастникКадровогоРезерва) Тогда
		Возврат;
	КонецЕсли;
	
	ТабличнаяЧастьРезерва = Параметры.ТабличнаяЧасть;
	ТабличнаяЧастьРезерва.Очистить();
	
	Если Параметры.Свойство("ДатаНачала") Тогда
		Период = Параметры.ДатаНачала;
	Иначе
	...
	ПозицииРезерва = КадровыйРезерв.ПозицииРезерваПоУчастникуКадровогоРезерва(Параметры.УчастникКадровогоРезерва,
		Период, Статусы);
	Если Не ЗначениеЗаполнено(ПозицииРезерва) Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого ПозицияРезерва Из ПозицииРезерва Цикл
		
		НоваяПозиция = ТабличнаяЧастьРезерва.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяПозиция, ПозицияРезерва);
		НоваяПозиция.Действие = Параметры.Действие;
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
			...
			МодульНаставничество.ДополнитьСтрокуПозицииКадровогоРезерваСведениямиОНаставничестве(Период, Параметры.УчастникКадровогоРезерва, НоваяПозиция, Параметры);
		КонецЕсли;
...
// Возвращаемое значение:
//  Структура:
//   * УчастникКадровогоРезерва - СправочникСсылка.ФизическиеЛица
...
//   * ДатаРождения - Дата
//   * ТекстВозраста - Строка
//   * МассивРабочихМест - Массив из Структура
Функция НовоеОписаниеУчастникаКадровогоРезерва() Экспорт

	Описание = Новый Структура;
	
	Описание.Вставить("УчастникКадровогоРезерва", Справочники.ФизическиеЛица.ПустаяСсылка());
	Описание.Вставить("ПолученныеОбразования", "");
	Описание.Вставить("ЗнанияЯзыков", "");
	Описание.Вставить("Профессии", "");
	Описание.Вставить("Специальности", "");
	Описание.Вставить("ТрудоваяДеятельность", "");
	Описание.Вставить("ДатаРождения", Дата(1, 1, 1));
	Описание.Вставить("ТекстВозраста", "");
	Описание.Вставить("МассивРабочихМест", Новый Массив);
...
// Возвращаемое значение:
//  ТаблицаЗначений - Новая таблица кандидатов в резервисты:
// * УчастникКадровогоРезерва - ОпределяемыйТип.УчастникКадровогоРезерва
// * Представление - Строка
Функция НоваяТаблицаКандидатовВРезервисты() Экспорт
	
	КандидатыВРезервисты = Новый ТаблицаЗначений;
	КандидатыВРезервисты.Колонки.Добавить("УчастникКадровогоРезерва",
		Метаданные.ОпределяемыеТипы.УчастникКадровогоРезерва.Тип);
	КандидатыВРезервисты.Колонки.Добавить("Представление", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	
	КандидатыВРезервисты.Индексы.Добавить("Представление");
	
	Возврат КандидатыВРезервисты;
...
Процедура СоздатьРеквизитыКадровогоРезерваВФормеШтатногоРасписания(Форма)
	...
	// Связанная позиция
	РеквизитКадровыйРезервСвязаннаяПозицияРезерва = Новый РеквизитФормы("КадровыйРезервСвязаннаяПозицияРезерва",
		Новый ОписаниеТипов("СправочникСсылка.КадровыйРезерв"));
	МассивРеквизитов.Добавить(РеквизитКадровыйРезервСвязаннаяПозицияРезерва);
	// ИнфоНадпись
	РеквизитКадровыйРезервИнфоНадписьГруппРезерва = Новый РеквизитФормы("КадровыйРезервИнфоНадписьГруппРезерва",
		Новый ОписаниеТипов("ФорматированнаяСтрока"));
	МассивРеквизитов.Добавить(РеквизитКадровыйРезервИнфоНадписьГруппРезерва);
	// ДинамическийСписок
	РеквизитКадровыйРезервПозицииШР = Новый РеквизитФормы("КадровыйРезервПозицииШР",
...
Процедура УстановитьПараметрыДинамическогоСпискаКадровыйРезервШР(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.КадровыйРезервПозицииШР,
		"ПозицияШтатногоРасписания", Форма.Объект.Ссылка, Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.КадровыйРезервПозицииШР, "ТекущаяДата",
		ТекущаяДатаСеанса(), Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.КадровыйРезервПозицииШР, "Статусы",
		Статусы, Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.КадровыйРезервПозицииШР, "Кандидат",
		НСтр("ru = 'Кандидат'"), Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.КадровыйРезервПозицииШР, "ЗачисленВРезерв",
		НСтр("ru = 'Зачислен в резерв'"), Истина);
...
Процедура УстановитьОбязательноеИспользованиеПолейДинамическогоСпискаКадровыйРезервШР(Форма)
	...
	РеквизитКадровыйРезервПозицииШР.УстановитьОбязательноеИспользование("УчастникКадровогоРезерва", Истина);
	РеквизитКадровыйРезервПозицииШР.УстановитьОбязательноеИспользование("ВидРезерва", Истина);

КонецПроцедуры

Процедура УстановитьЗаголовокКомандыСоздатьОткрытьВФормеШтатногоРасписания(Форма, КлючеваяПозицияШтатногоРасписания,
	Заголовок = "")
...
Процедура СоздатьЭлементыФормыТаблицаКадровыйРезерв(Форма)
	...
	Если ГруппаСписка = Неопределено Тогда
		// Создаем группу.
		ГруппаСписка = Форма.Элементы.Вставить("ГруппаСпискаКадровогоРезерва", Тип("ГруппаФормы"),
			Форма.Элементы.ГруппаКадровогоРезерва);
	...
	Если Форма.Элементы.Найти("Рейтинг") = Неопределено Тогда
		...
		ГруппаКомандРейтинга = Форма.Элементы.Вставить("Рейтинг", Тип("ГруппаФормы"),
			ТаблицаКадровыйРезерв.КонтекстноеМеню);
	...
	Если Форма.Элементы.Найти("КадровыйРезервПозицииШРВидРезерва") = Неопределено Тогда
		...
		ПолеКадровыйРезервПозицииШРВидРезерва = Форма.Элементы.Вставить("КадровыйРезервПозицииШРВидРезерва",
			Тип("ПолеФормы"), ТаблицаКадровыйРезерв);
	...
	Если Форма.Элементы.Найти("КадровыйРезервПозицииШРФизическоеЛицо") = Неопределено Тогда
		ПолеКадровыйРезервПозицииШРФизическоеЛицо = Форма.Элементы.Вставить("КадровыйРезервПозицииШРФизическоеЛицо",
			Тип("ПолеФормы"), ТаблицаКадровыйРезерв);
		ПолеКадровыйРезервПозицииШРФизическоеЛицо.ПутьКДанным = "КадровыйРезервПозицииШР.УчастникКадровогоРезерва";
	...
	Если Форма.Элементы.Найти("КадровыйРезервПозицииШРСтатус") = Неопределено Тогда
		ПолеКадровыйРезервПозицииШРСтатус = Форма.Элементы.Вставить("КадровыйРезервПозицииШРСтатус", Тип("ПолеФормы"),
			ТаблицаКадровыйРезерв);
	...
	Если Форма.Элементы.Найти("КадровыйРезервПозицииШРРейтинг") = Неопределено Тогда
		ПолеКадровыйРезервПозицииШРРейтинг = Форма.Элементы.Вставить("КадровыйРезервПозицииШРРейтинг", Тип("ПолеФормы"),
			ТаблицаКадровыйРезерв);
	...
	Если Форма.Элементы.Найти("КадровыйРезервПозицииШРДатаВключения") = Неопределено Тогда
		ПолеКадровыйРезервПозицииШРДатаВключения = Форма.Элементы.Вставить("КадровыйРезервПозицииШРДатаВключения",
			Тип("ПолеФормы"), ТаблицаКадровыйРезерв);
	...
	Если Форма.Элементы.Найти("КадровыйРезервПозицииШРДатаИсключения") = Неопределено Тогда
		ПолеКадровыйРезервПозицииШРДатаИсключения = Форма.Элементы.Вставить("КадровыйРезервПозицииШРДатаИсключения",
			Тип("ПолеФормы"), ТаблицаКадровыйРезерв);
...
Процедура СоздатьЭлементыФормыИнфоНадписьГруппРезерва(Форма, ГруппаПосадкаФормы)
	...
	Если Форма.Элементы.Найти("КадровыйРезервИнфоНадписьГруппРезерва") = Неопределено Тогда
		...
		Элемент.УстановитьДействие("ОбработкаНавигационнойСсылки",
			"Подключаемый_КадровыйРезервИнфоНадписьГруппРезерваОбработкаНавигационнойСсылки");
	КонецЕсли;
...
Процедура СоздатьТаблицуКадровыйРезервВФормеОбщегоДокумента(Форма, СоздаватьКолонкуСтатусаВТаблице,
	ДобавлятьЭлементыФормы = Истина, ДобавлятьРеквизитыФормы = Истина, ОтложенноеИзменение = Ложь)
	...
	Если ДобавлятьЭлементыФормы Тогда
		...
		Если ТаблицаКадровыйРезерв = Неопределено Тогда
			...
			ТаблицаКадровыйРезерв = Форма.Элементы.Вставить("КадровыйРезерв", Тип("ТаблицаФормы"),
				Форма.Элементы.ГруппаКадровогоРезерва);
			...
			ТаблицаКадровыйРезерв.УстановитьДействие("ПриАктивизацииСтроки",
				"Подключаемый_КадровыйРезервПриАктивизацииСтроки");
		КонецЕсли;
		
		Если Форма.Элементы.Найти("КадровыйРезервПозицияРезерва") = Неопределено Тогда
			ПолеКадровыйРезервПозицияРезерва = Форма.Элементы.Вставить("КадровыйРезервПозицияРезерва", Тип("ПолеФормы"),
				ТаблицаКадровыйРезерв);
		...
		Если СоздаватьКолонкуСтатусаВТаблице И Форма.Элементы.Найти("КадровыйРезервОписаниеСтатуса") = Неопределено Тогда
			ПолеКадровыйРезервОписаниеСтатуса = Форма.Элементы.Вставить("КадровыйРезервОписаниеСтатуса",
				Тип("ПолеФормы"), ТаблицаКадровыйРезерв);
		...
		Если Форма.Элементы.Найти("КадровыйРезервДействие") = Неопределено Тогда
			ПолеКадровыйРезервПозицияРезерва = Форма.Элементы.Вставить("КадровыйРезервДействие", Тип("ПолеФормы"),
				ТаблицаКадровыйРезерв);
...
Процедура СоздатьКомандыКадровогоРезерваВФормеОбщегоДокумента(Форма)
	...
	Если ГруппаКнопокКадровогоРезерва = Неопределено Тогда
		ГруппаКнопокКадровогоРезерва = Форма.Элементы.Вставить("КадровыйРезервГруппаКоманд", Тип("ГруппаФормы"),
			ГруппаПанель);
	...
	Если Форма.Элементы.Найти("КадровыйРезервКадровыйРезервОставить") = Неопределено Тогда
		Элемент = Форма.Элементы.Вставить("КадровыйРезервКадровыйРезервОставить", Тип("КнопкаФормы"),
			ГруппаКнопокКадровогоРезерва);
		Элемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
	...
	Если Форма.Элементы.Найти("КадровыйРезервКадровыйРезервИсключить") = Неопределено Тогда
		Элемент = Форма.Элементы.Вставить("КадровыйРезервКадровыйРезервИсключить", Тип("КнопкаФормы"),
			ГруппаКнопокКадровогоРезерва);
		Элемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
		Элемент.Отображение = ОтображениеКнопки.КартинкаИТекст;
...
Процедура ЗаполнитьКадровыйРезервВДокументеКадровыйПеревод(Форма) Экспорт
	...
	ЗаполнитьПозицииРезерваПоУчастникуКадровогоРезерва(ПараметрыЗаполнения);
	СписокПозицийРезерва = КадровыйРезерв.ПозицииКадровогоРезерваСогласноПозицииШтатногоРасписания(
		Объект.ДолжностьПоШтатномуРасписанию, , Объект.ДатаНачала);
...
Процедура ЗаполнитьКадровыйРезервВДокументеУвольнение(Форма) Экспорт
	...
	ЗаполнитьПозицииРезерваПоУчастникуКадровогоРезерва(ПараметрыЗаполнения);
	ОставитьВРезервеЕслиМестоРаботыНеОсновное(Объект);
	КадровыйРезервКлиентСервер.ЗаполнитьВторичныеДанныеКадровогоРезерва(Объект.КадровыйРезерв);
...
// Возвращает данные для создания документов исключения из кадрового резерва по результатам аттестации.
// 
// Параметры:
//  Объект - ДанныеФормыСтруктура
// 
// Возвращаемое значение:
//  см. ДанныеДляИсключенияИзКадровогоРезерва
Функция ДанныеДляИсключенияИзКадровогоРезерваПоРезультатамАттестации(Знач Объект) Экспорт

	Сотрудники = Новый Массив;
	Для Каждого ТекущаяСтрока Из Объект.РезультатыАттестации Цикл
		Если ТекущаяСтрока.РезультатАттестации <> АттестацииСотрудниковФормы.ПоложительныйРезультатАттестации(
			Объект.ВидАттестации) Тогда
			Сотрудники.Добавить(ТекущаяСтрока.Сотрудник);
		КонецЕсли;
	КонецЦикла;
	
	Данные = ДанныеДляИсключенияИзКадровогоРезерва(Объект.Ссылка, Сотрудники);
	
	Возврат Данные;
	
КонецФункции

// Возвращает данные для создания документов исключения из кадрового резерва по дисциплинарному взысканию.
// 
// Параметры:
//  Объект - ДанныеФормыСтруктура
// 
...
// Возвращает данные для создания документов исключения из кадрового резерва.
// 
// Параметры:
//  Основание - ОпределяемыйТип.ОснованиеИсключенияИзКадровогоРезерва
//  СотрудникиДокумента - Массив из СправочникСсылка.Сотрудники
// 
// Возвращаемое значение:
//  Структура:
//   * ДанныеДляИсключенияИзКадровогоРезерва - Массив из Структура:
//     ** УчастникКадровогоРезерва - СправочникСсылка.ФизическиеЛица
//     ** Подразделение - СправочникСсылка.СтруктураПредприятия
//   * ДокументыВведенныеНаОсновании - Массив из Структура:
//     ** Сотрудник - СправочникСсылка.Сотрудники 
//     ** Документ - ДокументСсылка.ИсключениеИзКадровогоРезерва
Функция ДанныеДляИсключенияИзКадровогоРезерва(Основание, СотрудникиДокумента)

	Данные = Новый Структура("ДанныеДляИсключенияИзКадровогоРезерва, ДокументыВведенныеНаОсновании");
	Данные.Вставить("ДанныеДляИсключенияИзКадровогоРезерва", Новый Массив);
	Данные.Вставить("ДокументыВведенныеНаОсновании", Новый Массив);
	
	ДокументыВведенныеНаОсновании = ИсключенияИзКадровогоРезерваВведенныеНаОсновании(Основание, СотрудникиДокумента);
	Данные.ДокументыВведенныеНаОсновании = ОбщегоНазначения.ТаблицаЗначенийВМассив(ДокументыВведенныеНаОсновании);
	Сотрудники = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СотрудникиДокумента,
		ОбщегоНазначения.ВыгрузитьКолонку(Данные.ДокументыВведенныеНаОсновании, "Сотрудник"));
	
	Если ЗначениеЗаполнено(Сотрудники) Тогда
		Данные.ДанныеДляИсключенияИзКадровогоРезерва = ДанныеДляИсключенияИзКадровогоРезерваПоСотрудникам(Сотрудники);
	КонецЕсли;
...
//   * Сотрудник - СправочникСсылка.Сотрудники 
//   * Документ - ДокументСсылка.ИсключениеИзКадровогоРезерва
Функция ИсключенияИзКадровогоРезерваВведенныеНаОсновании(Основание, Сотрудники)
	
	ПодчиненныеДокументы = Новый ТаблицаЗначений();
	ПодчиненныеДокументы.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ПодчиненныеДокументы.Колонки.Добавить("Документ", Новый ОписаниеТипов("ДокументСсылка.ИсключениеИзКадровогоРезерва"));
	...
	Запрос.Текст =
		...
		|	ИсключениеИзКадровогоРезерва.Ссылка КАК Документ
		|ИЗ
		|	ВТСотрудники КАК ВТСотрудники
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИсключениеИзКадровогоРезерва КАК ИсключениеИзКадровогоРезерва
		|		ПО ВТСотрудники.ФизическоеЛицо = ИсключениеИзКадровогоРезерва.УчастникКадровогоРезерва
		|		И ИсключениеИзКадровогоРезерва.ОснованиеОбъект = &Основание
		|		И НЕ ИсключениеИзКадровогоРезерва.ПометкаУдаления";
...
//   * УчастникКадровогоРезерва - СправочникСсылка.ФизическиеЛица
//   * Подразделение - СправочникСсылка.СтруктураПредприятия
Функция ДанныеДляИсключенияИзКадровогоРезерваПоСотрудникам(Знач Сотрудники)
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудники,
		"ФизическоеЛицо, МестоВСтруктуреПредприятия");
	
	Статусы = Новый Массив;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаВключениеВКадровыйРезерв") Тогда
		Статусы.Добавить(Перечисления.СостоянияСогласования.Рассматривается);
	КонецЕсли;
	Статусы.Добавить(Перечисления.СостоянияСогласования.Согласовано);
	
	Отбор = КадровыйРезерв.НовыйОтборКадровогоРезерваНаДату();
	Отбор.УчастникиКадровогоРезерва = КадровыеДанные.ВыгрузитьКолонку("ФизическоеЛицо");
	Отбор.Статусы = Статусы;
	ПозицииРезерва = КадровыйРезерв.КадровыйРезервНаДату(ТекущаяДатаСеанса(), Отбор);

	Если Не ЗначениеЗаполнено(ПозицииРезерва) Тогда
		Возврат Данные;
	КонецЕсли;
	
	Для Каждого ТекущаяСтрока Из КадровыеДанные Цикл
		Если ПозицииРезерва.Найти(ТекущаяСтрока.ФизическоеЛицо, "УчастникКадровогоРезерва") <> Неопределено Тогда
			Данные.Добавить(Новый Структура("УчастникКадровогоРезерва, Подразделение",
				ТекущаяСтрока.ФизическоеЛицо, ТекущаяСтрока.МестоВСтруктуреПредприятия));
		КонецЕсли;
	КонецЦикла;
...
Процедура ОставитьВРезервеЕслиМестоРаботыНеОсновное(Объект);
	...
	Если КадровыеДанные[0].ВидЗанятости <> Перечисления.ВидыЗанятости.ОсновноеМестоРаботы Тогда
		Для Каждого Строка Из Объект.КадровыйРезерв Цикл
			Строка.Действие = ПредопределенноеЗначение("Перечисление.ДействияСКадровымиРезервистами.Оставить");
		КонецЦикла;
	КонецЕсли;
...
Функция ПараметрыЗаполненияПозицииРезерваПоСотрудникуДляОбщегоДокумента(Объект, КадровыйПеревод = Истина)
	...
	ПараметрыЗаполнения = ПараметрыЗаполненияПозицииРезерваПоУчастникуКадровогоРезерва();
	ПараметрыЗаполнения.ТабличнаяЧасть = Объект.КадровыйРезерв;
	ПараметрыЗаполнения.УчастникКадровогоРезерва = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Сотрудник,
		"ФизическоеЛицо");
...
// Устанавливает значение реквизита и свойства поля с типом участника кадрового резерва в формах документов.
// 
// Параметры:
//  ТипУчастникаКадровогоРезерва - Тип
//  УчастникКадровогоРезерва - ОпределяемыйТип.УчастникКадровогоРезерва
//  ЭлементФормыТипУчастника - ПолеВвода,Неопределено
//  ЭлементФормыУчастникКадровогоРезерва - ПолеВвода,Неопределено
Процедура УстановитьТипУчастникаКадровогоРезерва(ТипУчастникаКадровогоРезерва, УчастникКадровогоРезерва,
	ЭлементФормыТипУчастника = Неопределено, ЭлементФормыУчастникКадровогоРезерва = Неопределено)
	
	СписокВыбора = Новый СписокЗначений();
	
	ТипыУчастниковКадровогоРезерва = КадровыйРезерв.ТипыУчастниковКадровогоРезерваДоступныхПоФункциональнымОпциям();
	ОбщегоНазначенияКлиентСервер.Проверить(ЗначениеЗаполнено(ТипыУчастниковКадровогоРезерва),
		НСтр("ru = 'Для текущего пользователя нет доступных типов участников кадрового резерва.'"),
		"УстановитьТипУчастникаКадровогоРезерва");
		
	Для Каждого Тип Из ТипыУчастниковКадровогоРезерва Цикл
		ОписаниеТипов = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип));
		СписокВыбора.Добавить(ОписаниеТипов);
	КонецЦикла;
	
	Если УчастникКадровогоРезерва = Неопределено Тогда
		ТипУчастникаКадровогоРезерва = СписокВыбора.Получить(0).Значение;
	Иначе
		ТипУчастникаКадровогоРезерва = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
			ТипЗнч(УчастникКадровогоРезерва)));
	КонецЕсли;
	
	ДоступноБолееОдногоТипаУчастников = СписокВыбора.Количество() > 1;
	Если ЭлементФормыТипУчастника <> Неопределено Тогда
		ЭлементФормыТипУчастника.СписокВыбора.ЗагрузитьЗначения(СписокВыбора.ВыгрузитьЗначения());
		ЭлементФормыТипУчастника.Видимость = ДоступноБолееОдногоТипаУчастников;
		Для Каждого ЭлементСписка Из ЭлементФормыТипУчастника.СписокВыбора Цикл
			ЭлементСписка.Представление = ПредставлениеТипаУчастникаКадровогоРезерва(ЭлементСписка.Значение)
		КонецЦикла;
	КонецЕсли;

	Если ЭлементФормыУчастникКадровогоРезерва <> Неопределено Тогда
		Если ДоступноБолееОдногоТипаУчастников Тогда
			ЭлементФормыУчастникКадровогоРезерва.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		Иначе
			ЭлементФормыУчастникКадровогоРезерва.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
			ЭлементФормыУчастникКадровогоРезерва.АвтоМаксимальнаяШирина = Истина;
		КонецЕсли;
	КонецЕсли;
...
// Возвращаемое значение:
//  Строка
Функция ПредставлениеТипаУчастникаКадровогоРезерва(ОписаниеТипа)
	
	Если ОписаниеТипа.СодержитТип(Тип("СправочникСсылка.ФизическиеЛица")) Тогда
		Представление = НСтр("ru = 'Сотрудник'");
	ИначеЕсли ОписаниеТипа.СодержитТип(Тип("СправочникСсылка.ЛичныеДанныеКандидатов")) Тогда
		Представление = НСтр("ru = 'Кандидат'");
	Иначе
		Представление = "";
	КонецЕсли;
...
Процедура ОбработкаВыбораУчастникаКадровогоРезерва(Знач ВыбранноеЗначение, Объект) Экспорт
	
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Сотрудники") Тогда
		Объект.УчастникКадровогоРезерва = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыбранноеЗначение, "ФизическоеЛицо");
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Кандидаты") Тогда
		Объект.УчастникКадровогоРезерва = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыбранноеЗначение, "ЛичныеДанные");
	Иначе
		Объект.УчастникКадровогоРезерва = ВыбранноеЗначение;
	КонецЕсли;
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
Процедура УстановитьСвойстваТипаУчастникаКадровогоРезерва(Форма) Экспорт
	
	УстановитьТипУчастникаКадровогоРезерва(Форма.ТипУчастникаКадровогоРезерва, Форма.Объект.УчастникКадровогоРезерва,
		Форма.Элементы.Найти("ТипУчастникаКадровогоРезерва"), Форма.Элементы.Найти("УчастникКадровогоРезерва"));
	
КонецПроцедуры

// Параметры:
//  Форма - ФормаКлиентскогоПриложения
Процедура УстановитьСвойстваУчастникаКадровогоРезерва(Форма) Экспорт
	
	Форма.Элементы.УчастникКадровогоРезерва.ОграничениеТипа = Форма.ТипУчастникаКадровогоРезерва;
	Форма.Объект.УчастникКадровогоРезерва = Форма.ТипУчастникаКадровогоРезерва.ПривестиЗначение(
		Форма.Объект.УчастникКадровогоРезерва);
...
Процедура ДополнитьКомандыИКнопкиПодбора(Форма, ИмяКоманднойПанели, ИмяТабличнойЧасти) Экспорт
	
	Элементы = Форма.Элементы;
	
	НоваяКоманда = Форма.Команды.Найти("ПодобратьКандидатов"); 
	Если НоваяКоманда = Неопределено Тогда 
		НоваяКоманда = Форма.Команды.Добавить("ПодобратьКандидатов");
	КонецЕсли; 
	НоваяКоманда.Действие = "Подключаемый_ПодобратьКандидатов";
	НоваяКоманда.Заголовок = НСтр("ru = 'Кандидаты'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Подсказка = НСтр("ru = 'Подобрать кандидатов'");
	
	КоманднаяПанель = Элементы.Найти(ИмяКоманднойПанели);
	Если КоманднаяПанель = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИмяГруппы = ИмяТабличнойЧасти + "ПодборМеню";
	ПодборМеню = Элементы.Найти(ИмяГруппы);
	Если ПодборМеню = Неопределено Тогда 
		ПодборМеню = Элементы.Вставить(ИмяГруппы, Тип("ГруппаФормы"), КоманднаяПанель,
			Элементы.Найти(ИмяТабличнойЧасти + "Подбор"));
	КонецЕсли; 
	ПодборМеню.Вид = ВидГруппыФормы.Подменю;
	ПодборМеню.Заголовок = НСтр("ru = 'Подбор'");
	
	ИмяКнопкиПодобратьСотрудников = ИмяТабличнойЧасти + "ПодобратьСотрудников";
	ПодобратьСотрудников = Элементы.Найти(ИмяКнопкиПодобратьСотрудников);
	Если ПодобратьСотрудников = Неопределено Тогда 
		ПодобратьСотрудников = Элементы.Вставить(ИмяКнопкиПодобратьСотрудников, Тип("КнопкаФормы"), ПодборМеню);
	КонецЕсли; 
	ПодобратьСотрудников.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	ПодобратьСотрудников.Заголовок = НСтр("ru = 'Сотрудники'");
	ПодобратьСотрудников.ИмяКоманды = "Подобрать";

	ИмяКнопкиПодобратьКандидатов = ИмяТабличнойЧасти + "ПодобратьКандидатов";
	ПодобратьКандидатов = Элементы.Найти(ИмяКнопкиПодобратьКандидатов);
	Если ПодобратьКандидатов = Неопределено Тогда 
		ПодобратьКандидатов = Элементы.Вставить(ИмяКнопкиПодобратьКандидатов, Тип("КнопкаФормы"), ПодборМеню);
	КонецЕсли; 
	ПодобратьКандидатов.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	ПодобратьКандидатов.Заголовок = НСтр("ru = 'Кандидаты'");
	ПодобратьКандидатов.ИмяКоманды = "ПодобратьКандидатов";
...
Функция ОтборИзПодразделения(Знач Подразделение) Экспорт

	ОтборИзПодразделения = Новый Структура;
	ОтборИзПодразделения.Вставить("Организация", Справочники.Организации.ПустаяСсылка());
	ОтборИзПодразделения.Вставить("Подразделение", Справочники.ПодразделенияОрганизаций.ПустаяСсылка());
	...
	Если ТипЗнч(Источник) = Тип("СправочникСсылка.Организации") Тогда
		ОтборИзПодразделения.Организация = Источник;
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
		ОтборИзПодразделения.Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник, "ГоловнаяОрганизация");
		ОтборИзПодразделения.Подразделение = Источник;
	КонецЕсли;
...
Процедура СоздатьКомандуВключениеВКадровыйРезерв(Форма)
...
Процедура СоздатьКомандуЗаявкаНаВключениеВКадровыйРезерв(Форма)
...
Процедура СоздатьКомандуИсключениеИзКадровогоРезерва(Форма)
