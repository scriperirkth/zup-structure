...
Процедура ПолучитьАлгоритмыШифрования(
		...
		ИспользоватьКеширование,
		ПараметрыОпределенияАлгоритма) Экспорт
	
	Если ИспользоватьКеширование Тогда
		АлгоритмыШифрования = ПараметрыПриложения["КриптографияЭДКО.АлгоритмыШифрования"];
		ТипКриптопровайдераИнициализации = ПараметрыПриложения["КриптографияЭДКО.ТипКриптопровайдераИнициализации"];
		Контекст = Новый Структура("ТипКриптопровайдераИнициализации", ТипКриптопровайдераИнициализации);
...
Процедура ПолучитьАлгоритмыШифрованияПослеСозданияМенеджераКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Контекст.Вставить("ТипКриптопровайдераИнициализации", ТипКриптопровайдераИнициализации);
...
Процедура ПолучитьАлгоритмыШифрованияПослеПолучитьСледующийАлгоритмШифрования(
	...
	Иначе
		Контекст = ВходящийКонтекст.ОповещениеОЗавершении.ДополнительныеПараметры;
		ПараметрыПриложения["КриптографияЭДКО.АлгоритмыШифрования"] = Контекст.АлгоритмыШифрования;
		ПараметрыПриложения["КриптографияЭДКО.ТипКриптопровайдераИнициализации"] = Контекст.ТипКриптопровайдераИнициализации;
		ВходящийКонтекст.Вставить("ТипКриптопровайдераИнициализации", Контекст.ТипКриптопровайдераИнициализации);
		ВыполнитьОбработкуОповещения(
			ВходящийКонтекст.ОповещениеОЗавершении,
			КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(
				Истина,
...
Процедура ИмпортироватьСертификатПослеСозданияМенеджераКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ДлительнаяОтправкаКлиент.ЗапомнитьКонтекстДлительнойОперации(ВходящийКонтекст,
			"КриптографияЭДКОКлиент.ИмпортироватьСертификатМассив");
...
Процедура ИмпортироватьСертификатКриптографии() Экспорт
	...
	ВходящийКонтекст = ДлительнаяОтправкаКлиент.КонтекстДлительнойОперации(
		"КриптографияЭДКОКлиент.ИмпортироватьСертификатМассив");
	Если ДлительнаяОтправкаКлиент.ЕстьКонтекстДлительнойОперации(
		"КриптографияЭДКОКлиент.ИмпортироватьСертификатМассив") Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ИмпортироватьСертификатКриптографии", 0.1, Истина);
	КонецЕсли;
	Если ВходящийКонтекст = Неопределено
		ИЛИ НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВходящийКонтекст, "ВходящиеПараметры") Тогда
		
		ТипВходящегоКонтекста = Строка(ТипЗнч(ВходящийКонтекст));
		КлючиВходящегоКонтекста = ?(ВходящийКонтекст = Неопределено, "",
			ОбщегоНазначенияКлиентСервер.КлючиСтруктурыВСтроку(ВходящийКонтекст));
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка при импорте сертификата криптографии: отсутствуют входящие параметры, тип контекста: ""%1"", ключи: ""%2"".'"),
			ТипВходящегоКонтекста,
			КлючиВходящегоКонтекста);
...
Процедура ЗашифроватьPKCS7(
	...
	ИначеЕсли СертификатыВЗащищенномХранилищеНаСервере И ТипыПодходятДляЭПСервиса
		И ЭлектроннаяПодписьВМоделиСервисаКлиентСервер.ИспользованиеВозможно() Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыОперацииВызова.АлгоритмШифрования) Тогда
			АлгоритмШифрованияВМоделиСервиса = КриптографияЭДКОКлиентСервер.АлгоритмШифрованияВМоделиСервиса(
				ПараметрыОперацииВызова.АлгоритмШифрования);
			СервисКриптографииКлиент.Зашифровать(
				Оповещение,
				ИсходныйФайл,
				Сертификаты,
				"CMS",
				Новый Структура("АлгоритмШифрования", АлгоритмШифрованияВМоделиСервиса));
...
Процедура ЗашифроватьPKCS7ПослеПолученияАлгоритмовШифрования(Результат, ВходящийКонтекст) Экспорт
	...
	ВходящиеПараметры = ВходящийКонтекст.ВходящиеПараметры;
	Если НЕ ВходящиеПараметры[5].АлгоритмШифрованияОбязателен Тогда
		...
		Если НЕ АлгоритмШифрованияНайден Тогда
			ВходящиеПараметры[5].ИспользуемыйАлгоритмШифрования = "GOST 28147-89"; // алгоритм шифрования
		КонецЕсли;
	КонецЕсли;
	...
	Если Алгоритм = "*" Тогда
		ПараметрыОпределенияАлгоритма = Новый Структура("Сертификат",
			?(ВходящиеПараметры[1].Количество() >= 1, ВходящиеПараметры[1].Сертификаты[0], Неопределено));
	Иначе
		ПараметрыОпределенияАлгоритма = Новый Структура("Алгоритм", Алгоритм);
	КонецЕсли;
	Если НЕ ВходящиеПараметры[5].АлгоритмШифрованияОбязателен
		ИЛИ ВРег(ВходящиеПараметры[5].ИспользуемыйАлгоритмШифрования) = "GOST 28147-89" Тогда
		ПараметрыОпределенияАлгоритма.Вставить("ПриоритетViPNet", Истина);
	КонецЕсли;
	ПредварительноСоздатьМенеджерКриптографии(
		ВходящийКонтекст.ОповещениеОЗавершении,
...
Процедура ЗашифроватьPKCS7ПриОшибкеВыполнения(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	Если ВходящиеПараметры[5].АлгоритмШифрованияОбязателен
		...
		Оповещение = Новый ОписаниеОповещения("ЗашифроватьPKCS7ПриОшибкеПослеПолученияАлгоритмов",
			ЭтотОбъект, ВходящийКонтекст);
		
		Если ВходящиеПараметры[5].АлгоритмШифрованияОбязателен Тогда
			ПолучитьАлгоритмыШифрования(
				Оповещение,
				ВходящийКонтекст.ВыводитьСообщения,
				Неопределено,
				Неопределено,
				Истина,
				?(ВРег(ВходящийКонтекст.ИспользуемыйАлгоритмШифрования) = "GOST 28147-89", Неопределено, Новый Структура));
...
Процедура ЗашифроватьPKCS7ПриОшибкеПослеПолученияАлгоритмов(Результат, ВходящийКонтекст) Экспорт
	...
	СтандартнаяОбработкаОшибки(
		ВходящийКонтекст.СтандартнаяОбработка,
		ВходящийКонтекст,
		"PKCS7ЗашифроватьДляНескольких",
		ОписаниеОшибкиНеУдалосьЗашифроватьФайл()
			+ ?(ВходящиеПараметры[5].АлгоритмШифрованияОбязателен И НЕ АлгоритмШифрованияНайден,
			" " + КриптографияЭДКОСлужебныйКлиентСервер.ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018(
				ВходящийКонтекст.ТипКриптопровайдераИнициализации), ""),
		ВходящийКонтекст.ИнформацияОбОшибке);
...
Процедура РасшифроватьPKCS7(
	...
	ПараметрыОперацииВызова = Новый Структура;
	ПараметрыОперацииВызова.Вставить("АлгоритмШифрованияОбязателен", Ложь); // может быть передано значение Неопределено
	Если ПараметрыОперации <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыОперацииВызова, ПараметрыОперации);
	КонецЕсли;
	
	ВходящиеПараметры =
		ПараметрыВМассив(5, ЗашифрованныйФайл, ИмяФайлаИлиРасширениеИлиАдрес, Сертификат, Алгоритм, ПараметрыОперацииВызова);
...
Процедура РасшифроватьPKCS7ПриОшибкеВыполнения(ИнформацияОбОшибке, СтандартнаяОбработка, ВходящийКонтекст) Экспорт
	...
	ВходящийКонтекст.Вставить("ОшибкаСервиса", ОшибкаСервиса);
	ВходящийКонтекст.Вставить("ИнформацияОбОшибке", ИнформацияОбОшибке);
	ВходящийКонтекст.Вставить("СтандартнаяОбработка", СтандартнаяОбработка);
	
	Оповещение = Новый ОписаниеОповещения("РасшифроватьPKCS7ПриОшибкеПослеПолученияАлгоритмов", ЭтотОбъект, ВходящийКонтекст);
	
	Если ВходящийКонтекст.ВходящиеПараметры.Количество() >= 5
		И ВходящийКонтекст.ВходящиеПараметры[4] <> Неопределено
		И ВходящийКонтекст.ВходящиеПараметры[4].АлгоритмШифрованияОбязателен = Истина Тогда
...
Процедура РасшифроватьPKCS7ПриОшибкеПослеПолученияАлгоритмов(Результат, ВходящийКонтекст) Экспорт
	
	ВходящиеПараметры = ВходящийКонтекст.ВходящиеПараметры;
	Если ВходящиеПараметры.Количество() >= 5
		И ВходящиеПараметры[4] <> Неопределено
		И ВходящиеПараметры[4].АлгоритмШифрованияОбязателен = Истина Тогда
	...
	СтандартнаяОбработкаОшибки(
		...
		ВходящийКонтекст,
		"PKCS7Расшифровать",
		ОписаниеОшибкиНеУдалосьРасшифроватьФайл()
			+ ?(ВходящиеПараметры.Количество() >= 5 И ВходящиеПараметры[4] <> Неопределено
				И ВходящиеПараметры[4].АлгоритмШифрованияОбязателен = Истина И НЕ АлгоритмШифрованияНайден,
			" " + КриптографияЭДКОСлужебныйКлиентСервер.ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018(
				ВходящийКонтекст.ТипКриптопровайдераИнициализации), ""),
		ВходящийКонтекст.ИнформацияОбОшибке,
		ВходящийКонтекст.ОшибкаСервиса);
...
Процедура МенеджерКриптографииПослеПолученияЗначенияСвойстваСписокОшибок(ЗначениеСвойства, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли ВходящийКонтекст.ИмяМетода = "PKCS7Зашифровать"
		ИЛИ ВходящийКонтекст.ИмяМетода = "PKCS7ЗашифроватьДляНескольких" Тогда
		
		Если (СтрНайти(ЗначениеСвойства, "Библиотека поставщика проинициализирована неправильно.") > 0
			ИЛИ СтрНайти(ЗначениеСвойства, "Provider DLL failed to initialize correctly") > 0)
			И ВходящийКонтекст.Свойство("ИспользуемыйАлгоритмШифрования")
			И ЗначениеЗаполнено(ВходящийКонтекст.ИспользуемыйАлгоритмШифрования)
			И ВходящийКонтекст.ИспользуемыйАлгоритмШифрования <> "GOST 28147-89" Тогда
			
			ЗначениеСвойства = НСтр("ru = 'Проверьте лицензию криптопровайдера.'") + " "
				+ НСтр("ru = 'При обмене с ФНС в случае использования моделей Рутокен ЭЦП 2.0, Рутокен Lite, JaCarta версии меньше 2 или JaCarta-2 SE в активном режиме (с неизвлекаемыми ключами) потребуется другая модель ключевого носителя и получение новых ключей.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиСообщениеПриНеобходимости(ВыводитьСообщения, Сообщение, Знач СвязанныйОбъект = Неопределено)
	...
	Если ВыводитьСообщения Тогда
		Если СтрНайти(Сообщение, "ГОСТ 34.12-2018") <> 0 Тогда
			КонтекстОбработки = Новый ФиксированнаяСтруктура("ТипКриптопровайдера", ?(СтрНайти(Сообщение, "ViPNet CSP") = 0,
			...
			ОбъектОснование = Новый ФиксированнаяСтруктура("ПриОшибкеГОСТ34122018", КонтекстОбработки);
