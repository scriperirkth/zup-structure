...
//
// Параметры:
//   УчитыватьФункциональныеОпции - Булево - 
Процедура ОбновитьСтруктуруПредприятия(УчитыватьФункциональныеОпции = Истина) Экспорт
	...
	Если Константы.СтруктураПредприятияНеСоответствуетСтруктуреЮридическихЛиц.Получить() Тогда
		ОрганизационнаяСтруктура.ЗаполнитьДатыСозданияВСтруктуреПредприятия();
	...
	Блокировка.Добавить("РегистрСведений.МестоПозицииШтатногоРасписанияВСтруктуреПредприятия");
...
Процедура СтруктураПредприятияПередЗаписью(ПодразделениеПредприятияОбъект, Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьШтатноеРасписание") 
		И ОрганизационнаяСтруктура.ПодсистемаСостоянияСтруктурыПредприятияСуществует() Тогда
		
		Если ПодразделениеПредприятияОбъект.ПометкаУдаления
			И ПодразделениеПредприятияОбъект.Сформировано
			И (Не ПодразделениеПредприятияОбъект.Расформировано) Тогда
			
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Нельзя пометить на удаление действующее подразделение ""%1""'"), ПодразделениеПредприятияОбъект);
...
Процедура ОбновитьЭлементСтруктурыПредприятия(Источник, Отказ) Экспорт
	...
	Попытка
		...
		Для Каждого СтрокаИсточника Из ТаблицаИзмененных Цикл
			...
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ПодразделениеОбъект, СтрокаИсточника, , СписокИсключаемыхРеквизитов(СтрокаИсточника.Источник));
...
Процедура ОбновитьПодчиненностьСтруктурныхЕдиниц(Источник, Отказ) Экспорт
	...
	Если Не (ЭтоНоваяСтруктурнаяЕдиница Или СменаРодителя) Тогда
		Возврат;
	КонецЕсли;
	
	ПодразделенияКОбновлению = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Источник.Ссылка);
	РегистрыСведений.ПодчиненностьСтруктурныхЕдиниц.ОбновитьПодчиненностьСтруктурныхЕдиниц(ПодразделенияКОбновлению);
	
	Если СменаРодителя Тогда
		Если ЗначениеЗаполнено(Источник.Родитель) Тогда
			ПодразделенияКОбновлению.Добавить(Источник.Родитель);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПредыдущийРодитель) Тогда
			РегистрыСведений.ПодчиненностьСтруктурныхЕдиниц.ОбновитьПодчиненностьСтруктурныхЕдиницПриСменеРодителя(
				Источник.Ссылка, ПредыдущийРодитель, Источник.Родитель);
			ПодразделенияКОбновлению.Добавить(ПредыдущийРодитель);
		КонецЕсли;
	КонецЕсли;
	
	ОрганизационнаяСтруктура.ОбновитьСведенияПодразделений(ПодразделенияКОбновлению);
		...
		Если ЗначениеЗаполнено(Выборка.Источник) 
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				Иначе
					ПодразделениеОбъект = Справочники.СтруктураПредприятия.СоздатьЭлемент();
				КонецЕсли;
				ЗаполнитьЗначенияСвойств(ПодразделениеОбъект, СтрокаИсточника, , СписокИсключаемыхРеквизитов(Выборка.Источник));
				ПодразделениеОбъект.КодИсточника = СтрокаИсточника.Код;
...
Функция СписокИсключаемыхРеквизитов(Источник)
	
	СписокИсключаемыхРеквизитов = Новый Массив;
	СписокИсключаемыхРеквизитов.Добавить("Код");

	ОрганизационнаяСтруктура.ДополнитьСписокИсключаемыхРеквизитов(СписокИсключаемыхРеквизитов, Источник.Метаданные().Реквизиты);

	Возврат СтрСоединить(СписокИсключаемыхРеквизитов, ",");
...
Функция ИзмененныеЭлементыИсточника(Источник)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#СтруктураИсточник", МетаданныеИсточника.ПолноеИмя());
	...
	Если Не МетаданныеИсточника.Иерархический Тогда
	...
	ОрганизационнаяСтруктура.УточнитьЗапросИзмененныхЭлементовИсточника(Запрос, МетаданныеИсточника.Реквизиты);
	
	РезультатЗапроса = Запрос.Выполнить();
	ИзмененныеЭлементы = РезультатЗапроса.Выгрузить();
	
	Если ТипЗнч(Источник) = Тип("СправочникСсылка.Организации") Тогда
		ОбработатьИзмененныеЭлементыОрганизаций(ИзмененныеЭлементы);
	КонецЕсли;
	
	Возврат ИзмененныеЭлементы;
	
КонецФункции

Процедура ОбработатьИзмененныеЭлементыОрганизаций(ИзмененныеЭлементы)
	
	Если ИзмененныеЭлементы.Колонки.Найти("ДатаСозданияЭлементаСтруктуры") = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ОбновляемыеЭлементыСтруктурыПредприятия = Новый Массив;
	Для Каждого СтрокаТаблицы Из ИзмененныеЭлементы Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.ДатаСозданияЭлементаСтруктуры) Тогда
			Продолжить;
		КонецЕсли;
		ОбновляемыеЭлементыСтруктурыПредприятия.Добавить(СтрокаТаблицы.Ссылка);
	КонецЦикла;
	
	ОрганизационнаяСтруктура.ОбновитьСведенияПодразделений(ОбновляемыеЭлементыСтруктурыПредприятия, Истина, Ложь);
