...
//                                        иначе см. ОписаниеОшибки.
//      * АлгоритмыШифрования - ФиксированныйМассив - список описаний алгоритмов
//        ** Идентификатор - Число  - идентификатор алгоритма.
//        ** Имя           - Строка - текстовове представление алгоритма, используется в процедуре "ЗашифроватьPKCS7".
//        ** ДлинаКлюча    - Число  - длина ключа в битах.
//      * МенеджерКриптографии - AddIn  - объект используемый для работы с файлами. Работать напрямую с объектом
//                                        запрещено. При ИспользоватьКеширование равном Истина может не возвращаться.
//      * Алгоритм             - Строка - алгоритм криптопровайдера созданного менеджера криптографии, при заданном
//                                        параметре МенеджерКриптографии возвращается параметр АлгоритмИлиСертификат.
//                                        При ИспользоватьКеширование равном Истина может не возвращаться.
//      * ОписаниеОшибки       - Строка - описание ошибки выполнения.
//
//  ВыводитьСообщения    - Булево - устанавливает признак необходимости выводить сообщения об ошибках.
//
...
//    * Отпечаток        - Строка - отпечаток сертификата. Используется для поиска сертификата если не указаны СерийныйНомер и Поставщик.
//    * Сертификат       - Строка - содержимое сертификата в Base64, если поле присутствует, используется при определении алгоритма.
//  ИспользоватьКеширование - Булево - возвращать данные из кеша приложения, если заполнен.
//  ПараметрыОпределенияАлгоритма - Структура - параметры для определения алгоритма, см. СоздатьМенеджерКриптографии,
//                                              передать Новый Структура для приоритета инициализации КриптоПро CSP.
//
Процедура ПолучитьАлгоритмыШифрования(
		...
		ИспользоватьКеширование = Истина,
		ПараметрыОпределенияАлгоритма = Неопределено) Экспорт
	
	КриптографияЭДКОСлужебныйКлиент.ПолучитьАлгоритмыШифрования(
		ОповещениеОЗавершении,
		ВыводитьСоообщения,
		МенеджерКриптографии,
		АлгоритмИлиСертификат,
...
Процедура ПолучитьСертификатИзКонтейнера(
	...
	КриптографияЭДКОСлужебныйКлиент.ПолучитьСертификатИзКонтейнера(
		ОповещениеОЗавершении,
		ИмяКонтейнераКлючей,
...
//    * Сертификат    - Строка - содержимое сертификата в Base64, если поле присутствует, используется при определении
//                               алгоритма.
//
//  ИмяФайлаИлиРасширениеИлиАдрес - Неопределено - если в параметре ИсходныйФайл передан адрес во временном хранилище,
//                                                 то результат будет помещен во временное хранилище по адресу исходного
//                                                 файла.
//                                               - если передано полное имя файла на клиенте,
//                                                 то результат будет помещен во вновь созданный файл на клиенте.
//                                - Строка - имя файла, в который необходимо сохранить результат.
...
//
//  Алгоритм               - Неопределено - использовать автоопределение алгоритма внешней компонентой в момент
//                                          операции.
//                           Строка       - алгоритм, возвращаемый
//                                          КриптографияЭДКОСлужебныйВызовСервера.ПолучитьСвойстваКриптосообщения(),
//                                          при значении "*" определять алгоритм и криптопровайдер по первому
//                                          сертификату.
//  ПараметрыОперации - Структура:
//    * АлгоритмШифрования           - Строка - необязательный, по умолчанию "GOST 28147-89" - ГОСТ 28147-89,
//                                              для ГОСТ 34.12-2018 и ГОСТ 34.13-2018 (КриптоПро CSP 5.0.12000):
//                                              "GR 34.12-15 K" - "Кузнечик", 128-битный размер блока,
//                                              "GR 34.12-15 M" - "Магма", 64-битный размер блока, вариант
//                                              ГОСТ 28147-89, при первом вызове проверяется, если алгоритм,
//                                              не поддерживается криптопровайдером, передается статистика об этом,
//                                              при вызовах в таком случае используется "GOST 28147-89", за исключением
//                                              передачи "АлгоритмШифрованияОбязателен" равным Истина.
//    * АлгоритмШифрованияОбязателен - Булево - по умолчанию Ложь.
//    * СобиратьСведенияОШифровании  - Булево - по умолчанию Ложь.
//
...
//  ПараметрыОперации - Структура:
//    * АлгоритмШифрованияОбязателен - Булево - по умолчанию Ложь, при значении Истина в случае ошибок расшифровки
//                                              ожидается наличие поддержки ГОСТ 34.12-2018 и ГОСТ 34.13-2018
...
Процедура ПолучитьУстройстваUSB(ОповещениеОЗавершении) Экспорт
	...
	КриптографияЭДКОСлужебныйКлиент.ПолучитьУстройстваUSB(ОповещениеОЗавершении);
...
Процедура УстановитьОбработчикСобытия(ОповещениеОЗавершении, Фильтр = "") Экспорт
	...
	КриптографияЭДКОСлужебныйКлиент.УстановитьОбработчикСобытия(ОповещениеОЗавершении, Фильтр);
...
// Получает путь по-умолчанию к КриптоПро на MacOS и Linux с проверкой существования пути
//
// Параметры:
//  ОбработкаЗавершения  - ОписаниеОповещения - описание процедуры, принимающей результат.
//    Результат - Структура:
...
Процедура ПолучитьПутьККриптоПроНаMacOSиLinux(ОбработкаЗавершения) Экспорт
	...
	КриптографияЭДКОСлужебныйКлиент.ПолучитьПутьККриптоПроНаMacOSиLinux(ОбработкаЗавершения);
	
КонецПроцедуры

// Возвращает список программ с названием по заданному фильтру. Если фильтр не задан, то возвращаются все программы
// Параметры:
//  ОповещениеОЗавершении - ОписаниеОповещения - описание процедуры, принимающей результат.
//    Результат - Структура:
//      * Выполнено         - Булево - если Истина, то процедура успешно выполнена и получен результат, иначе см. ОписаниеОшибки.
//      * Программы         - Массив структур с полям name и version
//      * ОписаниеОшибки    - Строка - описание ошибки выполнения.
//  ОповещениеОЗавершении    - ОписаниеОповещения - описание процедуры, принимающей результат.
//  ФильтрПрограмм			 - Строка -   
//
Процедура ПолучитьУстановленныеПрограммы(ОповещениеОЗавершении, ФильтрПрограмм = "") Экспорт

	КриптографияЭДКОСлужебныйКлиент.ПолучитьУстановленныеПрограммы(ОповещениеОЗавершении, ФильтрПрограмм);
