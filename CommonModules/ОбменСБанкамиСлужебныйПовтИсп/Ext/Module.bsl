...
Функция АктуальныеВидыЭД() Экспорт
	...
	// Депозиты
	Если Не МассивЭД.Найти(Перечисления.ВидыЭДОбменСБанками.ЗаявкаНаОткрытиеДепозита) = Неопределено Тогда
		МассивЭД.Добавить(Перечисления.ВидыЭДОбменСБанками.ЗапросСостоянияДепозита);
	КонецЕсли;
...
Функция СписокБанков() Экспорт
	...
	Иначе
		...
		Если ДанныеВнешнихФайлов = Неопределено ИЛИ НЕ ДанныеВнешнихФайлов.Свойство("СписокБанковРасширенный")
			ИЛИ (ДанныеВнешнихФайлов.СписокБанковРасширенный = Неопределено) Тогда // список еще не подкачивался из интернета
		...
		Иначе
			...
			Попытка
				...
				ДанныеВнешнихФайлов.СписокБанковРасширенный.Записать(ВремФайл);
			...
			Исключение
				...
				ВидОперации = НСтр("ru = 'Чтение списка банков расширенного сервиса 1С:ДиректБанк из скачанного файла.'");
...
//
// Возвращаемое значение:
//  Булево - Если Ложь, то открытие во внешнем браузере (по умолчанию);
