...
Процедура ОбработкаПроверкиЗаполнения(Ведомость, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Для Каждого СтрокаСостава Из Ведомость.Состав Цикл
		...
		ВыгружатьИНН = ОбменСБанкамиПоЗарплатнымПроектам.ЗарплатныйПроектПоддерживаетВыгрузкуИНН(Ведомость.ЗарплатныйПроект);
		...
		ИначеЕсли (СтрокаСостава.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР) 
			И Не ЗначениеЗаполнено(СтрокаСостава.НомерЛицевогоСчетаЦР) Тогда
				СообщитьПользователюНезаполненоЗначениеСпособаЗачисления(
				СтрокаСостава,
				"НомерЛицевогоСчетаЦР",
				НСтр("ru = 'лицевой счет ЦР'"),
				Ведомость,
				Отказ);
		КонецЕсли;
