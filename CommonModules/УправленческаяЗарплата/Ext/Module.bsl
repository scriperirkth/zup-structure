...
Процедура ДополнитьТекстЗапросаНачисленияПредварительноОтборомПоУправленческойОрганизации(Запрос) Экспорт
	...
	Для Каждого Оператор Из ПакетЗапроса.Операторы Цикл
		Если Оператор.Источники.НайтиПоИмени("РегистрНакопления.НачисленияУдержанияПоСотрудникам") <> Неопределено Тогда
			Оператор.Отбор.Добавить("НачисленияУдержанияПоСотрудникам.Организация <> &УправленческаяОрганизация");
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьТекстЗапросаДляТ61ОтборомПоУправленческойОрганизации(Запрос) Экспорт
	...
	Для Каждого Оператор Из ПакетЗапроса.Операторы Цикл
		...
		ИсточникЗапроса = Оператор.Источники.НайтиПоИмени("РегистрНакопления.ВзаиморасчетыССотрудниками.ОстаткиИОбороты");
		Если ИсточникЗапроса <> Неопределено Тогда
				
			МассивОтборов = Новый Массив;
			МассивОтборов.Добавить(ИсточникЗапроса.Источник.Параметры[4].Выражение);
			МассивОтборов.Добавить("Организация <> &УправленческаяОрганизация");
				
			ИсточникЗапроса.Источник.Параметры[4].Выражение = Новый ВыражениеСхемыЗапроса(СтрСоединить(МассивОтборов, " И "));
		КонецЕсли;
			
	КонецЦикла;
	
	ПакетЗапроса = СхемаЗапроса.ПакетЗапросов[2];
	Для Каждого Оператор Из ПакетЗапроса.Операторы Цикл
		Оператор.Отбор.Добавить("ВзаиморасчетыССотрудниками.Организация <> &УправленческаяОрганизация");
	КонецЦикла;
...
Процедура ЗаполнитьОбъектУправленческимиНачислениямиПозицийШтатногоРасписания(Объект, ДанныеПозиции, ИдентификаторСтрокиПозиции) Экспорт
	
	Если Не ИспользоватьУправленческуюЗарплату() Тогда
		Возврат;
	КонецЕсли;
	
	УправленческиеНачисления = ДанныеПозиции.УправленческиеНачисления;
	
	Если УправленческиеНачисления <> Неопределено Тогда 
		
		Для Каждого УправленческоеНачисление Из УправленческиеНачисления Цикл
			
			СтрокаНачисления = Объект.УправленческиеНачисления.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаНачисления, УправленческоеНачисление);
			СтрокаНачисления.ИдентификаторСтрокиПозиции = ИдентификаторСтрокиПозиции;
			
			Если УправленческоеНачисление.Показатели <> Неопределено Тогда
				Для Каждого ПоказательНачисления Из УправленческоеНачисление.Показатели Цикл
					СтрокаПоказателяНачисления = Объект.УправленческиеПоказатели.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаПоказателяНачисления, ПоказательНачисления);
					СтрокаПоказателяНачисления.ИдентификаторСтрокиПозиции = ИдентификаторСтрокиПозиции;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
