...
Процедура ЗаполнитьРабочиеМестаСотрудника(Форма, ТаблицаРабочихМест, СтруктураРеквизитов, ДополнятьТекущейПозицией = Ложь) Экспорт
	...
	ЗаполнитьСписокВыбораПозицииПодразделения(Форма, ТаблицаРабочихМест);
...
Процедура УстановитьПодразделениеСотрудника(Форма, ТаблицаРабочихМест, Подразделение) Экспорт

	Если ТаблицаРабочихМест.Количество() <> 0 Тогда
		Подразделение = ТаблицаРабочихМест[0].Подразделение;
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПоказателейСотрудникаВФорме(Форма, СтруктураРеквизитов, ТолькоПлановые = Ложь) Экспорт
	...
	ПоказателиСотрудника = КлючевыеПоказателиЭффективности.ПоказателиСотрудника(СтруктураРеквизитов.Сотрудник,
		СтруктураРеквизитов.Позиция,СтруктураРеквизитов.Период, СтруктураРеквизитов.Горизонт);
...
Процедура ЗаполнитьТаблицуРабочихМестСотрудника(ТаблицаРабочихМест, СтруктураРеквизитов)
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
		"ВТРабочиеМестаСотрудников", "Позиция");
	ОписаниеФильтра.СоответствиеИзмеренийРегистраИзмерениямФильтра.Вставить("Позиция", "ДолжностьПоШтатномуРасписанию");
	...
	Запрос.Текст = 
	    ...
	    |		ЛЕВОЕ СОЕДИНЕНИЕ ВТМестоПозицииШтатногоРасписанияВСтруктуреПредприятия КАК
	    |			МестоПозицииШтатногоРасписанияВСтруктуреПредприятия
	    |		ПО ПериодыРаботыСотрудников.ДолжностьПоШтатномуРасписанию = МестоПозицииШтатногоРасписанияВСтруктуреПредприятия.Позиция
	    |		И ПериодыРаботыСотрудников.Период = МестоПозицииШтатногоРасписанияВСтруктуреПредприятия.Период
	    |
	    |УПОРЯДОЧИТЬ ПО
	    |	ДатаНачала";
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура ЗаполнитьСписокВыбораПозицииПодразделения(Форма, ТаблицаРабочихМест)

	Форма.Элементы.Позиция.СписокВыбора.Очистить();
	Форма.Элементы.Подразделение.СписокВыбора.Очистить();
	ТаблицаПозиций = ТаблицаРабочихМест.Выгрузить(, "Позиция,Подразделение");
	ТаблицаПозиций.Свернуть("Позиция, Подразделение");
	
	Для каждого РабочееМесто Из ТаблицаПозиций Цикл
		Форма.Элементы.Позиция.СписокВыбора.Добавить(РабочееМесто.Позиция);
		Форма.Элементы.Подразделение.СписокВыбора.Добавить(РабочееМесто.Подразделение);
	КонецЦикла;
