...
Функция ПолучитьАдресДляПолученияНовостей(
	...
	ЭтотПродукт     = ПолучитьЗначениеПредопределеннойКатегории("ЭтотПродукт"); // Имя продукта и полная версия продукта
	ВерсияПлатформы = ПолучитьЗначениеПредопределеннойКатегории("ВерсияПлатформы"); // Только полная версия платформы
...
Функция ОписаниеСостоянияСистемы(РезультатКакМассивСтрок = Ложь) Экспорт
	...
	Результат = Новый Массив;
		...
		Результат.Добавить("Конфигурация(Метаданные): "
			+ ПолучитьЗначениеПредопределеннойКатегории("МетаданныеИмя") + ", "
			+ ПолучитьЗначениеПредопределеннойКатегории("МетаданныеВерсия"));
		Результат.Добавить("Конфигурация(Переопределение): "
			+ ПолучитьЗначениеПредопределеннойКатегории("ИмяКонфигурации") + ", "
			+ ПолучитьЗначениеПредопределеннойКатегории("ВерсияПродукта"));
		Результат.Добавить("Платформа: " + ПолучитьЗначениеПредопределеннойКатегории("ВерсияПлатформы"));
...
Функция ПолучитьНастройкиПоказаСкрытияБлоковТекста() Экспорт
	...
	РежимРазделенияДанных = ПолучитьЗначениеПредопределеннойКатегории("DataSeparation");
	СпособХраненияИБ      = ПолучитьЗначениеПредопределеннойКатегории("InfoBaseStorageType");
	РольПолныеПрава       = ЕстьРольПолныеПрава();
...
#Область Показать_Английский
	...
	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Show block if UserRole=Admin. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Show block if UserRole=Admin. End -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Ложь);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Show block if UserRole=User. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Show block if UserRole=User. End -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Истина);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Show block if DataSeparation=Separated. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Show block if DataSeparation=Separated. End -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных <> "Separated");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Show block if DataSeparation=Disabled. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Show block if DataSeparation=Disabled. End -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных <> "Disabled");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Show block if InfoBaseStorageType=File. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Show block if InfoBaseStorageType=File. End -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ <> "File");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Show block if InfoBaseStorageType=ClientServer. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Show block if InfoBaseStorageType=ClientServer. End -->", // АПК:1297 Это идентификатор.
...
#Область Скрыть_Английский
	...
	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Hide block if UserRole=Admin. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Hide block if UserRole=Admin. End -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Истина);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Hide block if UserRole=User. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Hide block if UserRole=User. End -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Ложь);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Hide block if DataSeparation=Separated. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Hide block if DataSeparation=Separated. End -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных = "Separated");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Hide block if DataSeparation=Disabled. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Hide block if DataSeparation=Disabled. End -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных = "Disabled");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Hide block if InfoBaseStorageType=File. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Hide block if InfoBaseStorageType=File. End -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ = "File");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Hide block if InfoBaseStorageType=ClientServer. Begin -->", // АПК:1297 Это идентификатор.
		"<!-- Hide block if InfoBaseStorageType=ClientServer. End -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ = "ClientServer");
	Результат.Добавить(СтруктураБлока);
...
#Область Показать_Русский
	...
	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Показать блок, если Роль пользователя=Администратор. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Показать блок, если Роль пользователя=Администратор. Конец -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Ложь);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Показать блок, если Роль пользователя=Пользователь. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Показать блок, если Роль пользователя=Пользователь. Конец -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Истина);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Показать блок, если Режим разделения данных=Модель сервиса. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Показать блок, если Режим разделения данных=Модель сервиса. Конец -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных <> "Separated");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Показать блок, если Режим разделения данных=Отключено. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Показать блок, если Режим разделения данных=Отключено. Конец -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных <> "Disabled");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Показать блок, если Способ хранения ИБ=Файловая. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Показать блок, если Способ хранения ИБ=Файловая. Конец -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ <> "File");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Показать блок, если Способ хранения ИБ=Клиент-серверная. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Показать блок, если Способ хранения ИБ=Клиент-серверная. Конец -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ <> "ClientServer");
	Результат.Добавить(СтруктураБлока);
...
#Область Скрыть_Русский
	...
	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Скрыть блок, если Роль пользователя=Администратор. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Скрыть блок, если Роль пользователя=Администратор. Конец -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Истина);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Скрыть блок, если Роль пользователя=Пользователь. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Скрыть блок, если Роль пользователя=Пользователь. Конец -->", // АПК:1297 Это идентификатор.
		РольПолныеПрава = Ложь);
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Скрыть блок, если Режим разделения данных=Модель сервиса. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Скрыть блок, если Режим разделения данных=Модель сервиса. Конец -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных = "Separated");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Скрыть блок, если Режим разделения данных=Отключено. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Скрыть блок, если Режим разделения данных=Отключено. Конец -->", // АПК:1297 Это идентификатор.
		РежимРазделенияДанных = "Disabled");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Скрыть блок, если Способ хранения ИБ=Файловая. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Скрыть блок, если Способ хранения ИБ=Файловая. Конец -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ = "File");
	Результат.Добавить(СтруктураБлока);

	СтруктураБлока = Новый Структура("ТэгНачалаБлока, ТэгКонцаБлока, Скрыть",
		"<!-- Скрыть блок, если Способ хранения ИБ=Клиент-серверная. Начало -->", // АПК:1297 Это идентификатор.
		"<!-- Скрыть блок, если Способ хранения ИБ=Клиент-серверная. Конец -->", // АПК:1297 Это идентификатор.
		СпособХраненияИБ = "ClientServer");
	Результат.Добавить(СтруктураБлока);
