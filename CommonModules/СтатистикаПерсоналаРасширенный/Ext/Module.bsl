...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОтключитьНекорректныеХарактеристики");
	Методы.Вставить("УстановитьНастройкуРасчетаЧисленности");
	Методы.Вставить("ОчиститьНекорректныеХарактеристики");
	Методы.Вставить("УстановитьВидФормыМониторингаВСтрокахОтчетности");
	Методы.Вставить("УстановитьНастройкуСтатистикиПерсоналаПоОрганизациям");
	Методы.Вставить("ЗаполнитьПодключаемыеХарактеристики");
	Методы.Вставить("ОчиститьПодключаемыеХарактеристики");
КонецПроцедуры
...
Процедура ПриПолученииСведенийОПоказателяхОтчета(ПоказателиОтчета, ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетСтатистикаФорма1РабочаяСила" И ИДРедакцииОтчета >= "ФормаОтчета2025Кв1" Тогда
		
		ДобавитьПоказателиФормы1РабочаяСила_2025(ПоказателиОтчета);
...
Процедура ПриЗаполненииРегламентированногоОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетСтатистикаФорма1РабочаяСила" И ИДРедакцииОтчета >= "ФормаОтчета2025Кв1" Тогда
		...
		ЗаполнитьПоказателиФормы1РабочаяСила_2025(ПараметрыОтчета, Контейнер);
...
Процедура ПриФормированииРасшифровкиРегламентированногоОтчета(Форма, ТабличныйДокумент, Отказ) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетСтатистикаФорма1РабочаяСила" Тогда
		ПараметрыОтчета.ВидОтчета = "РегламентированныйОтчетСтатистикаФорма1РабочаяСила_2025";
		Показатель = РазобратьИмяПоказателяРегламентированногоОтчета(ИмяПоказателя, "П?РР?????ССКК");
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыОтчета, Показатель, Истина); 

		Если Не ЗначениеЗаполнено(ПараметрыОтчета.НомерРаздела)
			Или Не ЗначениеЗаполнено(ПараметрыОтчета.НомерСтроки)
			Или Не ЗначениеЗаполнено(ПараметрыОтчета.НомерКолонки)
			Или ПараметрыОтчета.НомерСтроки = "03" 
			Или ПараметрыОтчета.НомерСтроки = "07" 
			Или ПараметрыОтчета.НомерСтроки = "08" 
			Или ПараметрыОтчета.НомерСтроки = "14" 
			Или ПараметрыОтчета.НомерСтроки = "16"
			Или ПараметрыОтчета.НомерСтроки = "17"
			Или ПараметрыОтчета.НомерСтроки = "19"
			Или ПараметрыОтчета.НомерСтроки = "21"
			Или ПараметрыОтчета.НомерСтроки = "22"
			Или ПараметрыОтчета.НомерСтроки = "23"
			Или ПараметрыОтчета.НомерСтроки = "23"
			Или ПараметрыОтчета.НомерСтроки = "25" 
			Или ПараметрыОтчета.НомерСтроки = "26" 
			Или ПараметрыОтчета.НомерСтроки = "27"
			Или ПараметрыОтчета.НомерСтроки = "28"
			Или ПараметрыОтчета.НомерСтроки = "29"
			Или ПараметрыОтчета.НомерСтроки = "30" 
			Или ПараметрыОтчета.НомерСтроки = "35" 
			Или ПараметрыОтчета.НомерСтроки = "36" 
			Или ПараметрыОтчета.НомерСтроки = "37" 
			Или ПараметрыОтчета.НомерСтроки = "38"
			Или ПараметрыОтчета.НомерСтроки = "40"
			Или ПараметрыОтчета.НомерСтроки = "43"
			Или ПараметрыОтчета.НомерСтроки = "44" Тогда
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		ПараметрыОтчета.Вставить("ДатаНачалаПериодаОтчета", ПараметрыОтчета.мДатаНачалаПериодаОтчета);
		ПараметрыОтчета.Вставить("ДатаКонцаПериодаОтчета", ПараметрыОтчета.мДатаКонцаПериодаОтчета);
		ПараметрыОтчета.Вставить("ДругиеКатегорииСтатистическогоНаблюдения", Истина);
...
Процедура СформироватьОтчетРасшифровку(Параметры, ДокументРезультат)
	...
	ИначеЕсли ВидСКД = "СхемаКомпоновкиДанныхКадры" Тогда
		...
		Если Параметры.Свойство("ДругиеКатегорииСтатистическогоНаблюдения") Тогда
			Параметры.Удалить("ДругиеКатегорииСтатистическогоНаблюдения");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОпределитьВидСКД(ПараметрыОтчета, Настройки = Неопределено, ВидСКД = "", ДополнитьКомпоновщикНастроек = Истина, 
	...
	ИначеЕсли ВидОтчета = "РегламентированныйОтчетСтатистикаФорма1Туст" Тогда	
		...
		Если НомерСтроки = "001" Тогда
			...
			Если НомерКолонки = "03" Тогда
				...
				ВидСКД = "СхемаКомпоновкиДанныхКадры";
			ИначеЕсли НомерКолонки = "04" Тогда
				ПолеРесурса = "ЧисленностьНаКонецПериодаЖенщинСписочногоСостава";
				ВидСКД = "СхемаКомпоновкиДанныхКадры";
			ИначеЕсли НомерКолонки = "05" Тогда
				ПолеРесурса = "ЧисленностьНаКонецПериодаСписочногоСоставаМладше18Лет";
				ВидСКД = "СхемаКомпоновкиДанныхКадры";
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ВидОтчета = "РегламентированныйОтчетСтатистикаФорма1РабочаяСила_2025" Тогда
		
		Если СтрНайти("03,07,08,14,16,17,19,21,22,23,25,26,27,28,29,30,38,43", НомерСтроки) > 0 Тогда
			Возврат
		КонецЕсли;
		
		// Отключение группировки по виду расчета для расшифровки численности.
		Если НомерСтроки = "01" Тогда
			Если ДополнитьКомпоновщикНастроек Тогда
				Настройки.Структура[0].Колонки[0].Использование = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		Если НомерСтроки = "01" Тогда
			ПолеРесурса = "СредняяЧисленностьЗаПериодСписочногоСостава";
			ВидСКД = "СхемаКомпоновкиДанныхКадры";
		ИначеЕсли НомерСтроки = "02" Тогда
			ПолеРесурса = "ОтработаноЧасовРаботникамиСписочногоСостава";
			ВидСКД = "СхемаКомпоновкиДанныхЗарплата";
		ИначеЕсли НомерСтроки = "04" Тогда
			ПолеРесурса = "ОтработаноЧасовРаботникамСписочногоСоставаВОтпуске";
			ВидСКД = "СхемаКомпоновкиДанныхЗарплата";
		ИначеЕсли НомерСтроки = "05" Тогда
			ПолеРесурса = "ОплаченоЧасовРаботникамСписочногоСоставаВПростое";
			ВидСКД = "СхемаКомпоновкиДанныхЗарплата";
		ИначеЕсли НомерСтроки = "06" Тогда
			ПолеРесурса = "ОплаченоЧасовРаботникамСписочногоСоставаПрочие";
			ВидСКД = "СхемаКомпоновкиДанныхЗарплата";
		Иначе
			ПолеРесурса = "НачисленоРаботникамСписочногоСостава";
			ВидСКД = "СхемаКомпоновкиДанныхЗарплата";
...
Процедура ЗаполнитьОтборДляОтчетаРасшифровки(Отбор, ПараметрыОтчета, ИмяСтроки)
	...
	ИначеЕсли ВидОтчета = "РегламентированныйОтчетСтатистикаФорма1РабочаяСила_2025" Тогда
		Если ПараметрыОтчета.НомерСтроки = "09" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ЗаработнаяПлата);
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "10" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДоплатыНадбавки); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "11" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ПремииВознаграждения); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "12" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ВыплатыПоРайонномуРегулированию); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "13" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеВидыОплатыТруда); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "15" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаЗаНеотработанноеВремя); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "18" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ЕдиновременныеПоощрительныеВыплаты); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "20" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаПитанияПроживания); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "24" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.БезвозмездныеСубсидии); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "31" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ВыходноеПособиеНачисленияНаПериодТрудоустройства); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "32" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.СуммыПоВременнойНетрудоспособности); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "33" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.МатериальнаяПомощь); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "34" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеРасходыНаСоциальнуюЗащиту); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "39" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.РасходыНаКомандировки);
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "41" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаПроезда); 
		ИначеЕсли ПараметрыОтчета.НомерСтроки = "42" Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор,
				"ВидРасчета.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила", 
				ВидСравненияКомпоновкиДанных.Равно, 
				Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеРасходы);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Процедура СоздатьВТТерриторииДоговоров(МенеджерВременныхТаблиц, ПараметрыОтчета)
	...
	Запрос = Новый Запрос(
		...
		|ПОМЕСТИТЬ ВТТерриторииДоговоров
		|ИЗ
		...
		|		ПО (ПараметрыПериодовСтажаПФР.Регистратор = ПериодыДействияДоговоровГражданскоПравовогоХарактера.Регистратор)
		|ГДЕ
	...
	Запрос.УстановитьПараметр("ДатаНачалаПериодаОтчета", ПараметрыОтчета.ДатаНачалаПериодаОтчета);
	Запрос.УстановитьПараметр("ДатаКонцаПериодаОтчета", ПараметрыОтчета.ДатаКонцаПериодаОтчета);
...
Функция ЗапросВТИсключаемостьСотрудниковПоТерриториям(ПоВключаемым)
	...
	Запрос = Новый Запрос(
		...
		|	ТерриторииДоговоров.Территория,
		|	ЛОЖЬ
		|ИЗ
		|	ВТДоговорники КАК Договорники
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТТерриторииДоговоров КАК ТерриторииДоговоров
		|		ПО (ТерриторииДоговоров.Договор = Договорники.Договор)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция КритерииОтбораПоРегистрацииРеспондента(ПараметрыОтчета)
	...
	Если СвойстваРеспондента.ЭтоОрганизация Тогда
		...
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		СоздатьВТТерриторииДоговоров(Запрос.МенеджерВременныхТаблиц, ПараметрыОтчета);
		...
		ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьЗапросы(Запрос,
			ЗарплатаКадрыПериодическиеРегистры.ЗапросВТПериодыИмяРегистра("КадроваяИсторияСотрудников", Истина, ОписаниеФильтра, ПараметрыПостроения));
...
Функция ЗаполнитьКомпоновщикНастроек(КлючВарианта, ПараметрыОтчета, СхемаКД) Экспорт
	...
	Если ПараметрыОтчета.Свойство("ДругиеКатегорииСтатистическогоНаблюдения") Тогда
		ДругиеКатегорииСтатистическогоНаблюдения = ЗарплатаКадрыОтчеты.НайтиПараметр(КомпоновщикНастроек, "ДругиеКатегорииСтатистическогоНаблюдения");
		ДругиеКатегорииСтатистическогоНаблюдения.Значение = ПараметрыОтчета.ДругиеКатегорииСтатистическогоНаблюдения;
	КонецЕсли;
...
Процедура ЗаполнитьПоказателиФормы1Туст_2020(ПараметрыОтчета, Контейнер)
	...
	Если СтрокаОрганизации <> Неопределено Тогда
		Для Каждого СтрокаЧисленности Из СтрокаОрганизации.Строки Цикл
			П000010100103 = П000010100103 + СтрокаЧисленности.ЧисленностьНаКонецПериодаСписочногоСостава;
			П000010100104 = П000010100104 + СтрокаЧисленности.ЧисленностьНаКонецПериодаЖенщинСписочногоСостава;
			П000010100105 = П000010100105 + СтрокаЧисленности.ЧисленностьНаКонецПериодаСписочногоСоставаМладше18Лет;
...
//  ПоказателиОтчета - структура
//
Процедура ДобавитьПоказателиФормы1РабочаяСила_2025(ПоказателиОтчета) Экспорт
	...
	Для ИндексТекущейСтроки = 1 По 7 Цикл
		Если СтрНайти("03,07", Формат(ИндексТекущейСтроки, "ЧЦ=2; ЧВН=")) > 0 Тогда
			Продолжить;
		КонецЕсли;
		ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П00100100" + Формат(ИндексТекущейСтроки, "ЧЦ=2; ЧВН=") + "04", Истина, Истина);
	КонецЦикла; 
	Для ИндексТекущейСтроки = 8 По 43 Цикл
		Если СтрНайти("08,14,16,17,19,21,22,23,25,26,27,28,29,30,35,36,37,38,40,43", Формат(ИндексТекущейСтроки, "ЧЦ=2; ЧВН=")) > 0 Тогда
			Продолжить;
		КонецЕсли;
		ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П00200100" + Формат(ИндексТекущейСтроки, "ЧЦ=2; ЧВН=") + "03", Истина, Истина);
	КонецЦикла;
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П002001001403", Истина, Ложь);
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П002001001603", Истина, Ложь);
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П002001001703", Истина, Ложь);
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П002001001903", Истина, Ложь);
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П002001002103", Истина, Ложь);
	ЗарплатаКадры.ДобавитьПоказательРегламентированнойОтчетности(ПоказателиОтчета, "П002001002203", Истина, Ложь);

КонецПроцедуры

Процедура ЗаполнитьПоказателиФормы1РабочаяСила_2025(ПараметрыОтчета, Контейнер) Экспорт
	
	// Очистим ячейки
	Попытка
		Для ИндексТекущейСтроки = 1 По 7 Цикл
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П00100100" + Формат(ИндексТекущейСтроки, "ЧЦ=2; ЧВН=") + "04"] = 0;
		КонецЦикла;
		Для ИндексТекущейСтроки = 8 По 43 Цикл
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П00200100" + Формат(ИндексТекущейСтроки, "ЧЦ=2; ЧВН=") + "03"] = 0;
		КонецЦикла;
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Порядок заполнения отчета устарел, рекомендуем обновить конфигурацию.%1'"), Символы.ПС);
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Отчет не заполнен.'");
	...
	ПараметрыОтчетаДляЗаполнения = СкопироватьКоллекцию(ПараметрыОтчета);
	ПараметрыОтчетаДляЗаполнения.Вставить("ДатаКонцаПериодаОтчета", ПараметрыОтчета.мДатаКонцаПериодаОтчета);
	ПараметрыОтчетаДляЗаполнения.Вставить("ДатаНачалаПериодаОтчета", ПараметрыОтчета.мДатаНачалаПериодаОтчета);
	...
	СтрокаОрганизации = ДанныеОтчета.Строки.Найти(ПараметрыОтчета.Организация, "Организация");
	Если СтрокаОрганизации <> Неопределено Тогда
		Контейнер.ПолеТабличногоДокументаФормаОтчета["П001001000104"] = СтрокаОрганизации.СредняяЧисленностьЗаПериодСписочногоСостава;
	КонецЕсли;
	// Получение зарплатных данных отчета
	ДанныеОтчета = Новый ДеревоЗначений;
	ПараметрыОтчетаДляЗаполнения.Вставить("ДругиеКатегорииСтатистическогоНаблюдения", Истина);
	Если ПараметрыОтчетаДляЗаполнения.Свойство("ЧисленностьПоОтработанномуВремени") Тогда
		ПараметрыОтчетаДляЗаполнения.Удалить("ЧисленностьПоОтработанномуВремени");
	КонецЕсли; 
	ДанныеДляЗаполненияРегламентированнойОтчетностиЗарплата("РегламентированныйОтчетСтатистикаФорма1РабочаяСила", ПараметрыОтчетаДляЗаполнения, ДанныеОтчета);
	СтрокаОрганизации = ДанныеОтчета.Строки.Найти(ПараметрыОтчета.Организация, "Организация");
	Если СтрокаОрганизации <> Неопределено Тогда
		Контейнер.ПолеТабличногоДокументаФормаОтчета["П001001000204"] = СтрокаОрганизации.ОтработаноЧасовРаботникамиСписочногоСостава;
		Контейнер.ПолеТабличногоДокументаФормаОтчета["П001001000404"] = СтрокаОрганизации.ОтработаноЧасовРаботникамСписочногоСоставаВОтпуске;
		Контейнер.ПолеТабличногоДокументаФормаОтчета["П001001000504"] = СтрокаОрганизации.ОплаченоЧасовРаботникамСписочногоСоставаВПростое;
		Контейнер.ПолеТабличногоДокументаФормаОтчета["П001001000604"] = СтрокаОрганизации.ОплаченоЧасовРаботникамСписочногоСоставаПрочие;
		СуммаЗарплатыВНеденежнойФорме = 0;
		СуммаЕдиновременныхВыплатВНеденежнойФорме = 0;
		СуммаОплатыПитанияВНеденежнойФорме = 0;
		Для Каждого СтрокаПоКатегории Из СтрокаОрганизации.Строки Цикл
			Категория = СтрокаПоКатегории.ВидРасчетаКатегорияСтатистическогоНаблюденияФорма1РабочаяСила;
			Если Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ЗаработнаяПлата Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001000903"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
				Для Каждого СтрокаПоВидуНачисления Из СтрокаПоКатегории.Строки Цикл
					ВидНачисления = СтрокаПоВидуНачисления.ВидРасчетаКатегорияНачисленияИлиНеоплаченногоВремени;
					Если ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ДоходВНатуральнойФорме
						Или ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаТрудаВНатуральнойФорме Тогда
						СуммаЗарплатыВНеденежнойФорме = СуммаЗарплатыВНеденежнойФорме + СтрокаПоВидуНачисления.НачисленоРаботникамСписочногоСостава;
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДоплатыНадбавки Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001003"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ПремииВознаграждения Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001103"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
				Для Каждого СтрокаПоВидуНачисления Из СтрокаПоКатегории.Строки Цикл
					ВидНачисления = СтрокаПоВидуНачисления.ВидРасчетаКатегорияНачисленияИлиНеоплаченногоВремени;
					Если ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ДоходВНатуральнойФорме
						Или ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаТрудаВНатуральнойФорме Тогда
					...
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ВыплатыПоРайонномуРегулированию Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001203"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеВидыОплатыТруда Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001303"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаЗаНеотработанноеВремя Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001503"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
				СуммаОтпусков = 0;
				СуммаПростоев = 0;
				Для Каждого СтрокаПоВидуНачисления Из СтрокаПоКатегории.Строки Цикл
					ВидНачисления = СтрокаПоВидуНачисления.ВидРасчетаКатегорияНачисленияИлиНеоплаченногоВремени;
					Если ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаОтпуска Тогда
						СуммаОтпусков = СуммаОтпусков + СтрокаПоВидуНачисления.НачисленоРаботникамСписочногоСостава;
					ИначеЕсли ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПростояПоВинеРаботодателя
						Или ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПростояПоНезависящимОтРаботодателяПричинам Тогда
						СуммаПростоев = СуммаПростоев + СтрокаПоВидуНачисления.НачисленоРаботникамСписочногоСостава;
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ЕдиновременныеПоощрительныеВыплаты Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001803"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
				Для Каждого СтрокаПоВидуНачисления Из СтрокаПоКатегории.Строки Цикл
					ВидНачисления = СтрокаПоВидуНачисления.ВидРасчетаКатегорияНачисленияИлиНеоплаченногоВремени;
					Если ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ДоходВНатуральнойФорме
						Или ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаТрудаВНатуральнойФорме Тогда
						СуммаЕдиновременныхВыплатВНеденежнойФорме = СуммаЕдиновременныхВыплатВНеденежнойФорме + СтрокаПоВидуНачисления.НачисленоРаботникамСписочногоСостава;
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаПитанияПроживания Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001002003"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
				Для Каждого СтрокаПоВидуНачисления Из СтрокаПоКатегории.Строки Цикл
					Если ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ДоходВНатуральнойФорме
						Или ВидНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаТрудаВНатуральнойФорме Тогда
						СуммаОплатыПитанияВНеденежнойФорме = СуммаОплатыПитанияВНеденежнойФорме + СтрокаПоВидуНачисления.НачисленоРаботникамСписочногоСостава;
					КонецЕсли;
				КонецЦикла;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.БезвозмездныеСубсидии Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001002403"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ВыходноеПособиеНачисленияНаПериодТрудоустройства Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001003103"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.СуммыПоВременнойНетрудоспособности Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001003203"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.МатериальнаяПомощь Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001003303"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеРасходыНаСоциальнуюЗащиту Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001003403"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.РасходыНаКомандировки Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001003903"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаПроезда Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001004103"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			ИначеЕсли Категория = Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеРасходы Тогда
				Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001004203"] = СтрокаПоКатегории.НачисленоРаботникамСписочногоСостава;
			КонецЕсли;
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001403"] = СуммаЗарплатыВНеденежнойФорме;
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001603"] = СуммаОтпусков;
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001703"] = СуммаПростоев;
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001001903"] = СуммаЕдиновременныхВыплатВНеденежнойФорме;
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П002001002103"] = СуммаОплатыПитанияВНеденежнойФорме;
