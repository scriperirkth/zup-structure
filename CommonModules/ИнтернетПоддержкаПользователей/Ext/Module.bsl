...
//  Логин - Неопределено - без аутентификации.
//        - Строка - логин basic authentication.
//  Пароль - Неопределено - без аутентификации.
//         - Строка - пароль basic authentication.
//  ПараметрыЗапроса - Неопределено - параметры запроса по умолчанию.
//                   - Структура - уточняющие параметры запроса:
//   * ФорматОтвета - Число - определяет формат возврата содержимого ответа операции:
//        0 - как файл (в поле ответа "Содержимое" будут возвращен файл во временном
//          каталоге с именем, указанным в параметре "ИмяФайла").
//        1 - как строка.
//        2 - как двоичные данные.
//        Любое другое значение - как HTTPОтвет.
//        Значение по умолчанию: 0
//   * Метод - Строка - http метод: "GET", "POST", "PUT" и т.п..
//       Значение по умолчанию: GET
//   * ДанныеДляОбработки - Строка, ДвоичныеДанные, Неопределено - данные для отправки
//       методов передаваемые методом POST, PUT, PATCH.
...
//       ** Ключ - Строка - имя заголовка запроса.
//       ** Значение - Строка - значение заголовка запроса.
//       Значение по умолчанию: Неопределено
//   * ИмяФайла - Строка - имя файла с расширением, которое будет использовано при
//       создании нового файла если ФорматОтвета = 0.
//   * Таймаут - Число - время ожидания завершения операции в секундах.
//       Значение по умолчанию: 30
//   * НастройкиПрокси - Неопределено - настройки прокси будут получены из информационной базы.
//                     - Структура - настройки подключения к прокси серверу.
//       Значение по умолчанию: Неопределено
//   * ИспользоватьЗащищенноеСоединение - Булево -признак использования защищенного соединения
//        для запросов https.
//        Значение по умолчанию: Истина
//   * КоличествоПеренаправлений - Число - максимальное количество перенаправлений,
//       которое следует обрабатывать.
//       Значение по умолчанию: 7
//   * ЗащищенноеСоединение - Неопределено - при использовании защищенного соединения будет
//       создан новый объект ЗащищенноеСоединениеOpenSSL.
//                          - ЗащищенноеСоединениеOpenSSL - защищенное соединение, которое
//       следует использовать при создании соединения.
//       Значение по умолчанию: Неопределено
//
// Возвращаемое значение:
//   Структура - содержит результат операции:
//    * КодОшибки - Строка - идентификатор ошибки:
//        LockError - запрос заблокирован настройками программы.
//        ConnectError - ошибка установки соединения с сервисом.
//        ServerError - ошибка на сервере.
//        ClientError - ошибка формирования запроса на клиенте.
//    * СообщениеОбОшибке - Строка - описание ошибки для пользователя.
//    * ИнформацияОбОшибке - Строка - описание ошибки для администратора.
//    * Содержимое - Строка, ДвоичныеДанные, HTTPОтвет, Неопределено - тело ответа.
//    * КодСостояния - Число - код состояния http, который вернул сервер.
//        Возможно передача кодов состояний -1 и -2 в случае блокирования запроса
//        со стороны программы 1С.
//    * ФорматОтвета - Число - определяет формат возврата содержимого ответа операции:
//        0 - как файл (в поле "Содержимое" будет возвращен файл во временном
//          каталоге с именем, указанным в параметре ПараметрыЗапроса.ИмяФайла).
//        1 - как строка.
//        2 - как двоичные данные.
//        Любое другое значение - как HTTPОтвет.
...
Функция ЗагрузитьСодержимоеИзИнтернет(
	...
	ПараметрыПолучения.Вставить("ИмяФайла"                , "");
	...
	ИмяФайлаОтвета         = Неопределено;	// Неопределено, Строка
	Если Результат.ФорматОтвета = 0 Тогда
		Если ПустаяСтрока(ПараметрыПолучения.ИмяФайла) Тогда
			// Удаление файла должен выполнить вызывающий метод
			// Имя файла не передано поэтому создается файл по умолчанию
			//@skip-check missing-temporary-file-deletion
			ИмяФайлаОтвета = ПолучитьИмяВременногоФайла();
		Иначе
			ИмяФайлаОтвета = ФайловаяСистема.СоздатьВременныйКаталог()
				+ ПараметрыЗапроса.ИмяФайла;
			Если Не ЭтоИмяВременногоФайла(ИмяФайлаОтвета) Тогда
				ВызватьИсключение НСтр("ru = 'Ошибка в адресе сохранения файла.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Пока КоличествоПеренаправлений < МаксКолвоПеренаправлений Цикл
		...
		Попытка
			...
			Если ПараметрыПолучения.Метод = "GET" Тогда
				Ответ = Соединение.Получить(HTTPЗапрос, ИмяФайлаОтвета);
...
//
Процедура УстановитьИспользованиеSMS()
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия")
		Или Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
		Возврат;
	КонецЕсли;
...
//
Процедура УстановитьИспользованиеШаблоновСообщений()
...
//
Процедура УстановитьИспользованиеЭлектроннойПочты()
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия")
		Или Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		Возврат;
	КонецЕсли;
...
//
Процедура УстановитьОтправкуПисемВФорматеHTML()
...
// Выполняет включение функциональных опций, необходимых для отправки шаблонов сообщений.
//
// Параметры:
//  ИспользоватьПодсистемуВзаимодействия - Булево - признак использования подсистемы Взаимодействия.
//
Процедура УстановитьИспользованиеОтправкиШаблоновСообщений(ИспользоватьПодсистемуВзаимодействия) Экспорт
	
	Если ИспользоватьПодсистемуВзаимодействия Тогда
		УстановитьИспользованиеSMS();
		УстановитьИспользованиеЭлектроннойПочты();
		УстановитьОтправкуПисемВФорматеHTML();
	КонецЕсли;
