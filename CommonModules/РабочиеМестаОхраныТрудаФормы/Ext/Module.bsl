...
Процедура УточнитьПараметрыОтбораФормыВыбораСотрудников(Форма, Организация, СписокНастройкиОтбора = Неопределено) Экспорт
	...
	Если Форма.Параметры.Отбор.Свойство("РабочееМесто")
		...
		Если СписокНастройкиОтбора <> Неопределено Тогда
			...
			СписокНастройкиОтбора.Добавить(СтрШаблон(НСтр("ru='Рабочее место: %1'"), РабочееМесто));
		КонецЕсли;
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Форма.Список, "РабочееМесто", РабочееМесто);
		
		Форма.Параметры.Отбор.Удалить("РабочееМесто");
	
	ИначеЕсли ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.Параметры.Отбор, "РабочееМестоЗаполнено", Ложь) Тогда
		Если СписокНастройкиОтбора <> Неопределено Тогда
			СписокНастройкиОтбора.Добавить(НСтр("ru='Рабочее место: заполнено'"));
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Форма.Список, "РабочееМесто", РабочееМесто,ВидСравненияКомпоновкиДанных.Заполнено);
		
		Форма.Параметры.Отбор.Удалить("РабочееМестоЗаполнено");
	КонецЕсли;
			
КонецПроцедуры

Функция КадровыеДанныеПодобранныхСотрудников(Форма, ВыбранныеСотрудники,
	Знач ТекущийОбъект, Знач КадровыеДанные = "РабочееМестоОхраныТруда") Экспорт
	
	КадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
		Истина, ВыбранныеСотрудники, КадровыеДанные, Форма.Объект.Дата);
	
	СтрокиБезРабочихМест = КадровыеДанныеСотрудников.НайтиСтроки(Новый Структура("РабочееМестоОхраныТруда", NULL));
	Для Каждого Строка Из СтрокиБезРабочихМест Цикл
		ТекстСообщения = НСтр("ru = 'Сотрудник ""%1"" не добавлен, т.к. для него не назначено рабочее место.'");
		...
		КадровыеДанныеСотрудников.Удалить(Строка);
	КонецЦикла;
