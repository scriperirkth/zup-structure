...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьИдентификаторСтрокиИПериодДействияОграниченияВзысканий");
	Методы.Вставить("УстановитьНастройкуУчитыватьЗадолженностьПоВзысканиямПриРасчетеУдержаний");
КонецПроцедуры
...
Функция РезультатОграниченияВзысканий(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	УдержанияПоФизическимЛицам.РанееУдержано КАК РанееУдержано
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	СохраняемыйЗаработок.Период КАК Период,
	               |	СохраняемыйЗаработок.ПрожиточныйМинимум КАК ПрожиточныйМинимум
	...
	Запрос.Текст =
		...
		|	УсловияУдержанияПоИсполнительномуДокументу.ПлатежныйАгент КАК ПлатежныйАгент,
		|	ИсполнительныеЛисты.РанееУдержано КАК РанееУдержано
		...
		|	ИсполнительныеЛисты.РанееУдержано
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	УдержанияПоФизическимЛицам.РанееУдержано
		|ИЗ
		|	ВТУдержанияПоФизическимЛицам КАК УдержанияПоФизическимЛицам
		|ГДЕ
		|	УдержанияПоФизическимЛицам.КатегорияУдержания <> ЗНАЧЕНИЕ(Перечисление.КатегорииУдержаний.ИсполнительныйЛист)
		...
		|	СУММА(ОчередностьУдержанийФизическиеЛица.РанееУдержано) КАК РанееУдержано,
		...
		|ГДЕ
		|	((ЗаработокФизическихЛиц.СуммаНачислений - ЗаработокФизическихЛиц.СуммаНДФЛ) * УдержанияФизическихЛиц.Коэффициент - УдержанияФизическихЛиц.РанееУдержано < УдержанияФизическихЛиц.Рассчитано
		...
		|	0 КАК Результат,
		|	ОчередностьУдержанийФизическиеЛица.РанееУдержано КАК РанееУдержано,
	...
	Пока Выборка.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("Очередность") Цикл
			...
			КонецЦикла;
			Для Каждого ПроцентВзыскания Из ПроцентыВзыскания Цикл
				МаксимальнаяСумма = ЗаработнаяПлата * ПроцентВзыскания;
				Для Каждого ИндексСтроки Из СписокИндексов Цикл
					ДанныеУдержания = УдержанияПоСтрокам[ИндексСтроки];
					РанееУдержано = РанееУдержано + ДанныеУдержания.РанееУдержано;
					Рассчитано = Рассчитано + ДанныеУдержания.Рассчитано;
				КонецЦикла;
				Если МаксимальнаяСумма <= Рассчитано + РанееУдержано Тогда
					УдержаннаяСумма = УдержаннаяСумма + РанееУдержано;
				КонецЕсли;
				РаспределяемаяСумма = МаксимальнаяСумма - УдержаннаяСумма;
...
Функция РезультатОграниченияВзысканийДокумента(Начисления, Удержания, НДФЛ, МенеджерВременныхТаблиц, ПериодРегистрации, ИсключаемыйРегистратор, РассчитыватьПоТекущемуДокументу = Ложь) Экспорт 
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("РассчитыватьПоТекущемуДокументу", РассчитыватьПоТекущемуДокументу);
	...
	ТекстЗапроса =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТУдержанныеСуммы.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ВТУдержанныеСуммы.ВидРасчета КАК ВидРасчета,
		|	ВТУдержанныеСуммы.ДокументОснование КАК ДокументОснование,
		|	ВТУдержанныеСуммы.Сумма КАК Сумма
		|ПОМЕСТИТЬ ВТРанееУдержанныеСуммы
		|ИЗ
		|	ВТУдержанныеСуммы КАК ВТУдержанныеСуммы
		|ГДЕ
		|	НЕ &РассчитыватьПоТекущемуДокументу
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(ВТРанееУдержанныеСуммы.Сумма, 0) КАК РанееУдержано
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРанееУдержанныеСуммы КАК ВТРанееУдержанныеСуммы
		|		ПО НачисленияУдержания.ФизическоеЛицо = ВТРанееУдержанныеСуммы.ФизическоеЛицо
		|			И НачисленияУдержания.НачислениеУдержание = ВТРанееУдержанныеСуммы.ВидРасчета
		|			И НачисленияУдержания.ДокументОснование = ВТРанееУдержанныеСуммы.ДокументОснование
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	УничтожитьВТ.Добавить("ВТРанееУдержанныеСуммы");
	ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, УничтожитьВТ);
...
Функция ТаблицаУдержанийФизическихЛиц()
	...
	ТаблицаУдержаний.Колонки.Добавить("РанееУдержано", Новый ОписаниеТипов("Число"));
