...
Функция ПолучитьСвойстваКриптосообщения(Знач Криптосообщение, Настройки = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Криптосообщение) = Тип("Строка") И НЕ ЭтоАдресВременногоХранилища(Криптосообщение) Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			Криптосообщение, "Криптосообщение", "ПолучитьСвойстваКриптосообщения");		
	КонецЕсли;
...
//     ИдентификаторКлючаСубъекта - ДвоичныеДанные или Неопределено - хеш SHA-1 публичного ключа.
//     ИспользованиеКлюча - Число - бит 0 при обратной записи, то есть самый старший, соответствующий десятичному 128,
//       задает флаг "digitalSignature", бит 2, соответствующий десятичному 32, задает флаг "keyEncipherment",
//       бит 3, соответствующий десятичному 16, задает флаг "dataEncipherment",
//       бит 4, соответствующий десятичному 8, задает флаг "keyAgreement", сравнить можно функцией "ПобитовоеИ".
...
//       Перечисления.ТипыКриптоПровайдеров.CryptoPro и Перечисления.ТипыКриптоПровайдеров.VipNet.
...
Функция ПолучитьСвойстваСертификата(Знач ДанныеСертификата, Настройки = Неопределено) Экспорт
	...
	Попытка
		...
		Если СвойстваБлока.Класс = "CONTEXT-SPECIFIC" И СвойстваБлока.Тег = Типы.BIT_STRING Тогда
			...
			ИспользованиеКлючаНайдено = Ложь;
			...
			Пока Истина Цикл
				...
				ИначеЕсли ИдентификаторРасширения = "2.5.29.15" Тогда
					Успешно = ПрочитатьOID2_5_29_15(ОбъектЧтениеДанных, СвойстваСертификата);
					Если Успешно Тогда
						ИспользованиеКлючаНайдено = Истина;
					Иначе
						Возврат СвойстваСертификата;
					КонецЕсли;
...
Функция ПрочитатьФайлВМассив(ИмяФайла)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ИмяФайла, "ИмяФайла", "ПрочитатьФайлВМассив");	
	//@skip-check 1c-fresh-audit
...
Процедура ЗаписатьФайлИзМассива(ИмяФайла, Массив)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ИмяФайла, "ИмяФайла", "ЗаписатьФайлИзМассива");
	//@skip-check 1c-fresh-audit
...
Функция ПрочитатьOID2_5_29_15(ОбъектЧтениеДанных, СвойстваСертификата)
	...
	СвойстваБлока = ПрочитатьСвойстваБлока(ОбъектЧтениеДанных, СвойстваСертификата.Размер);
	Если НужноПрерватьЧтениеДляBIT_STRING(СвойстваБлока, ОбъектЧтениеДанных) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Буфер = ОбъектЧтениеДанных.ПрочитатьВБуферДвоичныхДанных(СвойстваБлока.РазмерБлока);
	СвойстваСертификата.ИспользованиеКлюча = ?(Буфер.Размер >= 2, Буфер.Получить(Буфер.Размер - 1), 0);
	КоличествоНеиспользуемыхБитМладшегоБайта = ?(Буфер.Размер = 2, Буфер.Получить(0), 0);
	КоличествоНеиспользуемыхБитМладшегоБайта = Мин(КоличествоНеиспользуемыхБитМладшегоБайта, 8);
	Для НомерБита = 0 По КоличествоНеиспользуемыхБитМладшегоБайта - 1 Цикл
		СвойстваСертификата.ИспользованиеКлюча = УстановитьБит(СвойстваСертификата.ИспользованиеКлюча, НомерБита, Ложь);
	КонецЦикла;
	Возврат Истина;
...
Функция НужноПрерватьЧтениеДляBIT_STRING(СвойстваБлока, ОбъектЧтениеДанных)
...
Функция ШаблонРезультата()
	...
	СвойстваСертификата.Вставить("ИспользованиеКлюча", Неопределено);
...
Функция ПолучитьСвойстваИзЗашифрованногоXML(ДвоичныеДанныеСимметричногоКлючаИзXML) Экспорт
	...
	Исключение
		ИмяСобытия = НСтр("ru = 'Сервис криптографии. Чтение зашифрованного XML'", ОбщегоНазначения.КодОсновногоЯзыка());
