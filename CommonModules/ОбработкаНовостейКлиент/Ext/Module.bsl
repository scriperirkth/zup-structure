...
Функция ПоказатьНовость(
	...
	ПараметрыОткрытияФормыЭталон = Новый Структура;
		ПараметрыОткрытияФормыЭталон.Вставить("ИнициаторОткрытияНовости", "Неизвестно"); // АПК:1297 Это идентификатор.
		ПараметрыОткрытияФормыЭталон.Вставить("Ключ"                    , Неопределено); // АПК:1297 Это идентификатор.
	Если ТипЗнч(ПараметрыОткрытияФормы) = ТипСтруктура Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыОткрытияФормыЭталон, ПараметрыОткрытияФормы);
	КонецЕсли;
...
Функция ПоказатьНовости(
	...
	Если ТипЗнч(ПараметрыОткрытияФормы) <> ТипСтруктура Тогда
		...
		ПараметрыОткрытияФормы.Вставить("ИнициаторОткрытияНовости", "Неизвестно"); // АПК:1297 Это идентификатор.
	КонецЕсли;
...
Процедура КонтекстныеНовости_ОбработкаКомандыНовости(Форма, Команда) Экспорт
	...
	Если (Команда.Имя = "Команда_Новость_Список") ИЛИ (Команда.Имя = "КонтекстныеНовостиСписок") Тогда
		ПараметрыОткрытияФормыКонтекстныхНовостей = Новый Структура;
			...
			ПараметрыОткрытияФормыКонтекстныхНовостей.Вставить("РежимОткрытияОкна"            , "БлокироватьОкноВладельца"); // АПК:1297 Это идентификатор.
			ПараметрыОткрытияФормыКонтекстныхНовостей.Вставить("ИнициаторОткрытияНовости"     ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"КонтекстныеНовостиОткрытиеСпискаИзМеню/Метаданные=%1/Форма=%2", // АПК:1297 Это идентификатор.
			...
			Иначе
				Если НЕ НайденнаяНовость.Новость.Пустая() Тогда
					...
					ПараметрыОткрытияФормы.Вставить("ИнициаторОткрытияНовости",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							"КонтекстныеНовостиОткрытиеИзМеню/Метаданные=%1/Форма=%2", // АПК:1297 Это идентификатор.
							Форма.Новости.Метаданные,
							Форма.Новости.Форма));
...
Процедура КонтекстныеНовости_ОбработкаОповещения(
	...
	ИначеЕсли ИмяСобытия = ВсеОповещения.ПродолжитьАвтолистание Тогда // АПК:761 Идентификатор.
		...
		Если Параметр = Форма.УникальныйИдентификатор Тогда
			...
			Если ФормаНовостиЭталон.СпособОтображенияПанелиКонтекстныхНовостей = "Автолистание" Тогда // АПК:1297 Это идентификатор.
		...
		Если Параметр = Форма.УникальныйИдентификатор Тогда
			...
			Если (НайденнаяДекорацияКартинки <> Неопределено)
				...
				Если ТипЗнч(ПараметрыПриложения) <> ТипСоответствие Тогда
					ПараметрыПриложения = Новый Соответствие;
				КонецЕсли;
				ИмяПараметраПриложения = "ИнтернетПоддержкаПользователей.Новости.АнимированныеИконкиДляПанелейКонтекстныхНовостей"; // АПК:1297 Это идентификатор.
...
Процедура КонтекстныеНовости_ПоказатьНовостиТребующиеПрочтенияПриПроизвольномСобытии(
	...
	Если ТипЗнч(МассивСтруктурНовостей) = ТипМассив Тогда
		Для Каждого ТекущаяНовость Из МассивСтруктурНовостей Цикл
			Если (СтрСравнить(ТекущаяНовость.Форма, ИдентификаторФормы) <> 0) // Имя формы не совпало.
					...
					ИЛИ (ТекущаяНовость.СпособОповещения = "Отключено") Тогда // АПК:1297 Это идентификатор.
	...
	Если ЛокальныеИдентификаторыСобытий.Количество() = 0 Тогда
		ИнициаторОткрытияНовостиСобытие = "Отсутствует"; // АПК:1297 Это идентификатор.
	...
	ИнициаторОткрытияНовости = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"КонтекстныеНовостиОткрытиеПриСобытии/Метаданные=%1/Форма=%2/Событие=%3", // АПК:1297 Это идентификатор.
		ИдентификаторМетаданных,
		ИдентификаторФормы,
	...
	Если ТипЗнч(МассивСтруктурНовостей) = ТипМассив Тогда // Массив структур новостей
		...
		ПараметрыОткрытияФормы = Новый Структура;
			ПараметрыОткрытияФормы.Вставить("РежимОткрытияОкна"                    , "БлокироватьОкноВладельца"); // АПК:1297 Это идентификатор.
	...
	Если СтандартнаяОбработка <> Ложь Тогда
		...
		ИначеЕсли ВРег(Элемент.Имя) = ВРег("ПанельКонтекстныхНовостей_КартинкаСтрелкаВлево") Тогда
			...
			Если ТипЗнч(ПараметрыПриложения) <> ТипСоответствие Тогда
				ПараметрыПриложения = Новый Соответствие;
			КонецЕсли;
			ИмяПараметраПриложения = "ИнтернетПоддержкаПользователей.Новости.АвтолистаниеДляПанелейКонтекстныхНовостей"; // АПК:1297 Это идентификатор.
			...
			Если ТипЗнч(ФормаНовостиЭталон.Новости) = ТипМассив Тогда
				Для Каждого ТекущаяНовость Из ФормаНовостиЭталон.Новости Цикл
					...
					Если Строка(ТекущаяНовостьЭталон.НомерСтрокиНовости) = НомерНовости Тогда
						...
						ИнициаторОткрытияНовости = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							"КонтекстныеНовостиОткрытиеИзПанелиКонтекстныхНовостей/Метаданные=%1/Форма=%2", // АПК:1297 Это идентификатор.
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	ИнтервалСекунд = 0;
	ИнтеграцияПодсистемБИПКлиент.ПереопределитьВремяПодключенияОбработчиковНовостейПриСтартеПрограммы(ИнтервалСекунд);
	ОбработкаНовостейКлиентПереопределяемый.ПереопределитьВремяПодключенияОбработчиковНовостейПриСтартеПрограммы(ИнтервалСекунд);
	Если (ТипЗнч(ИнтервалСекунд) = ТипЧисло)
			И (ИнтервалСекунд > 0) Тогда
		Если (ИнтервалСекунд > 999) Тогда
			ИнтервалСекунд = 999;
		КонецЕсли;
		ПодключитьОбработчикОжидания("ПодключитьРаботуСНовостямиНаКлиентеСЗадержкой", ИнтервалСекунд, Истина);
	Иначе
...
Функция ВсеОповещения() Экспорт
	...
	Результат = Новый Структура;
		Результат.Вставить("АктивизироватьПапкуОтбора"                  , "Новости. Активизировать папку отбора");
		Результат.Вставить("АктивизироватьОткрытуюФормуСпискаНовостей"  , "Новости. Активизировать открытую форму списка новостей");
		Результат.Вставить("ОткрытьФормуНастроек"                       , "Новости. Открыть форму настроек");
		Результат.Вставить("ОткрытьФормуНастроекНастройкиЛентНовостей"  , "Новости. Открыть форму настроек / Ленты новостей");
		Результат.Вставить("НовостьПрочтена"                            , "Новости. Новость прочтена");
		Результат.Вставить("ПоказатьОповещение"                         , "Новости. Показать оповещение пользователю из списка новостей");
		Результат.Вставить("ИзмененаПометкаСпискаНовостей"              , "Новости. Изменена пометка списка новостей");
		Результат.Вставить("ИзмененаПрочтенностьСпискаНовостей"         , "Новости. Изменена прочтенность списка новостей");
		Результат.Вставить("ИзмененаПометкаУдаления"                    , "Новости. Изменен признак удаления из списка новостей");
		Результат.Вставить("ИзмененоСостояниеОповещенияНовости"         , "Новости. Изменено состояние оповещения о новости");
		Результат.Вставить("ИзмененПризнакПрочитатьПозже"               , "Новости. Изменен признак Прочитать позже");
		Результат.Вставить("ИзмененоСостояниеПодпискиНаЛентуНовостей"   , "Новости: Изменено состояние подписки на ленту новостей");
		Результат.Вставить("ИзмененоСостояниеОповещенияЛентыНовостей"   , "Новости: Изменено состояние оповещений ленты новостей");
		Результат.Вставить("ЗагруженыНовости"                           , "Новости. Загружены новости");
		Результат.Вставить("ОбновленыНастройкиЧтенияНовостей"           , "Новости. Обновлены настройки чтения новостей");
		Результат.Вставить("СкопированыВсеКатегории"                    , "Новости: скопированы все категории");
		Результат.Вставить("ПослеЗаписиНовости"                         , "Новости: после записи новости");
		Результат.Вставить("ОбновилсяСписокДляОтправкиНаСервер"         , "Новости: обновился список новостей для отправки на сервер новостного центра");
		Результат.Вставить("ПослеЗаписиЛентыНовостей"                   , "Новости: после записи ленты новостей");
		Результат.Вставить("КлассификаторыОбновлены"                    , "Новости. Обновились данные классификаторов новостей с сервера 1С");
		Результат.Вставить("ОчищенаДатаПоследнейЗагрузкиНовостей"       , "Новости. Очищена дата последней загрузки новостей");
		Результат.Вставить("ОчищенаДатаПоследнейЗагрузкиКлассификаторов", "Новости. Очищена дата последней загрузки классификатора");
		Результат.Вставить("ИзменилсяСтатусОперации"                    , "Новости. Изменился статус операции");
		Результат.Вставить("ОперацияЗавершена"                          , "Новости. Операция завершена");
		Результат.Вставить("ПродолжитьАвтолистание"                     , "Новости. Продолжить автолистание в Панели контекстных новостей");
		Результат.Вставить("ОтключитьАнимациюИконки"                    , "Новости. Выключить анимацию иконки в Панели контекстных новостей");
		Результат.Вставить("СобратьИнформациюОбОкнах"                   , "Новости. Собрать информацию об открытых окнах");
		Результат.Вставить("СкрытьПоказатьБлок"                         , "Новости. Скрыть показать блок");
	// АПК:1297-вкл
...
Процедура ПодключитьОбработчикОповещенияОВажныхИОченьВажныхНовостях_ПервыйЗапуск()
	...
	Если (ТипЗнч(ИнтервалСекунд) = ТипЧисло)
		...
		ИдентификаторПараметраЗапускаНачало = "ОтложитьПоказНовостей="; // АПК:1297 Это идентификатор.
...
Процедура ИзменитьПризнакПрочтенностиНовостей(Знач МассивНовостей, ПризнакПрочтенности = Истина) Экспорт
	...
	Оповестить(
		ВсеОповещения().ИзмененаПрочтенностьСпискаНовостей,
		ПризнакПрочтенности,
		МассивНовостей);
...
Процедура ИзменитьПризнакУдаленияИзСпискаНовостей(Знач МассивНовостей, Знач ПризнакУдаленияИзСписка = Истина) Экспорт
	...
	Оповестить(
		ВсеОповещения().ИзмененаПометкаУдаления,
...
Процедура ОбработкаНавигационнойСсылки(НовостьСсылка, Форма, Действие, СписокПараметров) Экспорт
	...
	Если Действие = "Переход по навигационной ссылке" Тогда // АПК:1297 Это идентификатор.
	...
	ИначеЕсли Действие = "Оповещение" Тогда
		...
		Источники.Добавить("УИНФормы=" + СокрЛП(Форма.УникальныйИдентификатор)); // АПК:1297 Это идентификатор.
			...
			Если НЕ ТекущаяНовость.Значение.Пустая() Тогда
				ПараметрыОткрытияФормы.Вставить("ИнициаторОткрытияНовости",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						ЛентаНовостейКод,
						УИННовости)); // АПК:1297 Это идентификатор.
...
Процедура ПросмотрНовости_ОбработкаОповещения(ИмяСобытия, Параметр, Источник, Форма) Экспорт
	...
	Если ИмяСобытия = ВсеОповещения.СкрытьПоказатьБлок Тогда
...
Функция ПолучитьДействиеОткрытияФайла(ПолноеИмяФайла, СпособОткрытия) Экспорт
	...
	Если ВРег(СпособОткрытия) = ВРег("КакТекстВ1С") Тогда
		ЛокальныйСпособОткрытия = "КакТекстВ1С"; // АПК:1297 Это идентификатор.
	Иначе
		ЛокальныйСпособОткрытия = "СредствамиОС"; // АПК:1297 Это идентификатор.
	КонецЕсли;
...
Процедура НачатьПоследовательныйПоказВажныхНовостей(ВажныеНовостиСВключеннымиНапоминаниями) Экспорт
	...
	Для Каждого ТекущаяНовость Из ВажныеНовостиСВключеннымиНапоминаниями Цикл
		...
		ПараметрыОткрытияФормы = Новый Структура;
			...
			ПараметрыОткрытияФормы.Вставить("РежимОткрытияОкна", "Независимое"); // АПК:1297 Это идентификатор.
			ПараметрыОткрытияФормы.Вставить("ИнициаторОткрытияНовости", "ВажнаяНовость"); // АПК:1297 Это идентификатор.
			ПараметрыОткрытияФормы.Вставить("НовостьНаименование", ТекущаяНовостьЭталон.НовостьНаименование); // Заголовок новости.
			ПараметрыОткрытияФормы.Вставить("НовостьКодЛентыНовостей", ТекущаяНовостьЭталон.НовостьКодЛентыНовостей); // Код ленты новостей.
