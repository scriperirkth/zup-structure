...
Функция РаспределениеУдержанийПоРабочимМестамИСтатьям(СтрокиУдержаний, БазаУдержаний, СтрокиУжеУдержано, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтрокаУдержаний Из СтрокиУдержаний Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаУдержаний.ВидУдержания, "ЯвляетсяВзысканием") Тогда
			ЭтоВзыскание = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаУдержаний.ВидУдержания, "ЯвляетсяВзысканием");
		Иначе
			ЭтоВзыскание = Ложь;
		КонецЕсли;
		...
		Если БазовыеВидыРасчета <> Неопределено Тогда
			...
			Иначе
				НовыеСтрокиРаспределения = Новый Массив;
				Для каждого СтрокаРаспределения Из СтрокиРаспределения Цикл
					Если СтрокаРаспределения.ВидДоходаИсполнительногоПроизводства <> Перечисления.ВидыДоходовИсполнительногоПроизводства.КомпенсацииНеоблагаемыеПериодические
						И СтрокаРаспределения.ВидДоходаИсполнительногоПроизводства <> Перечисления.ВидыДоходовИсполнительногоПроизводства.КомпенсацииНеоблагаемыеЕдиновременные Тогда
						НовыеСтрокиРаспределения.Добавить(СтрокаРаспределения);
					ИначеЕсли Не ЭтоВзыскание Тогда
						НовыеСтрокиРаспределения.Добавить(СтрокаРаспределения);
					КонецЕсли;
				КонецЦикла;
				СтрокиРаспределения = БазаУдержаний.Скопировать(НовыеСтрокиРаспределения);
