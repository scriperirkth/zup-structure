...
Функция СотрудникНаПозицииРуководителя(ПозицияРуководителя) Экспорт
	...
	ТаблицаРуководителей = СотрудникиПоПозициям(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПозицияРуководителя));
	
	Если Не ЗначениеЗаполнено(ТаблицаРуководителей) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ТаблицаРуководителей[0].Сотрудник;
...
Функция ДобавитьПолеСведенийОРуководителяхСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Процедура СоздатьВТСотрудникиНаПозицияхРуководителей(МенеджерВременныхТаблиц, МассивПозиций, ДатаСреза = Неопределено, ИмяВыходнойТаблицы = "ВТСотрудникиНаПозицияхРуководителей")
	...
	ПараметрыПолученияСотрудников.КадровыеДанные = "ДолжностьПоШтатномуРасписанию, Состояние, Наименование";
	...
	Запрос.Текст =
		...
		|	СотрудникиОрганизации.Наименование КАК Наименование,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ВТПриоритетРуководителей.Наименование) КАК Наименование,
		|	ВТПриоритетРуководителей.Позиция КАК Позиция,
		|	ВТПриоритетРуководителей.Приоритет КАК Приоритет
		|ПОМЕСТИТЬ ВТСотрудникиНаПозицияхРуководителейПоНаименованиям
		|ИЗ
		|	ВТПриоритетРуководителей КАК ВТПриоритетРуководителей
		...
		|СГРУППИРОВАТЬ ПО
		|	ВТПриоритетРуководителей.Позиция,
		|	ВТПриоритетРуководителей.Приоритет
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ВТПриоритетРуководителей КАК ВТПриоритетРуководителей
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиНаПозицияхРуководителейПоНаименованиям КАК ПриоритетНаименования
		|		ПО ВТПриоритетРуководителей.Позиция = ПриоритетНаименования.Позиция
		|			И ВТПриоритетРуководителей.Приоритет = ПриоритетНаименования.Приоритет
		|			И ВТПриоритетРуководителей.Наименование = ПриоритетНаименования.Наименование
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТПриоритетРуководителей.Позиция
		|;
...
Функция СотрудникиПоПозициям(Позиции)
	...
	СоздатьВТСотрудникиНаПозицияхРуководителей(Запрос.МенеджерВременныхТаблиц, Позиции);
	
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СотрудникиНаПозицияхРуководителей.Сотрудник КАК Сотрудник,
		|	СотрудникиНаПозицияхРуководителей.ФизическоеЛицо КАК ФизическоеЛицо,
		|	СотрудникиНаПозицияхРуководителей.Позиция КАК ДолжностьПоШтатномуРасписанию
		|ИЗ
		|	ВТСотрудникиНаПозицияхРуководителей КАК СотрудникиНаПозицияхРуководителей
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДолжностьПоШтатномуРасписанию";
