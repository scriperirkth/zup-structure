...
// Возвращает параметры обмена для настройки обмена с банком.
// 
// Параметры:
...
Функция РеквизитыБанка(БИК) Экспорт
	...
	ИмяПараметра = СтрШаблон("Банки.%1 = &Параметр", ОбменСБанкамиСлужебный.ИмяПрикладногоРеквизита("Банк.БИК"));
	Возврат ОбменСБанкамиСлужебный.РеквизитыБанкаЗапросомПоПараметру(ИмяПараметра, БИК);
...
// Получает параметры обмена по параметрам выписки.
// 
// Параметры:
//  ПараметрыВыписки - см. ОбменСБанкамиКлиентСервер.ПараметрыПолученияВыпискиБанка.
// Возвращаемое значение:
//  Неопределено, Структура - При ошибке возвращается Неопределено. В остальных случая структура с полями:
// * РеквизитыСчета - Структура - реквизиты банковского счета организации:
//  ** Организация - ОпределяемыйТип.Организация - ссылка на организацию.
...
// * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - действующая настройка обмена с банком.
//
Функция ПараметрыОбменаПоПараметрамВыписки(ПараметрыВыписки) Экспорт
	
	Если Не ОбменСБанками.ПравоВыполненияОбмена(Истина) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	РеквизитыОбмена = Новый Структура;
	РеквизитыОбмена.Вставить("Организация", ПараметрыВыписки.Организация);
	РеквизитыОбмена.Вставить("Банк", ПараметрыВыписки.БанкВладелец);
	РеквизитыОбмена.Вставить("НастройкаОбмена", ПараметрыВыписки.НастройкаОбмена);
	РеквизитыОбмена.Вставить("НомерСчета", "");
	
	МассивБанковскихСчетов = ОбменСБанкамиСлужебный.НомераБанковскихСчетов(
		ПараметрыВыписки.Организация, ПараметрыВыписки.БанкВладелец);
	
	Если МассивБанковскихСчетов.Количество() > 0 Тогда
		НомерСчета = МассивБанковскихСчетов[0];
		РеквизитыОбмена.НомерСчета = НомерСчета;
	КонецЕсли;
		...
		Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			...
			Если Не ТребуетсяПодпись Тогда
				...
				Если ЗначениеЗаполнено(МассивБанковскихСчетов) Тогда
					...
					РеквизитыБанка = ОбменСБанкамиСлужебный.РеквизитыБанкаПоСсылке(РеквизитыНастройкиОбмена.Банк);
					ДанныеБанковскихСчетов = ОбменСБанкамиСлужебный.НовыеДанныеБанковскихСчетов();
					
					Для каждого НомерСчета Из МассивБанковскихСчетов Цикл
						НоваяСтрока = ДанныеБанковскихСчетов.Добавить();
						НоваяСтрока.НомерСчета = НомерСчета;
						НоваяСтрока.БИК = РеквизитыБанка.БИК;
						НоваяСтрока.НаименованиеБанка = РеквизитыБанка.Наименование;
						НоваяСтрока.БанкВладелец = РеквизитыНастройкиОбмена.Банк;
					КонецЦикла;
					МассивСообщенийЗапросов = ОбменСБанкамиСлужебный.МассивЗапросовВыпискиАсинхронныйОбмен(
						НастройкаОбмена, ДатаНачала, ДатаОкончания, ДанныеБанковскихСчетов, Истина);
...
//   СправочникСсылка.НастройкиОбменСБанками, Неопределено - найденная настройка обмена с банком.
//
Функция НастройкаОбменаПоОтбору(Знач ПараметрыОтбора) Экспорт
	
	ТекущаяНастройкаОбмена = ОбменСБанкамиСлужебный.НастройкаОбменаПоОтбору(ПараметрыОтбора);
	Возврат ТекущаяНастройкаОбмена;
...
Функция ПараметрыОбменаПоРеквизитам(РеквизитыОбмена)
	...
	Иначе
		...
		ПараметрыОтбора.Организация = РеквизитыОбмена.Организация;
		ПараметрыОтбора.Банк = РеквизитыОбмена.Банк;
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена, "Банк, ПрограммаБанка,
...
Функция ЗапросыВыписок(
	...
	Если ЗначениеЗаполнено(ПараметрыПолученияВыписки.БанкВладелец) Тогда
		РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			НастройкаОбмена, "Организация, ПрограммаБанка, ИспользуетсяКриптография");
			
		БанкВладелец = ПараметрыПолученияВыписки.БанкВладелец;
	Иначе
		РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			НастройкаОбмена, "Организация, ПрограммаБанка, ИспользуетсяКриптография, Банк");
			
		БанкВладелец = РеквизитыНастройкиОбмена.Банк;
	КонецЕсли;
	...
	Иначе
		МассивБанковскихСчетов = ОбменСБанкамиСлужебный.НомераБанковскихСчетов(
			РеквизитыНастройкиОбмена.Организация, БанкВладелец);
	КонецЕсли;
	...
	Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АльфаБанкОнлайн Тогда
		МассивСообщенийЗапросов = МассивЗапросовВыписки(НастройкаОбмена, РеквизитыНастройкиОбмена.Организация,
			БанкВладелец, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов);
	Иначе
		
		РеквизитыБанка = ОбменСБанкамиСлужебный.РеквизитыБанкаПоСсылке(БанкВладелец);
		ДанныеБанковскихСчетов = ОбменСБанкамиСлужебный.НовыеДанныеБанковскихСчетов();
		...
		Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
			...
			НоваяСтрока.БИК = РеквизитыБанка.БИК;
			НоваяСтрока.НаименованиеБанка = РеквизитыБанка.Наименование;
			НоваяСтрока.БанкВладелец = БанкВладелец;
		КонецЦикла;
		
		МассивСообщенийЗапросов = ОбменСБанкамиСлужебный.МассивЗапросовВыпискиАсинхронныйОбмен(
			НастройкаОбмена, ДатаНачала, ДатаОкончания, ДанныеБанковскихСчетов, АвтоматическоеПолучение);
	КонецЕсли;
