...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьВремяВЧасахОтработанноеВремяПоСотрудникамКорректировка");
	Методы.Вставить("ЗаполнитьИдентификаторыСтрокОтработанноеВремяПоСотрудникамКорректировка");
КонецПроцедуры
...
Процедура ЗарегистрироватьНачисленияУдержанияПоКонтрагентамАкционерам(Движения, Отказ, Организация, ПериодРегистрации, Начисления = Неопределено, Удержания = Неопределено, НДФЛ = Неопределено, СпособРасчетов = Неопределено) Экспорт
	...
	Если Начисления <> Неопределено Тогда
		...
		Для Каждого Строка Из Начисления Цикл
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.ТерриторияВыполненияРаботВОрганизации) 
				И (ТипЗнч(Строка.Начисление) = Тип("СправочникСсылка.ВидыПрочихДоходовФизическихЛиц")
					ИЛИ ТипЗнч(Строка.Начисление) = Тип("СправочникСсылка.ВидыВыплатБывшимСотрудникам")) Тогда
				
				НоваяСтрока.ТерриторияВыполненияРаботВОрганизации = Строка.Подразделение;
																									 
			КонецЕсли; 
			
			Если ЗначениеЗаполнено(НоваяСтрока.ТерриторияВыполненияРаботВОрганизации) Тогда
				НоваяСтрока.РегистрацияВНалоговомОргане = 
					ЗарплатаКадры.РегистрацияВНалоговомОргане(Строка.Подразделение, ПериодРегистрации); 
			КонецЕсли;   
...
Функция НачисленныеОтпускаПоПериодамИВидам(Сотрудник, ВидыОтпусков, ИсключаемыеРегистраторы = Неопределено, РасчетДенежногоСодержания = Ложь) Экспорт
	...
	|	КОНЕЦ КАК Значение,
	|	СвойстваНачисления.ВидОтпуска КАК ВидОтпуска
	|ИЗ
	...
	ДанныеНачисленныхОтпусков.Колонки.Добавить("ВидОтпуска", Новый ОписаниеТипов("СправочникСсылка.ВидыОтпусков"));
	...
	ДанныеНачисленныхОтпусков.Сортировать("Приоритет, ВидОтпуска, ПериодДействия Убыв, ДатаНачалаОтпуска Убыв");
...
Процедура ПриОпределенииДоступныхДляПереводаМакетов(Макеты) Экспорт
	...
	УчетНачисленнойЗарплатыБазовый.ПриОпределенииДоступныхДляПереводаМакетов(Макеты);
	
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_РасчетныйЛистокСРазбивкойПоИсточникамФинансирования);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_РасчетныйЛистокСРазбивкойПоИсточникамФинансированияПерваяПоловинаМесяца);
...
Функция ЗапросВТНачисленныеДоходы(ИмяВТНачисленныеДоходы) Экспорт
	...
	|			ИЛИ НачисленияУдержанияПоСотрудникам.ГруппаНачисленияУдержанияВыплаты = ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Льготы)
