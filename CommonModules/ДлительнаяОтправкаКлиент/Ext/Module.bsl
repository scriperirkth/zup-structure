...
Функция ФормаДлительноеДействиеОткрыта() Экспорт
	
	ИщемФорму = "Обработка.ДокументооборотСКонтролирующимиОрганами.Форма.ДлительноеДействие";
	
	ЕстьОткрытаяФорма = Ложь;
	
	Окна = ПолучитьОкна();
	Если Окна <> Неопределено Тогда
		Для Каждого ОткрытоеОкно Из Окна Цикл
			Если ОткрытоеОкно.Содержимое.Количество()>0 Тогда
				Если ОткрытоеОкно.Содержимое[0].ИмяФормы = ИщемФорму Тогда
					ЕстьОткрытаяФорма = Истина;
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
