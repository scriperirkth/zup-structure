...
// Функция-конструктор параметров процедуры ОбменСБанками.НастройкаОбменаПоОтбору
//
// Возвращаемое значение:
//  Структура - параметры выполнения процедуры, содержит поля:
...
//    * ТолькоДействительнаяДляДополнительныхБанков - Булево - Истина - поиск только действующей настройки для банков
//                                                    из табличной части "Дополнительные банки",
//                                                    Ложь - поиск любой настройки.
//    * ОтборПоДополнительнымБанкам - Булево - Истина - выполнять поиск банка и в табличной части "Дополнительные банки".
//    * ИсключаяНастройки - Массив - массив ссылок на настройки обмена, которые исключаются из поиска.
//
Функция НовыеПараметрыОтбораНастройкиОбмена() Экспорт
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("Организация", Неопределено);
	ПараметрыОтбора.Вставить("Банк", Неопределено);
	ПараметрыОтбора.Вставить("ТолькоДействительная", Истина);
	ПараметрыОтбора.Вставить("ТолькоДействительнаяДляДополнительныхБанков", Истина);
	ПараметрыОтбора.Вставить("ОтборПоДополнительнымБанкам", Ложь);
	ПараметрыОтбора.Вставить("ИсключаяНастройки", Новый Массив);
...
//   * БанкВладелец - Неопределено, ОпределяемыйТип.БанкОбменСБанками - ссылка на банк, в котором открыт счет.
//   * НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка, через которую запрашивается выписка.
//   * Организация - ОпределяемыйТип.Организация - организация для запроса выписки
//
Функция ПараметрыПолученияВыпискиБанка() Экспорт
	
	ПараметрыПолученияВыписки = Новый Структура;
	ПараметрыПолученияВыписки.Вставить("ДатаНачала", '00010101');
	ПараметрыПолученияВыписки.Вставить("ДатаОкончания", '00010101');
	ПараметрыПолученияВыписки.Вставить("НомерСчета", "");
	ПараметрыПолученияВыписки.Вставить("БанкВладелец", Неопределено);
	ПараметрыПолученияВыписки.Вставить("НастройкаОбмена", Неопределено);
	ПараметрыПолученияВыписки.Вставить("Организация", Неопределено);
	
	Возврат ПараметрыПолученияВыписки;
