...
Процедура ДобавитьФайл(Архив, ИмяФайла) Экспорт
	...
	Пока Смещение < Размер Цикл
		...
		НайденнаяЗапись = ПрочитатьЗаписьКаталога(Буфер, Смещение);
		...
		Смещение = Смещение + НайденнаяЗапись.ДлинаЗаписи;
...
// Возвращаемое значение: 
//	Структура:
//	 * ЗаписиКаталога - ФайловыйПоток, ПотокВПамяти -
//   * Источник - ФайловыйПоток, ПотокВПамяти, УникальныйИдентификатор -
...
Функция ПрочитатьВложенныйНесжатыйАрхив(ДанныеАрхива, ИмяФайла) Экспорт
	...
	НайденнаяЗапись = НайтиЗапись(ДанныеАрхива, ИмяФайла);
...
// 
// Возвращаемое значение:
//  Неопределено, Структура - см. ПрочитатьЗаписьКаталога
Функция НайтиЗапись(ДанныеАрхива, ИмяФайла) Экспорт
	
	Смещение = ДанныеАрхива.СмещенияФайловВКаталоге.Получить(ИмяФайла);
	
	Если Смещение <> Неопределено Тогда
		
		Возврат ПрочитатьЗаписьКаталога(ДанныеАрхива.ЗаписиКаталога, Смещение);
...
Функция ПрочитатьАрхивВнутр(Знач Источник, Знач Смещение = 0, Знач Размер = Неопределено)
	...
	Если ТипЗнч(Источник) = Тип("УникальныйИдентификатор") Тогда
		...
		Если Не ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ПередачаДанных") 
			...
			ВызватьИсключение НСтр("ru = 'Получение данных из менеджера сервиса невозможно.'");
		КонецЕсли;
	...
	ДанныеАрхива.Вставить("СмещенияФайловВКаталоге", СмещенияФайловВКаталоге(ЗаписиКаталога));
	
	Возврат ДанныеАрхива;
	
КонецФункции

Функция СмещенияФайловВКаталоге(ЗаписиКаталога)
	
	СмещенияФайловВКаталоге = Новый Соответствие;
	
	Смещение = 0;
	Размер = ЗаписиКаталога.Размер;
	
	Пока Смещение < Размер Цикл
		
		Если ЗаписиКаталога.ПрочитатьЦелое32(Смещение) <> 33639248 Тогда
			ВызватьИсключение НСтр("ru = 'Неверный формат'");
		КонецЕсли;
	
		ДлинаИмениФайла = ЗаписиКаталога.ПрочитатьЦелое16(Смещение + 28);
		ДлинаДопДанных = ЗаписиКаталога.ПрочитатьЦелое16(Смещение + 30);
		ДлинаКомментария = ЗаписиКаталога.ПрочитатьЦелое16(Смещение + 32);
		ДлинаЗаписи = 46 + ДлинаИмениФайла + ДлинаДопДанных + ДлинаКомментария;
		
		Срез = ЗаписиКаталога.ПолучитьСрез(Смещение + 46, ДлинаИмениФайла);
		ИмяФайла = ПолучитьСтрокуИзБуфераДвоичныхДанных(Срез);
		
		СмещенияФайловВКаталоге.Вставить(ИмяФайла, Смещение);
		
		Смещение = Смещение + ДлинаЗаписи;
		
	КонецЦикла;
	
	Возврат СмещенияФайловВКаталоге;
