...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьГоловнуюОрганизациюВБольничных");
	Методы.Вставить("ЗаполнитьХешЭЛН");
	Методы.Вставить("ЗаполнитьУполномоченногоВОписях");
	Методы.Вставить("ЗаполнитьПризнакПостановкиНаУчетВРанниеСрокиБеременностиВРеестрах");
	Методы.Вставить("ПеренестиСостоянияЭЛН");
	Методы.Вставить("ОчиститьСлучайУходаВБольничных");
	Методы.Вставить("ДобавитьРолиЭЛНВПользовательскиеПрофили");
	Методы.Вставить("ЗаполнитьГоловнуюОрганизациюИЗафиксироватьПричинуИсправленияВРеестрахЭЛН");
	Методы.Вставить("ЗаполнитьПризнакПостановкиНаУчетВРанниеСрокиБеременностиВБольничных");
КонецПроцедуры
...
Функция ДатаНачалаОплатыЗаСчетФСС(ДатаНачалаСобытия, ПричинаНетрудоспособности, НачалоВПериодПриостановленияТД = Ложь) Экспорт
	КоличествоДнейЗаСчетРаботодателя = КоличествоДнейЗаСчетРаботодателя(ДатаНачалаСобытия, ПричинаНетрудоспособности,
		НачалоВПериодПриостановленияТД);
...
Функция ДождатьсяОкончанияЗаписиОбъекта(Параметры) Экспорт
	...
	Для НомерПопытки = 1 По КоличествоПопыток Цикл
		...
		Если Не Успех Тогда
			ПредставлениеВерсии = ?(ВерсияДанныхТекущая = Неопределено, НСтр("ru = 'Неопределено'"), ВерсияДанныхТекущая);
			Причина = Причина + Символы.ПС + СтрШаблон(НСтр("ru = 'Текущая версия данных: %1'"), ПредставлениеВерсии);
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурнала(),
...
Процедура ПриЗаписиРегистраГражданствоФизическихЛиц(РегистрСведенийНаборЗаписей, Отказ, Замещение) Экспорт
	...
	// Состав сведений см. в функции Документы.СведенияОЗастрахованномЛицеФСС.ИменаПолейТребуемыхКадровыхДанных().
	Если Отказ
		Или РегистрСведенийНаборЗаписей.Количество() = 0
...
Функция КоличествоДнейЗаСчетРаботодателя(ДатаНачалаСобытия, ПричинаНетрудоспособности, НачалоВПериодПриостановленияТД = Ложь) Экспорт
	// См. части 2 и 3 статьи 3 Федерального закона от 29.12.2006 № 255-ФЗ.
	...
	Если ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ОбщееЗаболевание
		И Не НачалоВПериодПриостановленияТД Тогда
...
Процедура РассчитатьПособиеПоНетрудоспособности(СтрокиРасчета, ДополнительныеПараметры, ПрименяетсяСторнирование = Истина) Экспорт
	...
	Для Каждого ГруппаСтрокРасчета Из ГруппыСтрокРасчета Цикл
		...
		СреднийПростоя = ОбщегоНазначенияБЗК.ЗначениеСвойства(СтрокаТаблицы.Дополнительно, "СреднийПростоя");
		Если СреднийПростоя = Неопределено Тогда
			СреднийПростоя = 0;
		КонецЕсли;
		...
		Иначе
			...
			// Ограничение пособия сверху по причине простоя.
			МаксимумЗаПериодПростоя = МаксимумПособияЗаПериодПростоя(СреднийПростоя, ОплачиваемоеВремя);
			Если МаксимумЗаПериодПростоя > 0 И РезультатВсего > МаксимумЗаПериодПростоя Тогда
				РезультатВсего = МаксимумЗаПериодПростоя;
				РезультатРК = 0;
				РезультатСН = 0;
			КонецЕсли;
			...
			Если РассчитыватьЧастьЗаСчетФБ Тогда
				...
				// Ограничение пособия сверху по причине простоя.
				Если МаксимумЗаПериодПростоя > 0 И РезультатВсегоБезЛьгот > МаксимумЗаПериодПростоя Тогда
					РезультатВсегоБезЛьгот = МаксимумЗаПериодПростоя;
					РезультатРКБезЛьгот = 0;
					РезультатСНБезЛьгот = 0;
				КонецЕсли;
...
Функция МаксимумПособияЗаПериодПростоя(СреднийПростоя, ОплачиваемоеВремя)
	// Федеральный закон № 255-ФЗ статья 7 часть 7: "Пособие по временной нетрудоспособности за период простоя
	// выплачивается в том же размере, в каком сохраняется за это время заработная плата,
	// но не выше размера пособия, которое застрахованное лицо получало бы по общим правилам".
	Если ТипЗнч(СреднийПростоя) <> Тип("Число") Или СреднийПростоя = 0 Или ОплачиваемоеВремя = 0 Тогда
		Возврат 0;
	КонецЕсли;
	Возврат Окр(СреднийПростоя * ОплачиваемоеВремя, 2);
КонецФункции
...
Функция ДанныеУчетаПриостановленийТД(Больничный) Экспорт
	...
	ЕстьОтборПоНачалуСобытия = ЗначениеЗаполнено(Больничный.ДатаНачалаСобытия);
	Если ЕстьОтборПоНачалуСобытия Тогда
		ОтборПоНачалуСобытия = ТаблицаОтбора.Добавить();
		ОтборПоНачалуСобытия.ГоловнаяОрганизация = Больничный.ГоловнаяОрганизация;
		ОтборПоНачалуСобытия.ФизическоеЛицо      = Больничный.ФизическоеЛицо;
		ОтборПоНачалуСобытия.НачалоПериода       = НачалоДня(Больничный.ДатаНачалаСобытия);
		ОтборПоНачалуСобытия.ОкончаниеПериода    = КонецДня(Больничный.ДатаНачалаСобытия);
	КонецЕсли;
	...
	НачалоВПериодПриостановленияТД = ?(ЕстьОтборПоНачалуСобытия, Ложь, Неопределено);
	...
	Пока ОбратныйИндекс > 0 Цикл
		...
		ИначеЕсли ЕстьОтборПоНачалуСобытия
			И СтрокаТаблицы.НачалоПериода <= ОтборПоНачалуСобытия.ОкончаниеПериода
			И СтрокаТаблицы.ОкончаниеПериода >= ОтборПоНачалуСобытия.НачалоПериода Тогда
			НачалоВПериодПриостановленияТД = Истина;
			ПриостановленияТД.Удалить(ОбратныйИндекс);
		КонецЕсли;
	КонецЦикла;
	
	Результат = Новый Структура;
	Результат.Вставить("ПриостановленияТД", ПриостановленияТД);
	Результат.Вставить("НачалоВПериодПриостановленияТД", НачалоВПериодПриостановленияТД);
	Возврат Результат;
КонецФункции
...
#Область ПроведениеДокументовПоУчетам

Функция РегистрыУчетаПособийСоциальногоСтрахования() Экспорт
	
	МассивРегистров = Новый Массив;
		                                              
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ПособияПоСоциальномуСтрахованию);
	МассивРегистров.Добавить(Метаданные.РегистрыНакопления.ПособияПоУходуЗаРебенком);
