...
Процедура ЗапросДанныеПечатиДокументов(Запрос) Экспорт
	...
	|	ЛОЖЬ КАК ЕстьРасшифровкаНачислений
	...
	|	ВТ_НастройкаРасчета.Ссылка КАК ВидРасчетаРезерва,
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РезервыПоОплатеТрудаПоказатели1.Показатель) КАК КоличествоПоказателей,
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_СвойстваПоказателей1.Идентификатор) КАК КоличествоПоказателей,
...
Функция ТекстЗапросаНастройкаРезервов() Экспорт
	...
	|	ГОД(&Период) >= 2026 КАК РаспределятьВыявленныеИзлишки
	|ИЗ
	|	Справочник.НастройкиРасчетаРезервовПоОплатеТруда КАК НастройкиРасчетаРезервовПоОплатеТруда
	|ГДЕ
	|	НастройкиРасчетаРезервовПоОплатеТруда.Организация = &Организация
...
Процедура ПереопределитьСпособОтраженияСтатьюРасходов(ТаблицаРасчетРезервов, Организация, Резерв, Период, ВидОперации) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаРасчетРезервовНачислено, ТаблицаРасчетРезервовНовая);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаРасчетРезервовПеренесено, ТаблицаРасчетРезервовНовая);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаРасчетРезервовСписано, ТаблицаРасчетРезервовНовая);
