...
// Параметры:
//  Настройки     - ТаблицаЗначений          - См. ЗарплатаКадрыРасширенныйСинхронизацияДанных.ШаблонОбработчика.
//  Данные        - Строка, ОбъектМетаданных - Имя или метаданные измерения, ресурса, или реквизита регистра.
//  РазделыДанных - Строка, Массив из Строка
//
Процедура ДляНезависимогоРегистра(Настройки, Данные, РазделыДанных = Неопределено) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.СинхронизацияДанных") Тогда
		МодульНастройкиОбработчиков = ОбщегоНазначения.ОбщийМодуль("СинхронизацияДанныхБЗКНастройкиОбработчиков");
		МодульНастройкиОбработчиков.ДляНезависимогоРегистра(Настройки, Данные, РазделыДанных);
	КонецЕсли;
...
// Определяет используемые обработчики для объектов с полями ГоловнаяОрганизация и ФизическоеЛицо.
//  Для головной организации получаются подчиненные организации
//  и используются в ОграничитьРегистрациюОбъектаОтборомПоОрганизациям.
//  Помимо головных организаций допустимо использовать для организаций (филиалов). В этом случае
//  в ИменаРеквизитов.ГоловнаяОрганизация необходимо указать имя реквизита со ссылкой на организацию (филиал).
//
// Параметры:
//   Настройки       - ТаблицаЗначений - См. ЗарплатаКадрыРасширенныйСинхронизацияДанных.ШаблонОбработчика.
//   РазделыДанных   - Строка, Массив из Строка
//   ИменаРеквизитов - Структура - Имена реквизитов объекта. Необязательный. По умолчанию
//       используются имена реквизитов "ГоловнаяОрганизация" и "ФизическоеЛицо".
