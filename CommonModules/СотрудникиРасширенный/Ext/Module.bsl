...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		...
		МодульРабочиеМестаОхраныТруда.ПриОбработкеПолученияДанныхВыбораСотрудников(Параметры.Отбор, ТекстыУсловий);
	КонецЕсли; 
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ПравилоФормированияПредставления)
		...
		Если ТребуетсяДатаУвольнения Тогда
			РабочаяДатаПользователя = ОбщегоНазначения.ТекущаяДатаПользователя();
	...
	Пока Обход.СледующийПоЗначениюПоля("ГоловнаяОрганизация") Цикл
		...
		Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Идентификатор  = "СотрудникиБезОтношений_"
				+  СтрЗаменить(Раздел.ПолноеИмя(), ".", "_") + СтрЗаменить(Обход.ТекущиеДанные().ГоловнаяОрганизация.УникальныйИдентификатор(), "-", "");
