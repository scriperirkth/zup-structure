...
Функция ТипОрганизации(Организация) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		СведенияОЮЛ = ?(Результат.ЭтоИП, Неопределено,
			РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация,, "ТипНП, ОГРН"));
		Результат.ЭтоИностраннаяОрганизация = НЕ Результат.ЭтоИП И (СведенияОЮЛ.ТипНП = 5
			ИЛИ НЕ ЗначениеЗаполнено(СведенияОЮЛ.ОГРН)
			ИЛИ ИнтерфейсыВзаимодействияБРО.ЭтоИностраннаяОрганизацияПоИНН(Организация));
				...
				Для каждого ЭлементАрхива Из ОбъектЧтение.Элементы Цикл
					...
					Если нрег(РасширениеЭлемента) = ".xml" ИЛИ нрег(РасширениеЭлемента) = ".sig"
						...
						Если НЕ ЭтоКвитанция Тогда
							...
							ИмяЭлемента = ЭлементАрхива.Имя;
							// Проверяем имя файла из ахрива на валидность
							РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ИмяЭлемента);
							Если нрег(РасширениеЭлемента) = ".xml" Тогда
								Если ЭтоОтзыв Тогда
									//@skip-check 1c-fresh-audit
									Результат.ДанныеЗаявленияНаОтзыв = Новый ДвоичныеДанные(КаталогРаспаковки + ИмяЭлемента);
								Иначе
									//@skip-check 1c-fresh-audit
									Результат.ДанныеВыгрузки = Новый ДвоичныеДанные(КаталогРаспаковки + ИмяЭлемента);
								КонецЕсли;
							ИначеЕсли нрег(РасширениеЭлемента) = ".sig" ИЛИ нрег(РасширениеЭлемента) = ".sign" Тогда
								//@skip-check 1c-fresh-audit
								ДанныеПодписи = Новый ДвоичныеДанные(КаталогРаспаковки + ИмяЭлемента);
...
Функция ПредставителиМЧДЕдиногоФормата(ДоверенностьСсылка, ТолькоФизическиеЛица = Ложь) Экспорт
	...
	Если Метаданные.РегистрыСведений.Найти(
		...
		Пока Выборка.Следующий() Цикл
			Если (ТолькоФизическиеЛица = Истина ИЛИ ТолькоФизическиеЛица = "РазрешитьВозвратФИО")
					И (ЗначениеЗаполнено(Выборка.ФизическоеЛицо)
				...
				ИначеЕсли ЗначениеЗаполнено(Выборка.ИННФЛ) Тогда
					...
					МассивРеквизитовПоискаФЛ = Новый Массив;
					МассивРеквизитовПоискаФЛ.Добавить(РеквизитыПоискаФизическогоЛица);
					...
					Если ЗначениеЗаполнено(Выборка.СНИЛС) Тогда
						СНИЛСБезРазделителей = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СНИЛСБезРазделителей(
							Выборка.СНИЛС);
						СНИЛСПредставителя = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.СНИЛССРазделителями(
						...
						Если ЕстьРеквизитСтраховойНомерПФР Тогда
							РеквизитыПоискаФЛСНИЛСБезРазделителей = ОбщегоНазначения.СкопироватьРекурсивно(РеквизитыПоискаФизическогоЛица);
							РеквизитыПоискаФЛБезСНИЛС = ОбщегоНазначения.СкопироватьРекурсивно(РеквизитыПоискаФизическогоЛица);
							РеквизитыПоискаФизическогоЛица.Вставить("СтраховойНомерПФР", СНИЛСПредставителя);
							РеквизитыПоискаФЛСНИЛСБезРазделителей.Вставить("СтраховойНомерПФР", СНИЛСБезРазделителей);
							МассивРеквизитовПоискаФЛ.Добавить(РеквизитыПоискаФЛСНИЛСБезРазделителей);
							МассивРеквизитовПоискаФЛ.Добавить(РеквизитыПоискаФЛБезСНИЛС);
						...
						Иначе
							...
							Если ЕстьРеквизитСНИЛС Тогда
								РеквизитыПоискаФЛСНИЛСБезРазделителей = ОбщегоНазначения.СкопироватьРекурсивно(РеквизитыПоискаФизическогоЛица);
								РеквизитыПоискаФЛБезСНИЛС = ОбщегоНазначения.СкопироватьРекурсивно(РеквизитыПоискаФизическогоЛица);
								РеквизитыПоискаФизическогоЛица.Вставить("СНИЛС", СНИЛСПредставителя);
								РеквизитыПоискаФЛСНИЛСБезРазделителей.Вставить("СНИЛС", СНИЛСБезРазделителей);
								МассивРеквизитовПоискаФЛ.Добавить(РеквизитыПоискаФЛСНИЛСБезРазделителей);
								МассивРеквизитовПоискаФЛ.Добавить(РеквизитыПоискаФЛБезСНИЛС);
							КонецЕсли;
						КонецЕсли;
					...
					ФизическоеЛицоСсылка = ОбщегоНазначенияЭДКО.НайтиЭлементИБ(Справочники.ФизическиеЛица.ПустаяСсылка(),
						МассивРеквизитовПоискаФЛ);
				КонецЕсли;
				
				Если НЕ ЗначениеЗаполнено(ФизическоеЛицоСсылка) И ЗначениеЗаполнено(Выборка.ФИО) Тогда
					ФИОПредставителя = СокрЛП(Выборка.ФИО);
					ФизическоеЛицоСсылка = НайтиФизическоеЛицо(ФИОПредставителя);
					Если НЕ ЗначениеЗаполнено(ФизическоеЛицоСсылка) И ТолькоФизическиеЛица = "РазрешитьВозвратФИО" Тогда
						ФизическоеЛицоСсылка = ФИОПредставителя;
					КонецЕсли;
				КонецЕсли;
	...
	Если Результат.Количество() = 0 Тогда
		...
		Если ТолькоФизическиеЛица = Истина ИЛИ ТолькоФизическиеЛица = "РазрешитьВозвратФИО" Тогда
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УстановитьПараметрСеансаРезультатыОбработкиМЧДСКО");
	Методы.Вставить("УстановитьПараметрСеансаОрганизацииПринятыхЗПЭД");
	Методы.Вставить("УстановитьПараметрСеансаТекущиеУчетныеЗаписиНалогоплательщика");
	Методы.Вставить("УстановитьПараметрСеансаРезультатАвторизацииНаСервереМЧДРРСКО");
	Методы.Вставить("ЗаполнитьРегистрЖурналОтправокВКонтролирующиеОрганы");
КонецПроцедуры

Функция СвойстваСертификатаСФР(ДанныеСертификата) Экспорт
	
	СвойстваСобытия =
		ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступилоРасширенная("Треб.к исп.ключа для СФР");
	ДатаНачалаСобытия = ?(СвойстваСобытия.СобытиеНеНайдено ИЛИ СвойстваСобытия.ОшибкаПолученияДанных,
		Неопределено, СвойстваСобытия.ДатаНачала);
	Если ДатаНачалаСобытия = '30000101' Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	НастройкиВызова = Новый Структура("ВозвращатьИсключения", Ложь);
	Возврат КриптографияЭДКОСлужебныйВызовСервера.ПолучитьСвойстваСертификата(ДанныеСертификата, НастройкиВызова);
...
Функция ПолнотекстовыйПоискВидовДокументов(СтрокаИлиМассив) Экспорт
	...
	ПарыСимволовЗамен = "З3з33З";
	КоличествоСтрокПоиска = СтрокиПоиска.Количество();
	Для ИндексСтрокиПоиска = 0 По КоличествоСтрокПоиска - 1 Цикл
		СтрокаПоиска = СтрокиПоиска[ИндексСтрокиПоиска];
		ПозицияСимвола = СтрНайти(СтрокаПоиска, "З");
		ПозицияСимвола = 1;
		Пока ПозицияСимвола < СтрДлина(ПарыСимволовЗамен) Цикл
			СимволПоиска = Сред(ПарыСимволовЗамен, ПозицияСимвола, 1);
			ПозицияЗамены = СтрНайти(СтрокаПоиска, СимволПоиска);
			Если ПозицияЗамены <> 0 Тогда
				СимволЗамены = Сред(ПарыСимволовЗамен, ПозицияСимвола + 1, 1);
				НоваяСтрокаПоиска = Лев(СтрокаПоиска, ПозицияЗамены - 1) + СимволЗамены + Сред(СтрокаПоиска, ПозицияЗамены + 1);
				СтрокиПоиска.Добавить(НоваяСтрокаПоиска);
			КонецЕсли;
			ПозицияСимвола = ПозицияСимвола + 2;
		КонецЦикла;
		Если СтрНайти(СтрокаПоиска, " ") <> 0 Тогда
			НоваяСтрокаПоиска = СтрЗаменить(СтрокаПоиска, " ", "");
			СтрокиПоиска.Добавить(НоваяСтрокаПоиска);
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого СтрокаПоиска Из СтрокиПоиска Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
			Продолжить;
		КонецЕсли;
		СтрокаПоискаПриведенная = СтрЗаменить(СтрокаПоиска, "(", " ");
		СтрокаПоискаПриведенная = СтрЗаменить(СтрокаПоискаПриведенная, ")", " ");
		СтрокаПоискаПриведенная = СокрЛП(СтрокаПоискаПриведенная);
		
		Если СтрЧислоВхождений(СтрокаПоискаПриведенная, """") % 2 <> 0 Тогда
			СтрокаПоискаПриведенная = СтрокаПоискаПриведенная + """";
		КонецЕсли;
		Если СтрЗаменить(СтрокаПоискаПриведенная, """", "") = ""
			ИЛИ нрег(СтрокаПоискаПриведенная) = "документ" ИЛИ нрег(СтрокаПоискаПриведенная) = "в" Тогда
			Продолжить;
		КонецЕсли;
		
		СписокПоиска = ПолнотекстовыйПоиск.СоздатьСписок(СтрокаПоискаПриведенная);
		
		ЭтоПоискПоКоду = СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаПоискаПриведенная);
		СписокПоиска.ПорогНечеткости = ?(ЭтоПоискПоКоду ИЛИ СтрДлина(СтрокаПоискаПриведенная) <= 2, 0, 20);
...
Функция ЭтоКонфигурацияОтчетность() Экспорт
	Возврат Метаданные.Имя = "Отчетность";
КонецФункции
