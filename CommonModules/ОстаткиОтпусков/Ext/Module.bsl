...
Функция ОписаниеТаблицыДанных() Экспорт
	...
	// Для расчета компенсации отпуска при сокращении работника
	СписокОснований = Новый Массив;
	СписокОснований.Добавить(Тип("СправочникСсылка.ОснованияУвольнения"));
	СписокОснований.Добавить(Тип("СправочникСсылка.ОснованияУвольненияПользовательские"));
	...
	КонецЕсли;
	ТаблицаДанных.Колонки.Добавить("ОснованиеУвольнения",			Новый ОписаниеТипов(СписокОснований));
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СформироватьУведомленияОНачалеОтпуска", Истина);
	Методы.Вставить("ЗаполнитьРегистрПериодыОтпусковИспр");
	Методы.Вставить("ЗаполнитьТаблицуПереносовВПереносеОтпуска");
	Методы.Вставить("ЗаполнитьПеренесеннуюДатаНачалаВРегистреПлановыеЕжегодныеОтпуска");
	Методы.Вставить("ЗаполнитьДокументОтпускаСотрудниковВРегистреПериодыОтпусков");
	Методы.Вставить("УточнитьНаименованиеОтпускаНаСанаторноКурортноеЛечение");
	Методы.Вставить("ЗаполнитьПеренесеннуюДатуОкончанияИКоличествоДнейВРегистреПлановыеЕжегодныеОтпуска");
	Методы.Вставить("ЗаполнитьДокументОтпускаСотрудниковВРегистреПериодыОтпусковИспр");
	Методы.Вставить("ЗаполнитьТаблицуСотрудникиДокументаОтпускаСотрудников");
	Методы.Вставить("ЗаполнитьРегистрПериодыОтпусков");
	Методы.Вставить("ЗаполнитьДокументОтпускаСотрудниковВРегистреДанныеОтпусковКарточкиСотрудника");
	Методы.Вставить("ЗаполнитьОснованияПолнойКомпенсацииОтпусковСотрудников");
	Методы.Вставить("ЗаполнитьПримечаниеВГрафикеОтпусков");
	Методы.Вставить("ЗаполнитьПримечаниеВРегистреПлановыеЕжегодныеОтпуска");
	Методы.Вставить("ЗаполнитьРегистрДанныеОтпусковКарточкиСотрудника");
КонецПроцедуры
				...
				Для Каждого ВидОтпуска Из ВидыОтпусковСотрудника Цикл
					...
					Для Каждого РабочийГод Из Результат.ЗаработанныеПрава Цикл
						...
						РабочийГодПредставление = РабочийГодПредставление + ?(РабочийГодПредставление = "", "", "; ")
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 дн. за период %2 - %3'"), КоличествоДнейЗаПериод, Формат(РабочийГод.ДатаНачала, ФорматДаты), Формат(РабочийГод.ДатаОкончания, ФорматДаты));
					КонецЦикла;
					Если КоличествоДней > 0 Тогда
						РабочийГодПредставление = РабочийГодПредставление + ?(РабочийГодПредставление = "", "", "; ")
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 дн. за будущие периоды'"), КоличествоДней);
					КонецЕсли;
					Если РабочийГодПредставление <> "" Тогда
						ОбластьМакета = Макет.ПолучитьОбласть("Строка");
						ОбластьМакета.Параметры.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: %2'"), ВидОтпуска, РабочийГодПредставление);
						ТабДокумент.Вывести(ОбластьМакета);
					КонецЕсли;
				КонецЦикла;
...
Функция ОписаниеПараметровДляОстаткиОтпусков() Экспорт
	...
	СтруктураПараметров.Вставить("ОснованияУвольнения");
...
Функция ОстаткиОтпусков(ПараметрыОстатков) Экспорт
	...
	ДатаОстатков = ?(ПараметрыОстатков.ЗаработанныеПраваНаГодВперед, ДобавитьМесяц(ПараметрыОстатков.ДатаОстатков, 16), ПараметрыОстатков.ДатаОстатков);
    РасчетЗаработанныхОтпусков(
		...
		ПараметрыОстатков.ОснованияУвольнения);
...
Функция ОстатокОтпускаСотрудникаНаДату(Сотрудник, ДатаОстатков = '00010101', ИсключаемыйРегистратор = Неопределено, ЭтоРасчетПриУвольнении = Ложь,
	ОтраслевыеОсобенности = Неопределено, ОснованиеУвольнения = Неопределено) Экспорт
	...
	Если ОснованиеУвольнения <> Неопределено Тогда 
		ОснованияУвольнения = Новый Соответствие;
		ОснованияУвольнения.Вставить(Сотрудник, ОснованиеУвольнения);
		ПараметрыОтбора.ОснованияУвольнения = ОснованияУвольнения;
	КонецЕсли;
...
Процедура СформироватьДвиженияОснованийУвольненияСотрудников(Движения, ОснованияУвольнения, ЗаписыватьСразу = Ложь) Экспорт 
	
	Движения.ОснованияУвольненияСотрудников.Записывать = Истина;
	
	Для Каждого ДанныеУвольнения Из ОснованияУвольнения Цикл
		ЗаполнитьЗначенияСвойств(Движения.ОснованияУвольненияСотрудников.Добавить(), ДанныеУвольнения);
	КонецЦикла;
	
	Если ЗаписыватьСразу Тогда
		Движения.ОснованияУвольненияСотрудников.БлокироватьДляИзменения = Истина;
		Движения.ОснованияУвольненияСотрудников.Записать();
		Движения.ОснованияУвольненияСотрудников.Записывать = Ложь;
	КонецЕсли;
...
Процедура РасчетЗаработанныхОтпусков(МассивСотрудников = Неопределено, Знач ДатаРасчета = Неопределено, ЭтоРасчетПриУвольнении = Ложь,
	ОтраслевыеОсобенности = Неопределено, ОснованияУвольнения = Неопределено) Экспорт
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.35.42";
	Обработчик.Процедура = "ОстаткиОтпусков.ЗаполнитьОснованияПолнойКомпенсацииОтпусковСотрудников";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("df9bb19d-fe0e-4682-93cb-3bc1f45635cc");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистра Основания полной компенсации отпусков сотрудников.'");
	Обработчик.НачальноеЗаполнение = Истина;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.9";
	Обработчик.Процедура = "ОстаткиОтпусков.УточнитьНаименованиеОтпускаНаСанаторноКурортноеЛечение";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("709ee388-e286-4d65-a551-7f33c215a59c");
	Обработчик.Комментарий = НСтр("ru = 'Уточнение наименования отпуска на СКЛ.'");
...
Процедура ЗаполнитьОснованияПолнойКомпенсацииОтпусковСотрудников(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТОснованияУвольнения
		|ИЗ
		|	Справочник.ОснованияУвольнения КАК ОснованияУвольнения
		|ГДЕ
		|	ОснованияУвольнения.ДокументОснование = ЗНАЧЕНИЕ(Перечисление.НормативныеДокументыОснованийКадровыхПриказов.ТКРФ)
		|	И ОснованияУвольнения.Статья = ""77""
		|	И ОснованияУвольнения.Пункт = ""8""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ОснованияУвольнения.ДокументОснование = ЗНАЧЕНИЕ(Перечисление.НормативныеДокументыОснованийКадровыхПриказов.ТКРФ)
		|	И ОснованияУвольнения.Статья = ""81""
		|	И ОснованияУвольнения.Пункт В (""1"", ""2"")
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ОснованияУвольнения.ДокументОснование = ЗНАЧЕНИЕ(Перечисление.НормативныеДокументыОснованийКадровыхПриказов.ТКРФ)
		|	И ОснованияУвольнения.Статья = ""83""
		|	И ОснованияУвольнения.Пункт В (""1"", ""5"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1000
		|	ОснованияУвольнения.Ссылка КАК ОснованиеУвольнения
		|ИЗ
		|	ВТОснованияУвольнения КАК ОснованияУвольнения
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОснованияПолнойКомпенсацииОтпусковСотрудников КАК ОснованияПолнойКомпенсацииОтпусковСотрудников
		|		ПО ОснованияУвольнения.Ссылка = ОснованияПолнойКомпенсацииОтпусковСотрудников.ОснованиеУвольнения
		|ГДЕ
		|	ОснованияПолнойКомпенсацииОтпусковСотрудников.ОснованиеУвольнения ЕСТЬ NULL";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрСведений.ОснованияПолнойКомпенсацииОтпусковСотрудников", "ОснованиеУвольнения", Выборка.ОснованиеУвольнения) Тогда
			Продолжить;
		КонецЕсли;
		
		НаборЗаписей = РегистрыСведений.ОснованияПолнойКомпенсацииОтпусковСотрудников.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ОснованиеУвольнения.Установить(Выборка.ОснованиеУвольнения);
...
Процедура УточнитьНаименованиеОтпускаНаСанаторноКурортноеЛечение(ПараметрыОбновления = Неопределено) Экспорт
	
	СвойстваВидовОтпусков = Справочники.ВидыОтпусков.СвойстваПредопределенныхВидовОтпусков();
	ОписаниеВидаОтпуска = СвойстваВидовОтпусков.ОтпускНаСанаторноКурортноеЛечение;
	...
	Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "Справочник.ВидыОтпусков", "Ссылка", ВидОтпускаСсылка) Тогда
		Возврат;
	КонецЕсли;
	
	ВидОтпускаОбъект = ВидОтпускаСсылка.ПолучитьОбъект();
	Если ВидОтпускаОбъект.Наименование = ОписаниеВидаОтпуска.Наименование Тогда 
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ЗавершитьОбновлениеДанных(ПараметрыОбновления);
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.УстановитьПараметрОбновления(ПараметрыОбновления, "ОбработкаЗавершена", Истина);
		Возврат;
	КонецЕсли;
	
	ВидОтпускаОбъект.Наименование = ОписаниеВидаОтпуска.Наименование;
	
	ОбновлениеИнформационнойБазы.ЗаписатьДанные(ВидОтпускаОбъект);
	ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ЗавершитьОбновлениеДанных(ПараметрыОбновления);
...
Функция ДобавитьПолеСведенийОЕжегодныхОтпусках(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Процедура СоздатьВТГрафикОтпусковДляЛичныхКабинетов(МенеджерВременныхТаблиц) Экспорт 
	...
	Запрос.Текст = 
		...
		|	ПлановыеЕжегодныеОтпуска.ВидОтпуска КАК ВидОтпуска,
		|	ПлановыеЕжегодныеОтпуска.ДатаНачала КАК ИсходнаяДатаНачала
...
Процедура СписатьОстатокОтпуска(ОстаткиОтпусковСотрудника, ОсталосьСписать, ВидыОтпусков = Неопределено)
	...
	Для Каждого СтрокаОстатка Из ОстаткиОтпусковСотрудника Цикл
		...
		Если ПродолжительностьОтпуска = Неопределено Тогда
			ПродолжительностьОтпуска = 0;
		КонецЕсли;
		КоличествоДней = Мин(ОсталосьСписать, ?(Цел(СтрокаОстатка.Дни) = СтрокаОстатка.Дни, СтрокаОстатка.Дни, Цел(СтрокаОстатка.Дни) + 1));
...
Функция ДоступноеКоличествоДнейКомпенсацииОтпуска(Остатки, ДатаАктуальности) Экспорт
	...
	Для Каждого СтрокаОстатков Из Остатки Цикл 
		Если СтрокаОстатков.ВидОтпуска = Основной Или СтрокаОстатков.ВидОтпуска = ОтпускЗаВредность Тогда
			...
			Продолжить;
		КонецЕсли;
		Если СтрокаОстатков.ВидОтпуска = ОтпускЧАЭС Тогда
			Продолжить;
		КонецЕсли;
...
Функция ДанныеГрафикаОтпусковДляЛичныхКабинетов(МенеджерВременныхТаблиц, ДатаАктуальности) Экспорт
	
	ГрафикОтпусков = Новый Структура("ДанныеГрафиков, ДанныеГрафиковПоРабочимМестам");
	
	СоздатьВТГрафикОтпусковДляЛичныхКабинетов(МенеджерВременныхТаблиц);
	...
	ОписательВременныхТаблиц = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеСотрудников(Запрос.МенеджерВременныхТаблиц, "ВТОтборСотрудников");
	
	КадровыеДанные = "ФизическоеЛицо, ВидЗанятости, ДатаУвольнения";
	...
	Запрос.Текст = 
		...
		|				И КадровыеДанные.ДатаУвольнения < &ДатаАктуальности
		|			ТОГДА 0
		...
		|ВЫБРАТЬ
		|	СотрудникиПриоритет.ФизическоеЛицо КАК ФизическоеЛицо,
		|	МАКСИМУМ(СотрудникиПриоритет.Сотрудник) КАК Сотрудник
	...
	Запрос.Текст = 
		...
		|	ГрафикОтпусков.ИсходнаяДатаНачала КАК ИсходнаяДатаНачала,
		|	ВЫБОР
		|		КОГДА ОсновныеСотрудники.Сотрудник ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК ОсновнойСотрудник
		|ИЗ
		|	ВТГрафикОтпусков КАК ГрафикОтпусков
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОсновныеСотрудники КАК ОсновныеСотрудники
		|		ПО ГрафикОтпусков.Сотрудник = ОсновныеСотрудники.Сотрудник
		|
		|УПОРЯДОЧИТЬ ПО
		|	ФизическоеЛицо,
		|	Сотрудник,
	...
	ДанныеГрафиковПоРабочимМестам = Новый Соответствие;
	Пока Выборка.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
		ПериодыГрафика = Новый Массив;
		ЭлементГрафика = Неопределено;
		ПериодыГрафикаПоРабочимМестам = Новый Массив;
		ЭлементГрафикаПоРабочимМестам = Неопределено;
		ВидыОтпусков = Новый Соответствие;
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.ДатаНачала) И ЗначениеЗаполнено(Выборка.ДатаОкончания) Тогда
				ЭлементГрафикаПоРабочимМестам = Новый Структура(
					"Сотрудник, 
					|ВидОтпуска,
					|ДатаНачала,
					|ДатаОкончания,
					|КоличествоДней,
					|ИсходнаяДатаНачала");
				ЭлементГрафикаПоРабочимМестам.Сотрудник = Выборка.Сотрудник;
				ЭлементГрафикаПоРабочимМестам.ВидОтпуска = Выборка.ВидОтпуска;
				ЭлементГрафикаПоРабочимМестам.ДатаНачала = Выборка.ДатаНачала;
				ЭлементГрафикаПоРабочимМестам.ДатаОкончания = Выборка.ДатаОкончания;
				ЭлементГрафикаПоРабочимМестам.КоличествоДней = Выборка.КоличествоДней;
				ЭлементГрафикаПоРабочимМестам.ИсходнаяДатаНачала = Выборка.ИсходнаяДатаНачала;
				ПериодыГрафикаПоРабочимМестам.Добавить(ЭлементГрафикаПоРабочимМестам);
			КонецЕсли;
			Если Выборка.ОсновнойСотрудник Тогда 
				Если ЗначениеЗаполнено(Выборка.ДатаНачала) И ЗначениеЗаполнено(Выборка.ДатаОкончания) Тогда
					Если ЭлементГрафика <> Неопределено
						И ВидыОтпусков[Выборка.ВидОтпуска] = Неопределено
						И Выборка.ДатаНачала = ЭлементГрафика.ДатаОкончания + 86400 Тогда
						ЭлементГрафика.ДатаОкончания = Выборка.ДатаОкончания;
						ВидыОтпусков.Вставить(Выборка.ВидОтпуска, Истина);
						Продолжить;
					КонецЕсли;
		...
		ДанныеГрафиковПоРабочимМестам.Вставить(Выборка.ФизическоеЛицо, ПериодыГрафикаПоРабочимМестам);
	КонецЦикла;
	
	ГрафикОтпусков.ДанныеГрафиков = ДанныеГрафиков;
	ГрафикОтпусков.ДанныеГрафиковПоРабочимМестам = ДанныеГрафиковПоРабочимМестам;
	...
	Для Каждого СтрокаДанных Из ДополненнаяТаблицаДанных Цикл
		...
		Для Каждого СтрокаРасшифровки Из РасшифровкаЗаработанныхПравСотрудника Цикл 
			...
			НовоеСобытие.Событие = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 - расчет заработанных прав'"), СтрокаРасшифровки.ВидЕжегодногоОтпуска);
...
Функция ЗапросВТФактическиеОтпускаСотрудников(ОписательВременныхТаблиц, ТолькоРазрешенные) Экспорт
	...
	|				И ФактическиеОтпуска.ДатаРегистрации <= ФактическиеОтпускаСторно.ДатаРегистрации   
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
...
Процедура СоздатьВТНеРассчитанныхСотрудников(ТаблицаДанных, МенеджерВременныхТаблиц, РассчитатьПравоНаОтпускаВсемСотрудникам)
	...
	Запрос.Текст =
		...
		|	ТаблицаДанных.ОснованиеУвольнения,
		...
		|	МАКСИМУМ(ВТТЗ.ОснованиеУвольнения) КАК ОснованиеУвольнения,
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ОснованияУвольненияСотрудников.Сотрудник КАК Сотрудник,
		|	МАКСИМУМ(ОснованияУвольненияСотрудников.ДатаУвольнения) КАК ДатаУвольнения
		|ПОМЕСТИТЬ ВТОснованияУвольненияДаты
		|ИЗ
		|	РегистрСведений.ОснованияУвольненияСотрудников КАК ОснованияУвольненияСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТаблицаДанных КАК ТаблицаДанных
		|		ПО ОснованияУвольненияСотрудников.Сотрудник = ТаблицаДанных.Сотрудник
		|
		|СГРУППИРОВАТЬ ПО
		|	ОснованияУвольненияСотрудников.Сотрудник
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ОснованияУвольненияСотрудников.Сотрудник КАК Сотрудник,
		|	МАКСИМУМ(ОснованияУвольненияСотрудников.ОснованиеУвольнения) КАК ОснованиеУвольнения
		|ПОМЕСТИТЬ ВТОснованияУвольненияСотрудников
		|ИЗ
		|	РегистрСведений.ОснованияУвольненияСотрудников КАК ОснованияУвольненияСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОснованияУвольненияДаты КАК ОснованияУвольненияДаты
		|		ПО ОснованияУвольненияСотрудников.Сотрудник = ОснованияУвольненияДаты.Сотрудник
		|			И ОснованияУвольненияСотрудников.ДатаУвольнения = ОснованияУвольненияДаты.ДатаУвольнения
		|
		|СГРУППИРОВАТЬ ПО
		|	ОснованияУвольненияСотрудников.Сотрудник";
	...
	Если РассчитатьПравоНаОтпускаВсемСотрудникам Тогда
		Запрос.Текст = Запрос.Текст + ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов() +
			...
			|	ВЫБОР
			|		КОГДА ТаблицаДанных.ЭтоРасчетПриУвольнении
			|			ТОГДА ТаблицаДанных.ОснованиеУвольнения
			|		ИНАЧЕ ЕСТЬNULL(ОснованияУвольненияСотрудников.ОснованиеУвольнения, ЗНАЧЕНИЕ(Справочник.ОснованияУвольнения.ПустаяСсылка))
			|	КОНЕЦ КАК ОснованиеУвольнения,
			|	ТаблицаДанных.ЭтоРасчетПриУвольнении КАК ЭтоРасчетПриУвольнении
			|ПОМЕСТИТЬ ВТНеРассчитанныеСотрудники
			|ИЗ
			|	ВТТаблицаДанных КАК ТаблицаДанных
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОснованияУвольненияСотрудников КАК ОснованияУвольненияСотрудников
			|		ПО ТаблицаДанных.Сотрудник = ОснованияУвольненияСотрудников.Сотрудник
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Сотрудник,
			|	ДатаОстатков
	...
	Иначе
		Запрос.Текст = Запрос.Текст + ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов() +
			...
			|	ТаблицаДанных.ДатаОкончания КАК ДатаОстатков,
			|	ВЫБОР
			|		КОГДА РегистрНеРассчитанныеСотрудники.ЭтоРасчетПриУвольнении
			|			ТОГДА ТаблицаДанных.ОснованиеУвольнения
			|		ИНАЧЕ ЕСТЬNULL(ОснованияУвольненияСотрудников.ОснованиеУвольнения, ЗНАЧЕНИЕ(Справочник.ОснованияУвольнения.ПустаяСсылка))
			|	КОНЕЦ КАК ОснованиеУвольнения,
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОснованияУвольненияСотрудников КАК ОснованияУвольненияСотрудников
			|		ПО РегистрНеРассчитанныеСотрудники.Сотрудник = ОснованияУвольненияСотрудников.Сотрудник
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Сотрудник,
			|	ДатаОстатков
	...
	ИменаВТ.Добавить("ВТОснованияУвольненияДаты");
	ИменаВТ.Добавить("ВТОснованияУвольненияСотрудников");
...
Процедура СоздатьВТИсходныхДанныхДляРасчетаОстатков(ТаблицаДанных, МенеджерВременныхТаблиц)
	...
	Если ПолучитьФункциональнуюОпцию("ПрекращатьДополнительныеОтпускаПриПриостановкеТрудовогоДоговора") Тогда 
		ОтборПоПериодам.Добавить(Перечисления.СостоянияСотрудника.ТрудовойДоговорПриостановлен);
	КонецЕсли;
	...
	ДатаОстатков = КонецДня(?(ТаблицаДанных.Количество() > 0, ТаблицаДанных[0].ДатаОстатков, ТекущаяДатаСеанса()));
	...
	Запрос.УстановитьПараметр("ТрудовойДоговорПриостановлен", Перечисления.СостоянияСотрудника.ТрудовойДоговорПриостановлен);
	...
	|					КОГДА СостоянияСотрудников.Состояние = &ТрудовойДоговорПриостановлен
	|						ТОГДА &ДатаОстатков
	|					ИНАЧЕ СостоянияСотрудников.Окончание
	|				КОНЕЦ
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ТрудовойДоговорПриостановлен,
	|	СостоянияСотрудников.Состояние КАК Состояние
	...
	|	СотрудникиБезПраваНаОтпуск.ОснованиеУвольнения КАК ОснованиеУвольнения,
	|	ВЫБОР
	|		КОГДА ОснованияПолнойКомпенсации.ОснованиеУвольнения ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПолнаяКомпенсацияПриУвольнении,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОснованияПолнойКомпенсацииОтпусковСотрудников КАК ОснованияПолнойКомпенсации
	|		ПО СотрудникиБезПраваНаОтпуск.ОснованиеУвольнения = ОснованияПолнойКомпенсации.ОснованиеУвольнения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПериодыПрерывающиеПравоНаОтпуск.ТрудовойДоговорПриостановлен КАК ТрудовойДоговорПриостановлен,
...
Функция ИсходныеДанныеДляРасчетаОстатков(ТаблицаДанных, МенеджерВременныхТаблиц, ОтраслевыеОсобенности = Неопределено)
	...
	|	Сотрудники.ПолнаяКомпенсацияПриУвольнении КАК ПолнаяКомпенсацияПриУвольнении,
	...
	|	Отклонения.ТрудовойДоговорПриостановлен КАК ТрудовойДоговорПриостановлен,
...
Функция ЗаработанныеОтпуска(ИсходныеДанные, МенеджерВременныхТаблиц)
	...
	|	ИсходныеДанныеОтклонения.ТрудовойДоговорПриостановлен
	|ПОМЕСТИТЬ ВТОтклонения
	|ИЗ
...
Функция РассчитатьПравоНаОтпуск(СотрудникБезПрава, СтрокаВидаОтпуска, СтруктураПараметров, ПерерывыПредоставленияРаздвигаютГод = Истина)
	...
	ОтборОтклонений = Новый Структура("Сотрудник", СотрудникБезПрава.Сотрудник);
	Если СтрокаВидаОтпуска.ОсновнойОтпуск Тогда
		ОтборОтклонений.Вставить("ТрудовойДоговорПриостановлен", Ложь);
	КонецЕсли;
	...
	ОтклоненияПоСотруднику = ТаблицаОтклонений.НайтиСтроки(ОтборОтклонений);
		...
		Для Каждого ПериодПредоставленияОтпуска Из ПериодыПредоставленияОтпусков Цикл
			Если ПериодыПредоставленияОтпусков.Индекс(ПериодПредоставленияОтпуска) <> 0 Тогда
				Если КонецДня(ПрошлыйПериод.ДатаОкончания) + 1 <> НачалоДня(ПериодПредоставленияОтпуска.ДатаНачала) Тогда
					...
					НовоеОтклонение.ТрудовойДоговорПриостановлен = Ложь;
...
Процедура РассчитатьЗаработанныеОтпускаСпискуСотрудников(СписокСотрудников, ДатаРасчета, ЭтоРасчетПриУвольнении = Ложь,
	...
	Для Каждого Сотрудник Из СписокСотрудников Цикл
		...
		Строка.ЭтоРасчетПриУвольнении	= ЭтоРасчетПриУвольнении;
		Если ОснованияУвольнения <> Неопределено Тогда 
			Строка.ОснованиеУвольнения = ОснованияУвольнения[Сотрудник];
		КонецЕсли;
	КонецЦикла;
...
Функция СобытияОтклоненийСотрудника(МенеджерВременныхТаблиц, Сотрудник)
	...
	Запрос.Текст = 
		...
		|	И НЕ(Отклонения.ТрудовойДоговорПриостановлен
		|				И ЕСТЬNULL(ЗаработанныеПраваНаОтпуска.ВидЕжегодногоОтпуска, ЗНАЧЕНИЕ(Справочник.ВидыОтпусков.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ВидыОтпусков.Основной))
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаСобытия,
...
Процедура ОчиститьПредыдущиеДвиженияСтажевыхОтпусков(Источник, Отказ, Замещение) Экспорт
	...
	СоздатьВТСведенияОДвиженииПоложенныхОтпусковЗависящихОтСтажа(Источник, МенеджерВременныхТаблицТекущий, Замещение);
...
Процедура ДополнитьДвиженияСтажевыхОтпусков(Источник, Отказ, Замещение) Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Если ДополнитьНаборЗаписейПоложенныеВидыЕжегодныхОтпусковВторичнымиЗаписями(Источник, МенеджерВременныхТаблицТекущий, Замещение) Тогда
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			СоздатьВТСведенияОДвиженииПоложенныхОтпусковЗависящихОтСтажа(Набор, МенеджерВременныхТаблицПоНаборуЗаписей, Замещение);
			Если Не ЗарплатаКадры.ВТСодержитСтроки(МенеджерВременныхТаблицПоНаборуЗаписей, "ВТСведенияОДвиженииПоложенныхОтпусковЗависящихОтСтажа") Тогда
				Продолжить;
			КонецЕсли; 
			Если ДополнитьНаборЗаписейПоложенныеВидыЕжегодныхОтпусковВторичнымиЗаписями(Набор, МенеджерВременныхТаблицПоНаборуЗаписей, Замещение) Тогда
				Набор.ДополнительныеСвойства.Вставить("ОтключитьПроверкуДатыЗапретаИзменения", Истина);
				Набор.ДополнительныеСвойства.Вставить("НеДобавлятьВторичныеЗаписи", Истина);
				Набор.Записать();
			КонецЕсли; 
...
Процедура СоздатьВТСведенияОДвиженииПоложенныхОтпусковЗависящихОтСтажа(НаборЗаписей, МенеджерВременныхТаблиц, Замещение) Экспорт
	...
	Если НаборЗаписей.Отбор.Найти("Регистратор") <> Неопределено И НаборЗаписей.Отбор.Регистратор.Использование Тогда 
		ЗаписываемыйНабор.ЗаполнитьЗначения(НаборЗаписей.Отбор.Регистратор.Значение, "Регистратор"); 
	КонецЕсли;
	...
	Для Каждого Регистратор Из Регистраторы Цикл
		...
		НайденныеСтроки = ЗаписываемыйНабор.НайтиСтроки(Отбор);
		Если ВремяРегистрацииДокумента = Неопределено Тогда
			СотрудникиДаты = ЗаписываемыйНабор.Скопировать(НайденныеСтроки, "Период, Сотрудник");
	...
	Запрос.Текст =
		...
		|	ЗаписываемыйНабор.Регистратор КАК Регистратор,
...
//
Функция ДополнитьНаборЗаписейПоложенныеВидыЕжегодныхОтпусковВторичнымиЗаписями(НаборЗаписей, МенеджерВременныхТаблиц, Замещение) Экспорт
...
Процедура УдалитьДублиПрав(МассивПрав)
	...
	Для Каждого ПравоНаОтпуск Из МассивПрав Цикл
		...
		Если ПредыдущееПраво.ВидЕжегодногоОтпуска <> ПравоНаОтпуск.ВидЕжегодногоОтпуска Тогда
			Продолжить;
		КонецЕсли;
		Если ПравоНаОтпуск.КоличествоДнейОтпускаВГод = ПредыдущееПраво.КоличествоДнейОтпускаВГод
			И ПравоНаОтпуск.Предоставляется = ПредыдущееПраво.Предоставляется Тогда
...
Процедура РассчитатьКоличествоДнейОтпуска(СотрудникБезПрава, СтрокаВидаОтпуска, РабочиеГода, МассивПравНаОтпуск, ТаблицаПереквалификацииДней)
	...
	Если СотрудникБезПрава.ПолнаяКомпенсацияПриУвольнении И ПолучитьФункциональнуюОпцию("ПолнаяКомпенсацияОтпускаПриУвольненииПоВыбраннымОснованиям") Тогда 
		ПоследнийРабочийГод = РабочиеГода[РабочиеГода.Количество() - 1];
		ОтработаноМесяцев = ПоследнийРабочийГод.РабочиеМесяцы.Количество();
		Если ОтработаноМесяцев > 5 И ОтработаноМесяцев < 12 Тогда 
			Если СотрудникБезПрава.ЭтоРасчетПриУвольнении 
				Или (ЗначениеЗаполнено(СотрудникБезПрава.ДатаУвольнения) И СотрудникБезПрава.ДатаОстатков >= СотрудникБезПрава.ДатаУвольнения) 
				Или (ЗначениеЗаполнено(СотрудникБезПрава.ДатаУвольнения) И ПоследнийРабочийГод.ДатаОкончания >= СотрудникБезПрава.ДатаУвольнения) Тогда 
				ПоследнийМесяц = ПоследнийРабочийГод.РабочиеМесяцы[ОтработаноМесяцев - 1];
				ПоследнийМесяц.КоличествоДней = ПоследнийМесяц.КоличествоДней + ПоследнийМесяц.КоличествоДней * (12 - ОтработаноМесяцев);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОстаткиОтпусковСУчетомБудущихПериодов(ПараметрыОстатков)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И ЗаработанныеПраваНаОтпуска.Период <= ДОБАВИТЬКДАТЕ(&ДатаОстатков, МЕСЯЦ, 16)
...
Функция ИнформацияОбОтпусках(Сотрудники, ТекущиеОстаткиОтпусков, ВРазрезеСотрудников = Ложь)
	...
	Запрос.Текст =
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТФизическиеЛица КАК ФизическиеЛица
		|		ПО ЗаявкаСотрудникаОтпуск.ФизическоеЛицо = ФизическиеЛица.ФизическоеЛицо
		|			И ЗаявкаСотрудникаОтпуск.РезультатВыполнения <> ЗНАЧЕНИЕ(Перечисление.РезультатыВыполненияЗаявокКабинетСотрудника.Отклонена)
		|			И ЗаявкаСотрудникаОтпуск.РезультатВыполнения <> ЗНАЧЕНИЕ(Перечисление.РезультатыВыполненияЗаявокКабинетСотрудника.Аннулирована)
		...
		|ГДЕ
		|	ОтменяющаяЗаявка.Номер ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ДанныеУведомленийОНачалеОтпуска(МенеджерВременныхТаблиц, НачалоПериода, ОкончаниеПериода, ИсключитьОформленныеОтпуска = Ложь) Экспорт 
	
	ДанныеУведомлений = Новый Массив;
	
	Запрос = Новый Запрос;
	
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	Запрос.УстановитьПараметр("ДатаАктуальности", ДобавитьМесяц(НачалоГода(НачалоПериода), -12));
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ПлановыеЕжегодныеОтпуска КАК ПлановыеЕжегодныеОтпуска
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиОтбор КАК Отбор
		|		ПО ПлановыеЕжегодныеОтпуска.Сотрудник = Отбор.Сотрудник
		|			И (ВЫБОР
		|				КОГДА ПлановыеЕжегодныеОтпуска.Перенесен
		|					ТОГДА ПлановыеЕжегодныеОтпуска.ПеренесеннаяДатаНачала
		|				ИНАЧЕ ПлановыеЕжегодныеОтпуска.ДатаНачала
	...
	Если ИсключитьОформленныеОтпуска Тогда 
		...
		Запрос.Текст = 
			...
			|	ПлановыеЕжегодныеОтпуска.ДатаОкончания КАК ДатаОкончания,
			|	ПлановыеЕжегодныеОтпуска.КоличествоДней КАК КоличествоДней,
			|	ПлановыеЕжегодныеОтпуска.ИсходнаяДатаНачала КАК ИсходнаяДатаНачала,
			|	ПлановыеЕжегодныеОтпуска.ДокументПланирования КАК ДокументПланирования
			|ИЗ
			|	ВТПлановыеЕжегодныеОтпуска КАК ПлановыеЕжегодныеОтпуска
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПериодыОтпусков КАК ПериодыОтпусков
			|		ПО ПлановыеЕжегодныеОтпуска.Сотрудник = ПериодыОтпусков.Сотрудник
			|			И ПлановыеЕжегодныеОтпуска.ВидОтпуска = ПериодыОтпусков.ВидОтпуска
			|			И ПлановыеЕжегодныеОтпуска.ДатаНачала = ПериодыОтпусков.ДатаНачала
			|			И ПлановыеЕжегодныеОтпуска.ДатаОкончания = ПериодыОтпусков.ДатаОкончания
			|ГДЕ
			|	ПериодыОтпусков.Сотрудник ЕСТЬ NULL
			|
			|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
		...
		ВидыОтпусков = Новый Соответствие;
		Пока Выборка.Следующий() Цикл
			ПериодОтпуска = Новый Структура("ВидОтпуска, ДатаНачала, ДатаОкончания, КоличествоДней");
			ЗаполнитьЗначенияСвойств(ПериодОтпуска, Выборка);
			Если ПредыдущаяСтрока <> Неопределено 
				И Выборка.ДатаНачала = ПредыдущаяСтрока.ДатаОкончания + 86400
				И ВидыОтпусков.Получить(Выборка.ВидОтпуска) = Неопределено Тогда
				ПредыдущаяСтрока.ДатаОкончания = Выборка.ДатаОкончания;
				ВидыОтпусков.Вставить(Выборка.ВидОтпуска, Истина);
				ПредыдущаяСтрока.Периоды.Добавить(ПериодОтпуска);
			Иначе
				Если Выборка.ДатаНачала > ОкончаниеПериода Тогда 
					Прервать;
				КонецЕсли;
				НоваяСтрока = Новый Структура("Организация, Сотрудник, ДатаНачала, ДатаОкончания, ДокументПланирования, Периоды");
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				НоваяСтрока.Периоды = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПериодОтпуска);
				Если Выборка.ДатаНачала >= НачалоПериода Тогда 
					ДанныеУведомлений.Добавить(НоваяСтрока);
				КонецЕсли;
				ПредыдущаяСтрока = НоваяСтрока;
				ВидыОтпусков = Новый Соответствие;
				ВидыОтпусков.Вставить(Выборка.ВидОтпуска, Истина);
...
Процедура СформироватьУведомленияОНачалеОтпуска(ЗапланированныеОтпуска) Экспорт 
	
	Сотрудники = ЗапланированныеОтпуска.ВыгрузитьКолонку("Сотрудник");
	...
	Для Каждого ДанныеОтпуска Из ЗапланированныеОтпуска Цикл 
		УведомлениеОбъект = Документы.УведомлениеОНачалеОтпуска.СоздатьДокумент();
		УведомлениеОбъект.Дата = ТекущаяДатаСеанса();
		УведомлениеОбъект.Организация = ДанныеОтпуска.Организация;
		УведомлениеОбъект.Сотрудник = ДанныеОтпуска.Сотрудник;
	    УведомлениеОбъект.ФизическоеЛицо = ФизическиеЛицаСотрудников[ДанныеОтпуска.Сотрудник];
	    УведомлениеОбъект.ДатаНачала = ДанныеОтпуска.ДатаНачала;
	    УведомлениеОбъект.ДатаОкончания = ДанныеОтпуска.ДатаОкончания;
		УведомлениеОбъект.Ответственный = Пользователи.ТекущийПользователь();
		Для Каждого ПериодОтпуска Из ДанныеОтпуска.ПериодыОтпуска Цикл 
			ЗаполнитьЗначенияСвойств(УведомлениеОбъект.Отпуска.Добавить(), ПериодОтпуска);
		КонецЦикла;
...
Функция СоздатьДокументыПереносОтпускаПоДаннымЗаявки(ДанныеЗаявки) Экспорт 
	
	ДокументыСсылки = Новый Массив;
	
	ИсходныеПериоды = Новый ТаблицаЗначений;
	ИсходныеПериоды.Колонки.Добавить("ВидОтпуска", Новый ОписаниеТипов("СправочникСсылка.ВидыОтпусков"));
	ИсходныеПериоды.Колонки.Добавить("ИсходнаяДатаНачала", Новый ОписаниеТипов("Дата"));
	ИсходныеПериоды.Колонки.Добавить("ДатаНачала", Новый ОписаниеТипов("Дата"));
	ИсходныеПериоды.Колонки.Добавить("ДатаОкончания", Новый ОписаниеТипов("Дата"));
	
	Для Каждого ИсходныйПериод Из ДанныеЗаявки.ИсходныеПериоды Цикл 
		ЗаполнитьЗначенияСвойств(ИсходныеПериоды.Добавить(), ИсходныйПериод);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ИсходныеПериоды", ИсходныеПериоды);
	Запрос.УстановитьПараметр("Организация", ДанныеЗаявки.Организация);
	Запрос.УстановитьПараметр("ФизическоеЛицо", ДанныеЗаявки.ФизическоеЛицо);
	Запрос.УстановитьПараметр("Сотрудник", ДанныеЗаявки.Сотрудник);
	
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ИсходныеПериоды.ВидОтпуска КАК ВидОтпуска,
		|	ИсходныеПериоды.ИсходнаяДатаНачала КАК ИсходнаяДатаНачала,
		|	ИсходныеПериоды.ДатаНачала КАК ДатаНачала,
		|	ИсходныеПериоды.ДатаОкончания КАК ДатаОкончания
		|ПОМЕСТИТЬ ВТИсходныеПериоды
		|ИЗ
		|	&ИсходныеПериоды КАК ИсходныеПериоды
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ИНАЧЕ ПлановыеЕжегодныеОтпуска.КоличествоДней
		|	КОНЕЦ КАК КоличествоДней
		|ПОМЕСТИТЬ ВТТекущиеПериоды
		|ИЗ
		|	РегистрСведений.ПлановыеЕжегодныеОтпуска КАК ПлановыеЕжегодныеОтпуска
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеПериоды КАК ИсходныеПериоды
		|		ПО ПлановыеЕжегодныеОтпуска.ВидОтпуска = ИсходныеПериоды.ВидОтпуска
		|			И ПлановыеЕжегодныеОтпуска.ДатаНачала = ИсходныеПериоды.ИсходнаяДатаНачала
		|			И (ПлановыеЕжегодныеОтпуска.Организация = &Организация)
		|			И (ПлановыеЕжегодныеОтпуска.ФизическоеЛицо = &ФизическоеЛицо)
		|			И (ПлановыеЕжегодныеОтпуска.Сотрудник = &Сотрудник)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущиеПериоды.Сотрудник КАК Сотрудник,
		|	ТекущиеПериоды.ВидОтпуска КАК ВидОтпуска,
		|	ТекущиеПериоды.ИсходнаяДатаНачала КАК ИсходнаяДатаНачала,
		|	ТекущиеПериоды.ДатаНачала КАК ДатаНачала,
		|	ТекущиеПериоды.ДатаОкончания КАК ДатаОкончания,
		|	ТекущиеПериоды.КоличествоДней КАК КоличествоДней
		|ИЗ
		|	ВТТекущиеПериоды КАК ТекущиеПериоды
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТИсходныеПериоды КАК ИсходныеПериоды
		|		ПО ТекущиеПериоды.ВидОтпуска = ИсходныеПериоды.ВидОтпуска
		|			И ТекущиеПериоды.ИсходнаяДатаНачала = ИсходныеПериоды.ИсходнаяДатаНачала
		|			И ТекущиеПериоды.ДатаНачала = ИсходныеПериоды.ДатаНачала
		|			И ТекущиеПериоды.ДатаОкончания = ИсходныеПериоды.ДатаОкончания
		|ГДЕ
		|	ИсходныеПериоды.ВидОтпуска ЕСТЬ NULL";
	
	ДополнительныеПериоды = Запрос.Выполнить().Выгрузить();
	
	СведенияОПодписях = ПодписиДокументов.СведенияОПодписяхПоУмолчаниюДляОбъектаМетаданных(
		Метаданные.Документы.ПереносОтпуска, ДанныеЗаявки.Организация);
		
	Для Каждого ИсходныйПериод Из ИсходныеПериоды Цикл 
		ПереносОтпускаОбъект = Документы.ПереносОтпуска.СоздатьДокумент();
		ЗаполнитьЗначенияСвойств(ПереносОтпускаОбъект, СведенияОПодписях);
		ПереносОтпускаОбъект.Дата = ТекущаяДатаСеанса();
		ПереносОтпускаОбъект.Организация = ДанныеЗаявки.Организация;
		ПереносОтпускаОбъект.Сотрудник = ДанныеЗаявки.Сотрудник;
	    ПереносОтпускаОбъект.ФизическоеЛицо = ДанныеЗаявки.ФизическоеЛицо;
	    ПереносОтпускаОбъект.ВидОтпуска = ИсходныйПериод.ВидОтпуска;
	    ПереносОтпускаОбъект.ИсходнаяДатаНачала = ИсходныйПериод.ИсходнаяДатаНачала;
		ПереносОтпускаОбъект.ПереносПоИнициативеСотрудника = Истина;
		ПереносОтпускаОбъект.Ответственный = Пользователи.ТекущийПользователь();
		ОписаниеВидаОтпуска = ОписаниеВидаОтпуска(ИсходныйПериод.ВидОтпуска);
		Для Каждого ПериодОтпуска Из ДанныеЗаявки.ПериодыОтпуска Цикл 
			Если ПериодОтпуска.ВидОтпуска <> ИсходныйПериод.ВидОтпуска Тогда 
				Продолжить;
			КонецЕсли;	
			ПеренесенныйПериод = ПереносОтпускаОбъект.Переносы.Добавить();	
			ЗаполнитьЗначенияСвойств(ПеренесенныйПериод, ПериодОтпуска);	
			ПеренесенныйПериод.КоличествоДней = УчетРабочегоВремениРасширенный.ДлительностьИнтервала(ПереносОтпускаОбъект.Сотрудник, 
				ПеренесенныйПериод.ДатаНачала, ПеренесенныйПериод.ДатаОкончания, ОписаниеВидаОтпуска.СпособРасчетаПоКалендарнымДням, ОписаниеВидаОтпуска.ЕжегодныйОтпуск);
		КонецЦикла;
		Отбор = Новый Структура("ВидОтпуска, ИсходнаяДатаНачала", ИсходныйПериод.ВидОтпуска, ИсходныйПериод.ИсходнаяДатаНачала); 
		НайденныеСтроки = ДополнительныеПериоды.НайтиСтроки(Отбор);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
			ЗаполнитьЗначенияСвойств(ПереносОтпускаОбъект.Переносы.Добавить(), НайденнаяСтрока);
		КонецЦикла;
		ПереносОтпускаОбъект.Переносы.Сортировать("ДатаНачала");
		ПереносОтпускаОбъект.Записать(РежимЗаписиДокумента.Проведение);
		ДокументыСсылки.Добавить(ПереносОтпускаОбъект.Ссылка);
	КонецЦикла;
	
	Возврат ДокументыСсылки;
	
КонецФункции

Функция ДанныеЗаявкиНаПереносОтпускаПоГрафику() Экспорт 
	
	ДанныеЗаявки = Новый Структура;
	ДанныеЗаявки.Вставить("Организация");
	ДанныеЗаявки.Вставить("Сотрудник");
	ДанныеЗаявки.Вставить("ФизическоеЛицо");
	ДанныеЗаявки.Вставить("ПериодыОтпуска", Новый Массив);
	ДанныеЗаявки.Вставить("ИсходныеПериоды", Новый Массив);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Раздел                               = НСтр("ru = 'Учет отпусков'");
	ПредставлениеЗаголовкаОтпуска        = НСтр("ru = 'Ежегодные отпуска'");
	ПредставлениеЗаголовкаОтпускаПоУходу = НСтр("ru = 'Отпуска по уходу за ребенком'");
	ПредставлениеОтпуска                 = НСтр("ru = 'Плановые отпуска по графику'");
	ПредставлениеОтпускаПоУходу          = НСтр("ru = 'Возврат из отпуска по уходу'");
	ПодсказкаОтпуска                     = НСтр("ru = 'Оформить отпуск или перенос планового отпуска сотрудникам с приближающимся отпуском по графику.'");
	ПодсказкаОтпускаПоУходу              = НСтр("ru = 'Оформить возврат из отпуска по уходу за ребенком.'");
	РазделОтпуска                        = "ПлановыеЕжегодныеОтпуска";
	РазделОтпускаПоУходу                 = "ВозвратИзОтпускаПоУходуЗаРебенком";
	
	Состояние = Перечисления.СостоянияСотрудника.ОтпускПоУходуЗаРебенком;
	НачалоПериода = НачалоДня(ТекущаяДатаСеанса());
	ОкончаниеПериода = НачалоПериода + 14 * 86400;
	Период = Новый СтандартныйПериод(НачалоПериода, ОкончаниеПериода);
	
	ПлановыеОтпускаОтключен = МодульТекущиеДелаСервер.ДелоОтключено(РазделОтпуска);
	ВозвратИзОтпускаПоУходуОтключен = МодульТекущиеДелаСервер.ДелоОтключено(РазделОтпускаПоУходу);
	
	Если ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ДанныеСостоянийСотрудников)
		И ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ПлановыеЕжегодныеОтпуска) Тогда
		
		ДелоЗаголовокОтпуска = ТекущиеДела.Добавить();
		ДелоЗаголовокОтпуска.Идентификатор  = РазделОтпуска;
		ДелоЗаголовокОтпуска.Важное         = Ложь;
		ДелоЗаголовокОтпуска.Владелец       = Раздел;
		ДелоЗаголовокОтпуска.Представление  = ПредставлениеЗаголовкаОтпуска;
		ДелоЗаголовокОтпуска.Подсказка      = ПредставлениеЗаголовкаОтпуска;
		ДелоЗаголовокОтпуска.ЕстьДела       = Ложь;
		ДелоЗаголовокОтпуска.Форма          = "Отчет.ОтпускаСотрудников.Форма";
		...
		ПользовательскиеНастройкиОтчета = КадровыйУчетРасширенный.ПользовательскиеНастройкиОтчетаПоСотрудникам(
			"ОтпускаСотрудников",
			"ПлановыеФактическиеОтпуска",
			Новый Структура);
		ПараметрыЗаголовка.Вставить("ПользовательскиеНастройки", ПользовательскиеНастройкиОтчета);
		
		Если Не ПлановыеОтпускаОтключен Тогда
			Запрос = ЗапросПриближающиесяОтпускаПоГрафику(НачалоПериода, ОкончаниеПериода);
			...
			Для Каждого КлючИЗначение Из ДанныеОрганизаций Цикл 
				...
				Дело.Идентификатор = РазделОтпуска + СтрЗаменить(Организация.УникальныйИдентификатор(), "-", "");
				Дело.Важное        = Истина;
				Дело.Владелец      = РазделОтпуска;
				Дело.Представление = ПредставлениеОтпуска + ", " + Строка(Организация);
				Дело.Подсказка     = ПодсказкаОтпуска;
				Дело.Форма         = "Отчет.ОтпускаСотрудников.Форма";
				...
				СтруктураОтбора.Вставить("Сотрудник", Сотрудники);
				СтруктураОтбора.Вставить("ПлановыеОтпуска", Истина);
				СтруктураОтбора.Вставить("ФактическиеОтпуска", Ложь);
				СтруктураОтбора.Вставить("НеиспользованныеОтпуска", Истина);
				ПараметрыФормы.Вставить("Отбор", СтруктураОтбора);
				ПараметрыФормы.Вставить("СформироватьПриОткрытии", Истина);
				
				Дело.ПараметрыФормы = ПараметрыФормы;
				
				ДелоЗаголовокОтпуска.ЕстьДела = Истина;
				
			КонецЦикла;
		КонецЕсли;
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Период", Период);
		СтруктураОтбора.Вставить("Сотрудник", СотрудникиОбщий);
		СтруктураОтбора.Вставить("ПлановыеОтпуска", Истина);
		СтруктураОтбора.Вставить("ФактическиеОтпуска", Ложь);

		ПараметрыЗаголовка.Вставить("Отбор", СтруктураОтбора);
		ДелоЗаголовокОтпуска.ПараметрыФормы = ПараметрыЗаголовка;
		
	КонецЕсли;
	
	Если ПравоДоступа("Изменение", Метаданные.Документы.ВозвратИзОтпускаПоУходуЗаРебенком) Тогда
		
		ДелоЗаголовокОтпускаПоУходу = ТекущиеДела.Добавить();
		ДелоЗаголовокОтпускаПоУходу.Идентификатор = РазделОтпускаПоУходу;
		ДелоЗаголовокОтпускаПоУходу.Важное        = Ложь;
		ДелоЗаголовокОтпускаПоУходу.Владелец      = Раздел;
		ДелоЗаголовокОтпускаПоУходу.Представление = ПредставлениеЗаголовкаОтпускаПоУходу;
		ДелоЗаголовокОтпускаПоУходу.Подсказка     = ПредставлениеЗаголовкаОтпускаПоУходу;
		ДелоЗаголовокОтпускаПоУходу.ЕстьДела      = Ложь;
		ДелоЗаголовокОтпускаПоУходу.Форма         = "Отчет.СостоянияСотрудников.Форма";
		ПараметрыЗаголовка = Новый Структура;
		ПараметрыЗаголовка.Вставить("СформироватьПриОткрытии", Истина);
		ПараметрыЗаголовка.Вставить("КлючВарианта", "ОтсутствияСотрудников");
		ПользовательскиеНастройкиОтчета = КадровыйУчетРасширенный.ПользовательскиеНастройкиОтчетаПоСотрудникам(
			"СостоянияСотрудников",
			"ОтсутствияСотрудников",
			Новый Структура);
		ПараметрыЗаголовка.Вставить("ПользовательскиеНастройки", ПользовательскиеНастройкиОтчета);
			
		Если Не ВозвратИзОтпускаПоУходуОтключен Тогда
			Запрос = ЗапросЗавершающиесяОтпускаПоУходу(НачалоПериода, ОкончаниеПериода, Состояние);
			...
			Для Каждого КлючИЗначение Из ДанныеОрганизаций Цикл 
				...
				Дело = ТекущиеДела.Добавить();
				Дело.Идентификатор = РазделОтпускаПоУходу + СтрЗаменить(Организация.УникальныйИдентификатор(), "-", "");
				Дело.Важное        = Истина;
				Дело.Владелец      = РазделОтпускаПоУходу;
				Дело.Представление = ПредставлениеОтпускаПоУходу + ", " + Строка(Организация);
				Дело.Подсказка     = ПодсказкаОтпускаПоУходу;
				Дело.Форма         = "Отчет.СостоянияСотрудников.Форма";
				Дело.ЕстьДела      = Истина;
				Дело.Количество    = Сотрудники.Количество();
				...
				СтруктураОтбора.Вставить("Организация", Организация);
				СтруктураОтбора.Вставить("Сотрудник", Сотрудники);
				СтруктураОтбора.Вставить("Состояние", Состояние);
				ПараметрыФормы.Вставить("Отбор", СтруктураОтбора);
				ПараметрыФормы.Вставить("СформироватьПриОткрытии", Истина);
				ПараметрыФормы.Вставить("КлючВарианта", "ОтсутствияСотрудников");
				
				Дело.ПараметрыФормы = ПараметрыФормы;
				
				ДелоЗаголовокОтпускаПоУходу.ЕстьДела = Истина;
				
			КонецЦикла;
		КонецЕсли;
		ПараметрыЗаголовка.Вставить("Отбор", Новый Структура("Период, Состояние, Сотрудник", Период, Состояние, СотрудникиОбщий));
		ДелоЗаголовокОтпускаПоУходу.ПараметрыФормы = ПараметрыЗаголовка;
		
	КонецЕсли;
	
КонецПроцедуры

Функция ЗапросПриближающиесяОтпускаПоГрафику(НачалоПериода, ОкончаниеПериода)
	...
	|		ПО ПлановыеЕжегодныеОтпуска.Сотрудник = ДанныеСостоянийСотрудников.Сотрудник
	|			И (ДанныеСостоянийСотрудников.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияСотрудника.Увольнение))
	|			И (ДанныеСостоянийСотрудников.Начало <= &ОкончаниеПериода)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ФактическиеОтпуска КАК ФактическиеОтпуска
	|		ПО ПлановыеЕжегодныеОтпуска.Сотрудник = ФактическиеОтпуска.Сотрудник
	|			И (ФактическиеОтпуска.Период >= &НачалоПериода)
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ПлановыеЕжегодныеОтпуска.Перенесен
	|				ТОГДА ПлановыеЕжегодныеОтпуска.ПеренесеннаяДатаНачала
	...
	|	И ФактическиеОтпуска.Сотрудник ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	Сотрудник";
...
Функция ЗапросЗавершающиесяОтпускаПоУходу(НачалоПериода, ОкончаниеПериода, Состояние)
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	УстановитьПривилегированныйРежим(Истина);
	СостоянияСотрудников.СоздатьВТСостоянияСотрудников(МенеджерВременныхТаблиц, , Состояние, НачалоПериода, ОкончаниеПериода);
	УстановитьПривилегированныйРежим(Ложь);
	...
	|		ПО ВТСостоянияСотрудников.Сотрудник = Сотрудники.Ссылка
	|ГДЕ
	|	ВТСостоянияСотрудников.Окончание = ДАТАВРЕМЯ(1, 1, 1)
	|	И ВТСостоянияСотрудников.ОкончаниеПредположительно МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	Сотрудник";
