...
Процедура УстановитьПредставлениеИнтервалаПоНижнейГранице(ТаблицаИнтервалов, ОбратнаяШкала = Ложь, ЗаполнятьЗначениеОт = Ложь, Смена = Ложь) Экспорт 
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					Если ЗаполнятьЗначениеОт Тогда
						ТекущаяСтрока.ЗначениеОт = ПредыдущаяСтрока.ЗначениеДо;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьВторичныеРеквизитыСтрокиПоказателя(Форма, ТекущиеДанные) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.Показатель)
		ИЛИ Форма.СоответствиеПоказателей = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ИндексСтрокиКоллекции(ТаблицаИнтервалов, ТекущаяСтрока) Экспорт
