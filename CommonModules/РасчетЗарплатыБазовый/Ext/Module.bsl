...
Процедура ЗаполнитьНачисления(ДанныеДляПроведенияНачисленияЗарплаты, Документ, ТаблицаНачислений, ПолеДатыДействия, ПолеВидаНачисления = Неопределено, ПолеВидаНачисленияВШапке = Неопределено, ФизическиеЛица) Экспорт
	...
	Для Каждого ИмяТаблицыНачислений Из ИменаТаблицНачислений Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			Если МетаданныеДокумента.ПолноеИмя() = "Документ.Премия" Тогда
...
Функция РезультатРасчетаНДФЛ(МенеджерВременныхТаблиц, Объект, ДатаОперации, ПланируемаяДатаВыплатыВТаблицеНачислений = Ложь, МесяцНачисленияИмя = "МесяцНачисления") Экспорт
	...
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп =
			ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизацийПовтИсп");
		ДатыВыплаты = МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатыВыплатыОрганизации(Объект.Организация,
			Объект[МесяцНачисленияИмя]);
	...
	УчетНДФЛ.СформироватьСоциальныеВычетыПоВременнойТаблице(Объект.Ссылка,
		НаборыЗаписей,
		Неопределено,
		Объект.Организация,
		ДатаОперации,
		...
		,
		ОкончательныйРасчет);
	УчетНДФЛ.СформироватьДоходыНДФЛПоНачислениям(НаборыЗаписей,
		Неопределено,
		Объект.Организация,
		ДатаОперации,
		?(ПланируемаяДатаВыплатыВТаблицеНачислений, Неопределено, ПланируемаяДатаВыплаты),
		МенеджерВременныхТаблиц,
		Объект[МесяцНачисленияИмя],
		,
		ОкончательныйРасчет);
	...
	УчетНДФЛ.СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(НаборыЗаписей,
		Неопределено,
		Объект.Организация,
		ДатаОперации,
	...
	// Расчет НДФЛ
	РезультатРасчетаНДФЛ = УчетНДФЛ.РезультатРасчетаНДФЛ(МенеджерВременныхТаблиц,
		Объект.Ссылка,
		Объект.Организация,
		Объект[МесяцНачисленияИмя],
		ОкончательныйРасчет,
		МаксимумДатыПолученияДохода,
		НаборыЗаписей);
