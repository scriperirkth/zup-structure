...
Функция ПолучитьФайлЧастямиССервераКакСтроку(Адрес, РазмерЧасти = 5242880) Экспорт
	...
	Если ДанныеФайла.Размер() > 0 Тогда
		...
		Для Каждого ЧастьФайла Из ЧастиФайла Цикл
			//@skip-check 1c-fresh-audit
			АдресаЧастейФайла.Добавить(ПоместитьВоВременноеХранилище(
				Новый ДвоичныеДанные(ЧастьФайла), Новый УникальныйИдентификатор));
		КонецЦикла;
		
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(КаталогРезультата);
