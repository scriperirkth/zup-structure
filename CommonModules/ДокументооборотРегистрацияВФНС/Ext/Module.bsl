...
Функция СервисРегистрацииФНСФормаЗявления(ДанныеВыгрузки) Экспорт
	...
	Если ПодготовитьРезультатЗапроса(РезультатВызова, Результат) Тогда
		ВсеФайлы = ПолучитьСодержимоеАрхива(
			ПолучитьДвоичныеДанныеИзBase64Строки(РезультатВызова.Результат.file));
		Если ВсеФайлы.Выполнено Тогда
			Для Каждого СтрокаКлюча Из ВсеФайлы.Файлы Цикл
				Если СтрЗаканчиваетсяНа(СтрокаКлюча.Значение.Файл.Расширение, "pdf") Тогда
					Результат.Вставить("Файл", СтрокаКлюча.Значение.ДвоичныеДанныеФайла);
					Прервать;
				КонецЕсли;
			КонецЦикла;
			...
			КонецЕсли;
			// Временный каталог получен внутри метода ПолучитьСодержимоеАрхива.
...
Функция ЗаписатьУдачнуюОтправку(ПараметрыОтправки, ИдентификаторСервера, ДатаОтправки, ВерсияОбмена) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрацииДляОшибки(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ПредметОтправки)
...
Функция ОбновитьОтправкуРегистрацииЮЛ(ОтправкаСсылка, ИзменяемыеРеквизиты) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		Результат = Ложь;
		ЗаписьЖурналаРегистрацииДляОшибки(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ОтправкаСсылка)
...
Функция ВыполнитьЗапросREST(ПараметрыПодключения, ПараметрыЗапроса = Неопределено, ПараметрыТела = Неопределено)
	...
	Исключение
		...
		ПодробныйТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ДанныеОшибки) + Символы.ПС
...
Функция ЗаписатьОбъектОтправки(ОбъектОтправки)
	...
	Если ОбъектОтправки.ПроверитьЗаполнение() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрацииДляОшибки(ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ОбъектОтправки.Ссылка);
			Удачно = Ложь;
		КонецПопытки;
...
Функция ПодготовитьОписаниеФайлов(ДвоичныеДанныеФайла, ВременныйКаталог)
	...
	ТекущийДОМ = ПодготовитьДокументDOM(ДвоичныеДанныеФайла);
	...
	Для Каждого СтрокаМассива Из ВсеФайлы Цикл
		...
		НовыйФайл.Вставить("ДвоичныеДанныеФайла", Неопределено);
		...
		Если СтрокаФайла.Количество() = 1 Тогда
			...
			РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(НовыйФайл.Имя);
			НовыйФайл.ПолноеИмя = ВременныйКаталог + НовыйФайл.Имя;
			//@skip-check 1c-fresh-audit
			НовыйФайл.ДвоичныеДанныеФайла = Новый ДвоичныеДанные(НовыйФайл.ПолноеИмя);
		КонецЕсли;
		...
		Если СтрокаПодписи.Количество() > 0 Тогда
			...
			РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(ФайлПодписи.Имя);
			ФайлПодписи.ПолноеИмя = ВременныйКаталог + ФайлПодписи.Имя;
			//@skip-check 1c-fresh-audit
			ФайлПодписи.ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ФайлПодписи.ПолноеИмя);
...
Функция ПолучитьСодержимоеАрхива(ДанныеКонтейнера, РежимВосстановления = Неопределено)
	...
	Попытка
		...
		// Имя файла создается внутри метода.
		//@skip-check 1c-fresh-audit
		...
		Для Каждого СтрокаМассива Из РаспакованныеФайлы Цикл
			...
			Если НЕ ЭтоКаталог Тогда
				...
				ДанныеФайла.Вставить("Файл", СтрокаМассива);
				//@skip-check 1c-fresh-audit
				ДанныеФайла.Вставить("ДвоичныеДанныеФайла",
					Новый ДвоичныеДанные(СтрокаМассива.ПолноеИмя));
				ТекущийКаталог.Вставить(СтрокаМассива.Имя, ДанныеФайла);
			КонецЕсли;
	...
	Исключение
		Результат.Ошибка = Результат.Ошибка + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПодготовитьДокументDOM(СсылкаНаДанные)
	...
	ИначеЕсли ТипЗнч(СсылкаНаДанные) = Тип("Строка") Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			СсылкаНаДанные, "СсылкаНаДанные", "ПодготовитьДокументDOM");
		ЭтоФайл = Истина;
	Иначе
		ТекстОшибки = НСтр("ru = 'Передан недопустимый параметр СсылкаНаДанные %1.'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки, СсылкаНаДанные);
...
Функция РазборТранспортногоКонтейнера(ИмяФайла, ДанныеКонтейнера)
	...
	Для Каждого СтрокаКлюча Из СодержимоеАрхива.Файлы Цикл
		Если НРег(СтрокаКлюча.Ключ) = "packagedescription.xml" Тогда
			ОписаниеПакета = ПодготовитьОписаниеФайлов(
				СтрокаКлюча.Значение.ДвоичныеДанныеФайла, СодержимоеАрхива.ВременныйКаталог);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ОписаниеПакета = Неопределено Тогда
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(СодержимоеАрхива.ВременныйКаталог);
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого СтрокаМассива Из ОписаниеПакета.Файлы Цикл
		ДанныеФайла = СтрокаМассива.ДвоичныеДанныеФайла;
		НоваяСтрока = Новый Структура;
