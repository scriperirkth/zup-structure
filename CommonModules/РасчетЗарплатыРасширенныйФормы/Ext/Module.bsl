...
Процедура РасчетЗарплатыНачисленияВДанныеФормы(ТаблицыНачислений, ТаблицаИсточник, Организация, МесяцНачисления = Неопределено, ПозицииВставки = Неопределено, РежимДоначисления = Ложь) Экспорт
	...
	Для Каждого СтрокаНачисления Из ТаблицаИсточник Цикл
		...
		Если ТаблицыНачислений["ЛьготыПерерасчет"] <> Неопределено
			И (СтрокаНачисления.Владелец = "ЛьготыПерерасчет"
			Или ЭтоЛьготы И ЭтоПерерасчет И Не ЗначениеЗаполнено(СтрокаНачисления.Владелец)) Тогда
...
Процедура УдалитьСтрокиРасчета(Таблица, СтрокиРасчета, ПолноеУдаление = Ложь) Экспорт
	...
	Если ПолноеУдаление Тогда
		Таблица.Очистить();
	Иначе
		СтрокиРасчета = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СтрокиРасчета);
...
Процедура ДополнитьСтрокиРасчета(Форма, ОписаниеДокумента, ОписаниеТаблицы, ИдентификаторыСтрок = Неопределено, ЗаполнятьСведенияСотрудников = Истина, ЗаполнятьЗначенияПоказателей = Истина, ПозицииВставки = Неопределено) Экспорт
	...
	Для Каждого СтрокаРасчета Из СтрокиРасчета Цикл
		...
		Если ОписаниеДокумента.ПериодДействияВШапке
			И ОписаниеДокумента.НачалоБазовогоПериодаИмя <> Неопределено
			И ОписаниеДокумента.ОкончаниеБазовогоПериодаИмя <> Неопределено Тогда
			БазовыйПериод = Новый СтандартныйПериод;
			БазовыйПериод.ДатаНачала = Форма.Объект[ОписаниеДокумента.НачалоБазовогоПериодаИмя];
			БазовыйПериод.ДатаОкончания = Форма.Объект[ОписаниеДокумента.ОкончаниеБазовогоПериодаИмя];
		Иначе
			БазовыйПериод = Неопределено;
		КонецЕсли;
		
		ЗаполнитьБазовыйПериодДанныеМенеджераРасчета(НоваяСтрока, ПериодыСтрокиРасчета, ЗначенияРеквизитов, БазовыйПериод);
		
		Если ОписаниеДокумента.ПериодДействияВШапке И ОписаниеДокумента.ФиксБазовыйПериодИмя <> Неопределено И Форма.Объект[ОписаниеДокумента.ФиксБазовыйПериодИмя] Тогда
			НоваяСтрока.ФиксБазовыйПериод = Истина;
		КонецЕсли;
...
#Область МетодыОбслуживанияФормСверхурочныхДокументов

Процедура ЗаполнитьСуточныеВидыВремени(Форма) Экспорт
	
	СуточныеВидыВремени = Новый Структура; 
	СуточныеВидыВремени.Вставить("ДневноеВремя", ПредопределенноеЗначение("Перечисление.ВидыВремениСуток.ДневноеВремя"));
	СуточныеВидыВремени.Вставить("ВечернееВремя", ПредопределенноеЗначение("Перечисление.ВидыВремениСуток.ВечернееВремя"));
	СуточныеВидыВремени.Вставить("НочноеВремя", ПредопределенноеЗначение("Перечисление.ВидыВремениСуток.НочноеВремя"));
	
	Форма.СуточныеВидыВремени = СуточныеВидыВремени;
