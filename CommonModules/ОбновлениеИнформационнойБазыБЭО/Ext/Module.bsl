...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = ВерсияОбработчика("4.0.1.35");
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБЭО.ЗаполнитьРазделенныеДанные";
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьОбщиеПоставляемыеДанные");
	Методы.Вставить("ЗаполнитьРазделенныеДанные");
КонецПроцедуры
...
Процедура ЗаполнитьОбщиеПоставляемыеДанные() Экспорт
	...
	Если Не ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.МакетыЭлектронныхКурсов) Тогда
		Возврат;
	КонецЕсли;
...
Функция ВерсияОбработчика(ВерсияБЭО)
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы") Тогда
			...
			МодульЗарплатаКадрыРасширенный.ПриОпределенииСоответствияВерсийОбработчиковОбновлений(СоответствиеВерсий);
		КонецЕсли;
