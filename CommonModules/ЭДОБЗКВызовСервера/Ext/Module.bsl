	...
	Для Каждого ОписаниеПечатнойФормы Из ОписанияПечатныхФорм Цикл
		...
		Для Каждого СоответствиеОбъектаПечатнойФорме Из ПечатныеФормыПоОбъектам Цикл
			...
			Если Не ЕстьДетальныеОбласти Тогда
				...
				СоздатьФайлПечатнойФормы(ДанныеПечатнойФормы, ИдентификаторХранилища);
				ПечатныеФормы.Добавить(ДанныеПечатнойФормы);
...
Функция ФайлыМЧДДляСохранениеНаДиск(СписокМЧД) Экспорт
	...
	Для Каждого МЧД Из СписокМЧД Цикл
		...
		ВывестиВФайлАрхиваМЧД(ФайлАрхива, МЧД);
...
Функция ДобавитьПечатнуюФорму(ПечатнаяФорма, ОбъектВладелец, ИдентификаторПечатнойФормы, Название, Организация, ФизическоеЛицо, ИмяФайла = "", РегистрироватьКОтправке = Ложь) Экспорт
	...
	СоздатьФайлПечатнойФормы(ДанныеПечатнойФормы, Новый УникальныйИдентификатор);
...
Функция ВидДокументаПечатнойФормы(ИдентификаторПечатнойФормы) Экспорт
	Настройки = НастройкиПечатнойФормы(ИдентификаторПечатнойФормы);
	Если Настройки = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СоздатьФайлПечатнойФормы(ДанныеПечатнойФормы, ИдентификаторХранилища)
...
Функция ФайлыСоШтампамиПодписей(ФайлыДляПечати)
	...
	Если ОписанияФайлов.Количество() > 0 Тогда
		...
		Для Каждого ОписаниеФайла Из ОписанияФайлов Цикл
			...
			ИмяФайлаСРасширением = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
				ИмяФайла + "." + НастройкиФормата.Расширение, "");
...
Процедура ВывестиВФайлАрхиваМЧД(ФайлАрхива, МЧД)
	...
	Если ДанныеФайла.ПодписанЭП Тогда
		...
		ВывестиВФайлАрхиваМЧДПодписи(УстановленныеПодписи, ФайлАрхива, ИмяФайла);
...
Процедура ВывестиВФайлАрхиваМЧДПодписи(ДанныеПодписиПодписей, ФайлАрхива, ИмяОсновногоФайлаСРасширением)
			...
			Если ЗначениеЗаполнено(ДанныеПодписи.НаборСвойствПодписиЭДОБЗК) Тогда
				Для Каждого СвойстваПодписи Из ДанныеПодписи.НаборСвойствПодписиЭДОБЗК Цикл
					Если ЗначениеЗаполнено(СвойстваПодписи.МЧД) Тогда
						ВывестиВФайлАрхиваМЧД(ФайлАрхива, СвойстваПодписи.МЧД);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Функция ДанныеФайлаДляОписания(ДанныеВыводимогоФайла)
	...
	Запрос.Текст =
		...
		|			ИЛИ ПодписиДокументовЭДОБЗК.ИдентификаторВладельцаПодписи <> """")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(ЭлектронныеПодписиЭДОБЗК.РольПодписанта, ЗНАЧЕНИЕ(Перечисление.РолиПодписантовЭДОБЗК.Исполнитель)) КАК РольПодписанта
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЭлектронныеПодписиЭДОБЗК КАК ЭлектронныеПодписиЭДОБЗК
		|			ПО ЭлектронныеПодписи.ПодписанныйОбъект = ЭлектронныеПодписиЭДОБЗК.ПодписанныйОбъект
		|				И ЭлектронныеПодписи.ИдентификаторПодписи = ЭлектронныеПодписиЭДОБЗК.ИдентификаторПодписи
		|		ПО ДанныеПодписейТолькоСотрудников.Ссылка.ЭлектронныйДокумент = ЭлектронныеПодписи.ПодписанныйОбъект
		|			И ДанныеПодписейТолькоСотрудников.Отпечаток = ЭлектронныеПодписи.Отпечаток
		|			И (ДанныеПодписейТолькоСотрудников.Отпечаток <> """")
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ДАТАВРЕМЯ(1, 1, 1),
		|	ЕСТЬNULL(ЭлектронныеПодписиЭДОБЗК.РольПодписанта, ЗНАЧЕНИЕ(Перечисление.РолиПодписантовЭДОБЗК.Исполнитель))
		|ИЗ
		|	Документ.ДокументЭДОБЗК КАК ДокументЭДОБЗК
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЭлектронныеПодписи КАК ЭлектронныеПодписи
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЭлектронныеПодписиЭДОБЗК КАК ЭлектронныеПодписиЭДОБЗК
	...
	ПараметрыПолучения.ИмяВТФизическиеЛица = "ВТДанныеПодписейДокумента";
	...
	Описатель = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеФизическихЛиц(
		Запрос.МенеджерВременныхТаблиц, "ВТДанныеПодписейДокумента");
	...
	Запрос.Текст =
		...
		|	КОНЕЦ КАК Должность,
		|	ДанныеПодписейСотрудников.РольПодписанта КАК РольПодписанта
		|ИЗ
		|	ВТДанныеПодписейДокумента КАК ДанныеПодписейСотрудников
	...
	Для Каждого ДанныеПодписиФайла Из ДанныеФайла.Подписи Цикл
		...
		Если ДанныеПодписиБазы <> Неопределено Тогда
			...
			Если ДанныеПодписиФайла.Размер > 0
				И ДанныеПодписиБазы.РольПодписанта = Перечисления.РолиПодписантовЭДОБЗК.Организация Тогда
...
Функция ИмяФайлаЭДОБЗК(Знач ИмяФайлаСРасширением) Экспорт
	...
	ИмяФайлаСРасширением = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
		СтрЗаменить(ИмяФайлаСРасширением, """", "_"), "");
...
Функция ИмяФайлаПодписи(Знач ИмяОсновногоФайлаСРасширением, Знач ВыводитьИндексПодписи = Ложь, Знач ПорядковыйНомерПодписи = 0) Экспорт
	...
	Если Не ВыводитьИндексПодписи Тогда
		...
		Возврат ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
			Файл.ИмяБезРасширения + "." + ЭлектроннаяПодпись.ПерсональныеНастройки().РасширениеДляФайловПодписи, "");
	КонецЕсли;
...
Функция ИмяФайлаСДополнением(Знач ИмяОсновногоФайлаСРасширением, Знач Дополнение, Знач Расширение)
	...
	Возврат ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(
		ИмяБезРасширения + "." + НРег(Расширение), "");
...
Функция PDFПредставлениеPDFФайлаСоШтампамиПодписей(Оригинал, АдресВХранилище, Знач ИмяФайла, ИдентификаторФормы)
	...
	Попытка
		...
		ПолноеИмяФайла = РаботаСФайламиБЗК.УникальноеИмяФайла(
			ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ИмяВременногоКаталога)
				+ ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ДанныеПрисоединенногоФайла.ИмяФайла, ""));
...
Функция ИмяВыгружаемогоФайлаМЧД(Ссылка) Экспорт
	...
	Возврат ИмяФайлаЭДОБЗК(ИмяФайлаДоверенности);
...
Функция УстановитьПризнакФорматФайлаPDF_A_1(ДокументЭДОБЗК) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось заблокировать документ ""%1"" для проверки (конвертации) на соответствие формату PDF/A-1A'"), ДокументЭДОБЗК);
	КонецПопытки;
	Возврат Результат;
КонецФункции
...
Процедура СконвертироватьДанныеДокументаВФорматPDF_A_1(ДокументЭДОБЗК, ДанныеДокумента, УникальныйИдентификатор) Экспорт
	...
	Попытка
		...
		Если СконвертироватьДанныеФайлаВPDF_A_1(
			...
			Для Каждого УстановленнаяПодпись Из УстановленныеПодписи Цикл
				Если Не ЗначениеЗаполнено(УстановленнаяПодпись.УстановившийПодпись) Тогда
					ОтветственныеЛица.Вставить(Пользователи.ТекущийПользователь(), Истина);
					Продолжить;
				КонецЕсли;
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УстановленнаяПодпись, "НаборСвойствПодписиЭДОБЗК")
					...
					Для Каждого СвойствоПодписи Из УстановленнаяПодпись.НаборСвойствПодписиЭДОБЗК Цикл
						Если СвойствоПодписи.РольПодписанта = Перечисления.РолиПодписантовЭДОБЗК.Организация
							...
							ОтветственныеЛица.Вставить(УстановленнаяПодпись.УстановившийПодпись, Истина);
							Прервать;
						КонецЕсли;
					КонецЦикла;
				Иначе
		...
		РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.ЗарегистрироватьОбработкуФайлов(
			...
			Перечисления.ДействияСФайламиДокументовЭДОБЗК.ПередатьВКабинетСотрудников,
			ОбщегоНазначения.ВыгрузитьКолонку(ОтветственныеЛица, "Ключ"));
...
Функция ФорматИсходногоФайлаPDF_A_1(ФайлPDF)
...
Функция СконвертироватьДанныеФайлаВPDF_A_1(АдресВХранилище, ИмяФайла, УникальныйИдентификатор) Экспорт
	Попытка
		...
		Если Не ФорматИсходногоФайлаPDF_A_1(ФайлPDF) Тогда
			Поток = Новый ПотокВПамяти();
...
Функция ФорматЭлектронногоДокументаPDF_A_1(ДанныеДокумента) Экспорт
	Возврат ФорматИсходногоФайлаPDF_A_1(ИсходныйФайлPDF(ДанныеДокумента.АдресВХранилище, ДанныеДокумента.ИмяФайла));
КонецФункции
