...
// Возвращает регистратор, который нужно исключить при получении действующих данных формы документа, например, начислений или отпусков.
Функция ИсключаемыйРегистраторДляДействующихДанных(Форма) Экспорт

	Объект = Форма.Объект;

	Если ЗначениеЗаполнено(Объект.ИсправленныйДокумент) И Не Объект.Проведен Тогда
		Возврат Объект.ИсправленныйДокумент;
	КонецЕсли;

	Возврат Форма.СсылкаНаОбъект;
...
Процедура ПрочитатьРеквизитыИсправления(Форма, РежимИсправления = "РасчетЗарплаты", ПолеПериодРегистрации = "ПериодРегистрации", ПроверятьНаличиеНачислений = Истина) Экспорт
	...
	Если СозданиеНового Тогда
		ИсправленныйДокумент = ?(ВозможностиИсправления.Исправление, Объект.ИсправленныйДокумент, Неопределено);
	Иначе
...
Процедура СторнироватьДвиженияПоВсемУчетам(Движения, ИсправленныйДокумент, ПараметрыСторнирования, СтруктураВидовУчета) Экспорт
	...
	Если СтруктураВидовУчета.СтраховыеВзносы И Не ПараметрыСторнирования.ИсправлениеДоначислением Тогда
...
//    исправленного документа. Состав полей см. в ПроведениеСервер.СтруктураВидовУчета().
...
Функция ПриПроведенииИсправления(ДокументСсылка, Движения, РежимПроведения, Отказ, РеквизитыДляПроведения = Неопределено, СтруктураВидовУчета = Неопределено, Объект = Неопределено, ИмяПериодРегистрации = "ПериодРегистрации") Экспорт
	...
	ЕстьДвиженияПоРР = МетаданныеИсправленногоДокумента.Движения.Содержит(Метаданные.РегистрыРасчета.Начисления);
	
	Если Объект <> Неопределено И (СтруктураВидовУчета.ПроведениеПоВсемУчетам Или СтруктураВидовУчета.ОстальныеВидыУчета) Тогда
		ДоступноИсправлениеПериодическихСведений = ОтменаДокумента
			Или Метаданные.ПодпискиНаСобытия.ПолучитьПрежнийИсправленный.Источник.СодержитТип(
	...
	Если ИсправлениеВТекущемПериодеВозможно Тогда
		Если ОтменаДокумента Тогда
			ПараметрыИсправляемогоДокумента = МенеджерДокумента.ПараметрыСторнируемогоДокумента(ИсправленныйДокумент);
		Иначе
	...
	Если ИсправлениеВТекущемПериодеВозможно И (Не ИсправлениеВТекущемПериоде) И ЕстьДвиженияПоРР Тогда
...
Функция НовыйОписаниеТаблицыНачислений() Экспорт
	...
	Описание.Вставить("ЭтоНачисленияПоДоговорам", Ложь);
...
Процедура ПроверитьСторнированиеОснований(Отказ, ДокументСсылка, ЭтоПроведение = Истина) Экспорт
	
	Если ТранзакцияАктивна() Тогда // Это временная запись документа.
		Возврат;
	КонецЕсли;
	
	Сообщение = ИсправлениеДокументовЗарплатаКадрыВызовСервера.НайтиСторнированныеОснования(ДокументСсылка, ЭтоПроведение);
...
Функция ДокументыИсправляемыеВТекущемПериодеДополнительно()
	
	Типы = Новый Массив();
	
	// Документы не имеющие движений по РР.Начисления.
...
Функция ВозможноИсправлениеТекущимПериодом(Документ) Экспорт
	...
	Возврат Метаданные.ОпределяемыеТипы.ДокументИсправляемыйВТекущемПериоде.Тип.СодержитТип(ТипЗнч(Документ))
		Или ДокументыИсправляемыеВТекущемПериодеДополнительно().СодержитТип(ТипЗнч(Документ));
...
Процедура ОтменитьИсправлениеВТекущемПериоде(Источник, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(ИсправленныйДокумент)
		И ВозможностиИсправления(ИсправленныйДокумент).ИсправлениеТекущимПериодом
		И ИсправленныйДокумент.Метаданные().Движения.Содержит(Метаданные.РегистрыРасчета.Начисления) Тогда
		РегистрыРасчета.Начисления.ПриОтменеИсправленияВТекущемПериоде(ИсправленныйДокумент);
	Иначе
