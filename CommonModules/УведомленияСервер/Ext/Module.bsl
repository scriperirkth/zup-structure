...
Процедура ПриПодготовкеШаблонаСообщения(Реквизиты, Вложения, НазначениеШаблона, ДополнительныеПараметры) Экспорт
	...
	УведомленияПереопределяемый.ПриПодготовкеШаблонаСообщенияПоУведомлению(
		ДополнительныеПараметры.ВладелецШаблона,
		ГруппаРеквизитовСобытия.Строки,
		Вложения,
		НазначениеШаблона,
...
Процедура ПриЗаполненииТелефоновПолучателейВСообщении(ПолучателиSMS, НазначениеШаблона, ПредметСообщения) Экспорт
	...
	Если ПараметрыСообщения.Свойство("Получатель", Получатель) И ПараметрыСообщения.Свойство("ВидКонтактнойИнформации", ВидКонтактнойИнформации) Тогда
		...
		Если ТипЗнч(Получатель) = Тип("СправочникСсылка.Кандидаты") Тогда
			ВладелецКонтакта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Получатель, "ЛичныеДанные", Истина);
		ИначеЕсли ТипЗнч(Получатель) = Тип("СправочникСсылка.Пользователи")
			Или ТипЗнч(Получатель) = Тип("СправочникСсылка.Сотрудники") Тогда
			ВладелецКонтакта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Получатель, "ФизическоеЛицо", Истина);
		Иначе
			ВладелецКонтакта = Получатель;
		КонецЕсли;
		
		ЗначениеКонтактнойИнформации = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(ВладелецКонтакта, ВидКонтактнойИнформации);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("НачальноеЗаполнениеДанныхПодсистемыУведомлений");
КонецПроцедуры
...
Процедура ПриНастройкеПроверкиМетодовВызываемыхКакПроизвольныйКод(Настройки) Экспорт
	...
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.УведомленияСервер";
	ИсключениеПроверки.ИмяПроцедуры       = "ОписаниеДанныхУведомления";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(СтруктураОписанияВидаУведомления.Инициализация,  МассивПараметров)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.УведомленияСервер";
	ИсключениеПроверки.ИмяПроцедуры       = "ДанныеДляУведомления";
	ИсключениеПроверки.ФрагментСодержания = "ОбщегоНазначения.ВыполнитьМетодКонфигурации(СтруктураОписанияВидаУведомления.ПолучениеДанных, МассивПараметров)";
	
	ИсключениеПроверки = Настройки.ИсключенияПроверки.Добавить();
	ИсключениеПроверки.ПолноеИмяОбъекта   = "ОбщийМодуль.УведомленияСервер";
	ИсключениеПроверки.ИмяПроцедуры       = "ЗапуститьФоновоеЗаданиеСозданияУведомлений";
	ИсключениеПроверки.ФрагментСодержания = "Возврат ДлительныеОперации.ВыполнитьВФоне(ИмяМетода, ПараметрыМетода, НастройкиЗапуска)";
...
Процедура ОтправитьУведомления(РассылкаУведомлений = Неопределено) Экспорт
	...
	Для Каждого СтрокаСообщения Из ТаблицаСообщений Цикл
		...
		Попытка
			Блокировка.Заблокировать();
			ОбработатьОтправкуУведомления(СтрокаСообщения, КодОсновногоЯзыка);
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
...
Процедура ОбработатьОтправкуУведомления(СтрокаСообщения, КодОсновногоЯзыка)
	...
	ИначеЕсли СтрокаСообщения.ВидТранспорта = Перечисления.ВидыТранспортаУведомлений.Обсуждения Тогда
		РезультатОтправки = ОтправитьУведомлениеВОбсуждение(СтрокаСообщения);
	Иначе
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при отправке уведомлений'", КодОсновногоЯзыка), УровеньЖурналаРегистрации.Ошибка,,СтрокаСообщения.Рассылка, СообщениеОбОшибке);
	КонецЕсли;
...
Функция ОтправитьУведомлениеВОбсуждение(СтрокаСообщения)
	...
	Возврат УведомленияОбсуждения.СформироватьСообщениеИОтправить(
		СтрокаСообщения, Предмет, СтруктураПараметровСобытия);
