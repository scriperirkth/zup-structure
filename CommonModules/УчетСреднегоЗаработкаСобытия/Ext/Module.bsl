...
Процедура ПодготовитьСведенияОПериодахСреднегоЗаработкаОбщий(Источник, Отказ, Замещение) Экспорт
	...
	СобратьДанныеДляРегистрацииПерерасчетов(Источник, Замещение);
...
Процедура ПроверитьНеобходимостьПерерасчетаСреднегоЗаработкаОбщий(Источник, Отказ, Замещение) Экспорт
	...
	ПроверитьНеобходимостьРегистрацииПерерасчетов(Источник, "Общий", Замещение);
...
Процедура СобратьДанныеДляРегистрацииПерерасчетов(Источник, Замещение) Экспорт
	...
	СобратьДанныеОДвиженияхРегистратора(ДанныеДляРегистрацииПерерасчетов, Регистраторы, ИмяРегистра, "ВТПредыдущиеДвижения");
	
	Источник.ДополнительныеСвойства.Вставить("ДанныеДляРегистрацииПерерасчетов", ДанныеДляРегистрацииПерерасчетов);
	
КонецПроцедуры

Процедура ПроверитьНеобходимостьРегистрацииПерерасчетов(Источник, ВидСреднегоЗаработка, Замещение) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ДанныеДляРегистрацииПерерасчетов", ДанныеДляРегистрацииПерерасчетов) Тогда
		...
		СобратьДанныеОДвиженияхРегистратора(ДанныеДляРегистрацииПерерасчетов, Регистраторы, ИмяРегистра, "ВТТекущиеДвижения");
		...
		УсловияСоединения = УсловияСоединения + Символы.ПС + "И "
			+ "ДвиженияПредыдущие.Регистратор = ДвиженияТекущие.Регистратор";
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			ЭтоИзменениеГрафика = ЭтоИзменениеГрафикаРаботы(ИмяРегистра);
			Для Каждого Регистратор Из Регистраторы Цикл 
				Если ЭтоИзменениеГрафика Тогда 
					ЗарегистрироватьПерерасчетыПриИзмененииГрафикаРаботы(ДанныеДляРегистрацииПерерасчетов, Регистратор);
				Иначе 
...
Функция СобратьДанныеОДвиженияхРегистратора(ДанныеДляРегистрацииПерерасчетов, Регистратор, ИмяРегистра, ИмяСоздаваемойВременнойТаблицы)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|ГДЕ
		|	Регистр.Регистратор В(&Регистратор)
...
Процедура ВыявитьИЗарегистрироватьПерерасчетыСреднегоЗаработка(ДанныеДляРегистрацииПерерасчетов, Регистратор, ВидСреднегоЗаработка, ЭтоИндексация = Ложь)
	...
	Если ЭтоИндексация Тогда
		...
		КатегорииНачисленийОплатыПериодаОтсутствий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ДенежноеСодержаниеНаПериодОтпуска);
...
Процедура ЗарегистрироватьПерерасчетыПриИзмененииГрафикаРаботы(ДанныеДляРегистрацииПерерасчетов, Регистратор)
	
	РеквизитОснование = Метаданные.РегистрыСведений.ПерерасчетСреднегоЗаработка.Измерения.Найти("ДокументОснование");
	Если РеквизитОснование = Неопределено Или Не РеквизитОснование.Тип.СодержитТип(ТипЗнч(Регистратор)) Тогда 
		Возврат;
	КонецЕсли;
	
	ТипыДокументов = Новый Массив;
	РеквизитДокументСреднегоЗаработка = Метаданные.РегистрыСведений.ПерерасчетСреднегоЗаработка.Измерения.Найти("ДокументСреднегоЗаработка");
	Если РеквизитДокументСреднегоЗаработка <> Неопределено Тогда 
		ТипыДокументов = РеквизитДокументСреднегоЗаработка.Тип.Типы();
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ТипыДокументов, Тип("ДокументСсылка.СторнированиеНачислений"));
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|			И (ВидыНачислений.ИспользуетСреднийЗаработокОбщий)
		|ГДЕ
		|	ТИПЗНАЧЕНИЯ(Начисления.Регистратор) В (&ТипыДокументов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
...
Функция ЭтоИзменениеГрафикаРаботы(ИмяРегистра)
	
	Возврат ВРег("РегистрыСведений." + ИмяРегистра) = ВРег("РегистрыСведений.ГрафикРаботыСотрудников");
