...
Процедура ДобавитьДанныеВыплатПоДоговорамОпеки(Запрос, Подразделение) Экспорт
	...
	Запрос.Текст = 
		...
		|	УсловияДоговораОпеки.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки КАК ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки,
		|	УсловияДоговораОпеки.РайонныйКоэффициент КАК РайонныйКоэффициент,
		|	УсловияДоговораОпеки.РайонныйКоэффициентРазмер КАК РайонныйКоэффициентРазмер,
		|	УсловияДоговораОпеки.СевернаяНадбавкаРазмер КАК СевернаяНадбавкаРазмер,
		|	0 КАК СуммаЕНВД,
		...
		|	0 КАК СуммаВычетаДополнительная,
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	УсловияВыплатПоОпеке.МесяцНачисления КАК Период,
		|	УсловияВыплатПоОпеке.Сотрудник КАК Сотрудник
		|ПОМЕСТИТЬ ВТСотрудникиПериодыДляПолученияПроцентаСевернойНадбавки
		|ИЗ
		|	ВТУсловияВыплатПоОпеке КАК УсловияВыплатПоОпеке
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Период,
		|	Сотрудник";
	...
	ИмяВТПроцентСевернойНадбавкиСотрудников = "ВТПроцентСевернойНадбавкиСотрудниковПоОпеке";
...
Процедура ВыделитьДолиРКиСН(Запрос) Экспорт
	
	МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
	ТаблицаВТНачисления = МенеджерВременныхТаблиц.Таблицы.Найти("ВТНачисления"); 
	ТаблицаВТУсловияВыплатПоОпеке = МенеджерВременныхТаблиц.Таблицы.Найти("ВТУсловияДоговораОпекиСрезПоследних"); 
	ТаблицаВТВТПроцентСевернойНадбавки = МенеджерВременныхТаблиц.Таблицы.Найти("ВТПроцентСевернойНадбавкиСотрудниковПоОпеке"); 
	
	Если ТаблицаВТНачисления = Неопределено Или ТаблицаВТУсловияВыплатПоОпеке = Неопределено Или ТаблицаВТВТПроцентСевернойНадбавки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТаблицаВТНачисления.Колонки.Найти("Сумма") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КолонкиВТНачисления = Новый Массив;
	КолонкиЗаполняемыеОсобо = "Сумма|КодДохода|КатегорияДохода|КодВычета|КодВычетаДополнительный";
	КолонкиЗаполняемыеОсобо = СтрРазделить(КолонкиЗаполняемыеОсобо, "|");
	Для Каждого КолонкаВТ Из ТаблицаВТНачисления.Колонки Цикл
		Если КолонкиЗаполняемыеОсобо.Найти(КолонкаВТ.Имя) = Неопределено Тогда
			КолонкиВТНачисления.Добавить(СтрШаблон("Начисления.%1 КАК %2", КолонкаВТ.Имя, КолонкаВТ.Имя));
		КонецЕсли;
	КонецЦикла;
	...
	|	УсловияДоговораОпеки.Сотрудник КАК Сотрудник,
	|	УсловияДоговораОпеки.ФизическоеЛицо КАК ФизическоеЛицо,
	|	УсловияДоговораОпеки.РайонныйКоэффициентРазмер + УсловияДоговораОпеки.СевернаяНадбавкаРазмер КАК Размер,
	|	ВЫБОР
	|		КОГДА КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ) < КОНЕЦПЕРИОДА(УсловияДоговораОпеки.ДатаОкончания, ДЕНЬ)
	|			ТОГДА ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ), СЕКУНДА, 1)
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(УсловияДоговораОпеки.ДатаОкончания, ДЕНЬ), СЕКУНДА, 1)
	|	КОНЕЦ КАК ДатаСуммы
	|ПОМЕСТИТЬ ВТДатыСуммыРКиСН
	|ИЗ
	|	ВТУсловияДоговораОпекиСрезПоследних КАК УсловияДоговораОпеки
	|ГДЕ
	|	УсловияДоговораОпеки.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки В (3, 4)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	УсловияДоговораОпеки.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ) = НАЧАЛОПЕРИОДА(ДатыСуммыРКиСН.ДатаСуммы, МЕСЯЦ)
	|			ТОГДА РАЗНОСТЬДАТ(УсловияДоговораОпеки.ДатаНачала, ДатыСуммыРКиСН.ДатаСуммы, ДЕНЬ) / (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ), КОНЕЦПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ), ДЕНЬ) + 1) * ДатыСуммыРКиСН.Размер
	|		ИНАЧЕ (РАЗНОСТЬДАТ(УсловияДоговораОпеки.ДатаНачала, КОНЕЦПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ), ДЕНЬ) + 1) / (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ), КОНЕЦПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ), ДЕНЬ) + 1) * ДатыСуммыРКиСН.Размер + (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(УсловияДоговораОпеки.ДатаНачала, МЕСЯЦ), НАЧАЛОПЕРИОДА(ДатыСуммыРКиСН.ДатаСуммы, МЕСЯЦ), МЕСЯЦ) - 1) * ДатыСуммыРКиСН.Размер + РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДатыСуммыРКиСН.ДатаСуммы, МЕСЯЦ), ДатыСуммыРКиСН.ДатаСуммы, ДЕНЬ) / (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДатыСуммыРКиСН.ДатаСуммы, МЕСЯЦ), КОНЕЦПЕРИОДА(ДатыСуммыРКиСН.ДатаСуммы, МЕСЯЦ), ДЕНЬ) + 1) * ДатыСуммыРКиСН.Размер
	|	КОНЕЦ КАК СуммаРКиСН
	|ПОМЕСТИТЬ ВТСуммыРКиСНДоговоровОпеки
	|ИЗ
	|	ВТУсловияДоговораОпекиСрезПоследних КАК УсловияДоговораОпеки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДатыСуммыРКиСН КАК ДатыСуммыРКиСН
	|		ПО УсловияДоговораОпеки.Договор = ДатыСуммыРКиСН.Договор
	|			И УсловияДоговораОпеки.Организация = ДатыСуммыРКиСН.Организация
	|			И УсловияДоговораОпеки.Сотрудник = ДатыСуммыРКиСН.Сотрудник
	|			И УсловияДоговораОпеки.ФизическоеЛицо = ДатыСуммыРКиСН.ФизическоеЛицо
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(НачисленияПоДоговорамГПХОбороты.СуммаРКиСНОборот) КАК Сумма
	|ПОМЕСТИТЬ ВТНачисленияРКиСНДоговоровОпеки
	|ИЗ
	|	РегистрНакопления.НачисленияПоДоговорамГПХ.Обороты(, , Регистратор, ) КАК НачисленияПоДоговорамГПХОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТУсловияДоговораОпекиСрезПоследних КАК УсловияДоговораОпеки
	|		ПО НачисленияПоДоговорамГПХОбороты.Организация = УсловияДоговораОпеки.Организация
	|			И НачисленияПоДоговорамГПХОбороты.ФизическоеЛицо = УсловияДоговораОпеки.ФизическоеЛицо
	|			И НачисленияПоДоговорамГПХОбороты.Договор = УсловияДоговораОпеки.Договор
	|ГДЕ
	|	НачисленияПоДоговорамГПХОбороты.Регистратор <> &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(НачисленияПоДоговорамГПХОбороты.СуммаРКиСНОборот) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТСуммыРКиСНДоговоровОпеки.Договор КАК Договор,
	|	ВТСуммыРКиСНДоговоровОпеки.Организация КАК Организация,
	|	ВТСуммыРКиСНДоговоровОпеки.Сотрудник КАК Сотрудник,
	|	ВТСуммыРКиСНДоговоровОпеки.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ВТСуммыРКиСНДоговоровОпеки.СуммаРКиСН - ЕСТЬNULL(ВТНачисленияРКиСНДоговоровОпеки.Сумма, 0) КАК Сумма
	|ПОМЕСТИТЬ ВТТекущийРазмерРКиСН
	|ИЗ
	|	ВТСуммыРКиСНДоговоровОпеки КАК ВТСуммыРКиСНДоговоровОпеки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияРКиСНДоговоровОпеки КАК ВТНачисленияРКиСНДоговоровОпеки
	|		ПО ВТСуммыРКиСНДоговоровОпеки.Договор = ВТНачисленияРКиСНДоговоровОпеки.Договор
	|			И ВТСуммыРКиСНДоговоровОпеки.Организация = ВТНачисленияРКиСНДоговоровОпеки.Организация
	|			И ВТСуммыРКиСНДоговоровОпеки.ФизическоеЛицо = ВТНачисленияРКиСНДоговоровОпеки.ФизическоеЛицо";
	Запрос.Выполнить();
	
	Запрос.Текст = 
	"ВЫБРАТЬ *
	|ПОМЕСТИТЬ ВТНачисленияСтарые
	|ИЗ
	...
	|	&КолонкиВТНачисления,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(УсловияВыплатПоОпеке.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки, 0) = 2
	|				И ЕСТЬNULL(УсловияВыплатПоОпеке.РайонныйКоэффициент, 1) > 1
	|			ТОГДА Начисления.Сумма / (УсловияВыплатПоОпеке.РайонныйКоэффициент + ЕСТЬNULL(ПроцентСевернойНадбавкиСотрудников.ПроцентСевернойНадбавки, 0) / 100)
	|		КОГДА ЕСТЬNULL(УсловияВыплатПоОпеке.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки, 0) = 4
	|			ТОГДА Начисления.Сумма - ЕСТЬNULL(ТекущийРазмерРКиСН.Сумма, 0)
	...
	|ПОМЕСТИТЬ ВТНачисленияПолныйРасчет
	|ИЗ
	|	ВТНачисленияСтарые КАК Начисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловияДоговораОпекиСрезПоследних КАК УсловияВыплатПоОпеке
	|		ПО Начисления.Договор = УсловияВыплатПоОпеке.Договор
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПроцентСевернойНадбавкиСотрудниковПоОпеке КАК ПроцентСевернойНадбавкиСотрудников
	|		ПО Начисления.Сотрудник = ПроцентСевернойНадбавкиСотрудников.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТТекущийРазмерРКиСН КАК ТекущийРазмерРКиСН
	|		ПО Начисления.Договор = ТекущийРазмерРКиСН.Договор
	|			И Начисления.Организация = ТекущийРазмерРКиСН.Организация
	|			И Начисления.ФизическоеЛицо = ТекущийРазмерРКиСН.ФизическоеЛицо
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&КолонкиВТНачисления,
	|	ВЫБОР
	|		КОГДА УсловияВыплатПоОпеке.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки = 2
	|				И ЕСТЬNULL(УсловияВыплатПоОпеке.РайонныйКоэффициент, 1) > 1
	|			ТОГДА Начисления.Сумма - Начисления.Сумма / (УсловияВыплатПоОпеке.РайонныйКоэффициент + ЕСТЬNULL(ПроцентСевернойНадбавкиСотрудников.ПроцентСевернойНадбавки,0) / 100)
	|		КОГДА УсловияВыплатПоОпеке.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки = 1
	|				И ЕСТЬNULL(УсловияВыплатПоОпеке.РайонныйКоэффициент, 1) > 1
	|			ТОГДА Начисления.Сумма * (УсловияВыплатПоОпеке.РайонныйКоэффициент - 1 + ЕСТЬNULL(ПроцентСевернойНадбавкиСотрудников.ПроцентСевернойНадбавки,0) / 100)
	|		КОГДА УсловияВыплатПоОпеке.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки = 3
	|				ИЛИ УсловияВыплатПоОпеке.ПорядокВыделенияРайонногоКоэффициентаСевернойНадбавки = 4
	|			ТОГДА ЕСТЬNULL(ТекущийРазмерРКиСН.Сумма, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.РайонныеСеверныеНадбавки),
	|	ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки),
	|	NULL,
	|	NULL
	|ИЗ
	|	ВТНачисленияСтарые КАК Начисления
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТУсловияДоговораОпекиСрезПоследних КАК УсловияВыплатПоОпеке
	|		ПО Начисления.Договор = УсловияВыплатПоОпеке.Договор
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПроцентСевернойНадбавкиСотрудниковПоОпеке КАК ПроцентСевернойНадбавкиСотрудников
	|		ПО Начисления.Сотрудник = ПроцентСевернойНадбавкиСотрудников.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТТекущийРазмерРКиСН КАК ТекущийРазмерРКиСН
	|		ПО (УсловияВыплатПоОпеке.Договор = ТекущийРазмерРКиСН.Договор)
	|			И (УсловияВыплатПоОпеке.Организация = ТекущийРазмерРКиСН.Организация)
	|			И (УсловияВыплатПоОпеке.ФизическоеЛицо = ТекущийРазмерРКиСН.ФизическоеЛицо)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ *
	...
	|ИЗ
	|	ВТНачисленияПолныйРасчет КАК НачисленияПолныйРасчет
	|ГДЕ
	|	НЕ НачисленияПолныйРасчет.Сумма = 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНачисленияСтарые
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТСуммыРКиСНДоговоровОпеки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДатыСуммыРКиСН
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНачисленияРКиСНДоговоровОпеки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНачисленияПолныйРасчет"
	;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&КолонкиВТНачисления", СтрСоединить(КолонкиВТНачисления, ","));
...
Процедура ДобавитьУничтожениеВТ(Запрос) Экспорт
	...
	Запрос.Текст = Запрос.Текст + "
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТТекущийРазмерРКиСН
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТСуммыВыплатПоОпеке
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТПроцентСевернойНадбавкиСотрудниковПоОпеке
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
