...
// * Подписант - ОпределяемыйТип.Пользователь,Строка
...
Функция ПустаяТаблицаТребованийКПодписанию() Экспорт
	...
	ТаблицаПодписания.Колонки.Добавить("Подписант", Новый ОписаниеТипов("СправочникСсылка.Пользователи, Строка"));
...
Функция РезультатыПроверкиМаршрутаПоПараметрамНастройки(Маршрут, ИспользуемыеСертификаты, ВидыЭД, 
	...
	Если ТипЗнч(Маршрут) = Тип("СправочникСсылка.МаршрутыПодписания") Тогда
		...
		Если Не ЗначениеЗаполнено(Маршрут) Тогда
			МаршрутПредопределенный = Ложь;
		Иначе
			МаршрутПредопределенный = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Маршрут, "Предопределенный");
		КонецЕсли;
		
		ВыполнятьПроверку = НЕ МаршрутПредопределенный;
...
//
// Параметры:
//  ЭлектронныйДокумент	 - СправочникСсылка.СообщениеЭДОПрисоединенныеФайлы, ДокументСсылка.СообщениеОбменСБанками - ссылка на
...
Функция ТаблицаМаршрутаПоПараметрам(ЭлектронныйДокумент, Маршрут, УсловиеПодписания = Неопределено, 
	...
	РеквизитыМаршрута = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Маршрут, "Предопределенный, Организация, ВидПодписи,
		|ТаблицаТребований");
	
	// Подготовим таблицу маршрута
	Если РеквизитыМаршрута.Предопределенный = Неопределено Или Не РеквизитыМаршрута.Предопределенный Тогда
		ТаблицаПодписания = ПустаяТаблицаТребованийКПодписанию();
		
		Если РеквизитыМаршрута.ТаблицаТребований <> Неопределено Тогда
			ТаблицаТребований = РеквизитыМаршрута.ТаблицаТребований.Выгрузить();
		...
		Если ЗначениеЗаполнено(РеквизитыМаршрута.Организация)
			И РеквизитыМаршрута.ВидПодписи = Перечисления.ВидыЭлектронныхПодписей.УсиленнаяКвалифицированная Тогда
				ТаблицаПодписания.ЗаполнитьЗначения(РеквизитыМаршрута.Организация, "Организация");
		КонецЕсли;
	Иначе
...
Функция ОбновитьМаршрутПриПодписании(ЭлектронныйДокумент, Знач Сертификат, ВесМаршрута = 0,
	...
	Если ТипЗнч(Сертификат) = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
		...
		Попытка
			...
			ЗначениеРеквизитовСертификата = КриптографияБЭД.ЗначениеРеквизитовСертификата(Сертификат, "Организация");
			Организация = ЗначениеРеквизитовСертификата.Организация;
...
//
// Параметры:
//  ЭлектронныеДокументы - Массив из СправочникСсылка.СообщениеЭДОПрисоединенныеФайлы,
...
Функция ПодписантыДляОповещения(ТаблицаПодписания, ОбъектМаршрута) Экспорт
	...
	СтрокиТекущихПодписантов = ТаблицаПодписания.НайтиСтроки(Новый Структура("Текущий, Требование",
		Истина, Перечисления.ТребованияКПодписаниюЭД.ПустаяСсылка()));
...
Функция ЗапросОбъектовДляПодписания(ИмяВременнойТаблицы, Отбор = Неопределено) Экспорт
	...
	ТекстЗапроса = 
		...
		|ГДЕ
		|	СостояниеПодписанияЭД.Текущий
		|	И СостояниеПодписанияЭД.Требование = &ПустоеТребование
	...
	ОписаниеЗапроса.СлужебныеПараметры.Вставить("ПустоеТребование", Перечисления.ТребованияКПодписаниюЭД.ПустаяСсылка());
...
// Параметры:
//  РодительскийУзелДерева - См. ПустоеДеревоТребованийКПодписанию
//                         - СтрокаДереваЗначений Из См. ПустоеДеревоТребованийКПодписанию - также дерево дополняется 
//                           служебными колонками. См. ДополнитьДеревоПодписанияСлужебнымиКолонками
//  УдалятьНевыполнимыеТребования - Булево
//  Оптимизировать - Булево
//  ПроставитьТекущие - Булево
//  УдалятьЛожные - Булево
//  ПросчитатьВес - Булево - максимальное количество доступных подписей. 
//  ЕстьНевыполнимыеТребования - Булево
//
Процедура ОбработатьДеревоПодписания(РодительскийУзелДерева, УдалятьНевыполнимыеТребования, Оптимизировать,
	...
	РодительскийУзелЭтоДерево = (ТипЗнч(РодительскийУзелДерева) = Тип("ДеревоЗначений"));
	Если РодительскийУзелЭтоДерево Или ЗначениеЗаполнено(РодительскийУзелДерева.Требование) Тогда
		СтрокиДляУдаления = Новый Массив;
		
		ПроставлятьТекущиеВоВсехСтроках = РодительскийУзелЭтоДерево
		...
		Для Каждого СтрокаДерева Из РодительскийУзелДерева.Строки Цикл
			...
			ЭтоГруппа = ЗначениеЗаполнено(СтрокаДерева.Требование);
			Если ЭтоГруппа Тогда
				ТекущаяГруппа = ТекущаяГруппа + 1;
			КонецЕсли;
			
			ДоступноПодписаниеСРодителемПоПорядку = Ложь;
			Если РодительскийУзелЭтоДерево Тогда
				ДоступноПодписаниеСРодителемПоПорядку = Ложь;
			ИначеЕсли РодительскийУзелДерева.Требование = Перечисления.ТребованияКПодписаниюЭД.ПоПорядку
				И ТекущаяГруппа = 1 Тогда
				ДоступноПодписаниеСРодителемПоПорядку = Истина;
			ИначеЕсли РодительскийУзелДерева.Требование = Перечисления.ТребованияКПодписаниюЭД.ПоПорядку
				И ТекущаяГруппа > 1 Тогда
				ДоступноПодписаниеСРодителемПоПорядку = Истина;
				Для Каждого СтрокаРодительскогоУзла Из РодительскийУзелДерева.Строки Цикл
					ИндексСтроки = РодительскийУзелДерева.Строки.Индекс(СтрокаРодительскогоУзла);
					Если Не СтрокаРодительскогоУзла.Выполнено
						И ИндексСтроки < ТекущаяГруппа - 1 Тогда
						ДоступноПодписаниеСРодителемПоПорядку = Ложь;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			...
			Если ЭтоГруппа И (ПроставлятьТекущиеВоВсехСтроках Или ДоступноПодписаниеСРодителемПоПорядку) Тогда
				СтрокаДерева.Текущий = Истина; // Установка текущих групп подписантов
			КонецЕсли;
			...
			Иначе
				...
				РодительскийУзелДереваТекущий = Не РодительскийУзелЭтоДерево И РодительскийУзелДерева.Текущий;
				Если Не ЭтоГруппа И РодительскийУзелДереваТекущий И Не ТекущийПроставлен Тогда
					СтрокаДерева.Текущий = Истина; // Установка текущих подписантов
...
Функция ВидОшибкиМаршрутПодписания()
	...
	ОписаниеРешения = НСтр("ru = '<a href = ""%1"">Проверьте:</a>
		|1. Доступность сертификатов, указанных в маршруте подписания.
		|2. Пользователей, указанных в сертификатах.'");
	ВидОшибки.ОписаниеРешения = СтрШаблон(ОписаниеРешения, "Проверьте");
