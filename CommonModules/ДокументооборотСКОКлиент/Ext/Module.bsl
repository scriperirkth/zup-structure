...
Процедура ОповеститьОЗавершенииОтправкиИПерерисоватьПанельОтправки(РезультатОтправки, ДополнительныеПараметры) Экспорт
	...
	Оповестить(РегламентированнаяОтчетностьКлиент.ИмяСобытияЗавершениеОтправкиВКонтролирующийОрган(), ПараметрыОповещения, );
...
Функция КодыВидовОперацийПолученныхСчетовФактур(ЭтоЖурнал = Ложь) Экспорт
	...
	//В соответствии с Письмом ФНС России от 16.07.2013 № АС-4-2/12705@ (в редакции письма ФНС России от 02.04.2025 № ЕА-4-15/3520@)
	ДобавитьКод(СписокКодов, "45", НСтр("ru = '45 - Вычет НДС при приобретении у иностранной организации, п. 2.1 ст. 171 НК РФ'"), НЕ ЭтоЖурнал);
	ДобавитьКод(СписокКодов, "46", НСтр("ru = '46 - Вычет НДС при ввозе товаров с Евразийского экономического союза'"), НЕ ЭтоЖурнал);
	ДобавитьКод(СписокКодов, "48", НСтр("ru = '48 - Вычет НДС за уплаченные суммы НДС в бюджет Республики Беларусь, при реализации товаров физ. лицам'"), НЕ ЭтоЖурнал);
	ДобавитьКод(СписокКодов, "49", НСтр("ru = '49 - Вычет за уплаченные суммы НДС в бюджет Республики Казахстан, при реализации товаров физ. лицам'"), НЕ ЭтоЖурнал);
	ДобавитьКод(СписокКодов, "50", НСтр("ru = '50 - Вычет за уплаченные суммы НДС в бюджет Республики Армения, при реализации товаров физ. лицам'"), НЕ ЭтоЖурнал);
	ДобавитьКод(СписокКодов, "51", НСтр("ru = '51 - Вычет за уплаченные суммы НДС в бюджет Кыргызской Республики, при реализации товаров физ. лицам'"), НЕ ЭтоЖурнал);
...
Функция КодыВидовОперацийВыданныхСчетовФактур(ЭтоЖурнал = Ложь, ЭтоНетКнигаПрод = Ложь) Экспорт
	...
	// В соответствии с Письмом ФНС России от 16.07.2013 № АС-4-2/12705@ (в редакции письма ФНС России от 02.04.2025 № ЕА-4-15/3520@)
	ДобавитьКод(СписокКодов, "45", НСтр("ru = '45 - Вычет НДС при приобретении у иностранной организации, п. 2.1 ст. 171 НК РФ'"), ЭтоНетКнигаПрод);
	ДобавитьКод(СписокКодов, "46", НСтр("ru = '46 - Вычет НДС при ввозе товаров с Евразийского экономического союза'"), ЭтоНетКнигаПрод);
	ДобавитьКод(СписокКодов, "47", НСтр("ru = '47 - Реализация транспортного средства, приобретенного у физ. лица'"), НЕ ЭтоЖурнал ИЛИ ЭтоНетКнигаПрод);
	ДобавитьКод(СписокКодов, "48", НСтр("ru = '48 - Вычет НДС за уплаченные суммы НДС в бюджет Республики Беларусь, при реализации товаров физ. лицам'"), ЭтоНетКнигаПрод);
	ДобавитьКод(СписокКодов, "49", НСтр("ru = '49 - Вычет за уплаченные суммы НДС в бюджет Республики Казахстан, при реализации товаров физ. лицам'"), ЭтоНетКнигаПрод);
	ДобавитьКод(СписокКодов, "50", НСтр("ru = '50 - Вычет за уплаченные суммы НДС в бюджет Республики Армения, при реализации товаров физ. лицам'"), ЭтоНетКнигаПрод);
	ДобавитьКод(СписокКодов, "51", НСтр("ru = '51 - Вычет за уплаченные суммы НДС в бюджет Кыргызской Республики, при реализации товаров физ. лицам'"), ЭтоНетКнигаПрод);
		...
		Если ДокументооборотСКОВызовСервера.ОтчетОтправлен(МЧД, ТекстСообщенияПолучитьОбработкуЭДО) Тогда
			Если ОповещениеОЗавершении <> Неопределено Тогда
				РезультатВыполнения = Новый Структура("Выполнено, ОписаниеОшибки",
					Ложь, НСтр("ru = 'Машиночитаемая доверенность (СФР) уже отправлена.'"));
...
Процедура ПодписатьМЧД(ОповещениеОЗавершении, МЧД, ОтпечатокРуководителя) Экспорт
	...
	ИначеЕсли ТипЗнч(МЧД) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФСС") Тогда
		...
		Если ДокументооборотСКОВызовСервера.ОтчетОтправлен(МЧД, ТекстСообщенияПолучитьОбработкуЭДО) Тогда
			Если ОповещениеОЗавершении <> Неопределено Тогда
				РезультатВыполнения = Новый Структура("Выполнено, ОписаниеОшибки",
					Ложь, НСтр("ru = 'Машиночитаемая доверенность (СФР) уже подписана и отправлена.'"));
				ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, РезультатВыполнения);
			КонецЕсли;
		...
		ИначеЕсли ДокументооборотСКОВызовСервера.МЧДПодписана(МЧД) Тогда
			Если ОповещениеОЗавершении <> Неопределено Тогда
				РезультатВыполнения = Новый Структура("Выполнено, ОписаниеОшибки",
					Ложь, НСтр("ru = 'Машиночитаемая доверенность (СФР) уже подписана.'"));
				ВыполнитьОбработкуОповещения(ОповещениеОЗавершении, РезультатВыполнения);
			КонецЕсли;
...
Процедура ПодписатьМЧДФНСПослеВводаПараметров(
	...
	Если РезультатВыгрузки = Неопределено ИЛИ ТипЗнч(РезультатВыгрузки) = Тип("Структура")
		...
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(?(ДополнительныеПараметры.ЭтоЗаявление,
			НСтр("ru = 'Не удалось выгрузить заявление'"), НСтр("ru = 'Не удалось выгрузить доверенность'")));
		Если ДополнительныеПараметры.ОповещениеОЗавершении <> Неопределено Тогда
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеОЗавершении, ДополнительныеПараметры.Результат);
		КонецЕсли;
		Возврат;
...
Процедура ПодписатьМЧДЕдиногоФорматаПослеПодписания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успех Тогда
		...
		Если Ошибки.Количество() > 0 Тогда
			ОписаниеОшибки = НСтр("ru = 'Не удалось добавить подпись по причине'") + " " + СтрСоединить(Ошибки, Символы.ПС);
			Если ДополнительныеПараметры.ОповещениеОЗавершении <> Неопределено Тогда
				РезультатВыполнения = Новый Структура("Выполнено, ОписаниеОшибки", Ложь, ОписаниеОшибки);
				ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеОЗавершении, РезультатВыполнения);
			КонецЕсли;
...
Процедура ОтправитьМЧДПослеОтправки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЭтоУспех Тогда
		Если НЕ ДополнительныеПараметры.ТолькоПодписание Тогда
			...
			Оповестить(РегламентированнаяОтчетностьКлиент.ИмяСобытияЗавершениеОтправки(), ПараметрыОповещения, ДополнительныеПараметры.ДоверенностьСсылка);
		КонецЕсли;
		...
		Если ДополнительныеПараметры.ОповещениеОЗавершении <> Неопределено Тогда
			...
			Если НЕ ЗначениеЗаполнено(ОписаниеОшибки) Тогда
				ОписаниеОшибки = ?(ДополнительныеПараметры.ТолькоПодписание, ?(ДополнительныеПараметры.ЭтоЗаявление,
						НСтр("ru = 'Не удалось подписать заявление'"), НСтр("ru = 'Не удалось подписать доверенность'")),
					?(ДополнительныеПараметры.ЭтоЗаявление,
						НСтр("ru = 'Не удалось отправить заявление'"), НСтр("ru = 'Не удалось отправить доверенность'")));
			КонецЕсли;
			РезультатВыполнения = Новый Структура("Выполнено, ОписаниеОшибки", Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеОЗавершении, РезультатВыполнения);
		КонецЕсли;
