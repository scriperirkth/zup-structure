...
Процедура ОтменитьИсправление(Форма, ОписаниеТаблицы) Экспорт 
	...
	Сотрудник = ДанныеСтроки[ОписаниеТаблицы.ИмяРеквизитаСотрудник];
	ДанныеСтроки = Неопределено; // Забываем текущие данные, так как после перезаполнения они могут измениться.
	Форма.ПерезаполнитьНачисленияСотрудника(Сотрудник, Ложь);
	
	// Уже после расчета заполняем значения контролируемых полей.
	СтарыеЗначенияКонтролируемыхПолей = Форма.ПолучитьСтарыеЗначенияКонтролируемыхПолей();
	УстановитьЗначенияКонтролируемыхПолей(
		ОписаниеТаблицы.ИмяТаблицы, Форма.Элементы[ИмяТаблицы].ТекущиеДанные, КонтролируемыеПоля, СтарыеЗначенияКонтролируемыхПолей);
...
Процедура ПередУдалениемСтрокиПерерасчета(Форма, ИмяТаблицы, Элемент, Отказ) Экспорт
	...
	КонецЦикла;
 	...
 	Если УдалениеСторно Тогда
		...
		Если ЗарплатаКадрыРасширенныйКлиентСервер.РазрешеноРедактироватьСторно(Форма, ИмяТаблицы) Тогда
