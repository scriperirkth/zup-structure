...
Функция ВыгрузитьИзмененияПрочихОбъектов(МенеджерОбмена) Экспорт
	...
	БылиОшибкиЭтапа = ВыгрузитьГрафикиОтпусковПодразделений(МенеджерОбмена);
	БылиОшибки = БылиОшибки Или БылиОшибкиЭтапа;
...
Функция ДанныеГрафиковОтпусковПредприятия(Ссылки)
	...
	Для каждого СтрокаТЗ Из ДанныеДокументов Цикл
		...
		НоваяСтрока.Состояние 						= СтрокаТЗ.СостояниеСбораГрафиковОтпусков;
		НоваяСтрока.ИнициаторСбора 					= СтрокаТЗ.ИнициаторСбора;
		НоваяСтрока.ВариантСбораГрафикаОтпусков 	= ?(СтрокаТЗ.СборПоПодразделениям, "ПоВыбраннымПодразделениям", "ПоВсемПодразделениям");
		НоваяСтрока.ДатаОкончанияПланирования 		= СтрокаТЗ.ДатаОкончанияСбора;
		Если ЗначениеЗаполнено(СтрокаТЗ.Настройки) Тогда
			
			Настройки = НовоеОписаниеНастройкиПланированияГрафикаОтпусков();
			Настройки.ВариантПланирования 						= ?(СтрокаТЗ.Настройки.ПланированиеОтпусковПоНорме, "ПоНорме", "ПоОстаткам");
			Настройки.РазрешеноНеПланировать 					= СтрокаТЗ.Настройки.РазрешитьОтпускНеПланирую;
			Настройки.НаличиеПериода14ДнейОбязательно 			= СтрокаТЗ.Настройки.ПроверятьПланирование14ДнейОтпуска;
			Настройки.ДоступнаКомпенсация 						= СтрокаТЗ.Настройки.КомпенсацияДополнительныхОтпусков;
			Настройки.ВариантДоступностиГрафиковРуководителю 	= СтрокаТЗ.Настройки.СхемаПредставленияПодразделений;
			НоваяСтрока.НастройкиПланирования = Настройки;
			
		КонецЕсли;
		
		ИдентификаторДокумент.Вставить(СтрокаТЗ.ИдентификаторДокумента, СтрокаТЗ.ГрафикСбора);
...
#Область ВыгрузкаГрафиковОтпусковПодразделений

Функция ВыгрузитьГрафикиОтпусковПодразделений(МенеджерОбмена)
	...
	Ссылки = ТаблицаИзменений.ВыгрузитьКолонку("Ссылка");
	Данные = ДанныеГрафиковОтпусковПодразделений(Ссылки);
...
Функция ДанныеГрафиковОтпусковПодразделений(Ссылки)
	
	Данные = Новый Структура("ТаблицаДанных,ИдентификаторДокумент");
	
	ДанныеДокументов = ИнтеграцияУправлениеПерсоналомСлужебный.ДанныеГрафиковОтпусковПодразделений(Ссылки);
	Если Не ЗначениеЗаполнено(ДанныеДокументов) Тогда
		Возврат Данные;
	КонецЕсли;
	
	ИдентификаторДокумент = Новый Соответствие;
	ТаблицаДанных = НоваяТаблицаГрафикОтпусковПодразделения();
	...
	Для каждого СтрокаТЗ Из ДанныеДокументов Цикл
		...
		НоваяСтрока.ИдентификаторДокумента 				= СтрокаТЗ.ИдентификаторДокумента;
		НоваяСтрока.ИдентификаторГрафикаПредприятия 	= СтрокаТЗ.ИдентификаторГрафикаПредприятия;
		НоваяСтрока.Подразделение 						= СтрокаТЗ.Подразделение;
		НоваяСтрока.Состояние							= СтрокаТЗ.СостояниеСбораГрафиковОтпусков;
		НоваяСтрока.Ответственный						= СтрокаТЗ.ОтветственныйЗаСбор;
		НоваяСтрока.Периоды 							= Новый Массив;
		НоваяСтрока.ПериодыКомпенсации 					= Новый Массив;
		НоваяСтрока.ПожеланияСотрудниковВидныТолькоОтветственному = СтрокаТЗ.ПожеланияСотрудниковВидныТолькоОтветственному;
...
Функция НовоеОписаниеНастройкиПланированияГрафикаОтпусков()
	
	Описание = Новый Структура("
	|ВариантПланирования,
	|РазрешеноНеПланировать,
	|ДоступнаКомпенсация,
	|НаличиеПериода14ДнейОбязательно,
	|ВариантДоступностиГрафиковРуководителю");
	
	Возврат Описание;
	
КонецФункции

Функция НоваяТаблицаГрафикОтпусковПодразделения()
	
	ИменаКолонок = "
	|ИдентификаторДокумента,
	|ИдентификаторГрафикаПредприятия,
	|Год,
	|Состояние,
	|Ответственный,
	|Подразделение,
