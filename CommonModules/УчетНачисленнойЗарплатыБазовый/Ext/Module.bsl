...
Процедура ПриОпределенииДоступныхДляПереводаМакетов(Макеты) Экспорт
	...
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_РасчетныйЛистокНастраиваемый);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_РасчетныйЛистокПерваяПоловинаМесяца);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т49);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т51);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т51ПерваяПоловинаМесяца);
	Макеты.Добавить(Метаданные.Отчеты.АнализНачисленийИУдержаний.Макеты.ПФ_MXL_РасчетныйЛисток);
...
Функция ПравилаУчетаНачисленийСотрудников() Экспорт
	...
	|				ИЛИ НачисленияВтораяТаблица.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛистаЗаСчетРаботодателя)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ДоступноИзменениеСпособаРасчетов
	|ИЗ
	|	ПланВидовРасчета.Начисления КАК Начисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовРасчета.Начисления КАК НачисленияВтораяТаблица
	|		ПО Начисления.ОсновнойВидРасчета = НачисленияВтораяТаблица.Ссылка";
