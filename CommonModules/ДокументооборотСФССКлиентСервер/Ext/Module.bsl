...
Функция ТипыСообщенийСЭДО(ВозвращатьСтроковыеПредставленияКодов = Ложь) Экспорт
	...
	Результат.Вставить("ТипСообщенияУведомлениеОРазмереСтраховыхВзносов", 								31); // Форма "ПротоколОбработкиHTML"
	...
	// ФК Управление доверенностями (МЧД).
	...
	Результат.Вставить("ЗаявлениеНаФормированиеСправкиОРасчетах", 										317);
	...
	Результат.Вставить("ТребованиеОПредставленииПоясненийИлиВнесенииИсправленийВОтчет", 				342);
	// Сообщения ФК "Реестр страхователей"
	Результат.Вставить("РешениеОбУстановленииСкидкиКСтраховомуТарифу", 									600);
	Результат.Вставить("РешениеОбОтменеУстановленнойСкидкиКСтраховомуТарифу", 							601);
	Результат.Вставить("РешениеОбУстановленииНадбавкиКСтраховомуТарифу", 								602);
	// Решениz о несоответствии сведений об ОВЭД
	Результат.Вставить("РешениеОНесоответствииСведенийОбОВЭДПоРезультатамКамеральнойПроверки", 			611);
	Результат.Вставить("РешениеОНесоответствииСведенийОбОВЭДПоРезультатамВыезднойПроверки", 			612);
...
Функция ПараметрыОтправкиВидаДокументаЧерезСЭДО(
	...
	ИначеЕсли ВидОтправляемогоДокумента =
		...
		ЭтоРеестрСтимулирующихВыплат2023 = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			НастройкиВызова,
			"ЭтоРеестрСтимулирующихВыплат2023",
			Ложь);
		ЭтоРеестрСтимулирующихВыплатГосслужащим = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			НастройкиВызова,
			"ЭтоРеестрСтимулирующихВыплатГосслужащим",
			Ложь);
		
		Результат.ОтправкаЧерезСЭДО = ?(ЭтоРеестрСтимулирующихВыплат2023 ИЛИ ЭтоРеестрСтимулирующихВыплатГосслужащим, Ложь,
			?(ПроверитьНаступлениеСобытия,
...
Функция ВыгрузкаРеестраСтимулирующихВыплатМедицинскимИСоциальнымРаботникамДляСЭДО(ТекстВыгрузки)
	...
	ИмяУзлаРеестраГосслужащим = "PaymentsFederalCivilServantsRequest";
	ИмяСхемыРеестраГосслужащим = "CovidWorkerRegistrFCS.xsd";
	...
	Для НомерРеестра = 1 По 2 Цикл
		...
		ИначеЕсли НомерРеестра = 1 Тогда
			Если СтрНайти(Результат, ИмяУзлаРеестраГосслужащим) = 0 Тогда
				ИмяУзлаРеестра = ИмяУзлаРеестра2023;
				ИмяСхемыРеестра = ИмяСхемыРеестра2023;
			Иначе
				ИмяУзлаРеестра = ИмяУзлаРеестраГосслужащим;
				ИмяСхемыРеестра = ИмяСхемыРеестраГосслужащим;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
