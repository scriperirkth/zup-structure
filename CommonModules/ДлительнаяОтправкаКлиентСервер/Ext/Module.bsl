...
Функция ТакаяОшибкаУжеЕсть(Ошибки, Знач НоваяОшибка) Экспорт
	...
	Перем ВхКонтекстыПриОшибкеГОСТ34122018, КонтекстыПриОшибкеГОСТ34122018;
	
	НужныИзмененияАрхива = Ложь;
	НужныИзменения = Ложь;
	НужныИзмененияПриОбработке = Ложь;
	НужныИзмененияПриОшибкеГОСТ34122018 = Ложь;
	НомерСтроки = 0;
	Для каждого Ошибка Из Ошибки Цикл
		Если Ошибка["ОписаниеОшибки"] = НоваяОшибка["ОписаниеОшибки"]
			...
			ИначеЕсли НоваяОшибка.Свойство("ПриОшибкеГОСТ34122018") Тогда
				Если НЕ Ошибка.Свойство("ПриОшибкеГОСТ34122018", ВхКонтекстыПриОшибкеГОСТ34122018) Тогда
					ВхКонтекстыПриОшибкеГОСТ34122018 = Новый Массив;
				КонецЕсли;
				Если НЕ НоваяОшибка.Свойство("ПриОшибкеГОСТ34122018", КонтекстыПриОшибкеГОСТ34122018) Тогда
					КонтекстыПриОшибкеГОСТ34122018 = Новый Массив;
				КонецЕсли;
				Для Каждого Контекст Из КонтекстыПриОшибкеГОСТ34122018 Цикл
					Если ВхКонтекстыПриОшибкеГОСТ34122018.Найти(Контекст) = Неопределено Тогда
						НужныИзмененияПриОшибкеГОСТ34122018 = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			
			Если НЕ НужныИзменения И НЕ НужныИзмененияПриОбработке И НЕ НужныИзмененияПриОшибкеГОСТ34122018 Тогда
				Возврат Истина;
			Иначе
				Прервать;
			КонецЕсли;
	...
	ИначеЕсли НужныИзмененияПриОшибкеГОСТ34122018 Тогда
		Ошибки = Новый Массив(Ошибки);
		Ошибки[НомерСтроки] = Новый Структура(Ошибки[НомерСтроки]);
		Ошибки[НомерСтроки].Счетчик = Ошибки[НомерСтроки].Счетчик + НоваяОшибка.Счетчик;
		
		Если Ошибки[НомерСтроки].Свойство("ПриОшибкеГОСТ34122018", ВхКонтекстыПриОшибкеГОСТ34122018) Тогда
			ВхКонтекстыПриОшибкеГОСТ34122018 = Новый Массив(ВхКонтекстыПриОшибкеГОСТ34122018);
		Иначе
			Ошибки[НомерСтроки].Вставить("ПриОшибкеГОСТ34122018");
			ВхКонтекстыПриОшибкеГОСТ34122018 = Новый Массив;
		КонецЕсли;
		Если НоваяОшибка.Свойство("ПриОшибкеГОСТ34122018", КонтекстыПриОшибкеГОСТ34122018) Тогда
			КонтекстыПриОшибкеГОСТ34122018 = Новый Массив(КонтекстыПриОшибкеГОСТ34122018);
		Иначе
			КонтекстыПриОшибкеГОСТ34122018 = Новый Массив;
		КонецЕсли;
		Для Каждого Контекст Из КонтекстыПриОшибкеГОСТ34122018 Цикл
			Если ВхКонтекстыПриОшибкеГОСТ34122018.Найти(Контекст) = Неопределено Тогда
				ВхКонтекстыПриОшибкеГОСТ34122018.Добавить(Контекст);
			КонецЕсли;
		КонецЦикла;
		
		Ошибки[НомерСтроки].ПриОшибкеГОСТ34122018 = Новый ФиксированныйМассив(ВхКонтекстыПриОшибкеГОСТ34122018);
		Ошибки[НомерСтроки] = Новый ФиксированнаяСтруктура(Ошибки[НомерСтроки]);
		Ошибки = Новый ФиксированныйМассив(Ошибки);
...
Функция НоваяОшибка(Знач ОписаниеОшибки, Знач ОбъектОснование = Неопределено) Экспорт
	...
	Если ОбъектОснование <> Неопределено
		...
		ИначеЕсли ОбъектОснование.Свойство("ПриОшибкеГОСТ34122018", КонтекстСообщение) Тогда
			НоваяОшибка.Вставить("ПриОшибкеГОСТ34122018", Новый Массив);
			НоваяОшибка.ПриОшибкеГОСТ34122018.Добавить(КонтекстСообщение);
			НоваяОшибка.ПриОшибкеГОСТ34122018 = Новый ФиксированныйМассив(НоваяОшибка.ПриОшибкеГОСТ34122018);
		КонецЕсли;
	КонецЕсли;
