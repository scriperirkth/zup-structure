...
Функция ТекстСообщенияФСС(СтруктураDOM)
	...
	Если УзелСообщения <> Неопределено Тогда
		...
		Если ТекстовоеСодержимое <> "" Тогда
			Возврат НСтр("ru = 'При обмене с СФР возникли ошибки. Ответ СФР:'") + Символы.ПС + ТекстовоеСодержимое;
		КонецЕсли;
	КонецЕсли;
	Возврат "";
КонецФункции
...
Функция РазрешенаЗагрузкаЭЛН(СтатусЭЛНВФСС, НомерЛН) Экспорт
	...
	// Карантинные ЭЛН 65+ разрешено загружать почти в любых состояниях (для авансовых выплат за счет СФР).
