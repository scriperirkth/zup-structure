...
Функция СкорректироватьПерерасчетыСУчетомФПД(ДанныеПерерасчетовСторно, ДанныеСотрудников, ПериодРегистрации, ИсправленныйДокумент)
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПовторноИсправленныйДокумент.ИсправленныйДокумент КАК ИсправленныйДокумент
		|ПОМЕСТИТЬ ПовторноИсправленныйДокумент
		|ИЗ
		|	#ПовторноИсправленныйДокумент КАК ПовторноИсправленныйДокумент
		|ГДЕ
		|	ПовторноИсправленныйДокумент.Ссылка = &ИсправленныйДокумент
		|	И ПовторноИсправленныйДокумент.ИсправленныйДокумент <> &ПустаяСсылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПовторноИсправленныйДокумент КАК ПовторноИсправленныйДокумент
		|		ПО НачисленияФактическийПериодДействия.СторнируемыйДокумент = ПовторноИсправленныйДокумент.ИсправленныйДокумент
		|ГДЕ
		|	ПовторноИсправленныйДокумент.ИсправленныйДокумент ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	ПолноеИмяДокумента = ИсправленныйДокумент.Метаданные().ПолноеИмя();
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ПовторноИсправленныйДокумент", ПолноеИмяДокумента);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПустаяСсылка", "ЗНАЧЕНИЕ(" + ПолноеИмяДокумента + ".ПустаяСсылка)");
