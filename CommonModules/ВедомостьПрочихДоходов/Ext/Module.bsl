...
Процедура ОбработкаПроверкиЗаполнения(ДокументОбъект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Для Каждого СтрокаСостава Из ДокументОбъект.Состав Цикл
		...
		ВыгружатьИНН = ОбменСБанкамиПоЗарплатнымПроектам.ЗарплатныйПроектПоддерживаетВыгрузкуИНН(ДокументОбъект.ЗарплатныйПроект);
		...
		Если ((СтрокаСостава.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету) 
			...
			ИначеЕсли (СтрокаСостава.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоСНИЛС) 
				...
				ОбщегоНазначения.СообщитьПользователю(
					...
					Отказ);
			ИначеЕсли (СтрокаСостава.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР) 
				И Не ЗначениеЗаполнено(СтрокаСостава.НомерЛицевогоСчетаЦР) Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'У сотрудника %1 не заполнен счет ЦР'"), 
					СтрокаСостава.ФизическоеЛицо);
				ОбщегоНазначения.СообщитьПользователю(
					ТекстОшибки, 
					ДокументОбъект, 
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Состав", СтрокаСостава.НомерСтроки, "НомерЛицевогоСчетаЦР"),,
					Отказ);
			КонецЕсли;
			Если ВыгружатьИНН И Не ЗначениеЗаполнено(СтрокаСостава.ИНН) Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'У сотрудника %1 не заполнен ИНН'"), 
					СтрокаСостава.ФизическоеЛицо);
				ОбщегоНазначения.СообщитьПользователю(
					ТекстОшибки, 
					ДокументОбъект, 
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Состав", СтрокаСостава.НомерСтроки, "ИНН"),,
					Отказ);
...
Процедура УстановитьВзыскания(Ведомость, ТаблицаВыплат) Экспорт
	...
	Если РассчитыватьВзысканныеСуммыПропорциональноВыплате И ИспользоватьСтатьиФинансированияЗарплата Тогда
		КолонкиГруппировок = КолонкиГруппировок + ", СтатьяФинансирования, СтатьяРасходов";
	КонецЕсли;
	
	// Получаем суммы, взысканные с указанных выплат.
	Финансирование = Новый Структура("СтатьяРасходов, СтатьяФинансирования", Ведомость.СтатьяРасходов, Ведомость.СтатьяФинансирования);
	Взыскания = ВзысканияПоИсполнительнымДокументам(ТаблицаВыплат, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование);
	...
	Для каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		...
		Если СуммаВзысканоФизическогоЛица > 0 Тогда
			...
			Для каждого ВзысканиеПоДокументамОснованиям Из Взыскания Цикл
				Если ВзысканиеПоДокументамОснованиям.СуммаВзыскано <> 0 Тогда
					
					ЗаполнитьЗначенияСвойств(Отбор, ВзысканиеПоДокументамОснованиям);
					ВзысканиеЗарплата = Ведомость.Выплаты.НайтиСтроки(Отбор);
					
					Если ВзысканиеЗарплата.Количество() > 0 Тогда
						// относим на первую строку зарплаты
						Если РассчитыватьВзысканныеСуммыПропорциональноВыплате И ВзысканиеПоДокументамОснованиям.СуммаНачисленнойЗарплаты <> 0 Тогда
	...
	// Переносим также взыскания в ведомость, связывая с первой строкой состава этого физлица.
	Для Каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		СтрокаСостава = Ведомость.Состав.Найти(ФизическоеЛицо, "ФизическоеЛицо");
		Если СтрокаСостава = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Взыскания = Ведомость.Выплаты.НайтиСтроки(Новый Структура("ФизическоеЛицо", ФизическоеЛицо));
...
Функция ВзысканияПоИсполнительнымДокументам(ТаблицаВыплат, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование)
	
	НачисленныеВыплаты = ВзаиморасчетыПоПрочимДоходам.НачисленныеВыплатыФизическихЛицПоДокументам(ТаблицаВыплат, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование);
	...
	Для Каждого Выплата Из Выплаты Цикл
		Если Выплата.Сумма < 0 Тогда
			Задолженности.Добавить(Выплата);
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(Отбор, Выплата);
		НачислениеВыплаты = НачисленныеВыплаты.НайтиСтроки(Отбор);
		
		Если НачислениеВыплаты.Количество() > 0 Тогда
			Выплата.СуммаНачисленнойЗарплаты = НачислениеВыплаты[0].Сумма;
		КонецЕсли;
	КонецЦикла;
	...
	ВзысканияПоДокументамОснованиям = ИсполнительныеЛисты.УдержанныеСуммыФизическихЛицПоДокументам(Выплаты, РассчитыватьВзысканныеСуммыПропорциональноВыплате, Финансирование);
