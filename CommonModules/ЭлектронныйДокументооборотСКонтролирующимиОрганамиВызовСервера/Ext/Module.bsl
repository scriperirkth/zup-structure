...
Процедура ИзменитьЗначениеВФорме1СОтчетность(Ссылка, ИмяПоля, Знач НовоеЗначение) Экспорт
	...
	Попытка
		...
		Если ВРег(ИмяПоля) = ВРег("Скрыт") ИЛИ ВРег(ИмяПоля) = ВРег("СтраницаЖурнала")
			ИЛИ ВРег(ИмяПоля) = ВРег("ДатаНачала") ИЛИ ВРег(ИмяПоля) = ВРег("ДатаНачала")
			ИЛИ ВРег(ИмяПоля) = ВРег("ВидДокумента") Тогда
			
			СтруктураКлюча = Новый Структура("Ссылка", Ссылка);
			Ключ = РегистрыСведений.ЖурналНовыхСобытий.СоздатьКлючЗаписи(СтруктураКлюча);
			ЗаблокироватьДанныеДляРедактирования(Ключ);
			
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ЖурналНовыхСобытий");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", Ссылка);
			Блокировка.Заблокировать();
...
Функция НовыеСобытияПоСтраницамЖурнала() Экспорт
	
	Возврат ДокументооборотСКОПовтИсп.ЭтоРежимОблачной1СО()
		И НЕ ДокументооборотСКОПовтИсп.ЭтоБизКуб();
	
КонецФункции

#КонецОбласти

#Область БаннерПоКнопкеЗаполнить

// Подготавливает структуру параметров для проверки статуса подключения к документообороту.
//
// Возвращаемое значение:
//  Структура:
//   * Организация - СправочникСсылка.Организации, Неопределено - Объект проверки.
//   * КонтролирующийОрган - Строка - Код государственного органа (ФНС, ПФР, ФСС и т.д.).
//   * СсылкаНаОтчет - ЛюбаяСсылка, Неопределено - Ссылка на отчет, из которого инициирована проверка.
//
Функция НовыеПараметрыПоказатьПредложениеОформитьЗаявлениеНаПодключениеКнопкаЗаполнить() Экспорт
...
// Проверяет, настроен ли и используется ли в данный момент электронный документооборот
// с контролирующими органами для конкретной организации.
//
// Параметры:
//  ПараметрыПроверки - Структура - Параметры, полученные через функцию 
//                      НовыеПараметрыПроверкиДокументооборотСОператоромИспользуется.
//
// Возвращаемое значение:
//  Булево - Истина, если документооборот используется (или функционал 1С-Отчетности отключен в константах),
//           Ложь, если документооборот не настроен.
//
Функция ПоказатьПредложениеОформитьЗаявлениеНаПодключениеКнопкаЗаполнить(ПараметрыПроверки) Экспорт
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.Используется1СОтчетность() Тогда
		Возврат Результат;
	КонецЕсли;
	
	КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
	
	НеПодключенаОтчетность = Ложь;
	Если КонтекстЭДОСервер <> Неопределено Тогда
		...
		Если СвойстваОрганизацииИУчетнойЗаписи <> Неопределено
			...
			И (СвойстваОрганизацииИУчетнойЗаписи.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.КалугаАстрал
				ИЛИ СвойстваОрганизацииИУчетнойЗаписи.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.Такском
				ИЛИ СвойстваОрганизацииИУчетнойЗаписи.СпецоператорСвязи = Перечисления.СпецоператорыСвязи.Форус)
			И НЕ КонтекстЭДОСервер.ЛицензияПросрочена(СвойстваОрганизацииИУчетнойЗаписи.УчетнаяЗаписьОбмена) Тогда
			Возврат Результат;
		Иначе
			НеПодключенаОтчетность = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ НеПодключенаОтчетность Тогда
		Возврат Результат;
	КонецЕсли;
	
	// Загружаем настройки по организациям
	ДанныеПоОрганизациям = ХранилищеОбщихНастроек.Загрузить(
		"Обработка.ОбщиеОбъектыРеглОтчетности",
		"НеПоказыватьПредложениеОформитьЗаявлениеНаПодключениеПоОрганизациям");
	
	Если НЕ ДанныеПоОрганизациям = Неопределено
		И ТипЗнч(ДанныеПоОрганизациям) = Тип("Структура") Тогда
		
		// Проверяем наличие организации в настройках с учётом периода повторного показа
		Если ДанныеПоОрганизациям.Свойство("Организации")
			И ТипЗнч(ДанныеПоОрганизациям.Организации) = Тип("Массив") Тогда
			
			Для Каждого ЭлементОрганизации Из ДанныеПоОрганизациям.Организации Цикл
				
				Если ТипЗнч(ЭлементОрганизации) = Тип("Структура")
					И ЭлементОрганизации.Свойство("Организация")
					И ЭлементОрганизации.Организация = Организация Тогда
					
					// Определяем дату следующего показа на основе периода
					ДатаСледующегоПоказа = ДатаСледующегоПоказаБаннера(ЭлементОрганизации);
					
					// Если срок ещё не истёк — не показываем баннер
					Если ТекущаяДата() < ДатаСледующегоПоказа Тогда
						Возврат Результат;
					КонецЕсли;
					
					// Срок истёк — обновляем дату показа и продолжаем
					ЭлементОрганизации.ДатаПоказа = ТекущаяДата();
	...
	ЕстьОтправленныеЗаявления = ОбработкаЗаявленийАбонентаВызовСервера.ЕстьОтправленныеИлиОдобренныеЗаявления(Организация);
	
	ОтчетОтравляетсяЧерезОператора = Ложь;
	
	ТипКонтролирующегоОргана = ВидКонтролирующегоОрганаПоИмениФормы(ИмяФормы);
	
	КООтправкаЧерезОператора = Новый Массив;
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ФСРАР);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.РПН);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ФТС);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ФСС);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ФНС);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ЦБ);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ФСГС);
	КООтправкаЧерезОператора.Добавить(Перечисления.ТипыКонтролирующихОрганов.ПФР);
	
	Если НЕ КООтправкаЧерезОператора.Найти(ТипКонтролирующегоОргана) = Неопределено Тогда
		ОтчетОтравляетсяЧерезОператора = Истина;
	КонецЕсли;
	
	Результат = ОтчетОтравляетсяЧерезОператора
		И НЕ ЕстьОтправленныеЗаявления
		И НеПодключенаОтчетность;
		
	Возврат Результат;
	
КонецФункции

// Вычисляет дату следующего показа баннера на основе периода и даты последнего показа
//
// Параметры:
//   ЭлементОрганизации - Структура - элемент из массива организаций со свойствами:
//     * Организация  - СправочникСсылка - ссылка на организацию
//     * ДатаПоказа   - Дата            - дата последнего показа баннера
//     * ПериодПоказа - Строка          - период: "День", "Неделя", "Месяц"
//
// Возвращаемое значение:
//   Дата - дата следующего допустимого показа баннера
//
Функция ДатаСледующегоПоказаБаннера(ЭлементОрганизации)
	
	ПериодПоказа = ПериодПоказаБаннераДляОрганизации();
	
	ДатаПоказа   = Неопределено;
	
	Если ЭлементОрганизации.Свойство("ДатаПоказа")
		И ЗначениеЗаполнено(ЭлементОрганизации.ДатаПоказа) Тогда
		ДатаПоказа = ЭлементОрганизации.ДатаПоказа;
	КонецЕсли;
	
	// Если дата не задана — показываем сразу
	Если НЕ ЗначениеЗаполнено(ДатаПоказа) Тогда
		Возврат ТекущаяДата();
	КонецЕсли;
	
	Если ПериодПоказа = "День" Тогда
		// Следующий показ через 1 сутки
		ДатаСледующегоПоказа = ДатаПоказа + 86400;
		
	ИначеЕсли ПериодПоказа = "Неделя" Тогда
		// Следующий показ через 7 суток
		ДатаСледующегоПоказа = ДатаПоказа + 7 * 86400;
		
	ИначеЕсли ПериодПоказа = "Месяц" Тогда
		// Следующий показ через 1 календарный месяц
		ДатаСледующегоПоказа = ДобавитьМесяц(ДатаПоказа, 1);
	...
	Возврат ДатаСледующегоПоказа;
	
КонецФункции

// Сохраняет настройку отказа от показа баннера для организации
//
// Параметры:
//   Организация  - СправочникСсылка - ссылка на организацию
//   ПериодПоказа - Строка           - период повторного показа: "День", "Неделя", "Месяц"
//
Процедура СохранитьОтказОтПоказаБаннераДляОрганизации(Организация) Экспорт
	...
	// Инициализируем структуру, если она ещё не создана или имеет неверный тип
	Если ДанныеПоОрганизациям = Неопределено
		ИЛИ НЕ ТипЗнч(ДанныеПоОрганизациям) = Тип("Структура") Тогда
		
		ДанныеПоОрганизациям = Новый Структура;
		ДанныеПоОрганизациям.Вставить("Организации", Новый Массив);
		
	КонецЕсли;
	
	Если НЕ ДанныеПоОрганизациям.Свойство("Организации")
		ИЛИ НЕ ТипЗнч(ДанныеПоОрганизациям.Организации) = Тип("Массив") Тогда
		
		ДанныеПоОрганизациям.Вставить("Организации", Новый Массив);
		
	КонецЕсли;
	
	// Ищем существующую запись для организации
	НайденныйЭлемент = Неопределено;
	Для Каждого ЭлементОрганизации Из ДанныеПоОрганизациям.Организации Цикл
		Если ТипЗнч(ЭлементОрганизации) = Тип("Структура")
			И ЭлементОрганизации.Свойство("Организация")
			И ЭлементОрганизации.Организация = Организация Тогда
			НайденныйЭлемент = ЭлементОрганизации;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если НайденныйЭлемент = Неопределено Тогда
		...
		НовыйЭлемент.Вставить("Организация",  Организация);
		НовыйЭлемент.Вставить("ДатаПоказа",   ТекущаяДата());
		НовыйЭлемент.Вставить("ПериодПоказа", ПериодПоказа);
		ДанныеПоОрганизациям.Организации.Добавить(НовыйЭлемент);
	Иначе
		// Обновляем существующую запись
		НайденныйЭлемент.ДатаПоказа   = ТекущаяДата();
		НайденныйЭлемент.ПериодПоказа = ПериодПоказа;
	КонецЕсли;
...
Функция ПериодПоказаБаннераДляОрганизации()
...
// Возвращает вид контролирующего органа по имени формы отчета.
//
// Параметры:
//   ИмяФормыОтчета - Строка - полное имя формы отчета. Например:
//     "Отчет.РегламентированныйОтчетНДС.Форма.ФормаОтчета2026Кв1"
//     или краткое имя метаданных формы.
//
// Возвращаемое значение:
//   ПеречислениеСсылка.ВидыКонтролирующихОрганов, Неопределено - вид контролирующего органа,
//     определяемый по имени формы
Функция ВидКонтролирующегоОрганаПоИмениФормы(Знач ИмяФормыОтчета) Экспорт
	
	// Проверка входных параметров
	Если НЕ ЗначениеЗаполнено(ИмяФормыОтчета) Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПрефиксВнешний = "Внешний";
	ПрефиксОтчет = "Отчет.";
	ДлинаПрефиксаОтчет = СтрДлина(ПрефиксОтчет);
	ИмяФормыОтчета = СтрЗаменить(ИмяФормыОтчета, ПрефиксВнешний, "");
	// Извлекаем имя отчета из полного пути формы
	ПозицияОтчет = СтрНайти(ИмяФормыОтчета, ПрефиксОтчет);
	Если ПозицияОтчет > 0 Тогда
		// Получаем строку после "Отчет."
		ИмяФормыОтчета = Сред(ИмяФормыОтчета, ПозицияОтчет + ДлинаПрефиксаОтчет);
		// Берем только первую часть до следующей точки (имя объекта метаданных)
		ПозицияТочки = СтрНайти(ИмяФормыОтчета, ".");
		Если ПозицияТочки > 0 Тогда
			ИмяФормыОтчета = Лев(ИмяФормыОтчета, ПозицияТочки - 1);
		КонецЕсли;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ИмяФормыОтчета) Тогда
		Возврат Неопределено;
	...
	// Получаем вид контролирующего органа
	Попытка
		Возврат РегламентированнаяОтчетность.ВидКонтролирующегоОргана(ИмяФормыОтчета);
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка при получении вида контролирующего органа для отчета: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, ,
			ИмяФормыОтчета,
