...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПроверкаПревышенияПлановогоОкладаСотрудников");
	Методы.Вставить("ПроверкаИныхНачисленийВОтпускеПоУходуЗаРебенком");
КонецПроцедуры
...
Функция РезультатПроверкиПревышенияПлановогоОкладаСотрудников()
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НАЧАЛОПЕРИОДА(НачисленияУдержанияПоСотрудникам.ДатаНачала, МЕСЯЦ) = &ТекущийРасчетныйМесяц
