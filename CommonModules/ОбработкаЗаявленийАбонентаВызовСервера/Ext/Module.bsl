...
Функция ОбновитьРеквизитыЗаявления(Документ, РеквизитыДокумента) Экспорт
	...
	Попытка
		...
		ДокументооборотСКОПереопределяемый.ИзменениеСтатусаПродленияСертификатаЭлектроннойПодписи(ДокументОбъект.Ссылка);
...
// Возвращает данные о владельце электронной подписи из последнего одобренного заявления.
//
// Параметры:
...
// Возвращаемое значение:
//  Структура:
//    * ВладелецЭЦП - СправочникСсылка.ФизическиеЛица
//    * ЭлектроннаяПочта - Строка
//
Функция ДанныеВладельцаСертификатаЭлектроннойПодписи(Организация) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ВладелецЭЦП", Неопределено);
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаявлениеАбонентаСпецоператораСвязи.ВладелецЭЦП КАК ВладелецЭЦП,
	|	ЗаявлениеАбонентаСпецоператораСвязи.ЭлектроннаяПочта КАК ЭлектроннаяПочта
	|ИЗ
	|	Документ.ЗаявлениеАбонентаСпецоператораСвязи КАК ЗаявлениеАбонентаСпецоператораСвязи
	...
	Если Выборка.Следующий() Тогда
		Результат.ВладелецЭЦП = Выборка.ВладелецЭЦП;
		Результат.ЭлектроннаяПочта = Выборка.ЭлектроннаяПочта;
	КонецЕсли;
	...
	Если кодРезультата = "0" Тогда
		...
		Если УзелDOM.Количество() > 0 Тогда
			...
			РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
				ИмяФайлаОтвета, "ИмяФайлаОтвета", "ОбменССерверомПолучитьОтвет");
