...
Функция РазобратьРезультатСдачиОтСервера(Знач ФайлРезультата, ОписаниеОшибки)
	...
	Попытка
		...
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ФайлРезультата, "ФайлРезультата", "РазобратьРезультатСдачиОтСервера");		
...
Функция ПолучитьИдентификаторИзФайлаРезультатаОтправки(ФайлРезультата, ОписаниеОшибки = "")
	...
	Попытка
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ФайлРезультата, "ФайлРезультата", "ПолучитьИдентификаторИзФайлаРезультатаОтправки");	
		ОбъектЧтение = Новый ЧтениеXML;
		//@skip-check 1c-fresh-audit
		ОбъектЧтение.ОткрытьФайл(ФайлРезультата);
		Пока ОбъектЧтение.Прочитать() Цикл
...
Функция ЗакодироватьФайлВBase64(Знач ИмяФайлаИсточника, ИмяФайлаПриемника, ОписаниеОшибки)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ИмяФайлаИсточника, "ИмяФайлаИсточника", "ЗакодироватьФайлВBase64");	
	//@skip-check 1c-fresh-audit
...
Функция ЗаписатьСтрокуВФайл(Стр, Знач ИмяФайлаПриемника, ОписаниеОшибки)
	
	Попытка
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ИмяФайлаПриемника, "ИмяФайлаПриемника", "ЗаписатьСтрокуВФайл");			
		//@skip-check 1c-fresh-audit
...
Функция СформироватьФайлПакета(
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьИмяФайла(КороткоеИмяФайлаПакета);
...
Функция ЗаписатьТекстВФайл(ТекстФайла, Знач ПолноеИмяФайла, КодировкаФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ЗаписатьТекстВФайл");	
	Попытка
		//@skip-check 1c-fresh-audit
...
Процедура ЗаписатьФайлInfoПакета(ТаблицаПараметровЗапроса, Знач ПолноеИмяФайлаInfo)
	
	Если НЕ ЗначениеЗаполнено(ПолноеИмяФайлаInfo) Тогда
		ПолноеИмяФайлаInfo = ПолучитьИмяВременногоФайла();
	Иначе
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ПолноеИмяФайлаInfo, "ПолноеИмяФайлаInfo", "ЗаписатьФайлInfoПакета");		
	КонецЕсли;
...
Функция ВыгрузитьСведенияОДоверенностиПоНаправлениюОтчета(Отчет, ДанныеДоверенности, ИмяФайлаДоверенности, Доверенность = Неопределено) Экспорт
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ФайлДоверенности, "ФайлДоверенности", "ВыгрузитьСведенияОДоверенностиПоНаправлениюОтчета");	
	// читаем двоичные данные из файла и удаляем файл с диска
	//@skip-check 1c-fresh-audit
	ДанныеДоверенности = Новый ДвоичныеДанные(ФайлДоверенности);
	ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ФайлДоверенности);
...
Функция ВыгрузитьДеревоДоверенностиВФайл(Узел, ПолноеИмяФайлаДоверенности, ОбъектЗапись = Неопределено)
	...
	Если ЭтоВерхнийУровеньРекурсии Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ПолноеИмяФайлаДоверенности, "ПолноеИмяФайлаДоверенности", "ВыгрузитьДеревоДоверенностиВФайл");		
...
Функция ПрочитатьТекстИзФайла(ИмяФайла, _КодировкаТекста = Неопределено, АвтоопределениеКодировкиXML = Ложь)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ИмяФайла, "ИмяФайла", "ПрочитатьТекстИзФайла");	
	//@skip-check 1c-fresh-audit
