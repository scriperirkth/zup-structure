...
Процедура ПолучитьФайлЧатбота(Чатбот, ОписаниеФайла, ПолученныйФайл)
	...
	Если Не ЗначениеЗаполнено(ХранениеФайла.ИмяФайла) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураURI.ПутьНаСервере) Тогда
			ПолноеИмяФайла = СтруктураURI.ПутьНаСервере;
...
Процедура СохранитьОтклик(Чатбот, Чат, Отправитель)
	...
	ОписаниеФайла = ОписаниеОтклика.ФайлРезюме.Получить();
	ФайлРезюме = Новый Структура();
	ФайлРезюме.Вставить("ИмяФайла", ОписаниеФайла.ИмяФайла);
	ФайлРезюме.Вставить("ПолноеИмя", "");
	ФайлРезюме.Вставить("Хранение", ПоместитьВоВременноеХранилище(ОписаниеФайла.Файл));
	ОписаниеОтклика.ФайлРезюме = ФайлРезюме;
