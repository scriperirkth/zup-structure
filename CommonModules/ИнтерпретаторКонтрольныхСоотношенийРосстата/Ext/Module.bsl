...
Функция ЗначениеВходитВДиапазон(Контекст, Значение, Диапазон)
	Если Диапазон = "*" Тогда 
		Возврат Истина;
	КонецЕсли;
	
	ЗначениеЧисло = Контекст.ОТЧ.ПривестиЗначение(Значение);
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Диапазон, ",") Цикл 
		Если Элт = "*" Тогда
			Возврат Истина;
		КонецЕсли;
		...
		Если СтрНайти(Элт, "-") > 0 Тогда
			...
			ВерхЧисло = Контекст.ОТЧ.ПривестиЗначение(ВерхНиз[0]);
			НизЧисло = Контекст.ОТЧ.ПривестиЗначение(ВерхНиз[1]);
			Если ЗначениеЧисло >= ВерхЧисло И ЗначениеЧисло <= НизЧисло Тогда 
				Возврат Истина;
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ЗначениеЧисло) И ЗначениеЧисло = Контекст.ОТЧ.ПривестиЗначение(Элт) Тогда 
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПреобразоватьПереборПоСтрокам(Контекст)
	КСДляЗамены = Новый Соответствие;
	
	Для Каждого Стр Из Контекст.ТаблицаКонтрольныхСоотношений Цикл 
		Аргументы = ПолучитьВсеАргументыВыражения(Контекст, Стр.rule);
		Если Аргументы.Количество() = 1 Тогда 
			Продолжить;
		КонецЕсли;
		...
		Для Каждого КЗ Из Аргументы Цикл 
			Секция = ПолучитьАргументСкобки(КЗ.Ключ, 1);
			Строка = ПолучитьАргументСкобки(КЗ.Ключ, 2);
			Ключи.Вставить("[" + Секция + "][" + Строка + "]");
			СтрокаЧисло = Контекст.ОТЧ.ПривестиЗначение(Строка);
			Если Контекст.ОТЧ.ПривестиЗначение(Секция) = 0 
				Или (СтрокаЧисло <> 0 И СтрокаЧисло = Цел(СтрокаЧисло)) Тогда 
				Ключи.Вставить(Новый УникальныйИдентификатор);
			КонецЕсли;
		КонецЦикла;
		Если Ключи.Количество() = 1 Тогда
			КСДляЗамены.Вставить(Стр, Аргументы);
		КонецЕсли;
	КонецЦикла;
	
	КСКУдалению = Новый Соответствие;
	ОтборПоСекции = Новый Структура("section");
	Для Каждого КЗ Из КСДляЗамены Цикл 
		Строка = ПолучитьАргументСкобки(КЗ.Ключ.rule, 2);
		ОтборПоСекции["section"] = Прав("000" + ПолучитьАргументСкобки(КЗ.Ключ.rule, 1), 3);
		СтрокиДляДетализации = Новый Соответствие;
		Для Каждого Стр Из Контекст.ТаблицаТиповСтрок.НайтиСтроки(ОтборПоСекции) Цикл 
			Если ЗначениеВходитВДиапазон(Контекст, Стр.row, ПолучитьАргументСкобки(ПолучитьПервыйКлючСоответствия(КЗ.Значение), 2)) Тогда 
				СтрокиДляДетализации.Вставить(Стр.row);
			КонецЕсли;
		КонецЦикла;
		
		Если ЗначениеЗаполнено(СтрокиДляДетализации.Количество()) Тогда
			УИД_КС = Новый УникальныйИдентификатор;
			Для Каждого Стр Из СтрокиДляДетализации Цикл
				НовоеКС = Контекст.ТаблицаКонтрольныхСоотношений.Добавить();
				ЗаполнитьЗначенияСвойств(НовоеКС, КЗ.Ключ);
				НовоеКС.uuid = УИД_КС;
				Для Каждого Арг Из КЗ.Значение Цикл 
					НовоеУсловие = Лев(Арг.Ключ, СтрНайти(Арг.Ключ, "[", , , 2)) 
						+ Формат(Контекст.ОТЧ.ПривестиЗначение(Стр.Ключ), "ЧГ=") 
						+ Сред(Арг.Ключ, СтрНайти(Арг.Ключ, "]", , , 2));
					
					НовоеКС.rule = СтрЗаменить(НовоеКС.rule, Арг.Ключ, НовоеУсловие);
				КонецЦикла;
			КонецЦикла;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки)
	...
	ПреобразоватьПереборПоСтрокам(Контекст);
	Для Каждого Стр Из Контекст.ТаблицаКонтрольныхСоотношений Цикл
...
Функция СформироватьФормулыАвторасчетаМножественныхСтрок(Контекст)
	Попытка
		...
		Возврат Новый Структура("РасчитываемыеЯчейкиМножественныхСтрок, ЗависимостиМножественныхСтрок",
			РасчитываемыеЯчейкиМножественныхСтрок, ЗависимостиМножественныхСтрок);
	Исключение
		Возврат Неопределено;
	КонецПопытки;
КонецФункции
