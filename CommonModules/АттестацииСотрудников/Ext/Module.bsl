...
Процедура УстановитьРабочийСтолСпециалистПоАттестациям(СоставФорм, РабочееМестоУстановлено) Экспорт
	
	Если ДоступенИнтерфейсРабочегоСтолаСпециалистПоАттестациям() И ПравоДоступа("Просмотр", Метаданные.Отчеты["СотрудникиПодлежащиеАттестации"]) Тогда
		СоставФорм.ЛеваяКолонка.Добавить("Отчет.СотрудникиПодлежащиеАттестации.Форма.ФормаДляРабочегоСтола");
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПервоначальноеЗаполнениеВидовАттестаций");
КонецПроцедуры
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.РезультатыАттестацииСотрудников.ЗаполнитьСтатусыГотовности";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("71850ac7-fcc6-4575-8912-f804f0bd1bd1");
	Обработчик.Комментарий =
		НСтр("ru = 'Заполнение статусов готовности резервистов в документах ""Результаты аттестации сотрудников"".'");
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.ЖурналыДокументов.АттестацииСотрудников, Истина);
	
КонецПроцедуры

Процедура ЗаполнитьПоставляемыеПрофилиГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт

	ОписаниеПрофиля = ОписаниеПрофиляУправлениеАттестациями();
	ОписанияПрофилей.Добавить(ОписаниеПрофиля);

КонецПроцедуры

Функция ДоступенИнтерфейсРабочегоСтолаСпециалистПоАттестациям() Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьАттестацииСотрудников") 
		И РольДоступна(Метаданные.Роли.ИнтерфейсРабочегоСтолаСпециалистПоАттестациям);
...
Функция ТекстЗапросаВТСотрудникиВключенныеВГрафикАттестации()
	...
	|	И НЕ ПриказОбАттестацииСотрудниковСотрудники.Ссылка.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ЗапросРезультатыАттестацииСотрудников(ПараметрыЗапроса)
	...
	|	ВидыАттестацийСотрудников.ЭтоВнешняяАттестация КАК ЭтоВнешняяАттестация,
	...
	|	ВЫБОР
	|		КОГДА ДокументРезультатыАттестации.ПриказОбАттестации ЕСТЬ NULL
	|				И ВидыАттестацийСотрудников.ЭтоВнешняяАттестация
	|			ТОГДА ЗНАЧЕНИЕ(Документ.НаправлениеНаВнешнююАттестацию.ПустаяСсылка)
	|		КОГДА ДокументРезультатыАттестации.ПриказОбАттестации ЕСТЬ NULL
	|				И НЕ ВидыАттестацийСотрудников.ЭтоВнешняяАттестация
	|			ТОГДА ЗНАЧЕНИЕ(Документ.ПриказОбАттестацииСотрудников.ПустаяСсылка)
	|		ИНАЧЕ ДокументРезультатыАттестации.ПриказОбАттестации
	|	КОНЕЦ КАК ПриказОбАттестации,
	|	ЕСТЬNULL(ДокументРезультатыАттестации.Номер, """") КАК Номер,
	|	ЕСТЬNULL(ДокументРезультатыАттестации.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК Дата,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.Специальность, ЗНАЧЕНИЕ(Справочник.КлассификаторСпециальностейПоОбразованию.ПустаяСсылка)) КАК Специальность,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.Категория, ЗНАЧЕНИЕ(Справочник.РазрядыКатегорииДолжностей.ПустаяСсылка)) КАК Категория,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.Должность, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)) КАК Должность,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.СерияДокумента, """") КАК СерияДокумента,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.НомерДокумента, """") КАК НомерДокумента,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.ДатаДокумента, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокумента,
	|	ЕСТЬNULL(СтрокиРезультатыАттестации.Основание, """") КАК Основание
	|ИЗ
	|	РегистрСведений.РезультатыАттестацийСотрудников КАК РезультатыАттестацийСотрудников
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
	|		ПО РезультатыАттестацийСотрудников.Сотрудник = Сотрудники.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыАттестацийСотрудников КАК ВидыАттестацийСотрудников
	|		ПО РезультатыАттестацийСотрудников.ВидАттестации = ВидыАттестацийСотрудников.Ссылка
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РезультатыАттестацииСотрудников КАК ДокументРезультатыАттестации
	|		ПО РезультатыАттестацийСотрудников.Регистратор = ДокументРезультатыАттестации.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РезультатыАттестацииСотрудников.РезультатыАттестации КАК СтрокиРезультатыАттестации
...
Процедура ПриОпределенииОбработчиковТекущихДел(Обработчики) Экспорт
	...
	Обработчики.Добавить(АттестацииСотрудников);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьАттестацииСотрудников") Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.РезультатыАттестацийСотрудников)
		Или Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ВнеочередныеАттестацииСотрудников) Тогда
		Возврат;
	КонецЕсли;
	
	Раздел = НСтр("ru = 'Аттестации и квалификационные экзамены'");
	ПредставлениеЗаголовка  = "Аттестации сотрудников";
	Представление  = НСтр("ru = 'Сотрудники, подлежащие аттестации'");
	Подсказка      = НСтр("ru = 'Необходимо провести плановую аттестацию сотрудников.'");
	...
	ДелоЗаголовок.Идентификатор = "СотрудникиПодлежащиеАттестации";
	...
	ПараметрыФормы.Вставить("Отбор", Новый Структура("ПериодОтчета", ПериодОтчета));
	...
	ДелоЗаголовок.Форма          = "Отчет.СотрудникиПодлежащиеАттестации.Форма";
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СотрудникиПодлежащиеАттестации.Сотрудник) КАК КоличествоСотрудников,
	|	СотрудникиПодлежащиеАттестации.Организация КАК Организация
	|ИЗ
	|	ВТСотрудникиПодлежащиеАттестации КАК СотрудникиПодлежащиеАттестации
	|
	|СГРУППИРОВАТЬ ПО
	|	СотрудникиПодлежащиеАттестации.Организация";
	...
	Пока Выборка.Следующий() Цикл
		...
		Дело.Идентификатор  = "СотрудникиПодлежащиеАттестации" + СтрЗаменить(Организация.УникальныйИдентификатор(), "-", "");
		Дело.ЕстьДела       = Истина;
		Дело.Важное         = Истина;
		Дело.Владелец       = Раздел;
		Дело.Представление  = Представление + ", " + Строка(Организация);
		Дело.Количество     = Выборка.КоличествоСотрудников;
		Дело.Подсказка      = Подсказка;
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Отбор", Новый Структура("ПериодОтчета, Организация", ПериодОтчета, Организация));
		ПараметрыФормы.Вставить("СформироватьПриОткрытии", Истина);
		
		Дело.ПараметрыФормы = ПараметрыФормы;
		Дело.Форма          = "Отчет.СотрудникиПодлежащиеАттестации.Форма";
...
Функция ИдентификаторПрофиляУправлениеАттестациями()
	Возврат "67c84028-22f9-48c0-8774-deb5954c90b8";
КонецФункции

Функция ОписаниеПрофиляУправлениеАттестациями(ПраваНаИнтерфейс = Ложь)
	
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Идентификатор = ИдентификаторПрофиляУправлениеАттестациями();
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Управление аттестациями'");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ДобавлениеИзменениеАттестацийСотрудников", "ЗарплатаКадрыПриложения.АттестацииСотрудников");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеАттестацийСотрудников", "ЗарплатаКадрыПриложения.АттестацииСотрудников");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеСведенийОбАккредитацииСпециалистов", "ЗарплатаКадрыПриложения.АккредитацияСпециалистов");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеКадровогоРезерва", "ЗарплатаКадрыПриложения.КадровыйРезерв");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеСвойстваКадровогоРезерваГосударственнойСлужбы", "ЗарплатаКадрыПриложения.КадровыйРезерв.КадровыйРезервГосслужащих");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеСтатусовРезервистов", "ЗарплатаКадрыПриложения.КадровыйРезерв");
	...
	ЗарплатаКадрыРасширенный.ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ИнтерфейсРабочегоСтолаСпециалистПоАттестациям");
