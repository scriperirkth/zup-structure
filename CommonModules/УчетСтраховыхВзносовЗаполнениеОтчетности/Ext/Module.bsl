...
Процедура РасчетПоказателей_РегламентированныйОтчетЕФС1ОСС(НачалоПериода, ОкончаниеПериода, ОрганизацияДляСбораДанных, ОписаниеТаблиц, ПодробныеДанные = Ложь)
	...
	Если ОписаниеТаблиц.Свойство("Доходы2023") Или ОписаниеТаблиц.Свойство("Взносы2023") Или ОписаниеТаблиц.Свойство("СКЕ2023") Или ОписаниеТаблиц.Свойство("ЧисленностьИнвалидов") Или ОписаниеТаблиц.Свойство("ОсновнойОКВЭД2023") Тогда
		...
		|	И НЕ СтраховыеВзносыСведенияОДоходах.ВидДохода В (&НеЯвляетсяОбъектом)
		|	И СтраховыеВзносыСведенияОДоходах.Начисление <> ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ДополнительнаяОблагаемаяБазаРуководителя)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			И (НЕ ДоходыПоФизлицам.ВидДохода В (&НеЯвляетсяОбъектом))
	...
	Если ОписаниеТаблиц.Свойство("ОсновнойОКВЭД2023") Тогда
		Если ИспользоватьСКЕ Тогда
			...
			Если ОписаниеТаблиц.СКЕ2023.Количество() = 0 Тогда
				...
				Запрос.УстановитьПараметр("НадбавкаБюджетная", ДанныеСтавки.НадбавкаБюджетная);
				Запрос.УстановитьПараметр("СкидкаБюджетная", ДанныеСтавки.СкидкаБюджетная);
				...
				|	&СкидкаБюджетная КАК Скидка,
				|	&НадбавкаБюджетная КАК Надбавка,
				|	СведенияОДоходах.БазаФСС_НС КАК БазаНС_ПЗ,
				|	ВЫБОР
				|		КОГДА СведенияОДоходах.Инвалид
				|			ТОГДА СведенияОДоходах.БазаФСС_НС
				...
				|	СведенияОДоходах.Скидка КАК П00211М100006,
				|	СведенияОДоходах.Надбавка КАК П00211М100007,
