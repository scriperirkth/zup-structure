...
// Открывает вопрос в поддержку в соответствии с параметрами, подготовленными на сервере.
//
// Параметры:
//   ВопросВПоддержку - Структура - См. СообщенияВСлужбуТехническойПоддержки.ПодготовитьСообщение().
//       Из документов СЭДО готовится при помощи функции СЭДОФСС.ПодготовитьВопросВПоддержку().
//
Процедура ОткрытьВопросВПоддержку(ВопросВПоддержку) Экспорт
	Если ЗначениеЗаполнено(ВопросВПоддержку.КодОшибки) Тогда
		ИнформированиеПользователяКлиент.ПоказатьПодробности(ВопросВПоддержку.СообщениеОбОшибке);
		Возврат;
	КонецЕсли;
	СообщенияВСлужбуТехническойПоддержкиКлиент.Отправить(ВопросВПоддержку);
КонецПроцедуры
...
Процедура ПродолжитьОтправкуПодтверждениеПолучения(РезультатОтправки, Контекст) Экспорт
	...
	// Возврат результата если сообщение уже отправлено.
	Если Контекст.ТекущийИндекс > Контекст.ВГраница Тогда
		// Обновление вторичных данных документов.
		СЭДОФССВызовСервера.ПослеОтправкиПодтвержденияПолучения(Контекст.МассивСсылок);
		// Вывод результата.
...
Процедура ПолучитьДанныеВложенияВходящегоДокумента(ОбработчикРезультата, Ссылка, ИдентификаторФормы) Экспорт
	...
	Если Контекст.ДанныеФайла = Неопределено Тогда
		Если Контекст.Загружен = Истина Тогда
			Шаблон = НСтр("ru = 'Документ ""%1"" загружен, но файл отказа пустой. Предположительно это ошибка в данных xml-сообщения (на стороне СФР). Рекомендуется обратиться в техническую поддержку.'");
		Иначе
	...
	Контекст.ЗначениеРезультата = Контекст.ДанныеФайла;
	ПредложитьПодтвердитьПолучение(Контекст);
	
КонецПроцедуры

Процедура ПредложитьПодтвердитьПолучение(Контекст) Экспорт
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru = 'Рекомендуется подтвердить получение документа ""%1"".'");
		ТекстВопроса = СтрШаблон(ТекстВопроса, Строка(Контекст.Ссылка));
...
Процедура ПослеОтправкиПодтвержденияПолученияВходящегоДокумента(Результат, Контекст) Экспорт
	Если Контекст.ОбработчикРезультата <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОбработчикРезультата, Контекст.ЗначениеРезультата);
	КонецЕсли;
КонецПроцедуры
...
Процедура НачатьПодборФизлиц(Форма, Обработчик, СтруктураОтбора, МножественныйВыбор = Истина, ТекущееФизическоеЛицо = Неопределено, СписокВыбора = Неопределено) Экспорт
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Свойство("Отбор", СтруктураОтбора);
	ПараметрыФормы.Вставить("РежимВыбора", Истина);
	Если ТекущееФизическоеЛицо <> Неопределено Тогда
		ПараметрыФормы.Вставить("ТекущаяСтрока", ТекущееФизическоеЛицо);
	КонецЕсли;
	...
	БлокироватьОкно = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	
	ОткрытьФорму("Справочник.ФизическиеЛица.ФормаВыбора", ПараметрыФормы, Форма, , , , Обработчик, БлокироватьОкно);
КонецПроцедуры
