...
// Открывает форму печати БРО для уведомления
//
// Параметры:
//  Владелец - Форма клиентского приложения - владелец окна
//  Ссылка   - ДокументСсылка.УведомлениеОСпецрежимахНалогообложения - ссылка на документ, который надо напечатать
//
Процедура ОткрытьПечатнуюФормуУведомления(Владелец, Ссылка) Экспорт
	Если ТипЗнч(Ссылка) <> Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения")
		Или ТипЗнч(Владелец) <> Тип("ФормаКлиентскогоПриложения") Тогда 
		
		ВызватьИсключение "Неправильный параметр";
	КонецЕсли;
	
	СписокПечатаемыхЛистов = УведомлениеОСпецрежимахНалогообложенияВызовСервера.ПечатьУведомленияБРО(Ссылка);
	РегламентированнаяОтчетностьКлиент.ОткрытьФормуПредварительногоПросмотра(
		Владелец, "Открыть", Ложь, СписокПечатаемыхЛистов, Новый Структура("ЗаголовокФормы", Строка(Ссылка)));
КонецПроцедуры
