...
Функция КонтекстПечати(МетаданныеМакета, МассивОбъектов = Неопределено, ОбъектыПечати = Неопределено, ТабличныйДокумент = Неопределено) Экспорт
	...
	Если ТабличныйДокумент = Неопределено Тогда
		ТабличныйДокумент = ТабличныйДокумент(МетаданныеМакета.Имя);
	КонецЕсли;
...
Процедура ВывестиОбластьСФорматомСтрок(ТабличныйДокумент, Макет, ИмяОбласти = Неопределено) Экспорт
	...
	КонецЕсли;
	ФорматыСтрок = ФорматыСтрок(ТаблицаОбласти);
	Для Каждого ФорматСтрок Из ФорматыСтрок Цикл
		ТаблицаОбластиФормата = ТаблицаОбласти.ПолучитьОбласть(ФорматСтрок.Верх, 1, ФорматСтрок.Низ, ФорматСтрок.ШиринаКолонок.Количество());
		// Вывод области в табличный документ.
		НомерПервойСтрокиПриемника = ТабличныйДокумент.ВысотаТаблицы + 1;
		ТабличныйДокумент.Вывести(ТаблицаОбластиФормата);
		...
		Для Счетчик = 1 По ФорматСтрок.ШиринаКолонок.Количество() Цикл
			Колонка = ТабличныйДокумент.Область(ОбластьПриемник.Верх, Счетчик, ОбластьПриемник.Низ, Счетчик);
			Колонка.ШиринаКолонки = ФорматСтрок.ШиринаКолонок[Счетчик - 1];
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
// Возвращает новый табличный документ с параметрами "по умолчанию".
//
// Параметры:
//   ИмяМакета - Строка - Имя макета печатной формы, как оно задано в метаданных.
//   ОриентацияПортрет - Булево - Ориентация страницы. Истина - Портрет, Ложь - Ландшафт.
//
// Возвращаемое значение:
//   ТабличныйДокумент - Пустой документ с параметрами "по умолчанию".
//
Функция ТабличныйДокумент(ИмяМакета = "", ОриентацияПортрет = Истина) Экспорт
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ОриентацияСтраницы = ?(ОриентацияПортрет, ОриентацияСтраницы.Портрет, ОриентацияСтраницы.Ландшафт);
	ТабличныйДокумент.АвтоМасштаб = Истина;
	Если ИмяМакета <> "" Тогда
		ЧистоеИмяМакета = ИмяМакета;
...
Процедура ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ЭДОБЗК.ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
...
Функция ФорматыСтрок(ТаблицаОбласти)
	
	ФорматыСтрок = Новый Массив;
	ШиринаКолонокПредыдущейСтроки = Неопределено;
	ФорматСтрок = Неопределено;
	Для НомерСтроки = 1 По ТаблицаОбласти.ВысотаТаблицы Цикл
		ШиринаКолонокСтроки = ШиринаКолонок(ТаблицаОбласти, НомерСтроки, ШиринаКолонокПредыдущейСтроки);
		Если ШиринаКолонокПредыдущейСтроки = Неопределено
			Или Не ОбщегоНазначения.КоллекцииИдентичны(ШиринаКолонокПредыдущейСтроки, ШиринаКолонокСтроки, , , Истина) Тогда
			
			ШиринаКолонокПредыдущейСтроки = ШиринаКолонокСтроки;
			ФорматСтрок = Новый Структура("Верх,Низ,ШиринаКолонок", НомерСтроки, НомерСтроки, ШиринаКолонокПредыдущейСтроки);
			ФорматыСтрок.Добавить(ФорматСтрок);
		Иначе
			ФорматСтрок.Низ = НомерСтроки;
		КонецЕсли;
	КонецЦикла;
	Возврат ФорматыСтрок;
	
КонецФункции

Функция ШиринаКолонок(ТаблицаОбласти, НомерСтроки, ШиринаКолонокПредыдущейСтроки)
	ШиринаКолонок = Новый Массив;
	Если ШиринаКолонокПредыдущейСтроки = Неопределено Тогда
		ШиринаКолонокПредыдущейСтроки = Новый Массив;
	КонецЕсли;
	ЕстьКолонкиСНулевойШириной = Ложь;
	КоллекцииПохожи = ШиринаКолонокПредыдущейСтроки.Количество() = ТаблицаОбласти.ШиринаТаблицы;
	Для Счетчик = 1 По ТаблицаОбласти.ШиринаТаблицы Цикл
		ОбластьЯчейки = ТаблицаОбласти.Область(НомерСтроки, Счетчик);
		Если ОбластьЯчейки.Лево < ОбластьЯчейки.Право Тогда
			Если ОбластьЯчейки.Верх = НомерСтроки Тогда
				ОбъединеннаяОбласть = ТаблицаОбласти.Область(
					ОбластьЯчейки.Верх,
					ОбластьЯчейки.Лево,
					ОбластьЯчейки.Низ,
					ОбластьЯчейки.Право);
				ОбъединеннаяОбласть.Разъединить();
				Для НомерКолонки = ОбластьЯчейки.Лево По ОбластьЯчейки.Право Цикл
					ШиринаКолонок.Добавить(ТаблицаОбласти.Область(НомерСтроки, НомерКолонки).ШиринаКолонки);
					Если НомерКолонки > Счетчик Тогда
						Счетчик = Счетчик + 1;
					КонецЕсли;
				КонецЦикла;
				ОбъединеннаяОбласть.Объединить();
			Иначе
				ШиринаКолонок.Добавить(0);
				ЕстьКолонкиСНулевойШириной = Истина;
			КонецЕсли;
		Иначе
			ШиринаКолонки = ОбластьЯчейки.ШиринаКолонки;
			Если ШиринаКолонки = 0 Тогда
				Если КоллекцииПохожи И ШиринаКолонокПредыдущейСтроки[ШиринаКолонок.Количество()] <> 0 Тогда
					ЕстьКолонкиСНулевойШириной = Истина;
				КонецЕсли;
			Иначе
				Если КоллекцииПохожи И ШиринаКолонки <> ШиринаКолонокПредыдущейСтроки[ШиринаКолонок.Количество()] Тогда
					КоллекцииПохожи = Ложь;
				КонецЕсли;
			КонецЕсли;
			ШиринаКолонок.Добавить(ШиринаКолонки);
		КонецЕсли;
	КонецЦикла;
	Если ЕстьКолонкиСНулевойШириной И КоллекцииПохожи Тогда
		Для Индекс = 0 По ШиринаКолонок.Количество() - 1 Цикл
			Если ШиринаКолонок[Индекс] = 0 Тогда
				ШиринаКолонок[Индекс] = ШиринаКолонокПредыдущейСтроки[Индекс];
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
