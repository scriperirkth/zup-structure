...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьРеквизитПеречислениеНДФЛВыполнено");
КонецПроцедуры
...
Функция ПараметрыРасчетаДокумента() Экспорт
	...
	ПараметрыРасчета.Вставить("ИспользуютсяТерриторииИлиУсловияТруда", Ложь);	
	ПараметрыРасчета.Вставить("ЗаполнятьТерритории", Ложь);
	ПараметрыРасчета.Вставить("ТаблицаСотрудникТерритория", Неопределено);
...
Функция СведенияОДоходахНДФЛДокумента(Объект, ПараметрыРасчета) Экспорт
	...
	Если Не ПараметрыРасчета.ЗаполнятьТерритории Тогда
		ОтборФизическоеЛицо = Новый Структура("ФизическоеЛицо"); 
		Для Каждого СтрокаНачислений Из ДоходыНДФЛ Цикл
			ЗаполнитьМестоПолученияДоходаВСтроке(ПараметрыРасчета, 
												ПараметрыРасчета.ТаблицаСотрудникТерритория, 
												СтрокаНачислений, 
												"Подразделение", 
												ОтборФизическоеЛицо);
		КонецЦикла;
...
Процедура РаспределитьНДФЛПоСтатьям(Объект, ПараметрыРасчета) Экспорт
	...
	Для Каждого СтрокаНДФЛ Из ПараметрыРасчета.РезультатРасчета.НДФЛ Цикл
		...
		Если Распределение.Количество() > 0 Тогда
			РаспределитьНДФЛПоБазе(СтрокаНДФЛ.Налог, Распределение, "Результат");
		КонецЕсли;
		Для каждого ИмяРесурса Из ИменаРесурсовНалога Цикл
			РаспределениеСПревышения = РаспределенияСПревышения[ИмяРесурса];
			Если РаспределениеСПревышения.Количество() > 0 Тогда
				РаспределитьНДФЛПоБазе(СтрокаНДФЛ[ИмяРесурса], РаспределениеСПревышения, "Результат");
			КонецЕсли;
...
Процедура ПровестиПоУчетам(РежимПроведения, ДокументСсылка, ВидыУчетов, Движения, Объект, Отказ) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Если Не РегистрироватьВзаиморасчеты Тогда
			...
			Если ТипЗнч(Объект) = Тип("ДокументСсылка.РегистрацияПрочихДоходов") Тогда
				КатегорияДоходаПоВидуПрочегоДохода = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВидПрочегоДохода, "КатегорияДохода");
				Для Каждого СтрокаДвижение Из Движения.РасчетыНалогоплательщиковСБюджетомПоНДФЛ Цикл
					Если Не ЗначениеЗаполнено(СтрокаДвижение.КатегорияДохода) Тогда
						СтрокаДвижение.КатегорияДохода = КатегорияДоходаПоВидуПрочегоДохода;	
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Процедура РезультатРасчетаЗаполнитьНачисления(Объект, ПараметрыРасчета) Экспорт
	...
	ОтборФизическоеЛицо = Новый Структура("ФизическоеЛицо"); 
	ТаблицаСотрудникТерритория = СформироватьТаблицуСотрудникТерритория(ПараметрыРасчета);
	...
	Для Каждого СтрокаНачислений Из Объект.НачисленияУдержанияВзносы Цикл
		...
		ЗаполнитьМестоПолученияДоходаВСтроке(ПараметрыРасчета, ТаблицаСотрудникТерритория, НоваяСтрокаНачислений, "МестоПолученияДохода", ОтборФизическоеЛицо);
		Если Не ЗначениеЗаполнено(НоваяСтрокаНачислений.МестоПолученияДохода) Тогда
			НоваяСтрокаНачислений.МестоПолученияДохода = ПредопределенноеЗначение("Справочник.ПодразделенияОрганизаций.ПустаяСсылка");
		КонецЕсли;
...
Процедура РассчитатьНДФЛИВзносыНарастающимИтогом(Объект, ПараметрыРасчета)
	...
	Для Каждого СтрокаРезультатаРасчетаНДФЛ Из РезультатРасчетаНДФЛ.НДФЛ Цикл
		...
		ЗаполнитьМестоПолученияДоходаВСтроке(ПараметрыРасчета, ТаблицаСотрудникТерритория, НоваяСтрокаНДФЛ, "Подразделение", ОтборФизическоеЛицо);
			...
			Если СтрокаНачислений <> Неопределено Тогда 
				...
				Иначе
					...
					Для Каждого СтрокаРаспределения Из СтрокаНачислений.РаспределениеПоСтатьям Цикл
						...
						НоваяСтрокаНДФЛ.Сумма = СтрокаРаспределения.Результат;
					КонецЦикла;
					РаспределитьНДФЛПоБазе(СтрокаНачислений.НДФЛ, ВременнаяТаблицаРаспределенияНДФЛ, "Сумма");
...
Процедура СоздатьВТДанныеДокумента(Объект, ПараметрыРасчета)
	...
	Если ИспользуютсяТерриторииИлиУсловияТруда Тогда
		...
		Если ПараметрыРасчета.ЭтоВыплатаБывшимСотрудникам ИЛИ   
			ТИПЗНЧ(ПараметрыРасчета.ДанныеШапки.Ссылка) = ТИП("ДокументСсылка.РегистрацияПрочихДоходов") Тогда 
			...
			Если НЕ ПараметрыРасчета.ЗаполнятьТерритории Тогда
				...
				|	ВЫБОР
				|		КОГДА ДанныеДокумента.МестоПолученияДохода ЕСТЬ NULL
				|				ИЛИ ДанныеДокумента.МестоПолученияДохода = НЕОПРЕДЕЛЕНО
				|				ИЛИ ДанныеДокумента.МестоПолученияДохода = ЗНАЧЕНИЕ(Справочник.ТерриторииВыполненияРабот.ПустаяСсылка)
				|			ТОГДА ДанныеДокумента.Подразделение
				|		ИНАЧЕ ДанныеДокумента.МестоПолученияДохода
...
Процедура СоздатьВТНачисленияДокумента(Объект, ПараметрыРасчета)
	...
	Если ПараметрыРасчета.ИспользоватьСтатьиФинансирования Тогда 
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.ОтражениеЗарплатыВБухгалтерскомУчетеРасширенная") Тогда
			МодульОтражениеЗарплатыВБухучетеРасширенный = ОбщегоНазначения.ОбщийМодуль("ОтражениеЗарплатыВБухучетеРасширенный");
			ИспользоватьБюджетныйТарифНСиПЗ = МодульОтражениеЗарплатыВБухучетеРасширенный.ИспользоватьБюджетныйТарифНСиПЗ(Объект.ПериодРегистрации);
		...
		Если ИспользоватьБюджетныйТарифНСиПЗ Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЛОЖЬ КАК ИспользоватьБюджетныйТарифНСиПЗ,","");
			...
			ОписаниеПоля = "ЕстьNULL(СтатьиБюджетныйТарифНСиПЗ.БюджетныйТарифНСиПЗ, ЛОЖЬ)";
			ЗапросВыбора.Операторы[0].ВыбираемыеПоля.Добавить(ОписаниеПоля);
			ЗапросВыбора.Колонки[ЗапросВыбора.Колонки.Количество()-1].Псевдоним = "ИспользоватьБюджетныйТарифНСиПЗ";
...
Функция ОсновнойБанковскийСчетКонтрагента(Контрагент) Экспорт 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.БазовыеСправочныеДанныеРасширенная") Тогда
		...
		Возврат МодульОсновныеБанковскиеСчета.ОсновнойБанковскийСчетКонтрагента(Контрагент);
	КонецЕсли;
...
Процедура СоздатьВТОсновныеБанковскиеСчетаКонтрагентов(МенеджерВременныхТаблиц, СписокКонтрагентов, Банк = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.БазовыеСправочныеДанныеРасширенная") Тогда
		МодульОсновныеБанковскиеСчета = ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ОсновныеБанковскиеСчетаКонтрагентов");
		МодульОсновныеБанковскиеСчета.СоздатьВТОсновныеБанковскиеСчетаКонтрагентов(МенеджерВременныхТаблиц, СписокКонтрагентов, Банк);
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	МИНИМУМ(БанковскиеСчетаКонтрагентов.Ссылка) КАК БанковскийСчетКонтрагента
		|ПОМЕСТИТЬ ВТБанковскиеСчетаКонтрагентов
		|ИЗ
		|	Справочник.БанковскиеСчетаКонтрагентов КАК БанковскиеСчетаКонтрагентов
		|ГДЕ
		|	БанковскиеСчетаКонтрагентов.Владелец В(&СписокКонтрагентов)
		...
		|	БанковскиеСчетаКонтрагентов.БанковскийСчетКонтрагента.Банк КАК Банк,
		|	БанковскиеСчетаКонтрагентов.БанковскийСчетКонтрагента.НомерСчета КАК НомерСчета
		|ПОМЕСТИТЬ ВТОсновныеБанковскиеСчетаКонтрагентов
		|ИЗ
		|	ВТБанковскиеСчетаКонтрагентов КАК БанковскиеСчетаКонтрагентов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
...
Функция ДанныеДляПроведенияВзносы(МенеджерВременныхТаблиц)
	...
	|	ДанныеДокумента.ИспользоватьБюджетныйТарифНСиПЗ КАК ИспользоватьБюджетныйТарифНСиПЗ
	|ИЗ
...
Функция ДанныеДляПроведения(Объект, СтруктураВидовУчета)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.РегистрацияПрочихДоходов") Тогда
		НастройкиРасчета = Объект.ПараметрыРасчета();
		
		Если НЕ НастройкиРасчета.ЗаполнятьТерритории Тогда
			
			ТаблицаНачисленияУдержанияВзносы = Объект.НачисленияУдержанияВзносы.Выгрузить();
			ТаблицаНачисленияУдержанияВзносы.Свернуть("ФизическоеЛицо,МестоПолученияДохода");
			
			Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
				Для Каждого СтрокаТаблицаНачисленияУдержанияВзносы Из ТаблицаНачисленияУдержанияВзносы Цикл
					Если НЕ ЗначениеЗаполнено(СтрокаТаблицаНачисленияУдержанияВзносы.МестоПолученияДохода) Тогда
						СтрокаТаблицаНачисленияУдержанияВзносы.МестоПолученияДохода = Объект.Подразделение;		
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	
			НастройкиРасчета.ТаблицаСотрудникТерритория = ТаблицаНачисленияУдержанияВзносы; 
			...
			Иначе
				...
				Для Каждого СтрокаНДФЛ Из ДанныеДляПроведения.НДФЛ Цикл
					...
					Если РаспределениеПоСтатьям.Количество() > 0 Тогда
						РаспределитьНДФЛПоБазе(СтрокаНДФЛ.НалогПоСтавке13, РаспределениеПоСтатьям, "Результат");
		...
		Иначе
			...
			Для Каждого СтрокаНачислений Из ПараметрыРасчета.РезультатРасчета.НачисленияУдержанияВзносы Цикл
				...
				Если СтрокаНачислений.РаспределениеПоСтатьям <> Неопределено И СтрокаНачислений.РаспределениеПоСтатьям.Количество() > 0 Тогда 
					...
					РаспределитьНДФЛПоБазе(СтрокаНачислений.НДФЛ, РаспределениеДоходов, "Результат");
...
Функция ТаблицаНачисленияУдержанияВзносы()
	...
	Таблица.Колонки.Добавить("МестоПолученияДохода", Метаданные.ОпределяемыеТипы.ТерриторияВыполненияРаботВОрганизации.Тип);
...
Функция ТерриторииСотрудника(ПараметрыРасчета)
	
	ФизическиеЛицаДокумента = ПараметрыРасчета.СписокФизическихЛиц;

	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ЗапросТерриторииСотрудника(ПараметрыРасчета, ФизическиеЛицаДокумента, Запрос);  

	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВТТерриторииСотрудников.Сотрудник КАК Сотрудник,
	|	ВТТерриторииСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ВТТерриторииСотрудников.Территория КАК МестоПолученияДохода
	|ИЗ
	|	ВТТерриторииСотрудников КАК ВТТерриторииСотрудников";

	ТаблицаСотрудникТерритория = Запрос.Выполнить().Выгрузить();
	
	Возврат ТаблицаСотрудникТерритория; 
	
КонецФункции

Функция ЗапросТерриторииСотрудника(ПараметрыРасчета, ФизическиеЛица, Запрос = Неопределено)
	...
	ПараметрыПолученияСотрудниковОрганизаций = КадровыйУчет.ПараметрыПолученияСотрудниковОрганизацийПоСпискуФизическихЛиц();
	ПараметрыПолученияСотрудниковОрганизаций.Организация = ПараметрыРасчета.ДанныеШапки.Организация;
	ПараметрыПолученияСотрудниковОрганизаций.НачалоПериода = НачалоМесяца(ПараметрыРасчета.ДанныеШапки.ПланируемаяДатаВыплаты);
	ПараметрыПолученияСотрудниковОрганизаций.СписокФизическихЛиц = ФизическиеЛица;
	КадровыйУчет.СоздатьВТПоследниеРаботавшиеСотрудникиФизическихЛиц(Запрос.МенеджерВременныхТаблиц, Истина,
		ФизическиеЛица, ПараметрыПолученияСотрудниковОрганизаций, "ВТПоследниеСотрудники");
...
Процедура ЗаполнитьМестоПолученияДоходаВСтроке(ПараметрыРасчета, 
											ТаблицаСотрудникТерритория, 
											НоваяСтрока, 
											ИмяРеквизитаТерритория, 
											ОтборФизическоеЛицо)

	Если ПараметрыРасчета.ИспользуютсяТерриторииИлиУсловияТруда И 
		(ТИПЗНЧ(ПараметрыРасчета.ДанныеШапки.Ссылка) = ТИП("ДокументСсылка.РегистрацияПрочихДоходов") 
			ИЛИ ТИПЗНЧ(ПараметрыРасчета.ДанныеШапки.Ссылка) = ТИП("ДокументСсылка.ВыплатаБывшимСотрудникам")) Тогда
		
			ОтборФизическоеЛицо.ФизическоеЛицо = НоваяСтрока.ФизическоеЛицо;
			СтрокиТерриторий = ТаблицаСотрудникТерритория.НайтиСтроки(ОтборФизическоеЛицо);  
			
			Если СтрокиТерриторий.Количество() > 0 Тогда
				НоваяСтрока[ИмяРеквизитаТерритория] = СтрокиТерриторий[0].МестоПолученияДохода;
			КонецЕсли;
...
Функция СформироватьТаблицуСотрудникТерритория(ПараметрыРасчета)
	
	ТаблицаСотрудникТерритория = Новый ТаблицаЗначений;
	
	Если ПараметрыРасчета.ИспользуютсяТерриторииИлиУсловияТруда И 
		(ТИПЗНЧ(ПараметрыРасчета.ДанныеШапки.Ссылка) = ТИП("ДокументСсылка.РегистрацияПрочихДоходов") 
			ИЛИ ТИПЗНЧ(ПараметрыРасчета.ДанныеШапки.Ссылка) = ТИП("ДокументСсылка.ВыплатаБывшимСотрудникам")) Тогда
		   
		Если ПараметрыРасчета.ЗаполнятьТерритории Тогда 
			ТаблицаСотрудникТерритория = ТерриторииСотрудника(ПараметрыРасчета);
		Иначе
			Если ПараметрыРасчета.ТаблицаСотрудникТерритория.Количество() = 0 Тогда
				ТаблицаСотрудникТерритория = ТерриторииСотрудника(ПараметрыРасчета);
			Иначе	
				ТаблицаСотрудникТерритория = ПараметрыРасчета.ТаблицаСотрудникТерритория; 
			КонецЕсли;
		КонецЕсли;
...
Процедура РаспределитьНДФЛПоБазе(Сумма, Распределение, ИмяКолонки)
	
	МассивБазовыхЗначений = ОбщегоНазначения.ВыгрузитьКолонку(Распределение, ИмяКолонки);
	РезультатРаспределения = ЗарплатаКадры.РаспределитьСуммуПропорциональноБазе(Сумма, МассивБазовыхЗначений, 0);
	
	Если РезультатРаспределения = Неопределено Тогда
		Для Индекс = 0 По Распределение.Количество()-1 Цикл
			Распределение[Индекс][ИмяКолонки] = 0;
		КонецЦикла;
	Иначе
		Для Индекс = 0 По Распределение.Количество()-1 Цикл
			Распределение[Индекс][ИмяКолонки] = РезультатРаспределения[Индекс];
		КонецЦикла;
	КонецЕсли;
