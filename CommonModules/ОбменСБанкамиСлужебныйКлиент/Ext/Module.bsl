...
Процедура ПодготовитьЗапросыВыписокДляОтправкиЧерезВК(Оповещение, НастройкаОбмена, ПараметрыЗапроса, ЭтоСинхронизация = Ложь) Экспорт
	...
	ПараметрыФормирования = ОбменСБанкамиКлиентСервер.ПараметрыПолученияВыпискиБанка();
...
//
Процедура ОтобразитьСекретныеДанные(Форма, Элемент, ИмяРеквизита, СтандартнаяОбработка) Экспорт
...
Процедура ЗапроситьВыпискуПослеПолученияМаркераБанка(ИдентификаторСессии, Параметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторСессии) Тогда
		Если Параметры.Свойство("ОповещениеПослеПолученияВыписки") Тогда
			Результат = Новый Структура;
			Результат.Вставить("Успех", Ложь);
			Результат.Вставить("Ошибка", НСтр("ru = 'Возникли проблемы с подключением.'"));
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеПолученияВыписки, Результат);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура НастроитьОбменСБанками(НастройкиОбъектов, Действия)
	...
	Если Счетчик = 1 Тогда
		...
		ПараметрыНастройки.Вставить("ТребуетсяПолныйОбмен", Истина);
		ОбменСБанкамиКлиент.ОткрытьСоздатьНастройкуОбмена(Организация, Банк, , Оповещение, ПараметрыНастройки);
	Иначе
