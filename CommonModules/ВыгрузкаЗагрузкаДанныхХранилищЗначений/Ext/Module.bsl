...
// Выполняется перед выгрузкой настроек.
// 
// Параметры:
...
Функция ЗаписатьХранилищеЗначенияВАртефакт(Контейнер, Знач ЗначениеХранилища)
	...
	// Двоичные данные более 64 Кб выгружать отдельными bin-файлами, остальные вместе с данными.
	ВыгружатьКакБинарное = ТипЗнч(ЗначениеХранилища) = Тип("ДвоичныеДанные") И ЗначениеХранилища.Размер() > 64 * 1024;
...
Функция ЗагрузитьДанныеСериализуемогоЗначения(ЗначениеАртефакта)
	...
	Если ТипЗнч(ЗначениеАртефакта) = Тип("ОбъектXDTO") Тогда
		...
		Исключение
			Если ЗначениеАртефакта.Тип() = XDTOТипНастройкиКомпоновкиДанных() Тогда
...
Функция XDTOТипНастройкиКомпоновкиДанных()
