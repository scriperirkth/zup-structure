...
// Функция возвращает структуру с описанием параметров для ОткрытьФормуПодбораИзКлассификатора.
//
// Возвращаемое значение:
//   Структура со свойствами, имеющими значение Неопределено:
// 	* ИмяСправочника	- обязательный. Строка, имя справочника как оно задано в конфигураторе.
// 	* ПолныйПутьКМакету	- обязательный. Строка - формат полного пути:
//												"Документ.<ИмяДокумента>.<ИмяМакета>"
//												"Обработка.<ИмяОбработки>.<ИмяМакета>"
//												"ОбщийМакет.<ИмяМакета>".
// 	* Заголовок			- обязательный. Строка.
...
// 	* ЗаголовкиПолей - Соответствие, задает соответствие заголовков полей именам полей классификатора.
// 	* ЗначенияЗамены - Соответствие, задает соответствие значений ссылочного типа строковым значениям классификатора.
// 	* ЗагружатьИерархию - Булево, если Истина элемент классификатора загружается вместе со всеми родителями.
//
Функция ОписаниеПараметровПодбораИзКлассификатора() Экспорт
	...
	|Заголовок,
	|СоответствиеПолей,
	|ЗаполнятьДополнительныеСведения,
	|ЗаголовкиПолей,
	|ЗначенияЗамены,
	|ЗагружатьИерархию");
...
//			ПараметрыПодбора				- см. ОписаниеПараметровПодбораИзКлассификатора.
//			ОповещениеЗавершения			- ОписаниеОповещения, которое необходимо выполнить по окончании
//												выбора элементов классификатора.
//
Процедура ОткрытьФормуПодбораИзКлассификатора(Форма, ПараметрыПодбора, ОповещениеЗавершения = Неопределено) Экспорт
	
	СтруктураПараметров = Новый Структура("ИмяСправочника,ПолныйПутьКМакету,Заголовок,ЗаполнятьДополнительныеСведения,ЗагружатьИерархию");
	ЗаполнитьЗначенияСвойств(СтруктураПараметров, ПараметрыПодбора);
		
	Если ПараметрыПодбора.СоответствиеПолей <> Неопределено Тогда
		СтруктураПараметров.Вставить("СоответствиеПолей", ПараметрыПодбора.СоответствиеПолей);
	КонецЕсли;
	Если ПараметрыПодбора.ЗаголовкиПолей <> Неопределено Тогда
		СтруктураПараметров.Вставить("ЗаголовкиПолей", ПараметрыПодбора.ЗаголовкиПолей);
	КонецЕсли;
	Если ПараметрыПодбора.ЗначенияЗамены <> Неопределено Тогда
		СтруктураПараметров.Вставить("ЗначенияЗамены", ПараметрыПодбора.ЗначенияЗамены);
	КонецЕсли;
...
//
Процедура ВыбратьСотрудниковРаботающихНаДатуПоПараметрамОткрытияФормыСписка(ВладелецФормыВыбора,
	Организация = Неопределено, Подразделение = Неопределено, ДатаПримененияОтбора = '00010101',
