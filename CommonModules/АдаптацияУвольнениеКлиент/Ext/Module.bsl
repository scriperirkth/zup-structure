...
Процедура ПоказатьВопросПерезаполнитьНазначенныеМерпориятия(Форма) Экспорт
	
	Если Не АдаптацияУвольнениеВызовСервера.ИспользуетсяАдаптацияУвольнение() Тогда
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru = 'Перезаполнить назначенные мероприятия?
						|Текущий список мероприятий будет очищен и заново заполнен.
						|Исполнители мероприятий будут актуализированы.'");
	...
	ПараметрыВопросаПользователю.Заголовок = НСтр("ru = 'Мероприятия'");
	ПараметрыВопросаПользователю.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
	
	ОбработчикОповещения = Новый ОписаниеОповещения("ПерезаполнитьНазначенныеМерпориятияЗавершение", Форма);
	СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
		ОбработчикОповещения, ТекстВопроса, Кнопки, ПараметрыВопросаПользователю);
