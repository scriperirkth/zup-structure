...
// Возвращает текущую настройку использования обмена документами с порталом "Работа России".
//
// Возвращаемое значение:
//  Булево - если Истина, передача доступна.
//
Функция ДоступнаПередачаДокументовНаРаботаРоссии() Экспорт
	
	// Не всем потребителям требуются базовые роли БЗК.
	Если ПравоДоступа("Чтение", Метаданные.Константы.ИспользоватьИнтеграциюСРаботаРоссии) Тогда
		Возврат Константы.ИспользоватьИнтеграциюСРаботаРоссии.Получить();
	КонецЕсли;
...
Процедура ПередатьНаПортал(ПрисоединенныйФайл) Экспорт
	...
	ИдентификаторДокумента = РегистрыСведений.ИдентификаторыДокументовНаРаботаРоссии.ИдентификаторДокумента(ПрисоединенныйФайл);
...
Процедура ВыполнитьПередачуПечатныхФорм() Экспорт
	...
	Если Не ДоступнаПередачаДокументовНаРаботаРоссии() Тогда
		Возврат;
	КонецЕсли;
	...
	ЗаданияКИсполнению = РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.ФайлыКОбработке(
		Ложь, Перечисления.ДействияСФайламиДокументовЭДОБЗК.ПередатьНаРаботаРоссии);
	
	СписокЗаданий = ЗаданияКИсполнению.Получить(Перечисления.ДействияСФайламиДокументовЭДОБЗК.ПередатьНаРаботаРоссии);
...
Функция НастройкиАвторизации(Знач Организация) Экспорт
	...
	Если СохраненныеНастройки = Неопределено Или (СохраненныеНастройки.Токен = Неопределено И СохраненныеНастройки.Логин = Неопределено) Тогда
		Владелец = ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Константа.ИспользоватьИнтеграциюСРаботаРоссии");
...
Процедура ЗапомнитьНастройкиАвторизации(Организация, Логин, Пароль, Идентификатор, Токен, ИспользоватьТокен) Экспорт
	...
	Иначе
		Владелец = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Константы.ИспользоватьИнтеграциюСРаботаРоссии.ПолноеИмя());
	КонецЕсли;
...
// Обработчик регламентного задания ПолучениеДокументовРаботаРоссии
//
Процедура ПолучениеДокументовРаботаРоссии() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии);
...
Процедура ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ДоступнаПередачаДокументовНаРаботаРоссии() Тогда
		
		КомандаФормы = КомандаПодписатьФормыПечатьДокументов(Форма);
		Если КомандаФормы = Неопределено Тогда
			
			СвойстваКоманды = СвойстваКомандыПередачиНаПорталРаботаРоссии();
...
Процедура ПриСозданииФормыСпискаФайлов(Форма) Экспорт
	...
	Если ЗарплатаКадры.ЭтоОбъектЗарплатноКадровойБиблиотеки("Справочник." + Форма.ИмяСправочникаХранилищаФайлов) Тогда
		...
		Если ДоступнаПередачаДокументовНаРаботаРоссии() Тогда
			...
			КнопкаПередатьНаРаботаРоссии = Форма.Элементы.Найти("ПередатьНаРаботаРоссии");
			Если КнопкаПередатьНаРаботаРоссии <> Неопределено Тогда
...
Функция СвойстваКомандыПередачиНаПорталРаботаРоссии() Экспорт
	Возврат Новый Структура("Заголовок,Подсказка,Картинка",
		НСтр("ru = 'Передать на ""Работа России""'"),
		НСтр("ru = 'Подписать и передать на портал ""Работа России""'"),
		БиблиотекаКартинок.ПередатьНаРаботаРоссии);
КонецФункции
...
Функция КлючНастроекОрганизации()
	...
	Возврат "НастройкиИнтеграцииСРаботаРоссии";
...
Функция ПолучитьАрхивыДокументов()
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ИдентификаторыДокументовНаРаботаРоссии.Идентификатор КАК Идентификатор,
		|	ДокументЭДОБЗК.Организация КАК Организация,
		|	&ТекущаяДата > ДОБАВИТЬКДАТЕ(ИдентификаторыДокументовНаРаботаРоссии.ДатаОтправки, МЕСЯЦ, 1) КАК Устарело
		|ИЗ
		|	РегистрСведений.ИдентификаторыДокументовНаРаботаРоссии КАК ИдентификаторыДокументовНаРаботаРоссии
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ДокументЭДОБЗК КАК ДокументЭДОБЗК
		|		ПО (ДокументЭДОБЗК.ЭлектронныйДокумент = ИдентификаторыДокументовНаРаботаРоссии.ПрисоединенныйФайл)
		|ГДЕ
		|	ИдентификаторыДокументовНаРаботаРоссии.ТребуетсяПодпись";
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Устарело Тогда
				РегистрыСведений.ИдентификаторыДокументовНаРаботаРоссии.ЗапомнитьПолучениеПодписиСотрудика(Выборка.Идентификатор);
				Продолжить;
			КонецЕсли;
...
Процедура СохранитьПодписьРаботника(ДанныеПодписи, ИдентификаторДокумента)
	...
	Документ = РегистрыСведений.ИдентификаторыДокументовНаРаботаРоссии.ДокументПоИдентификатору(ИдентификаторДокумента);
	...
	РегистрыСведений.ИдентификаторыДокументовНаРаботаРоссии.ЗапомнитьПолучениеПодписиСотрудика(ИдентификаторДокумента, ДанныеПодписи.ДатаПодписи, Документ);
...
Функция ПолучитьСНИЛСВнешнихПодписантов(ИдентификаторДокумента)
	...
	Запрос.Текст =
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИдентификаторыДокументовНаРаботаРоссии КАК ИдентификаторыДокументовНаРаботаРоссии
		|		ПО (ИдентификаторыДокументовНаРаботаРоссии.Идентификатор = &ИдентификаторДокументаРПП)
		|			И (ИдентификаторыДокументовНаРаботаРоссии.ПрисоединенныйФайл = ВнешниеПодписанты.Ссылка.ЭлектронныйДокумент)";
...
Функция ПередатьПрисоединенныйФайл(Организация, ДанныеФайла)
	...
	РегистрыСведений.ИдентификаторыДокументовНаРаботаРоссии.ЗапомнитьИдентификатор(ДанныеФайла.Ссылка, ИдентификаторДокумента);
...
Процедура УдалитьФайлИзОбработки(ПрисоединенныйФайл)
	...
	РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.УдалитьФайлыИзОбработки(
		...
		Перечисления.ДействияСФайламиДокументовЭДОБЗК.ПередатьНаРаботаРоссии);
...
Функция ИмяКомандыПодписатьФормыПечатьДокументов()
	...
	Возврат "ПередатьПодписанныеPDFНаРаботаРоссии";
	
КонецФункции

Функция ИмяСобытия()
	
	Возврат НСтр("ru = 'Интеграция с ""Работа России""'");
...
Функция ЕстьАктивныеФоновыеЗаданияПолучениеДокументов()
	...
	Возврат ЕстьАктивныеФоновыеЗадания(Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии.Ключ);
...
Функция ПроверитьРегламентныеЗадания() Экспорт
	...
	ИспользуемыеЗадания.Добавить(Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии);
...
Процедура ОбновитьРасписаниеЗаданияПолучениеДокументовРаботаРоссии()
	
	НеобходимоИспользовать = ДоступнаПередачаДокументовНаРаботаРоссии();
	МетаданныеЗадания = Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии;
...
Процедура ОбновитьРасписаниеРегламентныхЗаданий() Экспорт
	...
	ОбновитьРасписаниеЗаданияПолучениеДокументовРаботаРоссии();
