...
// Производит поиск настройки обмена с банком по заданному отбору. Возвращает первую найденную настройку обмена.
//
// Параметры:
//   ПараметрыОтбора - см. ОбменСБанкамиКлиентСервер.НовыеПараметрыОтбораНастройкиОбмена
//
// Возвращаемое значение:
//   СправочникСсылка.НастройкиОбменСБанками, Неопределено - найденная настройка обмена с банком.
//
Функция НастройкаОбменаПоОтбору(ПараметрыОтбора) Экспорт
	
	Возврат ОбменСБанкамиСлужебный.НастройкаОбменаПоОтбору(ПараметрыОтбора);
	
КонецФункции

// Определяет интеграторов, доступных для обмена банковской выписки и проверяет наличие 
// банковского счета для указанной организации.
//
// Параметры:
//   Организация - ОпределяемыйТип.Организация - организация, для которой выполняется проверка настроек.
//   Банк - ОпределяемыйТип.БанкОбменСБанками - банк, для которого выполняется поиск интеграторов.
//
// Возвращаемое значение:
//   ТаблицаЗначений:
//    * Банк - ОпределяемыйТип.БанкОбменСБанками - ссылка на интегратор, для которого есть активная настройка
//                                                 и хотя один банковский счет у организации.
//    * Наименование - Строка - наименование интегратора.
//
Функция ИнтеграторыБанковскойВыписки(Организация, Банк) Экспорт
	
	Интеграторы = Новый ТаблицаЗначений;
	Интеграторы.Колонки.Добавить("Банк", Метаданные.ОпределяемыеТипы.БанкОбменСБанками.Тип);
	Интеграторы.Колонки.Добавить("Наименование", ОбщегоНазначения.ОписаниеТипаСтрока(150));
	
	НастройкиИнтегратораИзМакета = ОбменСБанкамиСлужебный.НастройкиИнтеграторовИзМакета();
	БИКБанка = ОбменСБанкамиСлужебный.БИК(Банк);
	
	НомераИнтеграторов = Новый Массив;
	Для Каждого КлючИЗначение Из НастройкиИнтегратораИзМакета.БанкиИИдентификаторыИнтеграторов Цикл
		Индекс = КлючИЗначение.Значение.Найти(БИКБанка);
		Если Индекс <> Неопределено Тогда
			НомераИнтеграторов.Добавить(КлючИЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
	
	Если НомераИнтеграторов.Количество() = 0 Тогда
		Возврат Интеграторы;
	КонецЕсли;
	
	БИКиИнтеграторов = Новый Массив;
	Для Каждого НомерИнтегратора Из НомераИнтеграторов Цикл
		БИКИнтегратора = НастройкиИнтегратораИзМакета.ИдентификаторыБИКиИнтеграторов[НомерИнтегратора];
		БИКиИнтеграторов.Добавить(БИКИнтегратора);
	КонецЦикла;
	
	БанкиПоБИКам = ОбменСБанкамиСлужебный.БанкиПоБИКам(БИКиИнтеграторов);
	Если Не ЗначениеЗаполнено(БанкиПоБИКам) Тогда
		Возврат Интеграторы;
	КонецЕсли;
	...
	|	НастройкиОбменСБанками.Банк.Наименование КАК Наименование
	|ИЗ
	|	Справочник.НастройкиОбменСБанками КАК НастройкиОбменСБанками
	|ГДЕ
	|	НастройкиОбменСБанками.Организация = &Организация
	|	И НастройкиОбменСБанками.Банк В(&Банки)
	|	И НЕ НастройкиОбменСБанками.ПометкаУдаления
	|	И НЕ НастройкиОбменСБанками.Недействительна";
	
	РеквизитНаименование = ОбменСБанкамиСлужебный.ИмяПрикладногоРеквизита("Банк.Наименование");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Банк.Наименование", СтрШаблон("Банк.%1", РеквизитНаименование));
	
	Банки = БанкиПоБИКам.ВыгрузитьКолонку("Ссылка");
	...
	Пока Выборка.Следующий() Цикл
		НомераБанковскихСчетов = ОбменСБанкамиСлужебный.НомераБанковскихСчетов(Организация, Выборка.Банк);
		Если НомераБанковскихСчетов.Количество() > 0 Тогда
			НоваяСтрока = Интеграторы.Добавить();
...
// Возвращает настройки обмена с интегратором банковских выписок для указанных организации и банка.
//
// Параметры:
//   Организация - ОпределяемыйТип.Организация - организация, для которой выполняется поиск настроек.
//   Банк - ОпределяемыйТип.БанкОбменСБанками - банк, для которого выполняется поиск настроек.
//
// Возвращаемое значение:
//   Структура - настройки обмена с интегратором:
//     * НастройкаОбменаСБанком - СправочникСсылка.НастройкиОбменСБанками - найденная настройка обмена.
//     * БанкДляПрямогоОбмена - ОпределяемыйТип.БанкОбменСБанками - банк из настройки обмена.
//     * ЗагрузкаВыписокЧерезИнтегратор - Булево - признак загрузки выписок через банк интегратор.
//     * НаименованиеИнтегратора - Строка - наименование банка интегратора.
//
Функция НастройкиОбменаВыписки(Организация, Банк) Экспорт

	Настройки = Новый Структура;
	Настройки.Вставить("НастройкаОбменаСБанком", Справочники.НастройкиОбменСБанками.ПустаяСсылка());
	Настройки.Вставить("БанкДляПрямогоОбмена", ОбменСБанкамиСлужебный.ПустаяСсылкаНаСправочникБанки());
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменСБанками")Тогда
		...
		НастройкаОбмена = НастройкаОбменаПоОтбору(ПараметрыОтбора);
		
		Если НастройкаОбмена <> Неопределено Тогда
			Настройки.НастройкаОбменаСБанком = НастройкаОбмена;
			БанкНастройки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаОбмена, "Банк");
			Настройки.БанкДляПрямогоОбмена = БанкНастройки;
			Если БанкНастройки <> Банк Тогда
				Настройки.ЗагрузкаВыписокЧерезИнтегратор = Истина;
				Настройки.НаименованиеИнтегратора = ОбменСБанкамиСлужебный.НаименованиеБанка(БанкНастройки); 
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьДанныеВыпискиБанкаТекстовыйФормат(
	...
	ПолучитьДанныеВыпискиИзДереваВТекстовыйФормат(ДеревоДанных, СсылкаНаХранилище, Организация, МассивСчетов);
	
КонецПроцедуры

// Получает выписку банка в текстовом формате из разобранного дерева электронного документа,
// а также массив ссылок на банковские счета организаций в выписке.
//
// Параметры:
//  ДеревоДанных - Дерево - содержит разобранную выписку банка (см. ДеревоЭлектронногоДокументаБЭДСлужебный.ДеревоЭлектронногоДокумента)
//  Организация - СправочникСсылка.Организации - организация, по которой разбирается выписка
//  СсылкаНаХранилище - Строка - содержит ссылку на хранилище тестовых данных;
//  МассивСчетов - Массив - содержит ссылки на банковские счета организации.
//  Адрес - УникальныйИдентификатор,Строка - Уникальный идентификатор формы, во временное хранилище которой надо поместить данные
//
Процедура ПолучитьДанныеВыпискиИзДереваВТекстовыйФормат(ДеревоДанных,
													СсылкаНаХранилище,
													Организация,
	...
	СсылкаНаХранилище = ПоместитьВоВременноеХранилище(ДанныеФайла, Адрес);
...
Процедура ДобавитьПлательщикаИПолучателя(Текст, Операция)
	...
	ДобавитьНеПустойПараметр(
		Текст, Операция, "Выписки.НомерСтроки.Операции.НомерСтроки.Плательщик.ИдентификаторЦифровогоСчета",
		"ПлательщикИдентификаторЦифровогоСчета");
	...
	ДобавитьНеПустойПараметр(
		Текст, Операция, "Выписки.НомерСтроки.Операции.НомерСтроки.Получатель.ИдентификаторЦифровогоСчета",
		"ПолучательИдентификаторЦифровогоСчета");
