...
Процедура ЗагрузитьИзФайлаОтчетЗавершение(РезультатЗагрузки, ДополнительныеПараметры) Экспорт
	...
	Если СтатусЗагрузки Тогда
		...
		ПараметрыФормыОтчета.мДатаКонцаПериодаОтчета = КонецДня(ПараметрыФормыОтчета.мДатаКонцаПериодаОтчета);
		...
		Если ДополнительныеПараметры.ЭтоФормаРеглОтчета Тогда
			...
			Иначе
				...
				Попытка
					Форма.Инициализация(Форма.СтруктураРеквизитовФормы.мБезОткрытияФормы); // отсутствует в бухотчетности 2025 года
				Исключение
					Попытка
						...
						ФормаОтчетаПослеЗагрузки = ОткрытьФорму(ФормаОтчета.ИмяФормы, ПараметрыФормыОтчета, , Истина);
						
						Форма.Закрыть();
						Форма = ФормаОтчетаПослеЗагрузки;
					Исключение
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = НСтр("ru = 'Не предусмотрена загрузка данных из формы отчета.'");
						Сообщение.Сообщить();
					КонецПопытки;
				КонецПопытки;
...
Процедура ДобавитьДополнительныеФайлы(ФормаОтчета, ДанныеДляЗагрузки)
	...
	Если ДанныеДляЗагрузки.ТипДокумента = ТипДокументаБухгалтерскаяОтчетность Тогда
		...
		Для Каждого СвойстваФайла Из ДанныеДляЗагрузки.СвойстваФайлов Цикл
			...
			Если ФормаОтчета.СтруктураРеквизитовФормы.Свойство(ВидДополнительногоФайла, СтруктураХранения)
			   И ТипЗнч(СтруктураХранения) = Тип("Структура") Тогда
				ДобавленыФайлы = Истина;
			Иначе
				Продолжить;
			КонецЕсли;
			
			СтруктураХранения.ИмяФайла = "";
			...
			Исключение
				...
				Исключение
					Попытка
						ДобавитьДополнительныйФайл(ФормаОтчета, СтруктураХранения,
							СвойстваФайла.АдресДанных, СвойстваФайла.ИмяФайла, ВидДополнительногоФайла);
					Исключение
...
Процедура ДобавитьДополнительныйФайл(ФормаОтчета, СтруктураХранения, АдресДанных, ИмяФайла, ВидДополнительногоФайла)
	
	РасширениеФайла = НРег(ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ИмяФайла));
	Если РасширениеФайла <> "pdf" Тогда
		ТекстПредупреждения = НСтр("ru = 'Отправка файла с расширением, отличным от ""pdf"", приведет к отказу в приеме бухгалтерской отчетности. Сохраните файл в формате ""pdf"" и приложите его к отчету.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	ДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(АдресДанных);
	
	СтруктураХранения.СведенияИзменены = Истина;
	СтруктураХранения.ХранилищеФайла   = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, ФормаОтчета.УникальныйИдентификатор);
	СтруктураХранения.ИмяФайла         = ИмяФайла;
	СтруктураХранения.Размер           = ДвоичныеДанныеФайла.Размер();
	...
	Если ВидДополнительногоФайла = "ФайлПояснительнойЗаписки" Тогда
		...
		ИмяРеквизита = "ИмяФайлаПояснительнойЗаписки";
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета, ИмяРеквизита) Тогда
			ФормаОтчета[ИмяРеквизита] = СтруктураХранения.ИмяФайла;
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета, "ВариантПояснений") Тогда
				ФормаОтчета.ВариантПояснений = 1;
				ФормаОтчета.Элементы.ИмяФайлаПояснительнойЗаписки.Видимость = Истина;
				#Если ВебКлиент Тогда
				ФормаОтчета.Элементы.ГруппаСтраницыОтображенияПояснений.ТекущаяСтраница = 
					ФормаОтчета.Элементы.СтраницаОтображенияПоясненийВебКлиент;
				#Иначе
				ФормаОтчета.Элементы.ГруппаСтраницыОтображенияПояснений.ТекущаяСтраница = 
					ФормаОтчета.Элементы.СтраницаОтображенияПояснений;
				#КонецЕсли
			КонецЕсли;
			
			#Если НЕ ВебКлиент Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета, "ВнешниеПояснения") Тогда
				ПотокPDF = ДвоичныеДанныеФайла.ОткрытьПотокДляЧтения();
				ФормаОтчета.ВнешниеПояснения.Прочитать(ПотокPDF);
			КонецЕсли;
			#КонецЕсли
		КонецЕсли;
	...
	ИначеЕсли ВидДополнительногоФайла = "ФайлАудиторскогоЗаключения" Тогда
		...
		ИмяРеквизита = "ИмяФайлаАудиторскогоЗаключения";
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ФормаОтчета, ИмяРеквизита) Тогда
			ФормаОтчета[ИмяРеквизита] = СтруктураХранения.ИмяФайла;
		КонецЕсли;
