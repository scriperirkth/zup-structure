...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОбновитьПредопределенныеШаблоны_3_1_0");
КонецПроцедуры
...
Процедура ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
		...
		МодульНаставничество.ЗаполнитьНастройкиПечатныхФормПоУмолчанию(ОписанияНастроек);
	КонецЕсли;
...
Процедура УстановитьЗапросДинамическогоСпискаФормыВыбораПубликуемыхСотрудников(УправляемаяФорма) Экспорт
	...
	ТекстЗапроса =
		...
		|	ДанныеДляПодбораСотрудников.Организация КАК ГоловнаяОрганизация,
		|	ДанныеДляПодбораСотрудников.Филиал КАК Организация,
		|	ДанныеДляПодбораСотрудников.Наименование КАК Наименование,
		|	ВЫБОР
		|		КОГДА ДанныеДляПодбораСотрудников.Должность ЕСТЬ NULL
		|			ТОГДА ДанныеДляПодбораСотрудников.ДолжностьПоШтатномуРасписанию.Должность
		|		ИНАЧЕ ДанныеДляПодбораСотрудников.Должность
		|	КОНЕЦ КАК Должность,
		|	ВЫБОР
		...
		|ГДЕ
		|	&ДатаДокумента МЕЖДУ ДанныеДляПодбораСотрудников.Начало И ДанныеДляПодбораСотрудников.Окончание
		|	И ВЫБОР
		...
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ДанныеДляПодбораСотрудников.ВидСобытия <> ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.ПустаяСсылка)
