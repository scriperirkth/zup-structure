...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И Не НастройкиЗаписаны Тогда
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru = 'Настройки интеграции с порталом ""Работа России"" еще не записаны.'");
		Иначе
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	ЗаписатьУчетныеДанныеИнтеграцииСРаботаРоссии();
	Закрыть();
...
Процедура ПриВыбореОрганизации(НоваяОрганизация)
	...
	Если ТекДанные.Данные = Неопределено Тогда
		Данные = ПрочитатьНастройкиИнтеграцииСРаботаРоссии(ТекДанные.Организация);
...
Функция ПрочитатьНастройкиИнтеграцииСРаботаРоссии(Организация)
	
	Возврат ИнтеграцияСРаботаРоссии.НастройкиАвторизации(Организация);
...
&НаСервере
Процедура ЗаписатьУчетныеДанныеИнтеграцииСРаботаРоссии()
	
	Для Каждого Строка Из ДанныеОрганизаций Цикл
		Если Строка.Используется = Истина Тогда
			Данные = Строка.Данные;
			Если Данные <> Неопределено Тогда
				ИнтеграцияСРаботаРоссии.ЗапомнитьНастройкиАвторизации(
		...
		ИначеЕсли Строка.Используется = Ложь Тогда
			ИнтеграцияСРаботаРоссии.УдалитьНастройкиАвторизации(Строка.Организация);
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверитьАвторизациюНаСервере(Знач Организация, АдресСервера, ОписаниеОшибкиПРР)
	...
	Возврат ИнтеграцияСРаботаРоссии.ПроверитьАвторизацию(Организация, АдресСервера, ОписаниеОшибкиПРР);
...
Функция ПроверитьРегламентныеЗаданияНаСервере()
	...
	Возврат ИнтеграцияСРаботаРоссии.ПроверитьРегламентныеЗадания();
...
Процедура ВключитьРегламентныеЗаданияНаСервере()
	...
	ЭДОБЗК.ОбновитьРасписаниеЗаданийДляРаботаРоссии();
