...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		// Не открываем форму в неразделенном режиме.
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
			...
			Если ИспользуетсяРаботаВМоделиСервиса Тогда
				Элементы.СвязыванияБазДанныхСОблаком.Видимость = Не ДокументооборотСКОПовтИсп.ЭтоБизКуб();
...
Процедура ПриОткрытии()
	...
	ПрорисоватьПанельСФРИспользованиеКлюча();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПредупредитьОбИспользованииКлючаДляСФР" Тогда
		ПрорисоватьПанельСФРИспользованиеКлюча();
...
#КонецЕсли
		...
		Если ИмяСобытия = "Пометка прочтенным"
			...
			Иначе
				ТребуетсяОбновитьКоличествоНовых = Ложь;
				...
				Для Каждого СтрокаДерева Из СтрокиДерева Цикл
					...
					Если СтрокаДерева.Ссылка = Источник
						...
						ТребуетсяОбновитьКоличествоНовых = Истина;
						Если ИмяСобытия <> "Групповая пометка прочтенными/непрочтенными" Тогда
							Прервать;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
				Если ТребуетсяОбновитьКоличествоНовых Тогда
	...
	ИначеЕсли (ИмяСобытия = "Получены новые сообщения 1С-Отчетности"
			ИЛИ ИмяСобытия = "Получены новые сообщения 1С-Отчетности без смены страницы") И Источник <> ЭтотОбъект
		И (НЕ ЭтоРежимОблачной1СО
			ИЛИ НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.Облачная1СОИспользоватьФормуНовыхСобытий()) Тогда
...
&НаКлиенте
Процедура ПанельСФРИспользованиеКлючаОбработкаНавигационнойСсылкиПредварительное(Результат, ПараметрыОповещения) Экспорт
	
	ПанельСФРИспользованиеКлючаОбработкаНавигационнойСсылки(
...
&НаКлиенте
Процедура ПанельСФРИспользованиеКлючаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "СФРИспользованиеКлючаПодготовитьЗаявления" Тогда
		СтандартнаяОбработка = Ложь;
		...
		Если КонтекстЭДОКлиент = Неопределено И КонтекстУжеПодключался <> Истина Тогда
			...
			Оповещение = Новый ОписаниеОповещения(
				"ПанельСФРИспользованиеКлючаОбработкаНавигационнойСсылкиПредварительное",
				ЭтотОбъект,
				ПараметрыОповещения);
			ИнициализироватьКонтекстЭДО(Оповещение);
			Возврат;
		...
		ПредупредитьОбИспользованииКлючаДляСФР = ПараметрыПриложения["БРО.ПредупредитьОбИспользованииКлючаДляСФР"];
		Если ПредупредитьОбИспользованииКлючаДляСФР = Неопределено
			ИЛИ ПредупредитьОбИспользованииКлючаДляСФР.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
		ПараметрыФормыПредупреждения = Новый Структура();
		ПараметрыФормыПредупреждения.Вставить("Организации", ПредупредитьОбИспользованииКлючаДляСФР);
		ПараметрыФормыПредупреждения.Вставить("СкрытьНастройкуНапоминаний", Истина);
		
		Если ИспользуетсяОднаОрганизация Тогда
			КонтекстОткрытияЗаявления = Новый Структура;
			КонтекстОткрытияЗаявления.Вставить("Организация", ПараметрыФормыПредупреждения.Организации[0].Организация);
			КонтекстОткрытияЗаявления.Вставить("Отпечаток", ПараметрыФормыПредупреждения.Организации[0].Отпечаток);
			
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ПанельСФРИспользованиеКлючаОбработкаНавигационнойСсылкиПослеНажатияПодготовитьЗаявление",
				ЭтотОбъект,
				КонтекстОткрытияЗаявления);
			
			ОткрытьФорму(
				КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ОтправьтеЗаявлениеСФРИспользованиеКлючаПоОрганизации",
				ПараметрыФормыПредупреждения,,,,,
				ОписаниеОповещения);
			
		Иначе
			ОткрытьФорму(
				КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ОтправьтеЗаявлениеСФРИспользованиеКлючаПоОрганизациям",
				ПараметрыФормыПредупреждения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриПереключенииРаздела(Элемент)
	...
	Если Не ДокументооборотСКОВызовСервера.ЭтоКонфигурацияОтчетность() Тогда
...
Процедура СоздатьОтчет(знач ПараметрыФормы = Неопределено)
	...
	// Проверим открытие стартовой формы отчета
	ИмяФормыОтчета = "";
	ПараметрыФормы.Свойство("ИмяФормы", ИмяФормыОтчета);
	Если ИмяФормыОтчета <> "" И ПопытатьсяНайтиИАктивизироватьОкно(ИмяФормыОтчета) Тогда
		Возврат;
	КонецЕсли;
	
	// Проверим открытие формы создания отчета
	ИмяФормыСоздать = "Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаСозданияОтчета";
	Если ПопытатьсяНайтиИАктивизироватьОкно(ИмяФормыСоздать) Тогда
		Возврат;
	КонецЕсли;
	
	ЭтотОбъект.ПараметрыВыбораФормыСоздания = Неопределено;
	ОписаниеОповещенияОЗакрытииФормыСозданияОтчета = Новый ОписаниеОповещения("ВыполнитьПослеЗакрытияСозданияОтчета", ЭтотОбъект);
	ОткрытьФорму(ИмяФормыСоздать, ПараметрыФормы, ЭтотОбъект,,,, 
				ОписаниеОповещенияОЗакрытииФормыСозданияОтчета, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

// Проверяет, открыто ли окно формы, и активизирует его
// Параметры:
//  ИмяФормы - Строка - имя формы для поиска
// Возвращаемое значение:
//  Булево - Истина, если окно найдено и активировано
&НаКлиенте
Функция ПопытатьсяНайтиИАктивизироватьОкно(Знач ИмяФормы)
	
	НайденоОкно = Ложь;
	// Сначала попробуем найти его среди открытых стартовых форм.
	// Необходимо для предотвращения
	// открытия нескольких стартовых форм одного отчета.
	РегламентированнаяОтчетностьКлиент.ВебКлиентНайтиАктивизироватьОкно(ИмяФормы, ЭтотОбъект, НайденоОкно);
	
	Возврат НайденоОкно <> Неопределено И НайденоОкно;
...
Процедура ОбновитьДанные(Команда)
	...
	Если ДокументооборотСКОВызовСервера.ЭтоРежимОблачной1СО() 
		И ЗначениеЗаполнено(Организация) И Элементы.Разделы.ТекущаяСтраница.Имя <> "СтраницаКалендарь" Тогда
		
		ВыборВидаОбменаПриОтборе = ПолучитьВариантОбменаПриОтбореПоОрганизации();
		Если (ВыборВидаОбменаПриОтборе = 1) Тогда
			// по текущей организации отбора
			ДанныеДляОбмена.МассивОрганизаций.Очистить();
			ДанныеДляОбмена.МассивОрганизаций.Добавить(Организация);
		ИначеЕсли ВыборВидаОбменаПриОтборе = 0 Тогда
			ПараметрыОповещения = Новый Структура("КонтекстЭДОКлиент, ДанныеДляОбмена", КонтекстЭДОКлиент,
				ДанныеДляОбмена);
			Оповещение = Новый ОписаниеОповещения("ВыборВариантаПолученияСообщенийОбмена", ЭтотОбъект,
				ПараметрыОповещения);

			ПолучениеСообщенийСписокВыбора = Новый СписокЗначений;
			ПолучениеСообщенийСписокВыбора.Добавить(1, СтрШаблон(НСтр("ru = 'По организации в отборе (%1)'"),
				Организация));
			ОтключенныеОрганизацииПользователяДляСКО = ОтключенныеОрганизацииПользователяДляСКО();
			ОрганизацииПользователяДляСКО = ОрганизацииПользователяДляСКО();
			Если ОтключенныеОрганизацииПользователяДляСКО.Количество() = 0 Тогда
				ПолучениеСообщенийСписокВыбора.Добавить(2, НСтр("ru = 'По всем организациям'"));
			Иначе
				ПолучениеСообщенийСписокВыбора.Добавить(2, СтрШаблон(НСтр("ru = 'По выбранным организациям (%1)'"),
					ОрганизацииПользователяДляСКО.Количество()));
			КонецЕсли;

			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("СписокВыбора", ПолучениеСообщенийСписокВыбора);
			ОткрытьФорму("ОбщаяФорма.ВыборВариантаПолученияСообщенийОбмена", ПараметрыФормы, ЭтотОбъект, , , ,
	...
	ПолучитьСообщения(КонтекстЭДОКлиент, ДанныеДляОбмена);
...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ (ТекущиеДанныеНаименованиеОтчета = "Уведомление по УСН" ИЛИ ТекущиеДанныеНаименованиеОтчета = "Переход на УСН")
				И ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(Ссылка, "ИмяФормы") =
					"Форма2025_1"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ув.по УСН") Тогда
...
Процедура ОбновитьКоличествоНовых(Форма)
	...
	Если Форма.ЭтоРежимОблачной1СО Тогда
		// количество на кнопке перехода в новые сообщения
		ЭлементНовыеСообщения = Форма.Элементы.Найти("КалендарьОтчетностиНовыеСообщения");
		Если ЭлементНовыеСообщения <> Неопределено Тогда
			КоличествоНепрочитанных = СведенияОКоличестве.КоличествоНепрочитанных;
...
Процедура ВывестиТаблицуРаздела(Форма, РазделОтчетности)
	...
	ТекущийРаздел 		= ТекущийРазделФормы(Форма);
	ИмяТекущейТаблицы 	= СтрЗаменить(РазделОтчетности, "Гиперссылка", ""); 
...
&НаКлиенте
Процедура ПанельСФРИспользованиеКлючаОбработкаНавигационнойСсылкиПослеНажатияПодготовитьЗаявление(
		Результат,
		ВходящийКонтекст) Экспорт
	
	Если ТипЗнч(Результат) = Тип("ДокументСсылка.ЗаявлениеАбонентаСпецоператораСвязи") Тогда
		ПоказатьЗначение(, Результат);
		
	ИначеЕсли Результат = "ПодготовитьЗаявление" Тогда
		КонтекстЭДОКлиент.ОткрытьНовоеЗаявлениеСФРИспользованиеКлюча(ЭтотОбъект, ВходящийКонтекст.Организация);
	КонецЕсли;
...
Процедура УстановитьОтборыПоОтчетам(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Форма.ВидОтчета)
		...
		КонецЕсли;
		ТекстПодсказкиВидОтчета = НСтр("ru = 'По всем видам отчетов'");
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	Форма.Элементы.ОтборВидОтчета.ПодсказкаВвода = ТекстПодсказкиВидОтчета;
...
Процедура УправлениеВидимостьюЛичныхКабинетовСпецоператоров()
	...
	ПараметрыФункциональностиБРО = РегламентированнаяОтчетностьКлиентСервер.ПараметрыФункциональности();
	ОтображатьЛичныеКабинеты = ПараметрыФункциональностиБРО.ОтображатьЛичныеКабинетыОператоровСвязиНаФормеОтчетности;
	// Напрмиер, для Бизкуба не отображаем личные кабинеты спец.операторов, пользователи не могут их подключить.
	Если ОтображатьЛичныеКабинеты Тогда
...
Процедура Подключаемый_КалендарьОтчетностиПриВыводеПериода(Элемент, ОформлениеПериода)
	...
	Для Каждого СтрокаОформления Из ОформлениеПериода.Даты Цикл
		МассивДат.Добавить(СтрокаОформления.Дата);
	КонецЦикла;
	
	СтатусыДат = СтатусыДат(МассивДат);
	
	КалендарьОтчетностиБРОКлиентПереопределяемый.КалендарьПриВыводеПериода(Элемент, ОформлениеПериода, СтатусыДат);
...
Процедура ПечатьУведомленияИзСписка(УведомлениеДляПечати)
	...
	Если ДанныеДляПечатиУведомления.УдаленныйОбъектМетаданных Тогда 
		Попытка
			...
			Иначе
				СтруктураРеквизитовФормы = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ДанныеДляПечатиУведомления.СтруктураРеквизитовФормы);
...
&НаКлиенте
Процедура ПрорисоватьПанельСФРИспользованиеКлюча()
	
	ПредупредитьОбИспользованииКлючаДляСФР = ПараметрыПриложения["БРО.ПредупредитьОбИспользованииКлючаДляСФР"];
	
	ЭлементГруппаСФРИспользованиеКлюча = ?(ЭтоРежимОблачной1СО,
		Элементы.ГруппаСФРИспользованиеКлюча1СО, Элементы.ГруппаСФРИспользованиеКлюча);
	ЭлементПанельСФРИспользованиеКлюча = ?(ЭтоРежимОблачной1СО,
		Элементы.ПанельСФРИспользованиеКлюча1СО, Элементы.ПанельСФРИспользованиеКлюча);
	ЭлементПанельСФРИспользованиеКлючаСрокНаступил = ?(ЭтоРежимОблачной1СО,
		Элементы.ПанельСФРИспользованиеКлючаСрокНаступил1СО, Элементы.ПанельСФРИспользованиеКлючаСрокНаступил);
	ЭлементПанельСФРИспользованиеКлючаОднаОрганизация = ?(ЭтоРежимОблачной1СО,
		Элементы.ПанельСФРИспользованиеКлючаОднаОрганизация1СО, Элементы.ПанельСФРИспользованиеКлючаОднаОрганизация);
	ЭлементПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил = ?(ЭтоРежимОблачной1СО,
		Элементы.ПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил1СО,
		Элементы.ПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил);
	
	Если ПредупредитьОбИспользованииКлючаДляСФР = Неопределено
		ИЛИ ПредупредитьОбИспользованииКлючаДляСФР.Количество() = 0 Тогда
		
		ЭлементГруппаСФРИспользованиеКлюча.Видимость = Ложь;
	...
	Иначе
		...
		Если ИспользуетсяОднаОрганизация Тогда
			Если СрокНаступил Тогда
				ЭлементПанельСФРИспользованиеКлюча.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаСрокНаступил.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизация.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил.Видимость = Истина;
				
			Иначе
				ЭлементПанельСФРИспользованиеКлюча.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаСрокНаступил.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизация.Видимость = Истина;
			КонецЕсли;
			
			ЭлементГруппаСФРИспользованиеКлюча.Видимость = Истина;
		...
		Иначе
			Если СрокНаступил Тогда
				...
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизация.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаСрокНаступил.Видимость = Истина;
				
			Иначе
				ЭлементПанельСФРИспользованиеКлючаСрокНаступил.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизация.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлючаОднаОрганизацияСрокНаступил.Видимость = Ложь;
				ЭлементПанельСФРИспользованиеКлюча.Видимость = Истина;
			КонецЕсли;
...
&НаКлиенте
Процедура ЗакрытьПанельСФРИспользованиеКлючаНажатие(Элемент)
	
	ПараметрыПриложения["БРО.ПредупредитьОбИспользованииКлючаДляСФР"] = Неопределено;
	ЭлементГруппаСФРИспользованиеКлюча = ?(ЭтоРежимОблачной1СО,
		Элементы.ГруппаСФРИспользованиеКлюча1СО, Элементы.ГруппаСФРИспользованиеКлюча);
	ЭлементГруппаСФРИспользованиеКлюча.Видимость = Ложь;
...
&НаСервере
Функция СтатусыДат(МассивДат)
	
	Результат = Новый Соответствие;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.ЗадачиБухгалтера.КалендарьОтчетности") Тогда
		
		МодульКалендарьОтчетностиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("КалендарьОтчетностиКлиентСервер");
		Префикс = МодульКалендарьОтчетностиКлиентСервер.ПрефиксАтрибутовФормы();
		ВсеДаты = РеквизитФормыВЗначение(Префикс + "ВсеДаты");
		
		ОтборПериода = Новый Структура("Срок", Неопределено);
		Для Каждого ЭлементДаты Из МассивДат Цикл
			Статус = 0;
			ОтборПериода.Срок = ЭлементДаты;
			НашлиСтроки = ВсеДаты.НайтиСтроки(ОтборПериода);
			Если НашлиСтроки.Количество() > 0 Тогда
				Статус = НашлиСтроки[0]["Статус"] + 0;
			КонецЕсли;
			Результат.Вставить(ЭлементДаты, Статус);
		КонецЦикла;
...
&НаСервере
Функция ОрганизацииПользователяДляСКО()
	
	Возврат Обработки.ДокументооборотСКонтролирующимиОрганами.ОрганизацииПользователяДляСКО();
	
КонецФункции

&НаСервере
Функция ОтключенныеОрганизацииПользователяДляСКО()
	
	Возврат Обработки.ДокументооборотСКонтролирующимиОрганами.ОтключенныеОрганизацииПользователяДляСКО();
	
КонецФункции

&НаКлиенте
Процедура ПолучитьСообщения(КонтекстЭДОКлиент, ДанныеДляОбмена)
...
&НаКлиенте
Процедура ВыборВариантаПолученияСообщенийОбмена(Результат, ДополнительныеПараметры) Экспорт
	...
	ДанныеДляОбмена = ДополнительныеПараметры.ДанныеДляОбмена;
...
&НаСервере
Функция ПолучитьВариантОбменаПриОтбореПоОрганизации()
	
	Возврат ЭлектронныйДокументооборотСКонтролирующимиОрганами.ПолучитьВариантОбменаПриОтбореПоОрганизации();
