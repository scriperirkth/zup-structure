...
Процедура ЗаполнитьСписокПользователей()
	...
	Если Не УстановленаМонопольнаяБлокировка Тогда
		
		УстановитьМонопольнуюБлокировку();
		Элементы.ГруппаДекорацияОшибкаБлокировки.Видимость = Не УстановленаМонопольнаяБлокировка;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьИмяФайлаПоДаннымСодержимого(ПутьКФайлуСодержимого, ТипФайла, ТипДанных)
	...
	Пока ЧтениеСодержимого.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Если ОбъектXDTO.Type = ТипФайла
			И (ТипДанных = Неопределено
			Или ОбъектXDTO.Свойства().Получить("DataType") <> Неопределено И ОбъектXDTO.DataType = ТипДанных) Тогда
...
Процедура ЗапуститьЗагрузкуДанных()
	...
	Попытка
		...
		Если ОчиститьОкноСообщений Тогда
			ОчиститьСообщения();
			ОчиститьОкноСообщений = Ложь;
		КонецЕсли;
	Исключение
...
Процедура ЗапуститьЗагрузкуДанныхНаСервере()
	...
	ПараметрыЗагрузки.ЗагружатьПользователей = Истина;
	ПараметрыЗагрузки.СвернутьЭлементыСправочникаПользователи = Истина;
	ПараметрыЗагрузки.ИдентификаторСостояния = ИдентификаторСостояния;
	ПараметрыЗагрузки.ПропуститьВосстановлениеРасширений = Истина;
	ПараметрыЗагрузки.АдресХранилищаРезультата = АдресХранилища;
	ПараметрыЗагрузки.КоличествоПотоков = КоличествоЗаданийЗагрузкиДанных;
	ПараметрыЗагрузки.НомерКлиентскогоСеанса = НомерСеансаИнформационнойБазы;
...
Процедура ВосстановитьРасширенияИнформационнойБазы(ИмяФайлаНаСервере)
	...
	Результат = РасширенияДляВосстановления(ИмяФайлаНаСервере, ОчиститьОкноСообщений);
	КаркасыРасширенийДляВосстановления = КаркасыРасширенийДляВосстановления(ИмяФайлаНаСервере);
	
	ДанныеРасширений = Новый Структура();
		
	Если ЗначениеЗаполнено(Результат.РасширенияДляВосстановления) Тогда
		ДанныеРасширений.Вставить("РасширенияДляВосстановления", Результат.РасширенияДляВосстановления);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Результат.Расширения1СПлюс) Тогда
		ВыгрузкаЗагрузкаДанныхСлужебный.ВосстановитьРасширения1СПлюс(Результат.Расширения1СПлюс);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция РасширенияДляВосстановления(ИмяФайлаНаСервере, ОчиститьОкноСообщений)

	ЧтениеДанныхАрхива = Новый ЧтениеZipФайла(ИмяФайлаНаСервере);

	Попытка
		РасширенияДляВосстановления = РасширенияДляВосстановленияВнутр(ЧтениеДанныхАрхива, ОчиститьОкноСообщений);
	Исключение
		ЧтениеДанныхАрхива.Закрыть();
		ВызватьИсключение;
	КонецПопытки;
...
&НаСервереБезКонтекста
Функция РасширенияДляВосстановленияВнутр(ЧтениеДанныхАрхива, ОчиститьОкноСообщений)
	Результат = Новый Структура("РасширенияДляВосстановления, Расширения1СПлюс", Новый Массив(), Новый Массив());
	...
	Для Каждого ИнформацияОПользовательскомРасширении Из ИнформацияОПользовательскихРасширениях Цикл
		...
		Если ИнформацияОПользовательскомРасширении.Свойство("PlusExtVersionId") 
				И ИнформацияОПользовательскомРасширении.PlusExtVersionId <> "" Тогда
			Расширение1СПлюс = Новый Структура();
			Расширение1СПлюс.Вставить("ExtVersionId", ИнформацияОПользовательскомРасширении.PlusExtVersionId);
			Расширение1СПлюс.Вставить("ExtId", ИнформацияОПользовательскомРасширении.PlusExtId);
			Расширение1СПлюс.Вставить("Version", ИнформацияОПользовательскомРасширении.Version);
			Расширение1СПлюс.Вставить("Name", ИнформацияОПользовательскомРасширении.Name);
			
			ОписаниеФайла = ПолучитьДвоичныеДанныеРасширения(
				ЧтениеДанныхАрхива, ИнформацияОПользовательскомРасширении.FileName);
			Расширение1СПлюс.Вставить("Data", ОписаниеФайла.Данные);
			Расширение1СПлюс.Вставить("InstallTask", ИнформацияОПользовательскомРасширении.InstallTask);
			
			Результат.Расширения1СПлюс.Добавить(Расширение1СПлюс);
			
			УдалитьФайлыВПопытке(ОписаниеФайла.Путь);
		Иначе
			...
			ОписаниеФайла = ПолучитьДвоичныеДанныеРасширения(ЧтениеДанныхАрхива, ИмяФайлаПользовательскогоРасширения);
			...
			РасширениеДляВосстановления.Вставить("Данные", ОписаниеФайла.Данные);
			Результат.РасширенияДляВосстановления.Добавить(РасширениеДляВосстановления);
			
			УдалитьФайлыВПопытке(ОписаниеФайла.Путь);
		КонецЕсли;
		
		Если ИнформацияОПользовательскомРасширении.ModifiesDataStructure Тогда
			ОчиститьОкноСообщений = Истина;
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПолучитьДвоичныеДанныеРасширения(ЧтениеДанныхАрхива, ИмяФайлаПользовательскогоРасширения)
	...
	Возврат Новый Структура("Данные,Путь", 
		Новый ДвоичныеДанные(ПутьКФайлуПользовательскогоРасширения), ПутьКФайлуПользовательскогоРасширения);
КонецФункции
