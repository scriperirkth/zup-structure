...
Процедура ЗаполнитьСписокДокументов()
	...
	|	МИНИМУМ(ВЫРАЗИТЬ(ДокументОснованиеВедомости.Комментарий КАК СТРОКА(100))) КАК Комментарий,
	|	МИНИМУМ(&Работники) КАК Работники,
	|	МИНИМУМ(&ДатаСобытия) КАК ДатаСобытия
	|ИЗ
	|	#РегистрНакопления КАК ЗарплатаКВыплате
	...
	УсловиеПоПодразделению = ?(Подразделение = Справочники.ПодразделенияОрганизаций.ПустаяСсылка(), "ИСТИНА", "ЗарплатаКВыплате.Подразделение В ИЕРАРХИИ(&Подразделение)");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоПодразделению", УсловиеПоПодразделению);
	...
	ИменаВозможныхПолейДаты = Новый Массив;
	ИменаВозможныхПолейДаты.Добавить("ДатаУвольнения");
	ИменаВозможныхПолейДаты.Добавить("ДатаНачалаПериодаОтсутствия");
	ИменаВозможныхПолейДаты.Добавить("ДатаОсвобожденияОтДолжности");
	ИменаВозможныхПолейДаты.Добавить("ДатаНачала");
	...
	Для Каждого МетаданныеДокумента Из МетаданныеДокументов Цикл
		...
		ДатаСобытия = "ДАТАВРЕМЯ(1,1,1)";
		Для Каждого ИмяПоля Из ИменаВозможныхПолейДаты Цикл
			ПолеДокумента = МетаданныеДокумента.Реквизиты.Найти(ИмяПоля);
			Если ПолеДокумента <> Неопределено Тогда
				ДатаСобытия = "ДокументОснованиеВедомости." + ПолеДокумента.Имя;
				Прервать;
			КонецЕсли;	
		КонецЦикла;	
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДатаСобытия", ДатаСобытия);
		ЕстьРеквизитПланируемаяДатаВыплаты = МетаданныеДокумента.Реквизиты.Найти("ПланируемаяДатаВыплаты") <> Неопределено;
		РеквизитПланируемаяДатаВыплаты = ?(ЕстьРеквизитПланируемаяДатаВыплаты, "ДокументОснованиеВедомости.ПланируемаяДатаВыплаты", "ДАТАВРЕМЯ(1,1,1)");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПланируемаяДатаВыплаты", РеквизитПланируемаяДатаВыплаты);
		УсловиеПоДатеВыплаты = ?(ОтбиратьПоДатеВыплаты И ЕстьРеквизитПланируемаяДатаВыплаты, "ДокументОснованиеВедомости.ПланируемаяДатаВыплаты <= &ДатаВедомости", "ИСТИНА");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоДатеВыплаты", УсловиеПоДатеВыплаты);
		Запрос.УстановитьПараметр("ДатаВедомости", ДатаВедомости);
		ЭтоВыплатаАванса = ?(ВидДокумента = Перечисления.ВидыДокументовОснованийВедомостейНаВыплатуЗарплаты.НачислениеЗаПервуюПоловинуМесяца, Истина, Ложь);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#РегистрНакопления", ?(ЭтоВыплатаАванса, "РегистрНакопления.ЗарплатаКВыплатеАвансом", "РегистрНакопления.ЗарплатаКВыплате"));
		...
		Пока ВыборкаДокументов.Следующий() Цикл
			...
			Представление = 
				СтрШаблон(
					НСтр("ru = '%1 от %2 - %3, дата выплаты: %4%5%6'"), 
					ВыборкаДокументов.Номер, 
					Формат(ВыборкаДокументов.Дата, "ДЛФ=Д"),
					ВыборкаДокументов.Работники,
					?(ЗначениеЗаполнено(ВыборкаДокументов.ПланируемаяДатаВыплаты),
						Формат(ВыборкаДокументов.ПланируемаяДатаВыплаты, "ДЛФ=Д"), ""),
					?(ЗначениеЗаполнено(ВыборкаДокументов.ДатаСобытия), 
						СтрШаблон(НСтр("ru = ', дата события %1'"), Формат(ВыборкаДокументов.ДатаСобытия, "ДЛФ=Д")),""),
					?(ЗначениеЗаполнено(ВыборкаДокументов.Комментарий), СтрШаблон(", %1", ВыборкаДокументов.Комментарий),""));
