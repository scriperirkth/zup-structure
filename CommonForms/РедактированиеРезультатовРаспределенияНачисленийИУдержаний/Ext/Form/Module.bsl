...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовНачислений" Тогда
		...
		Если ОтражениеЗарплатыВБухучетеРасширенный.ИспользоватьБюджетныйТарифНСиПЗ(Период) Тогда
			СтатьиБюджетныйТарифНСиПЗ = Новый ФиксированноеСоответствие(ОтражениеЗарплатыВБухучетеРасширенный.СтатьиБюджетныйТарифНСиПЗ());
		КонецЕсли;
...
Процедура ОповеститьОбИзменениях(Отказ)
	...
	Если Модифицированность Тогда
		...
		Если ПроверитьЗаполнение() Тогда
			...
			НовоеРаспределениеРезультатов = ВозвращаемыйРезультат();
			
			ПараметрыОповещения = Новый Структура;   
			ПараметрыОповещения.Вставить("ПрименяетсяЕНВД", ПрименяетсяЕНВД);
			ПараметрыОповещения.Вставить("ИзменилсяБюджетныйТарифНСиПЗ", ИзменилсяБюджетныйТарифНСиПЗ(НовоеРаспределениеРезультатов));
			ПараметрыОповещения.Вставить("СотрудникФизическоеЛицо", Сотрудник);
			ПараметрыОповещения.Вставить("РаспределениеРезультатов", НовоеРаспределениеРезультатов);
...
&НаКлиенте
Функция ИзменилсяБюджетныйТарифНСиПЗ(НовоеРаспределениеРезультатов)
	
	Если Не ЗначениеЗаполнено(СтатьиБюджетныйТарифНСиПЗ) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	БылаДоляБюджетныйТарифНСиПЗ = 0;
	Для каждого СтрокаРаспределения Из ИсходноеРаспределениеРезультатов Цикл
		Если СтатьиБюджетныйТарифНСиПЗ.Получить(СтрокаРаспределения.СтатьяФинансирования) = Истина Тогда
			БылаДоляБюджетныйТарифНСиПЗ = БылаДоляБюджетныйТарифНСиПЗ + СтрокаРаспределения.Результат;
		КонецЕсли;
	КонецЦикла;
	
	СталаДоляБюджетныйТарифНСиПЗ = 0;
	Для каждого СтрокаРаспределения Из НовоеРаспределениеРезультатов Цикл
		Если СтатьиБюджетныйТарифНСиПЗ.Получить(СтрокаРаспределения.СтатьяФинансирования) = Истина Тогда
			СталаДоляБюджетныйТарифНСиПЗ = СталаДоляБюджетныйТарифНСиПЗ + СтрокаРаспределения.Результат;
		КонецЕсли;
	КонецЦикла;
	
	Возврат БылаДоляБюджетныйТарифНСиПЗ <> СталаДоляБюджетныйТарифНСиПЗ;
