...
Процедура ХарактеристикиВариантовОтветовПриИзменении(Элемент)
	...
	ТекущиеДанные = Элемент.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено И ЗначениеЗаполнено(ТекущиеДанные.Характеристика) Тогда
		ТекущиеДанные.ШкалаЗначений = ШкалаЗначенийХарактеристики(ТекущиеДанные.Характеристика);
	КонецЕсли;
...
Функция ДанныеВопросовШаблонаАнкеты(ШаблонАнкеты, Объект)
	...
	Запрос.Текст =		
		...
		|	НастройкаВопросовДляСобеседования.ХарактеристикаПерсонала.ШкалаЗначений КАК ШкалаЗначений
		|ИЗ
		|	(ВЫБРАТЬ
...
Процедура ПеренестиВыбраннуюХарактеристикуВТаблицу(Результат, ИдентификаторРодителя, ИдентификаторНовойСтроки)
	...
	Запрос.УстановитьПараметр("Владелец", Результат.ШкалаЗначений);
	...
	Запрос.Текст =
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ХарактеристикиПерсонала КАК ХарактеристикиПерсонала
		|		ПО ЗначенияХарактеристик.Владелец = ХарактеристикиПерсонала.ШкалаЗначений
		|ГДЕ
...
Функция ТекстЗапросаКритериев()
	...
	ТекстЗапроса = 
		...
		|	ВТПолнаяТаблица.Характеристика.ШкалаЗначений КАК ШкалаЗначений
		|ИЗ
		|	ВТПолнаяТаблица КАК ВТПолнаяТаблица
...
Процедура ЗаполнитьДанныеЭлектронногоКурса()
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|	НастройкаВопросовДляЭлектронногоТестирования.ХарактеристикаПерсонала.ШкалаЗначений КАК ШкалаЗначений,
...
&НаСервереБезКонтекста
Функция ШкалаЗначенийХарактеристики(Характеристика)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Характеристика, "ШкалаЗначений");
