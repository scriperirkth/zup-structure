...
Процедура РезультатСравненияТабличныйДокументПриАктивизации(Элемент)
	...
	Если РезультатТабличныйДокумент.ТекущаяОбласть <> Неопределено
	   И ТипЗнч(РезультатТабличныйДокумент.ТекущаяОбласть.Расшифровка) = Тип("Структура") Тогда
...
Процедура ОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные <> Неопределено
	   ...
	   И РезультатТабличныйДокумент.ТекущаяОбласть.Текст = "-" Тогда
...
Асинх Процедура ПоказатьДиалогВыбораФайла()
	...
	Диалог.Фильтр = "Табличный документ (*.mxl)|*.mxl|Лист Excel (*.xls)|*.xls|Лист Excel2007-... (*.xlsx)|*.xlsx";
	...
	ВыбранныйФайлРасширение = НРег(ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ВыбранныйФайл));
	
	Попытка
		Если ВыбранныйФайлРасширение = "xlsx" Тогда
			РезультатТабличныйДокумент.Записать(ВыбранныйФайл, ТипФайлаТабличногоДокумента.XLSX );
		ИначеЕсли ВыбранныйФайлРасширение = "xls" Тогда
			РезультатТабличныйДокумент.Записать(ВыбранныйФайл, ТипФайлаТабличногоДокумента.XLS);
		Иначе
			РезультатТабличныйДокумент.Записать(ВыбранныйФайл, ТипФайлаТабличногоДокумента.MXL);
		КонецЕсли;
	Исключение
