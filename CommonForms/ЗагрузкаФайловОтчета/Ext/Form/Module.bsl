...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.Интерфейс85") Тогда
		...
		МодульРегламентированнаяОтчетностьИнтерфейс85.УстановитьНазначениеДействияКоманды(
			Команды.ПередатьФайлыДляЗагрузкиИЗакрытьФорму);
	КонецЕсли;
...
Функция СвойстваВидаОтчетаПоИсточнику(Источник)
	...
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчетБухОтчетность" Тогда
		...
		СвойстваВидаОтчета.Вставить("БалансКоммерческойОрганизации", Истина);
				...
				Если ТипЗнч(XDTOОбъект.Документ) = Тип("ОбъектXDTO")
					...
					Если ТипЗнч(XDTOОбъект.Документ.Баланс) = Тип("ОбъектXDTO")
						...
						Если ТипЗнч(XDTOОбъектПассивБаланса) = Тип("ОбъектXDTO") Тогда
							Если XDTOОбъектПассивБаланса.Свойства().Получить("КапРез") <> Неопределено
							 ИЛИ XDTOОбъектПассивБаланса.Свойства().Получить("Капитал") <> Неопределено Тогда
								СвойстваВидаОтчета.БалансНекоммерческойОрганизации = Ложь;
								СвойстваВидаОтчета.БалансКоммерческойОрганизации = Истина;
							ИначеЕсли ТипЗнч(XDTOОбъектПассивБаланса.Свойства().Получить("ЦелевФин")) <> Неопределено Тогда
								СвойстваВидаОтчета.БалансНекоммерческойОрганизации = Истина;
								СвойстваВидаОтчета.БалансКоммерческойОрганизации = Ложь;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
	...
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчетБухОтчетностьМП" Тогда
			...
			Если ТипЗнч(XDTOОбъект) = Тип("ОбъектXDTO")
				...
				Если ТипЗнч(XDTOОбъект.Документ) = Тип("ОбъектXDTO")
					...
					Если ТипЗнч(XDTOОбъект.Документ.Баланс) = Тип("ОбъектXDTO")
						...
						XDTOОбъектПассивБаланса = XDTOОбъект.Документ.Баланс.Пассив;
						Если ТипЗнч(XDTOОбъектПассивБаланса) = Тип("ОбъектXDTO") Тогда
							Если XDTOОбъектПассивБаланса.Свойства().Получить("ЦелевСредства") <> Неопределено
							 ИЛИ XDTOОбъектПассивБаланса.Свойства().Получить("КапРез") = Неопределено Тогда
								СвойстваВидаОтчета.БалансНекоммерческойОрганизации = Истина;
								СвойстваВидаОтчета.БалансКоммерческойОрганизации = Ложь;
							КонецЕсли;
						КонецЕсли;
