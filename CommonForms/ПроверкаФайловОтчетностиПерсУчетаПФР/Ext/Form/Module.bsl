...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВыполнятьПроверкуНаСервере Тогда
		...
		Если Не (ПроверятьПрограммойCheckUFA И ФайлПрограммыCheckUFAОбнаружен) 
			И Не (ПроверятьПрограммойCheckXML И ФайлПрограммыCheckXMLОбнаружен)  
			И Не (ПроверятьПрограммойПОПД И ФайлПрограммыПОПДОбнаружен) Тогда 
...
Функция РезультатПроверкиФайловВДлительнойОперации()
	...
	НастройкиПроверочныхПрограмм = ПерсонифицированныйУчет.ОбщиеНастройкиПрограммПроверкиОтчетности();
	Если НастройкиПроверочныхПрограмм = Неопределено Тогда 
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось проверить файлы'"));
		Возврат Неопределено;
	КонецЕсли;
	
	СохраненныеФайлы = СохранитьФайлыНаСервере();
	
	СтруктураПараметров = ПараметрыПроверкиФайлов(ЭтаФорма);
	СтруктураПараметров.КаталогФайлов = СохраненныеФайлы.КаталогФайлов;
	СтруктураПараметров.ПутиКФайлам = СохраненныеФайлы.ПутиКФайлам;
	
	СтруктураПараметров.КаталогПрограммыСheckXml = НастройкиПроверочныхПрограмм.КаталогПрограммыCheckXML;
	СтруктураПараметров.КаталогПрограммыСheckUFA = НастройкиПроверочныхПрограмм.КаталогПрограммыCheckUFA;
	СтруктураПараметров.КаталогПрограммыПОПД = НастройкиПроверочныхПрограмм.КаталогПрограммыПОПД;
	...
	Если Результат.ЗаданиеВыполнено Тогда
		...
		УдалитьФайлыНаСервере(СохраненныеФайлы);
	КонецЕсли;
...
&НаСервере
Функция СохранитьФайлыНаСервере()
	
	СохраненныеФайлы = Новый Структура("КаталогФайлов, ПутиКФайлам");
	
	УстановитьПривилегированныйРежим(Истина);
	
	СоответствиеВладельцевПутиКФайлу = Новый Соответствие;
	
	СохраненныеФайлы.КаталогФайлов = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПолучитьИмяВременногоФайла(""));	
	
	КаталогФайлов = СохраненныеФайлы.КаталогФайлов;
	СоздатьКаталог(КаталогФайлов);
	
	Для Каждого ОписаниеФайла Из ОписаниеПроверяемыхФайлов Цикл
		ПолноеИмяФайла = КаталогФайлов + "\" + ОписаниеФайла.ИмяФайла;
		Если ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПолноеИмяФайла).Имя <> ОписаниеФайла.ИмяФайла Тогда 
			Продолжить;
		КонецЕсли;
	...
	СохраненныеФайлы.ПутиКФайлам = Новый ФиксированноеСоответствие(СоответствиеВладельцевПутиКФайлу);
	ПутиКФайлам = СохраненныеФайлы.ПутиКФайлам;
	
	Возврат СохраненныеФайлы;
...
&НаСервере
Процедура УдалитьФайлыНаСервере(СохраненныеФайлы)
	
	Файл = Новый Файл(СохраненныеФайлы.КаталогФайлов);
	
	Если Файл.Существует() И Файл.ЭтоКаталог() Тогда
		УдалитьФайлы(СохраненныеФайлы.КаталогФайлов);
	КонецЕсли;
