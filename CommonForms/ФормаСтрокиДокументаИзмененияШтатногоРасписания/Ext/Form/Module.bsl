...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СозданиеНовой И НЕ Параметры.Свойство("АдресДанныхПозицииВХранилище") Тогда
		...
		Иначе
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
				МодульОрганизационнаяСтруктура = ОбщегоНазначения.ОбщийМодуль("ОрганизационнаяСтруктура");
				МодульОрганизационнаяСтруктура.ПриСозданииНовойПозиции(ЭтотОбъект, СсылкаНаОбъект);
			КонецЕсли;
		...
		Если Параметры.Свойство("АдресДанныхПозицииВХранилище") Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ПрименятьРезультатыСпециальнойОценкиУсловийТруда") Тогда
				...
				КлассыУсловийТрудаПоДолжностям.ВыявленыВредныеОпасныеФакторы =
					ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеПозиции, "ВыявленыВредныеОпасныеФакторы", Ложь);
				КлассыУсловийТрудаКлиентСервер.УстановитьДоступностьФлагаВредныхФакторов(
					Элементы, КлассыУсловийТрудаПоДолжностям.КлассУсловийТруда);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		КлассыУсловийТрудаДоступныДляИзменения = МодульОхранаТруда.КлассыУсловийТрудаДоступныДляИзменения(ОрганизацияФормы);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		...
		МодульОрганизационнаяСтруктураКлиент.ОбработкаОповещенияФормыМестоПозицииВСтруктуреПредприятия(ЭтотОбъект,
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		МодульОрганизационнаяСтруктура = ОбщегоНазначения.ОбщийМодуль("ОрганизационнаяСтруктура");
		МодульОрганизационнаяСтруктура.ШтатноеРасписаниеОбработкаПроверкиЗаполненияНаСервере(ЭтотОбъект, Отказ);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_МестоВСтруктуреПредприятияИсторияИзменения(Элемент)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		МодульОрганизационнаяСтруктураКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОрганизационнаяСтруктураКлиент");
		МодульОрганизационнаяСтруктураКлиент.ОткрытьИсториюИзмененияМестаПозицииВСтруктуреПредприятия(ЭтотОбъект,
			?(ВнешниеДанные, РедактируемаяПозиция, СсылкаНаОбъект));
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьДоступностьЭлементов(Форма)
	...
	Если Не Форма.КлассыУсловийТрудаДоступныДляИзменения Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			"ГруппаСпецоценкаУсловийТрудаКлассыУсловийТруда",
			"ТолькоПросмотр",
...
Процедура ПрочитатьДанныеДолжностиСФР(ДанныеДолжности = Неопределено)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеДолжности, "ЯвляетсяДолжностьюЛетногоЭкипажа, ЯвляетсяШахтерскойДолжностью");
...
Функция АдресДанныхПозицииВХранилище()
	...
	ОписаниеПозиции.Вставить("ВыявленыВредныеОпасныеФакторы", КлассыУсловийТрудаПоДолжностям.ВыявленыВредныеОпасныеФакторы);
