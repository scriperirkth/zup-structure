...
Процедура СоздатьРеквизитыСреднегоЗаработка()
	...
	Для Каждого СпособРасчета Из СпособыРасчета Цикл
		...
		Если ИндексироватьСреднийЗаработокЧастично Тогда 
			...
			ЗаголовокРеквизитаИндексируемый = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (индексируемый)'"), ЗаголовокРеквизита);
			...
			ЗаголовокРеквизитаНеидексируемый = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (неиндексируемый)'"), ЗаголовокРеквизита);
...
Процедура ПриИзмененииПериодаРасчета(ДатаДоИзменения, ИмяРеквизита)
	...
	Иначе
		Если ОкончаниеПериодаРасчета > КонецМесяца(ДобавитьМесяц(ДатаНачалаСобытия, -1)) Тогда
			Если КонецМесяца(ОкончаниеПериодаРасчета) = КонецМесяца(ДатаНачалаСобытия) Тогда
				Если ИмяРеквизита = "ОкончаниеПериодаРасчета" Тогда 
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ДатаДоИзменения", ДатаДоИзменения);
	ДополнительныеПараметры.Вставить("ИмяРеквизита", ИмяРеквизита);
	
	Если ТребуетсяПодтверждение Тогда
		ТекстВопроса = НСтр("ru = 'Расчетный период включает месяц наступления события, с которым связано сохранение среднего заработка. Продолжить?'");
		Оповещение = Новый ОписаниеОповещения("ПриИзмененииПериодаРасчетаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
	Иначе 
		Ответ = ?(Отказ, КодВозвратаДиалога.Отмена, КодВозвратаДиалога.ОК);
		ПриИзмененииПериодаРасчетаЗавершение(Ответ, ДополнительныеПараметры);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПриИзмененииПериодаРасчетаЗавершение(Ответ, ДополнительныеПараметры) Экспорт 
	...
	ДатаДоИзменения = ДополнительныеПараметры.ДатаДоИзменения;
