...
Процедура КалендарьПриВыводеПериода(Элемент, ОформлениеПериода)
	Календарь = ОформлениеПериода.НачалоПериода;
...
&НаКлиенте
Процедура ШаблонДатПриИзменении(Элемент)
	Если ПустаяСтрока(ШаблонДат) Тогда
		Возврат;
	КонецЕсли;
	МассивНовыхДат = СтроковыеФункцииБЗККлиентСервер.МассивДатИзСтрокиБыстрогоВвода(ШаблонДат, Календарь);
	Для Каждого НоваяДата Из МассивНовыхДат Цикл
		ЭлементСписка = Список.НайтиПоЗначению(НоваяДата);
		Если ЭлементСписка = Неопределено Тогда
			Список.Добавить(НоваяДата);
		КонецЕсли;
	КонецЦикла;
	ШаблонДат = "";
