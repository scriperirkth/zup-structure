...
Процедура ОбработкаЗагрузки(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если ТипЗнч(ИмяПользователяИБ) = ТипПользователь Тогда
		лкИмяПользователяИБ = ОбработкаНовостейСлужебный.ПолучитьИмяПользователяИБ(ИмяПользователяИБ);
	ИначеЕсли ПустаяСтрока(ИмяПользователяИБ) Тогда
		лкИмяПользователяИБ = ОбработкаНовостейСлужебный.ПолучитьИмяПользователяИБ(Неопределено);
	Иначе
	...
	ИначеЕсли (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистраторомДляОбластиДанных")
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ОбработкаНовостейСлужебный.РазделениеВключено())
					И (ОбработкаНовостейСлужебный.ДоступноИспользованиеРазделенныхДанных()) Тогда
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейДляОбластиДанных" Тогда
		...
		// В конфигурации есть общие реквизиты с разделением и включена ФО РаботаВМоделиСервиса.
		Если (ОбработкаНовостейСлужебный.РазделениеВключено()) Тогда
			// Если НЕ вошли в область данных.
			Если (НЕ ОбработкаНовостейСлужебный.ДоступноИспользованиеРазделенныхДанных()) Тогда
	...
	ИначеЕсли КлючОбъекта = "НастройкиПоказаНовостей" Тогда
		...
		Если (ОбработкаНовостейСлужебный.РазделениеВключено())
				...
				И (ОбработкаНовостейСлужебный.СеансЗапущенБезРазделителей())
				// Не зашли в область данных.
				И (НЕ ОбработкаНовостейСлужебный.ДоступноИспользованиеРазделенныхДанных()) Тогда
		...
		Иначе
			...
			УстановитьПривилегированныйРежим(Истина);
				Настройки = Новый Структура;
				НайденныйПользователь = ОбработкаНовостейСлужебный.ПолучитьПользователяПоИмениПользователяИБ(лкИмяПользователяИБ);
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейПользовательские" Тогда
		...
		Иначе
			...
			Запрос.Текст = "
				...
				|	КОНЕЦ = ЛОЖЬ
				|";
			Запрос.УстановитьПараметр("ТекущийПользователь", ОбработкаНовостейСлужебный.ПолучитьПользователяПоИмениПользователяИБ(лкИмяПользователяИБ));
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеЛентыНовостей" Тогда
		Настройки = ОбработкаНовостей.ПолучитьОтключенныеЛентыНовостей(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключена" Тогда
		Настройки = ОбработкаНовостей.ПолучитьЛентыНовостейПодпискаВключена(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);

	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаВключенаПользователем" Тогда
		Настройки = ОбработкаНовостей.ПолучитьЛентыНовостейПодпискаВключенаПользователем(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);

	ИначеЕсли КлючОбъекта = "ЛентыНовостейПодпискаОтключенаПользователем" Тогда
		Настройки = ОбработкаНовостей.ПолучитьЛентыНовостейПодпискаОтключенаПользователем(лкИмяПользователяИБ);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);
	...
	ИначеЕсли КлючОбъекта = "ЛентыНовостейВсеНастройки" Тогда
		...
		КонецЕсли;
		Настройки = ОбработкаНовостей.ПолучитьНастройкиЛентНовостей(лкИмяПользователяИБ, ОтборПоЛентеНовостей);
		Настройки = ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек);
...
Функция ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек) Экспорт
	...
	ИначеЕсли КлючОбъекта = "НастройкиПоказаНовостей" Тогда
		...
		Результат = Новый Структура;
			...
			Результат.Вставить("Форма_РазмещениеПанелей"        , "Текст новости справа от списка новостей"); // АПК:1297 Это идентификатор.
			Результат.Вставить("ОтображаемыеНовости_Вариант"    , "Все"); // АПК:1297 Это идентификатор.
		...
		Если ТипЗнч(Настройки) = ТипСтруктура Тогда
			...
			Если ТипЗнч(Результат.Форма_РазмещениеПанелей) <> ТипСтрока
					ИЛИ (
						ВРег(Результат.Форма_РазмещениеПанелей) <> ВРег("Текст новости справа от списка новостей") // АПК:1297 Это идентификатор.
						И ВРег(Результат.Форма_РазмещениеПанелей) <> ВРег("Текст новости под списком новостей") // АПК:1297 Это идентификатор.
						И ВРег(Результат.Форма_РазмещениеПанелей) <> ВРег("Текст новости не отображается") // АПК:1297 Это идентификатор.
						И ВРег(Результат.Форма_РазмещениеПанелей) <> ВРег("Текст новости не отображается, есть предпросмотр")) Тогда // АПК:1297 Это идентификатор.
				Результат.Форма_РазмещениеПанелей = "Текст новости справа от списка новостей"; // АПК:1297 Это идентификатор.
			КонецЕсли;
		...
		Если (ОбработкаНовостейСлужебный.РазделениеВключено()) Тогда // Есть разделители и установлена ФО
			Если (ОбработкаНовостейСлужебный.СеансЗапущенБезРазделителей()) Тогда // Текущий пользователь настроен для работы без разделения (администратор)
