...
Процедура ПроверитьАктуальностьВидаРасчета(Отказ)
	...
	Если ВыводитьСообщениеОбОшибке Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Выборка.Сотрудник, "Объект.ВАрхиве" , , Отказ);
		КонецЦикла;
		...
		Если ФОИспользоватьШтатноеРасписание Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Выборка.Ссылка, "Объект.ВАрхиве" , , Отказ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
