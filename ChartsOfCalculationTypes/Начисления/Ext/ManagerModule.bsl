...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьВидДоходаИсполнительногоПроизводства2022");
	Методы.Вставить("ВосстановитьПорядокНачисленийПодбираемыхПоУмолчанию");
	Методы.Вставить("УточнитьВхождениеВУчетСреднегоЗаработкаКомпенсацииНеИспользованныхДнейЧасовОтдыха");
	Методы.Вставить("СоздатьНачисленияКомпенсацииНеИспользованныхОтгулов");
	Методы.Вставить("УточнитьФормулыНачисленийЗависящихОтСреднегоЗаработка");
	Методы.Вставить("ОтключитьПризнакПособийНачисляетсяПриРасчетеПервойПоловиныМесяца");
	Методы.Вставить("УточнитьПризнакУчитыватьИзмененияВремяВЧасах");
	Методы.Вставить("ВыделитьНачисленияРедактируемыеВОтдельныхПолях");
	Методы.Вставить("ЗаполнитьКатегориюСтатистикиРабочаяСила");
	Методы.Вставить("ДобавитьИндексациюВФормулуВыходногоПособия");
	Методы.Вставить("УточнитьВидНачисленияДляНУКатегорииКомпенсацияНеИспользованныхДнейЧасовОтдыха");
	Методы.Вставить("УточнитьСвойстваНДФЛСтраховыхВзносовКатегорииКомпенсацияНеИспользованныхДнейЧасовОтдыха");
	Методы.Вставить("УточнитьКодДоходаНДФЛКатегориюОписаниеРКиСН");
	Методы.Вставить("ОплатаДнейУходаЗаДетьмиИнвалидамиПредопределеннымСпособом");
	Методы.Вставить("ЗаполнитьВидДоходаИсполнительногоПроизводства");
	Методы.Вставить("УстановитьПризнакИспользоватьПособиеПоУходуЗаРебенкомДоТрехЛет");
	Методы.Вставить("ЗаполнитьКатегориюСтатистикиПособийПоУходуЗаРебенкомДоТрехЛет");
	Методы.Вставить("СоздатьНачисленияПростойОтстранениеПриостановкаРаботы");
	Методы.Вставить("ЗаполнитьРегистрЗависимостиНачислений");
	Методы.Вставить("ОбновитьВторичныеДанныеНачисленийСНакапливаемымиПоказателями");
	Методы.Вставить("ЗаполнитьПоказателиНачисленияОтпускаПострадавшимВАварииЧАЭС");
	Методы.Вставить("УстановитьПериодичностьПроверяемыхНачислений");
	Методы.Вставить("УточнитьНаименованияНачисленийУчебныхОтпусков");
	Методы.Вставить("ЗаполнитьРегистрОперативныеПоказателиДляВыявленияНачислений");
	Методы.Вставить("ЗаполнитьРегистрВидыВремениДляВыявленияНачислений");
	Методы.Вставить("УстановитьПризнакУчитыватьВесьЗаработок");
	Методы.Вставить("ЗаполнитьКатегориюСтатистики2020");
	Методы.Вставить("ОчиститьКатегориюСтатистическогоНаблюденияДоходаВНатуральнойФорме");
	Методы.Вставить("ЗаполнитьВидДоходаИсполнительногоПроизводстваМатпомощиПриРожденииРебенка");
	Методы.Вставить("ОтключитьНачислениеОтсутствиеПоБолезниВнутрисменное");
КонецПроцедуры
...
Функция СвойстваНачисленияПоКатегорииПоУмолчанию() Экспорт
	...
	Описание.РедактируетсяВОтдельныхПолях = Ложь;
...
Функция ОписаниеНачисления() Экспорт 
	...
	|СтратегияОтраженияВУчете,
	|ПериодичностьНачисления,
	|ДополнительныеВидыВремени,
	|СсылкаНаОбъект,
	|РедактируетсяВОтдельныхПолях"); // Содержит ссылку на объект, который уже был создан Из помощника начальной настройки программы.
...
Процедура СоздатьИзменитьНачисленияПоОписанию(Коллектор, Описание, ПроверятьНаличиеНачислений = Истина, ОбновлятьНачисленияПоСсылке = Ложь) Экспорт 
	...
	//Заполнение дополнительными видами времени
	Если Описание.ДополнительныеВидыВремени <> Неопределено Тогда
		Для Каждого ВидВремени Из Описание.ДополнительныеВидыВремени Цикл
			Если НачислениеОбъект.ВидыВремени.Найти(ВидВремени, "ВидВремени") <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрока = НачислениеОбъект.ВидыВремени.Добавить();
			НоваяСтрока.ВидВремени = ВидВремени;
			НоваяСтрока.ОпределяющийВидВремени = Истина;
		КонецЦикла;
	КонецЕсли;
	НачислениеОбъект.РедактируетсяВОтдельныхПолях = Описание.РедактируетсяВОтдельныхПолях;
...
Процедура СоздатьНачисленияПоНастройкам(ПараметрыПланаВидовРасчета = Неопределено, НачальноеЗаполнение = Ложь, ТекущиеНастройки = Неопределено) Экспорт
	...
	Описание.Наименование			= НСтр("ru = 'Дополнительный отпуск с сохранением среднего заработка'");
	Описание.КраткоеНаименование 	= НСтр("ru = 'Доп. отп.'");
	...
	// Дополнительный отпуск без сохранения среднего заработка.
	Описание = ОписаниеНачисления();
	Описание.КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОтпускБезОплаты;
	Описание.СвойстваПоКатегории 	= СвойстваНачислений[Описание.КатегорияНачисленияИлиНеоплаченногоВремени];
	Описание.Код					= НСтр("ru = 'ОТПУД'");
	Описание.Наименование			= НСтр("ru = 'Дополнительный отпуск без сохранения среднего заработка'");
	Описание.КраткоеНаименование 	= НСтр("ru = 'Доп. отп. без опл.'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("Наставничество");
		Модуль.ПриСозданииПланаВидовРасчетаПоНастройкам(НастройкиРасчетаЗарплаты, СвойстваНачислений, КоллекторНачислений, ПараметрыПланаВидовРасчета, ТекущиеНастройкиРасчетаЗарплаты);
	КонецЕсли;
...
Процедура СоздатьНачисленияРКиСН(КоллекторНачислений = Неопределено, СвойстваНачислений = Неопределено) Экспорт
	...
	Описание.КлючевыеСвойства		= "КатегорияНачисленияИлиНеоплаченногоВремени,КатегорияДохода,РедактируетсяВОтдельныхПолях";
	Если ВостребованностьРКиСН.РайонныйКоэффициент Тогда
		СоздатьИзменитьНачисленияПоОписанию(КоллекторНачислений, Описание);
	Иначе
		ОтключитьИспользованиеНачисленийПоОписанию(КоллекторНачислений, Описание);
	КонецЕсли;
	Описание.РедактируетсяВОтдельныхПолях = Истина;
	...
	Если РасчетЗарплаты.ПрименяетсяАУСН() Тогда
		СоздатьНачисленияРКиСНАУСН(КоллекторНачислений, СвойстваНачислений)
	КонецЕсли;
...
// Создает или отключает в зависимости от настроек начисления.
// «Районный коэффициент» и «Северная надбавка» для АУСН.
// 
Процедура СоздатьНачисленияРКиСНАУСН(КоллекторНачислений = Неопределено, СвойстваНачислений = Неопределено) Экспорт
...
Функция СвойстваНачисленийПоКатегориям() Экспорт
	...
	Описание.ПараметрыВыбора.КодДоходаНДФЛ.Отбор_Код.Добавить("ДСВО");
	Описание.ПараметрыВыбора.КодДоходаНДФЛ.Отбор_Код.Добавить("2007");
	...
	Описание.ПараметрыВыбора.КодДоходаНДФЛ.Отбор_Код.Добавить("2750");
	...
	Описание.СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПроизвольнаяФормула;
	Описание.ФормулаРасчета = "((СреднийЗаработокИндексируемый * КоэффициентИндексацииСреднегоЗаработка + СреднийЗаработокНеиндексируемый) - (СреднийЗаработокИндексируемыйРК * КоэффициентИндексацииСреднегоЗаработка + СреднийЗаработокНеиндексируемыйРК) - (СреднийЗаработокИндексируемыйСН * КоэффициентИндексацииСреднегоЗаработка + СреднийЗаработокНеиндексируемыйСН)) * ДнейЧасовВыходногоПособия";
	Описание.ДополнительныеФормулыРасчета.ФормулаРасчетаДолиРК = "(СреднийЗаработокИндексируемыйРК * КоэффициентИндексацииСреднегоЗаработка + СреднийЗаработокНеиндексируемыйРК) * ДнейЧасовВыходногоПособия";
	Описание.ДополнительныеФормулыРасчета.ФормулаРасчетаДолиСН = "(СреднийЗаработокИндексируемыйСН * КоэффициентИндексацииСреднегоЗаработка + СреднийЗаработокНеиндексируемыйСН) * ДнейЧасовВыходногоПособия";
	Описание.ВидВремени = Перечисления.ВидыРабочегоВремениСотрудников.ДополнительноОплачиваемоеВПределахНормы;
	Описание.КодДоходаНДФЛ = Неопределено;
	Описание.КатегорияДохода = Неопределено;
	...
	Описание.ПараметрыВыбора = ПараметрыВыбораСвойствНачисления();
	Описание.ПараметрыВыбора.КодДоходаНДФЛ = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыВыбораНДФЛ);
	Описание.ПараметрыВыбора.КодДоходаНДФЛ.Отбор_Код.Добавить("2300");
	Описание.ПараметрыВыбора.КодДоходаНДФЛ.Отбор_Код.Добавить("РКСН");
	Описание.ПериодРасчетаБазовыхНачислений = Перечисления.ПериодыРасчетаБазовыхНачислений.ТекущийМесяц;
...
Функция ВидыДокументовНачислений() Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДокументовНачисления.Увольнение),
	|	Начисления.Ссылка
	|ИЗ
	|	ПланВидовРасчета.Начисления КАК Начисления
	|ГДЕ
...
Функция ТребованияНачисленийТаблица(НачисленияОтбор) Экспорт
	...
	Запрос.УстановитьПараметр("КатегорииРазбиваемыхНачислений", КатегорииРазбиваемыхНачислений());
	...
	Запрос.Текст = 
		...
		|	НЕ МАКСИМУМ(ВЫБОР
		|				КОГДА ПоказателиРасчета.КатегорияНачисленияИлиНеоплаченногоВремени В (&КатегорииРазбиваемыхНачислений)
		|					ТОГДА ЛОЖЬ
		|				ИНАЧЕ ПоказателиРасчета.НачисляетсяВЦеломЗаМесяц
		|			КОНЕЦ) КАК Разбивать,
...
Функция НачислениеРайонныйКоэффициент(ПрименяетсяАУСН) Экспорт
	
	Если ПрименяетсяАУСН Тогда
		КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН;
	Иначе
		КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки;
	КонецЕсли;
	
	Отбор = Новый Структура("КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени, РедактируетсяВОтдельныхПолях",
		КатегорияДохода, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.РайонныйКоэффициент, Истина);
...
Функция НачислениеСевернаяНадбавка(ПрименяетсяАУСН) Экспорт
	...
	Отбор = Новый Структура("КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени,РедактируетсяВОтдельныхПолях",
		КатегорияДохода, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.СевернаяНадбавка, Истина);
	Возврат НачислениеПоУмолчанию(Отбор);
	
КонецФункции

// Возвращает начисление надбавка за вредность.
Функция НачислениеНадбавкаЗаВредность(ПрименяетсяАУСН) Экспорт
	...
	Отбор = Новый Структура("КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени,РедактируетсяВОтдельныхПолях",
		КатегорияДохода, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.НадбавкаЗаВредность, Истина);
	Возврат НачислениеПоУмолчанию(Отбор);
...
#Область ПроцедурыИФункцииКонвертацииДанных

Функция СоздатьНачислениеДолиРКСН(КатегорияОсновногоВидаРасчета, КатегорияВидаРасчетаДолиРКСН) Экспорт
	
	ВидРасчетаДолиРКСН = Неопределено;
	ВидРасчетаДолиРКСНОбъект = Неопределено;
	
	Отбор = Новый Структура;
	Отбор.Вставить("КатегорияНачисленияИлиНеоплаченногоВремени", КатегорияОсновногоВидаРасчета);
	...
	ОсновнойВидРасчета = НачислениеПоУмолчанию(Отбор);
	
	Если ОсновнойВидРасчета = Неопределено Тогда
		Возврат ВидРасчетаДолиРКСН;
	КонецЕсли;

	ЭтоДоляРК = КатегорияВидаРасчетаДолиРКСН = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке;
	...
	Попытка
		...
		СписокСвойств = НаследуемыеСвойстваОсновногоНачисления();
		ЗначенияСвойствОсновногоВидаРасчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОсновнойВидРасчета, СписокСвойств);
		
		Описание = ОписаниеДолиРКСН(ЭтоДоляРК, СвойстваНачислений, ЗначенияСвойствОсновногоВидаРасчета, ОсновнойВидРасчета);
		Описание.КлючевыеСвойства = "КатегорияНачисленияИлиНеоплаченногоВремени, ОсновнойВидРасчета";
		
		СоздатьИзменитьНачисленияПоОписанию(КоллекторНачислений, Описание);
		
		Для Каждого КлючИЗначение Из КоллекторНачислений Цикл
			ВидРасчетаДолиРКСН = КлючИЗначение.Ключ;
			ВидРасчетаДолиРКСНОбъект = КлючИЗначение.Значение;
		КонецЦикла;
		
		Если ВидРасчетаДолиРКСНОбъект <> Неопределено Тогда
			ВидРасчетаДолиРКСНОбъект.ВАрхиве = Истина;
		КонецЕсли;
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации("Ошибка записи начисления доли РК, СН", УровеньЖурналаРегистрации.Ошибка, Метаданные.ПланыВидовРасчета.Начисления, , ОписаниеОшибки(), РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
		Возврат Неопределено;
...
Функция КатегорияСтатистическогоНаблюденияФорма1РабочаяСила(КатегорияНачисленияИлиНеоплаченногоВремени) Экспорт
	
	Если КатегорииНачисленийСтатистическогоУчетаРабочаяСилаЗаработнаяПлата().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ЗаработнаяПлата;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаДоплатыНадбавки().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДоплатыНадбавки;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаПремииВознаграждения().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ПремииВознаграждения;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаВыплатыПоРайонномуРегулированию().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ВыплатыПоРайонномуРегулированию; 
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаДругиеВидыОплатыТруда().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ДругиеВидыОплатыТруда; 
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаОплатаЗаНеотработанноеВремя().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаЗаНеотработанноеВремя; 
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаЕдиновременныеПоощрительныеВыплаты().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ЕдиновременныеПоощрительныеВыплаты;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаОплатаПитанияПроживания().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ОплатаПитанияПроживания;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаВыходноеПособиеНачисленияНаПериодТрудоустройства().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ВыходноеПособиеНачисленияНаПериодТрудоустройства;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаСуммыПоВременнойНетрудоспособности().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.СуммыПоВременнойНетрудоспособности;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаМатериальнаяПомощь().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.МатериальнаяПомощь;
		
	ИначеЕсли КатегорииНачисленийСтатистическогоУчетаРабочаяСилаРасходыНаКомандировки().Найти(
		КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено Тогда
		
		Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.РасходыНаКомандировки;
		
	КонецЕсли;
	
	Возврат Перечисления.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ПустаяСсылка();
...
Процедура СоздатьНачислениеНадбавкаЗаВредность(КоллекторНачислений = Неопределено, СвойстваНачислений = Неопределено, НастройкиРасчетаЗарплаты = Неопределено)
	...
	Описание.ВидНачисленияДляНУ 	= Перечисления.ВидыНачисленийОплатыТрудаДляНУ.пп3ст255;
	Описание.НачисляетсяПриРасчетеПервойПоловиныМесяца = Истина;
	Описание.КатегорияДохода		= Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда;
	Описание.КлючевыеСвойства		= "КатегорияНачисленияИлиНеоплаченногоВремени,КатегорияДохода,РедактируетсяВОтдельныхПолях";
	...
	Если РасчетЗарплаты.ПрименяетсяАУСН() Тогда
		СоздатьНачислениеНадбавкаЗаВредностьАУСН(КоллекторНачислений, СвойстваНачислений, НастройкиРасчетаЗарплаты);
	КонецЕсли;
...
Процедура СоздатьНачислениеНадбавкаЗаВредностьАУСН(КоллекторНачислений = Неопределено, СвойстваНачислений = Неопределено, НастройкиРасчетаЗарплаты = Неопределено)
	...
	// Описываем создание надбавки при АУСН
	Описание = ОписаниеНачисления();
	Описание.КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.НадбавкаЗаВредность;
	Описание.СвойстваПоКатегории 	= СвойстваНачислений[Описание.КатегорияНачисленияИлиНеоплаченногоВремени];
	Описание.Код					= НСтр("ru = 'НВР'");
...
Процедура СоздатьНачисленияДляУчетаВремениСотрудниковВЧасах(КоллекторНачислений, СвойстваНачислений, НастройкиРасчетаЗарплаты, ПараметрыПланаВидовРасчета, ТекущиеНастройки = Неопределено)
	...
	Если НастройкиУчетаВремени.УчитыватьНочныеЧасы И ИспользоватьУчетВремениСотрудниковВЧасах Тогда
		Описание.ДополнительныеВидыВремени = Новый Массив;
		Если НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных Тогда
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиНочноеВремя"));
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиБезПовышеннойОплатыНочноеВремя"));
		КонецЕсли;
		Если НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных Тогда 
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СверхурочныеНочноеВремя"));
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СверхурочныеБезПовышеннойОплатыНочноеВремя"));
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы И ИспользоватьУчетВремениСотрудниковВЧасах Тогда
		Описание.ДополнительныеВидыВремени = Новый Массив;
		Если НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных Тогда 
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиВечернееВремя"));
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиБезПовышеннойОплатыВечернееВремя"));
		КонецЕсли;
		Если НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных Тогда 
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СверхурочныеВечернееВремя"));
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СверхурочныеБезПовышеннойОплатыВечернееВремя"));
		КонецЕсли;
	КонецЕсли;
	...
	Описание.ВидНачисленияДляНУ = Перечисления.ВидыНачисленийОплатыТрудаДляНУ.пп3ст255;
	Описание.КлючевыеСвойства = "ВидВремени,ОбозначениеВТабелеУчетаРабочегоВремени";
	Если НастройкиРасчетаЗарплаты.ИспользоватьОплатуСверхурочных И ИспользоватьУчетВремениСотрудниковВЧасах И НастройкиРасчетаЗарплаты.ИспользоватьОтгулы Тогда
		Описание.ДополнительныеВидыВремени = Новый Массив;
		Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы Тогда
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СверхурочныеБезПовышеннойОплатыВечернееВремя"));
		КонецЕсли;
	...
	Описание.ВидНачисленияДляНУ		= Перечисления.ВидыНачисленийОплатыТрудаДляНУ.пп3ст255;
	Описание.КлючевыеСвойства		= "ОбозначениеВТабелеУчетаРабочегоВремени";
	Если НастройкиРасчетаЗарплаты.ИспользоватьОплатуПраздничныхИВыходных И НастройкиРасчетаЗарплаты.ИспользоватьОтгулы Тогда
		Описание.ДополнительныеВидыВремени = Новый Массив;
		Если НастройкиУчетаВремени.УчитыватьВечерниеЧасы Тогда
			Описание.ДополнительныеВидыВремени.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ПраздникиБезПовышеннойОплатыВечернееВремя"));
		КонецЕсли;
...
Процедура СоздатьВнутрисменныеОтклонения(КоллекторНачислений, СвойстваНачислений, НастройкиРасчетаЗарплаты, ПараметрыПланаВидовРасчета, ТекущиеНастройкиРасчетаЗарплаты = Неопределено)
	...
	Описание.Наименование			= НСтр("ru = 'Дополнительный отпуск без сохранения среднего заработка (внутрисменный)'");
	Описание.КраткоеНаименование 	= НСтр("ru = 'Доп. отп. без опл'");
	Описание.ВидВремени 			= Перечисления.ВидыРабочегоВремениСотрудников.ЧасовоеНеотработанное;
	Описание.ОбозначениеВТабелеУчетаРабочегоВремени	= ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ОтпускНаОбучениеНеоплачиваемый");
	Описание.ВидОтпуска = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.ОтпускБезОплатыУчебный");
	...
	Если (ИспользоватьГосударственнуюСлужбу ИЛИ ИспользоватьМуниципальнуюСлужбу)
		...
		Попытка
			...
			Если ИспользоватьГосударственнуюСлужбу Тогда
				...
				Описание.Наименование								= НСтр("ru = 'Компенсация за неотработанные дни при увольнении ГС (сохр. ден. содержание)'");
				Описание.КраткоеНаименование						= НСтр("ru = 'Комп. при увольн. гос. сл.'");
			Иначе
				Описание.Код										= НСтр("ru = 'КУМ'");
				Описание.Наименование								= НСтр("ru = 'Компенсация за неотработанные дни при увольнении МС (сохр. ден. содержание)'");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Создание начислений компенсации неиспользованных отгулов'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.32.63";
	Обработчик.РежимВыполнения = "Монопольно";
	...
	Обработчик.Комментарий     = НСтр("ru = 'Изменение настроек НДФЛ, страховых взносов начислений с категорией ""Компенсация неиспользованных дней часов отдыха""'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.35.67";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура       = "ПланыВидовРасчета.Начисления.УточнитьВидНачисленияДляНУКатегорииКомпенсацияНеИспользованныхДнейЧасовОтдыха";
	Обработчик.Комментарий     = НСтр("ru = 'Изменение вида начисления для НУ начислений с категорией ""Компенсация неиспользованных дней часов отдыха""'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.35.71";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура       = "ПланыВидовРасчета.Начисления.УточнитьВхождениеВУчетСреднегоЗаработкаКомпенсацииНеИспользованныхДнейЧасовОтдыха";
	Обработчик.Комментарий     = НСтр("ru = 'Изменение признака учета в среднем заработке начислений с категорией ""Компенсация неиспользованных дней часов отдыха""'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.36.5";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура       = "ПланыВидовРасчета.Начисления.ДобавитьИндексациюВФормулуВыходногоПособия";
	Обработчик.Комментарий     = НСтр("ru = 'Изменение формулы расчета (добавленеи индексации) начислений с категорией ""Выходное пособие""'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.37.12";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор   = Новый УникальныйИдентификатор("3e7598c7-d666-4513-ae96-fe50d1005ad5");
	Обработчик.Процедура       = "ПланыВидовРасчета.Начисления.ЗаполнитьКатегориюСтатистикиРабочаяСила";
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение категории статистического наблюдения для Формы №1 (рабочая сила)'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.37.13";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура       = "ПланыВидовРасчета.Начисления.ВыделитьНачисленияРедактируемыеВОтдельныхПолях";
	Обработчик.Комментарий     = НСтр("ru = 'Выделяются начисления, редактируемые в отдельных полях'");
...
Процедура УточнитьВидНачисленияДляНУКатегорииКомпенсацияНеИспользованныхДнейЧасовОтдыха() Экспорт
	
	ВидНачисленияДляНУ = Перечисления.ВидыНачисленийОплатыТрудаДляНУ.пп9ст255;
	...
	Запрос.УстановитьПараметр("ВидНачисленияДляНУ", ВидНачисленияДляНУ);
...
Процедура УточнитьВхождениеВУчетСреднегоЗаработкаКомпенсацииНеИспользованныхДнейЧасовОтдыха() Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.КомпенсацияНеИспользованныхДнейЧасовОтдыха)
		|	И (НЕ Начисления.ВходитВУчетОбщегоСреднегоЗаработка
		|			ИЛИ НЕ Начисления.ВходитВУчетСреднегоЗаработкаФСС)";
	...
	Пока Выборка.Следующий() Цикл
		...
		НачислениеОбъект = Выборка.Ссылка.ПолучитьОбъект();
		НачислениеОбъект.ВходитВУчетОбщегоСреднегоЗаработка = Истина;
		НачислениеОбъект.ВходитВУчетСреднегоЗаработкаФСС = Истина;
		...
		Если НачислениеОбъект.СреднийЗаработокФСС.Количество() = 0 Тогда
			НоваяСтрока = НачислениеОбъект.СреднийЗаработокФСС.Добавить();
			НоваяСтрока.ПорядокРасчета = Перечисления.ПорядокРасчетаСреднегоЗаработкаФСС.Постановление2011;
			НоваяСтрока.Значение = Перечисления.УчетНачисленийВСреднемЗаработкеФСС.Включать;
		КонецЕсли;
...
Процедура ЗаполнитьКатегориюСтатистикиРабочаяСила(ПараметрыОбновления = Неопределено) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Начисления.Ссылка КАК Ссылка
	...
	|	ПланВидовРасчета.Начисления КАК Начисления
	|ГДЕ
	|	Начисления.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила = ЗНАЧЕНИЕ(Перечисление.КатегорииСтатистическогоНаблюденияФорма1РабочаяСила.ПустаяСсылка)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НачислениеОбъект = Выборка.Ссылка.ПолучитьОбъект();
		Если ЗначениеЗаполнено(НачислениеОбъект.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила) Тогда
			Продолжить;
		КонецЕсли;
		НачислениеОбъект.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила = 
			ПланыВидовРасчета.Начисления.КатегорияСтатистическогоНаблюденияФорма1РабочаяСила(
				НачислениеОбъект.КатегорияНачисленияИлиНеоплаченногоВремени);
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НачислениеОбъект);
	КонецЦикла;
...
Процедура ДобавитьИндексациюВФормулуВыходногоПособия() Экспорт
	...
	Запрос.УстановитьПараметр("КатегорияНачисленияИлиНеоплаченногоВремени", Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ВыходноеПособие);
	...
	Пока Выборка.Следующий() Цикл
		...
		НачислениеОбъект = Выборка.Ссылка.ПолучитьОбъект();
		НачислениеОбъект.ФормулаРасчета = "(СреднийЗаработокИндексируемый * КоэффициентИндексацииСреднегоЗаработка + СреднийЗаработокНеиндексируемый) * ДнейЧасовВыходногоПособия";
...
Процедура УточнитьНаименованияНачисленийУчебныхОтпусков(ПараметрыОбновления = Неопределено) Экспорт
	
    Запрос = Новый Запрос;
	Запрос.Текст = 
	 "ВЫБРАТЬ
	 |	Начисления.Ссылка КАК НачислениеСсылка,
	 |	СОКРЛП(Начисления.Наименование) КАК Наименование
	 |ИЗ
	 |	ПланВидовРасчета.Начисления КАК Начисления
	 |ГДЕ
	 |	Начисления.Наименование ПОДОБНО ""%Дополнительный учебный отпуск%""";
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		НачислениеОбъект = Выборка.НачислениеСсылка.ПолучитьОбъект();
		Если СтрНайти(Выборка.Наименование, "Дополнительный учебный отпуск (оплачиваемый)") > 0 Тогда
			НачислениеОбъект.Наименование 		= СтрЗаменить(НачислениеОбъект.Наименование, "Дополнительный учебный отпуск (оплачиваемый)", "Дополнительный отпуск с сохранением среднего заработка");
			НачислениеОбъект.КраткоеНаименование 	= НСтр("ru = 'Доп. отп.'"); 
		ИначеЕсли СтрНайти(Выборка.Наименование, "Дополнительный учебный отпуск без оплаты") > 0 Тогда
			НачислениеОбъект.Наименование 		= СтрЗаменить(НачислениеОбъект.Наименование, "Дополнительный учебный отпуск без оплаты", "Дополнительный отпуск без сохранения среднего заработка");
			НачислениеОбъект.КраткоеНаименование 	= НСтр("ru = 'Доп. отп. без опл.'");
		ИначеЕсли СтрНайти(Выборка.Наименование, "Дополнительный учебный отпуск (сохр. денежное содержание)") > 0 Тогда
			НачислениеОбъект.Наименование 		= СтрЗаменить(НачислениеОбъект.Наименование, "Дополнительный учебный отпуск (сохр. денежное содержание)", "Дополнительный отпуск с сохранением заработка (сохр. денежное содержание)");
			НачислениеОбъект.КраткоеНаименование 	= НСтр("ru = 'Доп. отп.'"); 
		КонецЕсли;
...
//		ДанныеОсновногоВидаРасчета - ПланВидовРасчетаОбъект.Начисления или коллекция со свойствами ПланыВидовРасчета.НаследуемыеСвойстваОсновногоНачисления().
...
Функция ОписаниеДолиРКСН(СозданиеДолиРК, СвойстваНачислений, ДанныеОсновногоВидаРасчета, ОсновнойВидрасчетаСсылка)
	...
	Описание.Наименование = СтрШаблон(НСтр("ru = '%1 %2'"), СокрЛП(ДанныеОсновногоВидаРасчета.Наименование), Постфикс);
...
Процедура СоздатьНачисленияКомпенсацииНеИспользованныхДнейЧасовОтдыха(НастройкиРасчетаЗарплаты = Неопределено, КоллекторНачислений = Неопределено, СвойстваНачислений = Неопределено)
	...
	Попытка
		...
		Описание.Наименование			= НСтр("ru = 'Компенсация неиспользованных дней отдыха'");
		...
		Описание.УчетВремениВЧасах		= Ложь;
		Описание.КлючевыеСвойства 		= "КатегорияНачисленияИлиНеоплаченногоВремени, УчетВремениВЧасах";
		...
		Описание.Наименование			= НСтр("ru = 'Компенсация неиспользованных часов отдыха'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка создания начислений «Компенсации не испольованного времени отдыха»'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.ПланыВидовРасчета.Начисления, ,
...
Функция КатегорииРазбиваемыхНачислений()

	СписокКатегорий = Новый Массив;
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.КомпенсацияНеИспользованныхДнейЧасовОтдыха);
	
	Возврат СписокКатегорий;
	
КонецФункции

// Категории начислений для статистики Форма №1 (рабочая сила)
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаЗаработнаяПлата()
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаДоплатыНадбавки()
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаПремииВознаграждения()
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаВыплатыПоРайонномуРегулированию()
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаДругиеВидыОплатыТруда()
	
	СписокКатегорий = Новый Массив;
	
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.Прочее);
	
	Возврат СписокКатегорий;
	
КонецФункции

Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаОплатаЗаНеотработанноеВремя()
	
	СписокКатегорий = Новый Массив;
	
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаОтпуска);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПростояПоВинеРаботодателя);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПростояПоНезависящимОтРаботодателяПричинам);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаПоСреднемуЗаработку);
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаЕдиновременныеПоощрительныеВыплаты()
	
	СписокКатегорий = Новый Массив;
	
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ЕдиновременнаяВыплатаКОтпуску);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ЕдиновременнаяВыплатаКОтпускуГосслужащего);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.КомпенсацияОтпуска);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.МатериальнаяПомощьПриОтпуске);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.МатериальнаяПомощьПриОтпускеВоеннослужащего);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.МатериальнаяПомощьПриОтпускеГосслужащего);
	
	Возврат СписокКатегорий;
	
КонецФункции

Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаОплатаПитанияПроживания()
	
	СписокКатегорий = Новый Массив;
	
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.КомпенсационныеВыплаты);
	
	Возврат СписокКатегорий;
	
КонецФункции

Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаВыходноеПособиеНачисленияНаПериодТрудоустройства()
	
	СписокКатегорий = Новый Массив;
	
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ВыходноеПособие);
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ВыходноеПособиеМесячноеДенежноеСодержание);
	
	Возврат СписокКатегорий;
	
КонецФункции

Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаСуммыПоВременнойНетрудоспособности()
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаМатериальнаяПомощь()
	
	СписокКатегорий = Новый Массив;
	
	СписокКатегорий.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.МатериальнаяПомощь);
...
Функция КатегорииНачисленийСтатистическогоУчетаРабочаяСилаРасходыНаКомандировки()
...
Процедура ВыделитьНачисленияРедактируемыеВОтдельныхПолях() Экспорт
	
	ВыделитьНачислениеРедактируемоеВОтдельныхПолях(
		Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.РайонныйКоэффициент);
	ВыделитьНачислениеРедактируемоеВОтдельныхПолях(
		Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавки, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.СевернаяНадбавка);
	ВыделитьНачислениеРедактируемоеВОтдельныхПолях(
		Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.НадбавкаЗаВредность);
	Если РасчетЗарплаты.ПрименяетсяАУСН() Тогда
		ВыделитьНачислениеРедактируемоеВОтдельныхПолях(
			Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.РайонныйКоэффициент);
		ВыделитьНачислениеРедактируемоеВОтдельныхПолях(
			Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.СевернаяНадбавка);
		ВыделитьНачислениеРедактируемоеВОтдельныхПолях(
			Перечисления.КатегорииДоходовНДФЛ.ОплатаТрудаАУСН, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.НадбавкаЗаВредность);
	КонецЕсли;
	
КонецПроцедуры

Процедура ВыделитьНачислениеРедактируемоеВОтдельныхПолях(КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени)
	
	Отбор = Новый Структура("КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени, РедактируетсяВОтдельныхПолях",
		КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени, Истина);
	Начисление = НачислениеПоУмолчанию(Отбор);
	Если Начисление = Неопределено Тогда
		Отбор = Новый Структура("КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени, РедактируетсяВОтдельныхПолях",
			КатегорияДохода, КатегорияНачисленияИлиНеоплаченногоВремени, Ложь);
		Начисление = НачислениеПоУмолчанию(Отбор);
		Если Начисление <> Неопределено Тогда
			НачислениеОбъект = Начисление.ПолучитьОбъект();
			НачислениеОбъект.РедактируетсяВОтдельныхПолях = Истина;
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НачислениеОбъект, Истина);
		КонецЕсли;
	КонецЕсли;
	Если Начисление = Неопределено Тогда
		Попытка
			Если (КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.РайонныйКоэффициент
				...
				И КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.РайонныеСеверныеНадбавкиАУСН Тогда
				
				СоздатьНачисленияРКиСНАУСН();
			ИначеЕсли КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.НадбавкаЗаВредность
				И КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.ОплатаТрудаАУСН Тогда
				
				СоздатьНачислениеНадбавкаЗаВредностьАУСН();
			КонецЕсли;
		Исключение
			ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации("", УровеньЖурналаРегистрации.Информация, , ,
				НСтр("ru = 'Не удалось выделить начисления, редактируемые в отдельных полях'"));
		КонецПопытки;
	КонецЕсли;
