...
Процедура ОбработатьИзменениеКатегорииНачисленияНаСервере()
	...
	ЗаполнитьСвойстваПоКатегории(ЭтаФорма);
	ЗаполнитьВидВремени();
	ЗаполнитьРедактированиеВОтдельныхПолях();
...
Процедура ЗаполнитьСвойстваПоКатегории(Форма)
	...
	Объект = Форма.Объект;
	РедактируетсяВОтдельныхПолях = Объект.РедактируетсяВОтдельныхПолях;
	...
	Объект.РедактируетсяВОтдельныхПолях = РедактируетсяВОтдельныхПолях;
...
&НаСервере
Процедура ЗаполнитьРедактированиеВОтдельныхПолях()
	
	Если Объект.РедактируетсяВОтдельныхПолях Тогда
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени,РедактируетсяВОтдельныхПолях",
		Объект.КатегорияНачисленияИлиНеоплаченногоВремени, Истина);
		Начисления = ПланыВидовРасчета.Начисления.НачисленияПоОтбору(Отбор);
		Если (Начисления.Количество() = 0)
			ИЛИ (Начисления.Количество() = 1 И Объект.Ссылка = Начисления[0])Тогда
			Объект.РедактируетсяВОтдельныхПолях = Истина;
		Иначе	
			Объект.РедактируетсяВОтдельныхПолях = Ложь;
		КонецЕсли;
	КонецЕсли;
