...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПланСхема = ТерриторииИПомещения.ПолучитьНавигационнуюСсылкуРеквизита(
	...
	Если ЗначениеЗаполнено(Объект.Родитель) Тогда
		РодительТекст = ТерриторииИПомещенияВызовСервера.ПолучитьПолныйПутьКПомещению(Объект.Родитель);
	КонецЕсли;
	
	ИспользоватьБронированиеПомещений = ТерриторииИПомещения.ИспользоватьБронированиеПомещений();
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если  ПланСхема <> ПланСхемаПриЗагрузке Тогда 
		Если ЗначениеЗаполнено(ПланСхема) Тогда 
			ТерриторииИПомещения.ЗаписатьИзображениеТерритории(Объект.Ссылка, ПланСхема, УникальныйИдентификатор);
		ИначеЕсли Не Объект.Ссылка.Пустая() Тогда 
			ТерриторииИПомещения.ОчиститьИзображениеТерритории(Объект.Ссылка, УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПланСхемаНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		ТерриторииИПомещенияКлиент.ПоказатьКартинку(ПараметрыПоказа);
...
Процедура ЗагрузитьИзображениеНаКлиентеПродолжение(Результат, Параметры)Экспорт 
	...
	ТерриторииИПомещенияКлиент.ВыбратьКартинкуИПоместитьВХранилище(
...
Функция СформироватьДанныеВыбораРодителя(Текст, ТекущееМесто)
	...
	Пока Выборка.Следующий() Цикл
		ПланТекст = ТерриторииИПомещенияВызовСервера.ПолучитьПолныйПутьКПомещению(Выборка.Ссылка);
