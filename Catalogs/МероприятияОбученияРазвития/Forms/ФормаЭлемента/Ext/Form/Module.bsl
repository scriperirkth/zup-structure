...
Процедура СоздатьПубликацию(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		ТекстСообщения = НСтр("ru = 'Перед началом добавления публикаций, необходимо записать мероприятие. Записать?'");
		Оповещение = Новый ОписаниеОповещения("ОбработкаОповещенияОЗаписиПриСозданииПубликации", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстСообщения, РежимДиалогаВопрос.ДаНет);
	Иначе
		ОткрытьФормуПубликацииМероприятия();
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаОповещенияОЗаписиПриСозданииПубликации(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуПубликацииМероприятия() Экспорт 
