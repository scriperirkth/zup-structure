...
&НаКлиенте
Процедура СсылкаНаQRКодНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПерейтиПоНавигационнойСсылке(Объект.СсылкаНаQRКод);
...
Процедура ОпределитьВидДокумента(КонтекстЭДОСервер)
	...
	ЭтоКвитанцияОПрисвоенииСтатусаДОПП = (Объект.ВидДокумента = Перечисления.ВидыНалоговыхДокументов.КвитанцияОПрисвоенииСтатусаДОПП);
...
Процедура УправлениеЭУ()
	...
	Элементы.ГруппаКвитанцияОПрисвоенииСтатусаДОПП.Видимость = ЭтоКвитанцияОПрисвоенииСтатусаДОПП;
	Элементы.СсылкаНаQRКод.Видимость = ЗначениеЗаполнено(Объект.СсылкаНаQRКод) И СтрДлина(Объект.СсылкаНаQRКод) <= 1024;
	...
	Элементы.ОшибкиНСП.Видимость = Объект.ОшибкиНСП.Количество() <> 0;
	Элементы.ГруппаРеквизитыНалогоплательщикаЮЛ.Видимость = ЗначениеЗаполнено(Объект.НаименованиеСобственника);
	Элементы.ГруппаРеквизитыНалогоплательщикаФЛ.Видимость = ЗначениеЗаполнено(Объект.ФИОСобственника);
	Элементы.ОшибкиКО.Видимость = Объект.ОшибкиНСП.Количество() <> 0;
	
	Если ЭтоКвитанцияОПрисвоенииСтатусаДОПП Тогда
		ТекстСтатусаДОПП = ИнтерфейсыВзаимодействияБРОВызовСервера.СтатусыДОПП(Объект.СтатусДОПП);
	КонецЕсли;
