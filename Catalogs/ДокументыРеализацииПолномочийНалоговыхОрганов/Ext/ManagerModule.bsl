...
Функция НаименованиеДокументаРеализацииПолномочий(Знач Требование, ПриложенияТребования = Неопределено) Экспорт
	...
	ИначеЕсли Требование.ВидДокумента = Перечисления.ВидыНалоговыхДокументов.КвитанцияОПрисвоенииСтатусаДОПП Тогда
		
		ПредставлениеНомераИлиДаты = ?(ЗначениеЗаполнено(Требование.НомерУведомления),
			НСтр("ru = '№'") + Требование.НомерУведомления, Формат(Требование.ДатаУведомления, "ДЛФ=Д"));
		
		Наименование = СтрШаблон(
			НСтр("ru = 'Квитанция о присвоении статуса ДОПП%1 от %2'"),
			?(ПредставлениеНомераИлиДаты = "", "", " " + ПредставлениеНомераИлиДаты),
			Дата);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОбновитьДокументыРеализацииПолномочийНалоговыхОрганов");
КонецПроцедуры
