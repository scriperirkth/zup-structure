...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		УстановитьДоступностьЭлементов(ЭтотОбъект, Объект);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	УстановитьДоступностьЭлементов(ЭтотОбъект, ТекущийОбъект);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ГосударственнаяСлужба");
		Модуль.ПроверитьЗаписьДолжностиВФорме(ЭтотОбъект, Отказ);
	КонецЕсли;
...
Процедура Подключаемый_СвойстваДолжностейГосударственнойСлужбыПриИзменении(Элемент)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль.ДолжностьФормаСвойстваДолжностейГосударственнойСлужбыПриИзменении(Элемент, ЭтотОбъект);
	КонецЕсли;
...
Процедура Подключаемый_СвойстваДолжностейКодНаименованияДолжностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль.СвойстваДолжностейКодНаименованияДолжностиНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора);
...
&НаКлиенте
Процедура Подключаемый_СсылкаНаОбработкуПоУстановкеКодовДолжностейНажатие(Элемент)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ГосударственнаяСлужбаКлиент");
		Модуль.ОткрытьОбработкуПоУстановкеКодовДолжностей();
...
Процедура Подключаемый_СвойстваДолжностейКодНаименованияДолжностиНачалоВыбораЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ГосударственнаяСлужбаКлиентСервер");
		Модуль.ДолжностьФормаСвойстваДолжностейГосударственнойСлужбыПриИзменении(ДополнительныеПараметры.Элемент, ЭтотОбъект);
...
Процедура ПрочитатьДанныеТрудовойФункции()
	...
	ТекстПодсказки = НСтр("ru = 'Не выбрана трудовая функция'");
	Если ЗначениеЗаполнено(Объект.ТрудоваяФункция) Тогда
		ТекстыПодсказки = Новый Массив;
		РеквизитыТрудовойФункции = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Объект.ТрудоваяФункция,
			"ОКПДТРКод,
			|ОКПДТРКЧ,
			|ОКПДТРКатегория,
			|КодПоОКЗ,
		...
		Если ЗначениеЗаполнено(РеквизитыТрудовойФункции.ОКПДТР2026Код) Тогда
			ТекстыПодсказки.Добавить(СтрШаблон(НСтр("ru = 'Код по ОКПДТР: %1'"), РеквизитыТрудовойФункции.ОКПДТР2026Код));
			Если ЗначениеЗаполнено(РеквизитыТрудовойФункции.ОКПДТР2026Категория) Тогда
				ТекстыПодсказки.Добавить(СтрШаблон(НСтр("ru = 'Категория: %1'"), РеквизитыТрудовойФункции.ОКПДТР2026Категория));
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекстыПодсказки) Тогда
			ТекстПодсказки = СтрШаблон(
				НСтр("ru = 'С 2026 действуют сведения ОКПДТР: %1'"), СтрСоединить(ТекстыПодсказки, ", "));
		Иначе
			ТекстПодсказки = НСтр("ru = 'Не выбран код классификатора ОКПДТР 2026 года'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(РеквизитыТрудовойФункции.КодПоОКЗ2026) Тогда
			ТекстПодсказки = СтрШаблон(НСтр("ru = '%1, Код ОКЗ: %2'"), ТекстПодсказки, РеквизитыТрудовойФункции.КодПоОКЗ2026);
		КонецЕсли;
		
	КонецЕсли;
	
	ЗарплатаКадрыКлиентСервер.УстановитьРасширеннуюПодсказкуЭлементуФормы(
		ЭтотОбъект, "ТрудоваяФункция", ТекстПодсказки);
