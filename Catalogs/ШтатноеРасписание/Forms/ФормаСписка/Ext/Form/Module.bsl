...
Процедура ОбновитьНастройкиФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"ФормаВидИерархия",
		"Видимость",
		Не НастройкиОтборовСписка.УстановленОтборПоПодразделению);
...
Процедура УстановитьИспользованиеОтбораПоОрганизации()
    ...
    ПолеВладелец = Список.Поля.Найти("ПолеВладелец");
		
	Если ПолеВладелец = Неопределено Тогда
		ПолеВладелец = Список.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
		ПолеВладелец.Поле = "ПолеВладелец";
		ПолеВладелец.ПутьКДанным = "ПолеВладелец";
	КонецЕсли;
