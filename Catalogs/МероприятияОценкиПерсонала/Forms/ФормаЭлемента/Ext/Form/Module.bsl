...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьХарактеристикиПерсонала" Тогда
...
Процедура ОткрытыеВопросыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ЗначениеЗаполнено(Элемент.ТекущиеДанные.Формулировка) Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если НоваяСтрока Тогда
		...
		Элемент.ТекущиеДанные.Наименование = Элемент.ТекущиеДанные.Формулировка;
...
Процедура ОценивающиеИндивидуальныеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	ОценивающиеПередНачаломДобавленияНаКлиенте(Элемент, Отказ);
КонецПроцедуры
...
Процедура СформироватьОтчетКорпоративныеРезультаты(Команда)
	...
	ОткрытьФорму("Отчет.ОценкаПерсонала.Форма", ПараметрыОтчета,, Новый УникальныйИдентификатор());
...
Функция ЗаписатьВариантОтветаНаСервере(ОткрытыйВопрос)
	...
	Иначе
		...
		Запрос.Текст = 
			...
			|	И ВопросыДляАнкетирования.Формулировка ПОДОБНО &Формулировка
		...
		Запрос.УстановитьПараметр("Формулировка", ОткрытыйВопрос.Формулировка);
	...
	ВопросОбъект.Формулировка = ОткрытыйВопрос.Формулировка;
...
Процедура ЗаписатьВопросыШаблонаАнкеты(ВопросыДляАнкеты)
	...
	Для каждого ВопросДляАнкеты Из ВопросыДляАнкеты Цикл
		...
		Если ВопросАнкетыОбъект.Пустая() Тогда
			...
			ЗаполнитьЗначенияСвойств(ВопросАнкетыОбъект,ВопросДляАнкеты.Вопрос,, "Код");
...
&НаКлиенте
Процедура ОценивающиеПередНачаломДобавленияНаКлиенте(Элемент, Отказ)
...
Процедура ПодготовитьТекстыПриглашенийИОтправитьАдресатам(МассивСтруктурРеспондентов)
	...
	Для Каждого ПриглашаемыйРеспондент Из МассивСтруктурРеспондентов Цикл
		...
		Если СтрНайти(ШаблонСообщенияТекст, "ВебСсылкаАнкетирования") > 0 Тогда	
...
Процедура ОтправитьРезультатыСервер(ПараметрыОтправки)
	...
	ПараметрыДоставки.Вставить("ВставлятьОтчетыВТекстПисьма", Ложь);
	ПараметрыДоставки.Вставить("ПрикреплятьОтчетыВоВложения", Истина);
...
Функция НовыйШаблонПриглашения(НаименованиеШаблона, ВладелецШаблона)
	...
	ПараметрыШаблона.ВладелецШаблона = ВладелецШаблона;
...
Процедура ЗаполнитьХарактеристикиИВопросы(ЗначениеКопирования = Неопределено)
	...
	Запрос.Текст = 
		...
		|	И ХарактеристикиПерсонала.ИспользоватьОценку360 = ИСТИНА
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ВопросыОценкиХарактеристикПерсонала.ХарактеристикаПерсонала,
		|		ВыбранныеВопросыМероприятия.ХарактеристикаПерсонала) КАК ХарактеристикаПерсонала,
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МероприятияОценкиПерсоналаВопросы.Вопрос КАК Вопрос,
		|	МероприятияОценкиПерсоналаВопросы.Вопрос.Формулировка КАК Формулировка,
...
Процедура ОбновитьСписокХарактеристик()
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ТекущиеХарактеристики.ХарактеристикаПерсонала,
		|	ТекущиеХарактеристики.Включена
		|ПОМЕСТИТЬ ВТВыбранныеХарактеристики
		|ИЗ
		|	&ВыбранныеХарактеристики КАК ТекущиеХарактеристики
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ХарактеристикиПерсонала.Ссылка КАК ХарактеристикаПерсонала,
		|	ЕСТЬNULL(ВТВыбранныеХарактеристики.Включена, ЛОЖЬ) КАК Включена
		|ИЗ
		|	ПланВидовХарактеристик.ХарактеристикиПерсонала КАК ХарактеристикиПерсонала
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВыбранныеХарактеристики КАК ВТВыбранныеХарактеристики
		|		ПО ХарактеристикиПерсонала.Ссылка = ВТВыбранныеХарактеристики.ХарактеристикаПерсонала
		|ГДЕ
		|	ХарактеристикиПерсонала.ВидХарактеристики = &ВидХарактеристикиЛичныеКачества
		|	И ХарактеристикиПерсонала.ИспользоватьОценку360 = ИСТИНА";
		
	Запрос.УстановитьПараметр("ВидХарактеристикиЛичныеКачества", ПредопределенноеЗначение("Перечисление.ВидыХарактеристикПерсонала.ЛичноеКачество"));
	Запрос.УстановитьПараметр("ВыбранныеХарактеристики", ХарактеристикиПерсонала.Выгрузить());
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ХарактеристикиПерсонала.Очистить();
	ХарактеристикиПерсонала.Загрузить(РезультатЗапроса.Выгрузить());
