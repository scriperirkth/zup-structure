...
Процедура ПолучателиУведомленийШаблонСообщенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаУведомлений.SMS") Тогда
		ВидСообщения = "СообщениеSMS";
	ИначеЕсли ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаУведомлений.Email") Тогда
		ВидСообщения = "Письмо";
	КонецЕсли;
...
Процедура ПолучателиУведомленийШаблонСообщенияСоздание(Элемент, СтандартнаяОбработка)
	...
	ВидСообщения = ?(ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаУведомлений.SMS"), "СообщениеSMS", "Письмо");
...
Процедура ВыбратьИзСпискаЭлементПоТипу(ИмяТипа)
	...
	ИначеЕсли ИмяТипа = "Справочник.ВнешниеПользователи" Тогда
		ОткрытьФорму("Справочник.ВнешниеПользователи.ФормаВыбора", СтруктураПараметров, Элементы.ПолучателиУведомлений.ПодчиненныеЭлементы.ПолучателиУведомленийПолучатель);
	КонецЕсли;
...
Функция ПроверкаЗаполнения()
	...
	Для Каждого Строка Из ПолучателиУведомлений Цикл
		Если Не ЗначениеЗаполнено(Строка.Получатель) Тогда
			...
			Отказ = Истина;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Строка.ВидКонтактнойИнформации)
			И ВидТранспорта <> ПредопределенноеЗначение("Перечисление.ВидыТранспортаУведомлений.Обсуждения") Тогда
			Сообщение = Новый СообщениеПользователю;
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьЭлементУсловногоОформления("Справочник.Пользователи", "Пользователь");
	УстановитьВидимостьКонтактнойИнформации();
	
	Если ВидТранспорта <> Перечисления.ВидыТранспортаУведомлений.Обсуждения Тогда
		УстановитьЭлементУсловногоОформления("Справочник.ФизическиеЛица", НСтр("ru='Физическое лицо'"));
		УдалитьЭлементСпискаВыбора("Справочник.ВнешниеПользователи");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьЭлементУсловногоОформления(ТипПолучателя, Текст)
	
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементУсловногоОформления.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементУсловногоОформления.Отбор, "ПолучателиУведомлений.ТипПолучателя", ТипПолучателя, ВидСравненияКомпоновкиДанных.Равно);
...
&НаСервере
Процедура УдалитьЭлементСпискаВыбора(Значение)
	
	ЭлементКУдалению = Элементы.ПолучателиУведомленийТипПолучателя.СписокВыбора.НайтиПоЗначению(Значение);
	Если ЭлементКУдалению <> Неопределено Тогда
		Элементы.ПолучателиУведомленийТипПолучателя.СписокВыбора.Удалить(ЭлементКУдалению);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьКонтактнойИнформации()
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ПолучателиУведомленийВидКонтактнойИнформации",
		"Видимость",
		ВидТранспорта <> Перечисления.ВидыТранспортаУведомлений.Обсуждения);
