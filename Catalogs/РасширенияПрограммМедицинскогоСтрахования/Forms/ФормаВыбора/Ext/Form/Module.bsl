...
Процедура ЗадатьВопросОВыбореПодобранного()
	...
	ПоказатьВопрос(Оповещение, НСтр("ru='Выполнить выбор отобранных Расширений медицинского страхования?'"), РежимДиалогаВопрос.ДаНетОтмена,);
...
Процедура ПодтвердитьВыборИЗакрыть()
	...
	Оповестить("РасширенияСтрахованияОбработкаВыбораЗавершение", СписокПодобранных.ВыгрузитьЗначения());
	СписокПодобранных.Очистить();
	Закрыть();
