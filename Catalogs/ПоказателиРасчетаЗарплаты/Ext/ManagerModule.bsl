...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СоздатьПоказательИсчисленныйНДФЛРасчетнойБазыУдержания");
	Методы.Вставить("СоздатьПоказательУчитыватьМРОТ");
	Методы.Вставить("СоздатьПоказателиИндексируемогоНеиндексируемогоСреднегоЗаработка");
	Методы.Вставить("СоздатьПоказательСтоимостьДняЧасаСУчетомРазовыхНачислений");
	Методы.Вставить("СоздатьПоказателиСреднегоЗаработкаРКиСН");
	Методы.Вставить("СоздатьПоказательКалендарныеДниМесяцаБезПраздников");
	Методы.Вставить("УстановитьТочностьПоказателейРасчетнаяБаза");
	Методы.Вставить("СоздатьПоказательНормаДнейЧасов");
	Методы.Вставить("СоздатьПоказательСреднемесячноеКоличествоЧасов");
	Методы.Вставить("СоздатьПоказательКоличествоДнейЧасовКомпенсацииОтгулов");
КонецПроцедуры
...
Процедура СоздатьПоказателиПоНастройкам(НастройкиРасчетаЗарплаты = Неопределено, ПараметрыПланаВидовРасчета = Неопределено, ТекущиеНастройкиРасчетаЗарплаты = Неопределено) Экспорт
	...
	СоздатьПоказательИсчисленныйНДФЛРасчетнойБазыУдержания();
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Наставничество") Тогда 
		...
		Модуль.СоздатьПоказателиДоплатыЗаНаставничество(НастройкиРасчетаЗарплаты, ТекущиеНастройкиРасчетаЗарплаты);
	КонецЕсли;
...
Функция НулевыеЗначенияПоказателейРКСН() Экспорт
		
	НулевыеЗначенияПоказателейРКСН = Новый Структура;
	
	НулевыеЗначенияПоказателейРКСН.Вставить("СреднийЗаработокРК", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СреднийЗаработокИндексируемыйРК", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СреднийЗаработокНеиндексируемыйРК", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СреднийЗаработокСН", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СреднийЗаработокИндексируемыйСН", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СреднийЗаработокНеиндексируемыйСН", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СохраняемоеДенежноеДовольствиеРК", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СохраняемоеДенежноеДовольствиеСН", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СохраняемоеДенежноеСодержаниеРК", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СохраняемоеДенежноеСодержаниеСН", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СохраняемоеДенежноеСодержаниеНеИндексируемоеРК", 0);
	НулевыеЗначенияПоказателейРКСН.Вставить("СохраняемоеДенежноеСодержаниеНеИндексируемоеСН", 0);
	
	Возврат НулевыеЗначенияПоказателейРКСН;
...
Процедура СоздатьПоказательИсчисленныйНДФЛРасчетнойБазыУдержания(ПараметрыОбновления = Неопределено) Экспорт

	ОписаниеПоказателя = ОписаниеПоказателя();
	ОписаниеПоказателя.Идентификатор = "ИсчисленныйНДФЛРасчетнойБазыУдержания";
	ОписаниеПоказателя.Наименование = НСтр("ru = 'Исчисленный НДФЛ расчетной базы удержания'");
	ОписаниеПоказателя.КраткоеНаименование = НСтр("ru = 'НДФЛ'");
	ЗаписатьПоказатель(ОписаниеПоказателя);	
