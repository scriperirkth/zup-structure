...
Процедура ДеревоГруппВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИдентификаторыСтрокВыбранныхГрупп = Новый Массив;
	Если ТипЗнч(ВыбраннаяСтрока) = Тип("Массив") Тогда
		ИдентификаторыСтрокВыбранныхГрупп = ВыбраннаяСтрока;
	Иначе
		ИдентификаторыСтрокВыбранныхГрупп = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыбраннаяСтрока);
	КонецЕсли;
	ВыбранныеГруппы = Новый Массив;
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрокВыбранныхГрупп Цикл
		ТекущиеДанные = ДеревоГрупп.НайтиПоИдентификатору(ИдентификаторСтроки);
		Если ТекущиеДанные <> Неопределено Тогда
			ВыбранныеГруппы.Добавить(ТекущиеДанные.Ссылка);
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ВыбранныеГруппы) Тогда
		СтандартнаяОбработка = Ложь;
		ОповеститьОВыборе(ВыбранныеГруппы);
	КонецЕсли;
