...
Функция МаксимальныйРазмерВложений(ТипКО) Экспорт
	...
	Если ТипКО = Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСФНС Тогда
		...
		Если ПисьмаВГосОрганыВызовСервера.СобытиеНаступилоПисьмаФНС1Мб() Тогда
			Результат.РазмерКонтейнера = 1;
			Результат.РазмерФайла = 1;
		Иначе
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("РазделитьПисьма", Истина);
КонецПроцедуры
