...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОписатьВидОтпускаСеверныйОтпуск");
	Методы.Вставить("ИзменитьПредставлениеОтпускПострадавшимВАварииЧАЭС");
	Методы.Вставить("УточнитьНаименованияВидовУчебныхОтпусков");
	Методы.Вставить("ОписатьВидОтпускаДополнительныйОтпускПострадавшимВАварииЧАЭСОплачиваемый");
	Методы.Вставить("ОписатьВидОтпускаОсновнойОтпуск");
КонецПроцедуры
...
Функция СвойстваПредопределенныхВидовОтпусков() Экспорт
	...
	ОписаниеВидаОтпуска.Наименование				= НСтр("ru = 'Дополнительный отпуск без сохранения среднего заработка'");
	ОписаниеВидаОтпуска.НаименованиеПолное			= НСтр("ru = 'Дополнительный отпуск без сохранения среднего заработка'");
	ОписаниеВидаОтпуска.СпособРасчетаОтпуска 		= Перечисления.СпособыРасчетаОтпуска.ВКалендарныхДнях;
	ОписаниеВидаОтпуска.ОтпускБезОплаты				= Истина;
	...
	ОписаниеВидаОтпуска.Наименование				= НСтр("ru = 'Дополнительный отпуск с сохранением среднего заработка'");
	ОписаниеВидаОтпуска.НаименованиеПолное			= НСтр("ru = 'Дополнительный отпуск с сохранением среднего заработка'");
	ОписаниеВидаОтпуска.СпособРасчетаОтпуска 		= Перечисления.СпособыРасчетаОтпуска.ВКалендарныхДнях;
	ОписаниеВидаОтпуска.ОтпускБезОплаты				= Ложь;
	ОписаниеВидаОтпуска.ОтпускЯвляетсяЕжегодным		= Ложь;
...
Процедура УточнитьНаименованияВидовУчебныхОтпусков(ПараметрыОбновления = Неопределено) Экспорт  
	
    Запрос = Новый Запрос;
	Запрос.Текст = 
	 "ВЫБРАТЬ
	 |	ВидыОтпусков.Ссылка КАК ВидОтпускаСсылка,
	 |	СОКРЛП(ВидыОтпусков.Наименование) КАК Наименование
	 |ИЗ
	 |	Справочник.ВидыОтпусков КАК ВидыОтпусков
	 |ГДЕ
	 |	ВидыОтпусков.Наименование ПОДОБНО ""%Дополнительный учебный отпуск%""";
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		ВидОтпускаОбъект = Выборка.ВидОтпускаСсылка.ПолучитьОбъект();
		Если СтрНайти(Выборка.Наименование, "Дополнительный учебный отпуск (оплачиваемый)") > 0 Тогда
			ВидОтпускаОбъект.Наименование 		= СтрЗаменить(ВидОтпускаОбъект.Наименование, "Дополнительный учебный отпуск (оплачиваемый)", "Дополнительный отпуск с сохранением среднего заработка");
			ВидОтпускаОбъект.НаименованиеПолное = СтрЗаменить(ВидОтпускаОбъект.НаименованиеПолное, "Дополнительный учебный отпуск (оплачиваемый)", "Дополнительный отпуск с сохранением среднего заработка");
		ИначеЕсли СтрНайти(Выборка.Наименование, "Дополнительный учебный отпуск без оплаты") > 0 Тогда
			ВидОтпускаОбъект.Наименование 		= СтрЗаменить(ВидОтпускаОбъект.Наименование, "Дополнительный учебный отпуск без оплаты", "Дополнительный отпуск без сохранения среднего заработка");
			ВидОтпускаОбъект.НаименованиеПолное = СтрЗаменить(ВидОтпускаОбъект.НаименованиеПолное, "Дополнительный учебный отпуск без оплаты", "Дополнительный отпуск без сохранения среднего заработка");
		КонецЕсли;
		
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВидОтпускаОбъект); 
