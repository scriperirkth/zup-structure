...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтправитьОбратнуюСвязьПоОтклику И Не ЗначениеЗаполнено(ОбратнаяСвязьПоОткликуСообщение) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Не указано сообщение кандидату.'"), , , "ОбратнаяСвязьПоОткликуСообщение", Отказ);
	КонецЕсли;
...
Процедура НаписатьПисьмоКандидатамОбОтказеПриИзменении(Элемент)
	УстановитьДоступностьШаблонаПисьмаОбОтказе(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьSMSКандидатамОбОтказеПриИзменении(Элемент)
	УстановитьДоступностьШаблонаSMSОбОтказе(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура НаписатьПисьмоКандидатамОбОдобренииПриИзменении(Элемент)
	УстановитьДоступностьШаблонаПисьмаОбОдобрении(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьSMSКандидатамОбОдобренииПриИзменении(Элемент)
	УстановитьДоступностьШаблонаSMSОбОдобрении(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура ЗакрыватьВакансиюПриИзменении(Элемент)
	УстановитьДоступностьОтклоненияДругихКандидатов(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура ОтклонитьДругихКандидатовПриИзменении(Элемент)
	УстановитьДоступностьДругихКандидатов(ЭтотОбъект);
КонецПроцедуры
...
&НаКлиенте
Процедура ОтправитьОбратнуюСвязьПоОткликуПриИзменении(Элемент)
	Элементы.ОбратнаяСвязьПоОткликуСообщение.Доступность = ОтправитьОбратнуюСвязьПоОтклику;
КонецПроцедуры
...
Процедура НаписатьПисьмо(Команда)
	...
	Если Не ИспользоватьШаблоныСообщений Тогда
		ВзаимодействияКлиент.СоздатьВзаимодействиеИлиПредмет("Документ.ЭлектронноеПисьмоИсходящее.ФормаОбъекта", Кандидат, ЭтотОбъект);
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьSMS(Команда)
	...
	Если Не ИспользоватьШаблоныСообщений Тогда
		ВзаимодействияКлиент.СоздатьВзаимодействиеИлиПредмет("Документ.СообщениеSMS.ФормаОбъекта", Кандидат, ЭтотОбъект);
		Возврат;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормы(ПараметрыФормы)
	...
	Элементы.ОбратнаяСвязьПоОткликуГруппа.Видимость = Ложь;
...
Процедура НастроитьЭлементыФормыДляСогласования(ПараметрыФормы)
	...
	ПараметрыПолучения.Согласование = ПолучитьФункциональнуюОпцию("ИспользоватьСогласованиеВакансий");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСогласованиеВакансий") Тогда
		ДанныеСогласования = ДанныеВакансии.Согласование;
	...
	Если ДанныеВакансии.ТекущееСостояние.Состояние = Перечисления.СостоянияВакансии.Закрыта Тогда
		ЗакрыватьВакансию = Ложь;
		Элементы.ЗакрыватьВакансию.Доступность = Ложь;
		Элементы.ВакансияРасширеннаяПодсказка.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = 'Закрыта %1'"), Формат(Данныевакансии.ТекущееСостояние.Период, "ДЛФ=D;")));
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыДляОтклонения(ПараметрыФормы)
	...
	Элементы.ОбратнаяСвязьПоОткликуГруппа.Видимость =
		ДанныеКандидата.ТекущийЭтап = Перечисления.ЭтапыРаботыСКандидатами.РассмотрениеОтклика
		И РекрутинговыеСайты.ДоступнаОтправкаОбратнойСвязиПоОтклику(Кандидат);
...
Функция РезультатВыбора()
	...
	Если ОтправитьОбратнуюСвязьПоОтклику Тогда
		Результат.ОбратнаяСвязьПоОтклику.Отправить = Истина;
		Результат.ОбратнаяСвязьПоОтклику.Отклонить = Истина;
		Результат.ОбратнаяСвязьПоОтклику.Сообщение = ОбратнаяСвязьПоОткликуСообщение;
	КонецЕсли;
