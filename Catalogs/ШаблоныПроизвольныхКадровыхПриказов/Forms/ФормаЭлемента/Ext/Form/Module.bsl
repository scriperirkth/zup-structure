...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Пока ВыборкаРеквизиты.Следующий() Цикл
		...
		Если ОписаниеРеквизитаНаФорме.ТипЗначения <> Реквизит.ТипЗначения Тогда
			...
			ТекстОшибки = ТекстОшибки + НСтр("ru = 'используется реквизит:'") + Реквизит
				+ НСтр("ru = '. Изменение типа значения невозможно.'");
	...
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "НесколькоСотрудников") <> Объект.НесколькоСотрудников 
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПараметрыОповещения.Вставить("Наименование", Объект.Наименование);
	ПараметрыОповещения.Вставить("Представление", Объект.Наименование);
...
Процедура РеквизитыДокументаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	УстановитьПустыеХарактеристики(Элемент, НоваяСтрока, Копирование);
...
Процедура ПечатныеФормы(Команда)
	...
	ОткрытьФорму("Справочник.ШаблоныПроизвольныхКадровыхПриказов.Форма.ПечатныеФормы", ПараметрыФормы, ЭтотОбъект, , , , 
...
&НаКлиенте
Процедура УстановитьПустыеХарактеристики(Элемент, НоваяСтрока, Копирование)
	
	Если НоваяСтрока И Копирование Тогда
		Элемент.ТекущиеДанные.Ссылка =
			ПредопределенноеЗначение("ПланВидовХарактеристик.РеквизитыПроизвольныхКадровыхПриказов.ПустаяСсылка");
	КонецЕсли;
