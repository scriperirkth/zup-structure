...
Процедура ПрочитатьПараметры()
	...
	ПоСотрудникам = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПоСотрудникам", Ложь);
...
Процедура УстановитьОтборПоОтветственному()
	...
	Если ФильтрПоОтветственному Тогда
		...
		Если ПоСотрудникам Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивПоказателей,
			КлючевыеПоказателиЭффективности.СписокПоказателейСотрудниковПоОтветственным(Период, КоллекцияОтборов));
