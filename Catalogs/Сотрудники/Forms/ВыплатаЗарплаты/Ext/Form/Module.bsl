		...
		Если МестоВыплаты.Вид = Перечисления.ВидыМестВыплатыЗарплаты.ЗарплатныйПроект
			...
			Если Не ЗначениеЗаполнено(ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.ДокументОснование) Тогда
				...
				Если ОбменСБанкамиПоЗарплатнымПроектамФормы.ЗаписьЛицевыеСчетаСотрудниковПоЗарплатнымПроектамИзменена(ЭтотОбъект) Тогда
					...
					ИначеЕсли ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР
						И Не ЗначениеЗаполнено(ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.НомерЛицевогоСчетаЦР) Тогда
						
						ТекстСообщения = НСтр("ru='У сотрудника не указан счет ЦР'");
						ОбщегоНазначения.СообщитьПользователю(
							ТекстСообщения,,,, Отказ);
...
&НаКлиенте
Процедура ЛицевойСчетЦРПриИзменении(Элемент)
	ИзменениеСпособаЗачисление();
...
Процедура СпособыЗачисленияНастроитьПоля()
	...
	Элементы.СпособПоСчетуЦР.СписокВыбора[0].Представление 
...
Процедура ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамУстановитьОтображение(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ГруппаПоЛицевомуСчетуЦР",
		"Видимость",
		Форма.ПоддерживаетсяЗачислениеПоНомеруЛицевогоСчетаЦР);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ГруппаПоЛицевомуСчету",
		"Видимость",
		Форма.ПоддерживаетсяЗачислениеПоНомеруЛицевогоСчета);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ЛицевыеСчетаСотрудниковПоЗарплатнымПроектамНомерЛицевогоСчета",
		"ПоложениеЗаголовка",
		?(Форма.ЗарплатныйПроектПоддерживаетСпособыЗачисленияВыплат Или Не Форма.ПоддерживаетсяЗачислениеПоНомеруЛицевогоСчета, 
...
Процедура СпособыЗачисленияУстановитьДоступность(Форма) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"ЛицевойСчетЦР",
		"Доступность",
		Форма.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления = ПредопределенноеЗначение("Перечисление.СпособыЗачисленияВыплат.ПоСчетуЦР"));
...
Процедура УстановитьЗаголовокЗачисления(Форма)
	...
	ИначеЕсли Форма.СпособЗачисления =
		ПредопределенноеЗначение("Перечисление.СпособыЗачисленияВыплат.ПоСчетуЦР") Тогда
			Форма.Элементы.ГруппаСпособыЗачисления.ЗаголовокСвернутогоОтображения = 
				СтрШаблон(НСтр("ru ='Зачислять по лицевому счету ЦР %1'"), 
					Форма.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.НомерЛицевогоСчетаЦР);
	КонецЕсли;
...
Процедура ЗаполнитьДоступностьСпособовЗачисления()
	...
	ОбменСБанкамиПоЗарплатнымПроектам.УстановитьДоступностьСпособовЗачисленияПоЗарплатномуПроекту(
		...
		Если СпособыЗачисления[ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления] = Ложь 
			Или СпособыЗачисления[ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления] = Неопределено Тогда
			Если СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету] = Истина Тогда
				ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления = Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету; 
			Иначе
				Для Каждого СпособЗачисленияЗарплатногоПроекта Из СпособыЗачисления Цикл
					Если СпособЗачисленияЗарплатногоПроекта.Значение = Истина Тогда
						ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СпособЗачисления = СпособЗачисленияЗарплатногоПроекта.Ключ;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	ПоддерживаетсяЗачислениеПоНомеруЛицевогоСчетаЦР		= СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСчетуЦР];
	ПоддерживаетсяЗачислениеПоНомеруЛицевогоСчета		= СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету];
КонецПроцедуры
