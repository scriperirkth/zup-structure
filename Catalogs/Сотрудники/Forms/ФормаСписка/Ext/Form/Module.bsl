...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПараметрыОтбораПоПериоду.ОтбиратьРаботающихВПериоде Тогда
		УстановитьОтборПоПериодуРаботы(ПараметрыОтбораПоПериоду.ПериодРаботы);
	Иначе
...
Процедура УстановитьОтборПоДате()
	...
	УстановитьПараметрыДатДинамическихСписков(ДатаНачала, ДатаОкончания);

КонецПроцедуры

&НаСервере
Процедура УстановитьОтборПоПериодуРаботы(ПериодРаботы)

	Если ЗначениеЗаполнено(ПериодРаботы.ДатаНачала) Тогда
		ДатаНачала = НачалоДня(ПериодРаботы.ДатаНачала);
	Иначе
		ДатаНачала = '00010101';
	КонецЕсли;

	Если ЗначениеЗаполнено(ПериодРаботы.ДатаОкончания) Тогда
		ДатаОкончания = КонецДня(ПериодРаботы.ДатаОкончания);
	Иначе
		ДатаОкончания = КонецДня(ОбщегоНазначения.ТекущаяДатаПользователя());
	КонецЕсли;
...
&НаСервере
Процедура УстановитьПараметрыДатДинамическихСписков(ДатаНачала, ДатаОкончания)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ОрганизационнаяСтруктура") Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
			ЭтотОбъект["ОрганизационнаяСтруктура"], "ДатаОкончания", ДатаОкончания, Истина);
	КонецЕсли;
