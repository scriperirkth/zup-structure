...
Процедура ПрочитатьДанные()
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	ЕСТЬNULL(ДоходыПредыдущегоМестаРаботыНДФЛ.Размер, 0) КАК Доход,
	...
	Выборка = Запрос.Выполнить().Выбрать();
	ДоходЗаПредыдущийМесяц = 0;
	Пока Выборка.Следующий() Цикл
		Год                 	= Год;
		СтрокаДоходов       	= Доходы.Добавить();
		СтрокаДоходов.Месяц 	= Выборка.Месяц;
		СтрокаДоходов.Сумма 	= Выборка.Доход - ДоходЗаПредыдущийМесяц;
		ДоходЗаПредыдущийМесяц 	= Выборка.Доход;
	КонецЦикла;
