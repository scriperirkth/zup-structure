	...
	Попытка
		...
		Иначе
			...
			ДоступенФункционалСинхронизация =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.3.2.3.CORP");
			ДоступенФункционалКонтрагенты =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("1.3.2.3", Истина);
			ДоступенФункционалШаблоны =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("2.0.8.1", Истина);
			ДоступенФункционалАвтоматическогоСозданияСвязей =
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ДоступенФункционалВерсииСервиса("2.1.0.1", Истина);
			...
			Элементы.НеСоздаватьСвязиПоСсылкам.Видимость = ДоступенФункционалАвтоматическогоСозданияСвязей;
...
Функция СписокОбъектовДокументооборота(Контрагенты)
	...
	Список.Добавить("DMIncomingDocument");
	Список.Добавить("DMOutgoingDocument");
	Список.Добавить("DMInternalDocument");
	Если Контрагенты Тогда
		Список.Добавить("DMCorrespondent");
	КонецЕсли;
	
	Для Каждого Элемент Из Список Цикл
		Элемент.Представление = ИнтеграцияС1СДокументооборотКлиентСервер.ПредставлениеТипаОбъектаXDTO(Элемент.Значение);
	КонецЦикла;
...
Процедура ЗаполнитьРеквизитыДокументооборота(ПриВыбореШаблона = Ложь)
	...
	Иначе
		...
		РеквизитыШаблона = Справочники.ПравилаИнтеграцииС1СДокументооборотом.ПолучитьРеквизитыШаблонаДокумента(
			Объект.ШаблонТип,
			Объект.ШаблонID,
			Объект.ШаблонЗапрещаетИзменение);
		...
		Для Каждого Строка Из Реквизиты Цикл
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденныеСтроки[0],, "ИмяРеквизитаОбъектаДО");
			КонецЕсли;
...
Процедура ЗаполнитьПоУмолчаниюПриВыгрузкеНаСервере(НачальноеЗаполнение = Ложь)
	...
	Если СтрокиВидаДокумента.Количество() > 0 Тогда
		...
		Если ВидДокументаДО <> Неопределено И ДоступенФункционалКонтрагенты Тогда
			...
			ЗапросЗначенийРеквизитов = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
				Прокси,
			...
			Ответ = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьЗапрос(
				Прокси,
				ЗапросЗначенийРеквизитов);
			ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьВозвратВебСервиса(Прокси, Ответ);
			СвойстваОбъекта = Ответ.object.Свойства();
			...
			Для Каждого СтрокаПравил Из ДеревоПравилВыгрузки.Строки Цикл
				...
				Если ЗначениеЗаполнено(СтрокаПравил.ИмяРеквизитаОбъектаДО)
					...
					ТекущееСвойство = Ответ.object[СтрокаПравил.ИмяРеквизитаОбъектаДО];
	...
	Если СтрНайти(Объект.ТипОбъектаИС, "Справочник.") <> 0 Тогда
	...
	ИначеЕсли СтрНайти(Объект.ТипОбъектаИС, "Документ.") <> 0 Тогда
		МетаданныеИсточника = Метаданные.НайтиПоПолномуИмени(Объект.ТипОбъектаИС);
		Если МетаданныеИсточника <> Неопределено Тогда
...
Процедура ЗаполнитьПоУмолчаниюПриЗагрузкеНаСервере(НачальноеЗаполнение = Ложь)
	...
	СтруктураСостояний = Новый Соответствие;
	...
	Для Каждого СтрокаПравил Из ДеревоПравилЗагрузки.Строки Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеЗаполнения) Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПравил.ПредставлениеРеквизитаОбъектаДО) Тогда
				СтрокаПравил.Вариант = ВариантРеквизит();
