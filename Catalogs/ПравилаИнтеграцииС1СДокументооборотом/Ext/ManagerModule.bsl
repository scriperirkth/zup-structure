			...
			Если ПравилоРеквизита.Вариант = ВариантРеквизит() Тогда
				...
				ИначеЕсли ЗначениеСвойства <> Неопределено И ЭтоОбъектноеЗначение Тогда
					...
					ЗначениеДляЗаполнения =
						ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеДляЗаполненияРеквизитаОбъектаИС(ЗначениеСвойства);
					ЗначениеДляЗаполнения.Владелец = СтрокаРеквизита.Свойство;
					ЗначениеРеквизита =
						ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗначениеРеквизитаОбъектаИСПоЗначениюДляЗаполнения(
							ТипРеквизита,
							ЗначениеДляЗаполнения,
				...
				ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипРеквизита) И ТипЗнч(ЗначениеСвойства) = Тип("Строка") Тогда
					...
					Если СтрНайти(ПолноеИмяТипаРеквизита, "Справочник.") > 0 Тогда
						...
						Ссылка = ИнтеграцияС1СДокументооборотБазоваяФункциональность.НайтиПоНаименованию(
							Менеджер, ЗначениеСвойства, Истина);
							...
							Для Каждого КолонкаРезультата Из КолонкиТаблицы Цикл
								...
								Если ЭтоСсылочныйТип Тогда
									...
									ИначеЕсли ЭтоПеречисление(ЗначениеСвойства) Тогда
										Если ОбщегоНазначения.СсылкаСуществует(ЗначениеСвойства) Тогда
											Элемент[КолонкаРезультата.Имя + "ID"] = ОбщегоНазначения.ИмяЗначенияПеречисления(ЗначениеСвойства);
										КонецЕсли;
										Элемент[КолонкаРезультата.Имя + "Тип"] = ЗначениеСвойства.Метаданные().ПолноеИмя();
									Иначе
					...
					Для НомерЭлемента = 0 По Таблица.Количество() - 1 Цикл
						...
						ИначеЕсли ЗначениеЗаполнено(Значение) Тогда // Ссылочный тип на стороне ДО.
							...
							ИначеЕсли ЭтоПеречисление(Значение) Тогда
								Если ОбщегоНазначения.СсылкаСуществует(Значение) Тогда
					...
					Для Каждого Элемент Из Таблица Цикл
						...
						ИначеЕсли ЗначениеЗаполнено(Параметры.Результат) Тогда // Ссылочный тип на стороне ДО.
							...
							ИначеЕсли ЭтоПеречисление(Параметры.Результат) Тогда
								Если ОбщегоНазначения.СсылкаСуществует(Параметры.Результат) Тогда
...
Функция РеквизитыОбъектаДО(ТипОбъектаДО, Параметры = Неопределено, НастройкиВида = Неопределено,
	...
	Реквизиты.Сортировать("Порядок УБЫВ, ТаблицаДляСортировки, ЭтоТаблица УБЫВ, Представление");
...
Процедура ДобавитьРеквизит(Реквизиты, Имя, Представление, Тип, ОбязательноеЗаполнение = Ложь)
	...
	Если НРег(Имя) = "documenttype" Тогда
...
Процедура ДобавитьТаблицу(Реквизиты, Имя, Представление)
	...
	НоваяСтрока.Имя = Имя;
	НоваяСтрока.ТаблицаДляСортировки = Имя;
	НоваяСтрока.Представление = Представление;
