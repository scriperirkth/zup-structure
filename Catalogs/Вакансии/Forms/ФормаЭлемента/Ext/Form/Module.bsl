...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ОбщегоНазначенияКлиентСервер.Проверить(
			Не ПолучитьФункциональнуюОпцию("ОткрытиеВакансииТребуетСогласования") Или ЗначениеЗаполнено(Объект.Основание),
			НСтр("ru = 'Открытие новой вакансии доступно только по согласованной заявке на подбор персонала.'"));
		...
		ИнициализироватьСостоянияВакансии();
	...
	Элементы.ХарактеристикиПерсоналаОткрытьПодборХарактеристикПерсонала.Доступность = 
		ДоступностьОписания И ДоступноДобавлениеИзменение;
	
	НастроитьСписокОткликов();
	НастроитьСписокПубликации();
	НастроитьСписокОплаты();
...
Процедура ПриОткрытии(Отказ)
	...
	ИзменитьОтборТолькоНовыеОтклики();
	ИзменитьОтборПоАктивнымПубликациям();
	
	ПодключитьОбработчикОжидания("ОбновитьИнформациюПоВакансии", ПериодичностьОбновления());
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	РедактированиеПериодическихСведенийКлиент.ОбработкаОповещения(ЭтотОбъект, ВакансияСсылка, ИмяСобытия, Параметр, Источник);
	...
	Если ИмяСобытия = "ОбновитьУсловноеОформление" Тогда
		УстановитьУсловноеОформлениеСпискаКандидатов();
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	РедактированиеПериодическихСведений.ПроверитьЗаписьВФорме(ЭтотОбъект, "СостоянияВакансий", ВакансияСсылка, Отказ);
...
Процедура СпособНабораПриИзменении(Элемент)
	...
	УстановитьДоступностьНазначенияНабора(ЭтотОбъект);
	УстановитьВидимостьИнформационнойСтрокиЗанятости(ЭтотОбъект);
	УстановитьВидимостьПричиныОткрытия(ЭтотОбъект);
	УстановитьВидимостьНоваяСуществующаяПозиция(ЭтотОбъект);
...
Процедура НоваяПозицияПриИзменении(Элемент)
	...
	УстановитьВидимостьРабочегоМеста(ЭтотОбъект);
	УстановитьДоступностьРабочегоМеста(ЭтотОбъект);
	УстановитьНаименованиеВручную(ЭтотОбъект);
	НачатьЗаполнениеИнформацииЗанятости();
...
Процедура НазначениеНабораПозицияПриИзменении(Элемент)
	...
	УстановитьДоступностьДолжностиПодразделения(ЭтотОбъект);
	НачатьЗаполнениеИнформацииЗанятости();
...
Процедура ПланируемаяДатаЗакрытияПриИзменении(Элемент)
	УстановитьПодсказкуДатыЗакрытия(ЭтотОбъект, ОбщегоНазначенияКлиент.ДатаСеанса());
КонецПроцедуры
...
Процедура СостояниеДекорацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ВыборДаты", ПараметрыВыбораДаты, ЭтотОбъект, , , , ОписаниеОповещения);
...
Процедура СоответствуетПрофилюДолжностиЧислоПриИзменении(Элемент)
	...
	УстановитьДоступностьОписанияВакансии(ЭтотОбъект);
...
Процедура ДополнятьПредставлениеПриИзменении(Элемент)
	УстановитьСвойстваУточненияНаименования(ЭтотОбъект);
КонецПроцедуры
...
Процедура НаименованиеВручнуюПриИзменении(Элемент)
	УстановитьСвойстваПоляНаименование(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура ТолькоКандидатыВРаботеПриИзменении(Элемент)
	
	УстановитьСнятьОтборТолькоВРаботеКандидатов(ЭтотОбъект, ТолькоКандидатыВРаботе);
	НачатьЗаполнениеИнформационнойСтрокиКандидатов();
...
&НаКлиенте
Процедура ОтборТолькоНовыеОткликиПриИзменении(Элемент)
	ИзменитьОтборТолькоНовыеОтклики();
КонецПроцедуры
...
Процедура КандидатыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Иначе
		...
		Если Копирование Тогда
			ОткрытьФорму("Справочник.Кандидаты.ФормаОбъекта");
		Иначе
			ОткрытьФорму("Справочник.Кандидаты.Форма.МастерСозданияКандидата",
				Новый Структура("Вакансия", ВакансияСсылка),,,,,,
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОткликиКандидатов

&НаКлиенте
Процедура ОткликиКандидатовПриАктивизацииСтроки(Элемент)
	
	Если Элементы.Найти("ОткликиОтклонить") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Элементы.ОткликиКандидатов.ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено Тогда
		Элементы.ОткликиОтклонить.Доступность = Ложь;
		Возврат;
	КонецЕсли;
	
	Элементы.ОткликиОтклонить.Доступность = Не ТекущиеДанные.Рассмотрен;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОткликиКандидатовПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	ПодборПерсоналаФормы.ЗаполнитьОжидаемыйДоходВВалютеПоУмолчанию(Настройки, Строки);
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПубликации
...
Процедура ХарактеристикиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле <> Элементы.ХарактеристикиЗначенияСтрокой Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИсторияРаботы(Команда)
	РедактированиеПериодическихСведенийКлиент.ОткрытьИсторию("СостоянияВакансий", ВакансияСсылка, ЭтотОбъект, ТолькоПросмотр);
КонецПроцедуры
...
Процедура СнятьСПубликации(Команда)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Иначе
			ДополнительныеПараметры = Новый Структура("Вакансия, Сайт, ИдентификаторВакансии",
				ВакансияСсылка, ДанныеСтроки.МестоПубликации, ДанныеСтроки.ИдентификаторВакансии);
...
&НаКлиенте
Процедура ПоказыватьАрхивныеПубликации(Команда)
	ПоказыватьАрхивныеПубликации = Не ПоказыватьАрхивныеПубликации;
	ИзменитьОтборПоАктивнымПубликациям();
КонецПроцедуры
...
&НаКлиенте
Процедура ПоказыватьПомеченныхНаУдалениеКандидатов(Команда)
	Кнопка = Элементы.ПоказыватьПомеченныхНаУдалениеКандидатов;
	УстановитьИспользованиеОтбораПомеченныхНаУдаление(ЭтотОбъект, Кандидаты, Кнопка.Пометка);
...
&НаКлиенте
Процедура КандидатыСоздатьПоВакансии(Команда)
...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект = Неопределено)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РабочиеМестаОхраныТруда") Тогда
		...
		РабочиеМестаОхраныТрудаИспользуются = МодульРабочиеМестаОхраныТруда.РабочиеМестаОхраныТрудаИспользуются();
	КонецЕсли;
	...
	УстановитьПодсказкуДатыЗакрытия(ЭтотОбъект, ТекущаяДатаСеанса());
...
Функция ДанныеФормыДляЗаписи()
	Возврат Справочники.Вакансии.СериализоватьДанныеФормы(ЭтотОбъект);
КонецФункции
...
&НаСервере
Процедура ИнициализироватьСостоянияВакансии()

	РедактированиеПериодическихСведений.ИнициализироватьЗаписьДляРедактированияВФорме(
		ЭтотОбъект, "СостоянияВакансий", ВакансияСсылка);
	
	// Используем обработчики заполнения нового объекта для инициализации набора состояний.
	СправочникОбъект = РеквизитФормыВЗначение("Объект");
	СправочникОбъект.Заполнить(Неопределено);

	ЗначениеВРеквизитФормы(
		СправочникОбъект.ДополнительныеСвойства.СостоянияВакансийНаборЗаписей, "СостоянияВакансийНаборЗаписей");
	
	СостоянияВакансий.Вакансия = ВакансияСсылка;
	СостоянияВакансий.Период = СостоянияВакансийНаборЗаписей[0].Период;
	СостоянияВакансий.Состояние = СостоянияВакансийНаборЗаписей[0].Состояние;
...
Процедура ПрочитатьСостоянияВакансий()
	РедактированиеПериодическихСведений.ПрочитатьЗаписьДляРедактированияВФорме(ЭтотОбъект, 
		"СостоянияВакансий", ВакансияСсылка);
	РедактированиеПериодическихСведенийКлиентСервер.ОбновитьНаборЗаписейИстории(ЭтотОбъект, 
		"СостоянияВакансий", ВакансияСсылка);
КонецПроцедуры
...
Процедура УстановитьДоступностьОписанияВакансии(Форма)
	...
	Форма.Элементы.ДействияСотрудниковПодборФункций.Доступность = ДоступностьОписания И Форма.ДоступноДобавлениеИзменение;
...
Процедура ЗаполнитьОписаниеИзПрофиляДолжности()
	...
	ВакансияОбъект.ЗаполнитьПоПрофилюДолжности(Объект.ПрофильДолжности, Ложь,, СоответствуетПрофилюДолжностиЧисло);
...
Процедура НачатьВозобновлениеНабораПоВакансии()
	...
	Если Не Модифицированность Тогда
		НачатьВозобновлениеНабораЗаписаннойВакансии();
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура НачатьВозобновлениеНабораЗаписаннойВакансии()
	...
	СостоянияВакансийКлиент.НачатьУстановкуСостояния(
		...
		Новый ОписаниеОповещения("ВозобновитьНаборНаКлиенте", ЭтотОбъект, ДополнительныеПараметры));
	
КонецПроцедуры

&НаКлиенте
Процедура ВозобновитьНаборНаКлиенте(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ИзменитьДатуСостоянияНаСервере(ДатаСостояния, Состояние, ПрежняяДатаСостояния)
	...
	Справочники.Вакансии.ИзменитьДатуСостояния(Объект.Ссылка, Состояние, ПрежняяДатаСостояния, ДатаСостояния);
...
Процедура ПолучитьОткликиССайтов()
	...
	ОбновитьОтображениеВкладкиОтклики();
...
Процедура Подключаемый_ОбновитьКомандыРаботыСПубликацией()
	...
	ВыделенныеСтроки = Элементы.Публикации.ВыделенныеСтроки;
	Если Не ЗначениеЗаполнено(ИспользуемыеСайты) Или ВыделенныеСтроки.Количество() < 1 Тогда
		Элементы.ОбновитьПубликацию.Доступность = Ложь;
		Элементы.СнятьСПубликации.Доступность = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура ПредложитьОтметитьВакансиюСнятойЗавершение(Ответ, Параметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		РекрутинговыеСайтыКлиент.ОтметитьПубликациюВакансииСнятой(
			Параметры.Вакансия, Параметры.Сайт, Параметры.ИдентификаторВакансии);
...
&НаКлиенте
Процедура ИзменитьОтборТолькоНовыеОтклики()
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ОткликиКандидатов.Отбор, "Рассмотрен", Ложь, , НСтр("ru = 'Только новые'"), ОтборТолькоНовыеОтклики);
	
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьОтборПоАктивнымПубликациям()
	
	Отбор = ОбщегоНазначенияКлиентСервер.НайтиЭлементОтбораПоПредставлению(Публикации.Отбор.Элементы, "Активные");
	Отбор.Использование = Не ПоказыватьАрхивныеПубликации;
	Элементы.ПоказыватьАрхивныеПубликации.Пометка = ПоказыватьАрхивныеПубликации;
...
Процедура СкопироватьШаблоныАнкетЭтаповРаботыСКандидатами()
	...
	Для каждого СтрокаЭтапаРаботы Из Объект.ЭтапыРаботыСКандидатами Цикл
		Если Не ЗначениеЗаполнено(СтрокаЭтапаРаботы.ШаблонАнкеты) Тогда
			Продолжить;
		КонецЕсли;
		Если ТипЗнч(СтрокаЭтапаРаботы.ШаблонАнкеты) = Тип("СправочникСсылка.ЭлектронныеКурсы") Тогда
			НовыеНастройкиТеста = РазработкаЭлектронныхКурсов.НовыеНастройкиТеста();
			НастройкиТеста = РазработкаЭлектронныхКурсов.НастройкиТеста(СтрокаЭтапаРаботы.ШаблонАнкеты);
			ЗаполнитьЗначенияСвойств(НовыеНастройкиТеста, НастройкиТеста);
			СозданныйТест = РазработкаЭлектронныхКурсов.УстановитьНастройкиТеста(НовыеНастройкиТеста);
			НовыйЭлементСсылка = СозданныйТест.ЭлектронныйКурс;
		Иначе
			НовыйЭлементСсылка = КопияШаблонаАнкеты(
				СтрокаЭтапаРаботы.ШаблонАнкеты, Объект.ПрофильДолжности, Объект.Ссылка);
		КонецЕсли;
...
Процедура НачатьЗаполнениеИнформацииЗанятости()
	...
	Если Не ВидимостьИнформационнойСтрокиЗанятости(ЭтотОбъект) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаНавигационнойСсылкиПозицияМассовогоНабора(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.ШтатноеРасписание.ФормаСписка", ПараметрыФормы, ЭтотОбъект);
...
&НаСервере
Процедура НастроитьСписокОткликов()
	
	ОтборТолькоНовыеОтклики = Истина;
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОткликиКандидатов, "ТекущаяДата", ТекущаяДатаСеанса());
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОткликиКандидатов, "НеУказан", НСтр("ru = 'Не указан'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОткликиКандидатов, "МеньшеГода", НСтр("ru = 'Меньше года'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОткликиКандидатов, "Мужчина", НСтр("ru = 'Мужчина'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ОткликиКандидатов, "Женщина", НСтр("ru = 'Женщина'"));
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ОткликиКандидатов.Отбор, "Вакансия", ВакансияСсылка);
	
	НастроитьОтображениеОткликов();

КонецПроцедуры

&НаСервере
Процедура НастроитьСписокПубликации()
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Публикации.Отбор, "Вакансия", ВакансияСсылка);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		Публикации.Отбор, "ДатаСнятияСПубликации", , ВидСравненияКомпоновкиДанных.НеЗаполнено, "Активные");
КонецПроцедуры

&НаСервере
Процедура НастроитьСписокОплаты()
...
Процедура НастроитьСписокКандидатов()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Кандидаты, 
		"Пользователь", Пользователи.АвторизованныйПользователь());
	
	ПодборПерсоналаФормы.ПриСозданииФормыСоСпискомКандидатов(ЭтотОбъект, Кандидаты);
	
	ТолькоКандидатыВРаботе = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Вакансия", 
		"ТолькоКандидатыВРаботе", Истина);
	УстановитьСнятьОтборТолькоВРаботеКандидатов(ЭтотОбъект, ТолькоКандидатыВРаботе);

	УстановитьИспользованиеОтбораПомеченныхНаУдаление(ЭтотОбъект, Кандидаты, Истина);
...
Процедура ПриИзмененииСохраняемойНастройки()
	...
	Настройки.ТолькоКандидатыВРаботе = ТолькоКандидатыВРаботе;
...
Функция СохраняемыеНастройки()
	...
	Настройки.Вставить("ТолькоКандидатыВРаботе", Истина);
	Возврат Настройки;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСнятьОтборТолькоВРаботеКандидатов(Форма, Использование)
	ПодборПерсоналаКлиентСервер.УстановитьСнятьОтборТолькоВРаботеКандидатов(Форма.Кандидаты, Использование);
КонецПроцедуры
...
Процедура ОбработатьОповещениеРедактированияИстории(ИмяСобытия, Параметр, Источник)
	...
	ЗаполнитьДатуОткрытия(ЭтотОбъект, ОбщегоНазначенияКлиент.ДатаСеанса());
...
&НаКлиенте
Процедура ОбновитьИнформациюПоВакансии()
	
	ОбновитьИнформационныеСтрокиВакансии();
	ОбновитьОтображениеВкладкиОтклики(Истина);
...
Процедура ОткрытьФормуНастройкиВопросов(ТекущиеДанные, СозданиеНового = Ложь)
	...
	Оповещение = Новый ОписаниеОповещения("НастройкаВопросовЗавершение", ЭтотОбъект, ТекущиеДанные);
...
Процедура ОткрытьФормуВыбораИнструментаОценки(ТекущиеДанные)
	...
	Оповещение = Новый ОписаниеОповещения("ВыборИнструментаОценкиЗавершение", ЭтотОбъект, ТекущиеДанные);
...
&НаКлиенте
Процедура ОбновитьОтображениеВкладкиОтклики(ПоказыватьОповещение = Ложь)
	
	КоличествоНовыхОткликов = УстановитьЗаголовокВкладкиОтклики();
	
	Если ПоказыватьОповещение И КоличествоНовыхОткликов <> 0 Тогда
		ПоказатьОповещениеПользователя(,
			ПолучитьНавигационнуюСсылку(Объект.Ссылка),
			СтрШаблон(НСтр("ru = '%1 на вакансию %2'"),
				СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = ';%1 новый отклик;;%1 новых отклика;%1 новых откликов;'"), КоличествоНовыхОткликов),
				Объект.Ссылка));
	КонецЕсли;
...
&НаСервере
Процедура НастроитьОтображениеОткликов()
	
	УстановитьЗаголовокВкладкиОтклики();
	ПодборПерсоналаФормы.УстановитьВидимостьЭлементаФормы(Элементы.ОткликиКандидатовЗагрузитьОткликиССайта);
	
	НастроитьУсловноеОформлениеОткликов();
	
КонецПроцедуры

&НаСервере
Процедура НастроитьУсловноеОформлениеОткликов()

	ОткликиКандидатов.УсловноеОформление.Элементы.Очистить();
	
	// Шрифт нерассмотренного отклика
	ЭлементОформления = ОткликиКандидатов.УсловноеОформление.Элементы.Добавить();
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", ШрифтыСтиля.ВажнаяНадписьШрифт);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементОформления.Отбор, "Рассмотрен", Ложь);
	
	// Цвет одобренного отклика
	ЭлементОформления = ОткликиКандидатов.УсловноеОформление.Элементы.Добавить();
	...
	ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("ДатаОтклика");
	...
	ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("ДатаОтклика");
...
&НаСервере
Процедура УстановитьУсловноеОформлениеСпискаКандидатов()
...
&НаСервере
Функция УстановитьЗаголовокВкладкиОтклики()
	
	ОткликиНаВакансию = ПодборПерсонала.ОткликиКандидатов(Новый Структура("Ссылка", Объект.Ссылка), Ложь);
	
	НовыхОткликов = 0;
	Для Каждого Отклик Из ОткликиНаВакансию Цикл
		НовыхОткликов = НовыхОткликов + ?(Отклик.Рассмотрен, 0, 1);
	КонецЦикла;
	
	Элементы.ОткликиСтраница.Заголовок =
		СтрШаблон(НСтр("ru = 'Отклики%1'"), ?(НовыхОткликов = 0, "", " " + СтрШаблон(НСтр("ru = '(+%1)'"), НовыхОткликов)));
	
	Возврат НовыхОткликов;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьИспользованиеОтбораПомеченныхНаУдаление(Форма, Список, Использование)
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		Список, "ПометкаУдаления", Ложь, , , Использование);
КонецПроцедуры
