...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РежимВыбора = Истина Тогда
		...
		Если ТипЗнч(Параметры.ТекущаяСтрока) = Тип("СписокЗначений") Тогда
			СписокПодобранных = Параметры.ТекущаяСтрока;
			ВыбранныеФизическиеЛица = Параметры.ТекущаяСтрока;
			ВыбранныеФизическиеЛица.ЗаполнитьПометки(Истина);
			Если ВыбранныеФизическиеЛица.Количество() > 0 Тогда
				Параметры.ТекущаяСтрока = ВыбранныеФизическиеЛица[0].Значение;
			Иначе
				Параметры.ТекущаяСтрока = Неопределено;
			КонецЕсли;
		КонецЕсли;
		
		Если Элементы.Список.РежимВыбора И Параметры.Свойство("СписокВыбора") Тогда
			РежимПодбораСФлажками = Истина;
			Для Каждого ЭлементСпискаВыбора Из Параметры.СписокВыбора Цикл
				Если ФизлицоЗаполнено(ЭлементСпискаВыбора.Значение) Тогда
					ЭлементСписка = ВыбранныеФизическиеЛица.НайтиПоЗначению(ЭлементСпискаВыбора.Значение);
					Если ЭлементСписка = Неопределено Тогда
						ЭлементСписка = ВыбранныеФизическиеЛица.Добавить();
						ЗаполнитьЗначенияСвойств(ЭлементСписка, ЭлементСпискаВыбора);
				...
				КонецЕсли;
			КонецЦикла;
			ВыбранныеФизическиеЛица.СортироватьПоПредставлению();
		КонецЕсли;
		
	КонецЕсли;
	
	Элементы.ВыбранныеЭлементыГруппа.Видимость = РежимПодбораСФлажками;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ВыбранныеФизическиеЛица.Количество() > 0 И Не РежимПодбораСФлажками Тогда
		РезультатВыбора = ВыбранныеФизическиеЛица.ВыгрузитьЗначения();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если РежимПодбораСФлажками И ИмяСобытия = "ИзменениеДанныхФизическогоЛица" И ФизлицоЗаполнено(Источник) Тогда
		ЭлементСписка = ВыбранныеФизическиеЛица.НайтиПоЗначению(Источник);
		Если ЭлементСписка <> Неопределено Тогда
			Если ЭлементСписка.Представление <> Параметр.Наименование Тогда
				ЭлементСписка.Представление = Параметр.Наименование;
				ВыбранныеФизическиеЛица.СортироватьПоПредставлению();
			КонецЕсли;
			Элементы.ВыбранныеФизическиеЛица.ТекущаяСтрока = ЭлементСписка.ПолучитьИдентификатор();
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	Если РежимПодбораСФлажками Тогда
		
		СтандартнаяОбработка = Ложь;
		ЕстьИзменения = Ложь;
		Если ТипЗнч(Значение) = Тип("Массив") Тогда
			Для Каждого ВложенноеЗначение Из Значение Цикл
				ПодобратьЗначение(ВложенноеЗначение, ЕстьИзменения);
			КонецЦикла;
		Иначе
			ПодобратьЗначение(ВложенноеЗначение, ЕстьИзменения);
		КонецЕсли;
	...
	ИначеЕсли Элементы.Список.РежимВыбора Тогда
		...
		Если СписокЗначений.Количество() > 0 Тогда
			...
			Если Элементы.Список.МножественныйВыбор Тогда
				...
				Если СписокЗначений.Количество() > 1 И Не РежимПодбораСФлажками Тогда
					Закрыть();
				КонецЕсли; 
...
&НаКлиенте
Процедура ВыбранныеФизическиеЛицаПриИзменении(Элемент)
	УстановитьСписокПодобранныхСотрудников();
КонецПроцедуры
...
Процедура КомандаОК(Команда)
	Закрыть(ВыбранныеФизическиеЛица);
КонецПроцедуры
...
&НаКлиенте
Процедура ПодобратьЗначение(ВыбраннаяСсылка, ЕстьИзменения)
	Если ФизлицоЗаполнено(ВыбраннаяСсылка) Тогда
		ДанныеСтроки = Элементы.Список.ДанныеСтроки(ВыбраннаяСсылка);
		Представление = ?(ДанныеСтроки = Неопределено, Строка(ВыбраннаяСсылка), ДанныеСтроки.Наименование);
		ЭлементСписка = ВыбранныеФизическиеЛица.НайтиПоЗначению(ВыбраннаяСсылка);
		Если ЭлементСписка = Неопределено Тогда
			ЕстьИзменения = Истина;
			ЭлементСписка = ВыбранныеФизическиеЛица.Добавить(ВыбраннаяСсылка, Представление, Истина);
		ИначеЕсли ЭлементСписка.Представление <> Представление Или ЭлементСписка.Пометка <> Истина Тогда
			ЕстьИзменения = Истина;
...
Процедура ОбновитьСписокПодобранных(Значение)
	...
	Для каждого ВыбранноеЗначение Из СписокЗначений Цикл
		Если СписокПодобранных.НайтиПоЗначению(ВыбранноеЗначение) = Неопределено Тогда
			...
			ВыбранныеФизическиеЛица.Добавить(ВыбранноеЗначение, , Истина);
		КонецЕсли; 
	КонецЦикла;
...
Процедура УстановитьСписокПодобранныхСотрудников()
	...
	Если ЭлементУсловногоОформления <> Неопределено Тогда
		Если РежимПодбораСФлажками Тогда
			ЭлементУсловногоОформления.Отбор.Элементы[0].ПравоеЗначение = ВыбранныеФизическиеЛица;
		Иначе
...
&НаСервереБезКонтекста
Функция ФизлицоЗаполнено(Ссылка) Экспорт
	Возврат ТипЗнч(Ссылка) = Тип("СправочникСсылка.ФизическиеЛица") И Не Ссылка.Пустая();
КонецФункции
