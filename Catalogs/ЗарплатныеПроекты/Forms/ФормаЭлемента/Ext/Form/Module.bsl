...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СменитьФорматФайла();
	УстановитьДоступностьДляПеречисленийВЦифровыхРублях(ЭтотОбъект);
	УстановитьВидимостьЭлементовДляЗарплатногоПроектаЦР(ЭтотОбъект);
...
&НаКлиенте
Процедура ДляПеречисленийВЦифровыхРубляхПриИзменении(Элемент)
	СменитьБанк();
	СформироватьАвтоНаименование();
	УстановитьВидимостьЭлементовДляЗарплатногоПроектаЦР(ЭтотОбъект);
КонецПроцедуры
...
Процедура УстановитьДоступностьЭлементовПриИзмененииПризнакаПеречисленийНаКартыМир(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ДляПеречисленийВЦифровыхРублях", "Видимость", ЗарплатныйПроектСБанком);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ДляПеречисленийНаКартыМир", "Видимость", ЗарплатныйПроектСБанком);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьДляПеречисленийВЦифровыхРублях(Форма)

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ДляПеречисленийВЦифровыхРублях", "Видимость", 
		Форма.ЗарплатныйПроектПоддерживаетПеречисленияВЦифровыхРублях);
		
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьЭлементовДляЗарплатногоПроектаЦР(Форма)

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "Договор", "Видимость", 
		Не Форма.Объект.ДляПеречисленийВЦифровыхРублях);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "БанковскиеКарты", "Видимость", 
		Не Форма.Объект.ДляПеречисленийВЦифровыхРублях);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "РеквизитыБанка", "Видимость", 
		Не Форма.Объект.ДляПеречисленийВЦифровыхРублях);
...
Процедура СменитьФорматФайла()
	...
	Если ЗначениеЗаполнено(Объект.ФорматФайла) Тогда
		ЗарплатныйПроектПоддерживаетПеречисленияВЦифровыхРублях = 
			ОбменСБанкамиПоЗарплатнымПроектам.ФорматФайлаПоддерживаетПеречисленияВЦифровыхРублях(
				Объект.ФорматФайла);
		Объект.ДляПеречисленийВЦифровыхРублях = ?(ЗарплатныйПроектПоддерживаетПеречисленияВЦифровыхРублях, 
			Объект.ДляПеречисленийВЦифровыхРублях,
			Ложь);
		УстановитьВидимостьЭлементовДляЗарплатногоПроектаЦР(ЭтотОбъект);
	КонецЕсли;
...
&НаСервере
Процедура СменитьБанк()

	Если Объект.ДляПеречисленийВЦифровыхРублях Тогда
		БИК = ОбменСБанкамиПоЗарплатнымПроектам.БИКПлатформыЦифровогоРубля();
		НайденныеБанки = РаботаСБанками.СведенияБИК(БИК);
		Если НайденныеБанки.Количество() = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не найден банк оператора платформы цифрового рубля с БИК %1. Загрузите или обновите классификатор банков.'"), БИК);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Объект.Банк");
		Иначе
			Объект.Банк = НайденныеБанки[0].Ссылка;
		КонецЕсли;
	Иначе
		Объект.Банк = Справочники.КлассификаторБанков.ПустаяСсылка();
	КонецЕсли;
