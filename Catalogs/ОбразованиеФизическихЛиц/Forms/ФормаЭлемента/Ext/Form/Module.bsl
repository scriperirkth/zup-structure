...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Оповестить("ЗаписанСправочникОбразованиеФизическихЛиц", , ЭтотОбъект);
КонецПроцедуры
...
Процедура ВидОбразованияПриИзменении(Элемент)
	...
	УстановитьСкрыватьВТ2();
	
	УстановитьОтображениеЭлементовФормы(ЭтотОбъект, Объект);
...
&НаКлиенте
Процедура ВидПрофессиональногоОбразованияПриИзменении(Элемент)
	
	УстановитьСкрыватьВТ2();
	УстановитьДоступностьСкрытьВТ2(ЭтотОбъект, Объект);
...
Процедура ПерейтиПоСсылке(Команда)
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(Объект.ЭлектронныйСертификат);
...
Процедура ПриПолученииДанных(ТекущийОбъект)
	...
	УстановитьОтображениеЭлементовФормы(ЭтотОбъект, ТекущийОбъект);
...
Процедура УстановитьОтображениеЭлементовФормы(Форма, ТекущийОбъект)
	...
	УстановитьДоступностьСкрытьВТ2(Форма, ТекущийОбъект);
...
Процедура УстановитьОтображениеВидаПослеВузовскогоОбразования(Форма, ТекущийОбъект)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ТекущийОбъект.ВидОбразования = ПредопределенныйЭлемент("Справочник.ВидыОбразованияФизическихЛиц.ВысшееОбразованиеПодготовкаКадровВысшейКвалификации")
			Или ТекущийОбъект.ВидОбразования = ПредопределенныйЭлемент("Справочник.ВидыОбразованияФизическихЛиц.ПослевузовскоеОбразование"));
...
Процедура УстановитьОтображениеВидаДополнительногоОбучения(Форма, ТекущийОбъект)
	...
	"Видимость",
	ТекущийОбъект.ВидОбразования = ПредопределенныйЭлемент("Справочник.ВидыОбразованияФизическихЛиц.ПрофессиональноеОбучение"));
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
	Форма.Элементы,
	"ВидПрофессиональногоОбразования",
	"Видимость",
	ТекущийОбъект.ВидОбразования = ПредопределенныйЭлемент("Справочник.ВидыОбразованияФизическихЛиц.ДополнительноеПрофессиональноеОбразование"));
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьСкрытьВТ2(Форма, ТекущийОбъект)
	
	ВидПрочее = ПредопределенныйЭлемент("Перечисление.ВидыПрофессиональнойПодготовки.Прочее");
	ДополнительноеОбразование = ПредопределенныйЭлемент("Справочник.ВидыОбразованияФизическихЛиц.ДополнительноеПрофессиональноеОбразование");
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		"СкрыватьВТ2",
		"ТолькоПросмотр",
		ТекущийОбъект.ВидДополнительногоОбучения = ВидПрочее);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		"СкрыватьВТ2",
		"Видимость",
		ТекущийОбъект.ВидОбразования = ДополнительноеОбразование);
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьСкрыватьВТ2()
	
	Если Объект.ВидДополнительногоОбучения = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыПрофессиональнойПодготовки.Прочее") Тогда
		Объект.СкрыватьВТ2 = Истина;
	ИначеЕсли Объект.ВидОбразования <> ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник.ВидыОбразованияФизическихЛиц.ДополнительноеПрофессиональноеОбразование") Тогда
		Объект.СкрыватьВТ2 = Ложь;
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	Возврат ОбщегоНазначения.ПредопределенныйЭлемент(ИмяПредопределенногоЭлемента);
#Иначе
	Возврат ОбщегоНазначенияКлиент.ПредопределенныйЭлемент(ИмяПредопределенногоЭлемента);
#КонецЕсли
