...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнениеИменПрисоединенныхФайловБО();
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КонтекстЭДОСервер = Неопределено Тогда 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Электронное представление не записано:%1.'"), Символы.ПС + ТекстСообщения);
...
Процедура Отправить(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ПриНажатииНаКнопкуОтправкиВКонтролирующийОрган(ЭтотОбъект, ИмяТекущегоТипаПолучателя(Объект.ТипПолучателя));
...
Процедура ДобавитьФайлПриложенияОтчетности(ТипФайлаОтчетности)
	АдресДанных = "";
	ДополнительныеПараметры = Новый Структура("ИмяФайлаОтчетности, ТипФайлаОтчетности", "", ТипФайлаОтчетности);
	
	Если ТипФайлаОтчетности = ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ПояснительнаяЗаписка") Тогда 
		СтрИмяФайла = Новый Структура("ИмяФайлаПояснительнойЗаписки", "");
		ЗаполнитьЗначенияСвойств(СтрИмяФайла, ЭтотОбъект);
		ДополнительныеПараметры.Вставить("ИмяФайлаОтчетности", СтрИмяФайла.ИмяФайлаПояснительнойЗаписки);
	ИначеЕсли ТипФайлаОтчетности = ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.АудиторскоеЗаключение") Тогда 
		СтрИмяФайла = Новый Структура("ИмяФайлаАудиторскогоЗаключения", "");
		ЗаполнитьЗначенияСвойств(СтрИмяФайла, ЭтотОбъект);
		ДополнительныеПараметры.Вставить("ИмяФайлаОтчетности", СтрИмяФайла.ИмяФайлаАудиторскогоЗаключения);
	ИначеЕсли ТипФайлаОтчетности = ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ЗаявлениеСоюзаСельхозПроизводителей") Тогда 
		СтрИмяФайла = Новый Структура("ИмяФайлаЗаявленияРевизионногоСоюза", "");
		ЗаполнитьЗначенияСвойств(СтрИмяФайла, ЭтотОбъект);
		ДополнительныеПараметры.Вставить("ИмяФайлаОтчетности", СтрИмяФайла.ИмяФайлаЗаявленияРевизионногоСоюза);
	КонецЕсли;
...
Процедура ДобавитьФайлПриложенияОтчетностиЗавершениеПослеПомещенияФайла(
	...
	Если ФайлыБылиВыбраны Тогда
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.ИмяФайлаОтчетности) Тогда 
			СвойстваФайла.Имя = ДополнительныеПараметры.ИмяФайлаОтчетности;
		КонецЕсли;
...
&НаСервере
Функция КороткоеИмяФайлаПоПолному(Знач ПолноеИмяФайла)
...
&НаСервере
Функция РазмерФайла(Знач ПолноеИмяФайла)
...
Функция ОпределитьТипФайлаУведомления(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов)
	...
	Если Лев(ИмяФайлаПриведенное, 11) = "ON_UVNPDUS_"
		...
		ИЛИ Лев(ИмяФайлаПриведенное, 14) = "NO_UVEDAUDZAK_"
		ИЛИ Лев(ИмяФайлаПриведенное, 14) = "NO_ZVPSODOKNO_" Тогда
		Возврат ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ОсновнойФайлОтчетности");
...
&НаСервере
Функция ОпределитьТипФайлаПФР(Знач ПолноеИмяФайла)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ОпределитьТипФайлаПФР");
...
&НаСервере
Функция ДеревоXMLИзФайла(Знач ПутьКФайлу)
	
	Попытка
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ПутьКФайлу, "ПутьКФайлу", "ДеревоXMLИзФайла");		
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиРПН(Знач ОсновнойФайл)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ОсновнойФайл, "ОсновнойФайл", "ПроанализироватьФайлВыгрузкиРПН");
	// загружаем XML в дерево
	//@skip-check 1c-fresh-audit
...
Функция ПроанализироватьФайлВыгрузкиФТС(ОсновнойФайл)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ОсновнойФайл, "ОсновнойФайл", "ПроанализироватьФайлВыгрузкиФТС");	
	// инициализируем текст на форме
	//@skip-check 1c-fresh-audit
	ЧтениеТекста = Новый ЧтениеТекста(ОсновнойФайл, "UTF-8");
	ПараметрыИмпорта.Вставить("Текст", ЧтениеТекста.Прочитать());
...
&НаСервере
Функция ЭтоФайлВыгрузкиФТС(Знач ОсновнойФайл)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ОсновнойФайл, "ОсновнойФайл", "ЭтоФайлВыгрузкиФТС");
...
&НаСервере
Функция ЭтоФайлВыгрузкиЦБ(Знач ОсновнойФайл)
	...
	Если Расширение = "xml" Тогда		
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ОсновнойФайл, "ОсновнойФайл", "ЭтоФайлВыгрузкиЦБ");		
		//@skip-check 1c-fresh-audit
		ФайлВыгрузки = Новый ЧтениеТекста(ОсновнойФайл, "windows-1251");
		ТекстВыгрузки = ФайлВыгрузки.Прочитать();
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиФНС(Знач ПолноеИмяФайла)
	...
	Если Расширение = "xml" Тогда
		...
		Попытка
			РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
				ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиФНС");			
			//@skip-check 1c-fresh-audit
	...
	ДобавлениеРеквизитовИменФайловБО(ПоказателиВыгрузки);
	Возврат Истина;
КонецФункции

&НаСервере
Процедура ДобавлениеРеквизитовИменФайловБО(ПоказателиВыгрузки)
	Если Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухгалтерскаяОтчетность
		Или Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухОтчетность
		Или Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухОтчетностьМП
		Или Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухОтчетностьСОНКО Тогда 
		
		// Для бухотчетности необходимо заполмнить корректные имена приложенных файлов и запомнить их
		ОписаниеТиповСтрока128 = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(128));
		ДобавляемыеРеквизиты = Новый Массив;
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяФайлаАудиторскогоЗаключения", ОписаниеТиповСтрока128));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяФайлаПояснительнойЗаписки", ОписаниеТиповСтрока128));
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ИмяФайлаЗаявленияРевизионногоСоюза", ОписаниеТиповСтрока128));
		ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		
		ПоказателиАудиторскогоЗаключения = "НаимФайлАЗ,НаимФайлАО";
		Для Каждого Элт Из СтрРазделить(ПоказателиАудиторскогоЗаключения, ",") Цикл 
			Если ЗначениеЗаполнено(ПоказателиВыгрузки[Элт]) Тогда 
				ЭтотОбъект["ИмяФайлаАудиторскогоЗаключения"] = ПоказателиВыгрузки[Элт];
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Элементы.ФайлАудиторскогоЗаключения.Доступность = ЗначениеЗаполнено(ЭтотОбъект["ИмяФайлаАудиторскогоЗаключения"]);
		
		ПоказателиПояснительнойЗаписки = "НаимФайлПЗ";
		Для Каждого Элт Из СтрРазделить(ПоказателиПояснительнойЗаписки, ",") Цикл 
			Если ЗначениеЗаполнено(ПоказателиВыгрузки[Элт]) Тогда 
				ЭтотОбъект["ИмяФайлаПояснительнойЗаписки"] = ПоказателиВыгрузки[Элт];
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Элементы.ФайлПояснительнойЗаписки.Доступность = ЗначениеЗаполнено(ЭтотОбъект["ИмяФайлаПояснительнойЗаписки"]);
		
		ПоказателиЗаявленияРевизионногоСоюза = "НаимФайлЗСХП";
		Для Каждого Элт Из СтрРазделить(ПоказателиЗаявленияРевизионногоСоюза, ",") Цикл 
			Если ЗначениеЗаполнено(ПоказателиВыгрузки[Элт]) Тогда 
				ЭтотОбъект["ИмяФайлаЗаявленияРевизионногоСоюза"] = ПоказателиВыгрузки[Элт];
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Элементы.ФайлЗаявленияРевизионногоСоюза.Доступность = ЗначениеЗаполнено(ЭтотОбъект["ИмяФайлаЗаявленияРевизионногоСоюза"]);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ПроанализироватьФайлВыгрузкиФСС(Знач ПолноеИмяФайла, ПреобразуемыйАдрес)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиФСС");	
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиОтчет4ФСС(Знач ПолноеИмяФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиОтчет4ФСС");			
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиРеестраСведенийФСС(Знач ПолноеИмяФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиРеестраСведенийФСС");	
	// инициализируем переменные, содержащие двоичные данные и имя файла
	//@skip-check 1c-fresh-audit
	ОбъектФайл = Новый Файл(ПолноеИмяФайла);
	КороткоеИмяФайла = ОбъектФайл.Имя;
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиРеестраЭЛН(Знач ПолноеИмяФайла, ПреобразуемыйАдрес)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиРеестраЭЛН");	
	// инициализируем переменные, содержащие двоичные данные и имя файла
	//@skip-check 1c-fresh-audit
	ОбъектФайл = Новый Файл(ПолноеИмяФайла);
	КороткоеИмяФайла = ОбъектФайл.Имя;
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиПодтвержденияВидаДеятельности(Знач ПолноеИмяФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиПодтвержденияВидаДеятельности");
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиРеестраСтимулирующихВыплатМедицинскимИСоциальнымРаботникам(Знач ПолноеИмяФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла,
		"ПолноеИмяФайла",
		"ПроанализироватьФайлВыгрузкиРеестраСтимулирующихВыплатМедицинскимИСоциальнымРаботникам");
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиФСГС(Знач ПолноеИмяФайла)
	...
	Попытка
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиФСГС");		
		//@skip-check 1c-fresh-audit
		ОбъектЧтениеXML.ОткрытьФайл(ПолноеИмяФайла);
		ОбъектЧтениеXML.ИгнорироватьПробелы = Ложь;
		Пока ОбъектЧтениеXML.Прочитать() Цикл
...
Функция ПроанализироватьФайлВыгрузкиЦБ(ПолноеИмяФайла)
	...
	Иначе
		...
		РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
			ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиЦБ");
...
Функция ОпределитьНомерКорректировкиПФР(ДеревоXML, КороткоеИмяФайла = "")
	...
	Если Не ЗначениеЗаполнено(НомерКорректировки) 
		И ВРЕГ(Лев(КороткоеИмяФайла, 4)) = "СФР_"
		И СтрНайти(ВРЕГ(КороткоеИмяФайла), "ЕФС-1") > 0 Тогда 
		Попытка
			УПВхДок = УзелПачкаВходящихДокументов;
			Для Каждого Элт Из СтрРазделить("ЕФС_1,ОСС,НомерКорректировки", ",") Цикл 
				Свойство = УПВхДок.Свойства().Получить(Элт);
				Если Свойство = Неопределено Тогда 
					УПВхДок = Неопределено;
					Прервать;
				КонецЕсли;
				УПВхДок = УПВхДок[Элт];
			КонецЦикла;
			
			Если ТипЗнч(УПВхДок) = Тип("Строка") Тогда 
				ОТЧ = Новый ОписаниеТипов("Число");
				НомерКорректировки = ОТЧ.ПривестиЗначение(УПВхДок);
			КонецЕсли;
		Исключение
			НомерКорректировки = Неопределено;
		КонецПопытки;
...
&НаСервере
Функция ЭтоФайлВыгрузкиФСГС(Знач ПолноеИмяФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ЭтоФайлВыгрузкиФСГС");
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиОтчет4аФСС(Знач ПолноеИмяФайла)
	
	РегламентированнаяОтчетностьКлиентСервер.ПроверитьПолноеИмяВременногоФайла(
		ПолноеИмяФайла, "ПолноеИмяФайла", "ПроанализироватьФайлВыгрузкиОтчет4аФСС");
...
Процедура УправлениеФормой(Форма)
	...
	ЗагружаетсяНесколькоФайлов = 
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПродлениеСрокаПредставленияДокументов")
	...
	Если Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ЗаявлениеОВвозеТоваров")
		ИЛИ (Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ПрочаяОтчетностьПФР")
			...
			И Объект.ВидОтчета <> ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЕФС1ОСС")
	...
	ИначеЕсли ЗагружаетсяНесколькоФайлов
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДокументацияПоКонтролируемымСделкам")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.АудиторскоеЗаключениеФинОтчетность")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПродлениеСрокаПредставленияДокументов")) Тогда
...
&НаСервере
Процедура ЗаполнениеИменПрисоединенныхФайловБО()
	Если Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухгалтерскаяОтчетность
		Или Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухОтчетность
		Или Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухОтчетностьМП
		Или Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.БухОтчетностьСОНКО Тогда 
		
		СтруктураОтбора = Новый Структура("ТипФайлаОтчетности", Перечисления.ТипыФайловБухОтчетности.ОсновнойФайлОтчетности);
		ОсновнойФайл = СвойстваФайлов.НайтиСтроки(СтруктураОтбора);
		Если ЗначениеЗаполнено(ОсновнойФайл) Тогда
			Попытка
				ОбъектЧтениеXML = Новый ЧтениеXML;
				БуферДвоичныхДанных = ПолучитьБуферДвоичныхДанныхИзДвоичныхДанных(ПолучитьИзВременногоХранилища(ОсновнойФайл[0].АдресДанных));
				ОбъектЧтениеXML.ОткрытьПоток(Новый ПотокВПамяти(БуферДвоичныхДанных));
				...
				ДобавлениеРеквизитовИменФайловБО(ПоказателиВыгрузки);
				ИменаФайлов = Новый Структура("ИмяФайлаАудиторскогоЗаключения,ИмяФайлаПояснительнойЗаписки,ИмяФайлаЗаявленияРевизионногоСоюза");
				ЗаполнитьЗначенияСвойств(ИменаФайлов, ЭтотОбъект);
				Если ЗначениеЗаполнено(ИменаФайлов["ИмяФайлаАудиторскогоЗаключения"]) Тогда
					СтруктураОтбора.ТипФайлаОтчетности = Перечисления.ТипыФайловБухОтчетности.АудиторскоеЗаключение;
					Для Каждого Стр Из СвойстваФайлов.НайтиСтроки(СтруктураОтбора) Цикл 
						Стр["ИмяФайла"] = ИменаФайлов["ИмяФайлаАудиторскогоЗаключения"];
						Прервать;
					КонецЦикла;
				КонецЕсли;
				Если ЗначениеЗаполнено(ИменаФайлов["ИмяФайлаПояснительнойЗаписки"]) Тогда
					СтруктураОтбора.ТипФайлаОтчетности = Перечисления.ТипыФайловБухОтчетности.ПояснительнаяЗаписка;
					Для Каждого Стр Из СвойстваФайлов.НайтиСтроки(СтруктураОтбора) Цикл 
						Стр["ИмяФайла"] = ИменаФайлов["ИмяФайлаПояснительнойЗаписки"];
						Прервать;
					КонецЦикла;
				КонецЕсли;
				Если ЗначениеЗаполнено(ИменаФайлов["ИмяФайлаЗаявленияРевизионногоСоюза"]) Тогда
					СтруктураОтбора.ТипФайлаОтчетности = Перечисления.ТипыФайловБухОтчетности.ЗаявлениеСоюзаСельхозПроизводителей;
					Для Каждого Стр Из СвойстваФайлов.НайтиСтроки(СтруктураОтбора) Цикл 
						Стр["ИмяФайла"] = ИменаФайлов["ИмяФайлаЗаявленияРевизионногоСоюза"];
						Прервать;
					КонецЦикла;
				КонецЕсли;
			Исключение
				ОбщегоНазначения.СообщитьПользователю("Не удалось распознать электронное представление");
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьОтправку(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОбновитьОтправкуИзПанелиОтправки(ЭтотОбъект, ИмяТекущегоТипаПолучателя(Объект.ТипПолучателя));
КонецПроцедуры

&НаКлиенте
Процедура ГиперссылкаПротоколНажатие(Элемент)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьПротоколИзПанелиОтправки(ЭтотОбъект, ИмяТекущегоТипаПолучателя(Объект.ТипПолучателя));
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьНеотправленноеИзвещение(Команда)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОтправитьНеотправленноеИзвещениеИзПанелиОтправки(ЭтотОбъект, ИмяТекущегоТипаПолучателя(Объект.ТипПолучателя));
КонецПроцедуры

&НаКлиенте
Процедура ЭтапыОтправкиНажатие(Элемент)
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьСостояниеОтправкиИзПанелиОтправки(ЭтотОбъект, ИмяТекущегоТипаПолучателя(Объект.ТипПолучателя));
КонецПроцедуры

&НаКлиенте
Процедура КритическиеОшибкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьКритическиеОшибкиИзПанелиОтправки(ЭтотОбъект, ИмяТекущегоТипаПолучателя(Объект.ТипПолучателя));
КонецПроцедуры
...
Процедура ВыгрузитьПакет(Команда)
	...
	Если Объект.ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФСРАР") Тогда
		...
		КонтекстЭДОКлиент.ВыгрузитьПакетДляОтправкиВФСРАР(ЭтотОбъект,ОписаниеОповещения);
...
Процедура ДоопределитьСвойства()
	...
	ПривестиПолучателяКТипу(ЭтотОбъект);
...
Функция ВыбранныеФайлыРазбитыПриНеобходимости(ВыбранныеФайлы)
	...
	Если ВыбранныеФайлы.Количество() = 1 
		...
		ВыбранныеФайлы 	= КонтекстЭДОСервер().РазбитьБольшойФайлУведомленияНаНесколькоМаленьких(
			ОсновнойФайл, УникальныйИдентификатор);
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если РасширениеВыбранногоФайла = ".txt"
			...
			РазрешеныНетекстовыеВложения = (Лев(ПриведенноеИмяФайла, 11) = "ON_UVNPDUS_"
				...
				ИЛИ Лев(ПриведенноеИмяФайла, 14) = "NO_ZVPSODOKNO_"
