...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьШкалуЗначенийХарактеристик();
...
&НаКлиенте
Процедура КлючиПриИзменении(Элемент)
	
	ТекущиеДанные = Элемент.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено И ЗначениеЗаполнено(ТекущиеДанные.ХарактеристикаПерсонала) Тогда
		ТекущиеДанные.ШкалаЗначений = ШкалаЗначенийХарактеристики(ТекущиеДанные.ХарактеристикаПерсонала);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьШкалуЗначенийХарактеристик()
	
	Если Объект.Ключи.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтрокаХарактеристика Из Объект.Ключи Цикл
		СтрокаХарактеристика.ШкалаЗначений = ШкалаЗначенийХарактеристики(СтрокаХарактеристика.ХарактеристикаПерсонала);
	КонецЦикла;
