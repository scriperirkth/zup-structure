...
// Парсит файл машиночитаемой доверенности и извлекает данные из XML-структуры.
//
// Параметры:
//   ВходящиеДанные - ДвоичныеДанные, Строка - файл с доверенностью (двоичные данные или адрес временного хранилища)
//   ОбновлятьСуществующийИлиСсылка - Булево - признак обновления существующей доверенности
//   Подпись - ДвоичныеДанные, Строка, Неопределено - подпись доверенности (двоичные данные, адрес временного хранилища или Неопределено)
//   ЗаявлениеНаОтзыв - ДвоичныеДанные, Строка, Неопределено - файл заявления на отзыв (двоичные данные, адрес временного хранилища, путь к файлу или Неопределено)
//   ПодписьЗаявленияНаОтзыв - ДвоичныеДанные, Строка, Неопределено - подпись заявления на отзыв (двоичные данные, адрес временного хранилища, путь к файлу или Неопределено)
//   ДанныеАрхива - ДвоичныеДанные, Строка, Неопределено - данные архива (двоичные данные, адрес временного хранилища, путь к файлу или Неопределено)
//
// Возвращаемое значение:
//   Структура:
//     * ДанныеВыгрузки - ДвоичныеДанные - исходные файлы доверенности
//     * ДанныеПодписи - ДвоичныеДанные - данные подписи доверенности
//     * ДанныеЗаявленияНаОтзыв - ДвоичныеДанные - данные заявления на отзыв
//     * ИмяФайлаЗаявленияНаОтзыв - Строка - имя файла заявления на отзыв
//     * УникальныйИдентификаторДоверенностиОтзыва - Строка - идентификатор доверенности для отзыва
//     * ДатаОтзыва - Дата - дата отзыва доверенности
//     * ПричинаОтзыва - Строка - причина отзыва доверенности
//     * ДанныеПодписиЗаявленияНаОтзыв - ДвоичныеДанные - подпись заявления на отзыв
//     * ДанныеАрхива - ДвоичныеДанные - данные архива
//     * ИмяФайлаВыгрузки - Строка - имя файла выгрузки
//     * ВерсияФормата - Строка - версия формата доверенности
...
//     * ПередовериеВозможно - Булево - признак возможного передоверия
...
//     * Доверитель - СправочникСсылка.Организации, Строка - доверитель
//     * Организация - СправочникСсылка.Организации, Строка - организация
//     * Владелец - СправочникСсылка.Организации, Строка - владелец
//     * ТекстОшибки - Строка - текст ошибки (при наличии ошибки)
//   - Структура:
//     * ТекстОшибки - Строка - текст ошибки при неопознанном типе доверенности
Функция ДанныеИзФайлаОбмена(
	...
	Если ПодписьЗаявленияНаОтзыв <> Неопределено Тогда
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Передан недопустимый параметр ПодписьЗаявленияНаОтзыв %1.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки, ПодписьЗаявленияНаОтзыв);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);			
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеАрхива <> Неопределено Тогда
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Передан недопустимый параметр ДанныеАрхива %1.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки, ДанныеАрхива);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);			
		КонецЕсли;
	КонецЕсли;
	...
	Если СведенияФайла.Количество() = 1 Тогда
		...
		Если ЗначениеЗаполнено(ИмяФайлаВыгрузки) Тогда
			Если ВРег(Лев(ИмяФайлаВыгрузки, 12)) = "ON_OFFDOVER_" Тогда
				ДанныеДоверенности = ДанныеИзФайлаОбменаДляОтмены(ДокументОбработки,
...
Функция ДанныеИзФайлаОбменаДляОтмены(ВходящиеДанные, ОбновлятьСуществующийИлиСсылка = Ложь, Подпись = Неопределено) Экспорт
	...
	Если Подпись <> Неопределено Тогда
		...
		ИначеЕсли ЭтоАдресВременногоХранилища(Подпись) Тогда
			ТекущиеДанные = ПолучитьИзВременногоХранилища(Подпись);
		Иначе
			ТекстОшибки = НСтр("ru = 'Передан недопустимый тип данных для параметра ""Подпись"" %1.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки, Подпись);
			ВызватьИсключение(ТекстОшибки, КатегорияОшибки.ОшибкаКонфигурации);
		КонецЕсли;
...
Функция ДанныеМЧДЕдиногоФормата(СправочникСсылка, Знач Реквизиты = "") Экспорт
	...
	// Используется адрес временного хранилища
