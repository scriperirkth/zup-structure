...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УстановитьПризнакУчитыватьНерабочиеДни");
КонецПроцедуры
...
Функция ЗапросДляПечатиГрафиков(МассивОбъектов, Год)
	...
	|ПОМЕСТИТЬ ВТГрафикиФильтр
	|ИЗ
	|	Справочник.ГрафикиРаботыСотрудников КАК ГрафикиРаботыСотрудников
	|ГДЕ
	|	ГрафикиРаботыСотрудников.Ссылка В(&МассивОбъектов)
	|	И ГрафикиРаботыСотрудников.СпособЗаполнения = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияГрафиковРаботыСотрудников.ПоСменам)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТГрафикиФильтр КАК ВТГрафикиФильтр
	|		ПО ГрафикиРаботыПоВидамВремени.ГрафикРаботы = ВТГрафикиФильтр.ГрафикРаботы
	|			И (ГрафикиРаботыПоВидамВремени.Месяц МЕЖДУ &НачалоГода И &КонецГода)
	|			И (НЕ ГрафикиРаботыПоВидамВремени.ВидУчетаВремени В (&РабочееВремя, &Выходной))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
