...
Процедура ПриОткрытии(Отказ)
	...
	ИспользоватьУсловиеПрименимостиПриВыгрузке = Число(Объект.ИспользоватьУсловиеПрименимостиПриВыгрузке);
	ИспользоватьУсловиеПрименимостиПриЗагрузке = Число(Объект.ИспользоватьУсловиеПрименимостиПриЗагрузке);
	УсловиеПрименимостиПриИзменении();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого Правило Из ПравилаЗаполненияРеквизитовИС.ПолучитьЭлементы() Цикл
		Если ПравилоЗагрузкиЗаполнено(Правило) Тогда
			...
			Если Правило.ЭтоТаблица Тогда
				Для Каждого ПодчиненноеПравило Из Правило.ПолучитьЭлементы() Цикл
					Если ПравилоЗагрузкиЗаполнено(ПодчиненноеПравило) Тогда
						НовоеПравило = ТекущийОбъект.ПравилаЗаполненияРеквизитовИС.Добавить();
	...
	Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ОбновитьПредставление(ТекущийОбъект);
...
&НаКлиенте
Процедура ИнструкцияУсловиеПрименимостиПриВыгрузкеПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ИнструкцияУсловиеПрименимостиПриЗагрузкеПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ИспользоватьУсловиеПрименимостиПриВыгрузкеПриИзменении(Элемент)
	
	Модифицированность = Истина;
	
	Объект.ИспользоватьУсловиеПрименимостиПриВыгрузке = Булево(ИспользоватьУсловиеПрименимостиПриВыгрузке);
	УсловиеПрименимостиПриИзменении();
	
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьУсловиеПрименимостиПриЗагрузкеПриИзменении(Элемент)
	
	Модифицированность = Истина;
	
	Объект.ИспользоватьУсловиеПрименимостиПриЗагрузке = Булево(ИспользоватьУсловиеПрименимостиПриЗагрузке);
	УсловиеПрименимостиПриИзменении();
...
Процедура КлючевыеРеквизитыИСЗначениеРеквизитаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыбратьЗначениеРеквизитаИС(
		...
		ТекущиеДанные.ЗначениеРеквизита,
		ТекущиеДанные.ДополнительныйРеквизитИС,
		Оповещение);
...
&НаКлиенте
Процедура ПроверитьВыражениеУсловияПрименимостиПриВыгрузке(Команда)
	
	ОчиститьСообщения();
	
	Если Не ЗначениеЗаполнено(Объект.ТипОбъектаИС) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Поле ""%1"" не заполнено'"), Элементы.ТипОбъектаИС.Заголовок),,
			"Объект.ПредставлениеОбъектаИС");
		Возврат;
	КонецЕсли;
	
	ПроверитьВыражениеУсловияПрименимостиПриВыгрузкеНаСервере();
	
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьВыражениеУсловияПрименимостиПриЗагрузке(Команда)
	
	ОчиститьСообщения();
	
	Если Не ЗначениеЗаполнено(Объект.ТипОбъектаДО) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Поле ""%1"" не заполнено'"), Элементы.ТипОбъектаДО.Заголовок),,
			"Объект.ПредставлениеОбъектаДО");
		Возврат;
	КонецЕсли;
	
	ПроверитьВыражениеУсловияПрименимостиПриЗагрузкеНаСервере();
...
Асинх Процедура ПриИзмененииОбъектаДО(Знач СтрокаПодходящиеПравилаЗагрузкиВДО)
	...
	Если СтрокаПодходящиеПравилаЗагрузкиВДО = Неопределено Тогда
		НовыйПредставлениеОбъектаДО = "";
		НовыйТипОбъектаДО = "";
		НовыйВидДокументаДО = "";
		НовыйВидДокументаДОID = "";
		НовыйПравилоЗагрузкиДанныхВДО = "";
	Иначе
		НовыйПредставлениеОбъектаДО = СтрокаПодходящиеПравилаЗагрузкиВДО.ПравилоЗагрузкиВДО;
		НовыйТипОбъектаДО = СтрокаПодходящиеПравилаЗагрузкиВДО.ТипОбъектаДО;
		НовыйВидДокументаДО = СтрокаПодходящиеПравилаЗагрузкиВДО.ВидДокументаДО;
		НовыйВидДокументаДОID = СтрокаПодходящиеПравилаЗагрузкиВДО.ВидДокументаДОID;
		НовыйПравилоЗагрузкиДанныхВДО = СтрокаПодходящиеПравилаЗагрузкиВДО.ПравилоЗагрузкиВДОID;
	КонецЕсли;
	
	Объект.ПредставлениеОбъектаДО = НовыйПредставлениеОбъектаДО;
	
	Если Объект.ТипОбъектаДО = НовыйТипОбъектаДО И Объект.ВидДокументаДОID = НовыйВидДокументаДОID Тогда
		Возврат;
	КонецЕсли;
	
	Объект.ТипОбъектаДО = НовыйТипОбъектаДО;
	Объект.ВидДокументаДО = НовыйВидДокументаДО;
	Объект.ВидДокументаДОID = НовыйВидДокументаДОID;
	Объект.ПравилоЗагрузкиДанныхВДО = НовыйПравилоЗагрузкиДанныхВДО;
	
	Модифицированность = Истина;
	
	ЗагрузитьДанныеИзДокументооборота(Ложь);
...
Процедура ПриИзмененииОбъектаИС(Имя, Синоним)
	...
	ЗагрузитьДанныеИзДокументооборота();
...
&НаКлиенте
Процедура УсловиеПрименимостиПриИзменении()
	
	Если ИспользоватьУсловиеПрименимостиПриВыгрузке Тогда
		Элементы.СтраницыУсловияПрименимостиПриВыгрузке.ТекущаяСтраница =
			Элементы.СтраницаВыражениеУсловияПрименимостиПриВыгрузке;
	Иначе
		Элементы.СтраницыУсловияПрименимостиПриВыгрузке.ТекущаяСтраница = Элементы.СтраницаКлючевыеРеквизитыИС;
	КонецЕсли;
	Элементы.ПроверитьВыражениеУсловияПрименимостиПриВыгрузке.Видимость = ИспользоватьУсловиеПрименимостиПриВыгрузке;
	Элементы.КоманднаяПанельКлючевыеРеквизитыИС.Видимость = Не ИспользоватьУсловиеПрименимостиПриВыгрузке;
	
	Если ИспользоватьУсловиеПрименимостиПриЗагрузке Тогда
		Элементы.СтраницыУсловияПрименимостиПриЗагрузке.ТекущаяСтраница =
			Элементы.СтраницаВыражениеУсловияПрименимостиПриЗагрузке;
	Иначе
		Элементы.СтраницыУсловияПрименимостиПриЗагрузке.ТекущаяСтраница = Элементы.СтраницаКлючевыеРеквизитыДО;
	КонецЕсли;
	Элементы.ПроверитьВыражениеУсловияПрименимостиПриЗагрузке.Видимость = ИспользоватьУсловиеПрименимостиПриЗагрузке;
	Элементы.КоманднаяПанельКлючевыеРеквизитыДО.Видимость = Не ИспользоватьУсловиеПрименимостиПриЗагрузке;
...
//
&НаКлиентеНаСервереБезКонтекста
Функция ПравилоЗагрузкиЗаполнено(Правило)
	
	Если Правило.ЭтоТаблица Тогда
		// Это таблица объекта ИС.
		Если Правило.Вариант = ВариантРеквизит() И ЗначениеЗаполнено(Правило.ИмяРеквизитаОбъектаДО) Тогда
			// Заполняется из таблицы объекта ДО.
			Возврат Истина;
			
		ИначеЕсли Правило.Вариант = ВариантВыражение() И ЗначениеЗаполнено(Правило.ВычисляемоеВыражение) Тогда
			// Заполняется из выражения на встроенном языке.
			Возврат Истина;
			
		ИначеЕсли Правило.Вариант = ВариантРеквизитТаблицы() Тогда
			// Заполняется по правилам для отдельных реквизитов.
			Для Каждого ПодчиненноеПравило Из Правило.ПолучитьЭлементы() Цикл
	...
	Иначе
		// Это реквизит (или реквизит таблицы) объекта ИС.
...
&НаСервере
Процедура ЗагрузитьДанныеИзДокументооборота(ЗаполнитьПодходящиеПравилаЗагрузкиВДО = Истина)
	...
	Если ЗаполнитьПодходящиеПравилаЗагрузкиВДО И ЗначениеЗаполнено(Объект.ТипОбъектаИС) Тогда
		...
		ДопЗапросы.Добавить(ЗапросПравилЗагрузкиВДО);
	КонецЕсли;
	
	Попытка
		ЗаполнитьРеквизитыДокументооборота(ДопЗапросы, ОтветыНаДопЗапросы);
	Исключение
	...
	Для Каждого Ответ Из ОтветыНаДопЗапросы Цикл
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, Ответ, "DMGetDataLoadingRulesResponse") Тогда
			ЗаполнитьПодходящиеПравилаЗагрузкиВДО(Ответ.dataLoadingRules);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ЗаполнитьИнструкциюУсловияПрименимостиПриВыгрузке()
	
	ПараметрыВыражения = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПараметрыВыраженияНаВстроенномЯзыке();
	ПараметрыВыражения.ЭтоУсловиеПрименимостиПриВыгрузке = Истина;
	ПараметрыВыражения.ТипОбъектаИС = Объект.ТипОбъектаИС;
	ИнструкцияУсловиеПрименимостиПриВыгрузке = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ИнструкцияВыражения(
		ПараметрыВыражения);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИнструкциюУсловияПрименимостиПриЗагрузке(РеквизитыXDTO)
	
	ПараметрыВыражения = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПараметрыВыраженияНаВстроенномЯзыке();
	ПараметрыВыражения.ЭтоУсловиеПрименимостиПриЗагрузке = Истина;
	ПараметрыВыражения.ТипОбъектаДО = Объект.ТипОбъектаДО;
	ПараметрыВыражения.ВидДокументаДОID = Объект.ВидДокументаДОID;
	ИнструкцияУсловиеПрименимостиПриЗагрузке = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ИнструкцияВыражения(
		ПараметрыВыражения,
		РеквизитыXDTO);
...
&НаСервере
Процедура ЗаполнитьПоУмолчаниюНаСервере(ЗаполнятьИзОбъектаДО = Истина)
...
&НаСервере
Процедура ЗаполнитьРеквизитыДокументооборота(ДопЗапросы, ОтветыНаДопЗапросы)
	
	МассивРеквизитов = Новый Массив;
	КлючевыеРеквизитыДО.Очистить();
	
	РеквизитыXDTO = Неопределено;
	Если ЗначениеЗаполнено(Объект.ТипОбъектаДО) Тогда
		...
		Для Каждого Строка Из РеквизитыXDTO Цикл
			...
			Если Строка.ЭтоОсновнойРеквизит Тогда
	...
	ИначеЕсли ДопЗапросы.Количество() > 0 Тогда
		Прокси = ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ПолучитьПрокси();
		ОтветНаЗапросыПакета = ИнтеграцияС1СДокументооборотБазоваяФункциональность.ВыполнитьПакетныйЗапрос(
			Прокси,
			ДопЗапросы,
			Истина);
		Для Каждого ОтветНаДопЗапрос Из ОтветНаЗапросыПакета.responses Цикл
	...
	ЗаполнитьИнструкциюУсловияПрименимостиПриЗагрузке(РеквизитыXDTO);
...
Процедура ЗаполнитьРеквизитыПотребителя()
	...
	ЗаполнитьИнструкциюУсловияПрименимостиПриВыгрузке();
	...
	Для Каждого Строка Из Реквизиты Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденныеСтроки[0],, "ИмяРеквизитаОбъектаИС");
		КонецЕсли;
...
&НаСервере
Процедура ПроверитьВыражениеУсловияПрименимостиПриВыгрузкеНаСервере()
	
	Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПроверитьВыражениеУсловияПрименимостиПриВыгрузке(
		Объект.УсловиеПрименимостиПриВыгрузке,
		Объект.ТипОбъектаИС);
...
&НаСервере
Процедура ПроверитьВыражениеУсловияПрименимостиПриЗагрузкеНаСервере()
	
	КонтекстВыражения = Справочники.ПравилаИнтеграцииС1СДокументооборотом3.КонтекстВыраженияНаВстроенномЯзыке();
	КонтекстВыражения.ВидДокументаДОID = Объект.ВидДокументаДОID;
	
	Справочники.ПравилаИнтеграцииС1СДокументооборотом3.ПроверитьВыражениеУсловияПрименимостиПриЗагрузке(
		Объект.УсловиеПрименимостиПриЗагрузке,
		Объект.ТипОбъектаДО,
		КонтекстВыражения);
