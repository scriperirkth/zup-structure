...
//
// Параметры:
//  МассивВыборочныхИмен - Массив - имена предопределенных элементов объекта Справочник.ВидыОтправляемыхДокументов,
//                       - Неопределено - получить для всех элементов,
//  ПропускатьГруппы - Булево.
//
// Возвращаемое значение:
//  Соответствие - ключ задает имя предопределенного элемента объекта Справочник.ВидыОтправляемыхДокументов,
//    * Наименование - Строка,
//    * Описание - Строка,
//    * Источник - Строка - имя объекта метаданных или "(Не требуется)",
//    * ТипПолучателя - Строка - согласно значениям Перечисление.ТипыКонтролирующихОрганов,
//    * ТипДокумента - Строка - согласно значениям Перечисление.ТипыОтправляемыхДокументов.
//
Функция ПолучитьСоответствиеСвойствВидовОтправляемыхДокументов(
		МассивВыборочныхИмен = Неопределено,
		ПропускатьГруппы = Ложь) Экспорт
	...
	Для Инд = 0 По МакетСписокДокументов.Области.Количество() - 1 Цикл
		...
		Если Не ПустаяСтрока(ИмяГруппы) Тогда
			...
			Для Ном = ТекОбласть.Верх + ?(ПропускатьГруппы, 1, 0) По ТекОбласть.Низ Цикл
...
// Обработчик обновления БРО 1.2.1.313
//
Процедура ОчиститьУдаленныеВидыОтправляемыхДокументовБухОтчетности(Параметры = Неопределено) Экспорт
	
	ОтборСсылки = Новый Массив;
	ОтборСсылки.Добавить(Справочники.ВидыОтправляемыхДокументов.УдалитьБухОтчетностьМП);
	ОтборСсылки.Добавить(Справочники.ВидыОтправляемыхДокументов.УдалитьБухОтчетностьСОНКО);
	
	ИсправитьВидыОтправляемыхДокументов(ОтборСсылки,, Истина);
...
Процедура ИсправитьВидыОтправляемыхДокументов(ОтборСсылки, ОтборИсточники = Неопределено, РазрешитьОчистку = Ложь)
	...
	Если ТаблицаЗапроса.Количество() > 0 Тогда
		...
		Для каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
			...
			Если ЭталонныеСвойства = Неопределено И НЕ РазрешитьОчистку Тогда
				Продолжить;
			КонецЕсли;
			...
			КонецЕсли;
			ВидыОтправляемыхДокументовОбъект.Описание =
				ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЭталонныеСвойства, "Описание", "");
			ВидыОтправляемыхДокументовОбъект.Источник =
				ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЭталонныеСвойства, "Источник", "");
			ВидыОтправляемыхДокументовОбъект.ТипПолучателя = ?(ЭталонныеСвойства = Неопределено,
				Перечисления.ТипыКонтролирующихОрганов.ПустаяСсылка(),
				Перечисления.ТипыКонтролирующихОрганов[ЭталонныеСвойства.ТипПолучателя]);
			ВидыОтправляемыхДокументовОбъект.ТипДокумента = ?(ЭталонныеСвойства = Неопределено,
				Перечисления.ТипыОтправляемыхДокументов.ПустаяСсылка(),
				Перечисления.ТипыОтправляемыхДокументов[ЭталонныеСвойства.ТипДокумента]);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ИсправитьОписаниеДекларацийПоАлкоголюРеестровПоНДС");
	Методы.Вставить("ИсправитьВидыОтправляемыхДокументовСЗВК");
	Методы.Вставить("ИсправитьИмяОбъектаМетаданныхСтатистикаФорма2ТПВоздух");
	Методы.Вставить("ИсправитьВидыОтправляемыхДокументовРеестровАкцизовСФ");
	Методы.Вставить("ИзменитьОписаниеДекларацийПоКосвеннымНалогам");
	Методы.Вставить("ИсправитьВидыОтправляемыхДокументовОбъемВиноградаДо2019");
	Методы.Вставить("ИсправитьВидыОтправляемыхДокументовРеестровНДС");
	Методы.Вставить("ИсправитьОписаниеФармацевтическихДекларацийНДФЛиАДВ1");
	Методы.Вставить("ОбновитьНаименованияПредопределенныхЭлементов");
	Методы.Вставить("ИзменитьОписаниеДекларацийПоФСРАР2021кв1");
	Методы.Вставить("ОчиститьУдаленныеВидыОтправляемыхДокументовБухОтчетности");
