...
Процедура ПереходКНовости(Элемент)
	...
	Если (ИдентификаторЭлемента > 0) И (ИдентификаторЭлемента <= ЭтотОбъект.Новости.Количество()) Тогда
		...
		ПараметрыОткрытияФормы.Вставить("ИнициаторОткрытияНовости", "ФормаПросмотраНовостейРабочийСтол"); // АПК:1297 Это идентификатор.
...
Функция НачатьПолучениеНовостейВФонеНаСервере()
	...
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне(
		"Справочники.Новости.ПолучитьСписокНовостейВФоне",
		ПараметрыМетода,
		ПараметрыВыполнения);
