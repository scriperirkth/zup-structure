	...
	Иначе
		...
		Если (ВыбраннаяСтрока <> Неопределено) Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				ПараметрыОткрытияФормы.Вставить("ИнициаторОткрытияНовости", "ФормаПросмотраНовостей"); // АПК:1297 Это идентификатор.
...
#Область Обработка
	...
	Если (Команда.Имя = "КомандаОповещенияВкл")
			И (ТекущиеДанные.СпособОповещения <> "ПоУмолчанию") Тогда // АПК:1297 Это идентификатор.
		ИзменитьСпособОповещенияНаКлиенте(ЛентаНовостей, "ПоУмолчанию", Истина); // АПК:1297 Это идентификатор.
	ИначеЕсли (Команда.Имя = "КомандаОповещенияСистемаВзаимодействия")
			И (ТекущиеДанные.СпособОповещения <> "СистемаВзаимодействия") Тогда // АПК:1297 Это идентификатор.
		ИзменитьСпособОповещенияНаКлиенте(ЛентаНовостей, "СистемаВзаимодействия", Истина); // АПК:1297 Это идентификатор.
	ИначеЕсли (Команда.Имя = "КомандаОповещенияОткл")
			И (ТекущиеДанные.СпособОповещения <> "Отключено") Тогда // АПК:1297 Это идентификатор.
		ИзменитьСпособОповещенияНаКлиенте(ЛентаНовостей, "Отключено", Истина); // АПК:1297 Это идентификатор.
	КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			НоваяСтрока = СтрокаВсе.ПолучитьЭлементы().Добавить();
				...
				Если Выборка.ОбязательныйКанал = Истина Тогда
					НоваяСтрока.СпособОповещения = "ПоУмолчанию"; // АПК:1297 Это идентификатор.
				...
				Иначе
					...
					Если Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.Отключено Тогда
						НоваяСтрока.СпособОповещения = "Отключено"; // АПК:1297 Это идентификатор.
					...
					ИначеЕсли Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
						НоваяСтрока.СпособОповещения = "СистемаВзаимодействия"; // АПК:1297 Это идентификатор.
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					НоваяСтрока = СтрокаВсе.ПолучитьЭлементы().Добавить();
						...
						ИначеЕсли Выборка.СпособОповещенияСсылка = Перечисления.СпособыОповещенияПользователяОНовостях.СистемаВзаимодействия Тогда
							НоваяСтрока.СпособОповещения = "СистемаВзаимодействия"; // АПК:1297 Это идентификатор.
						Иначе // "ПоУмолчанию"; // АПК:1297 Это идентификатор.
							НоваяСтрока.СпособОповещения = "ПоУмолчанию"; // АПК:1297 Это идентификатор.
						КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	// 5. Представление даты: сегодня.
		...
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = "Сегодня"; // АПК:1297 Это идентификатор.
	...
	// 6. Представление даты: вчера.
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Новости.ПредставлениеДатыПубликации");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = "Вчера"; // АПК:1297 Это идентификатор.
	...
	// 7. Представление даты: этот год.
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Новости.ПредставлениеДатыПубликации");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = "Этот год"; // АПК:1297 Это идентификатор.
	...
	// 8. Представление даты: полная дата.
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Новости.ПредставлениеДатыПубликации");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = "Полная дата"; // АПК:1297 Это идентификатор.
	...
	Если СостояниеПодписки = Истина Тогда
		...
		Если (ТекущиеДанные.ПолучитьРодителя() <> Неопределено)
			...
			Если СохраненнаяСтрока.ОбязательныйКанал = Истина Тогда
				СохраненнаяСтрока.СпособОповещения = "ПоУмолчанию"; // АПК:1297 Это идентификатор.
				СохраненнаяСтрока.Иконка           = БиблиотекаКартинок.НовостиОповещенияВключеныВсегда;
			Иначе
				// {При добавлении способов оповещения исправлять здесь}.
				Если СохраненнаяСтрока.СпособОповещения = "Отключено" Тогда // АПК:1297 Это идентификатор.
					СохраненнаяСтрока.Иконка = БиблиотекаКартинок.НовостиОповещенияОтключены;
				ИначеЕсли СохраненнаяСтрока.СпособОповещения = "СистемаВзаимодействия" Тогда // АПК:1297 Это идентификатор.
...
Процедура ИзменитьСпособОповещенияНаКлиенте(Знач ЛентаНовостей, СпособОповещения, СохранятьВБазе)
	...
	Если (СпособОповещенияСтрока = "ПоУмолчанию")
			И (ТекущиеДанные.СпособОповещения <> "ПоУмолчанию") Тогда // АПК:1297 Это идентификатор.
		// По-умолчанию.
		ТекущиеДанные.СпособОповещения = "ПоУмолчанию"; // АПК:1297 Это идентификатор.
	...
	ИначеЕсли (СпособОповещенияСтрока = "СистемаВзаимодействия")
			И (ТекущиеДанные.СпособОповещения <> "СистемаВзаимодействия") Тогда // АПК:1297 Это идентификатор.
		// По-умолчанию.
		ТекущиеДанные.СпособОповещения = "СистемаВзаимодействия"; // АПК:1297 Это идентификатор.
	...
	ИначеЕсли (СпособОповещенияСтрока = "Отключено")
		...
		ТекущиеДанные.СпособОповещения = "Отключено"; // АПК:1297 Это идентификатор.
