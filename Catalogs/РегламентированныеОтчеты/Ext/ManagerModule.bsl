...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УдалитьРеглОтчетИсполнениеКонтрактовГОЗИзГруппыОтчетностьПрочая");
	Методы.Вставить("ЗаполнитьСписокРегламентированныхОтчетов");
	Методы.Вставить("УстановитьСнятьПометкуНаУдаление");
	Методы.Вставить("ОчиститьВнешниеРеглОтчеты");
КонецПроцедуры
