...
Процедура СохранитьВыбранныеСтроки(Знач ВыбранныеСтроки, ТекущаяСсылка)
	...
	Для каждого НомерСтроки Из ВыбранныеСтроки Цикл
		...
		Для Каждого ТекущаяСтрока Из СтрокиЗначений Цикл
			...
			СтруктураХарактеристики = ХарактеристикиПерсонала.ХарактеристикаИзМакета(ТекущаяСтрока.Характеристика, 
				ТекущаяСтрока.Значение, БиблиотекаХарактеристикТаблица, СвойстваХарактеристики);
		...
		Для Каждого ТекущаяСтрока Из СтрокиЭтапов Цикл
			...
			Для Каждого ТекущийВопрос Из СтрокиВопросов Цикл
				...
				Иначе
					...
					Если НовыйВопросШаблона.ТипВопроса = Перечисления.ТипыВопросовШаблонаАнкеты.Комплексный Тогда
						ЭлектронноеИнтервью.ДобавитьПроверкуОтказаОтвечать(НовыйВопросШаблона.СоставКомплексногоВопроса);
					КонецЕсли;
		...
		Для Каждого ВопросДляСобеседования Из НастройкиВопросовДляСобеседования Цикл
			...
			Для Каждого ТекущаяНастройка Из СтрокиНастройки Цикл
				...
				КонецЕсли;
				СвойстваХарактеристики = Новый Структура("ИспользоватьЭлектронноеИнтервью", Истина);
				СтруктураХарактеристики = ХарактеристикиПерсонала.ХарактеристикаИзМакета(ТекущаяНастройка.ХарактеристикаПерсонала, 
					ТекущаяНастройка.ЗначениеХарактеристики, БиблиотекаХарактеристикТаблица, СвойстваХарактеристики);
				Если СтруктураХарактеристики <> Неопределено Тогда
...
Функция ЭлементарныйВопрос(ЭлементарныйВопросСтрокой, НаименованиеПрофиля, ЭтапРаботы, ВопросШаблонаАнкеты, БиблиотекаОтветовТаблица)
	...
	Если СтрокиОтветов.Количество() = 0 Тогда
		ЭлементарныйВопрос.ТипОтвета = Перечисления.ТипыОтветовНаВопрос.Строка;
		ЭлементарныйВопрос.Длина = 150;
	Иначе
