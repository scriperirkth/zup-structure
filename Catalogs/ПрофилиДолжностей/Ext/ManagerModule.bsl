...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗаполнитьВалютуИПериодичностьЗарплаты");
...
Функция ДанныеПрофилейДолжности(ПрофилиДолжности, ПараметрыПолучения = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	ПрофилиДолжностей.ЗарплатаОт,
		|	ПрофилиДолжностей.ЗарплатаДо,
		|	ПрофилиДолжностей.ВалютаЗарплаты,
		|	ПрофилиДолжностей.ПериодичностьЗарплаты,
		|	ПрофилиДолжностей.ТипЗанятости,
		|	ПрофилиДолжностей.РежимРаботы,
	...
	ВыбираемыеПоля = ПакетЗапросов.Операторы[0].ВыбираемыеПоля;
	...
	Если ПараметрыПолучения.ЭтапыРаботыСКандидатами Тогда
		...
		Поле = ПакетЗапросов.Колонки.Найти("ЭтапыРаботыСКандидатами").Поля[0].Поля[0];
		ПакетЗапросов.Порядок.Добавить(Поле);
	КонецЕсли;
...
Функция ПрофилиПоКадровойДолжности(Должность) Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.ПрофилиДолжностей КАК ПрофилиДолжностей
		|ГДЕ
		|	ПрофилиДолжностей.Должность = &Должность
		|	И ПрофилиДолжностей.ПометкаУдаления = ЛОЖЬ";
...
Процедура ЗаполнитьВалютуИПериодичностьЗарплаты(ПараметрыОбновления) Экспорт
	...
	Запрос.Текст =
		...
		|	Справочник.ПрофилиДолжностей КАК ПрофилиДолжностей
		|ГДЕ
		|	ПрофилиДолжностей.ВалютаЗарплаты = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)";
	
	НачатьТранзакцию();
	Попытка
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Справочник.ПрофилиДолжностей");
		...
		Пока Выборка.Следующий() Цикл
			ПрофильДолжности = Выборка.ПрофильДолжности.ПолучитьОбъект();
			ПрофильДолжности.ВалютаЗарплаты = ЗарплатаКадры.ВалютаУчетаЗаработнойПлаты();
			ПрофильДолжности.ПериодичностьЗарплаты = Перечисления.ПериодичностьОжидаемогоДоходаКандидатов.Месяц;
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ПрофильДолжности);
		КонецЦикла;
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
